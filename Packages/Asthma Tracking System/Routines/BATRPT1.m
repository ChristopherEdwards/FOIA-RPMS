BATRPT1 ; IHS/CMI/LAB - master list of all active patients ;
...
START ;
 D EXIT
 W:$D(IOF) @IOF
 W !!,"This report will produce a list of all patients on the register.",!,"You will select which patients (based on status) to display.",!!
 S BATS=""
...
ZIS ;
 ...
 S XBRP="PRINT^BATRPT1",XBRC="PROC^BATRPT1",XBRX="EXIT^BATRPT1",XBNS="BAT"
 D ^XBDBQUE
 D EXIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""PRINT^BATRPT1"")"
 S XBRC="PROC^BATRPT1",XBRX="EXIT^BATRPT1",XBIOP=0 D ^XBDBQUE
 Q
EXIT ;
 D EN^XBVK("BAT")
 Q
PROC ;
 S BATJ=$J,BATH=$H
 D XTMP^BATU("BATRPT1","ASTHMA REGISTER MASTER LIST")
 ...
 .D GETSORT
 .S ^XTMP("BATRPT1",BATJ,BATH,"PATIENTS",BATSRTV,BATX)=""
 .Q
 Q
GETSORT ;
 S BATSRTV=""
...
PRINT ;EP
 S BATQUIT=0,BATPG=0
 D HEADER
 I '$D(^XTMP("BATRPT1",BATJ,BATH,"PATIENTS")) W !!,"No patients to list" G EOJ
 S BATST="" F  S BATST=$O(^XTMP("BATRPT1",BATJ,BATH,"PATIENTS",BATST)) Q:BATST=""!(BATQUIT)  D
 .S DFN=0 F  S DFN=$O(^XTMP("BATRPT1",BATJ,BATH,"PATIENTS",BATST,DFN)) Q:DFN'=+DFN!(BATQUIT)  D
...
HEADER1 ;
 ...
 W $$CTR^BATU($$LOC^BATU),!
 W !,$$CTR^BATU("***  ASTHMA REGISTER MASTER LIST  ***",80),!
