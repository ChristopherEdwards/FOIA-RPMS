BATRPT6 ; IHS/CMI/LAB - master list of all active patients ;
...
START ;
 D EXIT
 W:$D(IOF) @IOF
 W !!,$$CTR^BATU("PERSISTENT PATIENTS WITHOUT PEAK FLOW DOCUMENTED IN PAST 2 YRS")
 W !!,"This report will list all persistent patients who have not had a ",!,"a peak flow documented in the past 2 years.",!!
STAT ;
 S BATS=""
...
ZIS ;
 ...
 S XBRP="PRINT^BATRPT6",XBRC="PROC^BATRPT6",XBRX="EXIT^BATRPT6",XBNS="BAT"
 D ^XBDBQUE
 D EXIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""PRINT^BATRPT6"")"
 S XBRC="PROC^BATRPT6",XBRX="EXIT^BATRPT6",XBIOP=0 D ^XBDBQUE
 Q
EXIT ;
 D EN^XBVK("BAT")
 Q
PROC ;
 S BATJ=$J,BATH=$H
 D XTMP^BATU("BATRPT6","ASTHMA REGISTER DUE LIST")
 ...
 .S X=$$LASTPBF^BATU(BATX,3)
 .I X]"",X>($$FMADD^XLFDT(DT,-(2*365))) Q
 .D GETSORT
 .S ^XTMP("BATRPT6",BATJ,BATH,"PATIENTS",BATSRTV,BATX)=""
 .Q
 Q
GETSORT ;
 S BATSRTV=""
...
PRINT ;EP
 S BATQUIT=0,BATPG=0
 D HEADER
 I '$D(^XTMP("BATRPT6",BATJ,BATH,"PATIENTS")) W !!,"No patients to list" G EOJ
 S BATST="" F  S BATST=$O(^XTMP("BATRPT6",BATJ,BATH,"PATIENTS",BATST)) Q:BATST=""!(BATQUIT)  D
 .S BATX=0 F  S BATX=$O(^XTMP("BATRPT6",BATJ,BATH,"PATIENTS",BATST,BATX)) Q:BATX'=+BATX!(BATQUIT)  D
 ...
 ..W !?5,"Last Peak Flow: ",$$LASTPBF^BATU(BATX,1) I $$LASTPBF^BATU(BATX,1)]"" W " documented on ",$$LASTPBF^BATU(BATX,2)
 ..Q
 .Q
EOJ ;
 I BATOPT'="B",$E(IOST)="C",IO=IO(0) S DIR(0)="EO",DIR("A")="End of report.  Press ENTER" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 K ^XTMP("BATRPT6",BATJ,BATH),BATX
 W:$D(IOF) @IOF
 Q
HEADER ;EP
 G:'BATPG HEADER1
...
HEADER1 ;
 ...
 W $$CTR^BATU($$LOC^BATU),!
 W !,$$CTR^BATU("***  PERSISTENT PATIENTS WITHOUT PEAK FLOW DOCUMENTED IN PAST 2 YEARS",80),!
