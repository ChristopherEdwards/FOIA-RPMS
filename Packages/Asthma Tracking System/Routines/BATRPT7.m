BATRPT7 ; IHS/CMI/LAB - master list of all active patients ;
...
START ;
 D EXIT
 W:$D(IOF) @IOF
 W !!,$$CTR^BATU("% OF PERSISTENT PATIENTS WITH ASTHMA MANAGEMENT PLANS")
 W !,$$CTR^BATU("THAT HAVE BEEN UPDATED IN A SPECIFIED TIME RANGE")
 W !!,"This report give a % of patients who have had a management plan updated",!,"or made in the past year.",!!
STAT ;
 S BATS=""
...
ZIS ;
 ...
 S XBRP="PRINT^BATRPT7",XBRC="PROC^BATRPT7",XBRX="EXIT^BATRPT7",XBNS="BAT"
 D ^XBDBQUE
 D EXIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""PRINT^BATRPT7"")"
 S XBRC="PROC^BATRPT7",XBRX="EXIT^BATRPT7",XBIOP=0 D ^XBDBQUE
 Q
EXIT ;
 D EN^XBVK("BAT")
 Q
PROC ;
 ...
 ..S Y=$P($G(^AUPNVAST(X,0)),U,12) I 'Y Q
...
PRINT ;EP
 ...
 W !," Total Number with Asthma Plan Updated: ",?40,$J(BATNUMER,7)
 S X=""
...
HEADER1 ;
 ...
 W $$CTR^BATU($$LOC^BATU),!
 W !,$$CTR^BATU("***  % OF PERSISTENT PATIENTS WITH MANAGEMENT PLAN UPDATED  ***",80),!
