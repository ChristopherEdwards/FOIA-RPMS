PSGWINIS ; ; 04-JAN-1994
 ;;2.3; Automatic Replenishment/Ward Stock ;;4 JAN 94
PAC(PKG,VER) ; called from package init (A5CSTBUL created this routine)
 ; PKG = $T(IXF) of the INIT routine.
 ; VER is an array that is contained in DIFROM from the INIT routine
 ;
 N %,%I,%H,DATE,DIFROM,NOW,PACKAGE,RUN,SERVER,SITE,START,X,XMDUZ,XMSUB,XMTEXT,XMY,Y K ^TMP("PSGWINIS",$J)
 ;
 ...
 ;
 ; Message for server
 S ^TMP("PSGWINIS",$J,1,0)="PACKAGE INSTALL"
 S ^TMP("PSGWINIS",$J,2,0)="SITE: "_SITE
 S ^TMP("PSGWINIS",$J,3,0)="PACKAGE: "_PACKAGE
 S ^TMP("PSGWINIS",$J,4,0)="VERSION: "_VER
 S ^TMP("PSGWINIS",$J,5,0)="Start time: "_START
 S ^TMP("PSGWINIS",$J,6,0)="Completion time: "_NOW
 S ^TMP("PSGWINIS",$J,7,0)="Run time: "_RUN
 S ^TMP("PSGWINIS",$J,8,0)="DATE: "_DATE
 ;
 ; Data is sent to server on FORUM - S.A5CSTS
 S XMY(SERVER)="",XMDUZ=.5,XMTEXT="^TMP(""PSGWINIS"",$J,",XMSUB=PACKAGE_" VERSION "_VER_" INSTALLATION"
 D ^XMD
 K ^TMP("PSGWINIS",$J)
 Q
