IS00002(INTT,INDA,INA,INDEST,INQUE,INORDUZ,INORDIV) ;Compiled from script 'Generated: HL IHS LOINC R01-O' on DEC 03, 2002
 ;Part 1
 ;Copyright 2002 SAIC
EN S X="ERROR^IS00002",@^%ZOSF("TRAP")
 G START
ERROR ;
...
START ;Initialize variables
 ...
 D ^BHLMSH,VST^BHLV,^BHLPID,ORU^BHLRLAB
 ...
 S D0=INDA S X=$G(INA("MET")),X=X S X=X,Y(1)=X S X=1,Y(2)=X S X=7,X=$E(Y(1),Y(2),X)
 ...
 S D0=INDA S X=$G(INA("PRID")),X=X S X=X,Y(1)=X S X=1,Y(2)=X S X=1,X=$E(Y(1),Y(2),X)
 ...
 S D0=INDA S X=$G(INA("ACA")),X=X S X=X,Y(1)=X S X=1,Y(2)=X S X=2,X=$E(Y(1),Y(2),X)
 S @INV@("MSH14")=X K DXS,D0
 ;SET MSH15 = $E(INTERNAL(@APA),1,2)
 S D0=INDA S X=$G(INA("APA")),X=X S X=X,Y(1)=X S X=1,Y(2)=X S X=2,X=$E(Y(1),Y(2),X)
 ...
 S D0=INDA S X=@INV@("MSH11"),Y(1)=X S X=1,Y(2)=X S X=8,X=$E(Y(1),Y(2),X) S L1=X
