IS00011E ;Compiled from script 'Generated: HL IHS IZV04 OUT-O' on FEB 28, 2013
 ;Part 6
 ;Copyright 2013 SAIC
EN S:$TR(L1,$G(SUBDELIM))="" L1="" D SETPIECE^INHU(.LINE,DELIM,48,L1,.CP) S L1=$G(@INV@("IN149")) S:$TR(L1,$G(SUBDELIM))="" L1="" D SETPIECE^INHU(.LINE,DELIM,50,L1,.CP)
 S LCT=LCT+1,^UTILITY("INH",$J,LCT)=LINE I $D(LINE)>9 M ^UTILITY("INH",$J,LCT)=LINE
 Q
H2 F I=1:1:MULT S INDA(I-1)=INDA(I)
 S INDA=INDA(0) K INDA(0) S MULT=MULT-1
 Q
H1 S INDA=INDA0 K INDA0
 SET INSETID=0
 ...
 .Q:'$D(^AUPNVIMM(INDA,0))
 .;SCREEN=I $G(INDA("BYIMALL"))=2!('$D(^BYIMEXP("D",INDA))&('$P($G(^AUPNVIMM(INDA,0)),U,15))&($P($G(^AUTTIMM(+$G(^AUPNVIMM(+$G(INDA),0)),0)),U,3)'=999))
 .I $D(^AUPNVIMM(INDA,0)) X "I $G(INDA(""BYIMALL""))=2!('$D(^BYIMEXP(""D"",INDA))&('$P($G(^AUPNVIMM(INDA,0)),U,15))&($P($G(^AUTTIMM(+$G(^AUPNVIMM(+$G(INDA),0)),0)),U,3)'=999))" E  Q
 .D ORC^BYIMSEGS
 ...
 .S D0=INDA S X=@INV@("ORC10"),Y(1)=$G(X) S X=1,Y(2)=$G(X) S X=50,X=$E(Y(1),Y(2),X) S L1=X
 .S:$TR(L1,$G(SUBDELIM))="" L1="" D SETPIECE^INHU(.LINE,DELIM,11,L1,.CP)
 .S D0=INDA S X=@INV@("ORC12"),Y(1)=$G(X) S X=1,Y(2)=$G(X) S X=48,X=$E(Y(1),Y(2),X) S L1=X
