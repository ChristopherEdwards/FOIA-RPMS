PSBMLLKU ;BIRMINGHAM/TEJ-BCMA RPC LOOKUP UTLILITIES ;08-Dec-2006 11:29;SM
 ;;3.0;BAR CODE MED ADMIN;**3,9,11,20,1005**;Mar 2004
 ;
 ; Reference/IA
 ; EN^PSJBCMA1/2829
 ; $$DOB^DPTLK1/3266
 ...
 ; File 50/221
 ;
 ; Modified - IHS/MSC/PLS - 05/22/06 - Line PTLKUP EP to support HRN
 ;
...
PTLKUP(RESULTS,PSBREC) ; Patient lookup handled separately for security
 ...
 .N X
 .S X=$$HRCNF^APSPFUNC($S($L(PSBDATA)=12:$P(PSBDATA,U),1:$$PAD($$GET1^DIQ(9999999.06,+DUZ(2),.12))_$$PAD(PSBDATA)))
 .I X<0 D  Q
...
PTREC(DFN) ;
 ...
 S $P(PSBXX,U,11)=$$HRN^AUPNPAT(DFN,DUZ(2))
 Q PSBXX
 ;
