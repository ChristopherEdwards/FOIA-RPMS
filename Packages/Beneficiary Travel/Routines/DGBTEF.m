...
DIE ;
 I $D(^DGMT(408.31,"AID",1,DFN)) S X=+$O(^DGMT(408.31,"AID",1,DFN,-DGBTA)) S X=+$O(^(X,0)) I $D(^DGMT(408.31,X,0)) S X=$P(^(0),"^",4),X2="0$" D COMMA^%DTC S DGBTMTI=X K X,X2 W !!,"REPORTED MEANS TEST INCOME: ",DGBTMTI
