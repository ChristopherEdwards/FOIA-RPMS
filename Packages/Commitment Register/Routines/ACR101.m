ACR101 ; GENERATED FROM 'ACR SF-44 DISPLAY' PRINT TEMPLATE (#3890) ; 09/30/09 ; (FILE 9002196, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 S X=$G(^ACRDOC(D0,"REQ")) W ?19 S Y=$P(X,U,2) W:Y]"" $S($D(DXS(3,Y)):DXS(3,Y),1:Y)
 ...
 D N:$X>40 Q:'DN  W ?40 W "[11] RQSD VENDER:"
 S X=$G(^ACRDOC(D0,"PO")) W ?59 S Y=$P(X,U,5) S Y=$S(Y="":Y,$D(^AUTTVNDR(Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,20)
 D N:$X>0 Q:'DN  W ?0 W "[5 ]*DATE OF REQ:"
 S X=$G(^ACRDOC(D0,"REQ")) W ?19 S Y=$P(X,U,5) D DT
 D N:$X>40 Q:'DN  W ?40 W "[12]*IDENTIFIER.:"
 S X=$G(^ACRDOC(D0,0)) W ?59,$E($P(X,U,14),1,16)
 D N:$X>0 Q:'DN  W ?0 W "[6 ]*DATE REQRED:"
 S X=$G(^ACRDOC(D0,"REQ")) W ?19 S Y=$P(X,U,11) D DT
 D N:$X>40 Q:'DN  W ?40 W "[13] DEPT PRIOR.:"
 S X=$G(^ACRDOC(D0,"REQ2")) W ?59 S Y=$P(X,U,6) W:Y]"" $J(Y,2,0)
 D N:$X>0 Q:'DN  W ?0 W "[7 ] PRIORITY...:"
 S X=$G(^ACRDOC(D0,"DT")) W ?19 S Y=$P(X,U,4) W:Y]"" $S($D(DXS(4,Y)):DXS(4,Y),1:Y)
 D N:$X>40 Q:'DN  W ?40 W "[14]*INITIATOR..:"
 S X=$G(^ACRDOC(D0,"REQ2")) W ?59 S Y=$P(X,U,8) S Y(0)=Y S Y=$$NAME2^ACRFUTL1(Y) W $E(Y,1,20)
 D N:$X>40 Q:'DN  W ?40 W "[15]*REQST TYPE.:"
