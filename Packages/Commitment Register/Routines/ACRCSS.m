ACRCSS ; DRIVER FOR COMPILED XREFS FOR FILE #9002193 ; 09/30/09
 ; 
...
DI S DIKM1=0,DIKUM=0,DA(0)="",DV=0 F  S DV=$O(DA(DV)) Q:DV'>0  S DIKUM=DIKUM+1,DIKUP(DV)=DA(DV)
 S:DV="" DV=-1 S DH(1)=9002193,DIKUP=DA
 I $D(DIKKS) D:DIKZ1=DH(1) ^ACRCSS1 S DA=DIKUP D:DIKZ1=DH(1) ^ACRCSS5 D:DIKZ1'=DH(1) KILL D:DIKZ1'=DH(1) DA D:DIKZ1'=DH(1) SET D DA Q
 I $D(DIKIL) D:DIKZ1=DH(1) ^ACRCSS1 S:DIKZ1=DH(1) DIKM1=1 D:DIKZ1'=DH(1) KILL S DA=DIKUP D:DIKM1>0 KIL1 D DA Q
 I $D(DIKST) D:DIKZ1=DH(1) ^ACRCSS5 D:DIKZ1'=DH(1) SET D DA Q
...
C I @("$O("_DIK_"DA))'>0") S DA=$$C1(DA),^(0)=$P(@(DIK_"0)"),U,1,2)_U_DA_U_DCNT K DCNT L:$D(DIKLK) -@DIKLK Q
 S (DIKY,DA)=$O(^(DA)) G C:$P($G(^(DA,0)),U)']"" S DU=1,DCNT=DCNT+1 S:DA="" (DIKY,DA)=-1 D:DIKZ1=DH(1) ^ACRCSS5 D:DIKZ1'=DH(1) SET D:DIKZ1'=DH(1) DA K DB(0) S DA=DIKY G C
 Q
C1(A) Q:$P($G(@(DIK_"A,0)")),U)]"" A
 F  S @("A=+$O("_DIK_"A),-1)") Q:$P($G(@(DIK_"A,0)")),U)]""!(A'>0)
 Q A
KILL S DIKILL=1,DIKZK=2
 I DIKZ1=9002193.01101,DIKUM'<1 S DIKM1=1 D A1^ACRCSS3 Q
 I DIKZ1=9002193.1001,DIKUM'<1 S DIKM1=1 D A1^ACRCSS4 Q
 Q
SET S DISET=1,DIKZK=1 K DIKPUSH
 I DIKZ1=9002193.01101,DIKUM'<1 S DIKM1=1 D A1^ACRCSS7 Q
 I DIKZ1=9002193.1001,DIKUM'<1 S DIKM1=1 D A1^ACRCSS8 Q
 Q
