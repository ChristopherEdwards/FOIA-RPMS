ACRPTI ; GENERATED FROM 'ACR TRAVEL ITINERARY' PRINT TEMPLATE (#3863) ; 09/30/09 ; (FILE 9002196, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 M DXS=^DIPT(3863,"DXS")
 S I(0)="^ACRDOC(",J(0)=9002196
 W ?0 W:$D(IOF) @IOF K DIP K:DN Y
 D N:$X>19 Q:'DN  W ?19 W "TRAVEL ITINERARY"
 D T Q:'DN  D N W ?0 W "TRAVEL ORDER NO.:"
 S X=$G(^ACRDOC(D0,0)) W ?19,$E($P(X,U,1),1,17)
 D N:$X>37 Q:'DN  W ?37 W "TRAVELER:"
 S X=$G(^ACRDOC(D0,"TO")) W ?48 S Y=$P(X,U,9) S Y(0)=Y S Y=$$NAME2^ACRFUTL1(Y) W $E(Y,1,30)
 ...
 D N:$X>61 Q:'DN  W ?61 W "STATE"
 D N:$X>68 Q:'DN  W ?68 W "SEAT"
 D N:$X>0 Q:'DN  W ?0 W "==========="
...
A1 ;
 ...
 S X=$G(^ACRAL(D0,"DT")) D N:$X>68 Q:'DN  W ?68,$E($P(X,U,8),1,4)
 D N:$X>20 Q:'DN  W ?20 S Y=$P(X,U,6) D DT
 D N:$X>41 Q:'DN  W ?41 S Y=$P(X,U,7) S Y=$S(Y="":Y,$D(^ACRPD(Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,18)
 D N:$X>61 Q:'DN  W ?61 X DXS(3,9) K DIP K:DN Y W X
 D T Q:'DN  W ?4 I IOSL-4<$Y D PAUSE^ACRFWARN W @IOF K DIP K:DN Y
 Q
A1R ;
 ...
 S DIXX(1)="B1",I(0,0)=D0 S I(0,0)=$G(D0) X DXS(4,9.3) S X="" S D0=I(0,0)
 G B1R
B1 ;
 ...
 D N:$X>33 Q:'DN  W ?33 X DXS(5,9.2) S X=$P(DIP(201),U,4) S D0=I(100,0) K DIP K:DN Y W X
 S X=$G(^ACRTV(D0,"DT")) D N:$X>47 Q:'DN  W ?47,$E($P(X,U,12),1,15)
 D N:$X>64 Q:'DN  W ?64 S Y=$P(X,U,16) W:Y]"" $S($D(DXS(11,Y)):DXS(11,Y),1:Y)
 D N:$X>13 Q:'DN  W ?13 X DXS(6,9.2) S X=$P(DIP(201),U,1) S D0=I(100,0) K DIP K:DN Y W X
 D N:$X>13 Q:'DN  W ?13 X DXS(7,9.2) S X=$P($G(^ACRPD(+$P(DIP(201),U,2),0)),U) S D0=I(100,0) K DIP K:DN Y W $E(X,1,14)
 W ?29 X DXS(8,9.3) S X=$P(DIP(301),U,2) S D0=I(100,0) K DIP K:DN Y W X
 X DXS(9,9) K DIP K:DN Y
 S X=$G(^ACRTV(D0,"DT")) D N:$X>13 Q:'DN  W ?13 S Y=$P(X,U,13) S Y=$S(Y="":Y,$D(^ACRRCOMP(Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,18)
 D N:$X>33 Q:'DN  W ?33 X DXS(10,9.2) S X=$P(DIP(201),U,2) S D0=I(100,0) K DIP K:DN Y W X
 S X=$G(^ACRTV(D0,"DT")) D N:$X>47 Q:'DN  W ?47,$E($P(X,U,14),1,15)
 W ?64 I IOSL-4<$Y D PAUSE^ACRFWARN W @IOF K DIP K:DN Y
 Q
B1R ;
 K J(100),I(100) S:$D(I(0,0)) D0=I(0,0)
 D T Q:'DN  W ?2 D PAUSE^ACRFWARN K DIP K:DN Y
 K Y
 Q
HEAD ;
 W !,"--------------------------------------------------------------------------------",!!
