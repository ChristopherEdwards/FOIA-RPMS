ACRPVND ; GENERATED FROM 'ACR VENDOR DATA-VENDOR-2' PRINT TEMPLATE (#3910) ; 09/29/09 ; (FILE 9999999.11, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 S X=$G(^AUTTVNDR(D0,14)) W ?49,$E($P(X,U,1),1,30)
 D N:$X>0 Q:'DN  W ?0 W "ST - 2:"
 S X=$G(^AUTTVNDR(D0,13)) W ?9,$E($P(X,U,10),1,30)
 D N:$X>40 Q:'DN  W ?40 W "ST - 2:"
 S X=$G(^AUTTVNDR(D0,14)) W ?49,$E($P(X,U,2),1,30)
 D N:$X>0 Q:'DN  W ?0 W "CITY..:"
 S X=$G(^AUTTVNDR(D0,13)) W ?9,$E($P(X,U,2),1,20)
 D N:$X>40 Q:'DN  W ?40 W "CITY..:"
 S X=$G(^AUTTVNDR(D0,14)) W ?49,$E($P(X,U,3),1,20)
 D N:$X>0 Q:'DN  W ?0 W "STATE.:"
 S X=$G(^AUTTVNDR(D0,13)) W ?9 S Y=$P(X,U,3) S Y=$S(Y="":Y,$D(^DIC(5,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 D N:$X>40 Q:'DN  W ?40 W "STATE.:"
 S X=$G(^AUTTVNDR(D0,14)) W ?49 S Y=$P(X,U,4) S Y=$S(Y="":Y,$D(^DIC(5,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 D N:$X>0 Q:'DN  W ?0 W "ZIP...:"
 S X=$G(^AUTTVNDR(D0,13)) W ?9,$E($P(X,U,4),1,10)
 D N:$X>40 Q:'DN  W ?40 W "ZIP...:"
 S X=$G(^AUTTVNDR(D0,14)) W ?49,$E($P(X,U,5),1,10)
 D N:$X>0 Q:'DN  W ?0 W "ATTN..:"
 S X=$G(^AUTTVNDR(D0,13)) W ?9,$E($P(X,U,5),1,20)
 D N:$X>40 Q:'DN  W ?40 W "ATTN..:"
 S X=$G(^AUTTVNDR(D0,14)) W ?49,$E($P(X,U,6),1,30)
 D N:$X>0 Q:'DN  W ?0 W "PHONE.:"
 S X=$G(^AUTTVNDR(D0,11)) W ?9,$E($P(X,U,9),1,15)
 D N:$X>40 Q:'DN  W ?40 W "PHONE.:"
 S X=$G(^AUTTVNDR(D0,14)) W ?49,$E($P(X,U,7),1,20)
 D N:$X>0 Q:'DN  W ?0 W "FAX...:"
 S X=$G(^AUTTVNDR(D0,11)) W ?9,$E($P(X,U,14),1,12)
