...
LIST ;
 ...
 S BNILIST=Y
ZIS ;call to XBDBQUE
 S XBRP="PRINT^BNIRP4",XBRC="PROCESS^BNIRP4",XBRX="XIT^BNIRP4",XBNS="BNIG"
 D ^XBDBQUE
 D XIT
 Q
PROCESS ;EP - called from xbdbque
 ...
 K BNIGDATA
 S ^XTMP("BNIRP4",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^BNI CPHAD ACTIVITY REPORT"
...
PROC1 ;
 ...
 Q
SET ;
 I BNILIST S ^XTMP("BNIRP4",BNIJ,BNIH,"RECORDS",$P(BNIREC,U),BNIR)=""
 S BNIGTOTR=BNIGTOTR+1
 S BNIGTOTT=BNIGTOTT+$P(BNIREC,U,9)
 S BNIPRV=$$VAL^XBDIQ1(90510,BNIR,.13)
...
LISTP ;
 D LHDR
 S BNID=0 F  S BNID=$O(^XTMP("BNIRP4",BNIJ,BNIH,"RECORDS",BNID)) Q:BNID'=+BNID!(BNIGQUIT)  D
 .S BNIR=0 F  S BNIR=$O(^XTMP("BNIRP4",BNIJ,BNIH,"RECORDS",BNID,BNIR)) Q:BNIR'=+BNIR!(BNIGQUIT)  D
...
LHDR1 ;
 ...
 W !,$$CTR("*** Activity Time by Type of Activity ***",80)
...
HEAD1 ;
 ...
 S X="Activity Dates: "_$$FMTE^XLFDT(BNIGBD)_" to "_$$FMTE^XLFDT(BNIGED) W !,$$CTR(X,80)
 W !,$$REPEAT^XLFSTR("-",80)
 W !,?1,"TYPE OF ACTIVITY",?55,"# RECORDS",?73,"Hrs"
 W !?55,"---------",?73,"---"
 W !
 Q
 ;
...
INFORM ;
 W:$D(IOF) @IOF
 W !!,$$CTR($$LOC)
 W !!,$$CTR("TIME SPENT BY TYPE OF ACTIVITY")
 W !
 Q
 ;
...
BNIG(BNIERR,BNIJOB,BNIBTH,BNIGBD,BNIGED,BNILIST,BNIRDT) ;PEP - gui call
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BNIRP4",ZTDESC=BNIOPT D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("PRINT^BNIRP4","^TMP($J,""BNIRP4"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BNIRP4",X)) Q:X'=+X  D
 . S C=C+1
 . N BNIDATA
 . S BNIDATA=$G(^TMP($J,"BNIRP4",X))
