...
LIST ;
 ...
 S BNILIST=Y
ZIS ;call to XBDBQUE
 S XBRP="PRINT^BNIRP5",XBRC="PROCESS^BNIRP5",XBRX="XIT^BNIRP5",XBNS="BNIG"
 D ^XBDBQUE
 D XIT
 Q
PROCESS ;EP - called from xbdbque
 ...
 K BNIGDATA
 S ^XTMP("BNIRP5",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^BNI CPHAD ACTIVITY REPORT"
...
PROC1 ;
 ...
 Q
SET ;
 I BNILIST S ^XTMP("BNIRP5",BNIJ,BNIH,"RECORDS",$P(BNIREC,U),BNIR)=""
 S BNIGTOTR=BNIGTOTR+1
 S BNIGTOTT=BNIGTOTT+$P(BNIREC,U,9)
 S BNIPRV=$$VAL^XBDIQ1(90510,BNIR,.15)
...
LISTP ;
 D LHDR
 S BNID=0 F  S BNID=$O(^XTMP("BNIRP5",BNIJ,BNIH,"RECORDS",BNID)) Q:BNID'=+BNID!(BNIGQUIT)  D
 .S BNIR=0 F  S BNIR=$O(^XTMP("BNIRP5",BNIJ,BNIH,"RECORDS",BNID,BNIR)) Q:BNIR'=+BNIR!(BNIGQUIT)  D
...
LHDR1 ;
 ...
 W !,$$CTR("*** Activity Time by Activity Setting ***",80)
...
HEAD1 ;
 ...
 S X="Activity Dates: "_$$FMTE^XLFDT(BNIGBD)_" to "_$$FMTE^XLFDT(BNIGED) W !,$$CTR(X,80)
 W !,$$REPEAT^XLFSTR("-",80)
 W !,?1,"ACTIVITY SETTING",?55,"# RECORDS",?73,"Hrs"
 W !?55,"---------",?73,"---"
 W !
 Q
 ;
...
INFORM ;
 W:$D(IOF) @IOF
 W !!,$$CTR($$LOC)
 W !!,$$CTR("TIME SPENT BY ACTIVITY SETTING")
 W !
 Q
 ;
...
BNIG(BNIERR,BNIJOB,BNIBTH,BNIGBD,BNIGED,BNILIST,BNIRDT) ;PEP - gui call
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BNIRP5",ZTDESC=BNIOPT D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("PRINT^BNIRP5","^TMP($J,""BNIRP5"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BNIRP5",X)) Q:X'=+X  D
 . S C=C+1
 . N BNIDATA
 . S BNIDATA=$G(^TMP($J,"BNIRP5",X))
