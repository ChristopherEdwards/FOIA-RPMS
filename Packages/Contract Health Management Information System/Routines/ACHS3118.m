...
MAIL ;
 ...
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""ACHS3118"",$J,",XMY(1)="",XMY(DUZ)=""
 ...
 Q
 ;
RSLT(%) S ^(0)=$G(^TMP("ACHS3118",$J,0))+1,^(^(0))=%
 Q
 ;
...
AUDS ;EP - From KIDS.
 ...
 S ^XTMP("ACHS3118",0)=$$FMADD^XLFDT(DT,10)_"^"_DT_"^"_$P($P($T(+1),";",2)," ",3,99)
 NEW ACHS
 S ACHS=0
 F  S ACHS=$O(^XTMP("XPDI",XPDA,"FIA",ACHS)) Q:'ACHS  D
 . I '$D(^XTMP("ACHS3118",ACHS,"DDA")) S ^XTMP("ACHS3118",ACHS,"DDA")=$G(^DD(ACHS,0,"DDA"))
 . D MES^XPDUTL(" File "_$$RJ^XLFSTR(ACHS,12)_" - "_$$LJ^XLFSTR(^XTMP("XPDI",XPDA,"FIA",ACHS),30)_"- DD audit was '"_$G(^XTMP("ACHS3118",ACHS,"DDA"))_"'"),MES^XPDUTL($$RJ^XLFSTR("Set to 'Y'",69))
 . S ^DD(ACHS,0,"DDA")="Y"
...
AUDR ; Restore the file data audit values to their original values.
 ...
 NEW ACHS
 S ACHS=0
 F  S ACHS=$O(^XTMP("ACHS3118",ACHS)) Q:'ACHS  D
 . S ^DD(ACHS,0,"DDA")=^XTMP("ACHS3118",ACHS,"DDA")
...
M278 ;
 ...
 ;
P6OPT ;EP - FR KIDS
 ;ADD NEW OPTIONS - P 6, DENIAL APPEAL,EDITS AND CANCEL, FI FIELD
...
NONREG ;EP - fr KIDS
 ...
 Q
DENREV ;EP - Fr KIDS
 ;SETS DEN STATUS IF DEN WAS REVERSED
 NEW ACHS
