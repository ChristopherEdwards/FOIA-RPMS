ACHS3123 ;IHS/OIT/FCJ - ACHS 3.1 PATCH 23 ;7/30/10  08:37
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**23**;JUNE 11,2001;Build 17
 ;3.1*16 12/1/2009;IHS/OIT/FCJ - ORIG RTN P21
 ;ENV CHECK TO ACHS31E1
 ;
PRE ;EP - From KIDS.
 I $$NEWCP^XPDUTL("PRE1","AUDS^ACHS3123")
 Q
 ;
POST ;EP - From KIDS.
 ;
...
MAIL ;
 ...
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""ACHS3123"",$J,",XMY(1)="",XMY(DUZ)=""
 ...
 Q
 ;
RSLT(%) S ^(0)=$G(^TMP("ACHS3123",$J,0))+1,^(^(0))=%
 Q
 ;
...
AUDS ;EP - From KIDS.
 ...
 S ^XTMP("ACHS3123",0)=$$FMADD^XLFDT(DT,10)_"^"_DT_"^"_$P($P($T(+1),";",2)," ",3,99)
 NEW ACHS
 S ACHS=0
 F  S ACHS=$O(^XTMP("XPDI",XPDA,"FIA",ACHS)) Q:'ACHS  D
 . I '$D(^XTMP("ACHS3123",ACHS,"DDA")) S ^XTMP("ACHS3123",ACHS,"DDA")=$G(^DD(ACHS,0,"DDA"))
 . D MES^XPDUTL(" File "_$$RJ^XLFSTR(ACHS,12)_" - "_$$LJ^XLFSTR(^XTMP("XPDI",XPDA,"FIA",ACHS),30)_"- DD audit was '"_$G(^XTMP("ACHS3123",ACHS,"DDA"))_"'"),MES^XPDUTL($$RJ^XLFSTR("Set to 'Y'",69))
 . S ^DD(ACHS,0,"DDA")="Y"
...
AUDR ; Restore the file data audit values to their original values.
 ...
 NEW ACHS
 S ACHS=0
 F  S ACHS=$O(^XTMP("ACHS3123",ACHS)) Q:'ACHS  D
 . S ^DD(ACHS,0,"DDA")=^XTMP("ACHS3123",ACHS,"DDA")
