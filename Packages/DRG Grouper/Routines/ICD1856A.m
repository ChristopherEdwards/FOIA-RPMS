ICD1856A ;ALB/MJB - YEARLY DRG UPDATE;8/9/2010
 ...
 ;
 ;Update the (#80.2) DRG file with FY 2012 DRG Grouper MS-DRG codes.
 ;
 Q
 ;
 ;Routines ICD1856* contain each FY 2012 MS-DRG code update values
...
DRG ;post-install driver (#80.2) DRG updates
 ...
 ;element values used to create the FY 2012 MS-DRG updates.
 ; Input:
 ...
 ;   ICDTOT - Total MS-DRG codes filed
 ;
 D BMES^XPDUTL(">>> Adding FY 2012 DRG Grouper updates to (#80.2) DRG file...")
 N ICDRTN,ICDI,ICDSUB,ICDEDIT,ICDADD,ICDTMP
 S (ICDEDIT,ICDADD)=0
 S ICDTOT=$G(ICDTOT) I ICDTOT']"" S ICDTOT=0
 S ICDTMP=$G(ICDTMP)
 I ICDTMP']"" S ICDTMP=$NA(^TMP("DRGFY2012",$J)) D
 . K @ICDTMP
 . S @ICDTMP@(0)="PATCH FY 2012 DRG UPDATE^"_$$NOW^XLFDT
 ;
 ;loop each sub-routine
 S ICDSUB="FGHIJK"
 F ICDI=1:1:6 S ICDRTN="^ICD1856"_$E(ICDSUB,ICDI) D
  .Q:($T(@ICDRTN)="")
...
EDITDRG(ICDLINE,ICDTMP,ICDTOT,ICDEDIT) ; edit existing (#80.2) DRG record
 ...
 S ICDREF="ICDTBL"_$S(ICDDRG<100:"0",1:$E(ICDDRG,1))_"D" ;ECF new line
...
WEIGHTS(ICDLINE,ICDTMP) ;--80.22D subfile - #20 FISCAL YEAR WEIGHTS&TRIM
 ...
 S ICDFYR=3120000,ICDLOW=1,ICDHIGH=99  ; *** default Low/High ???
 S ICDDRG=+$P(ICDLINE,U)
 I '$D(^ICD(ICDDRG,0)) D  Q
 . S @ICDTMP@("ERROR",ICDDRG,0)="MISSING (#80.2) DRG FILE RECORD"
 ;
