ICDEXC2 ;SLC/KER - ICD Extractor - Code APIs (cont) ;04/19/2016
 ;;18.0;DRG Grouper;**57,1**;Oct 20, 2000;Build 1
 ; AICD*4.0*1 IHS/OIT/FBD&NKD - Enhanced robustness: VCC, VCCP
 ;               
 ; Global Variables
 ;    ^ICD0("BA")         N/A
 ;    ^ICD0("ABA")        N/A
...
VCC(IEN,CDT,FMT) ; Return versioned Complication/Comorbidity
 ...
 ;            
 ;N ICDD,ICDI,ICDIC,ICDIC,ICDO,ICDE,ICDF S ICDF=+($G(FMT)),ICDI=+($G(IEN))  ;IHS/OIT/FBD&NKD - CDT TRANSFORM (ROBUST)
 N ICDD,ICDI,ICDIC,ICDIC,ICDO,ICDE,ICDF S ICDF=+($G(FMT)),ICDI=+($G(IEN)),CDT=$S($G(CDT)?7N:$G(CDT),1:$$DT^XLFDT)
