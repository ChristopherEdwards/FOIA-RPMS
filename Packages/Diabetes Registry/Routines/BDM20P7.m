BDM20P7 ; IHS/CMI/LAB - BDM V2.0 patch 7 environment check ; 
 ...
 I '$$INSTALLD("BDM*2.0*6") D SORRY(2)
 ;
 Q
 ;
PRE ;
 ...
 Q
POST ;
 S X=$$DELETE^XPDMENU("BDMMENU","APCDEDMUPD")
 S X=$$ADD^XPDMENU("BDM M MAIN DM MENU","BDM DM2014 AUDIT MENU","DM14",3)
 I 'X W !,"Attempt to add BDM DM2014 AUDIT MENU option failed.." H 3
 S X=$$ADD^XPDMENU("BDMMENU","BDMEEDMUPD","DMU",45)
 I 'X W !,"Attempt to add BDM UPDATE option failed.." H 3
 S X=$$DELETE^XPDMENU("BDMMENU","APCDEDMUPD")
 D BMXPO
 D MEDTAX
 D LABTAX
 D ^BDMTB1
 Q  ;ihs/cmi/maw p7 add necessary post install here
 ;
...
MEDTAX ;EP set up drug taxonomies
 ...
 S BDMX="DM AUDIT SGLT-2 INHIBITOR DRUG",BDMPG="BDM" D DRUG1
 Q
 ;
DRUG1 ;
...
CLEAN(APP) ;-- clean out the RPC multiple first
 ...
 ;
LAB1 ;
 S BDMDB=$O(^ATXLAB("B",BDMX,0))
 I BDMDB G UP41   ;taxonomy already exists
 W !,"Creating ",BDMX," Taxonomy..."
 S X=BDMX,DIC="^ATXLAB(",DIC(0)="L",DIADD=1,DLAYGO=9002228 D ^DIC K DIC,DA,DIADD,DLAYGO,I
 I Y=-1 W !!,"ERROR IN CREATING ",BDMX," TAX" Q
 S BDMDB=+Y,$P(^ATXLAB(BDMDB,0),U,2)=BDMX,$P(^(0),U,5)=DUZ,$P(^(0),U,6)=DT,$P(^(0),U,8)="B",$P(^(0),U,9)=60,$P(^(0),U,22)=0,$P(^(0),U,4)="n",$P(^(0),U,11)=BDMAP
 S ^ATXLAB(BDMDB,21,0)="^9002228.02101PA^0^0"
 S DA=BDMDB,DIK="^ATXAX(" D IX1^DIK
UP41 ;
 F BDMI=1:1 S BDMPI=$P(BDMPG,",",BDMI) Q:BDMPI=""  D
 .S BDMPI=$O(^DIC(9.4,"C","BDM",0))
 .Q:BDMPI=""  ;NO PACKAGE
 .Q:$D(^ATXLAB(BDMDB,41,"B",BDMPI))
 .S X="`"_BDMPI,DIC="^ATXLAB("_BDMDB_",41,",DIC(0)="L",DIC("P")=$P(^DD(9002228,4101,0),U,2),DA(1)=BDMDB
 .D ^DIC
 .I Y=-1 W !,"updating package multiple for ",BDMPI," entry ",$P(^ATXAX(BDMDB,0),U)," failed"
 .K DIC,DA,Y,X
 Q
