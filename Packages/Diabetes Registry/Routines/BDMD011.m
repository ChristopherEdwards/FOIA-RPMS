BDMD011 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ;
...
QUAN ;
 ...
 S Q=$G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,92))
 S V=$E($G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)))  ;test done?
 S T=$P($G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)),U,5)  ;type of test
 ...
 ;...I R[">" S $P(BDMCUML(145),U,14)=$P(BDMCUML(145),U,14)+1 Q
 ;...S R=$$STV^BDMD018(R,5,1) I R>300 S $P(BDMCUML(145),U,14)=$P(BDMCUML(145),U,14)+1 Q
...
EKG ;need date of last ekg
 ...
 S V=$$EKG^BDMD012(BDMPD,BDMRED,"I")
 I V]"" D
...
CREAT ;
 ...
 S V=$$CREAT^BDMD018(BDMPD,BDMBDAT,BDMADAT,"I")
 I V="" S $P(BDMCUML(170),U,5)=$P(BDMCUML(170),U,5)+1 G GFR
 S V=$P(V,U)
 S V=$$STV^BDMD018(V,5,1) I $E(V)'=+$E(V),$E(V)'="." S $P(BDMCUML(170),U,6)=$P(BDMCUML(170),U,6)+1 G GFR ;unable to determine result, not a number
...
GFR ;
 ...
 S V=$E($G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,79)))
...
TCHOL ;
 ...
 S V=$$CHOL^BDMD018(BDMPD,BDMBDAT,BDMADAT,"I")
...
LDL ;
 ...
 S V=$$LDL^BDMD018(BDMPD,BDMBDAT,BDMADAT,"I")
 I V="" S $P(BDMCUML(190),U,7)=$P(BDMCUML(190),U,7)+1 G HDL
 S V=$P(V,U)
 ;S V=$$STV^BDMD018(V,4)
 S V=$$STV^BDMD018(V,5,1) I $E(V)'=+$E(V)!(+V=0) S $P(BDMCUML(190),U,8)=$P(BDMCUML(190),U,8)+1 G HDL ;unable to determine result, not a number or blank
...
HDL ;
 ...
 S V=$$HDL^BDMD018(BDMPD,BDMBDAT,BDMADAT,"I")
 I V="" S $P(BDMCUML(195),U,7)=$P(BDMCUML(195),U,7)+1 G TRIG
 S V=$P(V,U)
 S V=$$STV^BDMD018(V,5,1) I $E(V)'=+$E(V)!(+V=0) S $P(BDMCUML(195),U,8)=$P(BDMCUML(195),U,8)+1 G TRIG ;unable to determine result, not a number
 S V=$P(V,"."),V=$$STV^BDMD018(V,5,1)
...
TRIG ;
 ...
 S V=$$TRIG^BDMD018(BDMPD,BDMBDAT,BDMADAT,"I")
 I V="" S $P(BDMCUML(200),U,7)=$P(BDMCUML(200),U,7)+1 G SELF
 S V=$P(V,U)
 S V=$$STV^BDMD018(V,5,1) I $E(V)'=+$E(V) S $P(BDMCUML(200),U,8)=$P(BDMCUML(200),U,8)+1 G SELF ;unable to determine result, not a number
...
CESS(P,BDATE,EDATE) ;EP - find any cessation hf in 12 months before E
 I '$G(P) Q ""
 I $E($$TOBACCO^BDMD01T(P,$$DOB^AUPNPAT(P),EDATE))'=1 Q ""
 ...
 ;I 'T Q "No"
 S (X,G)=0
 I T F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMD017(P,9999999.09,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G Q "3  Refused "_$P(G,U,6)_" "_$P(G,U,4)_" "_$P(G,U,3)_" "_$P(G,U,5)
 S X=0,G="" F  S X=$O(^AUTTEDT("C","TO-Q",X)) Q:X'=+X!(G)  D
 .S Y=$$REFUSAL^BDMD017(P,9999999.09,X,BDATE,EDATE) I $P(Y,U,1)=1 S G="3  Refused "_$P(Y,U,6)_" "_$P(Y,U,4)_" "_$P(Y,U,3)_" "_$P(Y,U,5)
 I G Q G
