BDMD012 ; IHS/CMI/LAB - 2010 DIABETES AUDIT ;
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**3**;JUN 14, 2007
 ;
 ;cmi/anch/maw 9/12/2010 code set versioning in DEPDX
 ;
SETN ;
...
EKG(P,EDATE,F) ;EP
 ...
 .Q:LEKG>$P(BDM(1),U)
 .S LEKG=$P(BDM(1),U)
 K BDM S %=P_"^LAST PROCEDURE 89.50;DURING "_$$DOB^AUPNPAT(P,"E")_"-"_EDATE,E=$$START1^APCLDF(%,"BDM("),E=$$START1^APCLDF(%,"BDM(")
 I $D(BDM(1)) D
 .Q:LEKG>$P(BDM(1),U)
 .S LEKG=$P(BDM(1),U)
 K BDM S %=P_"^LAST DX 794.31;DURING "_$$DOB^AUPNPAT(P,"E")_"-"_EDATE,E=$$START1^APCLDF(%,"BDM("),E=$$START1^APCLDF(%,"BDM(")
 I $D(BDM(1)) D
 .Q:LEKG>$P(BDM(1),U)
 .S LEKG=$P(BDM(1),U)
 ;check CPT codes in year prior to date range
 S T=$O(^ATXAX("B","DM AUDIT EKG CPTS",0))
 K BDM I T S BDM(1)=$$CPT^BDMD012(P,$$DOB^AUPNPAT(P),ED,T,3) D
 .I BDM(1)="" K BDM Q
 .Q:LEKG>$P(BDM(1),U)
 .S LEKG=$P(BDM(1),U)
 K BDM I T S BDM(1)=$$RAD^BDMD012(P,$$DOB^AUPNPAT(P),ED,T,3) D
 .I BDM(1)="" K BDM Q
 .Q:LEKG>$P(BDM(1),U)
 .S LEKG=$P(BDM(1),U)
...
REFMED(P,BDATE,EDATE) ;EP
 ...
 S T5=$O(^ATXAX("B","DM AUDIT INCRETIN MIMETIC",0))
 S T6=$O(^ATXAX("B","DM AUDIT DPP4 INHIBITOR DRUGS",0))
 S T7=$O(^ATXAX("B","DM AUDIT AMYLIN ANALOGUES",0))
 S T8=$O(^ATXAX("B","DM AUDIT SULFONYLUREA-LIKE",0))
 S G=0
...
TROG(P,BDATE,EDATE) ;EP
 ...
 I $D(BDM(1)) Q "X"
 Q ""
INCR(P,BDATE,EDATE) ;EP  cmi/maw 12/18/2007 DM2010
 NEW X,BDM,E
 S X=P_"^LAST MEDS [DM AUDIT INCRETIN MIMETIC"_";DURING "_BDATE_"-"_EDATE S E=$$START1^APCLDF(X,"BDM(")
 I $D(BDM(1)) Q "X"
 Q ""
 ;
DPP4(P,BDATE,EDATE) ;EP  cmi/maw 12/18/2010 DM2010
 NEW X,BDM,E
...
DEPDX(P,BDATE,EDATE) ;EP
 NEW BDM,X,BDMP,BDMP1,T,I,G,J,D
 K BDM
 S (G,X,I)=""
 ...
 ..Q:'BDMP
 ..S BDMP1=$P($G(^AMHPROB(BDMP,0)),U)
 ..I BDMP1=14 S BDM=BDM+1 Q
 ..I BDMP1=15 S BDM=BDM+1 Q
 ..I BDMP1=18 S BDM=BDM+1 Q
 ..I BDMP1=24 S BDM=BDM+1 Q
...
DEPSCR(P,BDATE,EDATE,F,R) ;EP
 ...
 I $G(R) Q $S(F="D":$P(BDMC,U),1:"1  Yes - "_$P(BDMC,U,2)_"  "_$$FMTE^XLFDT($P(BDMC,U)))
 I BDMC]"" Q "1  Yes - "_$P(BDMC,U,2)_"  "_$$FMTE^XLFDT($P(BDMC,U))
 ;refusal
 NEW G S G=$$REFUSAL^BDMD017(P,9999999.15,$O(^AUTTEXAM("B","DEPRESSION SCREENING",0)),BDATE,EDATE)
 I G,'$G(R) Q "3  Refused "_$P(G,U,3)
 ...
 .I $P($G(^AMHREC(V,14)),U,5)]"",($P(^AMHREC(V,14),U,5)="UAS"!($P(^AMHREC(V,14),U,5)'="REF")) S BDMC="3  Refused BH Exam 36 "_$$DATE^BDMD010(9999999-D) Q
 I BDMC]"" Q BDMC
 Q "2  No"
 ;
LOINC(A,B) ;
