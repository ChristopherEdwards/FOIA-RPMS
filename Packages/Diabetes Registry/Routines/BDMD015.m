BDMD015 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 03 Feb 2010 5:38 PM ; [ 03 Feb 2010 5:38 PM ]
...
CUML ;EP
 ...
 S BDMCUML(100)="ANTIPLATELET THERAPY (Over age 40)"
...
PROCESS ;
 S BDMNOGO=0
 S BDMPD=0 F  S BDMPD=$O(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD  D CUML1
 Q
 ;
CUML1 ;
GENDER ;
 I $$DODX^BDMD016(BDMPD,BDMDMRG,"I")>BDMADAT S BDMNOGO=BDMNOGO+1 Q
 ;gender BDMCUML(10)="Gender^total^females^males"
 S $P(BDMCUML(10),U,2)=$P(BDMCUML(10),U,2)+1
 S V=$G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,20))
...
TYPE ;
 ...
 S X=$$TYPE^BDMD016(BDMPD,BDMDMRG,BDMADAT)
...
DURDMC ;
 ...
 S V=$$DURDM^BDMD014(BDMPD,BDMDMRG,BDMADAT)
...
BMI ;
 ...
 ;S V=$G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,112))
 S H=$$LASTHT^BDMD013(BDMPD,BDMRED,"I") S:H]"" H=$J(H,5,2)
 S W=+$$LASTWT^BDMD013(BDMPD,BDMBDAT,BDMRED),W=$S(W=0:"",1:W),W=W+.5,W=$P(W,".")
 S V=$$BMIEPI^BDMD019(H,W)
 S $P(BDMCUML(40),U,2)=$P(BDMCUML(40),U,2)+1
 D
 .I V="" S $P(BDMCUML(40),U,5)=$P(BDMCUML(40),U,5)+1 Q
 .I $$OW^BDMD014(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,3)=$P(BDMCUML(40),U,3)+1 Q
 .I $$OB^BDMD014(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,4)=$P(BDMCUML(40),U,4)+1 Q
 .S $P(BDMCUML(40),U,6)=$P(BDMCUML(40),U,6)+1 Q
HGB ;
 ;use last hgba1c value only
 ;BDMCUML(50)=
 S $P(BDMCUML(50),U,2)=$P(BDMCUML(50),U,2)+1
 S V=$P($G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,78)),U,2)
 S P=""
 I V=""!(V="?") S P=9 G HGBS
 I V["<" S P=3
 I V[">" S P=8
...
TBSTAT ;
 ...
 S V=$$TBCODE^BDMD016(BDMPD,BDMRED,BDMDMRG)
 S $P(BDMCUML(70),U,(V+2))=$P(BDMCUML(70),U,(V+2))+1
TOBACCO ;
 S $P(BDMCUML(80),U,2)=$P(BDMCUML(80),U,2)+1
 S V=$G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,27))
 S V1=$G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,28))
...
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^BDMD016(BDMPD,BDM6MBD,BDMRED)  ;^ pieced with each item  or =1 for diet alone or =11 for refused
 I V=1 S $P(BDMCUML(90),U,3)=$P(BDMCUML(90),U,3)+1 G ASPIRIN
 I V="R" S $P(BDMCUML(90),U,13)=$P(BDMCUML(90),U,13)+1 G ASPIRIN
 N I,INS,ORAL,OTHER
 S (INS,ORAL,OTHER)=""
 F I=1:1 S Q=$P(V,U,I) Q:Q=""  D
 . S P=$S(Q=2:4,Q=3:5,Q=4:6,Q=5:7,Q=6:8,Q=7:9,Q=8:10,Q=9:11,Q=10:12,1:"") S $P(BDMCUML(90),U,P)=$P(BDMCUML(90),U,P)+1
 . I Q=2 S INS=1 Q
 . I Q=8 S OTHER=1 Q  ;byetta
 . I Q=10 S OTHER=1 Q  ;amylin
 . S ORAL=ORAL+1
 ;
 I INS,ORAL S $P(BDMCUML(90),U,15)=$P(BDMCUML(90),U,15)+1 G ASPIRIN
 I INS,OTHER S $P(BDMCUML(90),U,15)=$P(BDMCUML(90),U,15)+1 G ASPIRIN
 I INS G ASPIRIN
 I ORAL>1 S $P(BDMCUML(90),U,14)=$P(BDMCUML(90),U,14)+1  ;
 ;
ASPIRIN ;
 ...
 S V=$G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
...
ACE ;110 title^total pts^total pts with protein^# of those on ace^# with htn^# of those on ace"
 ...
 S P=$G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,92))  ;URINE PROTEIN VALUE
 S H=$E($G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,34)))
 S A=$E($G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,60)))
...
LIPID ;115
 S V=$G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,61))
 S L=$$LDL^BDMD018(BDMPD,BDMBDAT,BDMADAT,"I"),L=$P(L,U)
 S T=$$CHOL^BDMD018(BDMPD,BDMBDAT,BDMADAT,"I"),T=$P(T,U)
 S T=$$STV^BDMD018(T,5,1)
 S L=$$STV^BDMD018(L,5,1)
DEP ;
 S $P(BDMCUML(300),U,2)=$P(BDMCUML(300),U,2)+1
 S V=$G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,200))
 I $E(V)="1" S $P(BDMCUML(300),U,3)=$P(BDMCUML(300),U,3)+1
 I $E(V)'="1" S $P(BDMCUML(301),U,2)=$P(BDMCUML(301),U,2)+1 D
 .S V=$G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,210))
...
EXAMS ;
 ...
 S V=$G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,108))
...
EDUC ;
 ...
 S G=0,V=$G(^XTMP("BDMDM01",BDMJOB,BDMBTH,"AUDIT",BDMPD,44))
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X,Z S X=$$BPS^BDMD013(P,BDATE,EDATE,"I")
 I X="" Q ""
