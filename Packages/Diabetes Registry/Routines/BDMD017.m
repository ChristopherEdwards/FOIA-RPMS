BDMD017 ; IHS/CMI/LAB - 2010 DIABETES AUDIT ;
...
EXEDUC(P,BDATE,EDATE) ;EP
 ...
 I $D(BDM(1)) Q "1  Yes  "_$P(BDM(1),U,3)_"  "_$$FMTE^XLFDT($P(BDM(1),U,1))
 K BDM
 ...
 I $D(BDM(1)) Q "1  Yes  POV: "_$P(BDM(1),U,3)_"  "_$$FMTE^XLFDT($P(BDM(1),U))
 S G=0
...
DFE(P,BDATE,EDATE,F,R) ;EP - FOOT EXAM
 ...
 I $G(F)="" S F="E"
 NEW BDMY,BDMV,%,LASTI,A,D,V,G,PROV,E,T
 S LASTI=""
 S BDMY(1)=$$LASTDFE^APCLAPI2(P,BDATE,EDATE,"D")
 I BDMY(1) S LASTI=$P(BDMY(1),U)_U_"1  Yes - Diabetic Foot Exam - "_$$FMTE^XLFDT($P(BDMY(1),U))
 ...
 N PROV,D,V,G
 S (D,V)=0,G=""
 F  S D=$O(BDMY(D)) Q:D'=+D!(G)  S V=0 F  S V=$O(BDMY(D,V)) Q:V'=+V!(G)  S PROV=$$PRIMPROV^APCLV(V,"D") I (PROV=33!(PROV=25)!(PROV=84)),'$$DNKA(V) S G=9999999-D
 I G]"" D
 .Q:$P(LASTI,U)>G
 .S LASTI=G_U_"1  Yes - Podiatrist Visit - "_$$FMTE^XLFDT($P(G,U))
 S (D,V)=0,G=""
 F  S D=$O(BDMY(D)) Q:D'=+D!(G)  S V=0 F  S V=$O(BDMY(D,V)) Q:V'=+V!(G)  S PROV=$$CLINIC^APCLV(V,"C") I PROV=65!(PROV="B7"),'$$DNKA(V) S G=9999999-D
 I G]"" D
 .Q:$P(LASTI,U)>G
 .S LASTI=G_U_"1  Yes - Podiatry Clinic visit - "_$$FMTE^XLFDT($P(G,U))
 ;
...
REFUSAL(P,F,I,B,E) ;EP
 ...
 .S G=1_"^"_$P(^AUPNPREF(Y,0),U,7)_U_$$VAL^XBDIQ1(9000022,Y,.03)_U_$$VAL^XBDIQ1(9000022,Y,.04)_U_$$VAL^XBDIQ1(9000022,Y,.07)_U_$$VAL^XBDIQ1(9000022,Y,.01)_U_$P(^AUPNPREF(Y,0),U,3)
 Q G
 ;
EYE(P,BDATE,EDATE,F,R) ;EP
 ...
 I $G(EDATE)="" S EDATE=DT
 I $G(F)="" S F="E"
 NEW BDMY,BDMV,%,LASTI,BD,ED,T,D,%,Y,X,G,V,PROV,T
 S LASTI=$$LASTDEYE^APCLAPI2(P,BDATE,EDATE,"D")
 I LASTI]"" S $P(LASTI,U,2)="1  Yes - Diabetic Eye Exam - "_$$FMTE^XLFDT($P(LASTI,U))
 ;
 S X=$$LASTCPTT^BDMAPIU(P,BDATE,EDATE,$O(^ATXAX("B","DM AUDIT EYE EXAM CPTS",0)),"E")
 I $P(X,U)>$P(LASTI,U) S LASTI=$P(X,U)_U_"1  Yes - "_$P(X,U,2)_" - "_$$FMTE^XLFDT($P(X,U))
 K BDMV,BDMY
 ;
 K T
 S T="T"
 ...
 N PROV,D,V,G
 S (D,V)=0,G=""
 F  S D=$O(BDMY(D)) Q:D'=+D!(G)  S V=0 F  S V=$O(BDMY(D,V)) Q:V'=+V!(G)  S PROV=$$PRIMPROV^APCLV(V,"D") I (PROV=24!(PROV=79)!(PROV="08")),'$$DNKA(V),'$$REFR(V) S G=9999999-D
 I G]"" D
 .Q:$P(LASTI,U)>G
 .S LASTI=G_U_"1  Yes - Optometrist/Opthalmalogist Visit - "_$$FMTE^XLFDT($P(G,U))
 S (D,V)=0,G=""
 F  S D=$O(BDMY(D)) Q:D'=+D!(G)  S V=0 F  S V=$O(BDMY(D,V)) Q:V'=+V!(G)  S PROV=$$CLINIC^APCLV(V,"C") I PROV=17!(PROV=18)!(PROV=64)!(PROV="A2"),'$$DNKA(V),'$$REFR(V) S G=9999999-D
 I G]"" D
 .Q:$P(LASTI,U)>G
 .S LASTI=G_U_"1  Yes - Optometrist/Opthalmology Clinic visit - "_$$FMTE^XLFDT($P(G,U))
 ;
 I $G(R) Q $S(F="D":$P(LASTI,U),1:$P(LASTI,U,2))  ;no refusals
 ;
 I LASTI]"" Q $S(F="D":$P(LASTI,U),1:$P(LASTI,U,2))
...
DENTAL(P,BDATE,EDATE,F,R) ;EP
 ...
 I LASTI]"" S $P(LASTI,U,2)="1  Yes - Dental Exam - "_$$FMTE^XLFDT($P(LASTI,U))
 ;
 K BDMV,BDMY
 ;
 S T="T"
 ...
 N PROV,D,V,G
 S (D,V)=0,G=""
 F  S D=$O(BDMY(D)) Q:D'=+D!(G)  S V=0 F  S V=$O(BDMY(D,V)) Q:V'=+V!(G)  S PROV=$$PRIMPROV^APCLV(V,"D") I PROV=52,$$ADA(V),'$$DNKA(V) S G=9999999-D
 I G]"" D
 .Q:$P(LASTI,U)>G
 .S LASTI=G_U_"1  Yes - Dentist Visit - "_$$FMTE^XLFDT($P(G,U))
 S (D,V)=0,G=""
 ...
 I G]"" D
 .Q:$P(LASTI,U)>G
 .S LASTI=G_U_"1  Yes - Dental Clinic visit - "_$$FMTE^XLFDT($P(G,U))
 ;
 I $G(R) Q $S(F="D":$P(LASTI,U),1:$P(LASTI,U,2))  ;no refusals
 ;
 I LASTI]"" Q $S(F="D":$P(LASTI,U),1:$P(LASTI,U,2))
