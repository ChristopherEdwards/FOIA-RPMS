BDMD116 ; IHS/CMI/LAB - 2011 DIABETES AUDIT ;
...
TBCODE(P,EDATE,R) ;EP
 ...
 I $E(X)=1 Q 1
 I $E(X) Q 2
 S BDMR=$$PPD^BDMD118(P,EDATE)
 I $E($P(BDMR,"||",2))=1 D  Q BDMJ
 .I $$TBTX^BDMD112(P)["TX COMPLETE" S BDMJ=1 Q
 .S BDMJ=2
 .Q
 I $E($P(BDMR,"||",2))=2 S BDMJ=4 D  Q BDMJ
 .I $$DODX(P,R,"I")="" S BDMJ=6 Q
 .S D=$$DODX(P,R,"I"),E=$$PPD^BDMD118(P,EDATE,"I") S BDMJ=$S(D>E:4,1:3)
 .Q
 I $E(BDMR)=4 S BDMJ=5
 I $E(BDMR)=3 S BDMJ=7
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X S X=$$BPS^BDMD113(P,BDATE,EDATE,"I")
 I X="" Q ""
...
DIAMEAN(P,BDATE,EDATE) ;EP
 ...
 Q C\Z
PPDDATE(P,EDATE) ;EP
 NEW X S X=$$LASTNP^BDMD118(P,EDATE)
 Q X
XLIPID(P,BDATE,EDATE) ;EP
 ...
 I 'T G LIPIDN
 S X=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMD117(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G Q "5  Refused or Adverse Rxn"
LIPIDN ;
 NEW T S T=$O(^ATXAX("B","DM AUDIT STATIN DRUGS",0))
 I 'T G LIPIDA
 S (X,G)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMD117(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G Q "5  Refused or Adverse Rxn"
LIPIDA ;
...
ACE(P,BDATE,EDATE) ;EP
 ...
 I 'T Q "2  No"
 S (G,X)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMD117(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
...
SELF(P,BDATE,EDATE) ;EP
 ...
 S E=$$LASTHF^BDMD119(P,"DIABETES SELF MONITORING",BD,ED,"F")
...
SDM(P,BDATE,EDATE) ;EP
 ...
 S X=EDATE,%DT="P" D ^%DT S ED=Y
 S X=BDATE,%DT="P" D ^%DT S BD=Y
 S E=$$LASTHF^BDMD119(P,"STAGED DIABETES MANAGEMENT",BD,ED)
 I E Q "Yes"
...
THERAPY(P,BD,EDATE) ;EP - therapy code for epi
 ...
 S STR="",TNAME="",HO=""
 S X=$$INSULIN^BDMD112(P,BD,EDATE)
 I X]"" S STR=STR_2_U,HO=1
 S X=$$SULF^BDMD112(P,BD,EDATE)
 I X]"" S STR=STR_3_U,HO=1
 S X=$$SULFLIKE^BDMD112(P,BD,EDATE)
 I X]"" S STR=STR_4_U,HO=1
 S X=$$MET^BDMD112(P,BD,EDATE)
 I X]"" S STR=STR_5_U,HO=1
 S X=$$ACAR^BDMD112(P,BD,EDATE)
 I X]"" S STR=STR_6_U,HO=1
 S X=$$TROG^BDMD112(P,BD,EDATE)
 I X]"" S STR=STR_7_U,HO=1
 S X=$$INCR^BDMD112(P,BD,EDATE)
 I X]"" S STR=STR_8_U,HO=1
 S X=$$DPP4^BDMD112(P,BD,EDATE)
 I X]"" S STR=STR_9_U,HO=1
 S X=$$AMYLIN^BDMD112(P,BD,EDATE)
 I X]"" S STR=STR_10_U,HO=1
 S X=$$GLP1^BDMD112(P,BD,EDATE)
 I X]"" S STR=STR_11_U,HO=1
 S X=$$BROM^BDMD112(P,BD,EDATE)
 I X]"" S STR=STR_12_U,HO=1
 I HO Q STR
 S X=$$REFMED^BDMD112(P,BD,EDATE)
 I X Q "R"
 Q 1
 ;
...
REFMEDLE(P,BD,EDATE) ;EP - ANY REFUSAL?
 NEW STR,TNAME,X,Y,%DT
 S STR="",TNAME=""
 S X=$$STATIN^BDMD116(P,BD,EDATE)
 I X]"" Q 2
 S X=$$FIBRATE^BDMD116(P,BD,EDATE)
 I X]"" Q 2
 S X=$$NIACIN^BDMD116(P,BD,EDATE)
 I X]"" Q 2
 S X=$$BILE^BDMD116(P,BD,EDATE)
 I X]"" Q 2
 S X=$$EZET^BDMD116(P,BD,EDATE)
 I X]"" Q 2
 S X=$$FISHOIL^BDMD116(P,BD,EDATE)
 I X]"" Q 2
 S X=$$LOVAZA^BDMD116(P,BD,EDATE)
 I X]"" Q 2
 S X=$$REFMEDL^BDMD116(P,BD,EDATE)
 I X Q 1
 Q 2
 ;
...
DODX(P,R,F) ;EP - date of dx for epi file
 ...
 I $G(R) S DATE=$$CMSFDX^BDMD113(P,R,"ID")
 I DATE S EARLY=DATE
 S DATE=$$PLDMDOO^BDMD113(P,"I")
