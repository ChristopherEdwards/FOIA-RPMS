BDMD11H ; IHS/CMI/LAB - 2011 DIABETES AUDIT ;
...
1 ;
 ...
 .S R=$P(BDM(D,C),U,2)
 .I R["<"!(R[">") S R=$$STV^BDMD118(R,8) I R]"" S G="1  Yes  "_U_$P(BDM(D,C),U,2)_U_$$FMTE^XLFDT($P(BDM(D,C),U))_U_$P(BDM(D,C),U,3)_U_1_U_R
 .S R=$$STV^BDMD118(R,8)
 .I R="" D  Q
 ..S (E,X,J,L)=""
...
R ;
 S G=0
 S T=$O(^ATXLAB("B","DM AUDIT QUANT UACR",0))
 S X=0 F  S X=$O(^ATXLAB(T,21,X)) Q:X'=+X!(G)  I $$REFUSAL^BDMD117(P,60,$P(^ATXLAB(T,21,X,0),U),BDATE,EDATE) S G=1
 I G Q "3  Refused"
 S G=0
 S T=$O(^ATXLAB("B","DM AUDIT P/C RATIO TAX",0))
 S X=0 F  S X=$O(^ATXLAB(T,21,X)) Q:X'=+X!(G)  I $$REFUSAL^BDMD117(P,60,$P(^ATXLAB(T,21,X,0),U),BDATE,EDATE) S G=1
 I G Q "3  Refused"
 S G=0
 S T=$O(^ATXLAB("B","DM AUDIT 24HR URINE PROTEIN",0))
 S X=0 F  S X=$O(^ATXLAB(T,21,X)) Q:X'=+X!(G)  I $$REFUSAL^BDMD117(P,60,$P(^ATXLAB(T,21,X,0),U),BDATE,EDATE) S G=1
 I G Q "3  Refused"
 S G=0
 ...
 I G Q "3  Refused"
 S G=0
 S T=$O(^ATXLAB("B","DM AUDIT URINALYSIS TAX",0))
 S X=0 F  S X=$O(^ATXLAB(T,21,X)) Q:X'=+X!(G)  I $$REFUSAL^BDMD117(P,60,$P(^ATXLAB(T,21,X,0),U),BDATE,EDATE) S G=1
 I G Q "3  Refused"
 Q "2  No"
