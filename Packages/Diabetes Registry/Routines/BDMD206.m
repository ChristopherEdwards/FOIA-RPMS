BDMD206 ; IHS/CMI/LAB - 2000 DIABETES AUDIT ;
...
TBCODE(P,EDATE,R) ;EP
 ...
 I X]"" Q 2
 I $$PPD^BDMD208(P,EDATE)["POS" D  Q BDMJ
 .I $$TBTX^BDMD202(P)["TX COMPLETE" S BDMJ=1 Q
 .S BDMJ=2
 .Q
 I $$PPD^BDMD208(P,EDATE)["NEG" S BDMJ=4 D  Q BDMJ
 .I $$DODX(P,R,"I")="" S BDMJ=4 Q
 .S D=$$DODX(P,R,"I"),E=$$PPD^BDMD208(P,EDATE,"I") S BDMJ=$S(D>E:4,1:3)
 .Q
 S BDMJ=5
 Q BDMJ
 ;;
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X S X=$$BPS^BDMD207(P,BDATE,EDATE,"I")
 I X="" Q ""
...
DIAMEAN(P,BDATE,EDATE) ;EP
 ...
 Q C\3
PPDDATE(P,EDATE) ;EP
 NEW X S X=$$LASTNP^BDMD208(P,EDATE)
 Q X
FLU(P,BDATE,EDATE) ;EP
 ...
 K BDM I T S BDM(1)=$$CPT^BDMD202(P,BD,ED,T,3) D
 .I BDM(1)="" K BDM Q
 .Q:LFLU>$P(BDM(1),U)
 .S LFLU=$P(BDM(1),U)
 I LFLU]"" Q "Yes  "_$$FMTE^XLFDT(LFLU)
 ;
 I $$REFUSAL^BDMD207(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:88,1:12),0)),BD,EDATE) Q "Refused"
 Q "No"
PNEU(P,EDATE) ;EP
 NEW BDM,X,E
 ...
 I $$REFUSAL^BDMD207(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:33,1:19),0)),$$DOB^AUPNPAT(P,"E"),EDATE) Q "Refused"
 Q "No"
TD(P,EDATE) ;EP
 ...
 I $$REFUSAL^BDMD207(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:9,1:"02"),0)),B,EDATE) Q "Refused"
 I $$REFUSAL^BDMD207(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:1,1:"03"),0)),B,EDATE) Q "Refused"
 I $$REFUSAL^BDMD207(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:28,1:34),0)),B,EDATE) Q "Refused"
 I $$REFUSAL^BDMD207(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:20,1:42),0)),B,EDATE) Q "Refused"
 Q "No"
 ;
LIPID(P,BDATE,EDATE) ;EP
 NEW X,BDM,E
 S X=P_"^MEDS [DM AUDIT LIPID LOWERING DRUGS"_";DURING "_BDATE_"-"_EDATE S E=$$START1^APCLDF(X,"BDM(")
 I $D(BDM(1)) Q "Yes"
 Q "No"
 ;
...
TOBACCO1 ;check problem file for tobacco use
 ...
 . I $P($$ICDDX^ICDCODE($P(BDM(1),U,2)),U,2)=305.13 S BDMTOB="2  Not a Current User"_" - "_$E($P(^AUTNPOV($P(^AUPNPROB(+$P(BDM(1),U,4),0),U,5),0),U),1,30) Q  ;cmi/anch/maw 9/10/2007 csv
...
THERAPY(P,BD,EDATE) ;EP - therapy code for epi
 ...
 S STR="",TNAME=""
 S X=$$INSULIN^BDMD202(P,BD,EDATE)
 I X]"" S STR=STR_"2"
 S X=$$SULF^BDMD202(P,BD,EDATE)
 I X]"" S STR=STR_3
 S X=$$MET^BDMD202(P,BD,EDATE)
 I X]"" S STR=STR_4
 S X=$$ACAR^BDMD202(P,BD,EDATE)
 I X]"" S STR=STR_5
 S X=$$TROG^BDMD202(P,BD,EDATE)
 I X]"" S STR=STR_"6"
 I STR]"" Q STR
 Q 1
 ;
...
DODX(P,R,F) ;EP - date of dx for epi file
 ...
 I $G(R) S DATE=$$CMSFDX^BDMD207(P,R,"ID")
 I DATE]"" S EARLY=DATE
 S DATE=$$PLDMDOO^BDMD207(P,"I")
