BDMD216 ; IHS/CMI/LAB - 2001 DIABETES AUDIT ;
...
TBCODE(P,EDATE,R) ;EP
 ...
 I X]"" Q 2
 I $$PPD^BDMD218(P,EDATE)["POS" D  Q BDMJ
 .I $$TBTX^BDMD212(P)["TX COMPLETE" S BDMJ=1 Q
 .S BDMJ=2
 .Q
 I $$PPD^BDMD218(P,EDATE)["NEG" S BDMJ=4 D  Q BDMJ
 .I $$DODX(P,R,"I")="" S BDMJ=4 Q
 .S D=$$DODX(P,R,"I"),E=$$PPD^BDMD218(P,EDATE,"I") S BDMJ=$S(D>E:4,1:3)
 .Q
 S BDMJ=5
 Q BDMJ
 ;;
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X S X=$$BPS^BDMD217(P,BDATE,EDATE,"I")
 I X="" Q ""
...
DIAMEAN(P,BDATE,EDATE) ;EP
 ...
 Q C\3
PPDDATE(P,EDATE) ;EP
 NEW X S X=$$LASTNP^BDMD218(P,EDATE)
 Q X
FLU(P,BDATE,EDATE) ;EP
 ...
 K BDM I T S BDM(1)=$$CPT^BDMD212(P,BD,ED,T,3) D
 .I BDM(1)="" K BDM Q
 .Q:LFLU>$P(BDM(1),U)
 .S LFLU=$P(BDM(1),U)
 I LFLU]"" Q "Yes  "_$$FMTE^XLFDT(LFLU)
 ;
 I $$REFUSAL^BDMD217(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:88,1:12),0)),BD,EDATE) Q "Refused"
 Q "No"
PNEU(P,EDATE) ;EP
 NEW BDM,X,E
 ...
 I $$REFUSAL^BDMD217(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:33,1:19),0)),$$DOB^AUPNPAT(P,"E"),EDATE) Q "Refused"
 Q "No"
TD(P,EDATE) ;EP
 ...
 K BDM S X=P_"^LAST IMM "_$S($$BI:35,1:"04")_";DURING "_$$FMTE^XLFDT($$FMADD^XLFDT(B,-3653))_"-"_EDATE S E=$$START1^APCLDF(X,"BDM(")
 I $D(BDM(1)) S TDD($P(BDM(1),U))=""
 K BDM S BDM="",X=0 F  S X=$O(TDD(X)) Q:X'=+X  S BDM=X
 I BDM]"" Q "Yes - "_$$FMTE^XLFDT(BDM)
 S B=$$FMTE^XLFDT($$FMADD^XLFDT(B,-3653))
 I $$REFUSAL^BDMD217(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:9,1:"02"),0)),B,EDATE) Q "Refused"
 I $$REFUSAL^BDMD217(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:1,1:"03"),0)),B,EDATE) Q "Refused"
 I $$REFUSAL^BDMD217(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:28,1:34),0)),B,EDATE) Q "Refused"
 I $$REFUSAL^BDMD217(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:20,1:42),0)),B,EDATE) Q "Refused"
 I $$REFUSAL^BDMD217(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:35,1:04),0)),B,EDATE) Q "Refused"
 Q "No"
 ;
...
SELF(P,BDATE,EDATE) ;EP
 ...
 S E=$$LASTHF^BDMD219(P,"DIABETES SELF MONITORING",BD,ED,"F")
...
SDM(P,BDATE,EDATE) ;EP
 ...
 S X=EDATE,%DT="P" D ^%DT S ED=Y
 S X=BDATE,%DT="P" D ^%DT S BD=Y
 S E=$$LASTHF^BDMD219(P,"STAGED DIABETES MANAGEMENT",BD,ED)
 I E Q "Yes"
...
THERAPY(P,BD,EDATE) ;EP
 ...
 S STR="",TNAME=""
 S X=$$INSULIN^BDMD212(P,BD,EDATE)
 I X]"" S STR=STR_"2"
 S X=$$SULF^BDMD212(P,BD,EDATE)
 I X]"" S STR=STR_3
 S X=$$MET^BDMD212(P,BD,EDATE)
 I X]"" S STR=STR_4
 S X=$$ACAR^BDMD212(P,BD,EDATE)
 I X]"" S STR=STR_5
 S X=$$TROG^BDMD212(P,BD,EDATE)
 I X]"" S STR=STR_"6"
 I STR]"" Q STR
 Q 1
 ;
