BDMD218 ; IHS/CMI/LAB - 2001 DIABETES AUDIT ;
...
URIN(P,BDATE,EDATE) ;EP
 ...
 I 'T Q ""
 S X=0 F  S X=$O(^ATXLAB(T,21,X)) Q:X'=+X!(G)  I $$REFUSAL^BDMD217(P,60,$P(^ATXLAB(T,21,X,0),U),BDATE,EDATE) S G=1
 Q $S(G:"Refused",1:"No")
...
PPD(P,EDATE,F) ;EP
 ...
 .S X=EDATE,%DT="P" D ^%DT S BDATE=$$FMADD^XLFDT(Y,-365),BDATE=$$FMTE^XLFDT(BDATE),G=0 I $$REFUSAL^BDMD217(P,9999999.28,$O(^AUTTSK("B","PPD",0)),BDATE,EDATE) S G=1
