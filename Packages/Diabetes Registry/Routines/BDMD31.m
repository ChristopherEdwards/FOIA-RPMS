BDMD31 ; IHS/CMI/LAB - IHS Diabetes Audit 2003 ;
...
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMDM31",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
DEMO ;
 ...
 I BDMDEMO=-1 G IF
 I BDMPREP'=2 S XBRP="^BDMD31P",XBRC="^BDMD310",XBRX="XIT^BDMD31",XBNS="BDM"
 I BDMPREP=2 S XBRP="",XBRC="^BDMD310",XBRX="XIT^BDMD31",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
P ;
 S BDMSTP=0 K ^XTMP("BDMDM31",BDMJOB,BDMBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMDM31",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMDM31",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("BDMDM31",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMDM31",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMDM31",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
WRITEF ;EP write flat file
 ...
 K ^XTMP("BDMDM31",BDMJOB,BDMBTH),BDMJOB,BDMBTH
 Q
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPCP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN) ;PEP - gui call
 ...
 N BDMOPT  ;maw
 S BDMOPT="2003 Diabetes Program Audit"
 D NOW^%DTC
 S BDMNOW=$G(%)
 K DD,D0,DIC
 S X=BDMJOB_"."_BDMBTH
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMD31",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMD31P","^TMP($J,""BDMDM31"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMDM31",X)) Q:X'=+X  S ^BDMGUI(BDMIEN,11,X,0)=^TMP($J,"BDMDM31",X),C=C+1
...
TEST ;
 ...
 F X=1:1:10 S ^XTMP("BDMDM31",BDMJOB,BDMBTH,"PATS",X)=""
 D BDMG^BDMD31(BDMJOB,BDMBTH,1,DT,"P","","","","","","","",4,"TESTEPI",.BDMIEN)
 Q
