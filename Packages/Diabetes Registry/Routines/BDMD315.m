BDMD315 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ;
...
CUML ;EP
 ...
 ;
PROCESS ;
 S BDMPD=0 F  S BDMPD=$O(^XTMP("BDMDM31",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD  D CUML1
 Q
 ;
CUML1 ;
GENDER ;
 ...
 S V=$G(^XTMP("BDMDM31",BDMJOB,BDMBTH,"AUDIT",BDMPD,20))
...
TYPE ;
 ...
 S X=$$TYPE^BDMD316(BDMPD,BDMDMRG,BDMADAT)
...
DURDMC ;
 ...
 S V=$$DURDM^BDMD314(BDMPD,BDMDMRG,BDMADAT)
...
BMI ;
 ...
 S V=$G(^XTMP("BDMDM31",BDMJOB,BDMBTH,"AUDIT",BDMPD,112))
 S $P(BDMCUML(40),U,2)=$P(BDMCUML(40),U,2)+1
 D
 .I V="" S $P(BDMCUML(40),U,5)=$P(BDMCUML(40),U,5)+1 Q
 .I $$OW^BDMD314(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,3)=$P(BDMCUML(40),U,3)+1
 .I $$OB^BDMD314(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,4)=$P(BDMCUML(40),U,4)+1
HGB ;
 ;use last hgba1c value only
 ;BDMCUML(50)=
 S $P(BDMCUML(50),U,2)=$P(BDMCUML(50),U,2)+1
 S V=$P($G(^XTMP("BDMDM31",BDMJOB,BDMBTH,"AUDIT",BDMPD,78)),U)
...
TBSTAT ;
 ...
 S V=$$TBCODE^BDMD316(BDMPD,BDMRED,BDMDMRG)
 S $P(BDMCUML(70),U,(V+2))=$P(BDMCUML(70),U,(V+2))+1
TOBACCO ;
 S $P(BDMCUML(80),U,2)=$P(BDMCUML(80),U,2)+1
 S V=$G(^XTMP("BDMDM31",BDMJOB,BDMBTH,"AUDIT",BDMPD,27))
 S V1=$G(^XTMP("BDMDM31",BDMJOB,BDMBTH,"AUDIT",BDMPD,28))
...
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^BDMD316(BDMPD,BDM6MBD,BDMRED)
...
ASPIRIN ;
 ...
 S V=$G(^XTMP("BDMDM31",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
...
ACE ;110 title^total pts^total pts with protein^# of those on ace^# with htn^# of those on ace"
 ...
 S P=$G(^XTMP("BDMDM31",BDMJOB,BDMBTH,"AUDIT",BDMPD,94))
 S H=$G(^XTMP("BDMDM31",BDMJOB,BDMBTH,"AUDIT",BDMPD,34))
 S A=$G(^XTMP("BDMDM31",BDMJOB,BDMBTH,"AUDIT",BDMPD,60))
...
LIPID ;115
 S V=$G(^XTMP("BDMDM31",BDMJOB,BDMBTH,"AUDIT",BDMPD,61))
 S L=$$LDL^BDMD318(BDMPD,BDMRBD,BDMRED,"I"),L=$P(L,U)
 S T=$$CHOL^BDMD318(BDMPD,BDMRBD,BDMRED,"I"),T=$P(T,U)
 I T]"",T'<240 S $P(BDMCUML(115),U,3)=$P(BDMCUML(115),U,3)+1 I "SOB"[$E(V) S $P(BDMCUML(115),U,4)=$P(BDMCUML(115),U,4)+1
 I L]"",L>130 S $P(BDMCUML(115),U,5)=$P(BDMCUML(115),U,5)+1 I "SOB"[$E(V) S $P(BDMCUML(115),U,6)=$P(BDMCUML(115),U,6)+1 ;IHS/CMI/LAB CHANGED 130 TO 160 PER DR. SHIELDS
 ;lipid agents
...
EXAMS ;
 ...
 S V=$G(^XTMP("BDMDM31",BDMJOB,BDMBTH,"AUDIT",BDMPD,108))
