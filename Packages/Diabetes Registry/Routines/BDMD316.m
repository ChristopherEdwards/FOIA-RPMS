BDMD316 ; IHS/CMI/LAB - 2003 DIABETES AUDIT ;
...
TBCODE(P,EDATE,R) ;EP
 ...
 I X]"" Q 2
 I $$PPD^BDMD318(P,EDATE)["POS" D  Q BDMJ
 .I $$TBTX^BDMD312(P)["TX COMPLETE" S BDMJ=1 Q
 .S BDMJ=2
 .Q
 I $$PPD^BDMD318(P,EDATE)["NEG" S BDMJ=4 D  Q BDMJ
 .I $$DODX(P,R,"I")="" S BDMJ=4 Q
 .S D=$$DODX(P,R,"I"),E=$$PPD^BDMD318(P,EDATE,"I") S BDMJ=$S(D>E:4,1:3)
 .Q
 S BDMJ=5
 Q BDMJ
 ;;
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X S X=$$BPS^BDMD313(P,BDATE,EDATE,"I")
 I X="" Q ""
...
DIAMEAN(P,BDATE,EDATE) ;EP
 ...
 Q C\3
PPDDATE(P,EDATE) ;EP
 NEW X S X=$$LASTNP^BDMD318(P,EDATE)
 Q X
LIPID(P,BDATE,EDATE) ;EP
 ...
 I 'T Q "None"
 S X=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMD317(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G,$P(G,U,2)'="N" Q "Refused"
 I G Q "None - Not Medically Indicated"
 NEW T S T=$O(^ATXAX("B","DM AUDIT STATIN DRUGS",0))
 I 'T Q "None"
 S (X,G)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMD317(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G,$P(G,U,2)'="N" Q "Refused"
 I G Q "None - Not Medically Indicated"
 ;
 Q "None"
...
ACE(P,BDATE,EDATE) ;EP
 ...
 I 'T Q "No"
 S (G,X)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMD317(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G,$P(G,U,2)'="N" Q "Refused"
...
SELF(P,BDATE,EDATE) ;EP
 ...
 S E=$$LASTHF^BDMD319(P,"DIABETES SELF MONITORING",BD,ED,"F")
...
SDM(P,BDATE,EDATE) ;EP
 ...
 S X=EDATE,%DT="P" D ^%DT S ED=Y
 S X=BDATE,%DT="P" D ^%DT S BD=Y
 S E=$$LASTHF^BDMD319(P,"STAGED DIABETES MANAGEMENT",BD,ED)
 I E Q "Yes"
...
THERAPY(P,BD,EDATE) ;EP - therapy code for epi
 ...
 S STR="",TNAME=""
 S X=$$INSULIN^BDMD312(P,BD,EDATE)
 I X]"" S STR=STR_"2"
 S X=$$SULF^BDMD312(P,BD,EDATE)
 I X]"" S STR=STR_3
 S X=$$MET^BDMD312(P,BD,EDATE)
 I X]"" S STR=STR_4
 S X=$$ACAR^BDMD312(P,BD,EDATE)
 I X]"" S STR=STR_5
 S X=$$TROG^BDMD312(P,BD,EDATE)
 I X]"" S STR=STR_"6"
 I STR]"" Q STR
 Q 1
 ;
