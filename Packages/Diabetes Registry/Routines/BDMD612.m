BDMD612 ; IHS/CMI/LAB - 2006 DIABETES AUDIT ;
...
EKG(P,EDATE,F) ;EP
 ...
 K BDM I T S BDM(1)=$$CPT^BDMD612(P,$$DOB^AUPNPAT(P),ED,T,3) D
 .I BDM(1)="" K BDM Q
 .Q:LEKG>$P(BDM(1),U)
 .S LEKG=$P(BDM(1),U)
 K BDM I T S BDM(1)=$$RAD^BDMD612(P,$$DOB^AUPNPAT(P),ED,T,3) D
 .I BDM(1)="" K BDM Q
 .Q:LEKG>$P(BDM(1),U)
 .S LEKG=$P(BDM(1),U)
...
DEPSCR(P,BDATE,EDATE) ;EP
 ...
 I $D(BDM(1)) Q "Yes V79.0"_" "_$$DATE^BDMD610($P(BDM(1),U))
 ;check patient education
 S Y="BDM("
 S X=P_"^LAST EXAM 36;DURING "_BDATE_"-"_EDATE S E=$$START1^APCLDF(X,Y)
 I $D(BDM(1)) Q "Yes Exam 36-Dep Screen "_$$DATE^BDMD610($P(BDM(1),U))
 S Y="BDM("
 ...
 .I $P(T,"-",1)="DEP"!($P(T,"-",1)="BH")!($P(T,"-",1)="GAD")!($P(T,"-",1)="SB")!($P(T,"-",1)="PDEP") S D="Yes pt ed "_T_" "_$$DATE^BDMD610($P(BDM(X),U))
 K BDM
 I $P(D,U)]"" Q D
BHSCR ;
 ...
 .I $P($G(^AMHREC(V,14)),U,5)]"",$P(^AMHREC(V,14),U,5)'="UAS",$P(^AMHREC(V,14),U,5)'="REF" S BDMC="Yes BH Exam 36 "_$$DATE^BDMD610(9999999-D) Q
 .S X=0 F  S X=$O(^AMHRPRO("AD",V,X)) Q:X'=+X!(BDMC]"")  S BDMP=$P($G(^AMHRPRO(X,0)),U) D
 ..Q:'BDMP
 ..S BDMP=$P($G(^AMHPROB(BDMP,0)),U)
 ..I BDMP=14.1 S BDMC="Yes BH 14.1 "_$$DATE^BDMD610(9999999-D) Q
 ..I '$D(^AMHREDU("AD",V)) Q
 ...
 ...Q:'$D(^AUTTEDT(T,0))
 ...S T=$P(^AUTTEDT(T,0),U,2)
 ...I $P(T,"-",1)="DEP"!($P(T,"-",1)="BH")!($P(T,"-",1)="GAD")!($P(T,"-",1)="SB")!($P(T,"-",1)="PDEP") S BDMC="Yes BH pt ed "_T_" "_$$DATE^BDMD610(9999999-D)
 ...Q
 I BDMC]"" Q BDMC
 ;refusal
 NEW G S G=$$REFUSAL^BDMD617(P,9999999.15,$O(^AUTTEXAM("B","DEPRESSION SCREENING",0)),BDATE,EDATE)
 I G Q "Refused"
 S D=0,BDMC="",E=9999999-BDATE,D=9999999-EDATE-1_".99" F  S D=$O(^AMHREC("AE",P,D)) Q:D'=+D!($P(D,".")>E)!(BDMC]"")  S V=0 F  S V=$O(^AMHREC("AE",P,D,V)) Q:V'=+V!(BDMC]"")  D
 .I $P($G(^AMHREC(V,14)),U,5)]"",($P(^AMHREC(V,14),U,5)="UAS"!($P(^AMHREC(V,14),U,5)'="REF")) S BDMC="Refused BH Exam 36 "_$$DATE^BDMD610(9999999-D) Q
 I BDMC]"" Q BDMC
 Q "No"
LOINC(A,B) ;
 NEW %
