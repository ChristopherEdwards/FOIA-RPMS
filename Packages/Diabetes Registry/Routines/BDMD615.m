BDMD615 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ;
...
CUML ;EP
 ...
 ;
PROCESS ;
 S BDMPD=0 F  S BDMPD=$O(^XTMP("BDMDM61",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD  D CUML1
 Q
 ;
CUML1 ;
GENDER ;
 ...
 S V=$G(^XTMP("BDMDM61",BDMJOB,BDMBTH,"AUDIT",BDMPD,20))
...
TYPE ;
 ...
 S X=$$TYPE^BDMD616(BDMPD,BDMDMRG,BDMADAT)
...
DURDMC ;
 ...
 S V=$$DURDM^BDMD614(BDMPD,BDMDMRG,BDMADAT)
...
BMI ;
 ...
 S V=$G(^XTMP("BDMDM61",BDMJOB,BDMBTH,"AUDIT",BDMPD,112))
 S $P(BDMCUML(40),U,2)=$P(BDMCUML(40),U,2)+1
 D
 .I V="" S $P(BDMCUML(40),U,5)=$P(BDMCUML(40),U,5)+1 Q
 .I $$OW^BDMD614(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,3)=$P(BDMCUML(40),U,3)+1 Q
 .I $$OB^BDMD614(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,4)=$P(BDMCUML(40),U,4)+1 Q
 .S $P(BDMCUML(40),U,6)=$P(BDMCUML(40),U,6)+1 Q
HGB ;
 ;use last hgba1c value only
 ;BDMCUML(50)=
 S $P(BDMCUML(50),U,2)=$P(BDMCUML(50),U,2)+1
 S V=$P($G(^XTMP("BDMDM61",BDMJOB,BDMBTH,"AUDIT",BDMPD,78)),U)
 ;S V=$$STV^BDMD618(V,4)
...
TBSTAT ;
 ...
 S V=$$TBCODE^BDMD616(BDMPD,BDMRED,BDMDMRG)
 S $P(BDMCUML(70),U,(V+2))=$P(BDMCUML(70),U,(V+2))+1
TOBACCO ;
 S $P(BDMCUML(80),U,2)=$P(BDMCUML(80),U,2)+1
 S V=$G(^XTMP("BDMDM61",BDMJOB,BDMBTH,"AUDIT",BDMPD,27))
 S V1=$G(^XTMP("BDMDM61",BDMJOB,BDMBTH,"AUDIT",BDMPD,28))
...
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^BDMD616(BDMPD,BDM6MBD,BDMRED)
...
ASPIRIN ;
 ...
 S V=$G(^XTMP("BDMDM61",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
...
ACE ;110 title^total pts^total pts with protein^# of those on ace^# with htn^# of those on ace"
 ...
 S P=$G(^XTMP("BDMDM61",BDMJOB,BDMBTH,"AUDIT",BDMPD,94))
 S H=$G(^XTMP("BDMDM61",BDMJOB,BDMBTH,"AUDIT",BDMPD,34))
 S A=$G(^XTMP("BDMDM61",BDMJOB,BDMBTH,"AUDIT",BDMPD,60))
...
LIPID ;115
 S V=$G(^XTMP("BDMDM61",BDMJOB,BDMBTH,"AUDIT",BDMPD,61))
 S L=$$LDL^BDMD618(BDMPD,BDMBDAT,BDMADAT,"I"),L=$P(L,U)
 S T=$$CHOL^BDMD618(BDMPD,BDMBDAT,BDMADAT,"I"),T=$P(T,U)
...
DEP ;
 ...
 S V=$G(^XTMP("BDMDM61",BDMJOB,BDMBTH,"AUDIT",BDMPD,200))
 I $E(V)="Y" S $P(BDMCUML(300),U,3)=$P(BDMCUML(300),U,3)+1
 I $E(V)'="Y" S $P(BDMCUML(301),U,2)=$P(BDMCUML(301),U,2)+1 D
 .S V=$G(^XTMP("BDMDM61",BDMJOB,BDMBTH,"AUDIT",BDMPD,210))
...
EXAMS ;
 ...
 S V=$G(^XTMP("BDMDM61",BDMJOB,BDMBTH,"AUDIT",BDMPD,108))
