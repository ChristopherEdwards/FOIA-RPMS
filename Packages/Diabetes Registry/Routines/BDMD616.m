BDMD616 ; IHS/CMI/LAB - 2006 DIABETES AUDIT ;
...
TBCODE(P,EDATE,R) ;EP
 ...
 I X]"" Q 2
 S BDMR=$$PPD^BDMD618(P,EDATE)
 I BDMR["POS" D  Q BDMJ
 .I $$TBTX^BDMD612(P)["TX COMPLETE" S BDMJ=1 Q
 .S BDMJ=2
 .Q
 I BDMR["NEG" S BDMJ=4 D  Q BDMJ
 .I $$DODX(P,R,"I")="" S BDMJ=6 Q
 .S D=$$DODX(P,R,"I"),E=$$PPD^BDMD618(P,EDATE,"I") S BDMJ=$S(D>E:4,1:3)
 .Q
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X S X=$$BPS^BDMD613(P,BDATE,EDATE,"I")
 I X="" Q ""
...
DIAMEAN(P,BDATE,EDATE) ;EP
 ...
 Q C\3
PPDDATE(P,EDATE) ;EP
 NEW X S X=$$LASTNP^BDMD618(P,EDATE)
 Q X
LIPID(P,BDATE,EDATE) ;EP
 ...
 I 'T Q "None"
 S X=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMD617(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G Q "Refused or Adverse Rxn"
 NEW T S T=$O(^ATXAX("B","DM AUDIT STATIN DRUGS",0))
 I 'T G LIPIDA
 S (X,G)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMD617(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G Q "Refused or Adverse Rxn"
LIPIDA ;
...
ACE(P,BDATE,EDATE) ;EP
 ...
 I 'T Q "No"
 S (G,X)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMD617(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
...
SELF(P,BDATE,EDATE) ;EP
 ...
 S E=$$LASTHF^BDMD619(P,"DIABETES SELF MONITORING",BD,ED,"F")
...
SDM(P,BDATE,EDATE) ;EP
 ...
 S X=EDATE,%DT="P" D ^%DT S ED=Y
 S X=BDATE,%DT="P" D ^%DT S BD=Y
 S E=$$LASTHF^BDMD619(P,"STAGED DIABETES MANAGEMENT",BD,ED)
 I E Q "Yes"
...
THERAPY(P,BD,EDATE) ;EP - therapy code for epi
 ...
 S STR="",TNAME=""
 S X=$$INSULIN^BDMD612(P,BD,EDATE)
 I X]"" S STR=STR_"2"
 S X=$$SULF^BDMD612(P,BD,EDATE)
 I X]"" S STR=STR_3
 S X=$$MET^BDMD612(P,BD,EDATE)
 I X]"" S STR=STR_4
 S X=$$ACAR^BDMD612(P,BD,EDATE)
 I X]"" S STR=STR_5
 S X=$$TROG^BDMD612(P,BD,EDATE)
 I X]"" S STR=STR_"6"
 I STR]"" Q STR
 S X=$$REFMED^BDMD612(P,BD,EDATE)
 I X Q 9
 Q 1
 ;
...
DODX(P,R,F) ;EP - date of dx for epi file
 ...
 I $G(R) S DATE=$$CMSFDX^BDMD613(P,R,"ID")
 I DATE S EARLY=DATE
 S DATE=$$PLDMDOO^BDMD613(P,"I")
