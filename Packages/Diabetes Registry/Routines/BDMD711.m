BDMD711 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ;
...
URIN ;
 ...
 S V=$G(^XTMP("BDMDM71",BDMJOB,BDMBTH,"AUDIT",BDMPD,92))
 S A=$G(^XTMP("BDMDM71",BDMJOB,BDMBTH,"AUDIT",BDMPD,94))
 ...
 .S V=$G(^XTMP("BDMDM71",BDMJOB,BDMBTH,"AUDIT",BDMPD,96))
...
EKG ;need date of last ekg
 ...
 S V=$$EKG^BDMD712(BDMPD,BDMRED,"I")
 I V]"" D
...
CREAT ;
 ...
 S V=$$CREAT^BDMD718(BDMPD,BDMBDAT,BDMADAT,"I")
...
SELF ;
 ...
 ;S V=$G(^XTMP("BDMDM71",BDMJOB,BDMBTH,"AUDIT",BDMPD,98))
...
SDM  ;
 ...
 ;S V=$G(^XTMP("BDMDM71",BDMJOB,BDMBTH,"AUDIT",BDMPD,100))
 ;S P=$S($E(V)="Y":3,1:4)
...
CESS(P,B,D) ;EP - find any cessation hf in 12 months before E
 ...
 ;I 'T Q "No"
 S (X,G)=0
 I T F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  I $$REFUSAL^BDMD717(P,9999999.09,$P(^ATXAX(T,21,X,0),U),B,D) S G=1
 I G Q "Refused"
 S X=$O(^AUTTEDT("C","TO-Q",0))
 I X,$$REFUSAL^BDMD717(P,9999999.09,X,B,D) Q "Refused"
