BDMD811 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ;
...
QUAN ;
 ...
 ;S:'$D(BDMCUML(146)) BDMCUML(146)="Total Urine Prot Not Known"
 S V=$G(^XTMP("BDMDM81",BDMJOB,BDMBTH,"AUDIT",BDMPD,91))
 ;S V=$$UACR^BDMD81C(BDMPD,BDMBDAT,BDMADAT) ;cmi/maw 12/17/2007 DM2008
 ;S A=$$UPCR^BDMD81C(BDMPD,BDMBDAT,BDMADAT) ;cmi/maw 12/17/2007 DM2008
 ;S O=$$QUAN^BDMD81C(BDMPD,BDMBDAT,BDMADAT) ;cmi/maw 12/17/2007 DM2008
 S A=$G(^XTMP("BDMDM81",BDMJOB,BDMBTH,"AUDIT",BDMPD,93))
 S O=$G(^XTMP("BDMDM81",BDMJOB,BDMBTH,"AUDIT",BDMPD,95))
 I $E(V)="X" D
 .;S $P(BDMCUML(146),U,2)=$P(BDMCUML(146),U,2)+1  ;1/9/08
 .S $P(BDMCUML(145),U,2)=$P(BDMCUML(145),U,2)+1  ;added for total 1/9/08
 .S $P(BDMCUML(145),U,3)=$P(BDMCUML(145),U,3)+1
 .;S ^MAW($J,"UACR")=^MAW($J,"UACR")+1
 .I $P(V,U,2)]"",$P(V,U,2)["<" S $P(BDMCUML(145),U,6)=$P(BDMCUML(145),U,6)+1 Q  ;albumin normal
 .I $P(V,U,2)]"",$P(V,U,2)["-" S $P(BDMCUML(145),U,7)=$P(BDMCUML(145),U,7)+1 Q  ;micro
 .I $P(V,U,2)]"",$P(V,U,2)[">" S $P(BDMCUML(145),U,8)=$P(BDMCUML(145),U,8)+1 Q  ;overt
 .I $P(V,U,2)]"" S $P(V,U,2)=$$STV^BDMD818($P(V,U,2),9) ;if stripping chars evals to null quit
 .Q:$P(V,U,2)=""  ;don't count if all chars were alpha
 .I $P(V,U,2)]"",$P(V,U,2)<30 S $P(BDMCUML(145),U,6)=$P(BDMCUML(145),U,6)+1  ;albumin normal
 .I $P(V,U,2)]"",$P(V,U,2)>30,$P(V,U,2)<299.9 S $P(BDMCUML(145),U,7)=$P(BDMCUML(145),U,7)+1  ;micro present
 .I $P(V,U,2)]"",$P(V,U,2)>299.9 S $P(BDMCUML(145),U,8)=$P(BDMCUML(145),U,8)+1  ;overt
 I $E(A)="X" D
 . ;S BDMOFLG=1,$P(BDMCUML(146),U,2)=$P(BDMCUML(146),U,2)+1  ;1/9/08
 . S $P(BDMCUML(145),U,2)=$P(BDMCUML(145),U,2)+1  ;added for total 1/9/08
 . Q:$E(V)="X"
 . ;S ^MAW($J,"UPCR")=^MAW($J,"UPCR")+1
 . S $P(BDMCUML(145),U,4)=$P(BDMCUML(145),U,4)+1
 I $E(O)="X" D
 . ;S BDMOFLG=1,$P(BDMCUML(146),U,2)=$P(BDMCUML(146),U,2)+1  ;1/9/08
 . S $P(BDMCUML(145),U,2)=$P(BDMCUML(145),U,2)+1  ;added for total 1/9/08
 . Q:$E(V)="X"
 ...
 . S $P(BDMCUML(145),U,5)=$P(BDMCUML(145),U,5)+1
 ;S OV=$G(^XTMP("BDMDM81",BDMJOB,BDMBTH,"AUDIT",BDMPD,94))
 ;I $E(OV)="Y" S $P(BDMCUML(145),U,8)=$P(BDMCUML(145),U,8)+1
 ;S Q=$G(^XTMP("BDMDM81",BDMJOB,BDMBTH,"AUDIT",BDMPD,96))
 ;I $E(Q,1,3)="Pos" S $P(BDMCUML(145),U,7)=$P(BDMCUML(145),U,7)+1
 K BDMOFLG  ;1/9/08
URIN ;cmi/maw 12/26/2007 no longer needed DM2008
 ...
 ;145
 ;'$D(BDMCUML(146)) BDMCUML(146)="URINALYSIS"
 ;S $P(BDMCUML(146),U,2)=$P(BDMCUML(146),U,2)+1
 ;S V=$G(^XTMP("BDMDM81",BDMJOB,BDMBTH,"AUDIT",BDMPD,92))
 ;S A=$G(^XTMP("BDMDM81",BDMJOB,BDMBTH,"AUDIT",BDMPD,94))
 ...
 ;.S V=$G(^XTMP("BDMDM81",BDMJOB,BDMBTH,"AUDIT",BDMPD,96))
...
EKG ;need date of last ekg
 ...
 S V=$$EKG^BDMD812(BDMPD,BDMRED,"I")
 I V]"" D
...
CREAT ;
 ...
 S V=$$CREAT^BDMD818(BDMPD,BDMBDAT,BDMADAT,"I")
 ...
 ;
GFR ;
 S:'$D(BDMCUML(175)) BDMCUML(175)="Estimated GFR documented in the past 12 months"
 S $P(BDMCUML(175),U,2)=$P(BDMCUML(175),U,2)+1
 ;S V=$$GFR^BDMD81C(BDMPD,BDMBDAT,BDMADAT)
 S V=$G(^XTMP("BDMDM81",BDMJOB,BDMBTH,"AUDIT",BDMPD,79))  ;maw 1/18/08
 I V="Yes" S $P(BDMCUML(175),U,5)=$P(BDMCUML(175),U,5)+1
 ;
TCHOL ;
 S:'$D(BDMCUML(180)) BDMCUML(180)="Total Cholesterol obtained in past 12 months"
 S $P(BDMCUML(180),U,2)=$P(BDMCUML(180),U,2)+1
 S V=$$CHOL^BDMD818(BDMPD,BDMBDAT,BDMADAT,"I")
...
LDL ;
 ...
 S V=$$LDL^BDMD818(BDMPD,BDMBDAT,BDMADAT,"I")
...
HDL ;
 ...
 S V=$$HDL^BDMD818(BDMPD,BDMBDAT,BDMADAT,"I")
...
TRIG ;
 ...
 S V=$$TRIG^BDMD818(BDMPD,BDMBDAT,BDMADAT,"I")
...
SELF ;
 ...
 ;S V=$G(^XTMP("BDMDM81",BDMJOB,BDMBTH,"AUDIT",BDMPD,98))
...
SDM  ;
 ...
 ;S V=$G(^XTMP("BDMDM81",BDMJOB,BDMBTH,"AUDIT",BDMPD,100))
 ;S P=$S($E(V)="Y":3,1:4)
...
PED(P,BDATE,EDATE) ;EP
 ...
 ..I $E(T,1,4)="TO-Q" S %="Yes-"_T_" "_$$FMTE^XLFDT($P(BDMALED(X),U),5) Q
 ..I T="TO-LA" S %="Yes-"_T_" "_$$FMTE^XLFDT($P(BDMALED(X),U),5) Q
 K ^TMP($J,"A")
...
CESS(P,B,D) ;EP - find any cessation hf in 12 months before E
 ...
 ;I 'T Q "No"
 S (X,G)=0
 I T F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  I $$REFUSAL^BDMD817(P,9999999.09,$P(^ATXAX(T,21,X,0),U),B,D) S G=1
 I G Q "Refused"
 S X=$O(^AUTTEDT("C","TO-Q",0))
 I X,$$REFUSAL^BDMD817(P,9999999.09,X,B,D) Q "Refused"
