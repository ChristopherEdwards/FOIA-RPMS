BDMD817 ; IHS/CMI/LAB - 2008 DIABETES AUDIT ;
...
DFE(P,BDATE,EDATE) ;EP
 ...
 NEW X,Y,R S (X,Y)=0 F  S X=$O(BDM(X)) Q:X'=+X!(Y)  S R=$$PRIMPROV^APCLV($P(BDM(X),U,5),"D") I (R=33!(R=25)!(R=84)),'$$DNKA($P(BDM(X),U,5)) S Y=1_U_$P(BDM(X),U)
 I Y Q "Yes - Podiatrist Visit "_$$FMTE^XLFDT($P(Y,U,2),5)
 S X=0,Y=0 F  S X=$O(BDM(X)) Q:X'=+X!(Y)  S C=$$CLINIC^APCLV($P(BDM(X),U,5),"C") I C=65!(C="B7"),'$$DNKA($P(BDM(X),U,5)) S Y=1_U_$P(BDM(X),U)
