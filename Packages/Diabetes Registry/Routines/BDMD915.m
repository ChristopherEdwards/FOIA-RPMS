BDMD915 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ; [ 02/23/2009 8:14 PM ]
...
CUML ;EP
 ...
 S BDMCUML(150)="EKG (Age 30 and above)"
 S BDMCUML(170)="Creatinine obtained in the past 12 months"
 S BDMCUML(175)="Estimated GFR documented during audit period"
 S BDMCUML(180)="Total Cholesterol obtained in past 12 months"
 S BDMCUML(190)="LDL Cholesterol obtained in the past 12 months"
 S BDMCUML(195)="HDL Cholesterol obtained in the past 12 months"
 S BDMCUML(200)="Triglycerides obtained in past 12 months"
 ;
PROCESS ;
 S BDMNOGO=0
 S BDMPD=0 F  S BDMPD=$O(^XTMP("BDMDM91",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD  D CUML1
 Q
 ;
CUML1 ;
GENDER ;
 I $$DODX^BDMD916(BDMPD,BDMDMRG,"I")>BDMADAT S BDMNOGO=BDMNOGO+1 Q
 ;gender BDMCUML(10)="Gender^total^females^males"
 S $P(BDMCUML(10),U,2)=$P(BDMCUML(10),U,2)+1
 S V=$G(^XTMP("BDMDM91",BDMJOB,BDMBTH,"AUDIT",BDMPD,20))
...
TYPE ;
 ...
 S X=$$TYPE^BDMD916(BDMPD,BDMDMRG,BDMADAT)
 S $P(BDMCUML(25),U,2)=$P(BDMCUML(25),U,2)+1
 S P=$S(X="":5,X=1:3,X=2:4,1:5)
 S $P(BDMCUML(25),U,P)=$P(BDMCUML(25),U,P)+1
DURDMC ;
 ;BDMCUML(30)="Duration of Diabetes^total^<10^10 or more^no date of dx on problem list or cms register"
 S $P(BDMCUML(30),U,2)=$P(BDMCUML(30),U,2)+1
 S V=$$DURDM^BDMD914(BDMPD,BDMDMRG,BDMADAT)
...
BMI ;
 ...
 S V=$G(^XTMP("BDMDM91",BDMJOB,BDMBTH,"AUDIT",BDMPD,112))
 S $P(BDMCUML(40),U,2)=$P(BDMCUML(40),U,2)+1
 D
 .I V="" S $P(BDMCUML(40),U,5)=$P(BDMCUML(40),U,5)+1 Q
 .I $$OW^BDMD914(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,3)=$P(BDMCUML(40),U,3)+1 Q
 .I $$OB^BDMD914(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,4)=$P(BDMCUML(40),U,4)+1 Q
 .S $P(BDMCUML(40),U,6)=$P(BDMCUML(40),U,6)+1 Q
HGB ;
 ;use last hgba1c value only
 ;BDMCUML(50)=
 S $P(BDMCUML(50),U,2)=$P(BDMCUML(50),U,2)+1
 S V=$P($G(^XTMP("BDMDM91",BDMJOB,BDMBTH,"AUDIT",BDMPD,78)),U,2)
 S P=""
 I V=""!(V="?") S P=9 G HGBS
 I V["<" S P=3
 I V[">" S P=8
...
TBSTAT ;
 ...
 S V=$$TBCODE^BDMD916(BDMPD,BDMRED,BDMDMRG)
 S $P(BDMCUML(70),U,(V+2))=$P(BDMCUML(70),U,(V+2))+1
TOBACCO ;
 S $P(BDMCUML(80),U,2)=$P(BDMCUML(80),U,2)+1
 S V=$G(^XTMP("BDMDM91",BDMJOB,BDMBTH,"AUDIT",BDMPD,27))
 S V1=$G(^XTMP("BDMDM91",BDMJOB,BDMBTH,"AUDIT",BDMPD,28))
...
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^BDMD916(BDMPD,BDM6MBD,BDMRED)
 I V=1 S $P(BDMCUML(90),U,3)=$P(BDMCUML(90),U,3)+1
 I V="R" S $P(BDMCUML(90),U,12)=$P(BDMCUML(90),U,12)+1
 N I
 F I=1:1:$L(V) D
 . S Q=$E(V,I,I)
 . S P=$S(Q=2:4,Q=3:5,Q=4:6,Q=5:7,Q=6:8,Q=7:9,Q=8:10,Q=9:11,1:"") S $P(BDMCUML(90),U,P)=$P(BDMCUML(90),U,P)+1  ;cmi/maw 4/2/2009 modified for incretin/dpp4 per gary
 I $L(V)>1 D
 .I V[2 S $P(BDMCUML(90),U,14)=$P(BDMCUML(90),U,14)+1 Q
 .S C=0
 .I V[3 S C=C+1
 .I V[4 S C=C+1
 .I V[5 S C=C+1
...
ACE ;110 title^total pts^total pts with protein^# of those on ace^# with htn^# of those on ace"
 ...
 S P=$G(^XTMP("BDMDM91",BDMJOB,BDMBTH,"AUDIT",BDMPD,92))  ;01/09/08
 S H=$E($G(^XTMP("BDMDM91",BDMJOB,BDMBTH,"AUDIT",BDMPD,34)))
 S A=$E($G(^XTMP("BDMDM91",BDMJOB,BDMBTH,"AUDIT",BDMPD,60)))
 S $P(BDMCUML(110),U,3)=$P(BDMCUML(110),U,3)+1  ;TOTAL ACE
 I $E(H)=1 S $P(BDMCUML(110),U,5)=$P(BDMCUML(110),U,5)+1 I A=1 S $P(BDMCUML(110),U,7)=$P(BDMCUML(110),U,7)+1  ;TOTAL HTN
 I $P(P,U,5)=1 S V=$P(P,U,2) D
 .I V[">" S $P(BDMCUML(110),U,4)=$P(BDMCUML(110),U,4)+1 D  Q
 ..I A=1 S $P(BDMCUML(110),U,8)=$P(BDMCUML(110),U,8)+1 Q
 .S V=$$STV^BDMD918(V,8)
 .I V>299.99 S $P(BDMCUML(110),U,4)=$P(BDMCUML(110),U,4)+1 I A=1 S $P(BDMCUML(110),U,8)=$P(BDMCUML(110),U,8)+1 Q
 I $P(P,U,5)=2 S V=$P(P,U,2) D
 .I V["-" Q  ;S $P(BDMCUML(110),U,4)=$P(BDMCUML(110),U,4)+1 Q  ;I A S $P(BDMCUML(110),U,8)=$P(BDMCUML(110),U,8)+1 Q
 .I V["300" S $P(BDMCUML(110),U,4)=$P(BDMCUML(110),U,4)+1 D  Q
 ..I A=1 S $P(BDMCUML(110),U,8)=$P(BDMCUML(110),U,8)+1 Q
 .I V[">" S $P(BDMCUML(110),U,4)=$P(BDMCUML(110),U,4)+1 D  Q
 ..I A=1 S $P(BDMCUML(110),U,8)=$P(BDMCUML(110),U,8)+1 Q
 .S V=$$STV^BDMD918(V,5,1)
 .I +V>299.99 S $P(BDMCUML(110),U,4)=$P(BDMCUML(110),U,4)+1 D  Q
 ..I A=1 S $P(BDMCUML(110),U,8)=$P(BDMCUML(110),U,8)+1 Q
 I $P(P,U,5)=5 S V=$P(P,U,2) D
 .S $P(BDMCUML(110),U,4)=$P(BDMCUML(110),U,4)+1 I A=1 S $P(BDMCUML(110),U,8)=$P(BDMCUML(110),U,8)+1 Q
LIPID ;115
 S V=$G(^XTMP("BDMDM91",BDMJOB,BDMBTH,"AUDIT",BDMPD,61))
 S L=$$LDL^BDMD918(BDMPD,BDMBDAT,BDMADAT,"I"),L=$P(L,U)
 S T=$$CHOL^BDMD918(BDMPD,BDMBDAT,BDMADAT,"I"),T=$P(T,U)
 S T=$$STV^BDMD918(T,5,1)
 S L=$$STV^BDMD918(L,5,1)
 ;I $E(V)=4!(V=5) G LIPID1  ;NOT ON LIPID AGENT
 I T]"",T>239.999999 S $P(BDMCUML(115),U,3)=$P(BDMCUML(115),U,3)+1 I "123"[$E(V) S $P(BDMCUML(115),U,4)=$P(BDMCUML(115),U,4)+1
 I L]"",L>100 S $P(BDMCUML(115),U,5)=$P(BDMCUML(115),U,5)+1 I "123"[$E(V) S $P(BDMCUML(115),U,6)=$P(BDMCUML(115),U,6)+1
LIPID1 ;lipid agents
 ;7 - all w/agent  8 - all with statin only 9 - all with non-statin 10 - both.fx
 I "123"[$E(V) S $P(BDMCUML(115),U,7)=$P(BDMCUML(115),U,7)+1
 I $E(V)="1" S $P(BDMCUML(115),U,8)=$P(BDMCUML(115),U,8)+1
 I $E(V)="2" S $P(BDMCUML(115),U,9)=$P(BDMCUML(115),U,9)+1
 I $E(V)="3" S $P(BDMCUML(115),U,10)=$P(BDMCUML(115),U,10)+1
 I $E(V)="5" S $P(BDMCUML(115),U,11)=$P(BDMCUML(115),U,11)+1
DEP ;
 S $P(BDMCUML(300),U,2)=$P(BDMCUML(300),U,2)+1
 S V=$G(^XTMP("BDMDM91",BDMJOB,BDMBTH,"AUDIT",BDMPD,200))
 I $E(V)="1" S $P(BDMCUML(300),U,3)=$P(BDMCUML(300),U,3)+1
 I $E(V)'="1" S $P(BDMCUML(301),U,2)=$P(BDMCUML(301),U,2)+1 D
 .S V=$G(^XTMP("BDMDM91",BDMJOB,BDMBTH,"AUDIT",BDMPD,210))
...
EXAMS ;
 ...
 S V=$G(^XTMP("BDMDM91",BDMJOB,BDMBTH,"AUDIT",BDMPD,108))
...
EDUC ;
 ...
 S G=0,V=$G(^XTMP("BDMDM91",BDMJOB,BDMBTH,"AUDIT",BDMPD,44))
