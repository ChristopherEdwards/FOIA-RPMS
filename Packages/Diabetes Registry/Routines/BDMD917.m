BDMD917 ; IHS/CMI/LAB - 2009 DIABETES AUDIT ;
...
REFUSAL(P,F,I,B,E) ;EP
 ...
 .S G=1_"^"_$P(^AUPNPREF(Y,0),U,7)_U_$$VAL^XBDIQ1(9000022,Y,.03)_U_$$VAL^XBDIQ1(9000022,Y,.04)_U_$$VAL^XBDIQ1(9000022,Y,.07)_U_$$VAL^XBDIQ1(9000022,Y,.01)
 Q G
 ;
EYE(P,BDATE,EDATE,F,R) ;EP
 ...
 N PROV,D,V,G
 S (D,V)=0,G=""
 F  S D=$O(BDMY(D)) Q:D'=+D!(G)  S V=0 F  S V=$O(BDMY(D,V)) Q:V'=+V!(G)  S PROV=$$PRIMPROV^APCLV(V,"D") I (PROV=24!(PROV=79)!(PROV="08")),'$$DNKA(V) S G=9999999-D
 I G]"" D
 .Q:$P(LASTI,U)>G
 .S LASTI=G_U_"1  Yes - Optometrist/Opthalmalogist Visit "_$$FMTE^XLFDT($P(G,U))
 S (D,V)=0,G=""
 F  S D=$O(BDMY(D)) Q:D'=+D!(G)  S V=0 F  S V=$O(BDMY(D,V)) Q:V'=+V!(G)  S PROV=$$CLINIC^APCLV(V,"C") I PROV=17!(PROV=18)!(PROV=64)!(PROV="A2"),'$$DNKA(V) S G=9999999-D
 I G]"" D
 .Q:$P(LASTI,U)>G
