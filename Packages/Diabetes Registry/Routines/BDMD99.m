...
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMDM99",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
DEMO ;
 ...
 I BDMDEMO=-1 G IF
 I BDMPREP'=2 S XBRP="^BDMD99P",XBRC="^BDMD990",XBRX="XIT^BDMD99",XBNS="BDM"
 I BDMPREP=2 S XBRP="",XBRC="^BDMD990",XBRX="XIT^BDMD99",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
P ;
 S BDMSTP=0 K ^XTMP("BDMDM99",BDMJOB,BDMBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMDM99",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMDM99",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("BDMDM99",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMDM99",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMDM99",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
WRITEF ;EP write flat file
 ...
 S (X,N)=0 F  S X=$O(^BDMRECD(10,13,X)) Q:X'=+X  S N=N+1,^TMP($J,"BDM EPI",N)=^BDMRECD(10,13,X,0)
