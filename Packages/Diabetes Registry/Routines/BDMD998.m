BDMD998 ; IHS/CMI/LAB - 1999 DIABETES AUDIT ;
...
ALT(P,BDATE,EDATE) ;EP
 ...
 I '$D(BDM(1)) Q ""
 Q $P(^AUPNVLAB(+$P(BDM(1),U,4),0),U,4)_"  "_$$FMTE^XLFDT($P(BDM(1),U),5)
...
CREAT(P,BDATE,EDATE,F) ;EP
 ...
 I '$D(BDM(1)) Q ""
 I F="I" Q $P(^AUPNVLAB(+$P(BDM(1),U,4),0),U,4)_"^"_$P(BDM(1),U)
 Q $P(^AUPNVLAB(+$P(BDM(1),U,4),0),U,4)_$S($P(^AUPNVLAB(+$P(BDM(1),U,4),0),U,4)]"":" mg/dl ",1:"")_$$FMTE^XLFDT($P(BDM(1),U),5)
...
URIN(P,BDATE,EDATE) ;EP
 ...
 I 'T Q ""
 S X=0 F  S X=$O(^ATXLAB(T,21,X)) Q:X'=+X!(G)  I $$REFUSAL^BDMD997(P,60,$P(^ATXLAB(T,21,X,0),U),BDATE,EDATE) S G=1
 Q $S(G:"Refused",1:"No")
PROTEIN(P,BDATE,EDATE) ;EP
 ...
 I '$D(BDM(1)) Q ""
 ;Q $P(^AUPNVLAB(+$P(BDM(1),U,4),0),U,4)
 NEW %,%1 S %=$P(^AUPNVLAB(+$P(BDM(1),U,4),0),U,4)
 S %1=$S(%="":"No result",%["+":"Yes ",+%>30:"Yes ",1:"No ")
 Q %1_%_" "_$$FMTE^XLFDT($P(BDM(1),U),"5")
...
HGBA1C(P,BDATE,EDATE) ;EP
 NEW BDM,X,%,E,R,V
 K BDM
 S %=P_"^LAST 2 LAB [DM AUDIT HGB A1C;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BDM(")
 I '$D(BDM(1)) Q ""
 K X S X(9999999-$P(BDM(1),U),1)=$P(^AUPNVLAB(+$P(BDM(1),U,4),0),U,4) S:$D(BDM(2)) X(9999999-$P(BDM(2),U),2)=$P(^AUPNVLAB(+$P(BDM(2),U,4),0),U,4)
 S %=0,R="" F  S %=$O(X(%)) Q:%=""  S V=0 F  S V=$O(X(%,V)) Q:V=""  S R=R_X(%,V)_"^"_$$FMTE^XLFDT(9999999-%)_"^"_(9999999-%)_"^"
 Q R
BS(P,BDATE,EDATE) ;EP
 NEW BDM,X,%,E,R,V
 K BDM
 S %=P_"^LAST 3 LAB [DM AUDIT GLUCOSE TESTS TAX;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BDM(")
 I '$D(BDM(1)) Q ""
 S (%,R)="" F  S %=$O(BDM(%)) Q:%=""  S R=R_$P(^AUPNVLAB(+$P(BDM(%),U,4),0),U,4)_$S($P(^AUPNVLAB(+$P(BDM(%),U,4),0),U,4)]"":" mg/dl ",1:"")_$$FMTE^XLFDT($P(BDM(%),U))_"^"
 Q R
EKG(P,EDATE,F) ;EP
 I $G(F)="" S F="E"
...
PPD(P,EDATE,F) ;EP
 ...
 .S X=EDATE,%DT="P" D ^%DT S BDATE=$$FMADD^XLFDT(Y,-365),BDATE=$$FMTE^XLFDT(BDATE),G=0 I $$REFUSAL^BDMD997(P,9999999.28,$O(^AUTTSK("B","PPD",0)),BDATE,EDATE) S G=1
