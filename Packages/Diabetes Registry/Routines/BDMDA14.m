BDMDA14 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ; 22 Feb 2013  3:43 PM
...
BSC ;
 ...
 W !?7,"Not tested or no valid result",?53,$$C($P(V,U,9)),?65,$$P($P(V,U,2),$P(V,U,9))
BPC ;
 I $Y>(BDMIOSL-9) D HEADER Q:BDMQUIT
...
TOB ;
 ...
 W !?9,"Counseled - Yes",?30,"(n=",$$C($P(V,U,4)),")",?43,$$P($P(V,U,3),$P(V,U,4))
 W !?9,"Counseled - No",?30,"(n=",$$C($P(V,U,5)),")",?43,$$P($P(V,U,3),$P(V,U,5))
 ;W !?9,"Counseled - Refused",?53,$$C($P(V,U,6)),?65,$$P($P(V,U,3),$P(V,U,6))
...
TX ;
 ...
 W !?2,"Diabetes meds currently used, alone or in combination"
 W !?7,"Insulin",?53,$$C($P(V,U,4)),?65,$$P($P(V,U,2),$P(V,U,4))
 W !?7,"Sulfonylurea (glyburide, glipizide, others)",?53,$$C($P(V,U,5)),?65,$$P($P(V,U,2),$P(V,U,5))
 W !?7,"Glinide (Prandin, Starlix)",?53,$$C($P(V,U,6)),?65,$$P($P(V,U,2),$P(V,U,6))
 W !?7,"Metformin (Glucophage, others)",?53,$$C($P(V,U,7)),?65,$$P($P(V,U,2),$P(V,U,7))
 W !?7,"Acarbose (Precose)/Miglitol (Glyset)",?53,$$C($P(V,U,8)),?65,$$P($P(V,U,2),$P(V,U,8))
 W !?7,"Proglitizone (Actos) or rosiglitazone",!?10,"(Avandia)",?53,$$C($P(V,U,9)),?65,$$P($P(V,U,2),$P(V,U,9))
 W !?7,"GLP-1 med (Byetta, Bydureon, Victoza)",?53,$$C($P(V,U,10)),?65,$$P($P(V,U,2),$P(V,U,10))  ;cmi/maw 12/26/2007 DM2013
 W !?7,"DPP4 inhibitor (Januvia, Onglyza, Tradjenta)",?53,$$C($P(V,U,11)),?65,$$P($P(V,U,2),$P(V,U,11))
 W !?7,"Amylin analog (Symlin)",?53,$$C($P(V,U,12)),?65,$$P($P(V,U,2),$P(V,U,12))
 ;W !?7,"GLP-1 analog (Victoza)",?53,$$C($P(V,U,13)),?65,$$P($P(V,U,2),$P(V,U,13))
 W !?7,"Bromocriptine (Cycloset)",?53,$$C($P(V,U,14)),?65,$$P($P(V,U,2),$P(V,U,14))
 W !?7,"Colesevelam (Welchol)",?53,$$C($P(V,U,15)),?65,$$P($P(V,U,2),$P(V,U,15))
 W !!?2,"Number of diabetes meds currently used"
 W !?7,"One med",?53,$$C($P(V,U,17)),?65,$$P($P(V,U,2),$P(V,U,17))
 W !?7,"Two meds",?53,$$C($P(V,U,18)),?65,$$P($P(V,U,2),$P(V,U,18))
 W !?7,"Three meds",?53,$$C($P(V,U,19)),?65,$$P($P(V,U,2),$P(V,U,19))
 W !?7,"Four or more meds",?53,$$C($P(V,U,20)),?65,$$P($P(V,U,2),$P(V,U,20))
 D ^BDMDA1A
 Q
EXIT ;
...
HEADER1 ;
 ...
 W !,$$CTR("***  IHS/RPMS CARE & OUTCOMES AUDIT 2013  ***",80),!
 W $$CTR("FOR "_$P(^DIC(4,$S($G(BDMDUZ2):BDMDUZ2,1:DUZ(2)),0),U)),!
 S X="Reporting Period: "_$$FMTE^XLFDT(BDMBDAT)_" to "_$$FMTE^XLFDT(BDMADAT) W $$CTR(X,80),!
 W !,$TR($J("",80)," ","-")
 W !!,$P(BDMCUML(10),U,2)," patients were audited"
 W ?55," n",?63,"Percent"
 Q
