BDMDA1C ; IHS/CMI/LAB - 2013 DIABETES AUDIT ;
...
GATHER ;
 ...
 ...S R=$P(^AUPNVLAB(X,0),U,4),R=$$STV^BDMDA18(R,8,1) I R="" Q  ;no numeric result 
...
GFR(P,BDATE,EDATE) ;EP - Estimated GFR
 ...
 ..S X=0 F  S X=$O(^AUPNVLAB("AE",P,D,L,X)) Q:X'=+X  D
 ...Q:'$D(^AUPNVLAB(X,0))
 ...Q:$P(^AUPNVLAB(X,0),U,4)=""  ;dm2013 display value only
 ...Q:$$UP^XLFSTR($P(^AUPNVLAB(X,0),U,4))["CANC"
 ...Q:$$UP^XLFSTR($P(^AUPNVLAB(X,0),U,4))["COMMENT"
 ...I BDMLT,$P(^AUPNVLAB(X,0),U),$D(^ATXLAB(BDMLT,21,"B",$P(^AUPNVLAB(X,0),U))) D SETV Q
 ...Q:'BDMOT
...
LPROF ;
 K BDM,BDMX S BDMX=""
 Q  ;no longer count lipid profile because there is no result  2013 per Ray.
...
CALCNON ;
 NEW TC,HDL,V
 S TC=$P($$CHOL^BDMDA18(BDMPD,BDMBDAT,BDMADAT),U,1)
 S HDL=$P($$HDL^BDMDA18(BDMPD,BDMBDAT,BDMADAT),U,1)
 S TC=$$STV^BDMDA18(TC,5,1)
 S HDL=$$STV^BDMDA18(HDL,5,1)
...
NONHDLE(P,BDATE,EDATE) ;EP
 ...
 I V Q $P($P(V,"^")," ")
 Q ""
 ;S TC=$P($$CHOL^BDMDA18(BDMPD,BDMBDAT,BDMED,"I"),U) S X=$$STV^BDMDA18(TC,3) S TC=TC+.5,TC=$P(TC,".")
 ;I TC="" Q ""
 ;I TC'=+TC Q ""
 ;S HDL=$P($$HDL^BDMDA18(BDMPD,BDMBDAT,BDMED,"I"),U) S HDL=$$STV^BDMDA18(HDL,3) S HDL=HDL+.5,HDL=$P(HDL,".")
 ;I HDL="" Q ""
 ;I HDL'=+HDL Q ""
 ;Q TC-HDL
COMBINED(P) ;EP
 ...
 S G=0,ALL=""
 S A1C=$P(^XTMP("BDMDM13",BDMJOB,BDMBTH,"AUDIT",P,78),U,2)
 S V=A1C
 ...
 S G=G+S
CLDL ;
 S LDL=+^XTMP("BDMDM13",BDMJOB,BDMBTH,"AUDIT",P,88)
 I LDL,LDL<100 S G=G+1
 S ALL=ALL_"; LDL: "_$S(LDL:LDL,1:"<Not Documented>")
BP ;
 S S=$$SYSMEAN^BDMDA15(P,BDMRBD,BDMRED)
 S D=$$DIAMEAN^BDMDA15(P,BDMRBD,BDMRED)
 D
 .I S=""!(D="") Q
...
COMBINEE(P) ;
 NEW A1C,LDL,BP,ALL,G,V,S,D
 S G=0,ALL=""
 S A1C=$P($$HGBA1C^BDMDA18(BDMPD,BDMBDAT,BDMED),U,2)  ;,X=$$STV^BDMDA18(X,4)
 S V=A1C
 I V=""!(V="?") S G=0 G ELDL
 I V["<" S G=G+1 G ELDL
 I V[">" S G=G+0 G ELDL
 ...
 S G=G+S
ELDL ;
 S LDL=$P($$LDL^BDMDA18(BDMPD,BDMBDAT,BDMED,"I"),U) S LDL=$$STV^BDMDA18(LDL,3) S LDL=LDL+.5,LDL=$P(LDL,".")
 I LDL,LDL<100 S G=G+1
BPE ;
 S S=$$SYSMEAN^BDMDA15(P,BDMRBD,BDMRED)
 S D=$$DIAMEAN^BDMDA15(P,BDMRBD,BDMRED)
