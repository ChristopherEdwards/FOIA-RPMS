BDMDA1P ; IHS/CMI/LAB - 2013 DIABETES AUDIT PRINT ;
 ...
 S BDMPD=0,BDMGUIC=0 F  S BDMPD=$O(^XTMP("BDMDM13",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD!(BDMQUIT)  D
 .I $G(BDMGUI),BDMGUIC W !,"ZZZZZZZ",!  ;maw
 .S BDMGUIC=1
 .W:$D(IOF) @IOF
 .S BDMPG=BDMPG+1 W:$G(BDMGUI) !! W $$CTR("ASSESSMENT OF DIABETES CARE, 2013       DATE AUDIT RUN: "_$$FMTE^XLFDT(DT)_"  Page: "_BDMPG,80)
 ...
 .W !!,"Local Option question:  ",$$LOCN^BDMDA10(BDMPD,BDMDMRG)
 .W !,"Extended Local Option question:  ",$$LOCT^BDMDA10(BDMPD,BDMDMRG)
 .W !!!,?10,"*UACR is the preferred test.",!?10,"See Audit 2013 Instructions for more information."
 .I $E(IOST,1,2)'="P-" W !! S DIR(0)="E" D ^DIR K DIR ;CMI/GRL 1/18/07
CUML ;
 I BDMPREP=4!(BDMPREP=3) D CUML^BDMDA14
DONE ;
 K ^TMP($J)
 K ^XTMP("BDMDM13",BDMJOB,BDMBTH),BDMJOB,BDMBTH
 Q
I(I) ;
 Q $G(^XTMP("BDMDM13",BDMJOB,BDMBTH,"AUDIT",BDMPD,I))
 ;
PAGE ;
 Q:$G(BDMDSP)
 ...
 I $G(BDMGUI) W !!
 S BDMPG=BDMPG+1
 W $$CTR("ASSESSMENT OF DIABETES CARE, 2013         DATE AUDIT RUN: "_$$FMTE^XLFDT(DT)_"  Page: "_BDMPG,80)
