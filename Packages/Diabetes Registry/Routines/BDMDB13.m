...
LASTFLU(BDMPDFN,BDMBD,BDMED,BDMFORM) ;PEP - date of last FLU
 ...
 I T S X=0 F  S X=$O(^ATXAX(T,21,"B",X)) Q:X=""  S S(X)=""
 F X=15,16,88,111,123,135,140,141,144,149,150,151,153,155,158 S S(X)=""
...
FLU(P,BDATE,EDATE,R,F) ;EP
 ...
 .S G=$$REFUSAL^BDMDB17(P,9999999.14,$O(^AUTTIMM("C",A,0)),BDATE,EDATE)
...
PNEU(P,EDATE,R,F) ;EP
 ...
 I R Q ""
 S G=$$REFUSAL^BDMDB17(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:33,1:19),0)),$$FMTE^XLFDT($$FMADD^XLFDT(EDATE,-365)),EDATE)
 I G,$P(G,U,2)'="N" Q "3  Refused "_$$FMTE^XLFDT($P(G,U,3))
 I G Q "2  No - Not Medically Indicated "_$$FMTE^XLFDT($P(G,U,3))
 I '$$BI Q "2  No"
 S G=$$REFUSAL^BDMDB17(P,9999999.14,$O(^AUTTIMM("C",100,0)),$$FMTE^XLFDT($$FMADD^XLFDT(EDATE,-365)),EDATE)
 I G,$P(G,U,2)'="N" Q "3  Refused "_$$FMTE^XLFDT($P(G,U,3))
 I G Q "2  No - Not Medically Indicated "_$$FMTE^XLFDT($P(G,U,3))
 S G=$$REFUSAL^BDMDB17(P,9999999.14,$O(^AUTTIMM("C",109,0)),$$FMTE^XLFDT($$FMADD^XLFDT(EDATE,-365)),EDATE)
 I G,$P(G,U,2)'="N" Q "3  Refused "_$$FMTE^XLFDT($P(G,U,3))
 I G Q "2  No - Not Medically Indicated "_$$FMTE^XLFDT($P(G,U,3))
...
INCHES ;
 NEW F,FI,Z
 S (X,Z)=$$LASTHT^BDMDB13(BDMPD,BDMRED,"I")
 Q:X=""
 S X=X/12  ;get feet
 S F=$P(X,".")
...
I I BDMHEP=2,BDM10743 Q "1  Yes"  ;2 Hep B + 90743"
 ...
 S R=$$CPTREFT^BDMDBDU(P,$$DOB^AUPNPAT(P),EDATE,$O(^ATXAX("B","BGP HEPATITIS CPTS",0)))
