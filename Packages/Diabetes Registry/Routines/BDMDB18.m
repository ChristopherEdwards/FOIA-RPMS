BDMDB18 ; IHS/CMI/LAB - 2014 DIABETES AUDIT ;
...
PPDSPL ;CHECK PL
 ...
 N X,Y,I S (X,Y,I)=0 F  S X=$O(^AUPNPROB("AC",P,X)) Q:X'=+X!(I)  I $D(^AUPNPROB(X,0)) S Y=$P(^AUPNPROB(X,0),U) I $$ICD^BDMUTL(Y,$P(^ATXAX(T,0),U),9) S I=X  ;cmi/maw 05/15/2014 p8
...
AS(X) ;EP
 ...
 Q X
 ;
CALCGFR(P,BDATE,EDATE) ;
 NEW R,C,X,Y,Z,D
 S R=$$CREAT^BDMDB18(P,BDATE,EDATE,"I"),D=$P(R,U,2),R=$P(R,U,1)
 I R="" Q ""
 ;CALCULATE IT
