BDMDB1C ; IHS/CMI/LAB - 2014 DIABETES AUDIT ;
...
GATHER ;
 ...
 ...S R=$P(^AUPNVLAB(X,0),U,4),R=$$STV^BDMDB18(R,8,1) I R="" Q  ;no numeric result 
...
GFR(P,BDATE,EDATE,NOCALC) ;EP - Estimated GFR
 ...
 ...Q
 I '$D(BDM(1)),NOCALC Q ""
 I '$D(BDM(1)),'NOCALC S X=$$CALCGFR^BDMDB18(P,BDATE,EDATE) D  Q X
 .I X="" Q
...
NONHDL(P,BDATE,EDATE,F) ;EP
 ...
 K BDM
 S BDMC=0
 S BDMOT=$O(^ATXAX("B","BGP NON-HDL LOINC CODES",0))
 S BDMLT=$O(^ATXLAB("B","DM AUDIT NON-HDL TESTS",0))
 D GATHER
 I '$D(BDM(1)) S BDMX="" D CALCNON S:BDMX]"" BDMX=BDMX_" mg/dl^^Calculated Value" Q BDMX
 D SETN
 I F="I" Q $P(^AUPNVLAB(+$P(BDM(N),U,4),0),U,4)_"^"_$P(BDM(N),U)
 Q $P(^AUPNVLAB(+$P(BDM(N),U,4),0),U,4)_$S($P(^AUPNVLAB(+$P(BDM(N),U,4),0),U,4)]"":" mg/dl ",1:"")_U_$$FMTE^XLFDT($P(BDM(N),U))_U_$E($P(BDM(N),U,3),1,25)
CALCNON ;
 NEW TC,HDL,V
 S TC=$P($$CHOL^BDMDB18(BDMPD,BDMBDAT,BDMADAT),U,1)
 S HDL=$P($$HDL^BDMDB18(BDMPD,BDMBDAT,BDMADAT),U,1)
 S TC=$$STV^BDMDB18(TC,5,1)
 S HDL=$$STV^BDMDB18(HDL,5,1)
...
NONHDLE(P,BDATE,EDATE) ;EP
 ...
 I V Q $P($P(V,"^")," ")
 Q ""
 ;S TC=$P($$CHOL^BDMDB18(BDMPD,BDMBDAT,BDMED,"I"),U) S X=$$STV^BDMDB18(TC,3) S TC=TC+.5,TC=$P(TC,".")
 ;I TC="" Q ""
 ;I TC'=+TC Q ""
 ;S HDL=$P($$HDL^BDMDB18(BDMPD,BDMBDAT,BDMED,"I"),U) S HDL=$$STV^BDMDB18(HDL,3) S HDL=HDL+.5,HDL=$P(HDL,".")
 ;I HDL="" Q ""
 ;I HDL'=+HDL Q ""
 ;Q TC-HDL
COMBINED(P) ;EP
 ...
 S G=0,ALL=""
 S A1C=$P(^XTMP("BDMDM14",BDMJOB,BDMBTH,"AUDIT",P,78),U,2)
 S V=A1C
 ...
 S G=G+S
CLDL ;
 S LDL=+^XTMP("BDMDM14",BDMJOB,BDMBTH,"AUDIT",P,88)
 I LDL,LDL<100 S G=G+1
 S ALL=ALL_"; LDL: "_$S(LDL:LDL,1:"<Not Documented>")
BP ;
 S S=$$SYSMEAN^BDMDB15(P,BDMRBD,BDMRED)
 S D=$$DIAMEAN^BDMDB15(P,BDMRBD,BDMRED)
 D
 .I S=""!(D="") Q
...
COMBINEE(P) ;
 NEW A1C,LDL,BP,ALL,G,V,S,D
 S G=0,ALL=""
 S A1C=$P($$HGBA1C^BDMDB18(BDMPD,BDMBDAT,BDMED),U,2)  ;,X=$$STV^BDMDB18(X,4)
 S V=A1C
 I V=""!(V="?") S G=0 G ELDL
 I V["<" S G=G+1 G ELDL
 I V[">" S G=G+0 G ELDL
 ...
 S G=G+S
ELDL ;
 S LDL=$P($$LDL^BDMDB18(BDMPD,BDMBDAT,BDMED,"I"),U) S LDL=$$STV^BDMDB18(LDL,3) S LDL=LDL+.5,LDL=$P(LDL,".")
 I LDL,LDL<100 S G=G+1
BPE ;
 S S=$$SYSMEAN^BDMDB15(P,BDMRBD,BDMRED)
 S D=$$DIAMEAN^BDMDB15(P,BDMRBD,BDMRED)
...
EGFRUACR(P,BD,ED,F) ;EP - 
 ...
 S F=$G(F)
 NEW G,R
 S G=$$GFR^BDMDB1C(BDMPD,BD,ED)
 S R=$$URIN^BDMDB18(BDMPD,BD,ED)
