BDMDC11 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ; 10 Oct 2014  9:50 AM
...
IMM ;
 ...
 ;TDAP EVER
 S V=$G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,216))
...
QUAN ;
 ...
 S V=$E($G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)))  ;test done?
 ;S T=$P($G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)),U,5)  ;type of test
 ...
 S G=$P($G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,79)),U,2)  ;egfr value
 S H=0
 I $$AGE^AUPNPAT(BDMPD,BDMADAT)>17 D
 .S G=$$AS^BDMDC18(G)
 .S G=$$STV^BDMDC18(G,5,1)
 .I G="" Q
...
TCHOL ;
 ...
 S V=$$CHOL^BDMDC18(BDMPD,BDMBDAT,BDMADAT,"I")
...
NONHDL ;
 ...
 S V=$P($G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,117)),U,1)  ;non hdl
...
LDL ;
 ...
 S V=$P(V,U)
 ;S V=$$STV^BDMDC18(V,4)
 S V=$$STV^BDMDC18(V,5,1) I $E(V)'=+$E(V)!(+V=0) S $P(BDMCUML(190),U,8)=$P(BDMCUML(190),U,8)+1 G HDL
...
HDL ;
 ...
 S V=$$HDL^BDMDC18(BDMPD,BDMBDAT,BDMADAT,"I")
 S S=$P(^DPT(BDMPD,0),U,2)  ;GENDER
 I S="F" D
 .S $P(BDMCUML(195),U,2)=$P(BDMCUML(195),U,2)+1
 .I V="" S $P(BDMCUML(195),U,5)=$P(BDMCUML(195),U,5)+1 Q
 .S V=$P(V,U)
 .S V=$$STV^BDMDC18(V,5,1) I $E(V)'=+$E(V)!(+V=0) S $P(BDMCUML(195),U,5)=$P(BDMCUML(195),U,5)+1 Q
 .S V=$P(V,"."),V=$$STV^BDMDC18(V,5,1)
 ...
 .S V=$P(V,U)
 .S V=$$STV^BDMDC18(V,5,1) I $E(V)'=+$E(V)!(+V=0) S $P(BDMCUML(195),U,9)=$P(BDMCUML(195),U,9)+1 Q  ;unable to determine result, not a number
 .S V=$P(V,"."),V=$$STV^BDMDC18(V,5,1)
...
TRIG ;
 ...
 S V=$$TRIG^BDMDC18(BDMPD,BDMBDAT,BDMADAT,"I")
 I V="" S $P(BDMCUML(200),U,7)=$P(BDMCUML(200),U,7)+1 G SELF
 S V=$P(V,U)
 S V=$$STV^BDMDC18(V,5,1) I $E(V)'=+$E(V) S $P(BDMCUML(200),U,5)=$P(BDMCUML(200),U,5)+1 G SELF ;unable to determine result, not a number
...
CVD ;USE 250
 ...
 S V=$G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,116))
...
EGFRQUP ;
 ...
 S V=$G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,79)),V=$$STV^BDMDC18($P(V,U,2),5)
 S T=$P($G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)),U,5)  ;type of test
...
COMBINED ;USE 260
 ...
 S V=$G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,118))
...
CESS(P,BDATE,EDATE) ;EP - find any cessation hf in 12 months before E
 I '$G(P) Q ""
 I $P($$TOBACCO^BDMDC1T(P,$$DOB^AUPNPAT(P),EDATE),U,1)'=1 Q ""
 NEW BDM,E,X,G,T,O,D,H,C,Q,BDMLPED
 S BDMLPED=""
 K BDM
 ...
 .Q
 NEW BDMALLED,X,Y,%,T,G,A,B,E,Z,BDMMEDS1,Q
 K BDMALLED
 S Y="BDMALLED("
 ...
 S G=$$CPTI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("D1320")) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"CPT D1320"
 S G=$$TRANI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("D1320")) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"TRAN D1320"
 S G=$$CPTI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD(99406)) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"CPT 99406"
 S G=$$TRANI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD(99406)) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"TRAN 99406"
 S G=$$CPTI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD(99407)) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"CPT 99407"
 S G=$$TRANI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD(99407)) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"TRAN 99407"
 S G=$$CPTI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G0375")) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"CPT G0375"
 S G=$$CPTI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G0376")) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"CPT G0376"
 S G=$$CPTI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("4000F")) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"CPT 4000F"
 S G=$$TRANI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G0375")) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"TRAN G0375"
 S G=$$TRANI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G0376")) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"TRAN G0376"
 S G=$$TRANI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("4000F")) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"TRAN 4000F"
 S G=$$CPTI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("4001F")) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"CESSATION MED - CPT 4001F"
 S G=$$TRANI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("4001F")) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"TRAN 4001F"
 S G=$$CPTI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G8402")) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"CPT G8402"
 S G=$$TRANI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G8402")) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"TRAN G8402"
 S G=$$CPTI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G8453")) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"CPT G8453"
 S G=$$TRANI^BDMDCDU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G8453")) I G,$P(BDMLPED,U)<$P(G,U,2) S BDMLPED=$P(G,U,2)_U_"TRAN G8453"
 I BDMLPED]"" Q "1  Yes "_$$FMTE^XLFDT($P(BDMLPED,U,1))_" "_$P(BDMLPED,U,2)
 ;now check meds
 K BDMMEDS1
 D GETMEDS^BDMDCDU(P,BDATE,EDATE,,,,,.BDMMEDS1)
 ...
 I BDMLPED]"" Q "1  Yes "_$$FMTE^XLFDT($P(BDMLPED,U,1))_" "_$P(BDMLPED,U,2)
PEDREF ; REFUSALS REMOVED 2015 AUDIT
 Q "2  No"
