BDMDC15 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 03 Feb 2014 5:38 PM ; 
...
CUML ;EP
 ...
 S BDMCUML(65)="Tobacco Use Screening"
...
CUML1 ;
GENDER ;
 I $$DODX^BDMDC16(BDMPD,BDMDMRG,"I")>BDMADAT S BDMNOGO=BDMNOGO+1 Q
 ;gender BDMCUML(10)="Gender^total^females^males"
 S $P(BDMCUML(10),U,2)=$P(BDMCUML(10),U,2)+1
 S V=$G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,20))
...
TYPE ;
 ...
 S X=$$TYPE^BDMDC16(BDMPD,BDMDMRG,BDMADAT)
...
DURDMC ;
 ...
 S V=$$DURDM^BDMDC14(BDMPD,BDMDMRG,BDMADAT)
...
BMI ;
 ...
 ;S V=$G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,112))
 S H=$$LASTHT^BDMDC13(BDMPD,BDMRED,"I") S:H]"" H=$J(H,5,2)
 S W=+$$LASTWT^BDMDC13(BDMPD,BDMBDAT,BDMRED),W=$S(W=0:"",1:W),W=W+.5,W=$P(W,".")
 S V=$$BMIEPI^BDMDC19(H,W)
 S $P(BDMCUML(40),U,2)=$P(BDMCUML(40),U,2)+1
 D
 .I V="" S $P(BDMCUML(40),U,5)=$P(BDMCUML(40),U,5)+1 Q
 .I $$OW^BDMDC14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,3)=$P(BDMCUML(40),U,3)+1 Q
 .I $$OB^BDMDC14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,4)=$P(BDMCUML(40),U,4)+1 Q
...
TBSTAT ;
 ...
 S V=$G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,70))
 ...
 I $E($P(V,"||",2))=1,$E($G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,72)))=1 S $P(BDMCUML(70),U,7)=$P(BDMCUML(70),U,7)+1
 I $E($P(V,"||",2))=2 D
 .S $P(BDMCUML(70),U,9)=$P(BDMCUML(70),U,9)+1
 .S X=$G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,114))
 .I $$DODX^BDMDC16(BDMPD,BDMDMRG,"I")="" Q
 .S D=$$DODX^BDMDC16(BDMPD,BDMDMRG,"I"),E=X I D>E Q
...
TOBACCO ;
 ...
 S V1=$G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,28))
...
TOBSCR ;
 ...
 S V=$G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,215))
...
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^BDMDC16(BDMPD,BDM6MBD,BDMRED)  ;^ pieced with each item  or =1 for diet alone or =14 for refused
...
ASPIRIN ;
 ...
 I $E($G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,116)))'=1 G ACE  ;not cvd
 S $P(BDMCUML(100),U,2)=$P(BDMCUML(100),U,2)+1  ;total with cvd
 S V=$G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
...
ACE ;110 title^total pts^total pts with protein^# of those on ace^# with htn^# of those on ace"
 ...
 S P=$G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,92))  ;URINE PROTEIN VALUE
...
LIPIDAG ;lipid agents
 ...
 S V=$E($$STATIN^BDMDC16(BDMPD,BDM6M,BDMADAT))
 ...
 S C=0
 I $E($G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,116)))=1 D
...
EXAMS ;
 ...
 S V=$G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,108))
...
EDUC ;
 ...
 S G=0,V=$G(^XTMP("BDMDM15",BDMJOB,BDMBTH,"AUDIT",BDMPD,44))
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X,Z,C,Y
 S X=$$BPS^BDMDC13(P,BDATE,EDATE,"I")
 I X="" Q ""
