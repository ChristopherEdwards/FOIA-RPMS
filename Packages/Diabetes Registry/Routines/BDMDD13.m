BDMDD13 ; IHS/CMI/LAB - 2016 DIABETES AUDIT ;
...
FLU(P,BDATE,EDATE,R,F) ;EP
 ...
 .S G=$$REFUSAL^BDMDD17(P,9999999.14,$O(^AUTTIMM("C",A,0)),BDATE,EDATE)
...
HTNDX(P,EDATE) ;EP - is HTN on problem list
 ...
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2016,"HYPERTENSION DIAGNOSES",$P(^AUPNPROB(X,800),U,1)) S G=1
 .Q
 I G Q "1  Yes"
 K BDM
...
PLDMDXS(P) ;EP - get all DM dxs from problem list
 ...
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2016,"DIABETES DIAGNOSES",$P(^AUPNPROB(X,800),U,1)) S:D]"" D=D_";" S D=D_"SNOMED: "_$P(^AUPNPROB(X,800),U,1)
 .Q
 Q D
 ;
...
INCHES ;
 NEW F,FI,Z
 S (X,Z)=$$LASTHT^BDMDD13(BDMPD,BDMRED,"I")
 Q:X=""
 S X=X/12  ;get feet
 S F=$P(X,".")
...
I I BDMHEP=2,BDM10743 Q "1  Yes"  ;2 Hep B + 90743"
 ...
 I $$PLTAX^BDMDD12(P,"BGP HEP EVIDENCE") Q "4  Immune by DX"
 ;now go to Refusals
 ...
 S R=$$CPTREFT^BDMDDDU(P,$$DOB^AUPNPAT(P),EDATE,$O(^ATXAX("B","BGP HEPATITIS CPTS",0)))
