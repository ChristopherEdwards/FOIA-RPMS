BDMP512 ; IHS/CMI/LAB - 2003 DIABETES AUDIT ;
...
PAP(P,BDATE,EDATE) ; EP
 ...
 I T,$$REFUSAL^BDMP517(P,60,T,BDATE,EDATE) Q "Refused"
 S T=$O(^ATXLAB("B","BGP PAP SMEAR TAX",0))
 I 'T Q "No"
 S G=0
 S X=0 F  S X=$O(^ATXLAB(T,21,X)) Q:X'=+X!(G)  I $$REFUSAL^BDMP517(P,60,$P(^ATXLAB(T,21,X,0),U),BDATE,EDATE) S G=1
 Q $S(G:"Refused",1:"No")
...
EKG(P,EDATE,F) ;EP
 ...
 K BDM I T S BDM(1)=$$CPT^BDMP512(P,,ED,T,3) D
 .I BDM(1)="" K BDM Q
 .Q:LEKG>$P(BDM(1),U)
 .S LEKG=$P(BDM(1),U)
 K BDM I T S BDM(1)=$$RAD^BDMP512(P,,ED,T,3) D
 .I BDM(1)="" K BDM Q
 .Q:LEKG>$P(BDM(1),U)
 .S LEKG=$P(BDM(1),U)
