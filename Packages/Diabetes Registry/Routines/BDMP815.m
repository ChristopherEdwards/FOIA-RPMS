BDMP815 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ;
...
CUML ;EP
 ...
 ;
PROCESS ;
 S BDMPD=0 F  S BDMPD=$O(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD  D CUML1
 Q
 ;
CUML1 ;
GENDER ;
 ...
 S V=$G(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,20))
...
BMI ;
 ...
 S V=$G(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,112))
 S $P(BDMCUML(40),U,2)=$P(BDMCUML(40),U,2)+1
 D
 .I V="" S $P(BDMCUML(40),U,5)=$P(BDMCUML(40),U,5)+1 Q
 .I $$OW^BDMP814(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,3)=$P(BDMCUML(40),U,3)+1
 .I $$OB^BDMP814(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,4)=$P(BDMCUML(40),U,4)+1
...
TOBACCO ;
 ...
 S V=$G(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,27))
 S V1=$G(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,28))
...
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^BDMP816(BDMPD,BDM6MBD,BDMRED)
...
ASPIRIN ;
 ...
 S V=$G(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
...
ACE ;110 title^total pts^total pts with protein^# of those on ace^# with htn^# of those on ace"
 ...
 ;S P=$G(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,94))
 S H=$G(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,34))
 S A=$G(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,60))
...
LIPID ;115
 S V=$G(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,61))
