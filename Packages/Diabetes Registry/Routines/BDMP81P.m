BDMP81P ; IHS/CMI/LAB - 2003 DIABETES AUDIT PRINT ;
 ...
 S BDMPD=0,BDMGUIC=0 F  S BDMPD=$O(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD!(BDMQUIT)  D
 ...
 .W !?1,"1 IFG - ",$$I(200)
 .S X=200 F  S X=$O(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,X)) Q:X>200.99!(X="")  D
 ..W !?3,^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,X)
 .W !?1,"2 IGT - ",$$I(210)
 .S X=210 F  S X=$O(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,X)) Q:X>210.99!(X="")  D
 ..W !?3,^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,X)
 .W !?1,"3 METABOLIC SYNDROME - ",$$I(220)
 .S X=220 F  S X=$O(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,X)) Q:X>220.99!(X="")  D
 ..W !?3,^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,X)
 .W !?1,"OTHER ABNORMAL GLUCOSE (790.29) - ",$$I(230)
 .S X=230 F  S X=$O(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,X)) Q:X>230.99!(X="")  D
 ..W !?3,^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,X)
 .W !?1,"CMS Register DX: ",$$I(22)_" "_$$I(24)
 .I $D(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,25)) W !?1,^(25)
 .I $D(^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,26)) W !?1,^(26)
...
CUML ;
 I BDMPREP=2!(BDMPREP=3) D CUML^BDMP814
DONE ;
 K ^TMP($J)
 K ^XTMP("BDMP81",BDMJOB,BDMBTH),BDMJOB,BDMBTH
 Q
I(I) ;
 Q ^XTMP("BDMP81",BDMJOB,BDMBTH,"AUDIT",BDMPD,I)
 ;
PAGE ;
