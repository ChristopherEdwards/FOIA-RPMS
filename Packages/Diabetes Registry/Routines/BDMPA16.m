BDMPA16 ; IHS/CMI/LAB - 2003 DIABETES AUDIT ;
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X S X=$$BPS^BDMPA13(P,BDATE,EDATE,"I")
 I X="" Q ""
...
LIPID(P,BDATE,EDATE) ;EP
 ...
 I 'T Q "None"
 S X=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMPA17(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G,$P(G,U,2)'="N" Q "Refused"
 I G Q "None - Not Medically Indicated"
 NEW T S T=$O(^ATXAX("B","DM AUDIT STATIN DRUGS",0))
 I 'T Q "None"
 S (X,G)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMPA17(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G,$P(G,U,2)'="N" Q "Refused"
 I G Q "None - Not Medically Indicated"
 ;
 Q "None"
...
ACE(P,BDATE,EDATE) ;EP
 ...
 I 'T Q "No"
 S (G,X)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMPA17(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G,$P(G,U,2)'="N" Q "Refused"
...
SELF(P,BDATE,EDATE) ;EP
 ...
 S E=$$LASTHF^BDMPA19(P,"DIABETES SELF MONITORING",BD,ED,"F")
...
SDM(P,BDATE,EDATE) ;EP
 ...
 S X=EDATE,%DT="P" D ^%DT S ED=Y
 S X=BDATE,%DT="P" D ^%DT S BD=Y
 S E=$$LASTHF^BDMPA19(P,"STAGED DIABETES MANAGEMENT",BD,ED)
 I E Q "Yes"
...
TOBACCO1 ;check problem file for tobacco use
 ...
 . I $P($$ICDDX^BDMUTL($P(BDM(1),U,2)),U,2)=305.13 S BDMTOB="2  Not a Current User"_" - "_$E($P(^AUTNPOV($P(^AUPNPROB(+$P(BDM(1),U,4),0),U,5),0),U),1,30) Q  ;cmi/anch/maw 9/12/2007 csv
...
THERAPY(P,BD,EDATE) ;EP - therapy code for epi
 ...
 S STR="",TNAME=""
 S X=$$SULF^BDMPA12(P,BD,EDATE)
 I X]"" S STR=STR_5
 S X=$$MET^BDMPA12(P,BD,EDATE)
 I X]"" S STR=STR_2
 S X=$$ACAR^BDMPA12(P,BD,EDATE)
 I X]"" S STR=STR_3
 S X=$$TROG^BDMPA12(P,BD,EDATE)
 I X]"" S STR=STR_"4"
 I STR]"" Q STR
 Q 1
 ;
...
DODX(P,R,F) ;EP - date of dx for epi file
 ...
 I $G(R) S DATE=$$CMSFDX^BDMPA13(P,R,"ID")
 I DATE]"" S EARLY=DATE
 S DATE=$$PLDMDOO^BDMPA13(P,"I")
