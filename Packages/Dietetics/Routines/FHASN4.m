...
Q0	; Process Screening
	...
	G P0
F0	I WRDS,W1'=WRDS Q
	F DFN=0:0 S DFN=$O(^FHPT("AW",W1,DFN)) Q:DFN<1  S ADM=^(DFN) Q:ADM<1  S (NEW,OLD)=0 D Q1
	Q
Q1	; Process screening inpatients for status comparison
	S ADTE=$P($G(^FHPT(DFN,"A",ADM,0)),"^",1) Q:ADTE=""
	...
	S X4=$O(^FHPT(DFN,"S",X4)) G:'X4 Q2 S X5=^(X4,0)
...
Q3	; Tabulate status for old
	...
	S L1=$O(^FHPT(DFN,"S","B",L1)) G:L1=""!(L1\1>EDT) Q4
	S L1=9999999-L1,X6=$G(^FHPT(DFN,"S",L1,0)) G:X6="" Q4
	S X1=SDT\1-.0001,X1=X1\1,X2=3 D C^%DTC S THR=X
	I $P(X6,"^",1)>$S(THR:THR+.3,1:9999999) G:SDT\1=ADTE\1 Q4 S L1=$O(^FHPT(DFN,"S",L1)) G:L1="" Q4 S X6=$G(^(L1,0)) G:$P(X6,"^",1)<ADTE Q4
