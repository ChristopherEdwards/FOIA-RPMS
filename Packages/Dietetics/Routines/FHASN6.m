...
Q0	; Process Screening
	...
	F W1=0:0 S W1=$O(^FH(119.6,W1)) Q:W1'>0  D W1 F DFN=0:0 S DFN=$O(^FHPT("AW",W1,DFN)) Q:DFN<1  S ADM=^(DFN) Q:ADM<1  D Q1
	D P0
KIL	K ^TMP($J) G KILL^XUSCLEAN
Q1	; Tabulate status
	S DTE="",X5=$O(^FHPT(DFN,"S",0)) G:X5="" Q2 S X5=^(X5,0)
	I $P(X5,"^",1)<$S($D(^FHPT(DFN,"A",ADM,0)):$P(^(0),"^",1),1:9999999) G Q2
...
Q4	; Store Status
	I S1'=STS Q
	S X2=$G(^FHPT(DFN,"A",ADM,0)),RM=$P(X2,"^",9),RM=$P($G(^DG(405.4,+RM,0)),"^",1),X3=$G(^DPT(DFN,0)),PAT=$E($P(X3,"^",1),1,23) D PID^FHDPA
	D P1
	Q
P0	; Print summary
...
P1	I SRT="W" S NAM=$E($P($G(^FH(119.6,+XX,0)),"^",1),1,15)
	...
	Q:NAM=""  S:DTE="" DTE=$P(^FHPT(DFN,"A",ADM,0),"^",1) S ^TMP($J,NAM,DTE,PAT)=BID_"^"_$E(RM,1,10) Q
