...
Q0	; Process Screening all patients
	...
	F DFN=0:0 S DFN=$O(^FHPT(DFN)) Q:DFN<1  I $D(^(DFN,0)) K N S ND=0 D Q1,CALC
	G P0
Q1	; Tabulate status
	...
	I '$D(^FHPT(DFN,"S",0)) D Q2 Q
	D NS I '$D(^TMP($J,"FHNS")) D Q2 Q
	S NX="" F X4=0:0 S X4=$O(^TMP($J,"FHNS",X4)) Q:X4<1  S X5=$G(^(X4,0)),NX=X4 D CHK
	Q
CHK	; Check if inpat with ADM
	I $P(X5,"^",1)<$S($D(^FHPT(DFN,"A",ADM,0)):$P(^(0),"^",1),1:9999999) D GADM G:'$D(^FHPT(DFN,"A",ADM,0)) Q2 G:$P(X5,"^",1)<$S($D(^FHPT(DFN,"A",ADM,0)):$P(^(0),"^",1),1:9999999) Q2
	I DISC,$P(X5,"^",1)>DISC D GADM Q:'$D(^FHPT(DFN,"A",ADM,0))  Q:DISC&($P(X5,"^",1)>DISC)
...
GADM	; Get next ADM for pat
	...
	Q:'$D(^FHPT(DFN,"A",ADM,0))  Q:$P(X5,"^",1)<$S($D(^FHPT(DFN,"A",ADM,0)):$P(^(0),"^",1),1:9999999)
...
NS	; Nutrition Status in inverse date order
	...
	F XX=FHX1:0 S XX=$O(^FHPT(DFN,"S",XX)) Q:XX<1!(XX>FHX2)  S X=$G(^(XX,0)) D STOR
	I '$D(^TMP($J,"FHNS")) S XX=FHX1,FHX1=$O(^FHPT(DFN,"S",FHX1)) Q:FHX1=""  S X=$G(^(FHX1,0)) D STOR
	Q
