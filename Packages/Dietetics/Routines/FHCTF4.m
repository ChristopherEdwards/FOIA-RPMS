FHCTF4 ; HISC/REL/NCA - Check Ward Patients for a Clinician ;3/8/01  13:13
 ...
 Q
CHK D DAT F DFN=0:0 S DFN=$O(^FHPT("AW",WRD,DFN)) Q:DFN<1  S ADM=^(DFN) D PAT
 Q
CLN ; Clean up ticker file
 F K=0:0 S K=$O(^FH(119,FHDUZ,"I",K)) Q:K<1  S X=^(K,0) D C0
 Q
C0 S DFN=$P(X,"^",4),A0="" G:'DFN C1 S ADM=$P(X,"^",5) G:'ADM C1
 S W1=$P($G(^FHPT(DFN,"A",ADM,0)),"^",8) I W1="" G KIL
 S Y=$G(^FHPT("AW",W1,DFN)) I ADM'=Y G KIL
 S Y=$P($G(^FH(119.6,+W1,0)),"^",2)
 I Y'="",Y'=FHDUZ G KIL
 S A0=$G(^FHPT(DFN,"A",ADM,0))
C1 S TYP=$P(X,"^",2) G C2:TYP="C",C3:TYP="S",C4:TYP="D",C5:TYP="X",C6:TYP="T",C7:TYP="N" Q
C2 S FHDR=$P(X,"^",6),Y=^FHPT(DFN,"A",ADM,"DR",FHDR,0) I $P(Y,"^",8)'="A"!($P(Y,"^",5)'=FHDUZ) D KIL
 Q
C3 D:$P(X,"^",6)'=$P(A0,"^",7) KIL Q
C4 D:$P(X,"^",6)'=$P(A0,"^",2) KIL Q
C5 D:+X<NOW KIL Q
C6 D:$P(X,"^",6)'=$P(A0,"^",4) KIL Q
C7 S Y=$O(^FHPT(DFN,"S",0)) D:Y<$P(X,"^",6) KIL Q
KIL K ^FH(119,FHDUZ,"I",K) Q
PAT ; Check Patient
 S A0=$G(^FHPT(DFN,"A",ADM,0)),FHORD=$P(A0,"^",2) I FHORD S X1=$P(A0,"^",3) G P1
 S LST=$P(A0,"^",1),FHLD="X" S:'LST LST=+D("NP") G P2
P1 S X=$G(^FHPT(DFN,"A",ADM,"DI",FHORD,0)),FHLD=$P(X,"^",7),PD=$P(X,"^",13),LST=$P(X,"^",16) I 'LST S LST=$P(X,"^",9)
...
P4 ; Check Supplemental Feeding
 ...
 S LST=$P(^FHPT(DFN,"A",ADM,"SF",NO,0),"^",30) I 'LST S LST=$P(^(0),"^",2)
 G:LST>LR P5 S Y=$P(^FHPT(DFN,"A",ADM,"SF",NO,0),"^",4),Y=$P($G(^FH(118.1,+Y,0)),"^",1)
 S FHTF="S^"_Y_"^"_DFN_"^"_ADM_"^"_NO D FIL
P5 ; Check Tubefeeding
 S TF=$P(A0,"^",4) G:'TF P6 S LR=$G(D("TF")) G:'LR P6
 S LST=$P(^FHPT(DFN,"A",ADM,"TF",TF,0),"^",15) I 'LST S LST=$P(^(0),"^",1)
 G:LST>LR P6
 S Y="" F TF2=0:0 S TF2=$O(^FHPT(DFN,"A",ADM,"TF",TF,"P",TF2)) Q:TF2<1  S XX=^(TF2,0) S:Y'="" Y=Y_", " S Y=Y_$P($G(^FH(118.2,$P(XX,"^",1),0)),"^",1)
 S FHTF="T^"_Y_"^"_DFN_"^"_ADM_"^"_TF D FIL
P6 ; Check Status
 S F1=$O(^FHPT(DFN,"S",0)) G:'F1 P7 S A1=^FHPT(DFN,"S",F1,0) I $P(A1,"^",1)<$P(A0,"^",1) G P7
