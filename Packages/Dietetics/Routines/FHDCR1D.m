FHDCR1D ; HISC/REL/NCA - Build Separate Meal Diet Card ;2/23/00  09:52
 ;;5.0;Dietetics;**2,25,34**;Oct 11, 1995
BLD ; Build Diet Card list for a patient in three per page format
 ...
 F X6=0:0 S X6=$O(^FHPT(DFN,"P","B",X6)) Q:X6<1  F X7=0:0 S X7=$O(^FHPT(DFN,"P","B",X6,X7)) Q:X7<1  S PS=$P($G(^FH(115.2,+X6,0)),"^",4) I PS S P4=$G(^FH(114,+PS,0)),P1=$P(P4,"^",7)_"^"_+PS_"^"_$P(P4,"^",1) I +P1,$D(^TMP($J,"FHDEF",MEAL,+P1)) D
 ...
 .Q:FLG
 .F X6=0:0 S X6=$O(^FHPT(DFN,"P","B",X6)) Q:X6<1  F X7=0:0 S X7=$O(^FHPT(DFN,"P","B",X6,X7)) Q:X7<1  S PS=$P($G(^FH(115.2,+X6,0)),"^",4) I +PS S P4=$G(^FH(114,+PS,0)),P1=$P(P4,"^",7)_"^"_+PS_"^"_$P(P4,"^",1) I +P1,+P1=+LP D
 ..S C2=$G(^FHPT(DFN,"P",+X7,0)) Q:$P(C2,"^",2)'[MEAL
 ..S P2=+P1,P3=$P(P1,"^",3) S:'$D(N2(P2,P3)) N2(P2,P3)=+$P(P1,"^",2)_"^"_P3 Q
 .Q
