FHDMP2	; HISC/REL/NCA - Patient Data Log (cont) ;4/30/92  10:38 
	;;5.0;Dietetics;;Oct 11, 1995
	S LST=1 D:$Y>(S1-4) HDR^FHDMP G:QT="^" KIL^FHDMP W !,LN,!?29,"D I E T   O R D E R S"
	I '$D(^FHPT(DFN,"A",ADM,"DI")) W !!,"No Diet Orders for this Admission",! G ^FHDMP3
	I SDT F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"DI",K)) Q:K<1  S D1=$P(^(K,0),"^",9) Q:D1=""!(D1'<SDT)  S LST=K
	F FHORD=LST-1:0 S FHORD=$O(^FHPT(DFN,"A",ADM,"DI",FHORD)) Q:FHORD<1  I $D(^(FHORD,0)) S X=^(0) D:$Y>(S1-6) HDR^FHDMP G:QT="^" KIL^FHDMP D LIS
	W !
	S LST=1 D:$Y>(S1-3) HDR^FHDMP G:QT="^" KIL^FHDMP W !,LN,!?21,"D I E T   O R D E R   S E Q U E N C E",!
	I SDT F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"AC",K)) Q:K=""!(K'<SDT)  S LST=K
	S CT=0 F KK=LST-.0001:0 S KK=$O(^FHPT(DFN,"A",ADM,"AC",KK)) Q:KK<1  S FHORD=$P(^(KK,0),"^",2) D:$Y>(S1-2) HDR^FHDMP G:QT="^" KIL^FHDMP D T1
	I 'CT W !!,"No Diet Order Sequence for this Admission",!
	W ! G ^FHDMP3
LIS	S COM=$G(^FHPT(DFN,"A",ADM,"DI",FHORD,1))
