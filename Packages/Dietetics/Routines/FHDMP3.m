FHDMP3	; HISC/REL/NCA - Patient Data Log (cont) ;4/30/92  10:15 
	...
	I SDT F KK=0:0 S KK=$O(^FHPT(DFN,"A",ADM,"SF",KK)) Q:KK<1  S D1=$P(^(KK,0),"^",2) Q:D1=""!(D1'<SDT)  S LST=KK
	S CT=0 F NO=LST-1:0 S NO=$O(^FHPT(DFN,"A",ADM,"SF",NO)) Q:NO<1  I $D(^(NO,0)) S X=^(0) D:$Y>(S1-10) HDR^FHDMP G:QT="^" KIL^FHDMP D LIS
