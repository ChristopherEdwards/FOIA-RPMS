FHDMP4	; HISC/REL/NCA - Patient Data Log (cont) ;5/7/92  15:36 
	...
	I SDT F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"TF",K)) Q:K<1  S D1=$P(^(K,0),"^",1) Q:D1=""!(D1'<SDT)  S LST=K
	S CT=0 F TF=LST-1:0 S TF=$O(^FHPT(DFN,"A",ADM,"TF",TF)) Q:TF<1  I $D(^(TF,0)) S X=^(0) D TF G:QT="^" KIL^FHDMP
	...
	S CT=0 F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"DR",K)) Q:K<1  I $D(^(K,0)) S X=^(0) I $P(X,"^",1)'<SDT S CT=CT+1 D:$Y>(S1-6) HDR^FHDMP G:QT="^" KIL^FHDMP D CD
	I 'CT W !!,"No Consultations ordered for this Admission"
	W !
	D:$Y>(S1-4) HDR^FHDMP G:QT="^" KIL^FHDMP W !,LN,!?24,"E A R L Y / L A T E   T R A Y S"
	S CT=0 F K=SDT:0 S K=$O(^FHPT(DFN,"A",ADM,"EL",K)) Q:K<1  S X=^(K,0),CT=CT+1 D:$Y>(S1-4) HDR^FHDMP G:QT="^" KIL^FHDMP D EL
	I 'CT W !!,"No Early or Late Trays ordered for this Admission"
	W ! G ^FHDMP5
TF	W !!,"Order # ",TF S CT=CT+1
	F TF2=0:0 S TF2=$O(^FHPT(DFN,"A",ADM,"TF",TF,"P",TF2)) Q:TF2<1  S X1=^(TF2,0) D:$Y>(S1-8) HDR^FHDMP Q:QT="^"  D TF1
