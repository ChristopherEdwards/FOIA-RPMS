FHDMP5	; HISC/REL/NCA - Patient Data Log (cont) ;2/17/93  16:23 
	...
	S CT=0 F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"SP",K)) Q:K<1  I $D(^(K,0)) S X=^(0) I $P(X,"^",4)'<SDT D:$Y>(S1-6) HDR^FHDMP G:QT="^" KIL^FHDMP D SP
	I 'CT W !!,"No Standing Orders for this Admission"
	W !
	D:$Y>(S1-4) HDR^FHDMP G:QT="^" KIL^FHDMP W !,LN,!?23,"A D D I T I O N A L   O R D E R S"
	S CT=0 F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"OO",K)) Q:K<1  I $D(^(K,0)) S X=^(0) I $P(X,"^",2)'<SDT D:$Y>(S1-6) HDR^FHDMP G:QT="^" KIL^FHDMP D L1
