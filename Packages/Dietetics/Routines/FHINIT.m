FHINIT	; ;10/11/95  08:59
	;;5.0;Dietetics;;Oct 11, 1995
	;
	K DIF,DIFQ,DIFQR,DIFQN,DIK,DDF,DDT,DTO,D0,DLAYGO,DIC,DIDUZ,DIR,DA,DIFROM,DFR,DTN,DIX,DZ,DIRUT,DTOUT,DUOUT
	S DIOVRD=1,U="^",DIFQ=0,DIFROM="5.0" W !,"This version (#5.0) of 'FHINIT' was created on 11-OCT-1995"
	W !?9,"(at HISC, by VA FileMan V.21.0)",!
...
EN	; ENTER HERE TO BYPASS THE PRE-INIT PROGRAM
	...
	F DIFRIR=1:1:1 S DIFRRTN="^FHINIT"_$E("5",DIFRIR) D @DIFRRTN
	W:1 !,"I AM GOING TO SET UP THE FOLLOWING FILES:" F I=1:2:88 S DIF(I)=^UTILITY("DIF",$J,I) D 1 G Q:DIFQ!$D(DIRUT) K DIF(I)
	S DIFROM="5.0" D PKG:'$D(DIFROM(0)),^FHINIT1 G Q:'$D(DIFQ) S DIK(0)="AB"
	F DIF=1:2:88 S %=^UTILITY("DIF",$J,DIF),DIK=$P(%,";",5),N=$P(%,";",3),D=$P(%,";",4)_U_N D D K DIFQ(N)
	K DIFQR D ^FHINIT2,^FHINIT3
	L  S DUZ=DIDUZ W:1 !,"NO"_$P("TE THAT FILE",U,DSEC)_" SECURITY-CODE PROTECTION HAS BEEN MADE"
	D ^FHIPST,NOW^%DTC S DIFROM("INIT")=%
	I DIFROM F DIF=1:2:88 S %=^UTILITY("DIF",$J,DIF),N=+$P(%,";",3) I N,$P(%,";",8)="y" S ^DD(N,0,"VR")=DIFROM
	...
	I DIFROM,$D(^%ZTSK) S X="FHINIS" X ^%ZOSF("TEST") D:$T PAC^FHINIS($T(IXF),.DIFROM)
...
S	S DIFQR(N)=0 Q:$P(DIF(I),";",13)'="y"!$D(DIRUT)
	I $P(DIF(I),";",15)="y",$O(@(S_"0)"))>0 S DIF=$P(DIF(I),";",14)="o",DIR("A")="Want my data "_$P("merged with^to overwrite",U,DIF+1)_" yours",DIR("??")="^D DTA^DIFROMH1",DIR(0)="Y",DIR("B")="NO" D ^DIR S DIFQR(N)=$S('Y:Y,1:Y+DIF) Q
