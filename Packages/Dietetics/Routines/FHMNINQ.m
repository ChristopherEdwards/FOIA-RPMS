FHMNINQ ;Hines OIFO/JT,RTK - Dietetics Monitor Inquiry ;07/26/01  09:53
 ;;5.0;Dietetics;**33,34**;Oct 11, 1995
 ;
...
EN ;
 ...
 S FLG=0 I '$O(^FHPT(DFN,"A",FHADM,"MO",0)) D
 .W "NO MONITORS FOR THIS PATIENT" S FLG=1 Q
 Q:FLG
 S J=0 F  S J=$O(^FHPT(DFN,"A",FHADM,"MO",J)) Q:'J!(EX=U)  D
 .I $Y>(IOSL-4) D PG I EX=U Q
 .W !!?3,$P(^FHPT(DFN,"A",FHADM,"MO",J,0),U,1)
 .W !?6,"Entered: " S Y=$P(^(0),U,2) X ^DD("DD") W Y K Y
 .S Y=$P(^FHPT(DFN,"A",FHADM,"MO",J,0),U,5) Q:'Y
 .W ?40,"Cleared: " X ^DD("DD") W Y Q
 I IOST?1"C".E,EX'=U W ! K DIR S DIR(0)="E" D ^DIR
EX K %DT,X,Y,FHI,FHINDX,FHLST,FHADM,FHNDT,DFN,SSN,FLG,J
 Q
DEV ;device and queue info
 ...
 S ZTSAVE("FHLST")="",ZTSAVE("FHADM")="",ZTSAVE("DFN")=""
