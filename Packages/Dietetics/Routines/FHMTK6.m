FHMTK6 ; HISC/NCA - History of Diet Patterns ;2/23/96  15:59
 ;;5.0;Dietetics;**1**;FEB 23, 1996
 S ALL=0 D ^FHDPA G:'DFN KIL
 I $O(^FHPT(DFN,"A",0))<1 W !!,"NO ADMISSIONS ON FILE!" G FHMTK6
 S FHADM=ADM D CUR^FHORD7 G:FHLD'="" KIL S FHO1=FHORD,O1=Y,ANS=""
...
P0 W !!!,"Current Diet: ",$S(O1'="":O1,1:"No Current Diet") S FHORD=""
 K S S (LST,N1)=0 F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"DI",K)) Q:K<1  S X=$G(^(K,0)),X1=$G(^(2)),X2=$G(^(3)) I X1'="" S (FHORD,LST)=K D LIS
...
P1 R !!,"Detailed Display of which Pattern #? ",X:DTIME G:'$T!("^"[X) FHMTK6 I X'?1N.N!(X<1)!(X>N1) W *7," Enter Pattern # to List" G P1
 ...
 S PD=$P($G(^FHPT(DFN,"A",ADM,"DI",FHORD,0)),"^",13)
 S CLERK=$P($G(^FHPT(DFN,"A",ADM,"DI",FHORD,3)),"^",1)
 W !!?25,"Pattern Entered By: ",$E($P($G(^VA(200,+CLERK,0)),"^",1),1,30),!
 S Z=$O(^FHPT(DFN,"P",0)) I Z D PSE G:ANS="^" FHMTK6 D DISP^FHSEL1
 S FHOR=$P($G(^FHPT(DFN,"A",ADM,"DI",FHO1,0)),"^",2,6)
R5 R !!,"Do You Want to Store this Pattern As the Patient's Individual Pattern? N// ",Y:DTIME G:'$T!(Y="^") FHMTK6 S:Y="" Y="N" S X=Y D TR^FH S Y=X
 ...
 S ^FHPT(DFN,"A",FHADM,"DI",FHO1,2)=STR D NOW^%DTC S TIM=%
 S:$G(^FHPT(DFN,"A",FHADM,"DI",FHO1,3))="" ^FHPT(DFN,"A",FHADM,"DI",FHO1,3)=DUZ_"^"_TIM
 I PD S $P(^FHPT(DFN,"A",FHADM,"DI",FHO1,0),"^",13)=PD
 G FHMTK6
