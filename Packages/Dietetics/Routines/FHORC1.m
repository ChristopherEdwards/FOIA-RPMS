...
Q1	S DFN=0,X1=FHDZ I FHDZ G Q3
Q2	S X1=$O(^FHPT("ADRU",X1)) G:X1<1 Q10 S DFN=0
Q3	S DFN=$O(^FHPT("ADRU",X1,DFN)) G:DFN="" Q2:'FHDZ,Q10 S ADM=""
Q4	S ADM=$O(^FHPT("ADRU",X1,DFN,ADM)) G:ADM="" Q3 S DR=""
Q5	S DR=$O(^FHPT("ADRU",X1,DFN,ADM,DR)) G:DR="" Q4
	S Y=^FHPT(DFN,"A",ADM,"DR",DR,0)
	S REQ=$P(Y,"^",2),DTE=$P(Y,"^",1),DIET=$P(Y,"^",5)
	I '$D(^DGPM(ADM,0)) K ^FHPT("ADRU",X1,DFN) S $P(^FHPT(DFN,"A",ADM,"DR",DR,0),"^",8)="X" G Q5
	S Y=^DPT(DFN,0),PNAM=$P(Y,"^",1),FHWRD=$P($G(^FHPT(DFN,"A",ADM,0)),"^",8) D PID^FHDPA
