...
Q2 S (N1,DFN)=0
Q3 S DFN=$O(^FHPT("ADRU",FHX1,DFN)),ADM=0 I DFN="" W:'N1 !!,"No Consults to Clear" G Q1
Q4 S ADM=$O(^FHPT("ADRU",FHX1,DFN,ADM)) G:ADM="" Q3 S FHDR=0
Q5 S FHDR=$O(^FHPT("ADRU",FHX1,DFN,ADM,FHDR)) G:FHDR="" Q4
...
Q9 K ^FHPT("ADRU",FHX1,DFN,ADM,FHDR)
 D NOW^%DTC S $P(^FHPT(DFN,"A",ADM,"DR",FHDR,0),"^",8,11)=TYP_"^"_%_"^"_DUZ_"^"_S1 W "  ... done"
 D EN31^FHASE G Q5
Q10 K DIC S DIC="^VA(200,",DIC(0)="AEQM",DIC("A")="REASSIGN to Clinician: " W ! D ^DIC G KIL:U[X!$D(DTOUT),Q10:Y<1 S XMKK=+Y K DIC
 K ^FHPT("ADRU",FHX1,DFN,ADM,FHDR) S ^FHPT("ADRU",XMKK,DFN,ADM,FHDR)=""
 S $P(^FHPT(DFN,"A",ADM,"DR",FHDR,0),"^",5)=XMKK
 S REQ=CON D POST^FHORC
 W "  ... done" G Q5
EN2 ; Consult Inquiry
 S ALL=1 D ^FHDPA G:'DFN KIL
 S ADM=$O(^FHPT(DFN,"A",0)) I ADM<1 W !,"No Admissions on File" G EN2
 I $P(^FHPT(DFN,"A",0),"^",4)=1 G E2
E1 S DIC="^FHPT(DFN,""A"",",DIC(0)="Q",X="??" D ^DIC S DIC(0)="AQM" W ! D ^DIC K DIC G KIL:U[X!$D(DTOUT),E1:Y<1 S ADM=+Y
E2 S FHDR=$O(^FHPT(DFN,"A",ADM,"DR",0)) I FHDR<1 W !,"No Consultations for this admission" G EN2
 I $P(^FHPT(DFN,"A",ADM,"DR",0),"^",4)=1 G E4
E3 S DIC="^FHPT(DFN,""A"",ADM,""DR"",",DIC(0)="Q",X="??" D ^DIC S DIC(0)="AQM" W ! D ^DIC K DIC G KIL:U[X!$D(DTOUT),E2:Y<1 S FHDR=+Y
E4 S ALL=1 D DISP G EN2
DISP S Y=^FHPT(DFN,"A",ADM,"DR",FHDR,0)
 I '$D(^DGPM(ADM,0)) S XMKK=$P(Y,"^",5) K:XMKK ^FHPT("ADRU",XMKK,DFN) S $P(^FHPT(DFN,"A",ADM,"DR",FHDR,0),"^",8)="X",DTP=U Q
 S PNAM=$P(^DPT(DFN,0),"^",1),FHWRD=$P($G(^FHPT(DFN,"A",ADM,0)),"^",8),WARD=$S(FHWRD:$P($G(^FH(119.6,FHWRD,0)),"^",1),1:"Discharged") I $P($G(^DGPM(ADM,0)),"^",17) S WARD="Discharged"
...
KIL K %,%H,%I,ADM,ALL,ASE,COM,CON,D1,D2,DA,DFN,DIC,DIET,DTP,FHAP,FHWRD,FHWF,FHPV,FHX1,FHX2,FHX3,FHX4,LP,N1,PNAM,FHDR,STAT,S1,ST,TYP,REQ,WARD,X,X1,X9,Y,XMKK Q
