...
R1	S X1=$O(^FHPT("ADR",X1)) I X1<1!(X1>X2) G P1
	S DFN=0
R2	S DFN=$O(^FHPT("ADR",X1,DFN)) G:DFN="" R1 S ADM=0
R3	S ADM=$O(^FHPT("ADR",X1,DFN,ADM)) G:ADM="" R2 S DR=0
R4	S DR=$O(^FHPT("ADR",X1,DFN,ADM,DR)) G:DR="" R3
...
P5	S W1=$O(^TMP($J,D1,D2,DTP,W1)) G:W1="" P4 S DFN=^(W1)
