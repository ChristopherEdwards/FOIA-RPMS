...
E2	S DIC("A")="Select NEW Clinician: " W ! D ^DIC G KIL:U[X!$D(DTOUT),E2:Y<1 S NEW=+Y,N1=0,DFN=0
	S WRD="" I OLD=NEW W !!,"No change in Clinician!" G KIL
E3	S DFN=$O(^FHPT("ADRU",OLD,DFN)) G:DFN<1 E6 S ADM=0
E4	S ADM=$O(^FHPT("ADRU",OLD,DFN,ADM)) G:ADM<1 E3 S REQ=0
E5	S REQ=$O(^FHPT("ADRU",OLD,DFN,ADM,REQ)) G:REQ<1 E4
	I WRD,'$D(^FHPT("AW",WRD,DFN)) G E5
	S N1=N1+1 K ^FHPT("ADRU",OLD,DFN,ADM,REQ)
	S ^FHPT("ADRU",NEW,DFN,ADM,REQ)="",$P(^FHPT(DFN,"A",ADM,"DR",REQ,0),"^",5)=NEW G E5
E6	W !!,N1," Consults were transferred"
KIL	K ADM,DFN,DIC,N1,NEW,OLD,REQ,WRD,X,Y Q
...
N2	R X:DTIME Q:'$T!(X["^")  S:X="" X="N" D TR^FH I $P("YES",X,1)'="",$P("NO",X,1)'="" W *7," Answer YES or NO" G EN2
	Q:X'?1"Y".E  S (N1,DFN)=0,WRD=+DA D E3 Q
