...
Q1	; Print Diet Activity Report
	...
	F W1=0:0 S W1=$O(^FHPT("AW",W1)) Q:W1'>0  D WRD I FHP=D2 F DFN=0:0 S DFN=$O(^FHPT("AW",W1,DFN)) Q:DFN=""  S ADM=^(DFN) D:ADM Q3
	G ^FHORD11
WRD	S P0=$G(^FH(119.6,W1,0)),WRDN=$P(P0,"^",1),D2=$P(P0,"^",8),P0=$P(P0,"^",4) S:P0<1 P0=99 Q
Q3	Q:'$D(^FHPT(DFN,"A",ADM,0))
Q4	S X0=^FHPT(DFN,"A",ADM,0)
	S FHORD=$P(X0,"^",2),X1=$P(X0,"^",3) Q:'FHORD
	S E1="" F NX=TIM:0 S NX=$O(^FHPT(DFN,"A",ADM,"AC",NX)) Q:NX<1!(NX>NOW)  S E1=NX
	S OLW=$P(X0,"^",11),OLR=$P(X0,"^",12) I W1'=OLW S E1=1 I UPD S $P(^FHPT(DFN,"A",ADM,0),"^",11)=W1
	S R1=$G(^DPT(DFN,.101)) I R1'=OLR S E1=1 I UPD S $P(^FHPT(DFN,"A",ADM,0),"^",12)=R1
	S OLW=$S(OLW=W1:" ",'OLW:"",1:$P($G(^FH(119.6,OLW,0)),"^",1)),OLR=$S(R1=OLR:" ",1:OLR)
	S:E1 ^TMP($J,D2,$S(P0<10:"0"_P0,1:P0)_$S(R1'="":R1,1:"   "),DFN)=WRDN_"^"_R1_"^"_ADM_"^"_FHORD_"^"_$P(X0,"^",7)_"^"_$P(X0,"^",10)_"^"_OLW_"^"_OLR Q
KIL	K %,%H,%I,%DT,A1,ADM,ALG,CADM,COM,D2,D3,DA,DFN,DIC,DTP,E1,FHOR,FHP,H1,IS,K,L1,L2,LAB,N1,NOW,NX,OLW,OLR,FHORD,FHDU,FHIO,FHLD
