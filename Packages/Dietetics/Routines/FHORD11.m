FHORD11	; HISC/REL/NCA - Diet Activity Report (cont) ;4/26/93  16:37 
	...
	F D2=0:0 S D2=$O(^TMP($J,D2)) Q:D2<1  S P0="" F E1=0:0 S P0=$O(^TMP($J,D2,P0)) Q:P0=""  F DFN=0:0 S DFN=$O(^TMP($J,D2,P0,DFN)) Q:DFN<1  S X=^(DFN) D LST
	D DISC I LAB F L=1:1:18 W !
	W:'LAB ! I UPD S $P(^FH(119.73,FHP,0),"^",2)=NOW
	Q
LST	S W1=$P(X,"^",1),R1=$P(X,"^",2),ADM=$P(X,"^",3),FHORD=$P(X,"^",4),SF=$P(X,"^",5),IS=$P(X,"^",6),OLW=$P(X,"^",7),OLR=$P(X,"^",8) Q:'$D(^DPT(DFN,0))  S Y0=^(0)
	S SO=$D(^FHPT("ASP",DFN,ADM))
	S W1=$E(W1,1,15),R1=$E(R1,1,10),N1=$E($P(Y0,"^",1),1,22) D PID^FHDPA
	S X0=$G(^FHPT(DFN,"A",ADM,"DI",FHORD,0)),COM=$G(^FHPT(DFN,"A",ADM,"DI",FHORD,1))
...
CUR	S Y="" Q:X=""  S FHOR=$P(X,"^",2,6),FHLD=$P(X,"^",7)
	...
	Q
OLD	S X2="" F NX=0:0 S NX=$O(^FHPT(DFN,"A",ADM,"AC",NX)) Q:NX<1!(NX>TIM)  S X2=$P(^(NX,0),"^",2)
...
D2	S X=$G(^DGPM(DA,0)),DFN=$P(X,"^",3),ADM=$P(X,"^",14) Q:'DFN!('ADM)
	...
	I $D(^FHPT(DFN,"A",ADM,0)) S X=^(0),W1=$P(X,"^",11),R1=$P(X,"^",12),SF=$P(X,"^",7)
	S SO=$D(^FHPT("ASP",DFN,ADM))
