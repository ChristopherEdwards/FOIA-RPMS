...
F0	S ALL=0 D ^FHDPA G KIL:'DFN,F0:'$D(^DGPM(ADM,0))
...
F01	W !!,"List Orders from Date/Time: "_DTP_" // " R X:DTIME G:'$T!(X["^") KIL S:X="" Y=0 I X'="" S %DT="EXTS" D ^%DT G:Y<1 F01
	...
	I Y'="",FHORD>0 I $D(^FHPT(DFN,"A",ADM,"DI",FHORD,1)) W !,"Comment: ",^(1)
	S X1=^FHPT(DFN,"A",ADM,0),DTP=$P(X1,"^",3) I DTP D DTP^FH W !,"Expires: ",DTP
	S TF=$P(X1,"^",4) G:TF<1 F1
	S Y=^FHPT(DFN,"A",ADM,"TF",TF,0)
	S DTP=$P(Y,"^",1),COM=$P(Y,"^",5),TQU=$P(Y,"^",6),CAL=$P(Y,"^",7)
	D DTP^FH W !!,"Tubefeed Ordered: ",DTP
	F TF2=0:0 S TF2=$O(^FHPT(DFN,"A",ADM,"TF",TF,"P",TF2)) Q:TF2<1  S XY=^(TF2,0) D LP
	W !,"Total Quantity: ",TQU," cc",?42,"Total KCAL: ",CAL
	W:COM'="" !,"Comment: ",COM
F1	S CT=0 F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"OO",K)) Q:K<1  S X=^(K,0) I $P(X,"^",2)'<D4,$P(X,"^",5)="S" D L1
	S N1=0 F FHORD=0:0 S FHORD=$O(^FHPT(DFN,"A",ADM,"DI",FHORD)) Q:FHORD<1  S X=^(FHORD,0) D LST
	I 'N1 W !!,"No Diet Orders Entered",! G F0
	D TLN G F0
KIL	K %,%DT,A1,ADM,ALL,C,CAL,COM,CT,D1,D2,D3,D4,DA,DTP,DFN,FHDU,FHLD,FHOR,I,K,KK,N1,NOW,FHORD,FHWF,FHPV,POP,TYP,TF,TF2,TQU,TUN,STR,QUA,WARD,X,X1,X2,XY,Y Q
...
LST	S COM=$G(^FHPT(DFN,"A",ADM,"DI",FHORD,1)) Q:$P(X,"^",9)<D4
...
TLN	W !!?24,"----- Diet Orders Time Line -----",!,"    Effective     Type  Order",!
	F KK=0:0 S KK=$O(^FHPT(DFN,"A",ADM,"AC",KK)) Q:KK<1  S FHORD=$P(^(KK,0),"^",2) D T1
	Q
T1	S X=^FHPT(DFN,"A",ADM,"DI",FHORD,0) Q:$P(X,"^",9)<D4
