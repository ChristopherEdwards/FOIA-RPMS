...
F0 S ALL=0 D ^FHDPA G:'DFN KIL^FHORD1
 D NOW^%DTC S NOW=% D CUR,FUT^FHORD1
 W !!,"Place patient on NPO/HOLD-TRAY." D F5 Q:'$D(DFN)  D F7 W !!,"... done" G F0
F5 ; Process NPO
...
EN2 ; Cancel Withhold Order
 ...
 S ALL=0 D ^FHDPA G:'DFN KIL D CUR S OLD=FHLD
 S (A2,CT)=0 F KK=0:0 S KK=$O(^FHPT(DFN,"A",ADM,"AC",KK)) Q:KK<1!(KK'<NOW)  S A2=KK
 S XT="" K N F KK=A2-.000001:0 S KK=$O(^FHPT(DFN,"A",ADM,"AC",KK)) Q:KK<1  S FHORD=$P(^(KK,0),"^",2) D T1
...
T0 ; Update cancelled NPO
 S X=^FHPT(DFN,"A",ADM,"DI",FHORD,0),D1=$P(X,"^",9),D2=$S(D1'>NOW:NOW,1:D1)
 S $P(^FHPT(DFN,"A",ADM,"DI",FHORD,0),"^",10)=D2
 S $P(^FHPT(DFN,"A",ADM,"DI",FHORD,0),"^",18,19)=D2_"^"_DUZ
 S:FHWF'=2 FHORN=$P(^FHPT(DFN,"A",ADM,"DI",FHORD,0),"^",14)
 F K9=KK-.000001:0 S K9=$O(^FHPT(DFN,"A",ADM,"AC",K9)) Q:K9<1  I $P(^(K9,0),"^",2)=FHORD S D1=K9 D S0
 D UPD^FHORD7 W:FHWF'=2 "  ... done" Q:FHWF=2  D CUR
 I OLD'="","^^^^"'[FHOR S D1=NOW D ^FHORD1A
 Q
S0 ; Set AC cross-ref data field
 S X2=D1+.0000001,D2=$O(^FHPT(DFN,"A",ADM,"AC",D1)) S:D2<1 D2=""
...
NOR L +^FHPT(DFN,"A",ADM,"DI",0)
 I '$D(^FHPT(DFN,"A",ADM,"DI",0)) S ^FHPT(DFN,"A",ADM,"DI",0)="^115.02A^^"
 S X=^FHPT(DFN,"A",ADM,"DI",0),A2=$P(X,"^",3)+1,^(0)=$P(X,"^",1,2)_"^"_A2_"^"_($P(X,"^",4)+1) L -^FHPT(DFN,"A",ADM,"DI",0)
 S ^FHPT(DFN,"A",ADM,"DI",A2,0)=A2_"^^^^^^X^^"_D1_"^^"_DUZ_"^"_NOW,X="" Q
CUR D CUR^FHORD7 W !!,"Current Diet: ",$S(Y'="":Y,1:"No Current Order")
 Q:'FHORD  S X9=$P(X,"^",8) W:X9'="" " (",$S(X9="T":"Tray",X9="D":"Dining Room",1:"Cafe"),")" Q
T1 Q:'$D(^FHPT(DFN,"A",ADM,"DI",FHORD,0))!($D(N(FHORD)))  Q:$P(^(0),"^",7)=""  S P2=$P(^(0),"^",10)
