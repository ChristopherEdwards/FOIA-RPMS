...
S1 R !!,"Patient is on CAFETERIA/DINING ROOM Service. Change to TRAY? YES// ",X:DTIME G:'$T!(X["^") KIL
 ...
 G:X?1"N".E S2 D CUR^FHORD7 I "^^^^"[FHOR S $P(^FHPT(DFN,"A",ADM,0),"^",5)="T" G S2
 S TYP="T",D2=$P(X,"^",10),(D3,D4)=0,COM="",D1=NOW,DT=NOW\1 D STR^FHORD7
S2 ;
 W "  ... done" G KIL
F1 S X=$P(^FHPT(DFN,"A",ADM,0),"^",10)
...
F2 R !!,"Do you wish to remove? (Y/N) ",X:DTIME G:'$T!(X["^") KIL S:X="" X="*" D TR^FH I $P("YES",X,1)'="",$P("NO",X,1)'="" W *7,"  Answer YES or NO" G F2
 I X?1"Y".E D CAN S FHORN=$P(^FHPT(DFN,"A",ADM,0),"^",13) D:FHORN>0 CODE^FHWOR61 W "  ...removed" G KIL
 W "  ... no change made" G KIL
FIL ; File Isolation
 S $P(^FHPT(DFN,"A",ADM,0),"^",10)=IS,X=^FH(119.4,IS,0)
 S ^FHPT("AIS",DFN,ADM)=""
 S EVT="I^O^"_IS D ^FHORX Q
CAN ; Cancel Isolation
 S IS=$P(^FHPT(DFN,"A",ADM,0),"^",10),$P(^(0),"^",10)="" K ^FHPT("AIS",DFN,ADM)
 S EVT="I^C^"_IS D ^FHORX Q
KIL K %DT,ADM,ALL,BY,C,COM,D1,D2,D3,D4,DA,DFN,DHD,DIC,DIE,DR,FHDU,FHLD,FHORD,FHDR,FHOR,FHORN,FHWF,FHPV,FLDS,FR,I,IS,L,NOW,POP,TO,TYP,WARD,X,X1,Y Q
