FHORD6 ; HISC/REL/NCA/JH - Diet Inquiry ;5/3/01  11:04
 ;;5.0;Dietetics;**9,6,32**;Oct 11, 1995
 S ALL=0 D ^FHDPA G:'DFN KIL
 K IOP S %ZIS="MQ",%ZIS("B")="HOME" W ! D ^%ZIS K %ZIS,IOP G:POP KIL
 I $D(IO("Q")) S FHPGM="F0^FHORD6",FHLST="DFN^PID^ADM" D EN2^FH G KIL
 U IO D F0 D ^%ZISC K %ZIS,IOP G FHORD6
F0 ; Display Diet
 D NOW^%DTC S NOW=%,DT=NOW\1,QT=""
 S WARD=$P($G(^FHPT(DFN,"A",ADM,0)),"^",8) S:WARD WARD=$P($G(^FH(119.6,WARD,0)),"^",1)
 ...
 K N S P1=1 F K=0:0 S K=$O(^FHPT(DFN,"P",K)) Q:K<1  S X=^(K,0),M=$P(X,"^",2) S:M="A" M="BNE" D SP
 ...
 W ! K L,N,M,M1,M2
 G:QT="^" KIL ;P32
A0 S X(0)=^FHPT(DFN,"A",ADM,0),X=$P(X(0),"^",10) G:X="" F1
...
F1 D CUR^FHORD7 D:$Y>(IOSL-6) HDR G:QT="^" KIL W !!,"Current Diet: ",$S(Y'="":Y,1:"No current order")
 I Y'="",FHORD>0 I $D(^FHPT(DFN,"A",ADM,"DI",FHORD,1)) S COM=^(1) W:COM'="" !,"Comment: ",COM
 ...
 W:COM'="" !,"Comment: ",COM
F2 S NO=$P(X(0),"^",7),Y=$S('NO:"",1:^FHPT(DFN,"A",ADM,"SF",NO,0)) D:$Y>(IOSL-3) HDR G:QT="^" KIL
...
KIL ; Final variable kill
 K %,%H,%I,%T,%ZIS,A1,ADM,AGE,ALG,ALL,BAG,C,CAL,COM,CON,CT,D3,DA,DAS,FHDU,DFN,DOB,DTP,FHOR,FHLD,I,IOP,K,K1,K2,KK,L,L1,LST,MEAL,N,NO,NOW,FHORD,FHWF,FHPV
