FHORD61 ; HISC/REL/NCA - Diet Inquiry (cont) ;3/13/01  15:08
 ;;5.0;Dietetics;**29,37**;Oct 11, 1995
 S X1=NOW,X2=-1 D C^%DTC S LST=X,CT=0
 F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"OO",K)) Q:K<1  S X=^(K,0) I $P(X,"^",2)'<LST!($P(X,"^",5)="S"),$P(X,"^",5)'="X" D L1 Q:QT="^"
 G:QT="^" KIL^FHORD6
 W ! S CT=0 F KK=NOW:0 S KK=$O(^FHPT(DFN,"A",ADM,"AC",KK)) Q:KK<1  S FHORD=$P(^(KK,0),"^",2) D T1 Q:QT="^"
 G:QT="^" KIL^FHORD6
 D:$Y>(IOSL-2) HDR^FHORD6 G:QT="^" KIL^FHORD6 I 'CT W !,"No future Diet Orders exist"
F4 W ! S CT=0 F K=NOW:0 S K=$O(^FHPT(DFN,"A",ADM,"EL",K)) Q:K<1  S CT=CT+1,Y=^(K,0) D EL Q:QT="^"
 G:QT="^" KIL^FHORD6
 D:$Y>(IOSL-2) HDR^FHORD6 G:QT="^" KIL^FHORD6 I 'CT W !,"No future Early or Late Trays ordered"
 K N F K=0:0 S K=$O(^FHPT("ASP",DFN,ADM,K)) Q:K<1  S X=^FHPT(DFN,"A",ADM,"SP",K,0),M=$P(X,"^",3),M=$S(M="BNE":"A",1:$E(M,1)),N(M,K)=$P(X,"^",2,3),$P(N(M,K),"^",3)=$P(X,"^",8)
 ...
 G:QT="^" KIL^FHORD6
 K L,N,M,M1,M2
F5 S CT=0 F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"DR",K)) Q:K<1  D:$Y>(IOSL-5) HDR^FHORD6 Q:QT="^"  D CD
 G:QT="^" KIL^FHORD6
 I 'CT D:$Y>(IOSL-4) HDR^FHORD6 G:QT="^" KIL^FHORD6 W !!,"No Active Consultations for this Admission"
F6 S CT=0 F KK=0:0 S KK=$O(^FHPT(DFN,"A",ADM,"MO","AC",KK)) Q:KK<1!(QT="^")  F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"MO","AC",KK,K)) Q:K<1  D:$Y>(IOSL-5) HDR^FHORD6 Q:QT="^"  D MO
 G:QT="^" KIL^FHORD6
...
CD S Y=^FHPT(DFN,"A",ADM,"DR",K,0) Q:$P(Y,"^",8)'="A"  S CT=CT+1
...
MO ; Display Monitors
 S Y=$G(^FHPT(DFN,"A",ADM,"MO",K,0)) Q:Y=""  S CT=CT+1
