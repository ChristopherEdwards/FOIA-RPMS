FHORD71 ; HISC/REL - Diet Order Utilities (cont) ;10/1/96  10:00
 ;;5.0;Dietetics;**2,6,37**;Oct 11, 1995
...
D2 R !!,"Expiration Date/Time: ",X:DTIME G:'$T!(X["^") D3 Q:X=""  D:$P(X,"@",2)?1U CNV S %DT="ETSX" D ^%DT G D3:U[X,D2:Y<1
 ...
 S D2=+Y Q
D3 S D1=0 Q
CNV S DP=$P($G(^FHPT(DFN,"A",ADM,0)),"^",8)
...
ACR ; Store AC diet sequence data
 G:Z6<1 A1 I '$D(^FHPT(DFN,"A",ADM,"AC",0)) S ^(0)="^115.14^^"
 S ^FHPT(DFN,"A",ADM,"AC",+Z6,0)=Z6,$P(^FHPT(DFN,"A",ADM,"AC",0),"^",3)=+Z6
 I Z6'>NOW S Z6=1 G A1
 N X,X1,FHORN K ZTSAVE
 S ZTIO="",ZTRTN="UPD^FHORD7",ZTREQ="@",ZTDESC="Diet Update",ZTDTH=+Z6
 S ZTSAVE("DFN")=DFN,ZTSAVE("ADM")=ADM,ZTSAVE("Z6")=+Z6,ZTSAVE("ZTREQ")="" D ^%ZTLOAD K ZTSK
 S Z6=1
A1 S $P(^FHPT(DFN,"A",ADM,"AC",0),"^",4)=$P(^FHPT(DFN,"A",ADM,"AC",0),"^",4)+Z6 K Z6 Q
OE ; File OE/RR Diet Order
 ...
 I FHWF=1 D FILE S:FHDU $P(^FHPT(DFN,"A",ADM,"DI",FHORD,0),"^",15)=FHDU Q
 I FHWF=2 S FHDU=+FHORN_"^"_FHDU D FHWF2
 S:+FHDU $P(^FHPT(DFN,"A",ADM,"DI",FHORD,0),"^",14,15)=FHDU Q
OEU ; Update status of OE/RR orders
 N FHORN K A1 S A1=0
 F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"AC",K)) Q:K>NOW!(K<1)  S A1=K
 I A1 S X=$P(^FHPT(DFN,"A",ADM,"AC",A1,0),"^",2),A1(+X)=A1,A1=+X
 F K=NOW:0 S K=$O(^FHPT(DFN,"A",ADM,"AC",K)) Q:K<1  S X=$P(^(K,0),"^",2) I '$D(A1(+X)) S A1(+X)=K
 F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"DI",K)) Q:K=""  S FHORN=$P(^(K,0),"^",14) I FHORN S STS=$P(^(0),"^",15) D U1
 K A1,K,X,FHORN,FHL,FHO,FHMSG1,FHSAV,STS Q
U1 I '$D(A1(K)) Q:STS<3  S $P(^FHPT(DFN,"A",ADM,"DI",K,0),"^",15)=1,FHSTS=1,FHSAV=$G(^FHPT(DFN,"A",ADM,"DI",K,0)) Q:'$D(^OR(100,FHORN))  G U3
 N FHMSG1,FHO,FHSAV S FHSAV=$G(^FHPT(DFN,"A",ADM,"DI",K,0))
 S FHO=$P(FHSAV,"^",2,6),VAL="" D VAL^FHWORP(FHO,.VAL) Q:VAL=""
 S FHMSG1=$S($P(FHSAV,"^",7)="N":"N",1:"D")_";"_ADM_";"_K_";"_$P(FHSAV,"^",9)_";"_$P(FHSAV,"^",10)_";"_$P(FHSAV,"^",7)_";"_$G(^FHPT(DFN,"A",ADM,"DI",K,1))_";"_$P(FHSAV,"^",8)_";;"_VAL
 S FHDAT="",FHSTRT=$P(^FHPT(DFN,"A",ADM,"DI",K,0),"^",9) I FHSTRT'=A1(K) S FHDAT=A1(K)
 S FHDAT=FHDAT_"^"_$P(^FHPT(DFN,"A",ADM,"DI",K,0),"^",10)
 S FHSTS=$S(K=A1:6,1:8) I FHSTS'=STS S $P(^FHPT(DFN,"A",ADM,"DI",K,0),"^",15)=FHSTS
...
FHWF2 ; Perform if orders comes from OE/RR
 S $P(^FHPT(DFN,"A",ADM,"DI",FHORD,0),"^",14)=$S(+FHORN:+FHORN,1:0)
 Q:FHLD="X"!(FHLD="P")
