FHORD72 ; HISC/NCA - Diet Order Utilities (cont) ;12/4/00  10:36
 ;;5.0;Dietetics;**19,21,27,30**;Oct 11, 1995
ADD ; Add diet associated Standing Orders and Supplemental Feeding Menu
 S PDFLG=0 Q:'X1  I $D(FHYES) Q:FHYES
 N FHOR S FHOR=$P($G(^FHPT(DFN,"A",ADM,"DI",X1,0)),"^",2,6)
 S X3="" F NX=0:0 S NX=$O(^FHPT(DFN,"A",ADM,"AC",NX)) Q:NX<1!(NX'<A1)  S X3=$P(^(NX,0),"^",2)
 G:X3=X1 KIL G:X3="" A1
 I FHOR=$P($G(^FHPT(DFN,"A",ADM,"DI",X3,0)),"^",2,6) D IND
 K N,P S LN=0
 F K=0:0 S K=$O(^FHPT("ASP",DFN,ADM,K)) Q:K<1  S X=^FHPT(DFN,"A",ADM,"SP",K,0) I $P(X,"^",9)="Y" S M=$P(X,"^",3),M=$S(M="BNE":"A",1:$E(M,1)),N(M,K)=$P(X,"^",2,3)_"^"_$P(X,"^",8),LN=LN+1,P(LN,+N(M,K))=K_"^"_N(M,K)
...
A2 S SF=$P($G(^FHPT(DFN,"A",ADM,0)),"^",7) G:'SF A3 S X=$G(^FHPT(DFN,"A",ADM,"SF",SF,0))
...
SO ; Add Standing Order
 ...
 L -^FHPT(DFN,"A",ADM,"SP",0) I $D(^FHPT(DFN,"A",ADM,"SP",NO)) G SO
...
SF ; Add Supplemental Feeding
 ...
 L -^FHPT(DFN,"A",ADM,"SF",0) I $D(^FHPT(DFN,"A",ADM,"SF",NO)) G SF
 S ^FHPT(DFN,"A",ADM,"SF",NO,0)=NO_"^"_$P(PNN,"^",2,99)
 S $P(^FHPT(DFN,"A",ADM,0),"^",7)=NO
 I NO'="" S EVT="F^O^"_NO D ^FHORX  ;file event, P30
UPD S:NO $P(^FHPT(DFN,"A",ADM,"SF",NO,0),"^",30,31)=NOW_"^"_DUZ
 S:NO $P(^FHPT(DFN,"A",ADM,"SF",NO,0),"^",34)="Y" Q
IND ; Restore Individual Pattern
 Q:'$D(^FHPT(DFN,"A",ADM,"DI",X3,2))
 S ^FHPT(DFN,"A",ADM,"DI",X1,2)=$G(^FHPT(DFN,"A",ADM,"DI",X3,2))
 S $P(^FHPT(DFN,"A",ADM,"DI",X1,3),"^",1,2)=DUZ_"^"_NOW
 I $P(^FHPT(DFN,"A",ADM,"DI",X3,0),"^",13) S $P(^FHPT(DFN,"A",ADM,"DI",X1,0),"^",13)=$P(^FHPT(DFN,"A",ADM,"DI",X3,0),"^",13),PDFLG=1
 Q
