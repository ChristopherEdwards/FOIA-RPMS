FHORD82 ; HISC/REL/NCA - Diet Order Lists (cont) ;3/25/96  08:36
 ;;5.0;Dietetics;**2,37**;Mar 25, 1996
 S TF=$P(X0,"^",4) G:'TF L4
 S Y=^FHPT(DFN,"A",ADM,"TF",TF,0),X=$P(Y,"^",1),COM=$P(Y,"^",5),CAL=$P(Y,"^",7)
 W !,?13,"Tubefeed.: " S ZZ="" D DT S DTE=X
 F TF2=0:0 S TF2=$O(^FHPT(DFN,"A",ADM,"TF",TF,"P",TF2)) Q:TF2<1  S X3=$G(^(TF2,0)),TUN=+X3,XX=$G(^FH(118.2,TUN,0)),TUN=$P(XX,"^",1) D CALC S:ZZ'="" ZZ=ZZ_", " S ZZ=ZZ_P2_" "_TUN
 S ZZ=ZZ_", "_CAL_" Kcal/Day"
 S REC=1 D LNE
L3 W:COM'="" !?15,COM,!
L4 F K1=DT-.00001:0 S K1=$O(^FHPT(DFN,"A",ADM,"EL",K1)) Q:K1<1!(K1\1>K3)  S Y=^(K1,0) D EL
 D DISP^FHORD83
 K N F K=0:0 S K=$O(^FHPT("ASP",DFN,ADM,K)) Q:K<1  S X=^FHPT(DFN,"A",ADM,"SP",K,0),M=$P(X,"^",3),M=$S(M="BNE":"A",1:$E(M,1)),N(M,K)=$P(X,"^",2,4),$P(N(M,K),"^",4,5)=$P(X,"^",8,9)
 W !
 ...
 K L,N,M,M1,M2 S NM=$P(X0,"^",7) G:'NM L3^FHORD81 S Y=^FHPT(DFN,"A",ADM,"SF",NM,0)
...
LNE ; Break Line if longer than 45 chars
 ...
 Q:ZZ=""  W !?24
 G LNE
FLNE S DTP=$P(^DGPM(ADM,0),"^",1) D DTP^FH
 W !!,RM,?13,$E($P($G(^DPT(DFN,0)),"^",1),1,24),?38,BID,?47,DTP
 I $P($G(^FHPT(DFN,"A",ADM,0)),"^",5)'="" W ?67,$P(^FHPT(DFN,"A",ADM,0),"^",5)
 Q
