...
CODE	; Recode diet
	...
	S Z=0 D:FLG MIS
C1	S:Z $P(^FHPT(DFN,"A",ADM,"DI",FHORD,0),"^",13)=Z Q
...
INP	; Recode all inpatients
	...
	F W1=0:0 S W1=$O(^FHPT("AW",W1)) Q:W1'>0  F DFN=0:0 S DFN=$O(^FHPT("AW",W1,DFN)) Q:DFN<1  S ADM=$G(^FHPT("AW",W1,DFN)) D:ADM Z1
	K ADM,C,D,DFN,FHOR,FHORD,FLG,I,K1,K2,LC,M,W1,X,X1,Z Q
Z1	F FHORD=0:0 S FHORD=$O(^FHPT(DFN,"A",ADM,"DI",FHORD)) Q:FHORD<1  D Z2
	Q
Z2	S Z=$G(^FHPT(DFN,"A",ADM,"DI",FHORD,0)),FHOR=$P(Z,"^",2,6) Q:"^^^^"[FHOR  D CODE Q
