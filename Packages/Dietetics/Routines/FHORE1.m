FHORE1 ; HISC/REL - Early/Late Trays ;8/8/96  13:57 ;
 ...
 S ALL=0 D ^FHDPA G:'DFN KIL D F1 G:'$D(DFN) KIL I 'FHWF S FHORN="" D ^FHORE1A G KIL
 ; Set HL7
...
F6 R !,"Select Meal (B,N,E): ",MEAL:DTIME G:'$T!("^"[MEAL) AB S X=MEAL D TR^FH S MEAL=X
 ...
 G:SDT'=EDT F7 F K=SDT:0 S K=$O(^FHPT(DFN,"A",ADM,"EL",K)) Q:K<1!(K\1'=SDT)  I $P(^(K,0),"^",2)=MEAL W *7,!!,"Early/Late Meal Already Ordered for this Date!" G AB
...
F10 S BAG="N" I $P(FHPAR,"^",10)'="N" R !,"Bagged Meal: NO// ",BAG:DTIME G:'$T!(BAG=U) AB S:BAG="" BAG="N" S X=BAG D TR^FH S BAG=X I $P("YES",BAG,1)'="",$P("NO",BAG,1)'="" W *7," Enter Y or N" G F10
 S BAG=$E(BAG,1) W !?5 Q
DP S Y="" S W1=$P($G(^FHPT(DFN,"A",ADM,0)),"^",8),DP=$P($G(^FH(119.6,+W1,0)),"^",8)
...
CUR S A1=0,(FHOR,FHLD)="" F KK=0:0 S KK=$O(^FHPT(DFN,"A",ADM,"AC",KK)) Q:KK<1!(KK>DTE)  S A1=KK
...
KIL K %,%H,%I,%T,%DT,A1,ADM,ALL,BAG,C,DA,DFN,DP,DTE,DTP,EDT,FHDAY,FHLD,FHOR,FHPAR,FHWF,FHPV,I,K,K1,K2,KK,MEAL,N,NUM,NOW,FHORN,FHORD,SDT,SERV,TIM,WARD,WKD,X,X1,X2,X31,Y,Z Q
