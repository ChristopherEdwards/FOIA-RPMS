FHORE3 ; HISC/REL - Cancel Early/Late Trays ;6/21/96  10:30 ;
 ...
 S ALL=0 D ^FHDPA G:'DFN KIL D NOW^%DTC S NOW=%
S0 S CT=0 K N,NN W !!,"Order     Date        Time       Meal",!
 F K=NOW:0 S K=$O(^FHPT(DFN,"A",ADM,"EL",K)) Q:K<1  S CT=CT+1,N(CT)=K,Y=^(K,0) D D3
...
D2 S DTE=N(K3) S FHORN=$P($G(^FHPT(DFN,"A",ADM,"EL",DTE,0)),"^",7) I FHORN S:'$D(NN(FHORN,DTE)) NN(FHORN,DTE)=""
 K ^FHPT(DFN,"A",ADM,"EL",DTE),^FHPT("ADLT",DTE,DFN)
 S %=$S($D(^FHPT(DFN,"A",ADM,"EL",0)):$P(^(0),"^",4),1:0)-1 S:%'<0 $P(^(0),"^",4)=%
 Q:'FHORN  F I9=NOW:0 S I9=$O(^FHPT(DFN,"A",ADM,"EL",I9)) Q:I9<1  I $P(^(I9,0),"^",7)=FHORN G D21
...
KIL K %,%H,%I,ADM,ALL,BAG,CT,DA,WARD,DFN,DTE,DTP,FHORN,FHWF,FHPV,I9,K,K1,K2,K3,MEAL,N,NN,NOW,TIM,X,Y Q
