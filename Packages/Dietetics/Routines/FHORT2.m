...
EN2 ; Current Tubefeeding
 S (ALL,TF)=0 D ^FHDPA Q:'DFN
 S TF=$P(^FHPT(DFN,"A",ADM,0),"^",4)
 I TF<1 W !!,"No Tubefeeding Order Exists" Q
DIS ; Display Tubefeeding
 S T=^FHPT(DFN,"A",ADM,"TF",TF,0),DTP=$P(T,"^",1),NO=0 D DTP^FH W !!,"Date Ordered: ",DTP
 W ! F TF2=0:0 S TF2=$O(^FHPT(DFN,"A",ADM,"TF",TF,"P",TF2)) Q:TF2<1  S Y=^(TF2,0),TUN(+Y)=Y,NO=NO+1 D D1
 S TFCOM=$P(T,"^",5)
...
C1 R !!,"Do you wish to RESUME Tray Service? N// ",X:DTIME G:'$T!(X="^") KIL S:X="" X="N" D TR^FH I $P("YES",X,1)'="",$P("NO",X,1)'="" W *7,"  Answer YES or NO" G C1
 G:X'?1"Y".E KIL S A2=0 F KK=0:0 S KK=$O(^FHPT(DFN,"A",ADM,"AC",KK)) Q:KK<1!(KK'<NOW)  I $P(^(KK,0),"^",2)=FHORD S A2=KK
...
CAN ; Perform cancel
 ...
 S $P(^FHPT(DFN,"A",ADM,0),"^",4)="" K ^FHPT("ADTF",DFN,ADM)
 S $P(^FHPT(DFN,"A",ADM,"TF",TF,0),"^",11,12)=NOW_"^"_DUZ
 S FHORN=$P(^FHPT(DFN,"A",ADM,"TF",TF,0),"^",14)
 S FHSAV=$G(^FHPT(DFN,"A",ADM,"TF",TF,0))
 ...
 Q
KIL K %,%H,%I,A1,A2,C,D1,D2,D3,DA,KK,P2,FHDU,NOW,X1,X2,OLD,FHDR,FHORD,FHPAR,FHLD,FHWF,FHPV,FHOR,I,K9,ADM,ALL,COM,TFCOM,DFN,FHD,POP,PID,BID,DTP,QUA,STR,T,TF,TF2,TUN,WARD,X,X9,Y Q
