FHORX1C ; HISC/REL/NCA - List Dietetic Events ;11/15/00  14:38
 ;;5.0;Dietetics;**2,27,30**;Oct 11, 1995
...
L ; Location
 ...
 G LNE
D ; Diet
 S X=$G(^FHPT(DFN,"A",ADM,"DI",FHORD,0)),COM=$G(^(1)) D CUR S EVT="Diet: "_Y
 I FHLD="" S X9=$P(X,"^",8) I X9'="",X9'=TC S EVT=EVT_"  ("_$S(X9="T":"Tray",X9="D":"Dining Room",1:"Cafe")_")"
 S:COM'="" EVT=EVT_", "_COM D LNE
 S X2="" F NX=0:0 S NX=$O(^FHPT(DFN,"A",ADM,"AC",NX)) Q:NX<1!(NX>TIM)  S X2=$P(^(NX,0),"^",2)
 Q:X2=FHORD!(X2="")  S X=$G(^FHPT(DFN,"A",ADM,"DI",X2,0)) D CUR
 S EVT="Old:  "_Y G LNE
I ; Isolation
...
T ; Tubefeed
 S EVT=""
 F K1=0:0 S K1=$O(^FHPT(DFN,"A",ADM,"TF",FHORD,"P",K1)) Q:K1<1  S X3=$G(^(K1,0)),TUN=$P(X3,"^",1),XX=$G(^FH(118.2,TUN,0)) D CALC^FHORX3 S:EVT'="" EVT=EVT_", " S EVT=EVT_P2_" "_$P(XX,"^",1)
 S COM=$P($G(^FHPT(DFN,"A",ADM,"TF",FHORD,0)),"^",5) I COM'="" S EVT=EVT_", "_COM
 S EVT="TF: "_EVT S:ACT="C" EVT=EVT_" Cancelled"
 G LNE
O ; Additional Order
 S Y=$P($G(^FHPT(DFN,"A",ADM,"OO",FHORD,0)),"^",3) Q:Y=""
...
F ; Suppl. feedings P30
 S Z=$G(^FHPT(DFN,"A",ADM,"SF",+FHORD,0)) Q:Z=""  S SP=$P(Z,"^",4)
 S EVT="SF: "_$P($G(^FH(118.1,+SP,0)),"^",1) S:ACT="C" EVT=EVT_" Cancelled" G LNE
S ; Standing Orders
 S Z=$G(^FHPT(DFN,"A",ADM,"SP",+FHORD,0)) Q:Z=""  S SP=$P(Z,"^",2),MEAL=$P(Z,"^",3),QTY=$P(Z,"^",8)
