FHORX2	; HISC/REL - List Patient Events ;8/2/94  10:07
	;;5.0;Dietetics;;Oct 11, 1995
	S ALL=1 D ^FHDPA G:'DFN KIL D NOW^%DTC S DT=%\1
	I '$D(^FH(119.8,"AP",DFN)) W !!,"No Dietetic Events on File." G FHORX2
	D DT G:'SDT!('EDT) FHORX2 S EDT=EDT+.3
	K IOP S %ZIS="MQ",%ZIS("B")="HOME" W ! D ^%ZIS K %ZIS,IOP G:POP KIL
	I $D(IO("Q")) S FHPGM="Q1^FHORX2",FHLST="DFN^SDT^EDT" D EN2^FH G KIL
	U IO D Q1 D ^%ZISC K %ZIS,IOP G FHORX2
Q1	; Process List
	D HDR F KK=SDT-.1:0 S KK=$O(^FH(119.8,"AP",DFN,KK)) Q:KK>EDT!(KK<1)  F DA=0:0 S DA=$O(^FH(119.8,"AP",DFN,KK,DA)) Q:DA<1  D ^FHORX3
	G KIL
HDR	S X="Dietetic Events for "_$P($G(^DPT(DFN,0)),"^",1) W:$E(IOST,1,2)="C-" @IOF W !?(80-$L(X)\2),X
