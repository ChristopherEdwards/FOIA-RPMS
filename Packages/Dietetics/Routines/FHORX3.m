FHORX3 ; HISC/REL/NCA - List Dietetic Events ;2/23/01  09:17
 ;;5.0;Dietetics;**2,19,21,27,30**;Oct 11, 1995
 S Z=$G(^FH(119.8,DA,0)) Q:Z=""  S D1=$P(Z,"^",2),DFN=$P(Z,"^",3),ADM=$P(Z,"^",4),TYP=$P(Z,"^",5),ACT=$P(Z,"^",6),FHORD=$P(Z,"^",7),TXT=$P(Z,"^",8),CLK=$P(Z,"^",9)
 I "DITOS"[TYP,'FHORD Q
...
T ; Tubefeed
 S EVT=""
 F K1=0:0 S K1=$O(^FHPT(DFN,"A",ADM,"TF",FHORD,"P",K1)) Q:K1<1  S X3=$G(^(K1,0)),TUN=$P(X3,"^",1),XX=$G(^FH(118.2,TUN,0)) D CALC S:EVT'="" EVT=EVT_", " S EVT=EVT_P2_" "_$P(XX,"^",1)
