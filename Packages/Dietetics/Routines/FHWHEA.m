FHWHEA ; HISC/REL - Health Summary ;7/16/96  15:47
 ...
 F FHADM=0:0 S FHADM=$O(^FHPT(DFN,"A",FHADM)) Q:FHADM'>0  D CHK
 I GMTSNDM'>0 G KIL
...
CHK ;
 ...
 S FHP="" F FHI=FHS1:0 S FHI=$O(^FHPT(DFN,"A",FHADM,"AC",FHI)) Q:FHI=""!(FHI>FHS2)  I $D(^(FHI,0)) S FHX=^(0) D DI S ^UTILITY($J,"DI",(FH9-FHI),0)=FHX,FHN1=FHN1+1 S:FHP $P(^UTILITY($J,"DI",FHP,0),"^",2)=FHI S FHP=FH9-FHI
 ; Tubefeeding in inverse order
 F FHI=0:0 S FHI=$O(^FHPT(DFN,"A",FHADM,"TF",FHI)) Q:FHI=""  I $D(^(FHI,0)) S FHX=^(0) D TF I FHX S ^UTILITY($J,"TF",(FH9-FHX1),0)=FHX,FHN2=FHN2+1
 ; Supplemental feeding in inverse order
 F FHI=0:0 S FHI=$O(^FHPT(DFN,"A",FHADM,"SF",FHI)) Q:FHI=""  I $D(^(FHI,0)) S FHX=^(0) D SF I FHX S ^UTILITY($J,"SF",FH9-FHX1,0)=FHX,FHN3=FHN3+1
 Q
DI ; Decode Diet Order
 S FHX=^FHPT(DFN,"A",FHADM,"DI",$P(FHX,"^",2),0),FHX2=$G(^(1)),FHX3=""
...
TF ; Decode Tubefeeding
 ...
 S %=$O(^FHPT(DFN,"A",FHADM,"TF",FHI,"P",0)) S:% %=^(%,0)
