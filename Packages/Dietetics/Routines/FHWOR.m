...
EN(MSG) ; Entry Point for OE/RR 3 and pass MSG in FHMSG
 N ACT,ADM,BID,COM,DFN,EDT,FHPV,FHMSG,FHWF,NOW,SDT,CHK,DA,DATA,DATE,DIET,DUR,FHC,FHD,FHD1,FHD2,FOR,FTYP,IEN,ITVL,LP,MEAL,NAM,PER,PID,SERV,TIM,TIME,TM,TXT,TYPC,WARD,X,XX,YR
 ...
 I '$D(^FHPT(DFN,"A",ADM,0)) I '$D(^DGPM(ADM,0)) S TXT="Admission not found" D GETOR G ERR
 I '$D(^FHPT(DFN,"A",ADM,0)) S DA=DFN D ^FHWADM
 ; Check PV1
...
PURGE ; Purge OE/RR Orders
 ...
 I FTYP="A" S:$P($G(^FHPT(DFN,"A",ADM,"OO",FHDR,0)),"^",8)=FHORN $P(^FHPT(DFN,"A",ADM,"OO",FHDR,0),"^",8)=""
 I FTYP="D" S:$P($G(^FHPT(DFN,"A",ADM,"DI",FHDR,0)),"^",14)=FHORN $P(^FHPT(DFN,"A",ADM,"DI",FHDR,0),"^",14)=""
 I FTYP="E" S SDT=$P(FILL,";",4),EDT=$P(FILL,";",5) S:EDT<SDT EDT=SDT D
 .F EL=SDT\1:0 S EL=$O(^FHPT(DFN,"A",ADM,"EL",EL)) Q:EL<1!(EL>EDT)  D
 ..S:$P($G(^FHPT(DFN,"A",ADM,"EL",EL,0)),"^",7)=FHORN $P(^FHPT(DFN,"A",ADM,"EL",EL,0),"^",7)=""
 ..Q
 .Q
 I FTYP="N" S:$P($G(^FHPT(DFN,"A",ADM,"DI",FHDR,0)),"^",14)=FHORN $P(^FHPT(DFN,"A",ADM,"DI",FHDR,0),"^",14)=""
 I FTYP="T" S:$P($G(^FHPT(DFN,"A",ADM,"TF",FHDR,0)),"^",14)=FHORN $P(^FHPT(DFN,"A",ADM,"TF",FHDR,0),"^",14)=""
 I FTYP="I" S:$P($G(^FHPT(DFN,"A",ADM,0)),"^",13)=FHORN $P(^FHPT(DFN,"A",ADM,0),"^",13)=""
 K EL,FHDR
...
GETOR ; Call to Get FHORN
 D GETOR^FHWORR Q
KIL ; Kill Variables
 K ACT,ADM,BID,COM,DFN,EDT,FHPV,FHMSG,FHWF,NOW,SDT,CHK,DA,DATA,DATE,DIET,DUR,FHC,FHD,FHD1,FHD2,FOR,FTYP,IEN,ITVL,LP,MEAL,NAM,PER,PID,SERV,TIM,TIME,TM,TXT,TYPC,WARD,X,XX,YR Q
