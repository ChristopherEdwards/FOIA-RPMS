...
DIS ; Process Cancel of Diet/NPO for Discharge
 S FHGET=$G(^FHPT(DFN,"A",ADM,"DI",FHORD,0))
 I $P(FHGET,"^",14)>0,$P(FHGET,"^",15)>2 S $P(^FHPT(DFN,"A",ADM,"DI",FHORD,0),"^",15)=1
 D NOW^%DTC S NOW=%
 F A1=NOW:0 S A1=$O(^FHPT(DFN,"A",ADM,"AC",A1)) Q:A1=""  K ^FHPT(DFN,"A",ADM,"AC",A1)
 F FHDR=0:0 S FHDR=$O(^FHPT(DFN,"A",ADM,"DI",FHDR)) Q:FHDR<1  D D1
 S FHGET=$G(^FHPT(DFN,"A",ADM,0)),FHGET=$P(FHGET,"^",2) Q:'FHGET
 S FHX=$G(^FHPT(DFN,"A",ADM,"DI",FHGET,0))
 Q:$P(FHX,"^",7)="X"
 D ORD^FHORD7 S ^FHPT(DFN,"A",ADM,"DI",FHORD,0)=FHORD_"^^^^^^X^^"_NOW_"^^"_DUZ_"^"_NOW
 S ^FHPT(DFN,"A",ADM,"AC",NOW,0)=NOW_"^"_FHORD
 S $P(^FHPT(DFN,"A",ADM,0),"^",2,3)=FHORD_"^" K A1,FHDR,FHGET,FHX,FHOX1 Q
D1 ; Get all filler fields for Diet
 S FHOX1=$P($G(^FHPT(DFN,"A",ADM,"DI",FHDR,0)),"^",14,15)
 I +FHOX1>0,$P(FHOX1,"^",2)>2 S FHOX1=+FHOX1,$P(^FHPT(DFN,"A",ADM,"DI",FHDR,0),"^",15)=1
 Q
