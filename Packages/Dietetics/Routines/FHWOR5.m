FHWOR5 ; HISC/NCA - HL7 Tubefeeding ;10/10/00  14:57
 ...
 ; Process TF
 S FHTF=$P($G(^FHPT(DFN,"A",ADM,0)),"^",4) I FHTF D ORCAN
 D ^FHORT11 S $P(^FHPT(DFN,"A",ADM,"TF",TF,0),"^",14)=+FHORN
...
ORCAN ; Cancel Tubefeeding when Order Comes From OE/RR
 ...
 S $P(^FHPT(DFN,"A",ADM,"TF",FHTF,0),"^",11,12)=NOW_"^"_DUZ
 S FHORN=$P(^FHPT(DFN,"A",ADM,"TF",FHTF,0),"^",14)
 S FHSAV=$G(^FHPT(DFN,"A",ADM,"TF",FHTF,0))
...
CAN ; Process Cancel/Discontinue from OE/RR
 ...
 D GADM^FHWORR
 I '$D(^FHPT(DFN,"A",+ADM,"TF",+TF,0)) D CSEND^FHWOR Q
 D NOW^%DTC S NOW=%
 I $P($G(^FHPT(DFN,"A",ADM,0)),"^",4)'=TF D CSEND^FHWOR Q
 I $P($G(^FHPT(DFN,"A",ADM,0)),"^",4)="" D CSEND^FHWOR Q
 S $P(^FHPT(DFN,"A",ADM,0),"^",4)="" K ^FHPT("ADTF",DFN,ADM)
 ...
 D CSEND^FHWOR
 D CUR^FHORD7 Q:Y=""
 S FHDFN=DFN,FHCHK=$$RESUME^FHWORR(FHDFN) I 'FHCHK K FHDFN,FHRDER Q
...
RES ; Resume Current Tray Service From OE/RR.
 ...
 S A2=0 F KK=0:0 S KK=$O(^FHPT(DFN,"A",ADM,"AC",KK)) Q:KK<1!(KK'<NOW)  I $P(^(KK,0),"^",2)=FHORD S A2=KK
 I 'A2 Q
 S DT=$P(NOW,".",1),KK=A2,OLD=FHLD D T0^FHORD3
 K %,%H,%I,A1,A2,C,D1,D2,D3,DA,KK,P2,FHDU,NOW,X1,X2,OLD,FHDR,FHORD,FHPAR,FHLD,FHWF,FHPV,FHOR,I,K9,ADM,ALL,COM,TFCOM,DFN,FHD,POP,PID,BID,DTP,QUA,STR,T,TF,TF2,TUN,WARD,X,X9,Y
 Q
TF ; Code Tubefeeding
...
NA ; OE/RR Number Assign
 ...
 G:'+FHORN KIL
 S $P(^FHPT(DFN,"A",ADM,"TF",TF,0),"^",14)=+FHORN
