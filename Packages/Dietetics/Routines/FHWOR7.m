...
SF ; Get Current Supplemental Feeding order and store in ^TMP($J,"FHSF",1).
 ...
 S FHNO=$P($G(^FHPT(DFN,"A",FHADM,0)),"^",7) G:'FHNO TF
 S FHX=$G(^FHPT(DFN,"A",FHADM,"SF",FHNO,0)),FHX1=$P(FHX,"^",2)
...
TF ; Get Tubefeeding total Kcal/Day and store in ^TMP($J,"FHTF",1).
 S FHNO=$P($G(^FHPT(DFN,"A",FHADM,0)),"^",4) G:'FHNO FP
 S FHX=$G(^FHPT(DFN,"A",FHADM,"TF",FHNO,0)) S FHX=$P(FHX,"^",7)
 S ^TMP($J,"FHTF",1)=FHX
FP ; Get Food Preferences and store all Likes in ^TMP($J,"FHFP","L")
 ...
 S (FHD,FHL)=0 F FHNO=0:0 S FHNO=$O(^FHPT(DFN,"P",FHNO)) Q:FHNO<1  S FHX=$G(^(FHNO,0)) D SP
 G KIL
