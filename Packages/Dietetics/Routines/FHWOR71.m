...
F0 ; Display Diet
 ...
 I '$D(^FHPT(DFN,"A",ADM,0)) S FHQ="-1^No current diet order2" Q
 K N S P1=1 F K=0:0 S K=$O(^FHPT(DFN,"P",K)) Q:K<1  S X=^(K,0),M=$P(X,"^",2) I M'="" S:M="A" M="BNE" D SP
...
F1 D CUR^FHORD7 S FHX=$$SPACNG(1,FHX) S ^TMP($J,"FHPROF",DFN,FHX)="Current Diet: "_$S(Y'="":Y,1:"No current order")
 I Y'="",FHORD>0 I $D(^FHPT(DFN,"A",ADM,"DI",FHORD,1)) S COM=^(1) D
 ...
 I COM'="" S FHX=$$SPACNG(0,FHX) S ^TMP($J,"FHPROF",DFN,FHX)="Comment: "_COM
F2 S NO=$P(X(0),"^",7),Y=$S('NO:"",1:^FHPT(DFN,"A",ADM,"SF",NO,0))
