...
ALL	W !!,"Verifying Patient data base ^FHPT for ALL Patients & Admissions",!
	F DFN=0:0 S DFN=$O(^FHPT(DFN)) Q:DFN<1  D PAT F ADM=0:0 S ADM=$O(^FHPT(DFN,"A",ADM)) Q:ADM<1  D ADM
	Q
INP	W !!,"Verifying Patient data base ^FHPT for Current Inpatients Only",!
	S WRD="" F M=0:0 S WRD=$O(^DPT("CN",WRD)) Q:WRD=""  F DFN=0:0 S DFN=$O(^DPT("CN",WRD,DFN)) Q:DFN<1  D PAT S ADM=^DPT("CN",WRD,DFN) Q:ADM<1  D ADM
	Q
PAT	S ERR=0
	F K=0:0 S K=$O(^FHPT(DFN,"P",K)) Q:K<1  S X=+^(K,0) I '$D(^FH(115.2,X,0))#2 W !,$P(^DPT(DFN,0),"^",1)," - missing Food Preference ",X
	F K=0:0 S K=$O(^FHPT(DFN,"N",K)) Q:K<1  S X=^(K,0) D P1
	F K=0:0 S K=$O(^FHPT(DFN,"S",K)) Q:K<1  S X=$P(^(K,0),"^",2) I X,'$D(^FH(115.4,X,0))#2 W !,$P(^DPT(DFN,0),"^",1)," - missing Nutrition Status ",X
	Q
P1	S N=$P(X,"^",19) I N,'$D(^FH(115.4,N,0))#2 W !,$P(^DPT(DFN,0),"^",1)," - missing Nutrition Status ",N
	...
	Q
ADM	S ERR=0
	S N=$P(^FHPT(DFN,"A",ADM,0),"^",10) I N,'$D(^FH(119.4,N,0))#2 D HDR W !?5,"Isolation ",N," missing."
	S N=$P(^FHPT(DFN,"A",ADM,0),"^",8) I N,'$D(^FH(119.6,N,0))#2 D HDR W !?5,"Dietetic Ward ",N," missing."
	F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"DI",K)) Q:K<1  S X=^(K,0) D DI
	F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"SF",K)) Q:K<1  S X=^(K,0) D SF
	F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"TF",K)) Q:K<1  F L=0:0 S L=$O(^FHPT(DFN,"A",ADM,"TF",K,"P",L)) Q:L<1  S N=+^(L,0) D TF
	F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"SP",K)) Q:K<1  S N=+$P(^(K,0),"^",2) D SP
	F K=0:0 S K=$O(^FHPT(DFN,"A",ADM,"DR",K)) Q:K<1  S N=+$P(^(K,0),"^",2) D DR
	Q
