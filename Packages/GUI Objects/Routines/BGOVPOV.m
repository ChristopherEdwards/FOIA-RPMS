BGOVPOV ; IHS/BAO/TMD - Visit POV maintenance ;24-Oct-2013 15:54;PLS
 ;;1.1;BGO COMPONENTS;**1,3,4,5,6,7,10,11,12**;Mar 20, 2007;Build 5
...
GETICD(RET,INP) ;EP
 ...
 .I $$AICD^BGOUTL2 S IEN=$$ICDDX^ICDEX(ICD,CDT)
 .E  S IEN=$$ICDDX^ICDCODE(ICD,CDT)
...
GETCODE(RET,INP) ;EP
 ...
 I $$CSVACT^BGOUTL2("ICDCODE") D
 .;IHS/MSC/MGH Patch 12 changes
 .I $$AICD^BGOUTL2 S RET=$$ICDDX^ICDEX(ICDIEN,CDT)
 .E  S RET=$$ICDDX^ICDCODE(ICDIEN,CDT)
...
GET(RET,INP) ;EP
 ...
 ..;IHS/MSC/MGH Patch 12 changes
 ..I $$AICD^BGOUTL2 D
 ...S ICD=$P($$ICDDX^ICDEX(POV,VDATE),U,2)
 ...S ICDNAME=$P($$ICDDX^ICDEX(POV,VDATE),U,4)
 ..E  D
 ...S ICD=$P($G(^ICD9(POV,0)),U)
 ...S ICDNAME=$P($G(^(0)),U,3)
 ..Q:ICD=""
 ...
 ..I $$AICD^BGOUTL2 D
 ...S:ICAU ICAU=$P($$ICDDX^ICDEX(ICAU,VDATE),U,4)_"~"_ICAU
 ..E  D
 ...
 ...I ASTHMA=1 S CONTROL=$$ACONTROL^BGOASLK(DFN,VIEN) I CONTROL=""  S CONTROL="NONE RECORDED"
...
CHKICD(ICDIEN,ACTDT) ;EP
 ...
 I $$CSVACT^BGOUTL2("ICDCODE") D
 .;IHS/MSC/MGH Patch 12
 .I $$AICD^BGOUTL2 S X=$$ICDDX^ICDEX(ICDIEN,ACTDT)
 .E  S X=$$ICDDX^ICDCODE(ICDIEN,ACTDT)
...
SET(RET,INP) ;EP
 ...
 S TYPE=$S($E(TYPE)="`":$E(TYPE,2,99),$$AICD^BGOUTL2:$P($$ICDDX^ICDEX(TYPE,$$NOW^XLFDT),U,1),1:$O(^ICD9("AB",TYPE_$S($$CSVACT^BGOUTL2():" ",1:""),0)))
 ...
 . I CONTROL'="" D ASTHMA(DFN,CONTROL,VIEN)
...
CHECK(RET,INP) ;EP
 ...
 Q:RET<0
 ;IHS/MSC/MGH Patch 12
 I $$AICD^BGOUTL2 S X=$P($$ICDDX^ICDEX(ICDIEN,$$NOW^XLFDT),U,2)
 E  S X=$G(^ICD9(ICDIEN,0))
 I $P(X,U,10)'="",$P(X,U,10)'=$P(^DPT(DFN,0),U,2) S RET=$$ERR^BGOUTL(1095)
 E  S RET=""
 Q
 ; Return 0recent POV's by patient or by visit
...
RECBYVIS S VPOV=0
 ...
 Q
RECBUILD N REC,POV,ICD,ICDNAME,VIEN,F,FAC,FACNAM,VSIT,VDATE,PNAR,ONSET,CONTROL
 ...
 .S ICDNAME=$P($$ICDDX^ICDEX(POV,VDATE),U,4)
 E  D
 .S ICD=$P($$ICDDX^ICDCODE(POV,VDATE),U,2)
 .S ICDNAME=$P($$ICDDX^ICDCODE(POV,VDATE),U,4)
 ;Q:'ICD
 Q:ICDNAME=""
 ...
 .I ASTHMA=1 S CONTROL=$$ACONTROL^BGOASLK(DFN,VIEN) I CONTROL=""  S CONTROL="NONE RECORDED"
 S CNT=CNT+1
 S @RET@(CNT)=VDATE_U_FAC_U_FACNAM_U_ICD_U_ICDNAME_U_PNAR_U_VPOV_U_VIEN_U_POV_U_$$ISLOCKED^BEHOENCX(VIEN)_U_ONSET_U_CONTROL
 Q
