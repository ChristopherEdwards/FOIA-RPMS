GMTSADH5 ; SLC/DCM,KER - Health Summary Ad Hoc RPC's ;20-Sep-2012 11:05;DU
 ;;2.7;Health Summary;**36,35,37,49,63,1005**;Oct 20, 1995;Build 12
 ;IHS/MSC/MGH Modified for CPT selected component
 ;
 ; External References
 ;   DBIA  1268  ^AUTTHF(
 ;   DBIA  1268  ^AUTTHF("B"
...
FILESEL(GMTSRT,GMTSFI,GMTSFM,DIR) ; Get file entries
 ...
 ;IHS/MSC/MGH Uncommented out file for CPT selected, changed X ref from C to BA and return code first
 I GMTSFI=81 D  Q
 . F  Q:GMTSC'<GMTSCNT  S GMTSI=$O(^ICPT("BA",GMTSI),DIR) Q:GMTSI=""  S GMTSJ=0 F  S GMTSJ=$O(^ICPT("BA",GMTSI,GMTSJ)) Q:'GMTSJ   I $D(^ICPT(GMTSJ,0)) S X=^(0) D
 . . S GMTSC=GMTSC+1,^TMP("ORDATA",$J,1,GMTSC)=GMTSJ_"^"_$P(X,"^",1)_"   ["_$P(X,"^",2)_"]"
