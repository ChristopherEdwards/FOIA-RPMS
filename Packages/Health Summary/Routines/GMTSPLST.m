GMTSPLST ; SLC/JER,KER,TC - Problem List ;23-Jan-2014 10:31;DU
 ;;2.7;Health Summary;**28,35,52,86,1006**;Oct 20, 1995;Build 3
 ;
 ; External References
 ;   DBIA 10011  ^DIWP
...
WRT ;   Writes Problem List Component,X
 ...
 ;
 ;     ^TMP("GMPLHS",$J,#,#,"ICD9")      Other multiple mapped ICD-CM codes
 ;     ^TMP("GMPLHS",$J,#,"L")           Lexicon Term
 ;     ^TMP("GMPLHS",$J,#,"N")           Provider Narrative
 ;     ^TMP("GMPLHS",$J,#,"C",LOC,#,0))  Comments
 ;
 N GMREC,GMTSICL,GMTAB,LINE1,DIWL,GMTSTXTF,EXT,SNO
 ...
 . S:$L(GMDIAC) GMDIAS="(ICD-CM "_GMDIAC_")"
 ...
 . . S GMDIAS=$S($L(GMSCT):"(SCT ",1:"(VHAT ")_GMSCT_") (ICD-CM "_GMDIAC
...
QUAL ;Qualifiers ae displayed if there are any
 N GMR,NODE,DATE,X,UCNT,CNT,CMT,T,I S LOC="",CNT=0
 S (CNT,UCNT)=0
 S GMR=0 F  S GMR=$O(^TMP("GMPLHS",$J,GMREC,"Q",GMR)) Q:+GMR'>0  D  Q:$D(GMTSQIT)
 . S UCNT=1
 . S NODE=$G(^TMP("GMPLHS",$J,GMREC,"Q",GMR)) Q:NODE']""
 . S CMT=$P(NODE,U) I $L($G(CMT)) D LC2
 Q
...
LC2 ;   List Comments (unnumbered and numbered)
 K ^UTILITY($J,"W") D:UCNT CF(CNT,CMT) D:'UCNT CF(0,CMT)
 I $D(^UTILITY($J,"W",0)) N I,T S I=0 F  S I=$O(^UTILITY($J,"W",0,I)) Q:+I=0  D
 . S T=$$RT($G(^UTILITY($J,"W",0,I,0))) Q:'$L(T)  D CKP^GMTSUP Q:$D(GMTSQIT)  D:GMTSNPG SUBHDR2 W:I=1 ?3 W:I>1 ?7 W $G(T),!
 Q
