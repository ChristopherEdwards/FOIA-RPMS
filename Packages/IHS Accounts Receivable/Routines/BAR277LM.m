BAR277LM ; IHS/SD/LSL - LIST MANAGER API'S ;
 ;;1.8;IHS ACCOUNTS RECEIVABLE;;OCT 26, 2005
 ;
 ; Routine created by cmi/flag/maw for V1.6 Patch 3
 ;
 ; ********************************************************************
 ; cmi/flag/maw modified from XBLM for 276/277 Project
 ; Documentation APIs for BAR276LM Generic Display.
 ; This utility uses the Veterans Administration List Manager
 ; (VALM).
 ;
 ; APIs
 ;
 ; FILE^BAR277LM("Directory","File Name")
 ;     Displays file indicated.
 ;
 ; SFILE^BAR277LM
 ;     Selection of host file for display.
 ;
 ; VIEWR^BAR277LM("TAG^ROUTINE","Header")
 ;     Displays printout of the routine. (non - FM, using IO)
 ;
 ; VIEWD^BAR277LM("Tag^Routine","Header")
 ;     Displays printout of the routine. (FM - using EN1^DIP)
 ;
 ; DIQ^BAR277LM("DIC","DA")
 ;     Displays EN1^DIQ for the DIC,DA.
 ;
 ; ARRAY^BAR277LM("array(","Header")
 ;     Displays the array(..,n,0) (%RCR notation)
 ;
 ; >>GUI<<                                      
 ;
 ; GUIR^BAR277LM("TAG^ROUTINE","root(")
 ;     Returns the hard coded output in the array specified.
 ;     "(" not required.
 ;
 ; GUID^BAR277LM("TAG^ROUTINE","root(")
 ...
 ;
EN(BAR277PR) ;EP -- main entry point for BAR277 RESPONSE DISPLAY
 D EN^VALM(BAR277PR)
 Q
 ;
HDR ;EP -- header code
...
EXPND ;EP -- expand code
 Q
 ;
FILE(XBDIR,XBFN) ;EP - pull up a file into the TMP global for display
 ...
 .Q
 D EN^BAR277LM("BAR EDI 277 RESPONSE")
 KILL ^TMP("XBLM",$J,XBNODE)
EFILE ;
 Q
 ;
SFILE ;EP - Select a host file for display.
OPEN ;
 S IOP="HOME"
...
ESFILE ;
 ...
 Q
 ;
VIEWR(XBROU,XBHDR) ;EP ** USING XBROU print to a host file for viewing
 I '$D(XBHDR) S XBHDR=""
 ...
 Q
 ;
GUIR(XBROU,XBY) ;EP - give routine and target array
 Q:$L(XBY)=0
 ;
 S XBGUI=1
 D VIEWR^XBLM(XBROU,"")
 ...
 Q
 ;
GUID(XBROU,XBY) ;EP give routine and target array for FM prints
 Q:$L(XBY)=0
 S:XBY["(" XBY=$P(XBY,"(")
 S XBGUI=1
 D VIEWD^XBLM(XBROU,"")
 ...
 Q
 ;
VIEWD(XBROU,XBHDR) ;EP ** USING XBROU print to a host file for viewing
 S:'$D(XBHDR) XBHDR=""
...
DIQ(DIC,DA) ;EP - Display DIC and DA after call to EN^DIQ
 ...
 Q
 ;
ARRAY(XBAR,XBHDR) ;EP  Display an array that has (...,n,0) structure
 I '$D(XBHDR) S XBHDR=""
 NEW Y,X,I,XBNODE
 S XBNODE=$G(XQORS)+1
 KILL ^TMP("XBLM",$J,XBNODE)
