BAR50P04 ; IHS/SD/LSL - MATCH REASONS AND CLAIMS ; 01/09/2009
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**5,6,10,20,21,22,23**;OCT 26, 2005;Build 38
 ;;OCT 2012 P.OTTIS HEAT #87149 FIXING LINE +210
 ;;NOV 2012 P.OTTIS HEAT#82698 LEADING ZEROES IN BILL #
 ;;DEC 2012 FIX INIT VALUE OF CLMDA (+27)
...
INDEX ;EP
 ...
 . S CLMDA=0
 . F  S CLMDA=$O(^XTMP("BAR-LIST",$J,DUZ(2),BARBL,CLMDA)) Q:'CLMDA  K ERRS D CLMFLG(CLMDA,.ERRORS)  Q:$G(QFLG)=1  ;
 Q:QFLG=1  ;
 S BARFLG=$$EN^BAR50P0Z(IMPDA)  ;PLB/Pymt Rev/Neg pymt amt chks  ; REQ4
 D:$$IHS^BARUFUT(DUZ(2)) NEGBAL^BAR50EB(IMPDA,"ERA")  ;CHK FOR NEG BAL IN RPMS BILLS  ; REQ4
 ;;;D:$$IHSERA^BARUFUT(DUZ(2)) NEGBAL^BAR50EB(IMPDA,"ERA")  ;CHK FOR NEG BAL IN RPMS BILLS  ; REQ4
 ...
 ;;;D:$$IHSERA^BARUFUT(DUZ(2)) NONPAYCH^BAR50EP1(IMPDA)  ;CHK FOR PYMTS NOT MATCHED TO REV TO SEE IF BEING POSTED TO 'NONPYMT' BATCH ;BAR*1.8*6 SAC RTN TOO BIG
 K ERRORS
 Q
CLMFLG(CLMDA,ERRORS) ;EP - 
 ...
 S BARTMPM=$$OVERIDE^BAR50EP1(CLMDA) I BARTMPM D  Q  ;P.OTT
 ...
 I TRNAME["HIPAA" D
 . ;;;I $G(BARDBG) W !,"PERFORMING TRADITIONAL HIPAA CHECKS FOR CLAIM ",$P($G(^BAREDI("I",DUZ(2),IMPDA,30,CLMDA,0)),U)  ; REQ4
...
SETREA ;EP SET CAT & REA INTO E-CLM
 ...
 D ^DIE
 Q
HIPAAREA(IMPDA,CLMDA,ERRORS) ; ERRORS CAN BE MULTIPLE
...
HIPAACLM(IMPDA,CLMDA,ERRORS) ;BAR*1.8*5 SRS-80 TPF ERRORS CAN BE MULTIPLE
 ...
 S BARTEST=$$GET1^DIQ(90056.0205,BAREIENS,.01) ;full string ERA BILL#
 ...
 I $G(BARDBG) ;I BARMMFLG=1 D INS^BAR50DET(" PARTIAL MATCH FOUND FOR BILL "_BARBILL_": DOS "_BARMMVAL,0) D NOMATCH^BAR50DET ;INDICATE NOT MATCH
 I $G(BARDBG) ;I BARMMFLG=2 D INS^BAR50DET(" PARTIAL MATCH FOUND FOR BILL "_BARBILL_": AMOUNT "_BARMMVAL,0) D NOMATCH^BAR50DET ;INDICATE NOT MATCH
 I BARFND=0 D
 ...
 . ;S BARMSG=" MATCHED TO A/R BILL "_$P($G(^BARBL(DUZ(2),BARBIEN,0)),U) W " ",BARMSG
 . ;I $G(BARDBG) I '$D(BARX) D INS^BAR50DET(BARMSG,1)
 ;
 ;TO p.ott
...
CLM2 ;new tag bar*1.8*20 REQ4
 ;
 S NEWSTAT="" ;init value 12/12/2013  
 ...
 I BAR3PIEN]"" D   ;
 . ;;;I '$$ERAALOW2^BARUFUT(DUZ(2)) Q  ;NON IHS RESTRICT ERA POSTING OF 'Cancelled in 3P' P.OTT
