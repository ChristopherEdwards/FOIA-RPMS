...
EN(TRDA,IMPDA)  ;EP ;SCAN CLMS BUILT "B" STATUS
 ...
 F INDEX="B","X","C","R" D INDEX,PRT^BAR50DET ; BAR 1.8*23
 Q
INDEX ;EP
 S QFLG=0  ;
 W !,"Processing Claim Status using claim Index ",INDEX,!  ; REQ4
 S BARDBG=1 ; - BAR 1.8*23
 ...
 ;;;old code D:$$IHS^BARUFUT(DUZ(2)) NEGBAL^BAR50EB(IMPDA,"ERA")  ;CHK FOR NEG BAL IN RPMS BILLS  ; REQ4  - BAR 1.8*24
 D NEGBAL^BAR50EB(IMPDA,"ERA")  ;note: IHS/TRIBAL CHK INSIDE BAR50EB CHK FOR NEG BAL IN RPMS BILLS HEAT147572 MOD: 2/5/2014  - BAR 1.8*24
 ;NOTE: MATCHED PYMTS & CREDITS W/HAVE THE 'POST THIS CLM AS TYPE' FLD = TO 20=PYMT CREDIT
 ;;;old code D:$$IHS^BARUFUT(DUZ(2)) NONPAYCH^BAR50EP1(IMPDA)  ;CHK FOR PYMTS NOT MATCHED TO REV TO SEE IF BEING POSTED TO 'NONPYMT' BATCH ;BAR*1.8*6 SAC RTN TOO BIG
 D:$$IHSNEGB^BARUFUT(DUZ(2)) NONPAYCH^BAR50EP1(IMPDA)  ;CHK FOR PYMTS NOT MATCHED TO REV TO SEE IF BEING POSTED TO 'NONPYMT' BATCH ;BAR*1.8*6 SAC RTN TOO BIG
 K ERRORS
 Q
CLMFLG(CLMDA,ERRORS) ;EP - 
 ...
 . ;;;I $G(BARDBG) W " 1ST CHK  - SKIP"  - BAR 1.8*23
 ...
 S BARTMPM=$$OVERIDE^BAR50EP1(CLMDA) I BARTMPM D  Q  ; - BAR 1.8*24
 ...
 I $D(ERRORS("CT")) K ERRORS S ERRORS("CT")=""  ; REQ5 old code - BAR 1.8*24
...
HIPAACLM(IMPDA,CLMDA,ERRORS) ;BAR*1.8*5 SRS-80 TPF ERRORS CAN BE MULTIPLE
 ...
 S (BARBIEN,BARBILL)=$$GETBBILL(BARTEST) ;ENDS WITH FIRST NON ALPHANUMERIC CHAR  - BAR 1.8*23
 I BARTEST="" D
 . S CLMTYP="CF"
