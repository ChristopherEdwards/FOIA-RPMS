...
AUTOREV ;EP - AUTO REVIEW ;BAR*1.8*6 SCR120 ADD LIST MANAGER PROMPT FOR AUTO-REVIEW
 ...
 ;.K ERRORS
 ;.D CLMFLG^BAR50P04(CLMDA,.ERRORS)
 ;D:$$IHS^BARUFUT(DUZ(2)) NEGBAL^BAR50EB(IMPDA,"ERA")  ;CHECK FOR NEGATIVE BALANCE IN RPMS BILLS
 ...
 ;D:$$IHS^BARUFUT(DUZ(2)) REVLOOP^BAR50EP(IMPDA)  ;CHECK FOR PAYMENTS NOT MATCHED TO A REVERSAL TO SEE IF THEY ARE BEING POSTED TO A 'NONPAYMENT' BATCH  ;bar*1.8*20 REQ4
 ;END REVERSAL MOD
 ;D:$$IHS^BARUFUT(DUZ(2)) NONPAYCH^BAR50EP(IMPDA)  ;CHECK FOR PAYMENTS NOT MATCHED TO A REVERSAL TO SEE IF THEY ARE BEING POSTED TO A 'NONPAYMENT' BATCH
 D:$$IHS^BARUFUT(DUZ(2)) NONPAYCH^BAR50EP1(IMPDA)  ;CHECK FOR PAYMENTS NOT MATCHED TO A REVERSAL TO SEE IF THEY ARE BEING POSTED TO A 'NONPAYMENT' BATCH ;BAR*1.8*6 SAC RTN TOO BIG
 D GATHER(IMPDA)
 Q
 ;start new code bar*1.8*20 REQ5
VIEW ;EP
