...
SELI ;SELECT INSURANCE TYPE
 ...
 D ^DIC
 Q:$D(DUOUT)
 ;S:$G(Y(0)) BARDCI=$P(^BAR(90053.03,Y(0),0),U)  ;bar*1.8*22 SDR
 S:$G(Y(0)) BARDCI=$$GET1^DIQ(90053.03,Y(0),".01","E")  ;bar*1.8*22 SDR
 I $G(BARDCI)["NON-BEN" S BARDCI="PATIENT"  ;non-bens are listed under PATIENT in ^TMP("BARDM",$J) global  ;bar*1.8*22 SDR
 K DIC,DA
 Q
 ;
SELA ;SELECT ACCOUNT
...
SELD ;SELECT DETAIL
 ...
 I Y>0 S BARSEQ=Y D
 .D RRDT^BARDMU
 .;S L=$P(^TMP("BARDM",$J,BARSEQ),U),BARDIT=$P(^(BARSEQ),U,2),BARDI=$P(^(BARSEQ),U,3)  ;bar*1.8*22 SDR
 .S BARDL=$P(^TMP("BARDM",$J,BARSEQ),U),BARDIT=$P(^(BARSEQ),U,2),BARDI=$P(^(BARSEQ),U,3)  ;bar*1.8*22 SDR
 .D HDR2,DET
 K DIR
 Q
PRINT ;
 D HDR
 ;I BARCY="A" F L="CYCLE 1","CYCLE 2","CYCLE 3","CYCLE 4" Q:$G(BARDLQ)  D TOT  ;bar*1.8*22 SDR
 I BARCY="A" F BARDL="CYCLE 1","CYCLE 2","CYCLE 3","CYCLE 4" Q:$G(BARDLQ)  D TOT  ;bar*1.8*22 SDR
 ;E  S L=BARCYN D TOT  ;bar*1.8*22 SDR
...
HDR2 ;
 ...
 ;W @IOF,!,BARRDT,?23,"LETTERS IN THE QUEUE READY TO PRINT",?70,"PAGE: ",PG  ;bar*1.8*22 SDR
 W @IOF,!,BARRDT,?23,"LETTERS IN THE QUEUE READY TO PRINT",?70,"PAGE: ",BARPG  ;bar*1.8*22 SDR
 W !!,"A/R PARENT LOCATION: ",BARPSAT(DUZ(2),.01)
 ;W !,L  ;bar*1.8*22 SDR
 W ?65,BARDL  ;bar*1.8*22 SDR
 ;W !,"A/R ACCOUNT: ",BARDI,?62,"PERIOD: ",BARPCD($P(^TMP("BARDM",$J,BARSEQ),U,4))," Days"  ;bar*1.8*22 SDR
 ...
 Q
TOT ;PRINT TOTALS
 ;I $P($G(^TMP("BARDM",$J,L)),U)=0 W !!,L," Does not contain any queued Letters" S BMCQ=1 Q  ;bar*1.8*22 SDR
 I $P($G(^TMP("BARDM",$J,BARDL)),U)=0 W !!,BARDL," Does not contain any queued Letters" D  S BMCQ=1 Q  ;bar*1.8*22 SDR P.OTT PAUSE
 . K DIR S (X,Y)=""
 . S DIR(0)="E"
 ...
 . K DIR
 . QUIT
 ;S BARDIT=0  F  S BARDIT=$O(^TMP("BARDM",$J,L,BARDIT)) Q:BARDIT=""  D  Q:$G(BARDLQ)  ;bar*1.8*22 SDR
 S BARDIT=0  F  S BARDIT=$O(^TMP("BARDM",$J,BARDL,BARDIT)) Q:BARDIT=""  D  Q:$G(BARDLQ)  ;bar*1.8*22 SDR
 .;S BARDI=0 F  S BARDI=$O(^TMP("BARDM",$J,L,BARDIT,BARDI)) Q:BARDI=""  D  Q:$G(BARDLQ)  ;BAR*1.8*22 SDR
 .S BARDI=0 F  S BARDI=$O(^TMP("BARDM",$J,BARDL,BARDIT,BARDI)) Q:BARDI=""  D  Q:$G(BARDLQ)  ;BAR*1.8*22 SDR
 ..I $D(BARDCA) Q:BARDI'=BARDCA
 ..I $D(BARDCI) Q:BARDIT'=BARDCI
 ..S BARSEQ=BARSEQ+1,BARDITI=BARDIT_"/"_BARDI
 ..;W !,BARSEQ,?5,L,?13,$E(BARDITI,1,42),?55,$J($P(^TMP("BARDM",$J,L,BARDIT,BARDI),U),7)  ;bar*1.8*22 SDR
 ..W !,BARSEQ,?5,BARDL,?13,$E(BARDITI,1,42),?55,$J($P(^TMP("BARDM",$J,BARDL,BARDIT,BARDI),U),7)  ;bar*1.8*22 SDR
 ..;S X=$P(^TMP("BARDM",$J,L,BARDIT,BARDI),U,3) D COMMA^%DTC W ?66,$J(X,12)  ;bar*1.8*22 SDR
 ..S X=$P(^TMP("BARDM",$J,BARDL,BARDIT,BARDI),U,3) D COMMA^%DTC W ?66,$J(X,12)  ;bar*1.8*22 SDR
 ..;S ^TMP("BARDM",$J,BARSEQ)=L_U_BARDIT_U_BARDI_U_$E(L,7)  ;bar*1.8*22 SDR
 ..S ^TMP("BARDM",$J,BARSEQ)=BARDL_U_BARDIT_U_BARDI_U_$E(L,7)  ;bar*1.8*22 SDR
 ..I $Y>(IOSL-6) W ! D RTRN^BARDMU Q:$G(BARDLQ)  D HDR
 Q
DET ;DETAIL
 S BARDLQ=""
 ;start old code bar*1.8*22 SDR
 ;S BARBIL=0  F  S BARBIL=$O(^TMP("BARDM",$J,L,BARDIT,BARDI,BARBIL)) Q:BARBIL=""  D  Q:$G(BARDLQ)
 ;.S DFN=$P(^TMP("BARDM",$J,L,BARDIT,BARDI,BARBIL),U),BARHRN=$P(^AUPNPAT(DFN,41,DUZ(2),0),U,2),BARPAT=$P(^DPT(DFN,0),U)
 ;.S BARDT=$P(^TMP("BARDM",$J,L,BARDIT,BARDI,BARBIL),U,2)
 ;.S BARSDT=$E(BARDT,4,5)_"/"_$E(BARDT,6,7)_"/"_$E(BARDT,2,3)
 ;.S BARDT=$P(^TMP("BARDM",$J,L,BARDIT,BARDI,BARBIL),U,3)
 ;.S BARBDT=$E(BARDT,4,5)_"/"_$E(BARDT,6,7)_"/"_$E(BARDT,2,3)
 ;.W !,BARHRN,?7,BARBIL,?16,$E(BARPAT,1,24),?40,BARSDT,?49,BARBDT
 ;.S X=$P(^TMP("BARDM",$J,L,BARDIT,BARDI,BARBIL),U,4) W ?58,$J(X,10,2) ;D COMMA^%DTC W 
 ;.S X=$P(^TMP("BARDM",$J,L,BARDIT,BARDI,BARBIL),U,5) W ?70,$J(X,10,2) ;D COMMA^%DTC W ?60,X
 ;.I $Y>(IOSL-6) W ! D RTRN^BARDMU Q:$G(BARDLQ)  D HDR2
 ;Q:$G(BARDLQ)
 ;W !,?58,"----------",?70,"----------"
 ;S TMP=^TMP("BARDM",$J,L,BARDIT,BARDI)
