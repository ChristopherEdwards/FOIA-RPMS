...
CALC ;EP;START OF SET OF ARRAY
 ...
 .;S ^TMP("BARDM",$J,L)="0^0^0"  ;bar*1.8*22 SDR
...
A1 ; 
 S BARDM=0
 F  S BARDM=$O(^BARDM(DUZ(2),"S","Q",BARL,BARDM)) Q:BARDM'?1N.N  D  Q:BARCTQ
 .Q:$P(^BARDM(DUZ(2),BARDM,0),U,2)'="A"  ;bar*1.8*22 SDR
 .S BARDMC=0
 .F  S BARDMC=$O(^BARDM(DUZ(2),"S","Q",BARL,BARDM,BARDMC)) Q:BARDMC'?1N.N  D  Q:BARCTQ
 ...
 ..;S $P(^TMP("BARDM",$J,BARL),U)=$P(^TMP("BARDM",$J,BARL),U)+1  ;bar*1.8*22 SDR
 ...
 ..S ^TMP("BARDM",$J,"DMIR-DET",BARL,BARDIT,BARDI,BARDM)=""  ;bar*1.8*22 SDR
 Q
...
CYDAY ;EP;CYCLE DAY TEST
 ...
 ;S R=0
 ;S R=$O(^BARDM(DUZ(2),BARDM,100,"B","CYCLE "_(CY-1),R))
 ;S X2=$P(^BARDM(DUZ(2),BARDM,100,R,0),U,5)
 ...
 ;I (X+1)>(BARPCD(CY)-BARPCD(CY-1)) S BARLQ=1  ;bar*1.8*11 SDR
 I (X+1)>(BARPCD(CY)-BARPCD(CY-1)) S BARLQ=1  ;bar*1.8*11 SDR
 Q
