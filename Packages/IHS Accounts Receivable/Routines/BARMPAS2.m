BARMPAS2 ; IHS/SD/LSL - Patient Account Statement Print ;
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**2,4,19,20,23**;OCT 26, 2005
 ...
 ;P.OTTIS HEAT #58041 BUG FIX IN SUMMARY BY DAYS DUE
...
PRINT ; EP
 ...
 K BARF1
 S BARHOLD=DUZ(2)
 S DUZ(2)=0 F  S DUZ(2)=$O(^XTMP("BARPAS"_BARRUNDT,DUZ(2))) Q:'+DUZ(2)  D
 . S BARDUMMY="" F  S BARDUMMY=$O(^XTMP("BARPAS"_BARRUNDT,DUZ(2),0,BARDUMMY)) Q:BARDUMMY=""  D
...
AGE(BARACDA) ; EP
 ; AGE PAST BILLS
 ; P.OTTIS HEAT #58041 BUG FIX IN SUMMARY BY DAYS DUE
 NEW BARTMP ;PLACEHOLDER FOR CURRENT DUZ(2)
 K BARAGE,BARBL
 S (BARAGE)=0
 F I=0:1:3 S BARAGE(I)=0  ; set up Age array
 S BARTMP=DUZ(2)
 S DUZ(2)=0 F  S DUZ(2)=$O(^XTMP("BARPAS"_BARRUNDT,DUZ(2))) Q:DUZ(2)'>0  D  ;
