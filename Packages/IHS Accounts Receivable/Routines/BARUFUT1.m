BARUFUT1 ; IHS/SD/TPF - UTILITIES 2 FOR UFMS ; 04/28/2008
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**3,4,5,6,13,21,23**;OCT 26, 2005
 ;;
 ;Modify patch number in filename so batches are formatted
 ;correctly by the HUB
 ;MAR 2012 P.OTTIS HEAT62222  ADDED GETDAY0(FORM)
 ;APR 2013 P.OTTIS HEAT89920: MAKE ENA/DIS ABLING CASH SESSION DEPENDEDNT ON DUZ(2)
 Q
 ;
...
GETDISLM(FORM) ;EP - GET 'UFMS DISPLAY DATE LIMIT'
 ...
 Q DISLIM
 ;
GETDAY0(FORM) ;EP - GET 'BOOKING DATE IN UFMS' ("DAY ZERO") ;P.OTTIS HEAT # 62222 MAR 2012
 N DISLIM
 S FORM=$G(FORM)
 S:FORM="" FORM="E"
...
SITECHK ;EP - CHECK DUZ(2) FOR SITE SET UP
 ...
 . D EOP^BARUTL(1)
 Q
OUTOFORD(PARAM,OPTNAME) ;EP - PLACE OPTION OUT OF ORDER ;P.OTT
 Q:PARAM="NO"!(PARAM="") 0
 N OPTIEN,BARMSG,BARXREF
 S BARXREF="XXX"
 S OPTIEN=$O(^DIC(19,"B",OPTNAME,""))
 S $P(^DIC(19,BARXREF,DUZ(2),OPTIEN,0),U,3)="Option is disabled for this site"
 ;W !,"Cashiering menu options are now disabled for A/R Location: ",$P($G(^DIC(4,DUZ(2),0)),U) ;9/18/13
 Q OPTIEN
 ;
ENABLORD(PARAM,OPTNAME) ;EP - ENABLE OPTION CASH SESS P.OTT
 Q:PARAM="YES" 0
 N OPTIEN,BARXREF
 S BARXREF="XXX"
 S OPTIEN=$O(^DIC(19,"B",OPTNAME,""))
...
CSMENU(BARDUZ) ;
 NEW BARRET
 S BARRET=1
 I '$$ISENACS(BARDUZ) S BARRET=0 W !!,"Out of order:  Option is disabled for ",$P($G(^DIC(4,BARDUZ,0)),U),!! ;9/18/13
 Q BARRET
