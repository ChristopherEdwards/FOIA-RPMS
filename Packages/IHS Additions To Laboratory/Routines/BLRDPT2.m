BLRDPT2 ; IHS/DIR/FJE - ESTABLISH PATIENT VARIABLES 3/23/88 9:13 PM ;
 ;;5.2;BLR;;NOV 01, 1997
 ;
 ;;MAS VERSION 5.0;
5 ; -- INP call
 S (VAWD,VATS,VADX,VAPP,VARM)="" D NOW^%DTC S VANOW=% K VAMV,VAMV0
 ...
 ;
 G:'$D(VAMV0) DONE
 S (VAPRT,VAPRC,VACN)=1 D GET^BLRDPT30
 S VAMV0=^DGPM(VAMV,0),VAMVT=$P(VAMV0,"^",4),VACA=$P(VAMV0,"^",14),VACA0=$S($D(^DGPM(+VACA,0)):^(0),1:"")
 ;
 ; set: adm ifn(1) ; doctor(2) ; tr spec(3) ; ward(4) ; room(5)
 S @VAV@($P(VAS,"^",1))=VACA,@VAV@($P(VAS,"^",2))=VAPP,@VAV@($P(VAS,"^",3))=VATS,@VAV@($P(VAS,"^",4))=VAWD,@VAV@($P(VAS,"^",5))=$P(VARM,"^",2)
 ;
 ...
 ;
DONE K VAID,VANOW,VACA,VACA0,VAMV,VAMV0,VATD,VAMT,VAMVT D KVAR^BLRDPT30 Q
 ;
IB ;In-Bed status
...
CHK ; -- check if mvt exists and if 'while asih' type d/c
 ...
 I "^42^47^"[("^"_$P(VAMV0,"^",18)_"^"),$P(VAMV0,"^",22)'=2,$O(^DGPM("APMV",DFN,+$P(VAMV0,"^",14),VAID)),$O(^($O(^(VAID)),0)),$D(^DGPM($O(^(0)),0)),"^13^44^"[("^"_$P(^(0),"^",18)_"^") S VATD=VAID K VAMV,VAMV0
...
ADM ; -- send back adm ifn for dfn on vaindt or now
 ...
 F I=0:0 S VAID=$O(^DGPM("ATID1",DFN,VAID)) Q:'VAID  S VAMV=+$O(^DGPM("ATID1",DFN,VAID,0)) I $D(^DGPM(VAMV,0)) S VAMV0=^(0),VAMV1=$S($D(^DGPM(+$P(VAMV0,"^",17),0)):^(0),1:9999999.999999) I VAMV0'>VADT,VAMV1>VADT S VADMVT=VAMV Q
