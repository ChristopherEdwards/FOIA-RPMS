...
4 ;;Primary Eligibility ID Reset (All Patients)
 ...
 G Q4:%'=1
41 S VAOPT=4 D TASK^BLRDPT61 G Q4
QUE4 K VALL D BEG^BLRDPT61,ALL,END^BLRDPT61
Q4 Q
 ;
...
PAT ; -- entry point if DFN is defined
 F VAELG=0:0 S VAELG=$O(^DPT(DFN,"E",VAELG)) Q:'VAELG  I $D(^(VAELG,0)),$D(^DIC(8,VAELG,0)) W:'$D(VABATCH) !?5,"...",$P(^(0),U) D IX I '$D(VABATCH) D ASK^BLRDPT61 W ?40,$P(^DPT(DFN,"E",VAELG,0),U,3)_" / "_$P(^(0),U,4)
Q6 K DFN,VAELG
 Q
 ;
7 ;;Reset ALL ID's for ALL Patients
 ...
 G Q7:%'=1
 S VAOPT=7 D TASK^BLRDPT61 G Q7
QUE7 S VALL="" D BEG^BLRDPT61,ALL,END^BLRDPT61
Q7 K VALL
 Q
 ;
FILE ;
