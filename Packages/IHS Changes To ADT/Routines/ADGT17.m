ADGT17 ; GENERATED FROM 'ADGPM SPECIALTY TRANSFER' INPUT TEMPLATE(#1599), FILE 405;05/04/99
 D DE G BEGIN
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(2)=% S %=$P(%Z,U,4) S:%]"" DE(5)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="ADGT17",DQ=1
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=1599,U="^"
...
C2S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 I "^1^3^"[("^"_$P(^DGPM(DA,0),"^",2)_"^") S A1B2TAG="ADM" D ^A1B2XFR
 Q
X2 S %DT="T" D ^%DT K %DT S X=Y K:Y<1 X I $D(X),'$D(DGPMT) W !?3,*7,"USE BED CONTROL MOVEMENT OPTIONS!" K X
 Q
 ;
3 S DQ=4 ;@10
4 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=4 D X4 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X4 D ONLY^DGPMV36 S:'$D(DGPMSPI) Y="@20"
 Q
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW="0;4",DV="R*P405.1'X",DU="",DLB="TYPE OF MOVEMENT",DIFLD=.04
 S DE(DW)="C5^ADGT17"
 S DU="DG(405.1,"
 S X=DGPMSPI
 S Y=X
...
X9 S ^DISV(DUZ,"^DG(405.1,")=$S($D(^DISV(DUZ,"DGPM6")):^("DGPM6"),1:"")
 Q
10 D:$D(DG)>9 F^DIE17 G ^ADGT171
