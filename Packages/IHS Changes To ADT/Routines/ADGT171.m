...
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,4) S:%]"" DE(1)=% S %=$P(%Z,U,8) S:%]"" DE(6)=% S %=$P(%Z,U,9) S:%]"" DE(4)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="ADGT171",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="0;4",DV="R*P405.1'X",DU="",DLB="SPECIALTY TRANSFER TYPE",DIFLD=.04
 S DE(DW)="C1^ADGT171"
 S DU="DG(405.1,"
 G RE
...
C1S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X1 S DIC("S")="I $D(DGPMT),($P(^(0),""^"",2)=DGPMT),$P(^(0),""^"",4) S DGER=0,DGPMTYP=$P(^(0),""^"",3) D:DGPMT<4!(DGPMT=6) @(""DICS^DGPMV3""_DGPMT) I 'DGER" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=2 D X2 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X2 S ^DISV(DUZ,"DGPM6")=$S($D(^DISV(DUZ,"^DG(405.1,")):^("^DG(405.1,"),1:"")
 Q
3 S DQ=4 ;@30
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="0;9",DV="R*P45.7'",DU="",DLB="FACILITY TREATING SPECIALTY",DIFLD=.09
 S DE(DW)="C4^ADGT171"
 S DU="DIC(45.7,"
 G RE
...
C4S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X4 S DIC("S")="I $P(^DIC(45.7,+Y,9999999),U,3)=""Y""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
5 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=5 D X5 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X5 K DIE("NO^")
 Q
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="0;8",DV="*P200'X",DU="",DLB="ADMITTING PHYSICIAN",DIFLD=.08
 S DE(DW)="C6^ADGT171"
 S DU="VA(200,"
 G RE
...
C6S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X6 S DIC("S")="I $D(^XUSEC(""PROVIDER"",+Y)),$S('$D(^VA(200,+Y,""PS"")):0,'$P(^(""PS""),U,8):1,+$P(^(""PS""),U,8)>+^DGPM(DA,0):1,1:0)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
7 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=7 D X7 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X7 S:$D(DGPMBYP) Y="@40"
 Q
8 D:$D(DG)>9 F^DIE17 G ^ADGT172
