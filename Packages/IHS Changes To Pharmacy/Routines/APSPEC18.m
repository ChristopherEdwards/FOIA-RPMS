APSPEC18 ;IHS/CIA/PLS - APSP ENVIRONMENT CHECK ROUTINE ;05-Jun-2014 10:44;DU
 ;;7.0;IHS PHARMACY MODIFICATIONS;**1018**;Sep 23, 2004;Build 21
 ;
ENV ;EP
 ;
...
MES(TXT,QUIT) ;EP
 ...
 ;
PRE ;EP - Pre-init
 ;repurposing field 405 and setting to disabled
 N LP
 S LP=0 F  S LP=$O(^APSPCTRL(LP)) Q:'LP  D
 .S:$P($G(^APSPCTRL(LP,4)),U,5) $P(^APSPCTRL(LP,4),U,5)=0
 D NDEL^XPAR("SYS","APSP POV CACHE")
 Q
...
REMXPAR(PAR) ;Remove values stored for a given parameter
 ...
 Q
POST ;EP
 ;Restate APSP POV CACHE parameter
 D UPDPAR("SYS","APSP POV CACHE",,0)
 Q
