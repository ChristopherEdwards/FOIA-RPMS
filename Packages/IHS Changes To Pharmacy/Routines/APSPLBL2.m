APSPLBL2 ;BIR/SAB-LABEL OUTPUT CONT. ;02-Feb-2004 08:35;PLS
 ...
 ; Modified - IHS/CIA/PLS - 02/01/04 - Copy of PSOLBL2
...
SIG K OT S SGY="" F P=1:1:$L(SIG," ") S X=$P(SIG," ",P) D:X]""
 ...
 .S SGY=SGY_X_" "
 S X="",SGC=1 F J=1:1 S Z=$P(SGY," ",J) S:Z="" SGY(SGC)=X Q:Z=""  S:$L(X)+$L(Z)'<$S($P(PSOPAR,"^",28):46,1:32) SGY(SGC)=X,SGC=SGC+1,X="" S X=X_Z_" "
SIGOLD I '$P(PSOPAR,"^",28) D  K NHC
 .K DIC,DR,DIQ,NHC S DIC=2,DA=DFN,DR=148,DIQ="NHC",DIQ(0)="I"
 .D EN^DIQ1 K DIC,DR,DIQ
 .; Modified - IHS/CIA/PLS - 02/01/04 - Call sig formatter if label length defined
 .I $P(%APSITE,U,4) D SIGPH
 .E  D
...
SIGPH ;S SIGPH=SIG,X="",SGCPH=1 F J=1:1:100 S Z=$P(SIGPH," ",J) S:Z="" SIGPH(SGCPH)=X S:$L(X)+$L(Z)'<34 SIGPH(SGCPH)=X,SGCPH=SGCPH+1,X="" S X=X_Z_" "
 S SIGPH=SIG,X="",SGCPH=1 F J=1:1:100 S Z=$P(SIGPH," ",J) S:Z="" SIGPH(SGCPH)=X S:$L(X)+$L(Z)'<$P(%APSITE,U,4) SIGPH(SGCPH)=X,SGCPH=SGCPH+1,X="" S X=X_Z_" "
 Q
