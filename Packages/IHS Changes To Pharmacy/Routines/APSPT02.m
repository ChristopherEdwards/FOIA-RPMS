APSPT02 ; GENERATED FROM 'APSP DUE STUDY EDIT' INPUT TEMPLATE(#674), FILE 9009032.1;02/14/03
 D DE G BEGIN
...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="APSPT02",DQ=1
 S DR(99,1,9.2)="S DIC=9009032.2,DIC(0)=""EQLAM"",DIC(""S"")=""I $D(^APSPDUE(32.2,""""AC"""",I(0,0),Y))"" D ^DIC K DIADD S D0=+Y,DIC(.02)=I(0,0),DIH=9009032.2 D DICL^DICR:$P(Y,U,3) K DIC"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=674,U="^"
...
X7 S APSPDUEC=1
 Q
8 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=8 S I(0,0)=$S($D(D0):D0,1:"") X DR(99,1,9.2) S X=$S(D0>0:D0,1:""),D(0)=X S D0=I(0,0) S X=$S(D(0)>0:D(0),1:"")
 S DGO="^APSPT021",DC="^9009032.2^APSPDUE(32.2," G DIEZ^DIE0
R8 D DE G A
 ;
9 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=9 D X9 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X9 I $D(APSPFLG) S Y="@1"
 Q
10 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=10 D X10 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X10 K APSPFLG,APSPDUEC
 Q
11 G 0^DIE17
