APSPT081 ; IHS/DSD/ENM - NO DESCRIPTION PROVIDED ; [ 09/03/97 1:30 PM ]
 ;;6.0;IHS PHARMACY MODIFICATIONS;;09/03/97
 D DE G BEGIN
DE S DIE="^APSPDUE(32,",DIC=DIE,DP=9009032,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^APSPDUE(32,DA,-1))<0
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,5) S:%]"" DE(1)=% S %=$P(%Z,U,6) S:%]"" DE(2)=% S %=$P(%Z,U,7) S:%]"" DE(3)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="APSPT081",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="0;5",DV="F",DU="",DLB="RX #",DIFLD=.05
...
X1 K:$L(X)>11!($L(X)<1) X
 ...
 Q
 ;
2 S DW="0;6",DV="RP6'",DU="",DLB="PROVIDER",DIFLD=.06
 S DU="DIC(6,"
 ...
 G Z:X,RD
X2 Q
3 S DW="0;7",DV="R*P3'",DU="",DLB="PHARMACIST",DIFLD=.07
 S DU="DIC(3,"
 S X=DUZ
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z:X,RD
X3 S DIC("S")="I $D(^XUSEC(""PSRPH"",Y))" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
4 G 0^DIE17
