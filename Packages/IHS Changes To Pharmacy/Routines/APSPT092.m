APSPT092 ; IHS/DSD/ENM - NO DESCRIPTION PROVIDED ; [ 09/03/97 1:30 PM ]
 ;;6.0;IHS PHARMACY MODIFICATIONS;;09/03/97
 D DE G BEGIN
DE S DIE="^APSPP(31,D0,15,",DIC=DIE,DP=9009031.15,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^APSPP(31,D0,15,DA,-1))<0
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="APSPT092",DQ=1+D G B
1 S DW="0;1",DV="M*P9009031.2",DU="",DLB="LOCATION",DIFLD=.01
 S DE(DW)="C1^APSPT092"
 S DU="APSPP(31.2,"
 G RE:'D S DQ=2 G 2
C1 G C1S:$D(DE(1))[0 K DB S X=DE(1),DIC=DIE
 K ^APSPP(31,DA(1),15,"B",$E(X,1,30),DA)
C1S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 S ^APSPP(31,DA(1),15,"B",$E(X,1,30),DA)=""
 Q
X1 S DIC("S")="I $S('$D(^APSPP(31.2,Y,""I"")):1,1:0)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;2",DV="NJ3,0",DU="",DLB="NO. OF PREPACKS ISSUED",DIFLD=.02
 G RE
