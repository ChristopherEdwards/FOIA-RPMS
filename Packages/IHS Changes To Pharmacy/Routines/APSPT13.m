APSPT13 ; GENERATED FROM 'APSP PREPACK LOG EXPIRATION' PRINT TEMPLATE (#491) ; 02/14/03 ; (FILE 9009031, MARGIN=132)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(491,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
