APSPT14 ; GENERATED FROM 'APSP PREPACK LOG LOCATION LIST' PRINT TEMPLATE (#493) ; 02/14/03 ; (FILE 9009031, MARGIN=132)
 G BEGIN
CP G CP^DIO2
C S DQ(C)=Y
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(493,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
...
A1 ;
 ...
 D N:$X>112 Q:'DN  W ?112 X ^DD(9009031.15,.03,9.2) S Y(9009031.15,.03,2)=X S X=$P(Y(9009031.15,.03,3),U,2),Y=X,X=Y(9009031.15,.03,2),X=X*Y W:X'?."*" $J(X,20,2) K Y(9009031.15,.03) S Y=X,C=2 D A:Y'?."*"
 Q
A1R ;
 K Y
 Q
