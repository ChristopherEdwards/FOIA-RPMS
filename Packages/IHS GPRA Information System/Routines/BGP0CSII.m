BGP0CSII ; IHS/CMI/LAB - DISPLAY IND LISTS ;
...
EN ;EP -- main entry point for CMS PT LIST DISPLAY
 D EN^VALM("BGP 10 CMS MEASURE PT LISTS")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ; -- init variables and list array
 K BGPPLST S BGPHIGH=""
 S (X,Y,Z,C)=0 F  S X=$O(^BGPCMSMT("AO",BGPXX,X)) Q:X'=+X  S Y=$O(^BGPCMSMT("AO",BGPXX,X,Y)) Q:Y'=+Y  D
 .S C=C+1,BGPPLST(C,0)=C_")",$E(BGPPLST(C,0),5)=$P(^BGPCMSMT(Y,0),U,3),BGPPLST(C,C)=Y I $D(BGPPLSTL(BGPXX,Y)) S BGPPLST(C,0)="*"_BGPPLST(C,0)
 .Q
 S (VALMCNT,BGPHIGH)=C
 Q
 ;
