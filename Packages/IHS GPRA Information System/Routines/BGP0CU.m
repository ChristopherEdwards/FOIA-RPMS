BGP0CU ; IHS/CMI/LAB - calc CMS measures 26 Sep 2004 11:28 AM 04 May 2009 2:38 PM ;
...
CHESTXRY(P,BDATE,EDATE,BGPY) ;EP
 ...
 ...I $$ICD^ATXCHK($P(^AUPNVCPT(X,0),U),T,1) S C=C+1,BGPY(ED,C)=$$DATE^BGP0UTL(9999999-$P(ED,"."))_"  "_$P($$CPT^ICPTCOD($P(^AUPNVCPT(X,0),U),(9999999-$P(ED,"."))),U,2)_"  "_$P($$CPT^ICPTCOD($P(^AUPNVCPT(X,0),U),(9999999-$P(ED,"."))),U,3)
 ..;now go through v rads
 ..S X=0 F  S X=$O(^AUPNVRAD("AD",V,X)) Q:X'=+X  D
 ...S I=$P(^AUPNVRAD(X,0),U) Q:I=""  S I=$P($G(^RAMIS(71,I,0)),U,9) Q:I=""
 ...I $$ICD^ATXCHK(I,T,1) S C=C+1,BGPY(ED,C)=$$DATE^BGP0UTL(9999999-$P(ED,"."))_"  "_$P($$CPT^ICPTCOD(I,(9999999-$P(ED,"."))),U,2)_"  "_$P($$CPT^ICPTCOD(I,(9999999-$P(ED,"."))),U,3)_" Impression: "_$P($G(^AUPNVRAD(X,11)),U,1)
 ..; now v tran
 ..S X=0 F  S X=$O(^AUPNVTC("AD",V,X)) Q:X'=+X  D
 ...S I=$P(^AUPNVTC(X,0),U,7)
 ...Q:I=""
 ...I $$ICD^ATXCHK(I,T,1) S C=C+1,BGPY(ED,C)=$$DATE^BGP0UTL(9999999-$P(ED,"."))_"  "_$P($$CPT^ICPTCOD(I,(9999999-$P(ED,"."))),U,2)_"  "_$P($$CPT^ICPTCOD(I,(9999999-$P(ED,"."))),U,3)
 ..;now check V PROCEDURE
 ..S T=$O(^ATXAX("B","BGP CMS CHEST XRAY PROC",0))
 ..I 'T W BGPBOMB
 ..S X=0 F  S X=$O(^AUPNVPRC("AD",V,X)) Q:X'=+X  D
 ...I $$ICD^ATXCHK($P(^AUPNVPRC(X,0),U),T,0) S C=C+1,BGPY(ED,C)=$$DATE^BGP0UTL(9999999-$P(ED,"."))_"  "_$P($$ICDOP^ICDCODE($P(^AUPNVPRC(X,0),U),(9999999-$P(ED,"."))),U,2)_"  "_$$VAL^XBDIQ1(9000010.08,X,.04)
 ..;now check V POV
 ..S X=0 F  S X=$O(^AUPNVPOV("AD",V,X)) Q:X'=+X  D
 ...S I=$P($G(^AUPNVPOV(X,0)),U)
 ...Q:'I
 ...I $P($$ICDDX^ICDCODE(I),U,2)="V72.5",$$UP^XLFSTR($$VAL^XBDIQ1(9000010.07,X,.04))["CHEST" S C=C+1,BGPY(ED,C)=$$DATE^BGP0UTL(9999999-$P(ED,"."))_"  "_$P($$ICDDX^ICDCODE(I,(9999999-$P(ED,"."))),U,2)_"  "_$$VAL^XBDIQ1(9000010.07,X,.04)
 ...Q
 ..Q
 .Q
 Q
...
ERPNEU(P,BDATE,EDATE,T) ;EP - did patient have an er visit from bdate to edate without a DX in taxonomy T?
 ...
 .I 'E S G=0_U_"ER Visit: "_$$DATE^BGP0UTL($P($P(^AUPNVSIT(V,0),U),"."))
 I E Q "1^No"
 Q G
 ;
PNEUMODX(V) ;EP
...
LVADEX(P,BD,ED) ;EP
 NEW X
 S X=$$LASTPRC^BGP0UTL1(P,"BGP LVAD/HEART TRANSPLANT PROC",BD,ED)
 Q X
 ;
HFDX(V) ;EP
 NEW C,T
...
COMFORT(P,BDATE,EDATE) ;EP - any V66.7 on this visit? or during hospital stay?
 NEW X
 S X=$$LASTDXI^BGP0UTL1(DFN,"V66.7",BDATE,EDATE)
 I X="" Q ""
 Q $P(X,U,2)_"  "_$$DATE^BGP0UTL($P(X,U,3))
 ;
...
GETMEDS(P,BGPMBD,BGPMED,TAX1,TAX2,TAX3,EXP,ADM,BGPDNAME,BGPC,LAST) ;EP
 ...
 ..I $P(^AUPNVMED(Y,0),U,8)]"" S N=N_"  D/C "_$$DATE^BGP0UTL($P(^AUPNVMED(Y,0),U,8))
...
DSCHINST(P,BDATE,EDATE) ;EP - discharge instructions
 ...
 .I E="HF-DCHL" S %=$P(^AUTTEDT(T,0),U,1)_"  "_$$DATE^BGP0UTL($P(BGPG(X),U)) Q
 Q %
