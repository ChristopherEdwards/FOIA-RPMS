BGP0D2 ; IHS/CMI/LAB - measure 1,2,3,4 ;
...
I1 ;EP - measure 1 general processing
 ...
 S BGPVALUE="UP|||"_$$DATE^BGP0UTL($P(BGPG(1),U))_" "_$P(BGPG(1),U,2)
 Q
 ;
I3 ;EP
...
DMGC ;EP - called from elder care 
 ...
 S BGPXPTV=$P(BGPLHGB,U,4)
 S BGPVALUE=$S(BGPDMD1:"UP",1:"")_$S(BGPDMD2:",AD",1:"")_$S(BGPDMD3:",AAD",1:"")_"||| "_$$DATE^BGP0UTL($P(BGPLHGB,U,3))_" "_$P(BGPLHGB,U,4)
...
DMBP ;EP - called from elder care
 ...
 I BGPVALUE="" S BGPBP=$$BPCPT^BGP0D22(DFN,BGPBDATE,BGPEDATE) I BGPBP]"" S BGPN1=1 D  G BPX
...
BPX ;
 S BGPXPTV=$P(BGPVALUE,U,1),BGPXPTV=$P(BGPXPTV," ",1)
...
DMLDL ;EP - called from elder care
 ...
 ;I $P(BGPLP,U) S BGPVALUE=BGPVALUE_$S($P(BGPLP,U):"LP: "_$$DATE^BGP0UTL($P(BGPLP,U,2))_" "_$P(BGPLP,U,3),1:"")
 ;I '$P(BGPLP,U),BGPTRI,BGPHDL,BGPLDL S BGPVALUE=BGPVALUE_"TRI:"_$$DATE^BGP0UTL($P(BGPTRI,U,2))_" HDL:"_$$DATE^BGP0UTL($P(BGPHDL,U,2))_" LDL:"_$$DATE^BGP0UTL($P(BGPLDL,U,2))
 I $P(BGPVALUE,"|||",2)]"" S BGPVALUE=BGPVALUE_" ; "
 I BGPLDL]"" S BGPVALUE=BGPVALUE_"LDL DONE: "_$$DATE^BGP0UTL($P(BGPLDL,U,2))_" "_$P(BGPLDL,U,3)
...
HGBA1C(P,BDATE,EDATE) ;EP
 ...
 ...Q
 S %="",E=+$$CODEN^ICPTCOD("3044F"),%=$$CPTI^BGP0DU(P,BDATE,EDATE,E)
 I %]"" S BGPC=BGPC+1,BGPT(9999999-$P(%,U,2),BGPC)="CPT 3044F"
 S %="",E=+$$CODEN^ICPTCOD("3044F"),%=$$TRANI^BGP0DU(P,BDATE,EDATE,E)
...
HCPT ;
 S %=""
 S %="",E=+$$CODEN^ICPTCOD(83036),%=$$CPTI^BGP0DU(P,BDATE,EDATE,E)
 I %]"" S BGPC=BGPC+1,BGPT(9999999-$P(%,U,2),BGPC)="CPT 83036"
 S %="",E=+$$CODEN^ICPTCOD(83036),%=$$TRANI^BGP0DU(P,BDATE,EDATE,E)
 I %]"" S BGPC=BGPC+1,BGPT(9999999-$P(%,U,2),BGPC)="CPT/TRAN 83036"
 S %="",E=+$$CODEN^ICPTCOD(83037),%=$$CPTI^BGP0DU(P,BDATE,EDATE,E)
 I %]"" S BGPC=BGPC+1,BGPT(9999999-$P(%,U,2),BGPC)="CPT 83037"
 S %="",E=+$$CODEN^ICPTCOD(83037),%=$$TRANI^BGP0DU(P,BDATE,EDATE,E)
 I %]"" S BGPC=BGPC+1,BGPT(9999999-$P(%,U,2),BGPC)="CPT/TRAN 83037"
 S %="",E=+$$CODEN^ICPTCOD("3045F"),%=$$CPTI^BGP0DU(P,BDATE,EDATE,E)
 I %]"" S BGPC=BGPC+1,BGPT(9999999-$P(%,U,2),BGPC)="CPT 3045F"
 S %="",E=+$$CODEN^ICPTCOD("3045F"),%=$$TRANI^BGP0DU(P,BDATE,EDATE,E)
 I %]"" S BGPC=BGPC+1,BGPT(9999999-$P(%,U,2),BGPC)="CPT/TRAN 3045F"
 S %="",E=+$$CODEN^ICPTCOD("3046F"),%=$$CPTI^BGP0DU(P,BDATE,EDATE,E)
 I %]"" S BGPC=BGPC+1,BGPT(9999999-$P(%,U,2),BGPC)="CPT 3046F"
 S %="",E=+$$CODEN^ICPTCOD("3046F"),%=$$TRANI^BGP0DU(P,BDATE,EDATE,E)
 I %]"" S BGPC=BGPC+1,BGPT(9999999-$P(%,U,2),BGPC)="CPT/TRAN 3046F"
 S %="",E=+$$CODEN^ICPTCOD("3047F"),%=$$CPTI^BGP0DU(P,BDATE,EDATE,E)
 I %]"" S BGPC=BGPC+1,BGPT(9999999-$P(%,U,2),BGPC)="CPT 3047F"
 S %="",E=+$$CODEN^ICPTCOD("3047F"),%=$$TRANI^BGP0DU(P,BDATE,EDATE,E)
...
LIPID(P,BDATE,EDATE) ;EP
 K BGPC
 S BGPC=0
 S %="",E=+$$CODEN^ICPTCOD(80061),%=$$CPTI^BGP0DU(P,BDATE,EDATE,E)
 I %]"" Q 1_U_$P(%,U,2)_U_"80061"
 S %="",E=+$$CODEN^ICPTCOD(80061),%=$$TRANI^BGP0DU(P,BDATE,EDATE,E)
 I %]"" Q 1_U_$P(%,U,2)_U_"80061"
 ;now get all loinc/taxonomy tests
...
TRIG(P,BDATE,EDATE) ;EP
 K BGPC
 S BGPC=0
 S %="",E=+$$CODEN^ICPTCOD(84478),%=$$CPTI^BGP0DU(P,BDATE,EDATE,E)
 I %]"" Q 1_U_$P(%,U,2)
 S %="",E=+$$CODEN^ICPTCOD(84478),%=$$TRANI^BGP0DU(P,BDATE,EDATE,E)
 I %]"" Q 1_U_$P(%,U,2)
 ;now get all loinc/taxonomy tests
...
LDL(P,BDATE,EDATE,NORES) ;EP
 ...
 S %=$$CPT^BGP0DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP LDL CPTS",0)),5)
 I %]"" S BGPC=BGPC+1,BGPT(9999999-$P(%,U,1),BGPC)="CPT: "_$P(%,U,2)
 S %=$$TRAN^BGP0DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP LDL CPTS",0)),5)
 I %]"" S BGPC=BGPC+1,BGPT(9999999-$P(%,U,1),BGPC)="CPT: "_$P(%,U,2)
 I '$O(BGPT(0)) Q ""
...
BPCPT(P,BDATE,EDATE) ;EP
 NEW G
 S G=$$CPTI^BGP0DU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("3077F"),"",30)
 I G Q $$DATE^BGP0UTL($P(G,U,2))_" CPT 3077F SYSTOLIC BP >=140 UNC^3"
 S G=$$CPTI^BGP0DU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("3080F"),"",30)
 I G Q $$DATE^BGP0UTL($P(G,U,2))_" CPT 3080F DIASTOLIC BP >=90 UNC^3"
 Q ""
