BGP0D61 ; IHS/CMI/LAB - measure 31 01 Jul 2009 7:47 PM ;
...
CRCP ;EP - called from elder
 ...
 S BGPVALUE=""
 I BGPFOB]"" S BGPVALUE="FOB: "_$P(BGPFOB,U)_":"_$$DATE^BGP0UTL($P(BGPFOB,U,2))_" "
...
CRC(P,EDATE) ;EP
 ...
 I T D  I X]"" Q 1
 .S X=$$CPT^BGP0DU(P,$$DOB^AUPNPAT(P),EDATE,T,5) I X]"" Q
 .S X=$$TRAN^BGP0DU(P,$$DOB^AUPNPAT(P),EDATE,T,5)
 S BGPG=$$LASTPRC^BGP0UTL1(P,"BGP TOTAL COLECTOMY PROCS",$$DOB^AUPNPAT(P),EDATE)
 I BGPG Q 1
...
BE(P,EDATE,BDATE) ;EP
 ...
 I T D  I X]"" S BGPLBE="CPT BE "_$P(X,U,2)_U_$$DATE^BGP0UTL($P(X,U,1))_U_$P(X,U,1)
 .S X=$$CPT^BGP0DU(P,BDATE,EDATE,T,5) I X]"" Q
 .S X=$$TRAN^BGP0DU(P,BDATE,EDATE,T,5)
 .Q
 S T=$O(^ATXAX("B","BGP BE CPTS",0))
 I T D  I X]"",$P(BGPLBE,U,3)<X Q "RAD BE"_U_$$DATE^BGP0UTL(X)_U_X
 .S X=$$RAD^BGP0DU(P,BDATE,EDATE,T,3)
 .Q
 Q BGPLBE
 ;
LOINC(A,B) ;
...
REF(P,BDATE,EDATE) ;EP
 ...
 ..S G=$$REFUSAL^BGP0UTL1(P,60,BGPT1,BDATE,EDATE)
 F BGPT=82270,82274,89205,"G0107","G0328","G0394" D  Q:$P(T,U)=1
 .S C=+$$CODEN^ICPTCOD(BGPT) Q:C'>0
 .S T=""
 .S T=$$REFUSAL^BGP0UTL1(P,81,+$$CODEN^ICPTCOD(BGPT),$$FMADD^XLFDT(EDATE,-365),EDATE)
 .I T S T="1^"_$P(T,U,2)_"^ref FOBT cpt"
 I $P(T,U)=1 Q T
 S BGPT=$O(^ATXAX("B","BGP SIG CPTS",0))
 I BGPT S G=$$RADREF^BGP0UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"ref SIG cpt"
 S BGPT=$O(^ATXAX("B","BGP BE CPTS",0))
 I BGPT S G=$$RADREF^BGP0UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"ref DCBE cpt"
 S BGPT=$O(^ATXAX("B","BGP COLO CPTS",0))
 I BGPT S G=$$RADREF^BGP0UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"ref COLO cpt"
 S BGPT=$O(^ATXAX("B","BGP SIG CPTS",0))
 I BGPT S G=$$CPTREFT^BGP0UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"ref SIG cpt"
 S BGPT=$O(^ATXAX("B","BGP BE CPTS",0))
 I BGPT S G=$$CPTREFT^BGP0UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"ref DCBE cpt"
 S BGPT=$O(^ATXAX("B","BGP COLO CPTS",0))
 I BGPT S G=$$CPTREFT^BGP0UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"ref COLO cpt"
 F BGPT=45.24,45.42,45.22,45.23,45.25,45.43 D  Q:$P(T,U)=1
 .S C=+$$CODEN^ICDCODE(BGPT,80.1) Q:C'>0
 .S T=""
 .S T=$$REFUSAL^BGP0UTL1(P,80.1,C,$$FMADD^XLFDT(EDATE,-365),EDATE)
