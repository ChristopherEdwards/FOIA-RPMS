BGP0D721 ; IHS/CMI/LAB - measure AHR.A ;
...
IAHRA ;EP
 ...
 I 'BGPN2 S BGPBETA("ADR")=$$BETAALG1^BGP0D72(DFN,BGPEDATE) I BGPBETA("ADR") S BGPN4=1
 ...
 K BGPACE
 S BGPACE("RX")=$$ACERX^BGP0D722(DFN,BGPBDATE,BGPEDATE,179)  ;get 1_U_# days
 I BGPACE("RX") S BGPN10=1  ;pt has 180 days of ACE
 I 'BGPN10 S BGPACE("REF")=$$ACEREF^BGP0D722(DFN,BGPBDATE,BGPEDATE) I BGPACE("REF") S BGPN11=1
 I 'BGPN10 S BGPACE("CONTRA")=$$ACECONT^BGP0D722(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,BGPBDATE,BGPEDATE,BGPBDATE) I BGPACE("CONTRA") S BGPN12=1 ;beta contraindication
 I 'BGPN10 S BGPACE("ADR")=$$ACEALG^BGP0D722(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE) I BGPACE("ADR") S BGPN12=1
 I (BGPN10+BGPN11+BGPN12) S BGPN9=1
 K BGPSTAT
 S BGPSTAT("RX")=$$STATRX^BGP0D722(DFN,BGPBDATE,BGPEDATE,179)  ;get 1_U_# days
 I BGPSTAT("RX") S BGPN14=1  ;pt has 180 days of STAT
 I 'BGPN14 S BGPSTAT("REF")=$$STATREF^BGP0D722(DFN,BGPBDATE,BGPEDATE) I BGPSTAT("REF") S BGPN15=1
 I 'BGPN14 S BGPSTAT("CONTRA")=$$STATCON^BGP0D722(DFN,BGPBDATE,BGPEDATE,BGPBDATE,BGPEDATE) I BGPSTAT("CONTRA") S BGPN16=1 ;beta contraindication
 I 'BGPN14 S BGPSTAT("ADR")=$$STATALG^BGP0D722(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,BGPBDATE,BGPEDATE) I BGPSTAT("ADR") S BGPN16=1
...
BETACONT(P,BDATE,EDATE,NMIBD,NMIED) ;EP - BETA BLOCKER CONTRAINDICATION
 ...
 S BGPG=$$LASTDX^BGP0UTL1(P,"BGP HYPOTENSION DXS",BDATE,EDATE)
 I $P(BGPG,U)=1 Q 1_U_"hypotension dx contra"  ;has hypotension dx
 S BGPG=$$LASTDX^BGP0UTL1(P,"BGP CMS 2/3 HEART BLOCK DXS",BDATE,EDATE)
 I $P(BGPG,U)=1 Q 1_U_"heart blk contra"  ;has heart block dx
 S BGPG=$$LASTDXI^BGP0UTL1(P,"427.81",BDATE,EDATE)
 ...
 ...S G=1_U_"Beta Blocker contra NMI med "_$$DATE^BGP0UTL(Y)
 ..Q
 .Q
 I G Q G
 ;now cpt 8011 in past year
 S X=$$CPTI^BGP0DU(P,NMIBD,NMIED,+$$CODEN^ICPTCOD("G8011"))
 I X Q "1^Beta Blocker contra CPT code G8011: "_$$DATE^BGP0UTL($P(X,U,2))
 S X=$$TRANI^BGP0DU(P,NMIBD,NMIED,+$$CODEN^ICPTCOD("G8011"))
 I X Q "1^Beta Blocker contra TRAN code G8011: "_$$DATE^BGP0UTL($P(X,U,2))
 Q 0
...
ASAC2 ;now check for dx 459
 K BGPG S BGPG=$$LASTDXI^BGP0UTL1(P,"459.0",$$DOB^AUPNPAT(P),EDATE)
...
ASAALLEG(P,EDATE) ;EP
 K BGPG
 S X=$$ASAALLEG^BGP0CU1(P,$$DOB^AUPNPAT(P),EDATE)
 I 'X Q 0
 Q X
 ;
