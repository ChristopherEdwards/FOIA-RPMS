...
IHEDCWP ;EP
 ...
 S BGPN1=+BGPN
 S BGPVALUE=$S(BGPRTYPE=3:"",1:"UP")_$S(BGPD1:";AC",1:"")_"|||"_$$DATE^BGP0UTL($P(BGPN,U,2))_" "_$P(BGPN,U,3)
...
NEWP ;check V PROCEDURE
 S D=$$LASTPRCI^BGP0UTL1(P,"99.21",BDATE,EDATE)
 Q $P(D,U)
 ;
CANTI(P,BDATE,EDATE) ;
 K ^TMP($J,"A")
...
ACTA(P,EDATE) ;
 ...
 NEW A,B,E,Z,X,D,V,Y,G,M,T,T1
 K BGPMEDS1
 D GETMEDS^BGP0UTL2(P,$$FMADD^XLFDT(EDATE,-365),EDATE,,,,,.BGPMEDS1)
 I '$D(BGPMEDS1) Q 0
 S T1=$O(^ATXAX("B","BGP HEDIS ANTIBIOTICS MEDS",0))
 S T4=$O(^ATXAX("B","BGP HEDIS ANTIBIOTICS NDC",0))
 S (X,G,M,E)=0,D="" F  S X=$O(BGPMEDS1(X)) Q:X'=+X!(D)  S V=$P(BGPMEDS1(X),U,5),Y=+$P(BGPMEDS1(X),U,4) D
...
STREP(P,BDATE,EDATE) ;EP
 K BGPC
 S BGPC=0
 S %=$$CPT^BGP0DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP GROUP A STREP CPT",0)),5)
 I %]"" Q 1_U_%
 S %=$$TRAN^BGP0DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP GROUP A STREP CPT",0)),5)
 I %]"" Q 1_U_%
