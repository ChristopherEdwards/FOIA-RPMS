BGP0DNE ; IHS/CMI/LAB - NATL COMP EXPORT 05 Dec 2006 7:09 PM 07 Mar 2009 2:29 PM ;
...
INTRO ;
 D XIT
 S BGPXPFYC=$O(^BGPCTRL("B","2010",0))
 S X=0 F  S X=$O(^BGPCTRL(BGPXPFYC,54,X)) Q:X'=+X  D
 .I $Y>(IOSL-3) D EOP W @IOF
 .W !,^BGPCTRL(BGPXPFYC,54,X,0)
 D TAXCHK^BGP0XTCN
TP ;
 S BGPRTYPE=1,BGPXPRP=1,BGPXPFYY=2010,BGPXPFYI=309
 S BGPXPBD=2990701,BGPXPTDT=3100630
 W !!,"The date ranges for this report are: ",$$FMTE^XLFDT(BGPXPBD)," to ",?31,$$FMTE^XLFDT(BGPXPTDT)
COMM ;
...
EXPORT ;export to area or not?
 ...
 S BGPFN="CRSCNT"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP0UTL(BGPXPBD)_$$D^BGP0UTL(BGPXPTDT)_$$D^BGP0UTL(BGPNOW)_"_001_of_001.TXT"
LOCALF ;
 S BGPLOCAL=""
...
ZIS ;call to XBDBQUE
 ;CREATE REPORT ENTRY
 K DIC S X=$P(^VA(200,DUZ,0),U)_"-"_$$D^BGP0UTL(BGPNOW),DIC(0)="L",DIC="^BGPXPT(",DLAYGO=90377.11,DIADD=1
 S DIC("DR")=".02////"_BGPXPBD_";.03////"_BGPXPTDT_";.04////"_$P(^ATXAX(BGPTAXI,0),U)_";.05////"_$S(BGPMFITI:$P(^ATXAX(BGPMFITI,0),U),1:"")
 D ^DIC K DIC,DA,DR,DIADD,DLAYGO I Y=-1 S BGPERR="UNABLE TO CREATE REPORT FILE ENTRY!" H 4 D XIT Q
 S BGPXPRPT=+Y
 K ^BGPXPT(BGPXPRPT,13)
 S C=0,X="" F  S X=$O(BGPTAX(X)) Q:X=""  S C=C+1 S ^BGPXPT(BGPXPRPT,13,C,0)=X,^BGPXPT(BGPXPRPT,13,"B",X,C)=""
 S ^BGPXPT(BGPXPRPT,13,0)="^90377.111301A^"_C_"^"_C
 K ^BGPXPT(BGPXPRPT,14)
 I $G(BGPMFITI) S C=0,X="" F  S X=$O(^ATXAX(BGPMFITI,21,"B",X)) Q:X=""  S C=C+1,Y=$P($G(^DIC(4,X,0)),U) S ^BGPXPT(BGPXPRPT,14,C,0)=Y,^BGPXPT(BGPXPRPT,14,"B",Y,C)=""
 S ^BGPXPT(BGPXPRPT,14,0)="^90377.111401A^"_C_"^"_C
ONEF ;
 S BGPONEF=""
 K IOP,%ZIS W !! S %ZIS="PQM" D ^%ZIS
 I POP W !,"Report Aborted" S DA=BGPXPRPT,DIK="^BGPXPT(" D ^DIK K DIK D XIT Q
 I $D(IO("Q")) G TSKMN
DRIVER ;
 D PROC^BGP0DNE
 U IO
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP0DNE",ZTDTH="",ZTDESC="NATIONAL GPRA REPORT 09" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
CNTSF ;EP
 ...
 S BGPX=0,BGPTOT=0 F  S BGPX=$O(^BGPXPT(BGPXPRPT,11,BGPX)) Q:BGPX'=+BGPX  S BGPTOT=BGPTOT+1
 S BGPNF1=BGPTOT/65536
 I BGPNF1'>1,BGPEXPT G STOP ; - only 1 and it is created already
 S BGPNF=$S($P(BGPNF1,".",2)]"":BGPNF1+1,1:BGPNF1)
 S BGPNF=$P(BGPNF,".")
 S BGPX=0,BGPLX=0
 S ^BGPXPT(BGPXPRPT,12,0)="^90377.111201A^"_BGPNF_"^"_BGPNF
 F BGPZ=1:1:BGPNF D
 .S BGPFN="CRSLCNT"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP0UTL(BGPXPBD)_$$D^BGP0UTL(BGPXPTDT)_$$D^BGP0UTL(BGPNOW)_"_"_$$LZERO^BGP0UTL(BGPZ,3)_"_of_"_$$LZERO^BGP0UTL(BGPNF,3)_".TXT"
 .S ^BGPXPT(BGPXPRPT,12,BGPZ+1,0)=BGPFN_" --- "_BGPUF
 ...
 .F  S BGPX=$O(^BGPXPT(BGPXPRPT,11,BGPX)) Q:BGPX'=+BGPX!(BGPC>65535)  D
 ..W $G(^BGPXPT(BGPXPRPT,11,BGPX,0)),!
 ..S BGPC=BGPC+1
 ..S BGPLX=BGPX
 .D ^%ZISC
STOP ;
 K ^BGPXPT(BGPXPRPT,11)
 Q
CNTSF1 ;EP
 ;write out one flie only
 S BGPZ=1,BGPFN="CRSCNT"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP0UTL(BGPXPBD)_$$D^BGP0UTL(BGPXPTDT)_$$D^BGP0UTL(BGPNOW)_"_001_of_001.TXT"
 S ^BGPXPT(BGPXPRPT,12,BGPZ,0)=BGPFN_" --- "_BGPUF
 I '$D(ZTQUEUED) U IO W !?10,BGPFN
 L +^BGPDATA:10 Q:'$T
 ...
 S ^BGPDATA(BGPC)=Y
 S BGPX=0
 F  S BGPX=$O(^BGPXPT(BGPXPRPT,11,BGPX)) Q:BGPX'=+BGPX  D
 .S BGPC=BGPC+1
 .S ^BGPDATA(BGPC)=$G(^BGPXPT(BGPXPRPT,11,BGPX,0))
 ;D ^%ZISC
 S XBGL="BGPDATA"
...
PROC1 ;process each patient
 ...
 ..Q:'$$ACTUP^BGP0D1(DFN,$$FMADD^XLFDT(BGPXXX,-(3*365)),BGPXXX,BGPTAXI,1)
 ..S BGPXPUP=1
 .Q:'BGPXPUP  ;NOT IN ANY USER POP
 .D PROCYRS
 .S X=0 F  S X=$O(BGPXPDAT(X)) Q:X'=+X  D
 ..S BGPXPTOT=BGPXPTOT+1
 ..S ^BGPXPT(BGPXPRPT,11,BGPXPTOT,0)=BGPXPDAT(X)
 .Q
 S ^BGPXPT(BGPXPRPT,11,0)="^90377.111101A^"_BGPXPTOT_"^"_BGPXPTOT
 S BGPET=$H
 D CNTSF
 Q
PROCYRS ;
...
PROCIND ;
 ...
 S $P(BGPXPPD,U,3)=$$EDT^BGP0UTL(DT)
 S $P(BGPXPPD,U,4)=$$EDT^BGP0UTL(BGPXPBD)
 S $P(BGPXPPD,U,5)=$$EDT^BGP0UTL(BGPXPTDT)
 S $P(BGPXPPD,U,6)=$$UID^BGP0DCHW(DFN)
 S $P(BGPXPPD,U,7)=$$EDT^BGP0UTL($P(^DPT(DFN,0),U,3))
 S $P(BGPXPPD,U,8)=$P(^DPT(DFN,0),U,2)
 S $P(BGPXPPD,U,9)=$$STATE^BGP0DCHW(DFN)
 ...
 .S BGPXPTV=""
 .K ^TMP($J)
 .I $D(^BGPINDT(BGPXPX,1)) X ^BGPINDT(BGPXPX,1)
 .;now get each individual measure and set value
 .S BGPXPT=0 F  S BGPXPT=$O(^BGPCTRL(BGPXPFYC,55,BGPXPMI,11,BGPXPT)) Q:BGPXPT'=+BGPXPT  D
 ..S BGPXPMID=$P(^BGPCTRL(BGPXPFYC,55,BGPXPMI,11,BGPXPT,0),U,1)
 ..I '$D(BGPXPDAT(BGPXPMID)) S BGPXPDAT(BGPXPMID)=BGPXPPD
 ..S $P(BGPXPDAT(BGPXPMID),U,11)=BGPXPMID
 ..S $P(BGPXPDAT(BGPXPMID),U,12)=$P(^BGPCTRL(BGPXPFYC,55,BGPXPMI,11,BGPXPT,0),U,3)
 ..S X=""
 ..X ^BGPCTRL(BGPXPFYC,55,BGPXPMI,11,BGPXPT,1)
...
PRINT ;EP - CALLED FROM XBDBQUE
 ...
 S BGPX=0 F  S BGPX=$O(^BGPXPT(BGPXPRPT,12,BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 .I $Y>(IOSL-3) D HEADER Q:BGPQUIT
 .W !?5,$P(^BGPXPT(BGPXPRPT,12,BGPX,0),U)
 K BGPX,BGPQUIT
 Q
HEADER ;EP
 G:'BGPGPG HEADER1
...
HEADER1 ;
 ...
 W $$CTR("*** IHS 2010 Comprehensive National GPRA Export ***",80),!
 ...
 S X="Time Period: "_$$FMTE^XLFDT(BGPXPBD)_" to "_$$FMTE^XLFDT(BGPXPTDT) W !,$$CTR(X,80),!
 W $TR($J("",80)," ","-")
 Q
