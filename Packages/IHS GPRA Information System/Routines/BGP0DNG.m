BGP0DNG ; IHS/CMI/LAB - NATL COMP EXPORT 13 Nov 2006 12:31 PM ;
...
AI ;gather all gpra measures
 ;F X=1:100 I $D(^BGPINDT("GPRA",1,X)) S BGPIND(X)=""
...
EXPORT ;export to area or not?
 ...
 .W !,"beginning with ","""","BG10","""",") will not contain height and weight data."
...
ZIS ;call to XBDBQUE
 ...
 I BGPRPT="" D XIT Q
 I BGPEXPT D
 .W !!,"A file will be created called BG10",$P(^AUTTLOC(DUZ(2),0),U,10)_"."_BGPRPT," and will reside",!,"in the ",BGPUF," directory.",!
...
DRIVER ;
 ...
 D ^BGP0DP
 D ^%ZISC
 I BGPEXPT D GS^BGP0UTL
 ;I $G(BGPEXCEL) D EXCELGS^BGP0UTL
...
ONEF ;
 ...
 Q
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP0DNG",XBRX="XIT^BGP0DNG",XBNS="BGP"
 D ^XBDBQUE
 Q
 ;
NODEV1 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP0DNG",ZTDTH="",ZTDESC="NATIONAL GPRA REPORT 10" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
HWSF ;EP
 ...
 S BGPX=0,BGPTOT=0 F  S BGPX=$O(^BGPGPDCT(BGPRPT,88888,BGPX)) Q:BGPX'=+BGPX  S BGPTOT=BGPTOT+1
 S BGPNF1=BGPTOT/65536
 S BGPNF=$S($P(BGPNF1,".",2)]"":$P(BGPNF1,".")+1,1:BGPNF1)
 S BGPNF=$P(BGPNF,".")
 S BGPX=0,BGPLX=0
 F BGPZ=1:1:BGPNF D
 .S BGPFN="CRSHW"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP0UTL(BGPBD)_$$D^BGP0UTL(BGPED)_$$D^BGP0UTL(BGPHWNOW)_"_"_$$LZERO^BGP0UTL(BGPZ,3)_"_of_"_$$LZERO^BGP0UTL(BGPNF,3)_".TXT"
 ...
 .F  S BGPX=$O(^BGPGPDCT(BGPRPT,88888,BGPX)) Q:BGPX'=+BGPX!(BGPC>65535)  D
 ..W $G(^BGPGPDCT(BGPRPT,88888,BGPX,0)),!
 ..S BGPC=BGPC+1
 ..S BGPLX=BGPX
 .D ^%ZISC
 Q
HWSF1 ;EP
 ...
 S BGPZ=1,BGPFN="CRSHW"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP0UTL(BGPBD)_$$D^BGP0UTL(BGPED)_$$D^BGP0UTL(BGPHWNOW)_"_001_of_001.TXT"
 ...
 W "SERVICE UNIT^ASUFAC^UNIQUE DB ID^DATE RUN^BEG DATE^END DATE^PATIENT UID^DOB^TRIBE CODE^GENDER^STATE OF RESIDENCE^UNIQUE VISIT ID^DATE OF VISIT^TIME OF VISIT^HT CM^WT KG",!
 S BGPC=1,BGPX=0
 F  S BGPX=$O(^BGPGPDCT(BGPRPT,88888,BGPX)) Q:BGPX'=+BGPX  D
 .W $G(^BGPGPDCT(BGPRPT,88888,BGPX,0)),!
 .S BGPC=BGPC+1
 D ^%ZISC
 Q
...
CHW ;
 ...
 .S BGPHWNOW=$$NOW^XLFDT() S BGPHWNOW=$P(BGPHWNOW,".")_"."_$$RZERO^BGP0UTL($P(BGPHWNOW,".",2),6)
 .I BGPUF="" W:'$D(ZTQUEUED) !!,"Cannot continue.....can't find export directory name. EXCEL file",!,"not written." Q
 .S BGPFN="CRSHW"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP0UTL(BGPBD)_$$D^BGP0UTL(BGPED)_$$D^BGP0UTL(BGPHWNOW)_"_001_of_001"_".TXT"
 .D ONEF
 .Q
 Q
