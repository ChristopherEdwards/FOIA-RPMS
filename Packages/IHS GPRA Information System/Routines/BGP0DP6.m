BGP0DP6 ; IHS/CMI/LAB - print ind H ;
 ;;10.0;IHS CLINICAL REPORTING;;JUN 18, 2010
 ;
 ;
IC1 ;EP
 S BGPORXX=$P(^BGPINDT(BGPIC,12),U,6)
 I $Y>(BGPIOSL-16),'$G(BGPSUMON) D HEADER^BGP0DPH Q:BGPQUIT  W !,^BGPINDT(BGPIC,53,1,0) W:$D(^BGPINDT(BGPIC,53,2,0)) !,^BGPINDT(BGPIC,53,2,0)
 D H1^BGP0DPH
 F BGPPC1=BGPORXX_".1",BGPORXX_".2",BGPORXX_".3" Q:BGPQUIT  D PI^BGP0DP1C
 Q:BGPQUIT
 D:'$G(BGPSUMON) HEADER^BGP0DPH
 F BGPPC1=BGPORXX_".4",BGPORXX_".5",BGPORXX_".6" Q:BGPQUIT  D PI^BGP0DP1C
 Q:BGPQUIT
 D:'$G(BGPSUMON) HEADER^BGP0DPH
 F BGPPC1=BGPORXX_".7",BGPORXX_".8",BGPORXX_".9" Q:BGPQUIT  D PI^BGP0DP1C
 Q:BGPQUIT
 D I1AGE
 Q
...
I1AGE1 ;
 ;gather up all #'s
 S C=C+1
 S BGPF="C-1."_BGPX_".3" S BGPPC=$O(^BGPINDTC("C",BGPF,0))
...
I1AGE2 ;
 S D=D+1
 S BGPF="C-1."_BGPX_".2" S BGPPC=$O(^BGPINDTC("C",BGPF,0))
...
I1AGE3 ;
 S BGPF="C-1."_BGPX_".1" S BGPPC=$O(^BGPINDTC("C",BGPF,0))
 ;set 6th piece to numerator and 7th to %
 S BGPNF=$P(^BGPINDTC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90377.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(D),U,6)=$$V^BGP0DP1C(1,BGPRPT,N,P),$P(BGPDAC(D),U,7)=$S($P(BGPDAC(D),U,1):($P(BGPDAC(D),U,6)/$P(BGPDAC(D),U,1)*100),1:"")
 S $P(BGPDAP(D),U,6)=$$V^BGP0DP1C(2,BGPRPT,N,P),$P(BGPDAP(D),U,7)=$S($P(BGPDAP(D),U,1):($P(BGPDAP(D),U,6)/$P(BGPDAP(D),U,1)*100),1:"")
 S $P(BGPDAB(D),U,6)=$$V^BGP0DP1C(3,BGPRPT,N,P),$P(BGPDAB(D),U,7)=$S($P(BGPDAB(D),U,1):($P(BGPDAB(D),U,6)/$P(BGPDAB(D),U,1)*100),1:"")
I1AGE4 ;
 S BGPF="C-1."_BGPX_".4" S BGPPC=$O(^BGPINDTC("C",BGPF,0))
 ;set 8th piece to numerator and 9th to %
 S BGPNF=$P(^BGPINDTC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90377.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(D),U,8)=$$V^BGP0DP1C(1,BGPRPT,N,P),$P(BGPDAC(D),U,9)=$S($P(BGPDAC(D),U,1):($P(BGPDAC(D),U,8)/$P(BGPDAC(D),U,1)*100),1:"")
 S $P(BGPDAP(D),U,8)=$$V^BGP0DP1C(2,BGPRPT,N,P),$P(BGPDAP(D),U,9)=$S($P(BGPDAP(D),U,1):($P(BGPDAP(D),U,8)/$P(BGPDAP(D),U,1)*100),1:"")
 S $P(BGPDAB(D),U,8)=$$V^BGP0DP1C(3,BGPRPT,N,P),$P(BGPDAB(D),U,9)=$S($P(BGPDAB(D),U,1):($P(BGPDAB(D),U,8)/$P(BGPDAB(D),U,1)*100),1:"")
 Q
I1AGEP ;
 D I1AGEP^BGP0DP60
 Q
