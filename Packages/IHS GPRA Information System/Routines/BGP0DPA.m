BGP0DPA ; IHS/CMI/LAB - COMP NATIONAL GPRA FOR PTS W/APPT 03 Jun 2010 2:54 PM ; 
...
NT S BGPLIST="A"
 S BGPCPLC=0
 ;S XBRP="PRINT^BGP0DPAW",XBRC="PROC^BGP0DPA",XBRX="EOJ^BGP0DPA",XBNS="BGP"
 ;D ^XBDBQUE
 ;
 K IOP,%ZIS
 W !! S %ZIS="PQM" D ^%ZIS
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP0DPA",ZTDTH="",ZTDESC="CRS 09 SCHED REPORT" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D ^XBFMK
 K DIC,DIR,DFN
...
GUIEP ;EP
 ...
 K ^TMP($J,"BGP0DPA")
 S IOM=80
 D GUIR^XBLM("PRINT^BGP0DPA","^TMP($J,""BGP0DPA"",")
...
GUIECP ;EP
 K ^TMP($J,"BGP0DPA")
 S IOM=80
 D GUIR^XBLM("CPPRINT^BGP0DPA","^TMP($J,""BGP0DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP0DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP0DPA",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIT(BGPIEN,11,X,0)=BGPDATA,C=C+1
...
ZISCP ;
 ...
 ;
 I $D(IO("Q")) G TSKMNCP
 ;S XBRP="CPPRINT^BGP0DPA",XBRC="",XBRX="EOJ^BGP0DPA",XBNS="BGP"
 ;D ^XBDBQUE
 ;
...
TSKMNCP ;EP ENTRY POINT FROM TASKMAN
 ...
 I $D(IOM)#2,IOM S ZTIO=ZTIO_";"_IOM I $D(IOSL)#2,IOSL S ZTIO=ZTIO_";"_IOSL
 K ZTSAVE S ZTSAVE("BGP*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="CPPRINT^BGP0DPA",ZTDTH="",ZTDESC="CRS 09 SCHED REPORT DENOM" D ^%ZTLOAD D EOJ Q
 Q
