BGP0DPH ; IHS/CMI/LAB - AREA REPORT HEADER 01 Jul 2009 7:54 PM ;
...
HEADER1 ;
 ...
 I BGPRTYPE=4,$G(BGP0RPTH)="C" D W^BGP0DP("*** IHS 2010 Selected Measures with Community Specified Report ***",1,1,BGPPTYPE)
 I BGPRTYPE=4,$G(BGP0RPTH)="A" D W^BGP0DP("*** IHS 2010 Selected Measures with All Communities Report ***",1,1,BGPPTYPE)
 I BGPRTYPE=4,$G(BGP0RPTH)="P" D W^BGP0DP("*** IHS 2010 Selected Measures with Patient Panel Population Report ***",1,1,BGPPTYPE)
 I BGPRTYPE=1!(BGPRTYPE=9),$G(BGPNGR09) D W^BGP0DP("*** IHS 2011 National GPRA & PART Report, Run Using 2010 Logic ***",1,1,BGPPTYPE) G N
 I BGPRTYPE=1!(BGPRTYPE=9),$G(BGPDESGP) D W^BGP0DP("*** IHS 2010 National GPRA & PART Report by Designated Provider ***",1,1,BGPPTYPE)
 I BGPRTYPE=1!(BGPRTYPE=9),'$G(BGP0GPU) D W^BGP0DP("*** IHS 2010 National GPRA & PART Report ***",1,1,BGPPTYPE)
 I BGPRTYPE=1!(BGPRTYPE=9),$G(BGP0GPU) D W^BGP0DP("*** IHS 2010 GPRA & PART Performance Report ***",1,1,BGPPTYPE)
 I BGPRTYPE=9 D W^BGP0DP("*** Developmental Measures ***",1,1,BGPPTYPE)
 D:BGPRTYPE=3 W^BGP0DP("*** IHS 2010 HEDIS Clinical Performance ***",1,1,BGPPTYPE)
 D:BGPRTYPE=5 W^BGP0DP("*** IHS 2010 ELDER CARE Clinical Performance ***",1,1,BGPPTYPE)
 I BGPRTYPE=7 D W^BGP0DP("IHS 2010 Other National Measures Report ***",1,1,BGPPTYPE)
N I $G(BGPAREAA) D W^BGP0DP("AREA AGGREGATE",1,1,BGPPTYPE)
 I '$G(BGPAREAA) D W^BGP0DP($P(^DIC(4,DUZ(2),0),U),1,1,BGPPTYPE)
 I $G(BGPDESGP) D W^BGP0DP("Designated Provider: "_$P(^VA(200,BGPDESGP,0),U),1,1,BGPPTYPE)
