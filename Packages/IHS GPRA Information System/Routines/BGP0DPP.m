BGP0DPP ; IHS/CMI/LAB - IHS GPRA 10 SELECTED REPORT DRIVER ALL PATIENTS 29 Apr 2010 8:36 PM ;
 ;;10.0;IHS CLINICAL REPORTING;;JUN 18, 2010
 ;
 ;
 W:$D(IOF) @IOF
 W !!,$$CTR("2010 Clinical Performance Measure Report (Selected Measures)",80)
...
SUM ;display summary of this report
 W:$D(IOF) @IOF
 W !,$$CTR("SUMMARY OF 2010 CLINICAL MEASURE PERFORMANCE REPORT TO BE GENERATED")
 W !!,"The date ranges for this report are:"
 W !?5,"Report Period: ",?31,$$FMTE^XLFDT(BGPBD)," to ",?31,$$FMTE^XLFDT(BGPED)
 W !?5,"Previous Year Period: ",?31,$$FMTE^XLFDT(BGPPBD)," to ",?31,$$FMTE^XLFDT(BGPPED)
 W !?5,"Baseline Period: ",?31,$$FMTE^XLFDT(BGPBBD)," to ",?31,$$FMTE^XLFDT(BGPBED)
 ...
 W !!,"These measures will be calculated: " S X=0 F  S X=$O(BGPIND(X)) Q:X'=+X  W $P(^BGPINDT(X,0),U,3)," ; "
 W !!,"Lists will be produced for these measures: "
 S X=0 F  S X=$O(BGPLIST(X)) Q:X'=+X  W $P(^BGPINDT(X,0),U,3)," ; "
 D PT^BGP0DSL
 I BGPROT="" G LISTS
ZIS ;call to XBDBQUE
 D REPORT^BGP0UTL
...
EI ;
 ...
 Q
PI ;
 S X=0 F  S X=$O(^BGPINDTC("APR",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDTC(X,0),U,1))=""
 Q
SI ;
 K BGPIND
 D EN^BGP0DSI
...
LISTS ;any lists with measures?
 ...
 I $D(BGPLIST) D RT^BGP0DSL I '$D(BGPLIST)!($D(BGPQUIT)) G LISTS ;get report type for each list
 Q
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP0DPP",ZTDTH="",ZTDESC="GPRA 10 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP0DPP",XBRX="XIT^BGP0DPP",XBNS="BGP"
 D ^XBDBQUE
 ;D XIT
 Q
 ;
