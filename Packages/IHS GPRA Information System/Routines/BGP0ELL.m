BGP0ELL ; IHS/CMI/LAB - IHS gpra print 01 Jul 2009 7:56 PM ;
...
DEL ;
 ...
 D ^BGP0ELHH
 D DEL1
 D ^BGP0ELSL ;print lists to delimited file
 ...
 I '$D(BGPGUI) D
 .S XBFLT=1,XBFN=BGPDELF_".txt",XBMED="F",XBTLE="CRS 2010 ELDER DELIMITED OUTPUT",XBQ="N",XBF=0
 .D ^XBGSAVE
...
WP ;
 K ^UTILITY($J,"W")
 S BGPZ=0,BGPLCNT=0
 S DIWL=1,DIWR=80,DIWF="" F  S BGPZ=$O(^BGPELIT(BGPIC,BGPNODE,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 .S BGPLCNT=BGPLCNT+1
 .S X=^BGPELIT(BGPIC,BGPNODE,BGPY,1,BGPZ,0) S:BGPLCNT=1 X=" - "_X D ^DIWP
 .Q
WPS ;
