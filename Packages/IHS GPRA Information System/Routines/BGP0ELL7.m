BGP0ELL7 ; IHS/CMI/LAB - print ind ;
...
I1AGEP ;
 ...
 S X="# w/any Retinal Evalution-No Refusals (GPRA)" D S(X,1,1)
 ...
 S X="% B. # w/Other Eye Exams" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,7) S Y=$$SB^BGP0ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="# w/Retinal Exam Refusal" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,8) S Y=V D S(Y,,X+1)
 S X="% # w/Retinal Exam Refusal" D S(X,1,1)
