BGP0ELLD ; IHS/CMI/LAB - print ind ;
...
DSP ;
 ...
 S X="% "_BGPL1 D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,P2) S Y=$$SB^BGP0ELL2($J(V,6,1)) D S(Y,,X+1)
 Q
DSPP ;
 S X="# "_BGPL1 D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,P1) S Y=V D S(Y,,X+1)
 S X="% "_BGPL1 D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,P2) S Y=$$SB^BGP0ELL2($J(V,6,1)) D S(Y,,X+1)
 Q
 ;
DSPB ;
 S X="# "_BGPL1 D S(X,1,1)
 ...
 S X="% "_BGPL1 D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAB(X),U,P2) S Y=$$SB^BGP0ELL2($J(V,6,1)) D S(Y,,X+1)
 Q
 ;
V(T,R,N,P) ;EP
 I $G(BGPAREAA) G VA
