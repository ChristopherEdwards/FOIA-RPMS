BGP0ELLI ; IHS/CMI/LAB - print ind ;
...
I1AGE6 ;
 S H=H+1
 S BGPF="ELD."_$P(BGPX,".")_"."_($P(BGPX,".",2)+20) S BGPPC=$O(^BGPELIIT("C",BGPF,0))
 S BGPNF=$P(^BGPELIIT(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90379.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
I1AGE7 ;
 S I=I+1
 S BGPF="ELD."_$P(BGPX,".")_"."_($P(BGPX,".",2)+24) S BGPPC=$O(^BGPELIIT("C",BGPF,0))
 ;set 4th piece to numerator and 5th to %
 S BGPNF=$P(^BGPELIIT(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90379.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
I1AGEP ;
 ...
 F X=1:1:4 S V=$P(BGPDAC(X),U,13) S Y=$$SB^BGP0ELL2($J(V,6,1)) D S(Y,,X+1)
PR ; 
 ...
 S X="% E. w/Stage 2 HTN BP w/ % of Total Screened" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,13) S Y=$$SB^BGP0ELL2($J(V,6,1)) D S(Y,,X+1)
 ;percentage changes
 ...
 S X="E. w/Stage 2 HTN BP w/ % of Total Screened" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,13),O=$P(BGPDAP(X),U,13) S Y=$$SB^BGP0ELL2($J((N-O),6,1)) D S(Y,,X+1)
BL ;
 ...
 S X="% E. w/Stage 2 HTN BP w/ % of Total Screened" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAB(X),U,13) S Y=$$SB^BGP0ELL2($J(V,6,1)) D S(Y,,X+1)
 ;percentage changes
 ...
 S X="E. w/Stage 2 HTN BP w/ % of Total Screened" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,13),O=$P(BGPDAB(X),U,13) S Y=$$SB^BGP0ELL2($J((N-O),6,1)) D S(Y,,X+1)
 Q
