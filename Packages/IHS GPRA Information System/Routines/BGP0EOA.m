BGP0EOA ; IHS/CMI/LAB - IHS GPRA 10 REPORT DRIVER ;
...
NT .;END TEST STUFF
 ...
 S BGPZZ="A" S X=0 F  S X=$O(^BGPEOMT(X)) Q:X'=+X  S BGPIND(X)=""
 S BGPAREAA=1
TP ;get time period
 S BGPRTYPE=8
 S (BGPBD,BGPED,BGPTP)=""
...
ASU ;
 ...
 ;I C=1 S BGPRPTT="F",BGPSUCNT=1,Y=$O(BGPSUL(0)),X=$P(^BGPEOCT(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 I C>1 S BGPRPTT="A"
...
DRIVER ;
 U IO
 S BGPIFTR=0 D PRINT^BGP0EOP
 I $G(BGPIFTR) D EONT1^BGP0EOUT
 D ^%ZISC
 D XIT
 Q
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP0EOA",XBRX="XIT^BGP0EOA",XBNS="BGP"
 D ^XBDBQUE
 ;D XIT
 Q
 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP0EOA",ZTDTH="",ZTDESC="EO 09 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
