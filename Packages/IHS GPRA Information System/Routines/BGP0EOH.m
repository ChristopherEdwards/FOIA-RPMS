BGP0EOH ; IHS/CMI/LAB - cover page for ELDER 02 Jul 2009 9:27 AM ;
 ...
 D W("*** IHS 2010 Executive Order Quality Transparency Measures Report ***",1,2,BGPPTYPE)
 D W($$RPTVER^BGP0BAN,1,2,BGPPTYPE)
 ...
 D EOHDR
 I $G(BGPEXPT) D
 .D W("A file will be created called BG10"_$P(^AUTTLOC(DUZ(2),0),U,10)_".EO"_BGPRPT_".",0,2,BGPPTYPE)
...
AREACP ;EP - area cover page
 ...
 ;
 D ENDTIME
 S BGPCHSO="",X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  I $P(^BGPEOCT(X,0),U,17) S BGPCHSO=1
 S BGPCHSN="",X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  I '$P(^BGPEOCT(X,0),U,17) S BGPCHSN=1
 D EOHDR
 ...
 .S X=$P(^BGPEOCT(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPEOCT(BGPX,0),U,17):"*",1:"")_X D W(X,0,1,BGPPTYPE,1,3)
 .Q
 ...
 .S X="Community Taxonomy Name: "_$P(^BGPEOCT(BGPX,0),U,18) D W(X,0,1,BGPPTYPE,1,5)
 .S BGPXX=0,BGPXN=0,BGPXY="" F  S BGPXX=$O(^BGPEOCT(BGPX,9999,BGPXX)) Q:BGPXX'=+BGPXX  S BGPXN=BGPXN+1,BGPXY=BGPXY_$S(BGPXN=1:"",1:";")_$P(^BGPEOCT(BGPX,9999,BGPXX,0),U)
 ...
 ..Q
 .I $O(^BGPEOCT(BGPX,1111,0)) D
 ..D W("MFI Visit Locations: ",0,2,BGPPTYPE,1,5) S BGPXX=0,BGPXN=0,BGPXY="" F  S BGPXX=$O(^BGPEOCT(BGPX,1111,BGPXX)) Q:BGPXX'=+BGPXX  S BGPXN=BGPXN+1,BGPXY=BGPXY_$S(BGPXN=1:"",1:";")_$P(^BGPEOCT(BGPX,1111,BGPXX,0),U)
 ..S BGPX1=0,C=0 F BGPX1=1:3:BGPXN D
 ...D W($E($P(BGPXY,";",BGPX1),1,20),0,1,BGPPTYPE,1,10)
 ...D W($E($P(BGPXY,";",(BGPX1+1)),1,20),0,0,BGPPTYPE,2,30)
 ...D W($E($P(BGPXY,";",(BGPX1+2)),1,20),0,0,BGPPTYPE,3,60)
