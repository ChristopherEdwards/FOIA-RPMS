BGP0GAEO ; IHS/CMI/LAB - AREA NATIONAL GPRA REPORT 01 Jul 2009 7:58 PM ;
...
TESTNTL ;
 ...
 S BGPSUL(1)=""
 S BGPSUL(2)=""
 D EP(.ERR,1,2522,"BGP 10 AREA ELDER REPORT","A",.BGPSUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRPTT,BGPSUL,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPROT,BGPRTIME,BGPHC,BGPFILE) ;EP - called from GUI to produce national gpra report (AO-AGP)
 ...
 ;            from BGP 10 ELDER DATA CURRENT that match the following:
 ;            (SEE BGP0ASL or CALL ME)
 ;  BGPSUL  - array containing Iens from BGP 10 ELDER  DATA CURRENT that the user selected
...
EP1 ;
 ...
 S BGPZZ="A"
 S BGPSUCNT=C
 I BGPSUCNT=1 S Y=$O(BGPSUL(0)),X=$P(^BGPEOCT(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 ...
 S X=0 F  S X=$O(^BGPEOMT(X)) Q:X'=+X  S BGPIND(X)=""
 S BGPINDT="E"
 I '$G(BGPHC) D
 ...
 .S BGPFGNT1="CRSEONT1"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP0UTL(BGPED)_$$D^BGP0UTL(BGPNOW)_"_"_$$LZERO^BGP0UTL(BGPC,6)_".TXT"
 .Q
 ;create entry in GUI file
 D ^XBFMK
 S X=BGPFILE
 S DIC="^BGPGUIT(",DIC(0)="L",DIADD=1,DLAYGO=90378.08,DIC("DR")=".02////"_BGPUSER_";.03////"_$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT)_";.05///"_BGPOPTN_";.06///R;.07///"_$G(BGPROT)_";1///"_$G(BGPFGNT1)
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOEO^BGP0GAEO",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI AREA EO REPORT 09" D ^%ZTLOAD Q
 Q
AOEO ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 ;cmi/anch/maw added 5/12/2009 for word output
 D GUIR^XBLM("PRINT^BGP0EOP","^TMP($J,""BGPGUI"",")
