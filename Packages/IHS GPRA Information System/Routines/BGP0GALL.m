BGP0GALL ; IHS/CMI/LAB - GUI COM REPORT ;
...
TESTNTL ;
 ...
 S BGPND(4)="",BGPND(3)=""
 S BGPLIST(3)=""
 D EP(.ERR,1,2522,"BGP 10 SEL ALL PATS","S",.BGPND,1,3040000,3000000,1,"A",.BGPLIST,"","","B",$$NOW^XLFDT)
 W !,ERR
 Q
...
EP1 ;
 ...
 S BGPGUI=1
 S IOM=80,BGPIOSL=55
 S BGPRTYPE=4,BGP0RPTH="A",BGPALLPT=1
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LOCALL^BGP0GALL",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT ALL 05" D ^%ZTLOAD Q
 Q
LOCALL ;
 D ^BGP0D1
 K ^TMP($J,"BGPGUI")
