BGP0GEO ; IHS/CMI/LAB - GUI EO REPORT ;
...
TESTELD ;
 ...
 S BGPND(4)="",BGPND(3)=""
 S BGPLIST(3)=""
 D EP(.ERR,1,2522,"BGP 10 EO REPORT",338,"S",.BGPND,1,3040000,3000000,1,"A",.BGPLIST,"","",1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPINDZ,BGPIND,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPLSTT,BGPLIST,BGPLPRV,BGPLPROV,BGPEXPT,BGPROT,BGPRTIME,BGPMFITI,BGPHC,BGPFILE) ;EP - called from GUI to produce COM REPORT CI05-RPT-LOC-COM
 ...
 ;                        1 and 6 from the BGP 10 EO INDICATORS file (^BGPELIT GLOBAL).  When
...
EP1 ;
 ...
 S BGPFILEN=""
 I BGPEXPT S BGPFILEN="BG10"_$P(^AUTTLOC(DUZ(2),0),U,10)_".EO"_BGPRPT_" in directory "_BGPUF
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="OTHEO^BGP0GEO",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI EO 09 REPORT" D ^%ZTLOAD Q
 Q
OTHEO ;
 D ^BGP0D1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^XBLM("^BGP0EOP","^TMP($J,""BGPGUI"",")
