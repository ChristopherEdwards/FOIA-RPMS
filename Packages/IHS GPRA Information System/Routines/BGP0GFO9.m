BGP0GFO9 ; IHS/CMI/LAB - GUI Patient Forecast Report ;
...
EP1 ;
 ...
 S BGPGUI=1
 S IOM=80,BGPIOSL=55
 S BGPRTYPE=1,BGP0RPTH="",BGP0GPU=1,BGPALLPT=1,BGPBEN=3
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="GUIP^BGP0GFO9",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI FORECAST 09 FOR 09" D ^%ZTLOAD Q
 Q
GUIP ;
 D PROC^BGP0DPA9
 K ^TMP($J,"BGP0DPA")
 S IOM=80,BGPIOSL=55
 D GUIR^XBLM("PRINT^BGP0DPAW","^TMP($J,""BGP0DPA"",")
 ;cmi/anch/maw added 5/12/2008 for word output
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP0DPA",X)) Q:X'=+X  D
 . S C=C+1
 . N BGPDATA
 . S BGPDATA=$G(^TMP($J,"BGP0DPA",X))
