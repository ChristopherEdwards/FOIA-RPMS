BGP0GPED ; IHS/CMI/LAB - GUI COM REPORT ;
...
TESTELD ;
 ...
 S BGPND(4)="",BGPND(3)=""
 S BGPLIST(3)=""
 D EP(.ERR,1,2522,"BGP 10 PAT ED REPORT",338,"S",.BGPND,1,3040000,3000000,1,"A",.BGPLIST,"","",1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
...
BY ;get baseline year
 ...
 S BGPFILEN=""
 I BGPEXPT S BGPFILEN="BG10"_$P(^AUTTLOC(DUZ(2),0),U,10)_".PED"_BGPRPT_" in directory "_BGPUF
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="OTHPED^BGP0GPED",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI PAT ED REPORT" D ^%ZTLOAD Q
 Q
OTHPED ;
 D ^BGP0DPE1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^XBLM("^BGP0DPEP","^TMP($J,""BGPGUI"",")
