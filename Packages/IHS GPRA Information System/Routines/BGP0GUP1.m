BGP0GUP1 ; IHS/CMI/LAB - GUI Upload Continued ;
...
PROCEO ;EP
 ...
 ;add entry
 L +^BGPEOCT:10 I '$T W !!,"unable to lock global. TRY LATER" D EOJ^BGP0GUPL Q
 L +^BGPEOPT:10 I '$T W !!,"unable to lock global. TRY LATER" D EOJ^BGP0GUPL Q
 L +^BGPEOBT:10 I '$T W !!,"unable to lock global. TRY LATER" D EOJ^BGP0GUPL Q
 D GETIEN^BGP0EOUT
 I 'BGPIEN S BGPRET=0_"^error in file creation...call programmer." D EOJ^BGP0GUPL Q
ELCY ;
 S DINUM=BGPIEN,X=$P(BGP0,U),DLAYGO=90379.1,DIC="^BGPEOCT(",DIC(0)="L"
 K DD,D0,DO
 D FILE^DICN
 I Y=-1 S BGPRET=0_"^error uploading file" D EOJ^BGP0GUPL Q
 S BGPIEN=+Y
 D ^XBFMK
