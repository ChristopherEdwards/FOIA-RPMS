BGP0HEH ; IHS/CMI/LAB - cover page for HEDIS ;
 ;;10.0;IHS CLINICAL REPORTING;**1**;JUN 18, 2010
 ;
 ;
 I $G(BGPAREAA) G AREACP
 S BGPHPG=0  ;cmi/maw 3/19/2009 if not area this variable is not set
 ;W:$D(IOF) @IOF
 S BGPQHDR=""
 W !!,$$CTR("Cover Page",80)
 W:BGPRTYPE=3 !!,$$CTR("*** IHS 2010 HEDIS Performance Report ***",80)
 ...
 D ENDTIME
 D HEDISHDR
 I $G(BGPEXPT) W !!,"A file will be created called BG10",$P(^AUTTLOC(DUZ(2),0),U,10)_".HE"_BGPRPT,".",!,"It will reside in the public/export directory.",!,"This file should be sent to your Area Office.",!
...
AREACP ;EP - area cover page
 ...
 W !,"Population: ",$S(BGPBEN=1:"AI/AN Only (Classification 01)",BGPBEN=2:"non AI/AN Only (Classification NOT 01)",BGPBEN=3:"All (Both AI/AN and non AI/AN)",1:"")
 D ENDTIME
 S BGPCHSO="",X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  I $P(^BGPHEDCT(X,0),U,17) S BGPCHSO=1
 S BGPCHSN="",X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  I '$P(^BGPHEDCT(X,0),U,17) S BGPCHSN=1
 D HEDISHDR
 ;I $G(BGPEXPT) W !!,"A file will be created called BG10",$P(^AUTTLOC(DUZ(2),0),U,10)_".HE"_BGPRPT,".",!,"It will reside in the public/export directory.",!,"This file should be sent to your Area Office.",!
 I '$D(BGPGUI),BGPROT'="P" W !!,"A delimited output file called ",BGPDELF,!,"has been placed in the public directory for your use in Excel or some",!,"other software package.",!,"See your site manager to access this file.",!
 ...
 .S BGPC=BGPC+1
 .S X=$P(^BGPHEDCT(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .W !?3,BGPC,".  ",$S($P(^BGPHEDCT(BGPX,0),U,17):"*",1:""),X
 ...
 .W !!?3,BGPC,".  ",$S($P(^BGPHEDCT(BGPX,0),U,17):"*",1:""),X
 .W !?3,"Community Taxonomy Name: ",$P(^BGPHEDCT(BGPX,0),U,18)
 .;W !?5,"Communities: " W !?5,"Communities: " 
 .S BGPXX=0,BGPXN=0,BGPXY="" F  S BGPXX=$O(^BGPHEDCT(BGPX,9999,BGPXX)) Q:BGPXX'=+BGPXX  S BGPXN=BGPXN+1,BGPXY=BGPXY_$S(BGPXN=1:"",1:";")_$P(^BGPHEDCT(BGPX,9999,BGPXX,0),U)
 .S BGPX1=0,C=0 F BGPX1=1:3:BGPXN D
 ..I $Y>(BGPIOSL-4) D HDR Q:BGPQHDR
 ..W !?10,$E($P(BGPXY,";",BGPX1),1,20),?30,$E($P(BGPXY,";",(BGPX1+1)),1,20),?60,$E($P(BGPXY,";",(BGPX1+2)),1,20)
 .I $O(^BGPHEDCT(BGPX,1111,0)) D
 ..I $Y>(BGPIOSL-4) D HDR Q:BGPQHDR
 ..W !!?5,"MFI Visit Locations: " S BGPXX=0,BGPXN=0,BGPXY="" F  S BGPXX=$O(^BGPHEDCT(BGPX,1111,BGPXX)) Q:BGPXX'=+BGPXX  S BGPXN=BGPXN+1,BGPXY=BGPXY_$S(BGPXN=1:"",1:";")_$P(^BGPHEDCT(BGPX,1111,BGPXX,0),U)
