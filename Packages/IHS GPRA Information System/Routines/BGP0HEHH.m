BGP0HEHH ; IHS/CMI/LAB - cover page for HEDIS ;
 ...
 S X="*** IHS 2010 HEDIS Performance Report ***" D S(X,1,1)
 ...
 S X=" " D S(X,1,1)
 I $G(BGPEXPT) D
 .S X="A file will be created called BG10"_$P(^AUTTLOC(DUZ(2),0),U,10)_".HE"_BGPRPT_"." D S(X,1,1) S X="It will reside in the public/export directory.  This file should be sent to your Area Office." D S(X,1,1)
...
BGP D HEDISHDR
 S X=" " D S(X,1,1)
 I $G(BGPEXPT) D
 .S X="A file will be created called BG10"_$P(^AUTTLOC(DUZ(2),0),U,10)_".HE"_BGPRPT_"." D S(X,1,1) S X="It will reside in the public/export directory." D S(X,1,1) S X="This file should be sent to your Area Office." D S(X,1,1)
 I '$D(BGPGUI),BGPROT'="P" D
 ...
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPHEDCT(BGPX,0),U,17):"*",1:"")_X D S(X,1,1)
 .Q
 S X=" " D S(X,1,1)
 ...
 .S X=$P(^BGPHEDCT(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPHEDCT(BGPX,0),U,17):"*",1:"")_X D S(X,1,1)
 .S X="Community Taxonomy Name: "_$P(^BGPHEDCT(BGPX,0),U,18) D S(X,1,1)
 .;W !?5,"Communities: " S X="Communities: " 
 .D S(X,1,1) S X=0,N=0,Y="",Z="" F  S X=$O(^BGPHEDCT(BGPX,9999,X)) Q:X'=+X  S N=N+1,Y=Y_$S(N=1:"",1:";")_$P(^BGPHEDCT(BGPX,9999,X,0),U)
 .S X=0,C=0 F X=1:3:N S Z=$E($P(Y,";",X),1,20),$P(Z,U,2)=$E($P(Y,";",(X+1)),1,20),$P(Z,U,3)=$E($P(Y,";",(X+2)),1,20) D S(Z,1,1)
 .I $O(^BGPHEDCT(BGPX,1111,0))  D
 ..S X=" " D S(X,1,1)
 ..S X="MFI Site: Locations for visits: " D S(X,1,1) S X=0,N=0,Y="",Z="" F  S X=$O(^BGPHEDCT(BGPX,1111,X)) Q:X'=+X  S N=N+1,Y=Y_$S(N=1:"",1:";")_$P(^BGPHEDCT(BGPX,1111,X,0),U)
 ..S X=0,C=0 F X=1:3:N S Z=$E($P(Y,";",X),1,20),$P(Z,U,2)=$E($P(Y,";",(X+1)),1,20),$P(Z,U,3)=$E($P(Y,";",(X+2)),1,20) D S(Z,1,1)
 ..Q
 .S X=" " D S(X,1,1)
 .Q
