BGP0HEL ; IHS/CMI/LAB - IHS gpra print 01 Jul 2009 8:00 PM ;
...
DEL ;
 ...
 D ^BGP0HEHH
 D DEL1
 D ^BGP0HESL ;print lists to delimited file
 ...
 I '$D(BGPGUI) D
 .S XBFLT=1,XBFN=BGPDELF_".txt",XBMED="F",XBTLE="CRS 2010 HEDIS DELIMITED OUTPUT",XBQ="N"
 .D ^XBGSAVE
...
DEL1 ;EP
 ...
 .S X=$P(^BGPHEIT(BGPIC,0),U,3) D S(X,1,1)
 .S X=" " D S(X,1,1)
 .S X="Denominator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPHEIT(BGPIC,61,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPHEIT(BGPIC,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...I $P(^BGPHEIT(BGPIC,61,BGPY,0),U,2)'[BGPRTYPE Q  ;not a denom def for this report
 ...S BGPZ=0 F  S BGPZ=$O(^BGPHEIT(BGPIC,61,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S Y=^BGPHEIT(BGPIC,61,BGPY,1,BGPZ,0) S:BGPZ=1 Y=" - "_Y D S(Y,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .S X="Numerator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPHEIT(BGPIC,62,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPHEIT(BGPIC,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...I $P(^BGPHEIT(BGPIC,62,BGPY,0),U,2)'[BGPRTYPE Q  ;not a denom def for this report
 ...S BGPZ=0 F  S BGPZ=$O(^BGPHEIT(BGPIC,62,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S X=^BGPHEIT(BGPIC,62,BGPY,1,BGPZ,0) S:BGPZ=1 X=" - "_X D S(X,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .I $O(^BGPHEIT(BGPIC,11,0)) D S("Logic:",1,1) S BGPX=0 F  S BGPX=$O(^BGPHEIT(BGPIC,11,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPHEIT(BGPIC,11,BGPX,0) D S(X,1,1)
 .I $O(^BGPHEIT(BGPIC,51,0)) D S(" ",1,1) S X="Performance Measure Description :" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPHEIT(BGPIC,51,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPHEIT(BGPIC,51,BGPX,0) D S(X,1,1)
 .S X=" " D S(X,1,1) D S("HEDIS Rates:",1,1)
 .D:'$O(^BGPHEIT(BGPIC,52,0)) S("Not Reported",1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPHEIT(BGPIC,52,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPHEIT(BGPIC,52,BGPX,0) D S(X,1,1)
 .X ^BGPHEIT(BGPIC,4)
 Q
HEADER ;EP
 S Y=$P(^VA(200,DUZ,0),U,2),$E(Y,35)=$$FMTE^XLFDT(DT) D S(Y,1,1)
 I BGPRTYPE=3 S Y="*** IHS 2010 HEDIS Clinical Performance Report***" D S(Y,1,1)
