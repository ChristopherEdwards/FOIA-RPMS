BGP0PDH1 ; IHS/CMI/LAB - cover page for gpra del 0 ;
...
PEDCP ;EP
 ...
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPPEDCT(BGPX,0),U,17):"*",1:"")_X D SET(X,1,1)
 .Q
 S X=" " D SET(X,1,1)
 ...
 .S X=$P(^BGPPEDCT(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPPEDCT(BGPX,0),U,17):"*",1:"")_X D SET(X,1,1)
 .S X="Communities: " D SET(X,1,1) S X=0,N=0,Y="",Z="" F  S X=$O(^BGPPEDCT(BGPX,9999,X)) Q:X'=+X  S N=N+1,Y=Y_$S(N=1:"",1:";")_$P(^BGPPEDCT(BGPX,9999,X,0),U)
 .S X=0,C=0 F X=1:3:N S Z=$E($P(Y,";",X),1,20),$P(Z,U,2)=$E($P(Y,";",(X+1)),1,20),$P(Z,U,3)=$E($P(Y,";",(X+2)),1,20) D SET(Z,1,1)
 .I $O(^BGPPEDCT(BGPX,1111,0))  D
 ..S X=" " D SET(X,1,1)
 ..S X="MFI Site: Locations for visits: " D SET(X,1,1) S X=0,N=0,Y="",Z="" F  S X=$O(^BGPPEDCT(BGPX,1111,X)) Q:X'=+X  S N=N+1,Y=Y_$S(N=1:"",1:";")_$P(^BGPPEDCT(BGPX,1111,X,0),U)
 ..S X=0,C=0 F X=1:3:N S Z=$E($P(Y,";",X),1,20),$P(Z,U,2)=$E($P(Y,";",(X+1)),1,20),$P(Z,U,3)=$E($P(Y,";",(X+2)),1,20) D SET(Z,1,1)
 ..Q
 .S X=" " D SET(X,1,1)
 .Q
...
COMHDR ;EP
 ...
 .Q
 S X=" " D SET(X,1,1)
 I $G(BGP0GPU) D SET("See last pages of this report for Performance Summaries.",1,1) D SET(" ",1,1)
 Q
GPRAHDRS ;EP
 S X=" " D SET(X,1,1)
...
FOOTER ;EP
 ...
 D SET(" * Measure definition changed in 2008.",2,1)
 ...
 Q
FOOTER10 ;
 D SET("  * GPRA 2011 targets represented here are preliminary targets since they will",2,1)
 D SET("be adjusted for FY 2010 actual results and FY 2011 appropriations.",1,1)
 D SET(" ** Measure definition changed in 2008.",2,1)
 D SET("*** Not official GPRA measure but included to show percentage of refusals with",2,1)
 D SET("respect to GPRA measure.",1,1)
 D SET("  + Site Previous and Site Baseline values are not applicable for this measure.",2,1)
