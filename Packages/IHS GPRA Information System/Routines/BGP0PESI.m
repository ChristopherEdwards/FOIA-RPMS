BGP0PESI ; IHS/CMI/LAB - DISPLAY IND LISTS ;
...
EN ;EP -- main entry point for APCH HMR DISPLAY
 D EN^VALM("BGP 10 PATED MEASURE SEL")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ; -- init variables and list array
 K BGPTIND S BGPHIGH=""
 S (X,Y,Z,C)=0 F  S X=$O(^BGPPEIT("AO",X)) Q:X'=+X  S Y=$O(^BGPPEIT("AO",X,Y)) Q:Y'=+Y  D
 .S C=C+1,BGPTIND(C,0)=C_")",$E(BGPTIND(C,0),5)=$P(^BGPPEIT(Y,0),U,2),BGPTIND(C,C)=Y I $D(BGPIND(Y)) S BGPTIND(C,0)="*"_BGPTIND(C,0)
 .Q
 S (VALMCNT,BGPHIGH)=C
 Q
 ;
