BGP0XTV ; IHS/CMI/LAB - DISPLAY IND LISTS ; 15 Dec 2010  9:42 AM
...
EN ;EP -- main entry point for 
 D EN^VALM("BGP 10 TAXONOMY VIEW")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ;EP -- init variables and list array
 ...
 .Q:'$O(^BGPTAXT(BGPY,12,0))
 .S BGPTYPE=$P(^BGPTAXT(BGPY,0),U,2),BGPDESC=$G(^BGPTAXT(BGPY,11,1,0)),BGPEDIT=$P(^BGPTAXT(BGPY,0),U,4),J=J+1
 .I BGPTYPE'="L" D  Q:'I
 ...
 S T="",J=0,C=0 F  S T=$O(^BGPTAXT("B",T)) Q:T=""  D
 .S Y=0 F  S Y=$O(^BGPTAXT("B",T,Y)) Q:Y'=+Y  D
 ..S N=^BGPTAXT(Y,0)
 ..S Z=$P(N,U,2)  ;TYPE
 ...
 ...S A="",B=0 F  S B=$O(^BGPTAXT(Y,12,B)) Q:B'=+B  S R=$P(^BGPTAXT(Y,12,B,0),U) S:A]"" A=A_";" S A=A_$S(R=1:"National GPRA",R=2:"Local CRS",R=3:"HEDIS",R=4:"ELDER",R=5:"CMS",1:"")
 ...S $E(BGPTAX(J,0),60)=A
...
DISP ;EP - add an item to the selected list - called from a protocol
 ...
 D EP^BGP0XTV1(BGPTIEN,BGPTYPE,BGPFIEN)
DISPX ;
 D BACK
 Q
