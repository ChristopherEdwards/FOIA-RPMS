BGP1CP ; IHS/CMI/LAB - IHS gpra print 02 Jul 2010 9:06 AM ;
...
LIST1 ;DISPLAY LIST 1
 ...
 Q:BGPQUIT
 S BGPAST=0
 I '$D(^XTMP("BGP1C1",BGPJ,BGPH,"LIST",BGPIND,BGPPLSTL)) W !!,"No Visits to report" Q
 S BGPNAME="" F  S BGPNAME=$O(^XTMP("BGP1C1",BGPJ,BGPH,"LIST",BGPIND,BGPPLSTL,BGPNAME)) Q:BGPNAME=""!(BGPQUIT)  D
 .S DFN=0 F  S DFN=$O(^XTMP("BGP1C1",BGPJ,BGPH,"LIST",BGPIND,BGPPLSTL,BGPNAME,DFN)) Q:DFN'=+DFN!(BGPQUIT)  D
 ..S BGPVSIT=0 F  S BGPVSIT=$O(^XTMP("BGP1C1",BGPJ,BGPH,"LIST",BGPIND,BGPPLSTL,BGPNAME,DFN,BGPVSIT)) Q:BGPVSIT'=+BGPVSIT!(BGPQUIT)  D
 ...S BGPVSIT0=$G(^AUPNVSIT(BGPVSIT,0))
 ...S BGPVINP=$O(^AUPNVINP("AD",BGPVSIT,0))
 ...I $Y>(BGPIOSL-4) D HDR Q:BGPQUIT  D L1H
 ...W !!
 ...S BGPPEX=^XTMP("BGP1C1",BGPJ,BGPH,"LIST",BGPIND,BGPPLSTL,BGPNAME,DFN,BGPVSIT)
 ...
 ....W ?42,$$DATE^BGP1UTL($P($P(BGPVSIT0,U),"."))_"-"_$$DATE^BGP1UTL($$DSCH^BGP1CU(BGPVINP)) I BGPIND=1!(BGPIND=3) W $S(BGPPEX[2!(BGPPEX[1):"*",1:"")
 ....W ?62,$E($$VAL^XBDIQ1(9000010.02,BGPVINP,.07),1,18)
 ...I $P(^BGPCMSMB(BGPPLSTL,0),U,6) D
 ....W ?42,$E($$VAL^XBDIQ1(9000001,DFN,1111),1,19)
 ....W ?62,$$DATE^BGP1UTL($P($P(BGPVSIT0,U),"."))_"-"_$$DATE^BGP1UTL($$DSCH^BGP1CU(BGPVINP))
...
HEADER1 ;
 ...
 W $$CTR("*** IHS 2011 CMS Hospital Quality Reporting Initiative ***",80)
