BGP1CP2 ; IHS/CMI/LAB - IHS gpra print 02 Nov 2009 10:40 AM ;
...
AMI5W ;EP
 ...
 Q:BGPQUIT
 K Z
 S X=$$BETAALEG^BGP1CU2(DFN,$$DOB^AUPNPAT(DFN),$$DSCH^BGP1CU(BGPVINP))
 I X S X=$P(X,U,2)
 D WBETAAL^BGP1CPU
 Q:BGPQUIT
 K BGPBRADY
 D ALLDXS^BGP1CU2(DFN,$P($P(^AUPNVSIT(BGPVSIT,0),U),"."),$$DSCH^BGP1CU(BGPVINP),.BGPBRADY,0,"BGP CMS BRADYCARDIA DXS")
 ;on active med for beta blocker?
 K BGPBETA
 D BETARX^BGP1CU2(DFN,$$FMADD^XLFDT($P($P(^AUPNVSIT(BGPVSIT,0),U),"."),-180),$$DSCH^BGP1CU(BGPVINP),1,.BGPBETA)
 D WBRADY5^BGP1CPU2
 Q:BGPQUIT
 K BGPDATA
 K BGP23RD
 D ALLDXS^BGP1CU2(DFN,$P($P(^AUPNVSIT(BGPVSIT,0),U),"."),$$DSCH^BGP1CU(BGPVINP),.BGP23RD,0,"BGP CMS 2/3 HEART BLOCK DXS")
 S BGPPACE=$$PACE^BGP1CU2(DFN,$$DOB^AUPNPAT(DFN),$$DSCH^BGP1CU(BGPVINP))
 D W23RD^BGP1CPU2
 K BGPY
 D NMIDRUG^BGP1CU1(DFN,$P($P(^AUPNVSIT(BGPVSIT,0),U),"."),$$DSCH^BGP1CU(BGPVINP),.BGPDATA,"BGP CMS BETA BLOCKER MEDS",0)
 S E=+$$CODEN^ICPTCOD("G8011")
 S Z=$$CPTI^BGP1DU(DFN,$P($P(^AUPNVSIT(BGPVSIT,0),U),"."),$$DSCH^BGP1CU(BGPVINP),E) I Z]"" S Z=$$DATE^BGP1UTL($P(Z,U,2))_" [G8011]"
 D WNMIBETA^BGP1CPU
 Q:BGPQUIT
 S X=$$LASTMED^BGP1CU1(DFN,$$FMADD^XLFDT($P($P(^AUPNVSIT(BGPVSIT,0),U),"."),-365),$$FMADD^XLFDT($P($P(^AUPNVINP(BGPVINP,0),U),"."),30),"BGP CMS BETA BLOCKER MEDS","BGP CMS BETA BLOCKER NDC","BGP CMS BETA BLOCKER CLASS")
 S E=+$$CODEN^ICPTCOD("G8009")
 S Z=$$CPTI^BGP1DU(DFN,$$FMADD^XLFDT($P($P(^AUPNVSIT(BGPVSIT,0),U),"."),-365),$$FMADD^XLFDT($P($P(^AUPNVINP(BGPVINP,0),U),"."),30),E) I Z]"" S Z=$$DATE^BGP1UTL($P(Z,U,2))_" [G8009]"
 D WLASTBB^BGP1CPU
 Q:BGPQUIT
...
AMI5 ;EP
 ...
 D W23RD^BGP1CPU2
 K BGPY
 D NMIDRUG^BGP1CU1(DFN,$$VD^APCLV(BGPVSIT),$$DSCH^BGP1CU(BGPVINP),.BGPDATA,"BGP CMS BETA BLOCKER MEDS",0)
 S E=+$$CODEN^ICPTCOD("G8011")
 S Z=$$CPTI^BGP1DU(DFN,$P($P(^AUPNVSIT(BGPVSIT,0),U),"."),$$DSCH^BGP1CU(BGPVINP),E) I Z]"" S Z=$$DATE^BGP1UTL($P(Z,U,2))_" [G8011]"
 D WNMIBETA^BGP1CPU
 Q:BGPQUIT
...
AMI6 ;EP
 ...
 K BGPBETA
 D BETARX^BGP1CU2(DFN,$$FMADD^XLFDT($P($P(^AUPNVSIT(BGPVSIT,0),U),"."),-180),$$DSCH^BGP1CU(BGPVINP),1,.BGPBETA)
 S BGPASTER=1 D WBRADY6^BGP1CPU2 S BGPASTER=0
 Q:BGPQUIT
 K BGPDATA
A6WHF1 ;
 D ALLDXS^BGP1CU2(DFN,$P($P(^AUPNVSIT(BGPVSIT,0),U),"."),$$DSCH^BGP1CU(BGPVINP),.BGPDATA,0,"BGP CMS HEART FAILURE DXS")
 D WHF^BGP1CPU2
 K BGPDATA
 Q:BGPQUIT
 K BGP23RD
 ...
 Q:BGPQUIT
 K BGPDATA
 D ALLDXS^BGP1CU2(DFN,$P($P(^AUPNVSIT(BGPVSIT,0),U),"."),$$DSCH^BGP1CU(BGPVINP),.BGPDATA,0,"BGP CMS CIRCULATORY SHOCK DXS")
 D WCS^BGP1CPU2
 K BGPDATA
 Q:BGPQUIT
 K BGPY
...
AMI7AW ;EP
 ...
 S BGPFIB=""
 K BGPDATA
 D TARX^BGP1CU2(DFN,$$FMADD^XLFDT($P($P(^AUPNVSIT(BGPVSIT,0),U),"."),-365),$$FMADD^XLFDT($$DSCH^BGP1CU(BGPVINP),30),0,$P($P(^AUPNVSIT(BGPVSIT,0),U),"."),.BGPDATA)
 K BGPUD
