BGP1D23 ; IHS/CMI/LAB - measure I2 23 Jun 2010 10:08 AM ;
 ;;11.1;IHS CLINICAL REPORTING SYSTEM;;JUN 27, 2011;Build 33
 ;
IMS ;EP
 S (BGPN1,BGPN2,BGPN3,BGPN4,BGPN5,BGPN6,BGPN7,BGPN8,BGPN9,BGPD1,BGPD2)=0
 S BGPVALUE=""
 I $$LASTDX^BGP1UTL1(DFN,"SURVEILLANCE DIABETES",$P(^DPT(DFN,0),U,3),BGPEDATE) S BGPSTOP=1 Q  ;had diabetes dx
 ...
 S BGPVALUE=BGPVALUE_" "_$P(BGPD,U,2)_"|||NUM:"
 ;now set up numerators
IMS1 ;EP
 S C=0
 S BGPBP=$$BPSD(DFN,BGPBDATE,BGPEDATE)
 I BGPBP S BGPN1=1
 I BGPBP="" S BGPBP=$$BPCPT^BGP1D22(DFN,BGP365,BGPEDATE),C=1 I $P(BGPBP,U,2)]"" S BGPN1=1
 I BGPN1 S BGPVALUE=BGPVALUE_" 2 BPs" I C S BGPVALUE=BGPVALUE_" - CPT "_$P(BGPBP,U,2)
IMS2 ;LDL done
 S BGPLDL=$$LDL^BGP1D2(DFN,BGPBDATE,BGPEDATE,1)
 S BGPN2=$P(BGPLDL,U)
 I BGPN2 S BGPVALUE=BGPVALUE_";LDL: "_$$DATE^BGP1UTL($P(BGPLDL,U,2))_" "_$P(BGPLDL,U,3)
 K X,Y,Z,%,A,B,C,D,E,H,BDATE,EDATE,P,V,S,F,BGPLDL,BGPHDL,BGPTRI,BGPLP
IMS3 ;fasting glucose
 S BGPFG=$$FGT(DFN,BGPBDATE,BGPEDATE)
 S BGPN3=$P(BGPFG,U)
 I BGPN3 S BGPVALUE=BGPVALUE_";FG: "_$$DATE^BGP1UTL($P(BGPFG,U,2))
 ;S BGPUP=$$POSUR^BGP1D21A(DFN,BGP365,BGPEDATE)
 ...
 I BGPN4 D
 .I BGPESRD S BGPVALUE=BGPVALUE_$S(BGPESRD]"":";ESRD: "_$P(BGPESRD,U,2)_"-"_$$DATE^BGP1UTL($P(BGPESRD,U,3)),1:"") Q
 .S BGPVALUE=BGPVALUE_";GFR: "_$$DATE^BGP1UTL($P(BGPGFR,U,2))
 .S BGPVALUE=BGPVALUE_" & QUANT UP: "_$P(BGPQUP,U,2)_"-"_$$DATE^BGP1UTL($P(BGPQUP,U,3))
 K BGPFG
IMS5 ;
 K BGPX,BGPC1,X,Y,Z,%,A,B,C,D,E,H,BDATE,EDATE,P,V,S,F
 S BGPTOBV=""
 S BGPTOB=$$TOBACCO^BGP1D7(DFN,BGPBDATE,BGPEDATE)
 S BGPN5=$S(BGPTOB]"":1,1:0) I BGPN5 S BGPTOBV=$P(BGPTOB,U,2)_" "_$P(BGPTOB,U,1)
 S BGPSDX=$$DX^BGP1D7(DFN,BGPBDATE,BGPEDATE)
 S BGPXPBD=$$PED^BGP1D7(DFN,BGPBDATE,BGPEDATE)
 S BGP1320=$$DENT^BGP1D7(DFN,BGPBDATE,BGPEDATE)
 S BGPSCPT=$$CPTSM^BGP1D7(DFN,BGPBDATE,BGPEDATE)
 I BGPSDX]"" S BGPN5=1 I BGPTOBV="" S BGPTOBV=$$DATE^BGP1UTL($P(BGPSDX,U,2))_" "_$P(BGPSDX,U,1)
 I BGPXPBD]"" S BGPN5=1 I BGPTOBV="" S BGPTOBV=$$DATE^BGP1UTL($P(BGPXPBD,U,2))_" "_$P(BGPXPBD,U,1)
 I BGP1320]"" S BGPN5=1 I BGPTOBV="" S BGPTOBV=$$DATE^BGP1UTL($P(BGP1320,U,2))_" 1320"
 I BGPSCPT]"" S BGPN5=1 I BGPTOBV="" S BGPTOBV=$$DATE^BGP1UTL($P(BGPSCPT,U,2))_" "_$P(BGPSCPT,U,1)
 I BGPN5 S BGPVALUE=BGPVALUE_";TOB SCRN: "_BGPTOBV
IMS6 ;
 S BGPBMI=$$BMI^BGP1D6(DFN,BGPEDATE,BGPAGEE)
 ;I $P(BGPBMI,U)="" S BGPBMI=$$REF^BGP1D6(DFN,BGPBDATE,BGPEDATE,BGPAGEB)
 I $P(BGPBMI,U)]"" S BGPN6=1
 I BGPN6 S BGPVALUE=BGPVALUE_";BMI: "_$S(BGPBMI["HT":"ref "_$P(BGPBMI,U,2)_" "_$$DATE^BGP1UTL($P(BGPBMI,U,3))_" "_$P(BGPBMI,U,5)_" "_$$DATE^BGP1UTL($P(BGPBMI,U,6)),BGPBMI]"":$$SB^BGP1PDL1($J($P(BGPBMI,U),6,2)),1:"")
IMS7 ;
 S BGPLIFE=$$LIFED^BGP1D41(DFN,BGPBDATE,BGPEDATE)
 I $P(BGPLIFE,U) S BGPN7=1
 I BGPN7 S BGPVALUE=BGPVALUE_";LIFE: "_$$DATE^BGP1UTL($P(BGPLIFE,U,2))_":"_$P(BGPLIFE,U,3)
IMS8 ;
 S BGPDEP=$$DEP^BGP1D25(DFN,BGP365,BGPEDATE) I $P(BGPDEP,U)=1 S BGPN8=1
 S BGPDEPS=$$DEPSCR^BGP1D25(DFN,BGP365,BGPEDATE) I $P(BGPDEPS,U)=1 S BGPN8=1
 S BGPREF="" I 'BGPN8 S BGPREF=$$DEPREF^BGP1D25(DFN,BGP365,BGPEDATE) I $P(BGPREF,U)=1 S BGPN8=1
...
MS(P,BDATE,EDATE) ;EP
 ...
 .Q
 K ^TMP($J,"A")
 I $P(G,U)>1 Q 1_U_"277.7 on "_$$DATE^BGP1UTL($P(G,U,2))_"; "_$$DATE^BGP1UTL($P(G,U,3))
 ;now check for 3 or more of the following
 S BGPC=0,BGPK=""
 S X=$$BMI^BGP1D6(P,EDATE,$$AGE^AUPNPAT(P,EDATE))
 ...
 S BGPHTN=$$LASTDX^BGP1UTL1(P,"SURVEILLANCE HYPERTENSION",BGP365,EDATE)
 S BGPMBP=$$MEANBP(P,BGPBDATE,BGPEDATE)
 I $P(BGPHTN,U)!(BGPMBP]"") S BGPC=BGPC+1 S:BGPK]"" BGPK=BGPK_"; " S:$P(BGPHTN,U)=1 BGPK=BGPK_"HTN DX: "_$$DATE^BGP1UTL($P(BGPHTN,U,3)) I BGPMBP]"" S:BGPK]""&($P(BGPHTN,U)=1) BGPK=BGPK_"; " S BGPK=BGPK_BGPMBP
