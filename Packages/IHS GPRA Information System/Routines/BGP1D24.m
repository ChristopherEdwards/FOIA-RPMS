BGP1D24 ; IHS/CMI/LAB - STI MEASURE 18 Oct 2009 8:37 AM 03 Jul 2010 7:56 AM ;
...
ISTI ;EP
 ...
 .I H]"" S BGPNUMV(X)=BGPNUMV(X)_"Prior HIV DX (Contraind): "_$$DATE^BGP1UTL(H)
...
IASB ;EP
 ...
 S V="",X=0,D="" F  S X=$O(BGPERV(X)) Q:X'=+X  S D=D_$S(D]"":"; ",1:"")_"ER "_X_") "_$$DATE^BGP1UTL($P(BGPERV(X),U,1))_"POV "_$P(BGPERV(X),U,3) D
 .;S V=V_$S(V]"":"; ",1:"")
 .;S V=V_"ER Visit "_X_") "
 ...
 .S D=D_", BNI: "_$S($P(BGPERV(X),U,10)=1:$$DATE^BGP1UTL($P(BGPERV(X),U,8))_" At ER",1:"Not at ER")_" "_$P(BGPERV(X),U,9)
...
BNII(P,BD) ;
 NEW Y,BGPG,E,X,T,%,D
 S Y=$$CPT^BGP1DU(P,BD,$$FMADD^XLFDT(BD,7),$O(^ATXAX("B","BGP BNI CPTS",0)),6,"CT")
...
IPC ;EP
 ...
 S BGPCANCE=$$CANCER(DFN,BGPBDATE,BGPEDATE)
 ;BGPD1 OVER 18
 ;BGPD2 OVER 18 MALE
 ;BGPD3 OVER 18 FEMALE
 ;
 I BGPCANCE D
 .I BGPAGEB>17 S BGPD1=1
 .I BGPD1,$P(^DPT(DFN,0),U,2)="M" S BGPD2=1
 .I BGPD1,$P(^DPT(DFN,0),U,2)="F" S BGPD3=1
 .I BGPAGEB>17,BGPAGEB<55 S BGPD4=1
 .I BGPD4,$P(^DPT(DFN,0),U,2)="M" S BGPD5=1
 .I BGPD4,$P(^DPT(DFN,0),U,2)="F" S BGPD6=1
 .I BGPAGEB>54 S BGPD7=1
 .I BGPD7,$P(^DPT(DFN,0),U,2)="M" S BGPD8=1
 .I BGPD7,$P(^DPT(DFN,0),U,2)="F" S BGPD9=1
 ...
 S BGPVALUE="AC"_$S(BGPCANCE&(BGPAGEB>17):",CAN",1:"")_"|||"_BGPN1_$S(BGPN1'=1:" visits: ",1:" visit: ")_$P(BGPVAL,U,2)
 K BGPVAL
 Q
 ;
PCV(P,BDATE,EDATE) ;EP
 ...
 .S X($P(BGPG(X),U,5))=""
 .S C=C+1
 .S R=R_$S(R="":"",1:", ")_$$DATE^BGP1UTL($P($P(^AUPNVSIT($P(BGPG(X),U,5),0),U),"."))
 Q C_U_R
 ;
CANCER(P,BDATE,EDATE) ;EP - return # of different cancer visits
 NEW G,X
 S G=0
 S X=$$LASTDX^BGP1UTL1(P,"BGP MELANOMA CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP BREAST CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP COLON CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP GYN CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP PROSTATE CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP TESTES/MALE GU CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP HEAD AND NECK CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP URINARY TRACT CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP NON-MELANOMA CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP NON-COLON GI CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP LUNG CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP BRAIN CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP BONES/TISSUE CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP ENDOCRINE CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP PLEURA CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP NON-SPECIFIC CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP1UTL1(P,"BGP LYMPH NODE CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 Q 0
