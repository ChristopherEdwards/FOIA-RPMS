...
VAR(P,EDATE) ;EP
 ...
 I $$PLTAX^BGP1DU(P,"BGP VARICELLA EVIDENCE") Q 4_U_"evid vari"
 ...
 I $$PLTAX^BGP1DU(P,"BGP VARICELLA CONTRA") Q 4_U_"contra vari"
 F BGPZ=21,94 S X=$$MMRCONT^BGP1D31(P,BGPZ,EDATE) Q:X]""
 ...
 F BGPIMM=21,94 S R=$$IMMREF^BGP1D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
...
HEP(P,EDATE) ;EP
 ...
 I BGPHEP=2,BGP10743 Q 1_U_" 2 Hep B +(90743)"
 ;check for evidence of desease and contraindications and if yes, then quit
 K BGPG S %=P_"^LAST DX [BGP HEP EVIDENCE;DURING "_$$DOB^AUPNPAT(P)_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 4_U_"evid Hep B"
 I $$PLTAX^BGP1DU(P,"BGP HEP EVIDENCE") Q 4_U_"evid Hep B"
 ;now go to refusals
 S B=$$DOB^AUPNPAT(P),E=EDATE,BGPNMI="",R=""
 F BGPIMM="8",42,43,44,45,51,102,104,110,132  D
 .S I=$O(^AUTTIMM("C",BGPIMM,0)) Q:'I
 .S X=0 F  S X=$O(^AUPNPREF("AA",P,9999999.14,I,X)) Q:X'=+X  S Y=0 F  S Y=$O(^AUPNPREF("AA",P,9999999.14,I,X,Y)) Q:Y'=+Y  S D=$P(^AUPNPREF(Y,0),U,3) I D'<B&(D'>E) S:$P(^AUPNPREF(Y,0),U,7)="N" BGPNMI=1 S R=1
 I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI",1:"ref")_" Hep B"
 ;now check refusals in imm pkg
 F BGPIMM=8,42,43,44,45,51,102,104,110,132 S R=$$IMMREF^BGP1D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
 I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI",1:"ref")_" Hep B"
 F BGPZ=8,42,43,44,45,51,102,104,110,132 S X=$$ANCONT^BGP1D31(P,BGPZ,EDATE) Q:X]""  ;cmi/maw 12/17/07 missing edate
...
HIB(P,EDATE) ;EP
 ...
 ;I $$PLTAX^BGP1DU(P,"BGP HIB EVIDENCE") Q 4_U_"evid HIB"
 ;now go to refusals
 S B=$$DOB^AUPNPAT(P),E=EDATE,BGPNMI="",R=""
...
PNEUMO(P,EDATE,MINN) ;EP
 ...
 ;now check refusals in imm pkg
 F BGPIMM=33,100,109,133 S R=$$IMMREF^BGP1D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
 I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI",1:"ref")_" Pneumo"
 F BGPZ=33,100,109,133 S X=$$ANCONT^BGP1D31(P,BGPZ,EDATE) Q:X]""  ;cmi/maw 12/17/07 missing edate
