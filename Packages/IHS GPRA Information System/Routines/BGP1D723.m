BGP1D723 ; IHS/CMI/LAB - measure AHR.A ;
...
ACECONT(P,BDATE,EDATE,NMIB,NMIE,RPBD) ;EP ACEI contra
 ...
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP1UTL($P(BGPG(1),U))_" ACEI/ARB Contra POV "_$P(BGPG(1),U,2)
 ;
 ;nmi
 S BGPG=""
 ...
 ...S BGPG=1_U_$$DATE^BGP1UTL($P(^AUPNPREF(N,0),U,3))_" ACEI NMI Refusal"
 ..Q
 .Q
 I BGPG Q BGPG
 ;nmi
 ...
 ..S N=0 F  S N=$O(^AUPNPREF("AA",P,50,X,D,N)) Q:N'=+N  D
 ...Q:$P($G(^AUPNPREF(N,0)),U,7)'="N"
 ...S BGPG=1_U_$$DATE^BGP1UTL($P(^AUPNPREF(N,0),U,3))_" ARB NMI Refusal"
 ..Q
 .Q
 I BGPG Q BGPG
 ;PREGNANCY
 S X=$$PREG^BGP1D7(P,$S($G(RPBD):RPBD,1:NMIB),NMIE,0,1) I X Q 1_U_"ACEI/ARB contra pregnant"
 ;breastfeeding
...
STATALG(P,BDATE,EDATE,RPB,RPE) ;EP
 ...
 .S Z=$P(^AUPNVPOV(Y,0),U,18) I Z]"",$P($$ICDDX^ICDCODE(Z),U,2)="E942.9" S BGPC=1_U_"POV:  "_$$DATE^BGP1UTL($P(BGPG(X),U))_"  ["_$P(BGPG(X),U,2)_" + E942.9]  "_N Q
 .S Z=$P(^AUPNVPOV(Y,0),U,19) I Z]"",$P($$ICDDX^ICDCODE(Z),U,2)="E942.9" S BGPC=1_U_"POV:  "_$$DATE^BGP1UTL($P(BGPG(X),U))_"  ["_$P(BGPG(X),U,2)_" + E942.9]  "_N Q
 .Q
 I BGPC Q BGPC
 ...
 .I N["STATIN"!(N["STATINS") S BGPC=BGPC+1,BGPY(BGPC)=1_U_"alg statin POV:  "_$$DATE^BGP1UTL($P(BGPG(X),U))_"  ["_$P(BGPG(X),U,2)_"]  "_N
 I BGPC Q BGPC
 ;PL
 S BGPC=0
