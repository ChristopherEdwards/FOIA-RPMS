BGP1D861 ; IHS/CMI/LAB - measure C ;
...
SERUM6(P,BDATE,EDATE) ;EP
 ...
 S %=$$CPT^BGP1DU(P,BDATE,EDATE,T,3)
 I %]"" S BGPC=BGPC+1,BGPC($P(%,U,1))=""
 S T=$O(^ATXAX("B","BGP CREATININE CPTS",0))
 S %=$$TRAN^BGP1DU(P,BDATE,EDATE,T,3)
 I %]"" S BGPC=BGPC+1,BGPC($P(%,U,1))=""
 ;now get all loinc/taxonomy tests
...
POT(P,BDATE,EDATE) ;EP
 K BGPC
 S BGPC=0
 S %="",E=+$$CODEN^ICPTCOD(84132),%=$$CPTI^BGP1DU(P,BDATE,EDATE,E)
 I %]"" S BGPC=1_U_$P(%,U,2)_U_"84132"
 S %="",E=+$$CODEN^ICPTCOD(84132),%=$$TRANI^BGP1DU(P,BDATE,EDATE,E)
...
SERUM12(P,BDATE,EDATE) ;EP
 ...
 S BGPC=0
 S T=$O(^ATXAX("B","BGP CREATININE CPTS",0))
 S %="",E=+$$CODEN^ICPTCOD(85025),%=$$CPT^BGP1DU(P,BDATE,EDATE,T,3)
 I %]"" S BGPC=BGPC+1,BGPC($P(%,U,1))=""
 S T=$O(^ATXAX("B","BGP CREATININE CPTS",0))
 S %="",E=+$$CODEN^ICPTCOD(85025),%=$$TRAN^BGP1DU(P,BDATE,EDATE,T,3)
 I %]"" S BGPC=BGPC+1,BGPC($P(%,U,1))=""
 ;now get all loinc/taxonomy tests
 S T=$O(^ATXAX("B","BGP CREATININE LOINC CODES",0))
 S BGPLT=$O(^ATXLAB("B","DM AUDIT CREATININE TAX",0))
...
GLUCOSE(P,BDATE,EDATE) ;EP
 ...
 S %=$$CPT^BGP1DU(P,BDATE,EDATE,T,5)
 I %]"" S BGPC=1_U_$P(%,U,1)_U_$P(%,U,2)
 S T=$O(^ATXAX("B","BGP GLUCOSE CPTS",0))
 S %=$$TRAN^BGP1DU(P,BDATE,EDATE,T,5)
 I %]"" S BGPC=1_U_$P(%,U,1)_U_$P(%,U,2)
 ;now get all loinc/taxonomy tests
