BGP1DAR ; IHS/CMI/LAB - ihs area GPRA 02 Sep 2004 1:11 PM 09 Apr 2011 10:03 AM ;
...
TP ;
 S BGPAREAA=1
 S BGPRTYPE=1,BGPBEN=1,BGP1RPTH=""
...
ASU ;
 ...
 ;I C=1 S BGPRPTT="F",BGPSUCNT=1,Y=$O(BGPSUL(0)),X=$P(^BGPGPDCB(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
ZIS ;call to XBDBQUE
EISSEX ;
 ...
 .S BGPFGNT1="CRSGPRANT1"_$P(^AUTTLOC(DUZ(2),0),U,10)_2011063000000000_$$D^BGP1UTL(BGPNOW)_"_"_$$LZERO^BGP1UTL(BGPC,6)_".TXT"
 .S BGPFGNT2="CRSGPRANT2"_$P(^AUTTLOC(DUZ(2),0),U,10)_2011063000000000_$$D^BGP1UTL(BGPNOW)_"_"_$$LZERO^BGP1UTL(BGPC,6)_".TXT"
 .S BGPFDEV1="CRSGPRADEVNT1"_$P(^AUTTLOC(DUZ(2),0),U,10)_2011063000000000_$$D^BGP1UTL(BGPNOW)_"_"_$$LZERO^BGP1UTL(BGPC,6)_".TXT"
 .S BGPFDEV2="CRSGPRADEVNT2"_$P(^AUTTLOC(DUZ(2),0),U,10)_2011063000000000_$$D^BGP1UTL(BGPNOW)_"_"_$$LZERO^BGP1UTL(BGPC,6)_".TXT"
 .S BGPFDEV3="CRSGPRADEVNT3"_$P(^AUTTLOC(DUZ(2),0),U,10)_2011063000000000_$$D^BGP1UTL(BGPNOW)_"_"_$$LZERO^BGP1UTL(BGPC,6)_".TXT"
 .;S BGPFGNT3="CRSGPRANT3"_$P(^AUTTLOC(DUZ(2),0),U,10)_2011063000000000_$$D^BGP1UTL(BGPNOW)_"_"_$$LZERO^BGP1UTL(BGPC,6)_".TXT"
 .;S BGPFGNT4="CRSGPRANT4"_$P(^AUTTLOC(DUZ(2),0),U,10)_2011063000000000_$$D^BGP1UTL(BGPNOW)_"_"_$$LZERO^BGP1UTL(BGPC,6)_".TXT"
 .Q
...
DRIVER ;
 U IO
 D PRINT^BGP1PARP
 I BGPRPTT="A",BGPIFTR=1,BGPEXCEL D GNT1^BGP1UTL
 D ^%ZISC
 D EXIT
 Q
 ;
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP1DAR",ZTDTH="",ZTDESC="GPRA REPORT" D ^%ZTLOAD D HOME^%ZIS D EXIT Q
 Q
 ;
EXIT ;
 D ^%ZISC
 ...
 Q
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP1DAR",XBRX="EXIT^BGP1DAR",XBNS="BGP"
 D ^XBDBQUE
 Q
 ;
NODEV1 ;
 D PRINT^BGP1PARP
 I BGPRPTT="A",BGPIFTR,BGPEXCEL D GNT1^BGP1UTL
 D ^%ZISC
 D EXIT
 Q
 ;
B ;fiscal year
 S (BGPBPER,BGPVDT)=""
 W !!,"Enter the BASELINE year for the report.  Use a 4 digit ",!,"year, e.g. 2011"
 S DIR(0)="D^::EP"
...
F ;fiscal year
 ...
 W !
 S BGPVDT=""
 W !,"Enter the Fiscal Year (FY) for the report END date.  Use a 4 digit",!,"year, e.g. 2010, 2011"
 S DIR(0)="D^::EP"
 S DIR("A")="Enter FY"
 S DIR("?")="This report is compiled for a period.  Enter a valid date."
 D ^DIR
...
SUMONLY ;EP
 W:$D(IOF) @IOF
 S BGPA=$E($P(^AUTTLOC(DUZ(2),0),U,10),1,2),BGPA=$O(^AUTTAREA("C",BGPA,0)) S BGPA=$S(BGPA:$P(^AUTTAREA(BGPA,0),U),1:"UNKNOWN AREA")
 W !,$$CTR("IHS 2011 Area National GPRA & PART Report Clinical Performance Summaries")
 W !!,"This will produce ONLY the clinical performance summaries for the Area "
 W !,"National GPRA & PART Report for the 2011 GPRA year.  If you want the detailed "
