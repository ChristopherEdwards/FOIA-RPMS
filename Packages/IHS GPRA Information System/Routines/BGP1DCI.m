BGP1DCI ; IHS/CMI/LAB - IHS area GPRA 10 Dec 2006 9:12 AM ;
 ;;11.1;IHS CLINICAL REPORTING SYSTEM;;JUN 27, 2011;Build 33
 ;
CALCIND ;EP - CALCULATE ALL MEASURES
 S BGPIC=0 F  S BGPIC=$O(BGPIND(BGPIC)) Q:BGPIC'=+BGPIC  D
 .I BGPRTYPE=1,$P(^BGPINDB(BGPIC,0),U,7)'=1 Q  ;national gpra report
 .I BGPRTYPE=7,$P($G(^BGPINDB(BGPIC,12)),U,1)'=1 Q  ;OTHER NATIONAL
 ...
 .K BGPNUMV
 .K ^TMP($J)
 .I $D(^BGPINDB(BGPIC,1)) X ^BGPINDB(BGPIC,1)
 .K BGPVAL,BGPG,BGPC,BGPALLED,BGPV,A,B,C,D,E,F,G,H,I,J,K,M,N,O,P,Q,R,S,T,V,W,X,Y,Z
 .K ^TMP($J)
 .;Set up global for iCare
 .I $G(BQIGREF)'="",$D(BGPSTOP) S @BQIGREF@(DFN,BGPIC)=$P(^BGPINDB(BGPIC,0),U,3)_"^N/A" Q
 .I $D(BGPSTOP) Q  ;no need to set since no num/denom
 .;loop each individual to set numerator and denominator
 .S BGPI=0 F  S BGPI=$O(^BGPINDBC("B",BGPIC,BGPI)) Q:BGPI'=+BGPI  D
 ..S (BGPNUM,BGPDEN)=0
 ..X ^BGPINDBC(BGPI,1)
 ..X ^BGPINDBC(BGPI,2) ;denominator 1 or 0
 ..;set field counter
 ..S BGPNF=$P(^BGPINDBC(BGPI,0),U,9)
 ..S BGPN=$P(^DD(90545.03,BGPNF,0),U,4)
 ..S N=$P(BGPN,";"),P=$P(BGPN,";",2)
 ..D S(BGPRPT,BGPGBL,N,P,BGPNUM),S1("N",BGPNUM)
 ..S BGPDF=$P(^BGPINDBC(BGPI,0),U,8)
 ..S BGPN=$P(^DD(90545.03,BGPDF,0),U,4),N=$P(BGPN,";"),P=$P(BGPN,";",2)
 ..I BGPDEN'="NO" D S(BGPRPT,BGPGBL,N,P,BGPDEN),S1("D",BGPDEN)
 ..I $G(BGPCPPL) D CPL
 .I $D(BGPLIST(BGPIC)) D STMP^BGP1UTL
 .I $G(BGPNPL) D NPL
...
CPL ;comprehensive pat list check and set xtmp
 ...
 Q:BGPTIME'=1
 Q:$P($G(^BGPINDBC(BGPI,12)),U,1)=""
 X ^BGPINDBC(BGPI,3) Q:'$T
 S C=$S($P($G(^AUPNPAT(DFN,11)),U,18)]"":$P(^AUPNPAT(DFN,11),U,18),1:"UNKNOWN")
 S S=$P(^DPT(DFN,0),U,2)
 S D=$P(BGPVALUE,"|||")
 S F=$P($G(^XTMP("BGP18CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN)),"|||")
 I D["UP" S $P(F,"$$",1)="UP"
 I D["AC" S $P(F,"$$",2)="AC"
 I D["AD" S $P(F,"$$",3)="AD"
 I D["AAD" S $P(F,"$$",4)="AAD"
 ...
 I D["IMM" S $P(F,"$$",6)="IMM"
 I D["IHD" S $P(F,"$$",7)="IHD"
 I '$D(^XTMP("BGP18CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN)) D  Q
 .S ^XTMP("BGP18CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN)=F_"|||"_$P(^BGPINDBC(BGPI,12),U),BGPCPLC=BGPCPLC+1
 S $P(^XTMP("BGP18CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||")=F
 S $P(^XTMP("BGP18CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||",2)=$P(^XTMP("BGP18CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||",2)_"#"_$P(^BGPINDBC(BGPI,12),U)
 Q
NPL ;
 Q:BGPTIME'=1
 ...
 .X ^BGPNPLB(BGPX,12) K ^TMP($J) Q:'$T
 .S BGPINDL(BGPIC,BGPX)=$G(BGPINDL(BGPIC,BGPX))+1
 .I $G(BGP1NPLT) S ^XTMP("BGP1DNP",BGPJ,BGPH,"LIST",BGPORD,BGPIC,BGPX,DFN)="" Q
 .S BGPO=$S(BGPRTYPE=7:$P(^BGPNPLB(BGPX,0),U,6),1:$P(^BGPNPLB(BGPX,0),U,5))
 .I $P(^BGPNPLB(BGPX,0),U,7)=9,$G(BGPVALUD)]"" S BGPVALUE=BGPVALUD
 .S ^XTMP("BGP1DNP",BGPJ,BGPH,"LIST",BGPORD,BGPIC,BGPO,BGPX,$S($P($G(^AUPNPAT(DFN,11)),U,18)]"":$P(^AUPNPAT(DFN,11),U,18),1:"UNKNOWN"),$P(^DPT(DFN,0),U,2),BGPAGEB,DFN)=$G(BGPVALUE)
...
S1(BQITYP,BQIVAL) ; Return data by patient for iCare into global reference BQIGREF
 ...
 S BQIIDTA=$G(^BGPINDBC(BGPI,0))
 S BQIDTA=$G(^BGPINDBC(BGPI,14))
 ;
 ...
 S $P(@BQIGREF@(DFN,BGPIC),U,1)=$P(^BGPINDB(BGPIC,0),U,3)
