BGP1DELA ; IHS/CMI/LAB - IHS GPRA 10 REPORT DRIVER ;
 ;;11.1;IHS CLINICAL REPORTING SYSTEM;;JUN 27, 2011;Build 33
 ;
 ;
 W:$D(IOF) @IOF
 W !!,$$CTR("2011 Area Aggregate Elder Care Clinical Performance Measure Report",80)
INTRO ;
 D XIT
 ...
 D XIT
 S BGPZZ="A" S X=0 F  S X=$O(^BGPELIB(X)) Q:X'=+X  S BGPIND(X)=""
 S BGPAREAA=1
TP ;get time period
 S BGPRTYPE=5
 S (BGPBD,BGPED,BGPTP)=""
...
ASU ;
 ...
 I C=1 S BGPRPTT="F",BGPSUCNT=1,Y=$O(BGPSUL(0)),X=$P(^BGPELDCB(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 I C>1 S BGPRPTT="A"
ZIS ;call to XBDBQUE
  D ^XBFMK
...
DRIVER ;
 ...
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP1DELA",XBRX="XIT^BGP1DELA",XBNS="BGP"
 D ^XBDBQUE
 ;D XIT
 Q
 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP1DELA",ZTDTH="",ZTDESC="ELDER 09 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
