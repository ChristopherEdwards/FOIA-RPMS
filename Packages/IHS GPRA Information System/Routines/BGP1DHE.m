BGP1DHE ; IHS/CMI/LAB - IHS GPRA 10 REPORT DRIVER ;
 ;;11.1;IHS CLINICAL REPORTING SYSTEM;;JUN 27, 2011;Build 33
 ;
START ;
 W:$D(IOF) @IOF
 W !!,$$CTR("2011 HEDIS Clinical Performance Measure Report",80)
INTRO ;
 D XIT
 W !!,"This will produce a HEDIS Performance Measure Report for all HEDIS measures"
 W !,"for a year period you specify.  You will be asked to provide: 1) the"
 W !,"reporting period, 2) the baseline period to compare data to, 3) the "
 W !,"community taxonomy to determine which patients will be included, and"
 W !,"4) the patient population (i.e. AI/AN only, non AI/AN, or both) to"
 W !,"determine which patients will be included."
...
SUM ;display summary of this report
 ...
 W !!,"All HEDIS measures will be calculated."
 D PT^BGP1DHSL
 I BGPROT="" G BEN
ZIS ;call to XBDBQUE
 D REPORT^BGP1HUTL
 ...
 I BGPRPT="" D XIT Q
 I BGPEXPT D
 .W !!,"A file will be created called BG11",$P(^AUTTLOC(DUZ(2),0),U,10)_".HE"_BGPRPT," and will reside",!,"in the ",BGPUF," directory.",!
 ...
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPHEDCB(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPHEDPB(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPHEDBB(" D ^DIK K DIK D XIT Q
 I $D(IO("Q")) G TSKMN
DRIVER ;
 D ^BGP1D1
 U IO
 D ^BGP1DHEP
 D ^%ZISC
 I BGPEXPT D GS^BGP1HUTL
 D XIT
 Q
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP1DHE",XBRX="XIT^BGP1DHE",XBNS="BGP"
 D ^XBDBQUE
 ;D XIT
 Q
 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP1DHE",ZTDTH="",ZTDESC="HEDIS 09 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
LISTS ;any lists with measures?
 ...
 I $D(BGPLIST) D RT^BGP1DHSL I '$D(BGPLIST)!($D(BGPQUIT)) G LISTS ;get report type for each list
 Q
CHKY ;
