BGP1DHE1 ; IHS/CMI/LAB - measure HEDIS ;
...
I2 ;EP
 ...
 I $$CRC^BGP1D61(DFN,BGPEDATE) S BGPSTOP=1 Q  ;has colorectal cancer
 S BGPFOB=$$FOB^BGP1D62(DFN,BGPBDATE,BGPEDATE)
 S BGPSIG=$$SIG^BGP1D62(DFN,BGPEDATE)
 S BGPCOLO=$$COLO^BGP1D62(DFN,BGPEDATE)
 ;S BGPBE=$$BE^BGP1D61(DFN,BGPEDATE)
 ;S BGPRECT=$$RECT^BGP1D61(DFN,BGPEDATE)
 I BGPFOB]""!(BGPSIG]"")!(BGPCOLO]"") S BGPN1=1
 ;S BGPREF="" I 'BGPN1 S BGPREF=$$REF^BGP1D61(DFN,$$FMADD^XLFDT(BGPEDATE,-365),BGPEDATE) I BGPREF S BGPN4=1
 S BGPVALUE=""
 I BGPFOB]"" S BGPVALUE="FOBT: "_$P(BGPFOB,U)_" "_$$DATE^BGP1UTL($P(BGPFOB,U,2))_" "
 ...
 ;I BGPREF]"" S BGPVALUE=BGPVALUE_$P(BGPREF,U,3)_":"_$$DATE^BGP1UTL($P(BGPREF,U,2))
...
I5 ;EP
 ...
 S BGPN1=$$CHL^BGP1D53(DFN,BGP365,BGPEDATE)
 S BGPVALUE="AC"
...
I7 ;EP
 ...
 I $$V2IHD^BGP1D9(DFN,BGP365,BGPEDATE),$$FIRSTIHD^BGP1D9(DFN,BGPEDATE) S BGPIHD=1,BGPD1=1
 I 'BGPD1 S BGPSTOP=1 Q  ;not ihd patient
 S (BGPVALUE,BGPLP,BGPLDL,BGPHDL,BGPTRI)=""
 S BGPLDL=$$LDL^BGP1D2(DFN,BGP365,BGPEDATE) ;date^value
 ...
 S %=$S($P(BGPLDL,U,2):$$DATE^BGP1UTL($P(BGPLDL,U,2))_" "_$S($P(BGPLDL,U,3)]"":$P(BGPLDL,U,3),1:"no result"),1:"")
...
73 S BGPEYE=$$EYE(DFN,BGP365,BGPEDATE)
 ...
 I BGPN3 S BGPVALUE=BGPVALUE_";eye: "_$$DATE^BGP1UTL($P(BGPEYE,U,2))_" "_$P(BGPEYE,U,3)
 K BGPG
 K ^TMP($J,"A")
74 ;
 S BGPLDL=$$LDL^BGP1D2(DFN,BGP365,BGPEDATE)
...
741 ;
 I BGPN4 S BGPVALUE=BGPVALUE_";LDL: "_$$DATE^BGP1UTL($P(BGPLDL,U,2))_" "_$P(BGPLDL,U,3)
 K X,Y,Z,%,A,B,C,D,E,H,BDATE,EDATE,P,V,S,F,BGPLDL,BGPHDL,BGPTRI,BGPLP
77 ;
 S BGPUP=$$MEDNEPH^BGP1DHE2(DFN,BGP365,BGPEDATE)
 S BGPN7=0 I $P(BGPUP,U)=1 S BGPN7=1
 I BGPN7 S BGPVALUE=BGPVALUE_";NEP SCR/EVID: "_$$DATE^BGP1UTL($P(BGPUP,U,4))_" "_$P(BGPUP,U,2)_" "_$P(BGPUP,U,3)
BP ;
...
I013 ;EP
 ...
 S BGPVALUE=$$DENTSRV^BGP1D21(DFN,BGPBDATE,BGPEDATE,1)
 S BGPN1=0 I $P(BGPVALUE,U) S BGPN1=1
 S BGPVALUE=$S(BGPD1:"UP",1:"")_$S(BGPD2:";AD",1:"")_"|||"_$$DATE^BGP1UTL($P(BGPVALUE,U,2))_" "_$P(BGPVALUE,U,3)
 Q
...
EYE(P,BDATE,EDATE) ;EP  HEDIS ONLY
 ...
 K BGPG S BGPG=$$CPT^BGP1DU(P,BDATE,EDATE,T,6)
 I $P(BGPG,U)=1 Q "1^"_$P(BGPG,U,2)_"^Ret Scr CPT : "_$P(BGPG,U,3)
 K BGPG S BGPG=$$TRAN^BGP1DU(P,BDATE,EDATE,T,6)
 I $P(BGPG,U)=1 Q "1^"_$P(BGPG,U,2)_"^Ret Scr TRAN : "_$P(BGPG,U,3)
 K BGPG S BGPG=$$LASTPRC^BGP1UTL1(P,"BGP HEDIS RETINAL SCRN PROCS",BDATE,EDATE)
...
HGBA1C(P,BDATE,EDATE) ;EP
 ...
 I X="" D  Q G
 .S G=""
 .I $P(BGPT(D,C),U,2)="CPT 3046F"!($P(BGPT(D,C),U,2)="CPT 83036")!($P(BGPT(D,C),U,2)="CPT 83037") S G=1_U_1_U_0_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D) Q
 .I $P(BGPT(D,C),U,2)="CPT 3047F"!($P(BGPT(D,C),U,2)="CPT 3045F") S G=1_U_0_U_0_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D) Q
 .I $P(BGPT(D,C),U,2)="CPT 3044F" S G=1_U_0_U_1_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D) Q
 .S G=1_U_1_U_0_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D)_" (no result)" Q
 S X=$$STRIP^XLFSTR(X," ")  ;strip spaces
 I X[">9" Q 1_U_1_U_0_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D)_" "_X
 S X=$$STV^BGP1EO1(X)
 I X="" Q 1_U_1_U_0_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D)_" (no result) "_X
 I +X>9 Q 1_U_1_U_0_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D)_" "_X
 I +X<7 Q 1_U_0_U_1_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D)_" "_X
 Q 1_U_0_U_0_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D)_" result: "_X
 ;
