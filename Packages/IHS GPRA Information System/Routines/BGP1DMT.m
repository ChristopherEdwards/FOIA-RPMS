BGP1DMT ; IHS/CMI/LAB - national patient list 20 Dec 2004 9:24 AM ;
...
QUE ;
 ...
 S ZTRTN="PRINT^BGP1DMT",ZTDESC="BGP 11 MED TAX REPORT",ZTIO=ION,ZTDTH=""
 D ^%ZTLOAD
 D HOME^%ZIS
 D XIT
 Q
...
N ;GATHER UP AND DISPLAY ALL NATIONAL GPRA
 ...
 S BGPTNAME="" F  S BGPTNAME=$O(^BGPTAXB("B",BGPTNAME)) Q:BGPTNAME=""!(BGPQUIT)  D
 .S BGPTIEN=0,BGPTIEN=$O(^BGPTAXB("B",BGPTNAME,BGPTIEN))
 .Q:'BGPTIEN  ;oops, error in xref
 .Q:'$D(^BGPTAXB(BGPTIEN,0))  ;oops, error in xref
 .Q:$P(^BGPTAXB(BGPTIEN,0),U,2)'="M"  ;only DRUG/MEDICATION taxonomies in this report
 .S (G,X)=0 F  S X=$O(^BGPTAXB(BGPTIEN,12,"B",X)) Q:X'=+X!(G)  I $D(BGPRT(X)) S G=1
 .Q:'G
 .;now eliminate those in BGPNO
 .S (G,X)=0 F  S X=$O(^BGPTAXB(BGPTIEN,12,"B",X)) Q:X'=+X!(G)  D
 ..I $D(BGPNO(X)) S G=1
 .Q:G
...
INTROT ;introductory text
 ...
 ;;CRS 2011.  The report is organized by (1) taxonomies included in the
