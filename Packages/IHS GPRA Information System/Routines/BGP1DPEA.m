BGP1DPEA ; IHS/CMI/LAB - IHS GPRA 10 REPORT DRIVER 23 Oct 2009 2:04 PM ;
 ;;11.1;IHS CLINICAL REPORTING SYSTEM;;JUN 27, 2011;Build 33
 ;
 ;
 W:$D(IOF) @IOF
 W !,$$CTR("IHS 2011 Area Aggregate Patient Education Report",80),!!
INTRO ;
 D XIT
...
ASU ;
 ...
 I C=1 S BGPRPTT="F",BGPSUCNT=1,Y=$O(BGPSUL(0)),X=$P(^BGPPEDCB(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 I C>1 S BGPRPTT="A"
ZIS ;call to XBDBQUE
 D ^XBFMK
 S X=0 F  S X=$O(^BGPPEIB(X)) Q:X'=+X  S BGPIND(X)=""
...
DRIVER ;
 ...
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP1DPEA",XBRX="XIT^BGP1DPEA",XBNS="BGP"
 D ^XBDBQUE
 ;D XIT
 Q
 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP1DPEA",ZTDTH="",ZTDESC="PAT ED 09 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
