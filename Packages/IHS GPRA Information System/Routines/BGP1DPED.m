BGP1DPED ; IHS/CMI/LAB - IHS gpra print ;
...
DEL ;EP - create delimited output file
 ...
 K BGPSUMP
 D DEL1
 D ^BGP1DPEE ;print lists to delimited file
...
DEL1 ;EP
 ...
 .D S(" ",1,1),S(" ",1,1)
 .S X=$P(^BGPPEIB(BGPIC,0),U,2) D S(X,1,1)
 .D S(" ",1,1)
 .S X="Denominator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPPEIB(BGPIC,61,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIB(BGPIC,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPPEIB(BGPIC,61,BGPY,11,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S Y=^BGPPEIB(BGPIC,61,BGPY,11,BGPZ,0) S:BGPZ=1 Y=" - "_Y D S(Y,1,1)
 ....Q
 ...Q
 ..Q
 .D S(" ",1,1)
 .S X="Numerator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPPEIB(BGPIC,62,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIB(BGPIC,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPPEIB(BGPIC,62,BGPY,11,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S X=^BGPPEIB(BGPIC,62,BGPY,11,BGPZ,0) S:BGPZ=1 X=" - "_X D S(X,1,1)
 ....Q
 ...Q
 ..Q
 .D S(" ",1,1)
 .S BGPNODE=11
 .S BGPX=0 F  S BGPX=$O(^BGPPEIB(BGPIC,BGPNODE,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPPEIB(BGPIC,BGPNODE,BGPX,0) D S(X,1,1)
 .D @BGPIC
 Q
EXIT ;
 K ^TMP($J)
...
C(X,X2,X3) ;
 ...
 ;
1 ;
 D S(" ",1,1) ;S X=$P(^BGPPEIB(BGPIC,0),U,2) D S(X,1,1)
 D H1^BGP1PDL1
 D S(" ",1,1)
 ...
 Q
2 ;
 D S(" ",1,1) D S(" ",1,1) ;S X=$P(^BGPPEIB(BGPIC,0),U,2) D S(X,1,1)
 D H1^BGP1PDL1
 D S(" ",1,1)
 ...
 ..I $P($G(^BGPPEDCB(X,11)),U,4)]"",$P(^BGPPEDCB(X,11),U,4)<BGPCYD S BGPCYD=$P(^BGPPEDCB(X,11),U,4)
 ..I $P($G(^BGPPEDPB(X,11)),U,4)]"",$P(^BGPPEDPB(X,11),U,4)<BGPPRD S BGPPRD=$P(^BGPPEDPB(X,11),U,4)
 ..I $P($G(^BGPPEDBB(X,11)),U,4)]"",$P(^BGPPEDBB(X,11),U,4)<BGPBLD S BGPBLD=$P(^BGPPEDBB(X,11),U,4)
 ...
 .S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 ..I $P($G(^BGPPEDCB(X,11)),U,5)>BGPCYD S BGPCYD=$P(^BGPPEDCB(X,11),U,5)
 ..I $P($G(^BGPPEDPB(X,11)),U,5)>BGPPRD S BGPPRD=$P(^BGPPEDPB(X,11),U,5)
 ..I $P($G(^BGPPEDBB(X,11)),U,5)>BGPBLD S BGPBLD=$P(^BGPPEDBB(X,11),U,5)
 S X="" D S(X,1,1)
