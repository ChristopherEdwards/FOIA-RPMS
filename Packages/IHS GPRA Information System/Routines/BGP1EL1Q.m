BGP1EL1Q ; IHS/CMI/LAB - print ind ;
 ;;11.1;IHS CLINICAL REPORTING SYSTEM;;JUN 27, 2011;Build 33
 ;
I1AGEP ;EP
PR ; 
 D HEADER^BGP1DPH W !,^BGPELIB(BGPIC,53,1,0) S X=" " D S(X,1,1) D H3^BGP1EL1P S X=" " D S(X,1,1)
 ...
 ;F X=1:1:3 S V=$P(BGPDAP(X),U,20) S Y=$$C(V,0,6) D S(Y,,X+1)
 ;S X="% I. w/other benzodiazepine Rx" D S(X,1,1) S X="  w/ % of total meds" D S(X,1,1)
 ;F X=1:1:3 S V=$P(BGPDAP(X),U,21) S Y=$J(V,5,1) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="I. # w/calcium channel blocker Rx" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,22) S Y=$$C(V,0,6) D S(Y,,X+1)
 S X="% I. w/calcium channel blocker Rx" D S(X,1,1) S X="  w/ % of total meds" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,23) S Y=$J(V,5,1) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="J. # w/gastro antispasmodic Rx" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,24) S Y=$$C(V,0,6) D S(Y,,X+1)
 S X="% J. w/gastro antispasmodic Rx" D S(X,1,1) S X="  w/ % of total meds" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,25) S Y=$J(V,5,1) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="K. # w/belladonna alkaloid Rx" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,26) S Y=$$C(V,0,6) D S(Y,,X+1)
 S X="% K. w/belladonna alkaloid Rx" D S(X,1,1) S X="  w/ % of total meds" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,27) S Y=$J(V,5,1) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="L. # w/skeletal muscle relaxant Rx" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,28) S Y=$$C(V,0,6) D S(Y,,X+1)
 S X="% L. w/skeletal muscle relaxant Rx" D S(X,1,1) S X="  w/ % of total meds" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,29) S Y=$J(V,5,1) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="M. # w/oral estrogen Rx" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,30) S Y=$$C(V,0,6) D S(Y,,X+1)
 S X="% M. w/oral estrogen Rx" D S(X,1,1) S X="  w/ % of total meds" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,31) S Y=$J(V,5,1) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="N. # w/oral hypoglycemic Rx" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,32) S Y=$$C(V,0,6) D S(Y,,X+1)
 S X="% N. w/oral hypoglycemic Rx" D S(X,1,1) S X="  w/ % of total meds" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,33) S Y=$J(V,5,1) D S(Y,,X+1)
 D HEADER^BGP1DPH W !,^BGPELIB(BGPIC,53,1,0) S X=" " D S(X,1,1) D H3^BGP1EL1P S X=" " D S(X,1,1)
 S X=" " D S(X,1,1) S X="O. # w/narcotic Rx" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,34) S Y=$$C(V,0,6) D S(Y,,X+1)
 S X="% O. w/narcotic Rx" D S(X,1,1) S X="  w/ % of total meds" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,35) S Y=$J(V,5,1) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="P. # w/vasodilator Rx" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,36) S Y=$$C(V,0,6) D S(Y,,X+1)
 S X="% P. w/vasodilator Rx" D S(X,1,1) S X="  w/ % of total meds" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,37) S Y=$J(V,5,1) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="Q. # w/other avoid meds Rx" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,38) S Y=$$C(V,0,6) D S(Y,,X+1)
 S X="% Q. w/other avoid meds Rx" D S(X,1,1) S X="  w/ % of total meds" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,39) S Y=$J(V,5,1) D S(Y,,X+1)
 ...
 S BGP2="",BGP1="I. # w/calcium channel ",BGP2=" blocker Rx",BGPPIE=23 D PPC
 S BGP2="",BGP1="J. # w/gastro antispasmodic Rx",BGPPIE=25 D PPC
 S BGP2="",BGP1="K. # w/belladonna alkaloid Rx",BGPPIE=27 D PPC
 S BGP2="",BGP1="L. # w/skeletal muscle ",BGP2=" relaxant Rx",BGPPIE=29 D PPC
 S BGP2="",BGP1="M. # w/oral estrogen Rx",BGPPIE=31 D PPC
 S BGP2="",BGP1="N. # w/oral hypoglycemic Rx",BGPPIE=33 D PPC
 S BGP2="",BGP1="O. # w/narcotic Rx",BGPPIE=35 D PPC
 S BGP2="",BGP1="P. # w/vasodilator Rx",BGPPIE=37 D PPC
 S BGP2="",BGP1="Q. # w/other avoid meds Rx",BGPPIE=39 D PPC
...
S(Y,F,P) ;set up array
 ;Q:BGPQUIT
 ;I $Y>(IOSL-5) D HEADER^BGP1DPH Q:BGPQUIT
 ;W !,^BGPELIB(BGPIC,53,1,0) S X=" " D S(X,1,1) D H3 S X=" " D S(X,1,1)
 I $G(F) F J=1:1:F W !
