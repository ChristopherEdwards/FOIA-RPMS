BGP1ELL ; IHS/CMI/LAB - IHS gpra print 01 Jul 2010 7:56 PM ;
...
DEL ;
 ...
 D ^BGP1ELHH
 D DEL1
 D ^BGP1ELSL ;print lists to delimited file
 ...
 I '$D(BGPGUI) D
 .S XBFLT=1,XBFN=BGPDELF_".txt",XBMED="F",XBTLE="CRS 2011 ELDER DELIMITED OUTPUT",XBQ="N",XBF=0
 .;S XBUF=BGPUF
 .D ^XBGSAVE
...
DEL1 ;EP
 ...
 .S X=$P(^BGPELIB(BGPIC,0),U,3) D S(X,1,1)
 .S X=" " D S(X,1,1)
 .S X="Denominator(s):" D S(X,1,1)
 .S BGPNODE=61
 .S BGPX=0 F  S BGPX=$O(^BGPELIB(BGPIC,61,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPELIB(BGPIC,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...;S BGPZ=0 F  S BGPZ=$O(^BGPELIB(BGPIC,61,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ...;.S Y=^BGPELIB(BGPIC,61,BGPY,1,BGPZ,0) D S(Y,1,1)
 ...D WP ;.Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 ...
 .S BGPNODE=62
 .S BGPX=0 F  S BGPX=$O(^BGPELIB(BGPIC,62,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPELIB(BGPIC,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...;S BGPZ=0 F  S BGPZ=$O(^BGPELIB(BGPIC,62,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ...;.S X=^BGPELIB(BGPIC,62,BGPY,1,BGPZ,0) D S(X,1,1)
 ...D WP ;.Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .I $O(^BGPELIB(BGPIC,11,0)) D S("Logic",1,1) S BGPX=0 F  S BGPX=$O(^BGPELIB(BGPIC,11,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPELIB(BGPIC,11,BGPX,0) D S(X,1,1)
 .I $O(^BGPELIB(BGPIC,51,0)) D S(" ",1,1) S X="Performance Measure Description" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPELIB(BGPIC,51,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPELIB(BGPIC,51,BGPX,0) D S(X,1,1)
 .I $O(^BGPELIB(BGPIC,52,0)) S X=" " D S(X,1,1) D S("Past Performance and/or Target",1,1) S BGPX=0 F  S BGPX=$O(^BGPELIB(BGPIC,52,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPELIB(BGPIC,52,BGPX,0) D S(X,1,1)
 .X ^BGPELIB(BGPIC,4)
 Q
HEADER ;EP
 S Y=$P(^VA(200,DUZ,0),U,2),$E(Y,35)=$$FMTE^XLFDT(DT) D S(Y,1,1)
 I BGPRTYPE=3 S Y="*** IHS 2011 ELDER CARE Clinical Performance Report***" D S(Y,1,1)
