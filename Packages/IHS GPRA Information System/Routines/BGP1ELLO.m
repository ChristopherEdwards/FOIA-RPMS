BGP1ELLO ; IHS/CMI/LAB - print ind ;
...
I1AGEP ;
 ...
 S X="# w/osteoporosis screening in past 2 years-No Refusals" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,2) S Y=V D S(Y,,X+1)
 S X="% w/osteoporosis screening in past 2 years-No Refusals" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,3) S Y=$$SB^BGP1ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="# w/ Osteoporosis Screening Refusals" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,4) S Y=V D S(Y,,X+1)
 S X="% w/ Osteoporosis Screening Refusals" D S(X,1,1)
