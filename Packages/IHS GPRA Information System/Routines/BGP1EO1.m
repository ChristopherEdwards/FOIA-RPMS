BGP1EO1 ; IHS/CMI/LAB - calc measures 29 Apr 2010 7:38 PM 14 Nov 2006 5:02 PM 12 Nov 2009 11:03 AM 07 Apr 2010 7:00 AM ;
...
EODML1 ;EP
 ...
 S BGPD1=1  ;is active diabetic
 S BGPN1=0,BGPVALUE="",BGPV=""
 S BGPLDL=$$LDL^BGP1D2(DFN,BGPBDATE,BGPEDATE)
 ;now evaluate result
 S BGPN4=0 D
 ...
 S BGPN1=BGPN4
 ;
 S BGPV="" I BGPLDL]"" S BGPV="LDL DONE: "_$$DATE^BGP1UTL($P(BGPLDL,U,2))_" "_$P(BGPLDL,U,3)
 S BGPVALUE="AD||| "_BGPV
 Q
 ;
EOOX ;
 ...
 ;
 S BGPVALUE="UP "_BGPD_"||| "_"NUM: "_BGPN
 Q
 ;
EOST ;
 NEW BGPOXV,BGPD,BGPN
 ...
 .S BGPN=BGPN_$P(BGPOXV(C),U,1)_" THERAPY: "_$P($P(BGPOXV(C),U,2)," ",2)
 .Q
 .;S BGPD=BGPD_$S(BGPD]"":"; ",1:"")_$P(BGPOXV(C),U)
 .;S BGPN=BGPN_$S(BGPN]"":"; ",1:"")_$P(BGPOXV(C),U,2)
 ;
...
HGBA1C(P,BDATE,EDATE) ;EP
 ...
 S BGPC=0
 S G=$$CPT^BGP1DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP HGBA1C CPTS",0)),5)
 ...
 S C=0,C=$O(BGPT(D,C))
 S X=$P(BGPT(D,C),U,1)
 I $$UP^XLFSTR(X)="COMMENT" Q 1_U_1_U_0_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D)_" (no result) "_X
 I X="" D  Q G
 .S G=""
 .I $P(BGPT(D,C),U,2)="CPT 3046F" S G=1_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D) Q
 .I $P(BGPT(D,C),U,2)="CPT 3047F"!($P(BGPT(D,C),U,2)="CPT 3044F")!($P(BGPT(D,C),U,2)="CPT 3045F") S G=0_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D) Q
 .S G=1_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D)_" (no result)" Q
 S X=$$STRIP^XLFSTR(X," ")  ;strip spaces
 I X[">9" Q 1_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D)_" "_X
 S X=$$STV(X)
 I X="" Q 1_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D)_" (no result) "_X
 I +X>9 Q 1_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D)_" "_X
 Q 0_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP1UTL(9999999-D)_" result: "_X
 ;
...
TIAFIB(P,BDATE,EDATE,BGPR) ;EP
 ...
 ..S BGPC=BGPC+1,BGPR(BGPC)=BGPC_") "_$$DATE^BGP1UTL(BGPD)_" "_$P(BGPX(BGPD,BGPV),U,15)_"+427.31"  ;set denominator
 ..S G=$$ANTICOAG^BGP1EO11(P,$$FMADD^XLFDT(BGPD,-365),$$DSCHDATE^APCLV(BGPV),BGPD)  ; any ANTICOAG?
...
PNEUOX(P,BDATE,EDATE,BGPR) ;EP
 ...
 S BGPD=0,BGPC=0 F  S BGPD=$O(BGPX(BGPD)) Q:BGPD'=+BGPD  D
 .S BGPV=0 F  S BGPV=$O(BGPX(BGPD,BGPV)) Q:BGPV'=+BGPV  D
 ..S BGPC=BGPC+1,BGPR(BGPC)=BGPC_") "_$$DATE^BGP1UTL(BGPD)_" "_$P(BGPX(BGPD,BGPV),U,2)  ;set denominator
...
OXSAT(V) ;was there ox sat at the visit
 ...
 S X=0 F  S X=$O(^AUPNVMSR("AD",V,X)) Q:X'=+X!(BGPG]"")  I $$VAL^XBDIQ1(9000010.01,X,.01)="O2" S BGPG=$$DATE^BGP1UTL(BGPD)_" O2 MEAS^1"
 I BGPG]"" Q BGPG
 ;now check for cpts
 ...
 .S BGPG=$$DATE^BGP1UTL(BGPD)_" CPT ["_$P($$CPT^ICPTCOD(C),U,2)_"]^1"
 .Q
 I BGPG]"" Q BGPG
 ;now check v tran
 S T=$O(^ATXAX("B","BGP CMS ABG CPTS",0))
 ...
 .Q:C=""
 .Q:'$$ICD^ATXCHK(C,T,1)
 .S BGPG=$$DATE^BGP1UTL(BGPD)_" CPT/TRAN ["_$P($$CPT^ICPTCOD(C),U,2)_"]^1"
 .Q
 I BGPG]"" Q BGPG
 ...
 .Q:'$D(^AUPNVLAB(X,0))
 .I BGPLT,$P(^AUPNVLAB(X,0),U),$D(^ATXLAB(BGPLT,21,"B",$P(^AUPNVLAB(X,0),U))) S BGPG=$$DATE^BGP1UTL(BGPD)_" MET "_$$VAL^XBDIQ1(9000010.09,X,.01)_"^1" Q
 .Q:'T
 .S J=$P($G(^AUPNVLAB(X,11)),U,13) Q:J=""
 .Q:'$$LOINC^BGP1D21(J,T)
 .S BGPG=$$DATE^BGP1UTL(BGPD)_" LAB "_$$VAL^XBDIQ1(9000010.09,X,.01)_"^1" Q
 I BGPG]"" Q BGPG
 ;now go get refusals of any of the above
 ;
 S G=$$REFUSAL^BGP1UTL1(P,9999999.07,$O(^AUTTMSR("B","O2",0)),BGPD,BGPD)
 I G Q $$DATE^BGP1UTL(BGPD)_" REFUSAL O2 SAT MEAS^2"
 ;refusal of lab tests
 S T=$O(^ATXLAB("B","BGP CMS ABG TESTS",0))
 S L=0 F  S L=$O(^ATXLAB(T,21,"B",L)) Q:L'=+L!(BGPG]"")  D
 .S G=$$REFUSAL^BGP1UTL1(P,60,L,BGPD,BGPD)
 .I G S BGPG=$$DATE^BGP1UTL(BGPD)_" Refused LAB "_$P(^LAB(60,L,0),U)_"^2"
 I BGPG]"" Q BGPG
 S G=$$CPTREFT^BGP1UTL1(P,BGPD,BGPD,$O(^ATXAX("B","BGP CMS ABG CPTS",0)))
 I G Q $$DATE^BGP1UTL(BGPD)_" Refused CPT "_$P(G,U,4)_"^2"
 Q $$DATE^BGP1UTL(BGPD)_" None^3"
 ;
