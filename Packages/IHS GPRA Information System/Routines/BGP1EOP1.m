BGP1EOP1 ; IHS/CMI/LAB - EO report print ;
 ;;11.1;IHS CLINICAL REPORTING SYSTEM;;JUN 27, 2011;Build 33
 ;
IREG ;EP
 I BGPPTYPE="P",$Y>(BGPIOSL-8) D HEADER^BGP1EOP Q:BGPQUIT
 ;D W^BGP1EOH(^BGPEOMB(BGPIC,53,1,0),0,2,BGPPTYPE)
 D H1^BGP1EOP
...
PI1 ;EP
 ...
 S BGPNP=$P(^DD(90547.1,BGPDF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 ...
 I 'BGPDENP D
 .I BGPPTYPE="P",$Y>(BGPIOSL-10) D HEADER^BGP1EOP Q:BGPQUIT  D W^BGP1EOH(^BGPEOMB(BGPIC,53,1,0),0,2,BGPPTYPE) D:$D(^BGPEOMB(BGPIC,53,2,0)) W^BGP1EOH(^BGPEOMB(BGPIC,53,2,0),0,1,BGPPTYPE) D H1^BGP1EOP
 .D W^BGP1EOH($P(^BGPEOMIB(BGPPC,0),U,9),0,2,BGPPTYPE)
 .I $P(^BGPEOMIB(BGPPC,0),U,10)]"" D W^BGP1EOH($P(^BGPEOMIB(BGPPC,0),U,10),0,1,BGPPTYPE)
 .I $P(^BGPEOMIB(BGPPC,0),U,11)]"" D W^BGP1EOH($P(^BGPEOMIB(BGPPC,0),U,11),0,1,BGPPTYPE)
 .I BGPPTYPE="P" D
 ..D W^BGP1EOH($$C(BGPCYD,0,8),0,0,BGPPTYPE,1,20)
 ..D W^BGP1EOH($$C(BGPPRD,0,8),0,0,BGPPTYPE,1,35)
 ..D W^BGP1EOH($$C(BGPBLD,0,8),0,0,BGPPTYPE,1,58)
 ..D W^BGP1EOH("",0,1,BGPPTYPE)
 .I BGPPTYPE="D" D
 ..S Y=BGPCYD_"^^"_BGPPRD_"^^^"_BGPBLD D W^BGP1EOH(Y,0,0,BGPPTYPE,2),W^BGP1EOH(" ",0,1,BGPPTYPE)
 .S BGPDENP=1
 S BGPNF=$P(^BGPEOMIB(BGPPC,0),U,5)  ;numerator field
 S BGPNP=$P(^DD(90547.1,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 D SETN
 ;write header
 D W^BGP1EOH($P(^BGPEOMIB(BGPPC,0),U,6),0,1,BGPPTYPE)
 I $P(^BGPEOMIB(BGPPC,0),U,7)]"" D W^BGP1EOH($P(^BGPEOMIB(BGPPC,0),U,7),0,1,BGPPTYPE,1,1)
 I $P(^BGPEOMIB(BGPPC,0),U,8)]"" D W^BGP1EOH($P(^BGPEOMIB(BGPPC,0),U,8),0,1,BGPPTYPE,1,1)
 D H2^BGP1EOP
 Q
 ;
