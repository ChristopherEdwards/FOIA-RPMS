BGP1GAEL ; IHS/CMI/LAB - AREA NATIONAL GPRA REPORT ;
...
EP1 ;
 ...
 S BGPZZ="A"
 S BGPSUCNT=C
 I BGPSUCNT=1 S Y=$O(BGPSUL(0)),X=$P(^BGPELDCB(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 ...
 S X=0 F  S X=$O(^BGPELIB(X)) Q:X'=+X  S BGPIND(X)=""
 S BGPINDB="E"
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOELD^BGP1GAEL",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI AREA ELDER REPORT 06" D ^%ZTLOAD Q
 Q
AOELD ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 ;cmi/anch/maw added 5/12/2010 for word output
 D GUIR^BGPXBLM("PRINT^BGP1PHEL","^TMP($J,""BGPGUI"",")
