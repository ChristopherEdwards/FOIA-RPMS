BGP1GAEO ; IHS/CMI/LAB - AREA NATIONAL GPRA REPORT 01 Jul 2010 7:58 PM ;
...
TESTNTL ;
 ...
 S BGPSUL(1)=""
 S BGPSUL(2)=""
 D EP(.ERR,1,2522,"BGP 11 AREA ELDER REPORT","A",.BGPSUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRPTT,BGPSUL,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPROT,BGPRTIME,BGPHC,BGPFILE) ;EP - called from GUI to produce national gpra report (AO-AGP)
 ...
 ;            from BGP 11 ELDER DATA CURRENT that match the following:
 ;            (SEE BGP1ASL or CALL ME)
 ;  BGPSUL  - array containing Iens from BGP 11 ELDER  DATA CURRENT that the user selected
...
EP1 ;
 ...
 .S BGPFGNT1="CRSEONT1"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP1UTL(BGPED)_$$D^BGP1UTL(BGPNOW)_"_"_$$LZERO^BGP1UTL(BGPC,6)_".TXT"
 .Q
 ;create entry in GUI file
 D ^XBFMK
 S X=BGPFILE
 S DIC="^BGPGUIB(",DIC(0)="L",DIADD=1,DLAYGO=90546.08,DIC("DR")=".02////"_BGPUSER_";.03////"_$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT)_";.05///"_BGPOPTN_";.06///R;.07///"_$G(BGPROT)_";1///"_$G(BGPFGNT1)
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOEO^BGP1GAEO",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI AREA EO REPORT 11" D ^%ZTLOAD Q
 Q
AOEO ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 ;cmi/anch/maw added 5/12/2010 for word output
 D GUIR^BGPXBLM("PRINT^BGP1EOP","^TMP($J,""BGPGUI"",")
