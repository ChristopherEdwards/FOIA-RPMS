BGP1GAGP ; IHS/CMI/LAB - AREA NATIONAL GPRA REPORT ;
...
TESTNTL ;
 ...
 S BGPSUL(1)=""
 S BGPSUL(2)=""
 D EP(.ERR,1,2522,"BGP 11 AREA GPRA","A",.BGPSUL,"B",1,$$NOW^XLFDT)
 W !,ERR
 Q
...
EP1 ;
 ...
 ;I BGPSUCNT=1 S BGPRPTT="F",Y=$O(BGPSUL(0)),X=$P(^BGPGPDCB(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOAGP^BGP1GAGP",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT 11" D ^%ZTLOAD Q
 Q
AOAGP ;
 K ^TMP($J,"BGPGUI")
