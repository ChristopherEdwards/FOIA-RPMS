BGP1GAON ; IHS/CMI/LAB - AREA OTHER NATIONAL MEASURES GPRA REPORT 01 Jul 2010 1:10 PM ;
...
TESTNTL ;
 ...
 S BGPSUL(1)=""
 S BGPSUL(2)=""
 D EP(.ERR,1,2522,"BGP 11 AREA OTHER NATIONAL MEASURES REPORT","A",.BGPSUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRPTT,BGPSUL,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPROT,BGPRTIME,BGPHC,BGPFILE) ;EP - called from GUI to produce national gpra report (AO-AGP)
 ...
 ;  BGPSUL  - array containing Iens from BGP 11 ONM DATA CURRENT that the user selected
 ;  BGPSUL - ARRAY OF IENS FROM THE D GET^BGP1ASL(.BGPSUL,.BGPFILE,5,....... call that the user selected
...
EP1 ;
 ...
 S IOM=80,BGPIOSL=55
 S BGPAREAA=1
 S BGPRTYPE=7,BGP1RPTH=""
 S BGPHOME=$P($G(^BGPSITE(DUZ(2),0)),U,2)
 S X=0 F  S X=$O(^BGPINDB("ON",1,X)) Q:X'=+X  S BGPIND(X)=""
 S BGPINDB="G"
 I '$G(BGPHC) D
 ...
 S BGPNOW=$$NOW^XLFDT() S BGPNOW=$P(BGPNOW,".")_"."_$$RZERO^BGP1UTL($P(BGPNOW,".",2),6)
 S BGPFONN1="CRSONMNT1"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP1UTL(BGPNOW)_"_"_$$LZERO^BGP1UTL(BGPC,6)_".TXT"
 S BGPFONN2="CRSONMNT2"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP1UTL(BGPNOW)_"_"_$$LZERO^BGP1UTL(BGPC,6)_".TXT"
 S BGPFONN3="CRSONMNT3"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP1UTL(BGPNOW)_"_"_$$LZERO^BGP1UTL(BGPC,6)_".TXT"
 S BGPFONN4="CRSONMNT4"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP1UTL(BGPNOW)_"_"_$$LZERO^BGP1UTL(BGPC,6)_".TXT"
 ;S BGPFONN5="CRSONMNT5"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP1UTL(BGPNOW)_"_"_$$LZERO^BGP1UTL(BGPC,6)_".TXT"
 ;create entry in GUI file
 D ^XBFMK
 S X=BGPFILE
 S DIC="^BGPGUIB(",DIC(0)="L",DIADD=1,DLAYGO=90546.08
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AONM^BGP1GAON",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI AREA ONM REPORT" D ^%ZTLOAD Q
 Q
AONM ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 ;cmi/anch/maw added 5/12/2010 for word output
 D GUIR^BGPXBLM("PRINT^BGP1PARQ","^TMP($J,""BGPGUI"",")
