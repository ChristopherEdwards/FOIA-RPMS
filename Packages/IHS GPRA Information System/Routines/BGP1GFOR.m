BGP1GFOR ; IHS/CMI/LAB - GUI Patient Forecast Report ;
 ;;11.1;IHS CLINICAL REPORTING SYSTEM;;JUN 27, 2011;Build 33
 ;
 ;
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRT1,BGPDIVI,BGPABD,BGPAED,BGPCLN,BGPPATS,BGPRTIME,BGPFILE,BGPANY,BGPAO,BGPAOD,BGPSTMP) ;EP - called from GUI to produce national GPU report (OTH-GPU)
 ;  BGPUSER - DUZ
 ;  BGPDUZ2 - DUZ(2)
 ;  BGPOPTN - OPTION NAME
...
EP1 ;
 ...
 S BGPGUI=1
 S IOM=80,BGPIOSL=55
 S BGPRTYPE=1,BGP1RPTH="",BGP1GPU=1,BGPALLPT=1,BGPBEN=3
...
NT ;
 ...
 S DIC="^BGPGUIB(",DIC(0)="L",DIADD=1,DLAYGO=90546.08,DIC("DR")=".02////"_BGPUSER_";.03////"_$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT)_";.05///"_BGPOPTN_";.06///R;.07///"_$G(BGPROT)_";1///"
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="GUIP^BGP1GFOR",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI GPU GPRA REPORT 06" D ^%ZTLOAD Q
 Q
GUIP ;
 D PROC^BGP1DPA
 K ^TMP($J,"BGP1DPA")
 S IOM=80,BGPIOSL=55
 D GUIR^BGPXBLM("PRINT^BGP1DPAW","^TMP($J,""BGP1DPA"",")
 ;cmi/anch/maw added 5/12/2009 for word output
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP1DPA",X)) Q:X'=+X  D
 . S C=C+1
 . N BGPDATA
 . S BGPDATA=$G(^TMP($J,"BGP1DPA",X))
