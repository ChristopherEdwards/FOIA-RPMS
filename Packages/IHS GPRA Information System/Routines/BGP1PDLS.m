BGP1PDLS ; IHS/CMI/LAB - IHS gpra print ;
...
GO .;
 ...
 .S X=$P(^BGPINDB(BGPL,0),U,3) D S(X,1,1)
 .S X=" " D S(X,1,1)
 ...
 .S X="Logic" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPINDB(BGPL,BGPNODE,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDB(BGPL,BGPNODE,BGPX,0) D S(X,1,1)
 .S X=" " D S(X,1,1) S X="Performance Measure Description" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDB(BGPL,51,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDB(BGPL,51,BGPX,0) D S(X,1,1)
 .I $O(^BGPINDB(BGPL,52,0)) S X=" " D S(X,1,1) S X="Past Performance and/or Target" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDB(BGPL,52,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDB(BGPL,52,BGPX,0) D S(X,1,1)
 .I $O(^BGPINDB(BGPL,55,0)) S X=" " D S(X,1,1) S X="Source" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDB(BGPL,55,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDB(BGPL,55,BGPX,0) D S(X,1,1)
 .D:'$G(BGPSUMON) H1
...
HEADER1 ;
 ...
 W !,$$CTR("***  IHS 2011 Clinical Performance Measure Patient List  ***",80),!
...
H1 ;
 ...
 S X=" " D S(X,1,1)
 S Y=0 F  S Y=$O(^BGPINDB(BGPL,71,Y)) Q:Y'=+Y  S X=^BGPINDB(BGPL,71,Y,0) D S(X,1,1)
 S X=" " D S(X,1,1)
