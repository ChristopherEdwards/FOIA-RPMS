BGP2ASL ; IHS/CMI/LAB - DISPLAY IND LISTS 28 Apr 2006 2:10 PM ;
...
EN ;EP -- main entry point
 D EN^VALM("BGP 12 AREA SELECT SITES")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ; -- init variables and list array
 I BGPRTYPE=1 D NGR^BGP2ASL1 Q
 I BGPRTYPE=3 D HED^BGP2ASL1 Q
 I BGPRTYPE=5 D ELD^BGP2ASL1 Q
 I BGPRTYPE=6 D PED^BGP2ASL1 Q
 I BGPRTYPE=7 D ONM^BGP2ASL1 Q
 I BGPRTYPE=8 D EO^BGP2ASL1 Q
 Q
FAC(S) ;
...
NT ;
 ...
 . S BGPFAC=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPGPDCW(BGPX,0),U,17):" (CHS ONLY)",1:"")
 . S BGPEBD=$$DATE^BGP2UTL($P(V,U))
 . S BGPEED=$$DATE^BGP2UTL($P(V,U,2))
 . S BGPEBBD=$$DATE^BGP2UTL($P(V,U,5))
 . S BGPEBED=$$DATE^BGP2UTL($P(V,U,6))
 . S BGPEDRR=$$DATE^BGP2UTL($P(V,U,13))
...
G5 ;
 ...
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPFILE=90549.03
 S BGPX=0 F  S BGPX=$O(^BGPELDCW(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPELDCW(BGPX,0))
 . S V=^BGPELDCW(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
 ...
 .Q:$P(V,U,14)'=BGPBEN
 . S BGPSU=$E($$SU($P(V,U,11)),1,10)
 . S BGPFAC=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPELDCW(BGPX,0),U,17):" (CHS ONLY)",1:"")
 . S BGPEBD=$$DATE^BGP2UTL($P(V,U))
 . S BGPEED=$$DATE^BGP2UTL($P(V,U,2))
 . S BGPEBBD=$$DATE^BGP2UTL($P(V,U,5))
 . S BGPEBED=$$DATE^BGP2UTL($P(V,U,6))
...
G6 ;
 ...
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPFILE=90549.03
 S BGPX=0 F  S BGPX=$O(^BGPPEDCW(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPPEDCW(BGPX,0))
 . S V=^BGPPEDCW(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
...
G9 ;
 ...
 S BGPBBD=BGPBD-X,BGPBBD=$E(BGPBBD,1,3)_$E(BGPBD,4,7)
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPX=0 F  S BGPX=$O(^BGPGPDCW(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPGPDCW(BGPX,0))
 . S V=^BGPGPDCW(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
