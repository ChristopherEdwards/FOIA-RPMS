BGP2C13 ; IHS/CMI/LAB - calc CMS measures 26 Sep 2004 11:28 AM ;
...
ARBPOV ;
 ...
 .I N["ARB"!(N["ANGIOTENSIN RECEPTOR BLOCKER") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP2UTL($P(BGPG(X),U))_" ADR POV "_$P(BGPG(X),U,2) Q
 ...
 .S Z=$P(^AUPNVPOV(Y,0),U,19) I Z]"",$P($$ICDDX^ICDCODE(Z),U,2)="E942.6" S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP2UTL($P(BGPG(X),U))_" ADR POV "_$P(BGPG(X),U,2)_" + E942.6"
 .Q
 ...
 .I N["ARB"!(N["ANGIOTENSIN RECEPTOR BLOCKER") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP2UTL($P(BGPG(X),U))_" ADR POV "_$P(BGPG(X),U,2)
 ...
 .I Y="V14.8"!($$ICD^ATXCHK(I,T,9)),N["ARB"!(N["ANGIOTENSIN RECEPTOR BLOCKER") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP2UTL($P(^AUPNPROB(X,0),U,8))_" ADR PROBLEM LIST "_Y_" "_N
 .Q
 ;now check allergy tracking
 S X=0 F  S X=$O(^GMR(120.8,"B",P,X)) Q:X'=+X  D
 .Q:$P($P($G(^GMR(120.8,X,0)),U,4),".")>BGPD  ;entered after discharge date
 .S N=$P($G(^GMR(120.8,X,0)),U,2),N=$$UP^XLFSTR(N)
 .I N["ARB"!(N["ANGIOTENSIN RECEPTOR BLOCKER") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP2UTL($P(^GMR(120.8,X,0),U,4))_" ADR ALLERGY TRACKING"
 Q
ARBCON1 ;EP does patient have an ARB allergy
 ...
 ...S BGPC=BGPC+1,BGPY(BGPC)="NMI ARB: "_$$VAL^XBDIQ1(9000022,N,.04)_"   "_$$DATE^BGP2UTL($P(^AUPNPREF(N,0),U,3))_"  "_$$VAL^XBDIQ1(9000022,X,1101)
 ..Q
 .Q
 Q:BGPIND'=2
 S X=$$CPTI^BGP2DU(P,BGPD,BGPDDT,+$$CODEN^ICPTCOD("G8029"))
 I X S BGPC=BGPC+1,BGPY(BGPC)="CPT code G8029: "_$$DATE^BGP2UTL($P(X,U,2))
 S X=$$TRANI^BGP2DU(P,BGPD,BGPDDT,+$$CODEN^ICPTCOD("G8029"))
 I X S BGPC=BGPC+1,BGPY(BGPC)="Tran Code G8029: "_$$DATE^BGP2UTL($P(X,U,2))
 Q
ARBRX1 ;EP
 ...
 D GETMEDS^BGP2CU(P,BD,ED,"BGP CMS ARB MEDS","BGP CMS ARB MEDS NDC","BGP CMS ARB MEDS CLASS")
 I BGPIND=2 D
 .S X=$$CPTI^BGP2DU(P,BD,ED,+$$CODEN^ICPTCOD("G8027"))
 .I X S BGPC=BGPC+1,BGPY(BGPC)="CPT code G8027: "_$$DATE^BGP2UTL($P(X,U,2))
 .S X=$$TRANI^BGP2DU(P,BD,ED,+$$CODEN^ICPTCOD("G8027"))
 .I X S BGPC=BGPC+1,BGPY(BGPC)="Tran Code G8027: "_$$DATE^BGP2UTL($P(X,U,2))
 S BD=BGPA
...
ACEICON1(P,BGPD,BGPDDT,BGPV,BGPY) ;EP have an ACEI allergy
 ...
 I $D(BGPG(1)) S BGPC=BGPC+1,BGPY(BGPC)="POV:  "_$$DATE^BGP2UTL($P(BGPG(1),U))_" ["_$P(BGPG(1),U,2)_"]    "_$$VAL^XBDIQ1(9000010.07,+$P(BGPG(1),U,4),.04)
 ;
 ...
 ..S N=0 F  S N=$O(^AUPNPREF("AA",P,50,X,D,N)) Q:N'=+N  D
 ...Q:$P($G(^AUPNPREF(N,0)),U,7)'="N"
 ...S BGPC=BGPC+1,BGPY(BGPC)="NMI ACEI: "_$$VAL^XBDIQ1(9000022,N,.04)_"   "_$$DATE^BGP2UTL($P(^AUPNPREF(N,0),U,3))_"  "_$$VAL^XBDIQ1(9000022,X,1101)
 ..Q
 .Q
 D ARBCON1
 Q
