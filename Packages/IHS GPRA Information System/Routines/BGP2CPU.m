BGP2CPU ; IHS/CMI/LAB - calc CMS measures 26 Sep 2004 11:28 AM 04 May 2010 2:38 PM 02 Jul 2010 9:11 AM ;
 ;;12.0;IHS CLINICAL REPORTING;;JAN 9, 2012;Build 51
 ;
WDOB(P) ;EP
 I $Y>(BGPIOSL-3) D HDR^BGP2CP Q:BGPQUIT  D L1H^BGP2CP
 W !?2,"Date of Birth:  ",$$DATE^BGP2UTL($P(^DPT(P,0),U,3))
 Q
 ;
WRACE(P) ;EP
 I $Y>(BGPIOSL-3) D HDR^BGP2CP Q:BGPQUIT  D L1H^BGP2CP
...
WINS(V,P) ;EP
 ...
 . I $Y>(BGPIOSL-3) D HDR^BGP2CP Q:BGPQUIT  D L1H^BGP2CP
 . W !?3,"Medicare Coverage Type:  ",$P(^AUPNMCR(P,11,I,0),U,3)," Policy #:  ",$P(^AUPNMCR(P,0),U,3)," Effective Date:  ",$$DATE^BGP2UTL($P(^AUPNMCR(P,11,I,0),U))
 . S MCR=1
 . Q
 ;medicaid
 Q:BGPQUIT
 ...
 ..S MCD=1
 ..I $Y>(BGPIOSL-3) D HDR^BGP2CP Q:BGPQUIT  D L1H^BGP2CP
 ..W !?3,"Medicaid ",$$VAL^XBDIQ1(9000004,I,.11)," Coverage Type:  ",$P(^AUPNMCD(I,11,J,0),U,3)," Policy #:  ",$P(^AUPNMCD(I,0),U,3),!?10,"  Effective Date:  ",$$DATE^BGP2UTL($P(^AUPNMCD(I,11,J,0),U))
 ..Q
 .Q
 ;pi
 Q:BGPQUIT
 ...
 . W " Coverage Type:  ",C,!?10,"  Policy #:  ",N,"  Effective Date:  ",$$DATE^BGP2UTL($P(^AUPNPRVT(P,11,I,0),U,6))
 ;RR
 Q:BGPQUIT
 S I=0
 ...
 . I $Y>(BGPIOSL-3) D HDR^BGP2CP Q:BGPQUIT  D L1H^BGP2CP
 . W !?3,"Railroad Coverage Type:  ",$P(^AUPNRRE(P,11,I,0),U,3)," Policy #:  ",$P(^AUPNRRE(P,0),U,4),"  Effective Date:  ",$$DATE^BGP2UTL($P(^AUPNRRE(P,11,I,0),U))
 . S MCR=1
 . Q
...
WCESSMED ;EP write out CESSATION DATA
 ...
 S X=0 F  S X=$O(BGPDATA(X)) Q:X'=+X  W !?4,$$DATE^BGP2UTL($P(BGPDATA(X),U))," ",$P(BGPDATA(X),U,2)
...
WCESS ;EP write out CESSATION DATA
 ...
 S BGPXX=0 F  S BGPXX=$O(BGPDATA(BGPXX)) Q:BGPXX'=+BGPXX  W !?4,$$DATE^BGP2UTL($P(BGPDATA(BGPXX),U))," ",$P(BGPDATA(BGPXX),U,2)
 ;W !
 Q
...
WLVSD ;EP write out lsvd/cef
 NEW X,C S (X,C)=0 F  S X=$O(BGPDATA(X)) Q:X'=+X  S C=C+1
 I $Y>(BGPIOSL-(C+5)) D HDR^BGP2CP Q:BGPQUIT  D L1H^BGP2CP
...
WIVUD ;EP - write out all allergies from problem list
 ...
 W !!?2,"ALL Unit Dose/IV Meds during Hospital Stay: ",$$DATE^BGP2UTL($P($P(^AUPNVSIT(BGPVSIT,0),U),"."))," - ",$$DATE^BGP2UTL($P($P(^AUPNVINP(BGPVINP,0),U),"."))
...
WLASTAP ;EP - write out last rx
 ...
 ..D GETTXT^BGP2CPU4
 ..S BGPZZ=0 F  S BGPZZ=$O(BGPLETXT(BGPZZ)) Q:BGPZZ'=+BGPZZ!(BGPQUIT)  D
 ...D:$Y>(BGPIOSL-3) HDR^BGP2CP Q:BGPQUIT  W !?6,BGPLETXT(BGPZZ)
 ;W !
 Q
...
WDOD(V) ;EP - write dod
 I $Y>(BGPIOSL-2) D HDR^BGP2CP Q:BGPQUIT  D L1H^BGP2CP
 I $$DOD^AUPNPAT(V)]"" D
 .W !!?2,$S(BGPPEX[1!(BGPPEX[5):"*",1:""),"Date of Death: ",$$DATE^BGP2UTL($$DOD^AUPNPAT(V))
 Q
 ;
