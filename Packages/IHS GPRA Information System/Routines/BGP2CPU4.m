BGP2CPU4 ; IHS/CMI/LAB - calc CMS measures ;
 ...
 ;
WPOSTINF ;EP
 I $Y>(BGPIOSL-8) D HDR^BGP2CP Q:BGPQUIT  D L1H^BGP2CP
...
PERI ;EP
 ...
 W !!?2,$S(BGPPEX[1:"*",1:""),"Preoperative Infectious Disease Diagnosis?  ",$P($$ADMPRIM^BGP2CU5(BGPVINP,"BGP CMS INFECTIOUS DXS"),U,2)
 Q
 ;
WANTIRX ;EP
...
OTHSURG ;EP
 ...
 K BGPG
 S Y="BGPG("
 S X=DFN_"^ALL PROCEDURES;DURING "_$$VD^APCLV(BGPVSIT)_"-"_$$DSCH^BGP2CU(BGPVINP) S E=$$START1^APCLDF(X,Y)
 ...
 .W !?4,$$VAL^XBDIQ1(9000010.08,X,.01),"  ",$$DATE^BGP2UTL(D),"  ",$$VAL^XBDIQ1(9000010.08,X,.04) S F=1
 Q:'F
...
WPP1 ;EP
 ...
 W !?2,$S(BGPPPD<$P($P(BGPVSIT0,U),"."):"*",1:""),"Principle Procedure Date:  ",$$DATE^BGP2UTL(BGPPPD)
 Q
 ;
WPP ;EP
 I $Y>(BGPIOSL-3) D HDR^BGP2CP Q:BGPQUIT  D L1H^BGP2CP
