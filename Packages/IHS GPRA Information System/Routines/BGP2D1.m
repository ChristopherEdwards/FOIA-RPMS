BGP2D1 ; IHS/CMI/LAB - calc measures 29 Apr 2010 7:38 PM 14 Nov 2006 5:02 PM 12 Nov 2009 11:03 AM 27 May 2012 10:08 AM 12 May 2012 6:04 PM ; 26 Oct 2011  1:41 PM
...
PROC ;EP
 ...
 D XTMP^BGP2UTL("BGP28CPL","National GPRA Comp List")
 D XTMP^BGP2UTL("BGP2DNP","National GPRA Patient List")
 D XTMP^BGP2UTL("BGP2D","CRS Local Patient List")
 ...
 .;I DUZ=5634,'$D(^DIBT(4862,1,DFN)) Q
 .;I DUZ=5634,DFN'=28297 Q
 .;Q:DFN'=1241
 .;I DUZ=5634 Q:$$HRN^AUPNPAT(DFN,DUZ(2))'=900381
...
N ;
 I BGPRTYPE=1,$G(BGPEXPT),'$G(BGPYGPU) S ^BGPGPDCW(BGPRPT,88888,0)="^90548.0388888A^"_BGPCHWC_"^"_BGPCHWC
 I $G(BGPYWCHW) S ^BGPGPDCW(BGPRPT,88888,0)="^90548.0388888A^"_BGPCHWC_"^"_BGPCHWC
 S BGPET=$H
 Q
 ;
PROCCY ;EP - current time period
 ...
 S BGPEDATE=BGPED,BGPTIME=1,BGPBDATE=BGPBD,BGPGBL=$S(BGPRTYPE=3:"^BGPHEDCB(",BGPRTYPE=5:"^BGPELDCW(",BGPRTYPE=8:"^BGPEOCB(",1:"^BGPGPDCW(")
 S BGP365=BGPBDATE
 ...
 S BGPACTCL=$$ACTCL(DFN,BGP3YE,BGPEDATE,BGPCHSO,BGPURBAN) ;active clinical
 S BGPACTCB=$$ACTCLBH(DFN,BGP3YE,BGPEDATE,BGPCHSO,BGPURBAN)  ;active clinical + BH
 ...
 I $G(BGPIISO) D ISS^BGP2DPA1 K ^TMP($J,"A") Q  ;SCHEDULING OPTION
 D CALCIND
 I BGPRTYPE=1,$G(BGPYWCHW),'$G(BGPYGPU) D
...
PROCPY ;
 ...
 Q:$P(^DPT(DFN,0),U,2)=""
 Q:"FM"'[$P(^DPT(DFN,0),U,2)
 S BGPEDATE=BGPPED,BGPTIME=2,BGPBDATE=BGPPBD,BGPGBL=$S(BGPRTYPE=3:"^BGPHEDPB(",BGPRTYPE=5:"^BGPELDPW(",BGPRTYPE=8:"^BGPEOPB(",1:"^BGPGPDPW(")
 S BGP365=BGPBDATE
 ...
 S BGPACTCL=$$ACTCL(DFN,BGPP3YE,BGPEDATE,BGPCHSO,BGPURBAN) ;active clinical
 S BGPACTCB=$$ACTCLBH(DFN,BGP3YE,BGPEDATE,BGPCHSO,BGPURBAN)  ;active clinical + BH
 S BGPAGEB=$$AGE^AUPNPAT(DFN,BGPBDATE)
 S BGPAGEE=$$AGE^AUPNPAT(DFN,BGPEDATE)
 S BGPSEX=$P(^DPT(DFN,0),U,2)
...
PROCBY ;
 ...
 Q:$P(^DPT(DFN,0),U,2)=""
 Q:"FM"'[$P(^DPT(DFN,0),U,2)
 S BGPEDATE=BGPBED,BGPTIME=3,BGPBDATE=BGPBBD,BGPGBL=$S(BGPRTYPE=3:"^BGPHEDBB(",BGPRTYPE=5:"^BGPELDBW(",BGPRTYPE=8:"^BGPEOBB(",1:"^BGPGPDBW(")
 S BGP365=BGPBDATE
 ...
 I 'BGPACTUP Q  ;if not in user pop, don't use patient
 S BGPACTCL=$$ACTCL(DFN,BGPB3YE,BGPEDATE,BGPCHSO,BGPURBAN) ;active clinical
 S BGPACTCB=$$ACTCLBH(DFN,BGP3YE,BGPEDATE,BGPCHSO,BGPURBAN)  ;active clinical + BH
 S BGPAGEB=$$AGE^AUPNPAT(DFN,BGPBDATE)
 S BGPAGEE=$$AGE^AUPNPAT(DFN,BGPEDATE)
 S BGPSEX=$P(^DPT(DFN,0),U,2)
 ...
 Q
CALCIND ;
 I BGPRTYPE=3 D CALCIND^BGP2DCHI Q
 I BGPRTYPE=5 D CALCIND^BGP2DCEI Q
 I BGPRTYPE=8 D CALCIND^BGP2EOCI Q
 D CALCIND^BGP2DCI
 Q
...
ACTCL(P,BDATE,EDATE,CHS,URBAN) ;EP - clinical user
 ...
 .Q
 Q $S(F:1,1:0)
ACTCLBH(P,BDATE,EDATE,CHS,URBAN) ;EP - clinical user
 I CHS G CHSACTCL
 I $G(URBAN) G URBACTCL
 S (X,G,F,S)=0 F  S X=$O(^TMP($J,"A",X)) Q:X'=+X!(F)  S V=$P(^TMP($J,"A",X),U,5) D
 .Q:'$D(^AUPNVSIT(V,0))
 ...
 .Q:B=""
 .I 'G,$D(^BGPCTRL($O(^BGPCTRL("B",2012,0)),11,"B",B)) S G=V  ;must be a primary clinic S G=V
 .I V'=G,$D(^BGPCTRL($O(^BGPCTRL("B",2012,0)),95,"B",B)) S S=1
 .I G,S S F=1
 .Q
 Q $S(F:1,1:0)
 ;
...
URBACTCL ;URBAN
 ...
 .S B=$$CLINIC^APCLV(V,"C")
 .Q:B=""
 .Q:'$D(^BGPCTRL($O(^BGPCTRL("B",2012,0)),1301,"B",B))
 .S F=F+1
 Q $S(F>1:1,1:0)
FIRSTDM(P,EDATE) ;
 I $G(P)="" Q ""
