...
ISTI ;EP
 ...
 ; BGPTIME=1 ZW  
 ;D EN^XBNEW("STI^BGP2D24","DFN;BGPBDATE;BGPEDATE;BGPYAR;BGPZAR")
 D EN^BKMSTIDS(DFN,BGPBDATE,BGPEDATE,"KEY",.BGPYAR,.BGPZAR)
 ...
 .;I H]"" S:BGPNUMV(X)]"" BGPNUMV(X)=BGPNUMV(X)_", " S BGPNUMV(X)=BGPNUMV(X)_"Prior HIV DX (Contraind): "_$$DATE^BGP2UTL(H)
...
IASB ;EP
 ...
 .S D=D_", BNI: "_$S($P(BGPERV(X),U,10)=1:$$DATE^BGP2UTL($P(BGPERV(X),U,8))_" Yes ER",1:" Yes Not ER")_" "_$P(BGPERV(X),U,9)
...
BNII(P,BD) ;
 NEW Y,BGPG,E,X,T,%,D
 S Y=$$CPT^BGP2DU(P,BD,$$FMADD^XLFDT(BD,7),$O(^ATXAX("B","BGP BNI CPTS",0)),6,"CT")
...
PCV(P,BDATE,EDATE) ;EP
 ...
 .S X($P(BGPG(X),U,5))=""
 .S C=C+1
 .S R=R_$S(R="":"",1:", ")_$$DATE^BGP2UTL($P($P(^AUPNVSIT($P(BGPG(X),U,5),0),U),"."))
 Q C_U_R
 ;
CANCER(P,BDATE,EDATE) ;EP - return # of different cancer visits
 NEW G,X
 S G=0
 S X=$$LASTDX^BGP2UTL1(P,"BGP MELANOMA CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP BREAST CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP COLON CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP GYN CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP PROSTATE CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP TESTES/MALE GU CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP HEAD AND NECK CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP URINARY TRACT CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP NON-MELANOMA CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP NON-COLON GI CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP LUNG CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP BRAIN CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP BONES/TISSUE CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP ENDOCRINE CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP PLEURA CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
 S X=$$LASTDX^BGP2UTL1(P,"BGP NON-SPECIFIC CANCER DXS",BDATE,EDATE)
 I X S G=G+1
 I G>1 Q 1
