BGP2D25 ; IHS/CMI/LAB - measure 6 03 Jun 2012 3:16 PM ;
...
IA ;EP
 ...
 I BGPDMD2 S BGPD7=1
 I $$IHD^BGP2D721(DFN,BGP365,BGPEDATE),BGPACTCL S BGPIHD=1,BGPD8=1
...
DEPEP ;EP - called from elder
 ...
 I BGPD5!(BGPD9) S BGPVALUD=$S(BGPD5:"AC",BGPD9:"AC",1:"")_"|||" D
 .I BGPDEPS]"" S BGPVALUD=BGPVALUD_$S($P(BGPVALUD,"|||",2)]"":"; SCREEN: ",1:"SCREEN: ") S BGPVALUD=BGPVALUD_$P(BGPDEPS,U,3)_" "_$P(BGPDEPS,U,5)
...
DEP(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(2)) S BGPDEP((9999999-$P(BGPG(1),U)))="",BGPDEP((9999999-$P(BGPG(2),U)))="" Q 1_U_$$DATE^BGP2UTL($P(BGPG(2),U))_" POV "_$P(BGPG(2),U,2)_" + "_$$DATE^BGP2UTL($P(BGPG(1),U))_" POV "_$P(BGPG(1),U,2)
 ...
 I BGPC>1 Q 1_"^"_$$FMTE^XLFDT((9999999-X))_" BH POV "_BGPDEP(X)_" + "_$$FMTE^XLFDT((9999999-Y))_" BH POV "_BGPDEP(Y)
 Q ""
DEPSUIC(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(1)) Q 1_U_"POV V62.84"_U_$$DATE^BGP2UTL($P(BGPG(1),U))
 ;
 S BGPC=""
 ...
 ..Q:'BGPP
 ..S BGPP=$P($G(^AMHPROB(BGPP,0)),U)
 ..I BGPP=39 S BGPC=1_U_"BH POV 39"_U_$$DATE^BGP2UTL((9999999-$P(D,".")))
 ..Q
 Q BGPC
DEPSCR(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(1)) S BGPDEPS=1_"^DEP SCRN^"_$$DATE^BGP2UTL($P(BGPG(1),U))_U_$P(BGPG(1),U)_U_"Ex 36"
 K BGPG
 S Y="BGPG("
 S X=P_"^LAST DX V79.0;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE) S E=$$START1^APCLDF(X,Y)
 I $D(BGPG(1)),$P(BGPDEPS,U,4)<$P(BGPG(1),U) S BGPDEPS=1_U_"POV V79.0 DEP"_U_$$DATE^BGP2UTL($P(BGPG(1),U))_U_$P(BGPG(1),U)_U_"POV V79.0"
 ;CPT CODE ADDED IN 11.1
 S Y=$$CPTI^BGP2DU(DFN,BDATE,EDATE,+$$CODEN^ICPTCOD("1220F"))
 I Y,$P(BGPDEPS,U,4)<$P(Y,U,2) S BGPDEPS=1_U_"CPT 1220F"_U_$$DATE^BGP2UTL($P(Y,U,2))_U_$P(Y,U,2)_U_"CPT 1220F"
 ;now add in v measurements
 S BGPC=$$LASTITEM^BGP2DU(P,BDATE,EDATE,"MEASUREMENT","PHQ2")
 I $P(BGPDEPS,U,4)<$P(BGPC,U,2) S BGPDEPS=1_U_"Meas "_$P(BGPC,U,3)_U_$$DATE^BGP2UTL($P(BGPC,U,2))_U_$P(BGPC,U,2)_U_"Meas "_$P(BGPC,U,3)
 S BGPC=$$LASTITEM^BGP2DU(P,BDATE,EDATE,"MEASUREMENT","PHQ9")
 I $P(BGPDEPS,U,4)<$P(BGPC,U,2) S BGPDEPS=1_U_"Meas "_$P(BGPC,U,3)_U_$$DATE^BGP2UTL($P(BGPC,U,2))_U_$P(BGPC,U,2)_U_"Meas "_$P(BGPC,U,3)
BHSCR ;
 ...
 ..Q:'BGPP
 ..S BGPP=$P($G(^AMHPROB(BGPP,0)),U)
 ..I BGPP=14.1 S BGPC=1_U_"BH 14.1"_U_$$DATE^BGP2UTL(9999999-D)_U_(9999999-D)_U_"BH POV 14.1" Q
 .Q:BGPC
 .S X=0 F  S X=$O(^AMHRMSR("AD",V,X)) Q:X'=+X!(BGPC)  S BGPP=$P($G(^AMHRMSR(X,0)),U) D
 ..Q:'BGPP
 ..S BGPP=$P($G(^AUTTMSR(BGPP,0)),U)
 ..I BGPP="PHQ2"!(BGPP="PHQ9") S BGPC=1_U_"BH "_BGPP_U_$$DATE^BGP2UTL(9999999-D)_U_(9999999-D)_U_"BH Meas "_BGPP
 .I $P($G(^AMHREC(V,14)),U,5)="P"!($P($G(^AMHREC(V,14)),U,5)="N") S BGPC=1_U_"BH Dep Exam"_U_$$DATE^BGP2UTL(9999999-D)_U_(9999999-D)_U_"BH Exam 36"
 ...
 .Q:Y<BDATE
 .Q:Y>EDATE
 .S BGPC=1_U_"PRIME MD SCORE HEALTH FACTOR"_U_$$DATE^BGP2UTL(9999999-D)_U_(9999999-D)_U_"PRIME MD SCORE"
 .Q
 I $P(BGPDEPS,U,4)<$P(BGPC,U,4) S BGPDEPS=BGPC
 Q BGPDEPS
DEPREF(P,BDATE,EDATE) ;EP
 NEW G
 S G=$$REFUSAL^BGP2UTL1(P,9999999.15,$O(^AUTTEXAM("C",36,0)),$$FMADD^XLFDT(EDATE,-365),EDATE)
 I $P(G,U)=1 Q 1_"^Refused Ex 36^"_$$DATE^BGP2UTL($P(G,U,2))_U_$P(G,U,2)
 Q ""
DEPEDU(P,BDATE,EDATE) ;EP
 ...
 .I $P(T,"-",1)="DEP"!($P(T,"-",1)="BH")!($P(T,"-",1)="SB")!($P(T,"-",1)="PDEP"),$P(BGPDEPE,U,4)<$P(BGPG(X),U,1) S BGPDEPE=1_U_""_T_U_$$DATE^BGP2UTL($P(BGPG(X),U))_U_$P(BGPG(X),U)
 .I $P(T,"-",1)["296.2"!($P(T,"-",1["296.3")),$P(BGPDEPE,U,4)<$P(BGPG(X),U,1) S BGPDEPE=1_U_""_T_U_$$DATE^BGP2UTL($P(BGPG(X),U))_U_$P(BGPG(X),U)
 .I $P(T,"-",1)["995.5"!($P(T,"-",1)="995.80")!($P(T,"-",1)="995.81")!($P(T,"-")="995.82")!($P(T,"-")="995.83")!($P(T,"-")="995.84")!($P(T,"-")="995.85"),$P(BGPDEPE,U,4)<$P(BGPG(X),U,1) D
 ..S BGPDEPE=1_U_T_U_$$DATE^BGP2UTL($P(BGPG(X),U))_U_$P(BGPG(X),U)
 .;I $P(T,"-",1)["V40."!($P(T,"-",1)="V60.0")!($P(T,"-",1)="V61.11")!($P(T,"-")="V61.21")!($P(T,"-")["V62.")!($P(T,"-")="V67.3")!($P(T,"-")["V69."),$P(BGPDEPE,U,4)<$P(BGPG(X),U,1) D
 .;.S BGPDEPE=1_U_"EDUC: "_T_U_$$DATE^BGP2UTL($P(BGPG(X),U))_U_$P(BGPG(X),U)
...
DEPMH ;
 ...
 ..Q:'$D(^AUTTEDT(T,0))
 ..S T=$P(^AUTTEDT(T,0),U,2)
 ..I $P(T,"-",1)="DEP"!($P(T,"-",1)="BH")!($P(T,"-",1)="SB")!($P(T,"-",1)="PDEP") S BGPC=1_U_T_U_$$DATE^BGP2UTL((9999999-D))_U_(9999999-D)
 ..I $P(T,"-",1)["296.2"!($P(T,"-",1["296.3")) S BGPC=1_U_T_U_$$DATE^BGP2UTL((9999999-D))_U_(9999999-D)
 ..I $P(T,"-",1)["995.5"!($P(T,"-",1)="995.80")!($P(T,"-",1)="995.81")!($P(T,"-")="995.82")!($P(T,"-")="995.83")!($P(T,"-")="995.84")!($P(T,"-")="995.85") D
 ...S BGPC=1_U_T_U_$$DATE^BGP2UTL((9999999-D))_U_(9999999-D)
 ..;I $P(T,"-",1)["V40."!($P(T,"-",1)="V60.0")!($P(T,"-",1)="V61.11")!($P(T,"-")="V61.21")!($P(T,"-")["V62.")!($P(T,"-")="V67.3")!($P(T,"-")["V69.") D
 ..;.S BGPC=1_U_"EDUC: "_T_U_$$DATE^BGP2UTL((9999999-D))_U_(9999999-D)
...
EDUREF ;
 ...
 ...I $P(Y,"-")="DEP"!($P(Y,"-",1)="BH")!($P(Y,"-",1)="SB")!($P(Y,"-",1)="PDEP") S G=1_U_"Refused "_Y_U_$$DATE^BGP2UTL(Z)
 ...;I $P(Y,"-",1)["296.2"!($P(Y,"-",1["296.3")) S G=1_U_"Ref "_Y_U_$$DATE^BGP2UTL(Z)
 ...;I $P(Y,"-",1)["995.5"!($P(Y,"-",1)="995.80")!($P(Y,"-",1)="995.81")!($P(Y,"-")="995.82")!($P(Y,"-")="995.83")!($P(Y,"-")="995.84")!($P(Y,"-")="995.85") S G=1_U_"Ref "_Y_U_$$DATE^BGP2UTL(Z)
 ...;I $P(Y,"-",1)["300.9"!($P(Y,"-",1["684.44")) S G=1_U_"Ref "_Y_U_$$DATE^BGP2UTL(Z)
 ...;I +$E($P(Y,"-",1),1,3)>289,+$E($P(Y,"-",1),1,3)<320 S G=1_U_"Ref "_Y_U_$$DATE^BGP2UTL(Z)
 Q G
