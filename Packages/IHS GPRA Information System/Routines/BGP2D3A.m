BGP2D3A ; IHS/CMI/LAB - measure 11 17 Oct 2009 12:40 PM 15 Apr 2012 8:06 AM ;
...
SEALDEV(P,BDATE,EDATE) ;EP
 ...
 S BGPC=0
 S %=P_"^ALL ADA 1351;DURING "_$$DATE^BGP2UTL(BDATE)_"-"_$$DATE^BGP2UTL(EDATE),E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP2UTL($P(BGPG(1),U,1))_" ADA 1351 "
 S %="",E=+$$CODEN^ICPTCOD("D1351"),%=$$CPTI^BGP2DU(P,BDATE,EDATE,E)
 I % Q 1_"^"_$$DATE^BGP2UTL($P(%,U,2))_" CPT D1351"
 S %=P_"^ALL ADA 1352;DURING "_$$DATE^BGP2UTL(BDATE)_"-"_$$DATE^BGP2UTL(EDATE),E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP2UTL($P(BGPG(1),U,1))_" ADA 1352 "
 S %="",E=+$$CODEN^ICPTCOD("D1352"),%=$$CPTI^BGP2DU(P,BDATE,EDATE,E)
 I % Q 1_"^"_$$DATE^BGP2UTL($P(%,U,2))_" CPT D1352"
 S T=$$FMADD^XLFDT(EDATE,-1096)
 S %=P_"^ALL ADA 0007;DURING "_$$DATE^BGP2UTL(T)_"-"_$$DATE^BGP2UTL(EDATE),E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP2UTL($P(BGPG(1),U,1))_" ADA 0007"
 S T=$$FMADD^XLFDT(EDATE,-1096)
 S %="",E=+$$CODEN^ICPTCOD("D0007"),%=$$CPTI^BGP2DU(P,T,EDATE,E)
 I % Q 1_"^"_$$DATE^BGP2UTL($P(%,U,2))_" CPT D0007"
 Q ""
TF(P,BDATE,EDATE) ;EP
 ...
 .S G=$$REFUSAL^BGP2UTL1(P,9999999.31,BGPJ,BDATE,EDATE)
 ...
 S G=$$CPTREFT^BGP2UTL1(P,BDATE,EDATE,Z)
...
SEALREF ;
 ...
 S G=$$REFUSAL^BGP2UTL1(P,9999999.31,$O(^AUTTADA("B","1351",0)),BDATE,EDATE)
 I $P(G,U)=1 S BGPSEAL="1^"_$P(G,U,2)_"^Refused ADA 1351" Q
 S G=$$REFUSAL^BGP2UTL1(P,81,+$$CODEN^ICPTCOD("D1351"),BDATE,EDATE)
...
DTAP(P,BDATE,EDATE) ;EP
 ...
 I $O(BGPX(0)) Q 1_U_$$DATE^BGP2UTL($O(BGPX(0)))_" Imm 115"
 ;now get cpts
 ...
 I $D(BGPVARI) Q 1_U_$$DATE^BGP2UTL($O(BGPVARI(0)))_" CPT 90715"
 F BGPZ=115 S X=$$ANCONT^BGP2D31(P,BGPZ,EDATE) Q:X]""
 I X]"" Q 2_U_$$DATE^BGP2UTL($P(X,U,1))_" "_$P(X,U,2)
 ;now go to Refusals
 ...
 I R Q 2_U_$$DATE^BGP2UTL(R)_" "_"NMI Dtap"
 Q ""
DTAPTD(P,BDATE,EDATE) ;EP
 ...
 S LTD=$O(TDD(0))
 I LTD]"" S LTD=9999999-LTD Q 1_U_$$DATE^BGP2UTL(LTD)_" Imm "_TDD(9999999-LTD)
 ;now check cpt codes
 S ED=9999999-EDATE-1,BD=9999999-BDATE,G=0
 F  S ED=$O(^AUPNVSIT("AA",P,ED)) Q:ED=""!($P(ED,".")>BD)  D
 .S V=0 F  S V=$O(^AUPNVSIT("AA",P,ED,V)) Q:V'=+V  D
 ...
 I $D(BGPVARI) S Y=$O(BGPVARI(0)) Q 1_U_$$DATE^BGP2UTL($O(BGPVARI(0)))_" CPT "_BGPVARI(Y)
 K BGPG S %=P_"^LAST DX [BGP TD IZ DXS;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP2UTL($P(BGPG(1),U))_" DX V06.5"
 F BGPZ=115,9,113 S X=$$ANCONT^BGP2D31(P,BGPZ,EDATE) Q:X]""
 I X]"" Q 2_U_$$DATE^BGP2UTL($P(X,U,1))_" "_$P(X,U,2)
 ;now go to Refusals
 S B=$$DOB^AUPNPAT(P),E=EDATE,BGPNMI="",R=""
 ...
 ..Q:$P(^AUPNPREF(Y,0),U,7)'="N"
 ..S R=D    ;S:$P(^AUPNPREF(Y,0),U,7)="N" BGPNMI=1 S R=1
 I R Q 2_U_$$DATE^BGP2UTL(R)_" NMI Dtap/Td"
 Q ""
 ;
LASTTDN ;
