BGP2D61 ; IHS/CMI/LAB - measure 31 01 Jul 2010 7:47 PM ;
...
CRCP ;EP - called from elder
 ...
 S BGPVALUE=""
 I BGPFOB]"" S BGPVALUE="FOB: "_$$DATE^BGP2UTL($P(BGPFOB,U,2))_" "_$P(BGPFOB,U)
 ...
 I BGPREF]"" S BGPVALUE=BGPVALUE_$S(BGPVALUE]"":"; ",1:"") S BGPVALUE=BGPVALUE_$$DATE^BGP2UTL($P(BGPREF,U,2))_" "_$P(BGPREF,U,3)
...
CRC(P,EDATE) ;EP
 ...
 S BGPG=$$LASTPRC^BGP2UTL1(P,"BGP TOTAL COLECTOMY PROCS",$$DOB^AUPNPAT(P),EDATE)
 I BGPG Q 1
...
SIG(P,EDATE,BDATE) ;EP
 ...
 ;S BGPG=$$LASTPRCI^BGP2UTL1(P,"45.24",BDATE,EDATE)
 S BGPG=$$LASTPRC^BGP2UTL1(P,"BGP SIG PROCS",BDATE,EDATE)
 ;I $P(BGPG,U)=1 S BGPLSIG="SIG "_$P(BGPG,U,2)_"^"_$$DATE^BGP2UTL($P(BGPG,U,3))_U_$P(BGPG,U,3)
 I $P(BGPG,U)=1 S BGPLSIG="Proc "_$P(BGPG,U,2)_"^"_$$DATE^BGP2UTL($P(BGPG,U,3))_U_$P(BGPG,U,3)
 ;
 S T=$O(^ATXAX("B","BGP SIG CPTS",0))
 I T D  I X]"",$P(BGPLSIG,U,3)<$P(X,U,1) S BGPLSIG="CPT "_$P(X,U,2)_U_$$DATE^BGP2UTL($P(X,U,1))_U_$P(X,U,1)
...
COLO(P,EDATE,BDATE) ;EP
 ...
 S BGPG=$$LASTPRC^BGP2UTL1(P,"BGP COLO PROCS",BDATE,EDATE)
 I $P(BGPG,U)=1 S BGPLCOLO="Proc "_$P(BGPG,U,2)_"^"_$$DATE^BGP2UTL($P(BGPG,U,3))_U_$P(BGPG,U,3)
 S %=P_"^LAST DIAGNOSIS [BGP COLO DXS;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)),$P(BGPLCOLO,U,3)<$P(BGPG(1),U,1) S BGPLCOLO="POV V76.51"_"^"_$$DATE^BGP2UTL($P(BGPG(1),U))
 S T=$O(^ATXAX("B","BGP COLO CPTS",0))
 I T D  I X]"",$P(BGPLCOLO,U,3)<$P(X,U,1) S BGPLCOLO="CPT "_$P(X,U,2)_U_$$DATE^BGP2UTL($P(X,U,1))_U_$P(X,U,1)
 .S X=$$CPT^BGP2DU(P,BDATE,EDATE,T,5) I X]"" Q
 .S X=$$TRAN^BGP2DU(P,BDATE,EDATE,T,5)
 Q BGPLCOLO
...
BE(P,EDATE,BDATE) ;EP
 ...
 I T D  I X]"" S BGPLBE="CPT "_$P(X,U,2)_U_$$DATE^BGP2UTL($P(X,U,1))_U_$P(X,U,1)
 .S X=$$CPT^BGP2DU(P,BDATE,EDATE,T,5) I X]"" Q
 .S X=$$TRAN^BGP2DU(P,BDATE,EDATE,T,5)
 .Q
 S T=$O(^ATXAX("B","BGP BE CPTS",0))
 I T D  I X]"",$P(BGPLBE,U,3)<X Q "RAD "_$P(X,U,2)_U_$$DATE^BGP2UTL(X)_U_X
 .S X=$$RAD^BGP2DU(P,BDATE,EDATE,T,5)
 .Q
 Q BGPLBE
 ;
LOINC(A,B) ;
...
REF(P,BDATE,EDATE) ;EP
 ...
 ..S G=$$REFUSAL^BGP2UTL1(P,60,BGPT1,BDATE,EDATE)
 F BGPT=82270,82274,89205,"G0107","G0328","G0394" D  Q:$P(T,U)=1
 .S C=+$$CODEN^ICPTCOD(BGPT) Q:C'>0
 .S T=""
 .S T=$$REFUSAL^BGP2UTL1(P,81,+$$CODEN^ICPTCOD(BGPT),$$FMADD^XLFDT(EDATE,-365),EDATE)
 .I T S T="1^"_$P(T,U,2)_"^FOB: Refused CPT"
 I $P(T,U)=1 Q T
 S BGPT=$O(^ATXAX("B","BGP SIG CPTS",0))
 I BGPT S G=$$RADREF^BGP2UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"SIG: Refused CPT"
 S BGPT=$O(^ATXAX("B","BGP BE CPTS",0))
 I BGPT S G=$$RADREF^BGP2UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"BE: Refused CPT"
 S BGPT=$O(^ATXAX("B","BGP COLO CPTS",0))
 I BGPT S G=$$RADREF^BGP2UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"COLO: Refused CPT"
 S BGPT=$O(^ATXAX("B","BGP SIG CPTS",0))
 I BGPT S G=$$CPTREFT^BGP2UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"SIG: Refused CPT"
 S BGPT=$O(^ATXAX("B","BGP BE CPTS",0))
 I BGPT S G=$$CPTREFT^BGP2UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"BE: Refused CPT"
 S BGPT=$O(^ATXAX("B","BGP COLO CPTS",0))
 I BGPT S G=$$CPTREFT^BGP2UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"COLO: Refused CPT"
 ...
 .I T S T="1^"_$P(T,U,2)_"^"_$S(BGPT="45.24":"SIG: ",1:"COLO: ")_"Refused Proc"
 I $P(T,U)=1 Q T
 Q ""
RECT(P,EDATE) ;EP
