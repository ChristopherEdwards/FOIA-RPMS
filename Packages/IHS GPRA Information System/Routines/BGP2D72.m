...
AMIO(P,BDATE,EDATE) ;
 ...
 S BGPG=$$LASTPRC^BGP2UTL1(P,"BGP CABG PROCS",$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-60))
 I $P(BGPG,U) Q 1_U_"CABG PROC"
 ;now check cpts
 S BGPG=$$CPT^BGP2DU(P,$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-60),$O(^ATXAX("B","BGP CABG CPTS",0)),6)
 I $P(BGPG,U) Q 1_U_"CABG CPT"
 S BGPG=$$TRAN^BGP2DU(P,$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-60),$O(^ATXAX("B","BGP CABG CPTS",0)),6)
 I $P(BGPG,U) Q 1_U_"CABG TRAN"
 S BGPG=$$LASTDXI^BGP2UTL1(P,"V45.81",$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-60))
 I $P(BGPG,U) Q 1_U_"CABG POV V45.81"
 ;check for procedure in BGP PTCA PROCS
 S BGPG=$$LASTPRC^BGP2UTL1(P,"BGP PCI CM PROCS",$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-60))
 I $P(BGPG,U) Q 1_U_"PCI PROC"
 ;now check cpts
 S BGPG=$$CPT^BGP2DU(P,$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-60),$O(^ATXAX("B","BGP PCI CM CPTS",0)),6)
 I $P(BGPG,U) Q 1_U_"PCI CPT"
 S BGPG=$$TRAN^BGP2DU(P,$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-60),$O(^ATXAX("B","BGP PCI CM CPTS",0)),6)
 I $P(BGPG,U) Q 1_U_"PCI TRAN"
 S BGPG=$$LASTDXI^BGP2UTL1(P,"V45.82",$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-60))
 I $P(BGPG,U) Q 1_U_"PCI POV V45.82"
 ;now check IVD dxs
...
BETAPOV ;
 ...
 .S Z=$P(^AUPNVPOV(Y,0),U,9) I Z]"",$P($$ICDDX^ICDCODE(Z),U,2)="E942.0" S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP2UTL($P(BGPG(X),U))_" ADR POV "_$P(BGPG(X),U,2)_" + E942.0" Q
 .S Z=$P(^AUPNVPOV(Y,0),U,18) I Z]"",$P($$ICDDX^ICDCODE(Z),U,2)="E942.0" S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP2UTL($P(BGPG(X),U))_" ADR POV "_$P(BGPG(X),U,2)_" + E942.0" Q
 .S Z=$P(^AUPNVPOV(Y,0),U,19) I Z]"",$P($$ICDDX^ICDCODE(Z),U,2)="E942.0" S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP2UTL($P(BGPG(X),U))_" ADR POV "_$P(BGPG(X),U,2)_" + E942.0" Q
 .Q
 ...
 .I Y="V14.8"!($$ICD^ATXCHK(I,T,9)),N["BETA BLOCK"!(N["BBLOCK")!(N["B BLOCK") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP2UTL($P(^AUPNPROB(X,0),U,8))_" ADR Problem List  "_Y_"  "
 .Q
 I BGPC>0 Q 1_U_BGPY(BGPC)
 ;allergy tracking
 S BGPC=0
