BGP2D723 ; IHS/CMI/LAB - measure AHR.A ;
...
ACECONT(P,BDATE,EDATE,NMIB,NMIE,RPBD) ;EP ACEI Contra
 ...
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP2UTL($P(BGPG(1),U))_" ACEI/ARB Contra POV "_$P(BGPG(1),U,2)
 ;
 ;nmi
 S BGPG=""
 ...
 ...S BGPG=1_U_$$DATE^BGP2UTL($P(^AUPNPREF(N,0),U,3))_" ACEI NMI Refusal"
 ..Q
 .Q
 I BGPG Q BGPG
 ;nmi
 ...
 ..S N=0 F  S N=$O(^AUPNPREF("AA",P,50,X,D,N)) Q:N'=+N  D
 ...Q:$P($G(^AUPNPREF(N,0)),U,7)'="N"
 ...S BGPG=1_U_$$DATE^BGP2UTL($P(^AUPNPREF(N,0),U,3))_" ARB NMI Refusal"
 ..Q
 .Q
 I BGPG Q BGPG
 ;PREGNANCY
 S X=$$PREG^BGP2D7(P,$S($G(RPBD):RPBD,1:NMIB),NMIE,0,1) I X Q 1_U_"ACEI/ARB Contra pregnant"
 ;breastfeeding
...
STATALG(P,BDATE,EDATE,RPB,RPE) ;EP
 ...
 .S Z=$P(^AUPNVPOV(Y,0),U,9) I Z]"",$P($$ICDDX^ICDCODE(Z),U,2)="E942.9" S BGPC=1_U_"POV:  "_$$DATE^BGP2UTL($P(BGPG(X),U))_" "_$P(BGPG(X),U,2)_" w/E942.9 "_N Q
 .S Z=$P(^AUPNVPOV(Y,0),U,18) I Z]"",$P($$ICDDX^ICDCODE(Z),U,2)="E942.9" S BGPC=1_U_"POV:  "_$$DATE^BGP2UTL($P(BGPG(X),U))_" "_$P(BGPG(X),U,2)_" w/E942.9 "_N Q
 .S Z=$P(^AUPNVPOV(Y,0),U,19) I Z]"",$P($$ICDDX^ICDCODE(Z),U,2)="E942.9" S BGPC=1_U_"POV:  "_$$DATE^BGP2UTL($P(BGPG(X),U))_" "_$P(BGPG(X),U,2)_" w/E942.9 "_N Q
 .Q
 I BGPC Q BGPC
 ...
 .I N["STATIN"!(N["STATINS") S BGPC=BGPC+1,BGPY(BGPC)=1_U_"Alg statin POV:  "_$$DATE^BGP2UTL($P(BGPG(X),U))_" "_$P(BGPG(X),U,2)_" "_N
 I BGPC Q BGPC
 ;PL
 S BGPC=0
...
STATREF(P,BDATE,EDATE) ;EP
 ...
 ..S N=0 F  S N=$O(^AUPNPREF("AA",P,50,X,D,N)) Q:N'=+N!(G)  D
 ...Q:$P($G(^AUPNPREF(N,0)),U,7)'="R"
 ...S G=1_U_"Statin Refusal "_$$DATE^BGP2UTL(Y)
 ..Q
 .Q
 Q G
