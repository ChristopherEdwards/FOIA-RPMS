BGP2D75 ; IHS/CMI/LAB - measure 31 ;
...
IAMT ;EP
 ...
 I 'BGPN2 S BGPBETA("REF")=$$BETAREF^BGP2D721(DFN,$P(BGPAMI,U,2),$$FMADD^XLFDT($P(BGPAMI,U,4),7)) I BGPBETA("REF") S BGPN3=1
 I 'BGPN2 S BGPBETA("CONTRA")=$$BETACONT^BGP2D721(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,$P(BGPAMI,U,2),$P(BGPAMI,U,4)) I BGPBETA("CONTRA") S BGPN4=1 ;beta Contraindication
 I 'BGPN2 S BGPBETA("ADR")=$$BETAALG1^BGP2D72(DFN,BGPEDATE) I BGPBETA("ADR") S BGPN4=1
 ...
 I 'BGPN6 S BGPASA("REF")=$$ASAREF^BGP2D721(DFN,$P(BGPAMI,U,2),$$FMADD^XLFDT($P(BGPAMI,U,4),7)) I BGPASA("REF") S BGPN7=1
 ...
 I 'BGPN6 S BGPASA("ADR")=$$ASAALLEG^BGP2D721(DFN,BGPEDATE) I BGPASA("ADR") S BGPN8=1
 ...
 I 'BGPN10 S BGPACE("REF")=$$ACEREF^BGP2D722(DFN,$P(BGPAMI,U,2),$$FMADD^XLFDT($P(BGPAMI,U,4),7)) I BGPACE("REF") S BGPN11=1
 ...
 I 'BGPN10 S BGPACE("ADR")=$$ACEALG^BGP2D722(DFN,BGPBDATE,BGPEDATE) I BGPACE("ADR") S BGPN12=1
 ...
 I 'BGPN14 S BGPSTAT("REF")=$$STATREF^BGP2D722(DFN,$P(BGPAMI,U,2),$$FMADD^XLFDT($P(BGPAMI,U,4),7)) I BGPSTAT("REF") S BGPN15=1
 I 'BGPN14 S BGPSTAT("CONTRA")=$$STATCON^BGP2D722(DFN,BGPBDATE,BGPEDATE,$P(BGPAMI,U,2),$$FMADD^XLFDT($P(BGPAMI,U,4),7)) I BGPSTAT("CONTRA") S BGPN16=1 ;beta Contraindication
 I 'BGPN14 S BGPSTAT("ADR")=$$STATALG^BGP2D722(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,BGPBDATE,BGPEDATE) I BGPSTAT("ADR") S BGPN16=1
...
ASACONTR(P,BDATE,EDATE,BGPADM,BGPDD) ;does patient have an aspirin allergy
 K BGPMEDS1
 S K=0,R="",BGPG=""
 D GETMEDS^BGP2UTL2(P,$$FMADD^XLFDT(BGPADM,-180),BGPDD,,,,,.BGPMEDS1)
 I '$D(BGPMEDS1) Q ""
...
WAR71 .;
 ...
 .I E=BGPDD S BGPG=1_U_$$DATE^BGP2UTL($P($P(^AUPNVSIT(V,0),U),"."))_U_$P(^PSDRUG(D,0),U) Q
 ...
 I BGPG Q 1_U_$$DATE^BGP2UTL($P(BGPG,U,3))_" Contra POV 459.0 "
 ;
 ...
 ...S BGPG=1_U_$$DATE^BGP2UTL($P(^AUPNPREF(N,0),U,3))_" Contra NMI Aspirin "_$$VAL^XBDIQ1(9000022,N,.04)  ;_" "__" "_$$VAL^XBDIQ1(9000022,X,1101)
 ..Q
 .Q
 I BGPG Q BGPG
 ;now check for CPT code G8008
 S X=$$CPTI^BGP2DU(P,BGPADM,BGPDD,+$$CODEN^ICPTCOD("G8008"))
 I X Q 1_U_$$DATE^BGP2UTL($P(X,U,2))_" Contra CPT G8008"
 S X=$$TRANI^BGP2DU(P,BGPADM,BGPDD,+$$CODEN^ICPTCOD("G8008"))
 I X Q 1_U_$$DATE^BGP2UTL($P(X,U,2))_" Contra Tran G8008"
 Q ""
ACECONT(P,BDATE,EDATE,NMIB,NMIE,RPBD) ;EP does patient have an ACEI Contra
 ...
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP2UTL($P(BGPG(1),U))_" Contra POV "_$P(BGPG(1),U,2)  ;_"]"  ;_$$VAL^XBDIQ1(9000010.07,+$P(BGPG(1),U,4),.04)
 ;
 ;nmi in Refusal
 S BGPG=""
 S T=$O(^ATXAX("B","BGP HEDIS ACEI MEDS",0))
 ...
 ..S N=0 F  S N=$O(^AUPNPREF("AA",P,50,X,D,N)) Q:N'=+N  D
 ...Q:$P($G(^AUPNPREF(N,0)),U,7)'="N"
 ...S BGPG=1_U_$$DATE^BGP2UTL($P(^AUPNPREF(N,0),U,3))_" Contra NMI "_$$VAL^XBDIQ1(9000022,N,.04)  ;_"   "__"  "_$$VAL^XBDIQ1(9000022,X,1101)
 ..Q
 .Q
 I BGPG Q BGPG
 ;nmi
 ...
 I BGPG Q BGPG
 ;PREGNANCY
 S X=$$PREG^BGP2D7(P,$S($G(RPBD):RPBD,1:NMIB),EDATE,0,1) I X Q 1_U_"Contra pregnant"
 ;breastfeeding
 K BGPG S Y="BGPG(",X=P_"^LAST DX V24.1;DURING "_$$FMTE^XLFDT($S(RPBD:RPBD,1:NMIB))_"-"_$$FMTE^XLFDT(EDATE) S E=$$START1^APCLDF(X,Y)
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP2UTL($P(BGPG(1),U))_" Contra POV "_$P(BGPG(1),U,2)  ;_"]"  ;_$$VAL^XBDIQ1(9000010.07,+$P(BGPG(1),U,4),.04)
 K BGPG
 S Y="BGPG("
 ...
 I %]"" Q 1_U_$$DATE^BGP2UTL($P(%,U,2))_" Contra "_$P(%,U,1)
 Q ""
