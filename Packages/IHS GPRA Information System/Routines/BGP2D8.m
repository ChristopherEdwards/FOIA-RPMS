BGP2D8 ; IHS/CMI/LAB - measure C 03 Jul 2010 7:05 AM ;
...
IE2 ;EP
 ...
 I BGPACTCL,BGPSEX="F",$$PREG^BGP2D7(DFN,$$FMADD^XLFDT(BGPEDATE,-600),BGPEDATE,0,1),BGPNO2="" S BGPD1=1  ;active clinical, no hiv, pregnant
 S BGPN1=0
 S BGPN2=0
 I BGPD1 D
...
V ;set denominator value for selected
 ...
 S BGPVALUD="UP" I BGPD2,(BGPN1!(BGPN4)) S BGPVALUD="UP|||"_$S(BGPD2&(BGPHIV):"SCREEN (NO HIV DX): "_$P(BGPHIV,U,4)_"; ",1:"")_$S(BGPN3:"SCREEN COUNT (NO PRIOR DX): "_$P(BGPHIV,U,3)_" screens - "_$P(BGPHIV,U,2),1:"")
...
EXER(P,BDATE,EDATE) ;
 ...
 I $D(BGPG(1)) Q $P(BGPG(1),U)_U_"V65.41"_U_$$DATE^BGP2UTL($P(BGPG(1),U,1))
 S Y="BGPALLED("
...
HIVTEST(P,BDATE,EDATE) ;
 ...
 ...Q:'$$LOINC^BGP2D21(J,T)
 ...Q:$D(BGPA((9999999-D)))  ;already got one on this day
 ...
 ...Q
 ..Q
 .;now, if there on none on this date with a result get 1 w/o result
 .S L=0 F  S L=$O(^AUPNVLAB("AE",P,D,L)) Q:L'=+L  D
 ..S X=0 F  S X=$O(^AUPNVLAB("AE",P,D,L,X)) Q:X'=+X  D
 ...Q:'$D(^AUPNVLAB(X,0))
 ...S V=$P(^AUPNVLAB(X,0),U,3)
 ...
 .S X=""
 .S T=0 F  S T=$O(BGPC(T)) Q:T'=+T  S X=X_$S(X]"":", ",1:"")_$P(BGPC(T),U,2) ;_" result="_$P(BGPC(T),U,3)
...
HIVEDUC(P,BDATE,EDATE) ;
 ...
 .Q:'$D(^AUTTEDT(T,0))
 .S T=$P(^AUTTEDT(T,0),U,2)
 .I $P(T,"-",1)="HIV"!($P(T,"-",1)["042")!($P(T,"-",1)["043")!($P(T,"-",1)["044")!($P(T,"-",1)="V08.")!($P(T,"-",1)["795.71")!($P(T,"-",2)="HIV") S G=1_U_$$DATE^BGP2UTL($P(BGPG(X),U))_U_T
 I $P(G,U)=1 Q G
 ;
 K BGPG
 S Y="BGPG("
 S X=P_"^LAST DX V65.44;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE) S E=$$START1^APCLDF(X,Y)
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP2UTL($P(BGPG(1),U))_"V65.44"  ;has a dx
 Q ""
 ;
CD4(P,BDATE,EDATE) ;
 K BGPG
...
V2HIV(P,BDATE,EDATE) ;
 ...
 I '$D(^AUPNVSIT("AC",P)) Q ""
 NEW BGPMM
 S BGPMM=$$FMADD^XLFDT(EDATE,-(6*30))
 K ^TMP($J,"A")
