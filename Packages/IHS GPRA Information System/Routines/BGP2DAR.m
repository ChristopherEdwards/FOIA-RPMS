BGP2DAR ; IHS/CMI/LAB - ihs area GPRA 02 Sep 2004 1:11 PM 09 Apr 2012 10:03 AM ;
...
ASU ;
 ...
 ;I C=1 S BGPRPTT="F",BGPSUCNT=1,Y=$O(BGPSUL(0)),X=$P(^BGPGPDCW(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
ZIS ;call to XBDBQUE
EISSEX ;
 ...
 I BGPEXCEL=1 D
 .S BGPNOW=$$NOW^XLFDT() S BGPNOW=$$NOW^XLFDT() S BGPNOW=$P(BGPNOW,".")_"."_$$RZERO^BGP2UTL($P(BGPNOW,".",2),6)
 ...
 .S BGPFGNT1="CRSGPRANT1"_$P(^AUTTLOC(DUZ(2),0),U,10)_2012063000000000_$$D^BGP2UTL(BGPNOW)_"_"_$$LZERO^BGP2UTL(BGPC,6)_".TXT"
 .S BGPFGNT2="CRSGPRANT2"_$P(^AUTTLOC(DUZ(2),0),U,10)_2012063000000000_$$D^BGP2UTL(BGPNOW)_"_"_$$LZERO^BGP2UTL(BGPC,6)_".TXT"
 .S BGPFDEV1="CRSGPRADEVNT1"_$P(^AUTTLOC(DUZ(2),0),U,10)_2012063000000000_$$D^BGP2UTL(BGPNOW)_"_"_$$LZERO^BGP2UTL(BGPC,6)_".TXT"
 .S BGPFDEV2="CRSGPRADEVNT2"_$P(^AUTTLOC(DUZ(2),0),U,10)_2012063000000000_$$D^BGP2UTL(BGPNOW)_"_"_$$LZERO^BGP2UTL(BGPC,6)_".TXT"
 .S BGPFDEV3="CRSGPRADEVNT3"_$P(^AUTTLOC(DUZ(2),0),U,10)_2012063000000000_$$D^BGP2UTL(BGPNOW)_"_"_$$LZERO^BGP2UTL(BGPC,6)_".TXT"
 .;S BGPFGNT3="CRSGPRANT3"_$P(^AUTTLOC(DUZ(2),0),U,10)_2012063000000000_$$D^BGP2UTL(BGPNOW)_"_"_$$LZERO^BGP2UTL(BGPC,6)_".TXT"
 .;S BGPFGNT4="CRSGPRANT4"_$P(^AUTTLOC(DUZ(2),0),U,10)_2012063000000000_$$D^BGP2UTL(BGPNOW)_"_"_$$LZERO^BGP2UTL(BGPC,6)_".TXT"
 .Q
...
DRIVER ;
 U IO
 D PRINT^BGP2PARP
 I BGPRPTT="A",BGPIFTR=1,BGPEXCEL D GNT1^BGP2UTL
 D ^%ZISC
 D EXIT
 Q
 ;
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP2DAR",ZTDTH="",ZTDESC="GPRA REPORT" D ^%ZTLOAD D HOME^%ZIS D EXIT Q
 Q
 ;
EXIT ;
 D ^%ZISC
 ...
 Q
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP2DAR",XBRX="EXIT^BGP2DAR",XBNS="BGP"
 D ^XBDBQUE
 Q
 ;
NODEV1 ;
 D PRINT^BGP2PARP
 I BGPRPTT="A",BGPIFTR,BGPEXCEL D GNT1^BGP2UTL
 D ^%ZISC
 D EXIT
 Q
 ;
B ;fiscal year
 S (BGPBPER,BGPVDT)=""
 W !!,"Enter the BASELINE year for the report.  Use a 4 digit ",!,"year, e.g. 2012"
 S DIR(0)="D^::EP"
...
F ;fiscal year
 ...
 W !
 S BGPVDT=""
 W !,"Enter the Fiscal Year (FY) for the report END date.  Use a 4 digit",!,"year, e.g. 2010, 2012"
 S DIR(0)="D^::EP"
 S DIR("A")="Enter FY"
 S DIR("?")="This report is compiled for a period.  Enter a valid date."
 D ^DIR
...
SUMONLY ;EP
 W:$D(IOF) @IOF
 S BGPA=$E($P(^AUTTLOC(DUZ(2),0),U,10),1,2),BGPA=$O(^AUTTAREA("C",BGPA,0)) S BGPA=$S(BGPA:$P(^AUTTAREA(BGPA,0),U),1:"UNKNOWN AREA")
 W !,$$CTR("IHS 2012 Area National GPRA & PART Report Clinical Performance Summaries")
 W !!,"This will produce ONLY the clinical performance summaries for the Area "
 W !,"National GPRA & PART Report for the 2012 GPRA year.  If you want the detailed "
