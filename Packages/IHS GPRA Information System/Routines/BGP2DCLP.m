BGP2DCLP ; IHS/CMI/LAB - IHS gpra print ;
...
GO ;
 ...
 .D W^BGP2DP(^BGPCTRL(BGPY,28,BGPX,0),0,1,BGPPTYPE)
 S BGPXL=0
 D HEADER
...
PRINTL ;print one line
 ...
 S BGPPCNT=BGPPCNT+1
 D W^BGP2DP($E($P(^DPT(DFN,0),U),1,22),0,1,BGPPTYPE,1)
 D W^BGP2DP($$HRN^AUPNPAT(DFN,DUZ(2)),0,0,BGPPTYPE,2,24)
 D W^BGP2DP($E(BGPCOM,1,14),0,0,BGPPTYPE,3,31)
 D W^BGP2DP(BGPSEX,0,0,BGPPTYPE,4,46)
 D W^BGP2DP(BGPAGE,0,0,BGPPTYPE,5,49)
 ...
 D W^BGP2DP(W,0,0,BGPPTYPE,6,53)
 D W^BGP2DP(Z_$S(BGPPTYPE="P":"/",1:""),0,0,BGPPTYPE,7,53)
 D W^BGP2DP($$LAST(DFN,BGPED),0,0,BGPPTYPE,8,65)
 Q
 ;
LAST(P,EDATE) ;EP
 I '$D(^AUPNVSIT("AC",P)) Q ""
 ...
 .S G=V
 .Q
 I G Q $$PRIMPROV^APCLV(G,"N")_","_$S($$PRIMPROV^APCLV(G,"F"):$P(^DIC(7,+$$PRIMPROV^APCLV(G,"F"),0),U,2),1:"")_","_$$DATE^BGP2UTL($P($P(^AUPNVSIT(G,0),U),"."))
 Q ""
HEADER ;EP
...
HEADER1 ;
 ...
 I BGPPTYPE="P",$G(BGPGUI) D W^BGP2DP("ZZZZZZZ",0,1,BGPPTYPE) ;!  ;maw
 D W^BGP2DP("***** CONFIDENTIAL PATIENT INFORMATION, COVERED BY THE PRIVACY ACT *****",1,$S(BGPPTYPE="D":3,1:1),BGPPTYPE)
 I BGPPTYPE="P" S X=$P(^VA(200,DUZ,0),U,2),$E(X,35)=$$FMTE^XLFDT(DT),$E(X,70)="Page "_BGPGPG D W^BGP2EOH(X,1,1,BGPPTYPE)
 D W^BGP2DP("***  IHS 2012 Comprehensive National GPRA & PART Patient List  ***",1,1,BGPPTYPE)
 D W^BGP2DP("*** List of Patients Not Meeting a National GPRA or PART measure  ***",1,1,BGPPTYPE)
 ...
 D W^BGP2DP($S(BGPLIST="A":"All Patients",BGPLIST="R":"Random Patient List",1:"Patient List by Provider: "_BGPLPROV),1,1,BGPPTYPE)
...
H1 ;
 ...
 D W^BGP2DP("",0,1,BGPPTYPE)
 D W^BGP2DP("PATIENT NAME",0,1,BGPPTYPE)
 D W^BGP2DP("HRN",0,0,BGPPTYPE,2,24)
 D W^BGP2DP("COMMUNITY",0,0,BGPPTYPE,3,31)
 D W^BGP2DP("SEX",0,0,BGPPTYPE,4,45)
 D W^BGP2DP("AGE",0,0,BGPPTYPE,5,49)
 D W^BGP2DP("DENOMINATOR",0,0,BGPPTYPE,6,53)
 D W^BGP2DP("NOT MET/LST PRVDR",0,0,BGPPTYPE,7,65)
 D W^BGP2DP($TR($J("",80)," ","-"),0,1,BGPPTYPE)
 Q
