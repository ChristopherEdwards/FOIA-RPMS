...
START ;
 W:$D(IOF) @IOF
 W !!,$$CTR("2012 HEDIS Clinical Performance Measure Report",80)
INTRO ;
 D XIT
...
DRIVER ;
 ...
 D ^BGP2DHEP
 D ^%ZISC
 I BGPEXPT D GS^BGP2HUTL
 D XIT
 Q
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP2DHE",XBRX="XIT^BGP2DHE",XBNS="BGP"
 D ^XBDBQUE
 ;D XIT
 Q
 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP2DHE",ZTDTH="",ZTDESC="HEDIS 09 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
LISTS ;any lists with measures?
 ...
 I $D(BGPLIST) D RT^BGP2DHSL I '$D(BGPLIST)!($D(BGPQUIT)) G LISTS ;get report type for each list
 Q
CHKY ;
