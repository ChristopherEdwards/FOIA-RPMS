BGP2DL ; IHS/CMI/LAB - IHS GPRA 10 SELECTED REPORT DRIVER 21 May 2010 12:10 PM ;
 ...
 ;
 W:$D(IOF) @IOF
 W !!,$$CTR("IHS 2012 CRS - Clinical Performance Measure Report (Selected Measures)",80)
INTRO ;
 D XIT
...
SETIND ;
 ...
 I $D(DIRUT) D XIT Q
 S BGPINDW=$E(Y)
 I BGPINDW="S" D SI I '$D(BGPIND) G SETIND
...
SUM ;display summary of this report
 W:$D(IOF) @IOF
 W !,$$CTR("SUMMARY OF 2012 CLINICAL MEASURE PERFORMANCE REPORT TO BE GENERATED")
 W !!,"The date ranges for this report are:"
 W !?5,"Report Period: ",?31,$$FMTE^XLFDT(BGPBD)," to ",?31,$$FMTE^XLFDT(BGPED)
 W !?5,"Previous Year Period: ",?31,$$FMTE^XLFDT(BGPPBD)," to ",?31,$$FMTE^XLFDT(BGPPED)
 W !?5,"Baseline Period: ",?31,$$FMTE^XLFDT(BGPBBD)," to ",?31,$$FMTE^XLFDT(BGPBED)
 ...
 W !!,"These performance measures will be calculated: " S X=0 F  S X=$O(BGPIND(X)) Q:X'=+X  W $P(^BGPINDW(X,0),U,3)," ; "
 W !!,"Lists will be produced for these measures: "
 S X=0 F  S X=$O(BGPLIST(X)) Q:X'=+X  W $P(^BGPINDW(X,0),U,3)," ; "
 D PT^BGP2DSL
 I BGPROT="" G BEN
ZIS ;call to XBDBQUE
 D REPORT^BGP2UTL
...
AI ;EP
 ...
 Q
II ;EP
 S X=0 F  S X=$O(^BGPINDWC("AIPC",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDWC(X,0),U,1))=""
 Q
PI ;EP
...
LISTS ;any lists with measures?
 ...
 I $D(BGPLIST) D RT^BGP2DSL I '$D(BGPLIST)!($D(BGPQUIT)) G LISTS ;get report type for each list
 Q
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP2DL",ZTDTH="",ZTDESC="CRS 11 COM REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP2DL",XBRX="XIT^BGP2DL",XBNS="BGP"
 D ^XBDBQUE
 Q
 ;
XIT ;
