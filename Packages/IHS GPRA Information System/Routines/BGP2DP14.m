BGP2DP14 ; IHS/CMI/LAB - print ind 19 AGE DIST ;
...
I1AGE ;EP  special age tallies
 ...
 D:'$G(BGPSUMON) HEADER^BGP2DPH Q:BGPQUIT  W !,^BGPINDW(BGPIC,53,1,0) W:$D(^BGPINDW(BGPIC,53,2,0)) !,^BGPINDW(BGPIC,53,2,0)  D H10^BGP2DPH
...
I1AGE1 ;
 ;gather up all #'s
 S C=C+1
 S BGPF="295.4.1"_BGPA S BGPPC=$O(^BGPINDWC("OR",BGPF,0))
...
I1AGE2 ;
 S D=D+1
 S BGPF="295.5.1"_BGPA S BGPPC=$O(^BGPINDWC("OR",BGPF,0))
...
I1AGE3 ;
 S E=E+1
 S BGPF=BGPX_"1C" S BGPPC=$O(^BGPINDWC("OR",BGPF,0))
 ;set 4th piece to numerator and 5th to %
 S BGPNF=$P(^BGPINDWC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90548.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
PR ; 
 ;D:'$G(BGPSUMON) HEADER^BGP2DPH
 ;Q:BGPQUIT
 ;W !,^BGPINDW(BGPIC,53,1,0) W:$D(^BGPINDW(BGPIC,53,2,0)) !,^BGPINDW(BGPIC,53,2,0) D H4^BGP2DPH
