BGP2DP8 ; IHS/CMI/LAB - print ind H ;
 ;;12.1;IHS CLINICAL REPORTING;;MAY 17, 2012;Build 66
 ;
 ;
I1AGE ;EP special age tallies
 S BGPHD1="ACTIVE CLINICAL POPULATION",BGPHD2="# Active Clinical"
 I '$G(BGPSUMON) D HEADER^BGP2DPH Q:BGPQUIT  W !,^BGPINDW(BGPIC,53,1,0) W:$D(^BGPINDW(BGPIC,53,2,0)) !,^BGPINDW(BGPIC,53,2,0)  D H3
...
I1AGE1 ;
 ;gather up all #'s
 S C=C+1
 S BGPF="G.F.1"_BGPX S BGPPC=$O(^BGPINDWC("C",BGPF,0))
...
I1AGE2 ;
 S D=D+1
 S BGPF="G.G.1"_BGPX S BGPPC=$O(^BGPINDWC("C",BGPF,0))
 ...
 Q
I1AGE3 ;
 S BGPF="G.H.1"_BGPX S BGPPC=$O(^BGPINDWC("C",BGPF,0))
 S E=E+1
 S BGPDF=$P(^BGPINDWC(BGPPC,0),U,8)
 S BGPNP=$P(^DD(90548.03,BGPDF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(E),U,6)=$$V^BGP2DP1C(1,BGPRPT,N,P)
 S $P(BGPDAP(E),U,6)=$$V^BGP2DP1C(2,BGPRPT,N,P)
 S $P(BGPDAB(E),U,6)=$$V^BGP2DP1C(3,BGPRPT,N,P)
 ;set 6th piece to numerator and 7th to %
 S BGPNF=$P(^BGPINDWC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90548.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(D),U,7)=$$V^BGP2DP1C(1,BGPRPT,N,P),$P(BGPDAC(D),U,8)=$S($P(BGPDAC(D),U,6):($P(BGPDAC(D),U,7)/$P(BGPDAC(D),U,6)*100),1:"")
 S $P(BGPDAP(D),U,7)=$$V^BGP2DP1C(2,BGPRPT,N,P),$P(BGPDAP(D),U,8)=$S($P(BGPDAP(D),U,6):($P(BGPDAP(D),U,7)/$P(BGPDAP(D),U,6)*100),1:"")
 S $P(BGPDAB(D),U,7)=$$V^BGP2DP1C(3,BGPRPT,N,P),$P(BGPDAB(D),U,8)=$S($P(BGPDAB(D),U,6):($P(BGPDAB(D),U,7)/$P(BGPDAB(D),U,6)*100),1:"")
 Q
I1AGEP ;EP
...
PR ; 
 I $Y>(BGPIOSL-12) I '$G(BGPSUMON) D HEADER^BGP2DPH Q:BGPQUIT  W !,^BGPINDW(BGPIC,53,1,0) W:$D(^BGPINDW(BGPIC,53,2,0)) !,^BGPINDW(BGPIC,53,2,0) D H3
...
BL ;
 I $Y>(BGPIOSL-12) I '$G(BGPSUMON) D HEADER^BGP2DPH Q:BGPQUIT  W:'$G(BGPSUMON) !,^BGPINDW(BGPIC,53,1,0) W:$D(^BGPINDW(BGPIC,53,2,0)) !,^BGPINDW(BGPIC,53,2,0) D H3
