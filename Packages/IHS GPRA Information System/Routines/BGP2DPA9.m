...
EP ;EP - called from option interactive
 ...
 W !!,"This is the GPRA forecast patient list for the 2013 GPRA year using the"
...
ENDDATE ;
 ...
 S BGPPBD=3110701,BGPPED=3120630
 S BGPPER=3120000
 S BGPNGR09=1
 S BGPLIST="A"
 S BGPCPLC=0
...
NT S BGPLIST="A"
 S BGPCPLC=0
 ;S XBRP="PRINT^BGP2DPAW",XBRC="PROC^BGP2DPA9",XBRX="EOJ^BGP2DPA9",XBNS="BGP"
 ;D ^XBDBQUE
 ;
 K IOP,%ZIS
 W !! S %ZIS="PQM" D ^%ZIS
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP2DPA9",ZTDTH="",ZTDESC="CRS 11 SCHED REPORT" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D ^XBFMK
 K DIC,DIR,DFN
...
BDMG(BGPBD,BGPED,BGPCLN) ;EP - GUI DMS Entry Point
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BGP2DPA9",ZTDESC="GUI CN GRPA APPT" D ^%ZTLOAD
 D EOJ
 Q
GUIEP ;EP
 D PROC
 K ^TMP($J,"BGP2DPA9")
 S IOM=80
 D GUIR^XBLM("PRINT^BGP2DPA9","^TMP($J,""BGP2DPA9"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP2DPA9",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP2DPA9",X)
...
GUIECP ;EP
 K ^TMP($J,"BGP2DPA9")
 S IOM=80
 D GUIR^XBLM("CPPRINT^BGP2DPA9","^TMP($J,""BGP2DPA9"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP2DPA9",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP2DPA9",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIW(BGPIEN,11,X,0)=BGPDATA,C=C+1
...
PROC ;EP
 ...
 K ^XTMP("BGP2DPA9",BGPGPRAJ,BGPGPRAH)
 S ^XTMP("BGP2DPA9",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^"_"PTS WITH GPRA DATA"
 K BGPAPPT,BGPAPPTS
 S BGPTA=0
 I BGPRT1="A" D  Q
...
ZISCP ;
 ...
 ;
 I $D(IO("Q")) G TSKMNCP
 ;S XBRP="CPPRINT^BGP2DPA9",XBRC="",XBRX="EOJ^BGP2DPA9",XBNS="BGP"
 ;D ^XBDBQUE
 ;
...
TSKMNCP ;EP ENTRY POINT FROM TASKMAN
 ...
 I $D(IOM)#2,IOM S ZTIO=ZTIO_";"_IOM I $D(IOSL)#2,IOSL S ZTIO=ZTIO_";"_IOSL
 K ZTSAVE S ZTSAVE("BGP*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="CPPRINT^BGP2DPA9",ZTDTH="",ZTDESC="CRS 11 SCHED REPORT DENOM" D ^%ZTLOAD D EOJ Q
 Q
STMP ;EP
EN1 ;EP Help
 K BGPQUIT S BGPSTMP=""
...
POST ;EP - called from patch 1 post init
 NEW X
 S X=$$ADD^XPDMENU("BGP 12 MENU NATIONAL","BGP 12 FORECAST FOR 2013","FO12",34)
 I 'X D MES^XPDUTL("Adding Forecast 2013 report to menu failed.",80)
 Q
