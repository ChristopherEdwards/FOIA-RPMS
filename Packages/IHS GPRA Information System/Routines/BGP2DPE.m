BGP2DPE ; IHS/CMI/LAB - IHS GPRA 10 REPORT DRIVER ;
 ...
 ;
 W:$D(IOF) @IOF
 ;W !,$$CTR("IHS 2012 Patient Education w/Community Specified Report",80),!!
INTRO ;
 D XIT
INTRO1 ;
...
MSR ;BEGIN TEST STUFF
 ...
 I $D(DIRUT) D XIT Q
 S BGPZZ=Y
 I BGPZZ="S" D EN^BGP2PESI I '$D(BGPIND) W !!,"No measures selected" G INTRO
...
ZIS ;call to XBDBQUE
 ...
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPPEDCW(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPPEDPW(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPPEDBW(" D ^DIK K DIK D XIT Q
 I $D(IO("Q")) G TSKMN
DRIVER ;
 D ^BGP2DPE1
 U IO
 D ^BGP2DPEP
 D ^%ZISC
 I BGPPEEXP D GS^BGP2PUTL
 D XIT
 Q
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP2DPE",XBRX="XIT^BGP2DPE",XBNS="BGP"
 D ^XBDBQUE
 ;D XIT
 Q
 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP2DPE",ZTDTH="",ZTDESC="PAT ED 09 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
LISTS ;any lists with measures?
 ...
 I $D(BGPLIST) D RT^BGP2PESL I '$D(BGPLIST)!($D(BGPQUIT)) G LISTS
 Q
CHKY ;
