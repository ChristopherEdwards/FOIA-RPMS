BGP2ELH ; IHS/CMI/LAB - cover page for ELDER 25 Jun 2010 10:20 AM ;
 ...
 ;W:$D(IOF) @IOF
 W !!,$$CTR("Cover Page",80)
 W !!,$$CTR("*** IHS 2012 Elder Care Report ***",80)
 ...
 D ENDTIME
 D ELDERHDR
 I $G(BGPEXPT) W !!,"A file will be created called BG121",$P(^AUTTLOC(DUZ(2),0),U,10)_".EL"_BGPRPT,".",!,"It will reside in the public/export directory.",!,"This file should be sent to your Area Office.",!
...
AREACP ;EP - area cover page
 ...
 .W !?3,BGPC,".  ",$S($P(^BGPELDCW(BGPX,0),U,17):"*",$P(^BGPELDCW(BGPX,0),U,19):"**",1:""),X
 W !!?1,"The following communities are included in this report:"
 NEW BGPX
 S BGPX="",BGPC=0 F  S BGPX=$O(BGPSUL(BGPX)) Q:BGPX=""  D
 .S BGPC=BGPC+1
 .S X=$P(^BGPELDCW(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .W !!?3,BGPC,".  ",$S($P(^BGPELDCW(BGPX,0),U,17):"*",1:""),X
 .;W !?5,"Communities: " 
 .W !?3,"Community Taxonomy Name: ",$P(^BGPELDCW(BGPX,0),U,18)
 .;W !?5,"Communities: " 
 .S BGPXX=0,BGPXN=0,BGPXY="" F  S BGPXX=$O(^BGPELDCW(BGPX,9999,BGPXX)) Q:BGPXX'=+BGPXX  S BGPXN=BGPXN+1,BGPXY=BGPXY_$S(BGPXN=1:"",1:";")_$P(^BGPELDCW(BGPX,9999,BGPXX,0),U)
 ...
 ..W !!?5,"MFI Visit Locations: " S BGPXX=0,BGPXN=0,BGPXY="" F  S BGPXX=$O(^BGPELDCW(BGPX,1111,BGPXX)) Q:BGPXX'=+BGPXX  S BGPXN=BGPXN+1,BGPXY=BGPXY_$S(BGPXN=1:"",1:";")_$P(^BGPELDCW(BGPX,1111,BGPXX,0),U)
 ...
 .W !,"* CHS-only site.  Uses Active Clinical CHS Population definition vs. Active Clinical." ;D W^BGP2DP(X,0,1,BGPPTYPE)
 I BGPURBO D
 .;S X=" " D W^BGP2DP(X,0,1,BGPPTYPE)
 .W !,"**Urban Outreach & Referral-only site.  Uses Active Clinical Behavioral Health",!,"Population definition vs. Active Clinical." ;D W^BGP2DP(X,0,1,BGPPTYPE)
 W ! ;S X=" " D W^BGP2DP(X,0,1,BGPPTYPE)
 K BGPX,BGPQUIT
 Q
