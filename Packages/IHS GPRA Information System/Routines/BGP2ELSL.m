BGP2ELSL ; IHS/CMI/LAB - IHS gpra print ;
...
GO .;
 ...
 .S X=$P(^BGPELIW(BGPL,0),U,3) D S(X,1,1)
 .S X=" " D S(X,1,1)
 .S X="Denominator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPELIW(BGPL,61,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPELIW(BGPL,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPELIW(BGPL,61,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S X=^BGPELIW(BGPL,61,BGPY,1,BGPZ,0) S:BGPZ=1 X=" - "_X D S(X,1,1)   ;S Y=^BGPELIW(BGPL,61,BGPY,1,BGPZ,0) D S(Y,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .S X="Numerator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPELIW(BGPL,62,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPELIW(BGPL,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPELIW(BGPL,62,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S X=^BGPELIW(BGPL,62,BGPY,1,BGPZ,0) S:BGPZ=1 X=" - "_X D S(X,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPELIW(BGPL,11,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPELIW(BGPL,11,BGPX,0) D S(X,1,1)
 .S X=" " D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPELIW(BGPL,51,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPELIW(BGPL,51,BGPX,0) D S(X,1,1)
 .S X=" " D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPELIW(BGPL,52,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPELIW(BGPL,52,BGPX,0) D S(X,1,1)
 .D H1
...
HEADER1 ;
 ...
 W !,$$CTR("***  IHS 2012 ELDER CARE Clinical Performance Report Patient List  ***",80),!
...
H1 ;
 ...
 S Y=0 F  S Y=$O(^BGPELIW(BGPL,71,Y)) Q:Y'=+Y  S X=^BGPELIW(BGPL,71,Y,0) D S(X,1,1)
 S X=" " D S(X,1,1)
