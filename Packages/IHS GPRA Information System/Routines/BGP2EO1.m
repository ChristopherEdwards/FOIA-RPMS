BGP2EO1 ; IHS/CMI/LAB - calc measures 29 Apr 2010 7:38 PM 14 Nov 2006 5:02 PM 12 Nov 2009 11:03 AM 07 Apr 2010 7:00 AM ;
...
EODML1 ;EP
 ...
 S BGPD1=1  ;is active diabetic
 S BGPN1=0,BGPVALUE="",BGPV=""
 S BGPLDL=$$LDL^BGP2D2(DFN,BGPBDATE,BGPEDATE)
 ;now evaluate result
 S BGPN4=0 D
 ...
 S BGPN1=BGPN4
 ;
 S BGPV="" I BGPLDL]"" S BGPV="LDL DONE: "_$$DATE^BGP2UTL($P(BGPLDL,U,2))_" "_$P(BGPLDL,U,3)
 S BGPVALUE="AD||| "_BGPV
 Q
 ;
EOOX ;
...
HGBA1C(P,BDATE,EDATE) ;EP
 ...
 S BGPC=0
 S G=$$CPT^BGP2DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP HGBA1C CPTS",0)),5)
 ...
 S C=0,C=$O(BGPT(D,C))
 S X=$P(BGPT(D,C),U,1)
 I $$UP^XLFSTR(X)="COMMENT" Q 1_U_1_U_0_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP2UTL(9999999-D)_" (no result) "_X
 I X="" D  Q G
 .S G=""
 .I $P(BGPT(D,C),U,2)="CPT 3046F" S G=1_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP2UTL(9999999-D) Q
 .I $P(BGPT(D,C),U,2)="CPT 3047F"!($P(BGPT(D,C),U,2)="CPT 3044F")!($P(BGPT(D,C),U,2)="CPT 3045F") S G=0_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP2UTL(9999999-D) Q
 .S G=1_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP2UTL(9999999-D)_" (no result)" Q
 S X=$$STRIP^XLFSTR(X," ")  ;strip spaces
 I X[">9" Q 1_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP2UTL(9999999-D)_" "_X
 S X=$$STV(X)
 I X="" Q 1_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP2UTL(9999999-D)_" (no result) "_X
 I +X>9 Q 1_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP2UTL(9999999-D)_" "_X
 Q 0_U_$P(BGPT(D,C),U,2)_" "_$$DATE^BGP2UTL(9999999-D)_" result: "_X
 ;
...
TIAFIB(P,BDATE,EDATE,BGPR) ;EP
 ...
 ..S G=$$ANTICOAG^BGP2EO11(P,$$FMADD^XLFDT(BGPD,-365),$$DSCHDATE^APCLV(BGPV),BGPD)  ; any ANTICOAG?
...
OXSAT(V) ;was there ox sat at the visit
 ...
 ;
 S G=$$REFUSAL^BGP2UTL1(P,9999999.07,$O(^AUTTMSR("B","O2",0)),BGPD,BGPD)
 I G Q $$DATE^BGP2UTL(BGPD)_" Refused O2 SAT MEAS^2"
 ;refusal of lab tests
 S T=$O(^ATXLAB("B","BGP CMS ABG TESTS",0))
 S L=0 F  S L=$O(^ATXLAB(T,21,"B",L)) Q:L'=+L!(BGPG]"")  D
 .S G=$$REFUSAL^BGP2UTL1(P,60,L,BGPD,BGPD)
 .I G S BGPG=$$DATE^BGP2UTL(BGPD)_" Refused LAB "_$P(^LAB(60,L,0),U)_"^2"
 I BGPG]"" Q BGPG
 S G=$$CPTREFT^BGP2UTL1(P,BGPD,BGPD,$O(^ATXAX("B","BGP CMS ABG CPTS",0)))
 I G Q $$DATE^BGP2UTL(BGPD)_" Refused CPT "_$P(G,U,4)_"^2"
