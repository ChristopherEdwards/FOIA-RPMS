BGP2GAEO ; IHS/CMI/LAB - AREA NATIONAL GPRA REPORT 01 Jul 2010 7:58 PM ;
...
TESTNTL ;
 ...
 S BGPSUL(1)=""
 S BGPSUL(2)=""
 D EP(.ERR,1,2522,"BGP 12 AREA ELDER REPORT","A",.BGPSUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRPTT,BGPSUL,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPROT,BGPRTIME,BGPHC,BGPFILE) ;EP - called from GUI to produce national gpra report (AO-AGP)
 ...
 ;            from BGP 12 ELDER DATA CURRENT that match the following:
 ;            (SEE BGP2ASL or CALL ME)
 ;  BGPSUL  - array containing Iens from BGP 12 ELDER  DATA CURRENT that the user selected
...
EP1 ;
 ...
 S BGPZZ="A"
 S BGPSUCNT=C
 I BGPSUCNT=1 S Y=$O(BGPSUL(0)),X=$P(^BGPEOCB(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 ...
 S X=0 F  S X=$O(^BGPEOMB(X)) Q:X'=+X  S BGPIND(X)=""
 S BGPINDW="E"
 I '$G(BGPHC) D
 ...
 .S BGPFGNT1="CRSEONT1"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP2UTL(BGPED)_$$D^BGP2UTL(BGPNOW)_"_"_$$LZERO^BGP2UTL(BGPC,6)_".TXT"
 .Q
 ;create entry in GUI file
 D ^XBFMK
 S X=BGPFILE
 S DIC="^BGPGUIW(",DIC(0)="L",DIADD=1,DLAYGO=90546.19,DIC("DR")=".02////"_BGPUSER_";.03////"_$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT)_";.05///"_BGPOPTN_";.06///R;.07///"_$G(BGPROT)_";1///"_$G(BGPFGNT1)
 ...
 D TSKMN
 S BGPRET=BGPGIEN
 S $P(BGPRET,"~",3)=$G(BGPFGNT1)
 Q
 ;
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOEO^BGP2GAEO",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI AREA EO REPORT 11" D ^%ZTLOAD Q
 Q
AOEO ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 ;cmi/anch/maw added 5/12/2010 for word output
 D GUIR^BGPXBLM("PRINT^BGP2EOP","^TMP($J,""BGPGUI"",")
