BGP2GPP ; IHS/CMI/LAB - GUI PP REPORT ;
...
TESTNTL ;
 ...
 S BGPND(4)="",BGPND(3)=""
 S BGPLIST(3)=""
 D EP(.ERR,1,2522,"BGP 12 SEL PATIENT PANEL",338,"D",.BGPND,1,3040000,3000000,1801,"A",.BGPLIST,"","","B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPINDW,BGPIND,BGPQTR,BGPPER,BGPVDT,BGPSEAT,BGPLSTT,BGPLIST,BGPLPRV,BGPLPROV,BGPROT,BGPRTIME,BGPFILE) ;EP - called from GUI to produce PP REPORT CI05-RPT-LOC-PP
 ; SEE ROUTINE BGP2DL if you have questions about any of these variables
 ;  BGPUSER - DUZ
 ;  BGPDUZ2 - DUZ(2)
 ;  BGPOPTN - OPTION NAME
 ;  BGPTAXI - IEN OF COMMUNITY TAXONOMY NAME
 ;  BGPINDW - $E (1st character) of the answer to the following DIR call:
 ...
 ;           so if BGPINDW="S" pass this array
...
EP1 ;
 ...
 I $G(BGPINDW)="" S BGPRET=0_"^INDICATOR TYPE NOT PASSED" Q
 I "DCWESIPA"'[BGPINDW S BGPRET=0_"^INDICATOR TYPE INVALID" Q
 I BGPINDW="S",'$D(BGPIND) S BGPRET=0_"^INDICATOR ARRAY NOT PASSED" Q
 I BGPINDW="S",'$O(BGPIND(0)) S BGPRET=0_"^INDICATOR ARRAY NOT PASSED" Q
 ...
 I BGPINDW="D" K BGPIND D DI
 I BGPINDW="C" K BGPIND D CI
 I BGPINDW="W" K BGPIND D WI
 I BGPINDW="E" K BGPIND D EI
 I BGPINDW="I" K BGPIND D II^BGP2DL
 I BGPINDW="P" K BGPIND D PI^BGP2DL
 I BGPINDW="A" K BGPIND D AI^BGP2DL
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LOCPP^BGP2GPP",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT COM 05" D ^%ZTLOAD Q
 Q
LOCPP ;
 D ^BGP2D10
 K ^TMP($J,"BGPGUI")
