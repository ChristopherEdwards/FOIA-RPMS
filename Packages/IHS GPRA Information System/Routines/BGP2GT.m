BGP2GT ; IHS/CMI/LAB - BGPG Gui CRS Tables 2/2/2005 10:24:22 AM ;
...
TAX(RETVAL,BGPSTR) ;-- generic taxonomy table
 S X="MERR^BGP2GU",@^%ZOSF("TRAP") ; m error trap
 N P,BGPRFL,BGPI,BGPERR,BGPIEN,BGPTAXW,BGPTAX,BGPDA,BGPNONC,BGPXRF
 N BGPGL,BGPGRF,BGPP
 S P="|"
 ...
 F BGPP=3:1 S BGPTAXW=$P(BGPSTR,P,BGPP) Q:$G(BGPTAXW)=""  D
 . Q:$G(BGPTAXW)=""
 . ;S BGPTAXW=$P(BGPSTR,P)
 . S BGPTAX=$O(^ATXAX("B",BGPTAXW,0))
 . Q:'$G(BGPTAX)
...
GI(RETVAL,BGPSTR) ;-- get GPRA measures
 ...
 .. I BGPRTYP=1 Q:$P(^BGPINDW(Y,0),U,7)'=1
 .. I BGPRTYP=7 Q:$P($G(^BGPINDW(Y,12)),U,1)'=1
 .. I $G(BGPNPL),'$D(^BGPNPLW("AR",Y,$S(BGPRTYP=1:"N",1:"O")))
 .. S BGPI=BGPI+1
...
GSI(RETVAL,BGPSTR) ;-- get sub measure based on passed in measure
 ...
 .. S BGPI=BGPI+1
 .. S ^BGPTMP($J,BGPI)=Y_U_$P($G(^BGPNPLW(Y,0)),U,3)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
GIALL(RETVAL) ;-- get all GPRA measures
 ...
 S BGPI=0
 S ^BGPTMP($J,BGPI)="T00007BMXIEN^T00050Measure"_$C(30)
 S X=0 F  S X=$O(^BGPINDW("AO",X)) Q:X'=+X  D
 . S Y=0 F  S Y=$O(^BGPINDW("AO",X,Y)) Q:Y'=+Y  D
...
ECALL(RETVAL) ;-- get all elder care measures
 ...
 S BGPI=0
 S ^BGPTMP($J,BGPI)="T00007BMXIEN^T00050Measure"_$C(30)
 S X=0 F  S X=$O(^BGPELIW("AO",X)) Q:X'=+X  D
 . S Y=0 F  S Y=$O(^BGPELIW("AO",X,Y)) Q:Y'=+Y  D
 .. ;Q:$P(^BGPINDW(Y,0),U,7)'=1
 .. S BGPI=BGPI+1
 .. S ^BGPTMP($J,BGPI)=Y_U_$P($G(^BGPELIW(Y,0)),U,4)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
...
PCALL(RETVAL) ;-- get all patient education measures
 ...
 S BGPI=0
 S ^BGPTMP($J,BGPI)="T00007BMXIEN^T00050Measure"_$C(30)
 S X=0 F  S X=$O(^BGPPEIW("AO",X)) Q:X'=+X  D
 . S Y=0 F  S Y=$O(^BGPPEIW("AO",X,Y)) Q:Y'=+Y  D
 ..;Q:$P(^BGPINDS(Y,0),U,7)'=1
 .. S BGPI=BGPI+1
 .. S ^BGPTMP($J,BGPI)=Y_U_$P($G(^BGPPEIW(Y,0)),U,2)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
PATLST(RETVAL,BGPSTR) ;-- get all GPRA measures
 ...
 . S BGPIND($P(^BGPINDW(BGPINDI,0),U,2),BGPINDI)=""
 S O=0 F  S O=$O(BGPIND(O)) Q:O'=+O  S X=$O(BGPIND(O,0)) D
 . Q:$P(^BGPINDW(X,0),U,5)=""
 . S BGPI=BGPI+1
 . S ^BGPTMP($J,BGPI)=X_U_$P($G(^BGPINDW(X,0)),U,5)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
EPATLST(RETVAL,BGPSTR) ;-- get all GPRA measures
 ...
 . Q:$P(^BGPELIW(X,0),U,5)=""
 . S BGPI=BGPI+1
 . S ^BGPTMP($J,BGPI)=X_U_$P($G(^BGPELIW(X,0)),U,5)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
...
PPATLST(RETVAL,BGPSTR) ;-- get all GPRA measures
 ...
 . Q:$P(^BGPPEIW(X,0),U,4)=""
 . S BGPI=BGPI+1
 . S ^BGPTMP($J,BGPI)=X_U_$P($G(^BGPPEIW(X,0)),U,4)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
