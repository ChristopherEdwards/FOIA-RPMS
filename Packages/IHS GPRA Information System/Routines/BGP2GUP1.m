BGP2GUP1 ; IHS/CMI/LAB - GUI Upload Continued ;
...
PROCEO ;EP
 ...
 ;add entry
 L +^BGPEOCB:10 I '$T W !!,"unable to lock global. TRY LATER" D EOJ^BGP2GUPL Q
 L +^BGPEOPB:10 I '$T W !!,"unable to lock global. TRY LATER" D EOJ^BGP2GUPL Q
 L +^BGPEOBB:10 I '$T W !!,"unable to lock global. TRY LATER" D EOJ^BGP2GUPL Q
 D GETIEN^BGP2EOUT
 I 'BGPIEN S BGPRET=0_"^error in file creation...call programmer." D EOJ^BGP2GUPL Q
ELCY ;
 S DINUM=BGPIEN,X=$P(BGP2,U),DLAYGO=90549.1,DIC="^BGPEOCB(",DIC(0)="L"
 K DD,D0,DO
 D FILE^DICN
 I Y=-1 S BGPRET=0_"^error uploading file" D EOJ^BGP2GUPL Q
 S BGPIEN=+Y
 D ^XBFMK
...
ELPY ;
 S DINUM=BGPIEN,X=$P(BGP2,U),DLAYGO=90549.11,DIC="^BGPEOPB(",DIC(0)="L"
 K DD,D0,DO
 D FILE^DICN
 I Y=-1 S BGPRET=0_"^error uploading file" D EOJ^BGP2GUPL Q
 S BGPIEN=+Y
...
ELBY ;
 S DINUM=BGPIEN,X=$P(BGP2,U),DLAYGO=90549.12,DIC="^BGPEOBB(",DIC(0)="L"
 K DD,D0,DO
 D FILE^DICN
 I Y=-1 S BGPRET=0_"^error uploading file" D EOJ^BGP2GUPL Q
 S BGPIEN=+Y
