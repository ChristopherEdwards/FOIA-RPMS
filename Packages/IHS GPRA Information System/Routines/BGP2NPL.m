...
SI1 ;NOW SELECT ONE OR MORE W/IN THE TOPIC
 ...
 K BGPINDO
 S BGPIND=0 F  S BGPIND=$O(BGPIND(BGPIND)) Q:BGPIND'=+BGPIND  S BGPINDO($P(^BGPINDW(BGPIND,12),U,6),BGPIND)=""
 ...
 .K BGPX S BGPO=0,X=0,BGPC=0 F  S BGPO=$O(^BGPNPLW(BGPCR,BGPIND,BGPO)) Q:BGPO'=+BGPO!($D(DIRUT))  D
 ...
 ;get report type
 I $D(DIRUT) G SI
 K BGPQUIT D RT^BGP2DSL I '$D(BGPLIST)!($D(BGPQUIT)) G SI
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP2NPL",ZTDTH="",ZTDESC="NATIONAL GPRA REPORT 06" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
