...
DEL ;
 ...
 D ^BGP2PDLH
 D DEL1
 I BGPRTYPE=1 D NONSUM^BGP2PDLN
 I BGPRTYPE=1 D DEVSUM^BGP2PDLD
 ;I BGPRTYPE=1 D SUMMARY
 I BGPRTYPE=1 D SDP^BGP2PDLN
 I BGPRTYPE=1 D SDP^BGP2PDLD
 ;I BGPRTYPE=1 D SDP
 I BGPRTYPE=7 D OTHSUM^BGP2PDLO
 D ^BGP2PDLS ;print lists to delimited file
 I $G(BGPCPPL) D CPPL1^BGP2DCLD
...
DEL1 ;EP
 ...
 S BGPIC=0 F  S BGPIC=$O(BGPIND(BGPIC)) Q:BGPIC=""!(BGPQUIT)  I $P(^BGPINDW(BGPIC,0),U,14) D PRINT2 D
 .Q:BGPQUIT
 .I $G(BGPNPL),$D(BGPINDL(BGPIC)),'$D(BGPYNPLT) S BGPINDW=BGPIC D NPL1^BGP2NPLP ;printed nat gpra list
 .I $G(BGPNPL),$D(BGPINDL(BGPIC)),$D(BGPYNPLT) S BGPINDW=BGPIC D CT^BGP2DSTM ;printed nat gpra SEARCH TEMPLATE
 Q:BGPQUIT
...
PRINT2 ;
 ...
 .S X=$P(^BGPINDW(BGPIC,0),U,3) D S(X,1,1)
 .S X=" " D S(X,1,1)
 ...
 .S X="Logic:" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPINDW(BGPIC,BGPNODE,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDW(BGPIC,BGPNODE,BGPX,0) D S(X,1,1)
 .I BGPRTYPE=9 G CALC
 .S X=" " D S(X,1,1) S X="Performance Measure Description:" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDW(BGPIC,$S($G(BGPNGR09):57,1:51),BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDW(BGPIC,$S($G(BGPNGR09):57,1:51),BGPX,0) D S(X,1,1)
 .I $O(^BGPINDW(BGPIC,52,0)) S X=" " D S(X,1,1) S X="Past Performance and/or Target:" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDW(BGPIC,52,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDW(BGPIC,52,BGPX,0) D S(X,1,1)
 .I $O(^BGPINDW(BGPIC,55,0)) S X=" " D S(X,1,1) S X="Source:" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDW(BGPIC,55,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDW(BGPIC,55,BGPX,0) D S(X,1,1)
 .I '$O(^BGPINDW(BGPIC,55,0)) D S(" ",1,1)
CALC .X ^BGPINDW(BGPIC,4)
 Q
HEADER ;EP
 S Y=$P(^VA(200,DUZ,0),U,2),$E(Y,35)=$$FMTE^XLFDT(DT) D S(Y,1,1)
 I BGPRTYPE=7 S Y="*** IHS 2012 Other National Measures Report ***" D S(Y,1,1)
 I BGPRTYPE=4 S Y="*** IHS 2012 Clinical Performance Report ***" D S(Y,1,1)
 I BGPRTYPE=1,'$G(BGPNGR09) S Y="*** IHS 2012 National GPRA & PART Report ***" D S(Y,1,1)
 I BGPRTYPE=1,$G(BGPNGR09) S Y="*** IHS 2013 National GPRA & PART Report, Run Using 2012 Logic ***" D S(Y,1,1)
