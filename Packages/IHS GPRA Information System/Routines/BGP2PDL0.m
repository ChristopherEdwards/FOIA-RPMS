BGP2PDL0 ; IHS/CMI/LAB - print ind 1 01 Jul 2010 8:03 PM ;
 ;;12.0;IHS CLINICAL REPORTING;;JAN 9, 2012;Build 51
 ;
 ;
IC1 ;EP
 S BGPXXX=$P(^BGPINDW(BGPIC,12),U,6)
 I BGPINDW'="D" D:'$G(BGPSUMON) H1^BGP2PDL1
 I BGPINDW'="D" F BGPPC1=BGPXXX_".1",BGPXXX_".2",BGPXXX_".3" D PI^BGP2PDL1
 I BGPINDW'="D" D:'$G(BGPSUMON) H1^BGP2PDL1
 I BGPINDW'="D" F BGPPC1=BGPXXX_".4",BGPXXX_".5",BGPXXX_".6" D PI^BGP2PDL1
 D:'$G(BGPSUMON) H1^BGP2PDL1
 F BGPPC1=BGPXXX_".7",BGPXXX_".8",BGPXXX_".9" D PI^BGP2PDL1
 D I1AGE
 Q
I1AGE ;EP
 Q:BGPINDW="D"
 ...
 S X=^BGPINDW(BGPIC,53,1,0) D S(X,1,1) S X=" " D S(X,1,1) D H3 S X=" " D S(X,1,1)
...
I1AGE1 ;
 S C=C+1
 S BGPF="C-1."_BGPX_".3" S BGPPC=$O(^BGPINDWC("C",BGPF,0))
...
I1AGE2 ;
 S D=D+1
 S BGPF="C-1."_BGPX_".2" S BGPPC=$O(^BGPINDWC("C",BGPF,0))
...
I1AGE3 ;
 S BGPF="C-1."_BGPX_".1" S BGPPC=$O(^BGPINDWC("C",BGPF,0))
 ;set 6th piece to numerator and 7th to %
 S BGPNF=$P(^BGPINDWC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90548.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(D),U,6)=$$V^BGP2DP1C(1,BGPRPT,N,P),$P(BGPDAC(D),U,7)=$S($P(BGPDAC(D),U,1):($P(BGPDAC(D),U,6)/$P(BGPDAC(D),U,1)*100),1:"")
 S $P(BGPDAP(D),U,6)=$$V^BGP2DP1C(2,BGPRPT,N,P),$P(BGPDAP(D),U,7)=$S($P(BGPDAP(D),U,1):($P(BGPDAP(D),U,6)/$P(BGPDAP(D),U,1)*100),1:"")
 S $P(BGPDAB(D),U,6)=$$V^BGP2DP1C(3,BGPRPT,N,P),$P(BGPDAB(D),U,7)=$S($P(BGPDAB(D),U,1):($P(BGPDAB(D),U,6)/$P(BGPDAB(D),U,1)*100),1:"")
I1AGE4 ;
 S BGPF="C-1."_BGPX_".4" S BGPPC=$O(^BGPINDWC("C",BGPF,0))
 ;set 8th piece to numerator and 9th to %
 S BGPNF=$P(^BGPINDWC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90548.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(D),U,8)=$$V^BGP2DP1C(1,BGPRPT,N,P),$P(BGPDAC(D),U,9)=$S($P(BGPDAC(D),U,1):($P(BGPDAC(D),U,8)/$P(BGPDAC(D),U,1)*100),1:"")
 S $P(BGPDAP(D),U,8)=$$V^BGP2DP1C(2,BGPRPT,N,P),$P(BGPDAP(D),U,9)=$S($P(BGPDAP(D),U,1):($P(BGPDAP(D),U,8)/$P(BGPDAP(D),U,1)*100),1:"")
 S $P(BGPDAB(D),U,8)=$$V^BGP2DP1C(3,BGPRPT,N,P),$P(BGPDAB(D),U,9)=$S($P(BGPDAB(D),U,1):($P(BGPDAB(D),U,8)/$P(BGPDAB(D),U,1)*100),1:"")
 Q
I1AGEP ;
 D I1AGEP^BGP2PDL5
 Q
