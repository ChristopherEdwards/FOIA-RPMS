BGP2PDL4 ; IHS/CMI/LAB - print ind 1 01 Jul 2010 8:04 PM ;
...
I031 ;EP
 ...
 I BGPRTYPE=4,BGPINDW="C" D  Q
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".7.3D",0)) D PI1^BGP2PDL1
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".8.3D",0)) D PI1^BGP2PDL1
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".9.3D",0)) D PI1^BGP2PDL1
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".7.3E",0)) D PI1^BGP2PDL1
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".8.3E",0)) D PI1^BGP2PDL1
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".9.3E",0)) D PI1^BGP2PDL1
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".7.3F",0)) D PI1^BGP2PDL1
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".8.3F",0)) D PI1^BGP2PDL1
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".9.3F",0)) D PI1^BGP2PDL1
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".7.3G",0)) D PI1^BGP2PDL1
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".8.3G",0)) D PI1^BGP2PDL1
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".9.3G",0)) D PI1^BGP2PDL1
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".7.3H",0)) D PI1^BGP2PDL1
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".8.3H",0)) D PI1^BGP2PDL1
 .S BGPDENP=0 S BGPPC=$O(^BGPINDWC("OR",BGPORXX_".9.3H",0)) D PI1^BGP2PDL1
 F BGPPC1=BGPORXX_".1",BGPORXX_".2",BGPORXX_".3" D PI^BGP2PDL1
 F BGPPC1=BGPORXX_".4",BGPORXX_".5",BGPORXX_".6" D PI^BGP2PDL1
 D I1AGE
 Q
I1AGE ;EP
 ;Q:BGPRTYPE'=4
 I BGPINDW="W",BGPRTYPE=4 G FEM
 S BGPHD1="TOTAL ACTIVE CLINICAL POPULATION",BGPHD2="Total # Active Clin"
 S X=^BGPINDW(BGPIC,53,1,0) D S(X,3,1) S X=" " D S(X,1,1) D H3 S X=" " D S(X,1,1)
 ...
 ;S X=^BGPINDW(BGPIC,53,1,0) D S(X,1,1) D H3
...
I1AGE1 ;
 S C=C+1
 S BGPF="031."_BGPX_".1" S BGPPC=$O(^BGPINDWC("C",BGPF,0))
...
I1AGE2 ;
 S D=D+1
 S BGPF="031."_BGPX_".2" S BGPPC=$O(^BGPINDWC("C",BGPF,0))
 ;set 4th piece to numerator and 5th to %
 S BGPNF=$P(^BGPINDWC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90548.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(D),U,4)=$$V^BGP2DP1C(1,BGPRPT,N,P),$P(BGPDAC(D),U,5)=$S($P(BGPDAC(D),U,2):($P(BGPDAC(D),U,4)/$P(BGPDAC(D),U,2)*100),1:"")
 S $P(BGPDAP(D),U,4)=$$V^BGP2DP1C(2,BGPRPT,N,P),$P(BGPDAP(D),U,5)=$S($P(BGPDAP(D),U,2):($P(BGPDAP(D),U,4)/$P(BGPDAP(D),U,2)*100),1:"")
 S $P(BGPDAB(D),U,4)=$$V^BGP2DP1C(3,BGPRPT,N,P),$P(BGPDAB(D),U,5)=$S($P(BGPDAB(D),U,2):($P(BGPDAB(D),U,4)/$P(BGPDAB(D),U,2)*100),1:"")
I1AGE3 ;
 S BGPF="031."_BGPX_".3" S BGPPC=$O(^BGPINDWC("C",BGPF,0))
 S BGPNF=$P(^BGPINDWC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90548.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(D),U,6)=$$V^BGP2DP1C(1,BGPRPT,N,P),$P(BGPDAC(D),U,7)=$S($P(BGPDAC(D),U,2):($P(BGPDAC(D),U,6)/$P(BGPDAC(D),U,2)*100),1:"")
 S $P(BGPDAP(D),U,6)=$$V^BGP2DP1C(2,BGPRPT,N,P),$P(BGPDAP(D),U,7)=$S($P(BGPDAP(D),U,2):($P(BGPDAP(D),U,6)/$P(BGPDAP(D),U,2)*100),1:"")
 S $P(BGPDAB(D),U,6)=$$V^BGP2DP1C(3,BGPRPT,N,P),$P(BGPDAB(D),U,7)=$S($P(BGPDAB(D),U,2):($P(BGPDAB(D),U,6)/$P(BGPDAB(D),U,2)*100),1:"")
I1AGE4 ;
 S BGPF="031."_BGPX_".4" S BGPPC=$O(^BGPINDWC("C",BGPF,0))
 S BGPNF=$P(^BGPINDWC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90548.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(D),U,8)=$$V^BGP2DP1C(1,BGPRPT,N,P),$P(BGPDAC(D),U,9)=$S($P(BGPDAC(D),U,2):($P(BGPDAC(D),U,8)/$P(BGPDAC(D),U,2)*100),1:"")
 S $P(BGPDAP(D),U,8)=$$V^BGP2DP1C(2,BGPRPT,N,P),$P(BGPDAP(D),U,9)=$S($P(BGPDAP(D),U,2):($P(BGPDAP(D),U,8)/$P(BGPDAP(D),U,2)*100),1:"")
 S $P(BGPDAB(D),U,8)=$$V^BGP2DP1C(3,BGPRPT,N,P),$P(BGPDAB(D),U,9)=$S($P(BGPDAB(D),U,2):($P(BGPDAB(D),U,8)/$P(BGPDAB(D),U,2)*100),1:"")
I1AGE5 ;
 S BGPF="031."_BGPX_".5" S BGPPC=$O(^BGPINDWC("C",BGPF,0))
 S BGPNF=$P(^BGPINDWC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90548.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(D),U,10)=$$V^BGP2DP1C(1,BGPRPT,N,P),$P(BGPDAC(D),U,11)=$S($P(BGPDAC(D),U,2):($P(BGPDAC(D),U,10)/$P(BGPDAC(D),U,2)*100),1:"")
 S $P(BGPDAP(D),U,10)=$$V^BGP2DP1C(2,BGPRPT,N,P),$P(BGPDAP(D),U,11)=$S($P(BGPDAP(D),U,2):($P(BGPDAP(D),U,10)/$P(BGPDAP(D),U,2)*100),1:"")
 S $P(BGPDAB(D),U,10)=$$V^BGP2DP1C(3,BGPRPT,N,P),$P(BGPDAB(D),U,11)=$S($P(BGPDAB(D),U,2):($P(BGPDAB(D),U,10)/$P(BGPDAB(D),U,2)*100),1:"")
 Q
I1AGEP ;
