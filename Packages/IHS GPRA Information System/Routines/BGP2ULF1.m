BGP2ULF1 ; IHS/CMI/LAB - NO DESCRIPTION PROVIDED 02 Jul 2010 9:38 AM ;
...
PROCEO ;EP
 ...
 ;add entry
 L +^BGPEOCB:10 I '$T W !!,"unable to lock global. TRY LATER" D EOJ^BGP2ULF Q
 L +^BGPEOPB:10 I '$T W !!,"unable to lock global. TRY LATER" D EOJ^BGP2ULF Q
 L +^BGPEOBB:10 I '$T W !!,"unable to lock global. TRY LATER" D EOJ^BGP2ULF Q
 D GETIEN^BGP2EOUT
 I 'BGPIEN W !!,"error in file creation...call programmer." D EOJ^BGP2ULF Q
ELCY ;
 S DINUM=BGPIEN,X=$P(BGP0,U),DLAYGO=90549.1,DIC="^BGPEOCB(",DIC(0)="L"
 K DD,D0,DO
 D FILE^DICN
 I Y=-1 W !,"error uploading file......" H 4 D EOJ^BGP2ULF Q
 S BGPIEN=+Y
 D ^XBFMK
...
ELPY ;
 S DINUM=BGPIEN,X=$P(BGP0,U),DLAYGO=90549.11,DIC="^BGPEOPB(",DIC(0)="L"
 K DD,D0,DO
 D FILE^DICN
 I Y=-1 W !,"error uploading file......" H 4 D EOJ^BGP2ULF Q
 S BGPIEN=+Y
...
ELBY ;
 S DINUM=BGPIEN,X=$P(BGP0,U),DLAYGO=90549.12,DIC="^BGPEOBB(",DIC(0)="L"
 K DD,D0,DO
 D FILE^DICN
 I Y=-1 W !,"error uploading file......" H 4 D EOJ^BGP2ULF Q
 S BGPIEN=+Y
