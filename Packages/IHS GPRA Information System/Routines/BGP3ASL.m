BGP3ASL ; IHS/CMI/LAB - DISPLAY IND LISTS 28 Apr 2006 2:10 PM ;
...
EN ;EP -- main entry point
 D EN^VALM("BGP 13 AREA SELECT SITES")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ; -- init variables and list array
 I BGPRTYPE=1 D NGR^BGP3ASL1 Q
 I BGPRTYPE=5 D ELD^BGP3ASL1 Q
 I BGPRTYPE=6 D PED^BGP3ASL1 Q
 I BGPRTYPE=7 D ONM^BGP3ASL1 Q
 Q
FAC(S) ;
...
G1 ;
 ...
 . S BGPPBD=3120701,BGPPBD=3130630
 . S BGPPER=3120000
...
NT ;
 ...
 . S BGPFAC=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPGPDCH(BGPX,0),U,17):" (CHS ONLY)",1:"")
 . S BGPEBD=$$DATE^BGP3UTL($P(V,U))
 . S BGPEED=$$DATE^BGP3UTL($P(V,U,2))
 . S BGPEBBD=$$DATE^BGP3UTL($P(V,U,5))
 . S BGPEBED=$$DATE^BGP3UTL($P(V,U,6))
 . S BGPEDRR=$$DATE^BGP3UTL($P(V,U,13))
...
G5 ;
 ...
 S BGPFILE=90551.03
 S BGPX=0 F  S BGPX=$O(^BGPELDCH(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPELDCH(BGPX,0))
 . S V=^BGPELDCH(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
 ...
 .Q:$P(V,U,14)'=BGPBEN
 . S BGPSU=$E($$SU($P(V,U,11)),1,10)
 . S BGPFAC=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPELDCH(BGPX,0),U,17):" (CHS ONLY)",1:"")
 . S BGPEBD=$$DATE^BGP3UTL($P(V,U))
 . S BGPEED=$$DATE^BGP3UTL($P(V,U,2))
 . S BGPEBBD=$$DATE^BGP3UTL($P(V,U,5))
 . S BGPEBED=$$DATE^BGP3UTL($P(V,U,6))
...
G6 ;
 ...
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPFILE=90550.12
 S BGPX=0 F  S BGPX=$O(^BGPPEDCH(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPPEDCH(BGPX,0))
 . S V=^BGPPEDCH(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
...
G9 ;
 ...
 S BGPBBD=BGPBD-X,BGPBBD=$E(BGPBBD,1,3)_$E(BGPBD,4,7)
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPX=0 F  S BGPX=$O(^BGPGPDCH(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPGPDCH(BGPX,0))
 . S V=^BGPGPDCH(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
 ...
 .Q
 Q
G8 ;childhood weight  == 13.0 NO HEIGHT/WEIGHT
 ;S BGPCHWE=1
 ;D G1
 ;Q
