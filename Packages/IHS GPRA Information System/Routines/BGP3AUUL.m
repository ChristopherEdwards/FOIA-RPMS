BGP3AUUL ;IHS/CMI/LAB - AREA UPLOAD;
...
PROC ;
 ...
 ;add entry
 L +^BGPGPDCH:10 I '$T D EOJ Q
 L +^BGPGPDPH:10 I '$T D EOJ Q
 L +^BGPGPDBH:10 I '$T D EOJ Q
 D GETIEN^BGP3UTL
 I 'BGPIEN D EOJ Q
CY ;
...
SCH ;EP - called from option to schedule the area aggregate for the next "2nd Friday"
 ...
 S BGPTASK=$$CHKFQT()
 I BGPTASK W !!,"The option is already scheduled to run: TASK # ",BGPTASK," at ",$$HTE^XLFDT($P(^%ZTSK(BGPTASK,0),U,6)),"." D DEL,PAUSE^BGP3DU,XIT^BGP3AUUP Q
 NEW BGPDT,BGPX,BGPY
 S BGPDT=DT
 ...
 I $D(DIRUT) Q
 I 'Y Q
 ;S BGPOPT="BGP 13 AUTO PROC SITE FILES"
 ;S BGPOPTD0=$O(^DIC(19,"B",BGPOPT,0))
 ;I 'BGPOPTD0 D  Q
 ;. D BMES^XPDUTL("'BGP 13 AUTO PROC SITE FILES' OPTION NOT FOUND!")
 ;. D PAUSE^BGP3DU
 ;. D XIT^BGP3AUUP
 ;Q:'BGPOPTD0
 ...
 S ZTDTH=BGPX
 ;S ZTSAVE("BGP*")=""
 S ZTRTN="AUTO^BGP3AUUP"
 S ZTDESC="BGP3 AUTO GPRA AREA AGGREGATE"
 S ZTIO=""
 D ^%ZTLOAD
 S BGPTSK=$G(ZTSK)
 D BMES^XPDUTL("OPTION 'BGP3 AUTO AREA AGGREGATE' SCHEDULED AS TASK #"_BGPTSK)
 D PAUSE^BGP3DU,XIT^BGP3AUUP
 Q
...
CHKFQT() ;EP - check for queued task (BGP AUTO GPRA EXTRACT and BGPSITE variable within the task
 ...
 .Q:$P($G(^%ZTSK(X,0)),U,1,2)'="AUTO^BGP3AUUP"
 .Q:$P($G(^%ZTSK(X,.03)),U,1)'="BGP3 AUTO GPRA AREA AGGREGATE"  ;"BGP 13 AUTO GPRA EXTRACT"  ;not the gpra export
