BGP3AUUP ; IHS/CMI/LAB - UPLOAD SITE FILES, CREATE EXCEL FILES ;
...
EN(BGPBEGDT,BGPENDDT,BGPMAN) ;EP -- MAIN ENTRY POINT
 ...
 ;
 K ^TMP("BGPGPAM",$J,"FILE")
 K BGPLIST S BGPLIST="",X=$$LIST^%ZISH(BGPPATH,"BGPAM130*",.BGPLIST)
 ...
 . I $G(BGPMAN) D  Q
 . . W !,BGPERR
 . . D MSG1^BGP3AU1M(BGPBEGDT,BGPENDDT,.BGPFACS)
 ...
 Q:BGPOUT
 ;
 ;upload all files into the BGP 13 DATA* files
 D UPLOAD(BGPPATH,BGPBEGDT,BGPENDDT,.BGPFACS)
 ;
 ;create the 5 excel files
 D EN^BGP3AU1A(BGPBEGDT,BGPENDDT,.BGPEND,.BGPF)
 ;
 ...
 D EN^BGP3AU1D(BGPBEGDT,BGPEND,.BGPDFILE)
 ;
 ;
...
CHK(BGPBEGDT,BGPFACS,BGPOUT) ;
 ...
 . Q:BGPDATA'["BGPAM130"
 . S BGPFILE="BGPAM130"_$P(BGPDATA,"BGPAM130",2)
...
CLEAN(BGPBEGDT,BGPENDDT) ;
 ...
 S BGPD0=0 F  S BGPD0=$O(^BGPGPDCH("B",BGPBEGDT,BGPD0)) Q:'BGPD0  D
 . S BGPASU=$P(^BGPGPDCH(BGPD0,0),U,9)
 . I $G(BGPFACS(BGPENDDT,BGPASU))="" Q
 . Q:$P(^BGPGPDCH(BGPD0,0),U,12)'=1  ;DON'T DELETE OTHER NATIONAL, GPU, ETC.
 . S DA=BGPD0
 . S DIK="^BGPGPDCH("
 . D ^DIK
 ;
 S BGPD0=0 F  S BGPD0=$O(^BGPGPDPH("B",BGPBEGDT,BGPD0)) Q:'BGPD0  D
 . S BGPASU=$P(^BGPGPDPH(BGPD0,0),U,9)
 . I $G(BGPFACS(BGPENDDT,BGPASU))="" Q
 . Q:$P(^BGPGPDPH(BGPD0,0),U,12)'=1  ;DON'T DELETE OTHER NATIONAL, GPU, ETC.
 . S DA=BGPD0
 . S DIK="^BGPGPDPH("
 . D ^DIK
 ;
 S BGPD0=0 F  S BGPD0=$O(^BGPGPDBH("B",BGPBEGDT,BGPD0)) Q:'BGPD0  D
 . S BGPASU=$P(^BGPGPDBH(BGPD0,0),U,9)
 . I $G(BGPFACS(BGPENDDT,BGPASU))="" Q
 . Q:$P(^BGPGPDBH(BGPD0,0),U,12)'=1  ;DON'T DELETE OTHER NATIONAL, GPU, ETC.
 . S DA=BGPD0
 . S DIK="^BGPGPDBH("
 . D ^DIK
 Q
