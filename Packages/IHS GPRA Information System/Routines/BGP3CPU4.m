BGP3CPU4 ; IHS/CMI/LAB - calc CMS measures ;
 ;;13.0;IHS CLINICAL REPORTING;;NOV 20, 2012;Build 81
 ;
 ;
WPOSTINF ;EP
 I $Y>(BGPIOSL-8) D HDR^BGP3CP Q:BGPQUIT  D L1H^BGP3CP
...
PERI ;EP
 ...
 W !!?2,$S(BGPPEX[1:"*",1:""),"Preoperative Infectious Disease Diagnosis?  ",$P($$ADMPRIM^BGP3CU5(BGPVINP,"BGP CMS INFECTIOUS DXS"),U,2)
 Q
 ;
WANTIRX ;EP
...
OTHSURG ;EP
 ...
 K BGPG
 S Y="BGPG("
 S X=DFN_"^ALL PROCEDURES;DURING "_$$VD^APCLV(BGPVSIT)_"-"_$$DSCH^BGP3CU(BGPVINP) S E=$$START1^APCLDF(X,Y)
 ...
 .W !?4,$$VAL^XBDIQ1(9000010.08,X,.01),"  ",$$DATE^BGP3UTL(D),"  ",$$VAL^XBDIQ1(9000010.08,X,.04) S F=1
 Q:'F
...
WOTHPROC ;EP
 K BGPXX
 S BGPC=0
 S BGPB=(9999999-$$DSCH^BGP3CU(BGPVINP))-1,BGPE=9999999-$P($P(^AUPNVSIT(BGPVSIT,0),U),".")
 ...
 NEW D,BD,ED,Y,D,V
 S BGPB=9999999-$$DSCH^BGP3CU(BGPVINP),BGPE=9999999-$P($P(^AUPNVSIT(BGPVSIT,0),U),".")
...
WPP1 ;EP
 ...
 W !?2,$S(BGPPPD<$P($P(BGPVSIT0,U),"."):"*",1:""),"Principle Procedure Date:  ",$$DATE^BGP3UTL(BGPPPD)
 Q
 ;
WPP ;EP
 I $Y>(BGPIOSL-3) D HDR^BGP3CP Q:BGPQUIT  D L1H^BGP3CP
...
WDOD(V) ;EP - write dod
 ...
 I $$DOD^AUPNPAT(V)]"" D
 .W !!?2,"*Date of Death:  ",$$DATE^BGP3UTL($$DOD^AUPNPAT(V))
 Q
 ;
