...
DMNA ;EP - called from elder care
 ...
 S BGPBD2=BGPBDATE
I61 ;EP
 S BGPGFR=$$GFR^BGP3D211(DFN,BGPBD1,BGPEDATE)
 S BGPESRD=$$ESRD^BGP3D211(DFN,$P(^DPT(DFN,0),U,3),BGPEDATE)
 S BGPQUP=$$QUANTUP^BGP3D211(DFN,BGPBD2,BGPEDATE)
 ...
 I BGPN1 D
 .I BGPESRD S BGPVALUE=BGPVALUE_$S(BGPESRD]"":"ESRD: "_$$DATE^BGP3UTL($P(BGPESRD,U,3))_" "_$P(BGPESRD,U,2),1:"") Q
 .S BGPVALUE=BGPVALUE_$S(BGPESRD:"; ",1:""),BGPVALUE=BGPVALUE_"GFR: "_$$DATE^BGP3UTL($P(BGPGFR,U,2))
...
DMEYE ;EP - called from elder care
 ...
 S BGPVALUE=$S(BGPDMD1:"UP",1:"")_$S(BGPDMD2:",AD",1:"")_$S(BGPDMD3:",AAD",1:"")_"|||"
 S X=$S(BGPEYE:"Eval: "_$$DATE^BGP3UTL($P(BGPEYE,U,2))_" "_$P(BGPEYE,U,3),1:"")
...
I8 ;EP
 ...
 I BGPRTYPE=1 S BGPVALUE="AD|||" I BGPN1 S BGPVALUE=BGPVALUE_$$DATE^BGP3UTL($P(BGPVALUE,U,2))_" "_$P(BGPVALUE,U,3)
 I BGPRTYPE'=1 S BGPVALUE="AD|||"_$$DATE^BGP3UTL($P(BGPVALUE,U,2))_" "_$P(BGPVALUE,U,3)
 Q
I9 ;EP
 K BGPN1,BGPVALUE,BGPN2,BGPN3,BGPN4,BGPN5,BGPN6
 ...
 S BGPVALUD="" I BGPN3 S BGPVALUD="UP w/exam "_$$DATE^BGP3UTL($P(BGPV1,U,2))_" "_$P(BGPV1,U,3)
 S BGPVALUD=BGPVALUD_$S(BGPD10:",PREG",1:"")
 S BGPVALUD=BGPVALUD_"|||"
 I BGPTC S BGPVALUD=BGPVALUD_"TC: "_$$DATE^BGP3UTL($P(BGPTC,U,2))_" "_$P(BGPTC,U,3)
 I BGPN6 S BGPVALUD=BGPVALUD_$S(BGPTC:"; ",1:""),BGPVALUD=BGPVALUD_"PN: "_$$DATE^BGP3UTL($P(BGPPN,U,2))_" "_$P(BGPPN,U,3)
 ;S BGPN3=0 I BGPN1,'BGPN2 S BGPN3=1
 I BGPRTYPE'=1 S BGPVALUE="UP|||"_$$DATE^BGP3UTL($P(BGPVALUE,U,2))_" "_$P(BGPVALUE,U,3)
 I BGPRTYPE=1 S X=BGPVALUE S BGPVALUE="UP|||" I BGPN1 S BGPVALUE=BGPVALUE_$$DATE^BGP3UTL($P(X,U,2))_" "_$P(X,U,3)
 Q
PREG(P,BDATE,EDATE) ;
 NEW X,Y,Z
 S (X,Y)=0
 I $P(^DPT(P,0),U,2)'="F" Q 0
 S X=$$PREG^BGP3D7(P,$$FMADD^XLFDT(EDATE,-608),EDATE,1,1)
...
BF(P,BDATE,EDATE) ;
 ...
 S X=$$LASTDX^BGP3UTL1(P,"BGP BREASTFEEDING DXS",BDATE,EDATE)
 I X Q 1
 K BGPG
 S Y="BGPG("
...
DENTDEV(P,BDATE,EDATE) ;
 ...
 S BGPC=$$CPT^BGP3DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP DENTAL EXAM CPTS",0)),5)
 I BGPC Q 1_U_BGPC
 Q ""
TC(P,BDATE,EDATE) ;
 NEW BGPG,BGPC,%
...
EYE(P,BDATE,EDATE) ;EP
 ...
 .S X=$$CPT^BGP3DU(P,BDATE,EDATE,T,5) I X]"" Q
 .S X=$$TRAN^BGP3DU(P,BDATE,EDATE,T,5)
 ;S X=$$LASTPRC^BGP3UTL1(P,"BGP EYE EXAM PROCS",BDATE,EDATE) I X]"" Q 3_U_$P(X,U,3)_U_"Proc "_$P(X,U,2)
...
GFR(P,BDATE,EDATE) ;EP
 ...
 ...I T,$P(^AUPNVLAB(X,0),U)=T S BGPC=1_U_$$DATE^BGP3UTL((9999999-D)) Q
...
DMACE ;EP
 ...
 ;BGPN4 AND BGPN5, BGPN6 ARE GPRA DEV 2013
 I 'BGPDMD2 S BGPSTOP=1 Q  ;don't process this measure, pt not diabetic per active diabetic definition
 ;
 ;does patient have HTN?
 I '$$V1HTN^BGP3D9(DFN,BGP365,BGPEDATE) S BGPSTOP=1 Q  ;no 1 visit of htn during report period
 I '$$FIRSTHTN^BGP3D9(DFN,BGPEDATE) S BGPSTOP=1 Q  ;no htn prior to report period
 S BGPD1=1,BGPN1=0
 ...
 S BGPACEC=$$ACECONT^BGP3D723(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,BGPBDATE,BGPEDATE,BGPBDATE)
 I BGPACEC S BGPN2=1,BGPVALUE="AD|||"_$P(BGPACEC,U,2) G DMACEX
 S BGPACEA=$$ACEALG^BGP3D723(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)
...
ACE1 .;
 ...
 .S K=S+K
 .I R]"" S R=R_";"
 .S R=R_$$DATE^BGP3UTL($P($P(^AUPNVSIT(V,0),U),"."))
...
JVN(P,BDATE,EDATE) ;EP
 ...
 .I Y S $P(BGPVAL,U,3)="Optom: "_$$DATE^BGP3UTL(D)_" Prv "_R
 Q BGPVAL
