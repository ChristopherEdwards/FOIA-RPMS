BGP3D3 ; IHS/CMI/LAB - measure 11 17 Oct 2009 12:40 PM ;
 ;;13.0;IHS CLINICAL REPORTING;**1**;NOV 20, 2012;Build 7
 ;
 ;
I10 ;EP
 S (BGPN1,BGPN2,BGPN3,BGPN4,BGPN5,BGPN6,BGPN7,BGPN8,BGPN9,BGPD1,BGPD2,BGPD3,BGPD4,BGPD5,BGPD6,BGPD7)=0
 S (BGPSEAL,BGPSEALD)=""
 K BGPSEALS,BGPSELDS
 D SEAL^BGP3D3A(DFN,BGPBDATE,BGPEDATE) ;this is for the number of sealants only
 S BGPN1=$P(BGPSEAL,U)
 I BGPAGEB>1,BGPAGEB<16 S BGPN2=BGPN1
 I BGPAGEB>1,BGPAGEB<16,BGPN1 S BGPN7=1
 I BGPAGEB>15 S BGPN3=BGPN1
 I BGPAGEB>18 S BGPN4=BGPN1
I101 ;
 ;get new sealant definition 2013 gpra dev
 S BGPSEALD=""
 I 'BGPN1 S BGPSEALD=$$SEALDEV^BGP3D3A(DFN,BGPBDATE,BGPEDATE)
 ...
 I 'BGPN1,BGPD7 S BGPVALUE=BGPVALUE_$P(BGPSEALD,U,2)
 I BGPN1 S BGPVALUE=BGPVALUE_"; "_+BGPN1_" sealants" S X=0 F  S X=$O(BGPSEALS(X)) Q:X'=+X  S D=0 F  S D=$O(BGPSEALS(X,D)) Q:D'=+D  S BGPVALUE=BGPVALUE_$S(X=1:"",1:"; ")_$$DATE^BGP3UTL(D)_" "_BGPSEALS(X,D)
 K BGPSEAL,BGPSEALD
 Q
I11 ;EP
 ...
 S BGPFLUOR=$$TF^BGP3D3A(DFN,BGPBDATE,BGPEDATE)
...
I111 ;
 ...
 I +BGPN1 S BGPVALUE=BGPVALUE_": " F X=2:1:5 I $P(BGPFLUOR,U,X)]"" S BGPVALUE=BGPVALUE_$S(X>2:", ",1:"")_$$DATE^BGP3UTL($P($P(BGPFLUOR,U,X),"|",1))_" "_$P($P(BGPFLUOR,U,X),"|",2)
 ;I BGPN2 S BGPVALUE=BGPVALUE_": "_$$DATE^BGP3UTL($P(BGPFLUOR,U,2))_" "_$P(BGPFLUOR,U,3)
 S BGPVALUD="UP|||"
 I +BGPN1 S BGPVALUD=BGPVALUD_+BGPN1_" topical fluoride" I +BGPN1 S BGPVALUD=BGPVALUD_": " F X=2:1:5 I $P(BGPFLUOR,U,X)]"" S BGPVALUD=BGPVALUD_$S(X>2:", ",1:"")_$$DATE^BGP3UTL($P($P(BGPFLUOR,U,X),"|",1))_" "_$P($P(BGPFLUOR,U,X),"|",2)
 ;I BGPN2 S BGPVALUD=BGPVALUD_": "_$$DATE^BGP3UTL($P(BGPFLUOR,U,2))_" "_$P(BGPFLUOR,U,3)
 K ^TMP($J,"A"),BGPFLUOR
 Q
I12 ;EP
 NEW BGPHR,V
 ...
 S BGPHR=$$HIGHR^BGP3D3A(DFN,BGPEDATE)
 ...
 .I BGPD4 S BGPDV=BGPDV_$S(BGPDV]"":",AD",1:"AD")
 .I BGPHR S BGPDV=BGPDV_$S(BGPDV]"":",HR",1:"HR")
 I BGPRTYPE'=1 S BGPVALUE=BGPDV_"|||"_$$DATE^BGP3UTL($P(BGPVALUE,U,1))_" "_$P(BGPVALUE,U,2)
 I BGPRTYPE=1 D
 .S BGPDV=BGPDV_"|||"
 .I BGPN7 S BGPVALUE=BGPDV_$$DATE^BGP3UTL($P(BGPVALUE,U,1))_" "_$P(BGPVALUE,U,2) Q
 .S BGPVALUE=BGPDV
...
I13 ;EP
 ...
 I BGPAGEB>17,BGPAGEB<65,BGPACTCL,$$HIGHRP^BGP3D3A(DFN,BGPEDATE) S BGPD4=1
 I BGPAGEB>17,BGPAGEB<65,BGPACTUP,$$HIGHRP^BGP3D3A(DFN,BGPEDATE) S BGPD5=1
...
PN ;EP - called from elder
 S BGPVALUE=$$PNEU^BGP3D31(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE) ;set to date of PNEU shot
 ...
 S BGPVAL=$$PNEU^BGP3D31(DFN,$$FMADD^XLFDT(BGPEDATE,-(5*365)),BGPEDATE)
 ...
 S BGPN7=0,BGPA65="" I BGPAGEB>65 S BGPA65=$$PNEU^BGP3D31(DFN,$$FMADD^XLFDT($$DOB^AUPNPAT(DFN),+(65*365)),BGPEDATE) I $P(BGPA65,U,3)=1!($P(BGPA65,U,3)=3) S BGPN4=1,BGPN7=1 ;over 65 and had one after 65
 ...
 .I BGPN1 S BGPVALH=BGPVALUE,BGPVALUE=BGPDV_"|||" I BGPVALH]"" S BGPVALUE=BGPVALUE_"Pneumo: "_$$DATE^BGP3UTL($P(BGPVALH,U,1))_" "_$P(BGPVALH,U,2)_$S(BGPVALH]""&($P(BGPVALH,U,2)'["Ref"):" (ever)",1:"") Q
 .S BGPVALUE=BGPDV_"|||"
 I BGPRTYPE=4 D
 .S BGPDV="UP"
 ...
 .I BGPD2 S BGPDV=BGPDV_$S(BGPDV]"":",AD",1:"AD")
 I BGPRTYPE=7 S BGPDV=$S(BGPD6:"AC",1:"") S:BGPD4 BGPDV=BGPDV_$S(BGPDV]"":",HR",1:"HR") S:BGPD2 BGPDV=BGPDV_$S(BGPDV]"":",AD",1:"AD")
 I BGPRTYPE=3!(BGPRTYPE=5) S BGPVALUE="AC"_"|||"_$$DATE^BGP3UTL($P(BGPVALUE,U,1))_" "_$P(BGPVALUE,U,2) I 1
 E  D
 .I BGPRTYPE'=1 S BGPVALH=BGPVALUE,BGPVALUE=BGPDV_"|||" I BGPVALH]"" S BGPVALUE=BGPVALUE_"Pneumo: "_$$DATE^BGP3UTL($P(BGPVALH,U,1))_" "_$P(BGPVALH,U,2)_$S(BGPVALH]""&($P(BGPVALH,U,2)'["Ref"):" (ever)",1:"")_" "_$S(BGPN8:" (up-to-date)",1:"")
 .S BGPVALUD="AC"_"|||"
 .I BGPN4 S BGPVALUD=BGPVALUD_"Pneumo: "_$$DATE^BGP3UTL($P(BGPVALH,U,1))_" "_$P(BGPVALH,U,2)_$S(BGPVALH]""&($P(BGPVALH,U,2)'["Ref"):" (ever)",1:"")_" "
 .S BGPVALUD=BGPVALUD_$S(BGPN8:"(up-to-date)",1:"")
TD ;new tdap and td stuff for v11.1
 S BGPTDAP=$$DTAP^BGP3D3A(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)
 I BGPTDAP S BGPN9=1
 S BGPTD=$$DTAPTD^BGP3D3A(DFN,$$FMADD^XLFDT(BGPEDATE,-(10*365)),BGPEDATE)
 I BGPTD S BGPN10=1
 I BGPRTYPE=1 G TDE
 I BGPTDAP S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":"; ",1:""),BGPVALUE=BGPVALUE_"TDAP: "_$P(BGPTDAP,U,2)_" (ever)"
 ;I BGPTDAP,'BGPTD S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":"; ",1:""),BGPVALUE=BGPVALUE_"TDAP: "_$P(BGPTDAP,U,2)_" (ever)"
...
TDE K BGPLPNU,BGPVAL,BGPA65,BGPVALH
 Q
I15 ;EP
 ...
 S (BGPD1,BGPD2,BGPD3,BGPD4,BGPN5,BGPN6)=0
 S BGPI7=$$DEN7(DFN,BGPAGEB,BGPAGEE,BGPSEX,BGPEDATE)
 I BGPACTUP,BGPI7 S BGPI7DA=1
 I BGPACTCL,BGPI7 S BGPI7DB=1
 I BGPACTCL,BGPI7,BGPAGEB>24,BGPAGEE<65 S BGPI7DC=1,BGPD4=1  ;2013 GPRA DEV
 I BGPACTUP,BGPI7,BGPAGEB>24,BGPAGEE<65 S BGPI7DD=1
 I BGPACTCL,BGPI7,BGPAGEB>29,BGPAGEE<65 S BGPD1=1
 I BGPACTCL,BGPI7,BGPAGEB>24,BGPAGEB<30 S BGPD2=1
 ...
 S BGPN4=0 I $P(BGPPAP4,U,1)=1 S BGPN4=1  ;PAP IN 4 YRS, NO REFUSALS
 ;for gpra dev, if they didn't have pap in 4 years check HPV
 I 'BGPN4,BGPD1 D
 .S BGPPAP6=$$PAP(DFN,BGPEDATE,6)
 .S BGPHPV=$$HPV(DFN,BGPEDATE,6)
 .I BGPHPV,BGPPAP6 S BGPN5=1
 I BGPN4!(BGPN5) S BGPN6=1
 ;I BGPTIME=1 W ?40,"BGPN4=",BGPN4,?50,"BGPN5=",?60,"BGPN6=",BGPN6
 S BGPVALUE=$S(BGPI7DD:"UP",1:"")_$S(BGPI7DC:",AC",1:"")_"|||"_$$DATE^BGP3UTL($P(BGPPAP4,U,2))_" "_$P(BGPPAP4,U,3)
 ;I BGPRTYPE=1 S BGPVALUE=$S(BGPI7DA:"UP",1:"")_$S(BGPI7DB:",AC",1:"")_"|||" I BGPN3 S BGPVALUE=BGPVALUE_$$DATE^BGP3UTL($P(BGPPAP,U,2))_" "_$P(BGPPAP,U,3)
 ;I BGPRTYPE=3 S BGPVALUE="AC|||"_$$DATE^BGP3UTL($P(BGPPAP,U,2))_" "_$P(BGPPAP,U,3)
 S BGPVALUD="AC|||" I BGPN4 S BGPVALUD=BGPVALUD_$$DATE^BGP3UTL($P(BGPPAP4,U,2))_" "_$P(BGPPAP4,U,3)
 I BGPN6,'BGPN4 S BGPVALUD=BGPVALUD_"HPV: "_$$DATE^BGP3UTL($P(BGPHPV,U,2))_" PAP: "_$$DATE^BGP3UTL($P(BGPPAP6,U,2))
...
HPV(P,EDATE,YEARS) ;
 ...
 K BGP
 S T="BGP HPV DXS"
 S X=$$LASTDX^BGP3UTL1(P,T,BDATE,EDATE) I X,$P(BGPLPAP,U,2)<$P(X,U,3) S BGPLPAP="1^"_$P(X,U,3)_"^POV "_$P(X,U,2)
...
PAP(P,EDATE,YEARS,REFUSAL) ;EP
 ...
 S X=$$LASTDX^BGP3UTL1(P,T,BDATE,EDATE) I X,$P(BGPLPAP,U,2)<$P(X,U,3) S BGPLPAP="1^"_$P(X,U,3)_"^POV "_$P(X,U,2)
 K BGP S BGP(1)=$$LASTPRC^BGP3UTL1(P,"BGP PAP PROCEDURES",BDATE,EDATE)
 ...
 S T=$$REFUSAL^BGP3UTL1(P,60,$O(^LAB(60,"B","PAP SMEAR",0)),$$FMADD^XLFDT(EDATE,-365),EDATE)
 ...
 .Q:'Y
 .S T=$$REFUSAL^BGP3UTL1(P,60,Y,$$FMADD^XLFDT(EDATE,-365),EDATE)
 I T Q 1_"^"_$P(T,U,2)_"^Refused Lab "
 ;add cpt Refusals v11.1
 S T=$$CPTREFT^BGP3UTL1(P,$$FMADD^XLFDT(EDATE,-365),EDATE,$O(^ATXAX("B","BGP CPT PAP",0)))
...
HYSTER(P,EDATE) ;EP 
 I '$G(P) Q ""
 NEW X,T
 S X=$$LASTPRC^BGP3UTL1(P,"BGP HYSTERECTOMY PROCEDURES",$$DOB^AUPNPAT(P),EDATE)
 I X Q 1
 S T="HYSTERECTOMY",T=$O(^BWPN("B",T,0))
 I T D  I X]"" Q 1
 .S X=$$WH^BGP3DU(P,$$DOB^AUPNPAT(P),EDATE,T,2)
 S T=$O(^ATXAX("B","BGP HYSTERECTOMY CPTS",0))
 I T D  I X]"" Q 1
 .S X=$$CPT^BGP3DU(P,$P(^DPT(P,0),U,3),EDATE,T,3) I X]"" Q
 .S X=$$TRAN^BGP3DU(P,$P(^DPT(P,0),U,3),EDATE,T,3)
 S X=$$LASTDX^BGP3UTL1(P,"BGP HYSTERECTOMY DXS",$$DOB^AUPNPAT(P),EDATE)
 I X Q 1
 S X=$$PLTAX^BGP3DU(P,"BGP HYSTERECTOMY DXS")
 I X Q 1
 Q ""
