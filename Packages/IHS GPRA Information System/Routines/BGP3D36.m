BGP3D36 ; IHS/CMI/LAB - measure C 06 Nov 2009 2:03 PM 28 May 2013 4:23 PM ;
...
MEN(P,EDATE) ;EP
 ...
 F BGPZ=32,108,114,136,147 S X=$$ANCONT^BGP3D31(P,BGPZ,EDATE) Q:X]""
 ...
 F BGPIMM=32,108,114,136,147 S R=$$IMMREF^BGP3D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
...
HPV(P,EDATE) ;EP
 ...
 .S I=+$$CODEN^ICPTCOD(BGPIMM) Q:'I
 .S X=0 F  S X=$O(^AUPNPREF("AA",P,81,I,X)) Q:X'=+X  S Y=0 F  S Y=$O(^AUPNPREF("AA",P,81,I,X,Y)) Q:Y'=+Y  S D=$P(^AUPNPREF(Y,0),U,3) I D'<B&(D'>E) S:$P(^AUPNPREF(Y,0),U,7)="N" BGPNMI=1 S R=1
 I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI HPV",1:"Ref HPV")
 ;now check Refusals in imm pkg
 F BGPIMM=62,118 S R=$$IMMREF^BGP3D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
 I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI",1:"Ref")_" HPV"
 F BGPZ=62,118 S X=$$ANCONT^BGP3D31(P,BGPZ,EDATE) Q:X]""
...
H1N1 ;EP - called from measure
 ...
 I BGPN1!(BGPN2) S BGPVALUE=BGPVALUE_$S($P(BGPH1N1,U,1):$$DATE^BGP3UTL($P(BGPH1N1,U,1))_" "_$P(BGPH1N1,U,2),1:"")
 I $P(BGPH1N1,U,3) S BGPVALUE=BGPVALUE_"; "_$$DATE^BGP3UTL($P(BGPH1N1,U,3))_" "_$P(BGPH1N1,U,4)  ;:"2 doses",$P(BGPH1N1,U,1):"1 dose",1:"")_" "_$$DATE^BGP3UTL($P(BGPH1N1,U))_" "_$$DATE^BGP3UTL($P(BGPH1N1,U,2))
 Q
...
ACTPREG(P,ED) ;EP
 ...
 K BGPG S BGPG=$$LASTPRC^BGP3UTL1(P,"BGP ABORTION PROCEDURES",LPD,ED)
