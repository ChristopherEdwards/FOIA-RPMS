BGP3D38 ; IHS/CMI/LAB - IMMUNIZATIONS ;
...
INFLU(P,EDATE) ;EP
 ...
 ;
 F BGPZ=15,16,88,111,135,140,141 S X=$$ANEGCONT^BGP3D31(P,BGPZ,EDATE) Q:X]""  ;cmi/maw 12/17/07 missing edate
 ...
 S R=$$CPTREFT^BGP3UTL1(P,$$DOB^AUPNPAT(P),EDATE,$O(^ATXAX("B","BGP CPT FLU",0)))
 I R S:$P(R,U,3)="N" BGPNMI=1 Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI",1:"Ref")_" Influenza"
 ;now check Refusals in imm pkg
 S BGPNMI=""
 F BGPIMM=15,16,88,111,135,140,141,144 S R=$$IMMREF^BGP3D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
...
HIB3(P,EDATE) ;EP
 ...
 ;now check Refusals in imm pkg
 ;F BGPIMM=49,51 S R=$$IMMREF^BGP3D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
 ;I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI",1:"Ref")_" 3 DOSE Hib"
 F BGPZ=49,51 S X=$$ANCONT^BGP3D31(P,BGPZ,EDATE) Q:X]""  ;cmi/maw 12/17/07 missing edate
...
HIB4(P,EDATE) ;EP
 ...
 ;now check Refusals in imm pkg
 ;F BGPIMM=17,22,46,47,48,49,50,51,102,120,132 S R=$$IMMREF^BGP3D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
