BGP3D3A ; IHS/CMI/LAB - measure 11 17 Oct 2009 12:40 PM 15 Apr 2013 8:06 AM ;
...
SEALDEV(P,BDATE,EDATE) ;EP
 ...
 K BGPG
 S %=P_"^ALL ADA 0007;DURING "_$$DATE^BGP3UTL(T)_"-"_$$DATE^BGP3UTL(EDATE),E=$$START1^APCLDF(%,"BGPG(")
 S %="",X=0
 F  S X=$O(BGPG(X)) Q:X'=+X  S V=$P(BGPG(X),U,5) I '$$HAS13(V) S %=1_U_$$DATE^BGP3UTL($P(BGPG(1),U,1))_" ADA 0007"
 Q %
HAS13(V) ;
 NEW X,Y,Z
 S Z=0
 ...
 Q Z
 ;I $D(BGPG(1)) Q 1_U_$$DATE^BGP3UTL($P(BGPG(1),U,1))_" ADA 0007"
 ;S T=$$FMADD^XLFDT(EDATE,-1096)
 ;S %="",E=+$$CODEN^ICPTCOD("D0007"),%=$$CPTI^BGP3DU(P,T,EDATE,E)
 ;I % Q 1_"^"_$$DATE^BGP3UTL($P(%,U,2))_" CPT D0007"
 Q ""
TF(P,BDATE,EDATE) ;EP
 ...
 ...I $$ICD^ATXCHK(A,$O(^ATXAX("B","BGP TOPICAL FLUORIDE DXS",0)),9) S $P(BGPC,U,1)=$P(BGPC,U,1)+1,G=1,$P(BGPC,U,BGPC+1)=$$VD^APCLV($P(^AUPNVPOV(Y,0),U,3))_"|POV "_$$VAL^XBDIQ1(9000010.07,Y,.01)  ;one per visit
 ...Q
 I BGPC Q BGPC
 Q ""
...
DTAP(P,BDATE,EDATE) ;EP
 ...
 I $O(BGPX(0)) Q 1_U_$$DATE^BGP3UTL($O(BGPX(0)))_" Imm 115"
 ;now get cpts
 ...
 I $D(BGPVARI) Q 1_U_$$DATE^BGP3UTL($O(BGPVARI(0)))_" CPT 90715"
 F BGPZ=115 S X=$$ANCONT^BGP3D31(P,BGPZ,EDATE) Q:X]""
 I X]"" Q 2_U_$$DATE^BGP3UTL($P(X,U,1))_" "_$P(X,U,2)
 ;now go to Refusals
 ...
 I R Q 2_U_$$DATE^BGP3UTL(R)_" "_"NMI Dtap"
 Q ""
DTAPTD(P,BDATE,EDATE) ;EP
 ...
 S LTD=$O(TDD(0))
 I LTD]"" S LTD=9999999-LTD Q 1_U_$$DATE^BGP3UTL(LTD)_" Imm "_TDD(9999999-LTD)
 ;now check cpt codes
 S ED=9999999-EDATE-1,BD=9999999-BDATE,G=0
 F  S ED=$O(^AUPNVSIT("AA",P,ED)) Q:ED=""!($P(ED,".")>BD)  D
 .S V=0 F  S V=$O(^AUPNVSIT("AA",P,ED,V)) Q:V'=+V  D
 ...
 I $D(BGPVARI) S Y=$O(BGPVARI(0)) Q 1_U_$$DATE^BGP3UTL($O(BGPVARI(0)))_" CPT "_BGPVARI(Y)
 K BGPG S %=P_"^LAST DX [BGP TD IZ DXS;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP3UTL($P(BGPG(1),U))_" DX V06.5"
 F BGPZ=115,9,113 S X=$$ANCONT^BGP3D31(P,BGPZ,EDATE) Q:X]""
 I X]"" Q 2_U_$$DATE^BGP3UTL($P(X,U,1))_" "_$P(X,U,2)
 ;now go to Refusals
 S B=$$DOB^AUPNPAT(P),E=EDATE,BGPNMI="",R=""
 ...
 ..Q:$P(^AUPNPREF(Y,0),U,7)'="N"
 ..S R=D    ;S:$P(^AUPNPREF(Y,0),U,7)="N" BGPNMI=1 S R=1
 I R Q 2_U_$$DATE^BGP3UTL(R)_" NMI Dtap/Td"
 Q ""
 ;
LASTTDN ;
