BGP3D5 ; IHS/CMI/LAB - measure calc ;
...
I20 ;EP
 ...
 S BGPN3=$$ALDX^BGP3D5A(DFN,BGP365,BGPEDATE)
...
DV ;EP - called from elder care
 ...
 I $P(BGPDVEX,U)=1 S BGPVALUE=BGPVALUE_"EXAM: "_$$DATE^BGP3UTL($P(BGPDVEX,U,2))_" "_$P(BGPDVEX,U,4)
 I $P(BGPDVDX,U)=1 S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":"; DX: ",1:"DX: ")_$$DATE^BGP3UTL($P(BGPDVDX,U,2))_" "_$P(BGPDVDX,U,4)
 I $P(BGPDVPED,U)=1 S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":"; PT ED: ",1:"PT ED: ")_$$DATE^BGP3UTL($P(BGPDVPED,U,2))_" "_$P(BGPDVPED,U,4)
 I $P(BGPDVCOU,U)=1 S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":"; COUNSEL ",1:"COUNSEL ")_$$DATE^BGP3UTL($P(BGPDVCOU,U,2))_" "_$P(BGPDVCOU,U,4)
 I BGPRTYPE'=1,$P(BGPDVREF,U)=1 S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":"; ",1:"")_$$DATE^BGP3UTL($P(BGPDVREF,U,2))_" "_$P(BGPDVREF,U,3)
 S BGPVALUD="" I BGPD4 S BGPVALUD="AC+BH"_"|||" D
 .I $P(BGPDVEX,U)=1 S BGPVALUD=BGPVALUD_"EXAM: "_$$DATE^BGP3UTL($P(BGPDVEX,U,2))_" "_$P(BGPDVEX,U,4)
 .I $P(BGPDVDX,U)=1 S BGPVALUD=BGPVALUD_$S($P(BGPVALUD,"|||",2)]"":"; DX: ",1:"DX: ")_$$DATE^BGP3UTL($P(BGPDVDX,U,2))_" "_$P(BGPDVDX,U,4)
 .I $P(BGPDVPED,U)=1 S BGPVALUD=BGPVALUD_$S($P(BGPVALUD,"|||",2)]"":"; PT ED: ",1:"PT ED: ")_$$DATE^BGP3UTL($P(BGPDVPED,U,2))_" "_$P(BGPDVPED,U,4)
 .I $P(BGPDVCOU,U)=1 S BGPVALUD=BGPVALUD_$S($P(BGPVALUD,"|||",2)]"":"; COUNSEL ",1:"COUNSEL ")_$$DATE^BGP3UTL($P(BGPDVCOU,U,2))_" "_$P(BGPDVCOU,U,4)
 K X,Y,Z,%,A,B,C,D,E,H,BDATE,EDATE,P,V,S,F,T
 Q
 ;
DVEX(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(1)) S BGPLDV=1_"^"_$P(BGPG(1),U)_"^IPV EXAM 34"_U_"Ex 34"_U_$$DATE^BGP3UTL($P(BGPG(1),U))
 S BGPC=0,BGPV=""
 ...
 .I X="REF" Q
 .S BGPC=BGPC+1,BGPV="1^"_(9999999-D)_"^BH IPV EXAM"_U_" BH Ex 34"_U_$$DATE^BGP3UTL((9999999-D))
 I $P(BGPLDV,U,2)<$P(BGPC,U,2) S BGPLDV=BGPC
 Q BGPLDV
REFDV(P,BDATE,EDATE) ;EP
 ;add Refusal for exam 34
 S G=$$REFUSAL^BGP3UTL1(P,9999999.15,$O(^AUTTEXAM("C",34,0)),$$FMADD^XLFDT(EDATE,-365),EDATE)
...
DVPED(P,BDATE,EDATE) ;EP
 ...
 .I $P(T,"-",1)="DV"!($P(T,"-",2)="DV") S BGPLDV=1_"^"_$P(BGPG(X),U)_"^PED "_T_U_"PED "_T_U_$$DATE^BGP3UTL($P(BGPG(X),U)) Q
 .S S=$P(T,"-",1)
 .S S=$$ICDDX^ICDCODE(S)
 .I $P(S,U,1)'="-1",$$ICD^ATXCHK($P(S,U,1),$O(^ATXAX("B","BGP IPV/DV EDUC DXS",0)),9) S BGPLDV=1_"^"_$P(BGPG(X),U)_"^PED "_T_U_"PED "_T_U_$$DATE^BGP3UTL($P(BGPG(X),U)) Q
 ;
BHPED ;
 ...
 ...S BGPLDV=1_"^"_(9999999-$P(D,"."))_"^BH PED "_T_U_"BH PED "_T_U_$$DATE^BGP3UTL((9999999-$P(D,".")))
 ..S S=$P(T,"-",1)
 ..S S=$$ICDDX^ICDCODE(S)
...
DV61(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(1)) S BGPLDV=1_"^"_$P(BGPG(1),U)_"^"_$P(BGPG(1),U,2)_U_"POV "_$P(BGPG(1),U,2)_U_$$DATE^BGP3UTL($P(BGPG(1),U))
 Q BGPLDV
ALHF(P,BDATE,EDATE) ;EP - alcohol hf or screening pov
 ...
 I $D(BGPG(1)) S BGPLAL=1_"^Ex 35^"_$$DATE^BGP3UTL($P(BGPG(1),U))_U_$P(BGPG(1),U,1)
 ;now add in v measurements
 S BGPC=$$LASTITEM^BGP3DU(P,BDATE,EDATE,"MEASUREMENT","AUDT")
 I $P(BGPLAL,U,4)<$P(BGPC,U,2) S BGPLAL=1_U_"Meas "_$P(BGPC,U,3)_U_$$DATE^BGP3UTL($P(BGPC,U,2))_U_$P(BGPC,U,2)
 S BGPC=$$LASTITEM^BGP3DU(P,BDATE,EDATE,"MEASUREMENT","AUDC")
 I $P(BGPLAL,U,4)<$P(BGPC,U,2) S BGPLAL=1_U_"Meas "_$P(BGPC,U,3)_U_$$DATE^BGP3UTL($P(BGPC,U,2))_U_$P(BGPC,U,2)
 S BGPC=$$LASTITEM^BGP3DU(P,BDATE,EDATE,"MEASUREMENT","CRFT")
 I $P(BGPLAL,U,4)<$P(BGPC,U,2) S BGPLAL=1_U_"Meas "_$P(BGPC,U,3)_U_$$DATE^BGP3UTL($P(BGPC,U,2))_U_$P(BGPC,U,2)
 ...
 I $D(BGPG(1)) I $P(BGPLAL,U,4)<$P(BGPG(1),U,1) S BGPLAL=1_"^HF CAGE 0/4^"_$$DATE^BGP3UTL($P(BGPG(1),U))_U_$P(BGPG(1),U,1)
 K BGPG S %=P_"^LAST HEALTH CAGE 1/4;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) I $P(BGPLAL,U,4)<$P(BGPG(1),U,1) S BGPLAL=1_"^HF CAGE 1/4^"_$$DATE^BGP3UTL($P(BGPG(1),U))_U_$P(BGPG(1),U,1)
 K BGPG S %=P_"^LAST HEALTH CAGE 2/4;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) I $P(BGPLAL,U,4)<$P(BGPG(1),U,1) S BGPLAL=1_"^HF CAGE 2/4^"_$$DATE^BGP3UTL($P(BGPG(1),U))_U_$P(BGPG(1),U,1)
 K BGPG S %=P_"^LAST HEALTH CAGE 3/4;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) I $P(BGPLAL,U,4)<$P(BGPG(1),U,1) S BGPLAL=1_"^HF CAGE 3/4^"_$$DATE^BGP3UTL($P(BGPG(1),U))_U_$P(BGPG(1),U,1)
 K BGPG S %=P_"^LAST HEALTH CAGE 4/4;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) I $P(BGPLAL,U,4)<$P(BGPG(1),U,1) S BGPLAL=1_"^HF CAGE 4/4^"_$$DATE^BGP3UTL($P(BGPG(1),U))_U_$P(BGPG(1),U,1)
 ;CHECK BH HF FILE
 ...
 ..Q:'T
 ..I $P($G(^AUTTHF(F,0)),U)["CAGE " S BGPC=1_U_"BH HF "_$P(^AUTTHF(F,0),U)_U_$$DATE^BGP3UTL((9999999-D))_U_(9999999-D)
 ..Q
 .S X=0 F  S X=$O(^AMHRMSR("AD",V,X)) Q:X'=+X!(BGPC)  S F=$P($G(^AMHRMSR(X,0)),U) D
 ..Q:'F
 ..Q:'$D(^AUTTMSR(F,0))
 ..S T=$P(^AUTTMSR(F,0),U,1)
 ..I T="AUDT"!(T="AUDC")!(T="CRFT") S BGPC=1_U_"BH Meas "_T_U_$$DATE^BGP3UTL((9999999-D))_U_(9999999-D)
 ...
 I $D(BGPG(1)),$P(BGPLAL,U,4)<$P(BGPG(1),U,1) S BGPLAL=1_U_"POV "_$P(BGPG(1),U,2)_U_$$DATE^BGP3UTL($P(BGPG(1),U))_U_$P(BGPG(1),U,1)
 ;now add in CPT codes
 S BGPC=$$CPT^BGP3DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP ALCOHOL SCREENING CPTS",0)),6)
 I $P(BGPLAL,U,4)<$P(BGPC,U,2) S BGPLAL=1_U_"CPT "_$P(BGPC,U,3)_U_$$DATE^BGP3UTL($P(BGPC,U,2))_U_$P(BGPC,U,2)
 ...
 ..I $E(X)="U" Q  ;don't count Refusal here
 ..I X="REF" Q
 ..S BGPC=1_U_"BH Alcohol scrn"_U_$$DATE^BGP3UTL((9999999-$P(D,".")))_U_(9999999-$P(D,"."))
 .S X=0 F  S X=$O(^AMHRPRO("AD",V,X)) Q:X'=+X!(BGPC)  S BGPP=$P($G(^AMHRPRO(X,0)),U) D
 ..Q:'BGPP
 ..S BGPP=$P($G(^AMHPROB(BGPP,0)),U)
 ..I BGPP=29.1 S BGPC=1_U_"BH POV 29.1"_U_$$DATE^BGP3UTL((9999999-$P(D,".")))_U_(9999999-$P(D,".")) Q
 ..Q
 .S T=$O(^ATXAX("B","BGP ALCOHOL SCREENING CPTS",0))
 .S X=0 F  S X=$O(^AMHRPROC("AD",V,X)) Q:X'=+X!(BGPC)  S F=$P($G(^AMHRPROC(X,0)),U) D
 ..Q:'F
 ..I '$$ICD^ATXCHK($P(+$G(^AUPNVCPT(X,0)),U,1),T,1) Q
 ..S I=$$VAL^XBDIQ1(81,F,.01)
 ..S BGPC=1_U_"BH CPT "_I_U_$$DATE^BGP3UTL((9999999-D))_U_(9999999-D)
 I $P(BGPLAL,U,4)<$P(BGPC,U,4) S BGPLAL=BGPC
 ;
 Q BGPLAL
ALPRC(P,BDATE,EDATE) ;EP
 S BGPG=$$LASTPRC^BGP3UTL1(P,"BGP ALCOHOL PROCEDURES",BDATE,EDATE)
 I BGPG Q 1_U_"Proc "_$P(BGPG,U,2)_U_$$DATE^BGP3UTL($P(BGPG,U,3))_U_$P(BGPG,U,3)
 Q ""
ALPED(P,BDATE,EDATE) ;EP
 ...
 .Q:'$D(^AUTTEDT(T,0))
 .S T=$P(^AUTTEDT(T,0),U,2)
 .I $P(T,"-",1)="CD"!($P(T,"-",2)="CD")!($P(T,"-",1)="AOD")!($P(T,"-",2)="AOD"),$P(BGPLPED,U,4)<$P(BGPG(X),U) S BGPLPED=1_U_T_U_$$DATE^BGP3UTL($P(BGPG(X),U))_U_$P(BGPG(X),U)
 .S S=$P(T,"-",1)
 .S S=$$ICDDX^ICDCODE(S)
 .I $P(S,U,1)'="-1",$$ICD^ATXCHK($P(S,U,1),$O(^ATXAX("B","BGP ALCOHOL EDUC DXS",0)),9),$P(BGPLPED,U,4)<$P(BGPG(X),U,1) S BGPLPED=1_U_""_T_U_$$DATE^BGP3UTL($P(BGPG(X),U))_U_$P(BGPG(X),U)
 .I $P(T,"-",1)="99408"!($P(T,"-",1)="99409")!($P(T,"-",1)["G0396")!($P(T,"-",1)["G0397")!($P(T,"-")["H0049")!($P(T,"-")["H0050")!($P(T,"-")["3016F"),$P(BGPLPED,U,4)<$P(BGPG(X),U) D
 ..S BGPLPED=1_U_T_U_$$DATE^BGP3UTL($P(BGPG(X),U))_U_$P(BGPG(X),U)
ALMH ;
 ...
 ..Q:'$D(^AUTTEDT(T,0))
 ..S T=$P(^AUTTEDT(T,0),U,2)
 ..I $P(T,"-",1)="CD"!($P(T,"-",2)="CD")!($P(T,"-",1)="AOD")!($P(T,"-",2)="AOD"),$P(BGPLPED,U,4)<(9999999-D) S BGPLPED=1_U_T_U_$$DATE^BGP3UTL((9999999-D))_U_(9999999-D)
 ..S S=$P(T,"-",1)
 ..S S=$$ICDDX^ICDCODE(S)
 ..I $P(S,U,1)'="-1",$$ICD^ATXCHK($P(S,U,1),$O(^ATXAX("B","BGP ALCOHOL EDUC DXS",0)),9) D
 ...S BGPLPED=1_U_T_U_$$DATE^BGP3UTL((9999999-D))_U_(9999999-D)
...
LASTHF(P,C,BDATE,EDATE) ;EP - get last factor in category C for patient P
 ...
 S D=$O(O(0))
 I D="" Q D
 Q $$VAL^XBDIQ1(9000010.23,O(D),.01)_"^"_$$DATE^BGP3UTL(9999999-D)_U_(9999999-D)
 ;
ALREF(P,BDATE,EDATE) ;EP
 ;add Refusal for exam 35
 S G=$$REFUSAL^BGP3UTL1(P,9999999.15,$O(^AUTTEXAM("C",35,0)),$$FMADD^XLFDT(EDATE,-365),EDATE)
 I $P(G,U)=1 Q 1_"^Refused Ex 35^"_$$DATE^BGP3UTL($P(G,U,2))_U_$P(G,U,2)
 Q ""
LOINC(A,B) ;
 NEW %
