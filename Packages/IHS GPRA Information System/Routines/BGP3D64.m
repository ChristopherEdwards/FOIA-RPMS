BGP3D64 ; IHS/CMI/LAB - measure 31 01 Jul 2010 7:47 PM ;
...
ICCS ;EP
 ...
 S BGPBREAS=$$DEN8^BGP3D4(DFN,BGPAGEB,BGPAGEE,BGPSEX,BGPEDATE)
 S BGPCRCS="" I BGPAGEB>49,BGPAGEB<76,'$$CRC^BGP3D62(DFN,BGPEDATE) S BGPCRCS=1
 ;if cervical cancer, did they have a pap
 S BGPPAP="" I BGPCERV S BGPPAP=$$PAP^BGP3D3(DFN,BGPEDATE,4,1)
 S BGPMAM="" I BGPBREAS S BGPMAM=$$MAM^BGP3D4(DFN,BGPEDATE,2,1)
 ...
 S BGPVALUE=$S(BGPPAP]"":"PAP: "_$$DATE^BGP3UTL($P(BGPPAP,U,2))_" "_$P(BGPPAP,U,3),1:"")
 I BGPMAM]"" S:BGPVALUE]"" BGPVALUE=BGPVALUE_"; " S BGPVALUE=BGPVALUE_$S(BGPMAM]"":"MAM: "_$$DATE^BGP3UTL($P(BGPMAM,U,2))_" "_$P(BGPMAM,U,3),1:"")
...
DEN7(P,AGEB,AGEE,SEX,EDATE) ;EP
 ...
 I AGEB<25 Q 0
 I AGEE>64 Q 0
 I $$HYSTER^BGP3D3(P,EDATE) Q 0
 Q 1
CRCS(P,BDATE,EDATE) ;
 NEW VALUE
 S VALUE=""
 S VALUE=$$SIG^BGP3D62(P,EDATE)
 I VALUE]"" Q VALUE
 S VALUE=$$COLO^BGP3D62(P,EDATE)
 I VALUE]"" Q VALUE
 S VALUE=$$FOB^BGP3D62(P,BDATE,EDATE)
 I VALUE]"" Q $P(VALUE,U,1)_"^"_$$DATE^BGP3UTL($P(VALUE,U,2))_"^"_VALUE
 Q ""
SIG(P,EDATE,BDATE) ;EP
 ...
 I $P(BGPG,U)=1 S BGPLSIG="SIG "_$P(BGPG,U,2)_"^"_$$DATE^BGP3UTL($P(BGPG,U,3))_U_$P(BGPG,U,3)
 ;
 S T=$O(^ATXAX("B","BGP SIG CPTS",0))
 I T D  I X]"",$P(BGPLSIG,U,3)<$P(X,U,1) S BGPLSIG="CPT SIG "_$P(X,U,2)_U_$$DATE^BGP3UTL($P(X,U,1))_U_$P(X,U,1)
...
COLO(P,EDATE,BDATE) ;EP
 ...
 I $P(BGPG,U)=1 S BGPLCOLO="COLO "_$P(BGPG,U,2)_"^"_$$DATE^BGP3UTL($P(BGPG,U,3))_U_$P(BGPG,U,3)
 S T=$O(^ATXAX("B","BGP COLO CPTS",0))
 I T D  I X]"",$P(BGPLCOLO,U,3)<$P(X,U,1) S BGPLCOLO="CPT COLO "_$P(X,U,2)_U_$$DATE^BGP3UTL($P(X,U,1))_U_$P(X,U,1)
 .S X=$$CPT^BGP3DU(P,BDATE,EDATE,T,5) I X]"" Q
 .S X=$$TRAN^BGP3DU(P,BDATE,EDATE,T,5)
 Q BGPLCOLO
FOB(P,BDATE,EDATE) ;EP
 ...
 I BGPLFOB="" Q ""
 Q $P(BGPLFOB,U,1)_U_$$DATE^BGP3UTL($P(BGPLFOB,U,2))
 ;
LOINC(A,B) ;
 NEW %
