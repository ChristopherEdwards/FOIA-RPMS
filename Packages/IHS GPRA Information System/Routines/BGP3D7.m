...
TA ;EP - called from elder
 ...
 S BGPTOB=$$TOBHF^BGP3D712(DFN,BGPBDATE,BGPEDATE)  ;get last HF from the 3 categories during the report period
 ...
 .I (BGPSDX]""&(%="")) S BGPN2=1,BGPVAL=$$DATE^BGP3UTL($P(BGPSDX,U,2))_" SCREEN, "_$$DATE^BGP3UTL($P(BGPSDX,U,2))_" USER" Q
 .I ($P(BGPSCPT,U)="1034F")!($P(BGPSCPT,U)="1035F")!($P(BGPSCPT,U)="G0376")!($P(BGPSCPT,U)="G0375")!($P(BGPSCPT,U)=99407) S BGPN2=1,BGPVAL=$$DATE^BGP3UTL($P(BGPSCPT,U,2))_" SCREEN, "_$$DATE^BGP3UTL($P(BGPSCPT,U,2))_" USER" Q
 .I $P(BGPSCPT,U)=99406!($P(BGPSCPT,U)="G8455")!($P(BGPSCPT,U)="G8456")!($P(BGPSCPT,U)="G8402")!($P(BGPSCPT,U)="G8453") S BGPN2=1,BGPVAL=$$DATE^BGP3UTL($P(BGPSCPT,U,2))_" SCREEN, "_$$DATE^BGP3UTL($P(BGPSCPT,U,2))_" USER" Q
 ;BGPN3 - SMOKER OR NOT
 D
 .I $P(F,U,1)["CURRENT SMOKER"!($P(F,U,1)="CESSATION-SMOKER") S BGPN3=1,BGPVAL=BGPVAL_", "_$P(F,U,2)_" SMOKER" Q
 .I BGPSDX]""&(%="") S BGPN3=1,BGPVAL=BGPVAL_", "_$$DATE^BGP3UTL($P(BGPSDX,U,2))_" SMOKER" Q
 ...
 ..S BGPN3=1,BGPVAL=BGPVAL_", "_$$DATE^BGP3UTL($P(BGPSCPT,U,2))_" SMOKER" Q
 ;BGPN4 - SMOKELESS
 D
 ...
 .I $P(BGPSCPT,U)="1035F"!($P(BGPSCPT,U)="G8456") S BGPN4=1,BGPVAL=BGPVAL_", "_$$DATE^BGP3UTL($P(BGPSCPT,U,2))_" SMOKELESS" Q
 ;BGPN5 - ETS
 ...
 .I BGPSDX]"" S BGPVAL=$$DATE^BGP3UTL($P(BGPSDX,U,2))_" SCREEN" Q
 .I BGPSCPT]"" S BGPVAL=$$DATE^BGP3UTL($P(BGPSCPT,U,2))_" SCREEN" Q
 .I BGPXPHD]"" S BGPVAL=$$DATE^BGP3UTL($P(BGPXPHD,U,2))_" SCREEN" Q
 .I BGP1320]"" S BGPVAL=$$DATE^BGP3UTL($P(BGP1320,U,2))_" SCREEN" Q
...
MA ;now check for abortion or miscarriage
 ...
 S BGPG=$$LASTPRC^BGP3UTL1(P,"BGP ABORTION PROCEDURES",BDATE,EDATE)
 I BGPG Q 0
 ...
 S %=$$CPT^BGP3DU(P,BGPD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT MISCARRIAGE",0))
 S %=$$CPT^BGP3DU(P,BGPD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT ABORTION",0))
 S %=$$TRAN^BGP3DU(P,BGPD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT MISCARRIAGE",0))
 S %=$$TRAN^BGP3DU(P,BGPD,EDATE,T,3)
 I %]"" Q 0
...
DX(P,BDATE,EDATE) ;EP
 K BGPG
 S BGPG(1)=$$LASTDX^BGP3UTL1(P,"BGP GPRA SMOKING DXS",BDATE,EDATE)
...
TOBACCO0 ;lookup in health status
 ...
 S Y=$P(^AUTTHF(Y,0),U)
 S BGPTOB=Y_"^"_$$DATE^BGP3UTL(9999999-E)_"^"_(9999999-E)
 K Y,E,X
 Q
 ;
LASTHF(P,C,BDATE,EDATE) ;EP - get last factor in category C for patient P
 ...
 S D=$O(O(0))
 I D="" Q D
 Q $$VAL^XBDIQ1(9000010.23,O(D),.01)_"^"_$$DATE^BGP3UTL(9999999-D)_"^"_(9999999-D)
 ;
LOINC(A,B) ;
 NEW %
...
CPTSM(P,BDATE,EDATE) ;EP - did pat have TOBACCO SCREENING cpt?
 NEW X
 S X=$$CPT^BGP3DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP TOBACCO SCREEN CPTS",0)),5)
