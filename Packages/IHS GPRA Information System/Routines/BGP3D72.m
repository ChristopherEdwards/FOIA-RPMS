BGP3D72 ; IHS/CMI/LAB - measure 31 ;
...
IHEDCHM ;EP
 ...
 S BGPLDL=$$LDL^BGP3D2(DFN,BGPBDATE,BGPEDATE,$S(BGPRTYPE=3:1,1:""))
 ...
 I $P(BGPLDL,U) S V=V_$$DATE^BGP3UTL($P(BGPLDL,U,2))_" LDL: "_$P(BGPLDL,U,3)
 S BGPVALUE=V
 K V,BGPAMI,BGPLDL,D
 K ^TMP($J)
 Q
...
AMIO(P,BDATE,EDATE) ;
 ...
 ;
 S BGPG=$$LASTPRC^BGP3UTL1(P,"BGP CABG PROCS",$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-61))
 I $P(BGPG,U) Q 1_U_"CABG PROC"
 ;
 S BGPG=$$CPT^BGP3DU(P,$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-61),$O(^ATXAX("B","BGP CABG CPTS",0)),6)
 I $P(BGPG,U) Q 1_U_"CABG CPT"
 S BGPG=$$TRAN^BGP3DU(P,$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-61),$O(^ATXAX("B","BGP CABG CPTS",0)),6)
 I $P(BGPG,U) Q 1_U_"CABG TRAN"
 S BGPG=$$LASTDX^BGP3UTL1(P,"BGP CABG DXS",$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-61))
 I $P(BGPG,U) Q 1_U_"CABG POV "_$P(BGPG,U,2)
 ;
 S BGPG=$$LASTPRC^BGP3UTL1(P,"BGP PCI CM PROCS",$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-61))
 I $P(BGPG,U) Q 1_U_"PCI PROC"
 ;
 S BGPG=$$CPT^BGP3DU(P,$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-61),$O(^ATXAX("B","BGP PCI CM CPTS",0)),6)
 I $P(BGPG,U) Q 1_U_"PCI CPT"
 S BGPG=$$TRAN^BGP3DU(P,$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-61),$O(^ATXAX("B","BGP PCI CM CPTS",0)),6)
 I $P(BGPG,U) Q 1_U_"PCI TRAN"
 S BGPG=$$LASTDX^BGP3UTL1(P,"BGP PCI DXS",$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-61))
 I $P(BGPG,U) Q 1_U_"PCI POV "_$P(BGPG,U,2)
 ;
 S BGPG(1)=$$LASTDX^BGP3UTL1(P,"BGP IVD DXS",BDATE,EDATE)
 S BGPG(2)=$$LASTDX^BGP3UTL1(P,"BGP IVD DXS",$$FMADD^XLFDT(BDATE,-365),BDATE)
...
BETACONT(P,BDATE,EDATE) ;EP BETA BLOCKER CONTRA
 ...
 S BGPG=$$LASTDX^BGP3UTL1(P,"BGP HYPOTENSION DXS",$$DOB^AUPNPAT(P),EDATE)
 I $P(BGPG,U)=1 Q 1_U_"hypotension POV Contra"  ;has hypotension
 S BGPG=$$LASTDX^BGP3UTL1(P,"BGP CMS 2/3 HEART BLOCK DXS",$$DOB^AUPNPAT(P),EDATE)
...
BETAPOV ;
 ...
 .I N["BETA BLOCK"!(N["BBLOCK")!(N["B BLOCK") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP3UTL($P(BGPG(X),U))_" ADR POV  "_$P(BGPG(X),U,2)_"" Q
 ...
 .I N["BETA BLOCK"!(N["BBLOCK")!(N["B BLOCK") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP3UTL($P(BGPG(X),U))_" ADR POV "_$P(BGPG(X),U,2)_"  "
 I BGPC>0 Q 1_U_BGPY(BGPC)
 ;check problem list
 S BGPC=0
 ...
 .I $$ICD^ATXCHK(I,$O(^ATXAX("B","BGP HX DRUG ALLERGY NEC",0)),9)!($$ICD^ATXCHK(I,T,9)),N["BETA BLOCK"!(N["BBLOCK")!(N["B BLOCK") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP3UTL($P(^AUPNPROB(X,0),U,8))_" ADR Problem List  "_Y_"  "
 .Q
 I BGPC>0 Q 1_U_BGPY(BGPC)
 ;allergy tracking
 S BGPC=0
 ...
 .I N["BETA BLOCK" S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP3UTL($P(^GMR(120.8,X,0),U,4))_" ADR ALLERGY TRACKING "_N
 I BGPC>0 Q 1_U_BGPY(BGPC)
 Q 0
