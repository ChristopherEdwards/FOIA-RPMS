BGP3D723 ; IHS/CMI/LAB - measure AHR.A ;
 ;;13.0;IHS CLINICAL REPORTING;;NOV 20, 2012;Build 81
 ;
 ;
ACEALG(P,BDATE,EDATE) ;EP
 K BGPG
 D ACEIALG1^BGP3C11(P,EDATE,.BGPG)
 S X=$O(BGPG(0))
 I 'X Q ""
...
ACECONT(P,BDATE,EDATE,NMIB,NMIE,RPBD) ;EP ACEI Contra
 ...
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP3UTL($P(BGPG(1),U))_" ACEI/ARB Contra POV "_$P(BGPG(1),U,2)
 ;
 ;nmi
 S BGPG=""
 ...
 ...S BGPG=1_U_$$DATE^BGP3UTL($P(^AUPNPREF(N,0),U,3))_" ACEI NMI Refusal"
 ..Q
 .Q
 I BGPG Q BGPG
 ;nmi
 ...
 ..S N=0 F  S N=$O(^AUPNPREF("AA",P,50,X,D,N)) Q:N'=+N  D
 ...Q:$P($G(^AUPNPREF(N,0)),U,7)'="N"
 ...S BGPG=1_U_$$DATE^BGP3UTL($P(^AUPNPREF(N,0),U,3))_" ARB NMI Refusal"
 ..Q
 .Q
 I BGPG Q BGPG
 ;PREGNANCY
 S X=$$PREG^BGP3D7(P,$S($G(RPBD):RPBD,1:NMIB),NMIE,1,1) I X Q 1_U_"ACEI/ARB Contra pregnant"
 ;breastfeeding
...
ACEPRIO ;now add in any before
 K BGPMEDS1
 D GETMEDS^BGP3UTL2(P,$$FMADD^XLFDT(BDATE,-365),BDATE,,,,,.BGPMEDS1)
 I '$D(BGPMEDS1) Q ""
 S X=0 F  S X=$O(BGPMEDS1(X)) Q:X'=+X  S Y=+$P(BGPMEDS1(X),U,4) D
 .Q:'$D(^AUPNVMED(Y,0))
 .Q:$$UP^XLFSTR($P($G(^AUPNVMED(Y,11)),U))["RETURNED TO STOCK"
...
ACEREF(P,BDATE,EDATE) ;EP
 ...
 ...S G=1_U_"ACEI Refusal "_$$DATE^BGP3UTL(Y)
 ..Q
 .Q
 I G Q G
 S T=$O(^ATXAX("B","BGP HEDIS ARB MEDS",0))
 ...
 ..I Y>EDATE Q
 ..S N=0 F  S N=$O(^AUPNPREF("AA",P,50,X,D,N)) Q:N'=+N!(G)  D
 ...S G=1_U_"ARB Refusal "_$$DATE^BGP3UTL(Y)
 ..Q
 .Q
 Q G
STATALG(P,BDATE,EDATE,RPB,RPE) ;EP
 ...
 .I N["STATIN"!(N["STATINS") S BGPC=1_U_"Alg Statin POV:  "_$$DATE^BGP3UTL($P(BGPG(X),U))_" "_$P(BGPG(X),U,2)_" "_N
 ...
 .I N["STATIN"!(N["STATINS") S BGPC=BGPC+1,BGPY(BGPC)=1_U_"Alg statin POV:  "_$$DATE^BGP3UTL($P(BGPG(X),U))_" "_$P(BGPG(X),U,2)_" "_N
 I BGPC Q BGPC
 ;PL
 S BGPC=0
 ...
 .I $$ICD^ATXCHK(I,$O(^ATXAX("B","BGP HX DRUG ALLERGY NEC",0)),9)!($$ICD^ATXCHK(I,T,9)),N["STATIN"!(N["STATINS") S BGPC=1_U_"alg statin PROBLEM LIST:  "_$$DATE^BGP3UTL($P(^AUPNPROB(X,0),U,8))_" "_Y_" "_N
 .Q
 I BGPC Q BGPC
 ;ART
 S BGPC=0
 ...
 .I N["STATIN" S BGPC=1_U_" alg statin ALLERGY TRACKING:  "_$$DATE^BGP3UTL($P(^GMR(120.8,X,0),U,4))_"  "_N
 I BGPC Q BGPC
 ...
 I $D(BGPG(1)) Q 1_U_"Statin allergy POV:  "_$$DATE^BGP3UTL($P(BGPG(1),U))_" "_$P(BGPG(1),U,2)_" "_$$VAL^XBDIQ1(9000010.07,+$P(BGPG(1),U,4),.04)
 ...
 ...Q:'$D(^AUPNVLAB(X,0))
 ...I BGPLT,$P(^AUPNVLAB(X,0),U),$D(^ATXLAB(BGPLT,21,"B",$P(^AUPNVLAB(X,0),U))) I $$RESCK(X) S BGPG=1_U_"adr statin creat kinase of "_$P(^AUPNVLAB(X,0),U,4)_" on "_$$DATE^BGP3UTL((9999999-D)) Q
 ...Q:'T
 ...S J=$P($G(^AUPNVLAB(X,11)),U,13) Q:J=""
 ...Q:'$$LOINC^BGP3D2(J,T)
 ...I $$RESCK(X) S BGPG=1_U_"adr statin creat kinase of "_$P(^AUPNVLAB(X,0),U,4)_" on "_$$DATE^BGP3UTL((9999999-D)) Q
 ...Q
 I BGPG Q BGPG
 ...
 ...S J=$P($G(^AUPNVLAB(X,11)),U,13) Q:J=""
 ...I '$$LOINC^BGP3D2(J,T),'$$LOINC^BGP3D2(J,T2)
...
STATREF(P,BDATE,EDATE) ;EP
 ...
 ..S N=0 F  S N=$O(^AUPNPREF("AA",P,50,X,D,N)) Q:N'=+N!(G)  D
 ...Q:$P($G(^AUPNPREF(N,0)),U,7)'="R"
 ...S G=1_U_"Statin Refusal "_$$DATE^BGP3UTL(Y)
 ..Q
 .Q
 Q G
