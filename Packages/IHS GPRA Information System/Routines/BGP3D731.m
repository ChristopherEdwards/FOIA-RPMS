BGP3D731 ; IHS/CMI/LAB - measure AHR.A ;
...
IPAMT ;EP
 ...
 K BGPBETA
 S BGPBETA("RX")=$$BETA^BGP3D721(DFN,BGPADDAT,$$FMADD^XLFDT(BGPADDAT,180),134)
 I BGPBETA("RX") S BGPN2=1  ;pt has 135 days of beta blocker
 ;I 'BGPN2 S BGPBETA("REF")=$$BETAREF^BGP3D721(DFN,$P(BGPAMI,U,2),$$FMADD^XLFDT(BGPADDAT,180)) I BGPBETA("REF") S BGPN3=1
 I 'BGPN2 S BGPBETA("CONTRA")=$$BETACONT^BGP3D721(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,$P(BGPAMI,U,2),$$FMADD^XLFDT(BGPADDAT,180)) I BGPBETA("CONTRA") S BGPN4=1 ;beta Contraindication
 I 'BGPN2 S BGPBETA("ADR")=$$BETAALG1^BGP3D72(DFN,$$FMADD^XLFDT(BGPADDAT,180)) I BGPBETA("ADR") S BGPN4=1
 I (BGPN2+BGPN4) S BGPN1=1
 K BGPASA
 S BGPASA("RX")=$$ASA^BGP3D721(DFN,BGPADDAT,$$FMADD^XLFDT(BGPADDAT,180),134)  ;get 1_U_# days
 I BGPASA("RX") S BGPN6=1  ;pt has 135 days of beta blocker
 ;I 'BGPN6 S BGPASA("REF")=$$ASAREF^BGP3D721(DFN,$P(BGPAMI,U,2),$$FMADD^XLFDT(BGPADDAT,180)) I BGPASA("REF") S BGPN7=1
 ...
 I 'BGPN6 S BGPASA("ADR")=$$ASAALLEG^BGP3D721(DFN,$$FMADD^XLFDT(BGPADDAT,180)) I BGPASA("ADR") S BGPN8=1
 I (BGPN6+BGPN8) S BGPN5=1
 K BGPACE
 S BGPACE("RX")=$$ACERX^BGP3D722(DFN,BGPADDAT,$$FMADD^XLFDT(BGPADDAT,180),134)  ;get 1_U_# days
 I BGPACE("RX") S BGPN10=1  ;pt has 135 days of ACE
 ;I 'BGPN10 S BGPACE("REF")=$$ACEREF^BGP3D722(DFN,$P(BGPAMI,U,2),$$FMADD^XLFDT(BGPADDAT,180)) I BGPACE("REF") S BGPN11=1
 I 'BGPN10 S BGPACE("CONTRA")=$$ACECONT^BGP3D722(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,$P(BGPAMI,U,2),$$FMADD^XLFDT(BGPADDAT,180),BGPBDATE) I BGPACE("CONTRA") S BGPN12=1 ;beta Contraindication
 I 'BGPN10 S BGPACE("ADR")=$$ACEALG^BGP3D722(DFN,BGPBDATE,$$FMADD^XLFDT(BGPADDAT,180)) I BGPACE("ADR") S BGPN12=1
 I (BGPN10+BGPN12) S BGPN9=1
 K BGPSTAT
 S BGPSTAT("RX")=$$STATRX^BGP3D722(DFN,BGPADDAT,$$FMADD^XLFDT(BGPADDAT,180),134)  ;get 1_U_# days
 I BGPSTAT("RX") S BGPN14=1  ;pt has 135 days of STAT
 ;I 'BGPN14 S BGPSTAT("REF")=$$STATREF^BGP3D722(DFN,$P(BGPAMI,U,2),$$FMADD^XLFDT(BGPADDAT,180)) I BGPSTAT("REF") S BGPN15=1
 I 'BGPN14 S BGPSTAT("CONTRA")=$$STATCON^BGP3D722(DFN,BGPADDAT,$$FMADD^XLFDT(BGPADDAT,180),BGPADDAT,$$FMADD^XLFDT(BGPADDAT,180)) I BGPSTAT("CONTRA") S BGPN16=1 ;beta Contraindication
 I 'BGPN14 S BGPSTAT("ADR")=$$STATALG^BGP3D722(DFN,$$DOB^AUPNPAT(DFN),$$FMADD^XLFDT(BGPADDAT,180),BGPADDAT,$$FMADD^XLFDT(BGPADDAT,180)) I BGPSTAT("ADR") S BGPN16=1
...
AMI(P,BDATE,EDATE) ;
 ...
 ..I 'H Q
 ..Q:$$AMA^BGP3D72(H)  ;ama
 ..Q:$$TRANS^BGP3D72(H)  ;transferred
 ..Q:$$EXPIRED^BGP3D72(H)  ;died
 ..S B=1
...
ASACONTR(P,BGPADM,BGPDD,BDATE,EDATE) ;does patient have an aspirin allergy
 K BGPMEDS1
 S K=0,R="",BGPG=""
 D GETMEDS^BGP3UTL2(P,$$FMADD^XLFDT(BGPADM,-90),BGPDD,,,,,.BGPMEDS1)
 I '$D(BGPMEDS1) Q ""
...
WAR71 .;
 ...
 .I E'<BGPADM,E'>BGPDD S BGPG=1_U_$$DATE^BGP3UTL(E)_U_$P(^PSDRUG(D,0),U)
 .S S=$P(^AUPNVMED(Y,0),U,7)
 .S Z=$$FMDIFF^XLFDT(BGPADM,$P($P(^AUPNVSIT(V,0),U),"."))
 .I S>Z S BGPG=1_U_$$DATE^BGP3UTL($P($P(^AUPNVSIT(V,0),U),"."))_U_" Contra "_$P(^PSDRUG(D,0),U)
 .Q
 I BGPG Q BGPG
 ...
 I X Q 1_U_$$DATE^BGP3UTL($P(X,U,2))_U_" Contra CPT G8008"
 S X=$$TRANI^BGP3DU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G8008"))
 I X Q 1_U_$$DATE^BGP3UTL($P(X,U,2))_U_" Contra CPT/TRAN G8008"
 Q ""
