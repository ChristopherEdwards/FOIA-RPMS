BGP3D81 ; IHS/CMI/LAB - measure C ; 17 Oct 2012  8:51 AM
...
DXS I '$D(^AUPNVSIT("AC",P)) Q ""
 ...
 Q ""
ASEX(P,EDATE) ;
 ;2013 - exclusion changed to be active problem with intermittent asthma
...
INHALED(P,BDATE,EDATE) ;EP - any inhaled steroid?
 ...
 ..S T=T+1
 ..I D="" S D=1_U_$$DATE^BGP3UTL($P($P(^AUPNVSIT(V,0),U),"."))_$P(^PSDRUG(Z,0),U) Q  ;it is an inhaled steroid that wasn't d/c'ed so 1 dispensing event
 ..S D=D_", "_$$DATE^BGP3UTL($P($P(^AUPNVSIT(V,0),U),"."))_" "_$P(^PSDRUG(Z,0),U)
 I T<2 Q ""
 Q D
...
SABA(P,BDATE,EDATE) ;EP - any SABA?
 ...
 K BGPMEDS1
 D GETMEDS^BGP3UTL2(P,BDATE,EDATE,"BGP PQA SABA MEDS","BGP PQA SABA NDC",,,.BGPMEDS1)
 ...
 K BGPMEDS1,^TMP($J,"A")
 D GETMEDS^BGP3UTL2(P,SD,E,"BGP PQA CONTROLLER MEDS","BGP PQA CONTROLLER NDC",,,.BGPMEDS1)
 ...
 .S V1D=$$VD^APCLV(V)
 .S C=1_U_"CONT THPY: "_$$DATE^BGP3UTL(V1D)_" "_$$VAL^XBDIQ1(9000010.14,M,.01)
 I 'C Q G
...
CHK90 ;
 ...
 S C=C+1,T(C)=$$DATE^BGP3UTL(SD)_U_$$VAL^XBDIQ1(9000010.14,M,.01)_U_"("_Q_")"
 ...
 .S C=C+1,T(C)=$$DATE^BGP3UTL(V1D)_U_$$VAL^XBDIQ1(9000010.14,M1,.01)_U_" ("_Q_")"
 .S TQ=TQ+Q
 Q:TQ<4
...
CONT(P,BDATE,EDATE) ;controller meds (at least 2)
 ...
 K BGPMEDS1,^TMP($J,"A")
 D GETMEDS^BGP3UTL2(P,BDATE,EDATE,"BGP PQA CONTROLLER MEDS","BGP PQA CONTROLLER NDC",,,.BGPMEDS1)
 S G=""
 I '$D(BGPMEDS1) Q G  ; no CONTROLLER meds
 S BGPISD=""
 ...
 .S V1D=$$VD^APCLV(V)
 .S T=T+1
 .I C="" S C=1_U_""_$$DATE^BGP3UTL(V1D)_" "_$$VAL^XBDIQ1(9000010.14,M,.01) I 1
 .E  S C=C_", "_$$DATE^BGP3UTL(V1D)_" "_$$VAL^XBDIQ1(9000010.14,M,.01)
 I T>1 Q C
 Q ""
 ;
LABA(P,BDATE,EDATE) ;EP - any inhaled steroid?
 ...
 .I $D(^ATXAX(T1,21,"B",Z))!($$NDC(Z,T4)),'$$STATDC(Y) D
 ..S T=T+1
 ..I D="" S D=1_U_"LABA: "_$P(^PSDRUG(Z,0),U)_U_$$DATE^BGP3UTL($P($P(^AUPNVSIT(V,0),U),".")) Q  ;it is an inhaled steroid that wasn't d/c'ed so 1 dispensing event
 ..S D=D_", "_$P(^PSDRUG(Z,0),U)_U_$$DATE^BGP3UTL($P($P(^AUPNVSIT(V,0),U),"."))
 I T<2 Q ""
 Q D
