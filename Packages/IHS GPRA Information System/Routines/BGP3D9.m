...
I0303 ;EP
 ...
 I $$ESRD^BGP3D211(DFN,$P(^DPT(DFN,0),U,3),BGPEDATE) S BGPSTOP=1 Q  ;esrd anytime before end date
 ...
 S BGPVALUE=$$MEANBP^BGP3D41(DFN,BGPBDATE,BGPEDATE,1)
 I BGPVALUE["unknown" S BGPVALUE=$$BPCPT^BGP3D22(DFN,BGPBDATE,BGPEDATE) I BGPVALUE]"" S BGPN1=1,BGPVALUE=$P(BGPVALUE,U,2)_U_0 G V
...
MTM(P,BDATE,EDATE) ;
 NEW %
 S %=""
 S %=$$CPT^BGP3DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP CPT MTM",0)),5)
 ...
 ..I C'="D1",C'="D2" Q
 ..S G=1_U_$$DATE^BGP3UTL($$VD^APCLV(V))_" Cl "_C
 I G]"" Q G
 Q ""
ISMT ;
 S (BGPD1,BGPN1)=0
 I 'BGPACTCL Q  ;only active clinical in this measure
 S BGPVALUE=$$LASTHF^BGP3D7(DFN,"CONFIDENCE IN MANAGING HEALTH PROBLEMS",BGPBDATE,BGPEDATE)
...
ESRD(P,EDATE) ;
 ...
 I $D(BGPG(1)) Q 1
 S E=+$$CODEN^ICPTCOD(90921),X=$$CPTI^BGP3DU(P,$$DOB^AUPNPAT(P),EDATE,E)
 I $P(X,U) Q 1
 S E=+$$CODEN^ICPTCOD(90925),X=$$CPTI^BGP3DU(P,$$DOB^AUPNPAT(P),EDATE,E)
 I $P(X,U) Q 1
 S E=+$$CODEN^ICPTCOD(90921),X=$$TRANI^BGP3DU(P,$$DOB^AUPNPAT(P),EDATE,E)
 I $P(X,U) Q 1
 S E=+$$CODEN^ICPTCOD(90925),X=$$TRANI^BGP3DU(P,$$DOB^AUPNPAT(P),EDATE,E)
 I $P(X,U) Q 1
 Q ""
FIRSTIHD(P,EDATE) ;EP
 I $G(P)="" Q ""
...
MH ;EP
 ...
 I $$ESRD^BGP3D211(DFN,$P(^DPT(DFN,0),U,3),BGPEDATE) S BGPSTOP=1 Q  ;esrd anytime before end date
 S X=$$PREG^BGP3D7(DFN,BGPBDATE,BGPEDATE,1,1) I X S BGPSTOP=1 Q
...
MHHTN(P,BDATE,EDATE) ;EP
 ...
 S X=$$LASTDX^BGP3UTL1(P,"BGP HYPERTENSION DXS",B,S)
 ...
 S X=$$LASTDX^BGP3UTL1(P,"BGP HYPERTENSION DXS",BDATE,EDATE)
 I X Q 1
 Q 0
LASTBP(P,BDATE,EDATE) ;
 NEW X
 S X=$$BPS^BGP3D2(P,BDATE,EDATE,"I",1)
 I X="" Q ""
 Q $P(X,";",1)
