BGP3DA ; IHS/CMI/LAB - IHS GPRA - report for area export ;
 ;;7.0;IHS CLINICAL REPORTING;;JAN 24, 2007
 ;
 ;
 W:$D(IOF) @IOF
 W !!,$$CTR("IHS FY03 Annual Area Clinical Performance Indicator Report for Export to Area",80)
INTRO ;
 D XIT
 W !!,"This will produce an Annual Area Clinical Performance Indicator Report for one",!,"or more indicators for a year period ending on a date you specify."
 W !,"You will be asked to provide 1) the baseline year to compare",!,"data to, and 2) the Community taxonomy to determine which patients will be",!,"included.",!
 ...
 W !,"This option will also produce an output file for export to the EISS system.",!
 D TAXCHK^BGP3TXCH
TP ;get time period
 D XIT
 S BGPRTYPE=2
 S (BGPBD,BGPED,BGPTP)=""
 S DIR(0)="S^Q:Area Dir's Quarter End (Q1 Sept 30, Q2 Dec 31, Q3 Mar 31);A:Area Director's Reporting Year End (June 30);",DIR("A")="Run Report for which time period",DIR("B")="Q" KILL DA D ^DIR KILL DIR
 I $D(DIRUT) D XIT Q
 S BGPTP=Y
 D @BGPTP
 I $D(BGPQUIT) G TP
BY ;get baseline year
 W !!,"The baseline year is defined as 2000."
 ;W !
 S BGPVDT=""
 ...
 S BGPVDT=3000000  ;set standard by Terry
...
GI ;gather all area indicators
 S X=0 F  S X=$O(^BGPINDC("AREA",X)) Q:X'=+X  S Y=0 F  S Y=$O(^BGPINDC("AREA",X,Y)) Q:Y'=+Y  S BGPIND($P(^BGPINDC(Y,0),U,1))=""
 S BGPINDT="A"
SUM ;display summary of this report
 W:$D(IOF) @IOF
 W !,$$CTR("SUMMARY OF AREA DIRECTOR'S CLINICAL PERFORMANCE REPORT TO BE GENERATED")
 W !!,"The date ranges for this report are:"
 W !?5,"Reporting Period: ",?31,$$FMTE^XLFDT(BGPBD)," to ",?31,$$FMTE^XLFDT(BGPED)
 W !?5,"Previous Year Period: ",?31,$$FMTE^XLFDT(BGPPBD)," to ",?31,$$FMTE^XLFDT(BGPPED)
 W !?5,"Baseline Period: ",?31,$$FMTE^XLFDT(BGPBBD)," to ",?31,$$FMTE^XLFDT(BGPBED)
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP3DA",ZTDTH="",ZTDESC="GPRA 03 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
OLD ;
...
Q ;which quarter
 ...
 S DIR("?",1)="Select the end date for your report:",DIR("?",2)="     1  September 30",DIR("?",3)="     2  December 31",DIR("?")="     3  March 31"
 S DIR(0)="N^1:3:0",DIR("A")="Which Area Director's Quarter End Date" KILL DA D ^DIR KILL DIR
