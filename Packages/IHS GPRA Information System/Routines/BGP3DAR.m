BGP3DAR ; IHS/CMI/LAB - ihs area GPRA ;
...
TP ;get time period
 ...
 S BGPRTYPE=1
 S (BGPBD,BGPED,BGPTP)=""
 S DIR(0)="S^Q:FY Quarter (FY Quarter 1, 2, 3 or 4);F:Full Fiscal Year (October-September)",DIR("A")="Run Report for which time period",DIR("B")="Q" KILL DA D ^DIR KILL DIR
 I $D(DIRUT) D EXIT Q
 S BGPTP=Y
 D @BGPTP
 I $D(BGPQUIT) G TP
...
ASU ;
 ...
 .K BGPSUL S BGPX=0 F  S BGPX=$O(^BGPGPDC(BGPX)) Q:BGPX'=+BGPX  S V=^BGPGPDC(BGPX,0) I $P(V,U)=BGPBD,$P(V,U,2)=BGPED,$P(V,U,7)=BGPPER,$P(V,U,12)=1,$P(V,U,5)=BGPBBD,$P(V,U,6)=BGPBED S BGPSUL(BGPX)="",BGPSUCNT=BGPSUCNT+1
 ...
 G:BGPRPTT="F" ZIS
GETSU ;
 K BGPSUL S BGPX=0 F  S BGPX=$O(^BGPGPDC(BGPX)) Q:BGPX'=+BGPX  S V=^BGPGPDC(BGPX,0) I $P(V,U)=BGPBD,$P(V,U,2)=BGPED,$P(V,U,7)=BGPPER,$P(V,U,5)=BGPBBD,$P(V,U,6)=BGPBED,$P(V,U,12)=1 S BGPSUL(BGPX)=""
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP3DAR",ZTDTH="",ZTDESC="GPRA REPORT" D ^%ZTLOAD D EXIT Q
 Q
 ;
FAC(S) ;
