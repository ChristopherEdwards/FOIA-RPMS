BGP3DCI ; IHS/CMI/LAB - IHS area GPRA 10 Dec 2006 9:12 AM ; 19 Feb 2013  11:04 AM
...
CALCIND ;EP - CALCULATE ALL MEASURES
 ...
 .I BGPRTYPE=1,$P(^BGPINDH(BGPIC,0),U,7)'=1 Q  ;national gpra report
 .I BGPRTYPE=7,$P($G(^BGPINDH(BGPIC,12)),U,1)'=1 Q  ;OTHER NATIONAL
 ...
 .K BGPNUMV
 .K ^TMP($J)
 .I $D(^BGPINDH(BGPIC,1)) X ^BGPINDH(BGPIC,1)
 .K BGPVAL,BGPG,BGPC,BGPALLED,BGPV,A,B,C,D,E,F,G,H,I,J,K,M,N,O,P,Q,R,S,T,V,W,X,Y,Z
 .K ^TMP($J)
 .;Set up global for iCare
 .I $G(BQIGREF)'="",$D(BGPSTOP) S @BQIGREF@(DFN,BGPIC)=$P(^BGPINDH(BGPIC,0),U,3)_"^N/A" Q
 .I $D(BGPSTOP) Q  ;no need to set since no num/denom
 .;loop each individual to set numerator and denominator
 .S BGPI=0 F  S BGPI=$O(^BGPINDHC("B",BGPIC,BGPI)) Q:BGPI'=+BGPI  D
 ..S (BGPNUM,BGPDEN)=0
 ..X ^BGPINDHC(BGPI,1)
 ..X ^BGPINDHC(BGPI,2) ;denominator 1 or 0
 ..;set field counter
 ..S BGPNF=$P(^BGPINDHC(BGPI,0),U,9)
 ..S BGPN=$P(^DD(90550.03,BGPNF,0),U,4)
 ..S N=$P(BGPN,";"),P=$P(BGPN,";",2)
 ..D S(BGPRPT,BGPGBL,N,P,BGPNUM),S1("N",BGPNUM)
 ..S BGPDF=$P(^BGPINDHC(BGPI,0),U,8)
 ..S BGPN=$P(^DD(90550.03,BGPDF,0),U,4),N=$P(BGPN,";"),P=$P(BGPN,";",2)
...
CPL ;comprehensive pat list check and set xtmp
 ...
 Q:BGPTIME'=1
 Q:$P($G(^BGPINDHC(BGPI,12)),U,1)=""
 X ^BGPINDHC(BGPI,3) Q:'$T
 ...
 .S ^XTMP("BGP28CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN)=F_"|||"_$P(^BGPINDHC(BGPI,12),U),BGPCPLC=BGPCPLC+1
 S $P(^XTMP("BGP28CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||")=F
 S $P(^XTMP("BGP28CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||",2)=$P(^XTMP("BGP28CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||",2)_"#"_$P(^BGPINDHC(BGPI,12),U)
 Q
NPL ;
 Q:BGPTIME'=1
 ...
 .X ^BGPNPLH(BGPX,12) K ^TMP($J) Q:'$T
 .S BGPINDL(BGPIC,BGPX)=$G(BGPINDL(BGPIC,BGPX))+1
 .I $G(BGPYNPLT) S ^XTMP("BGP3DNP",BGPJ,BGPH,"LIST",BGPORD,BGPIC,BGPX,DFN)="" Q
 .S BGPO=$S(BGPRTYPE=7:$P(^BGPNPLH(BGPX,0),U,6),1:$P(^BGPNPLH(BGPX,0),U,5))
 .S A=""
 .I $P(^BGPINDH(BGPIC,0),U,2)=151 S A=BGPAGEE G SL
 .I $P(^BGPINDH(BGPIC,0),U,2)=171 S A=BGPAGEE G SL
 .S A=BGPAGEB
SL .I $P(^BGPNPLH(BGPX,0),U,7)=9,$G(BGPVALUD)]"" S BGPVALUE=BGPVALUD
 .S ^XTMP("BGP3DNP",BGPJ,BGPH,"LIST",BGPORD,BGPIC,BGPO,BGPX,$S($P($G(^AUPNPAT(DFN,11)),U,18)]"":$P(^AUPNPAT(DFN,11),U,18),1:"UNKNOWN"),$P(^DPT(DFN,0),U,2),A,DFN)=$G(BGPVALUE)
...
S1(BQITYP,BQIVAL) ; Return data by patient for iCare into global reference BQIGREF
 ...
 S BQIIDTA=$G(^BGPINDHC(BGPI,0))
 S BQIDTA=$G(^BGPINDHC(BGPI,14))
 ;
 ...
 S $P(@BQIGREF@(DFN,BGPIC),U,1)=$P(^BGPINDH(BGPIC,0),U,3)
