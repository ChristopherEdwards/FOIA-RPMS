BGP3DL ; IHS/CMI/LAB - IHS GPRA 10 SELECTED REPORT DRIVER 21 May 2010 12:10 PM ; 05 Oct 2012  8:52 AM
 ;;13.0;IHS CLINICAL REPORTING;;NOV 20, 2012;Build 81
 ;
 ;
 W:$D(IOF) @IOF
 W !!,$$CTR("IHS 2013 CRS - Clinical Performance Measure Report (Selected Measures)",80)
INTRO ;
 D XIT
...
SETIND ;
 ...
 I $D(DIRUT) D XIT Q
 S BGPINDH=$E(Y)
 I BGPINDH="S" D SI I '$D(BGPIND) G SETIND
...
SUM ;display summary of this report
 W:$D(IOF) @IOF
 W !,$$CTR("SUMMARY OF 2013 CLINICAL MEASURE PERFORMANCE REPORT TO BE GENERATED")
 W !!,"The date ranges for this report are:"
 W !?5,"Report Period: ",?31,$$FMTE^XLFDT(BGPBD)," to ",?31,$$FMTE^XLFDT(BGPED)
 W !?5,"Previous Year Period: ",?31,$$FMTE^XLFDT(BGPPBD)," to ",?31,$$FMTE^XLFDT(BGPPED)
 W !?5,"Baseline Period: ",?31,$$FMTE^XLFDT(BGPBBD)," to ",?31,$$FMTE^XLFDT(BGPBED)
 ...
 W !!,"These performance measures will be calculated: " S X=0 F  S X=$O(BGPIND(X)) Q:X'=+X  W $P(^BGPINDH(X,0),U,3)," ; "
 W !!,"Lists will be produced for these measures: "
 S X=0 F  S X=$O(BGPLIST(X)) Q:X'=+X  W $P(^BGPINDH(X,0),U,3)," ; "
 D PT^BGP3DSL
 I BGPROT="" G BEN
ZIS ;call to XBDBQUE
 D REPORT^BGP3UTL
...
CI ;EP
 ...
 Q
AI ;EP
 S X=0 F  S X=$O(^BGPINDHC("AAST",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDHC(X,0),U,1))=""
 Q
II ;EP
 S X=0 F  S X=$O(^BGPINDHC("AIPC",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDHC(X,0),U,1))=""
 Q
PI ;EP
 S X=0 F  S X=$O(^BGPINDHC("APQA",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDHC(X,0),U,1))=""
 Q
WI ;EP
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP3DL",ZTDTH="",ZTDESC="CRS 13 COM REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP3DL",XBRX="XIT^BGP3DL",XBNS="BGP"
 D ^XBDBQUE
 Q
 ;
XIT ;
