BGP3DNE ; IHS/CMI/LAB - NATL COMP EXPORT 05 Dec 2006 7:09 PM 07 Mar 2010 2:29 PM ;
...
INTRO ;
 D XIT
 S BGPXPFYC=$O(^BGPCTRL("B","2013",0))
 S X=0 F  S X=$O(^BGPCTRL(BGPXPFYC,54,X)) Q:X'=+X  D
 .I $Y>(IOSL-3) D EOP W @IOF
 .W !,^BGPCTRL(BGPXPFYC,54,X,0)
 D TAXCHK^BGP3XTCN
TP ;
 S BGPRTYPE=1,BGPXPRP=1,BGPXPFYY=2013,BGPXPFYI=309
 S BGPXPHD=2990701,BGPXPHDT=3110630
 W !!,"The date ranges for this report are: ",$$FMTE^XLFDT(BGPXPHD)," to ",?31,$$FMTE^XLFDT(BGPXPHDT)
COMM ;
...
EXPORT ;export to area or not?
 ...
 I BGPEXPT,BGPUF="" W:'$D(ZTQUEUED) !!,"Cannot continue.....can't find export directory name. EXCEL file",!,"not written." D PAUSE^BGP3DU,XIT Q
 S BGPFN="CRSCNT"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP3UTL(BGPXPHD)_$$D^BGP3UTL(BGPXPHDT)_$$D^BGP3UTL(BGPNOW)_"_001_of_001.TXT"
LOCALF ;
 S BGPLOCAL=""
...
ZIS ;call to XBDBQUE
 ;CREATE REPORT ENTRY
 K DIC S X=$P(^VA(200,DUZ,0),U)_"-"_$$D^BGP3UTL(BGPNOW),DIC(0)="L",DIC="^BGPXPH(",DLAYGO=90550.11,DIADD=1
 S DIC("DR")=".02////"_BGPXPHD_";.03////"_BGPXPHDT_";.04////"_$P(^ATXAX(BGPTAXI,0),U)_";.05////"_$S(BGPMFITI:$P(^ATXAX(BGPMFITI,0),U),1:"")
 D ^DIC K DIC,DA,DR,DIADD,DLAYGO I Y=-1 S BGPERR="UNABLE TO CREATE REPORT FILE ENTRY!" H 4 D XIT Q
 S BGPXPRPT=+Y
 K ^BGPXPH(BGPXPRPT,13)
 S C=0,X="" F  S X=$O(BGPTAX(X)) Q:X=""  S C=C+1 S ^BGPXPH(BGPXPRPT,13,C,0)=X,^BGPXPH(BGPXPRPT,13,"B",X,C)=""
 S ^BGPXPH(BGPXPRPT,13,0)="^90550.111301A^"_C_"^"_C
 K ^BGPXPH(BGPXPRPT,14)
 I $G(BGPMFITI) S C=0,X="" F  S X=$O(^ATXAX(BGPMFITI,21,"B",X)) Q:X=""  S C=C+1,Y=$P($G(^DIC(4,X,0)),U) S ^BGPXPH(BGPXPRPT,14,C,0)=Y,^BGPXPH(BGPXPRPT,14,"B",Y,C)=""
 S ^BGPXPH(BGPXPRPT,14,0)="^90550.111401A^"_C_"^"_C
ONEF ;
 S BGPONEF=""
 K IOP,%ZIS W !! S %ZIS="PQM" D ^%ZIS
 I POP W !,"Report Aborted" S DA=BGPXPRPT,DIK="^BGPXPH(" D ^DIK K DIK D XIT Q
 I $D(IO("Q")) G TSKMN
DRIVER ;
 D PROC^BGP3DNE
 U IO
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP3DNE",ZTDTH="",ZTDESC="NATIONAL GPRA REPORT 11" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
CNTSF ;EP
 ...
 S BGPX=0,BGPTOT=0 F  S BGPX=$O(^BGPXPH(BGPXPRPT,11,BGPX)) Q:BGPX'=+BGPX  S BGPTOT=BGPTOT+1
 S BGPNF1=BGPTOT/65536
 I BGPNF1'>1,BGPEXPT G STOP ; - only 1 and it is created already
 S BGPNF=$S($P(BGPNF1,".",2)]"":BGPNF1+1,1:BGPNF1)
 S BGPNF=$P(BGPNF,".")
 S BGPX=0,BGPLX=0
 S ^BGPXPH(BGPXPRPT,12,0)="^90550.111201A^"_BGPNF_"^"_BGPNF
 F BGPZ=1:1:BGPNF D
 .S BGPFN="CRSLCNT"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP3UTL(BGPXPHD)_$$D^BGP3UTL(BGPXPHDT)_$$D^BGP3UTL(BGPNOW)_"_"_$$LZERO^BGP3UTL(BGPZ,3)_"_of_"_$$LZERO^BGP3UTL(BGPNF,3)_".TXT"
 .S ^BGPXPH(BGPXPRPT,12,BGPZ+1,0)=BGPFN_" --- "_BGPUF
 ...
 .F  S BGPX=$O(^BGPXPH(BGPXPRPT,11,BGPX)) Q:BGPX'=+BGPX!(BGPC>65535)  D
 ..W $G(^BGPXPH(BGPXPRPT,11,BGPX,0)),!
 ..S BGPC=BGPC+1
 ..S BGPLX=BGPX
 .D ^%ZISC
STOP ;
 K ^BGPXPH(BGPXPRPT,11)
 Q
CNTSF1 ;EP
 ;write out one flie only
 S BGPZ=1,BGPFN="CRSCNT"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP3UTL(BGPXPHD)_$$D^BGP3UTL(BGPXPHDT)_$$D^BGP3UTL(BGPNOW)_"_001_of_001.TXT"
 S ^BGPXPH(BGPXPRPT,12,BGPZ,0)=BGPFN_" --- "_BGPUF
 I '$D(ZTQUEUED) U IO W !?10,BGPFN
 L +^BGPDATA:10 Q:'$T
 ...
 S ^BGPDATA(BGPC)=Y
 S BGPX=0
 F  S BGPX=$O(^BGPXPH(BGPXPRPT,11,BGPX)) Q:BGPX'=+BGPX  D
 .S BGPC=BGPC+1
 .S ^BGPDATA(BGPC)=$G(^BGPXPH(BGPXPRPT,11,BGPX,0))
 ;D ^%ZISC
 S XBGL="BGPDATA"
 S XBUF=BGPUF
...
PROC1 ;process each patient
 ...
 ..Q:'$$ACTUP^BGP3D1(DFN,$$FMADD^XLFDT(BGPXXX,-(3*365)),BGPXXX,BGPTAXI,1)
 ..S BGPXPUP=1
 .Q:'BGPXPUP  ;NOT IN ANY USER POP
 .D PROCYRS
 .S X=0 F  S X=$O(BGPXPDAT(X)) Q:X'=+X  D
 ..S BGPXPHOT=BGPXPHOT+1
 ..S ^BGPXPH(BGPXPRPT,11,BGPXPHOT,0)=BGPXPDAT(X)
 .Q
 S ^BGPXPH(BGPXPRPT,11,0)="^90550.111101A^"_BGPXPHOT_"^"_BGPXPHOT
 S BGPET=$H
 D CNTSF
 Q
PROCYRS ;
...
PROCIND ;
 ...
 S $P(BGPXPPD,U,3)=$$EDT^BGP3UTL(DT)
 S $P(BGPXPPD,U,4)=$$EDT^BGP3UTL(BGPXPHD)
 S $P(BGPXPPD,U,5)=$$EDT^BGP3UTL(BGPXPHDT)
 ;S $P(BGPXPPD,U,6)=$$UID^BGP3DCHW(DFN)
 S $P(BGPXPPD,U,7)=$$EDT^BGP3UTL($P(^DPT(DFN,0),U,3))
 S $P(BGPXPPD,U,8)=$P(^DPT(DFN,0),U,2)
 ;S $P(BGPXPPD,U,9)=$$STATE^BGP3DCHW(DFN)
 ...
 .S BGPXPHV=""
 .K ^TMP($J)
 .I $D(^BGPINDH(BGPXPX,1)) X ^BGPINDH(BGPXPX,1)
 .;now get each individual measure and set value
 .S BGPXPH=0 F  S BGPXPH=$O(^BGPCTRL(BGPXPFYC,55,BGPXPMI,11,BGPXPH)) Q:BGPXPH'=+BGPXPH  D
 ..S BGPXPMID=$P(^BGPCTRL(BGPXPFYC,55,BGPXPMI,11,BGPXPH,0),U,1)
 ..I '$D(BGPXPDAT(BGPXPMID)) S BGPXPDAT(BGPXPMID)=BGPXPPD
 ..S $P(BGPXPDAT(BGPXPMID),U,11)=BGPXPMID
 ..S $P(BGPXPDAT(BGPXPMID),U,12)=$P(^BGPCTRL(BGPXPFYC,55,BGPXPMI,11,BGPXPH,0),U,3)
 ..S X=""
 ..X ^BGPCTRL(BGPXPFYC,55,BGPXPMI,11,BGPXPH,1)
...
PRINT ;EP - CALLED FROM XBDBQUE
 ...
 S BGPX=0 F  S BGPX=$O(^BGPXPH(BGPXPRPT,12,BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 .I $Y>(IOSL-3) D HEADER Q:BGPQUIT
 .W !?5,$P(^BGPXPH(BGPXPRPT,12,BGPX,0),U)
 K BGPX,BGPQUIT
 Q
HEADER ;EP
 G:'BGPGPG HEADER1
...
HEADER1 ;
 ...
 W $$CTR("*** IHS 2013 Comprehensive National GPRA Export ***",80),!
 ...
 S X="Time Period: "_$$FMTE^XLFDT(BGPXPHD)_" to "_$$FMTE^XLFDT(BGPXPHDT) W !,$$CTR(X,80),!
 W $TR($J("",80)," ","-")
 Q
