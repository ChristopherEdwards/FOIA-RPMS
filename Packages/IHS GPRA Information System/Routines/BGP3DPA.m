...
NT S BGPLIST="A"
 S BGPCPLC=0
 ;S XBRP="PRINT^BGP3DPAW",XBRC="PROC^BGP3DPA",XBRX="EOJ^BGP3DPA",XBNS="BGP"
 ;D ^XBDBQUE
 ;
 K IOP,%ZIS
 W !! S %ZIS="PQM" D ^%ZIS
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP3DPA",ZTDTH="",ZTDESC="CRS 13 SCHED REPORT" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D ^XBFMK
 K DIC,DIR,DFN
...
BDMG(BGPBD,BGPED,BGPCLN) ;EP - GUI DMS Entry Point
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BGP3DPA",ZTDESC="GUI CN GRPA APPT" D ^%ZTLOAD
 D EOJ
 Q
GUIEP ;EP
 D PROC
 K ^TMP($J,"BGP3DPA")
 S IOM=80
 D GUIR^XBLM("PRINT^BGP3DPA","^TMP($J,""BGP3DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP3DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP3DPA",X)
...
GUIECP ;EP
 K ^TMP($J,"BGP3DPA")
 S IOM=80
 D GUIR^XBLM("CPPRINT^BGP3DPA","^TMP($J,""BGP3DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP3DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP3DPA",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIH(BGPIEN,11,X,0)=BGPDATA,C=C+1
...
PROC ;EP
 ...
 S ^XTMP("BGP3DPA",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^"_"PTS WITH GPRA DATA"
 K BGPAPPT,BGPAPPTS
 S BGPTA=0
 I BGPRT1="A" D  Q
...
ZISCP ;
 ...
 ;
 I $D(IO("Q")) G TSKMNCP
 ;S XBRP="CPPRINT^BGP3DPA",XBRC="",XBRX="EOJ^BGP3DPA",XBNS="BGP"
 ;D ^XBDBQUE
 ;
...
TSKMNCP ;EP ENTRY POINT FROM TASKMAN
 ...
 I $D(IOM)#2,IOM S ZTIO=ZTIO_";"_IOM I $D(IOSL)#2,IOSL S ZTIO=ZTIO_";"_IOSL
 K ZTSAVE S ZTSAVE("BGP*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="CPPRINT^BGP3DPA",ZTDTH="",ZTDESC="CRS 13 SCHED REPORT DENOM" D ^%ZTLOAD D EOJ Q
 Q
STMP ;EP
EN1 ;EP Help
 K BGPQUIT S BGPSTMP=""
