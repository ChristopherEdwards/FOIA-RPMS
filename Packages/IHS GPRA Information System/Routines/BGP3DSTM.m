BGP3DSTM ; IHS/CMI/LAB - national patient list 20 Dec 2004 9:24 AM 07 Mar 2010 2:31 PM ;
...
TP ;get time period
 D XIT
 S BGPRTYPE=1,BGPYRPTH="",BGPNPL=1,BGPINDH="G",BGPYGPU=1,BGPYNPLT=1
SI ;
 K DIRUT
 K BGPIND,BGPSTALL
 D EN^BGP3DSI
...
SI1 ;NOW SELECT ONE OR MORE W/IN THE TOPIC
 ...
 .K BGPX S BGPO=0,X=0,BGPC=0 F  S BGPO=$O(^BGPNPLH("AN",BGPIND,BGPO)) Q:BGPO'=+BGPO!($D(DIRUT))  D
 ..S X=$O(^BGPNPLH(BGPCR,BGPIND,BGPO,0))
 ..;I BGPRTYPE=1,$P(^BGPNPLH(X,0),U,4)'="N" Q
 ..;I BGPRTYPE=7,$P(^BGPNPLH(X,0),U,4)'="O" Q
 ..S BGPX(BGPO,X)="",BGPC=BGPC+1
 .;display the choices
 .W !!!,"Please select one or more of these report choices within the",!,IORVON,$P(^BGPINDH(BGPIND,0),U,3),IORVOFF," performance measure topic.",!
 .K BGPY S X=0,BGPC=0,BGPO=0 F  S BGPO=$O(BGPX(BGPO)) Q:BGPO'=+BGPO!($D(DIRUT))  S X=0 F  S X=$O(BGPX(BGPO,X)) Q:X'=+X!($D(DIRUT))  S BGPC=BGPC+1 W !?5,BGPC,")",?9,$P(^BGPNPLH(X,0),U,3) S BGPY(BGPC)=X
 ...
 I $D(BGPQUIT) G TP
 I '$D(BGPINDL) G TP
 D RT^BGP3DSL I '$D(BGPLIST)!($D(BGPQUIT)) G SI1
...
SUM ;display summary of this report
 ...
 .W !?2,$P(^BGPNPLH(Y,0),U,3),":  ",$P(^DIBT(BGPINDL(X,Y,"TEMP"),0),U)
 D PT^BGP3DSL
 I BGPROT="" G COMM
ZIS ;call to XBDBQUE
 D REPORT^BGP3UTL
...
DRIVER ;
 ...
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP3NPL",XBRX="XIT^BGP3NPL",XBNS="BGP"
 D ^XBDBQUE
 Q
 ;
NODEV1 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP3NPL",ZTDTH="",ZTDESC="NATIONAL GPRA/GPRAMA REPORT 06" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
F ;calendar year
 S (BGPPER,BGPVDT)=""
 W !!,"Enter the Calendar Year for the report END date.  Use a 4 digit",!,"year, e.g. 2013"
 S DIR(0)="D^::EP"
 S DIR("A")="Enter Year"
...
EN1 ;EP Help
 ...
 S X=0 F  S X=$O(^BGPNPLH(BGPII,11,X)) Q:X'=+X  W !?3,^BGPNPLH(BGPII,11,X,0)
...
CT ;EP - create search templates and write message
 ...
 I BGPPTYPE="P" D HEADER^BGP3DPH
 ;I BGPPTYPEW !,$$REPEAT^XLFSTR("-",80)
 I BGPPTYPE="D" D W^BGP3DP(" ",0,2,BGPPTYPE)  ;S X=" " D S^BGP3NPLD(X,1,1) S X=" " D S^BGP3NPLD(X,1,1)
 S BGPORD=$P($G(^BGPINDH(BGPIC,12)),U,6)
 S O=0 F  S O=$O(^XTMP("BGP3DNP",BGPJ,BGPH,"LIST",O)) Q:O'=+O  D
 .S X=0 F  S X=$O(^XTMP("BGP3DNP",BGPJ,BGPH,"LIST",O,X)) Q:X'=+X  D
 ..S Y=0 F  S Y=$O(^XTMP("BGP3DNP",BGPJ,BGPH,"LIST",O,X,Y)) Q:Y'=+Y  D
 ...S T=$G(BGPINDL(X,Y,"TEMP"))
 ...I T="" Q
 ...I '$D(^DIBT(T,0)) Q
 ...L +^DIBT(T):10 I '$T Q
 ...S C=0
 ...S P=0 F  S P=$O(^XTMP("BGP3DNP",BGPJ,BGPH,"LIST",O,X,Y,P)) Q:P'=+P  D
 ....S C=C+1
 ....S ^DIBT(T,1,P)=""
 ....Q
 ...L -^DIBT(T)
 ...D W^BGP3DP("Search template: "_$P(^DIBT(T,0),U)_" created with "_C_" members.",0,2,BGPPTYPE)
 D W^BGP3DP("",0,1,BGPPTYPE)
 Q
