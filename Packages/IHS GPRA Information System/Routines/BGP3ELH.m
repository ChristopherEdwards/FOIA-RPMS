BGP3ELH ; IHS/CMI/LAB - cover page for ELDER 25 Jun 2010 10:20 AM ;
 ...
 ;W:$D(IOF) @IOF
 W !!,$$CTR("Cover Page",80)
 W !!,$$CTR("*** IHS 2013 Elder Care Report ***",80)
 ...
 D ENDTIME
 D ELDERHDR
 I $G(BGPEXPT) W !!,"A file will be created called BG130",$P(^AUTTLOC(DUZ(2),0),U,10)_".EL"_BGPRPT,".",!,"It will reside in the public/export directory.",!,"This file should be sent to your Area Office.",!
...
AREACP ;EP - area cover page
 ...
 W !,"Population: ",$S(BGPBEN=1:"AI/AN Only (Classification 01)",BGPBEN=2:"non AI/AN Only (Classification NOT 01)",BGPBEN=3:"All (Both AI/AN and non AI/AN)",1:"")
 D ENDTIME
 S BGPCHSO="",X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  I $P(^BGPELDCH(X,0),U,17) S BGPCHSO=1
 S BGPCHSN="",X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  I '$P(^BGPELDCH(X,0),U,17) S BGPCHSN=1
 S BGPURBO="",X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  I $P(^BGPELDCH(X,0),U,19) S BGPURBO=1
 S BGPURBN="",X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  I '$P(^BGPELDCH(X,0),U,19) S BGPURBN=1
 D ELDERHDR
 ;I $G(BGPEXPT) W !!,"A file will be created called BG130",$P(^AUTTLOC(DUZ(2),0),U,10)_".EL"_BGPRPT,".",!,"It will reside in the public/export directory.",!,"This file should be sent to your Area Office.",!
 I BGPROT'="P",'$D(BGPGUI) W !!,"A delimited output file called ",BGPDELF,!,"has been placed in the "_$$GETDEDIR^BGP3UTL2()_" directory for your use in Excel or some",!,"other software package.",!,"See your site manager to access this file.",!
 ...
 .S BGPC=BGPC+1
 .S X=$P(^BGPELDCH(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .W !?3,BGPC,".  ",$S($P(^BGPELDCH(BGPX,0),U,17):"*",$P(^BGPELDCH(BGPX,0),U,19):"**",1:""),X
 W !!?1,"The following communities are included in this report:"
 NEW BGPX
 S BGPX="",BGPC=0 F  S BGPX=$O(BGPSUL(BGPX)) Q:BGPX=""  D
 .S BGPC=BGPC+1
 .S X=$P(^BGPELDCH(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .W !!?3,BGPC,".  ",$S($P(^BGPELDCH(BGPX,0),U,17):"*",1:""),X
 .;W !?5,"Communities: " 
 .W !?3,"Community Taxonomy Name: ",$P(^BGPELDCH(BGPX,0),U,18)
 .;W !?5,"Communities: " 
 .S BGPXX=0,BGPXN=0,BGPXY="" F  S BGPXX=$O(^BGPELDCH(BGPX,9999,BGPXX)) Q:BGPXX'=+BGPXX  S BGPXN=BGPXN+1,BGPXY=BGPXY_$S(BGPXN=1:"",1:";")_$P(^BGPELDCH(BGPX,9999,BGPXX,0),U)
 .S BGPX1=0,C=0 F BGPX1=1:3:BGPXN D
 ..W !?10,$E($P(BGPXY,";",BGPX1),1,20),?30,$E($P(BGPXY,";",(BGPX1+1)),1,20),?60,$E($P(BGPXY,";",(BGPX1+2)),1,20)
 .I $O(^BGPELDCH(BGPX,1111,0)) D
 ..W !!?5,"MFI Visit Locations: " S BGPXX=0,BGPXN=0,BGPXY="" F  S BGPXX=$O(^BGPELDCH(BGPX,1111,BGPXX)) Q:BGPXX'=+BGPXX  S BGPXN=BGPXN+1,BGPXY=BGPXY_$S(BGPXN=1:"",1:";")_$P(^BGPELDCH(BGPX,1111,BGPXX,0),U)
 ...
 .W !,"* CHS-only site.  Uses Active Clinical CHS Population definition vs. Active Clinical." ;D W^BGP3DP(X,0,1,BGPPTYPE)
 I BGPURBO D
 .;S X=" " D W^BGP3DP(X,0,1,BGPPTYPE)
 .W !,"**Urban Outreach & Referral-only site.  Uses Active Clinical Behavioral Health",!,"Population definition vs. Active Clinical." ;D W^BGP3DP(X,0,1,BGPPTYPE)
 W ! ;S X=" " D W^BGP3DP(X,0,1,BGPPTYPE)
 K BGPX,BGPQUIT
 Q
