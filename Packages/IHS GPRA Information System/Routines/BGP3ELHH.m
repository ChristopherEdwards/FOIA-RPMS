BGP3ELHH ; IHS/CMI/LAB - cover page for ELDER 25 Jun 2010 10:20 AM ;
 ...
 S X="*** IHS 2013 Elder Care Report ***" D S(X,1,1)
 ...
 S X=" " D S(X,1,1)
 I $G(BGPEXPT) D
 .S X="A file will be created called BG130"_$P(^AUTTLOC(DUZ(2),0),U,10)_".EL"_BGPRPT_"." D S(X,1,1) S X="It will reside in the public/export directory.  This file should be sent to your Area Office." D S(X,1,1)
 I BGPROT'="P",'$D(BGPGUI) D
 .S X="A delimited output file called "_BGPDELF D S(X,1,1) S X="has been placed in the "_$$GETDEDIR^BGP3UTL2()_" directory for your use in Excel or some other software package." D
...
AREACP ;EP - area cover page
 ...
 S X=" " D S(X,1,1)
 I BGPROT'="P",'$D(BGPGUI) D
 .S X="A delimited output file called "_BGPDELF D S(X,1,1) S X="has been placed in the "_$$GETDEDIR^BGP3UTL2()_" directory for your use in Excel or some" D S(X,1,1) D
 ...
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPELDCH(BGPX,0),U,17):"*",1:"")_X D S(X,1,1)
 .Q
 S X=" " D S(X,1,1)
 ...
 .S X="Community Taxonomy Name: "_$P(^BGPELDCH(BGPX,0),U,18) D S(X,1,1)
 .;W !?5,"Communities: " 
 .S X=0,N=0,Y="",Z="" F  S X=$O(^BGPELDCH(BGPX,9999,X)) Q:X'=+X  S N=N+1,Y=Y_$S(N=1:"",1:";")_$P(^BGPELDCH(BGPX,9999,X,0),U)
 .S X=0,C=0 F X=1:3:N S Z=$E($P(Y,";",X),1,20),$P(Z,U,2)=$E($P(Y,";",(X+1)),1,20),$P(Z,U,3)=$E($P(Y,";",(X+2)),1,20) D S(Z,1,1)
 .I $O(^BGPELDCH(BGPX,1111,0))  D
 ..S X=" " D S(X,1,1)
 ..S X="MFI Site: Locations for visits: " D S(X,1,1) S X=0,N=0,Y="",Z="" F  S X=$O(^BGPELDCH(BGPX,1111,X)) Q:X'=+X  S N=N+1,Y=Y_$S(N=1:"",1:";")_$P(^BGPELDCH(BGPX,1111,X,0),U)
 ..S X=0,C=0 F X=1:3:N S Z=$E($P(Y,";",X),1,20),$P(Z,U,2)=$E($P(Y,";",(X+1)),1,20),$P(Z,U,3)=$E($P(Y,";",(X+2)),1,20) D S(Z,1,1)
 ..Q
 .S X=" " D S(X,1,1)
 .Q
