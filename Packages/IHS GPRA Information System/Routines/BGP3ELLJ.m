BGP3ELLJ ; IHS/CMI/LAB - print ind ;
...
I1AGEP ;
 ...
 F X=1:1:4 S V=$P(BGPDAC(X),U,15) S Y=$$SB^BGP3ELL2($J(V,6,1)) D S(Y,,X+1)
PR ; 
 ...
 S X="% D. # w/ LDL >160 w/ % of Total LDL Screen" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,15) S Y=$$SB^BGP3ELL2($J(V,6,1)) D S(Y,,X+1)
 ...
 S X="D. # w/ LDL >160 w/ % of Total LDL Screen" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,15),O=$P(BGPDAP(X),U,15) S Y=$$SB^BGP3ELL2($J((N-O),6,1)) D S(Y,,X+1)
BL ;
 D BL^BGP3ELLV
 Q
V(T,R,N,P) ;
