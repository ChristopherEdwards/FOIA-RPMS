BGP3ELLU ; IHS/CMI/LAB - print ind ;
...
BL ; 
 ...
 F X=1:1:3 S V=$P(BGPDAB(X),U,15) S Y=$$SB^BGP3ELL2($J(V,6,1)) D S(Y,,X+1)
 ...
 F X=1:1:3 S V=$P(BGPDAB(X),U,17) S Y=$$SB^BGP3ELL2($J(V,6,1)) D S(Y,,X+1)
 ...
 F X=1:1:3 S V=$P(BGPDAB(X),U,19) S Y=$$SB^BGP3ELL2($J(V,6,1)) D S(Y,,X+1)
 ...
 F X=1:1:3 S V=$P(BGPDAB(X),U,41) S Y=$$SB^BGP3ELL2($J(V,6,1)) D S(Y,,X+1)
 ;% changes
...
PPC ;
 S X=BGP1 D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,BGPPIE),O=$P(BGPDAB(X),U,BGPPIE) S Y=$$SB^BGP3ELL2($J((N-O),6,1)) D S(Y,,X+1)
 Q
C(X,X2,X3) ;
 D COMMA^%DTC
 Q X
