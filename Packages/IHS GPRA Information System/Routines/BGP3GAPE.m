BGP3GAPE ; IHS/CMI/LAB - AREA NATIONAL GPRA REPORT ;
...
TESTNTL ;
 ...
 S BGPSUL(1)=""
 S BGPSUL(2)=""
 D EP(.ERR,1,2522,"BGP 13 AREA PAT ED RPT","A",.BGPSUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRPTT,BGPSUL,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPROT,BGPRTIME,BGPFILE,BGPBEGIN) ;EP - called from GUI to produce national gpra report (AO-AGP)
 ...
 ;            from BGP 13 ELDER DATA CURRENT that match the following:
 ;            (SEE BGP3ASL or CALL ME)
 ;  BGPSUL  - array containing Iens from BGP 13 ELDER  DATA CURRENT that the user selected
...
EP1 ;
 ...
 S BGPSUCNT=C
 S BGPSUNM=""
 I BGPSUCNT=1 S Y=$O(BGPSUL(0)),X=$P(^BGPPEDCH(Y,0),U,9) I X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 ...
 S X=0 F  S X=$O(^BGPPEIH(X)) Q:X'=+X  S BGPIND(X)=""
 S BGPINDH="H"
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOELD^BGP3GAPE",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI AREA PAT ED REPORT" D ^%ZTLOAD Q
 Q
AOELD ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 ;cmi/anch/maw added 5/12/2009 for word output
 D GUIR^BGPXBLM("PRINT^BGP3PPEL","^TMP($J,""BGPGUI"",")
