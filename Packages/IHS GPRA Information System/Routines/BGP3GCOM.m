BGP3GCOM ; IHS/CMI/LAB - GUI COM REPORT ;
...
TESTNTL ;
 ...
 S BGPND(4)="",BGPND(3)=""
 S BGPLIST(3)=""
 D EP(.ERR,1,2522,"BGP 13 SELECTED IND REPORT",338,"S",.BGPND,1,3040000,3000000,1,"A",.BGPLIST,"","","B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPINDH,BGPIND,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPLSTT,BGPLIST,BGPLPRV,BGPLPROV,BGPROT,BGPRTIME,BGPMFITI,BGPFILE) ;EP - called from GUI to produce COM REPORT CI05-RPT-LOC-COM
 ; SEE ROUTINE BGP3DL if you have questions about any of these variables
 ;  BGPUSER - DUZ
 ;  BGPDUZ2 - DUZ(2)
 ;  BGPOPTN - OPTION NAME
 ;  BGPTAXI - IEN OF COMMUNITY TAXONOMY NAME
 ;  BGPINDH - $E (1st character) of the answer to the following DIR call:
 ...
 ;
 ;  BGPIND - note:  THIS ARRAY IS ONLY REQUIRED IF BGPINDH="S"
...
EP1 ;
 ...
 I $G(BGPINDH)="" S BGPRET=0_"^INDICATOR TYPE NOT PASSED" Q
 I "DCWESIPA"'[BGPINDH S BGPRET=0_"^INDICATOR TYPE INVALID" Q
 I BGPINDH="S",'$D(BGPIND) S BGPRET=0_"^INDICATOR ARRAY NOT PASSED" Q
 I BGPINDH="S",'$O(BGPIND(0)) S BGPRET=0_"^INDICATOR ARRAY NOT PASSED" Q
 ...
 I BGPINDH="D" K BGPIND D DI
 I BGPINDH="C" K BGPIND D CI
 I BGPINDH="W" K BGPIND D WI
 I BGPINDH="E" K BGPIND D EI
 I BGPINDH="I" K BGPIND D II^BGP3DL
 I BGPINDH="P" K BGPIND D PI^BGP3DL
 I BGPINDH="A" K BGPIND D AI^BGP3DL
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LOCCOM^BGP3GCOM",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT COM 05" D ^%ZTLOAD Q
 Q
LOCCOM ;
 D ^BGP3D1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^BGPXBLM("^BGP3DP","^TMP($J,""BGPGUI"",")
...
XIT ;
 ...
 Q
DI ;
 S X=0 F  S X=$O(^BGPINDHC("ADM",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDHC(X,0),U,1))=""
 Q
CI ;
 S X=0 F  S X=$O(^BGPINDHC("ACARD",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDHC(X,0),U,1))=""
 Q
WI ;
 S X=0 F  S X=$O(^BGPINDHC("AWH",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDHC(X,0),U,1))=""
 Q
EI ;
 S X=0 F  S X=$O(^BGPINDHC("AEL",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDHC(X,0),U,1))=""
 Q
 ;
