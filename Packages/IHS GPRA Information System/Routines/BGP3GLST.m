BGP3GLST ; IHS/CMI/LAB - GUI CMS REPORT ;
...
EP1 ;
 ...
 S DIC="^BGPGUIH(",DIC(0)="L",DIADD=1,DLAYGO=90549.19,DIC("DR")=".02////"_BGPUSER_";.03////"_$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT)_";.05///"_BGPOPTN_";.06///R"
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOLST^BGP3GLST",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI 06 UPLOAD FILE LIST" D ^%ZTLOAD Q
 Q
AOLST ;
 K BGPLIST S BGPLIST="",X=$$LIST^%ZISH(BGPDIR,"BG130*",.BGPLIST)
 I $O(BGPLIST(""))="" S ^BGPGUIH(BGPGIEN,11,C,0)="There are no files in that directory." D ENDLOG,XIT Q
 S X=0,C=0 F  S X=$O(BGPLIST(X)) Q:X'=+X  S C=C+1,^BGPGUIH(BGPGIEN,11,C,0)=BGPLIST(X)
