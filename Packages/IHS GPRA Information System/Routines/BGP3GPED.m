BGP3GPED ; IHS/CMI/LAB - GUI COM REPORT ;
...
TESTELD ;
 ...
 S BGPND(4)="",BGPND(3)=""
 S BGPLIST(3)=""
 D EP(.ERR,1,2522,"BGP 13 PAT ED REPORT",338,"S",.BGPND,1,3040000,3000000,1,"A",.BGPLIST,"","",1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPZZ,BGPIND,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPLSTT,BGPLIST,BGPLPRV,BGPLPROV,BGPEXPT,BGPROT,BGPRTIME,BGPMFITI,BGPFILE,BGPSEAT,BGPBEGIN) ;EP - called from GUI to produce COM REPORT CI05-RPT-LOC-COM
 ; SEE ROUTINE BGP3DEL if you have questions about any of these variables
 ; TAXONOMY CHECK IS IN ^BGP3XTEL
 ;  BGPUSER - DUZ
 ;  BGPDUZ2 - DUZ(2)
 ;  BGPOPTN - OPTION NAME
 ...
 ;                        1 and 6 from the BGP 13 ELDER INDICATORS file (^BGPELIH GLOBAL).  When
...
EP1 ;
 ...
 I BGPZZ="A" S X=0 F  S X=$O(^BGPPEIH(X)) Q:X'=+X  S BGPIND(X)=""
 I BGPZZ='"A" S BGPEXPT=""
...
BY ;get baseline year
 ...
 S BGPFILEN=""
 I BGPEXPT S BGPFILEN="BG130"_$P(^AUTTLOC(DUZ(2),0),U,10)_".PED"_BGPRPT_" in directory "_BGPUF
 ;create entry in GUI file
 D ^XBFMK
 S X=BGPFILE
 S BGPGFNM=X
 S DIC="^BGPGUIH(",DIC(0)="L",DIADD=1,DLAYGO=90549.19,DIC("DR")=".02////"_BGPUSER_";.03////"_$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT)_";.05///"_BGPOPTN_";.06///R;.07///"_$G(BGPROT)_";1///"_BGPFILEN
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="OTHPED^BGP3GPED",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI PAT ED REPORT" D ^%ZTLOAD Q
 Q
OTHPED ;
 D ^BGP3DPE1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^BGPXBLM("^BGP3DPEP","^TMP($J,""BGPGUI"",")
