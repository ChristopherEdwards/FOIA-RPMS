BGP3GPP ; IHS/CMI/LAB - GUI PP REPORT ;
...
TESTNTL ;
 ...
 S BGPND(4)="",BGPND(3)=""
 S BGPLIST(3)=""
 D EP(.ERR,1,2522,"BGP 13 SEL PATIENT PANEL",338,"D",.BGPND,1,3040000,3000000,1801,"A",.BGPLIST,"","","B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPINDH,BGPIND,BGPQTR,BGPPER,BGPVDT,BGPSEAT,BGPLSTT,BGPLIST,BGPLPRV,BGPLPROV,BGPROT,BGPRTIME,BGPFILE) ;EP - called from GUI to produce PP REPORT CI05-RPT-LOC-PP
 ...
 ;           so if BGPINDH="S" pass this array
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LOCPP^BGP3GPP",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT COM 05" D ^%ZTLOAD Q
 Q
LOCPP ;
 D ^BGP3D10
 K ^TMP($J,"BGPGUI")
