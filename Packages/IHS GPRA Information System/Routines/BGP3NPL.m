BGP3NPL ; IHS/CMI/LAB - national patient list 20 Dec 2004 9:24 AM 30 Jun 2010 5:21 PM ;
...
TP ;get time period
 D XIT
 S BGPRTYPE=1,BGPYRPTH="",BGPNPL=1,BGPINDH="G",BGPYGPU=1
SI ;
 K DIRUT
 K BGPIND
 D EN^BGP3DSI
...
SI1 ;NOW SELECT ONE OR MORE W/IN THE TOPIC
 ...
 K BGPINDO
 S BGPIND=0 F  S BGPIND=$O(BGPIND(BGPIND)) Q:BGPIND'=+BGPIND  S BGPINDO($P(^BGPINDH(BGPIND,12),U,6),BGPIND)=""
 ...
 .K BGPX S BGPO=0,X=0,BGPC=0 F  S BGPO=$O(^BGPNPLH(BGPCR,BGPIND,BGPO)) Q:BGPO'=+BGPO!($D(DIRUT))  D
 ...
 ;get report type
 I $D(DIRUT) G SI
 K BGPQUIT D RT^BGP3DSL I '$D(BGPLIST)!($D(BGPQUIT)) G SI
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP3NPL",ZTDTH="",ZTDESC="NATIONAL GPRA REPORT 06" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
TPNON ;get time period
 D XIT
 S BGPRTYPE=7,BGPYRPTH="",BGPNPL=1,BGPINDH="G",BGPYGPU=1,BGPONMR=1,BGPRTC="U"
 G SI
