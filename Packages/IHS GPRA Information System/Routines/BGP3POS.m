BGP3POS ; IHS/CMI/LAB - NO DESCRIPTION PROVIDED 08 Dec 2010 3:10 PM ;
 ...
 ;I $E($$VERSION^XPDUTL("BGP"),1,4)<"12.1" D MES^XPDUTL($$CJ^XLFSTR("Version 12.1 of BGP is required.  Not installed.",80)) D SORRY(2) I 1
 I '$$INSTALLD("BGP*12.1*1") D SORRY(2)
 I '$$INSTALLD("BQI*2.3*1") D SORRY(2)
 Q
 ;
PRE ;EP
 D PRE^BGP3POS2
...
ADA ;
 ...
 ;prenatal
 S BGPDA=0 S BGPDA=$O(^ATXAX("B","BGP PRENATAL DENTAL ADA CODES",BGPDA))
 I BGPDA S DIK="^ATXAX(",DA=BGPDA D ^DIK  ;get rid of existing one
 W !,"Creating/Updating PRENATAL DENTAL EXAM ADA Codes Taxonomy..."
 S X="BGP PRENATAL DENTAL ADA CODES",DIC="^ATXAX(",DIC(0)="L",DIADD=1,DLAYGO=9002226 D ^DIC K DIC,DA,DIADD,DLAYGO,I
 I Y=-1 W !!,"ERROR IN CREATING PRENATAL DENTAL EXAM ADA CODES TAX" Q
 S BGPTX=+Y,$P(^ATXAX(BGPTX,0),U,2)="BGP PRENATAL DENTAL ADA CODES",$P(^(0),U,5)=DUZ,$P(^(0),U,8)=0,$P(^(0),U,9)=DT,$P(^(0),U,12)=174,$P(^(0),U,13)=0,$P(^(0),U,15)=9999999.31,^ATXAX(BGPTX,21,0)="^9002226.02101A^0^0"
 S BGPX=0
 F X=9340,9341 S DIC="^AUTTADA(",DIC(0)="M" D ^DIC K DIC,DA,DR,DIADD,DLAYGO,DQ,DI,D1,D0 I $P(Y,U)>0 D
 .S BGPX=BGPX+1
 .S ^ATXAX(BGPTX,21,BGPX,0)=+Y,$P(^ATXAX(BGPTX,21,0),U,3)=BGPX,$P(^(0),U,4)=BGPX,^ATXAX(BGPTX,21,"AA",+Y,BGPX)=""
 .Q
 S DA=BGPTX,DIK="^ATXAX(" D IX1^DIK
