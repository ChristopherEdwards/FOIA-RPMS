BGP3SDPD ; IHS/CMI/LAB - IHS summary page ;
...
START ;
 ...
 ..I BGPPTYPE="P" D
 ...D W^BGP3DP($P(^BGPINDHC(BGPPC,22),U,4),0,2,BGPPTYPE,1,1)
 ...
 ...D W^BGP3DP(F_$S($P(^BGPINDHC(BGPPC,0),U,4)["014.A"!($P(^BGPINDHC(BGPPC,0),U,4)["023.")!($P(^BGPINDHC(BGPPC,0),U,4)="016.A.1")!($P(^BGPINDHC(BGPPC,0),U,4)="016.A.5")!($P($G(^BGPINDHC(BGPPC,19)),U,13)):"",1:"%"),0,0,BGPPTYPE,5,50)
 ...D W^BGP3DP($P(^BGPINDHC(BGPPC,22),U,2),0,0,BGPPTYPE,6,60)  ;,?65,$P(^BGPINDHC(BGPPC,22),U,3)
 ...I $P(^BGPINDHC(BGPPC,22),U,9)]""!($P(^BGPINDHC(BGPPC,22),U,10)]"")!($P(^BGPINDHC(BGPPC,22),U,11)]"") D W^BGP3DP($TR($P(^BGPINDHC(BGPPC,22),U,10),"$","^"),0,1,BGPPTYPE,6,60)
 ...
 ....I $P(^BGPINDHC(BGPPC,0),U,4)["014.A"!($P(^BGPINDHC(BGPPC,0),U,4)["023.")!($P(^BGPINDHC(BGPPC,0),U,4)="016.A.1")!($P($G(^BGPINDHC(BGPPC,19)),U,13)) D  I 1
 .....I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 .....D W^BGP3DP(BGPSASU,0,1,BGPPTYPE,1,2)
 .....D W^BGP3DP($E(BGPSNAM,1,12),0,0,BGPPTYPE,2,8)
 .....D W^BGP3DP($J($P(^TMP($J,"SUMMARY DETAIL PAGE DEVEL",BGPC,BGPO,BGPPC,BGPSN),U),7,0),0,0,BGPPTYPE,2,20)
 .....D W^BGP3DP($J($P(^TMP($J,"SUMMARY DETAIL PAGE DEVEL",BGPC,BGPO,BGPPC,BGPSN),U,2),7,0),0,0,BGPPTYPE,3,29)
 .....D W^BGP3DP($J($P(^TMP($J,"SUMMARY DETAIL PAGE DEVEL",BGPC,BGPO,BGPPC,BGPSN),U,3),7,0),0,0,BGPPTYPE,4,38)
 ....E  D
 .....I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 .....D W^BGP3DP(BGPSASU,0,1,BGPPTYPE,1,2)
 .....D W^BGP3DP($E(BGPSNAM,1,12),0,0,BGPPTYPE,2,8)
 .....D W^BGP3DP($J($P(^TMP($J,"SUMMARY DETAIL PAGE DEVEL",BGPC,BGPO,BGPPC,BGPSN),U),7,1)_"%",0,0,BGPPTYPE,2,20)
 .....D W^BGP3DP($J($P(^TMP($J,"SUMMARY DETAIL PAGE DEVEL",BGPC,BGPO,BGPPC,BGPSN),U,2),7,1)_"%",0,0,BGPPTYPE,3,29)
 .....D W^BGP3DP($J($P(^TMP($J,"SUMMARY DETAIL PAGE DEVEL",BGPC,BGPO,BGPPC,BGPSN),U,3),7,1)_"%",0,0,BGPPTYPE,4,38)
 ..I BGPPTYPE="D" D
 ...D W^BGP3DP("",0,1,BGPPTYPE)
 ...S XX=" "_$P(^BGPINDHC(BGPPC,22),U,4)
 ...I $P(^BGPINDHC(BGPPC,22),U,7)]"" D W^BGP3DP(XX,0,1,BGPPTYPE,1) S XX=" "_$P(^BGPINDHC(BGPPC,22),U,7)
 ...I $P(^BGPINDHC(BGPPC,22),U,12)]"" D W^BGP3DP(XX,0,1,BGPPTYPE,1) S XX=" "_$P(^BGPINDHC(BGPPC,22),U,12)
...
HEADER ;EP
 ...
 Q
H1 ;
 S X="GPRA DEVELOPMENTAL MEASURES CLINICAL PERFORMANCE DETAIL" D W^BGP3DP(X,1,1,BGPPTYPE)
 D W^BGP3DP("Site",0,1,BGPPTYPE,2,21),W^BGP3DP("Site",0,0,BGPPTYPE,3,32),W^BGP3DP("Site",0,0,BGPPTYPE,4,40),W^BGP3DP("Area",0,0,BGPPTYPE,5,50) ;,W^BGP3DP("Nat'l",0,0,BGPPTYPE,6,60)
 D W^BGP3DP("Current",0,1,BGPPTYPE,2,22),W^BGP3DP("Prev",0,0,BGPPTYPE,3,32),W^BGP3DP("Base",0,0,BGPPTYPE,4,40),W^BGP3DP("Current",0,0,BGPPTYPE,5,50) ;W^BGP3DP("2012",0,0,BGPPTYPE,6,60)
...
PART ;
 ...
 ..I $Y>(BGPIOSL-3) D HEADERP Q:BGPQUIT
 ..W !!?1,$P(^BGPINDHC(BGPPC,22),U,4)
 ..I $P(^BGPINDHC(BGPPC,22),U,7)]"" W !,$P(^BGPINDHC(BGPPC,22),U,7)
 ..I $P(^BGPINDHC(BGPPC,22),U,12)]"" W !,$P(^BGPINDHC(BGPPC,22),U,12)
 ..S F=$O(^TMP($J,"SUMMARY DETAIL PAGE DEVEL",BGPC,BGPO,BGPPC,0))
 ..S F=$P(^TMP($J,"SUMMARY DETAIL PAGE DEVEL",BGPC,BGPO,BGPPC,F),U,4)
 ..W ?50,F,$S($P(^BGPINDHC(BGPPC,0),U,4)["014.A"!($P(^BGPINDHC(BGPPC,0),U,4)["023.")!($P(^BGPINDHC(BGPPC,0),U,4)="016.A.1"):"",1:"%"),?60,$P(^BGPINDHC(BGPPC,22),U,P1),?65,$P(^BGPINDHC(BGPPC,22),U,2),?74,$P(^BGPINDHC(BGPPC,22),U,3)
 ..I $P(^BGPINDHC(BGPPC,22),U,9)]""!($P(^BGPINDHC(BGPPC,22),U,10)]"")!($P(^BGPINDHC(BGPPC,22),U,11)]"") W !?60,$TR($P(^BGPINDHC(BGPPC,22),U,9),"$","^"),?64,$TR($P(^BGPINDHC(BGPPC,22),U,10),"$","^"),?73,$P(^BGPINDHC(BGPPC,22),U,11)
 ..S BGPSN=0 F  S BGPSN=$O(^TMP($J,"SUMMARY DETAIL PAGE DEVEL",BGPC,BGPO,BGPPC,BGPSN)) Q:BGPSN'=+BGPSN!(BGPQUIT)  D
 ...S BGPSASU=$P(^BGPGPDCH(BGPSN,0),U,9),X=$O(^AUTTLOC("C",BGPSASU,0)) S BGPSNAM=$S(X:$P(^DIC(4,X,0),U),1:"?????"),BGPSNAM=$S($P(^BGPGPDCH(BGPSN,0),U,17):"+"_BGPSNAM,1:BGPSNAM)
 ...I $P(^BGPINDHC(BGPPC,0),U,4)["014.A"!($P(^BGPINDHC(BGPPC,0),U,4)["023.")!($P(^BGPINDHC(BGPPC,0),U,4)="016.A.1") D  I 1
 ....I $Y>(BGPIOSL-3) D HEADERP Q:BGPQUIT
...
H1P ;
 ...
 W !?22," Site",?32,"Site",?40,"Site",?50,"Area",?60,$S($G(BGPNGR09):"PART10",1:"PART09"),?64,"Nat'l",?74,"2013"
 W !?22,"Current",?32,"Prev",?40,"Base",?50,"Current",?60,"Target"_$S($G(BGPNGR09):"*",1:""),?65,"2012",?74,"Target"
 W !,$TR($J("",80)," ","-")
 W !!,"PART MEASURE"
 W !,"------------"
 Q
