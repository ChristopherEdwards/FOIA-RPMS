BGP3XTCH ; IHS/CMI/LAB - TAXONOMY CHECK FOR FY04 CRS REPORT 16 Jan 2009 4:02 PM ;
 ;;13.0;IHS CLINICAL REPORTING;;NOV 20, 2012;Build 81
 ;
 ;
 D HOME^%ZIS
 W:$D(IOF) @IOF
 W !!,"Checking for Taxonomies to support the 2013 CRS Report. ",!,"Please enter the device for printing.",!
ZIS ;
 K IOP,%ZIS
 W !! S %ZIS="PQM" D ^%ZIS
 I POP D XIT Q
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="TAXCHK^BGP3XTCH",ZTDTH="",ZTDESC="CRS 13 TAX REPORT" D ^%ZTLOAD D XIT Q
 Q
TAXCHK ;EP
 ;D HOME^%ZIS
 K BGPQUIT
GUICHK ;EP
 ...
 S BGPT="" F  S BGPT=$O(^BGPTAXH("B",BGPT)) Q:BGPT=""  D
 .S BGPY=$O(^BGPTAXH("B",BGPT,0))
 .Q:'$D(^BGPTAXH(BGPY,12,"B",2))
 .;I $P(^BGPTAXH(BGPY,0),U,2)'="L" S BGPX=$O(^ATXAX("B",BGPT,0))
 .;I $P(^BGPTAXH(BGPY,0),U,2)="L" S BGPX=$O(^ATXLAB("B",BGPT,0))
 .S BGPTYPE=$P(^BGPTAXH(BGPY,0),U,2),Y=$G(^BGPTAXH(BGPY,11,1,0))
 .I BGPTYPE'="L" D
