BGP3XTV ; IHS/CMI/LAB - DISPLAY IND LISTS 15 Dec 2010 9:42 AM ;
...
EN ;EP -- main entry point for 
 D EN^VALM("BGP 13 TAXONOMY VIEW")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ;EP -- init variables and list array
 ...
 S BGPT=""
 F  S BGPT=$O(^BGPTAXH("B",BGPT)) Q:BGPT=""  D
 .S BGPY=$O(^BGPTAXH("B",BGPT,0))
 .Q:'$O(^BGPTAXH(BGPY,12,0))
 .S BGPTYPE=$P(^BGPTAXH(BGPY,0),U,2),BGPDESC=$G(^BGPTAXH(BGPY,11,1,0)),BGPEDIT=$P(^BGPTAXH(BGPY,0),U,4),J=J+1
 .I BGPTYPE'="L" D  Q:'I
 ...
 S T="",J=0,C=0 F  S T=$O(^BGPTAXH("B",T)) Q:T=""  D
 .S Y=0 F  S Y=$O(^BGPTAXH("B",T,Y)) Q:Y'=+Y  D
 ..S N=^BGPTAXH(Y,0)
 ..S Z=$P(N,U,2)  ;TYPE
 ...
 ...S A="",B=0 F  S B=$O(^BGPTAXH(Y,12,B)) Q:B'=+B  S R=$P(^BGPTAXH(Y,12,B,0),U) S:A]"" A=A_";" S A=A_$S(R=1:"National GPRA",R=2:"Local CRS",R=3:"HEDIS",R=4:"ELDER",R=5:"CMS",1:"")
 ...S $E(BGPTAX(J,0),60)=A
...
DISP ;EP - add an item to the selected list - called from a protocol
 ...
 D EP^BGP3XTV1(BGPTIEN,BGPTYPE,BGPFIEN)
DISPX ;
 D BACK
 Q
