BGP4ASL ; IHS/CMI/LAB - DISPLAY IND LISTS 28 Apr 2006 2:10 PM ;
...
EN ;EP -- main entry point
 D EN^VALM("BGP 14 AREA SELECT SITES")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ; -- init variables and list array
 I BGPRTYPE=1 D NGR^BGP4ASL1 Q
 I BGPRTYPE=5 D ELD^BGP4ASL1 Q
 I BGPRTYPE=6 D PED^BGP4ASL1 Q
 I BGPRTYPE=7 D ONM^BGP4ASL1 Q
 Q
FAC(S) ;
...
NT ;
 ...
 . S BGPEBD=$$DATE^BGP4UTL($P(V,U))
 . S BGPEED=$$DATE^BGP4UTL($P(V,U,2))
 . S BGPEBBD=$$DATE^BGP4UTL($P(V,U,5))
 . S BGPEBED=$$DATE^BGP4UTL($P(V,U,6))
 . S BGPEDRR=$$DATE^BGP4UTL($P(V,U,13))
...
G5 ;
 ...
 S BGPFILE=90553.03
 S BGPX=0 F  S BGPX=$O(^BGPELDCJ(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPELDCJ(BGPX,0))
 . S V=^BGPELDCJ(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
...
G6 ;
 ...
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPFILE=90552.12
 S BGPX=0 F  S BGPX=$O(^BGPPEDCJ(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPPEDCJ(BGPX,0))
 . S V=^BGPPEDCJ(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
...
G9 ;
 ...
 S BGPBBD=BGPBD-X,BGPBBD=$E(BGPBBD,1,3)_$E(BGPBD,4,7)
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPX=0 F  S BGPX=$O(^BGPGPDCJ(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPGPDCJ(BGPX,0))
 . S V=^BGPGPDCJ(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
