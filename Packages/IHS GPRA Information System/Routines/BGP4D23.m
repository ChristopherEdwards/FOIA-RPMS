BGP4D23 ; IHS/CMI/LAB - measure I2 23 Jun 2010 10:08 AM ;
...
IMS ;EP
 ...
 S BGPVALUE=""
 I $$LASTDX^BGP4UTL1(DFN,"SURVEILLANCE DIABETES",$P(^DPT(DFN,0),U,3),BGPEDATE) S BGPSTOP=1 Q  ;had diabetes dx
...
IMS1 ;EP
 ...
 I BGPBP S BGPN1=1
 I BGPBP="" S BGPBP=$$BPCPT^BGP4D22(DFN,BGP365,BGPEDATE),C=1 I $P(BGPBP,U,2)]"" S BGPN1=1
 I BGPN1 S BGPVALUE=BGPVALUE_" 2 BPs" I C S BGPVALUE=BGPVALUE ;_$S($P(BGPBP,U,2)["V":" - POV ",1:" - CPT ")_$P(BGPBP,U,2)
IMS2 ;LDL done
 S BGPLDL=$$LDL^BGP4D2(DFN,BGPBDATE,BGPEDATE,1)
 S BGPN2=$P(BGPLDL,U)
 I BGPN2 D
 .I $P(BGPVALUE,"|||",2)]"" S BGPVALUE=BGPVALUE_"; "
 .S BGPVALUE=BGPVALUE_"LDL: "_$$DATE^BGP4UTL($P(BGPLDL,U,2))_" "_$P(BGPLDL,U,3)
...
IMS3 ;fasting glucose
 ...
 I BGPN3 D
 .I $P(BGPVALUE,"|||",2)]"" S BGPVALUE=BGPVALUE_"; "
 .S BGPVALUE=BGPVALUE_"FG: "_$$DATE^BGP4UTL($P(BGPFG,U,2))
 S BGPA1C=$$HGBA1C^BGP4D2(DFN,BGPBDATE,BGPEDATE)
 ...
 .S BGPN12=1 Q
 S BGPGFR="" ;$$GFR^BGP4D211(DFN,BGP365,BGPEDATE)
 S BGPESRD="" ;$$ESRD^BGP4D211(DFN,$P(^DPT(DFN,0),U,3),BGPEDATE)
 S BGPQUP="" ;$$QUANTUP^BGP4D211(DFN,BGPBDATE,BGPEDATE)
 I BGPESRD S BGPN4=1
 I $P(BGPGFR,U),$P(BGPQUP,U,1) S BGPN4=1
 I BGPN4 D
 .I BGPESRD S BGPVALUE=BGPVALUE_$S(BGPESRD]"":";ESRD: "_$P(BGPESRD,U,2)_"-"_$$DATE^BGP4UTL($P(BGPESRD,U,3)),1:"") Q
 .S BGPVALUE=BGPVALUE_";GFR: "_$$DATE^BGP4UTL($P(BGPGFR,U,2))
 .S BGPVALUE=BGPVALUE_" & UACR: "_$P(BGPQUP,U,2)_"-"_$$DATE^BGP4UTL($P(BGPQUP,U,3))
 K BGPFG
IMS5 ;
 K BGPX,BGPC1,X,Y,Z,%,A,B,C,D,E,H,BDATE,EDATE,P,V,S,F
 S BGPTOBV=""
 S BGPTOB=$$TOBACCO^BGP4D7(DFN,BGPBDATE,BGPEDATE)
 S BGPN5=$S(BGPTOB]"":1,1:0) I BGPN5 S BGPTOBV=$P(BGPTOB,U,2)_" "_$P(BGPTOB,U,1)
 S BGPSDX=$$DX^BGP4D7(DFN,BGPBDATE,BGPEDATE)
 S BGPXPHD=$$PED^BGP4D7(DFN,BGPBDATE,BGPEDATE)
 S BGP1320=$$DENT^BGP4D7(DFN,BGPBDATE,BGPEDATE)
 S BGPSCPT=$$CPTSM^BGP4D7(DFN,BGPBDATE,BGPEDATE)
 I BGPSDX]"" S BGPN5=1 I BGPTOBV="" S BGPTOBV=$$DATE^BGP4UTL($P(BGPSDX,U,2))_" "_$P(BGPSDX,U,1)
 I BGPXPHD]"" S BGPN5=1 I BGPTOBV="" S BGPTOBV=$$DATE^BGP4UTL($P(BGPXPHD,U,2))_" "_$P(BGPXPHD,U,1)
 I BGP1320]"" S BGPN5=1 I BGPTOBV="" S BGPTOBV=$$DATE^BGP4UTL($P(BGP1320,U,2))_" 1320"
 I BGPSCPT]"" S BGPN5=1 I BGPTOBV="" S BGPTOBV=$$DATE^BGP4UTL($P(BGPSCPT,U,2))_" "_$P(BGPSCPT,U,1)
 I BGPN5 D
 .I $P(BGPVALUE,"|||",2)]"" S BGPVALUE=BGPVALUE_"; "
 .S BGPVALUE=BGPVALUE_"TOB: "_BGPTOBV
IMS6 ;
 S BGPBMI=$$BMI^BGP4D6(DFN,BGPEDATE,BGPAGEE)
 ;I $P(BGPBMI,U)="" S BGPBMI=$$REF^BGP4D6(DFN,BGPBDATE,BGPEDATE,BGPAGEB)
 I $P(BGPBMI,U)]"" S BGPN6=1
 I BGPN6 D
 .I $P(BGPVALUE,"|||",2)]"" S BGPVALUE=BGPVALUE_"; "
 .S BGPVALUE=BGPVALUE_"BMI: "_$S(BGPBMI["HT":"Ref "_$P(BGPBMI,U,2)_" "_$$DATE^BGP4UTL($P(BGPBMI,U,3))_" "_$P(BGPBMI,U,5)_" "_$$DATE^BGP4UTL($P(BGPBMI,U,6)),BGPBMI]"":$$SB^BGP4PDL1($J($P(BGPBMI,U),6,2)),1:"")
IMS7 ;
 S BGPLIFE=$$LIFED^BGP4D41(DFN,BGPBDATE,BGPEDATE)
 I $P(BGPLIFE,U) S BGPN7=1
 I BGPN7 D
 .I $P(BGPVALUE,"|||",2)]"" S BGPVALUE=BGPVALUE_"; "
 .S BGPVALUE=BGPVALUE_"LIFE: "_$$DATE^BGP4UTL($P(BGPLIFE,U,2))_" "_$P(BGPLIFE,U,3)
IMS8 ;
 S BGPDEP=$$DEP^BGP4D25(DFN,BGP365,BGPEDATE) I $P(BGPDEP,U)=1 S BGPN8=1
 S BGPDEPS=$$DEPSCR^BGP4D25(DFN,BGP365,BGPEDATE) I $P(BGPDEPS,U)=1 S BGPN8=1
 S BGPREF="" I 'BGPN8 S BGPREF=$$DEPREF^BGP4D25(DFN,BGP365,BGPEDATE) I $P(BGPREF,U)=1 S BGPN8=1
 I BGPDEP]"" D  I 1
 .I $P(BGPVALUE,"|||",2)]"" S BGPVALUE=BGPVALUE_"; "
...
MS(P,BDATE,EDATE) ;EP
 ...
 ..S %=$P(^AUPNVPOV(Y,0),U)
 ..Q:'%
 ..;I $P($$ICDDX^BGP4UTL2(%),U,2)=277.7 S D=1
 ..I $$ICD^BGP4UTL2(%,$O(^ATXAX("B","BGP PRE DM MET SYN DX",0)),9) S D=1,E=Y
 .Q:'D
 .S $P(G,U)=$P(G,U)+1,$P(G,U,($P(G,U)+1))=$$DATE^BGP4UTL($P($P(^AUPNVSIT(V,0),U),"."))_" POV "_$$VAL^XBDIQ1(9000010.07,E,.01)
 .Q
 K ^TMP($J,"A")
 I $P(G,U)>1 Q 1_U_" on "_$P(G,U,2)_"; "_$P(G,U,3)
 ;now check for 3 or more of the following
 S BGPC=0,BGPK=""
 S X=$$BMI^BGP4D6(P,EDATE,$$AGE^AUPNPAT(P,EDATE))
 I $E(X,1,2)>29 S BGPC=BGPC+1,BGPK="BMI="_$$STRIP^XLFSTR($J(X,6,2)," ")
 S X=$$TRIG^BGP4D231(P,BDATE,EDATE) I X]"" S BGPC=BGPC+1 S:BGPK]"" BGPK=BGPK_"; " S BGPK=BGPK_X
 S X=$$HDL(P,BDATE,EDATE) I X]"" S BGPC=BGPC+1 S:BGPK]"" BGPK=BGPK_"; " S BGPK=BGPK_X
 S BGPHTN=$$LASTDX^BGP4UTL1(P,"SURVEILLANCE HYPERTENSION",BGP365,EDATE)
 S BGPMBP=$$MEANBP(P,BGPBDATE,BGPEDATE)
 I $P(BGPHTN,U)!(BGPMBP]"") S BGPC=BGPC+1 S:BGPK]"" BGPK=BGPK_"; " S:$P(BGPHTN,U)=1 BGPK=BGPK_"HTN DX: "_$$DATE^BGP4UTL($P(BGPHTN,U,3)) I BGPMBP]"" S:BGPK]""&($P(BGPHTN,U)=1) BGPK=BGPK_"; " S BGPK=BGPK_BGPMBP
 S X=$$FG(P,BGPBDATE,BGPEDATE) I X]"" S BGPC=BGPC+1 S:BGPK]"" BGPK=BGPK_"; " S BGPK=BGPK_X
 S X=$$WC^BGP4D231(P,BGPBDATE,BGPEDATE) I X]"" S BGPC=BGPC+1 S:BGPK]"" BGPK=BGPK_"; " S BGPK=BGPK_X
...
WT(P,BDATE,EDATE) ;EP
 ...
 .. S D=$P(BGPL(BGPLN),U)
 .. S ICD=$P($$ICDDX^BGP4UTL2($P(^AUPNVPOV(BGPLD,0),U),D),U,2) D
 ...I $E(ICD,1,3)="V22" Q
 ...I $E(ICD,1,3)="V23" Q
 ...I $E(ICD,1,3)="V27" Q
 ...I $E(ICD,1,3)="V28" Q
...
HDL(P,BDATE,EDATE) ;EP
 K BGPC1
 S BGPC1=0,R=""
 ;S %="",E=+$$CODEN^ICPTCOD(83718),%=$$CPTI^BGP4DU(P,BDATE,EDATE,E)
 ;I %]"" Q 1_U_$P(%,U,2)
 ;S %="",E=+$$CODEN^ICPTCOD(83718),%=$$TRANI^BGP4DU(P,BDATE,EDATE,E)
 ;I %]"" Q 1_U_$P(%,U,2)
