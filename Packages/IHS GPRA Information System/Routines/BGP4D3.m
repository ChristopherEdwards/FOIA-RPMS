BGP4D3 ; IHS/CMI/LAB - indicator 11 ;
...
I11 ;EP
 ...
 S BGPDV=""
 I BGPRTYPE'=3 S BGPDV=$S(BGPD5:"UP",1:"") D
...
I12 ;EP
 ...
 I BGPVAL]"" S BGPN2=1
 S BGPDV=""
 I BGPRTYPE'=3 S BGPDV=$S(BGPD3:"UP",1:"") D
 .I BGPD1 S BGPDV=BGPDV_$S(BGPDV]"":",AC",1:"AC")
...
I13 ;EP
 ...
 I BGPRTYPE'=3 S BGPVALUE=$S(BGPI7DA:"UP",1:"")_$S(BGPI7DB:",AC",1:"")_"; "_$$DATE^BGP4UTL($P(BGPPAP,U,2))_" "_$P(BGPPAP,U,3)
 I BGPRTYPE=3 S BGPVALUE=$$DATE^BGP4UTL($P(BGPPAP,U,2))_" "_$P(BGPPAP,U,3)
 Q
 ;
SEAL(P,BDATE,EDATE) ;
 K BGPG S BGPC=0
...
FLU(P,ED) ;
 ...
 I T D  I X]"" Q $$DATE^BGP4UTL(X)_" CPT"
 .S X=$$CPT^BGPDU(P,,ED,T,3)
 S G=$$REFUSAL^BGP4UTL1(P,9999999.14,$O(^AUTTIMM("C",88,0)),$$FMADD^XLFDT(ED,-365),ED)
 I $P(G,U)=1 Q $$DATE^BGP4UTL($P(G,U,2))_" Refused"
 S G=$$REFUSAL^BGP4UTL1(P,9999999.14,$O(^AUTTIMM("C",111,0)),$$FMADD^XLFDT(ED,-365),ED)
 I $P(G,U)=1 Q $$DATE^BGP4UTL($P(G,U,2))_" Refused"
 S G=$$REFUSAL^BGP4UTL1(P,9999999.14,$O(^AUTTIMM("C",15,0)),$$FMADD^XLFDT(ED,-365),ED)
 I $P(G,U)=1 Q $$DATE^BGP4UTL($P(G,U,2))_" Refused"
 S G=$$REFUSAL^BGP4UTL1(P,9999999.14,$O(^AUTTIMM("C",16,0)),$$FMADD^XLFDT(ED,-365),ED)
 I $P(G,U)=1 Q $$DATE^BGP4UTL($P(G,U,2))_" Refused"
 ...
 .Q:$P(^BIPC(X,0),U,4)>ED
 .S G=1
 I G Q $$DATE^BGP4UTL($P(G,U,2))_" Refused Imm pkg"
...
PAP(P,EDATE,YEARS) ;
 ...
 ...Q:'$D(^AUPNVLAB(X,0))
 ...S Z=$P(^AUPNVLAB(X,0),U),Z=$P($G(^LAB(60,Z,0)),U) I Z="PAP SMEAR" S BGPC="1^"_(9999999-D)_"^VLab" Q
 ...I BGPLT,$P(^AUPNVLAB(X,0),U),$D(^ATXLAB(BGPLT,21,"B",$P(^AUPNVLAB(X,0),U))) S BGPC="1^"_(9999999-D)_"VLab" Q
 ...Q:'T
 ...S J=$P($G(^AUPNVLAB(X,11)),U,13) Q:J=""
 ...Q:'$$LOINC(J,T)
 ...S BGPC="1^"_(9999999-D)_"VLab-loinc" Q
 ...Q
 I BGPC]"" Q BGPC
 ...
 S T=$$REFUSAL^BGP4UTL1(P,60,$O(^LAB(60,"B","PAP SMEAR",0)),$$FMADD^XLFDT(EDATE,-(3*365)),EDATE)
 I T Q "1^"_$P(T,U,2)_"^ref"
 S G=$$REFUSAL^BGP4UTL1(P,9999999.15,$O(^AUTTEXAM("C",15,0)),$$FMADD^XLFDT(EDATE,-(3*365)),EDATE)
 I $P(G,U)=1 Q "1^"_$P(G,U,2)_"^ref"
 Q 0
LOINC(A,B) ;
 NEW %
