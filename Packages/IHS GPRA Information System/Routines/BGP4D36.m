BGP4D36 ; IHS/CMI/LAB - measure C 06 Nov 2009 2:03 PM 28 May 2014 4:23 PM ;
...
MEN(P,EDATE) ;EP
 ...
 F BGPZ=32,108,114,136,147 S X=$$ANCONT^BGP4D31(P,BGPZ,EDATE) Q:X]""
 ...
 ;F BGPIMM=32,108,114,136,147 S R=$$IMMREF^BGP4D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
...
HPV(P,EDATE) ;EP
 ...
 ;now check Refusals in imm pkg
 ;F BGPIMM=62,118 S R=$$IMMREF^BGP4D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
 ;I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI",1:"Ref")_" HPV"
 F BGPZ=62,118 S X=$$ANCONT^BGP4D31(P,BGPZ,EDATE) Q:X]""
...
H1N1 ;EP - called from measure
 ...
 I BGPN1!(BGPN2) S BGPVALUE=BGPVALUE_$S($P(BGPH1N1,U,1):$$DATE^BGP4UTL($P(BGPH1N1,U,1))_" "_$P(BGPH1N1,U,2),1:"")
 I $P(BGPH1N1,U,3) S BGPVALUE=BGPVALUE_"; "_$$DATE^BGP4UTL($P(BGPH1N1,U,3))_" "_$P(BGPH1N1,U,4)  ;:"2 doses",$P(BGPH1N1,U,1):"1 dose",1:"")_" "_$$DATE^BGP4UTL($P(BGPH1N1,U))_" "_$$DATE^BGP4UTL($P(BGPH1N1,U,2))
 Q
H1N1VAC(P,BDATE,EDATE) ;EP
 ...
 ..Q:'$D(^AUPNVCPT("AD",V))
 ..S X=0 F  S X=$O(^AUPNVCPT("AD",V,X)) Q:X'=+X  D
 ...I $$ICD^BGP4UTL2($P(^AUPNVCPT(X,0),U),T,1) S C=C+1,M($$VD^APCLV(V))=$$VAL^XBDIQ1(9000010.18,X,.01)
 ...Q
 ..Q
 .Q
 I '$D(M) Q ""
...
ACTPREG(P,ED) ;EP
 ...
 ..Q:Q=""
 ..Q:'$$ICD^BGP4UTL2(Q,T,9)  ;not in taxonomy
 ..S G=1
 ...
 K BGPG S BGPG=$$LASTPRC^BGP4UTL1(P,"BGP ABORTION PROCEDURES",LPD,ED)
