BGP4D722 ; IHS/CMI/LAB - measure AHR.A ;
...
ACECONT(P,BDATE,EDATE,NMIB,NMIE,RPBD) ;EP
 ...
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP4UTL($P(BGPG(1),U))_" Contra Aortic POV "_$P(BGPG(1),U,2)
 ;
 ;nmi
 S BGPG=""
 ...
 ...S BGPG=1_U_$$DATE^BGP4UTL($P(^AUPNPREF(N,0),U,3))_" Contra NMI "_$$VAL^XBDIQ1(9000024,N,.04)
 ..Q
 .Q
 I BGPG Q BGPG
 ;nmi
 ...
 ..S N=0 F  S N=$O(^AUPNPREF("AA",P,50,X,D,N)) Q:N'=+N  D
 ...Q:$P($G(^AUPNPREF(N,0)),U,7)'="N"
 ...S BGPG=1_U_$$DATE^BGP4UTL($P(^AUPNPREF(N,0),U,3))_" Contra NMI "_$$VAL^XBDIQ1(9000022,N,.04)
 ..Q
 .Q
 I BGPG Q BGPG
 ;PREGNANCY
 S X=$$PREG^BGP4D7(P,$S($G(RPBD):RPBD,1:NMIB),NMIE,1,1) I X Q 1_U_"Contra pregnant"
 ;breastfeeding
 ...
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP4UTL($P(BGPG(1),U))_" Contra POV "_$P(BGPG(1),U,2)
 K BGPG
 S Y="BGPG("
 ...
 I %]"" Q 1_U_$$DATE^BGP4UTL($P(%,U,2))_" Contra "_$P(%,U,1)  ;"ACEI Contra - "_%
 Q ""
...
ACEREF(P,BDATE,EDATE) ;EP
 ...
 ...S G=1_U_$$DATE^BGP4UTL(Y)_" Refused "_$P(^PSDRUG(X,0),U,1)
 ..Q
 .Q
 I G Q G
 S T=$O(^ATXAX("B","BGP HEDIS ARB MEDS",0))
...
STATALG(P,BDATE,EDATE,RPB,RPE) ;EP
 ...
 .I N["STATIN"!(N["STATINS") S BGPC=1_U_$$DATE^BGP4UTL($P(BGPG(X),U))_" ADR POV "_$P(BGPG(X),U,2)
 .S T=$O(^ATXAX("B","BGP ADV EFF CARDIOVASC NEC",0))
 .S Z=$P(^AUPNVPOV(Y,0),U,9) I Z]"",$$ICD^BGP4UTL2(Z,T,9) S BGPC=1_U_$$DATE^BGP1UTL($P(BGPG(X),U))_" ADR POV ["_$P(BGPG(X),U,2)_" + "_$P($$ICDDX^BGP4UTL2(Z),U,2)_"]  "_N Q
 .S Z=$P(^AUPNVPOV(Y,0),U,18) I Z]"",$$ICD^BGP4UTL2(Z,T,9) S BGPC=1_U_$$DATE^BGP1UTL($P(BGPG(X),U))_" ADR POV ["_$P(BGPG(X),U,2)_" + "_$P($$ICDDX^BGP4UTL2(Z),U,2)_"]  "_N Q
 .S Z=$P(^AUPNVPOV(Y,0),U,19) I Z]"",$$ICD^BGP4UTL2(Z,T,9) S BGPC=1_U_$$DATE^BGP1UTL($P(BGPG(X),U))_" ADR POV ["_$P(BGPG(X),U,2)_" + "_$P($$ICDDX^BGP4UTL2(Z),U,2)_"]  "_N Q
 .Q
 I BGPC Q BGPC
 ...
 .I N["STATIN"!(N["STATINS") S BGPC=1_U_$$DATE^BGP4UTL($P(BGPG(X),U))_" ADR POV "_$P(BGPG(X),U,2)  ;_"]"
 I BGPC Q BGPC
 ;PL
 S BGPC=0
 ...
 .I $$ICD^BGP4UTL2(I,$O(^ATXAX("B","BGP HX DRUG ALLERGY NEC",0)),9)!($$ICD^BGP4UTL2(I,T,9)),N["STATIN"!(N["STATINS") S BGPC=1_U_$$DATE^BGP4UTL($P(^AUPNPROB(X,0),U,8))_" ADR Problem List "_Y  ;_"]"
 .Q
 I BGPC Q BGPC
 ;ART
 S BGPC=0
 ...
 .I N["STATIN" S BGPC=1_U_$$DATE^BGP4UTL($P(^GMR(120.8,X,0),U,4))_" ADR Allergy Tracking "_N
 I BGPC Q BGPC
 ...
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP4UTL($P(BGPG(1),U))_" ADR POV "_$P(BGPG(1),U,2)  ;_"]"
 ...
 ...Q:'$D(^AUPNVLAB(X,0))
 ...I BGPLT,$P(^AUPNVLAB(X,0),U),$D(^ATXLAB(BGPLT,21,"B",$P(^AUPNVLAB(X,0),U))) I $$RESCK(X) S BGPG=1_U_$$DATE^BGP4UTL((9999999-D))_" ADR creat kinase of "_$P(^AUPNVLAB(X,0),U,4) Q
 ...Q:'T
 ...S J=$P($G(^AUPNVLAB(X,11)),U,13) Q:J=""
 ...Q:'$$LOINC^BGP4D2(J,T)
 ...I $$RESCK(X) S BGPG=1_U_$$DATE^BGP4UTL((9999999-D))_" ADR creat kinase of "_$P(^AUPNVLAB(X,0),U,4) Q
 ...Q
 I BGPG Q BGPG
...
STATCON(P,BDATE,EDATE,NMIB,NMIE) ;EP does patient have an STATIN Contra
 ...
 ;
 ;pregnant
 S X=$$PREG^BGP4D7(P,BDATE,EDATE,1,1) I X Q 1_U_"Contra pregnant"
 ;nmi
 S BGPG=""
 ...
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP4UTL($P(BGPG(1),U))_"Contra POV "_$P(BGPG(1),U,2)  ;_"]"  ;_$$VAL^XBDIQ1(9000010.07,+$P(BGPG(1),U,4),.04)
 ;now check education
 K BGPG
 S Y="BGPG("
 ...
 .I T="BF-MK" S %=T_U_$P(BGPG(X),U) Q
 .I T="BF-N" S %=T_U_$P(BGPG(X),U) Q
 I %]"" Q 1_U_$$DATE^BGP4UTL($P(%,U,2))_"Contra "_$P(%,U,1)
