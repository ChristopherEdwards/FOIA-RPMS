...
ASU ;
 ...
 .K BGPSUL S BGPX=0 F  S BGPX=$O(^BGPGPDCF(BGPX)) Q:BGPX'=+BGPX  S V=^BGPGPDCF(BGPX,0) I $P(V,U)=BGPBD,$P(V,U,2)=BGPED,$P(V,U,7)=BGPPER,$P(V,U,12)=1,$P(V,U,5)=BGPBBD,$P(V,U,6)=BGPBED,$P(V,U,14)=1 S BGPSUL(BGPX)="",BGPSUCNT=BGPSUCNT+1
 ...
 .S X=0 F  S X=$O(BGPSUL1(X)) Q:X'=+X  S BGP0=^BGPGPDCF(BGPSUL1(X),0) W !?2,X,")",?2,"FY: ",$$FMTE^XLFDT($P(BGP0,U,2)),?11,"END DATE: ",$$VAL^XBDIQ1(90244.03,BGPSUL1(X),.08) D
 ...
 G:BGPRPTT="F" ZIS
GETSU ;
 K BGPSUL S BGPX=0 F  S BGPX=$O(^BGPGPDCF(BGPX)) Q:BGPX'=+BGPX  S V=^BGPGPDCF(BGPX,0) I $P(V,U)=BGPBD,$P(V,U,2)=BGPED,$P(V,U,7)=BGPPER,$P(V,U,5)=BGPBBD,$P(V,U,6)=BGPBED,$P(V,U,12)=1,$P(V,U,14)=1 S BGPSUL(BGPX)=""
 ...
 S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  S BGP0=^BGPGPDCF(X,0) W !?2,"FY: ",$$FMTE^XLFDT($P(BGP0,U,7)),?11,"END DATE: ",$$VAL^XBDIQ1(90244.03,X,.08),?36,"SU: ",$$SU($P(BGP0,U,11)),?55,"Facility: ",$E($$FAC($P(BGP0,U,9)),1,15)
ZIS ;call to XBDBQUE
EISSEX ;
 S BGPEXCEL=""
 S BGPUF=""
 ...
 .S BGPFN="GPRAEX"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP4UTL(BGPED)_"00000000"_$$D^BGP4UTL(BGPNOW)_"_"_$$LZERO^BGP4UTL(BDWC,6)_".TXT"  ;IHS/CMI/LAB - patch 1 no $E
 .Q
...
GI ;gather all gpra indicators
 ...
 ;I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPGPDCF(" D ^DIK K DIK D EXIT Q
 I $D(IO("Q")) G TSKMN
DRIVER ;
 U IO
 D PRINT^BGP4PARP
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP4DAR",ZTDTH="",ZTDESC="GPRA REPORT" D ^%ZTLOAD D EXIT Q
 Q
 ;
FAC(S) ;
...
Q ;which quarter
 ...
 S DIR(0)="N^1:4:0",DIR("A")="Which FY Quarter" KILL DA D ^DIR KILL DIR
