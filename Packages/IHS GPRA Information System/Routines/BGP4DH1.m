BGP4DH1 ; IHS/CMI/LAB - cover page for gpra 28 Apr 2010 11:30 AM 02 Jul 2010 8:28 AM ;
...
AHDR ;EP
 ...
 I $E(IOST)="C",IO=IO(0) D W^BGP4DP("",0,1,BGPPTYPE) S DIR(0)="EO" D ^DIR K DIR I Y=0!(Y="^")!($D(DTOUT)) S BGPQHDR=1 Q
AHDR1 ;
...
AHDR10 D W^BGP4DP("Cover Page "_$S(BGPPTYPE="P":BGPHPG,1:""),1,1,BGPPTYPE)
 ...
 I BGPRTYPE=6 D W^BGP4DP("*** IHS 2014 Patient Education Report with Community Specified ***",1,2,BGPPTYPE)
...
N D W^BGP4DP("AREA AGGREGATE",1,1,BGPPTYPE)
 D W^BGP4DP($$RPTVER^BGP4BAN,1,1,BGPPTYPE)
 S X="Date Report Run:  "_$$FMTE^XLFDT(DT) D W^BGP4DP(X,1,1,BGPPTYPE)
 S X="Site where Run:  "_$$VAL^XBDIQ1(9999999.06,DUZ(2),.04) D W^BGP4DP(X,1,1,BGPPTYPE)
 D W^BGP4DP("Report Generated by: "_$$USR^BGP4DH,1,1,BGPPTYPE)
...
PPHDR ;EP
 ...
 .I BGPPTYPE="P",$Y>(BGPIOSL-2) D HDR^BGP4DH I BGPPTYPE="P" Q:BGPQHDR
 .D W^BGP4DP(^BGPCTRL(BGPX,18,BGPY,0),0,1,BGPPTYPE)
 .Q
 D W^BGP4DP("",0,1,BGPPTYPE)
 Q
GPUPPHDR ;EP
 ...
 .I BGPPTYPE="P",$Y>(BGPIOSL-2) D HDR^BGP4DH I BGPPTYPE="P" Q:BGPQHDR
 .D W^BGP4DP(^BGPCTRL(BGPX,83,BGPY,0),0,1,BGPPTYPE)
 .Q
 D W^BGP4DP("",0,1,BGPPTYPE)
 Q
ALLHDR ;EP
...
AREACP ;EP
 ...
 D PD^BGP4DH
 D ENDTIME^BGP4DH
 I BGPRTYPE=6 D PEDCP^BGP4DH Q
 I $G(BGPDASH) D DASHHDRA^BGP4DH2 G N1
 I BGPRTYPE=1,'$G(BGPYGPU),'$G(BGPSUMON) D GPRAHDRA^BGP4DH2
 I BGPRTYPE=1,$G(BGPYGPU) D GPRAHDRA^BGP4DH2
 I BGPRTYPE=1,'$G(BGPYGPU),$G(BGPSUMON) D GPRAHDRS
 I BGPRTYPE=7 D ONMHDRA^BGP4DH2
N1 I $G(BGPEXCEL),'$G(BGPYGPU),BGPRTYPE=1 D
 .D W^BGP4DP("GPRA Developmental filenames:  ",0,2,BGPPTYPE),W^BGP4DP(BGPFDEV1,0,1,BGPPTYPE,1,15),W^BGP4DP(BGPFDEV2,0,1,BGPPTYPE,1,15),W^BGP4DP(BGPFDEV3,0,1,BGPPTYPE,1,15)
 .D W^BGP4DP("National GPRA filenames:  ",0,2,BGPPTYPE),W^BGP4DP(BGPFGNT1,0,1,BGPPTYPE,1,15),W^BGP4DP(BGPFGNT2,0,1,BGPPTYPE,1,15),W^BGP4DP(BGPFGNT3,0,1,BGPPTYPE,1,15)
 I $G(BGPEXCEL),BGPRTYPE=7 D W^BGP4DP("Other National Measures filenames: ",0,1,BGPPTYPE),W^BGP4DP(BGPFONN1,0,1,BGPPTYPE,1,15),W^BGP4DP(BGPFONN2,0,1,BGPPTYPE,1,15),W^BGP4DP(BGPFONN3,0,1,BGPPTYPE,1,15),W^BGP4DP(BGPFONN4,0,1,BGPPTYPE,1,15) D
 .D W^BGP4DP(BGPFONN5,0,1,BGPPTYPE,1,15)
 I BGPROT'="P",'$D(BGPGUI),BGPDELT="F" D W^BGP4DP("A delimited output file called "_BGPDELF,0,2,BGPPTYPE) D
 .D W^BGP4DP("has been placed in the "_$$GETDEDIR^BGP4UTL2()_" directory for your use in Excel or some",0,1,BGPPTYPE)
 .D W^BGP4DP("other software package.",0,1,BGPPTYPE)
 .D W^BGP4DP("See your site manager to access this file.",0,1,BGPPTYPE)
 .D W^BGP4DP("",0,1,BGPPTYPE)
 ...
 .S X=$P(^BGPGPDCJ(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPGPDCJ(BGPX,0),U,17):"*",$P(^BGPGPDCJ(BGPX,0),U,19):"**",1:"")_X D W^BGP4DP(X,0,1,BGPPTYPE,1,3)
 .Q
 S X=" " D W^BGP4DP(X,0,1,BGPPTYPE)
 S X="The following communities are included in this report:" D W^BGP4DP(X,0,1,BGPPTYPE,1,1)
 S BGPX="",BGPC=0 F  S BGPX=$O(BGPSUL(BGPX)) Q:BGPX=""  D
 .S X=$P(^BGPGPDCJ(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPGPDCJ(BGPX,0),U,17):"*",1:"")_X D W^BGP4DP(X,0,1,BGPPTYPE,1,3)
 .;S X="Communities: " D W^BGP4DP(X,0,1,BGPPTYPE,1,5)
 .S X="Community Taxonomy Name: "_$P(^BGPGPDCJ(BGPX,0),U,18) D W^BGP4DP(X,0,1,BGPPTYPE,1,5)
 .S BGPXX=0,BGPXN=0,BGPXY="" F  S BGPXX=$O(^BGPGPDCJ(BGPX,9999,BGPXX)) Q:BGPXX'=+BGPXX  S BGPXN=BGPXN+1,BGPXY=BGPXY_$S(BGPXN=1:"",1:";")_$P(^BGPGPDCJ(BGPX,9999,BGPXX,0),U)
 ...
 ..Q
 .I $O(^BGPGPDCJ(BGPX,1111,0)) D
 ..D W^BGP4DP("MFI Visit Locations: ",0,2,BGPPTYPE,1,5) S BGPXX=0,BGPXN=0,BGPXY="" F  S BGPXX=$O(^BGPGPDCJ(BGPX,1111,BGPXX)) Q:BGPXX'=+BGPXX  S BGPXN=BGPXN+1,BGPXY=BGPXY_$S(BGPXN=1:"",1:";")_$P(^BGPGPDCJ(BGPX,1111,BGPXX,0),U)
 ..S BGPX1=0,C=0 F BGPX1=1:3:BGPXN D
 ...D W^BGP4DP($E($P(BGPXY,";",BGPX1),1,20),0,1,BGPPTYPE,1,10)
 ...D W^BGP4DP($E($P(BGPXY,";",(BGPX1+1)),1,20),0,0,BGPPTYPE,2,30)
 ...D W^BGP4DP($E($P(BGPXY,";",(BGPX1+2)),1,20),0,0,BGPPTYPE,3,60)
 ...
 D W^BGP4DP(" ",0,1,BGPPTYPE)
 S X=" " D W^BGP4DP(X,0,1,BGPPTYPE)
 I BGPPTYPE="D" D W^BGP4DP("ENDCOVERPAGE",0,1,BGPPTYPE)
 K BGPX,BGPQUIT
 Q
ONH1 ;;
