...
ASU ;
 ...
 .K BGPSUL S BGPX=0 F  S BGPX=$O(^BGPHEDCF(BGPX)) Q:BGPX'=+BGPX  S V=^BGPHEDCF(BGPX,0) I $P(V,U)=BGPBD,$P(V,U,2)=BGPED,$P(V,U,7)=BGPPER,$P(V,U,12)=3,$P(V,U,5)=BGPBBD,$P(V,U,6)=BGPBED,$P(V,U,14)=BGPBEN S BGPSUL(BGPX)="",BGPSUCNT=BGPSUCNT+1
 ...
 .S X=0 F  S X=$O(BGPSUL1(X)) Q:X'=+X  S BGP0=^BGPHEDCF(BGPSUL1(X),0) W !?2,X,")",?2,"FY: ",$$FMTE^XLFDT($P(BGP0,U,7)),?11,"END DATE: ",$$VAL^XBDIQ1(90370.03,BGPSUL1(X),.02) D
 ...
 .I 'Y S BGPQUIT=1 Q
 .K BGPSUL S BGPSUL(BGPSUL1(Y))="",BGPSUCNT=1,X=$P(^BGPHEDCF(BGPSUL1(Y),0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 .Q
 G:BGPRPTT="F" ZIS
GETSU ;
 K BGPSUL S BGPX=0 F  S BGPX=$O(^BGPHEDCF(BGPX)) Q:BGPX'=+BGPX  S V=^BGPHEDCF(BGPX,0) I $P(V,U)=BGPBD,$P(V,U,2)=BGPED,$P(V,U,7)=BGPPER,$P(V,U,5)=BGPBBD,$P(V,U,6)=BGPBED,$P(V,U,12)=3,$P(V,U,14)=BGPBEN S BGPSUL(BGPX)=""
 ...
 S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  S BGP0=^BGPHEDCF(X,0) W !?2,"FY: ",$$FMTE^XLFDT($P(BGP0,U,7)),?11,"END DATE: ",$$VAL^XBDIQ1(90370.03,X,.02),?35,"SU: ",$$SU($P(BGP0,U,11)),?55,"Facility: ",$E($$FAC($P(BGP0,U,9)),1,15)
ZIS ;call to XBDBQUE
 D ^XBFMK
 K DIC,DIADD,DLAYGO,DR,DA,DD,X,Y,DINUM
GI ;gather all gpra indicators
 S X=0 F  S X=$O(^BGPHEIF(X)) Q:X'=+X  S BGPIND(X)=""
 S BGPINDT="H"
 ;
 D PT^BGP4DHSL
 I BGPROT="" G ASU
 ...
 ;I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPHEDFC(" D ^DIK K DIK D EXIT Q
 I $D(IO("Q")) G TSKMN
DRIVER ;
 U IO
 D PRINT^BGP4PHEP
