BGP4DMT ; IHS/CMI/LAB - national patient list 20 Dec 2004 9:24 AM ;
...
QUE ;
 ...
 S ZTRTN="PRINT^BGP4DMT",ZTDESC="BGP 14 MED TAX REPORT",ZTIO=ION,ZTDTH=""
 D ^%ZTLOAD
 D HOME^%ZIS
 D XIT
 Q
...
N ;GATHER UP AND DISPLAY ALL NATIONAL GPRA
 ...
 S BGPTNAME="" F  S BGPTNAME=$O(^BGPTAXJ("B",BGPTNAME)) Q:BGPTNAME=""!(BGPQUIT)  D
 .S BGPTIEN=0,BGPTIEN=$O(^BGPTAXJ("B",BGPTNAME,BGPTIEN))
 .Q:'BGPTIEN  ;oops, error in xref
 .Q:'$D(^BGPTAXJ(BGPTIEN,0))  ;oops, error in xref
 .Q:$P(^BGPTAXJ(BGPTIEN,0),U,2)'="M"  ;only DRUG/MEDICATION taxonomies in this report
 .S (G,X)=0 F  S X=$O(^BGPTAXJ(BGPTIEN,12,"B",X)) Q:X'=+X!(G)  I $D(BGPRT(X)) S G=1
 .Q:'G
 .;now eliminate those in BGPNO
 .S (G,X)=0 F  S X=$O(^BGPTAXJ(BGPTIEN,12,"B",X)) Q:X'=+X!(G)  D
 ..I $D(BGPNO(X)) S G=1
 .Q:G
...
INTROT ;introductory text
 ...
 ;;CRS 2014.  The report is organized by (1) taxonomies included in the
