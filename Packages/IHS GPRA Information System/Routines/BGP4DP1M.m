BGP4DP1M ; IHS/CMI/LAB - print ind 1 12 Nov 2010 7:38 AM ;
...
I1AGE1 ;
 ...
 Q
AGES ;
 S BGPF="MTA.B."_BGPXX_BGPZ S BGPPC=$O(^BGPINDJC("C",BGPF,0))
 D
...
I1AGEP ;
 ...
 ;I '$G(BGPSUMON),BGPPTYPE="P" D HEADER^BGP4DPH Q:BGPQUIT  D W^BGP4DP(^BGPINDJ(BGPIC,53,1,0),0,1,BGPPTYPE) D:$D(^BGPINDJ(BGPIC,53,2,0)) W^BGP4DP(^BGPINDJ(BGPIC,53,2,0),0,1,BGPPTYPE) D AH
 I '$G(BGPSUMON),BGPPTYPE="P" D W^BGP4DP(" ",0,2,BGPPTYPE)
 I BGPPTYPE'="P" D W^BGP4DP("",0,2,BGPPTYPE) D AH
...
I1AGEP1 ;
 ...
 D W^BGP4DP("# w/ 2 or more controller Rx",0,2,BGPPTYPE)
 S T=31 F X=1:1:4 S V=$P(@BGPARR@(X),U,2) D W^BGP4DP($S(BGPPTYPE="P":$$C(V,0,6),1:$S(V:V,1:0)),0,0,BGPPTYPE,X+1,T) S T=T+11
 D W^BGP4DP("% w/ 2 or more controller Rx",0,1,BGPPTYPE)
 S T=30 F X=1:1:4 S V=$P(@BGPARR@(X),U,3) D W^BGP4DP($S(BGPPTYPE="P":$J(V,6,1),1:$$SB($J(V,6,1))),0,0,BGPPTYPE,X+1,T) S T=T+11
 I BGPPTYPE="D" D W^BGP4DP("# w/ 2 or more inhaled steroid Rx",0,2,BGPPTYPE)
 I BGPPTYPE="P" D W^BGP4DP("# w/ 2 or more inhaled",0,2,BGPPTYPE),W^BGP4DP(" steroid Rx",0,1,BGPPTYPE)
 S T=31 F X=1:1:4 S V=$P(@BGPARR@(X),U,4) D W^BGP4DP($S(BGPPTYPE="P":$$C(V,0,6),1:$S(V:V,1:0)),0,0,BGPPTYPE,X+1,T) S T=T+11
 I BGPPTYPE="D" D W^BGP4DP("% w/ 2 or more inhaled steroid Rx",0,1,BGPPTYPE)
 I BGPPTYPE="P" D W^BGP4DP("% w/ 2 or more inhaled",0,1,BGPPTYPE),W^BGP4DP(" steroid Rx",0,1,BGPPTYPE)
 ...
 .I BGPPTYPE="P" D W^BGP4DP("# w/ 2 or more inhaled",0,1,BGPPTYPE) S L=" steroid Rx",P=5 D PBY
...
AH ;EP
 Q:$G(BGPSUMON)
 I BGPPTYPE="D" D W^BGP4DP(BGPHD1,1,2,BGPPTYPE)
 I BGPPTYPE="P" D W^BGP4DP(BGPHD2,1,2,BGPPTYPE)
 I BGPPTYPE="P" D W^BGP4DP(BGPHD3,1,1,BGPPTYPE)
