...
I24 ;EP
 D H1^BGP4DPH
 F BGPPC1="24.1","24.2","24.3" Q:BGPQUIT  D PI^BGP4DP1
 Q:BGPQUIT
 ;I BGPRTYPE=4 D HEADER^BGP4DPH
 F BGPPC1="24.4" Q:BGPQUIT  D PI^BGP4DP1
 Q:BGPQUIT
 I BGPRTYPE=4 D HEADER^BGP4DPH
 F BGPPC1="24.5","24.6","24.7" Q:BGPQUIT  D PI^BGP4DP1
 Q:BGPQUIT
 D I1AGE
 Q
...
I1AGE1 ;
 ;gather up all #'s
 S C=C+1
 S BGPF="H."_BGPX_".1" S BGPPC=$O(^BGPINDFC("C",BGPF,0))
...
I1AGE2 ;
 S D=D+1
 S BGPF="H."_BGPX_".2" S BGPPC=$O(^BGPINDFC("C",BGPF,0))
...
I1AGE3 ;
 S BGPF="H."_BGPX_".3" S BGPPC=$O(^BGPINDFC("C",BGPF,0))
...
I1AGE4 ;
 S BGPF="H."_BGPX_".4" S BGPPC=$O(^BGPINDFC("C",BGPF,0))
...
I1AGE5 ;
 G I1AGE6
 S BGPF="H."_BGPX_".5" S BGPPC=$O(^BGPINDFC("C",BGPF,0))
...
I1AGE6 ;
 S BGPF="H."_BGPX_".6" S BGPPC=$O(^BGPINDFC("C",BGPF,0))
 ;set 12th piece to numerator and 13th to %
 S BGPNF=$P(^BGPINDFC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90244.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(D),U,12)=$$V(1,BGPRPT,N,P),$P(BGPDAC(D),U,13)=$S($P(BGPDAC(D),U,1):($P(BGPDAC(D),U,12)/$P(BGPDAC(D),U,1)*100),1:"")
 S $P(BGPDAP(D),U,12)=$$V(2,BGPRPT,N,P),$P(BGPDAP(D),U,13)=$S($P(BGPDAP(D),U,1):($P(BGPDAP(D),U,12)/$P(BGPDAP(D),U,1)*100),1:"")
 S $P(BGPDAB(D),U,12)=$$V(3,BGPRPT,N,P),$P(BGPDAB(D),U,13)=$S($P(BGPDAB(D),U,1):($P(BGPDAB(D),U,12)/$P(BGPDAB(D),U,1)*100),1:"")
 Q
I1AGEP ;
 D I1AGEP^BGP4DP50
 Q
