BGP4DPA ; IHS/CMI/LAB - COMP NATIONAL GPRA FOR PTS W/APPT 03 Jun 2014 2:54 PM ; 17 Apr 2014  11:15 AM
...
NT S BGPLIST="A"
 S BGPCPLC=0
 ;S XBRP="PRINT^BGP4DPAW",XBRC="PROC^BGP4DPA",XBRX="EOJ^BGP4DPA",XBNS="BGP"
 ;D ^XBDBQUE
 ;
 K IOP,%ZIS
 W !! S %ZIS="PQM" D ^%ZIS
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP4DPA",ZTDTH="",ZTDESC="CRS 14 SCHED REPORT" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D ^XBFMK
 K DIC,DIR,DFN
...
BDMG(BGPBD,BGPED,BGPCLN) ;EP - GUI DMS Entry Point
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BGP4DPA",ZTDESC="GUI CN GRPA APPT" D ^%ZTLOAD
 D EOJ
 Q
GUIEP ;EP
 D PROC
 K ^TMP($J,"BGP4DPA")
 S IOM=80
 D GUIR^XBLM("PRINT^BGP4DPA","^TMP($J,""BGP4DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP4DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP4DPA",X)
...
GUIECP ;EP
 K ^TMP($J,"BGP4DPA")
 S IOM=80
 D GUIR^XBLM("CPPRINT^BGP4DPA","^TMP($J,""BGP4DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP4DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP4DPA",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIJ(BGPIEN,11,X,0)=BGPDATA,C=C+1
...
PROC ;EP
 ...
 S ^XTMP("BGP4DPA",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^"_"PTS WITH GPRA DATA"
 K BGPAPPT,BGPAPPTS
 S BGPTA=0
 I BGPRT1="A" D  Q
...
ZISCP ;
 ...
 ;
 I $D(IO("Q")) G TSKMNCP
 ;S XBRP="CPPRINT^BGP4DPA",XBRC="",XBRX="EOJ^BGP4DPA",XBNS="BGP"
 ;D ^XBDBQUE
 ;
...
TSKMNCP ;EP ENTRY POINT FROM TASKMAN
 ...
 I $D(IOM)#2,IOM S ZTIO=ZTIO_";"_IOM I $D(IOSL)#2,IOSL S ZTIO=ZTIO_";"_IOSL
 K ZTSAVE S ZTSAVE("BGP*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="CPPRINT^BGP4DPA",ZTDTH="",ZTDESC="CRS 14 SCHED REPORT DENOM" D ^%ZTLOAD D EOJ Q
 Q
STMP ;EP
EN1 ;EP Help
 K BGPQUIT S BGPSTMP=""
...
F ;calendar year
 ...
 S DIR(0)="D^::EP"
 S DIR("A")="Run report for GPRA year 2014 or 2015"
 S DIR("?")="This report is compiled for a period.  Enter a valid date."
 D ^DIR KILL DIR
 I $D(DIRUT) Q
 I $D(DUOUT) S DIRUT=1 Q
 I Y'=3140000,Y'=3150000 W !,"Must be 2014 or 2015" G F
 S BGPVDT=Y
 I $E(Y,4,7)'="0000" W !!,"Please enter a year only!",! G F
 S BGPPER=BGPVDT
 I BGPPER="3150000" S BGPNGR09=1
 Q
