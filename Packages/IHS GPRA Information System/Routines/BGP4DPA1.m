BGP4DPA1 ; IHS/CMI/LAB - COMP NATIONAL GPRA FOR PTS W/APPT ;
...
ISS ;EP
 ...
 .K BGPNUMV
 .K ^TMP($J,"A")
 .S BGPGPRAI=0 F  S BGPGPRAI=$O(^BGPINDJC("B",BGPIC,BGPGPRAI)) Q:BGPGPRAI'=+BGPGPRAI  D
 ..Q:$P($G(^BGPINDJC(BGPGPRAI,12)),U,9)=""
 ..S (BGPNUM,BGPDEN)=0
 ..S BGPISSV="",BGPSKIP=0
 ..Q:'$D(^BGPINDJC(BGPGPRAI,4))
 ..X ^BGPINDJC(BGPGPRAI,4)
 ..S $P(BGPISSV,U,4)=$$TITLE2^BGP4DPA1(BGPGPRAI)
 ..Q:BGPSKIP
 ..S C=$S($P($G(^AUPNPAT(DFN,11)),U,18)]"":$P(^AUPNPAT(DFN,11),U,18),1:"UNKNOWN")
 ..S S=$P(^DPT(DFN,0),U,2)
 ..I BGPISST="A" D
 ...S ^XTMP("BGP4DPA",BGPGPRAJ,BGPGPRAH,"ANY",$P(^DPT(DFN,0),U,1),DFN,BGPIC,BGPGPRAI)=BGPISSV
 ..I BGPISST="C" D
 ...S X=BGPAPPTS(BGPSOX)
 ...S ^XTMP("BGP4DPA",BGPGPRAJ,BGPGPRAH,"APPTS",$P(^SC($P(X,U,2),0),U),$P($P(X,U,3),"."),$P(X,U,3),$P(^DPT(DFN,0),U),DFN,BGPIC,BGPGPRAI)=BGPISSV ;_"%%"_$G(BGPISSV(1))
 ..I BGPISST="P" D
 ...S X=BGPAPPTS(BGPSOX)
 ...S ^XTMP("BGP4DPA",BGPGPRAJ,BGPGPRAH,"PATS",$P(^DPT(DFN,0),U),DFN,$P($P(X,U,3),"."),$P(^SC($P(X,U,2),0),U),$P(X,U,3),BGPIC,BGPGPRAI)=BGPISSV ;_"%%"_$G(BGPISSV(1))
 ..I BGPISST="D" D
 ...S X=BGPAPPTS(BGPSOX)
 ...S ^XTMP("BGP4DPA",BGPGPRAJ,BGPGPRAH,"APPTS",BGPDIVI,$P(^SC($P(X,U,2),0),U),$P($P(X,U,3),"."),$P(X,U,3),$P(^DPT(DFN,0),U),DFN,BGPIC,BGPGPRAI)=BGPISSV ;_"%%"_$G(BGPISSV(1))
...
I003 ;EP - DM ideal glycemic control
 ...
 S BGPIN=""
 K BGPISSV
 S BGPV=$$HGBA1C^BGP4D2(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)
 I BGPV="" S BGPISSV=$P($G(^BGPINDJC(BGPGPRAI,12)),U,9)_U_"Last A1c: Never|Overdue as of: "_$$DATE^BGP4UTL(BGPBDATE) Q
 S BGPD=$P(BGPV,U,3)
 S BGPND=$$FMADD^XLFDT(BGPD,365)
 I BGPND'>BGPED S BGPIN=1
 S BGPND=$$DATE^BGP4UTL(BGPND)
 ...
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last A1c: "_$$DATE^BGP4UTL(BGPD)_" value: "_$P(BGPV,U,4)_" - "_BGPMET_"|"_$S(BGPIN:"Overdue as of: ",1:"Next Due: ")_BGPND
...
I004 ;EP - DM BP control
 ...
 S (BGPISSV,BGPIN)=""
 K BGPISSV
 S BGPV=$$MEANBP^BGP4D2(DFN,$$FMADD^XLFDT(BGPED,-365),BGPED)
 I BGPV="" S BGPV=$$BPCPT^BGP4D22(DFN,BGP365,BGPED) I $P(BGPV,U) S BGPV=U_2_U_$P(BGPV,U,2)
 ;
 I BGPV="" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Mean BPs: 2 BPs not documented "_$$DATE^BGP4UTL(BGPBD)_"-"_$$DATE^BGP4UTL(BGPED)_"|Overdue as of: "_$$DATE^BGP4UTL(BGPBD)  Q
...
I005 ;EP - DM LDL
 ...
 S BGPISSV="",BGPIN=""
 K BGPISSV
 S BGPVALUE=$$LDL^BGP4D2(DFN,$$DOB^AUPNPAT(DFN),BGPED)
 I BGPVALUE="" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last LDL: Never|Overdue as of: "_$$DATE^BGP4UTL(BGPBD) Q
 S BGPD=$P(BGPVALUE,U,2)
 S BGPND=$$FMADD^XLFDT(BGPD,365)
 I BGPND'>BGPED S BGPIN=1
 I 'BGPIN S BGPSKIP=1 Q  ;had LDL in time period
 S BGPND=$$DATE^BGP4UTL(BGPND)
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last LDL: "_$$DATE^BGP4UTL(BGPD)_"|"_"Overdue as of: "_BGPND
 Q
I006 ;EP - DM Nephropathy
 ...
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Est GFR: "_$S($P(BGPGFR,U,2)]"":$$DATE^BGP4UTL($P(BGPGFR,U,2)),1:"Never")_"|"_"Last UACR: "_$S($P(BGPQUP,U,3)]"":$$DATE^BGP4UTL($P(BGPQUP,U,3)),1:"Never")
 ...
 S BGPISSV=BGPISSV_"|Est GFR & UACR Overdue as of: "_$$DATE^BGP4UTL(BGPND)
I006X ;
...
I007 ;EP DM eye exam
 ...
 S BGPISSV="",BGPIN=""
 K BGPISSV
 S BGPVALUE=$$EYE^BGP4DPA4(DFN,$$DOB^AUPNPAT(DFN),BGPED,1)
 S BGPREF=""
 I BGPVALUE="",BGPREF="" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Evaluation: Never|Overdue as of: "_$$DATE^BGP4UTL(BGPBD) Q
 I BGPVALUE="",BGPREF]"" D  Q
 .S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Evaluation: Never|Patient refused on "_$$DATE^BGP4UTL($P(BGPREF,U,2))_"|Overdue as of: "_$$DATE^BGP4UTL(BGPBD) S BGPD=$P(BGPVALUE,U,2)
 S BGPD=$P(BGPVALUE,U,2)
 S BGPND=$$FMADD^XLFDT(BGPD,365)
 I BGPND'>BGPED S BGPIN=1
 I 'BGPIN S BGPSKIP=1 Q  ;not due this gpra year so don't display
 S BGPND=$$DATE^BGP4UTL(BGPND)
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Evaluation: "_$$DATE^BGP4UTL(BGPD)
 ;I BGPREF]"" S BGPISSV=BGPISSV_"|Patient refused on "_$$DATE^BGP4UTL($P(BGPREF,U,2))
...
I009 ;EP DENTAL EXAM
 ...
 S BGPISSV="",BGPIN=""
 K BGPISSV
 S BGPVALUE=$$DENTSRV^BGP4D21(DFN,$$DOB^AUPNPAT(DFN),BGPED,1)
 I BGPVALUE="" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Dental Exam: Never|Overdue as of: "_$$DATE^BGP4UTL(BGPBD) Q
 S BGPD=$P(BGPVALUE,U,2)
 S BGPND=$$FMADD^XLFDT(BGPD,365)
 I BGPND'>BGPED S BGPIN=1
 I 'BGPIN S BGPSKIP=1 Q  ;not due this rp
 S BGPND=$$DATE^BGP4UTL(BGPND)
 S BGPREF=$$DENTSRV^BGP4D21(DFN,BGPBD,BGPED,1)
 ;I BGPREF["Ref" S BGPSKIP=1 Q
 ;I BGPVALUE["Refused"  S BGPSKIP=1 Q   ;S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Dental Exam: patient refused on "_$$DATE^BGP4UTL($P(BGPVALUE,U,2))_"|Overdue as of: "_$$DATE^BGP4UTL(BGPBD) Q
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Dental Exam: "_$$DATE^BGP4UTL(BGPD)_"|Overdue as of: "_BGPND
...
I012 ;EP - FLU
 ...
 S BGPISSV="",BGPIN=""
 K BGPISSV
 S BGPVALUE=$$FLU^BGP4D3(DFN,BGPBD,BGPED,1)
 I BGPVALUE]"",$P(BGPVALUE,U,3)'=2 S BGPSKIP=1 Q
 S BGPVALUE=$$FLU^BGP4D3(DFN,$$DOB^AUPNPAT(DFN),BGPED,1)
 I BGPVALUE="" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last IZ: Never|Overdue as of: "_$$DATE^BGP4UTL(BGPBD) Q
 S BGPD=$P(BGPVALUE,U,1)
 S BGPND=$$FMADD^XLFDT(BGPD,365)
 S BGPND=$$DATE^BGP4UTL(BGPND)
 I BGPVALUE["Refus" S BGPIN=1 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last IZ: patient refused on "_$$DATE^BGP4UTL($P(BGPVALUE,U,1))_"|Overdue as of: "_$$DATE^BGP4UTL(BGPBD) Q
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last IZ: "_$$DATE^BGP4UTL(BGPD)_"|Overdue as of: "_BGPND
 Q
I013 ;EP - PNEU
 ;get last date and value for patient DFN
 ;date of last^value of last^next date due
 ...
 S BGPISSV="",BGPIN=""
 K BGPISSV
 S BGPVALUE=$$PNEU^BGP4D31(DFN,$$DOB^AUPNPAT(DFN),BGPED,1)
 I BGPVALUE="" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last IZ: Never|Overdue as of: "_$$DATE^BGP4UTL(BGPBD) Q
 S BGPD=$P(BGPVALUE,U,1)
 S BGPND=DT
 ...
 S BGPA65="" S BGPA65=$$PNEU^BGP4D31(DFN,$$FMADD^XLFDT($$DOB^AUPNPAT(DFN),+(65*365)),BGPEDATE) I $P(BGPA65,U,3)=1!($P(BGPA65,U,3)=3) S BGPIN=0
...
I015 ;EP - PAP
 ...
 S BGPI7=$$DEN7^BGP4D3(DFN,BGPAGEB,BGPAGEE,BGPSEX,BGPEDATE)
 ...
 S BGPVALUE=$$PAP^BGP4D3(DFN,BGPEDATE,$$AGE^AUPNPAT(DFN),1)
 S BGPREF=""  ;no refusals
 I BGPVALUE="" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last PAP: Never|Overdue as of: "_$$DATE^BGP4UTL(BGPBD) D I015X Q  ; as of "_$$DATE^BGP4UTL(DT) Q
 S BGPD=$P(BGPVALUE,U,2)
 ...
 I BGPVALUE="",BGPREF]"" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Pap Smear: Never - patient refused on "_$$DATE^BGP4UTL($P(BGPVALUE,U,2))_"|Overdue as of: "_$$DATE^BGP4UTL(BGPBD) D I015X Q
 S BGPND=$$DATE^BGP4UTL(BGPND)
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Pap: "_$$DATE^BGP4UTL(BGPD)_"|Overdue as of: "_BGPND_$S(BGPREF]"":"|Patient refused a Pap Smear on "_$$DATE^BGP4UTL($P(BGPREF,U,2)),1:"")
I015X ;
...
I016 ;EP -MAMMOGRAM
 ...
 S BGPI8=$$DEN8^BGP4D4(DFN,BGPAGEB,BGPAGEE,BGPSEX,BGPEDATE)
 I 'BGPI8 S BGPSKIP=1 D I016X Q
 S BGPVALUE=$$MAM^BGP4D4(DFN,BGPEDATE,15,1)
 S BGPREF=""
 I BGPVALUE="" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Mammogram: Never|Overdue as of: "_$$DATE^BGP4UTL(BGPBD) D I016X Q  ; as of "_$$DATE^BGP4UTL(DT) Q
 S BGPD=$P(BGPVALUE,U,2)
 S BGPND=$S(BGPD="":DT,1:$$FMADD^XLFDT(BGPD,(2*365)))
 I BGPND]"",BGPND'>BGPED S BGPIN=1
 I 'BGPIN S BGPSKIP=1 D I015X Q  ;not due, don't display
 I BGPVALUE="",BGPREF]"" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Pap Smear: Never - patient refused on "_$$DATE^BGP4UTL($P(BGPVALUE,U,2))_"|Overdue as of: "_$$DATE^BGP4UTL(BGPBD) D I016X Q
 S BGPND=$$DATE^BGP4UTL(BGPND)
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Mammogram: "_$$DATE^BGP4UTL(BGPD)_"|Overdue as of: "_BGPND_$S(BGPREF]"":"|Patient refused a Mammogram on "_$$DATE^BGP4UTL($P(BGPREF,U,2)),1:"")
 ;
I016X ;
 K BGPN1,BGPN2,BGPN3,BGPN4,BGPVALUE,BGPPAP,BGPI7,BGPI7DA,BGPI7DB
 K BGPN5,BGPN6,BGPN7,BGPN8,BGPN9,BGPD1,BGPD2,BGPD3,BGPD4,BGPD5,BGPD6,BGPD7,BGPD8,BGPD9,BGPN10,BGPN11,BGPN12,BGPN13,BGPN14,BGPN15,BGPN16,BGPN17,BGPN18,BGPN19,BGPN20,BGPN21,BGPN22,BGPN23,BGPN24,BGPN25,BGPN26,BGPN27
...
I031A ;
 Q
TITLE(I) ;
 Q $P($G(^BGPINDJC(I,12)),U,9)
TITLE2(I) ;EP
 Q $P($G(^BGPINDJC(I,12)),U,16)
