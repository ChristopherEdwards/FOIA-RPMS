BGP4DPA9 ; IHS/CMI/LAB - COMP NATIONAL GPRA FOR PTS W/APPT 03 Jun 2014 2:54 PM ; 09 Sep 2014  10:30 AM
...
EP ;EP - called from option interactive
 ...
 W:$D(IOF) @IOF
 D TERM^VALM0
 S BGPCTRL=$O(^BGPCTRL("B",2014,0))
 W !!,"This is the GPRA forecast patient list for the 2015 GPRA year using the"
 W !,"same performance measure definitions from CRS Version 14.0."
 W !,"This patient list is linked to the Scheduling Menu and enables users to"
 W !,"run a list of patients that are scheduled for appointments during a"
 W !,"user-defined time period to list of clinics at the facility defined by"
 W !,"the user and shows the GPRA measures the patient will not meet as of the"
 W !,"date of the appointment.  The list uses revised CRS logic for the GPRA"
 W !,"measures, which is defined in the report, and also includes information"
 W !,"for the provider on how to fulfill the GPRA measure."
 ...
 I 'X W !!,"Exiting Report....." D PAUSE^BGP4DU,EOJ Q
RTYPE ;
 S BGPRT1=""
...
ADDON ;
 ...
 ;
ZIS ;
 S BGPRTYPE=1,BGPYRPTH="",BGPCPPL=1,BGPINDJ="G",BGPYGPU=1,BGPALLPT=1,BGPBEN=3
...
ENDDATE ;
 ...
 S BGPPBD=3130701,BGPPED=3140630
 S BGPPER=3130000
 S BGPNGR09=1
 S BGPLIST="A"
 S BGPCPLC=0
...
NT S BGPLIST="A"
 S BGPCPLC=0
 ;S XBRP="PRINT^BGP4DPAW",XBRC="PROC^BGP4DPA9",XBRX="EOJ^BGP4DPA9",XBNS="BGP"
 ;D ^XBDBQUE
 ;
 K IOP,%ZIS
 W !! S %ZIS="PQM" D ^%ZIS
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP4DPA9",ZTDTH="",ZTDESC="CRS 14 SCHED REPORT" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D ^XBFMK
 K DIC,DIR,DFN
...
SELPT ;
 ...
 .S DIR("?",5)="",DIR("?")="     ""[??"" will list your templates.",DIR("??")="^D LIST^BGP4DPAP"
 .D ^DIR K DIR
 .S:X[U X=U
 .I $E(X)="[" D  Q
 .. S X=$E(X,2,$L(X))
...
BDMG(BGPBD,BGPED,BGPCLN) ;EP - GUI DMS Entry Point
 ...
 S BGPRTYPE=1,BGPYRPTH="",BGPCPPL=1,BGPINDJ="G",BGPYGPU=1
 D NOW^%DTC
 S BGPNOW=$G(%)
 K DD,D0,DIC
 S X=DUZ_$$NOW^XLFDT
 S DIC("DR")=".02////"_DUZ_";.03////"_BGPNOW_";.06///"_$G(BGPOPT)_";.07////R"
 S DIC="^BGPGUIJ(",DIC(0)="L",DIADD=1,DLAYGO=9001004.4
 D FILE^DICN
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BGP4DPA9",ZTDESC="GUI CN GRPA APPT" D ^%ZTLOAD
 D EOJ
 Q
GUIEP ;EP
 D PROC
 K ^TMP($J,"BGP4DPA9")
 S IOM=80
 D GUIR^XBLM("PRINT^BGP4DPA9","^TMP($J,""BGP4DPA9"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP4DPA9",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP4DPA9",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIJ(BGPIEN,11,X,0)=BGPDATA,C=C+1
 S ^BGPGUIJ(BGPIEN,11,0)="^^"_C_"^"_C_"^"_DT_"^"
 S DA=BGPIEN,DIK="^BGPGUIJ(" D IX1^DIK
 D ENDLOG
 K ^TMP($J,"BGP4DPA9")
 S ZTREQ="@"
 Q
...
GUIECP ;EP
 K ^TMP($J,"BGP4DPA9")
 S IOM=80
 D GUIR^XBLM("CPPRINT^BGP4DPA9","^TMP($J,""BGP4DPA9"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP4DPA9",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP4DPA9",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIJ(BGPIEN,11,X,0)=BGPDATA,C=C+1
...
ENDLOG ;-- write the end of the log
 D NOW^%DTC
 S BGPNOW=$G(%)
 S DIE="^BGPGUIJ(",DA=BGPIEN,DR=".04////"_BGPNOW_";.06////C"
 D ^DIE
 K DIE,DR,DA
 Q
 ;
PROC ;EP
 ...
 K ^XTMP("BGP4DPA9",BGPGPRAJ,BGPGPRAH)
 S ^XTMP("BGP4DPA9",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^"_"PTS WITH GPRA DATA"
 K BGPAPPT,BGPAPPTS
 S BGPTA=0
 I BGPRT1="A" D  Q
 ...
 ..S BGPIISO=1,BGPISST="A" D PROCCY^BGP4D1
 .Q
 ...
 .S X=$O(^DIBT("B","RPMS DEMO PATIENT NAMES",0)) I X Q:$D(^DIBT(X,1,DFN))
 .I $G(BGPSTMP) S ^DIBT(BGPSTMP,1,DFN)="" Q
 .S BGPIISO=1,BGPISST=BGPRT1,BGPISSO=1 D PROCCY^BGP4D1
 .Q
 Q
...
ZISCP ;
 ...
 ;
 I $D(IO("Q")) G TSKMNCP
 ;S XBRP="CPPRINT^BGP4DPA9",XBRC="",XBRX="EOJ^BGP4DPA9",XBNS="BGP"
 ;D ^XBDBQUE
 ;
CPPRINT ;EP - called from xbdbque
 ...
 D CPHEADER
 S BGPGYR=2014,BGPGYR=$O(^BGPCTRL("B",BGPGYR,0))
...
TSKMNCP ;EP ENTRY POINT FROM TASKMAN
 ...
 I $D(IOM)#2,IOM S ZTIO=ZTIO_";"_IOM I $D(IOSL)#2,IOSL S ZTIO=ZTIO_";"_IOSL
 K ZTSAVE S ZTSAVE("BGP*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="CPPRINT^BGP4DPA9",ZTDTH="",ZTDESC="CRS 14 SCHED REPORT DENOM" D ^%ZTLOAD D EOJ Q
 Q
STMP ;EP
EN1 ;EP Help
 K BGPQUIT S BGPSTMP=""
...
DUP I '$P(Y,U,3) D  I Q K BGPSTMP,Y G EN2
 ...
 .K DIE,DA,DR
 Q
POST ;EP - called from patch 1 post init
 NEW X
 S X=$$ADD^XPDMENU("BGP 14 MENU NATIONAL","BGP 14 FORECAST FOR 2014","FO12",34)
 I 'X D MES^XPDUTL("Adding Forecast 2014 report to menu failed.",80)
 Q
PRE ;EP - CALLED FROM PATCH 1 PRE INIT.
 NEW BGPX,DA,BGPX
