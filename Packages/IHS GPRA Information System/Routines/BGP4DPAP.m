...
ENDDATE ;
 ...
 ;S BGPPER=""
 ;K DIR S DIR(0)="DO^::EXP",DIR("A")="Enter End Date of the Report Period",DIR("B")="June 30, 2014"
 ...
 ;I $G(BGPQUIT) D EOJ Q
 ;I BGPRPT="" D EOJ Q
 S XBRP="PRINT^BGP4DPAP",XBRC="PROC^BGP4DPAP",XBRX="EOJ^BGP4DPAP",XBNS="BGP"
 D ^XBDBQUE
 Q
EOJ ;
 D ^XBFMK
...
GUIEP ;EP
 ...
 K ^TMP($J,"BGP4DPAP")
 S IOM=80
 D GUIR^XBLM("PRINT^BGP4DPA","^TMP($J,""BGP4DPAP"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP4DPAP",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP4DPAP",X)
...
ENDLOG ;-- write the end of the log
 D NOW^%DTC
 S BGPNOW=$G(%)
 S DIE="^BGPGUIJ(",DA=BGPIEN,DR=".04////"_BGPNOW_";.07////C"
 D ^DIE
 K DIE,DR,DA
 Q
 ;
PROC ;
 ...
 K ^XTMP("BGP4DPAP",BGPGPRAJ,BGPGPRAH)
 S ^XTMP("BGP4DPAP",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^"_"PTS WITH AN APPT"
 K BGPAPPT,BGPAPPTS
 S BGPSOX=0 F  S BGPSOX=$O(BGPSPAT(BGPSOX)) Q:BGPSOX'=+BGPSOX  D
 .S DFN=BGPSPAT(BGPSOX)
 .S BGPIISO=1,BGPISST="P" D PROCCY^BGP4D1
 .Q
 Q
DONE ;
 K DIR
...
PRINT ;EP - called from xbdbque
 ...
 I '$D(^XTMP("BGP4DPAP",BGPGPRAJ,BGPGPRAH,"PATS")) D HEADER S BGPNOD=1 W !!,"NO DATA TO REPORT",! G DONE
 D HEADER
 S BGPNAME=0 F  S BGPNAME=$O(^XTMP("BGP4DPAP",BGPGPRAJ,BGPGPRAH,"PATS",BGPNAME)) Q:BGPNAME=""!($D(BGPQ))  D
 .S DFN=0 F  S DFN=$O(^XTMP("BGP4DPAP",BGPGPRAJ,BGPGPRAH,"PATS",BGPNAME,DFN)) Q:DFN'=+DFN!($D(BGPQ))  D
 ..I $Y>(BGPIOSL-8) D HEADER Q:$D(BGPQ)
 ..W !!,$$HRN^AUPNPAT(DFN,DUZ(2)),?7,$E($P(^DPT(DFN,0),U),1,25),?38,$P(^DPT(DFN,0),U,2),?42,$$DATE^BGP4UTL($P(^DPT(DFN,0),U,3)),?51,$$COMMRES^AUPNPAT(DFN,"E")
 ..S BGPI=0 F  S BGPI=$O(^XTMP("BGP4DPAP",BGPGPRAJ,BGPGPRAH,"PATS",BGPNAME,DFN,BGPI)) Q:BGPI'=+BGPI!($D(BGPQ))  D
 ...I $Y>(IOSL-3) D HEADER Q:$D(BGPQ)
 ...S BGPT=$P(^XTMP("BGP4DPAP",BGPGPRAJ,BGPGPRAH,"PATS",BGPNAME,DFN,BGPI),U)
 ...S BGPT1=$P(^XTMP("BGP4DPAP",BGPGPRAJ,BGPGPRAH,"PATS",BGPNAME,DFN,BGPI),U,4)
 ...S BGPX=$P(^XTMP("BGP4DPAP",BGPGPRAJ,BGPGPRAH,"PATS",BGPNAME,DFN,BGPI),U,2)
 ...S BGPX1=$P(^XTMP("BGP4DPAP",BGPGPRAJ,BGPGPRAH,"PATS",BGPNAME,DFN,BGPI),U,3)
 ...W !?1,BGPT
