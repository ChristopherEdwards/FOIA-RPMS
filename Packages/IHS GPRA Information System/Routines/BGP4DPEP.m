BGP4DPEP ; IHS/CMI/LAB - IHS gpra print 01 Jul 2010 7:54 PM ;
...
WP ;
 K ^UTILITY($J,"W")
 S BGPZ=0,BGPLCNT=0
 S DIWL=1,DIWR=80,DIWF="",BGPZ=0 F  S BGPZ=$O(^BGPPEIJ(BGPIC,BGPNODE,BGPY,11,BGPZ)) Q:BGPZ'=+BGPZ  D
 .S BGPLCNT=BGPLCNT+1
 .S X=^BGPPEIJ(BGPIC,BGPNODE,BGPY,11,BGPZ,0) S:BGPLCNT=1 X=" - "_X D ^DIWP
 .Q
WPS ;
 S Z=0 F  S Z=$O(^UTILITY($J,"W",DIWL,Z)) Q:Z'=+Z  D
 .I $Y>(BGPIOSL-3) D HEADER^BGP4DPH Q:BGPQUIT
...
PRINT1 ;EP
 ...
 .W !!,$P(^BGPPEIJ(BGPIC,0),U,2)
 .I $G(BGPDNT) G CALC
 .I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 .W !!,"Denominator(s):"
 .S BGPNODE=61
 .S BGPX=0 F  S BGPX=$O(^BGPPEIJ(BGPIC,61,"B",BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIJ(BGPIC,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY!(BGPQUIT)  D
 ...D WP
 .I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 .W !!,"Numerator(s):"
 .S BGPNODE=62
 .S BGPX=0 F  S BGPX=$O(^BGPPEIJ(BGPIC,62,"B",BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIJ(BGPIC,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY!(BGPQUIT)  D
 ...D WP
 .S BGPNODE=11
 .W ! S BGPX=0 F  S BGPX=$O(^BGPPEIJ(BGPIC,BGPNODE,BGPX)) Q:BGPX'=+BGPX  D
 ..I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ..W !,^BGPPEIJ(BGPIC,BGPNODE,BGPX,0)
CALC .D @BGPIC
 .Q:BGPQUIT
 ;
 D ^BGP4DPEL
 ...
 Q
1 ;
 I $Y>(BGPIOSL-6) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIJ(BGPIC,0),U,2)
 D H1^BGP4DPH
...
2 ;
 ...
 ..I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIJ(BGPIC,0),U,2) D H1^BGP4DPH W !
 ...
 I $Y>(BGPIOSL-6) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIJ(BGPIC,0),U,2) D H1^BGP4DPH W !
 ...
 ..I $P($G(^BGPPEDCJ(X,11)),U,4)]"",$P($G(^BGPPEDCJ(X,11)),U,4)<BGPCYD S BGPCYD=$P($G(^BGPPEDCJ(X,11)),U,4)
 ..I $P($G(^BGPPEDPJ(X,11)),U,4)]"",$P($G(^BGPPEDPJ(X,11)),U,4)<BGPPRD S BGPPRD=$P($G(^BGPPEDPJ(X,11)),U,4)
 ..I $P($G(^BGPPEDBJ(X,11)),U,4)]"",$P($G(^BGPPEDBJ(X,11)),U,4)<BGPBLD S BGPBLD=$P($G(^BGPPEDBJ(X,11)),U,4)
 ...
 I $Y>(BGPIOSL-4) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIJ(BGPIC,0),U,2) D H1^BGP4DPH W !
 W !,"Minimum Time Spent",!,"All Providers",!,"(minutes)"
 W ?20,$$C(BGPCYD,0,8),?35,$$C(BGPPRD,0,8),?58,$$C(BGPBLD,0,8),!
 I $G(BGPAREAA) D
 .S (BGPCYD,BGPPRD,BGPBLD)=""
 .S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 ..I $P($G(^BGPPEDCJ(X,11)),U,5)>BGPCYD S BGPCYD=$P($G(^BGPPEDCJ(X,11)),U,5)
 ..I $P($G(^BGPPEDPJ(X,11)),U,5)>BGPPRD S BGPPRD=$P($G(^BGPPEDPJ(X,11)),U,5)
 ..I $P($G(^BGPPEDBJ(X,11)),U,5)>BGPBLD S BGPBLD=$P($G(^BGPPEDBJ(X,11)),U,5)
 I '$G(BGPAREAA) D
...
HEADER1 ;
 ...
 I '$G(BGPSEAT) W $$CTR("*** IHS 2014 Patient Education with Community Specified Report ***",80),!
 I $G(BGPSEAT) W $$CTR("*** IHS 2014 Patient Education with Patient Panel Population Report ***",80),!
...
SETEXA(T,N,P) ;EP - set denominator
 ...
 NEW X,Y,Z
 S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 .I T=1 S $P(BGPEXCT(X,T),U,2)=$P($G(^BGPPEDCJ(X,N)),U,P)
 .I T=2 S $P(BGPEXCT(X,T),U,2)=$P($G(^BGPPEDPJ(X,N)),U,P)
