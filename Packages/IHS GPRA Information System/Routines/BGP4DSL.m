...
EN ;EP -- main entry point for GPRA LIST DISPLAY
 D EN^VALM("BGP 14 LIST SELECTION")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ1
 ...
 ;
HDR ; -- header code
 S VALMHDR(1)="IHS 2014 Clinical Performance Measure Lists of Patients"
 S VALMHDR(2)="* indicates the list has been selected"
 Q
 ;
INIT ; -- init variables and list array
 K BGPGLIST,BGPNOLI S BGPHIGH=""
 S (X,C,I,O)=0 F  S O=$O(^BGPINDJ("AOI",O)) Q:O'=+O  S X=$O(^BGPINDJ("AOI",O,0)) I $D(BGPIND(X)) D
 .I $P(^BGPINDJ(X,0),U,5)]"" S C=C+1 D  Q
 ..S BGPGLIST(C,0)=C_")",$E(BGPGLIST(C,0),5)=$P(^BGPINDJ(X,0),U,5),BGPGLIST("IDX",C,C)=X I $D(BGPLIST(X)) S BGPGLIST(C,0)="*"_BGPGLIST(C,0)
 .I $P(^BGPINDJ(X,0),U,5)="" S C=C+1 D
 ..S BGPGLIST(C,0)=$P(^BGPINDJ(X,0),U,4)_" NO patient list available for measure:  ",BGPGLIST("IDX",C,C)=X,BGPNOLI(X)="" I $D(BGPLIST(X)) S BGPGLIST(C,0)="*"_BGPGLIST(C,0)
 S (VALMCNT,BGPHIGH)=C
 Q
 ;
HELP ; -- help code
