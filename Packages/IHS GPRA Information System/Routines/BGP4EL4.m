BGP4EL4 ; IHS/CMI/LAB - measure 1,2,3,4 05 Apr 2010 1:44 PM ;
...
PALLCARE ;EP 
 ...
 I BGPAGEB>74,BGPAGEB<85 S BGPD4=1
 I BGPAGEB>84 S BGPD5=1
 S BGPVAL=$$PCV^BGP4D24(DFN,BGPBDATE,BGPEDATE)  ;return #visits^list string
 S BGPN1=$P(BGPVAL,U)
 ;S BGPCANCE=$$CANCER^BGP4D24(DFN,BGPBDATE,BGPEDATE)
...
FUNCTION(P,BDATE,EDATE) ;EP
 ...
 .S $P(BGPQ,U,2)=$P(BGPQ,U,2)_$$DATE^BGP4UTL((9999999-Y))_" "_BGPV(Y)_" "
 K BGPV,BGPG
 Q BGPQ
 ;
...
TXBMD(P,BDATE,EDATE,HOSP) ;EP
 ...
 S HOSP=$G(HOSP)
 K BGPG
 S BGPG=$$LASTPRC^BGP4UTL1(P,"BGP BMD PROCEDURES",BDATE,EDATE)
 I $P(BGPG,U)=1 Q 1_U_$$DATE^BGP4UTL($P(BGPG,U,3))_" PROC "_$P(BGPG,U,2)
 ;now check dx
 S BGPG=$$LASTDX^BGP4UTL1(P,"BGP BMD DXS",BDATE,EDATE)
 I $P(BGPG,U)=1 Q 1_U_$$DATE^BGP4UTL($P(BGPG,U,3))_"DX "_$P(BGPG,U,2)
 ;now check cpts
 S T=$O(^ATXAX("B","BGP BMD CPTS",0))
 S BGPG=$$CPT^BGP4DU(P,BDATE,EDATE,T,5)
 I BGPG]"" Q 1_U_$$DATE^BGP4UTL($P(BGPG,U,1))_" CPT "_$P(BGPG,U,2)
 S T=$O(^ATXAX("B","BGP BMD CPTS",0))
 S BGPG=$$TRAN^BGP4DU(P,BDATE,EDATE,T,5)
 I BGPG]"" Q 1_U_$$DATE^BGP4UTL($P(BGPG,U,1))_" CPT/TRAN "_$P(BGPG,U,2)
 ;now check RAD
 S T=$O(^ATXAX("B","BGP BMD CPTS",0))
 S BGPG=$$RAD^BGP4DU(P,BDATE,EDATE,T,5)
 I BGPG]"" Q 1_U_$$DATE^BGP4UTL($P(BGPG,U,1))_" RAD/CPT "_$P(BGPG,U,2)
 I HOSP Q ""
 ;now check all meds
 K ^TMP($J,"MEDS")
 S G=0 K BGPZ
 ...
 .Q
 K ^TMP($J,"MEDS")
 I G Q 1_U_$$DATE^BGP4UTL($P(G,U,1))_" Med "_$P(G,U,2)
 Q ""
