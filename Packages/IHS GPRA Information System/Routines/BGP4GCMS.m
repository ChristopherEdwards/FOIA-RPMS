BGP4GCMS ; IHS/CMI/LAB - GUI CMS REPORT 27 Apr 2010 10:30 PM ;
...
TESTNTL ;
 S ERR=""
 F X=1:1:3 S BGPND(X)=""
 D EP(.ERR,1,2522,"BGP 14 CMS PERF RPT",2522,.BGPND,1,3020000,$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPHOSP,BGPIND,BGPPLSTL,BGPQTR,BGPPER,BGPRTIME,BGPUDBD,BGPED,BGPBEN,BGPTEXD,BGPFILE) ;EP - called from GUI to produce COM REPORT CI05-RPT-LOC-CMS
 ; SEE ROUTINE BGP4DC if you have questions about any of these variables
 ;  BGPUSER - DUZ
 ;  BGPDUZ2 - DUZ(2)
 ;  BGPOPTN - OPTION NAME
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LOCCMS^BGP4GCMS",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI 09 CMS LIST" D ^%ZTLOAD Q
 Q
LOCCMS ;
 ;D ^BGP4C1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^BGPXBLM("^BGP4CP","^TMP($J,""BGPGUI"",")
