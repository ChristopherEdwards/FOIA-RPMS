BGP4GCOM ; IHS/CMI/LAB - GUI COM REPORT ;
...
TESTNTL ;
 ...
 S BGPND(4)="",BGPND(3)=""
 S BGPLIST(3)=""
 D EP(.ERR,1,2522,"BGP 14 SELECTED IND REPORT",338,"S",.BGPND,1,3040000,3000000,1,"A",.BGPLIST,"","","B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPINDJ,BGPIND,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPLSTT,BGPLIST,BGPLPRV,BGPLPROV,BGPROT,BGPRTIME,BGPMFITI,BGPFILE,BGPDNT) ;EP - called from GUI to produce COM REPORT CI05-RPT-LOC-COM
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LOCCOM^BGP4GCOM",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT COM 05" D ^%ZTLOAD Q
 Q
LOCCOM ;
 D ^BGP4D1
 K ^TMP($J,"BGPGUI")
