BGP4GFO9 ; IHS/CMI/LAB - GUI Patient Forecast Report ;
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="GUIP^BGP4GFO9",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI FORECAST 09 FOR 09" D ^%ZTLOAD Q
 Q
GUIP ;
 D PROC^BGP4DPA9
 K ^TMP($J,"BGP4DPA")
 S IOM=80,BGPIOSL=55
 D GUIR^BGPXBLM("PRINT^BGP4DPAW","^TMP($J,""BGP4DPA"",")
 ;cmi/anch/maw added 5/12/2009 for word output
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP4DPA",X)) Q:X'=+X  D
 . S C=C+1
 . N BGPDATA
 . S BGPDATA=$G(^TMP($J,"BGP4DPA",X))
