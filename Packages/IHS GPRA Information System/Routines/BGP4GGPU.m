BGP4GGPU ; IHS/CMI/LAB - IHS Diabetes Audit 2003 ;
 ;;14.0;IHS CLINICAL REPORTING;;NOV 14, 2013;Build 101
 ;
 ;
TESTNTL ;
 S ERR=""
 D EP(.ERR,1,2522,"BGP 14 GPRA PERF REPORT",338,1,3040000,3000000,1,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPEXPT,BGPROT,BGPRTIME,BGPMFITI,BGPFILE,BGPYNPN,BGPPN,BGPFP,BGPDP,BGPDNT) ;EP - called from GUI to produce national GPU report (OTH-GPU)
 ;  BGPUSER - DUZ
 ;  BGPDUZ2 - DUZ(2)
 ;  BGPOPTN - OPTION NAME
...
EP1 ;
 ...
 S BGPFILEN="" I BGPEXPT S BGPFILEN="BG140"_$P(^AUTTLOC(DUZ(2),0),U,10)_"."_BGPRPT_"  in directory "_BGPUF
 S BGPDELT=""
 ...
 S BGPGFNM=X
 S DIC="^BGPGUIJ(",DIC(0)="L",DIADD=1,DLAYGO=90552.19,DIC("DR")=".02////"_BGPUSER_";.03////"_$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT)_";.05///"_BGPOPTN_";.06///R;.07///"_$G(BGPROT)_";1///"_$G(BGPFILEN)
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="OTHGPU^BGP4GGPU",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI GPU GPRA REPORT 06" D ^%ZTLOAD Q
 Q
OTHGPU ;
 I $G(BGPSEAT) D ^BGP4D10
 I '$G(BGPSEAT) D ^BGP4D1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
