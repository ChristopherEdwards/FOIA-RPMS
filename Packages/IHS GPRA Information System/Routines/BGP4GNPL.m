BGP4GNPL ; IHS/CMI/LAB - IHS Diabetes Audit 2003 26 Mar 2010 5:09 PM ;
...
TESTNTL ;
 ...
 D EP(.ERR,1,2522,"BGP 14 NATIONAL PAT LISTS",338,.BGPND,.BGPINDL,1,3040000,"A","","","B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPIND,BGPINDL,BGPQTR,BGPPER,BGPLIST,BGPLPRV,BGPLPROV,BGPROT,BGPRTIME,BGPMFITI,BGPVDT,BGPBEN,BGPFILE,BGPDNT) ;EP - called from GUI to produce national gpra report (NTL-GP)
 ; SEE ROUTINE BGP4NPL if you have questions about any of these variables
 ;  BGPUSER - DUZ
 ;  BGPDUZ2 - DUZ(2)
 ;  BGPOPTN - OPTION NAME
 ...
 ;                        ^BGPINDJ("AGPRA",1,ien) xref as the ones with a second
 ...
 ;            entries from BGP 14 NATIONAL PATIENT LISTS that point to that
 ;            measure by using the "B" index on ^BGPNPLJ(.  ^BGPNPLJ("B",measure ien,ien)
...
EP1 ;
 ...
 S IOM=80,BGPIOSL=55
 S BGPRTYPE=1,BGPYRPTH="",BGPNPL=1,BGPINDJ="G",BGPYGPU=1  ;maw orig 3/11/2010
 ;I BGPOPTN="CRS 14 OTHER NATIONAL MEASURES PAT LISTS" D  ;maw orig 3/11/2010
 ;. S BGPRTYPE=7,BGPYRPTH="",BGPNPL=1,BGPINDJ="G",BGPYGPU=1,BGPONMR=1  ;maw new 3/11/2010
 I BGPOPTN="CRS 14 OTHER NATIONAL MEASURES PAT LISTS" D  ;maw orig 3/11/2010
 . S BGPRTYPE=7,BGPYRPTH="",BGPNPL=1,BGPINDJ="G",BGPYGPU=1,BGPONMR=1,BGPRTC="U"  ;maw new 6/9/2014
 K BGPTAX S X=0
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="NTLGP^BGP4GNPL",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT LISTS 09" D ^%ZTLOAD Q
 Q
NTLGP ;
 D ^BGP4D1
 K ^TMP($J,"BGPGUI")
