BGP4GPED ; IHS/CMI/LAB - GUI COM REPORT ;
...
TESTELD ;
 ...
 S BGPND(4)="",BGPND(3)=""
 S BGPLIST(3)=""
 D EP(.ERR,1,2522,"BGP 14 PAT ED REPORT",338,"S",.BGPND,1,3040000,3000000,1,"A",.BGPLIST,"","",1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPZZ,BGPIND,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPLSTT,BGPLIST,BGPLPRV,BGPLPROV,BGPEXPT,BGPROT,BGPRTIME,BGPMFITI,BGPFILE,BGPSEAT,BGPBEGIN,BGPDNT) ;EP - called from GUI to produce COM REPORT CI05-RPT-LOC-COM
 ; SEE ROUTINE BGP4DEL if you have questions about any of these variables
 ; TAXONOMY CHECK IS IN ^BGP4XTEL
 ;  BGPUSER - DUZ
 ;  BGPDUZ2 - DUZ(2)
 ;  BGPOPTN - OPTION NAME
 ...
 ;                        1 and 6 from the BGP 14 ELDER INDICATORS file (^BGPELIJ GLOBAL).  When
...
BY ;get baseline year
 ...
 S BGPFILEN=""
 I BGPEXPT S BGPFILEN="BG140"_$P(^AUTTLOC(DUZ(2),0),U,10)_".PED"_BGPRPT_" in directory "_BGPUF
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="OTHPED^BGP4GPED",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI PAT ED REPORT" D ^%ZTLOAD Q
 Q
OTHPED ;
 D ^BGP4DPE1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^BGPXBLM("^BGP4DPEP","^TMP($J,""BGPGUI"",")
