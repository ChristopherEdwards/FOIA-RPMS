BGP4GU ; IHS/CMI/LAB - BGP Gui Utilities 10/29/2004 3:28:39 PM 19 Sep 2005 5:28 PM 27 Apr 2010 10:28 PM ;
 ;;14.1;IHS CLINICAL REPORTING;;MAY 29, 2014;Build 114
 ;
 ;
DEBUG(RETVAL,BGPSTR) ;run the debugger
 D DEBUG^%Serenji("UPL^BGP4GU(.RETVAL,.BGPSTR)")
 Q
 ;
...
FAC(RETVAL,BGPSTR) ;-- return facilities based on parameters passed
 ...
 D GET^BGP4ASL(.BGPFAC,.BGPFILE,BGPRT,BGPPER,BGPQTR,BGPBAS,BGPBEN)
...
LST(RETVAL,BGPSTR) ;-- list files
 ...
 S RETVAL="^BGPTMP("_$J_")"
 S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 D EP^BGP4GLST(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPCT,BGPYN,BGPOT,BGPRT)
 S BGPI=BGPI+1
 ...
 Q
 ;
DELRPT(RETVAL,BGPSTR) ;-- delete a report out of the BGP 14 GUI REPORT file
...
UPL(RETVAL,BGPSTR) ;-- upload a file
 ...
 . D EP^BGP4GUPL(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPDIR,BGPFL,DT)
...
CHK(BGPRET,BGPSTR) ;check report status
 ...
 N BGPDA
 S BGPDA=0 F  S BGPDA=$O(^BGPGUIJ("AUSR",BGPDUZ,BGPDA)) Q:'BGPDA  D
 . N BGPIEN
 . S BGPIEN=0 F  S BGPIEN=$O(^BGPGUIJ("AUSR",BGPDUZ,BGPDA,BGPIEN)) Q:'BGPIEN  D
 .. N BGPDATA,BGPNM,BGPUSER,BGPST,BGPET,BGPTOR,BGPRS,BGPTOO,BGPFLS,BGPSTI,BGPETI
 .. S BGPDATA=$G(^BGPGUIJ(BGPIEN,0))
 .. S BGPNM=$P(BGPDATA,U)
 ...
 ... S DIK="^BGPGUIJ(",DA=BGPIEN D ^DIK
 .. S BGPTOR=$P(BGPDATA,U,5)
 .. S BGPRS=$$GET1^DIQ(90552.19,BGPIEN,.06)
 .. I BGPSTI>$$NOW^XLFDT() D
 ... Q:$G(BGPRS)="COMPLETED"
 ... S BGPRS="QUEUED"
 .. S BGPTOO=$$GET1^DIQ(90552.19,BGPIEN,.07)
 .. S BGPFLS=$$GET1^DIQ(90552.19,BGPIEN,1)
 .. S BGPI=BGPI+1
...
LABTAXC(RETVAL,BGPSTR) ;EP - from remote procedure
 ...
 S RETVAL="^BGPTMP("_$J_")"
 S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 D EP^BGP4GLTX(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPRT,BGPR,BGPFN)
 S BGPI=BGPI+1
 S ^BGPTMP($J,BGPI)=+$G(BGPERR)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)
 D EN^XBVK("BGP")
...
MEDTAXC(RETVAL,BGPSTR) ;EP - from remote procedure
 ...
 S RETVAL="^BGPTMP("_$J_")"
 S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 D EP^BGP4GMTX(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPRT,BGPR,BGPFN)
 S BGPI=BGPI+1
 S ^BGPTMP($J,BGPI)=+$G(BGPERR)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)
 D EN^XBVK("BGP")
