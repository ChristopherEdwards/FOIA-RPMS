BGP4PDLH ; IHS/CMI/LAB - cover page;
 ...
 I BGPRTYPE=4,$G(BGPYRPTH)="C" S X="*** IHS 2014 Selected Measures with Community Specified Report ***" D SET(X,1,1)
 I BGPRTYPE=4,$G(BGPYRPTH)="A" S X="*** IHS 2014 Selected Measures with All Communities Report ***" D SET(X,1,1)
 I BGPRTYPE=4,$G(BGPYRPTH)="P" S X="*** IHS 2014 Selected Measures with Patient Panel Population Report ***" D SET(X,1,1)
 I BGPRTYPE=1,$G(BGPNGR09) S X="*** IHS 2014 National GPRA/GPRAMA Report, Run Using 2014 Logic ***" D SET(X,1,1) G N
 I BGPRTYPE=1,$G(BGPDESGP) S X="*** IHS 2014 National GPRA/GPRAMA Report by Designated Provider ***" D SET(X,1,1) G N
 I BGPRTYPE=1,'$G(BGPYGPU),'$G(BGPSUMON) S X="*** IHS 2014 National GPRA/GPRAMA Report ***" D SET(X,1,1)
 I BGPRTYPE=1,'$G(BGPYGPU),$G(BGPSUMON) S X="*** IHS 2014 National GPRA/GPRAMA Report Clinical Performance Summaries ***" D SET(X,1,1)
 I BGPRTYPE=1,$G(BGPYGPU) S X="*** IHS 2014 GPRA/GPRAMA Report ***" D SET(X,1,1)
 I BGPRTYPE=7 S X="*** IHS 2014 Other National Measures Report ***" D SET(X,1,1)
 I BGPRTYPE=6,'$G(BGPEDPP) S X="*** IHS 2014 Patient Education with Community Specified Report ***" D SET(X,1,1)
 I BGPRTYPE=6,$G(BGPEDPP) S X="*** IHS 2014 Patient Education with Patient Panel Population Report ***" D SET(X,1,1)
N ;
 I $G(BGPCPPL) S X="** Including Comprehensive Patient List **" D SET(X,1,1)
 S X=$$RPTVER^BGP4BAN D SET(X,1,1)
 ...
 I BGPRTYPE=4 S X="Measures: "_$P($T(@BGPINDJ),";;",2) D SET(X,1,1)
 ...
 I BGPROT'="P" D
 .Q:$D(BGPGUI)
 .S X="A delimited output file called "_BGPDELF D SET(X,1,1) S X="has been placed in the "_$$GETDEDIR^BGP4UTL2()_" directory for your use in Excel or some" D SET(X,1,1) S X="other software package." D SET(X,1,1)
...
ENDTIME ;
 ...
 Q
AREACP ;EP
 I BGPRTYPE=6 S X="*** IHS 2014 Patient Education with Community Specified Report***" D SET(X,1,1)
 I BGPRTYPE=1,$G(BGPNGR09) S X="*** IHS 2014 National GPRA/GPRAMA Report, Run Using 2014 Logic ***" D SET(X,1,1) G NA
 I BGPRTYPE=1,'$G(BGPYGPU),'$G(BGPSUMON) S X="*** IHS 2014 National GPRA/GPRAMA Report ***" D SET(X,1,1)
 I BGPRTYPE=1,'$G(BGPYGPU),$G(BGPSUMON) S X="*** IHS 2014 National GPRA/GPRAMA Report Clinical Performance Summaries ***" D SET(X,1,1)
 I BGPRTYPE=1,$G(BGPYGPU) S X="*** IHS 2014 GPRA/GPRAMA Performance Report ***" D SET(X,1,1)
 I BGPRTYPE=7 D SET("*** IHS 2014 Other National Measures Report ***",1,1)
NA S X="AREA AGGREGATE" D SET(X,1,1)
 ...
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPGPDCJ(BGPX,0),U,17):"*",1:"")_X D SET(X,1,1)
 .Q
 S X=" " D SET(X,1,1)
 ...
 .S X=$P(^BGPGPDCJ(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPGPDCJ(BGPX,0),U,17):"*",1:"")_X D SET(X,1,1)
 .S X="Community Taxonomy Name: "_$P(^BGPGPDCJ(BGPX,0),U,18) D SET(X,1,1)
 .S X=0,N=0,Y="",Z="" F  S X=$O(^BGPGPDCJ(BGPX,9999,X)) Q:X'=+X  S N=N+1,Y=Y_$S(N=1:"",1:";")_$P(^BGPGPDCJ(BGPX,9999,X,0),U)
 .S X=0,C=0 F X=1:3:N S Z=$E($P(Y,";",X),1,20),$P(Z,U,2)=$E($P(Y,";",(X+1)),1,20),$P(Z,U,3)=$E($P(Y,";",(X+2)),1,20) D SET(Z,1,1)
 .I $O(^BGPGPDCJ(BGPX,1111,0))  D
 ..S X=" " D SET(X,1,1)
 ..S X="MFI Site: Locations for visits: " D SET(X,1,1) S X=0,N=0,Y="",Z="" F  S X=$O(^BGPGPDCJ(BGPX,1111,X)) Q:X'=+X  S N=N+1,Y=Y_$S(N=1:"",1:";")_$P(^BGPGPDCJ(BGPX,1111,X,0),U)
 ..S X=0,C=0 F X=1:3:N S Z=$E($P(Y,";",X),1,20),$P(Z,U,2)=$E($P(Y,";",(X+1)),1,20),$P(Z,U,3)=$E($P(Y,";",(X+2)),1,20) D SET(Z,1,1)
 ..Q
 .S X=" " D SET(X,1,1)
 .Q
