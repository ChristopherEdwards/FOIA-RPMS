BGP4XTCH ; IHS/CMI/LAB - TAXONOMY CHECK FOR FY04 CRS REPORT 16 Jan 2009 4:02 PM ;
 ;;14.0;IHS CLINICAL REPORTING;;NOV 14, 2013;Build 101
 ;
 ;
 D HOME^%ZIS
 W:$D(IOF) @IOF
 W !!,"Checking for Taxonomies to support the 2014 CRS Report. ",!,"Please enter the device for printing.",!
ZIS ;
 K IOP,%ZIS
 W !! S %ZIS="PQM" D ^%ZIS
 I POP D XIT Q
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="TAXCHK^BGP4XTCH",ZTDTH="",ZTDESC="CRS 14 TAX REPORT" D ^%ZTLOAD D XIT Q
 Q
TAXCHK ;EP
 ;D HOME^%ZIS
 K BGPQUIT
GUICHK ;EP
 ...
 .Q:'$D(^BGPTAXJ(BGPY,12,"B",2))
 .;I $P(^BGPTAXJ(BGPY,0),U,2)'="L" S BGPX=$O(^ATXAX("B",BGPT,0))
 .;I $P(^BGPTAXJ(BGPY,0),U,2)="L" S BGPX=$O(^ATXLAB("B",BGPT,0))
 .S BGPTYPE=$P(^BGPTAXJ(BGPY,0),U,2),Y=$G(^BGPTAXJ(BGPY,11,1,0))
 .I BGPTYPE'="L" D
