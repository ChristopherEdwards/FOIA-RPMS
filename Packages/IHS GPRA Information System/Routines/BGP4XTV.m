BGP4XTV ; IHS/CMI/LAB - DISPLAY IND LISTS 15 Dec 2010 9:42 AM ;
...
EN ;EP -- main entry point for 
 D EN^VALM("BGP 14 TAXONOMY VIEW")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ;EP -- init variables and list array
 ...
 S BGPT=""
 F  S BGPT=$O(^BGPTAXJ("B",BGPT)) Q:BGPT=""  D
 .S BGPY=$O(^BGPTAXJ("B",BGPT,0))
 .Q:'$O(^BGPTAXJ(BGPY,12,0))
 .S BGPTYPE=$P(^BGPTAXJ(BGPY,0),U,2),BGPDESC=$G(^BGPTAXJ(BGPY,11,1,0)),BGPEDIT=$P(^BGPTAXJ(BGPY,0),U,4)
 .I BGPTYPE'="L" D  Q:'I
...
SET .;
 ...
 S T="",J=0,C=0 F  S T=$O(^BGPTAXJ("B",T)) Q:T=""  D
 .S Y=0 F  S Y=$O(^BGPTAXJ("B",T,Y)) Q:Y'=+Y  D
 ..S N=^BGPTAXJ(Y,0)
 ..S Z=$P(N,U,2)  ;TYPE
 ...
 ..I Z="" Q
 ..S J=J+1
 ..S BGPTAX(J,0)=J_")  "_T,$E(BGPTAX(J,0),39)=$$VAL^XBDIQ1(90552.08,Y,.02) D
 ...S A="",B=0 F  S B=$O(^BGPTAXJ(Y,12,B)) Q:B'=+B  S R=$P(^BGPTAXJ(Y,12,B,0),U) S:A]"" A=A_";" S A=A_$S(R=1:"National GPRA",R=2:"Local CRS",R=3:"HEDIS",R=4:"ELDER",R=5:"CMS",1:"")
 ...S $E(BGPTAX(J,0),60)=A
