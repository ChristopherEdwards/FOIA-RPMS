...
EN ;EP -- main entry point
 D EN^VALM("BGP 15 AREA SELECT SITES")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ; -- init variables and list array
 I BGPRTYPE=1 D NGR^BGP5ASL1 Q
 I BGPRTYPE=5 D ELD^BGP5ASL1 Q
 I BGPRTYPE=6 D PED^BGP5ASL1 Q
 I BGPRTYPE=7 D ONM^BGP5ASL1 Q
 Q
FAC(S) ;
...
G5 ;
 ...
 S BGPFILE=90555.03
 S BGPX=0 F  S BGPX=$O(^BGPELDCK(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPELDCK(BGPX,0))
 . S V=^BGPELDCK(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
...
G6 ;
 ...
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPFILE=90554.12
 S BGPX=0 F  S BGPX=$O(^BGPPEDCK(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPPEDCK(BGPX,0))
 . S V=^BGPPEDCK(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
...
G9 ;
 ...
 S BGPBBD=BGPBD-X,BGPBBD=$E(BGPBBD,1,3)_$E(BGPBD,4,7)
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPX=0 F  S BGPX=$O(^BGPGPDCK(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPGPDCK(BGPX,0))
 . S V=^BGPGPDCK(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
