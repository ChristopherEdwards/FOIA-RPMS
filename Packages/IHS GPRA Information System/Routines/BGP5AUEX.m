BGP5AUEX ; IHS/CMI/LAB - BUILD SITE GPRA FILES, EXPORT TO AREA ; 05 Nov 2014  12:13 PM
 ...
 ;;Some code in EN taken from the BGP5DGPU routine.
 ;
DESC ;----- ROUTINE DESCRIPTION
 ...
 ;;to run each month via option BGP5AUEX AUTO GPRA EXTRACT.
 ;;$$END
 ;
...
AUTO ;EP -- AUTOQUEUED JOB ENTRY POINT
 ...
 I Q Q  ;W !!,"These values must be entered into the parameter file",!,"before you can run this option.",! D PAUSE^BGP5DU Q
...
MAN ;EP -- MANUALLY RUN GPRA EXTRACT
 ...
 I Q W !!,"These values must be entered into the parameter file",!,"before you can run this option.",! D PAUSE^BGP5DU Q
 I BGPT="T" D  G MAN1
 .S BGPDT=$$FMADD^XLFDT(DT,-60)
 .S BGPY=$E(BGPDT,1,3)
 .S BGPM=+$E(BGPDT,4,5)
...
MAN1 S BGPAMEX=1,BGPERRM=""
 ...
 D ^DIC
 I Y=-1 W !!,"Exiting Report..." D PAUSE^BGP5DU,XIT Q
 S BGPTAXI=+Y
 ;S BGPAMFN="BGPGPAM121"_DT_$P(^AUTTLOC(BGPSITE,0),U,10)_$$LZERO^BGP5UTL(BGPLOG)_".TXT"
 W:$D(IOF) @IOF
 W !,$$CTR^BGP5DNG("SUMMARY OF NATIONAL GPRA/GPRAMA REPORT TO BE GENERATED")
...
QUE(BGPEND,ZTDTH) ;EP
 ...
 S ZTSAVE("BGP*")=""
 S ZTRTN="DQ^BGP5AUEX"
 S ZTDESC="BGP5 AUTO GPRA DATA EXTRACT"
 S ZTIO=""
 D ^%ZTLOAD
 ;
 Q
...
EN(BGPEND) ;EP -- MAIN ENTRY POINT
 ...
 N BGPHOME,BGPINDK,BGPEXPT,BGPEXCEL,BGPUF,BGPQUIT,BGPRPT,BGPFILE
 ;
 ...
 S BGPT=$P(^BGPGP2PM(BGPSITE,0),U,2)
 I BGPT="G" D
 .S X=$O(^BGPCTRL("B",2015,0))  ;get GPRA year dates
...
COM ;
 ...
 D REPORT^BGP5UTL   ;***CREATES THE ENTRIES IN THE BGP 15 DATA FILES AND RETURNS BGPRPT
 I $G(BGPQUIT) D XIT Q
 I BGPRPT="" D XIT Q
 S BGPAMFN="BGPAM140"_BGPEND_$P(^AUTTLOC(BGPSITE,0),U,10)_$$LZERO^BGP5UTL(BGPRPT,6)_".TXT"
 D ^BGP5D1
 D GS^BGP5UTL  ;***CREATES BG151 FILE
 S BGPFILE=BGPAMFN
...
NT ;
 D BMES^XPDUTL("SETTING OPTION 'BGP 15 AUTO GPRA EXTRACT' to run in taskman")
 ;
 ;S BGPOPT="BGP 15 AUTO GPRA EXTRACT"
 ;S BGPOPTD0=$O(^DIC(19,"B",BGPOPT,0))
 ;I 'BGPOPTD0 D  Q
 ;. D BMES^XPDUTL("'BGP 15 AUTO GPRA EXTRACT' OPTION NOT FOUND!")
 ;Q:'BGPOPTD0
 ;S BGPD0=$O(^DIC(19.2,"B",BGPOPTD0,0))
 ;D ADDOPT(BGPOPTD0,.BGPD0)
 ;I 'BGPD0 D  Q
 ;. D BMES^XPDUTL("UNABLE TO SCHEDULE OPTION 'BGP 15 AUTO GPRA EXTRACT'")
 ;D RESCH^XUTMOPT("BGP 15 AUTO GPRA EXTRACT",BGPX,"","1M","L",.BGPERR)
 ;D; EDITOPT(BGPD0)
 ...
 D ^%ZTLOAD
 S BGPTSK=$G(ZTSK)
 D BMES^XPDUTL("OPTION 'BGP5 AUTO GPRA EXTRACT' SCHEDULED AS TASK #"_BGPTSK)
 Q
...
EDITPAR ;
 ...
 I Q W !!,"These values must be entered into the parameter file",!,"before you can schedule the automated report option.",! D PAUSE^BGP5DU Q
...
CHKFQT(F) ;EP -check for queued task (BGP AUTO GPRA EXTRACT and BGPSITE variable within the task
 ...
 .Q:$P($G(^%ZTSK(X,0)),U,1,2)'="AUTO^BGP5AUEX"
 .Q:$P($G(^%ZTSK(X,.03)),U,1)'="BGP5 AUTO GPRA DATA EXTRACT"  ;"BGP 15 AUTO GPRA EXTRACT"  ;not the gpra export
