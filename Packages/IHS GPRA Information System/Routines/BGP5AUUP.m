BGP5AUUP ; IHS/CMI/LAB - UPLOAD SITE FILES, CREATE EXCEL FILES ;
...
EN(BGPBEGDT,BGPENDDT,BGPMAN) ;EP -- MAIN ENTRY POINT
 ...
 . I $G(BGPMAN) D  Q
 . . W !,BGPERR
 . . D MSG1^BGP5AU1M(BGPBEGDT,BGPENDDT,.BGPFACS)
 ...
 Q:BGPOUT
 ;
 ;upload all files into the BGP 15 DATA* files
 D UPLOAD(BGPPATH,BGPBEGDT,BGPENDDT,.BGPFACS)
 ;
 ;create the 5 excel files
 D EN^BGP5AU1A(BGPBEGDT,BGPENDDT,.BGPEND,.BGPF)
 ;
 ...
 D EN^BGP5AU1D(BGPBEGDT,BGPEND,.BGPDFILE)
 ;
 ;
...
CLEAN(BGPBEGDT,BGPENDDT) ;
 ...
 S BGPD0=0 F  S BGPD0=$O(^BGPGPDCK("B",BGPBEGDT,BGPD0)) Q:'BGPD0  D
 . S BGPASU=$P(^BGPGPDCK(BGPD0,0),U,9)
 . I $G(BGPFACS(BGPENDDT,BGPASU))="" Q
 . Q:$P(^BGPGPDCK(BGPD0,0),U,12)'=1  ;DON'T DELETE OTHER NATIONAL, GPU, ETC.
 . S DA=BGPD0
 . S DIK="^BGPGPDCK("
 . D ^DIK
 ;
 S BGPD0=0 F  S BGPD0=$O(^BGPGPDPK("B",BGPBEGDT,BGPD0)) Q:'BGPD0  D
 . S BGPASU=$P(^BGPGPDPK(BGPD0,0),U,9)
 . I $G(BGPFACS(BGPENDDT,BGPASU))="" Q
 . Q:$P(^BGPGPDPK(BGPD0,0),U,12)'=1  ;DON'T DELETE OTHER NATIONAL, GPU, ETC.
 . S DA=BGPD0
 . S DIK="^BGPGPDPK("
 . D ^DIK
 ;
 S BGPD0=0 F  S BGPD0=$O(^BGPGPDBK("B",BGPBEGDT,BGPD0)) Q:'BGPD0  D
 . S BGPASU=$P(^BGPGPDBK(BGPD0,0),U,9)
 . I $G(BGPFACS(BGPENDDT,BGPASU))="" Q
 . Q:$P(^BGPGPDBK(BGPD0,0),U,12)'=1  ;DON'T DELETE OTHER NATIONAL, GPU, ETC.
 . S DA=BGPD0
 . S DIK="^BGPGPDBK("
 . D ^DIK
 Q
