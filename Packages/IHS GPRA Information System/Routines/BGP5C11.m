BGP5C11 ; IHS/CMI/LAB - calc CMS measures 02 Jul 2010 8:31 AM ;
...
CEFCPT ;
 ...
 ..S V=$P($P($G(^AUPNVSIT(V,0)),U),".")
 ..I V>BGPD Q
 ..S BGPC=BGPC+1,BGPY(BGPC)="CEF TRAN CODE:  "_$$DATE^BGP5UTL(V)_"  ["_Y_"]  "_$P($$CPT^ICPTCOD(I,V),U,3)
 Q
...
ACEIPOV ;
 ...
 .I N["ACEI"!(N["ACE I") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP5UTL($P(BGPG(X),U))_" ADR POV "_$P(BGPG(X),U,2) Q
 .S T=$O(^ATXAX("B","BGP ADV EFF ANTIHYPERTEN AGT",0))
 .S Z=$P(^AUPNVPOV(Y,0),U,9) I Z]"",$$ICD^BGP5UTL2(Z,T,9) S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP5UTL($P(BGPG(X),U))_" ADR POV ["_$P(BGPG(X),U,2)_" + "_$P($$ICDDX^BGP5UTL2(Z),U,2)_"]" Q
 .S Z=$P(^AUPNVPOV(Y,0),U,18) I Z]"",$$ICD^BGP5UTL2(Z,T,9) S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP5UTL($P(BGPG(X),U))_" ADR POV ["_$P(BGPG(X),U,2)_" + "_$P($$ICDDX^BGP5UTL2(Z),U,2)_"]" Q
 .S Z=$P(^AUPNVPOV(Y,0),U,19) I Z]"",$$ICD^BGP5UTL2(Z,T,9) S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP5UTL($P(BGPG(X),U))_" ADR POV ["_$P(BGPG(X),U,2)_" + "_$P($$ICDDX^BGP5UTL2(Z),U,2)_"]" Q
 .Q
 S G=""
 ...
 I $D(BGPG(1)) S BGPC=BGPC+1,BGPY(BGPC)="ADR POV:  "_$$DATE^BGP5UTL($P(BGPG(1),U))_"  ["_$P(BGPG(1),U,2)_"]"
 ...
 .I N["ACEI"!(N["ACE I") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP5UTL($P(BGPG(X),U))_" ADR POV "_$P(BGPG(X),U,2)
 ...
 .I $$ICD^BGP5UTL2(I,$O(^ATXAX("B","BGP HX DRUG ALLERGY NEC",0)),9)!($$ICD^BGP5UTL2(I,T,9)),N["ACEI"!(N["ACE I") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP5UTL($P(^AUPNPROB(X,0),U,8))_" ADR Problem List "_Y_" "_N Q
 .I $$ICD^BGP5UTL2(I,$O(^ATXAX("B","BGP ADV EFF ANTIHYPER 10",0)),9) S BGPC=BGPC+1,BGPY(BGPC)="ADR PROBLEM LIST:  "_$$DATE^BGP5UTL($P(^AUPNPROB(X,0),U,8))_"  ["_Y_"]  "
 .Q
 S X=0 F  S X=$O(^GMR(120.8,"B",P,X)) Q:X'=+X  D
 .Q:$P($P($G(^GMR(120.8,X,0)),U,4),".")>BGPD
 .S N=$P($G(^GMR(120.8,X,0)),U,2),N=$$UP^XLFSTR(N)
 .I N["ACEI"!(N["ACE INHIBITOR") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP5UTL($P(^GMR(120.8,X,0),U,4))_" ADR ALLERGY TRACKING  "
 D ARBALG1^BGP5C13
 Q
ACEIRX1(P,BGPA,BGPD,BGPY) ;EP
 ...
 D GETMEDS^BGP5CU(P,BD,ED,"BGP CMS ACEI MEDS","BGP CMS ACEI MEDS NDC","BGP CMS ACEI MEDS CLASS")
 S BD=BGPA
...
BETACON1(P,BGPD,BGPDDT,BGPV,BGPY) ;have an ACEI allergy
 ...
 .S V=$P(^AUPNVPOV(X,0),U,3)
 .S V=$P($P($G(^AUPNVSIT(V,0)),U),".")
 .I $$ICD^BGP5UTL2(I,T,9) S BGPC=BGPC+1,BGPY(BGPC)="BRADYCARDIA POV:  "_$$DATE^BGP5UTL(V)_"  ["_Y_"]  "_$$VAL^XBDIQ1(9000010.07,X,.04) Q
 .I $$ICD^BGP5UTL2(I,T1,9) S BGPC=BGPC+1,BGPY(BGPC)="2ND OR 3RD DEGREE HEART BLOCK POV:  "_$$DATE^BGP5UTL(V)_"  ["_Y_"]  "_$$VAL^XBDIQ1(9000010.07,X,.04) Q
 .I $$ICD^BGP5UTL2(I,T2,9) S BGPC=BGPC+1,BGPY(BGPC)="CIRCULATORY SHOCK POV:  "_$$DATE^BGP5UTL(V)_"  ["_Y_"]  "_$$VAL^XBDIQ1(9000010.07,X,.04) Q
 .I $$ICD^BGP5UTL2(I,T3,9) S BGPC=BGPC+1,BGPY(BGPC)="HEART FAILURE POV:  "_$$DATE^BGP5UTL(V)_"  ["_Y_"]  "_$$VAL^XBDIQ1(9000010.07,X,.04) Q
 .Q
 ;
 S Z=0 F  S Z=$O(^AUPNVSIT("AD",BGPV,Z)) Q:Z'=+Z  D
 ...
 ..Q
 .Q
 S X=$$CPTI^BGP5DU(P,BGPD,BGPDDT,+$$CODEN^ICPTCOD("G8011"))
 I X S BGPC=BGPC+1,BGPY(BGPC)="CPT code G8011: "_$$DATE^BGP5UTL($P(X,U,2))
 S X=$$TRANI^BGP5DU(P,BGPD,BGPDDT,+$$CODEN^ICPTCOD("G8011"))
 I X S BGPC=BGPC+1,BGPY(BGPC)="TRAN code G8011: "_$$DATE^BGP5UTL($P(X,U,2))
 S X=$$CPTI^BGP5DU(P,BGPD,BGPDDT,+$$CODEN^ICPTCOD("G9190"))
 I X S BGPC=BGPC+1,BGPY(BGPC)="CPT code G9190: "_$$DATE^BGP5UTL($P(X,U,2))
 S X=$$TRANI^BGP5DU(P,BGPD,BGPDDT,+$$CODEN^ICPTCOD("G9190"))
 I X S BGPC=BGPC+1,BGPY(BGPC)="TRAN code G9190: "_$$DATE^BGP5UTL($P(X,U,2))
 K BGPG
 Q
BETARX1(P,BGPA,BGPD,BGPY) ;
 ...
 S ED=$$FMADD^XLFDT(BGPA,-1)
 S BD=$$FMADD^XLFDT(BGPA,-365)
 D GETMEDS^BGP5CU(P,BD,ED,"BGP CMS BETA BLOCKER MEDS","BGP CMS BETA BLOCKER NDC","BGP CMS BETA BLOCKER CLASS")
 S X=$$CPTI^BGP5DU(P,BD,ED,+$$CODEN^ICPTCOD("G8009"))
 I X S BGPC=BGPC+1,BGPY(BGPC)="CPT code G8009: "_$$DATE^BGP5UTL($P(X,U,2))
 S X=$$TRANI^BGP5DU(P,BD,ED,+$$CODEN^ICPTCOD("G8009"))
 I X S BGPC=BGPC+1,BGPY(BGPC)="TRAN code G8009: "_$$DATE^BGP5UTL($P(X,U,2))
 K BGPG
 S BD=BGPA
 S ED=$$FMADD^XLFDT(BGPD,30)
 D GETMEDS^BGP5CU(P,BD,ED,"BGP CMS BETA BLOCKER MEDS","BGP CMS BETA BLOCKER NDC","BGP CMS BETA BLOCKER CLASS")
 ...
 I X S BGPC=BGPC+1,BGPY(BGPC)="CPT code G8009: "_$$DATE^BGP5UTL($P(X,U,2))
 S X=$$TRANI^BGP5DU(P,BD,ED,+$$CODEN^ICPTCOD("G8009"))
 I X S BGPC=BGPC+1,BGPY(BGPC)="TRANBG code G8009: "_$$DATE^BGP5UTL($P(X,U,2))
 K BGPG
 Q
