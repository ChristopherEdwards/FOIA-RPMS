BGP5C13 ; IHS/CMI/LAB - calc CMS measures 26 Sep 2004 11:28 AM ;
...
ARBPOV ;
 ...
 .I N["ARB"!(N["ANGIOTENSIN RECEPTOR BLOCKER") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP5UTL($P(BGPG(X),U))_" ADR POV "_$P(BGPG(X),U,2) Q
 ...
 .I N["ARB"!(N["ANGIOTENSIN RECEPTOR BLOCKER") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP5UTL($P(BGPG(X),U))_" ADR POV "_$P(BGPG(X),U,2)
 ...
 .I $$ICD^BGP5UTL2(I,$O(^ATXAX("B","BGP HX DRUG ALLERGY NEC",0)),9)!($$ICD^BGP5UTL2(I,T,9)),N["ARB"!(N["ANGIOTENSIN RECEPTOR BLOCKER") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP5UTL($P(^AUPNPROB(X,0),U,8))_" ADR PROBLEM LIST "_Y_" "_N
 .Q
 ;now check allergy tracking
 S X=0 F  S X=$O(^GMR(120.8,"B",P,X)) Q:X'=+X  D
 .Q:$P($P($G(^GMR(120.8,X,0)),U,4),".")>BGPD  ;entered after discharge date
 .S N=$P($G(^GMR(120.8,X,0)),U,2),N=$$UP^XLFSTR(N)
 .I N["ARB"!(N["ANGIOTENSIN RECEPTOR BLOCKER") S BGPC=BGPC+1,BGPY(BGPC)=$$DATE^BGP5UTL($P(^GMR(120.8,X,0),U,4))_" ADR ALLERGY TRACKING"
 Q
ARBCON1 ;EP does patient have an ARB allergy
 ...
 .Q
 Q:BGPIND'=2
 S X=$$CPTI^BGP5DU(P,BGPD,BGPDDT,+$$CODEN^ICPTCOD("G8029"))
 I X S BGPC=BGPC+1,BGPY(BGPC)="CPT code G8029: "_$$DATE^BGP5UTL($P(X,U,2))
 S X=$$TRANI^BGP5DU(P,BGPD,BGPDDT,+$$CODEN^ICPTCOD("G8029"))
 I X S BGPC=BGPC+1,BGPY(BGPC)="Tran Code G8029: "_$$DATE^BGP5UTL($P(X,U,2))
 Q
ARBRX1 ;EP
 ...
 D GETMEDS^BGP5CU(P,BD,ED,"BGP CMS ARB MEDS","BGP CMS ARB MEDS NDC","BGP CMS ARB MEDS CLASS")
 I BGPIND=2 D
 .S X=$$CPTI^BGP5DU(P,BD,ED,+$$CODEN^ICPTCOD("G8027"))
 .I X S BGPC=BGPC+1,BGPY(BGPC)="CPT code G8027: "_$$DATE^BGP5UTL($P(X,U,2))
 .S X=$$TRANI^BGP5DU(P,BD,ED,+$$CODEN^ICPTCOD("G8027"))
 .I X S BGPC=BGPC+1,BGPY(BGPC)="Tran Code G8027: "_$$DATE^BGP5UTL($P(X,U,2))
 S BD=BGPA
