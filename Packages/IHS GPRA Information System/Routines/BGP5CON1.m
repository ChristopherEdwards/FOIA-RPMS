BGP5CON1 ; IHS/CMI/LAB - measure AHR.A 30 May 2010 9:32 AM ;
...
BETA ;EP - BETA BLOCKER CONTRAINDICATION/NMI REFUSAL
 ...
 ...S G=1_U_"Beta Blocker contra NMI med "_$$DATE^BGP5UTL(Y)
 ..Q
 .Q
 I G Q G
 ;now cpt 8011 OR G9190 BETWEEN NMIB,NMIE
 S X=$$CPTI^BGP5DU(P,NMIB,NMIE,+$$CODEN^ICPTCOD("G8011"))
 I X Q "1^Beta Blocker Contra CPT code G8011: "_$$DATE^BGP5UTL($P(X,U,2))
 S X=$$TRANI^BGP5DU(P,NMIB,NMIE,+$$CODEN^ICPTCOD("G8011"))
 I X Q "1^Beta Blocker Contra TRAN code G8011: "_$$DATE^BGP5UTL($P(X,U,2))
 S X=$$CPTI^BGP5DU(P,NMIB,NMIE,+$$CODEN^ICPTCOD("G9190"))
 I X Q "1^Beta Blocker Contra CPT code G9190: "_$$DATE^BGP5UTL($P(X,U,2))
 S X=$$TRANI^BGP5DU(P,NMIB,NMIE,+$$CODEN^ICPTCOD("G9190"))
 I X Q "1^Beta Blocker Contra TRAN code G9190: "_$$DATE^BGP5UTL($P(X,U,2))
 Q ""
 ;
...
WAR71 .;
 ...
 S X=$$CPTI^BGP5DU(P,NMIB,NMIE,+$$CODEN^ICPTCOD("G8008"))
 I X Q 1_U_"asa Contra CPT code G8008: "_$$DATE^BGP5UTL($P(X,U,2))
 S X=$$TRANI^BGP5DU(P,NMIB,NMIE,+$$CODEN^ICPTCOD("G8008"))
...
ACEI ;EP does patient have an ACEI Contraidication
 ...
 I $D(BGPG(1)) Q 1_U_"ACEI Contra POV:  "_$$DATE^BGP5UTL($P(BGPG(1),U))_" ["_$P(BGPG(1),U,2)_"]    "_$$VAL^XBDIQ1(9000010.07,+$P(BGPG(1),U,4),.04)
 ;
...
STATIN ;EP does patient have an STATIN Contraidication
 ...
 .I T="BF-BC" S %=T_" "_$$DATE^BGP5UTL($P(BGPG(X),U)) Q
 .I T="BF-BP" S %=T_" "_$$DATE^BGP5UTL($P(BGPG(X),U)) Q
 .I T="BF-CS" S %=T_" "_$$DATE^BGP5UTL($P(BGPG(X),U)) Q
 .I T="BF-EQ" S %=T_" "_$$DATE^BGP5UTL($P(BGPG(X),U)) Q
 .I T="BF-FU" S %=T_" "_$$DATE^BGP5UTL($P(BGPG(X),U)) Q
 .I T="BF-HC" S %=T_" "_$$DATE^BGP5UTL($P(BGPG(X),U)) Q
 .I T="BF-ON" S %=T_" "_$$DATE^BGP5UTL($P(BGPG(X),U)) Q
 .I T="BF-M" S %=T_" "_$$DATE^BGP5UTL($P(BGPG(X),U)) Q
 .I T="BF-MK" S %=T_" "_$$DATE^BGP5UTL($P(BGPG(X),U)) Q
 .I T="BF-N" S %=T_" "_$$DATE^BGP5UTL($P(BGPG(X),U)) Q
