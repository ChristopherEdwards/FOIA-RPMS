...
I36 ;EP
 ...
 S BGPFRAC=$$FRACTURE^BGP5EL3(DFN,$$FMADD^XLFDT(BGPBDATE,-180),$$FMADD^XLFDT(BGPEDATE,180))
...
I38 ;EP
 ...
 S BGPASTH=$$PERASTH(DFN,$$FMADD^XLFDT(BGPBDATE,-365),BGPBDATE) I '$P(BGPASTH,U) S BGPSTOP=1 K ^TMP($J,"A") Q  ;does not have persistent asthma
 K ^TMP($J,"A")
 I BGPACTCL S BGPD1=1
 I BGPACTUP S BGPD2=1
 ...
 S BGPVALUE=$S(BGPRTYPE=3:"",BGPD2:"UP;",1:"")_$S(BGPD1:"AC",1:"")_" "_$P(BGPASTH,U,2)_"|||"_$P(BGPVALUE,U,2)_" "_$P(BGPVALUE,U,3)
 K ^TMP($J,"A")
 Q
I39 ;EP
...
PERASTH(P,BDATE,EDATE) ;EP
 ...
 .S G=1  ;got one
 ;
 I G Q 1_U_"DX ON HOSP/OR ER"  ;had prim dx on 30 or H so meets denom
PER3 ;
 ;now check for meds
 S BGPT=$O(^ATXAX("B","BGP ASTHMA DXS",0))
 S T=$O(^ATXAX("B","BGP ASTHMA CONTROLLERS",0))
 S T3=$O(^ATXAX("B","BGP ASTHMA CONTROLLER NDC",0))
 S T1=$O(^ATXAX("B","BGP ASTHMA INHALED STEROIDS",0))
 S T4=$O(^ATXAX("B","BGP ASTHMA INHALED STEROID NDC",0))
 S T2=$O(^ATXAX("B","BGP ASTHMA LEUKOTRIENE",0))
 S T5=$O(^ATXAX("B","BGP ASTHMA LEUKOTRIENE NDC",0))
...
ASTHTHER(P,BDATE,EDATE) ;EP
 ...
 .Q:'$D(^AUPNVSIT(V,0))
 .S Z=$P($G(^AUPNVMED(Y,0)),U) ;get drug ien
 .I $D(^ATXAX(T1,21,"B",Z))!($$NDC(Z,T4)),$P(^AUPNVMED(Y,0),U,8)="" S D=1_U_$P(^PSDRUG(Z,0),U)_U_$$DATE^BGP5UTL($P($P(^AUPNVSIT(V,0),U),".")) Q  ;it is an inhaled steroid that wasn't d/c'ed so 1 dispensing event
 .I $D(^ATXAX(T,21,"B",Z))!($$NDC(Z,T3)),$P(^AUPNVMED(Y,0),U,8)="" S D=1_U_$P(^PSDRUG(Z,0),U)_U_$$DATE^BGP5UTL($P($P(^AUPNVSIT(V,0),U),".")) Q
 .I $D(^ATXAX(T2,21,"B",Z))!($$NDC(Z,T5)),$P(^AUPNVMED(Y,0),U,8)="" S D=1_U_$P(^PSDRUG(Z,0),U)_U_$$DATE^BGP5UTL($P($P(^AUPNVSIT(V,0),U),".")) Q
 Q D
