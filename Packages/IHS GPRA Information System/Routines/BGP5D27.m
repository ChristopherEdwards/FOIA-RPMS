BGP5D27 ; IHS/CMI/LAB - measure I2 ;
...
AMP(P,EDATE) ;EP - DID PATIENT HAVE AMPUTATION
 ...
 ..S BGPX(D)=""
 ..;
 ..S M=$P(^AUPNVCPT(Y,0),U,8) I M S M=$P($G(^AUTTCMOD(M,0)),U) I M=50 S %=1_U_$$DATE^BGP5UTL(D)_" FOOT AMP "_$P(^ICPT(X,0),U,1)
 ..S M=$P(^AUPNVCPT(Y,0),U,9) I M S M=$P($G(^AUTTCMOD(M,0)),U) I M=50 S %=1_U_$$DATE^BGP5UTL(D)_" FOOT AMP "_$P(^ICPT(X,0),U,1)
 ..Q
 .Q
 ...
 ..I D>EDATE Q
 ..S BGPX(D)=""
 ..S M=$P(^AUPNVTC(E,0),U,12) I M S M=$P($G(^AUTTCMOD(M,0)),U) I M=50 S %=1_U_$$DATE^BGP5UTL(D)_" FOOT AMP "_$P(^ICPT(Y,0),U,1)
 ..S M=$P(^AUPNVTC(E,0),U,15) I M S M=$P($G(^AUTTCMOD(M,0)),U) I M=50 S %=1_U_$$DATE^BGP5UTL(D)_" FOOT AMP "_$P(^ICPT(Y,0),U,1)
 ..Q
 .Q
 ;see if 2 on different dates
 S %=0,X=0,C=0 F  S X=$O(BGPX(X)) Q:X'=+X  S C=C+1
 I C>1 Q 1
 S BGPG=$$LASTPRC^BGP5UTL1(P,"BGP BILAT FOOT AMP PROCEDURES",$$DOB^AUPNPAT(P),EDATE)
 I BGPG Q 1
 S BGPG=$$CPT^BGP5DU(P,$$DOB^AUPNPAT(P),EDATE,$O(^ATXAX("B","BGP CPT BILAT FOOT AMP",0)))
 I BGPG Q 1
