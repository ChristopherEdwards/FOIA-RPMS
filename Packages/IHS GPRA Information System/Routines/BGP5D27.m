...
BH S D=0,BGPC="",E=9999999-BDATE,D=9999999-EDATE-1_".99" F  S D=$O(^AMHREC("AE",P,D)) Q:D'=+D!($P(D,".")>E)!(BGPC)  S V=0 F  S V=$O(^AMHREC("AE",P,D,V)) Q:V'=+V!(BGPC)  D
 ...
 .I $P($G(^AMHREC(V,14)),U,5)="P"!($P($G(^AMHREC(V,14)),U,5)="N") S BGPC=1_U_C_U_$$DATE^BGP5UTL(9999999-D)_U_(9999999-D)_U_C
 .Q:BGPC
 .S X=0 F  S X=$O(^AMHRPRO("AD",V,X)) Q:X'=+X!(BGPC)  S BGPP=$P($G(^AMHRPRO(X,0)),U) D
 ..Q:'BGPP
 ..S BGPP=$P($G(^AMHPROB(BGPP,0)),U)
 ..I BGPP=14.1 S BGPC=1_U_C_U_$$DATE^BGP5UTL(9999999-D)_U_(9999999-D)_U_C Q
 .Q:BGPC
 .S X=0 F  S X=$O(^AMHRMSR("AD",V,X)) Q:X'=+X!(BGPC)  S BGPP=$P($G(^AMHRMSR(X,0)),U) D
 ..Q:'BGPP
 ..S BGPP=$P($G(^AUTTMSR(BGPP,0)),U)
 ..I BGPP="PHQ2"!(BGPP="PHQ9") S BGPC=1_U_C_U_$$DATE^BGP5UTL(9999999-D)_U_(9999999-D)_U_C
 .Q:BGPC
 ...
 ..S BGPC=1_U_C_U_$$DATE^BGP5UTL(9999999-D)_U_(9999999-D)_U_C
 ...
 ..Q:C=""
 ..Q:'$D(^BGPCTRL(BGPTC,50,"B",C))  ;not BH clinic
 ..I $P(BGPDEPS,U,4)<$P(BGPG(E),U) S BGPDEPS=1_"^"_C_U_$$DATE^BGP5UTL($P(BGPG(E),U))_U_$P(BGPG(E),U)_U_C
 K BGPG
 S Y="BGPG("
 ...
 ..Q:C=""
 ..Q:'$D(^BGPCTRL(BGPTC,50,"B",C))  ;not BH clinic
 ..I $P(BGPDEPS,U,4)<$P(BGPG(E),U) S BGPDEPS=1_C_U_$$DATE^BGP5UTL($P(BGPG(E),U))_U_$P(BGPG(E),U)_U_C
 K BGPG
 S Y="BGPG("
...
BHSCRC ;
 ...
 ...I $$ICD^BGP5UTL2($P(^AUPNVCPT(X,0),U),$O(^ATXAX("B","BGP DEPRESSION SCREEN CPTS",0)),1) S G=1_U_C_U_$$DATE^BGP5UTL($$VD^APCLV(V))_U_$$VD^APCLV(V)_U_C
 ...Q
 ..Q
 .Q
