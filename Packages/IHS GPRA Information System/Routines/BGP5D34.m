...
DTPPV ;
 ...
 K BGPG ;D SETPRC^BGP5UTL1(P,$$DOB^AUPNPAT(P),EDATE,"BGP DTP IZ PROCS",.BGPG)
...
TETPV ;
 ...
 K BGPG ;D SETPRC^BGP5UTL1(P,$$DOB^AUPNPAT(P),EDATE,"BGP TETANUS TOXOID IZ PROCS",.BGPG)
 ;I $D(BGPG(1)) S X=0 F  S X=$O(BGPG(X)) Q:X'=+X  S BGPTET($P(BGPG(X),U))="",BGPATET($P(BGPG(X),U))=""
DIPPV ;
 ...
 K BGPG ;D SETPRC^BGP5UTL1(P,$$DOB^AUPNPAT(P),EDATE,"BGP DIPHTHERIA IZ PROCS",.BGPG)
 ;I $D(BGPG(1)) S X=0 F  S X=$O(BGPG(X)) Q:X'=+X  S BGPDIP($P(BGPG(X),U))="",BGPADIP($P(BGPG(X),U))=""
PERPV ;
 ...
 K BGPG ;D SETPRC^BGP5UTL1(P,$$DOB^AUPNPAT(P),EDATE,"BGP PERTUSSIS IZ PROCS",.BGPG)
 ;I $D(BGPG(1)) S X=0 F  S X=$O(BGPG(X)) Q:X'=+X  S BGPPER($P(BGPG(X),U))="",BGPAPER($P(BGPG(X),U))=""
TDPV ;
 K BGPG S %=P_"^ALL DX [BGP TD IZ DXS;DURING "_$$DOB^AUPNPAT(P)_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) S X=0 F  S X=$O(BGPG(X)) Q:X'=+X  S BGPTD($P(BGPG(X),U))="",BGPATD($P(BGPG(X),U))=""
 S BGPCODE=2 D TEST^BGP5D341
EVIDTET ;
 S BGPEVTD=""
 D RESETD
...
REF ;
 ...
 ;F BGPIMM=20,50,106,107,110,1,22,102,115,120,130,132,146 Q:R  S R=$$IMMREF^BGP5D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
 ;I R Q 3_U_" Ref Dtap or DT"
 S BGPRBEG=$$FMADD^XLFDT(ED,-365)
 S R=$$CPTREFT^BGP5UTL1(P,$$DOB^AUPNPAT(P),EDATE,$O(^ATXAX("B","BGP CPT DTAP/DTP/TDAP",0)),"N")
 ...
 F BGPZ=20,50,106,107,110,120,130,132,146 S X=$$ANCONT^BGP5D31(P,BGPZ,EDATE) Q:X]""
 I X]"" Q 4_U_"Contra DTaP"
 F BGPZ=1,22 S X=$$ANCONT^BGP5D31(P,BGPZ,EDATE) Q:X]""
 I X]"" Q 4_U_"Contra DTP"
 F BGPZ=115 S X=$$ANCONT^BGP5D31(P,BGPZ,EDATE) Q:X]""
 I X]"" Q 4_U_"Contra Tdap"
 F BGPZ=28 S X=$$ANCONT^BGP5D31(P,BGPZ,EDATE) Q:X]""
 I X]"" Q 4_U_"Contra DT"
 F BGPZ=9,113,138,139 S X=$$ANCONT^BGP5D31(P,BGPZ,EDATE) Q:X]""
 I X]"" Q 4_U_"Contra td"
 F BGPZ=35,112 S X=$$ANCONT^BGP5D31(P,BGPZ,EDATE) Q:X]""
 I X]"" Q 4_U_"Contra tetanus"
 F BGPZ=11 S X=$$ANCONT^BGP5D31(P,BGPZ,EDATE) Q:X]""
