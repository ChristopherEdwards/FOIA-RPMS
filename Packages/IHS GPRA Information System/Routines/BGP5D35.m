BGP5D35 ; IHS/CMI/LAB - indicator C ;
...
VAR(P,EDATE) ;EP
 ...
 ...S Y=$P(^AUPNVCPT(X,0),U),Y=$P($$CPT^ICPTCOD(Y),U,2) I Y=90710!(Y=90716) S BGPVARI(9999999-ED)=""
 ...
 I $$PLTAX^BGPDU(P,"BGP VARICELLA EVIDENCE") Q 4_U_"evid var"
 ...
 I $$PLTAX^BGPDU(P,"BGP VARICELLA CONTRA") Q 4_U_"contra var"
 ;now go to refusals
...
HEP(P,EDATE) ;EP
 ...
 ..Q:'$D(^AUPNVCPT("AD",V))
 ..S X=0 F  S X=$O(^AUPNVCPT("AD",V,X)) Q:X'=+X  D
 ...S Y=$P(^AUPNVCPT(X,0),U),Y=$P($$CPT^ICPTCOD(Y),U,2) I Y=90636!(Y=90723)!(Y=90731)!(Y=90740)!(Y=90743)!(Y=90744)!(Y=90745)!(Y=90746)!(Y=90747)!(Y=90748) S BGPHEP(9999999-ED)=""
 ...
 I $$PLTAX^BGPDU(P,"BGP HEP EVIDENCE") Q 4_U_"evid hep"
 ;now go to refusals
 S B=$$DOB^AUPNPAT(P),E=EDATE,BGPNMI="",R=""
 ...
 ;now check refusals in imm pkg
 F BGPIMM=8,42,43,44,45,51,102,104,110 S R=$$IMMREF^BGP5D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
 I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI",1:"ref")_" hep"
 Q ""
HIB(P,EDATE) ;EP
 K BGPC,BGPG,BGPX
 ...
 ..Q:'$D(^AUPNVCPT("AD",V))
 ..S X=0 F  S X=$O(^AUPNVCPT("AD",V,X)) Q:X'=+X  D
 ...S Y=$P(^AUPNVCPT(X,0),U),Y=$P($$CPT^ICPTCOD(Y),U,2) I Y=90645!(Y=90646)!(Y=90647)!(Y=90648)!(Y=90698)!(Y=90720)!(Y=90721)!(Y=90748) S BGPHIB(9999999-ED)=""
 ;now check to see if they are all spaced 10 days apart, if not, kill off the odd ones
 ...
 I $D(BGPG(1)) S X=0 F  S X=$O(BGPG(X)) Q:X'=+X  S BGPHIB($P(BGPG(1),U))="",BGPAHIB(X)=""
 ;now check to see if they are all spaced 10 days apart, if not, kill off the odd ones
 S X="",Y="",C=0 F  S X=$O(BGPHIB(X)) Q:X'=+X  S C=C+1 D
 .I C=1 S Y=X Q
 .I $$FMDIFF^XLFDT(X,Y)<11 K BGPHIB(X) Q
 ...
 I $$PLTAX^BGPDU(P,"BGP HIB EVIDENCE") Q 4_U_"evid HIB"
 ;now go to refusals
 S B=$$DOB^AUPNPAT(P),E=EDATE,BGPNMI="",R=""
 F BGPIMM=22,46,47,48,49,50,51,102,104 D
 .S I=$O(^AUTTIMM("C",BGPIMM,0)) Q:'I
 .S X=0 F  S X=$O(^AUPNPREF("AA",P,9999999.14,I,X)) Q:X'=+X  S Y=0 F  S Y=$O(^AUPNPREF("AA",P,9999999.14,I,X,Y)) Q:Y'=+Y  S D=$P(^AUPNPREF(Y,0),U,3) I D'<B&(D'>E) S:$P(^AUPNPREF(Y,0),U,7)="N" BGPNMI=1 S R=1
 I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI",1:"ref")_" hib"
 ;now check refusals in imm pkg
 F BGPIMM=22,46,47,48,49,50,51,102,104 S R=$$IMMREF^BGP5D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
 I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI",1:"ref")_" hib"
 Q ""
