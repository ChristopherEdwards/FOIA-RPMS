...
MEN(P,EDATE) ;EP
 ...
 F BGPZ=32,108,114,136,147 S X=$$ANCONT^BGP5D31(P,BGPZ,EDATE) Q:X]""
 ...
 ;F BGPIMM=32,108,114,136,147 S R=$$IMMREF^BGP5D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
...
H1N1 ;EP - called from measure
 ...
 I BGPN1!(BGPN2) S BGPVALUE=BGPVALUE_$S($P(BGPH1N1,U,1):$$DATE^BGP5UTL($P(BGPH1N1,U,1))_" "_$P(BGPH1N1,U,2),1:"")
 I $P(BGPH1N1,U,3) S BGPVALUE=BGPVALUE_"; "_$$DATE^BGP5UTL($P(BGPH1N1,U,3))_" "_$P(BGPH1N1,U,4)  ;:"2 doses",$P(BGPH1N1,U,1):"1 dose",1:"")_" "_$$DATE^BGP5UTL($P(BGPH1N1,U))_" "_$$DATE^BGP5UTL($P(BGPH1N1,U,2))
 Q
H1N1VAC(P,BDATE,EDATE) ;EP
 ...
 ..Q:'$D(^AUPNVCPT("AD",V))
 ..S X=0 F  S X=$O(^AUPNVCPT("AD",V,X)) Q:X'=+X  D
 ...I $$ICD^BGP5UTL2($P(^AUPNVCPT(X,0),U),T,1) S C=C+1,M($$VD^APCLV(V))=$$VAL^XBDIQ1(9000010.18,X,.01)
 ...Q
 ..Q
 .Q
 I '$D(M) Q ""
...
ACTPREG(P,ED) ;EP
 ...
 ..Q:Q=""
 ..Q:'$$ICD^BGP5UTL2(Q,T,9)  ;not in taxonomy
 ..S G=1
 ...
 K BGPG S BGPG=$$LASTPRC^BGP5UTL1(P,"BGP ABORTION PROCEDURES",LPD,ED)
