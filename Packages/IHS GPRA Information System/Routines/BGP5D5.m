BGP5D5 ; IHS/CMI/LAB - indicator calc ;
...
ALHF(P,BDATE,EDATE) ;alcohol hf or screening pov
 ;ADD CHECK FOR PROBLEM CODE 29.1
 ...
 ..Q:'T
 ..I $P($G(^AUTTHF(T,0)),U)["ALCOHOL" S BGPC=1_U_"BH HF: "_$P(^AUTTHF(F,0),U)_U_$$DATE^BGP5UTL((9999999-D))
 ..Q
 I BGPC Q BGPC
 K BGPG S %=P_"^LAST DX V11.3;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 1_U_"DX: V11.3"_U_$$DATE^BGP5UTL($P(BGPG(1),U))
 S %=P_"^LAST DX V79.1;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 1_U_"DX: V79.1"_U_$$DATE^BGP5UTL($P(BGPG(1),U))
 ;go through BH record file and find up to 1 visits in date range
 S E=9999999-BDATE,D=9999999-EDATE-1_".99" F  S D=$O(^AMHREC("AE",P,D)) Q:D'=+D!($P(D,".")>E)!(BGPC)  S V=0 F  S V=$O(^AMHREC("AE",P,D,V)) Q:V'=+V!(BGPC)  D
 .S X=0 F  S X=$O(^AMHRPRO("AD",V,X)) Q:X'=+X!(BGPC)  S BGPP=$P($G(^AMHRPRO(X,0)),U) D
 ..Q:'BGPP
 ..S BGPP=$P($G(^AMHPROB(BGPP,0)),U)
 ..I BGPP=29.1 S BGPC=1_U_"BH dx: 29.1"_U_$$DATE^BGP5UTL((9999999-$P(D,"."))) Q
 ..Q
 I BGPC Q BGPC
 Q ""
ALDX(P,BDATE,EDATE) ;
 ...
 I $D(BGPG(1)) Q 1_U_"DX: "_$P(BGPG(1),U,2)_U_$$DATE^BGP5UTL($P(BGPG(1),U))
 S BGPC=0
 ;go through BH record file and find up to 1 visits in date range
 S E=9999999-BDATE,D=9999999-EDATE-1_".99" F  S D=$O(^AMHREC("AE",P,D)) Q:D'=+D!($P(D,".")>E)!(BGPC)  S V=0 F  S V=$O(^AMHREC("AE",P,D,V)) Q:V'=+V!(BGPC)  D
 .S X=0 F  S X=$O(^AMHRPRO("AD",V,X)) Q:X'=+X!(BGPC)  S BGPP=$P($G(^AMHRPRO(X,0)),U) D
 ..Q:'BGPP
 ..S BGPP=$P($G(^AMHPROB(BGPP,0)),U)
 ..I BGPP=10 S BGPC=1_U_"BH dx: 10"_U_$$DATE^BGP5UTL((9999999-D)) Q
 ..I BGPP=27 S BGPC=1_U_"BH dx: 27"_U_$$DATE^BGP5UTL((9999999-D)) Q
 ..I BGPP=29 S BGPC=1_U_"BH dx: 29"_U_$$DATE^BGP5UTL((9999999-D)) Q
 ..;I BGPP=29.1 S BGPC=1_U_"BH dx: 29"_U_$$DATE^BGP5UTL((9999999-D)) Q
 ..I $E(BGPP,1,3)=303  S BGPC=1_U_"BH dx: "_BGPP_U_$$DATE^BGP5UTL((9999999-D)) Q
 ..I $E(BGPP,1,5)=305.0 S BGPC=1_U_"BH dx: "_BGPP_U_$$DATE^BGP5UTL((9999999-D)) Q
 ..I $E(BGPP,1,3)=291 S BGPC=1_U_"BH dx: "_BGPP_U_$$DATE^BGP5UTL((9999999-D)) Q
 ..I $E(BGPP,1,5)=357.5 S BGPC=1_U_"BH dx: "_BGPP_U_$$DATE^BGP5UTL((9999999-D)) Q
 ..Q
 I BGPC Q BGPC
 ...
 .S Y=$P(^AUPNPROB(X,0),U)
 .Q:'$$ICD^ATXCHK(Y,T,9)
 .S G=1_U_"PROB LIST: "_$P($$ICDDX^ICDCODE(Y),U,2)_U_$$DATE^BGP5UTL((9999999-D))
 .Q
 I G Q G
 S (X,G)=0 F  S X=$O(^AMHPPROB("AC",P,X)) Q:X'=+X!(G)  D
 .Q:$P(^AMHPPROB(X,0),U,12)'="A"
 ...
 .S Y=$P(^AMHPPROB(X,0),U)
 .S Y=$P($G(^AMHPROB(Y,0)),U)
 .I $E(Y,1,3)="303" S G=1_U_"BH PL: "_Y_U_$$DATE^BGP5UTL($P(^AMHPPROB(X,0),U,3)) Q
 .I $E(Y,1,5)="305.0" S G=1_U_"BH PL: "_Y_U_$$DATE^BGP5UTL($P(^AMHPPROB(X,0),U,3)) Q
 .I $E(Y,1,3)=291 S G=1_U_"BH PL: "_Y_U_$$DATE^BGP5UTL($P(^AMHPPROB(X,0),U,3)) Q
 .I $E(Y,1,5)=357.5 S G=1_U_"BH PL: "_Y_U_$$DATE^BGP5UTL($P(^AMHPPROB(X,0),U,3)) Q
 .I Y=10 S G=1_U_"BH PL: "_Y_U_$$DATE^BGP5UTL($P(^AMHPPROB(X,0),U,3)) Q
 .I Y=27 S G=1_U_"BH PL: "_Y_U_$$DATE^BGP5UTL($P(^AMHPPROB(X,0),U,3)) Q
 .I Y=29 S G=1_U_"BH PL: "_Y_U_$$DATE^BGP5UTL($P(^AMHPPROB(X,0),U,3)) Q
 .Q
 Q G
ALPED(P,BDATE,EDATE) ;
 K BGPG
 ...
 .Q:'$D(^AUTTEDT(T,0))
 .S T=$P(^AUTTEDT(T,0),U,2)
 .I $P(T,"-",1)="CD"!($P(T,"-",2)="CD")!($P(T,"-")="AOD")!($P(T,"-")="AOD") S D=1_U_"EDUC: "_T_U_$$DATE^BGP5UTL($P(BGPG(1),U))
 I D Q D
ALMH ;
 ...
 ..Q:'$D(^AUTTEDT(T,0))
 ..S T=$P(^AUTTEDT(T,0),U,2)
 ..I $P(T,"-",1)="CD"!($P(T,"-",2)="CD")!($P(T,"-")="AOD")!($P(T,"-")="AOD") S BGPC=1_U_"EDUC: "_T_U_$$DATE^BGP5UTL((9999999-D))
 Q BGPC
 ;
