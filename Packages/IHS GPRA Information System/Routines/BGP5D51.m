BGP5D51 ; IHS/CMI/LAB - measure I2 ; 26 Mar 2015  10:09 AM
...
ICRSAMM ;EP
 ...
 S BGPN2=$$EAPT^BGP5D52(DFN,$P(BGPC2,U),$$FMADD^XLFDT($P(BGPC2,U),114),84,30,114)
 S BGPN3=$$EAPT^BGP5D52(DFN,$P(BGPC2,U),$$FMADD^XLFDT($P(BGPC2,U),231),180,51,231)
 I BGPACTCB S BGPD1=1
 I BGPACTUP S BGPD2=1
 S BGPVALUE=$S($G(BGPRTYPE)=3:"AC+BH",BGPD1:"UP,AC+BH",1:"UP")
 S BGPV="IPSD: "_$$DATE^BGP5UTL($P(BGPC1,U,2))
 ...
 ;S BGPVALUE=BGPVALUE_" IPSD: "_$$DATE^BGP5UTL($P(BGPC1,U,2))_"|||"_BGPV
...
CRIT1DEN(P,BDATE,EDATE) ;
 ...
 .I $$ICD^BGP5UTL2(Y,T,9) S S($P($P(^AUPNVSIT(V,0),U),"."))=$S($P(^AUPNVSIT(V,0),U,7)'="H":$P($P(^AUPNVSIT(V,0),U),"."),1:$$DSCHDATE^APCLV(V,"I"))_U_V,G=1 Q  ;had one primary dx of major depression
 ...
 .S F=0 F  S Y=$O(Q(Y)) Q:Y'=+Y  I $$ICD^BGP5UTL2($P(Q(Y),U,3),T,9) S F=1
...
CPTV(V,T) ;does this visit have a cpt code in taxonomy T
 ...
 I '$D(^AUPNVSIT(V,0)) Q ""
 S G=0
 S X=$P(^AUPNVSIT(V,0),U,17) I X,$$ICD^BGP5UTL2(X,T,1) Q 1
 S X=0 F  S X=$O(^AUPNVCPT("AD",V,X)) Q:X'=+X!(G)  D
 .Q:'$D(^AUPNVCPT(X,0))
 .S Z=$P(^AUPNVCPT(X,0),U)
 .I $$ICD^BGP5UTL2(Z,T,1) S G=1
 .Q
 Q G
