BGP5D61 ; IHS/CMI/LAB - measure 31 01 Jul 2010 7:47 PM ;
...
COLO(P,EDATE,BDATE) ;EP
 ...
 I $D(BGPG(1)),$P(BGPLCOLO,U,3)<$P(BGPG(1),U,1) S BGPLCOLO="POV "_$P(BGPG(1),U,2)_"^"_$$DATE^BGP5UTL($P(BGPG(1),U))
...
BE(P,EDATE,BDATE) ;EP
 ...
 I T D  I X]"" S BGPLBE="CPT "_$P(X,U,2)_U_$$DATE^BGP5UTL($P(X,U,1))_U_$P(X,U,1)
 .S X=$$CPT^BGP5DU(P,BDATE,EDATE,T,5) I X]"" Q
 .S X=$$TRAN^BGP5DU(P,BDATE,EDATE,T,5)
 .Q
 S T=$O(^ATXAX("B","BGP BE CPTS",0))
 I T D  I X]"",$P(BGPLBE,U,3)<X Q "RAD "_$P(X,U,2)_U_$$DATE^BGP5UTL(X)_U_X
 .S X=$$RAD^BGP5DU(P,BDATE,EDATE,T,5)
 .Q
 Q BGPLBE
 ;
LOINC(A,B) ;
...
REF(P,BDATE,EDATE) ;EP
 ...
 .S T=""
 .S T=$$REFUSAL^BGP5UTL1(P,81,+$$CODEN^ICPTCOD(BGPT),$$FMADD^XLFDT(EDATE,-365),EDATE)
 .I T S T="1^"_$P(T,U,2)_"^FOB: Refused CPT"
 I $P(T,U)=1 Q T
 S BGPT=$O(^ATXAX("B","BGP SIG CPTS",0))
 I BGPT S G=$$RADREF^BGP5UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"SIG: Refused CPT"
 ;S BGPT=$O(^ATXAX("B","BGP BE CPTS",0))
 ;I BGPT S G=$$RADREF^BGP5UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"BE: Refused CPT"
 S BGPT=$O(^ATXAX("B","BGP COLO CPTS",0))
 I BGPT S G=$$RADREF^BGP5UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"COLO: Refused CPT"
 S BGPT=$O(^ATXAX("B","BGP SIG CPTS",0))
 I BGPT S G=$$CPTREFT^BGP5UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"SIG: Refused CPT"
 ;S BGPT=$O(^ATXAX("B","BGP BE CPTS",0))
 ;I BGPT S G=$$CPTREFT^BGP5UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"BE: Refused CPT"
 S BGPT=$O(^ATXAX("B","BGP COLO CPTS",0))
 I BGPT S G=$$CPTREFT^BGP5UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"COLO: Refused CPT"
 S T=$$PRCREFT^BGP5UTL1(P,BDATE,EDATE,$O(^ATXAX("B","BGP COLO PROCS",0)))
 I T Q "1^"_$P(T,U,2)_"^ COLO: Refused Proc"
 S T=$$PRCREFT^BGP5UTL1(P,BDATE,EDATE,$O(^ATXAX("B","BGP SIG PROCS",0)))
