...
TA ;EP - called from elder
 S BGPTOB=$$TOBHF^BGP5D712(DFN,BGPBDATE,BGPEDATE)  ;get last HF from the 3 categories
 ...
 .I BGPUDX]"" S BGPN2=1,BGPVAL=$$DATE^BGP5UTL($P(BGPUDX,U,2))_" SCREEN, "_$$DATE^BGP5UTL($P(BGPUDX,U,2))_" USER" Q
 .I BGPSMDX]"" S BGPN2=1,BGPVAL=$$DATE^BGP5UTL($P(BGPSMDX,U,2))_" SCREEN, "_$$DATE^BGP5UTL($P(BGPSMDX,U,2))_" USER" Q
 .I BGPSLDX]"" S BGPN2=1,BGPVAL=$$DATE^BGP5UTL($P(BGPSLDX,U,2))_" SCREEN, "_$$DATE^BGP5UTL($P(BGPSLDX,U,2))_" USER" Q
 .I ($P(BGPSCPT,U)="1034F")!($P(BGPSCPT,U)="1035F")!($P(BGPSCPT,U)="G0376")!($P(BGPSCPT,U)="G0375")!($P(BGPSCPT,U)=99407)!($P(BGPSCPT,U)="G9276") D  Q
 ..S BGPN2=1,BGPVAL=$$DATE^BGP5UTL($P(BGPSCPT,U,2))_" SCREEN, "_$$DATE^BGP5UTL($P(BGPSCPT,U,2))_" USER" Q
 .I $P(BGPSCPT,U)=99406!($P(BGPSCPT,U)="G8455")!($P(BGPSCPT,U)="G8456")!($P(BGPSCPT,U)="G8402")!($P(BGPSCPT,U)="G8453") S BGPN2=1,BGPVAL=$$DATE^BGP5UTL($P(BGPSCPT,U,2))_" SCREEN, "_$$DATE^BGP5UTL($P(BGPSCPT,U,2))_" USER" Q
 ;BGPN3 - SMOKER OR NOT
 D
 ...
 .I BGPSMDX]"" S BGPN3=1,BGPVAL=BGPVAL_", "_$$DATE^BGP5UTL($P(BGPSMDX,U,2))_" SMOKER" Q
 ...
 ..S BGPN3=1,BGPVAL=BGPVAL_", "_$$DATE^BGP5UTL($P(BGPSCPT,U,2))_" SMOKER" Q
 ;BGPN4 - SMOKELESS
 D
 ...
 .I $P(BGPSCPT,U)="1035F"!($P(BGPSCPT,U)="G8456") S BGPN4=1,BGPVAL=BGPVAL_", "_$$DATE^BGP5UTL($P(BGPSCPT,U,2))_" SMOKELESS" Q
 .I BGPSLDX]"" S BGPN4=1,BGPVAL=BGPVAL_", "_$$DATE^BGP5UTL($P(BGPSLDX,U,2))_" SMOKELESS" Q
 ;BGPN5 - ETS
 ...
 .I BGPSDX]"" S BGPVAL=$$DATE^BGP5UTL($P(BGPSDX,U,2))_" SCREEN" Q
 .I BGPSCPT]"" S BGPVAL=$$DATE^BGP5UTL($P(BGPSCPT,U,2))_" SCREEN" Q
 .I BGPXPHD]"" S BGPVAL=$$DATE^BGP5UTL($P(BGPXPHD,U,2))_" SCREEN" Q
 .I BGP1320]"" S BGPVAL=$$DATE^BGP5UTL($P(BGP1320,U,2))_" SCREEN" Q
...
PED(P,BDATE,EDATE) ;EP
 ...
 .I $P(S,U,1)'="-1",$$ICD^BGP5UTL2($P(S,U,1),$O(^ATXAX("B","BGP TOBACCO DXS",0)),9) S %=T_U_$P(BGPG(X),U) Q
...
MA ;now check for abortion or miscarriage
 ...
 S BGPG=$$LASTPRC^BGP5UTL1(P,"BGP ABORTION PROCEDURES",BGPD,EDATE)
 I BGPG Q 0
 ...
 S %=$$CPT^BGP5DU(P,BGPD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT MISCARRIAGE",0))
 S %=$$CPT^BGP5DU(P,BGPD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT ABORTION",0))
 S %=$$TRAN^BGP5DU(P,BGPD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT MISCARRIAGE",0))
 S %=$$TRAN^BGP5DU(P,BGPD,EDATE,T,3)
 I %]"" Q 0
...
DX(P,BDATE,EDATE) ;EP  - WAS THERE SCREENING?
 K BGPG
 S BGPG(1)=$$LASTDX^BGP5UTL1(P,"BGP TOBACCO DXS",BDATE,EDATE)
 I BGPG(1)]"" Q $P($$ICDDX^BGP5UTL2($P(BGPG(1),U,4),$P(BGPG(1),U,1)),U,2)_U_$P(BGPG(1),U,3)_U_$P(BGPG(1),U,4)
 ...
 .S Y=$P(^AUPNPROB(X,0),U)
 .Q:'$$ICD^BGP5UTL2(Y,T,9)
 .S G=$P($$ICDDX^BGP5UTL2(Y),U,2)_U_$P(^AUPNPROB(X,0),U,3)_U_Y
 .Q
 Q G
DXSL(P,BDATE,EDATE) ;EP - WAS THERE A SMOKELESS USER DX?
 K BGPG
 S BGPG(1)=$$LASTDX^BGP5UTL1(P,"BGP GPRA SMOKELESS DXS",BDATE,EDATE)
 I BGPG(1)]"" Q $P($$ICDDX^BGP5UTL2($P(BGPG(1),U,4),$P(BGPG(1),U,1)),U,2)_U_$P(BGPG(1),U,3)_U_$P(BGPG(1),U,4)
...
DXU(P,BDATE,EDATE) ;EP - WAS THERE A USER DX?
 K BGPG
 S BGPG(1)=$$LASTDX^BGP5UTL1(P,"BGP TOBACCO USER DXS",BDATE,EDATE)
 I BGPG(1)]"" Q $P($$ICDDX^BGP5UTL2($P(BGPG(1),U,4),$P(BGPG(1),U,1)),U,2)_U_$P(BGPG(1),U,3)_U_$P(BGPG(1),U,4)
...
DXS(P,BDATE,EDATE) ;EP - WAS THERE A SMOKING USER DX?
 K BGPG
 S BGPG(1)=$$LASTDX^BGP5UTL1(P,"BGP GPRA SMOKING DXS",BDATE,EDATE)
 I BGPG(1)]"" Q $P($$ICDDX^BGP5UTL2($P(BGPG(1),U,4),$P(BGPG(1),U,1)),U,2)_U_$P(BGPG(1),U,3)_U_$P(BGPG(1),U,4)
...
TOBACCO0 ;lookup in health status
 ...
 S Y=$P(^AUTTHF(Y,0),U)
 S BGPTOB=Y_"^"_$$DATE^BGP5UTL(9999999-E)_"^"_(9999999-E)
 K Y,E,X
 Q
 ;
LASTHF(P,C,BDATE,EDATE) ;EP - get last factor in category C for patient P
 ...
 S D=$O(O(0))
 I D="" Q D
 Q $$VAL^XBDIQ1(9000010.23,O(D),.01)_"^"_$$DATE^BGP5UTL(9999999-D)_"^"_(9999999-D)
 ;
LOINC(A,B) ;
 NEW %
...
CPTSM(P,BDATE,EDATE) ;EP - did pat have TOBACCO SCREENING cpt?
 NEW X
 S X=$$CPT^BGP5DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP TOBACCO SCREEN CPTS",0)),5)
