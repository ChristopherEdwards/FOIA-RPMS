BGP5D72 ; IHS/CMI/LAB - indicator 31 ;
...
I30 ;EP
 ...
 I $$READM(DFN,$P(BGPAMI,U,4)) S BGPSTOP=1 Q
 S BGPV=$P(BGPAMI,U,3)
...
I32 ;EP
 ...
 S BGPAMI=$$AMIO(DFN,$$FMADD^XLFDT(BGPBDATE,-365),$$FMADD^XLFDT(BGPBDATE,-1)) I '$P(BGPAMI,U) S BGPSTOP=1 Q
 I BGPACTUP S BGPD2=1
 I BGPACTCL S BGPD1=1
 I BGPRTYPE=3,'BGPD1 S BGPSTOP=1 Q
 S D=$P(BGPAMI,U,2),BGPLDL=$$LDL^BGP5D2(DFN,$$FMADD^XLFDT(D,60),$$FMADD^XLFDT(D,365))
 I $P(BGPLDL,U)=1 S BGPN1=1
 I $P(BGPLDL,U,3)]"" D
 .S V=$P(BGPLDL,U,3)
 .I V]"",+V'>100 S BGPN2=1
 .I +V>100,+V<131 S BGPN3=1
 S V=$S(BGPRTYPE=3:"",BGPD2:"UP")_$S(BGPD1:";AC",1:"")_";"_$P(BGPAMI,U,3)_"|||"
...
AMIO(P,BDATE,EDATE) ;
 ...
 I $D(BGPG(1)) D  Q 1_U_D_U_"AMI DX"  ;has a dx
 .S D=""
 .S E=+$P(BGPG(1),U,4)  ;ien of v pOV
 .S V=$P($G(^AUPNVPOV(E,0)),U,3)
 .I $P($G(^AUPNVSIT(V,0)),U,7)'="H" S D=$P(BGPG(1),U) Q
 .S D=$$DSCHDATE^APCLV(V,"I")
 .I D="" S D=$P(BGPG(1),U)
 .Q
 ;check for procedure in BGP CABG PROCS
 S BGPG=$$LASTPRC^BGP5UTL1(P,"BGP CABG PROCS",BDATE,EDATE)
 I $P(BGPG,U) D  Q 1_U_D_U_"CABG PROC"
 .S D=""
 .S E=+$P(BGPG,U,5)  ;ien of v proc
 .S V=$P($G(^AUPNVPRC(E,0)),U,3)
 .I $P($G(^AUPNVSIT(V,0)),U,7)'="H" S D=$P(BGPG,U,3) Q
 .S D=$$DSCHDATE^APCLV(V,"I")
 .I D="" S D=$P(BGPG,U,3)
 .Q
 ;now check cpts
 S BGPG=$$CPT^BGPDU(P,BDATE,EDATE,"BGP CABG CPTS",6)
 I $P(BGPG,U) D  Q 1_U_D_U_"CABG CPT"
 .S D=""
 .S E=+$P(BGPG,U,4)  ;ien of v proc
 .S V=$P($G(^AUPNVCPT(E,0)),U,3)
 .I $P($G(^AUPNVSIT(V,0)),U,7)'="H" S D=$P(BGPG,U,2) Q
 .S D=$$DSCHDATE^APCLV(V,"I")
 .I D="" S D=$P(BGPG,U,2)
 .Q
 ;now check for PTCA
 ;check for procedure in BGP PTCA PROCS
 S BGPG=$$LASTPRC^BGP5UTL1(P,"BGP PTCA PROCS",BDATE,EDATE)
 I $P(BGPG,U) D  Q 1_U_D_U_"PTCA PROC"
 .S D=""
 .S E=+$P(BGPG,U,5)  ;ien of v proc
 .S V=$P($G(^AUPNVPRC(E,0)),U,3)
 .I $P($G(^AUPNVSIT(V,0)),U,7)'="H" S D=$P(BGPG,U,3) Q
 ...
 .Q
 ;now check cpts
 S BGPG=$$CPT^BGPDU(P,BDATE,EDATE,"BGP PTCA CPTS",6)
 I $P(BGPG,U) D  Q 1_U_D_U_"PTCA CPT"
 .S D=""
 .S E=+$P(BGPG,U,4)  ;ien of v proc
 .S V=$P($G(^AUPNVCPT(E,0)),U,3)
 .I $P($G(^AUPNVSIT(V,0)),U,7)'="H" S D=$P(BGPG,U,2) Q
...
BETACONT(P,EDATE) ;
 ...
 I G>1 Q 1  ;has 2 dx of asthma so contraindication
 S BGPG=$$LASTDX^BGP5UTL1(P,"BGP HYPOTENSION DXS",$$DOB^AUPNPAT(P),EDATE)
 I $P(BGPG,U)=1 Q 1  ;has hypotension dx
 S BGPG=$$LASTDX^BGP5UTL1(P,"BGP CMS 2/3 HEART BLOCK DXS",$$DOB^AUPNPAT(P),EDATE)
 I $P(BGPG,U)=1 Q 1  ;has heart block dx
 K BGPG,BGPD
 S Y="BGPG("
...
BETA(P,BGPADMD,BGPDD) ;EP
 ...
 S T1=$O(^ATXAX("B","BGP CMS BETA BLOCKER NDC",0))
 S T2=$O(^ATXAX("B","BGP CMS BETA BLOCKER CLASS",0))
...
BETA1 .;
 ...
 I K>134 Q 1_U_R_" total days beta blocker: "_K
...
BETA8 .;
 ...
 .Q
 Q BGPG
BETAALG1(P,BGPD) ;does patient have an ACEI allergy
...
BETAPOV ;
 ...
 .S Z=$P(^AUPNVPOV(Y,0),U,9) I Z]"",$P($$ICDDX^ICDCODE(Z),U,2)="E942.0" S BGPC=BGPC+1,BGPY(BGPC)="POV:  "_$$DATE^BGP5UTL($P(BGPG(X),U))_"  ["_$P(BGPG(X),U,2)_" + E942.0]  "_N
 .Q
 I BGPC>0 Q 1
 ...
 .I Y="V14.8"!($$ICD^ATXCHK(I,T,9)),N["BETA BLOCK"!(N["BBLOCK")!(N["B BLOCK") S BGPC=BGPC+1,BGPY(BGPC)="PROBLEM LIST:  "_$$DATE^BGP5UTL($P(^AUPNPROB(X,0),U,8))_"  ["_Y_"]  "_N
 .Q
 I BGPC>0 Q 1
