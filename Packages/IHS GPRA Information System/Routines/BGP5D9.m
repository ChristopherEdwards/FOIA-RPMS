...
I0303 ;EP
 ...
 I BGPRTYPE=4 I $$ESRD^BGP5D211(DFN,$P(^DPT(DFN,0),U,3),BGPEDATE) S BGPSTOP=1 Q  ;esrd anytime before end date
 ...
 S BGPVALUE=$$MEANBP^BGP5D41(DFN,BGPBDATE,BGPEDATE,1)
 I BGPVALUE["unknown" S BGPVALUE=$$BPCPT^BGP5D22(DFN,BGPBDATE,BGPEDATE) I BGPVALUE]"" S BGPN1=1,BGPVALUE=$P(BGPVALUE,U,2)_U_0 G V
...
V S X=$$MEANBPD^BGP5D2(DFN,BGPBDATE,BGPEDATE,1,BGPAGEB)
 ;I X="" S X=$$BPCPT^BGP5D22(DFN,BGPBDATE,BGPEDATE,1) I X]"" D  G V1
...
IC2 ;EP
 ...
 S V=V_"|||"_$$DATE^BGP5UTL($P(BGPVALUE,U,1))_" "_$P(BGPVALUE,U,2)
 S BGPVALUE=V
...
MTM(P,BDATE,EDATE) ;
 NEW %
 S %=""
 S %=$$CPT^BGP5DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP CPT MTM",0)),5)
 ...
 ..I C'="D1",C'="D2",C'="D5" Q
 ..S G=1_U_$$DATE^BGP5UTL($$VD^APCLV(V))_" Cl "_C
 I G]"" Q G
 Q ""
ISMT ;
 S (BGPD1,BGPN1)=0
 I 'BGPACTCL Q  ;only active clinical in this measure
 S BGPVALUE=$$LASTHF^BGP5D7(DFN,"CONFIDENCE IN MANAGING HEALTH PROBLEMS",BGPBDATE,BGPEDATE)
...
ESRD(P,EDATE) ;
 ...
 I $D(BGPG(1)) Q 1
 S E=+$$CODEN^ICPTCOD(90921),X=$$CPTI^BGP5DU(P,$$DOB^AUPNPAT(P),EDATE,E)
 I $P(X,U) Q 1
 S E=+$$CODEN^ICPTCOD(90925),X=$$CPTI^BGP5DU(P,$$DOB^AUPNPAT(P),EDATE,E)
 I $P(X,U) Q 1
 S E=+$$CODEN^ICPTCOD(90921),X=$$TRANI^BGP5DU(P,$$DOB^AUPNPAT(P),EDATE,E)
 I $P(X,U) Q 1
 S E=+$$CODEN^ICPTCOD(90925),X=$$TRANI^BGP5DU(P,$$DOB^AUPNPAT(P),EDATE,E)
 I $P(X,U) Q 1
 Q ""
FIRSTIHD(P,EDATE) ;EP
 I $G(P)="" Q ""
...
MH ;EP
 ...
 I $$ESRD^BGP5D211(DFN,$P(^DPT(DFN,0),U,3),BGPEDATE) S BGPSTOP=1 Q  ;esrd anytime before end date
 S X=$$PREG^BGP5D7(DFN,BGPBDATE,BGPEDATE,1,1) I X S BGPSTOP=1 Q
...
MHHTN(P,BDATE,EDATE) ;EP
 ...
 S X=$$LASTDX^BGP5UTL1(P,"BGP HYPERTENSION DXS",B,S)
 ...
 S X=$$LASTDX^BGP5UTL1(P,"BGP HYPERTENSION DXS",BDATE,EDATE)
 I X Q 1
 Q 0
LASTBP(P,BDATE,EDATE) ;EP
 NEW X
 S X=$$BPS^BGP5D2(P,BDATE,EDATE,"I",1)
 I X="" Q ""
 Q $P(X,";",1)
