...
IC1 ;EP
 ...
 I $Y>(BGPIOSL-16),'$G(BGPSUMON) D HEADER^BGP5DPH Q:BGPQUIT  W !,^BGPINDK(BGPIC,53,1,0) W:$D(^BGPINDK(BGPIC,53,2,0)) !,^BGPINDK(BGPIC,53,2,0)
 D H1^BGP5DPH
 ...
 Q:BGPQUIT
 D:'$G(BGPSUMON) HEADER^BGP5DPH
 F BGPPC1=BGPORXX_".7",BGPORXX_".8",BGPORXX_".9" Q:BGPQUIT  D PI^BGP5DP1C
 Q:BGPQUIT
 D I1AGE
 Q
...
I1AGE1 ;
 ;gather up all #'s
 S C=C+1
 S BGPF="C-1."_BGPX_".3" S BGPPC=$O(^BGPINDKC("C",BGPF,0))
...
I1AGE2 ;
 S D=D+1
 S BGPF="C-1."_BGPX_".2" S BGPPC=$O(^BGPINDKC("C",BGPF,0))
...
I1AGE3 ;
 S BGPF="C-1."_BGPX_".1" S BGPPC=$O(^BGPINDKC("C",BGPF,0))
 ;set 6th piece to numerator and 7th to %
 S BGPNF=$P(^BGPINDKC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90554.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(D),U,6)=$$V^BGP5DP1C(1,BGPRPT,N,P),$P(BGPDAC(D),U,7)=$S($P(BGPDAC(D),U,1):($P(BGPDAC(D),U,6)/$P(BGPDAC(D),U,1)*100),1:"")
 S $P(BGPDAP(D),U,6)=$$V^BGP5DP1C(2,BGPRPT,N,P),$P(BGPDAP(D),U,7)=$S($P(BGPDAP(D),U,1):($P(BGPDAP(D),U,6)/$P(BGPDAP(D),U,1)*100),1:"")
 S $P(BGPDAB(D),U,6)=$$V^BGP5DP1C(3,BGPRPT,N,P),$P(BGPDAB(D),U,7)=$S($P(BGPDAB(D),U,1):($P(BGPDAB(D),U,6)/$P(BGPDAB(D),U,1)*100),1:"")
I1AGE4 ;
 S BGPF="C-1."_BGPX_".4" S BGPPC=$O(^BGPINDKC("C",BGPF,0))
 ;set 8th piece to numerator and 9th to %
 S BGPNF=$P(^BGPINDKC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90554.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(D),U,8)=$$V^BGP5DP1C(1,BGPRPT,N,P),$P(BGPDAC(D),U,9)=$S($P(BGPDAC(D),U,1):($P(BGPDAC(D),U,8)/$P(BGPDAC(D),U,1)*100),1:"")
 S $P(BGPDAP(D),U,8)=$$V^BGP5DP1C(2,BGPRPT,N,P),$P(BGPDAP(D),U,9)=$S($P(BGPDAP(D),U,1):($P(BGPDAP(D),U,8)/$P(BGPDAP(D),U,1)*100),1:"")
 S $P(BGPDAB(D),U,8)=$$V^BGP5DP1C(3,BGPRPT,N,P),$P(BGPDAB(D),U,9)=$S($P(BGPDAB(D),U,1):($P(BGPDAB(D),U,8)/$P(BGPDAB(D),U,1)*100),1:"")
 Q
I1AGEP ;
 D I1AGEP^BGP5DP60
 Q
