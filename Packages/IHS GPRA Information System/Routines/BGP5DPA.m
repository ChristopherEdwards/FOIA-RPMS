BGP5DPA ; IHS/CMI/LAB - COMP NATIONAL GPRA FOR PTS W/APPT 03 Jun 2015 2:54 PM ; 30 Oct 2014  12:13 PM
...
EP ;EP - called from option interactive
 ...
 I 'X W !!,"Exiting Report....." D PAUSE^BGP5DU,EOJ Q
RTYPE ;
 S BGPRT1=""
...
ADDON ;
 ...
 ;
ZIS ;
 S BGPRTYPE=1,BGPYRPTH="",BGPCPPL=1,BGPINDK="G",BGPYGPU=1,BGPALLPT=1,BGPBEN=3
...
NT S BGPLIST="A"
 S BGPCPLC=0
 ;S XBRP="PRINT^BGP5DPAW",XBRC="PROC^BGP5DPA",XBRX="EOJ^BGP5DPA",XBNS="BGP"
 ;D ^XBDBQUE
 ;
 K IOP,%ZIS
 W !! S %ZIS="PQM" D ^%ZIS
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP5DPA",ZTDTH="",ZTDESC="CRS 14 SCHED REPORT" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D ^XBFMK
 K DIC,DIR,DFN
...
SELPT ;
 ...
 .S DIR("?",5)="",DIR("?")="     ""[??"" will list your templates.",DIR("??")="^D LIST^BGP5DPAP"
 .D ^DIR K DIR
 .S:X[U X=U
 .I $E(X)="[" D  Q
 .. S X=$E(X,2,$L(X))
...
BDMG(BGPBD,BGPED,BGPCLN) ;EP - GUI DMS Entry Point
 ...
 S BGPRTYPE=1,BGPYRPTH="",BGPCPPL=1,BGPINDK="G",BGPYGPU=1
 D NOW^%DTC
 S BGPNOW=$G(%)
 K DD,D0,DIC
 S X=DUZ_$$NOW^XLFDT
 S DIC("DR")=".02////"_DUZ_";.03////"_BGPNOW_";.06///"_$G(BGPOPT)_";.07////R"
 S DIC="^BGPGUIK(",DIC(0)="L",DIADD=1,DLAYGO=9001004.4
 D FILE^DICN
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BGP5DPA",ZTDESC="GUI CN GRPA APPT" D ^%ZTLOAD
 D EOJ
 Q
GUIEP ;EP
 D PROC
 K ^TMP($J,"BGP5DPA")
 S IOM=80
 D GUIR^XBLM("PRINT^BGP5DPA","^TMP($J,""BGP5DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP5DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP5DPA",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIK(BGPIEN,11,X,0)=BGPDATA,C=C+1
 S ^BGPGUIK(BGPIEN,11,0)="^^"_C_"^"_C_"^"_DT_"^"
 S DA=BGPIEN,DIK="^BGPGUIK(" D IX1^DIK
 D ENDLOG
 K ^TMP($J,"BGP5DPA")
 S ZTREQ="@"
 Q
...
GUIECP ;EP
 K ^TMP($J,"BGP5DPA")
 S IOM=80
 D GUIR^XBLM("CPPRINT^BGP5DPA","^TMP($J,""BGP5DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP5DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP5DPA",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIK(BGPIEN,11,X,0)=BGPDATA,C=C+1
...
ENDLOG ;-- write the end of the log
 D NOW^%DTC
 S BGPNOW=$G(%)
 S DIE="^BGPGUIK(",DA=BGPIEN,DR=".04////"_BGPNOW_";.06////C"
 D ^DIE
 K DIE,DR,DA
 Q
 ;
PROC ;EP
 ...
 K ^XTMP("BGP5DPA",BGPGPRAJ,BGPGPRAH)
 S ^XTMP("BGP5DPA",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^"_"PTS WITH GPRA DATA"
 K BGPAPPT,BGPAPPTS
 S BGPTA=0
 I BGPRT1="A" D  Q
 ...
 ..S BGPIISO=1,BGPISST="A" D PROCCY^BGP5D1
 .Q
 ...
 .S X=$O(^DIBT("B","RPMS DEMO PATIENT NAMES",0)) I X Q:$D(^DIBT(X,1,DFN))
 .I $G(BGPSTMP) S ^DIBT(BGPSTMP,1,DFN)="" Q
 .S BGPIISO=1,BGPISST=BGPRT1,BGPISSO=1 D PROCCY^BGP5D1
 .Q
 Q
...
ZISCP ;
 ...
 ;
 I $D(IO("Q")) G TSKMNCP
 ;S XBRP="CPPRINT^BGP5DPA",XBRC="",XBRX="EOJ^BGP5DPA",XBNS="BGP"
 ;D ^XBDBQUE
 ;
CPPRINT ;EP - called from xbdbque
 ...
 D CPHEADER
 S BGPGYR=2015,BGPGYR=$O(^BGPCTRL("B",BGPGYR,0))
...
TSKMNCP ;EP ENTRY POINT FROM TASKMAN
 ...
 I $D(IOM)#2,IOM S ZTIO=ZTIO_";"_IOM I $D(IOSL)#2,IOSL S ZTIO=ZTIO_";"_IOSL
 K ZTSAVE S ZTSAVE("BGP*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="CPPRINT^BGP5DPA",ZTDTH="",ZTDESC="CRS 14 SCHED REPORT DENOM" D ^%ZTLOAD D EOJ Q
 Q
STMP ;EP
EN1 ;EP Help
 K BGPQUIT S BGPSTMP=""
