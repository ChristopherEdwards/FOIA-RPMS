BGP5DPE ; IHS/CMI/LAB - IHS GPRA 10 REPORT DRIVER ;
 ;;15.0;IHS CLINICAL REPORTING;;NOV 18, 2014;Build 134
 ;
 ;
 W:$D(IOF) @IOF
 ;W !,$$CTR("IHS 2015 Patient Education w/Community Specified Report",80),!!
INTRO ;
 D XIT
INTRO1 ;
...
MSR ;BEGIN TEST STUFF
 ...
 I $D(DIRUT) D XIT Q
 S BGPZZ=Y
 I BGPZZ="S" D EN^BGP5PESI I '$D(BGPIND) W !!,"No measures selected" G INTRO
...
SUM ;display summary of this report
 ...
 I BGPPEEXP,BGPUF="" W:'$D(ZTQUEUED) !!,"Cannot continue.....can't find export directory name. EXCEL file",!,"not written." D PAUSE^BGP5DU,XIT Q
 W:$D(IOF) @IOF
 W !,$$CTR("SUMMARY OF IHS 2015 PATIENT EDUCATION REPORT TO BE GENERATED")
 W !!,"The date ranges for this report are:"
 W !?5,"Report Period: ",?31,$$FMTE^XLFDT(BGPBD)," to ",?31,$$FMTE^XLFDT(BGPED)
 W !?5,"Previous Year Period: ",?31,$$FMTE^XLFDT(BGPPBD)," to ",?31,$$FMTE^XLFDT(BGPPED)
 W !?5,"Baseline Period: ",?31,$$FMTE^XLFDT(BGPBBD)," to ",?31,$$FMTE^XLFDT(BGPBED)
...
ZIS ;call to XBDBQUE
 ...
 I BGPRPT="" D XIT Q
 I BGPPEEXP D
 .W !!,"A file will be created called BG150",$P(^AUTTLOC(DUZ(2),0),U,10)_".PED"_BGPRPT," and will reside",!,"in the ",BGPUF," directory.",!
 ...
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPPEDCK(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPPEDPK(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPPEDBK(" D ^DIK K DIK D XIT Q
 I $D(IO("Q")) G TSKMN
DRIVER ;
 D ^BGP5DPE1
 U IO
 D ^BGP5DPEP
 D ^%ZISC
 I BGPPEEXP D GS^BGP5PUTL
 D XIT
 Q
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP5DPE",XBRX="XIT^BGP5DPE",XBNS="BGP"
 D ^XBDBQUE
 ;D XIT
 Q
 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP5DPE",ZTDTH="",ZTDESC="PAT ED 09 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
LISTS ;any lists with measures?
 ...
 I $D(BGPLIST) D RT^BGP5PESL I '$D(BGPLIST)!($D(BGPQUIT)) G LISTS
 Q
CHKY ;
