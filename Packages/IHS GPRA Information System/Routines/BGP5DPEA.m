BGP5DPEA ; IHS/CMI/LAB - IHS GPRA 10 REPORT DRIVER 23 Oct 2009 2:04 PM ;
 ;;15.0;IHS CLINICAL REPORTING;;NOV 18, 2014;Build 134
 ;
 ;
 W:$D(IOF) @IOF
 W !,$$CTR("IHS 2015 Area Aggregate Patient Education Report",80),!!
INTRO ;
 D XIT
...
ASU ;
 ...
 I C=1 S BGPRPTT="F",BGPSUCNT=1,Y=$O(BGPSUL(0)),X=$P(^BGPPEDCK(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 I C>1 S BGPRPTT="A"
ZIS ;call to XBDBQUE
 D ^XBFMK
 S X=0 F  S X=$O(^BGPPEIK(X)) Q:X'=+X  S BGPIND(X)=""
...
DRIVER ;
 ...
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP5DPEA",XBRX="XIT^BGP5DPEA",XBNS="BGP"
 D ^XBDBQUE
 ;D XIT
 Q
 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP5DPEA",ZTDTH="",ZTDESC="PAT ED 09 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
