BGP5DPEP ; IHS/CMI/LAB - IHS gpra print 01 Jul 2010 7:54 PM ;
...
DEL ;create delimited output file
 ...
 S BGPQHDR=0
 D DEL^BGP5DPED
 K ^XTMP("BGP5PE",BGPJ,BGPH)
 K ^TMP($J)
 Q
WP ;
 K ^UTILITY($J,"W")
 S BGPZ=0,BGPLCNT=0
 S DIWL=1,DIWR=80,DIWF="",BGPZ=0 F  S BGPZ=$O(^BGPPEIK(BGPIC,BGPNODE,BGPY,11,BGPZ)) Q:BGPZ'=+BGPZ  D
 .S BGPLCNT=BGPLCNT+1
 .S X=^BGPPEIK(BGPIC,BGPNODE,BGPY,11,BGPZ,0) S:BGPLCNT=1 X=" - "_X D ^DIWP
 .Q
WPS ;
...
PRINT1 ;EP
 ...
 .W !!,$P(^BGPPEIK(BGPIC,0),U,2)
 .I $G(BGPDNT) G CALC
 .I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 .W !!,"Denominator(s):"
 .S BGPNODE=61
 .S BGPX=0 F  S BGPX=$O(^BGPPEIK(BGPIC,61,"B",BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIK(BGPIC,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY!(BGPQUIT)  D
 ...D WP
 .I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 .W !!,"Numerator(s):"
 .S BGPNODE=62
 .S BGPX=0 F  S BGPX=$O(^BGPPEIK(BGPIC,62,"B",BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIK(BGPIC,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY!(BGPQUIT)  D
 ...D WP
 .S BGPNODE=11
 .W ! S BGPX=0 F  S BGPX=$O(^BGPPEIK(BGPIC,BGPNODE,BGPX)) Q:BGPX'=+BGPX  D
 ..I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ..W !,^BGPPEIK(BGPIC,BGPNODE,BGPX,0)
CALC .D @BGPIC
 .Q:BGPQUIT
 ;
 D ^BGP5DPEL
 ...
 Q
1 ;
 I $Y>(BGPIOSL-6) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIK(BGPIC,0),U,2)
 D H1^BGP5DPH
...
2 ;
 ...
 ..I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIK(BGPIC,0),U,2) D H1^BGP5DPH W !
 ...
 I $Y>(BGPIOSL-6) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIK(BGPIC,0),U,2) D H1^BGP5DPH W !
 ...
 ..I $P($G(^BGPPEDCK(X,11)),U,4)]"",$P($G(^BGPPEDCK(X,11)),U,4)<BGPCYD S BGPCYD=$P($G(^BGPPEDCK(X,11)),U,4)
 ..I $P($G(^BGPPEDPK(X,11)),U,4)]"",$P($G(^BGPPEDPK(X,11)),U,4)<BGPPRD S BGPPRD=$P($G(^BGPPEDPK(X,11)),U,4)
 ..I $P($G(^BGPPEDBK(X,11)),U,4)]"",$P($G(^BGPPEDBK(X,11)),U,4)<BGPBLD S BGPBLD=$P($G(^BGPPEDBK(X,11)),U,4)
 ...
 I $Y>(BGPIOSL-4) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIK(BGPIC,0),U,2) D H1^BGP5DPH W !
 W !,"Minimum Time Spent",!,"All Providers",!,"(minutes)"
 W ?20,$$C(BGPCYD,0,8),?35,$$C(BGPPRD,0,8),?58,$$C(BGPBLD,0,8),!
 I $G(BGPAREAA) D
 .S (BGPCYD,BGPPRD,BGPBLD)=""
 .S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 ..I $P($G(^BGPPEDCK(X,11)),U,5)>BGPCYD S BGPCYD=$P($G(^BGPPEDCK(X,11)),U,5)
 ..I $P($G(^BGPPEDPK(X,11)),U,5)>BGPPRD S BGPPRD=$P($G(^BGPPEDPK(X,11)),U,5)
 ..I $P($G(^BGPPEDBK(X,11)),U,5)>BGPBLD S BGPBLD=$P($G(^BGPPEDBK(X,11)),U,5)
 I '$G(BGPAREAA) D
...
HEADER1 ;
 ...
 I '$G(BGPSEAT) W $$CTR("*** IHS 2015 Patient Education with Community Specified Report ***",80),!
 I $G(BGPSEAT) W $$CTR("*** IHS 2015 Patient Education with Patient Panel Population Report ***",80),!
...
SETEXA(T,N,P) ;EP - set denominator
 ...
 NEW X,Y,Z
 S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 .I T=1 S $P(BGPEXCT(X,T),U,2)=$P($G(^BGPPEDCK(X,N)),U,P)
 .I T=2 S $P(BGPEXCT(X,T),U,2)=$P($G(^BGPPEDPK(X,N)),U,P)
 .I T=3 S $P(BGPEXCT(X,T),U,2)=$P($G(^BGPGPDBK(X,N)),U,P)
 Q
V(T,R,N,P,ND) ;EP ;SPDX
 I $G(BGPAREAA) G VA
 NEW X
 I T=1 S X=$P($G(^BGPPEDCK(R,N)),U,P) Q $S(X]"":X,1:0)
 I T=2 S X=$P($G(^BGPPEDPK(R,N)),U,P) Q $S(X]"":X,1:0)
 I T=3 S X=$P($G(^BGPPEDBK(R,N)),U,P) Q $S(X]"":X,1:0)
 Q ""
VA ;
 NEW X,V,C S X=0,C="" F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 .I T=1 S C=C+$P($G(^BGPPEDCK(X,N)),U,P)
 .I T=2 S C=C+$P($G(^BGPPEDPK(X,N)),U,P)
 .I T=3 S C=C+$P($G(^BGPPEDBK(X,N)),U,P)
 .Q
 Q $S(C]"":C,1:0)
