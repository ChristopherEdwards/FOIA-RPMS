BGP5DSTM ; IHS/CMI/LAB - national patient list 20 Dec 2004 9:24 AM 07 Mar 2010 2:31 PM ;
...
TP ;get time period
 D XIT
 S BGPRTYPE=1,BGPYRPTH="",BGPNPL=1,BGPINDK="G",BGPYGPU=1,BGPYNPLT=1
SI ;
 K DIRUT
 K BGPIND,BGPSTALL
 D EN^BGP5DSI
...
SI1 ;NOW SELECT ONE OR MORE W/IN THE TOPIC
 ...
 .K BGPX S BGPO=0,X=0,BGPC=0 F  S BGPO=$O(^BGPNPLK("AN",BGPIND,BGPO)) Q:BGPO'=+BGPO!($D(DIRUT))  D
 ..S X=$O(^BGPNPLK(BGPCR,BGPIND,BGPO,0))
 ..;I BGPRTYPE=1,$P(^BGPNPLK(X,0),U,4)'="N" Q
 ..;I BGPRTYPE=7,$P(^BGPNPLK(X,0),U,4)'="O" Q
 ..S BGPX(BGPO,X)="",BGPC=BGPC+1
 .;display the choices
 .W !!!,"Please select one or more of these report choices within the",!,IORVON,$P(^BGPINDK(BGPIND,0),U,3),IORVOFF," performance measure topic.",!
 .K BGPY S X=0,BGPC=0,BGPO=0 F  S BGPO=$O(BGPX(BGPO)) Q:BGPO'=+BGPO!($D(DIRUT))  S X=0 F  S X=$O(BGPX(BGPO,X)) Q:X'=+X!($D(DIRUT))  S BGPC=BGPC+1 W !?5,BGPC,")",?9,$P(^BGPNPLK(X,0),U,3) S BGPY(BGPC)=X
...
SUM ;display summary of this report
 ...
 .W !?2,$P(^BGPNPLK(Y,0),U,3),":  ",$P(^DIBT(BGPINDL(X,Y,"TEMP"),0),U)
 D PT^BGP5DSL
 I BGPROT="" G COMM
ZIS ;call to XBDBQUE
 D REPORT^BGP5UTL
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP5NPL",ZTDTH="",ZTDESC="NATIONAL GPRA/GPRAMA REPORT 06" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
EN1 ;EP Help
 ...
 S X=0 F  S X=$O(^BGPNPLK(BGPII,11,X)) Q:X'=+X  W !?3,^BGPNPLK(BGPII,11,X,0)
...
CT ;EP - create search templates and write message
 ...
 I BGPPTYPE="D" D W^BGP5DP(" ",0,2,BGPPTYPE)  ;S X=" " D S^BGP5NPLD(X,1,1) S X=" " D S^BGP5NPLD(X,1,1)
 S BGPORD=$P($G(^BGPINDK(BGPIC,12)),U,6)
 S O=0 F  S O=$O(^XTMP("BGP5DNP",BGPJ,BGPH,"LIST",O)) Q:O'=+O  D
 .S X=0 F  S X=$O(^XTMP("BGP5DNP",BGPJ,BGPH,"LIST",O,X)) Q:X'=+X  D
 ..S Y=0 F  S Y=$O(^XTMP("BGP5DNP",BGPJ,BGPH,"LIST",O,X,Y)) Q:Y'=+Y  D
 ...S T=$G(BGPINDL(X,Y,"TEMP"))
 ...I T="" Q
 ...I '$D(^DIBT(T,0)) Q
 ...L +^DIBT(T):10 I '$T Q
 ...S C=0
 ...S P=0 F  S P=$O(^XTMP("BGP5DNP",BGPJ,BGPH,"LIST",O,X,Y,P)) Q:P'=+P  D
 ....S C=C+1
 ....S ^DIBT(T,1,P)=""
 ....Q
 ...L -^DIBT(T)
 ...D W^BGP5DP("Search template: "_$P(^DIBT(T,0),U)_" created with "_C_" members.",0,2,BGPPTYPE)
 D W^BGP5DP("",0,1,BGPPTYPE)
 Q
