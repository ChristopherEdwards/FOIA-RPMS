BGP5ELHH ; IHS/CMI/LAB - cover page for ELDER ;
 ...
 S X="*** IHS 2005 ELDER Care Clinical Performance Indicators ***" D S(X,1,1)
 ...
 S X=" " D S(X,1,1)
 I BGPZZ="A" S X="Indicators: ELDER CARE Indicators" D S(X,1,1)
 I BGPZZ="S" S X="Indicators: Selected ELDER CARE Indicators (User Defined)" D S(X,1,1)
 ...
 S X=" " D S(X,1,1)
 I $G(BGPEXPT) D
 .S X="A file will be created called BG05"_$P(^AUTTLOC(DUZ(2),0),U,10)_".EL"_BGPRPT_"." D S(X,1,1) S X="It will reside in the public/export directory.  This file should be sent to your Area Office." D S(X,1,1)
...
ELDERHDR ;
 ...
 S BGPY=0 F  S BGPY=$O(^BGPCTRL(BGPX,22,BGPY)) Q:BGPY'=+BGPY  D
 .S X=^BGPCTRL(BGPX,22,BGPY,0) D S(X,1,1)
 .Q
 Q
ENDTIME ;
...
AREACP ;EP - area cover page
 ...
 S X=" " D S(X,1,1)
 S X="Indicators: ELDER CARE Indicators" D S(X,1,1)
 ...
 .S X=$P(^BGPELDCV(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .D S(X,1,1)
 .S X="Communities: " S X=0,N=0,Y="" F  S X=$O(^BGPELDCV(BGPX,9999,X)) Q:X'=+X  S N=N+1,Y=Y_$S(N=1:"",1:";")_$P(^BGPELDCV(BGPX,9999,X,0),U)
