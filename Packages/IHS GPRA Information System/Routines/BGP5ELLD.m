BGP5ELLD ; IHS/CMI/LAB - print ind ;
...
FEMALE ;
 ...
 Q
I1AGE1 ;
 S BGPF="ELD."_$P(BGPX,".")_"."_($P(BGPX,".",2)+I) S BGPPC=$O(^BGPELIIK("C",BGPF,0))
 I I=0 D
...
BL ;
 ...
 ;S X="# w/Flex Sig & DCBE or Colonoscopy" D S(X,1,1)
 ;F X=1:1:4 S N=$P(BGPDAC(X),U,13),O=$P(BGPDAB(X),U,13) S Y=$$SB^BGP5ELL2($J((N-O),6,1)) D S(Y,,X+1)
 Q
DSP ;
 S X="# "_BGPL1 D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,P1) S Y=V D S(Y,,X+1)
 S X="% "_BGPL1 D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,P2) S Y=$$SB^BGP5ELL2($J(V,6,1)) D S(Y,,X+1)
 Q
DSPP ;
 S X="# "_BGPL1 D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,P1) S Y=V D S(Y,,X+1)
 S X="% "_BGPL1 D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,P2) S Y=$$SB^BGP5ELL2($J(V,6,1)) D S(Y,,X+1)
 Q
 ;
DSPB ;
 S X="# "_BGPL1 D S(X,1,1)
 ...
 S X="% "_BGPL1 D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAB(X),U,P2) S Y=$$SB^BGP5ELL2($J(V,6,1)) D S(Y,,X+1)
 Q
 ;
V(T,R,N,P) ;EP
 I $G(BGPAREAA) G VA
