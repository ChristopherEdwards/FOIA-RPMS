BGP5ELLI ; IHS/CMI/LAB - print ind ;
...
I1AGE6 ;
 S H=H+1
 S BGPF="ELD."_$P(BGPX,".")_"."_($P(BGPX,".",2)+20) S BGPPC=$O(^BGPELIIV("C",BGPF,0))
 ;set 4th piece to numerator and 5th to %
 S BGPNF=$P(^BGPELIIV(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90373.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
I1AGE7 ;
 S I=I+1
 S BGPF="ELD."_$P(BGPX,".")_"."_($P(BGPX,".",2)+24) S BGPPC=$O(^BGPELIIV("C",BGPF,0))
 ;set 4th piece to numerator and 5th to %
 S BGPNF=$P(^BGPELIIV(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90373.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
I1AGEP ;
 ...
 S X=" " D S(X,1,1) S X="# Normal BP" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,4) S Y=V D S(Y,,X+1)
 S X="% Normal BP" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,5) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="# w/Pre HTN I BP" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,6) S Y=V D S(Y,,X+1)
 S X="% w/Pre HTN I BP" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,7) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="# w/Pre HTN II BP" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,8) S Y=V D S(Y,,X+1)
 S X="% w/Pre HTN II BP" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,9) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="# w/Stage 1 HTN BP" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,10) S Y=V D S(Y,,X+1)
 S X="% w/Stage 1 HTN BP" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,11) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="# w/Stage 2 HTN BP" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,12) S Y=V D S(Y,,X+1)
 S X="% w/Stage 2 HTN BP" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,13) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
 ;S X=" " D S(X,1,1) S X="# w/Systolic HTN" D S(X,1,1)
 ;F X=1:1:4 S V=$P(BGPDAC(X),U,14) S Y=V D S(Y,,X+1)
 ;S X="% w/Systolic HTN" D S(X,1,1)
 ;F X=1:1:4 S V=$P(BGPDAC(X),U,15) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
PR ; 
 ...
 S X="% w/Stage 2 HTN BP" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,13) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
 ;S X=" " D S(X,1,1) S X="# w/Systolic HTN" D S(X,1,1)
 ;F X=1:1:4 S V=$P(BGPDAP(X),U,14) S Y=V D S(Y,,X+1)
 ;S X="% w/Systolic HTN" D S(X,1,1)
 ;F X=1:1:4 S V=$P(BGPDAP(X),U,15) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
 ;percentage changes
 ...
 S X="w/Stage 2 HTN BP" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,13),O=$P(BGPDAP(X),U,13) S Y=$$SB($J((N-O),6,1)) D S(Y,,X+1)
 ;S X="w/Systolic HTN" D S(X,1,1)
 ;F X=1:1:4 S N=$P(BGPDAC(X),U,15),O=$P(BGPDAP(X),U,15) S Y=$$SB($J((N-O),6,1)) D S(Y,,X+1)
BL ;
