BGP5ELLJ ; IHS/CMI/LAB - print ind ;
...
I1AGE6 ;
 S H=H+1
 S BGPF="ELD."_$P(BGPX,".")_"."_($P(BGPX,".",2)+20) S BGPPC=$O(^BGPELIIK("C",BGPF,0))
 S BGPNF=$P(^BGPELIIK(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90555.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
I1AGE7 ;
 S I=I+1
 S BGPF="ELD."_$P(BGPX,".")_"."_($P(BGPX,".",2)+24) S BGPPC=$O(^BGPELIIK("C",BGPF,0))
 S BGPNF=$P(^BGPELIIK(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90555.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
I1AGEP ;
 ...
 F X=1:1:4 S V=$P(BGPDAC(X),U,13) S Y=$$SB^BGP5ELL2($J(V,6,1)) D S(Y,,X+1)
 ...
 F X=1:1:4 S V=$P(BGPDAC(X),U,15) S Y=$$SB^BGP5ELL2($J(V,6,1)) D S(Y,,X+1)
PR ; 
 ...
 S X="C. % # w/ LDL 131-160 w/ % of Total LDL Screen" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,13) S Y=$$SB^BGP5ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="D. # w/ LDL >160 w/ % of Total LDL Screen" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,14) S Y=V D S(Y,,X+1)
 S X="D. % # w/ LDL >160 w/ % of Total LDL Screen" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,15) S Y=$$SB^BGP5ELL2($J(V,6,1)) D S(Y,,X+1)
 ...
 S X="C. # w/ LDL 131-160 w/ % of Total LDL Screen" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,13),O=$P(BGPDAP(X),U,13) S Y=$$SB^BGP5ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="D. # w/ LDL >160 w/ % of Total LDL Screen" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,15),O=$P(BGPDAP(X),U,15) S Y=$$SB^BGP5ELL2($J((N-O),6,1)) D S(Y,,X+1)
BL ;
 D BL^BGP5ELLV
 Q
V(T,R,N,P) ;
