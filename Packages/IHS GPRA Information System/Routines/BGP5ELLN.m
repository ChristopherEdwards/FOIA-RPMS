BGP5ELLN ; IHS/CMI/LAB - print ind 1 ;
...
PI1 ;
 ...
 D SETN
 S X=$P(^BGPELIIK(BGPPC,0),U,15) D S(X,1,1)
 I $P(^BGPELIIK(BGPPC,0),U,16)]"" S X=$P(^BGPELIIK(BGPPC,0),U,16) D S(X,1,1)
 I $P(^BGPELIIK(BGPPC,0),U,19)]"" S Y=$P(^BGPELIIK(BGPPC,0),U,19) D S(Y,1,1)
 D H2
 Q
H2 ;
 S BGPX="",BGPX=BGPCYN
 S $P(BGPX,U,2)="",$P(BGPX,U,3)=BGPPRN,$P(BGPX,U,4)="",$P(BGPX,U,5)=$$SB^BGP5ELL2($J($$CALC(BGPCYN,BGPPRN),6)),$P(BGPX,U,6)=BGPBLN,$P(BGPX,U,7)=""
 S $P(BGPX,U,8)=$$SB^BGP5ELL2($J($$CALC(BGPCYN,BGPBLN),6))
 D S(BGPX,,2)
 Q
H1 ;EP
