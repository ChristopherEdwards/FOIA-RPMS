...
I22 ;EP
 D H1
 F BGPPC1="22.1","22.2" S X="" D S(X,1,1) D PI
 Q
PI ;EP
 S BGPPC=0 F  S BGPPC=$O(^BGPELIIV("AP",BGPPC1,BGPPC)) Q:BGPPC=""  D PI1
 Q
PI1 ;
 ...
 D SETN
 S X=$P(^BGPELIIV(BGPPC,0),U,15) D S(X,1,1)
 I $P(^BGPELIIV(BGPPC,0),U,16)]"" S X=$P(^BGPELIIV(BGPPC,0),U,16) D S(X,1,1)
 I $P(^BGPELIIV(BGPPC,0),U,19)]"" S Y=$P(^BGPELIIV(BGPPC,0),U,19) D S(Y,1,1)
 D H2
 Q
H2 ;
