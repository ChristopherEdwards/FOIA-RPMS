BGP5ELSL ; IHS/CMI/LAB - IHS gpra print ;
...
GO .;
 ...
 .S X=$P(^BGPELIK(BGPL,0),U,3) D S(X,1,1)
 .S X=" " D S(X,1,1)
 .I $G(BGPDNT) G CALC
 .S X="Denominator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPELIK(BGPL,61,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPELIK(BGPL,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPELIK(BGPL,61,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S X=^BGPELIK(BGPL,61,BGPY,1,BGPZ,0) S:BGPZ=1 X=" - "_X D S(X,1,1)   ;S Y=^BGPELIK(BGPL,61,BGPY,1,BGPZ,0) D S(Y,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .S X="Numerator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPELIK(BGPL,62,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPELIK(BGPL,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPELIK(BGPL,62,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S X=^BGPELIK(BGPL,62,BGPY,1,BGPZ,0) S:BGPZ=1 X=" - "_X D S(X,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPELIK(BGPL,11,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPELIK(BGPL,11,BGPX,0) D S(X,1,1)
 .S X=" " D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPELIK(BGPL,51,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPELIK(BGPL,51,BGPX,0) D S(X,1,1)
 .S X=" " D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPELIK(BGPL,52,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPELIK(BGPL,52,BGPX,0) D S(X,1,1)
CALC .;
 .D H1
...
PRINTL ;print one line
 Q:(BGPCOUNT#BGPCNT)
 S BGPPCNT=BGPPCNT+1
 S X=$E($P(^DPT(DFN,0),U),1,22) D S(X,1,1) S X=$$HRN^AUPNPAT(DFN,DUZ(2)) D S(X,,2) S X=$E(BGPCOM,1,14) D S(X,,3) S X=BGPSEX D S(X,,4) S X=BGPAGE D S(X,,5) S X=$$EDT^BGP5UTL($$DOB^AUPNPAT(DFN)) D S(X,,6)
 S X=$P(^XTMP("BGP5D",BGPJ,BGPH,"LIST",BGPL,BGPCOM,BGPSEX,BGPAGE,DFN),"|||",1) D S(X,,7)
 S X=$P(^XTMP("BGP5D",BGPJ,BGPH,"LIST",BGPL,BGPCOM,BGPSEX,BGPAGE,DFN),"|||",2) D S(X,,8)
 Q
 ;
HEADER ;EP
 G:'BGPGPG HEADER1
...
HEADER1 ;
 ...
 W !,$$CTR("***  IHS 2015 ELDER CARE Clinical Performance Report Patient List  ***",80),!
...
H1 ;
 ...
 S Y=0 F  S Y=$O(^BGPELIK(BGPL,71,Y)) Q:Y'=+Y  S X=^BGPELIK(BGPL,71,Y,0) D S(X,1,1)
 S X=" " D S(X,1,1)
