BGP5GAON ; IHS/CMI/LAB - AREA OTHER NATIONAL MEASURES GPRA REPORT 01 Jul 2010 1:10 PM ;
...
TESTNTL ;
 ...
 S BGPSUL(1)=""
 S BGPSUL(2)=""
 D EP(.ERR,1,2522,"BGP 15 AREA OTHER NATIONAL MEASURES REPORT","A",.BGPSUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRPTT,BGPSUL,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPROT,BGPRTIME,BGPHC,BGPFILE,BGPDNT) ;EP - called from GUI to produce national gpra report (AO-AGP)
 ...
 ;  BGPSUL  - array containing Iens from BGP 15 ONM DATA CURRENT that the user selected
 ;  BGPSUL - ARRAY OF IENS FROM THE D GET^BGP5ASL(.BGPSUL,.BGPFILE,5,....... call that the user selected
...
EP1 ;
 ...
 S BGPSUCNT=0
 S BGPSUCNT=C
 I BGPSUCNT=1 S Y=$O(BGPSUL(0)),X=$P(^BGPGPDCK(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 ...
 S X=0 F  S X=$O(^BGPINDK("ON",1,X)) Q:X'=+X  S BGPIND(X)=""
 S BGPINDK="G"
 I '$G(BGPHC) D
 ...
 S BGPFONN1="CRSONMNT1"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP5UTL(BGPNOW)_"_"_$$LZERO^BGP5UTL(BGPC,6)_".TXT"
 S BGPFONN2="CRSONMNT2"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP5UTL(BGPNOW)_"_"_$$LZERO^BGP5UTL(BGPC,6)_".TXT"
 S BGPFONN3="CRSONMNT3"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP5UTL(BGPNOW)_"_"_$$LZERO^BGP5UTL(BGPC,6)_".TXT"
 S BGPFONN4="CRSONMNT4"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP5UTL(BGPNOW)_"_"_$$LZERO^BGP5UTL(BGPC,6)_".TXT"
 ;S BGPFONN5="CRSONMNT5"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP5UTL(BGPNOW)_"_"_$$LZERO^BGP5UTL(BGPC,6)_".TXT"
 ;create entry in GUI file
 D ^XBFMK
 S X=BGPFILE
 S DIC="^BGPGUIK(",DIC(0)="L",DIADD=1,DLAYGO=90554.19
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AONM^BGP5GAON",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI AREA ONM REPORT" D ^%ZTLOAD
...
AONM ;
 ...
 D GUIR^BGPXBLM("PRINT^BGP5PARQ","^TMP($J,""BGPGUI"",")
