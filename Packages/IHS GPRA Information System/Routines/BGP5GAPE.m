BGP5GAPE ; IHS/CMI/LAB - AREA NATIONAL GPRA REPORT ;
...
TESTNTL ;
 ...
 S BGPSUL(1)=""
 S BGPSUL(2)=""
 D EP(.ERR,1,2522,"BGP 15 AREA PAT ED RPT","A",.BGPSUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRPTT,BGPSUL,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPROT,BGPRTIME,BGPFILE,BGPBEGIN,BGPDNT) ;EP - called from GUI to produce national gpra report (AO-AGP)
 ...
 ;            from BGP 15 ELDER DATA CURRENT that match the following:
 ;            (SEE BGP5ASL or CALL ME)
 ;  BGPSUL  - array containing Iens from BGP 15 ELDER  DATA CURRENT that the user selected
...
EP1 ;
 ...
 S BGPSUCNT=C
 S BGPSUNM=""
 I BGPSUCNT=1 S Y=$O(BGPSUL(0)),X=$P(^BGPPEDCK(Y,0),U,9) I X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 ...
 S X=0 F  S X=$O(^BGPPEIK(X)) Q:X'=+X  S BGPIND(X)=""
 S BGPINDK="H"
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOELD^BGP5GAPE",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI AREA PAT ED REPORT" D ^%ZTLOAD
...
AOELD ;
 ...
 D GUIR^BGPXBLM("PRINT^BGP5PPEL","^TMP($J,""BGPGUI"",")
