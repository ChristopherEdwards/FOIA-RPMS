BGP5GLST ; IHS/CMI/LAB - GUI CMS REPORT ;
...
EP1 ;
 ...
 S DIC="^BGPGUIK(",DIC(0)="L",DIADD=1,DLAYGO=90554.19,DIC("DR")=".02////"_BGPUSER_";.03////"_$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT)_";.05///"_BGPOPTN_";.06///R"
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOLST^BGP5GLST",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI 15 UPLOAD FILE LIST" D ^%ZTLOAD
 D UPLOG^BGPGUA(BGPGIEN,ZTSK)
 Q
AOLST ;
 K BGPLIST S BGPLIST="",X=$$LIST^%ZISH(BGPDIR,"BG150*",.BGPLIST)
 I $O(BGPLIST(""))="" S ^BGPGUIK(BGPGIEN,11,C,0)="There are no files in that directory." D ENDLOG,XIT Q
 S X=0,C=0 F  S X=$O(BGPLIST(X)) Q:X'=+X  S C=C+1,^BGPGUIK(BGPGIEN,11,C,0)=BGPLIST(X)
