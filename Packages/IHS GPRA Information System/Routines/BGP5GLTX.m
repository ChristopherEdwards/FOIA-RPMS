BGP5GLTX ; IHS/CMI/LAB - IHS Diabetes Audit 2003 ;
 ;;15.0;IHS CLINICAL REPORTING;;NOV 18, 2014;Build 134
 ;
 ;
TESTCMP ;
 S ERR=""
 D EP(.ERR,1,2522,"BGP 15 LAB TAX REPORT",$$NOW^XLFDT)
 W !,ERR
 Q
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LTX^BGP5GLTX",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI LAB TAXONOMY REPORT" D ^%ZTLOAD
 D UPLOG^BGPGUA(BGPGIEN,ZTSK)
 Q
LTX ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D @$S(BGPRT=1:"GPRA^BGP5DLT",BGPRT=7:"ONM^BGP5DLT",BGPRT=2:"CRS^BGP5DLT",BGPRT=4:"ELDER^BGP5DLT",BGPRT=3:"HEDIS^BGP5DLT",BGPRT=5:"CMS^BGP5DLT",BGPRT=8:"EO^BGP5DLT")  ;cmi/maw added 10/30/2010
 D GUIR^BGPXBLM("PRINT^BGP5DLT","^TMP($J,""BGPGUI"",")
...
XIT ;
 K ^TMP($J)
 ;D EN^XBVK("BGP")  ;done in LABTAXC^BGP5GU
 S:$D(ZTQUEUED) ZTREQ="@"
