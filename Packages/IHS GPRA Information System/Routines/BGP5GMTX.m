BGP5GMTX ; IHS/CMI/LAB - IHS Diabetes Audit 2003 ;
 ;;15.0;IHS CLINICAL REPORTING;;NOV 18, 2014;Build 134
 ;
 ;
TESTCMP ;
 S ERR=""
 D EP(.ERR,1,2522,"BGP 15 MED TAX REPORT",$$NOW^XLFDT)
 W !,ERR
 Q
...
EP1 ;
 ...
 S X=BGPFILE
 S BGPGFNM=X
 S DIC="^BGPGUIK(",DIC(0)="L",DIADD=1,DLAYGO=90554.19,DIC("DR")=".02////"_BGPUSER_";.03////"_$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT)_";.05///"_BGPOPTN_";.06///R;.07///P"
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LTX^BGP5GMTX",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI MEDICATION TAXONOMY REPORT" D ^%ZTLOAD
 D UPLOG^BGPGUA(BGPGIEN,ZTSK)
 Q
LTX ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D @$S(BGPRT=1:"GPRA^BGP5DMT",BGPRT=7:"ONM^BGP5DMT",BGPRT=2:"CRS^BGP5DMT",BGPRT=4:"ELDER^BGP5DMT",BGPRT=3:"HEDIS^BGP5DMT",BGPRT=5:"CMS^BGP5DMT",BGPRT=8:"EO^BGP5DMT")  ;cmi/maw added 10/30/2010
 D GUIR^BGPXBLM("PRINT^BGP5DMT","^TMP($J,""BGPGUI"",")
...
XIT ;
 K ^TMP($J)
 ;D EN^XBVK("BGP") done in MEDTAXC^BGP5GU
 S:$D(ZTQUEUED) ZTREQ="@"
