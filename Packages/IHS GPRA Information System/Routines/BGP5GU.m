BGP5GU ; IHS/CMI/LAB - BGP Gui Utilities 10/29/2004 3:28:39 PM ;
 ;;7.0;IHS CLINICAL REPORTING;;JAN 24, 2007
 ;
 ;
DEBUG(RETVAL,BGPSTR) ;run the debugger
 ;D DEBUG^%Serenji("APER^BGPGU(.RETVAL,.BGPSTR)")
 Q
 ;
...
SP(RETVAL) ;-- get the site parameter entry
 ...
 S RETVAL="^BGPTMP("_$J_")"
 S BGPI=0
 S ^BGPTMP($J,BGPI)="T00010BMXIEN^T00050Location^T00050Home Location^T00050Community Taxonomy^T00001EISS^T00001CHS"_$C(30)
 S BGPDA=0 F  S BGPDA=$O(^BGPSITE("B",DUZ(2),BGPDA)) Q:'BGPDA  D
 . N BGPDATA,BGPLI,BGPL,BGPLA,BGPHI,BGPH,BGPHA,BGPTX,BGPTXI,BGPEISS,BGPCHS
 ...
 . S BGPEISS=$P(BGPDATA,U,4)
 . S BGPCHS=$P(BGPDATA,U,6)
 . S BGPI=BGPI+1
 . S ^BGPTMP($J,BGPI)=BGPDA_U_$S($G(BGPLI):BGPLI_"~"_BGPL,1:"")_U_$S($G(BGPHI):BGPHI_"~"_BGPH,1:"")_U_$S($G(BGPTXI):BGPTXI_"~"_$G(BGPTX),1:"")_U_$G(BGPEISS)_U_$G(BGPCHS)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
...
SITE(RETVAL,BGPSTR) ;-- save the site parameters
 S X="MERR^BGPGU",@^%ZOSF("TRAP") ; m error trap
 N BGPI,BGPJ,BGPDATA,BGPDA,P,BGPERR,BGPFAC,BGPTAX,BGPHLOC,R,BGPEISS,BGPCHS
 S P="|",R="~"
 S BGPI=0
 S BGPERR=""
