BGP5HESL ; IHS/CMI/LAB - IHS gpra print ;
...
GO .;
 ...
 .S X=$P(^BGPHEIV(BGPL,0),U,3) D S(X,1,1)
 .S X=" " D S(X,1,1)
 .S X="Denominator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPHEIV(BGPL,61,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPHEIV(BGPL,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...I $P(^BGPHEIV(BGPL,61,BGPY,0),U,2)'[BGPRTYPE Q  ;not a denom def for this report
 ...S BGPZ=0 F  S BGPZ=$O(^BGPHEIV(BGPL,61,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S Y=^BGPHEIV(BGPL,61,BGPY,1,BGPZ,0) D S(Y,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .S X="Numerator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPHEIV(BGPL,62,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPHEIV(BGPL,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...I $P(^BGPHEIV(BGPL,62,BGPY,0),U,2)'[BGPRTYPE Q  ;not a denom def for this report
 ...S BGPZ=0 F  S BGPZ=$O(^BGPHEIV(BGPL,62,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S X=^BGPHEIV(BGPL,62,BGPY,1,BGPZ,0) D S(X,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPHEIV(BGPL,11,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPHEIV(BGPL,11,BGPX,0) D S(X,1,1)
 .S X=" " D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPHEIV(BGPL,51,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPHEIV(BGPL,51,BGPX,0) D S(X,1,1)
 .S X=" " D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPHEIV(BGPL,52,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPHEIV(BGPL,52,BGPX,0) D S(X,1,1)
 .D H1
...
HEADER1 ;
 ...
 W !,$$CTR("***  IHS 2005 HEDIS Clinical Performance Report Patient List  ***",80),!
...
H1 ;
 ...
 S Y=0 F  S Y=$O(^BGPHEIV(BGPL,71,Y)) Q:Y'=+Y  S X=^BGPHEIV(BGPL,71,Y,0) D S(X,1,1)
 S X=" " D S(X,1,1)
