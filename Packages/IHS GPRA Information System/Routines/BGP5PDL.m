...
DEL ;
 ...
 D ^BGP5PDLH
 D DEL1
 I BGPRTYPE=1 D NONSUM^BGP5PDLN
 I BGPRTYPE=1 D DEVSUM^BGP5PDLD
 ;I BGPRTYPE=1 D SUMMARY
 I BGPRTYPE=1 D SDP^BGP5PDLN
 I BGPRTYPE=1 D SDP^BGP5PDLD
 ;I BGPRTYPE=1 D SDP
 I BGPRTYPE=7 D OTHSUM^BGP5PDLO
 D ^BGP5PDLS ;print lists to delimited file
 I $G(BGPCPPL) D CPPL1^BGP5DCLD
...
DEL1 ;EP
 ...
 S BGPIC=0 F  S BGPIC=$O(BGPIND(BGPIC)) Q:BGPIC=""!(BGPQUIT)  I $P(^BGPINDK(BGPIC,0),U,14) D PRINT2 D
 .Q:BGPQUIT
 .I $G(BGPNPL),$D(BGPINDL(BGPIC)),'$D(BGPYNPLT) S BGPINDK=BGPIC D NPL1^BGP5NPLP ;printed nat gpra list
 .I $G(BGPNPL),$D(BGPINDL(BGPIC)),$D(BGPYNPLT) S BGPINDK=BGPIC D CT^BGP5DSTM ;printed nat gpra SEARCH TEMPLATE
 Q:BGPQUIT
...
PRINT2 ;
 ...
 .S X=$P(^BGPINDK(BGPIC,0),U,3) D S(X,1,1)
 .S X=" " D S(X,1,1)
 ...
 .S X="Logic:" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPINDK(BGPIC,BGPNODE,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDK(BGPIC,BGPNODE,BGPX,0) D S(X,1,1)
 .I BGPRTYPE=9 G CALC
 .S X=" " D S(X,1,1) S X="Performance Measure Description:" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDK(BGPIC,$S($G(BGPNGR09):57,1:51),BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDK(BGPIC,$S($G(BGPNGR09):57,1:51),BGPX,0) D S(X,1,1)
 .I $O(^BGPINDK(BGPIC,52,0)) S X=" " D S(X,1,1) S X="Past Performance and/or Target:" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDK(BGPIC,52,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDK(BGPIC,52,BGPX,0) D S(X,1,1)
 .I $O(^BGPINDK(BGPIC,55,0)) S X=" " D S(X,1,1) S X="Source:" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDK(BGPIC,55,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDK(BGPIC,55,BGPX,0) D S(X,1,1)
 .I '$O(^BGPINDK(BGPIC,55,0)) D S(" ",1,1)
CALC .X ^BGPINDK(BGPIC,4)
 Q
HEADER ;EP
 S Y=$P(^VA(200,DUZ,0),U,2),$E(Y,35)=$$FMTE^XLFDT(DT) D S(Y,1,1)
 I BGPRTYPE=7 S Y="*** IHS 2015 Other National Measures Report ***" D S(Y,1,1)
 I BGPRTYPE=4 S Y="*** IHS 2015 Clinical Performance Report ***" D S(Y,1,1)
 I BGPRTYPE=1,'$G(BGPNGR09) S Y="*** IHS 2015 National GPRA/GPRAMA Report ***" D S(Y,1,1)
 I BGPRTYPE=1,$G(BGPNGR09) S Y="*** IHS 2015 National GPRA/GPRAMA Report, Run Using 2015 Logic ***" D S(Y,1,1)
