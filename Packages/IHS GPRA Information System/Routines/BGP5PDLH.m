BGP5PDLH ; IHS/CMI/LAB - cover page;
 ...
 I BGPRTYPE=4,$G(BGPYRPTH)="C" S X="*** IHS 2015 Selected Measures with Community Specified Report ***" D SET(X,1,1)
 I BGPRTYPE=4,$G(BGPYRPTH)="A" S X="*** IHS 2015 Selected Measures with All Communities Report ***" D SET(X,1,1)
 I BGPRTYPE=4,$G(BGPYRPTH)="P" S X="*** IHS 2015 Selected Measures with Patient Panel Population Report ***" D SET(X,1,1)
 I BGPRTYPE=1,$G(BGPNGR09) S X="*** IHS 2015 National GPRA/GPRAMA Report, Run Using 2015 Logic ***" D SET(X,1,1) G N
 I BGPRTYPE=1,$G(BGPDESGP) S X="*** IHS 2015 National GPRA/GPRAMA Report by Designated Provider ***" D SET(X,1,1) G N
 I BGPRTYPE=1,'$G(BGPYGPU),'$G(BGPSUMON) S X="*** IHS 2015 National GPRA/GPRAMA Report ***" D SET(X,1,1)
 I BGPRTYPE=1,'$G(BGPYGPU),$G(BGPSUMON) S X="*** IHS 2015 National GPRA/GPRAMA Report Clinical Performance Summaries ***" D SET(X,1,1)
 I BGPRTYPE=1,$G(BGPYGPU) S X="*** IHS 2015 GPRA/GPRAMA Report ***" D SET(X,1,1)
 I BGPRTYPE=7 S X="*** IHS 2015 Other National Measures Report ***" D SET(X,1,1)
 I BGPRTYPE=6,'$G(BGPEDPP) S X="*** IHS 2015 Patient Education with Community Specified Report ***" D SET(X,1,1)
 I BGPRTYPE=6,$G(BGPEDPP) S X="*** IHS 2015 Patient Education with Patient Panel Population Report ***" D SET(X,1,1)
N ;
 I $G(BGPCPPL) S X="** Including Comprehensive Patient List **" D SET(X,1,1)
 S X=$$RPTVER^BGP5BAN D SET(X,1,1)
 ...
 I BGPRTYPE=4 S X="Measures: "_$P($T(@BGPINDK),";;",2) D SET(X,1,1)
 ...
 I BGPROT'="P" D
 .Q:$D(BGPGUI)
 .S X="A delimited output file called "_BGPDELF D SET(X,1,1) S X="has been placed in the "_$$GETDEDIR^BGP5UTL2()_" directory for your use in Excel or some" D SET(X,1,1) S X="other software package." D SET(X,1,1)
...
ENDTIME ;
 ...
 Q
AREACP ;EP
 I BGPRTYPE=6 S X="*** IHS 2015 Patient Education with Community Specified Report***" D SET(X,1,1)
 I BGPRTYPE=1,$G(BGPNGR09) S X="*** IHS 2015 National GPRA/GPRAMA Report, Run Using 2015 Logic ***" D SET(X,1,1) G NA
 I BGPRTYPE=1,'$G(BGPYGPU),'$G(BGPSUMON) S X="*** IHS 2015 National GPRA/GPRAMA Report ***" D SET(X,1,1)
 I BGPRTYPE=1,'$G(BGPYGPU),$G(BGPSUMON) S X="*** IHS 2015 National GPRA/GPRAMA Report Clinical Performance Summaries ***" D SET(X,1,1)
 I BGPRTYPE=1,$G(BGPYGPU) S X="*** IHS 2015 GPRA/GPRAMA Performance Report ***" D SET(X,1,1)
 I BGPRTYPE=7 D SET("*** IHS 2015 Other National Measures Report ***",1,1)
NA S X="AREA AGGREGATE" D SET(X,1,1)
 ...
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPGPDCK(BGPX,0),U,17):"*",1:"")_X D SET(X,1,1)
 .Q
 S X=" " D SET(X,1,1)
