BGP5XTV ; IHS/CMI/LAB - DISPLAY IND LISTS 15 Dec 2010 9:42 AM ;
...
EN ;EP -- main entry point for 
 D EN^VALM("BGP 15 TAXONOMY VIEW")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ;EP -- init variables and list array
 ...
 S BGPT=""
 F  S BGPT=$O(^BGPTAXK("B",BGPT)) Q:BGPT=""  D
 .S BGPY=$O(^BGPTAXK("B",BGPT,0))
 .Q:'$O(^BGPTAXK(BGPY,12,0))
 .S BGPTYPE=$P(^BGPTAXK(BGPY,0),U,2),BGPDESC=$G(^BGPTAXK(BGPY,11,1,0)),BGPEDIT=$P(^BGPTAXK(BGPY,0),U,4)
 .I BGPTYPE'="L" D  Q:'I
...
SET .;
 ...
 .S $E(BGPTAX(J,0),38)=$$VAL^XBDIQ1(90554.08,BGPY,.02)
 ...
 S T="",J=0,C=0 F  S T=$O(^BGPTAXK("B",T)) Q:T=""  D
 .S Y=0 F  S Y=$O(^BGPTAXK("B",T,Y)) Q:Y'=+Y  D
 ..S N=^BGPTAXK(Y,0)
 ..S Z=$P(N,U,2)  ;TYPE
 ...
 ..I Z="" Q
 ..S J=J+1
 ..S BGPTAX(J,0)=J_")  "_T,$E(BGPTAX(J,0),39)=$$VAL^XBDIQ1(90554.08,Y,.02) D
 ...S A="",B=0 F  S B=$O(^BGPTAXK(Y,12,B)) Q:B'=+B  S R=$P(^BGPTAXK(Y,12,B,0),U) S:A]"" A=A_";" S A=A_$S(R=1:"National GPRA",R=2:"Local CRS",R=3:"HEDIS",R=4:"ELDER",R=5:"CMS",1:"")
 ...S $E(BGPTAX(J,0),60)=A
...
DISP ;EP - add an item to the selected list - called from a protocol
 ...
 D EP^BGP5XTV1(BGPTIEN,BGPTYPE,BGPFIEN)
DISPX ;
 D BACK
 Q
