BGP6CTS ; IHS/CMI/LAB - DISPLAY IND LISTS ;
...
EN ;EP -- main entry point for 
 D EN^VALM("BGP 06 CRS TAXONOMY UPDATE")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
PAUSE ;EP
 ...
 Q
HDR ; -- header code
 S VALMHDR(1)="TAXONOMIES TO SUPPORT 2006 "_BGPRPTT2_" REPORTING"
 Q
 ;
INIT ;EP -- init variables and list array
 ...
 .Q:$P(^BGPTAXS(BGPY,0),U,4)'=1
 .I BGPRPTT1=9 Q:'$O(^BGPTAXS(BGPY,12,0))
 .I BGPRPTT1'=9,'$D(^BGPTAXS(BGPY,12,"B",BGPRPTT1)) Q
 .S BGPTYPE=$P(^BGPTAXS(BGPY,0),U,2),BGPDESC=$G(^BGPTAXS(BGPY,11,1,0)),BGPEDIT=$P(^BGPTAXS(BGPY,0),U,4),J=J+1
 .I BGPTYPE'="L" D
 ...
 .S $E(BGPTAX(J,0),38)=$$VAL^XBDIQ1(90374.08,BGPY,.02)
...
ADD ;EP - add an item to the selected list - called from a protocol
 ...
 S BGPTAXI=$P(BGPTAX("IDX",Y,Y),U,1),BGPTAXT=$P(BGPTAX("IDX",Y,Y),U,2),BGPTAXS=$P(BGPTAX("IDX",Y,Y),U,3),BGPEDIT=$P(^BGPTAXS(BGPTAXS,0),U,4)
