BGP6CTS ; IHS/CMI/LAB - DISPLAY IND LISTS 15 Dec 2010 9:42 AM ;
...
EN ;EP -- main entry point for 
 D EN^VALM("BGP 16 CRS TAXONOMY UPDATE")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
PAUSE ;EP
 ...
 Q
HDR ; -- header code
 S VALMHDR(1)="TAXONOMIES TO SUPPORT 2016 "_BGPRPTT2_" REPORTING"
 Q
 ;
INIT ;EP -- init variables and list array
 ...
 .Q:$P(^BGPTAXM(BGPY,0),U,4)'=1
 .I BGPRPTT1=9 Q:'$O(^BGPTAXM(BGPY,12,0))
 .I BGPRPTT1'=9,'$D(^BGPTAXM(BGPY,12,"B",BGPRPTT1)) Q
 .S BGPTYPE=$P(^BGPTAXM(BGPY,0),U,2),BGPDESC=$G(^BGPTAXM(BGPY,11,1,0)),BGPEDIT=$P(^BGPTAXM(BGPY,0),U,4),J=J+1
 .I BGPTYPE'="L" D  Q:'I
...
ADD ;EP - add an item to the selected list - called from a protocol
 ...
 S BGPTAXI=$P(BGPTAX("IDX",Y,Y),U,1),BGPTAXM=$P(BGPTAX("IDX",Y,Y),U,2),BGPTAXN=$P(BGPTAX("IDX",Y,Y),U,3),BGPEDIT=$P(^BGPTAXM(BGPTAXN,0),U,4)
 I BGPTAXM="L" S BGPTAXN=$P(^ATXLAB(BGPTAXI,0),U)
 I BGPTAXM="T" S BGPTAXN=$P(^ATXAX(BGPTAXI,0),U)
 I BGPTAXM="L",$P(^ATXLAB(BGPTAXI,0),U,22)!('BGPEDIT) W !!,"The ",$P(^ATXLAB(BGPTAXI,0),U)," is VIEW ONLY.",!,"You can not update it." D PAUSE G ADDX
 I BGPTAXM="T",$P(^ATXAX(BGPTAXI,0),U,22)!('BGPEDIT) W !!,"The ",$P(^ATXAX(BGPTAXI,0),U)," is VIEW ONLY.",!,"You can not update it." D PAUSE G ADDX
...
DISP1 ;EP - add an item to the selected list - called from a protocol
 ...
 I $D(DIRUT) W !,"No taxonomy selected." G DISPX
 D FULL^VALM1 W:$D(IOF) @IOF
 S BGPTAXI=$P(BGPTAX("IDX",Y,Y),U,1),BGPTAXM=$P(BGPTAX("IDX",Y,Y),U,2)
 W !!!,$S(BGPTAXM="L":$P(^ATXLAB(BGPTAXI,0),U),1:$P(^ATXAX(BGPTAXI,0),U))
 W !!,"Items currently defined to this taxonomy:"
 I BGPTAXM="L" S X=0 F  S X=$O(^ATXLAB(BGPTAXI,21,"B",X)) Q:X=""  D
 .S Y=$P($G(^LAB(60,X,0)),U) W !?5,Y
 I BGPTAXM="T",'$P(^ATXAX(BGPTAXI,0),U,13) S X=0 F  S X=$O(^ATXAX(BGPTAXI,21,"B",X)) Q:X=""  D
