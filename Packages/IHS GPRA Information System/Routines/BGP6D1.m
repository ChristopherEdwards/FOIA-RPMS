...
PROC ;EP
 ...
 K ^XTMP("BGP15TAX",$J)
 D UNFOLDTX^BGP6UTL2
 D XTMP^BGP6UTL("BGP28CPL","National GPRA Comp List")
...
PROCCY ;EP - current time period
 ...
 S BGPEDATE=BGPED,BGPTIME=1,BGPBDATE=BGPBD,BGPGBL=$S(BGPRTYPE=3:"^BGPHEDCB(",BGPRTYPE=5:"^BGPELDCM(",BGPRTYPE=8:"^BGPEOCB(",1:"^BGPGPDCM(")
 S BGP365=BGPBDATE
 ...
 I $G(BGPIISO) D ISS^BGP6DPA1 K ^TMP($J,"A") Q  ;SCHEDULING OPTION
 D CALCIND
 K ^TMP($J,"A")
 Q
PROCPY ;
 ...
 Q:$P(^DPT(DFN,0),U,2)=""
 Q:"FM"'[$P(^DPT(DFN,0),U,2)
 S BGPEDATE=BGPPED,BGPTIME=2,BGPBDATE=BGPPBD,BGPGBL=$S(BGPRTYPE=3:"^BGPHEDPB(",BGPRTYPE=5:"^BGPELDPM(",BGPRTYPE=8:"^BGPEOPB(",1:"^BGPGPDPM(")
 S BGP365=BGPBDATE
 ...
 S BGPAGEE=$$AGE^AUPNPAT(DFN,BGPEDATE)
 S BGPSEX=$P(^DPT(DFN,0),U,2)
 ;I $$V2IHD^BGP6D9(DFN,BGP365,BGPEDATE),$$FIRSTIHD^BGP6D9(DFN,BGPEDATE) S BGPIHD=1
 S BGPDM1=$$DM(DFN,,BGPEDATE) ;dm diagnosis ever?
 S BGPDM2=$$DM(DFN,BGPBDATE,BGPEDATE) ;dm diagnosis in past year
 I BGPDM1 D  ;set up 4 dm demoninators
 .S BGPFDMD=$$FIRSTDM(DFN,BGPEDATE) ;1 OR 0 FIRST DX BEFORE BEG
...
PROCBY ;
 ...
 Q:$P(^DPT(DFN,0),U,2)=""
 Q:"FM"'[$P(^DPT(DFN,0),U,2)
 S BGPEDATE=BGPBED,BGPTIME=3,BGPBDATE=BGPBBD,BGPGBL=$S(BGPRTYPE=3:"^BGPHEDBB(",BGPRTYPE=5:"^BGPELDBM(",BGPRTYPE=8:"^BGPEOBB(",1:"^BGPGPDBM(")
 S BGP365=BGPBDATE
...
DM(P,BDATE,EDATE) ;EP is patient diabetic 1 or 0
 I $G(BDATE)="" S BDATE=$$DOB^AUPNPAT(P)
 NEW BGPG
 S BGPG(1)=$$LASTDX^BGP6UTL1(P,"SURVEILLANCE DIABETES",BDATE,EDATE)
...
ACTCLBH(P,BDATE,EDATE) ;EP - clinical user
 ...
 .Q:B=""
 .I 'G,$D(^BGPCTRL($O(^BGPCTRL("B",2016,0)),11,"B",B)) S G=V  ;must be a primary clinic S G=V
 .I V'=G,$D(^BGPCTRL($O(^BGPCTRL("B",2016,0)),95,"B",B)) S S=1
 .I G,S S F=1
 .Q
 Q $S(F:1,1:0)
FIRSTDM(P,EDATE) ;
