...
DMNA ;EP - called from elder care
 ...
 S BGPBD2=BGPBDATE
I61 ;EP
 S BGPGFR=$$GFR^BGP6D211(DFN,BGPBD1,BGPEDATE)
 S BGPESRD=$$ESRD^BGP6D211(DFN,$P(^DPT(DFN,0),U,3),BGPEDATE)
 S BGPQUP=$$QUANTUP^BGP6D211(DFN,BGPBD2,BGPEDATE)
 ...
 I BGPN1 D
 .I BGPESRD S BGPVALUE=BGPVALUE_$S(BGPESRD]"":"ESRD: "_$$DATE^BGP6UTL($P(BGPESRD,U,3))_" "_$P(BGPESRD,U,2),1:"") Q
 .S BGPVALUE=BGPVALUE_$S(BGPESRD:"; ",1:""),BGPVALUE=BGPVALUE_"GFR: "_$$DATE^BGP6UTL($P(BGPGFR,U,2))
...
I7 ;EP
 ...
 NEW BGPEYE,BGPJVN
 I 'BGPDM1 S BGPSTOP=1 Q
 S X=$$LASTDX^BGP6UTL1(DFN,"BGP BILATERAL BLINDNESS DXS",$$DOB^AUPNPAT(DFN),BGPEDATE)
 I 'X S X=$$BLINDPL^BGP6D21A(DFN,BGPEDATE)
...
DMEYE ;EP - called from elder care
 ...
 S BGPVALUE=$S(BGPDMD1:"UP",1:"")_$S(BGPDMD2:",AD",1:"")_$S(BGPDMD3:",AAD",1:"")_"|||"
 S X=$S(BGPEYE:"Eval: "_$$DATE^BGP6UTL($P(BGPEYE,U,2))_" "_$P(BGPEYE,U,3),1:"")
...
I8 ;EP
 ...
 I BGPRTYPE=1 S BGPVALUE="AD|||" I BGPN1 S BGPVALUE=BGPVALUE_$$DATE^BGP6UTL($P(BGPVALUE,U,2))_" "_$P(BGPVALUE,U,3)
 I BGPRTYPE'=1 S BGPVALUE="AD|||"_$$DATE^BGP6UTL($P(BGPVALUE,U,2))_" "_$P(BGPVALUE,U,3)
 Q
I9 ;EP
 ...
 S BGPGAX=""
 I BGPD2 D
 .S BGPGAX=$$GA^BGP6D3A(DFN,BGPBDATE,BGPEDATE)
 .S BGPN7=$P(BGPGAX,U,1),BGPN8=$P(BGPGAX,U,2)
 S BGPVALUD="" I BGPN3 S BGPVALUD="UP w/exam "_$$DATE^BGP6UTL($P(BGPV1,U,2))_" "_$P(BGPV1,U,3)
 S BGPVALUD=BGPVALUD_$S(BGPD10:",PREG",1:"")
 S BGPVALUD=BGPVALUD_"|||"
 I BGPTC S BGPVALUD=BGPVALUD_"TC: "_$$DATE^BGP6UTL($P(BGPTC,U,2))_" "_$P(BGPTC,U,3)
 I BGPN6 S BGPVALUD=BGPVALUD_$S(BGPTC:"; ",1:""),BGPVALUD=BGPVALUD_"PN: "_$$DATE^BGP6UTL($P(BGPPN,U,2))_" "_$P(BGPPN,U,3)
 I BGPN7 S BGPVALUD=BGPVALUD_" GEN/SSC: "_$P(BGPGAX,U,3)
 ;S BGPN3=0 I BGPN1,'BGPN2 S BGPN3=1
 I BGPRTYPE'=1 S BGPVALUE="UP|||"_$$DATE^BGP6UTL($P(BGPVALUE,U,2))_" "_$P(BGPVALUE,U,3)
 I BGPRTYPE=1 S X=BGPVALUE S BGPVALUE="UP|||" I BGPN1 S BGPVALUE=BGPVALUE_$$DATE^BGP6UTL($P(X,U,2))_" "_$P(X,U,3)
 Q
PREG(P,BDATE,EDATE) ;EP
 NEW X,Y,Z
 S (X,Y)=0
 I $P(^DPT(P,0),U,2)'="F" Q 0
 S X=$$PREG^BGP6D7(P,$$FMADD^XLFDT(EDATE,-608),EDATE,1,1)
...
BF(P,BDATE,EDATE) ;EP
 ...
 S X=$$LASTDX^BGP6UTL1(P,"BGP BREASTFEEDING DXS",BDATE,EDATE)
...
DENTDEV(P,BDATE,EDATE) ;
 ...
 S BGPC=$$CPT^BGP6DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP DENTAL EXAM CPTS",0)),5)
 I BGPC Q 1_U_BGPC
 Q ""
TC(P,BDATE,EDATE) ;
 NEW BGPG,BGPC,%
...
DENTSRV(P,BDATE,EDATE,FORECAST) ;EP
 ...
 I $D(BGPG(1)),$P(BGPG(1),U)>$P(BGPC,U) S BGPC=$P(BGPG(1),U)_"^"_"ADA "_$P(BGPG(1),U,2)
 K BGPG
 S BGPG(1)=$$CPT^BGP6DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP DENTAL EXAM CPT 0190",0)),5)
 I $D(BGPG(1)),$P(BGPG(1),U)>$P(BGPC,U) S BGPC=$P(BGPG(1),U)_"^"_"ADA "_$P(BGPG(1),U,2)
 K BGPG
...
GFR(P,BDATE,EDATE) ;EP
 ...
 ...I T,$P(^AUPNVLAB(X,0),U)=T S BGPC=1_U_$$DATE^BGP6UTL((9999999-D)) Q
...
DMACE ;EP
 ...
 ;BGPN4 AND BGPN5, BGPN6 ARE GPRA DEV 2016
 I 'BGPDMD2 S BGPSTOP=1 Q  ;don't process this measure, pt not diabetic per active diabetic definition
 I $$ESRD^BGP6D211(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE) S BGPSTOP=1 Q  ;no one with ESRD 14.1
 ;
 ;does patient have HTN?
 I '$$V1HTN^BGP6D9(DFN,BGP365,BGPEDATE) S BGPSTOP=1 Q  ;no 1 visit of htn during report period
 I '$$FIRSTHTN^BGP6D9(DFN,BGPEDATE) S BGPSTOP=1 Q  ;no htn prior to report period
 S BGPD1=1,BGPN1=0
 ...
 S BGPACEC=$$ACECONT^BGP6D723(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,BGPBDATE,BGPEDATE,BGPBDATE)
 I BGPACEC S BGPN2=1,BGPVALUE="AD|||"_$P(BGPACEC,U,2) G DMACEX
 S BGPACEA=$$ACEALG^BGP6D723(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)
...
ACE1 .;
 ...
 .S K=S+K
 .I R]"" S R=R_";"
 .S R=R_$$DATE^BGP6UTL($P($P(^AUPNVSIT(V,0),U),"."))
