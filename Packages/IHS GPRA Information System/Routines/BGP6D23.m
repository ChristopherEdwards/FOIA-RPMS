BGP6D23 ; IHS/CMI/LAB - measure I2 ;
 ;;7.0;IHS CLINICAL REPORTING;;JAN 24, 2007
 ;
IMS ;EP
 S (BGPN1,BGPN2,BGPN3,BGPN4,BGPN5,BGPN6,BGPN7,BGPN8,BGPN9,BGPD1,BGPD2)=0
 S BGPVALUE=""
 I $$LASTDX^BGP6UTL1(DFN,"SURVEILLANCE DIABETES",$P(^DPT(DFN,0),U,3),BGPEDATE) S BGPSTOP=1 Q  ;had diabetes dx
...
IMS2 ;LDL done
 S BGPLDL=$$LDL^BGP6D2(DFN,BGPBDATE,BGPEDATE)
 S BGPN2=$P(BGPLDL,U)
 I BGPN2 S BGPVALUE=BGPVALUE_";LDL: "_$$DATE^BGP6UTL($P(BGPLDL,U,2))_" "_$P(BGPLDL,U,3)
 K X,Y,Z,%,A,B,C,D,E,H,BDATE,EDATE,P,V,S,F,BGPLDL,BGPHDL,BGPTRI,BGPLP
IMS3 ;fasting glucose
 S BGPFG=$$FGT(DFN,BGPBDATE,BGPEDATE)
 S BGPN3=$P(BGPFG,U)
 I BGPN3 S BGPVALUE=BGPVALUE_";FG: "_$$DATE^BGP6UTL($P(BGPFG,U,2))
 S BGPUP=$$POSUR^BGP6D21(DFN,BGP365,BGPEDATE)
 I $P(BGPUP,U) S BGPN4=1
 I BGPN4 S BGPVALUE=BGPVALUE_";POS/M/ESRD: "_$$DATE^BGP6UTL($P(BGPUP,U,4))_" "_$P(BGPUP,U,3)
 K BGPFG
IMS5 ;
 K BGPX,BGPC1,X,Y,Z,%,A,B,C,D,E,H,BDATE,EDATE,P,V,S,F
 S BGPTOBV=""
 S BGPTOB=$$TOBACCO^BGP6D7(DFN,BGPBDATE,BGPEDATE)
 S BGPN5=$S(BGPTOB]"":1,1:0),BGPTOBV=$P(BGPTOB,U,2)
 S BGPSDX=$$DX^BGP6D7(DFN,BGPBDATE,BGPEDATE)
 S BGPXPED=$$PED^BGP6D7(DFN,BGPBDATE,BGPEDATE)
 S BGP1320=$$DENT^BGP6D7(DFN,BGPBDATE,BGPEDATE)
 I BGPSDX]"" S BGPN5=1 I BGPTOBV="" S BGPTOBV=$$DATE^BGP6UTL($P(BGPSDX,U,2))
 I BGPXPED]"" S BGPN5=1 I BGPTOBV="" S BGPTOBV=$$DATE^BGP6UTL($P(BGPXPED,U,2))
 I BGP1320]"" S BGPN5=1 I BGPTOBV="" S BGPTOBV=$$DATE^BGP6UTL($P(BGP1320,U,2))
 I BGPN5 S BGPVALUE=BGPVALUE_";TOB SCRN: "_BGPTOBV
IMS6 ;
 S BGPBMI=$$BMI^BGP6D6(DFN,BGPEDATE,BGPAGEE)
 I $P(BGPBMI,U)="" S BGPBMI=$$REF^BGP6D6(DFN,BGPBDATE,BGPEDATE,BGPAGEB)
 I $P(BGPBMI,U)]"" S BGPN6=1
 I BGPN6 S BGPVALUE=BGPVALUE_";BMI: "_$S(BGPBMI["HT":"ref "_$P(BGPBMI,U,2)_" "_$$DATE^BGP6UTL($P(BGPBMI,U,3))_" "_$P(BGPBMI,U,5)_" "_$$DATE^BGP6UTL($P(BGPBMI,U,6)),BGPBMI]"":$$SB^BGP6PDL1($J($P(BGPBMI,U),6,2)),1:"")
IMS7 ;
 S BGPLIFE=$$LIFE^BGP6D41(DFN,BGPBDATE,BGPEDATE)
 I $P(BGPLIFE,U) S BGPN7=1
 I BGPN7 S BGPVALUE=BGPVALUE_";LIFE: "_$$DATE^BGP6UTL($P(BGPLIFE,U,2))_":"_$P(BGPLIFE,U,3)
IMS8 ;
 S BGPDEP=$$DEP^BGP6D21(DFN,BGP365,BGPEDATE) I $P(BGPDEP,U)=1 S BGPN8=1
 S BGPDEPS=$$DEPSCR^BGP6D21(DFN,BGP365,BGPEDATE) I $P(BGPDEPS,U)=1 S BGPN8=1
 S BGPREF="" I 'BGPN8 S BGPREF=$$DEPREF^BGP6D21(DFN,BGP365,BGPEDATE) I $P(BGPREF,U)=1 S BGPN8=1
 I BGPDEP]"" S BGPVALUE=BGPVALUE_"; "_$P(BGPDEP,U,2)_":"_$P(BGPDEP,U,3)
 E  I BGPDEPS]"" S BGPVALUE=BGPVALUE_"; "_$P(BGPDEPS,U,2)_":"_$P(BGPDEPS,U,3)
 E  I BGPREF]"" S BGPVALUE=BGPVALUE_"; "_$P(BGPREF,U,2)_":"_$P(BGPREF,U,3)
IMS9 ;
...
MS(P,BDATE,EDATE) ;EP
 ...
 ..S %=$P(^AUPNVPOV(Y,0),U)
 ..Q:'%
 ..;I $P($G(^ICD9(%,0)),U)=277.7 S D=1
 ..I $P($$ICDDX^ICDCODE(%),U,2)=277.7 S D=1
 .Q:'D
 .S $P(G,U)=$P(G,U)+1,$P(G,U,($P(G,U)+1))=$P($P(^AUPNVSIT(V,0),U),".")
 .Q
 K ^TMP($J,"A")
 I $P(G,U)>1 Q 1_U_"277.7 on "_$$DATE^BGP6UTL($P(G,U,2))_"; "_$$DATE^BGP6UTL($P(G,U,3))
 ;now check for 3 or more of the following
 S BGPC=0,BGPK=""
 S X=$$BMI^BGP6D6(P,EDATE,$$AGE^AUPNPAT(P,EDATE))
 ...
 S BGPHTN=$$LASTDX^BGP6UTL1(P,"SURVEILLANCE HYPERTENSION",BGP365,EDATE)
 S BGPMBP=$$MEANBP(P,BGPBDATE,BGPEDATE)
 I $P(BGPHTN,U)!(BGPMBP]"") S BGPC=BGPC+1 S:BGPK]"" BGPK=BGPK_"; " S:$P(BGPHTN,U)=1 BGPK=BGPK_"HTN DX: "_$$DATE^BGP6UTL($P(BGPHTN,U,3)) I BGPMBP]"" S:BGPK]""&($P(BGPHTN,U)=1) BGPK=BGPK_"; " S BGPK=BGPK_BGPMBP
...
HDL(P,BDATE,EDATE) ;EP
 K BGPC1
 S BGPC1=0,R=""
 S %="",E=+$$CODEN^ICPTCOD(83718),%=$$CPTI^BGP6DU(P,BDATE,EDATE,E)
 I %]"" Q 1_U_$P(%,U,2)
 ;now get all loinc/taxonomy tests
 S S=$P(^DPT(DFN,0),U,2)
