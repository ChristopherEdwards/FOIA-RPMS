BGP6D25 ; IHS/CMI/LAB - measure 6 03 Jun 2016 3:16 PM ; 11 Sep 2015  10:56 AM
...
IA ;EP
 ...
 I BGPDMD2 S BGPD7=1
 I $$CHD^BGP6D729(DFN,BGP365,BGPEDATE),BGPACTCL S BGPIHD=1,BGPD8=1
...
DEPEP ;EP - called from elder
 ...
 S BGPN11=0 S BGPSBHC=$$DEPSCRBH^BGP6D27(DFN,BGPBDATE,BGPEDATE) I $P(BGPSBHC,U)=1 S BGPN11=1
 I BGPN2 S BGPN1=1
 I BGPN3 S BGPN1=1
 S BGPREF=""
...
DEP(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(2)) S BGPDEP((9999999-$P(BGPG(1),U)))="",BGPDEP((9999999-$P(BGPG(2),U)))="" Q 1_U_$$DATE^BGP6UTL($P(BGPG(2),U))_" POV "_$P(BGPG(2),U,2)_" + "_$$DATE^BGP6UTL($P(BGPG(1),U))_" POV "_$P(BGPG(1),U,2)_U_U_$P(BGPG(1),U,1)
...
DEPSUIC(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(1)) Q 1_U_"POV "_$P(BGPG(1),U,2)_U_$$DATE^BGP6UTL($P(BGPG(1),U))
 ;
 S BGPC=""
 ...
 ..Q:'BGPP
 ..S BGPP=$P($G(^AMHPROB(BGPP,0)),U)
 ..I BGPP=39 S BGPC=1_U_"BH POV 39"_U_$$DATE^BGP6UTL((9999999-$P(D,".")))
 ..Q
 Q BGPC
DEPSCR(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(1)) S BGPDEPS=1_"^DEP SCRN^"_$$DATE^BGP6UTL($P(BGPG(1),U))_U_$P(BGPG(1),U)_U_"Ex 36"
 K BGPG
 S Y="BGPG("
 S X=P_"^LAST DX [BGP DEPRESSION SCRN DXS;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE) S E=$$START1^APCLDF(X,Y)
 I $D(BGPG(1)),$P(BGPDEPS,U,4)<$P(BGPG(1),U) S BGPDEPS=1_U_"POV "_$P(BGPG(1),U,2)_" DEP"_U_$$DATE^BGP6UTL($P(BGPG(1),U))_U_$P(BGPG(1),U)_U_"POV "_$P(BGPG(1),U,2)
 ;CPT CODE ADDED IN 11.1
 S Y=$$CPT^BGP6DU(DFN,BDATE,EDATE,$O(^ATXAX("B","BGP DEPRESSION SCREEN CPTS",0)),5)
 I Y,$P(BGPDEPS,U,4)<$P(Y,U,1) S BGPDEPS=1_U_"CPT "_$P(Y,U,2)_U_$$DATE^BGP6UTL($P(Y,U,1))_U_$P(Y,U,1)_U_"CPT "_$P(Y,U,2)
 ;now add in v measurements
 S BGPC=$$LASTITEM^BGP6DU(P,BDATE,EDATE,"MEASUREMENT","PHQ2")
 I $P(BGPDEPS,U,4)<$P(BGPC,U,2) S BGPDEPS=1_U_"Meas "_$P(BGPC,U,3)_U_$$DATE^BGP6UTL($P(BGPC,U,2))_U_$P(BGPC,U,2)_U_"Meas "_$P(BGPC,U,3)
 S BGPC=$$LASTITEM^BGP6DU(P,BDATE,EDATE,"MEASUREMENT","PHQ9")
 I $P(BGPDEPS,U,4)<$P(BGPC,U,2) S BGPDEPS=1_U_"Meas "_$P(BGPC,U,3)_U_$$DATE^BGP6UTL($P(BGPC,U,2))_U_$P(BGPC,U,2)_U_"Meas "_$P(BGPC,U,3)
BHSCR ;
 ...
 .I $P($G(^AMHREC(V,14)),U,5)="P"!($P($G(^AMHREC(V,14)),U,5)="N") S BGPC=1_U_"BH Dep Exam"_U_$$DATE^BGP6UTL(9999999-D)_U_(9999999-D)_U_"BH Exam 36"
 .Q:BGPC
 .S X=0 F  S X=$O(^AMHRPRO("AD",V,X)) Q:X'=+X!(BGPC)  S BGPP=$P($G(^AMHRPRO(X,0)),U) D
 ..Q:'BGPP
 ..S BGPP=$P($G(^AMHPROB(BGPP,0)),U)
 ..I BGPP=14.1 S BGPC=1_U_"BH 14.1"_U_$$DATE^BGP6UTL(9999999-D)_U_(9999999-D)_U_"BH POV 14.1" Q
 .Q:BGPC
 .S X=0 F  S X=$O(^AMHRMSR("AD",V,X)) Q:X'=+X!(BGPC)  S BGPP=$P($G(^AMHRMSR(X,0)),U) D
 ..Q:'BGPP
 ..S BGPP=$P($G(^AUTTMSR(BGPP,0)),U)
 ..I BGPP="PHQ2"!(BGPP="PHQ9") S BGPC=1_U_"BH "_BGPP_U_$$DATE^BGP6UTL(9999999-D)_U_(9999999-D)_U_"BH Meas "_BGPP
 .Q:BGPC
 ...
 ..S BGPC=1_U_"BH CPT: "_$P(^ICPT(BGPP,0),U,1)_U_$$DATE^BGP6UTL(9999999-D)_U_(9999999-D)_U_"BH CPT "_$P(^ICPT(BGPP,0),U,1)
 ...
 .Q:Y<BDATE
 .Q:Y>EDATE
 .S BGPC=1_U_"PRIME MD SCORE HEALTH FACTOR"_U_$$DATE^BGP6UTL(9999999-D)_U_(9999999-D)_U_"PRIME MD SCORE"
 .Q
...
DEPSCRD(P,BDATE,EDATE) ;EP
 ...
 S BGPC=$$LASTITEM^BGP6DU(P,BDATE,EDATE,"MEASUREMENT","PHQ9")
 I $P(BGPDEPS,U,4)<$P(BGPC,U,2) S BGPDEPS=1_U_"Meas "_$P(BGPC,U,3)_U_$$DATE^BGP6UTL($P(BGPC,U,2))_U_$P(BGPC,U,2)_U_"Meas "_$P(BGPC,U,3)
 S BGPC=$$LASTITEM^BGP6DU(P,BDATE,EDATE,"MEASUREMENT","PHQT")
 I $P(BGPDEPS,U,4)<$P(BGPC,U,2) S BGPDEPS=1_U_"Meas "_$P(BGPC,U,3)_U_$$DATE^BGP6UTL($P(BGPC,U,2))_U_$P(BGPC,U,2)_U_"Meas "_$P(BGPC,U,3)
BHSCRD ;
 S D=0,BGPC="",E=9999999-BDATE,D=9999999-EDATE-1_".99" F  S D=$O(^AMHREC("AE",P,D)) Q:D'=+D!($P(D,".")>E)!(BGPC)  S V=0 F  S V=$O(^AMHREC("AE",P,D,V)) Q:V'=+V!(BGPC)  D
 .S X=0 F  S X=$O(^AMHRPRO("AD",V,X)) Q:X'=+X!(BGPC)  S BGPP=$P($G(^AMHRPRO(X,0)),U) D
 ...
 ..Q:'BGPP
 ..S BGPP=$P($G(^AUTTMSR(BGPP,0)),U)
 ..I BGPP="PHQ2"!(BGPP="PHQ9")!(BGPP="PHQT") S BGPC=1_U_"BH "_BGPP_U_$$DATE^BGP6UTL(9999999-D)_U_(9999999-D)_U_"BH Meas "_BGPP
 .I $P($G(^AMHREC(V,14)),U,5)="P"!($P($G(^AMHREC(V,14)),U,5)="N") S BGPC=1_U_"BH Dep Exam"_U_$$DATE^BGP6UTL(9999999-D)_U_(9999999-D)_U_"BH Exam 36"
 I BGPC]"",$P(BGPDEPS,U,4)<$P(BGPC,U,4) S BGPDEPS=BGPC
 ;add in measurements
 ;ANMC
...
DEPREF(P,BDATE,EDATE) ;EP
 ...
 I $P(G,U)=1 Q 1_"^Refused Ex 36^"_$$DATE^BGP6UTL($P(G,U,2))_U_$P(G,U,2)
 Q ""
DEPEDU(P,BDATE,EDATE) ;EP
 NEW BGPG,Y,X,E,D,T,BGPDEPE,V,BGPC,G,I,Z,S,SN
 S SN=$O(^BGPSNOMM("B","DEPRESSION SCREEN PATIENT ED",0))
 K BGPG
 S Y="BGPG(",BGPDEPE=""
 ...
 .I $P(T,"-",1)="DEP"!($P(T,"-",1)="BH")!($P(T,"-",1)="SB")!($P(T,"-",1)="PDEP"),$P(BGPDEPE,U,4)<$P(BGPG(X),U,1) S BGPDEPE=1_U_""_T_U_$$DATE^BGP6UTL($P(BGPG(X),U))_U_$P(BGPG(X),U)
 .S S=$P(T,"-",1)
 .S S=$$ICDDX^BGP6UTL2(S)
 .I $P(S,U,1)'="-1",$$ICD^BGP6UTL2($P(S,U,1),$O(^ATXAX("B","BGP DEP SCREEN EDUC DXS",0)),9),$P(BGPDEPE,U,4)<$P(BGPG(X),U,1) S BGPDEPE=1_U_""_T_U_$$DATE^BGP6UTL($P(BGPG(X),U))_U_$P(BGPG(X),U)
 .I $P(T,"-",1)]"",$D(^BGPSNOMM(SN,11,"B",$P(T,"-",1))),$P(BGPDEPE,U,4)<$P(BGPG(X),U,1) S BGPDEPE=1_U_""_T_U_$$DATE^BGP6UTL($P(BGPG(X),U))_U_$P(BGPG(X),U)
DEPMH ;
 ...
 ..Q:'$D(^AUTTEDT(T,0))
 ..S T=$P(^AUTTEDT(T,0),U,2)
 ..I $P(T,"-",1)="DEP"!($P(T,"-",1)="BH")!($P(T,"-",1)="SB")!($P(T,"-",1)="PDEP") S BGPC=1_U_""_T_U_$$DATE^BGP6UTL((9999999-D))_U_(9999999-D)
 ..S S=$P(T,"-",1)
 ..S S=$$ICDDX^BGP6UTL2(S)
 ..I $P(S,U,1)'="-1",$$ICD^BGP6UTL2($P(S,U,1),$O(^ATXAX("B","BGP DEP SCREEN EDUC DXS",0)),9) D
 ...S BGPC=1_U_T_U_$$DATE^BGP6UTL((9999999-D))_U_(9999999-D)
 ..I $P(T,"-")]"",$D(^BGPSNOMM(SN,11,"B",$P(T,"-"))) S BGPC=1_U_T_U_$$DATE^BGP6UTL((9999999-D))_U_(9999999-D)
