...
I15 ;EP
 ...
 I BGPRTYPE'=3 S BGPVALUE=$S(BGPI7DA:"UP",1:"")_$S(BGPI7DB:",AC",1:"")_"|||"_$$DATE^BGP6UTL($P(BGPPAP,U,2))_" "_$P(BGPPAP,U,3)
...
FLU(P,ED) ;EP
 ...
 I $D(BGPG(1)) Q $$DATE^BGP6UTL($P(BGPG(1),U))_" Imm 88"
 S X=P_"^LAST IMM 111;DURING "_BDATE_"-"_EDATE S E=$$START1^APCLDF(X,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP6UTL($P(BGPG(1),U))_" Imm 111"
 S X=P_"^LAST IMM 15;DURING "_BDATE_"-"_EDATE S E=$$START1^APCLDF(X,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP6UTL($P(BGPG(1),U))_" Imm 15"
 S X=P_"^LAST IMM 16;DURING "_BDATE_"-"_EDATE S E=$$START1^APCLDF(X,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP6UTL($P(BGPG(1),U))_" Imm 16"
 S BGPG=$$LASTPRCI^BGP6UTL1(P,"99.52",BDATE,EDATE)
 I $P(BGPG,U,1)=1 Q $$DATE^BGP6UTL($P(BGPG,U,3))_" 99.52"
 ;K BGPG S %=P_"^LAST PROCEDURE 99.52;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 ;I $D(BGPG(1)) Q $$DATE^BGP6UTL($P(BGPG(1),U))_" 99.52"
 K BGPG S %=P_"^LAST DX V04.8;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP6UTL($P(BGPG(1),U))_" V04.8"
 K BGPG S %=P_"^LAST DX V04.81;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP6UTL($P(BGPG(1),U))_" V04.81"
 K BGPG S %=P_"^LAST DX V06.6;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP6UTL($P(BGPG(1),U))_" V06.6"
 S T=$O(^ATXAX("B","BGP CPT FLU",0))
 I T D  I X]"" Q $$DATE^BGP6UTL(X)_" CPT"
 .S X=$$CPT^BGP6DU(P,,ED,T,3)
 S G=$$REFUSAL^BGP6UTL1(P,9999999.14,$O(^AUTTIMM("C",88,0)),$$FMADD^XLFDT(ED,-365),ED)
 I $P(G,U)=1 Q $$DATE^BGP6UTL($P(G,U,2))_" Refused"
 S G=$$REFUSAL^BGP6UTL1(P,9999999.14,$O(^AUTTIMM("C",111,0)),$$FMADD^XLFDT(ED,-365),ED)
 I $P(G,U)=1 Q $$DATE^BGP6UTL($P(G,U,2))_" Refused"
 S G=$$REFUSAL^BGP6UTL1(P,9999999.14,$O(^AUTTIMM("C",15,0)),$$FMADD^XLFDT(ED,-365),ED)
 I $P(G,U)=1 Q $$DATE^BGP6UTL($P(G,U,2))_" Refused"
 S G=$$REFUSAL^BGP6UTL1(P,9999999.14,$O(^AUTTIMM("C",16,0)),$$FMADD^XLFDT(ED,-365),ED)
 I $P(G,U)=1 Q $$DATE^BGP6UTL($P(G,U,2))_" Refused"
 ...
 .Q:$P(^BIPC(X,0),U,4)>ED
 .S G=1
 I G Q $$DATE^BGP6UTL($P(G,U,2))_" Refused Imm pkg"
...
PAP(P,EDATE,YEARS) ;
 ...
 .S X=$$CPT^BGP6DU(P,$$FMADD^XLFDT(EDATE,-(3*365)),EDATE,T,5)
 S T="PAP SMEAR",T=$O(^BWPN("B",T,0))
 I T D  I X]"" Q "1^"_X_"^WH"
 .S X=$$WH^BGP6DU(P,$$FMADD^XLFDT(EDATE,-(3*365)),BGPEDATE,T,3)
...
HYSTER(P,EDATE) ;EP 
 ...
 I T D  I X]"" Q 1
 .S X=$$CPT^BGP6DU(P,$P(^DPT(P,0),U,3),EDATE,T,3)
 Q ""
