BGP6D31 ; IHS/CMI/LAB - measure C 01 Jun 2015 12:51 PM 01 Nov 2014 6:34 PM ; 
...
II ;EP
 ...
 I BGPSEV="" S BGPN8=1
 S Z=$P(BGPN1,U,2)
 S BGPVALUE=$S(BGPD2:"AC",1:"")_"|||" I BGPN1 S BGPVALUE=BGPVALUE_Z_$S(BGPHOSP:"; Hospital: "_$$DATE^BGP6UTL($P(BGPHOSP,U,2)),1:"")_$S(BGPN3:"; "_$P(BGPNER,U,2),1:"")_$S(BGPSEV:"; Severity: "_BGPSEV,1:"")
...
ERURG(P,BDATE,EDATE) ;EP
 ...
 .S %=$$PRIMPOV^APCLV(V,"I") I $$ICD^BGP6UTL2(%,T,9) S G=1,H=$P($P(^AUPNVSIT(V,0),U),".")
 .Q
...
V2ASTH(P,BDATE,EDATE) ;EP
 ...
 .Q:'D
 .S $P(G,U)=$P(G,U)+1,$P(G,U,2)=$P(G,U,2)_$S(G>1:", ",1:" ")_$$DATE^BGP6UTL($P(^TMP($J,"A",X),U))
 .Q
 ...
 I S>1 Q 1_U_"Severity "_S_" on visit "_$$DATE^BGP6UTL(9999999-D)
 Q ""
 ;
...
PNEU(P,BDATE,EDATE,FORE) ;EP
 ...
 S %=$$CPT^BGP6DU(P,BD,ED,$O(^ATXAX("B","BGP PNEUMO IZ CPTS",0)),5)
 I $P(BGPLPNU,U,1)<$P(%,U,1) S BGPLPNU=$P(%,U,1)_U_"CPT "_$P(%,U,2)
 S %=$$TRAN^BGP6DU(P,BD,ED,$O(^ATXAX("B","BGP PNEUMO IZ CPTS",0)),5)
 I $P(BGPLPNU,U,1)<$P(%,U,1) S BGPLPNU=$P(%,U,1)_U_"CPT "_$P(%,U,2)
 ...
 I $P(G,U)=1 Q $P(G,U,2)_U_"NMI Refusal"_U_3
 S R=$$CPTREFT^BGP6UTL1(P,$$DOB^AUPNPAT(P),ED,$O(^ATXAX("B","BGP PNEUMO IZ CPTS",0)),"N")
