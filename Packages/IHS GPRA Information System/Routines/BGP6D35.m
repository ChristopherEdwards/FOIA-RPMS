...
VAR(P,EDATE) ;EP
 ...
 I $$PLTAX^BGP6DU(P,"BGP VARICELLA EVIDENCE") Q 4_U_"evid var"
 ...
 I $$PLTAX^BGP6DU(P,"BGP VARICELLA CONTRA") Q 4_U_"contra var"
 ;now go to refusals
...
HEP(P,EDATE) ;EP
 ...
 I $$PLTAX^BGP6DU(P,"BGP HEP EVIDENCE") Q 4_U_"evid hep"
 ;now go to refusals
 S B=$$DOB^AUPNPAT(P),E=EDATE,BGPNMI="",R=""
 ...
 ;now check refusals in imm pkg
 F BGPIMM=8,42,43,44,45,51,102,104,110 S R=$$IMMREF^BGP6D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
 I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI",1:"ref")_" hep"
 Q ""
HIB(P,EDATE) ;EP
 K BGPC,BGPG,BGPX
 ...
 ;now go to refusals
 S B=$$DOB^AUPNPAT(P),E=EDATE,BGPNMI="",R=""
 F BGPIMM=22,46,47,48,49,50,51,102 D
 .S I=$O(^AUTTIMM("C",BGPIMM,0)) Q:'I
 .S X=0 F  S X=$O(^AUPNPREF("AA",P,9999999.14,I,X)) Q:X'=+X  S Y=0 F  S Y=$O(^AUPNPREF("AA",P,9999999.14,I,X,Y)) Q:Y'=+Y  S D=$P(^AUPNPREF(Y,0),U,3) I D'<B&(D'>E) S:$P(^AUPNPREF(Y,0),U,7)="N" BGPNMI=1 S R=1
 I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI",1:"ref")_" hib"
 ;now check refusals in imm pkg
 F BGPIMM=22,46,47,48,49,50,51,102 S R=$$IMMREF^BGP6D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
 I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI",1:"ref")_" hib"
 Q ""
PNEUMO(P,EDATE) ;EP
 K BGPC,BGPG,BGPX
 ...
 ;now check refusals in imm pkg
 F BGPIMM=33,100,109 S R=$$IMMREF^BGP6D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
 I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI",1:"ref")_" Pneumo"
 Q ""
