BGP6D37 ; IHS/CMI/LAB - IMMUNIZATIONS ;
...
FLU ;EP
 ...
 ;S BGPG=$$LASTPRC^BGP6UTL1(P,"BGP FLU IZ PROCEDURES",BD,ED)  ;NOT IN 14.1
 ...
 ..S V=$P(BGPG(X),U,5)
 ..S G=0
 ..S J=0 F  S J=$O(^AUPNVCPT("AD",V,J)) Q:J'=+J  S I=$P(^AUPNVCPT(J,0),U,1) I $$ICD^BGP6UTL2(I,T,1)!($$H1(I)) S G=1
 ..Q:G=1
 ...
 .S X=$$CPT^BGP6DU(P,,ED,T,5) I X]"" Q
 .S X=$$TRAN^BGP6DU(P,,ED,T,5)
 ...
 .S G=$$NMIREF^BGP6UTL1(P,9999999.14,$O(^AUTTIMM("C",BGPZ,0)),$$DOB^AUPNPAT(P),ED)
 .I $P(G,U)=1 S G=$P(G,U,2)_U_"NMI Refusal"_U_3
 I G Q G
 S R=$$CPTREFT^BGP6UTL1(P,$$DOB^AUPNPAT(P),ED,$O(^ATXAX("B","BGP CPT FLU",0)),"N")
...
ROTA2(P,EDATE) ;EP
 ...
 F BGPZ=119 S X=$$ANIMCONT^BGP6D31(P,BGPZ,EDATE) Q:X]""  ;cmi/maw 12/17/07 missing edate
 ...
 ;now check Refusals in imm pkg
 S BGPNMI=""
 ;F BGPIMM=119 S R=$$IMMREF^BGP6D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
...
ROTA3(P,EDATE) ;EP
 ...
 F BGPZ=119,74,116,122 S X=$$ANIMCONT^BGP6D31(P,BGPZ,EDATE) Q:X]""  ;cmi/maw 12/17/07 missing edate
 I X]"" Q 4_U_"Contra Rota"
 ;
 S B=$$DOB^AUPNPAT(P),E=EDATE,BGPNMI="",R=""
 ...
 ;now check Refusals in imm pkg
 S BGPNMI=""
 ;F BGPIMM=119,74,116,122 S R=$$IMMREF^BGP6D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
 ;I R Q 3_U_$S(BGPNMI:"NMI",1:"Ref")_" Rota"
 Q ""
HEPA(P,EDATE) ;EP
 ...
 I $$PLTAX^BGP6DU(P,"BGP HEPATITIS A EVIDENCE") Q 4_U_"Evid Hep A"
 ;
 ...
 ;now check Refusals in imm pkg
 S BGPNMI=""
 ;F BGPIMM=31,52,83,84,85,104 S R=$$IMMREF^BGP6D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
