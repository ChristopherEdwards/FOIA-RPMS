...
I0302A ;EP
 ...
 ;S BGPIHD=$$IHDCCVD^BGP6D729(DFN,BGPBDATE,BGPEDATE)
 S BGPCHD=0
...
I0302ASC ;EP
 ...
 S BGPBPC=""
 S BGPBP=$$MEANBP^BGP6D26(DFN,$$FMADD^XLFDT(BGPEDATE,-(2*365)),BGPEDATE)
 I BGPBP["unknown" S BGPBPC=$$BPCPT^BGP6D22(DFN,$$FMADD^XLFDT(BGPEDATE,-(2*365)),BGPEDATE) I BGPBPC]"" S BGPBP=$P(BGPBPC,U,2)_"^1"
 S BGPBPDEV=$$MEANBP^BGP6D26(DFN,$$FMADD^XLFDT(BGPEDATE,-(2*365)),BGPEDATE,1)
 I BGPBPDEV["unknown" S BGPBPCD=$$BPCPT^BGP6D22(DFN,$$FMADD^XLFDT(BGPEDATE,-(2*365)),BGPEDATE,1) I BGPBPCD]"" S BGPBPDEV=$P(BGPBPCD,U,2)_"^1"
 S BGPN1=$S($P(BGPBP,U,2):1,1:0)
 S BGPN16=$S($P(BGPBPDEV,U,2):1,1:0)
 ;S BGPLDL=$$LDL^BGP6D2(DFN,$$FMADD^XLFDT(BGPEDATE,-(5*365)),BGPEDATE,1) ;date^value
 ;I $P(BGPLDL,U) S BGPN11=1  ;had ldl done
 ;S BGPLDL1=$$LDL^BGP6D2(DFN,BGPBDATE,BGPEDATE,1) ;date^value
 ;I $P(BGPLDL1,U) S BGPN2=1  ;had ldl done
 S BGPTOB=$$TOBACCO(DFN,BGP365,BGPEDATE)
 I $P(BGPTOB,U) S BGPN3=1
 S BGPBMI=$$BMI^BGP6D6(DFN,BGPEDATE,BGPAGEE),BGPN4=$S(BGPBMI]"":1,1:0)
 ;S BGPBMIR="" I 'BGPN4 S BGPBMIR=$$REF^BGP6D6(DFN,BGPBDATE,BGPEDATE,BGPAGEE) I $P(BGPBMIR,U) S BGPN4=1,BGPN8=1,BGPBMIR=1_U_"BMI: Ref ht "_$$DATE^BGP6UTL($P(BGPBMIR,U,3))_" wt "_$$DATE^BGP6UTL($P(BGPBMIR,U,6))
 ...
 ;I 'BGPN6 S BGPDEP=$$DEPREF^BGP6D25(DFN,BGPBDATE,BGPEDATE) I BGPDEP S BGPN6=1
 ...
 ;S %=$S($P(BGPLDL1,U,2):"LDL: "_$$DATE^BGP6UTL($P(BGPLDL1,U,2)),1:"")
 ;I %]"" S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":";",1:"") S BGPVALUE=BGPVALUE_" "_%
 S %="" I $P(BGPTOB,U) S %="TOB: "_$S($P(BGPTOB,U,3)["/":$P(BGPTOB,U,3),1:$$DATE^BGP6UTL($P(BGPTOB,U,3)))_" "_$P(BGPTOB,U,2)
 I %]"" S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":";",1:"") S BGPVALUE=BGPVALUE_" "_%
 I BGPN10 D
 .S %=$S($P(BGPBMI,U)]"":"BMI: "_$J(BGPBMI,5,1),1:$P(BGPBMIR,U,2))
 .I %]"" S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":";",1:"") S BGPVALUE=BGPVALUE_" "_%
 S %=$S($P(BGPLIFED,U):"LIFE: "_$$DATE^BGP6UTL($P(BGPLIFED,U,2))_" "_$P(BGPLIFED,U,3),1:"")
 I %]"" S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":";",1:"") S BGPVALUE=BGPVALUE_" "_%
...
I0302 ;EP
 ...
 I BGPACTCL,$$CHD^BGP6D729(DFN,BGP365,BGPEDATE) S BGPIHD=1,BGPD3=1
...
BPCV ;EP - called from elder care routine
 ...
 I BGPVALUE["unknown" S BGPVALUE=$$BPCPT^BGP6D22(DFN,BGPBDATE,BGPEDATE) I BGPVALUE]"" S BGPN1=1,BGPVALUE=$P(BGPVALUE,U,2) G V  ;CPTS COUNT IN NUMERATOR 1 ONLY
...
DEP(P,BDATE,EDATE) ;EP
 S X=$$DEP^BGP6D25(P,BDATE,EDATE) I $P(X,U) Q X
 S X=$$DEPSCR^BGP6D25(P,BDATE,EDATE) I $P(X,U) Q X
 Q ""
DEPCHD(P,BDATE,EDATE) ;EP
 S X=$$DEP^BGP6D25(P,BDATE,EDATE) I $P(X,U) Q X
 S X=$$DEPSCR^BGP6D25(P,BDATE,EDATE) I $P(X,U) Q X
 S X=$$DEPSUIC^BGP6D25(P,BDATE,EDATE) I $P(X,U) Q X
 Q ""
LIFE(P,BDATE,EDATE) ;EP
 S X=$$MEDNUTR^BGP6D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$SPECNUTR^BGP6D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$SPECEX^BGP6D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$OTHREL^BGP6D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 Q ""
LIFED(P,BDATE,EDATE) ;EP
 S X=$$MEDNUTRD^BGP6D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$SPECNUTR^BGP6D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$SPECEX^BGP6D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$OTHREL^BGP6D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 Q ""
TOBACCO(P,BDATE,EDATE) ;EP
 S X=$$TOBACCO^BGP6D7(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$TOBDX(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$PED^BGP6D7(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$DENT^BGP6D7(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$CPTSM^BGP6D7(P,BDATE,EDATE) I X]"" Q 1_U_X
 Q ""
...
TOBDX(P,BDATE,EDATE) ;EP
 ...
 .I $E($P($$ICDDX^BGP6UTL2(Y),U,2),1,5)'="305.1",$P($$ICDDX^BGP6UTL2(Y),U,2)'="V15.82" Q
 .S G=$P($$ICDDX^BGP6UTL2(Y),U,2)_" PL"
 .Q
 Q G
