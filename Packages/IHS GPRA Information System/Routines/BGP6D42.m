...
OSTEOSCR(P,BDATE,EDATE) ;EP
 S X=$$LASTPRCI^BGP6UTL1(P,"88.98",BDATE,EDATE)
 I X Q X
 S X=$$LASTDXI^BGP6UTL1(P,"V82.81",BDATE,EDATE)
 I X Q X
 S X=$$CPT^BGP6DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP OSTEO SCREEN CPTS",0)),6)
 I X Q 1_U_$P(X,U,3)_U_$P(X,U,2)
 ;now check for refusal
 S X=$$RAD^BGP6DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP OSTEO SCREEN CPTS",0)),6)
 I X Q 1_U_$P(X,U,3)_U_$P(X,U,2)
 ;
 S X=$$RADREF^BGP6UTL1(P,$$FMADD^XLFDT(EDATE,-365),EDATE,$O(^ATXAX("B","BGP OSTEO SCREEN CPTS",0)))
