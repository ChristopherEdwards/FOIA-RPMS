BGP6D5 ; IHS/CMI/LAB - measure calc ;
...
I20 ;EP
 ...
 I BGPAGEB<14 S BGPSTOP=1 Q
 I BGPAGEB>46 S BGPSTOP=1 Q
 I BGPACTUP S BGPD2=1
 I BGPACTCL S BGPD1=1
 ;S BGPN2=$$ALHF(DFN,BGP365,BGPEDATE)
 K BGPALL
 D ALSCRN^BGP6D55(DFN,BGPBDATE,BGPEDATE,.BGPALL)
 I $D(BGPALL) S BGPN2=1
 S BGPN3=$$ALDX^BGP6D5A(DFN,BGP365,BGPEDATE)
 ...
 I $P(BGPN2,U)=1 S BGPVALUE=BGPVALUE_"SCREEN: "_$P(C,U,3)_" "_$P(C,U,2)_" result="_$S($P(C,U,6)]"":$E($P(C,U,6),1,3),1:"NO RESULT")
...
DV ;EP - called from elder care
 ...
 I $P(BGPDVEX,U)=1 S BGPVALUE=BGPVALUE_"EXAM: "_$$DATE^BGP6UTL($P(BGPDVEX,U,2))_" "_$P(BGPDVEX,U,4)
 I $P(BGPDVDX,U)=1 S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":"; DX: ",1:"DX: ")_$$DATE^BGP6UTL($P(BGPDVDX,U,2))_" "_$P(BGPDVDX,U,4)
 I $P(BGPDVPED,U)=1 S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":"; PT ED: ",1:"PT ED: ")_$$DATE^BGP6UTL($P(BGPDVPED,U,2))_" "_$P(BGPDVPED,U,4)
 I $P(BGPDVCOU,U)=1 S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":"; COUNSEL ",1:"COUNSEL ")_$$DATE^BGP6UTL($P(BGPDVCOU,U,2))_" "_$P(BGPDVCOU,U,4)
 I BGPRTYPE'=1,$P(BGPDVREF,U)=1 S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":"; ",1:"")_$$DATE^BGP6UTL($P(BGPDVREF,U,2))_" "_$P(BGPDVREF,U,3)
 S BGPVALUD="" I BGPD4 S BGPVALUD="AC"_"|||" D
 .I $P(BGPDVEX,U)=1 S BGPVALUD=BGPVALUD_"EXAM: "_$$DATE^BGP6UTL($P(BGPDVEX,U,2))_" "_$P(BGPDVEX,U,4)
 .I $P(BGPDVDX,U)=1 S BGPVALUD=BGPVALUD_$S($P(BGPVALUD,"|||",2)]"":"; DX: ",1:"DX: ")_$$DATE^BGP6UTL($P(BGPDVDX,U,2))_" "_$P(BGPDVDX,U,4)
 .I $P(BGPDVPED,U)=1 S BGPVALUD=BGPVALUD_$S($P(BGPVALUD,"|||",2)]"":"; PT ED: ",1:"PT ED: ")_$$DATE^BGP6UTL($P(BGPDVPED,U,2))_" "_$P(BGPDVPED,U,4)
 .I $P(BGPDVCOU,U)=1 S BGPVALUD=BGPVALUD_$S($P(BGPVALUD,"|||",2)]"":"; COUNSEL ",1:"COUNSEL ")_$$DATE^BGP6UTL($P(BGPDVCOU,U,2))_" "_$P(BGPDVCOU,U,4)
...
DVEX(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(1)) S BGPLDV=1_"^"_$P(BGPG(1),U)_"^IPV EXAM 34"_U_"Ex 34"_U_$$DATE^BGP6UTL($P(BGPG(1),U))
 S BGPC=0,BGPV=""
 ...
 .I X="REF" Q
 .S BGPC=BGPC+1,BGPV="1^"_(9999999-D)_"^BH IPV EXAM"_U_" BH Ex 34"_U_$$DATE^BGP6UTL((9999999-D))
 I $P(BGPLDV,U,2)<$P(BGPC,U,2) S BGPLDV=BGPC
 Q BGPLDV
REFDV(P,BDATE,EDATE) ;EP
 ;add Refusal for exam 34
 S G=$$REFUSAL^BGP6UTL1(P,9999999.15,$O(^AUTTEXAM("C",34,0)),$$FMADD^XLFDT(EDATE,-365),EDATE)
...
DVPED(P,BDATE,EDATE) ;EP
 ...
 K BGPG
 S Y="BGPG(",BGPLDV=""
 S SN=$O(^BGPSNOMM("B","IPV/DV PATIENT ED",0))
 ...
 .I $P(T,"-",1)="DV"!($P(T,"-",2)="DV") S BGPLDV=1_"^"_$P(BGPG(X),U)_"^PED "_T_U_"PED "_T_U_$$DATE^BGP6UTL($P(BGPG(X),U)) Q
 .S S=$P(T,"-",1)
 .S S=$$ICDDX^BGP6UTL2(S)
 .I $P(S,U,1)'="-1",$$ICD^BGP6UTL2($P(S,U,1),$O(^ATXAX("B","BGP IPV/DV EDUC DXS",0)),9) S BGPLDV=1_"^"_$P(BGPG(X),U)_"^PED "_T_U_"PED "_T_U_$$DATE^BGP6UTL($P(BGPG(X),U)) Q
 .I $P(T,"-",1)]"",$D(^BGPSNOMM(SN,11,"B",$P(T,"-",1))) S BGPLDV=1_"^"_$P(BGPG(X),U)_"^PED "_T_U_"PED "_T_U_$$DATE^BGP6UTL($P(BGPG(X),U)) Q
 ;
BHPED ;
 ...
 ...S BGPLDV=1_"^"_(9999999-$P(D,"."))_"^BH PED "_T_U_"BH PED "_T_U_$$DATE^BGP6UTL((9999999-$P(D,".")))
 ..S S=$P(T,"-",1)
 ..S S=$$ICDDX^BGP6UTL2(S)
 ..I $P(S,U,1)'="-1",$$ICD^BGP6UTL2($P(S,U,1),$O(^ATXAX("B","BGP IPV/DV EDUC DXS",0)),9),$P(BGPLDV,U,2)<(9999999-$P(D,".")) D
 ...S BGPLDV=1_"^"_(9999999-$P(D,"."))_"^BH PED "_T_U_"BH PED "_T_U_$$DATE^BGP6UTL((9999999-$P(D,".")))
 ..I $P(T,"-",1)]"",$D(^BGPSNOMM(SN,11,"B",$P(T,"-",1))) S BGPLDV=1_"^"_(9999999-$P(D,"."))_"^BH PED "_T_U_"BH PED "_T_U_$$DATE^BGP6UTL((9999999-$P(D,"."))) Q
...
DV61(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(1)) S BGPLDV=1_"^"_$P(BGPG(1),U)_"^"_$P(BGPG(1),U,2)_U_"POV "_$P(BGPG(1),U,2)_U_$$DATE^BGP6UTL($P(BGPG(1),U))
 Q BGPLDV
ALPRC(P,BDATE,EDATE) ;EP
 S BGPG=$$LASTPRC^BGP6UTL1(P,"BGP ALCOHOL PROCEDURES",BDATE,EDATE)
 I BGPG Q 1_U_"Proc "_$P(BGPG,U,2)_U_$$DATE^BGP6UTL($P(BGPG,U,3))_U_$P(BGPG,U,3)
 Q ""
ALPED(P,BDATE,EDATE) ;EP
 NEW BGPG
 NEW X,Y,D,E,T,%,S,BGPLPED,SN
 S Y="BGPG(",BGPLPED=""
 S SN=$O(^BGPSNOMM("B","ALCOHOL SCREEN PATIENT ED",0))
 S X=P_"^ALL EDUC;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE) S E=$$START1^APCLDF(X,Y)
 ...
 .Q:'$D(^AUTTEDT(T,0))
 .S T=$P(^AUTTEDT(T,0),U,2)
 .I $P(T,"-",1)="CD"!($P(T,"-",2)="CD")!($P(T,"-",1)="AOD")!($P(T,"-",2)="AOD"),$P(BGPLPED,U,4)<$P(BGPG(X),U) S BGPLPED=1_U_T_U_$$DATE^BGP6UTL($P(BGPG(X),U))_U_$P(BGPG(X),U)
 .S S=$P(T,"-",1)
 .S S=$$ICDDX^BGP6UTL2(S)
 .I $P(S,U,1)'="-1",$$ICD^BGP6UTL2($P(S,U,1),$O(^ATXAX("B","BGP ALCOHOL EDUC DXS",0)),9),$P(BGPLPED,U,4)<$P(BGPG(X),U,1) S BGPLPED=1_U_""_T_U_$$DATE^BGP6UTL($P(BGPG(X),U))_U_$P(BGPG(X),U)
 .I $P(T,"-",1)="99408"!($P(T,"-",1)="99409")!($P(T,"-",1)["G0396")!($P(T,"-",1)["G0397")!($P(T,"-")["H0049")!($P(T,"-")["H0050")!($P(T,"-")["3016F"),$P(BGPLPED,U,4)<$P(BGPG(X),U) D
 ..S BGPLPED=1_U_T_U_$$DATE^BGP6UTL($P(BGPG(X),U))_U_$P(BGPG(X),U)
 .I $P(T,"-")]"",$D(^BGPSNOMM(SN,11,"B",$P(T,"-"))),$P(BGPLPED,U,4)<$P(BGPG(X),U) D
 ..S BGPLPED=1_U_T_U_$$DATE^BGP6UTL($P(BGPG(X),U))_U_$P(BGPG(X),U)
ALMH S BGPC="",T="" S E=9999999-BDATE,D=9999999-EDATE-1_".99" F  S D=$O(^AMHREC("AE",P,D)) Q:D'=+D!($P(D,".")>E)!(BGPC)  S V=0 F  S V=$O(^AMHREC("AE",P,D,V)) Q:V'=+V!(BGPC)  D
 ...
 ..Q:'$D(^AUTTEDT(T,0))
 ..S T=$P(^AUTTEDT(T,0),U,2)
 ..I $P(T,"-",1)="CD"!($P(T,"-",2)="CD")!($P(T,"-",1)="AOD")!($P(T,"-",2)="AOD"),$P(BGPLPED,U,4)<(9999999-D) S BGPLPED=1_U_T_U_$$DATE^BGP6UTL((9999999-D))_U_(9999999-D)
 ..S S=$P(T,"-",1)
 ..S S=$$ICDDX^BGP6UTL2(S)
 ..I $P(S,U,1)'="-1",$$ICD^BGP6UTL2($P(S,U,1),$O(^ATXAX("B","BGP ALCOHOL EDUC DXS",0)),9),$P(BGPLPED,U,4)<(9999999-D) D
 ...S BGPLPED=1_U_T_U_$$DATE^BGP6UTL((9999999-D))_U_(9999999-D)
 ..I $P(T,"-",1)="99408"!($P(T,"-",1)="99409")!($P(T,"-",1)["G0396")!($P(T,"-",1)["G0397")!($P(T,"-")["H0049")!($P(T,"-")["H0050"),$P(BGPLPED,U,4)<(9999999-D) D
 ...S BGPLPED=1_U_T_U_$$DATE^BGP6UTL((9999999-D))_U_(9999999-D)
 ..I $P(T,"-")]"",$D(^BGPSNOMM(SN,11,"B",$P(T,"-"))),$P(BGPLPED,U,4)<(9999999-D) D
 ...S BGPLPED=1_U_T_U_$$DATE^BGP6UTL((9999999-D))_U_(9999999-D)
 Q BGPLPED
 ;
LASTHF(P,C,BDATE,EDATE) ;EP - get last factor in category C for patient P
 ...
 S D=$O(O(0))
 I D="" Q D
 Q $$VAL^XBDIQ1(9000010.23,O(D),.01)_"^"_$$DATE^BGP6UTL(9999999-D)_U_(9999999-D)
 ;
ALREF(P,BDATE,EDATE) ;EP
 ;add Refusal for exam 35
 S G=$$REFUSAL^BGP6UTL1(P,9999999.15,$O(^AUTTEXAM("C",35,0)),$$FMADD^XLFDT(EDATE,-365),EDATE)
 I $P(G,U)=1 Q 1_"^Refused Ex 35^"_$$DATE^BGP6UTL($P(G,U,2))_U_$P(G,U,2)
 Q ""
LOINC(A,B) ;
 NEW %
