...
REFDV(P,BDATE,EDATE) ;EP
 ...
 ;add refusals for education topics
 S G=""
...
ALHF(P,BDATE,EDATE) ;alcohol hf or screening pov
 ...
 I $D(BGPG(1)) Q 1_"^exam 35^"_$$DATE^BGP6UTL($P(BGPG(1),U))
 ...
 ..Q:'T
 ..I $P($G(^AUTTHF(T,0)),U)["ALCOHOL" S BGPC=1_U_"BH HF: "_$P(^AUTTHF(F,0),U)_U_$$DATE^BGP6UTL((9999999-D))
 ..Q
 I BGPC Q BGPC
 K BGPG S %=P_"^LAST DX V11.3;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 1_U_"DX: V11.3"_U_$$DATE^BGP6UTL($P(BGPG(1),U))
 S %=P_"^LAST DX V79.1;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 1_U_"DX: V79.1"_U_$$DATE^BGP6UTL($P(BGPG(1),U))
 ;go through BH record file and find up to 1 visits in date range
 S E=9999999-BDATE,D=9999999-EDATE-1_".99" F  S D=$O(^AMHREC("AE",P,D)) Q:D'=+D!($P(D,".")>E)!(BGPC)  S V=0 F  S V=$O(^AMHREC("AE",P,D,V)) Q:V'=+V!(BGPC)  D
 .S X=0 F  S X=$O(^AMHRPRO("AD",V,X)) Q:X'=+X!(BGPC)  S BGPP=$P($G(^AMHRPRO(X,0)),U) D
 ..Q:'BGPP
 ..S BGPP=$P($G(^AMHPROB(BGPP,0)),U)
 ..I BGPP=29.1 S BGPC=1_U_"BH dx: 29.1"_U_$$DATE^BGP6UTL((9999999-$P(D,"."))) Q
 ..Q
 I BGPC Q BGPC
 Q ""
ALDX(P,BDATE,EDATE) ;
 ...
 I $D(BGPG(1)) Q 1_U_"DX: "_$P(BGPG(1),U,2)_U_$$DATE^BGP6UTL($P(BGPG(1),U))
 S BGPC=0
 ;go through BH record file and find up to 1 visits in date range
 S E=9999999-BDATE,D=9999999-EDATE-1_".99" F  S D=$O(^AMHREC("AE",P,D)) Q:D'=+D!($P(D,".")>E)!(BGPC)  S V=0 F  S V=$O(^AMHREC("AE",P,D,V)) Q:V'=+V!(BGPC)  D
 .S X=0 F  S X=$O(^AMHRPRO("AD",V,X)) Q:X'=+X!(BGPC)  S BGPP=$P($G(^AMHRPRO(X,0)),U) D
 ..Q:'BGPP
 ..S BGPP=$P($G(^AMHPROB(BGPP,0)),U)
 ..I BGPP=10 S BGPC=1_U_"BH dx: 10"_U_$$DATE^BGP6UTL((9999999-D)) Q
 ..I BGPP=27 S BGPC=1_U_"BH dx: 27"_U_$$DATE^BGP6UTL((9999999-D)) Q
 ..I BGPP=29 S BGPC=1_U_"BH dx: 29"_U_$$DATE^BGP6UTL((9999999-D)) Q
 ..I $E(BGPP,1,3)=303  S BGPC=1_U_"BH dx: "_BGPP_U_$$DATE^BGP6UTL((9999999-D)) Q
 ..I $E(BGPP,1,5)=305.0 S BGPC=1_U_"BH dx: "_BGPP_U_$$DATE^BGP6UTL((9999999-D)) Q
 ..I $E(BGPP,1,3)=291 S BGPC=1_U_"BH dx: "_BGPP_U_$$DATE^BGP6UTL((9999999-D)) Q
 ..I $E(BGPP,1,5)=357.5 S BGPC=1_U_"BH dx: "_BGPP_U_$$DATE^BGP6UTL((9999999-D)) Q
 ..Q
 I BGPC Q BGPC
 ...
 .S Y=$P(^AUPNPROB(X,0),U)
 .Q:'$$ICD^ATXCHK(Y,T,9)
 .S G=1_U_"PROB LIST: "_$P($$ICDDX^ICDCODE(Y),U,2)_U_$$DATE^BGP6UTL((9999999-D))
 .Q
 I G Q G
 S (X,G)=0 F  S X=$O(^AMHPPROB("AC",P,X)) Q:X'=+X!(G)  D
 .Q:$P(^AMHPPROB(X,0),U,12)'="A"
 ...
 .S Y=$P(^AMHPPROB(X,0),U)
 .S Y=$P($G(^AMHPROB(Y,0)),U)
 .I $E(Y,1,3)="303" S G=1_U_"BH PL: "_Y_U_$$DATE^BGP6UTL($P(^AMHPPROB(X,0),U,3)) Q
 .I $E(Y,1,5)="305.0" S G=1_U_"BH PL: "_Y_U_$$DATE^BGP6UTL($P(^AMHPPROB(X,0),U,3)) Q
 .I $E(Y,1,3)=291 S G=1_U_"BH PL: "_Y_U_$$DATE^BGP6UTL($P(^AMHPPROB(X,0),U,3)) Q
 .I $E(Y,1,5)=357.5 S G=1_U_"BH PL: "_Y_U_$$DATE^BGP6UTL($P(^AMHPPROB(X,0),U,3)) Q
 .I Y=10 S G=1_U_"BH PL: "_Y_U_$$DATE^BGP6UTL($P(^AMHPPROB(X,0),U,3)) Q
 .I Y=27 S G=1_U_"BH PL: "_Y_U_$$DATE^BGP6UTL($P(^AMHPPROB(X,0),U,3)) Q
 .I Y=29 S G=1_U_"BH PL: "_Y_U_$$DATE^BGP6UTL($P(^AMHPPROB(X,0),U,3)) Q
 .Q
 Q G
ALPED(P,BDATE,EDATE) ;
 K BGPG
 ...
 .Q:'$D(^AUTTEDT(T,0))
 .S T=$P(^AUTTEDT(T,0),U,2)
 .I $P(T,"-",1)="CD"!($P(T,"-",2)="CD")!($P(T,"-",1)="AOD")!($P(T,"-",2)="AOD") S D=1_U_"EDUC: "_T_U_$$DATE^BGP6UTL($P(BGPG(1),U))
 I D Q D
ALMH ;
 ...
 ..Q:'$D(^AUTTEDT(T,0))
 ..S T=$P(^AUTTEDT(T,0),U,2)
 ..I $P(T,"-",1)="CD"!($P(T,"-",2)="CD")!($P(T,"-",1)="AOD")!($P(T,"-",2)="AOD") S BGPC=1_U_"EDUC: "_T_U_$$DATE^BGP6UTL((9999999-D))
 Q BGPC
 ;
LASTHF(P,C,BDATE,EDATE) ;EP - get last factor in category C for patient P
 ...
 S D=$O(O(0))
 I D="" Q D
 Q $$VAL^XBDIQ1(9000010.23,O(D),.01)_"^"_$$DATE^BGP6UTL(9999999-D)
 ;
CHL(P,BDATE,EDATE) ;EP
 I '$G(P) Q ""
 S BGPC=""
 K BGPG S %=P_"^LAST DX V73.88;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP6UTL($P(BGPG(1),U))_U_"V73.88"
 S %=P_"^LAST DX V73.98;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 1_U_"V73.88"_U_$$DATE^BGP6UTL($P(BGPG(1),U))
 ;check cpt taxonomy
 S T=$O(^ATXAX("B","BGP CHLAMYDIA CPTS",0))
 I T D  I X Q 1_U_$P(X,U,3)_U_$$DATE^BGP6UTL($P(X,U,2))
 .S X=$$CPT^BGP6DU(P,,EDATE,T,6)
 ...
 ...S BGPC=1_U_"lab loinc"_U_$$DATE^BGP6UTL((9999999-D))
 ...Q
 I BGPC Q BGPC
 ...
 ...Q:'$D(^AUPNVMIC(X,0))
 ...I BGPLT,$P(^AUPNVMIC(X,0),U),$D(^ATXLAB(BGPLT,21,"B",$P(^AUPNVMIC(X,0),U))) S BGPC=1_U_"v micro test"_U_$$DATE^BGP6UTL((9999999-D)) Q
 ...Q:'T
 ...S J=$P($G(^AUPNVMIC(X,11)),U,13) Q:J=""
 ...Q:'$$LOINC(J,T)
 ...S R=$P(^AUPNVMIC(X,0),U,4)
 ...S BGPC=1_U_"v micro loinc"_U_$$DATE^BGP6UTL((9999999-D))
 ...Q
 Q BGPC
ALREF(P,BDATE,EDATE) ;EP
 ...
 I $P(G,U)=1 Q 1_"^ref exam 35^"_$$DATE^BGP6UTL($P(G,U,2))
 Q ""
LOINC(A,B) ;
 NEW %
