BGP6D55 ; IHS/CMI/LAB - measure calc ;
...
ALEP ;did they have screening?
 ...
 ;set numerators
 S BGPN2=$$ALPED^BGP6D5(DFN,BGPBDATE,BGPEDATE)
...
ALDX(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(1)) S BGPLAL=1_U_"DX "_$P(BGPG(1),U,2)_U_$$DATE^BGP6UTL($P(BGPG(1),U))_U_$P(BGPG(1),U)
 S BGPC=""
 ...
 ..I $$ICD^BGP6UTL2(BGPP,BGPT,9) S BGPC=1_U_"BH POV "_$P(^AMHPROB(BGPP,0),U)_U_$$DATE^BGP6UTL((9999999-$P(D,".")))_U_(9999999-$P(D,".")) Q
 ..S BGPP=$P($G(^AMHPROB(BGPP,0)),U)
 ..I $D(^ATXAX(BGPJ,21,"B",BGPP)) S BGPC=1_U_"BH POV "_BGPP_U_$$DATE^BGP6UTL((9999999-$P(D,".")))_U_(9999999-$P(D,".")) Q
 ..Q
 ...
 .I $$ICD^BGP6UTL2(Y,BGPT,9) S G=1_U_"BH PL "_Y_U_$$DATE^BGP6UTL($P(^AMHPPROB(X,0),U,3))_U_$P(^AMHPPROB(X,0),U,3) Q
 .S Y=$P($G(^AMHPROB(Y,0)),U)
 .I $D(^ATXAX(BGPJ,21,"B",Y)) S G=1_U_"BH PL "_Y_U_$$DATE^BGP6UTL($P(^AMHPPROB(X,0),U,3))_U_$P(^AMHPPROB(X,0),U,3) Q
 .Q
 I $P(BGPLAL,U,4)<$P(G,U,4) S BGPLAL=G
 ;
 K BGPG
 S BGPG=$$LASTPRC^BGP6UTL1(P,"BGP ALCOHOL PROCEDURES",BDATE,EDATE)
 I $P(BGPLAL,U,4)<$P(BGPG,U,3) S BGPLAL=1_U_"Proc "_$P(BGPG,U,2)_U_$$DATE^BGP6UTL($P(BGPG,U,3))_U_$P(BGPG,U,3)
 Q BGPLAL
...
PCCEX .;
 ...
 ..S BGPALL(BGPIVD,BGPSC)=1_"^Ex 35^"_$$DATE^BGP6UTL(BGPVD)_U_BGPVD_U_$S(R="POSITIVE":1,1:0)_U_R
PCCHF .;
 ...
 ..S BGPSC=BGPSC+1,BGPALL(BGPIVD,BGPSC)=1_"^HF "_I_"^"_$$DATE^BGP6UTL(BGPVD)_U_BGPVD_U_$S(R="POSITIVE":1,1:0)_U_R
PCCDX .;
 .S E=0 F  S E=$O(^AUPNVPOV("AD",BGPV,E)) Q:E'=+E  D
 ..S I=$$VAL^XBDIQ1(9000010.07,E,.01)
 ..I '$$ICD^BGP6UTL2($$VALI^XBDIQ1(9000010.07,E,.01),$O(^ATXAX("B",$S('SBI:"BGP ALCOHOL SCREEN DXS",1:"BGP SCREEN FOR ALCOHOLISM DX"),0)),9) Q
 ..S BGPSC=BGPSC+1,BGPALL(BGPIVD,BGPSC)=1_U_"POV "_I_U_$$DATE^BGP6UTL(BGPVD)_U_BGPVD
PCCCPT .;
 ...
 ..S J=$P(^ICPT(I,0),U,1)
 ..S R=$S($$ICD^BGP6UTL2(I,$O(^ATXAX("B","BGP ALCOHOL POSITIVE SCRN CPTS",0)),1):"POSITIVE",1:"")
 ..S BGPSC=BGPSC+1
 ..S BGPALL(BGPIVD,BGPSC)=1_"^CPT "_J_"^"_$$DATE^BGP6UTL(BGPVD)_U_BGPVD_U_$S(R="POSITIVE":1,1:0)_U_R
PCCMEAS .;now add in v measurements
 ...
 ..S BGPALL(BGPIVD,BGPSC)=1_"^Meas "_I_"^"_$$DATE^BGP6UTL(BGPVD)_U_BGPVD_U_T_U_$S(T:"POSITIVE: "_R,1:"NEGATIVE: "_R)
BH ;CHECK BH VISITS
 S BGPC="",T="",F=""
...
BHEX .;
 ...
 ..S BGPSC=BGPSC+1
 ..S BGPALL(BGPIVD,BGPSC)=1_"^BH Ex^"_$$DATE^BGP6UTL(BGPVD)_U_BGPVD_U_$S(R="POSITIVE":1,1:0)_U_R
BHHF .;
 ...
 ..I I'="CAGE 0/4",I'="CAGE 1/4",I'="CAGE 2/4",I'="CAGE 3/4",I'="CAGE 4/4" Q  ;cage only
 ..S R=$S(I["0":"NEGATIVE",1:"POSITIVE")
 ..S BGPSC=BGPSC+1,BGPALL(BGPIVD,BGPSC)=1_"^BH HF "_I_"^"_$$DATE^BGP6UTL(BGPVD)_U_BGPVD_U_$S(R="POSITIVE":1,1:0)_U_R
 ..Q
BHDX .;
 .S X=0 F  S X=$O(^AMHRPRO("AD",V,X)) Q:X'=+X  D
 ..S I=$$VAL^XBDIQ1(9002011.01,X,.01)
 ..I '$$ICD^BGP6UTL2($$VALI^XBDIQ1(9002011.01,X,.01),$O(^ATXAX("B",$S('SBI:"BGP ALCOHOL SCREEN DXS",1:"BGP SCREEN FOR ALCOHOLISM DX"),0)),9),I'="29.1" Q
 ..S BGPSC=BGPSC+1,BGPALL(BGPIVD,BGPSC)=1_U_"BH POV "_I_U_$$DATE^BGP6UTL(BGPVD)_U_BGPVD
 ..Q
BHMEAS .;
 ...
 ..I I="AUDC",$P(^DPT(P,0),U,2)="F",R>2 S T=1
 ..I I="CRFT",R>1,R<7 S T=1
 ..S BGPALL(BGPIVD,BGPSC)=1_"^BH Meas "_I_"^"_$$DATE^BGP6UTL(BGPVD)_U_BGPVD_U_T_U_$S(T=1:"POSITIVE: "_R,1:"NEGATIVE")
BHCPT .;now add in CPT codes
 ...
 ..S R=$S($$ICD^BGP6UTL2(I,$O(^ATXAX("B","BGP ALCOHOL POSITIVE SCRN CPTS",0)),1):"POSITIVE",1:"")
 ..S BGPSC=BGPSC+1
 ..S BGPALL(BGPIVD,BGPSC)=1_"^BH CPT "_J_"^"_$$DATE^BGP6UTL(BGPVD)_U_BGPVD_U_$S(R="POSITIVE":1,1:0)_U_R
 ..Q
 .Q
 Q
