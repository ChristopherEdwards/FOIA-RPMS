BGP6D61 ; IHS/CMI/LAB - measure 31 ;
...
SIG(P,EDATE) ;
 ...
 S BGPG=$$LASTPRCI^BGP6UTL1(P,"45.24",BDATE,EDATE)
 I $P(BGPG,U)=1 Q "SIG 45.24"_"^"_$$DATE^BGP6UTL($P(BGPG,U,3))
 S BGPG=$$LASTPRCI^BGP6UTL1(P,"45.42",BDATE,EDATE)
 I $P(BGPG,U)=1 Q "SIG 45.42"_"^"_$$DATE^BGP6UTL($P(BGPG,U,3))
 S T=$O(^ATXAX("B","BGP SIG CPTS",0))
 I T D  I X]"" Q "CPT SIG "_$P(X,U,2)_U_$$DATE^BGP6UTL($P(X,U,1))
...
COLO(P,EDATE) ;
 ...
 ;I $D(BGPG(1)) Q "COLO 45.22"_"^"_$$DATE^BGP6UTL($P(BGPG(1),U))
 S BGPG=$$LASTPRCI^BGP6UTL1(P,"45.22",BDATE,EDATE)
 I $P(BGPG,U)=1 Q "COLO 45.22"_"^"_$$DATE^BGP6UTL($P(BGPG,U,3))
 S BGPG=$$LASTPRCI^BGP6UTL1(P,"45.23",BDATE,EDATE)
 I $P(BGPG,U)=1 Q "COLO 45.23"_"^"_$$DATE^BGP6UTL($P(BGPG,U,3))
 S BGPG=$$LASTPRCI^BGP6UTL1(P,"45.25",BDATE,EDATE)
 I $P(BGPG,U)=1 Q "COLO 45.25"_"^"_$$DATE^BGP6UTL($P(BGPG,U,3))
 S BGPG=$$LASTPRCI^BGP6UTL1(P,"45.43",BDATE,EDATE)
 I $P(BGPG,U)=1 Q "COLO 45.43"_"^"_$$DATE^BGP6UTL($P(BGPG,U,3))
 S %=P_"^LAST DIAGNOSIS V76.51;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q "COLO DX V76.51"_"^"_$$DATE^BGP6UTL($P(BGPG(1),U))
 S T=$O(^ATXAX("B","BGP COLO CPTS",0))
 I T D  I X]"" Q "CPT COLO "_$P(X,U,2)_U_$$DATE^BGP6UTL($P(X,U,1))
 .S X=$$CPT^BGP6DU(P,BDATE,EDATE,T,5)
 Q ""
FOB(P,BDATE,EDATE) ;EP
 ...
 ...Q:'$D(^AUPNVLAB(X,0))
 ...I BGPLT,$P(^AUPNVLAB(X,0),U),$D(^ATXLAB(BGPLT,21,"B",$P(^AUPNVLAB(X,0),U))) S BGPC="FOB V LAB"_U_$$DATE^BGP6UTL((9999999-D)) Q
 ...Q:'T
 ...S J=$P($G(^AUPNVLAB(X,11)),U,13) Q:J=""
 ...Q:'$$LOINC(J,T)
 ...S BGPC="FOB LOINC"_U_$$DATE^BGP6UTL((9999999-D)) Q
 ...Q
 I BGPC]"" Q BGPC
 S E=+$$CODEN^ICPTCOD(82270) I E S %=$$CPTI^BGP6DU(P,BDATE,EDATE,E) I $P(%,U)=1 Q "FOB CPT 82270"_"^"_$$DATE^BGP6UTL($P(%,U,2))
 S E=+$$CODEN^ICPTCOD(82274) I E S %=$$CPTI^BGP6DU(P,BDATE,EDATE,E) I $P(%,U)=1 Q "FOB CPT 82274"_"^"_$$DATE^BGP6UTL($P(%,U,2))
 S E=+$$CODEN^ICPTCOD(89205) I E S %=$$CPTI^BGP6DU(P,BDATE,EDATE,E) I $P(%,U)=1 Q "FOB CPT 89205"_"^"_$$DATE^BGP6UTL($P(%,U,2))
 S E=+$$CODEN^ICPTCOD("G0107") I E S %=$$CPTI^BGP6DU(P,BDATE,EDATE,E) I $P(%,U)=1 Q "FOB CPT G0107"_"^"_$$DATE^BGP6UTL($P(%,U,2))
 Q ""
BE(P,EDATE) ;EP
 S BDATE=$$FMADD^XLFDT(EDATE,5*(-365))
 S T=$O(^ATXAX("B","BGP BE CPTS",0))
 I T D  I X]"" Q "CPT BE "_$P(X,U,2)_U_$$DATE^BGP6UTL($P(X,U,1))
 .S X=$$CPT^BGP6DU(P,BDATE,EDATE,T,5)
 .Q
 S T=$O(^ATXAX("B","BGP BE CPTS",0))
 I T D  I X]"" Q "RAD BE"_U_$$DATE^BGP6UTL(X)
 .S X=$$RAD^BGP6DU(P,BDATE,EDATE,T,3)
 .Q
 Q ""
 ;
LOINC(A,B) ;
...
REF(P,BDATE,EDATE) ;EP
 ...
 I BGPT S G=$$RADREF^BGP6UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q G
