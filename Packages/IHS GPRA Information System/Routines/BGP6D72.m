...
IHEDCHM ;EP
 ...
 .I +V>130 S BGPN4=1
 S V=$S(BGPRTYPE=3:"",BGPD2:"UP")_$S(BGPD1:";AC",1:"")_";"_$P(BGPAMI,U,2)_"|||"
 I $P(BGPLDL,U) S V=V_"LDL "_$$DATE^BGP6UTL($P(BGPLDL,U,2))_" "_$P(BGPLDL,U,3)
 S BGPVALUE=V
 K V,BGPAMI,BGPLDL,D
 K ^TMP($J)
 Q
...
AMIO(P,BDATE,EDATE) ;
 ...
 S BGPG=$$LASTPRC^BGP6UTL1(P,"BGP CABG PROCS",BDATE,EDATE)
 I $P(BGPG,U) Q 1_U_"CABG PROC"
 ;now check cpts
 S BGPG=$$CPT^BGP6DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP CABG CPTS",0)),6)
 I $P(BGPG,U) Q 1_U_"CABG CPT"
 ;now check for PTCA
 ;check for procedure in BGP PTCA PROCS
 S BGPG=$$LASTPRC^BGP6UTL1(P,"BGP PTCA PROCS",BDATE,EDATE)
 I $P(BGPG,U) Q 1_U_"PTCA PROC"
 ;now check cpts
 S BGPG=$$CPT^BGP6DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PTCA CPTS",0)),6)
 I $P(BGPG,U) Q 1_U_"PTCA CPT"
 ;now check IVD dxs
 S BGPG=$$LASTDX^BGP6UTL1(P,"BGP IVD DXS",BDATE,EDATE)
...
BETACONT(P,EDATE) ;
 ...
 S BGPG=$$LASTDX^BGP6UTL1(P,"BGP HYPOTENSION DXS",$$DOB^AUPNPAT(P),EDATE)
 I $P(BGPG,U)=1 Q 1  ;has hypotension dx
 S BGPG=$$LASTDX^BGP6UTL1(P,"BGP CMS 2/3 HEART BLOCK DXS",$$DOB^AUPNPAT(P),EDATE)
