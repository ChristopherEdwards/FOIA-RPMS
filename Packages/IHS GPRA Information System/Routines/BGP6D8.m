...
IE2 ;EP
 ...
 I '$$PREG^BGP6D7(DFN,$$FMADD^XLFDT(BGPEDATE,-600),BGPEDATE) S BGPSTOP=1 Q  ;not pregnant
...
HIVTEST(P,BDATE,EDATE) ;
 ...
 I T D  I X]"" Q 1_U_"CPT: "_$$DATE^BGP6UTL($P(X,U,1))_" "_$P(X,U,2)
 ...
 ...Q:'$D(^AUPNVLAB(X,0))
 ...I BGPLT,$P(^AUPNVLAB(X,0),U),$D(^ATXLAB(BGPLT,21,"B",$P(^AUPNVLAB(X,0),U))) S BGPC=1_U_$$DATE^BGP6UTL((9999999-D))_U_"LAB" Q
 ...Q:'T
 ...S J=$P($G(^AUPNVLAB(X,11)),U,13) Q:J=""
 ...Q:'$$LOINC^BGP6D21(J,T)
 ...S BGPC=1_U_$$DATE^BGP6UTL((9999999-D))_U_$P($G(^LAB(95.3,J,0)),U)_"-"_$P($G(^LAB(95.3,J,0)),U,15)
 ...Q
 I $P(BGPC,U)=1 Q BGPC
 ...
 .S G=$$REFUSAL^BGP6UTL1(P,60,BGPT1,$$FMADD^XLFDT(EDATE,-600),EDATE) I $P(G,U)=1 S G=2_U_$$DATE^BGP6UTL($P(G,U,2))_" lab refusal"
 Q G
HIVEDUC(P,BDATE,EDATE) ;
 ...
 .I $P(T,"-",1)="HIV"!($P(T,"-",1)["042")!($P(T,"-",1)["043")!($P(T,"-",1)["044")!($P(T,"-",1)="V08.")!($P(T,"-",1)["795.71")!($P(T,"-",2)="HIV") S G=1_U_$$DATE^BGP6UTL($P(BGPG(X),U))_U_T
 I $P(G,U)=1 Q G
 ;
 K BGPG
 S Y="BGPG("
 S X=P_"^LAST DX V65.44;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE) S E=$$START1^APCLDF(X,Y)
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP6UTL($P(BGPG(1),U))_"V65.44"  ;has a dx
 Q ""
 ;
CD4(P,BDATE,EDATE) ;
 K BGPG
 ...
 I $D(BGPG(1)) Q 1
 S E=+$$CODEN^ICPTCOD(86361),%=$$CPTI^BGP6DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^86361"
 S E=+$$CODEN^ICPTCOD(86360),%=$$CPTI^BGP6DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^86360"
 S E=+$$CODEN^ICPTCOD(86359),%=$$CPTI^BGP6DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^86359"
 ...
 .I $$LOINC^BGP6D21(J,T) S G=1
 Q G
PCR(P,BDATE,EDATE) ;
 K BGPG
 S %=P_"^LAST LAB [BGP HIV VIRAL LOAD TAX;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 1
 S E=+$$CODEN^ICPTCOD(87536),%=$$CPTI^BGP6DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^87536"
 S E=+$$CODEN^ICPTCOD(87539),%=$$CPTI^BGP6DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^87539"
 ;now go through all labs and check loinc codes
 K ^TMP($J,"A")
 S A="^TMP($J,""A"",",%=P_"^ALL LAB;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,A)
 I '$D(^TMP($J,"A",1)) Q ""
