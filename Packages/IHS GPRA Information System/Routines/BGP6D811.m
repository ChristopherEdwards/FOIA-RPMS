...
PCR(P,BDATE,EDATE) ;EP
 ...
 S E=+$$CODEN^ICPTCOD(87536),%=$$CPTI^BGP6DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^87536"
 S E=+$$CODEN^ICPTCOD(87539),%=$$CPTI^BGP6DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^87539"
 S E=+$$CODEN^ICPTCOD("G9242"),%=$$CPTI^BGP6DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^G9242"
 S E=+$$CODEN^ICPTCOD("G9243"),%=$$CPTI^BGP6DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^G9243"
 S E=+$$CODEN^ICPTCOD(87536),%=$$TRANI^BGP6DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^87536 TRAN"
 S E=+$$CODEN^ICPTCOD(87539),%=$$TRANI^BGP6DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^87539 TRAN"
 S E=+$$CODEN^ICPTCOD("G9242"),%=$$TRANI^BGP6DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^G9242 TRAN"
 S E=+$$CODEN^ICPTCOD("G9243"),%=$$TRANI^BGP6DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^G9243 TRAN"
...
ART(P,BDATE,EDATE) ;EP -antiviral med?
 ...
 K BGPMEDS1,^TMP($J,"A")
 D GETMEDS^BGP6UTL2(P,BDATE,EDATE,"BGP PQA ANTIRETROVIRAL MEDS","BGP PQA ANTIRETROVIRAL NDC",,,.BGPMEDS1)
 S C=""
 ...
 .S V1D=$$VD^APCLV(V)
 .S T=T+1
 .S C=1_U_""_$$DATE^BGP6UTL(V1D)_" "_$$VAL^XBDIQ1(9000010.14,M,.01)
 Q C
MMR(P,EDATE) ;EP
...
PVS ;
 ...
 K BGPG ; D SETPRC^BGP6UTL1(P,$$DOB^AUPNPAT(P),EDATE,"BGP MMR IZ PROCS",.BGPG)
 ;S X=0 F  S X=$O(BGPG(X)) Q:X'=+X  S BGPMMR($P(BGPG(X),U))=""
 S X="",Y="",C=0 F  S X=$O(BGPMMR(X)) Q:X'=+X  S C=C+1 D
 .I C=1 S Y=X Q
 .I $$FMDIFF^XLFDT(X,Y)<11 K BGPMMR(X) Q
...
MEPV ;
 ...
 K BGPG ;D SETPRC^BGP6UTL1(P,$$DOB^AUPNPAT(P),EDATE,"BGP MEASLES IZ PROCS",.BGPG)
 ;S X=0 F  S X=$O(BGPG(X)) Q:X'=+X  S BGPME($P(BGPG(X),U))=""
 S X="",Y="",C=0 F  S X=$O(BGPME(X)) Q:X'=+X  S C=C+1 D
 .I C=1 S Y=X Q
 .I $$FMDIFF^XLFDT(X,Y)<11 K BGPME(X) Q
...
MUPV ;
 ...
 K BGPG ;D SETPRC^BGP6UTL1(P,$$DOB^AUPNPAT(P),EDATE,"BGP MUMPS IZ PROCS",.BGPG)
 ;S X=0 F  S X=$O(BGPG(X)) Q:X'=+X  S BGPMU($P(BGPG(X),U))=""
 S X="",Y="",C=0 F  S X=$O(BGPMU(X)) Q:X'=+X  S C=C+1 D
 .I C=1 S Y=X Q
 .I $$FMDIFF^XLFDT(X,Y)<11 K BGPMU(X) Q
...
RUBPV ;
 ...
 K BGPG ;D SETPRC^BGP6UTL1(P,$$DOB^AUPNPAT(P),EDATE,"BGP RUBELLA IZ PROCS",.BGPG)
 ;S X=0 F  S X=$O(BGPG(X)) Q:X'=+X  S BGPRUB($P(BGPG(X),U))=""
 S X="",Y="",C=0 F  S X=$O(BGPRUB(X)) Q:X'=+X  S C=C+1 D
 .I C=1 S Y=X Q
 .I $$FMDIFF^XLFDT(X,Y)<11 K BGPRUB(X) Q
...
MMRC ;
 ;
 F BGPZ=3,94 S X=$$MMRCONT^BGP6D31(P,BGPZ,EDATE) Q:X]""
...
MEX ;
 ...
 I $$PLTAX^BGP6DU(P,"BGP MEASLES EVIDENCE") S BGPME=4,BGPMEEV=1
 I BGPME=0 D
 .S B=$$DOB^AUPNPAT(P),E=EDATE
 .F BGPIMM=5 D
 ..S I=$O(^AUTTIMM("C",BGPIMM,0)) Q:'I
...
MUX ;
 ...
 I $$PLTAX^BGP6DU(P,"BGP MUMPS EVIDENCE") S BGPMU=4,BGPMUEV=1
 I BGPMU=0 D
 .S B=$$DOB^AUPNPAT(P),E=EDATE
 .F BGPIMM=7 D
 ..S I=$O(^AUTTIMM("C",BGPIMM,0)) Q:'I
...
RUBX ;
 ...
 I $$PLTAX^BGP6DU(P,"BGP RUBELLA EVIDENCE") S BGPRUB=4,BGPRUEV=1
 I BGPRUB=0 D
 .S B=$$DOB^AUPNPAT(P),E=EDATE
 .F BGPIMM=6 D
 ..S I=$O(^AUTTIMM("C",BGPIMM,0)) Q:'I
 ...
 ;F BGPIMM=6 I $$IMMREF^BGP6D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE) S BGPRUB=3
