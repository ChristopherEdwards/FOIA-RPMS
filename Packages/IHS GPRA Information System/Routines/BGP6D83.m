...
IHEDCWP ;EP
 ...
 S BGPN1=+BGPN
 S BGPVALUE="UP"_$S(BGPD1:";AC",1:"")_"|||"_$$DATE^BGP6UTL($P(BGPN,U,2))_" "_$P(BGPN,U,3)
...
ACTA(P,EDATE) ;
 K ^TMP($J,"A")
 NEW A,B,E,Z,X,D,V,Y,G,M,T,T1
 S A="^TMP($J,""A"",",B=P_"^ALL MEDS;DURING "_$$FMADD^XLFDT(EDATE,-365)_"-"_EDATE,E=$$START1^APCLDF(B,A)
...
STREP(P,BDATE,EDATE) ;EP
 K BGPC
 S BGPC=0
 S %=$$CPT^BGP6DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP GROUP A STREP CPT",0)),5)
 I %]"" Q 1_U_%
