BGP6D83 ; IHS/CMI/LAB - measure C ; 07 Jan 2016  11:45 AM
...
IHEDCWP ;EP
 ...
 S BGPN1=+BGPN
 S BGPVALUE=$S(BGPRTYPE=3:"",1:"UP")_$S(BGPD1:",AC",1:"")_"|||"_$$DATE^BGP6UTL($P(BGPN,U,2))_" "_$P(BGPN,U,3)
...
NEWP ;check V PROCEDURE
 S D=$$LASTPRC^BGP6UTL1(P,"BGP INJECTION ANTIBIOTIC PROCS",BDATE,EDATE)
 Q $P(D,U)
 ;
CANTI(P,BDATE,EDATE) ;
 K ^TMP($J,"A")
...
ACTA(P,EDATE) ;
 ...
 NEW A,B,E,Z,X,D,V,Y,G,M,T,T1
 K BGPMEDS1
 D GETMEDS^BGP6UTL2(P,$$FMADD^XLFDT(EDATE,-365),EDATE,,,,,.BGPMEDS1)
 I '$D(BGPMEDS1) Q 0
 S T1=$O(^ATXAX("B","BGP HEDIS ANTIBIOTICS MEDS",0))
 S T4=$O(^ATXAX("B","BGP HEDIS ANTIBIOTICS NDC",0))
 S (X,G,M,E)=0,D="" F  S X=$O(BGPMEDS1(X)) Q:X'=+X!(D)  S V=$P(BGPMEDS1(X),U,5),Y=+$P(BGPMEDS1(X),U,4) D
...
HOSPPHAR(P,D) ;is there a hosp with pharyngitis on date D or 1 day later
 ...
 ...Q:T=""
 ...S T=$P($$ICDDX^BGP6UTL2(T),U,2)
 ...Q:T=""
 ...Q:'$$ICD^BGP6UTL2(T,$O(^ATXAX("B","BGP PHARYNGITIS DXS",0)),9)
 ...S S=1
 ..Q:'S
 ..S Q=1
 .Q
...
STREP(P,BDATE,EDATE) ;EP
 ...
 S %=$$TRAN^BGP6DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP GROUP A STREP CPT",0)),5)
 I %]"" S $P(%,U,2)="CPT "_$P(%,U,2) Q 1_U_%
