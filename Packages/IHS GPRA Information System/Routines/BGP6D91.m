BGP6D91 ; IHS/CMI/LAB - calc measures 29 Apr 2010 7:38 PM 14 Nov 2006 5:02 PM 12 Nov 2009 11:03 AM 07 Apr 2010 7:00 AM ;
...
EOST ;
 ...
 I BGPTHER]"" S BGPN1=1
 ;
 S BGPVALUE="UP"_"|||"_"DX: "_$$DATE^BGP6UTL(BGPTIA)_" THERAPY: "_$S(BGPTHER]"":BGPTHER,1:"None")
 Q
 ;
TIAFIB(P,BDATE,EDATE) ;EP
 ...
 ..Q:C=""
 ..I $$ICD^BGP6UTL2(C,$O(^ATXAX("B","BGP ATRIAL FIBRILLATION DXS",0)),9) S G=$$VD^APCLV(V)
 I 'G Q "" ;;NO DX
 Q G
ANTICOAG(P,BDATE,EDATE) ;EP - was there ANTICOAG
 ...
 .;I $P($P(^AUPNVSIT($P(^AUPNVMED(N,0),U,3),0),U),".")=EDATE S G=$$DATE^BGP6UTL(EDATE)_" MET: "_$P(C,U,2)_"^1"  ;PRESCRIBED ON DISCHARGE DATE
 .;S V=$P(^AUPNVMED(N,0),U,3)
 .;S V=$P($P(^AUPNVSIT(V,0),U),".")
 .;I $$FMADD^XLFDT(V,S)<EDATE Q  ;not valid through discharge date
 .S G=$$DATE^BGP6UTL($P(BGPG(X),U,1))_" "_$P(C,U,2)
 I G]"" Q G
 Q ""
 ;
ANTIDRUG(N) ;
 ...
 I $$VAPI^BGP6D81(I,$O(^ATXAX("B","BGP CMS WARFARIN VAPI",0))) Q "1^WARF"
 Q ""
 ;
EOOX ;
 NEW BGPOXV,BGPD,BGPN
