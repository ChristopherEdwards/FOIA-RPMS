BGP6DCLD ; IHS/CMI/LAB - IHS gpra print ;
...
GO ;
 ...
 D H1
 S BGPH1P=0 S BGPCOM="",BGPCOUNT=0 F  S BGPCOM=$O(^XTMP("BGP06CPL",BGPJ,BGPH,"LIST",BGPCOM)) Q:BGPCOM=""  D CPL1
 S X="Total # of patients on list: "_$G(BGPPCNT) D S(X,1,1)
 Q
CPL1 ;EP
 S BGPSEX="" F  S BGPSEX=$O(^XTMP("BGP06CPL",BGPJ,BGPH,"LIST",BGPCOM,BGPSEX)) Q:BGPSEX=""  D CPL2
 Q
CPL2 ;
 S BGPAGE="" F  S BGPAGE=$O(^XTMP("BGP06CPL",BGPJ,BGPH,"LIST",BGPCOM,BGPSEX,BGPAGE)) Q:BGPAGE=""  D
 .S DFN=0 F  S DFN=$O(^XTMP("BGP06CPL",BGPJ,BGPH,"LIST",BGPCOM,BGPSEX,BGPAGE,DFN)) Q:DFN'=+DFN  S BGPCOUNT=BGPCOUNT+1 D PRINTL
 Q
...
HEADER ;EP
 ...
 S X="CRS 2006, Version 6.1" D S(X,1,1)
...
H1 ;
 ...
 S X="PATIENT NAME" D S(X,1,1) S X="HRN" D S(X,,2) S X="COMMUNITY" D S(X,,3) S X="SEX" D S(X,,4) S X="AGE" D S(X,,5) S X="DENOMINATOR" D S(X,,6) S X="MEASURE NOT MET" D S(X,,7)
 S X=$TR($J("",80)," ","-") D S(X,1,1)
 Q
