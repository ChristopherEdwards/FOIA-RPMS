...
GO ;
 ...
 .D W^BGP6DP(^BGPCTRL(BGPY,28,BGPX,0),0,1,BGPPTYPE)
 S BGPXL=0
 D HEADER
...
PRINTL ;print one line
 ...
 S BGPPCNT=BGPPCNT+1
 D W^BGP6DP($E($P(^DPT(DFN,0),U),1,22),0,1,BGPPTYPE,1)
 D W^BGP6DP($$HRN^AUPNPAT(DFN,DUZ(2)),0,0,BGPPTYPE,2,24)
 D W^BGP6DP($E(BGPCOM,1,14),0,0,BGPPTYPE,3,31)
 D W^BGP6DP(BGPSEX,0,0,BGPPTYPE,4,46)
 D W^BGP6DP(BGPAGE,0,0,BGPPTYPE,5,49)
 ...
 D W^BGP6DP(W,0,0,BGPPTYPE,6,53)
 D W^BGP6DP(Z_$S(BGPPTYPE="P":"/",1:""),0,0,BGPPTYPE,7,53)
 D W^BGP6DP($$LAST(DFN,BGPED),0,0,BGPPTYPE,8,65)
 Q
 ;
LAST(P,EDATE) ;EP
 I '$D(^AUPNVSIT("AC",P)) Q ""
 ...
 .S G=V
 .Q
 I G Q $$PRIMPROV^APCLV(G,"N")_","_$S($$PRIMPROV^APCLV(G,"F"):$P(^DIC(7,+$$PRIMPROV^APCLV(G,"F"),0),U,2),1:"")_","_$$DATE^BGP6UTL($P($P(^AUPNVSIT(G,0),U),"."))
 Q ""
HEADER ;EP
...
HEADER1 ;
 ...
 I BGPPTYPE="P",$G(BGPGUI) D W^BGP6DP("ZZZZZZZ",0,1,BGPPTYPE) ;!  ;maw
 D W^BGP6DP("***** CONFIDENTIAL PATIENT INFORMATION, COVERED BY THE PRIVACY ACT *****",1,$S(BGPPTYPE="D":3,1:1),BGPPTYPE)
 I BGPPTYPE="P" S X=$P(^VA(200,DUZ,0),U,2),$E(X,35)=$$FMTE^XLFDT(DT),$E(X,70)="Page "_BGPGPG D W^BGP6DP(X,1,1,BGPPTYPE)
 D W^BGP6DP("***  IHS 2016 Comprehensive National GPRA/GPRAMA Patient List  ***",1,1,BGPPTYPE)
 D W^BGP6DP("*** List of Patients Not Meeting a National GPRA or PART measure  ***",1,1,BGPPTYPE)
 ...
 D W^BGP6DP($S(BGPLIST="A":"All Patients",BGPLIST="R":"Random Patient List",1:"Patient List by Provider: "_BGPLPROV),1,1,BGPPTYPE)
...
H1 ;
 ...
 D W^BGP6DP("PREG=Pregnant Female; IMM=Active IMM Pkg Pt",0,1,BGPPTYPE)
 D W^BGP6DP("",0,1,BGPPTYPE)
 D W^BGP6DP("PATIENT NAME",0,1,BGPPTYPE)
 D W^BGP6DP("HRN",0,0,BGPPTYPE,2,24)
 D W^BGP6DP("COMMUNITY",0,0,BGPPTYPE,3,31)
 D W^BGP6DP("SEX",0,0,BGPPTYPE,4,45)
 D W^BGP6DP("AGE",0,0,BGPPTYPE,5,49)
 D W^BGP6DP("DENOMINATOR",0,0,BGPPTYPE,6,53)
 D W^BGP6DP("NOT MET/LST PRVDR",0,0,BGPPTYPE,7,65)
 D W^BGP6DP($TR($J("",80)," ","-"),0,1,BGPPTYPE)
 Q
