BGP6DH ; IHS/CMI/LAB - cover page for gpra 28 Apr 2006 11:30 AM ; 
 ...
 I BGPRTYPE=1,$G(BGP6GPU) W !,"Population: ",$S(BGPBEN=1:"AI/AN Only (Classification 01)",BGPBEN=2:"non AI/AN Only (Classification NOT 01)",BGPBEN=3:"All (Both AI/AN and non AI/AN)",1:"")
 I BGPRTYPE=4,BGP6RPTH'="P" W !,"Population: ",$S(BGPBEN=1:"AI/AN Only (Classification 01)",BGPBEN=2:"non AI/AN Only (Classification NOT 01)",BGPBEN=3:"All (Both AI/AN and non AI/AN)",1:"")
 I BGPRTYPE=4,BGP6RPTH="P" W !,"Population:  ",$P(^DIBT(BGPSEAT,0),U)
 I BGPRTYPE=1,'$G(BGP6GPU) W !!,"Population:  AI/AN Only (Classification 01)"
 D ENDTIME
 I BGPRTYPE=4,BGP6RPTH="C" D COMHDR
 I BGPRTYPE=4,BGP6RPTH="P" D PPHDR
 I BGPRTYPE=4,BGP6RPTH="A" D ALLHDR
 I BGPRTYPE=1,'$G(BGP6GPU) D GPRAHDR
 I BGPRTYPE=1,$G(BGP6GPU) D COMHDR
 Q:BGPQHDR
 I $G(BGPEXPT) W !!,"A file will be created called BG06",$P(^AUTTLOC(DUZ(2),0),U,10)_"."_BGPRPT,".",!,"It will reside in the public/export directory.",!,"This file should be sent to your Area Office.",!
...
HDR1 ;
 ...
 ;I $G(BGPCPPL) W !!,$$CTR("*** IHS 2006 National GPRA Clinical Performance Report ***") G N
 I BGPRTYPE=1,'$G(BGP6GPU) W !!,$$CTR("*** IHS 2006 National GPRA Clinical Performance Report ***",80)
...
AREACP ;EP - area cover page
 ...
 ;I BGPRTYPE=1,'$G(BGP6GPU) W !!,"Population:  AI/AN Only (Classification 01)"
 D ENDTIME
 S BGPCHSO="",X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  I $P(^BGPGPDCS(X,0),U,17) S BGPCHSO=1
 S BGPCHSN="",X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  I '$P(^BGPGPDCS(X,0),U,17) S BGPCHSN=1
 I BGPRTYPE=1,'$G(BGP6GPU) D GPRAHDRA
 I BGPRTYPE=1,$G(BGP6GPU) D COMHDRA
 I $G(BGPEXCEL),'$G(BGP6GPU) W !!,"National GPRA filename:  ",BGPFNEIS
 I '$G(BGP6GPU) W !,"National Reporting filenames: ",!,"                         ",BGPFN,"                          ",BGPFN2
 I BGPROT'="P",'$D(BGPGUI) W !!,"A delimited output file called ",BGPDELF,!,"has been placed in the public directory for your use in Excel or some",!,"other software package.  See your site manager to access this file.",!
 ...
 .S BGPC=BGPC+1
 .S X=$P(^BGPGPDCS(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .W !?3,BGPC,".  ",$S($P(^BGPGPDCS(BGPX,0),U,17):"*",1:""),X
 ...
 .S X=$P(^BGPGPDCS(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .W !!?3,BGPC,".  ",$S($P(^BGPGPDCS(BGPX,0),U,17):"*",1:""),X
 .W !?5,"Communities: " S BGPXX=0,BGPXN=0,BGPXY="" F  S BGPXX=$O(^BGPGPDCS(BGPX,9999,BGPXX)) Q:BGPXX'=+BGPXX  S BGPXN=BGPXN+1,BGPXY=BGPXY_$S(BGPXN=1:"",1:";")_$P(^BGPGPDCS(BGPX,9999,BGPXX,0),U)
 .S BGPX1=0,C=0 F BGPX1=1:3:BGPXN D
 ..I $Y>(BGPIOSL-2) D AHDR Q:BGPQHDR
 ..W !?10,$E($P(BGPXY,";",BGPX1),1,20),?30,$E($P(BGPXY,";",(BGPX1+1)),1,20),?60,$E($P(BGPXY,";",(BGPX1+2)),1,20)
 .Q:BGPQHDR
 .I $O(^BGPGPDCS(BGPX,1111,0)) D
 ..I $Y>(BGPIOSL-2) D AHDR Q:BGPQHDR
 ..W !!?5,"MFI Visit Locations: " S BGPXX=0,BGPXN=0,BGPXY="" F  S BGPXX=$O(^BGPGPDCS(BGPX,1111,BGPXX)) Q:BGPXX'=+BGPXX  S BGPXN=BGPXN+1,BGPXY=BGPXY_$S(BGPXN=1:"",1:";")_$P(^BGPGPDCS(BGPX,1111,BGPXX,0),U)
