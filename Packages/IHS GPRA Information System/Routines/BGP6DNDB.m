BGP6DNDB ; IHS/CMI/LAB - NATL COMP EXPORT 13 Nov 2006 12:31 PM 17 Apr 2016 11:15 AM ; 
...
PNST ;
 ...
 I $D(DIRUT) W !!,"No providers selected.  Exiting report..." D PAUSE^BGP6DU,XIT Q
 S BGPXX=Y
 I BGPXX="T" D  I '$O(BGPDESGP(0)) W !!,"No providers selected.  Exiting report..." D PAUSE^BGP6DU,XIT Q
 ...
 .Q
 I BGPXX="P" D  I '$O(BGPDESGP(0)) W !!,"No providers selected.  Exiting report..." D PAUSE^BGP6DU,XIT Q
...
AI ;gather all gpra measures
 ;F X=1:100 I $D(^BGPINDM("GPRA",1,X)) S BGPIND(X)=""
...
ZIS ;call to XBDBQUE
 ...
 I BGPRPF="F" D REPORT^BGP6UTL
 I $G(BGPQUIT) D XIT Q
 I BGPRPT="" D XIT Q
 K IOP,%ZIS I BGPROT="D",BGPDELT="F" D NODEV,XIT Q
 K IOP,%ZIS W !! S %ZIS=$S(BGPDELT'="S":"PQM",1:"PM") D ^%ZIS
 I POP,BGPRPF="F" D  D XIT Q
 .W !,"Report Aborted." S DA=BGPRPT,DIK="^BGPGPDCM(" D ^DIK K DIK
 .S DA=BGPRPT,DIK="^BGPGPDPM(" D ^DIK K DIK
 .S DA=BGPRPT,DIK="^BGPGPDBM(" D ^DIK K DIK
 I POP,BGPRPF="P" D  D XIT Q
 .W !,"Report Aborted."
 .S BGPXX=0 F  S BGPXX=$O(BGPDESGP(BGPXX)) Q:BGPXX'=+BGPXX  S BGPRPT=BGPDESGP(BGPXX) D
 ..S DA=BGPRPT,DIK="^BGPGPDCM(" D ^DIK K DIK
 ..S DA=BGPRPT,DIK="^BGPGPDPM(" D ^DIK K DIK
 ..S DA=BGPRPT,DIK="^BGPGPDBM(" D ^DIK K DIK
 I $D(IO("Q")) G TSKMN
DRIVER ;
 ...
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP6DNDB",XBRX="XIT^BGP6DNDB",XBNS="BGP"
 D ^XBDBQUE
 Q
 ;
NODEV1 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP6DNDB",ZTDTH="",ZTDESC="NATIONAL GPRA REPORT 11" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
F ;fiscal year
 S (BGPPER,BGPVDT)=""
 W !!,"Enter the year for the report.  Use a 4 digit ",!,"year, e.g. 2016"
 S DIR(0)="D^::EP"
 S DIR("A")="Enter year"
...
FY ;calendar year
 ...
 I $D(DIRUT) Q
 I $D(DUOUT) S DIRUT=1 Q
 I Y'=3160000,Y'=3170000 W !,"Must be 2016 or 2017" G FY
 S BGPVDT=Y
