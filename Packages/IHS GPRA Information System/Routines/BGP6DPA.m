BGP6DPA ; IHS/CMI/LAB - COMP NATIONAL GPRA FOR PTS W/APPT 03 Jun 2016 2:54 PM 06 May 2016 1:23 PM ; 
...
EP ;EP - called from option interactive
 ...
 I 'X W !!,"Exiting Report....." D PAUSE^BGP6DU,EOJ Q
RTYPE ;
 K BGPQUIT
 S BGPRT1=""
...
NT S BGPLIST="A"
 S BGPCPLC=0
 ;S XBRP="PRINT^BGP6DPAW",XBRC="PROC^BGP6DPA",XBRX="EOJ^BGP6DPA",XBNS="BGP"
 ;D ^XBDBQUE
 ;
 K IOP,%ZIS
 W !! S %ZIS="PQM" D ^%ZIS
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP6DPA",ZTDTH="",ZTDESC="CRS 16 SCHED REPORT" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D ^XBFMK
 K DIC,DIR,DFN
...
GUIEP ;EP
 ...
 K ^TMP($J,"BGP6DPA")
 S IOM=80
 D GUIR^XBLM("PRINT^BGP6DPA","^TMP($J,""BGP6DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP6DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP6DPA",X)
...
GUIECP ;EP
 K ^TMP($J,"BGP6DPA")
 S IOM=80
 D GUIR^XBLM("CPPRINT^BGP6DPA","^TMP($J,""BGP6DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP6DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP6DPA",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIM(BGPIEN,11,X,0)=BGPDATA,C=C+1
...
ENDLOG ;-- write the end of the log
 D NOW^%DTC
 S BGPNOW=$G(%)
 S DIE="^BGPGUIM(",DA=BGPIEN,DR=".04////"_BGPNOW_";.06////C"
 D ^DIE
 K DIE,DR,DA
 Q
 ;
PROC ;EP
 ...
 K ^XTMP("BGP6DPA",BGPGPRAJ,BGPGPRAH)
 S ^XTMP("BGP6DPA",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^"_"PTS WITH GPRA DATA"
 K BGPAPPT,BGPAPPTS
 S BGPTA=0
 I BGPRT1="A" D  Q
 ...
 ..S BGPIISO=1,BGPISST="A" D PROCCY^BGP6D1
 .Q
 ...
 .S X=$O(^DIBT("B","RPMS DEMO PATIENT NAMES",0)) I X Q:$D(^DIBT(X,1,DFN))
 .I $G(BGPSTMP) S ^DIBT(BGPSTMP,1,DFN)="" Q
 .S BGPIISO=1,BGPISST=BGPRT1,BGPISSO=1 D PROCCY^BGP6D1
 .Q
 K ^XTMP("BGP15TAX",$J)
 Q
...
ZISCP ;
 ...
 ;
 I $D(IO("Q")) G TSKMNCP
 ;S XBRP="CPPRINT^BGP6DPA",XBRC="",XBRX="EOJ^BGP6DPA",XBNS="BGP"
 ;D ^XBDBQUE
 ;
CPPRINT ;EP - called from xbdbque
 ...
 D CPHEADER
 S BGPGYR=2016,BGPGYR=$O(^BGPCTRL("B",BGPGYR,0))
...
TSKMNCP ;EP ENTRY POINT FROM TASKMAN
 ...
 I $D(IOM)#2,IOM S ZTIO=ZTIO_";"_IOM I $D(IOSL)#2,IOSL S ZTIO=ZTIO_";"_IOSL
 K ZTSAVE S ZTSAVE("BGP*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="CPPRINT^BGP6DPA",ZTDTH="",ZTDESC="CRS 15 SCHED REPORT DENOM" D ^%ZTLOAD D EOJ Q
 Q
STMP ;EP
EN1 ;EP Help
 K BGPQUIT S BGPSTMP=""
...
F ;calendar year
 ...
 I Y'=3160000,Y'=3170000 W !,"Must be 2016 or 2017" G F
 S BGPVDT=Y
 I $E(Y,4,7)'="0000" W !!,"Please enter a year only!",! G F
 S BGPPER=BGPVDT
 I BGPPER="3170000" S BGPNGR09=1
 Q
