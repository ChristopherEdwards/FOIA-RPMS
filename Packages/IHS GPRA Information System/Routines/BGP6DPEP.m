BGP6DPEP ; IHS/CMI/LAB - IHS gpra print 01 Jul 2010 7:54 PM ; 27 Jan 2016  12:32 PM
...
DEL ;create delimited output file
 ...
 S BGPQHDR=0
 D DEL^BGP6DPED
 K ^XTMP("BGP6PE",BGPJ,BGPH)
 K ^TMP($J)
 Q
WP ;
 K ^UTILITY($J,"W")
 S BGPZ=0,BGPLCNT=0
 S DIWL=1,DIWR=80,DIWF="",BGPZ=0 F  S BGPZ=$O(^BGPPEIM(BGPIC,BGPNODE,BGPY,11,BGPZ)) Q:BGPZ'=+BGPZ  D
 .S BGPLCNT=BGPLCNT+1
 .S X=^BGPPEIM(BGPIC,BGPNODE,BGPY,11,BGPZ,0) S:BGPLCNT=1 X=" - "_X D ^DIWP
 .Q
WPS ;
...
PRINT1 ;EP
 ...
 .W !!,$P(^BGPPEIM(BGPIC,0),U,2)
 .I $G(BGPDNT) G CALC
 .I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 .W !!,"Denominator(s):"
 .S BGPNODE=61
 .S BGPX=0 F  S BGPX=$O(^BGPPEIM(BGPIC,61,"B",BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIM(BGPIC,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY!(BGPQUIT)  D
 ...D WP
 .I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 .W !!,"Numerator(s):"
 .S BGPNODE=62
 .S BGPX=0 F  S BGPX=$O(^BGPPEIM(BGPIC,62,"B",BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIM(BGPIC,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY!(BGPQUIT)  D
 ...D WP
 .S BGPNODE=11
 .W ! S BGPX=0 F  S BGPX=$O(^BGPPEIM(BGPIC,BGPNODE,BGPX)) Q:BGPX'=+BGPX  D
 ..I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ..W !,^BGPPEIM(BGPIC,BGPNODE,BGPX,0)
CALC .D @BGPIC
 .Q:BGPQUIT
 ;
 D ^BGP6DPEL
 ...
 Q
1 ;
 I $Y>(BGPIOSL-6) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIM(BGPIC,0),U,2)
 D H1^BGP6DPH
...
2 ;
 ...
 ..I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIM(BGPIC,0),U,2) D H1^BGP6DPH W !
 ...
 I $Y>(BGPIOSL-6) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIM(BGPIC,0),U,2) D H1^BGP6DPH W !
 ...
 ..I $P($G(^BGPPEDCM(X,11)),U,4)]"",$P($G(^BGPPEDCM(X,11)),U,4)<BGPCYD S BGPCYD=$P($G(^BGPPEDCM(X,11)),U,4)
 ..I $P($G(^BGPPEDPM(X,11)),U,4)]"",$P($G(^BGPPEDPM(X,11)),U,4)<BGPPRD S BGPPRD=$P($G(^BGPPEDPM(X,11)),U,4)
 ..I $P($G(^BGPPEDBM(X,11)),U,4)]"",$P($G(^BGPPEDBM(X,11)),U,4)<BGPBLD S BGPBLD=$P($G(^BGPPEDBM(X,11)),U,4)
 ...
 I $Y>(BGPIOSL-4) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIM(BGPIC,0),U,2) D H1^BGP6DPH W !
 W !,"Minimum Time Spent",!,"All Providers",!,"(minutes)"
 W ?20,$$C(BGPCYD,0,8),?35,$$C(BGPPRD,0,8),?58,$$C(BGPBLD,0,8),!
 I $G(BGPAREAA) D
 .S (BGPCYD,BGPPRD,BGPBLD)=""
 .S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 ..I $P($G(^BGPPEDCM(X,11)),U,5)>BGPCYD S BGPCYD=$P($G(^BGPPEDCM(X,11)),U,5)
 ..I $P($G(^BGPPEDPM(X,11)),U,5)>BGPPRD S BGPPRD=$P($G(^BGPPEDPM(X,11)),U,5)
 ..I $P($G(^BGPPEDBM(X,11)),U,5)>BGPBLD S BGPBLD=$P($G(^BGPPEDBM(X,11)),U,5)
 I '$G(BGPAREAA) D
...
HEADER1 ;
 ...
 I '$G(BGPSEAT) W $$CTR("*** IHS 2016 Patient Education with Community Specified Report ***",80),!
 I $G(BGPSEAT) W $$CTR("*** IHS 2016 Patient Education with Patient Panel Population Report ***",80),!
...
SETNM ;
 ...
 S X=0 F  S X=$O(^BGPPEDCM(BGPRPT,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDCM(BGPRPT,N,X,0),U),L=$P(^BGPPEDCM(BGPRPT,N,X,0),U,2),M=$P(^BGPPEDCM(BGPRPT,N,X,0),U,3)
 .S $P(BGPPROVS(C,L),U,1)=M
 S X=0 F  S X=$O(^BGPPEDPM(BGPRPT,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDPM(BGPRPT,N,X,0),U),L=$P(^BGPPEDPM(BGPRPT,N,X,0),U,2),M=$P(^BGPPEDPM(BGPRPT,N,X,0),U,3)
 .S $P(BGPPROVS(C,L),U,2)=M
 S X=0 F  S X=$O(^BGPPEDBM(BGPRPT,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDBM(BGPRPT,N,X,0),U),L=$P(^BGPPEDBM(BGPRPT,N,X,0),U,2),M=$P(^BGPPEDBM(BGPRPT,N,X,0),U,3)
...
SETNMA ;
 ...
 Q
SETNMA1 ;
 S X=0 F  S X=$O(^BGPPEDCM(Z,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDCM(Z,N,X,0),U),L=$P(^BGPPEDCM(Z,N,X,0),U,2),M=$P(^BGPPEDCM(Z,N,X,0),U,3)
 .S $P(BGPPROVS(C,L),U,1)=$P($G(BGPPROVS(C,L)),U,1)+M
 S X=0 F  S X=$O(^BGPPEDPM(Z,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDPM(Z,N,X,0),U),L=$P(^BGPPEDPM(Z,N,X,0),U,2),M=$P(^BGPPEDPM(Z,N,X,0),U,3)
 .S $P(BGPPROVS(C,L),U,2)=$P($G(BGPPROVS(C,L)),U,2)+M
 S X=0 F  S X=$O(^BGPPEDBM(Z,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDBM(Z,N,X,0),U),L=$P(^BGPPEDBM(Z,N,X,0),U,2),M=$P(^BGPPEDBM(Z,N,X,0),U,3)
...
SETEXA(T,N,P) ;EP - set denominator
 ...
 NEW X,Y,Z
 S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 .I T=1 S $P(BGPEXCT(X,T),U,2)=$P($G(^BGPPEDCM(X,N)),U,P)
 .I T=2 S $P(BGPEXCT(X,T),U,2)=$P($G(^BGPPEDPM(X,N)),U,P)
 .I T=3 S $P(BGPEXCT(X,T),U,2)=$P($G(^BGPGPDBM(X,N)),U,P)
 Q
V(T,R,N,P,ND) ;EP ;SPDX
 I $G(BGPAREAA) G VA
 NEW X
 I T=1 S X=$P($G(^BGPPEDCM(R,N)),U,P) Q $S(X]"":X,1:0)
 I T=2 S X=$P($G(^BGPPEDPM(R,N)),U,P) Q $S(X]"":X,1:0)
 I T=3 S X=$P($G(^BGPPEDBM(R,N)),U,P) Q $S(X]"":X,1:0)
 Q ""
VA ;
 NEW X,V,C S X=0,C="" F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 .I T=1 S C=C+$P($G(^BGPPEDCM(X,N)),U,P)
 .I T=2 S C=C+$P($G(^BGPPEDPM(X,N)),U,P)
 .I T=3 S C=C+$P($G(^BGPPEDBM(X,N)),U,P)
 .Q
 Q $S(C]"":C,1:0)
