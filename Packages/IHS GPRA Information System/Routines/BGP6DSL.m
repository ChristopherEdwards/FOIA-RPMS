BGP6DSL ; IHS/CMI/LAB - IHS 2016 DISPLAY IND LISTS ;
...
EN ;EP -- main entry point for GPRA LIST DISPLAY
 D EN^VALM("BGP 16 LIST SELECTION")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ1
 ...
 ;
HDR ; -- header code
 S VALMHDR(1)="IHS 2016 Clinical Performance Measure Lists of Patients"
 S VALMHDR(2)="* indicates the list has been selected"
 Q
 ;
INIT ; -- init variables and list array
 K BGPGLIST,BGPNOLI S BGPHIGH=""
 S (X,C,I,O)=0 F  S O=$O(^BGPINDM("AOI",O)) Q:O'=+O  S X=$O(^BGPINDM("AOI",O,0)) I $D(BGPIND(X)) D
 .I $P(^BGPINDM(X,0),U,5)]"" S C=C+1 D  Q
 ..S BGPGLIST(C,0)=C_")",$E(BGPGLIST(C,0),5)=$P(^BGPINDM(X,0),U,5),BGPGLIST("IDX",C,C)=X I $D(BGPLIST(X)) S BGPGLIST(C,0)="*"_BGPGLIST(C,0)
 .I $P(^BGPINDM(X,0),U,5)="" S C=C+1 D
 ..S BGPGLIST(C,0)=$P(^BGPINDM(X,0),U,4)_" NO patient list available for measure:  ",BGPGLIST("IDX",C,C)=X,BGPNOLI(X)="" I $D(BGPLIST(X)) S BGPGLIST(C,0)="*"_BGPGLIST(C,0)
 S (VALMCNT,BGPHIGH)=C
 Q
 ;
HELP ; -- help code
...
PT1 I '$O(BGPDESGP(0)) S DIR(0)="F^1:40",DIR("A")="Enter a filename for the delimited output (no more than 40 characters)" KILL DA D ^DIR KILL DIR
 ...
 S BGPDELF=Y
 W !!,"When the report is finished your delimited output will be found in the",!,$$GETDEDIR^BGP6UTL2()," directory."
