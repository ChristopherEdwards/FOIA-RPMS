BGP6EL4 ; IHS/CMI/LAB - measure 1,2,3,4 05 Apr 2006 1:44 PM ;
...
I18 ;EP
 ...
 S BGPVALUE=$$CHOL^BGP6D73(DFN,$$FMADD^XLFDT(BGPEDATE,-(5*365)),BGPEDATE)
 ...
 .I +V>160 S BGPN7=1
 S V="AC"_"|||"
 I BGPVALUE]"" S V=V_"CHOL "_$$DATE^BGP6UTL($P(BGPVALUE,U,1))_" "_$P(BGPVALUE,U,3)
...
FUNCTION(P,BDATE,EDATE) ;EP
 ...
 .S $P(BGPQ,U,2)=$P(BGPQ,U,2)_$$DATE^BGP6UTL((9999999-Y))_": "_BGPV(Y)_" "
 K BGPV,BGPG
 Q BGPQ
 ;
...
TXBMD(P,BDATE,EDATE,HOSP) ;EP
 ...
 S HOSP=$G(HOSP)
 K BGPG
 S BGPG=$$LASTPRC^BGP6UTL1(P,"BGP BMD PROCEDURES",BDATE,EDATE)
 I $P(BGPG,U)=1 Q 1_U_"bmd proc "_$P(BGPG,U,2)_" "_$$DATE^BGP6UTL($P(BGPG,U,3))
 ;now check dx
 S BGPG=$$LASTDX^BGP6UTL1(P,"BGP BMD DXS",BDATE,EDATE)
 I $P(BGPG,U)=1 Q 1_U_"bmd DX "_$P(BGPG,U,2)_" "_$$DATE^BGP6UTL($P(BGPG,U,3))
 ;now check cpts
 S T=$O(^ATXAX("B","BGP BMD CPTS",0))
 S BGPG=$$CPT^BGP6DU(P,BDATE,EDATE,T,5)
 I BGPG]"" Q 1_U_$P(BGPG,U,2)_" "_$$DATE^BGP6UTL($P(BGPG,U,1)) ;had a cpt
 ;now check RAD
 S T=$O(^ATXAX("B","BGP BMD CPTS",0))
 S BGPG=$$RAD^BGP6DU(P,BDATE,EDATE,T,5)
 I BGPG]"" Q 1_U_$P(BGPG,U,2)_" "_$$DATE^BGP6UTL($P(BGPG,U,1))  ;had a cpt
 I HOSP Q ""
 ;now check all meds
 K ^TMP($J,"MEDS")
 ...
 .Q
 K ^TMP($J,"MEDS")
 I G Q 1_U_"osteo med: "_$$DATE^BGP6UTL(G)
 Q ""
