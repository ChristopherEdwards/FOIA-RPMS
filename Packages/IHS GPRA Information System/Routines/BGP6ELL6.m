BGP6ELL6 ; IHS/CMI/LAB - print ind ;
...
I1AGE ;EP
 ...
 K BGPDAC,BGPDAP,BGPDAB S (C,D,E,F,G,H,I,J,K)=0 F BGPX="NPB.5","NPB.6","NPB.7","NPB.8" D I1AGE1,I1AGE2,I1AGE3
 D I1AGEP
 Q
I1AGE1 ;
 S C=C+1
...
I1AGEP ;
 ...
 S X="# w/pos urine protein or any microalbuminuria (GPRA)" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,2) S Y=V D S(Y,,X+1)
 S X="% w/pos urine protein or any microalbuminuria (GPRA)" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,3) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="#  w/est GFR with result" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,4) S Y=V D S(Y,,X+1)
 S X="% w/est GFR with result" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,5) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="# w/both urine protein/micro and GFR" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,6) S Y=V D S(Y,,X+1)
 S X="% w/both urine protein/micro and GFR" D S(X,1,1)
...
BL ;
 ...
 S X="w/pos urine protein or any microalbuminuria" D S(X,1,1)
...
CALC(N,O) ;
 ...
 Q Z
H3 ;EP
 S X="Diabetes: Nephropathy Assessment" D S(X,1,1) S Y=" " D S(Y,1,1) S X=BGPHD1 D S(X,1,1) S Y=" " D S(Y,1,1)
