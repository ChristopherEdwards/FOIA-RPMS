BGP6ELL7 ; IHS/CMI/LAB - print ind ;
...
I1AGEP ;
 ...
 S X=" " D S(X,1,1) S X="A. # w/ DM Retinal exam or refusal" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,4) S Y=V D S(Y,,X+1)
 S X="% A. w/ DM Retinal exam or refusal" D S(X,1,1)
...
CALC(N,O) ;
 ...
 Q Z
H3 ;EP
 S X="Diabetes: Retinopathy" D S(X,1,1) S Y=" " D S(Y,1,1) S X=BGPHD1 D S(X,1,1) S Y=" " D S(Y,1,1)
