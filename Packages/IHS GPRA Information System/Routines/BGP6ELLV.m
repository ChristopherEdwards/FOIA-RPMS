BGP6ELLV ; IHS/CMI/LAB - print ind ;
...
BL ;EP
 ...
 S X="% w/ Total Cholesterol screen w/in 5 yrs (GPRA)" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAB(X),U,3) S Y=$$SB^BGP6ELL2($J(V,6,1)) D S(Y,,X+1)
 ...
 F X=1:1:4 S V=$P(BGPDAB(X),U,13) S Y=$$SB^BGP6ELL2($J(V,6,1)) D S(Y,,X+1)
 ...
 F X=1:1:4 S V=$P(BGPDAB(X),U,15) S Y=$$SB^BGP6ELL2($J(V,6,1)) D S(Y,,X+1)
 ...
 S X="D. # w/ LDL >160 w/ % of Total LDL Screen" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,15),O=$P(BGPDAB(X),U,15) S Y=$$SB^BGP6ELL2($J((N-O),6,1)) D S(Y,,X+1)
 Q
V(T,R,N,P) ;
 I $G(BGPAREAA) G VA
 NEW X
