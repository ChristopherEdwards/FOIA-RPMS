BGP6ELSL ; IHS/CMI/LAB - IHS gpra print ;
...
GO .;
 ...
 .S X=$P(^BGPELIM(BGPL,0),U,3) D S(X,1,1)
 .S X=" " D S(X,1,1)
 .I $G(BGPDNT) G CALC
 .S X="Denominator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPELIM(BGPL,61,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPELIM(BGPL,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPELIM(BGPL,61,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S X=^BGPELIM(BGPL,61,BGPY,1,BGPZ,0) S:BGPZ=1 X=" - "_X D S(X,1,1)   ;S Y=^BGPELIM(BGPL,61,BGPY,1,BGPZ,0) D S(Y,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .S X="Numerator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPELIM(BGPL,62,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPELIM(BGPL,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPELIM(BGPL,62,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S X=^BGPELIM(BGPL,62,BGPY,1,BGPZ,0) S:BGPZ=1 X=" - "_X D S(X,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPELIM(BGPL,11,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPELIM(BGPL,11,BGPX,0) D S(X,1,1)
 .S X=" " D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPELIM(BGPL,51,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPELIM(BGPL,51,BGPX,0) D S(X,1,1)
 .S X=" " D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPELIM(BGPL,52,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPELIM(BGPL,52,BGPX,0) D S(X,1,1)
CALC .;
 .D H1
...
PRINTL ;print one line
 ...
 S X=$P(^XTMP("BGP6D",BGPJ,BGPH,"LIST",BGPL,BGPCOM,BGPSEX,BGPAGE,DFN),"|||",1) D S(X,,7)
 S X=$P(^XTMP("BGP6D",BGPJ,BGPH,"LIST",BGPL,BGPCOM,BGPSEX,BGPAGE,DFN),"|||",2) D S(X,,8)
 Q
 ;
HEADER ;EP
 G:'BGPGPG HEADER1
...
HEADER1 ;
 ...
 W !,$$CTR("***  IHS 2016 ELDER CARE Clinical Performance Report Patient List  ***",80),!
...
H1 ;
 ...
 S Y=0 F  S Y=$O(^BGPELIM(BGPL,71,Y)) Q:Y'=+Y  S X=^BGPELIM(BGPL,71,Y,0) D S(X,1,1)
 S X=" " D S(X,1,1)
