BGP6GACW ; IHS/CMI/LAB - AREA NATIONAL GPRA REPORT ;
...
TESTCHW ;
 ...
 D EP(.ERR,1,2522,"BGP 06 AREA GPRA","A",.LORISUL,$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRPTT,BGPSUL,BGPRTIME) ;EP - called from GUI to produce national gpra report (AO-AGP)
...
EP1 ;
 ...
 S BGPRTYPE=1,BGPBEN=1,BGP6RPTH="",BGPCHWE=1
 ...
 S BGPFN="HW"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP6DCHA(BGPBBD)_$$D^BGP6DCHA(BGPED)_BGPNOW_".TXT"
 S BGPASUF=$P(^AUTTLOC(DUZ(2),0),U,10)
 S BGPDELT=""
 ...
 D TSKMN
 ;S BGPRET=BGPGIEN
 S BGPRET=1_U_BGPFN_" in directory "_BGPUF
 Q
 ;
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOCHW^BGP6GACW",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI CHILDHOOD HT/WT EXPORT" D ^%ZTLOAD Q
 Q
AOCHW ;
 K ^TMP($J,"BGPGUI")
 D GS^BGP6DCHA
