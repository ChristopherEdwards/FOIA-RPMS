...
TESTNTL ;
 ...
 S LORISUL(1)=""
 S LORISUL(2)=""
 D EP(.ERR,1,2522,"BGP 06 AREA GPRA","A",.LORISUL,"B",1,$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRPTT,BGPSUL,BGPROT,BGPEXCEL,BGPRTIME) ;EP - called from GUI to produce national gpra report (AO-AGP)
...
EP1 ;
 ...
 S BGPSUCNT=C
 I BGPSUCNT=1 S BGPRPTT="F",Y=$O(BGPSUL(0)),X=$P(^BGPGPDCS(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 ...
 D TSKMN
 S BGPRET=BGPGIEN
 I $G(BGPEXCEL) S $P(BGPRET,U,3)=BGPFN_" in directory "_BGPUF
 Q
 ;
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOAGP^BGP6GAGP",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT 05" D ^%ZTLOAD Q
 Q
AOAGP ;
 K ^TMP($J,"BGPGUI")
