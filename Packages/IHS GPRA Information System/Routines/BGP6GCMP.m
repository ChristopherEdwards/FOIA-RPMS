...
TESTCMP ;
 S ERR=""
 D EP(.ERR,1,2522,"BGP 16 COMPREHENSIVE PAT LIST","A",,,1,3050000,338,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPLIST,BGPLPRV,BGPLPROV,BGPQTR,BGPPER,BGPTAXI,BGPROT,BGPRTIME,BGPMFITI,BGPVDT,BGPBEN,BGPFILE,BGPDNT) ;EP - called from GUI to produce national gpra report (NTL-GP)
 ;  BGPUSER - DUZ
 ;  BGPDUZ2 - DUZ(2)
 ;  BGPOPTN - OPTION NAME = "BGP 16 COMPREHENSIVE PAT LIST"
 ;  
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="NTLCMP^BGP6GCMP",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI COMPREHENSIVE PATIENT LIST 09" D ^%ZTLOAD
