BGP6GCMP ; IHS/CMI/LAB - IHS Diabetes Audit 2003 ;
 ;;7.0;IHS CLINICAL REPORTING;;JAN 24, 2007
 ;
 ;
TESTCMP ;
 S ERR=""
 D EP(.ERR,1,2522,"BGP 06 COMPREHENSIVE PAT LIST","A",,,1,3050000,338,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPLIST,BGPLPRV,BGPLPROV,BGPQTR,BGPPER,BGPTAXI,BGPROT,BGPRTIME,BGPMFITI) ;EP - called from GUI to produce national gpra report (NTL-GP)
 ;  BGPUSER - DUZ
 ;  BGPDUZ2 - DUZ(2)
 ...
 ;           with a 1 through 4  e.g  3060000 (fileman imprecise date for 2006)
...
EP1 ;
 ...
 S BGPGUI=1
 S IOM=80,BGPIOSL=55
 S BGPRTYPE=1,BGP6RPTH="",BGPCPPL=1,BGPINDT="G",BGP6GPU=1
 K BGPTAX S X=0
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="NTLCMP^BGP6GCMP",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI COMPREHENSIVE PATIENT LIST 06" D ^%ZTLOAD Q
 Q
NTLCMP ;
 S BGPCPLC=0
 D ^BGP6D1
