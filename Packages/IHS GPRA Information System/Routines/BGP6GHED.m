BGP6GHED ; IHS/CMI/LAB - IHS Diabetes Audit 2003 ;
...
TESTNTL ;
 ...
 D EP(.ERR,1,2522,"BGP 06 HEDIS REPORT",338,1,3040000,3000000,1,1,"A",.LORILST,"","","B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPEXPT,BGPLSTT,BGPLIST,BGPLPRV,BGPLPROV,BGPROT,BGPRTIME,BGPMFITI) ;EP - called from GUI to produce national GPU report (OTH-HED)
 ...
 ;        NOTE:  THE GLOBAL TO USE TO DISPLAY THE AVAILABLE LISTS IS ^BGPHEIS(
 ;        e.g.  BGPLIST="A"
 ;              BGPLIST(3)=""
 ;
...
BY ;get baseline year
 ...
 S BGPFILEN="BG06"_$P(^AUTTLOC(DUZ(2),0),U,10)_".HE"_BGPRPT_" in directory "_BGPUF
 S BGPDELT=""
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="OTHHED^BGP6GHED",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI HEDIS GPRA REPORT 06" D ^%ZTLOAD Q
 Q
OTHHED ;
 D ^BGP6D1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^XBLM("^BGP6DHEP","^TMP($J,""BGPGUI"",")
