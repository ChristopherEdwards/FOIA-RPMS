...
TESTCMP ;
 S ERR=""
 D EP(.ERR,1,2522,"BGP 16 LAB TAX REPORT",$$NOW^XLFDT)
 W !,ERR
 Q
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LTX^BGP6GLTX",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI LAB TAXONOMY REPORT" D ^%ZTLOAD
 D UPLOG^BGP6GUA(BGPGIEN,ZTSK)
 Q
LTX ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D @$S(BGPRT=1:"GPRA^BGP6DLT",BGPRT=7:"ONM^BGP6DLT",BGPRT=2:"CRS^BGP6DLT",BGPRT=4:"ELDER^BGP6DLT",BGPRT=3:"HEDIS^BGP6DLT",BGPRT=5:"CMS^BGP6DLT",BGPRT=8:"EO^BGP6DLT")  ;cmi/maw added 10/30/2010
 D GUIR^BGPXBLM("PRINT^BGP6DLT","^TMP($J,""BGPGUI"",")
...
XIT ;
 K ^TMP($J)
 ;D EN^XBVK("BGP")  ;done in LABTAXC^BGP6GU
 S:$D(ZTQUEUED) ZTREQ="@"
