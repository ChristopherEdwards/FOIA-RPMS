BGP6GM ; IHS/CMI/LAB - BGPG Visual CRS Reports ;
...
MUEP(RETVAL,BGPSTR) ;sel measures by comm
 ...
 D EP^BGP6GMUE(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPCT,BGPTP,.BGPIND,BGPQTR,BGPRE,BGPBAS,$G(BGPBEB),BGPLSTT,.BGPLIST,BGPPRV,BGPPROV,$G(BGPOT),BGPRDT,BGPMFITI,BGPFN)
 S BGPI=BGPI+1
...
MUHOS(RETVAL,BGPSTR) ;sel measures by comm
 ...
 S RETVAL="^BGPTMP("_$J_")"
 S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 D EP^BGP6GMUH(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPCT,BGPTP,.BGPIND,BGPQTR,BGPRE,BGPBAS,$G(BGPBEB),BGPLSTT,.BGPLIST,BGPPRV,BGPPROV,$G(BGPOT),BGPRDT,BGPMFITI,BGPFN)
 S BGPI=BGPI+1
 S ^BGPTMP($J,BGPI)=+$G(BGPERR)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)
 D EN^XBVK("BGP")
