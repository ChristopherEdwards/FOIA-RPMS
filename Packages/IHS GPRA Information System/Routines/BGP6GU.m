BGP6GU ; IHS/CMI/LAB - BGP Gui Utilities 10/29/2004 3:28:39 PM 19 Sep 2005 5:28 PM 27 Apr 2010 10:28 PM ;
 ;;16.1;IHS CLINICAL REPORTING;;MAR 22, 2016;Build 170
 ;
 ;
DEBUG(RETVAL,BGPSTR) ;run the debugger
 D DEBUG^%Serenji("UPL^BGP6GU(.RETVAL,.BGPSTR)")
 Q
 ;
...
FAC(RETVAL,BGPSTR) ;-- return facilities based on parameters passed
 ...
 D GET^BGP6ASL(.BGPFAC,.BGPFILE,BGPRT,BGPPER,BGPQTR,BGPBAS,BGPBEN)
...
LST(RETVAL,BGPSTR) ;-- list files
 ...
 S RETVAL="^BGPTMP("_$J_")"
 S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 D EP^BGP6GLST(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPCT,BGPYN,BGPOT,BGPRT)
 S BGPI=BGPI+1
 ...
 Q
 ;
DELRPT(RETVAL,BGPSTR) ;-- delete a report out of the BGP 16 GUI REPORT file
...
UPL(RETVAL,BGPSTR) ;-- upload a file
 ...
 . D EP^BGP6GUPL(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPDIR,BGPFL,DT)
...
CHK(BGPRET,BGPSTR) ;check report status
 ...
 N BGPDA
 S BGPDA=0 F  S BGPDA=$O(^BGPGUIM("AUSR",BGPDUZ,BGPDA)) Q:'BGPDA  D
 . N BGPIEN
 . S BGPIEN=0 F  S BGPIEN=$O(^BGPGUIM("AUSR",BGPDUZ,BGPDA,BGPIEN)) Q:'BGPIEN  D
 .. N BGPDATA,BGPNM,BGPUSER,BGPST,BGPET,BGPTOR,BGPRS,BGPTOO,BGPFLS,BGPSTI,BGPETI
 .. S BGPDATA=$G(^BGPGUIM(BGPIEN,0))
 .. S BGPNM=$P(BGPDATA,U)
 ...
 ... S DIK="^BGPGUIM(",DA=BGPIEN D ^DIK
 .. S BGPTOR=$P(BGPDATA,U,5)
 .. S BGPRS=$$GET1^DIQ(90556.19,BGPIEN,.06)
 .. I BGPSTI>$$NOW^XLFDT() D
 ... Q:$G(BGPRS)="COMPLETED"
 ... S BGPRS="QUEUED"
 .. S BGPTSK=$$CHKFQT^BGP6GUA(BGPIEN)
 .. I $G(BGPTSK) S BGPRS="ERRORED"
 .. S BGPTOO=$$GET1^DIQ(90556.19,BGPIEN,.07)
 .. S BGPFLS=$$GET1^DIQ(90556.19,BGPIEN,1)
 .. S BGPI=BGPI+1
...
LABTAXC(RETVAL,BGPSTR) ;EP - from remote procedure
 ...
 S BGPERR=""
 S BGPOPT=$P(BGPSTR,P)
 S BGPOPT="CRS 16 LAB TAXONOMY REPORT"  ;8.0 p2
 ...
 S RETVAL="^BGPTMP("_$J_")"
 S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 D EP^BGP6GLTX(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPRT,BGPR,BGPFN)
 S BGPI=BGPI+1
 S ^BGPTMP($J,BGPI)=+$G(BGPERR)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)
 D EN^XBVK("BGP")
...
MEDTAXC(RETVAL,BGPSTR) ;EP - from remote procedure
 ...
 S BGPERR=""
 S BGPOPT=$P(BGPSTR,P)
 S BGPOPT="CRS 16 MEDICATION TAXONOMY REPORT"  ;8.0 p2
 S BGPRT=$P(BGPSTR,P,2)
 S BGPR=$P(BGPSTR,P,3)
 S BGPFN=$P(BGPSTR,P,4)
 S BGPJ=0
 ...
 S RETVAL="^BGPTMP("_$J_")"
 S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 D EP^BGP6GMTX(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPRT,BGPR,BGPFN)
 S BGPI=BGPI+1
 S ^BGPTMP($J,BGPI)=+$G(BGPERR)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)
 D EN^XBVK("BGP")
