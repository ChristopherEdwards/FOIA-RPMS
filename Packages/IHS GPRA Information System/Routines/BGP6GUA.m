...
DEBUG(RETVAL,BGPSTR) ;run the debugger
 D DEBUG^%Serenji("LABTAXC^BGP6GU(.RETVAL,.BGPSTR)")
 Q
 ;
...
CHKFQT(BGPX) ;EP - check for queued task (BGP AUTO GPRA EXTRACT and BGPSITE variable within the task
 NEW X,BGPY,TASKJ
 S BGPY=$P($G(^BGPGUIM(BGPX,0)),U,9)
 I '$G(BGPY) Q 0
 I '$D(^%ZTSK(BGPY,0)),$P($G(^BGPGUIM(BGPX,0)),U,6)="R" Q 1  ;v16.0 check for deleted task and mark as errored if so
