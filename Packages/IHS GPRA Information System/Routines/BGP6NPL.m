BGP6NPL ; IHS/CMI/LAB - national patient list 20 Dec 2004 9:24 AM 30 Jun 2010 5:21 PM ;
...
TP ;get time period
 D XIT
 S BGPRTYPE=1,BGPYRPTH="",BGPNPL=1,BGPINDM="G",BGPYGPU=1
SI ;
 K DIRUT
 K BGPIND
 D EN^BGP6DSI
...
SI1 ;NOW SELECT ONE OR MORE W/IN THE TOPIC
 ...
 K BGPINDO
 S BGPIND=0 F  S BGPIND=$O(BGPIND(BGPIND)) Q:BGPIND'=+BGPIND  S BGPINDO($P(^BGPINDM(BGPIND,12),U,6),BGPIND)=""
 ...
 .K BGPX S BGPO=0,X=0,BGPC=0 F  S BGPO=$O(^BGPNPLM(BGPCR,BGPIND,BGPO)) Q:BGPO'=+BGPO!($D(DIRUT))  D
 ...
 ;get report type
 I $D(DIRUT) G SI
 K BGPQUIT D RT^BGP6DSL I '$D(BGPLIST)!($D(BGPQUIT)) G SI
...
TPNON ;get time period
 D XIT
 S BGPRTYPE=7,BGPYRPTH="",BGPNPL=1,BGPINDM="G",BGPYGPU=1,BGPONMR=1,BGPRTC="U"
 G SI
