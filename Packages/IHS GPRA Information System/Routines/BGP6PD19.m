BGP6PD19 ; IHS/CMI/LAB - print ind 03 Jul 2010 8:56 AM ;
...
I1AGE1 ;
 S C=C+1
 S BGPF="013.B.1"_BGPX S BGPPC=$O(^BGPINDMC("C",BGPF,0))
 S BGPDF=$P(^BGPINDMC(BGPPC,0),U,8)
 S BGPNP=$P(^DD(90556.03,BGPDF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(C),U)=$$V^BGP6DP1C(1,BGPRPT,N,P) I $G(BGPAREAA) D SETEXA^BGP6DP1C(1,N,P)
 S $P(BGPDAP(C),U)=$$V^BGP6DP1C(2,BGPRPT,N,P) I $G(BGPAREAA) D SETEXA^BGP6DP1C(2,N,P)
 S $P(BGPDAB(C),U)=$$V^BGP6DP1C(3,BGPRPT,N,P) I $G(BGPAREAA) D SETEXA^BGP6DP1C(3,N,P)
 ...
 I BGPF="013.B.1A"!(BGPF="013.B.1B") D GPRANT1^BGP6DP1C
 Q
I1AGE2 ;
 S D=D+1
 S BGPF="013.B.2"_BGPX S BGPPC=$O(^BGPINDMC("C",BGPF,0))
...
PR ; 
 ;S X=^BGPINDM(BGPIC,53,1,0) D S(X,1,1) D H9
