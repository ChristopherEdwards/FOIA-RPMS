...
EDD(P,BDATE,EDATE) ;EP - find EDD
 ...
 S Y=$$LASTITEM^BGP6DU(P,BDATE,EDATE,"MEASUREMENT","EGA")
 I Y D  I EDD Q EDD
...
RCIS(P,BDATE,EDATE,ICDC,CPTC) ;EP
 ...
 Q 1_"^"_$P($P(^BMCREF(G,0),U),".")_"^"_$$DATE^BGP6UTL($P($P(^BMCREF(G,0),U),"."))_"^"_"RCIS referral"_"^"_X_"^"_"90001"_"^"_G
 ;
...
UNFOLDTX ;EP
 ...
 S BGPDA=0 F  S BGPDA=$O(^BGPTAXM(BGPDA)) Q:'BGPDA  D
 . S BGPTAX=$P($G(^BGPTAXM(BGPDA,0)),U)
 . S BGPFL=$P($G(^BGPTAXM(BGPDA,0)),U,2)
