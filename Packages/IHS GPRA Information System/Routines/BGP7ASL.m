...
EN ;EP -- main entry point for APCH HMR DISPLAY
 D EN^VALM("BGP 07 AREA SELECT SITES")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ; -- init variables and list array
 ...
 I BGPRTYPE=6 D PED Q
 Q
NGR ;
 K BGPTIND S BGPHIGH=""
 S BGPSUCNT=0,BGPSU="",BGPSUC=0
 S BGPX=0 F  S BGPX=$O(^BGPGPDCA(BGPX)) Q:BGPX'=+BGPX  S V=$G(^BGPGPDCA(BGPX,0)) D
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
 .Q:$P(V,U,7)'=BGPPER
 .I $G(BGP7GPU),$P(V,U,12)'=9 Q
 .I '$G(BGP7GPU),$P(V,U,12)'=1 Q
 .I $G(BGPCHWE) Q:'$O(^BGPGPDCA(BGPX,88888,0))
 .Q:$P(V,U,5)'=BGPBBD
 .Q:$P(V,U,6)'=BGPBED
 .Q:$P(V,U,14)'=BGPBEN
 ...
 .S $E(Y,15)=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPGPDCA(BGPX,0),U,17):"+",1:"")
...
ELD ;
 K BGPTIND S BGPHIGH=""
 S BGPSUCNT=0,BGPSU="",BGPSUC=0
 S BGPX=0 F  S BGPX=$O(^BGPELDCA(BGPX)) Q:BGPX'=+BGPX  I BGPX S V=$G(^BGPELDCA(BGPX,0)) D
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
 .Q:$P(V,U,7)'=BGPPER
 .Q:$P(V,U,12)'=5
 ...
 .S Y=""
 .S $E(Y,4)=$E($$SU($P(V,U,11)),1,10)
 .S $E(Y,15)=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPELDCA(BGPX,0),U,17):"+",1:"")
 .S $E(Y,30)=$$DATE^BGP7UTL($P(V,U))
 .S $E(Y,40)=$$DATE^BGP7UTL($P(V,U,2))
 .S $E(Y,50)=$$DATE^BGP7UTL($P(V,U,5))
 .S $E(Y,60)=$$DATE^BGP7UTL($P(V,U,6))
...
HED ;
 K BGPTIND S BGPHIGH=""
 S BGPSUCNT=0,BGPSU="",BGPSUC=0
 S BGPX=0 F  S BGPX=$O(^BGPHEDCA(BGPX)) Q:'BGPX  I BGPX S V=$G(^BGPHEDCA(BGPX,0)) D
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
 .Q:$P(V,U,7)'=BGPPER
 .Q:$P(V,U,12)'=3
 ...
 .S Y=""
 .S $E(Y,4)=$E($$SU($P(V,U,11)),1,10)
 .S $E(Y,15)=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPHEDCA(BGPX,0),U,17):"+",1:"")
 .S $E(Y,30)=$$DATE^BGP7UTL($P(V,U))
 .S $E(Y,40)=$$DATE^BGP7UTL($P(V,U,2))
 .S $E(Y,50)=$$DATE^BGP7UTL($P(V,U,5))
 .S $E(Y,60)=$$DATE^BGP7UTL($P(V,U,6))
...
PED ;
 K BGPTIND S BGPHIGH=""
 S BGPSUCNT=0,BGPSU="",BGPSUC=0
 S BGPX=0 F  S BGPX=$O(^BGPPEDCA(BGPX)) Q:BGPX'=+BGPX  I BGPX S V=$G(^BGPPEDCA(BGPX,0)) D
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
 .Q:$P(V,U,7)'=BGPPER
 .Q:$P(V,U,5)'=BGPBBD
 ...
 .S $E(Y,15)=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPPEDCA(BGPX,0),U,17):"+",1:"")
 .S $E(Y,30)=$$DATE^BGP7UTL($P(V,U))
 .S $E(Y,40)=$$DATE^BGP7UTL($P(V,U,2))
 .S $E(Y,50)=$$DATE^BGP7UTL($P(V,U,5))
 .S $E(Y,60)=$$DATE^BGP7UTL($P(V,U,6))
...
G1 ;
 ...
 S BGPPER=3070000,BGPQTR=3,BGPBEN=1
 ;LORI PLEASE FIX AFTER TESTING
 ;S BGPBD=3030101,BGPED=3031231
 ;S BGPBBD=3000101,BGPBED=3001231
 ;S BGPPBD=3020101,BGPPED=3021231
 ; BGPPER=3030000,BGPQTR=3,BGPBEN=1
 S BGPFILE=90530.03
 S BGPX=0 F  S BGPX=$O(^BGPGPDCA(BGPX)) Q:BGPX'=+BGPX  S V=^BGPGPDCA(BGPX,0) D
 ...
 . S BGPFAC=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPGPDCA(BGPX,0),U,17):" (CHS ONLY)",1:"")
...
G3 ;
 ...
 S BGPFILE=90531.03
 S BGPX=0 F  S BGPX=$O(^BGPHEDCA(BGPX)) Q:'BGPX  I BGPX S V=^BGPHEDCA(BGPX,0) D
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
 .Q:$P(V,U,7)'=BGPPER
 .Q:$P(V,U,12)'=3
 ...
 .Q:$P(V,U,14)'=BGPBEN
 . S BGPSU=$E($$SU($P(V,U,11)),1,10)
 . S BGPFAC=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPHEDCA(BGPX,0),U,17):" (CHS ONLY)",1:"")
 . S BGPEBD=$$DATE^BGP7UTL($P(V,U))
 . S BGPEED=$$DATE^BGP7UTL($P(V,U,2))
 . S BGPEBBD=$$DATE^BGP7UTL($P(V,U,5))
 . S BGPEBED=$$DATE^BGP7UTL($P(V,U,6))
...
G5 ;
 ...
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPFILE=90532.03
 S BGPX=0 F  S BGPX=$O(^BGPELDCA(BGPX)) Q:'BGPX  I BGPX S V=^BGPELDCA(BGPX,0) D
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
 .Q:$P(V,U,7)'=BGPPER
 .Q:$P(V,U,12)'=5
 ...
 .Q:$P(V,U,14)'=BGPBEN
 . S BGPSU=$E($$SU($P(V,U,11)),1,10)
 . S BGPFAC=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPELDCA(BGPX,0),U,17):" (CHS ONLY)",1:"")
 . S BGPEBD=$$DATE^BGP7UTL($P(V,U))
 . S BGPEED=$$DATE^BGP7UTL($P(V,U,2))
 . S BGPEBBD=$$DATE^BGP7UTL($P(V,U,5))
 . S BGPEBED=$$DATE^BGP7UTL($P(V,U,6))
...
G6 ;
 ...
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPFILE=90532.03
 S BGPX=0 F  S BGPX=$O(^BGPPEDCA(BGPX)) Q:'BGPX  I BGPX S V=^BGPPEDCA(BGPX,0) D
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
 .Q:$P(V,U,7)'=BGPPER
 .Q:$P(V,U,5)'=BGPBBD
...
G9 ;
 ...
 S BGPBBD=BGPBD-X,BGPBBD=$E(BGPBBD,1,3)_$E(BGPBD,4,7)
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPX=0 F  S BGPX=$O(^BGPGPDCA(BGPX)) Q:'BGPX  I BGPX S V=^BGPGPDCA(BGPX,0) D
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
 .Q:$P(V,U,7)'=BGPPER
 .Q:$P(V,U,12)'=9
...
G8 ;childhood weight
 ...
 S BGPPER=3070000,BGPQTR=3,BGPBEN=1
 ;LORI PLEASE FIX AFTER TESTING
 ; BGPBD=3030101,BGPED=3031231
 ; BGPBBD=3000101,BGPBED=3001231
 ; BGPPBD=3020101,BGPPED=3021231
