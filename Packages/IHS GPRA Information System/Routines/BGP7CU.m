...
CHESTXRY(P,BDATE,EDATE,BGPY) ;EP
 ...
 ...I $$ICD^BGP7UTL2($P(^AUPNVCPT(X,0),U),T,1) S C=C+1,BGPY(ED,C)=$$DATE^BGP7UTL(9999999-$P(ED,"."))_"  "_$P($$CPT^ICPTCOD($P(^AUPNVCPT(X,0),U),(9999999-$P(ED,"."))),U,2)_"  "_$P($$CPT^ICPTCOD($P(^AUPNVCPT(X,0),U),(9999999-$P(ED,"."))),U,3)
 ..;now go through v rads
 ..S X=0 F  S X=$O(^AUPNVRAD("AD",V,X)) Q:X'=+X  D
 ...S I=$P(^AUPNVRAD(X,0),U) Q:I=""  S I=$P($G(^RAMIS(71,I,0)),U,9) Q:I=""
 ...I $$ICD^BGP7UTL2(I,T,1) S C=C+1,BGPY(ED,C)=$$DATE^BGP7UTL(9999999-$P(ED,"."))_"  "_$P($$CPT^ICPTCOD(I,(9999999-$P(ED,"."))),U,2)_"  "_$P($$CPT^ICPTCOD(I,(9999999-$P(ED,"."))),U,3)_" Impression: "_$P($G(^AUPNVRAD(X,11)),U,1)
 ..; now v tran
 ..S X=0 F  S X=$O(^AUPNVTC("AD",V,X)) Q:X'=+X  D
 ...S I=$P(^AUPNVTC(X,0),U,7)
 ...Q:I=""
 ...I $$ICD^BGP7UTL2(I,T,1) S C=C+1,BGPY(ED,C)=$$DATE^BGP7UTL(9999999-$P(ED,"."))_"  "_$P($$CPT^ICPTCOD(I,(9999999-$P(ED,"."))),U,2)_"  "_$P($$CPT^ICPTCOD(I,(9999999-$P(ED,"."))),U,3)
 ..;now check V PROCEDURE
 ..S T=$O(^ATXAX("B","BGP CMS CHEST XRAY PROC",0))
 ..I 'T W BGPBOMB
 ..S X=0 F  S X=$O(^AUPNVPRC("AD",V,X)) Q:X'=+X  D
 ...I $$ICD^BGP7UTL2($P(^AUPNVPRC(X,0),U),T,0) S C=C+1,BGPY(ED,C)=$$DATE^BGP7UTL(9999999-$P(ED,"."))_"  "_$P($$ICDOP^BGP7UTL2($P(^AUPNVPRC(X,0),U),(9999999-$P(ED,"."))),U,2)_"  "_$$VAL^XBDIQ1(9000010.08,X,.04)
 ..;now check V POV
 ..S X=0 F  S X=$O(^AUPNVPOV("AD",V,X)) Q:X'=+X  D
 ...S I=$P($G(^AUPNVPOV(X,0)),U)
 ...Q:'I
 ...I $P($$ICDDX^BGP7UTL2(I),U,2)="V72.5",$$UP^XLFSTR($$VAL^XBDIQ1(9000010.07,X,.04))["CHEST" S C=C+1,BGPY(ED,C)=$$DATE^BGP7UTL(9999999-$P(ED,"."))_"  "_$P($$ICDDX^BGP7UTL2(I,(9999999-$P(ED,"."))),U,2)_"  "_$$VAL^XBDIQ1(9000010.07,X,.04)
 ...Q
 ..Q
 .Q
 Q
...
ABGPO(P,BD,ED,BGPY) ;EP
 ...
 .S BGPC=BGPC+1,BGPY(BGPC)="MEASUREMENT O2:  "_$$DATE^BGP7UTL($P(BGPG(X),U))_"  value: "_N
 .Q
 ;now check for cpts
 ...
 .Q:D<BD
 .Q:D>ED
 .S BGPC=BGPC+1,BGPY(BGPC)="CPT: "_$P($$CPT^ICPTCOD(C),U,2)_" "_$P($$CPT^ICPTCOD(C,D),U,3)_"  "_$$DATE^BGP7UTL(D)
 .Q
 ;now check v tran
 S T=$O(^ATXAX("B","BGP CMS ABG CPTS",0))
 ...
 .Q:D<BD
 .Q:D>ED
 .S BGPC=BGPC+1,BGPY(BGPC)="TRAN CODE CPT: "_$P($$CPT^ICPTCOD(C),U,2)_" "_$P($$CPT^ICPTCOD(C,D),U,3)_"  "_$$DATE^BGP7UTL(D)
 .Q
 ...
 ...Q:'$D(^AUPNVLAB(X,0))
 ...I BGPLT,$P(^AUPNVLAB(X,0),U),$D(^ATXLAB(BGPLT,21,"B",$P(^AUPNVLAB(X,0),U))) S BGPC=BGPC+1,BGPY(BGPC)="LAB:  "_$$VAL^XBDIQ1(9000010.09,X,.01)_"  "_$$DATE^BGP7UTL((9999999-D))_"  value: "_$P(^AUPNVLAB(X,0),U,4) Q
 ...Q:'T
 ...S J=$P($G(^AUPNVLAB(X,11)),U,13) Q:J=""
 ...Q:'$$LOINC^BGP7D21(J,T)
 ...S BGPC=BGPC+1,BGPY(BGPC)="LAB:  "_$$VAL^XBDIQ1(9000010.09,X,.01)_"  "_$$DATE^BGP7UTL((9999999-D))_"  value: "_$P(^AUPNVLAB(X,0),U,4)
 ...Q
 Q
ADMDX(H,T) ;EP
 S T=$O(^ATXAX("B",T,0))
 ...
 I '$$ICD^BGP7UTL2(I,T,9) Q ""
 Q 1_U_$P($$ICDDX^BGP7UTL2(I),U,2)
 ;
ERPNEU(P,BDATE,EDATE,T) ;EP - did patient have an er visit from bdate to edate without a DX in taxonomy T?
 ...
 ..I $$ICD^BGP7UTL2(I,T,9) S E=1 Q
 .I 'E S G=0_U_"ER Visit: "_$$DATE^BGP7UTL($P($P(^AUPNVSIT(V,0),U),"."))
 I E Q "1^No"
 Q G
 ;
PNEUMODX(V) ;EP
 ...
 I $$ICD^BGP7UTL2(C,T,9) Q $P($$ICDDX^BGP7UTL2(C),U,2)_" (Primary) "_$$PRIMPOV^APCLV(V,"N")  ;primary dx of pneumonia
 ;PRIMARY of resp failure and seconday of pneumonia
 S T=$O(^ATXAX("B","BGP CMS SEPTI/RESP FAIL DXS",0))
 I '$$ICD^BGP7UTL2(C,T,9) Q ""   ;resp failure not primary pov
 S C=$P($$ICDDX^BGP7UTL2(C),U,2)_" (Primary) "_$$PRIMPOV^APCLV(V,"N")
 S T=$O(^ATXAX("B","BGP CMS PNEUMONIA DXS",0))
 S (X,G)="" F  S X=$O(^AUPNVPOV("AD",V,X)) Q:X'=+X!(G]"")  D
 .Q:'$D(^AUPNVPOV(X,0))
 .Q:$P(^AUPNVPOV(X,0),U,12)="P"
 .S I=$P(^AUPNVPOV(X,0),U)
 .Q:'$$ICD^BGP7UTL2(I,T,9)
 .S G=$P($$ICDDX^BGP7UTL2(I),U,2)_" (Secondary) "_$$VAL^XBDIQ1(9000010.07,X,.04)
 .Q
 I G]"" Q C_U_G
 Q ""
 ;
...
HFDX(V) ;EP
 ...
 I 'T Q
 Q $$ICD^BGP7UTL2(C,T,9)
 ;
COMFORT(P,BDATE,EDATE) ;EP - any V66.7 on this visit? or during hospital stay?
 NEW X
 S X=$$LASTDX^BGP7UTL1(DFN,"BGP PALLIATIVE CARE DXS",BDATE,EDATE)
 I X="" Q ""
 Q $P(X,U,2)_"  "_$$DATE^BGP7UTL($P(X,U,3))
 ;
...
DSCHINST(P,BDATE,EDATE) ;EP - discharge instructions
 ...
 .I E="HF-DCHL" S %=$P(^AUTTEDT(T,0),U,1)_"  "_$$DATE^BGP7UTL($P(BGPG(X),U)) Q
 Q %
