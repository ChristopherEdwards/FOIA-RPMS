...
22 ;BPS to set numerator 2
 S BGPBP=$$MEANBP^BGP7D2(DFN,BGP365,BGPEDATE)
...
23 ;
 ...
 S BGPN3=$P(BGPLDL,U)
 I BGPN3 S BGPVALUE=BGPVALUE_";LDL: "_$$DATE^BGP7UTL($P(BGPLDL,U,2))_" "_$P(BGPLDL,U,3)
...
24 ;micro or pos urine AND GFR
 ...
 ;S BGPGFR=$$GFR^BGP7D21(DFN,BGP365,BGPEDATE)
...
25 ;
 ...
 I BGPN5 S BGPVALUE=BGPVALUE_";EYE: "_$$DATE^BGP7UTL($P(BGPEYE,U,2))_" "_$P(BGPEYE,U,3)
 K BGPG
 K ^TMP($J,"A")
26 ;FOOT EXAM
 S BGPFOOT=$$FOOT(DFN,BGPBDATE,BGPEDATE)
 S BGPN8=$P(BGPFOOT,U)
 I BGPN8 S BGPVALUE=BGPVALUE_";FOOT EXAM: "_$$DATE^BGP7UTL($P(BGPFOOT,U,2))_" "_$P(BGPFOOT,U,3)
...
IOMW ;EP
 ...
 S BGPFRAC=$$FRACTURE^BGP7EL3(DFN,$$FMADD^XLFDT(BGPBDATE,-180),$$FMADD^XLFDT(BGPBDATE,180))
...
IL ;EP
 ...
 I $P(BGPGFR,U) S BGPVALUE=BGPVALUE_"GFR: "_$$DATE^BGP7UTL($P(BGPGFR,U,3))_" "_$P(BGPGFR,U,2)
 K BGPGFR
 Q
...
ASSEV(P,EDATE) ;NOW CHECK ASTHMA PACKAGE SEVERITY
 ...
 ..I S>1 S G="1^Severity > 1 in V Asthma "_$$DATE^BGP7UTL((9999999-D))
 Q G
NDC(A,B) ;
 ;a is drug ien
 ;b is taxonomy ien
...
FOOT(P,BDATE,EDATE) ;
 ...
 S X=0,Y=0 F  S X=$O(^TMP($J,"A",X)) Q:X'=+X!(Y)  S R=$$CLINIC^APCLV($P(^TMP($J,"A",X),U,5),"C") I R=65,'$$DNKA^BGP7D21($P(^TMP($J,"A",X),U,5)) S Y=1,D=$P(^TMP($J,"A",X),U)
 I Y Q 1_"^"_D_"^Cl: "_R
