BGP7D23 ; IHS/CMI/LAB - measure I2 ;
 ;;7.0;IHS CLINICAL REPORTING;;JAN 24, 2007
 ;
IMS ;EP
 S (BGPN1,BGPN2,BGPN3,BGPN4,BGPN5,BGPN6,BGPN7,BGPN8,BGPN9,BGPD1,BGPD2)=0
 S BGPVALUE=""
 I $$LASTDX^BGP7UTL1(DFN,"SURVEILLANCE DIABETES",$P(^DPT(DFN,0),U,3),BGPEDATE) S BGPSTOP=1 Q  ;had diabetes dx
...
IMS1 ;EP
 S BGPN1=$$BPSD(DFN,BGPBDATE,BGPEDATE)
 I BGPN1 S BGPVALUE=BGPVALUE_" 2 BPs"
IMS2 ;LDL done
 S BGPLDL=$$LDL^BGP7D2(DFN,BGPBDATE,BGPEDATE)
 S BGPN2=$P(BGPLDL,U)
 I BGPN2 S BGPVALUE=BGPVALUE_";LDL: "_$$DATE^BGP7UTL($P(BGPLDL,U,2))_" "_$P(BGPLDL,U,3)
 K X,Y,Z,%,A,B,C,D,E,H,BDATE,EDATE,P,V,S,F,BGPLDL,BGPHDL,BGPTRI,BGPLP
IMS3 ;fasting glucose
 S BGPFG=$$FGT(DFN,BGPBDATE,BGPEDATE)
 S BGPN3=$P(BGPFG,U)
 I BGPN3 S BGPVALUE=BGPVALUE_";FG: "_$$DATE^BGP7UTL($P(BGPFG,U,2))
 ;S BGPUP=$$POSUR^BGP7D21A(DFN,BGP365,BGPEDATE)
...
IMS5 ;
 ...
 S BGPTOBV=""
 S BGPTOB=$$TOBACCO^BGP7D7(DFN,BGPBDATE,BGPEDATE)
 S BGPN5=$S(BGPTOB]"":1,1:0),BGPTOBV=$P(BGPTOB,U,2)
 S BGPSDX=$$DX^BGP7D7(DFN,BGPBDATE,BGPEDATE)
 S BGPXPED=$$PED^BGP7D7(DFN,BGPBDATE,BGPEDATE)
 S BGP1320=$$DENT^BGP7D7(DFN,BGPBDATE,BGPEDATE)
 I BGPSDX]"" S BGPN5=1 I BGPTOBV="" S BGPTOBV=$$DATE^BGP7UTL($P(BGPSDX,U,2))
 I BGPXPED]"" S BGPN5=1 I BGPTOBV="" S BGPTOBV=$$DATE^BGP7UTL($P(BGPXPED,U,2))
 I BGP1320]"" S BGPN5=1 I BGPTOBV="" S BGPTOBV=$$DATE^BGP7UTL($P(BGP1320,U,2))
 I BGPN5 S BGPVALUE=BGPVALUE_";TOB SCRN: "_BGPTOBV
IMS6 ;
 S BGPBMI=$$BMI^BGP7D6(DFN,BGPEDATE,BGPAGEE)
 I $P(BGPBMI,U)="" S BGPBMI=$$REF^BGP7D6(DFN,BGPBDATE,BGPEDATE,BGPAGEB)
 I $P(BGPBMI,U)]"" S BGPN6=1
 I BGPN6 S BGPVALUE=BGPVALUE_";BMI: "_$S(BGPBMI["HT":"ref "_$P(BGPBMI,U,2)_" "_$$DATE^BGP7UTL($P(BGPBMI,U,3))_" "_$P(BGPBMI,U,5)_" "_$$DATE^BGP7UTL($P(BGPBMI,U,6)),BGPBMI]"":$$SB^BGP7PDL1($J($P(BGPBMI,U),6,2)),1:"")
IMS7 ;
 S BGPLIFE=$$LIFE^BGP7D41(DFN,BGPBDATE,BGPEDATE)
 I $P(BGPLIFE,U) S BGPN7=1
 I BGPN7 S BGPVALUE=BGPVALUE_";LIFE: "_$$DATE^BGP7UTL($P(BGPLIFE,U,2))_":"_$P(BGPLIFE,U,3)
IMS8 ;
 S BGPDEP=$$DEP^BGP7D21(DFN,BGP365,BGPEDATE) I $P(BGPDEP,U)=1 S BGPN8=1
 S BGPDEPS=$$DEPSCR^BGP7D21(DFN,BGP365,BGPEDATE) I $P(BGPDEPS,U)=1 S BGPN8=1
 S BGPREF="" I 'BGPN8 S BGPREF=$$DEPREF^BGP7D21(DFN,BGP365,BGPEDATE) I $P(BGPREF,U)=1 S BGPN8=1
...
MS(P,BDATE,EDATE) ;EP
 ...
 .Q
 K ^TMP($J,"A")
 I $P(G,U)>1 Q 1_U_"277.7 on "_$$DATE^BGP7UTL($P(G,U,2))_"; "_$$DATE^BGP7UTL($P(G,U,3))
 ;now check for 3 or more of the following
 S BGPC=0,BGPK=""
 S X=$$BMI^BGP7D6(P,EDATE,$$AGE^AUPNPAT(P,EDATE))
 ...
 S BGPHTN=$$LASTDX^BGP7UTL1(P,"SURVEILLANCE HYPERTENSION",BGP365,EDATE)
 S BGPMBP=$$MEANBP(P,BGPBDATE,BGPEDATE)
 I $P(BGPHTN,U)!(BGPMBP]"") S BGPC=BGPC+1 S:BGPK]"" BGPK=BGPK_"; " S:$P(BGPHTN,U)=1 BGPK=BGPK_"HTN DX: "_$$DATE^BGP7UTL($P(BGPHTN,U,3)) I BGPMBP]"" S:BGPK]""&($P(BGPHTN,U)=1) BGPK=BGPK_"; " S BGPK=BGPK_BGPMBP
...
TRIG(P,BDATE,EDATE) ;EP
 ...
 ....S V=$P(^AUPNVLAB(X,0),U,4) Q:+V<150  S R="TRIG="_V
 ...Q:'T
...
HDL(P,BDATE,EDATE) ;EP
 ...
 .S L=0 F  S L=$O(^AUPNVLAB("AE",P,D,L)) Q:L'=+L!(BGPC1)  D
 ..S X=0 F  S X=$O(^AUPNVLAB("AE",P,D,L,X)) Q:X'=+X!(BGPC1)  D
 ...Q:'$D(^AUPNVLAB(X,0))
 ...I BGPLT,$P(^AUPNVLAB(X,0),U),$D(^ATXLAB(BGPLT,21,"B",$P(^AUPNVLAB(X,0),U))) D  Q:R]""
