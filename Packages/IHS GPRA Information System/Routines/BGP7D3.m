...
I11 ;EP
 ...
 I BGPN1>0 S BGPD1=1
 I BGPN1>4 S BGPN1=4
 S BGPVALUE="UP|||"_BGPN1_$S($P(BGPFLUOR,U,3)]"":" "_$P(BGPFLUOR,U,3),1:" topical flouride ")
 K ^TMP($J,"A"),BGPFLUOR
 Q
I12 ;EP
...
I13 ;EP
 ...
 I BGPRTYPE'=3 S BGPVALUE=BGPDV_"|||"_BGPVALUE_$S(BGPVALUE]"":" (ever)",1:"")_" "_BGPVAL_$S(BGPVAL]"":" (past 5 yrs)",1:"")
 Q
I15 ;EP
 ...
 I BGPRTYPE'=3 S BGPVALUE=$S(BGPI7DA:"UP",1:"")_$S(BGPI7DB:",AC",1:"")_"|||"_$$DATE^BGP7UTL($P(BGPPAP,U,2))_" "_$P(BGPPAP,U,3)
...
SEAL(P,BDATE,EDATE) ;
 ...
 S G=$$REFUSAL^BGP7UTL1(P,9999999.31,$O(^AUTTADA("B","1351",0)),BDATE,EDATE)
...
TF(P,BDATE,EDATE) ;
 ...
 S G=$$REFUSAL^BGP7UTL1(P,9999999.31,$O(^AUTTADA("B","1201",0)),BDATE,EDATE)
 I $P(G,U)=1 Q "1^"_$P(G,U,2)_"^Refused ADA 1201"
 S G=$$REFUSAL^BGP7UTL1(P,9999999.31,$O(^AUTTADA("B","1203",0)),BDATE,EDATE)
 I $P(G,U)=1 Q "1^"_$P(G,U,2)_"^Refused ADA 1203"
 S G=$$REFUSAL^BGP7UTL1(P,9999999.31,$O(^AUTTADA("B","1204",0)),BDATE,EDATE)
 I $P(G,U)=1 Q "1^"_$P(G,U,2)_"^Refused ADA 1204"
 S G=$$REFUSAL^BGP7UTL1(P,9999999.31,$O(^AUTTADA("B","1205",0)),BDATE,EDATE)
 I $P(G,U)=1 Q "1^"_$P(G,U,2)_"^Refused ADA 1205"
 S G=$$REFUSAL^BGP7UTL1(P,9999999.31,$O(^AUTTADA("B","1206",0)),BDATE,EDATE)
...
FLU(P,ED) ;EP
 ...
 I $D(BGPG(1)) Q $$DATE^BGP7UTL($P(BGPG(1),U))_" Imm 88"
 S X=P_"^LAST IMM 111;DURING "_BDATE_"-"_EDATE S E=$$START1^APCLDF(X,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP7UTL($P(BGPG(1),U))_" Imm 111"
 S X=P_"^LAST IMM 15;DURING "_BDATE_"-"_EDATE S E=$$START1^APCLDF(X,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP7UTL($P(BGPG(1),U))_" Imm 15"
 S X=P_"^LAST IMM 16;DURING "_BDATE_"-"_EDATE S E=$$START1^APCLDF(X,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP7UTL($P(BGPG(1),U))_" Imm 16"
 S BGPG=$$LASTPRCI^BGP7UTL1(P,"99.52",BDATE,EDATE)
 I $P(BGPG,U,1)=1 Q $$DATE^BGP7UTL($P(BGPG,U,3))_" 99.52"
 K BGPG S %=P_"^LAST DX V04.8;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP7UTL($P(BGPG(1),U))_" V04.8"
 K BGPG S %=P_"^LAST DX V04.81;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP7UTL($P(BGPG(1),U))_" V04.81"
 K BGPG S %=P_"^LAST DX V06.6;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP7UTL($P(BGPG(1),U))_" V06.6"
 S T=$O(^ATXAX("B","BGP CPT FLU",0))
 I T D  I X]"" Q $$DATE^BGP7UTL(X)_" CPT"
 .S X=$$CPT^BGP7DU(P,,ED,T,3) I X]"" Q
 .S X=$$TRAN^BGP7DU(P,,ED,T,3)
 S G=$$REFUSAL^BGP7UTL1(P,9999999.14,$O(^AUTTIMM("C",88,0)),$$FMADD^XLFDT(ED,-365),ED)
 I $P(G,U)=1 Q $$DATE^BGP7UTL($P(G,U,2))_" Refused"
 S G=$$REFUSAL^BGP7UTL1(P,9999999.14,$O(^AUTTIMM("C",111,0)),$$FMADD^XLFDT(ED,-365),ED)
 I $P(G,U)=1 Q $$DATE^BGP7UTL($P(G,U,2))_" Refused"
 S G=$$REFUSAL^BGP7UTL1(P,9999999.14,$O(^AUTTIMM("C",15,0)),$$FMADD^XLFDT(ED,-365),ED)
 I $P(G,U)=1 Q $$DATE^BGP7UTL($P(G,U,2))_" Refused"
 S G=$$REFUSAL^BGP7UTL1(P,9999999.14,$O(^AUTTIMM("C",16,0)),$$FMADD^XLFDT(ED,-365),ED)
 I $P(G,U)=1 Q $$DATE^BGP7UTL($P(G,U,2))_" Refused"
 ...
 .Q:$P(^BIPC(X,0),U,4)>ED
 .S G=1
 I G Q $$DATE^BGP7UTL($P(G,U,2))_" Refused Imm pkg"
...
PAP(P,EDATE,YEARS) ;
 ...
 S %=P_"^LAST DX V72.3;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT($S(EDATE<3041001:EDATE,1:3040930)),E=$$START1^APCLDF(%,"BGP(")
 ...
 S %=P_"^LAST DX 795.06;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BGP(")
 I $D(BGP(1)) Q "1^"_$P(BGP(1),U)_"^795.06"
 ...
 .S X=$$CPT^BGP7DU(P,$$FMADD^XLFDT(EDATE,-(3*365)),EDATE,T,5) I X]"" Q
 .S X=$$TRAN^BGP7DU(P,$$FMADD^XLFDT(EDATE,-(3*365)),EDATE,T,5)
 S T="PAP SMEAR",T=$O(^BWPN("B",T,0))
 I T D  I X]"" Q "1^"_X_"^WH"
 .S X=$$WH^BGP7DU(P,$$FMADD^XLFDT(EDATE,-(3*365)),BGPEDATE,T,3)
