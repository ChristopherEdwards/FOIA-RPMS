...
II ;EP
 ...
 S BGPVALUE=$S(BGPD2:"AC",1:"")_"|||"_$S(BGPN1:$$LAST(DFN,BGP365,BGPEDATE),1:"")_" "_$S(BGPHOSP:"H "_$$DATE^BGP7UTL($P(BGPHOSP,U,2)),1:"")
...
PNEU(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(1)) Q $$DATE^BGP7UTL($P(BGPG(1),U))_" Imm 33"
 S X=P_"^LAST IMM 100;DURING "_BDATE_"-"_EDATE S E=$$START1^APCLDF(X,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP7UTL($P(BGPG(1),U))_" Imm 100"
 S X=P_"^LAST IMM 109;DURING "_BDATE_"-"_EDATE S E=$$START1^APCLDF(X,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP7UTL($P(BGPG(1),U))_" Imm 109"
 K BGPG S %=P_"^LAST PROCEDURE 99.55;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP7UTL($P(BGPG(1),U))_" 99.55"
 K BGPG S %=P_"^LAST DX V03.82;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP7UTL($P(BGPG(1),U))_" V03.82"
 K BGPG S %=P_"^LAST DX V03.89;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP7UTL($P(BGPG(1),U))_" V03.89"
 K BGPG S %=P_"^LAST DX V06.6;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q $$DATE^BGP7UTL($P(BGPG(1),U))_" V06.6"
 S %="",E=+$$CODEN^ICPTCOD(90732),%=$$CPTI^BGP7DU(P,BD,ED,E) I %]"" Q $$DATE^BGP7UTL($P(%,U,2))_" 90732"
 S %="",E=+$$CODEN^ICPTCOD(90669),%=$$CPTI^BGP7DU(P,BD,ED,E) I %]"" Q $$DATE^BGP7UTL($P(%,U,2))_" 90669"
 S %="",E=+$$CODEN^ICPTCOD(90732),%=$$TRANI^BGP7DU(P,BD,ED,E) I %]"" Q $$DATE^BGP7UTL($P(%,U,2))_" 90732 TRAN"
 S %="",E=+$$CODEN^ICPTCOD(90669),%=$$TRANI^BGP7DU(P,BD,ED,E) I %]"" Q $$DATE^BGP7UTL($P(%,U,2))_" 90669 TRAN"
 S G=$$REFUSAL^BGP7UTL1(P,9999999.14,$O(^AUTTIMM("C",33,0)),$$FMTE^XLFDT($$FMADD^XLFDT(ED,-365)),EDATE)
 I $P(G,U)=1 Q $$DATE^BGP7UTL($P(G,U,2))_" Refused"
 S G=$$REFUSAL^BGP7UTL1(P,9999999.14,$O(^AUTTIMM("C",100,0)),$$FMTE^XLFDT($$FMADD^XLFDT(ED,-365)),EDATE)
 I $P(G,U)=1 Q $$DATE^BGP7UTL($P(G,U,2))_" Refused"
 S G=$$REFUSAL^BGP7UTL1(P,9999999.14,$O(^AUTTIMM("C",109,0)),$$FMTE^XLFDT($$FMADD^XLFDT(ED,-365)),EDATE)
 I $P(G,U)=1 Q $$DATE^BGP7UTL($P(G,U,2))_" Refused"
 ...
 .Q:$P(^BIPC(X,0),U,4)>ED
 .S G=1
 I G Q $$DATE^BGP7UTL($P(G,U,2))_" Refusal Imm pkg"
