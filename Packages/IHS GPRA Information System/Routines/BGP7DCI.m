BGP7DCI ; IHS/CMI/LAB - IHS area GPRA ; 08 Dec 2006  9:12 AM
 ;;7.0;IHS CLINICAL REPORTING;;JAN 24, 2007
 ;
CALCIND ;EP
 S BGPIC=0 F  S BGPIC=$O(BGPIND(BGPIC)) Q:BGPIC'=+BGPIC  D
 .I BGPRTYPE=1,$P(^BGPINDA(BGPIC,0),U,7)'=1 Q  ;national gpra report
 .I BGPRTYPE=4,$P(^BGPINDA(BGPIC,0),U,11)'=1 Q  ;local
 ...
 .K BGPNUMV
 .K ^TMP($J)
 .I $D(^BGPINDA(BGPIC,1)) X ^BGPINDA(BGPIC,1)
 .K BGPVAL,BGPG,BGPC,BGPALLED,BGPV,A,B,C,D,E,F,G,H,I,J,K,M,N,O,P,Q,R,S,T,V,W,X,Y,Z
 .K ^TMP($J)
 .;Set up global for iCare
 .I $G(BQIGREF)'="",$D(BGPSTOP) S @BQIGREF@(DFN,BGPIC)=$P(^BGPINDA(BGPIC,0),U,3)_"^N/A" Q
 .I $D(BGPSTOP) Q  ;no need to set since no num/denom
 .;loop each individual to set numerator and denominator
 .S BGPI=0 F  S BGPI=$O(^BGPINDAC("B",BGPIC,BGPI)) Q:BGPI'=+BGPI  D
 ..S (BGPNUM,BGPDEN)=0
 ..X ^BGPINDAC(BGPI,1)
 ..X ^BGPINDAC(BGPI,2) ;denominator 1 or 0
 ..;set field counter
 ..S BGPNF=$P(^BGPINDAC(BGPI,0),U,9)
 ..S BGPN=$P(^DD(90530.03,BGPNF,0),U,4),N=$P(BGPN,";"),P=$P(BGPN,";",2)
 ..D S(BGPRPT,BGPGBL,N,P,BGPNUM),S1("N",BGPNUM)
 ..S BGPDF=$P(^BGPINDAC(BGPI,0),U,8)
 ..S BGPN=$P(^DD(90530.03,BGPDF,0),U,4),N=$P(BGPN,";"),P=$P(BGPN,";",2)
...
CPL ;comprehensive pat list check and set xtmp
 ...
 Q:BGPTIME'=1
 Q:$P($G(^BGPINDAC(BGPI,12)),U,1)=""
 X ^BGPINDAC(BGPI,3) Q:'$T
 S C=$S($P($G(^AUPNPAT(DFN,11)),U,18)]"":$P(^AUPNPAT(DFN,11),U,18),1:"UNKNOWN")
 S S=$P(^DPT(DFN,0),U,2)
 S D=$P(BGPVALUE,"|||")
 S F=$P($G(^XTMP("BGP07CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN)),"|||")
 I D["UP" S $P(F,"$$",1)="UP"
 I D["AC" S $P(F,"$$",2)="AC"
 I D["AD" S $P(F,"$$",3)="AD"
 I D["AAD" S $P(F,"$$",4)="AAD"
 ...
 I D["IMM" S $P(F,"$$",6)="IMM"
 I D["IHD" S $P(F,"$$",7)="IHD"
 I '$D(^XTMP("BGP07CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN)) D  Q
 .S ^XTMP("BGP07CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN)=F_"|||"_$P(^BGPINDAC(BGPI,12),U),BGPCPLC=BGPCPLC+1
 S $P(^XTMP("BGP07CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||")=F
 S $P(^XTMP("BGP07CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||",2)=$P(^XTMP("BGP07CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||",2)_"#"_$P(^BGPINDAC(BGPI,12),U)
 Q
NPL ;
 Q:BGPTIME'=1
 ...
 .X ^BGPANPL(BGPX,12) K ^TMP($J) Q:'$T
 .S BGPINDL(BGPIC,BGPX)=$G(BGPINDL(BGPIC,BGPX))+1
 .I $G(BGP7NPLT) S ^XTMP("BGP7DNP",BGPJ,BGPH,"LIST",BGPIC,BGPX,DFN)="" Q
 .S ^XTMP("BGP7DNP",BGPJ,BGPH,"LIST",BGPIC,BGPX,$S($P($G(^AUPNPAT(DFN,11)),U,18)]"":$P(^AUPNPAT(DFN,11),U,18),1:"UNKNOWN"),$P(^DPT(DFN,0),U,2),BGPAGEB,DFN)=$G(BGPVALUE)
...
S1(BQITYP,BQIVAL) ; Return data by patient for iCare into global reference BQIGREF
 ...
 S BQIIDTA=$G(^BGPINDAC(BGPI,0))
 S BQIDTA=$G(^BGPINDAC(BGPI,14))
 ;
 ...
 S $P(@BQIGREF@(DFN,BGPIC),U,1)=$P(^BGPINDA(BGPIC,0),U,3)
