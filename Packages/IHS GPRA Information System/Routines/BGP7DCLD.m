...
GO ;
 ...
 S X=" " D S(X,3,1)
 D HEADER
 S BGPY=$O(^BGPCTRL("B",2007,0))
 S BGPX=0 F  S BGPX=$O(^BGPCTRL(BGPY,28,BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 .S X=^BGPCTRL(BGPY,28,BGPX,0) D S(X,1,1)
 D H1
 S BGPH1P=0 S BGPCOM="",BGPCOUNT=0 F  S BGPCOM=$O(^XTMP("BGP07CPL",BGPJ,BGPH,"LIST",BGPCOM)) Q:BGPCOM=""  D CPL1
 S X="Total # of patients on list: "_$G(BGPPCNT) D S(X,1,1)
 Q
CPL1 ;EP
 S BGPSEX="" F  S BGPSEX=$O(^XTMP("BGP07CPL",BGPJ,BGPH,"LIST",BGPCOM,BGPSEX)) Q:BGPSEX=""  D CPL2
 Q
CPL2 ;
 S BGPAGE="" F  S BGPAGE=$O(^XTMP("BGP07CPL",BGPJ,BGPH,"LIST",BGPCOM,BGPSEX,BGPAGE)) Q:BGPAGE=""  D
 .S DFN=0 F  S DFN=$O(^XTMP("BGP07CPL",BGPJ,BGPH,"LIST",BGPCOM,BGPSEX,BGPAGE,DFN)) Q:DFN'=+DFN  S BGPCOUNT=BGPCOUNT+1 D PRINTL
 Q
...
PRINTL ;print one line
 ...
 S W="",X=$P(^XTMP("BGP07CPL",BGPJ,BGPH,"LIST",BGPCOM,BGPSEX,BGPAGE,DFN),"|||",1) F Y=1:1:12 I $P(X,"$$",Y)]"" S:W]"" W=W_"," S W=W_$P(X,"$$",Y)
 S Z="",X=$P(^XTMP("BGP07CPL",BGPJ,BGPH,"LIST",BGPCOM,BGPSEX,BGPAGE,DFN),"|||",2) F Y=1:1  Q:$P(X,"#",Y)=""  S:Z]"" Z=Z_", " S Z=Z_$P(X,"#",Y)
 D S(W,,6),S(Z,,7)
...
HEADER ;EP
 ...
 S X="***  IHS Comprehensive National GPRA Patient List ***" D S(X,1,1)
