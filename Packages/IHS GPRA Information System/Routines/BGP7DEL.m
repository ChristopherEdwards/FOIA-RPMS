BGP7DEL ; IHS/CMI/LAB - IHS GPRA 07 REPORT DRIVER ;
 ;;7.0;IHS CLINICAL REPORTING;;JAN 24, 2007
 ;
 ;
 W:$D(IOF) @IOF
 W !!,$$CTR("2007 Elder Care Clinical Performance Measure Report",80)
INTRO ;
 D XIT
 ...
 W !,"to compare data to, and 3) the Community taxonomy to determine which"
 ...
 W !!,"There are 26 measures in the Elder Care Performance Measure Report."
 D XIT
 K DIR
 ...
 I BGPZZ="A" S X=0 F  S X=$O(^BGPELIA(X)) Q:X'=+X  S BGPIND(X)=""
 D TAXCHK^BGP7XTEL
TP ;get time period
 S BGPRTYPE=5
 S (BGPBD,BGPED,BGPTP)=""
...
SUM ;display summary of this report
 ...
 W:$D(IOF) @IOF
 W !,$$CTR("SUMMARY OF FY 07 ELDER REPORT TO BE GENERATED")
 W !!,"The date ranges for this report are:"
 W !?5,"Report Period: ",?31,$$FMTE^XLFDT(BGPBD)," to ",?31,$$FMTE^XLFDT(BGPED)
 W !?5,"Previous Year Period: ",?31,$$FMTE^XLFDT(BGPPBD)," to ",?31,$$FMTE^XLFDT(BGPPED)
 W !?5,"Baseline Period: ",?31,$$FMTE^XLFDT(BGPBBD)," to ",?31,$$FMTE^XLFDT(BGPBED)
...
ZIS ;call to XBDBQUE
 ...
 I BGPRPT="" D XIT Q
 I BGPEXPT D
 .W !!,"A file will be created called BG07",$P(^AUTTLOC(DUZ(2),0),U,10)_".EL"_BGPRPT," and will reside",!,"in the ",BGPUF," directory.",!
 ...
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPELDCA(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPELDPA(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPELDBA(" D ^DIK K DIK D XIT Q
 I $D(IO("Q")) G TSKMN
DRIVER ;
 D ^BGP7D1
 U IO
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP7DEL",ZTDTH="",ZTDESC="ELDER 05 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
