BGP7DMT ; IHS/CMI/LAB - national patient list 20 Dec 2004 9:24 AM ;
...
QUE ;
 ...
 S ZTRTN="PRINT^BGP7DMT",ZTDESC="BGP 07 MED TAX REPORT",ZTIO=ION,ZTDTH=""
 D ^%ZTLOAD
 D HOME^%ZIS
 D XIT
 Q
...
PRINT ;
 ...
 K BGPRT,BGPNO S BGPRT(1)=""
 D N
 Q:BGPQUIT
 S BGPSUBH="CRS TAXONOMIES EXCLUDING NATIONAL GPRA REPORT TAXONOMIES"
 K BGPRT,BGPNO S BGPRT(3)="",BGPRT(4)="",BGPRT(2)="",BGPRT(5)="",BGPNO(1)=""
 W !!
 D N
 Q:BGPQUIT
...
N ;GATHER UP AND DISPLAY ALL NATIONAL GPRA
 ...
 W !,BGPSUBH
 S BGPTNAME="" F  S BGPTNAME=$O(^BGPTAXA("B",BGPTNAME)) Q:BGPTNAME=""!(BGPQUIT)  D
 .S BGPTIEN=0,BGPTIEN=$O(^BGPTAXA("B",BGPTNAME,BGPTIEN))
 .Q:'BGPTIEN  ;oops, error in xref
 .Q:'$D(^BGPTAXA(BGPTIEN,0))  ;oops, error in xref
 .Q:$P(^BGPTAXA(BGPTIEN,0),U,2)'="M"  ;only DRUG/MEDICATION taxonomies in this report
 .S (G,X)=0 F  S X=$O(^BGPTAXA(BGPTIEN,12,"B",X)) Q:X'=+X!(G)  I $D(BGPRT(X)) S G=1
 .Q:'G
 .;now eliminate those in BGPNO
 .S (G,X)=0 F  S X=$O(^BGPTAXA(BGPTIEN,12,"B",X)) Q:X'=+X!(G)  D
 ..I $D(BGPNO(X)) S G=1
 .Q:G
...
INTROT ;introductory text
 ...
 ;;CRS 2007.  The report is organized by (1) taxonomies included in the
