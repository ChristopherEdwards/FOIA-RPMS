BGP7DNE9 ; IHS/CMI/LAB - NATL COMP EXPORT 17 Jul 2008 9:24 AM ; 
...
INTRO ;
 ...
 S X=0 F  S X=$O(^BGPCTRL(BGPXPFYC,78,X)) Q:X'=+X  D
 .I $Y>(IOSL-3) D EOP W @IOF
 .W !,^BGPCTRL(BGPXPFYC,78,X,0)
 K DIR S DIR(0)="E",DIR("A")="Press enter to continue: " D ^DIR K DIR
 D TAXCHK^BGP7XTCN
TP ;
 S BGPRTYPE=1,BGPXPRP=1,BGPXPFYY=2009,BGPXPFYI=309
 S BGPXPBD=3080701,BGPXPEDT=3090630
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP7DNE9",ZTDTH="",ZTDESC="NATIONAL GPRA REPORT 09" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
CNTSF ;EP
 ...
 .S P=13 F X=2008:1:BGPXPFYY S $P(Y,U,P)="GPRA YR "_X,P=P+1
 .W Y,!
...
CNTSF1 ;EP
 ...
 S BGPC=0
 S Y="SITE NAME^ASUFAC^DATE FILE RUN^FILE BEGIN DATE^FILE END DATE^PATIENT UNIQUE REGISTRATION ID^DOB^GENDER^STATE^COUNTY^MEASURE ID^MEASURE"
 S P=13 F X=2009:1:BGPXPFYY S $P(Y,U,P)="GPRA YR "_X,P=P+1
 S BGPC=BGPC+1
 S ^BGPDATA(BGPC)=Y
 S BGPX=0
...
HEADER1 ;
 ...
 W $$CTR("*** IHS 2007 Comprehensive National GPRA Export for 2009***",80),!
 W !,$$CTR("CRS 2007, Version 7.0, Updated 2009",80)
