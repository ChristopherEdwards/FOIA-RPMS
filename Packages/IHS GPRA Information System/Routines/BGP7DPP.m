BGP7DPP ; IHS/CMI/LAB - IHS GPRA 07 SELECTED REPORT DRIVER ALL PATIENTS ;
 ;;7.0;IHS CLINICAL REPORTING;;JAN 24, 2007
 ;
 ;
 W:$D(IOF) @IOF
 W !!,$$CTR("2007 Clinical Performance Measure Report (Selected Measures)",80)
...
SUM ;display summary of this report
 ...
 W !!,"These measures will be calculated: " S X=0 F  S X=$O(BGPIND(X)) Q:X'=+X  W $P(^BGPINDA(X,0),U,3)," ; "
...
EI ;
 ...
 Q
PI ;
 S X=0 F  S X=$O(^BGPINDAC("APR",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDAC(X,0),U,1))=""
 Q
SI ;
 K BGPIND
 D EN^BGP7DSI
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP7DPP",ZTDTH="",ZTDESC="GPRA 07 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
NODEV ;
