BGP7EL4 ; IHS/CMI/LAB - measure 1,2,3,4 05 Apr 2007 1:44 PM ;
...
I18 ;EP
 ...
 S BGPVALUE=$$CHOL^BGP7D73(DFN,$$FMADD^XLFDT(BGPEDATE,-(5*365)),BGPEDATE)
...
TXBMD(P,BDATE,EDATE,HOSP) ;EP
 ...
 S HOSP=$G(HOSP)
 K BGPG
 S BGPG=$$LASTPRC^BGP7UTL1(P,"BGP BMD PROCEDURES",BDATE,EDATE)
 I $P(BGPG,U)=1 Q 1_U_"bmd proc "_$P(BGPG,U,2)_" "_$$DATE^BGP7UTL($P(BGPG,U,3))
 ;now check dx
 S BGPG=$$LASTDX^BGP7UTL1(P,"BGP BMD DXS",BDATE,EDATE)
 I $P(BGPG,U)=1 Q 1_U_"bmd DX "_$P(BGPG,U,2)_" "_$$DATE^BGP7UTL($P(BGPG,U,3))
 ;now check cpts
 S T=$O(^ATXAX("B","BGP BMD CPTS",0))
 S BGPG=$$CPT^BGP7DU(P,BDATE,EDATE,T,5)
 I BGPG]"" Q 1_U_$P(BGPG,U,2)_" "_$$DATE^BGP7UTL($P(BGPG,U,1)) ;had a cpt
 S T=$O(^ATXAX("B","BGP BMD CPTS",0))
 S BGPG=$$TRAN^BGP7DU(P,BDATE,EDATE,T,5)
 I BGPG]"" Q 1_U_$P(BGPG,U,2)_" "_$$DATE^BGP7UTL($P(BGPG,U,1)) ;had a TRAN
 ;now check RAD
 S T=$O(^ATXAX("B","BGP BMD CPTS",0))
 S BGPG=$$RAD^BGP7DU(P,BDATE,EDATE,T,5)
 I BGPG]"" Q 1_U_$P(BGPG,U,2)_" "_$$DATE^BGP7UTL($P(BGPG,U,1))  ;had a cpt
 I HOSP Q ""
 ;now check all meds
 K ^TMP($J,"MEDS")
 ...
 .Q
 K ^TMP($J,"MEDS")
 I G Q 1_U_"osteo med: "_$$DATE^BGP7UTL(G)
 Q ""
