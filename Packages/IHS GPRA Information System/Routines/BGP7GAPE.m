...
TESTNTL ;
 ...
 S LORISUL(1)=""
 S LORISUL(2)=""
 D EP(.ERR,1,2522,"BGP 07 AREA PAT ED RPT","A",.LORISUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
...
EP1 ;
 ...
 S BGPSUCNT=C
 S BGPSUNM=""
 I BGPSUCNT=1 S Y=$O(BGPSUL(0)),X=$P(^BGPPEDCA(Y,0),U,9) I X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOELD^BGP7GAPE",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI AREA PAT ED REPORT" D ^%ZTLOAD Q
 Q
AOELD ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 ;cmi/anch/maw added 5/12/2007 for word output
 D GUIR^XBLM("PRINT^BGP7PPEL","^TMP($J,""BGPGUI"",")
