...
TESTCMP ;
 S ERR=""
 D EP(.ERR,1,2522,"BGP 07 COMPREHENSIVE PAT LIST","A",,,1,3050000,338,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPLIST,BGPLPRV,BGPLPROV,BGPQTR,BGPPER,BGPTAXI,BGPROT,BGPRTIME,BGPMFITI) ;EP - called from GUI to produce national gpra report (NTL-GP)
 ...
 ;           with a 1 through 4  e.g  3060000 (fileman imprecise date for 2007)
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="NTLCMP^BGP7GCMP",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI COMPREHENSIVE PATIENT LIST 07" D ^%ZTLOAD Q
 Q
NTLCMP ;
 S BGPCPLC=0
 D ^BGP7D1
