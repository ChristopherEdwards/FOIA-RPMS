BGP7GCMS ; IHS/CMI/LAB - GUI CMS REPORT 27 Apr 2007 10:30 PM ; 
...
TESTNTL ;
 ...
 D EP(.ERR,1,2522,"BGP 07 CMS PERF RPT",2522,.LORIND,1,3020000,$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPHOSP,BGPIND,BGPQTR,BGPPER,BGPRTIME,BGPUDBD,BGPED) ;EP - called from GUI to produce COM REPORT CI05-RPT-LOC-CMS
 ...
 ;
 ;  BGPIND - array of iens from items selected from ^BGPCMSIA GLOBAL
 ;
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LOCCMS^BGP7GCMS",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI 06 CMS LIST" D ^%ZTLOAD Q
 Q
LOCCMS ;
 D ^BGP7C1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^XBLM("^BGP7CP","^TMP($J,""BGPGUI"",")
