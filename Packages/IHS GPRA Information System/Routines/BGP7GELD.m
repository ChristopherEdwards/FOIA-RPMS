...
TESTELD ;
 ...
 S LORIND(4)="",LORIND(3)=""
 S LORILIST(3)=""
 D EP(.ERR,1,2522,"BGP 07 ELDER REPORT",338,"S",.LORIND,1,3040000,3000000,1,"A",.LORILIST,"","",1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPZZ,BGPIND,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPLSTT,BGPLIST,BGPLPRV,BGPLPROV,BGPEXPT,BGPROT,BGPRTIME,BGPMFITI) ;EP - called from GUI to produce COM REPORT CI05-RPT-LOC-COM
 ...
 ;                        1 and 6 from the BGP 07 ELDER INDICATORS file (^BGPELIA GLOBAL).  When
...
BY ;get baseline year
 ...
 S BGPFILEN=""
 I BGPEXPT S BGPFILEN="BG07"_$P(^AUTTLOC(DUZ(2),0),U,10)_".EL"_BGPRPT_" in directory "_BGPUF
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="OTHELD^BGP7GELD",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI ELDER 05 REPORT" D ^%ZTLOAD Q
 Q
OTHELD ;
 D ^BGP7D1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^XBLM("^BGP7DELP","^TMP($J,""BGPGUI"",")
