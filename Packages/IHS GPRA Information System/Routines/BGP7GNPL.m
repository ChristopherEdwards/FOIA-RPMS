BGP7GNPL ; IHS/CMI/LAB - IHS Diabetes Audit 2003 ;
...
TESTNTL ;
 ...
 D EP(.ERR,1,2522,"BGP 07 NATIONAL PAT LISTS",338,.LORIND,.LORIINDL,1,3040000,"A","","","B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPIND,BGPINDL,BGPQTR,BGPPER,BGPLIST,BGPLPRV,BGPLPROV,BGPROT,BGPRTIME,BGPMFITI) ;EP - called from GUI to produce national gpra report (NTL-GP)
 ...
 ;                        1 and 6 from the BGP 07 INDICATORS file.  When
 ;                        you present them to the user for selection use the
 ;                        ^BGPINDA("AGPRA",1,ien) xref as the ones with a second
 ...
 ;            entries from BGP 07 NATIONAL PATIENT LISTS that point to that
 ;            measure by using the "B" index on ^BGPANPL(.  ^BGPANPL("B",measure ien,ien)
...
EP1 ;
 ...
 S BGPGUI=1
 S IOM=80,BGPIOSL=55
 S BGPRTYPE=1,BGP7RPTH="",BGPNPL=1,BGPINDT="G",BGP7GPU=1
 K BGPTAX S X=0
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="NTLGP^BGP7GNPL",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT LISTS 05" D ^%ZTLOAD Q
 Q
NTLGP ;
 D ^BGP7D1
 K ^TMP($J,"BGPGUI")
