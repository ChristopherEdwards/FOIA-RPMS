...
TESTELD ;
 ...
 S LORIND(4)="",LORIND(3)=""
 S LORILIST(3)=""
 D EP(.ERR,1,2522,"BGP 07 PAT ED REPORT",338,"S",.LORIND,1,3040000,3000000,1,"A",.LORILIST,"","",1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
...
EP1 ;
 ...
 I BGPZZ="A" S X=0 F  S X=$O(^BGPPEIA(X)) Q:X'=+X  S BGPIND(X)=""
 I BGPZZ='"A" S BGPEXPT=""
...
BY ;get baseline year
 ...
 S BGPFILEN=""
 I BGPEXPT S BGPFILEN="BG07"_$P(^AUTTLOC(DUZ(2),0),U,10)_".PED"_BGPRPT_" in directory "_BGPUF
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="OTHPED^BGP7GPED",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI PAT ED REPORT" D ^%ZTLOAD Q
 Q
OTHPED ;
 D ^BGP7DPE1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^XBLM("^BGP7DPEP","^TMP($J,""BGPGUI"",")
