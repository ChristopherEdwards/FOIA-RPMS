...
GI(RETVAL,BGPSTR) ;-- get GPRA measures
 ...
 S X=0 F  S X=$O(^BGPINDA("AO",X)) Q:X'=+X  D
 . S Y=0 F  S Y=$O(^BGPINDA("AO",X,Y)) Q:Y'=+Y  D
 .. Q:$P(^BGPINDE(Y,0),U,7)'=1
 .. S BGPI=BGPI+1
 .. S ^BGPTMP($J,BGPI)=Y_U_$P($G(^BGPINDA(Y,0)),U,4)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
GSI(RETVAL,BGPSTR) ;-- get sub measure based on passed in measure
 ...
 S X=0 F  S X=$O(^BGPANPL("B",BGPIND,X)) Q:'X  D
 . S BGPI=BGPI+1
 . S ^BGPTMP($J,BGPI)=X_U_$P($G(^BGPANPL(X,0)),U,3)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
...
ECALL(RETVAL) ;-- get all elder care measures
 ...
 S BGPI=0
 S ^BGPTMP($J,BGPI)="T00007BMXIEN^T00050Measure"_$C(30)
 S X=0 F  S X=$O(^BGPELIA("AO",X)) Q:X'=+X  D
 . S Y=0 F  S Y=$O(^BGPELIA("AO",X,Y)) Q:Y'=+Y  D
 .. ;Q:$P(^BGPINDA(Y,0),U,7)'=1
 .. S BGPI=BGPI+1
 .. S ^BGPTMP($J,BGPI)=Y_U_$P($G(^BGPELIA(Y,0)),U,4)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
PCALL(RETVAL) ;-- get all patient education measures
 ...
 S BGPI=0
 S ^BGPTMP($J,BGPI)="T00007BMXIEN^T00050Measure"_$C(30)
 S X=0 F  S X=$O(^BGPPEIA("AO",X)) Q:X'=+X  D
 . S Y=0 F  S Y=$O(^BGPPEIA("AO",X,Y)) Q:Y'=+Y  D
 .. ;Q:$P(^BGPINDS(Y,0),U,7)'=1
 .. S BGPI=BGPI+1
 .. S ^BGPTMP($J,BGPI)=Y_U_$P($G(^BGPPEIA(Y,0)),U,2)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
PATLST(RETVAL,BGPSTR) ;-- get all GPRA measures
 ...
 . Q:$P(^BGPINDA(X,0),U,5)=""
 . S BGPI=BGPI+1
 . S ^BGPTMP($J,BGPI)=X_U_$P($G(^BGPINDA(X,0)),U,4)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
EPATLST(RETVAL,BGPSTR) ;-- get all GPRA measures
 ...
 . Q:$P(^BGPELIA(X,0),U,5)=""
 . S BGPI=BGPI+1
 . S ^BGPTMP($J,BGPI)=X_U_$P($G(^BGPELIA(X,0)),U,5)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
PPATLST(RETVAL,BGPSTR) ;-- get all GPRA measures
 ...
 . Q:$P(^BGPPEIA(X,0),U,4)=""
 . S BGPI=BGPI+1
 . S ^BGPTMP($J,BGPI)=X_U_$P($G(^BGPPEIA(X,0)),U,4)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
COM(RETVAL,BGPSTR) ;-- get measures based on user selection
 ...
 I BGPSTR="E" D
 . S XREF="AEL"
 S BGPLP="^BGPINDAC("""_XREF_""",1)"
 S ^BGPTMP($J,BGPI)="T00007BMXIEN^T00050Measure"_$C(30)
 K ^TMP($J,"BGPG")
 S X=0 F  S X=$O(@BGPLP@(X)) Q:X'=+X  D
 . ;Q:$P(^BGPINDA(Y,0),U,7)'=1
 . S ^TMP($J,"BGPG",$P($G(^BGPINDAC(X,0)),U))=""
 S Y=0  F  S Y=$O(^TMP($J,"BGPG",Y)) Q:'Y  D
 . S BGPI=BGPI+1
 . S ^BGPTMP($J,BGPI)=Y_U_$P($G(^BGPINDA(Y,0)),U,5)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
CMSIND(RETVAL) ;-- get CMS measures
 ...
 S BGPI=0
 S ^BGPTMP($J,BGPI)="T00007BMXIEN^T00050Measure"_$C(30)
 S X=0 F  S X=$O(^BGPCMSIA("AO",X)) Q:X'=+X  D
 . S Y=0 F  S Y=$O(^BGPCMSIA("AO",X,Y)) Q:Y'=+Y  D
 .. ;Q:$P(^BGPINDA(Y,0),U,7)'=1
 .. S BGPI=BGPI+1
 .. S ^BGPTMP($J,BGPI)=Y_U_$P($G(^BGPCMSIA(Y,0)),U,4)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
