BGP7GU ; IHS/CMI/LAB - BGP Gui Utilities 10/29/2004 3:28:39 PM 19 Sep 2005 5:28 PM 27 Apr 2007 10:28 PM ; 
...
LST(RETVAL,BGPSTR) ;-- list files
 ...
 Q
 ;
DELRPT(RETVAL,BGPSTR) ;-- delete a report out of the BGP 07 GUI REPORT file
...
CHK(BGPRET,BGPSTR) ;check report status
 ...
 N BGPDA
 S BGPDA=0 F  S BGPDA=$O(^BGPGUIA("AUSR",BGPDUZ,BGPDA)) Q:'BGPDA  D
 . N BGPIEN
 . S BGPIEN=0 F  S BGPIEN=$O(^BGPGUIA("AUSR",BGPDUZ,BGPDA,BGPIEN)) Q:'BGPIEN  D
 .. N BGPDATA,BGPNM,BGPUSER,BGPST,BGPET,BGPTOR,BGPRS,BGPTOO
 .. S BGPDATA=$G(^BGPGUIA(BGPIEN,0))
 .. S BGPNM=$P(BGPDATA,U)
 ...
 ... S DIK="^BGPGUIA(",DA=BGPIEN D ^DIK
 .. S BGPTOR=$P(BGPDATA,U,5)
 .. S BGPRS=$$GET1^DIQ(90531.08,BGPIEN,.06)
 .. S BGPTOO=$$GET1^DIQ(90531.08,BGPIEN,.07)
 .. S BGPI=BGPI+1
...
LABTAXC(RETVAL,BGPSTR) ;EP - from remote procedure
 ...
 S RETVAL="^BGPTMP("_$J_")"
 S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 D EP^BGP7GLTX(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPRT)
 S BGPI=BGPI+1
 S ^BGPTMP($J,BGPI)=+$G(BGPERR)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)
 D EN^XBVK("BGP")
...
MEDTAXC(RETVAL,BGPSTR) ;EP - from remote procedure
 ...
 S RETVAL="^BGPTMP("_$J_")"
 S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 D EP^BGP7GMTX(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPRT)
 S BGPI=BGPI+1
 S ^BGPTMP($J,BGPI)=+$G(BGPERR)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)
 D EN^XBVK("BGP")
