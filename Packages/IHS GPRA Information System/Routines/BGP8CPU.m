BGP8CPU ; IHS/CMI/LAB - calc CMS measures 26 Sep 2004 11:28 AM 04 May 2008 2:38 PM ; 02 Jul 2008  9:11 AM
 ;;8.0;IHS CLINICAL REPORTING;**2**;MAR 12, 2008
 ;
WDOB(P) ;EP
 I $Y>(BGPIOSL-3) D HDR^BGP8CP Q:BGPQUIT  D L1H^BGP8CP
 W !?2,"Date of Birth:  ",$$DATE^BGP8UTL($P(^DPT(P,0),U,3))
 Q
 ;
WRACE(P) ;EP
 I $Y>(BGPIOSL-3) D HDR^BGP8CP Q:BGPQUIT  D L1H^BGP8CP
...
WINS(V,P) ;EP
 ...
 . I $Y>(BGPIOSL-3) D HDR^BGP8CP Q:BGPQUIT  D L1H^BGP8CP
 . W !?3,"Medicare Coverage Type:  ",$P(^AUPNMCR(P,11,I,0),U,3)," Policy #:  ",$P(^AUPNMCR(P,0),U,3)," Effective Date:  ",$$DATE^BGP8UTL($P(^AUPNMCR(P,11,I,0),U))
 . S MCR=1
 . Q
 ;medicaid
 Q:BGPQUIT
 ...
 ..S MCD=1
 ..I $Y>(BGPIOSL-3) D HDR^BGP8CP Q:BGPQUIT  D L1H^BGP8CP
 ..W !?3,"Medicaid ",$$VAL^XBDIQ1(9000004,I,.11)," Coverage Type:  ",$P(^AUPNMCD(I,11,J,0),U,3)," Policy #:  ",$P(^AUPNMCD(I,0),U,3),!?10,"  Effective Date:  ",$$DATE^BGP8UTL($P(^AUPNMCD(I,11,J,0),U))
 ..Q
 .Q
 ;pi
 Q:BGPQUIT
 ...
 . W " Coverage Type:  ",C,!?10,"  Policy #:  ",N,"  Effective Date:  ",$$DATE^BGP8UTL($P(^AUPNPRVT(P,11,I,0),U,6))
 ;RR
 Q:BGPQUIT
 S I=0
 ...
 . I $Y>(BGPIOSL-3) D HDR^BGP8CP Q:BGPQUIT  D L1H^BGP8CP
 . W !?3,"Railroad Coverage Type:  ",$P(^AUPNRRE(P,11,I,0),U,3)," Policy #:  ",$P(^AUPNRRE(P,0),U,4),"  Effective Date:  ",$$DATE^BGP8UTL($P(^AUPNRRE(P,11,I,0),U))
 . S MCR=1
 . Q
...
WLASTBB ;EP write out beta blocker status
 ...
 .D:$Y>(BGPIOSL-3) HDR^BGP8CP Q:BGPQUIT  W !?4,BGPLETXT(BGPZZ)
 ;W !?4,X
 I Z]"" W !?4,Z
 ;W !
 Q
WNMIBETA ;EP - write out nmi BETA BLOCKER
 ...
 .I $Y>(BGPIOSL-3) D HDR^BGP8CP Q:BGPQUIT  D L1H^BGP8CP
 .W !!?2,"NMI Refusal?  No"
 I $Y>(BGPIOSL-6) D HDR^BGP8CP Q:BGPQUIT  D L1H^BGP8CP
 W !!?2,"NMI Refusal:  Yes"
...
WCESSMED ;EP write out CESSATION DATA
 ...
 I $Y>(BGPIOSL-(C+3)) D HDR^BGP8CP Q:BGPQUIT  D L1H^BGP8CP
 W !!?2,"Smoking Cessation Medication Rx Status?  "
 I '$D(BGPDATA) W "No Rxs documented" Q
 S X=0 F  S X=$O(BGPDATA(X)) Q:X'=+X  W !?4,$$DATE^BGP8UTL($P(BGPDATA(X),U))," ",$P(BGPDATA(X),U,2)
...
WCESS ;EP write out CESSATION DATA
 NEW X,C S (X,C)=0 F  S X=$O(BGPDATA(X)) Q:X'=+X  S C=C+1
 I $Y>(BGPIOSL-(C+2)) D HDR^BGP8CP Q:BGPQUIT  D L1H^BGP8CP
 W !!?2,"Smoking Cessation Advice/Counseling:  "
 I '$D(BGPDATA) W "Nothing recorded"
 S BGPXX=0 F  S BGPXX=$O(BGPDATA(BGPXX)) Q:BGPXX'=+BGPXX  W !?4,$$DATE^BGP8UTL($P(BGPDATA(BGPXX),U))," ",$P(BGPDATA(BGPXX),U,2)
 ;W !
 Q
...
WDXS ;EP write out DX
 ...
 I $Y>(BGPIOSL-(C+4)) D HDR^BGP8CP Q:BGPQUIT  D L1H^BGP8CP
...
WLVSD ;EP write out lsvd/cef
 NEW X,C S (X,C)=0 F  S X=$O(BGPDATA(X)) Q:X'=+X  S C=C+1
 I $Y>(BGPIOSL-(C+5)) D HDR^BGP8CP Q:BGPQUIT  D L1H^BGP8CP
...
WIVUD ;EP - write out all allergies from problem list
 NEW X,C S (X,C)=0 F  S X=$O(BGPDATA(X)) Q:X'=+X  S C=C+1
 I $Y>(BGPIOSL-5) D HDR^BGP8CP Q:BGPQUIT  D L1H^BGP8CP
 W !!?2,"ALL Unit Dose/IV Meds during Hospital Stay: ",$$DATE^BGP8UTL($P($P(^AUPNVSIT(BGPVSIT,0),U),"."))," - ",$$DATE^BGP8UTL($P($P(^AUPNVINP(BGPVINP,0),U),"."))
...
WLASTAP ;EP - write out last rx
 ...
 ..D GETTXT^BGP8CPU4
 ..S BGPZZ=0 F  S BGPZZ=$O(BGPLETXT(BGPZZ)) Q:BGPZZ'=+BGPZZ!(BGPQUIT)  D
 ...D:$Y>(BGPIOSL-3) HDR^BGP8CP Q:BGPQUIT  W !?6,BGPLETXT(BGPZZ)
 ;W !
 Q
WLASTASP ;EP - write out last rx
 ...
 ;..I $Y>(BGPIOSL-2) D HDR^BGP8CP Q:BGPQUIT  D L1H^BGP8CP
 ;..W !?6,BGPUD(BGPXX)
...
WDOD(V) ;EP - write dod
 I $Y>(BGPIOSL-2) D HDR^BGP8CP Q:BGPQUIT  D L1H^BGP8CP
 I $$DOD^AUPNPAT(V)]"" D
 .W !!?2,$S(BGPPEX[1!(BGPPEX[5):"*",1:""),"Date of Death: ",$$DATE^BGP8UTL($$DOD^AUPNPAT(V))
 Q
 ;
