BGP8CU2 ; IHS/CMI/LAB - calc CMS measures 26 Sep 2004 11:28 AM 04 May 2008 2:38 PM ;
...
SMOKER(P,BDATE,EDATE,BGPY) ;EP
 ...
 S BGPTOB=$$TOBACCO^BGP8D7(P,BDATE,EDATE)
 ...
 S BGPDX=$$DX^BGP8D7(P,BDATE,EDATE)
 I BGPDX]"",$P(BGPDX,U)'="305.13",$P(BGPDX,U)'="V15.82" S BGPC=BGPC+1,BGPY(BGPC)="Yes, Diagnosis: "_$P(BGPDX,U)_" on "_$$DATE^BGP8UTL($P(BGPDX,U,2))
 S BGPDENT=$$DENT^BGP8D7(P,BDATE,EDATE)
 I BGPDENT]"" S BGPC=BGPC+1,BGPY(BGPC)="Yes, "_$P(BGPDENT,U)_" on "_$$DATE^BGP8UTL($P(BGPDENT,U,2))
 S BGPCPT=$$CPTSM^BGP8D7(P,BDATE,EDATE)
 I BGPCPT]"",$P(BGPCPT,U,1)'="1036F" S BGPC=BGPC+1,BGPY(BGPC)="Yes, CPT "_$P(BGPCPT,U,1)_" on "_$$DATE^BGP8UTL($P(BGPCPT,U,2))
 Q
CESS(P,BDATE,EDATE,BGPY,BGPC) ;EP
 ...
 .Q
 S G=$$CPTI^BGP8DU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G0375")) I G S BGPC=BGPC+1,BGPY(BGPC)=$P(G,U,2)_U_"CPT G0375"
 S G=$$CPTI^BGP8DU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G0376")) I G S BGPC=BGPC+1,BGPY(BGPC)=$P(G,U,2)_U_"CPT G0376"
 S G=$$CPTI^BGP8DU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("4000F")) I G S BGPC=BGPC+1,BGPY(BGPC)=$P(G,U,2)_U_"CPT 4000F"
 S G=$$TRANI^BGP8DU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G0375")) I G S BGPC=BGPC+1,BGPY(BGPC)=$P(G,U,2)_U_"TRAN G0375"
 S G=$$TRANI^BGP8DU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G0376")) I G S BGPC=BGPC+1,BGPY(BGPC)=$P(G,U,2)_U_"TRAN G0376"
 S G=$$TRANI^BGP8DU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("4000F")) I G S BGPC=BGPC+1,BGPY(BGPC)=$P(G,U,2)_U_"TRAN 4000F"
...
CESSMEDS(P,BDATE,EDATE,BGPY) ;EP
 ...
 S G=$$CPTI^BGP8DU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("4001F")) I G S BGPC=BGPC+1,BGPY(BGPC)=$P(G,U,2)_U_"CPT 4001F"
 S G=$$TRANI^BGP8DU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("4001F")) I G S BGPC=BGPC+1,BGPY(BGPC)=$P(G,U,2)_U_"TRAN 4001F"
 Q
 ;
BETAALEG(P,BDATE,EDATE) ;EP
 ...
 .I N["BETA BLOCK"!(N["BBLOCK")!(N["B BLOCK") S G=1_U_"POV:  "_$$DATE^BGP8UTL($P(BGPG(X),U))_"  ["_$P(BGPG(X),U,2)_"]  "_N
 .S Z=$P(^AUPNVPOV(Y,0),U,9) I Z]"",$P($$ICDDX^ICDCODE(Z),U,2)="E942.0" S G=1_U_"POV:  "_$$DATE^BGP8UTL($P(BGPG(X),U))_"  ["_$P(BGPG(X),U,2)_" + E942.0]  "_N
 .Q
 I G Q G
 ...
 .I Y="V14.8"!($$ICD^ATXCHK(I,T,9)),N["BETA BLOCK"!(N["BBLOCK")!(N["B BLOCK") S G=1_U_"PROBLEM LIST:  "_$$DATE^BGP8UTL($P(^AUPNPROB(X,0),U,8))_"  ["_Y_"]  "_N
 .Q
 I G Q G
 ...
 .I N["BETA BLOCK"!(N["BBLOCK")!(N["B BLOCK") S G=1_U_"ALLERGY TRACKING:  "_$$DATE^BGP8UTL($P(^GMR(120.8,X,0),U,4))_"  "_N
 Q G
 ;
BETARX(P,BDATE,EDATE,EXP,BGPY) ;EP
 ...
 S BGPC=0
 K BGPY
 D GETMEDS^BGP8CU(P,BDATE,EDATE,"BGP CMS BETA BLOCKER MEDS","BGP CMS BETA BLOCKER NDC","BGP CMS BETA BLOCKER CLASS",EXP,EDATE,"",0)
 Q
 ;
PACE(P,BDATE,EDATE) ;EP
 NEW G
 S G=$$LASTDX^BGP8UTL1(P,"BGP CMS PACEMAKER DXS",BDATE,EDATE)
 I G Q 1_U_$$DATE^BGP8UTL($P(G,U,3))_" ["_$P(G,U,2)_"] "_$$VAL^XBDIQ1(9000010.07,$P(G,U,5),.04)
 S G=$$LASTPRC^BGP8UTL1(P,"BGP CMS PACEMAKER PROCS",BDATE,EDATE)
 I G Q 1_U_$$DATE^BGP8UTL($P(G,U,3))_" ["_$P(G,U,2)_"] "_$$VAL^XBDIQ1(9000010.08,$P(G,U,5),.04)
 S G=$$CPT^BGP8DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP CMS PACEMAKER CPTS",0)),6)
 I G Q 1_U_$$DATE^BGP8UTL($P(G,U,2))_" ["_$P(G,U,3)_"]"
 Q ""
 ;
...
LBBBDX(P,BDATE,EDATE) ;EP
 Q $$LASTDXI^BGP8UTL1(P,"426.3",BDATE,EDATE)
 ;
...
TARX(P,BDATE,EDATE,EXP,ADM,BGPY) ;EP
 ...
 S BGPC=0
 K BGPY
 D GETMEDS^BGP8CU(P,BDATE,EDATE,"BGP CMS THROMBOLYTIC MEDS","","BGP THROMBOLYTIC AGENTS CLASS",EXP,ADM,"",0)
 Q
 ;
PNEUVAX(P,BDATE,EDATE,BGPDD,BGPY) ;EP
 ...
 .Q:D<BDATE
 .Q:D>EDATE
 .S BGPC=BGPC+1,BGPY(BGPC)="Immunization CVX: "_C_"  "_$$DATE^BGP8UTL($P($P(^AUPNVSIT($P(^AUPNVIMM(X,0),U,3),0),U),"."))
 ...
 .Q:'Y
 .S D=$P(BGPG(X),U)
 .S BGPC=BGPC+1,BGPY(BGPC)="Procedure 99.55:   "_$$DATE^BGP8UTL(D)
 .Q
 ...
 .Q:'Y
 .S D=$P(BGPG(X),U)
 .S BGPC=BGPC+1,BGPY(BGPC)="Diagnosis V03.82:  "_$$DATE^BGP8UTL(D)
 .Q
 ...
 .Q:'Y
 .S D=$P(BGPG(X),U)
 .S BGPC=BGPC+1,BGPY(BGPC)="Diagnosis V06.06:  "_$$DATE^BGP8UTL(D)
 .Q
 ;now check for cpts
 ...
 .Q:D<BDATE
 .Q:D>EDATE
 .S BGPC=BGPC+1,BGPY(BGPC)="CPT: "_C1_"  "_$$DATE^BGP8UTL(D)
 .Q
 ;tran codes
 ...
 .Q:D<BDATE
 .Q:D>EDATE
 .S BGPC=BGPC+1,BGPY(BGPC)="Tran code: "_C1_"  "_$$DATE^BGP8UTL(D)
 .Q
 ;refusals?
 ...
 ...Q:D<(9999999-EDATE)
 ...S BGPC=BGPC+1
 ...S BGPY(BGPC)="REFUSAL: "_$$VAL^XBDIQ1(9000022,I,.07)_" - "_$$VAL^XBDIQ1(9000022,I,.04)_"  "_$$DATE^BGP8UTL($P(^AUPNPREF(I,0),U,3))_"  "_$$VAL^XBDIQ1(9000022,X,1101)
 ..Q
 .Q
 ...
 .Q:D<BDATE
 .Q:D>EDATE
 .S BGPC=BGPC+1,BGPY(BGPC)="REFUSAL: Immunization Package CVX 33 "_$$DATE^BGP8UTL(D)
 ...
 .Q:D<BDATE
 .Q:D>EDATE
 .S BGPC=BGPC+1,BGPY(BGPC)="REFUSAL: Immunization Package CVX 100 "_$$DATE^BGP8UTL(D)
 ...
 .Q:D<BDATE
 .Q:D>EDATE
 .S BGPC=BGPC+1,BGPY(BGPC)="REFUSAL: Immunization Package CVX 109 "_$$DATE^BGP8UTL(D)
 F BGPZ=33,100,109 S X=$$ANCONT^BGP8D31(P,BGPZ,EDATE) I X]"" S BGPC=BGPC+1,BGPY(BGPC)=$P(X,U,2)_" "_$$DATE^BGP8UTL($P(X,U,1))
 ;now check for bone marrow contraindication
 S X=$$LASTPRC^BGP8UTL1(P,"BGP CMS BONE MARROW PROC",$$FMADD^XLFDT(BGPDD,-365),BGPDD)
 I X S BGPC=BGPC+1,BGPY(BGPC)="Bone Marrow Contraindication: ["_$P(X,U,2)_"] "_$$DATE^BGP8UTL($P(X,U,3))
 S X=$$CPT^BGP8DU(P,$$FMADD^XLFDT(BGPDD,-365),BGPDD,$O(^ATXAX("B","BGP CMS BONE MARROW CPT",0)),6)
 I X S BGPC=BGPC+1,BGPY(BGPC)="Bone Marrow Contraindication: ["_$P(X,U,3)_"] "_$$DATE^BGP8UTL($P(X,U,2))
 Q
