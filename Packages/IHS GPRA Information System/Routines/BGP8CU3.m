...
ERBC(P,BDATE,EDATE,BGPY) ;EP - did patient have an er visit from bdate to edate without a DX in taxonomy T?
 ...
 .D BC(V,.BGPB)
 .I '$D(BGPB) Q
 .S BGPC=BGPC+1,BGPY(BGPC)="ER Visit: "_$$DATE^BGP8UTL($P(BGPG(X),U))
 Q
 ;
BC(V,BGPZ) ;any blood culture tests on visit V
 ...
 ..Q:'$$LOINC^BGP8D21(J,T)
 ..S BGPC=BGPC+1,BGPZ(BGPC)="  MICRO:  "_$$VAL^XBDIQ1(9000010.25,A,.01)_"  "_$$DATE^BGP8UTL($P(BGPG(X),U))_"  value: "_$$VAL^XBDIQ1(9000010.25,A,.04)_" "_$P(^AUPNVMIC(A,0),U,7)
 Q
...
ANTIRX(P,BDATE,EDATE,BGPY,EXP) ;EP
 ...
 S BGPC=0
 S EXP=$G(EXP)
 D GETMEDS^BGP8CU(P,BDATE,EDATE,"BGP CMS ANTIBIOTIC MEDS","BGP CMS ANTIBIOTIC MEDS NDC","BGP CMS ANTIBIOTICS MEDS CLASS",EXP,"","",0,1)
 I $G(BGPY)]"" S BGPC=BGPC+1,BGPY(BGPC)=BGPY
 S X=$$CPTI^BGP8DU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G8012"))
 I X S BGPC=BGPC+1,BGPY(BGPC)="CPT code G8012: "_$$DATE^BGP8UTL($P(X,U,2))
 S X=$$TRANI^BGP8DU(P,BDATE,EDATE,+$$CODEN^ICPTCOD("G8012"))
 ...
 K BGPG
 D IVUD^BGP8CU1(P,BDATE,EDATE,"BGP CMS ANTIBIOTIC MEDS",.BGPG,"BGP CMS ANTIBIOTIC MEDS NDC","BGP CMS ANTIBIOTICS MEDS CLASS")
...
CEFCPT ;now get all cpts
 ...
 ..S V=$P($P($G(^AUPNVSIT(V,0)),U),".")
 ..I V>EDATE Q  ;after discharge
 ..S BGPC=BGPC+1,BGPY(BGPC)="ECHOCARDIOGRAM PROCEDURE:  "_$$DATE^BGP8UTL(V)_"  ["_Y_"]  "_$$VAL^XBDIQ1(9000010.08,X,.04)
 ;NUCLEAR MEDICINE
 S X=0 F  S X=$O(^AUPNVPRC("AC",P,X)) Q:X'=+X  D
 .Q:'$D(^AUPNVPRC(X,0))
 .S I=$P($G(^AUPNVPRC(X,0)),U) Q:'I
 ...
 ..S V=$P($P($G(^AUPNVSIT(V,0)),U),".")
 ..I V>EDATE Q  ;after discharge
 ..S BGPC=BGPC+1,BGPY(BGPC)="NUCLEAR MEDICINE PROCEDURE:  "_$$DATE^BGP8UTL(V)_"  ["_Y_"]  "_$$VAL^XBDIQ1(9000010.08,X,.04)
 ;CARDIAC CATH
 S X=0 F  S X=$O(^AUPNVPRC("AC",P,X)) Q:X'=+X  D
 .Q:'$D(^AUPNVPRC(X,0))
 .S I=$P($G(^AUPNVPRC(X,0)),U) Q:'I
 ...
 ..S V=$P($P($G(^AUPNVSIT(V,0)),U),".")
 ..I V>EDATE Q  ;after discharge
 ..S BGPC=BGPC+1,BGPY(BGPC)="CARDIAC CATHETERIZATION PROCEDURE:  "_$$DATE^BGP8UTL(V)_"  ["_Y_"]  "_$$VAL^XBDIQ1(9000010.08,X,.04)
 Q
CPTC(Z) ;
