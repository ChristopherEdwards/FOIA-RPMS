BGP8CU4 ; IHS/CMI/LAB - calc CMS measures 26 Sep 2004 11:28 AM 04 May 2008 2:38 PM 05 Dec 2007 5:23 PM ; 
...
EXCLCOMP(P,ADMD,DSCHD,BGPY) ;EP
 ...
 I X S BGPC=BGPC+1,BGPY(BGPC)="HIV/AIDS: ["_$P(X,U,2)_"]  "_$$DATE^BGP8UTL($P(X,U,3))
 ;
...
HIV(P,BDATE,EDATE) ;EP
 NEW X
 S X=$$LASTDX^BGP8UTL1(P,"BGP HIV/AIDS DXS",BDATE,EDATE)
 I X Q 1_U_$P(X,U,2)_"  "_$$DATE^BGP8UTL($P(X,U,3))
 Q ""
 ;
SYSCHEMO(P,BDATE,EDATE) ;EP
 NEW X
 ;
 S X=$$LASTDXI^BGP8UTL1(P,"V58.11",BDATE,EDATE)
 I X S $P(X,U,6)="SYSTEMIC CHEMOTHERAPY: [V58.11]  "_$$DATE^BGP8UTL($P(X,U,3)) Q X
 ;
 S X=$$LASTPRCI^BGP8UTL1(P,"99.25",BDATE,EDATE)
 I X S $P(X,U,6)="SYSTEMIC CHEMOTHERAPY: [99.25]  "_$$DATE^BGP8UTL($P(X,U,3)) Q X
 ;
 NEW BGPG,I,D,G,C
 K BGPG
 S G=""
 D GETMEDS^BGP8UTL2(P,BDATE,EDATE,"","","","",.BGPG)
...
SYSIMMUN(P,BDATE,EDATE) ;EP
 NEW X
 ;
 S X=$$LASTDXI^BGP8UTL1(P,"V58.12",BDATE,EDATE)
 I X S $P(X,U,6)="SYSTEMIC IMMUNOSUPPRESSIVE THERAPY: [V58.12]  "_$$DATE^BGP8UTL($P(X,U,3)) Q X
 ;
 S X=$$LASTPRCI^BGP8UTL1(P,"00.15",BDATE,EDATE)
 I X S $P(X,U,6)="SYSTEMIC IMMUNOSUPPRESSIVE THERAPY: [00.15]  "_$$DATE^BGP8UTL($P(X,U,3)) Q X
 ;
 S X=$$LASTPRCI^BGP8UTL1(P,"99.28",BDATE,EDATE)
 I X S $P(X,U,6)="SYSTEMIC IMMUNOSUPPRESSIVE THERAPY: [99.28]  "_$$DATE^BGP8UTL($P(X,U,3)) Q X
 ;
 NEW BGPG,I,D,G,C
 K BGPG
 S G=""
...
LEUKEMIA(P,BDATE,EDATE) ;EP
 NEW X
 S X=$$LASTDX^BGP8UTL1(P,"BGP CMS LEUKEMIA DXS",BDATE,EDATE)
 I X S $P(X,U,6)="LEUKEMIA  ["_$P(X,U,2)_"]  "_$$DATE^BGP8UTL($P(X,U,3)) Q X
 Q ""
 ;
LYMPHOMA(P,BDATE,EDATE) ;EP
 NEW X
 S X=$$LASTDX^BGP8UTL1(P,"BGP CMS LYMPHOMA DXS",BDATE,EDATE)
 I X S $P(X,U,6)="LYMPHOMA  ["_$P(X,U,2)_"]  "_$$DATE^BGP8UTL($P(X,U,3)) Q X
 Q ""
 ;
RADTHER(P,BDATE,EDATE) ;EP
 NEW X
 ;
 S X=$$LASTDXI^BGP8UTL1(P,"V58.0",BDATE,EDATE)
 I X S $P(X,U,6)="RADIATION THERAPY: [V58.0]  "_$$DATE^BGP8UTL($P(X,U,3)) Q X
 ;
 S X=$$LASTPRC^BGP8UTL1(P,"BGP CMS RADIATION THER DXS",BDATE,EDATE)
 I X S $P(X,U,6)="RADIATION THERAPY: ["_$P(X,U,2)_"]  "_$$DATE^BGP8UTL($P(X,U,3)) Q X
 ;
 Q ""
 ;
PRIORHOS(P,BDATE,EDATE,VSIT) ;EP
 ...
 .Q:$$TRANS^BGP8CU(X)
 .S G=1_U_"Prior Hospital Stay: "_$$DATE^BGP8UTL($P($P(^AUPNVSIT(V,0),U),"."))_"-"_$$DATE^BGP8UTL(D)
 .Q
 Q G
 ;
HOS2DAYS(P,BDATE,EDATE) ;EP
 ...
 .;Q:$P($G(^AUPNVSIT(X,0)),U,3)="C"
 .;Q:$$TRANS^BGP8CU(X)
 .S C=C+$$LOS^APCLV(V),E=E+1,Y(E)=$$DATE^BGP8UTL($P($P(^AUPNVSIT(V,0),U),"."))_"-"_$$DATE^BGP8UTL(D)
 .Q
 I C>1 D  Q G
...
NURSHOME(P,BDATE,EDATE) ;EP
 ...
 .S G=1_U_"Nursing Home Visit: "_$$DATE^BGP8UTL($P($P(^AUPNVSIT(V,0),U),"."))
 .Q
 Q G
 ;
CHRDIAL(P,BDATE,EDATE) ;EP
 ...
 ..Q
 .Q:'R
 .S G=1_U_$P(Q,U,2)_" ; "_$P(R,U,2)_" "_$$DATE^BGP8UTL($$VD^APCLV(V))
 .Q
 Q G
 ;
...
COPD(P,EDATE) ;EP
 ...
 S X=$$LASTDX^BGP8UTL1(P,"BGP COPD DXS",$$DOB^AUPNPAT(P),EDATE)
 I X Q 1_U_"COPD DX: "_$P(X,U,2)_"  "_$$DATE^BGP8UTL($P(X,U,3))_"  "_$$VAL^XBDIQ1(9000010.07,$P(X,U,5),.04)
 Q ""
 ;
FLUVAX(P,BDATE,EDATE,BGPDD,BGPY) ;EP
 ...
 .Q:'Y
 .S D=$P(BGPG(X),U)
 .S BGPC=BGPC+1,BGPY(BGPC)="Procedure 99.52:   "_$$DATE^BGP8UTL(D)
 .Q
 ...
 .Q:'Y
 .S D=$P(BGPG(X),U)
 .S BGPC=BGPC+1,BGPY(BGPC)="Diagnosis V04.8:  "_$$DATE^BGP8UTL(D)
 .Q
 ...
 .Q:'Y
 .S D=$P(BGPG(X),U)
 .S BGPC=BGPC+1,BGPY(BGPC)="Diagnosis V04.81:  "_$$DATE^BGP8UTL(D)
 .Q
 ...
 ...Q:(9999999-D)<BDATE
 ...Q:(9999999-D)>EDATE
 ...S BGPC=BGPC+1,BGPY(BGPC)="REFUSAL: "_$$VAL^XBDIQ1(9000022,I,.07)_" - "_$$VAL^XBDIQ1(9000022,I,.04)_"   "_$$DATE^BGP8UTL($P(^AUPNPREF(I,0),U,3))_"  "_$$VAL^XBDIQ1(9000022,I,1101)
 ..Q
 .Q
 ...
 .Q:D<BDATE
 .Q:D>EDATE
 .S BGPC=BGPC+1,BGPY(BGPC)="REFUSAL: Immunization Package CVX 88 "_$$DATE^BGP8UTL(D)
 ...
 .Q:D<BDATE
 .Q:D>EDATE
 .S BGPC=BGPC+1,BGPY(BGPC)="REFUSAL: Immunization Package CVX 15 "_$$DATE^BGP8UTL(D)
 ...
 .Q:D<BDATE
 .Q:D>EDATE
 .S BGPC=BGPC+1,BGPY(BGPC)="REFUSAL: Immunization Package CVX 16 "_$$DATE^BGP8UTL(D)
 ...
 .Q:D<BDATE
 .Q:D>EDATE
 .S BGPC=BGPC+1,BGPY(BGPC)="REFUSAL: Immunization Package CVX 111 "_$$DATE^BGP8UTL(D)
 ;contraindication new in 8.0
 F BGPZ=15,16,88,111 S X=$$FLCONT^BGP8D3(P,BGPZ,$$DOB^AUPNPAT(DFN),EDATE) Q:X]""
 I X]"" S BGPC=BGPC+1,BGPY(BGPC)="NMI: "_$$DATE^BGP8UTL($P(X,U))_" "_$P(X,U,2)
 ;now check for bone marrow contraindication
 S X=$$LASTDXI^BGP8UTL1(P,"357.0",$$FMADD^XLFDT(BGPDD,-365),BGPDD)
