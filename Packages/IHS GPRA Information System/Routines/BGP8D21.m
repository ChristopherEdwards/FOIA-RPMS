...
DMEYE ;EP - called from elder care
 ...
 S BGPVALUE=$S(BGPDMD1:"UP",1:"")_$S(BGPDMD2:",AD",1:"")_$S(BGPDMD3:",AAD",1:"")_"|||"_$$DATE^BGP8UTL($P(BGPEYE,U,2))_" "_$P(BGPEYE,U,3)
 K BGPG
 K ^TMP($J,"A")
 Q
 ;
...
DENTSRV(P,BDATE,EDATE,FORECAST) ;EP
 ...
 .S G=$$REFUSAL^BGP8UTL1(P,9999999.15,$O(^AUTTEXAM("C",30,0)),$$FMADD^XLFDT(EDATE,-365),EDATE)
 .I G,$P(G,U,2)>$P(BGPC,U,2) S BGPC="2^"_$P(G,U,2)_"^Refused EXAM 30"
 .S G=$$REFUSAL^BGP8UTL1(P,9999999.31,$O(^AUTTADA("B","0000",0)),$$FMADD^XLFDT(EDATE,-365),EDATE)
 .I G,$P(G,U,2)>$P(BGPC,U,2) S BGPC="2^"_$P(G,U,2)_"^Refused ADA 0000"
 .S G=$$REFUSAL^BGP8UTL1(P,9999999.31,$O(^AUTTADA("B",0190,0)),$$FMADD^XLFDT(EDATE,-365),EDATE)
 .I G,$P(G,U,2)>$P(BGPC,U,2) S BGPC="2^"_$P(G,U,2)_"^Refused ADA 0190"
 S G=$$REFUSAL^BGP8UTL1(P,9999999.15,$O(^AUTTEXAM("C",30,0)),BDATE,EDATE)
 I $P(G,U)=1 Q "2^"_$P(G,U,2)_"^Refused EXAM 30"
 S G=$$REFUSAL^BGP8UTL1(P,9999999.31,$O(^AUTTADA("B","0000",0)),BDATE,EDATE)
 I $P(G,U)=1 Q "2^"_$P(G,U,2)_"^Refused ADA 0000"
 S G=$$REFUSAL^BGP8UTL1(P,9999999.31,$O(^AUTTADA("B","0190",0)),BDATE,EDATE)
...
EYE(P,BDATE,EDATE) ;EP
 ...
 .S X=$$CPT^BGP8DU(P,BDATE,EDATE,T,5) I X]"" Q
 .S X=$$TRAN^BGP8DU(P,BDATE,EDATE,T,5)
 S X=$$LASTPRCI^BGP8UTL1(P,"95.02",BDATE,EDATE) I X]"" Q 3_U_$P(X,U,3)_U_"PROC: 95.02"
