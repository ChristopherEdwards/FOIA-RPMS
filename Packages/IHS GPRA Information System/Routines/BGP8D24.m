BGP8D24 ; IHS/CMI/LAB - STI MEASURE 18 Oct 2007 8:37 AM ; 03 Jul 2008  7:56 AM
...
ISTI ;EP
 ...
 .I H]"" S BGPNUMV(X)=BGPNUMV(X)_"Prior HIV DX (Contraind): "_$$DATE^BGP8UTL(H)
...
IASB ;EP
 ...
 S V="",X=0,D="" F  S X=$O(BGPERV(X)) Q:X'=+X  S D=D_$S(D]"":"; ",1:"")_"ER Visit "_X_") "_$$DATE^BGP8UTL($P(BGPERV(X),U,1))_" ["_$P(BGPERV(X),U,3)_"]" D
 .S V=V_$S(V]"":"; ",1:"")
 .S V=V_"ER Visit "_X_") "
 ...
 .S V=V_", BNI "_$S($P(BGPERV(X),U,10)=1:"at ER",1:"in 7")_": "_$$DATE^BGP8UTL($P(BGPERV(X),U,8))_" "_$P(BGPERV(X),U,9)
...
BNII(P,BD) ;
 NEW Y,BGPG,E,X,T,%,D
 S Y=$$CPT^BGP8DU(P,BD,$$FMADD^XLFDT(BD,7),$O(^ATXAX("B","BGP BNI CPTS",0)),6,"CT")
