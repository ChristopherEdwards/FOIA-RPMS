...
IHEDCHM ;EP
 ...
 S BGPLDL=$$LDL^BGP8D2(DFN,BGPBDATE,BGPEDATE)
 ...
 I $P(BGPLDL,U) S V=V_"LDL "_$$DATE^BGP8UTL($P(BGPLDL,U,2))_" "_$P(BGPLDL,U,3)
 S BGPVALUE=V
 K V,BGPAMI,BGPLDL,D
 K ^TMP($J)
 Q
...
AMIO(P,BDATE,EDATE) ;
 ...
 S BGPG=$$LASTPRC^BGP8UTL1(P,"BGP CABG PROCS",$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-60))
 I $P(BGPG,U) Q 1_U_"CABG PROC"
 ;now check cpts
 S BGPG=$$CPT^BGP8DU(P,$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-60),$O(^ATXAX("B","BGP CABG CPTS",0)),6)
 I $P(BGPG,U) Q 1_U_"CABG CPT"
 S BGPG=$$TRAN^BGP8DU(P,$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-60),$O(^ATXAX("B","BGP CABG CPTS",0)),6)
 I $P(BGPG,U) Q 1_U_"CABG TRAN"
 ;check for procedure in BGP PTCA PROCS
 S BGPG=$$LASTPRC^BGP8UTL1(P,"BGP PTCA PROCS",$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-60))
 I $P(BGPG,U) Q 1_U_"PTCA PROC"
 ;now check cpts
 S BGPG=$$CPT^BGP8DU(P,$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-60),$O(^ATXAX("B","BGP PTCA CPTS",0)),6)
 I $P(BGPG,U) Q 1_U_"PTCA CPT"
 S BGPG=$$TRAN^BGP8DU(P,$$FMADD^XLFDT(BDATE,-365),$$FMADD^XLFDT(BDATE,-60),$O(^ATXAX("B","BGP PTCA CPTS",0)),6)
...
BETACONT(P,BDATE,EDATE) ;EP BETA BLOCKER CONTRAINDICATION
 ...
 S BGPG=$$LASTDX^BGP8UTL1(P,"BGP HYPOTENSION DXS",$$DOB^AUPNPAT(P),EDATE)
 I $P(BGPG,U)=1 Q 1_U_"hypotension dx contra"  ;has hypotension dx
 S BGPG=$$LASTDX^BGP8UTL1(P,"BGP CMS 2/3 HEART BLOCK DXS",$$DOB^AUPNPAT(P),EDATE)
