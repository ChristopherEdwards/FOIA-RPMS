BGP8D8 ; IHS/CMI/LAB - measure C ; 03 Jul 2008  7:05 AM
...
IE2 ;EP
 ...
 I BGPACTCL,BGPSEX="F",$$PREG^BGP8D7(DFN,$$FMADD^XLFDT(BGPEDATE,-600),BGPEDATE),BGPNO2="" S BGPD1=1  ;not pregnant
 ...
 E  S BGPVALUE=$S(BGPNO2="":"UP",1:"")_$S(BGPD1:";AC PREG",1:"")_"|||"_$S(BGPD2&(BGPHIV):"HIV SCREEN MEASURE (NO HIV DX EVER): "_$P(BGPHIV,U,4)_" ; ",1:"")_$P(BGPHIV,U,2)
...
HIVTEST(P,BDATE,EDATE) ;
 ...
 ....I $$ICD^ATXCHK($P(^AUPNVCPT(X,0),U),T,1) I '$D(BGPA((9999999-$P(ED,".")))) S BGPC=BGPC+1,BGPC(BGPC)=1_U_$$DATE^BGP8UTL((9999999-$P(ED,".")))_" CPT "_$$VAL^XBDIQ1(9000010.18,X,.01),BGPA((9999999-$P(ED,".")))=""
 ....Q
 ...Q
 ..Q
 ...
 ...I BGPLT,$P(^AUPNVLAB(X,0),U),$D(^ATXLAB(BGPLT,21,"B",$P(^AUPNVLAB(X,0),U))),'$D(BGPA((9999999-D))) S BGPC=BGPC+1,BGPC(BGPC)=1_U_$$DATE^BGP8UTL((9999999-D))_" LAB",BGPA((9999999-D))="" Q
 ...Q:'T
 ...S J=$P($G(^AUPNVLAB(X,11)),U,13) Q:J=""
 ...Q:'$$LOINC^BGP8D21(J,T)
 ...I '$D(BGPA((9999999-D))) S BGPC=BGPC+1,BGPC(BGPC)=1_U_$$DATE^BGP8UTL((9999999-D))_"  LAB "_$P($G(^LAB(95.3,J,0)),U)_"-"_$P($G(^LAB(95.3,J,0)),U,15),BGPA((9999999-D))=""
 ...Q
 ...
 .S T=$$REFUSAL^BGP8UTL1(P,60,BGPT1,$$FMADD^XLFDT(EDATE,-600),EDATE) I $P(T,U)=1 S G=2_U_$$DATE^BGP8UTL($P(T,U,2))_" lab refusal"
 Q G
HIVEDUC(P,BDATE,EDATE) ;
 ...
 .I $P(T,"-",1)="HIV"!($P(T,"-",1)["042")!($P(T,"-",1)["043")!($P(T,"-",1)["044")!($P(T,"-",1)="V08.")!($P(T,"-",1)["795.71")!($P(T,"-",2)="HIV") S G=1_U_$$DATE^BGP8UTL($P(BGPG(X),U))_U_T
 I $P(G,U)=1 Q G
 ;
 K BGPG
 S Y="BGPG("
 S X=P_"^LAST DX V65.44;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE) S E=$$START1^APCLDF(X,Y)
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP8UTL($P(BGPG(1),U))_"V65.44"  ;has a dx
 Q ""
 ;
CD4(P,BDATE,EDATE) ;
 K BGPG
 ...
 I $D(BGPG(1)) Q 1
 S E=+$$CODEN^ICPTCOD(86361),%=$$CPTI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^86361"
 S E=+$$CODEN^ICPTCOD(86360),%=$$CPTI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^86360"
 S E=+$$CODEN^ICPTCOD(86359),%=$$CPTI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^86359"
 ;tran codes
 S E=+$$CODEN^ICPTCOD(86361),%=$$TRANI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^86361 TRAN"
 S E=+$$CODEN^ICPTCOD(86360),%=$$TRANI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^86360 TRAN"
 S E=+$$CODEN^ICPTCOD(86359),%=$$TRANI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^86359 TRAN"
 ...
 .I $$LOINC^BGP8D21(J,T) S G=1
 Q G
PCR(P,BDATE,EDATE) ;
 K BGPG
 S %=P_"^LAST LAB [BGP HIV VIRAL LOAD TAX;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 1
 S E=+$$CODEN^ICPTCOD(87536),%=$$CPTI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^87536"
 S E=+$$CODEN^ICPTCOD(87539),%=$$CPTI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^87539"
 S E=+$$CODEN^ICPTCOD(87536),%=$$TRANI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^87536 TRAN"
 S E=+$$CODEN^ICPTCOD(87539),%=$$TRANI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q $P(%,U)_"^87539 TRAN"
 ;now go through all labs and check loinc codes
 K ^TMP($J,"A")
 S A="^TMP($J,""A"",",%=P_"^ALL LAB;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,A)
 I '$D(^TMP($J,"A",1)) Q ""
...
V2HIV(P,BDATE,EDATE) ;
 ...
 I '$D(^AUPNVSIT("AC",P)) Q ""
 NEW BGP8M
 S BGP8M=$$FMADD^XLFDT(EDATE,-(6*30))
 K ^TMP($J,"A")
 ...
 .Q:'D
 .I $P($P(^AUPNVSIT(V,0),U),".")'<BGP8M S H=1
 .S G=G+1
 .Q
 I G>1,H=1 Q 1
 Q ""
