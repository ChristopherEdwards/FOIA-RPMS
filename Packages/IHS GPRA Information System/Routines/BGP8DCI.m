BGP8DCI ; IHS/CMI/LAB - IHS area GPRA 08 Dec 2006 9:12 AM ; 
 ;;8.0;IHS CLINICAL REPORTING;**2**;MAR 12, 2008
 ;
CALCIND ;EP - CALCULATE ALL MEASURES
 S BGPIC=0 F  S BGPIC=$O(BGPIND(BGPIC)) Q:BGPIC'=+BGPIC  D
 .I BGPRTYPE=1,$P(^BGPINDE(BGPIC,0),U,7)'=1 Q  ;national gpra report
 .I BGPRTYPE=7,$P($G(^BGPINDE(BGPIC,12)),U,1)'=1 Q  ;OTHER NATIONAL
 ...
 .K BGPNUMV
 .K ^TMP($J)
 .I $D(^BGPINDE(BGPIC,1)) X ^BGPINDE(BGPIC,1)
 .K BGPVAL,BGPG,BGPC,BGPALLED,BGPV,A,B,C,D,E,F,G,H,I,J,K,M,N,O,P,Q,R,S,T,V,W,X,Y,Z
 .K ^TMP($J)
 .;Set up global for iCare
 .I $G(BQIGREF)'="",$D(BGPSTOP) S @BQIGREF@(DFN,BGPIC)=$P(^BGPINDE(BGPIC,0),U,3)_"^N/A" Q
 .I $D(BGPSTOP) Q  ;no need to set since no num/denom
 .;loop each individual to set numerator and denominator
 .S BGPI=0 F  S BGPI=$O(^BGPINDEC("B",BGPIC,BGPI)) Q:BGPI'=+BGPI  D
 ..S (BGPNUM,BGPDEN)=0
 ..X ^BGPINDEC(BGPI,1)
 ..X ^BGPINDEC(BGPI,2) ;denominator 1 or 0
 ..;set field counter
 ..S BGPNF=$P(^BGPINDEC(BGPI,0),U,9)
 ..S BGPN=$P(^DD(90533.03,BGPNF,0),U,4),N=$P(BGPN,";"),P=$P(BGPN,";",2)
 ..D S(BGPRPT,BGPGBL,N,P,BGPNUM),S1("N",BGPNUM)
 ..S BGPDF=$P(^BGPINDEC(BGPI,0),U,8)
 ..S BGPN=$P(^DD(90533.03,BGPDF,0),U,4),N=$P(BGPN,";"),P=$P(BGPN,";",2)
...
CPL ;comprehensive pat list check and set xtmp
 ...
 Q:BGPTIME'=1
 Q:$P($G(^BGPINDEC(BGPI,12)),U,1)=""
 X ^BGPINDEC(BGPI,3) Q:'$T
 ...
 .S ^XTMP("BGP08CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN)=F_"|||"_$P(^BGPINDEC(BGPI,12),U),BGPCPLC=BGPCPLC+1
 S $P(^XTMP("BGP08CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||")=F
 S $P(^XTMP("BGP08CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||",2)=$P(^XTMP("BGP08CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||",2)_"#"_$P(^BGPINDEC(BGPI,12),U)
 Q
NPL ;
 Q:BGPTIME'=1
 ...
 .X ^BGPNPLE(BGPX,12) K ^TMP($J) Q:'$T
 .S BGPINDL(BGPIC,BGPX)=$G(BGPINDL(BGPIC,BGPX))+1
 .I $G(BGP8NPLT) S ^XTMP("BGP8DNP",BGPJ,BGPH,"LIST",BGPIC,BGPX,DFN)="" Q
 .S BGPO=$S(BGPRTYPE=7:$P(^BGPNPLE(BGPX,0),U,6),1:$P(^BGPNPLE(BGPX,0),U,5))
 .S ^XTMP("BGP8DNP",BGPJ,BGPH,"LIST",BGPIC,BGPO,BGPX,$S($P($G(^AUPNPAT(DFN,11)),U,18)]"":$P(^AUPNPAT(DFN,11),U,18),1:"UNKNOWN"),$P(^DPT(DFN,0),U,2),BGPAGEB,DFN)=$G(BGPVALUE)
...
S1(BQITYP,BQIVAL) ; Return data by patient for iCare into global reference BQIGREF
 ...
 S BQIIDTA=$G(^BGPINDEC(BGPI,0))
 S BQIDTA=$G(^BGPINDEC(BGPI,14))
 ;
 ...
 S $P(@BQIGREF@(DFN,BGPIC),U,1)=$P(^BGPINDE(BGPIC,0),U,3)
