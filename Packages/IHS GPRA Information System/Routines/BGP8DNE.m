BGP8DNE ; IHS/CMI/LAB - NATL COMP EXPORT 05 Dec 2006 7:09 PM ; 07 Mar 2008  2:29 PM
...
TP ;
 ...
 S BGPXPBD=2990701,BGPXPEDT=3080630
...
ZIS ;call to XBDBQUE
 ;CREATE REPORT ENTRY
 K DIC S X=$P(^VA(200,DUZ,0),U)_"-"_$$D^BGP8UTL(BGPNOW),DIC(0)="L",DIC="^BGPXPE(",DLAYGO=90533.11,DIADD=1
 ...
 S BGPXPRPT=+Y
 K ^BGPXPE(BGPXPRPT,13)
 S C=0,X="" F  S X=$O(BGPTAX(X)) Q:X=""  S C=C+1 S ^BGPXPE(BGPXPRPT,13,C,0)=X,^BGPXPE(BGPXPRPT,13,"B",X,C)=""
 S ^BGPXPE(BGPXPRPT,13,0)="^90533.111301A^"_C_"^"_C
 K ^BGPXPE(BGPXPRPT,14)
 I $G(BGPMFITI) S C=0,X="" F  S X=$O(^ATXAX(BGPMFITI,21,"B",X)) Q:X=""  S C=C+1,Y=$P($G(^DIC(4,X,0)),U) S ^BGPXPE(BGPXPRPT,14,C,0)=Y,^BGPXPE(BGPXPRPT,14,"B",Y,C)=""
 S ^BGPXPE(BGPXPRPT,14,0)="^90533.111401A^"_C_"^"_C
ONEF ;
 S BGPONEF=""
 K IOP,%ZIS W !! S %ZIS="PQM" D ^%ZIS
 I POP W !,"Report Aborted" S DA=BGPXPRPT,DIK="^BGPXPE(" D ^DIK K DIK D XIT Q
 I $D(IO("Q")) G TSKMN
DRIVER ;
 D PROC^BGP8DNE
 U IO
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP8DNE",ZTDTH="",ZTDESC="NATIONAL GPRA REPORT 08" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
CNTSF ;EP
 ...
 S BGPX=0,BGPTOT=0 F  S BGPX=$O(^BGPXPE(BGPXPRPT,11,BGPX)) Q:BGPX'=+BGPX  S BGPTOT=BGPTOT+1
 S BGPNF1=BGPTOT/65536
 I BGPNF1'>1,BGPEXPT G STOP ; - only 1 and it is created already
 S BGPNF=$S($P(BGPNF1,".",2)]"":BGPNF1+1,1:BGPNF1)
 S BGPNF=$P(BGPNF,".")
 S BGPX=0,BGPLX=0
 S ^BGPXPE(BGPXPRPT,12,0)="^90533.111201A^"_BGPNF_"^"_BGPNF
 F BGPZ=1:1:BGPNF D
 ...
 .S ^BGPXPE(BGPXPRPT,12,BGPZ+1,0)=BGPFN_" --- "_BGPUF
 ...
 .F  S BGPX=$O(^BGPXPE(BGPXPRPT,11,BGPX)) Q:BGPX'=+BGPX!(BGPC>65535)  D
 ..W $G(^BGPXPE(BGPXPRPT,11,BGPX,0)),!
 ..S BGPC=BGPC+1
 ..S BGPLX=BGPX
 .D ^%ZISC
STOP ;
 K ^BGPXPE(BGPXPRPT,11)
 Q
CNTSF1 ;EP
 ...
 S ^BGPXPE(BGPXPRPT,12,BGPZ,0)=BGPFN_" --- "_BGPUF
 I '$D(ZTQUEUED) U IO W !?10,BGPFN
 L +^BGPDATA:10 Q:'$T
 ...
 S ^BGPDATA(BGPC)=Y
 S BGPX=0
 F  S BGPX=$O(^BGPXPE(BGPXPRPT,11,BGPX)) Q:BGPX'=+BGPX  D
 .S BGPC=BGPC+1
 .S ^BGPDATA(BGPC)=$G(^BGPXPE(BGPXPRPT,11,BGPX,0))
 ;D ^%ZISC
 S XBGL="BGPDATA"
...
PROC1 ;process each patient
 ...
 ..S ^BGPXPE(BGPXPRPT,11,BGPXPTOT,0)=BGPXPDAT(X)
 .Q
 S ^BGPXPE(BGPXPRPT,11,0)="^90533.111101A^"_BGPXPTOT_"^"_BGPXPTOT
 S BGPET=$H
 D CNTSF
 Q
PROCYRS ;
...
PROCIND ;
 ...
 .S BGPXPNV=""
 .K ^TMP($J)
 .I $D(^BGPINDE(BGPXPX,1)) X ^BGPINDE(BGPXPX,1)
 .;now get each individual measure and set value
 .S BGPXPE=0 F  S BGPXPE=$O(^BGPCTRL(BGPXPFYC,55,BGPXPMI,11,BGPXPE)) Q:BGPXPE'=+BGPXPE  D
 ..S BGPXPMID=$P(^BGPCTRL(BGPXPFYC,55,BGPXPMI,11,BGPXPE,0),U,1)
 ..I '$D(BGPXPDAT(BGPXPMID)) S BGPXPDAT(BGPXPMID)=BGPXPPD
 ..S $P(BGPXPDAT(BGPXPMID),U,11)=BGPXPMID
 ..S $P(BGPXPDAT(BGPXPMID),U,12)=$P(^BGPCTRL(BGPXPFYC,55,BGPXPMI,11,BGPXPE,0),U,3)
 ..S X=""
 ..X ^BGPCTRL(BGPXPFYC,55,BGPXPMI,11,BGPXPE,1)
...
PRINT ;EP - CALLED FROM XBDBQUE
 ...
 S BGPX=0 F  S BGPX=$O(^BGPXPE(BGPXPRPT,12,BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 .I $Y>(IOSL-3) D HEADER Q:BGPQUIT
 .W !?5,$P(^BGPXPE(BGPXPRPT,12,BGPX,0),U)
 K BGPX,BGPQUIT
 Q
HEADER ;EP
 G:'BGPGPG HEADER1
...
HEADER1 ;
 ...
 W $$CTR("*** IHS 2008 Comprehensive National GPRA Export ***",80),!
