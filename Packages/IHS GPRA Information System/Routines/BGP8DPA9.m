BGP8DPA9 ; IHS/CMI/LAB - COMP NATIONAL GPRA FOR PTS W/APPT ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP8DPA9",ZTDTH="",ZTDESC="CRS 08 SCHED REPORT FOR 09" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D ^XBFMK
 K DIC,DIR,DFN
...
BDMG(BGPBD,BGPED,BGPCLN) ;EP - GUI DMS Entry Point
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BGP8DPA9",ZTDESC="GUI CN GRPA APPT" D ^%ZTLOAD
 D EOJ
 Q
GUIEP ;EP
 D PROC
 K ^TMP($J,"BGP8DPA")
 S IOM=80
 D GUIR^XBLM("PRINT^BGP8DPA9","^TMP($J,""BGP8DPA"",")
...
GUIECP ;EP
 K ^TMP($J,"BGP8DPA")
 S IOM=80
 D GUIR^XBLM("CPPRINT^BGP8DPA9","^TMP($J,""BGP8DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP8DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP8DPA",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIE(BGPIEN,11,X,0)=BGPDATA,C=C+1
