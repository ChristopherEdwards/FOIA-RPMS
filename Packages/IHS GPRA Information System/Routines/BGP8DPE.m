BGP8DPE ; IHS/CMI/LAB - IHS GPRA 08 REPORT DRIVER ;
 ;;8.0;IHS CLINICAL REPORTING;**2**;MAR 12, 2008
 ;
 ;
 W:$D(IOF) @IOF
 W !,$$CTR("IHS 2008 Patient Education Report",80),!!
INTRO ;
 D XIT
 S BGPCTRL=$O(^BGPCTRL("B",2008,0))
 S X=0 F  S X=$O(^BGPCTRL(BGPCTRL,61,X)) Q:X'=+X  W !,^BGPCTRL(BGPCTRL,61,X,0)
...
SUM ;display summary of this report
 ...
 W:$D(IOF) @IOF
 W !,$$CTR("SUMMARY OF FY 08 PATIENT EDUCATION REPORT TO BE GENERATED")
 W !!,"The date ranges for this report are:"
 W !?5,"Report Period: ",?31,$$FMTE^XLFDT(BGPBD)," to ",?31,$$FMTE^XLFDT(BGPED)
 W !?5,"Previous Year Period: ",?31,$$FMTE^XLFDT(BGPPBD)," to ",?31,$$FMTE^XLFDT(BGPPED)
 W !?5,"Baseline Period: ",?31,$$FMTE^XLFDT(BGPBBD)," to ",?31,$$FMTE^XLFDT(BGPBED)
...
ZIS ;call to XBDBQUE
 ...
 I BGPRPT="" D XIT Q
 I BGPPEEXP D
 .W !!,"A file will be created called BG08",$P(^AUTTLOC(DUZ(2),0),U,10)_".PED"_BGPRPT," and will reside",!,"in the ",BGPUF," directory.",!
 ...
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPPEDCE(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPPEDPE(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPPEDBE(" D ^DIK K DIK D XIT Q
 I $D(IO("Q")) G TSKMN
DRIVER ;
 D ^BGP8DPE1
 U IO
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP8DPE",ZTDTH="",ZTDESC="PAT ED 08 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
