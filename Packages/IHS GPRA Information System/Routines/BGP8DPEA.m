BGP8DPEA ; IHS/CMI/LAB - IHS GPRA 08 REPORT DRIVER 23 Oct 2007 2:04 PM ; 
 ;;8.0;IHS CLINICAL REPORTING;**2**;MAR 12, 2008
 ;
 ;
 W:$D(IOF) @IOF
 W !,$$CTR("IHS 2008 Area Aggregate Patient Education Report",80),!!
INTRO ;
 D XIT
...
ASU ;
 ...
 I C=1 S BGPRPTT="F",BGPSUCNT=1,Y=$O(BGPSUL(0)),X=$P(^BGPPEDCE(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 I C>1 S BGPRPTT="A"
ZIS ;call to XBDBQUE
 D ^XBFMK
 S X=0 F  S X=$O(^BGPPEIE(X)) Q:X'=+X  S BGPIND(X)=""
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP8DPEA",ZTDTH="",ZTDESC="PAT ED 08 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
