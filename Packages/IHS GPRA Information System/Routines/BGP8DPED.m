BGP8DPED ; IHS/CMI/LAB - IHS gpra print ;
...
DEL ;EP - create delimited output file
 ...
 I '$D(BGPGUI) D
 .S XBFLT=1,XBFN=BGPDELF_".txt",XBMED="F",XBTLE="GPRA 08 DELIMITED PT ED OUTPUT",XBQ="N",XBF=0
 .D ^XBGSAVE
...
DEL1 ;EP
 ...
 .D S(" ",1,1),S(" ",1,1)
 .S X=$P(^BGPPEIE(BGPIC,0),U,2) D S(X,1,1)
 .D S(" ",1,1)
 .S X="Denominator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPPEIE(BGPIC,61,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIE(BGPIC,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPPEIE(BGPIC,61,BGPY,11,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S Y=^BGPPEIE(BGPIC,61,BGPY,11,BGPZ,0) D S(Y,1,1)
 ....Q
 ...Q
 ..Q
 .D S(" ",1,1)
 .S X="Numerator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPPEIE(BGPIC,62,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIE(BGPIC,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPPEIE(BGPIC,62,BGPY,11,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S X=^BGPPEIE(BGPIC,62,BGPY,11,BGPZ,0) D S(X,1,1)
 ....Q
 ...Q
 ..Q
 .D S(" ",1,1)
 .S BGPNODE=11
 .S BGPX=0 F  S BGPX=$O(^BGPPEIE(BGPIC,BGPNODE,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPPEIE(BGPIC,BGPNODE,BGPX,0) D S(X,1,1)
 .D @BGPIC
 Q
EXIT ;
 K ^TMP($J)
...
C(X,X2,X3) ;
 ...
 ;
1 ;
 D S(" ",1,1) ;S X=$P(^BGPPEIE(BGPIC,0),U,2) D S(X,1,1)
 D H1^BGP8PDL1
 D S(" ",1,1)
 ...
 Q
2 ;
 D S(" ",1,1) D S(" ",1,1) ;S X=$P(^BGPPEIE(BGPIC,0),U,2) D S(X,1,1)
 D H1^BGP8PDL1
 D S(" ",1,1)
 ...
 ..I $P(^BGPPEDCE(X,11),U,4)]"",$P(^BGPPEDCE(X,11),U,4)<BGPCYD S BGPCYD=$P(^BGPPEDCE(X,11),U,4)
 ..I $P(^BGPPEDPE(X,11),U,4)]"",$P(^BGPPEDPE(X,11),U,4)<BGPPRD S BGPPRD=$P(^BGPPEDPE(X,11),U,4)
 ..I $P(^BGPPEDBE(X,11),U,4)]"",$P(^BGPPEDBE(X,11),U,4)<BGPBLD S BGPBLD=$P(^BGPPEDBE(X,11),U,4)
 ...
 .S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 ..I $P(^BGPPEDCE(X,11),U,5)>BGPCYD S BGPCYD=$P(^BGPPEDCE(X,11),U,5)
 ..I $P(^BGPPEDPE(X,11),U,5)>BGPPRD S BGPPRD=$P(^BGPPEDPE(X,11),U,5)
 ..I $P(^BGPPEDBE(X,11),U,5)>BGPBLD S BGPBLD=$P(^BGPPEDBE(X,11),U,5)
 S X="" D S(X,1,1)
