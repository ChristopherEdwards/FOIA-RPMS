BGP8DPEP ; IHS/CMI/LAB - IHS gpra print ; 01 Jul 2008  7:54 PM
 ;;8.0;IHS CLINICAL REPORTING;**2**;MAR 12, 2008
 ;
 ;
PRINT ;
 S BGPRTYPE=6,BGP8RPTH=""
 K ^TMP($J)
 K BGPDELIM
 I BGPROT="D" G DEL
...
PRINT1 ;EP
 ...
 .W !!,$P(^BGPPEIE(BGPIC,0),U,2)
 .I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 .W !!,"Denominator(s):"
 .S BGPX=0 F  S BGPX=$O(^BGPPEIE(BGPIC,61,"B",BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIE(BGPIC,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY!(BGPQUIT)  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPPEIE(BGPIC,61,BGPY,11,BGPZ)) Q:BGPZ'=+BGPZ!(BGPQUIT)  D
 ....I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ....W !,^BGPPEIE(BGPIC,61,BGPY,11,BGPZ,0)
 ....Q
 ...Q
 ..Q
 .I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 .W !!,"Numerator(s):"
 .S BGPX=0 F  S BGPX=$O(^BGPPEIE(BGPIC,62,"B",BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIE(BGPIC,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY!(BGPQUIT)  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPPEIE(BGPIC,62,BGPY,11,BGPZ)) Q:BGPZ'=+BGPZ!(BGPQUIT)  D
 ....I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ....W !,^BGPPEIE(BGPIC,62,BGPY,11,BGPZ,0)
 ....Q
 ...;W !
 ...Q
 ..Q
 .S BGPNODE=11
 .W ! S BGPX=0 F  S BGPX=$O(^BGPPEIE(BGPIC,BGPNODE,BGPX)) Q:BGPX'=+BGPX  D
 ..I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ..W !,^BGPPEIE(BGPIC,BGPNODE,BGPX,0)
 .D @BGPIC
 .Q:BGPQUIT
 ;
 D ^BGP8DPEL
 ...
 Q
1 ;
 I $Y>(BGPIOSL-6) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIE(BGPIC,0),U,2)
 D H1^BGP8DPH
...
2 ;
 ...
 ..I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIE(BGPIC,0),U,2) D H1^BGP8DPH W !
 ...
 I $Y>(BGPIOSL-6) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIE(BGPIC,0),U,2) D H1^BGP8DPH W !
 ...
 ..I $P($G(^BGPPEDCE(X,11)),U,4)]"",$P($G(^BGPPEDCE(X,11)),U,4)<BGPCYD S BGPCYD=$P($G(^BGPPEDCE(X,11)),U,4)
 ..I $P($G(^BGPPEDPE(X,11)),U,4)]"",$P($G(^BGPPEDPE(X,11)),U,4)<BGPPRD S BGPPRD=$P($G(^BGPPEDPE(X,11)),U,4)
 ..I $P($G(^BGPPEDBE(X,11)),U,4)]"",$P($G(^BGPPEDBE(X,11)),U,4)<BGPBLD S BGPBLD=$P($G(^BGPPEDBE(X,11)),U,4)
 ...
 I $Y>(BGPIOSL-4) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIE(BGPIC,0),U,2) D H1^BGP8DPH W !
 W !,"Minimum Time Spent",!,"All Providers",!,"(minutes)"
 W ?20,$$C(BGPCYD,0,8),?35,$$C(BGPPRD,0,8),?58,$$C(BGPBLD,0,8),!
 I $G(BGPAREAA) D
 .S (BGPCYD,BGPPRD,BGPBLD)=""
 .S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 ..I $P($G(^BGPPEDCE(X,11)),U,5)>BGPCYD S BGPCYD=$P($G(^BGPPEDCE(X,11)),U,5)
 ..I $P($G(^BGPPEDPE(X,11)),U,5)>BGPPRD S BGPPRD=$P($G(^BGPPEDPE(X,11)),U,5)
 ..I $P($G(^BGPPEDBE(X,11)),U,5)>BGPBLD S BGPBLD=$P($G(^BGPPEDBE(X,11)),U,5)
 I '$G(BGPAREAA) D
...
SETNM ;
 ...
 S X=0 F  S X=$O(^BGPPEDCE(BGPRPT,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDCE(BGPRPT,N,X,0),U),L=$P(^BGPPEDCE(BGPRPT,N,X,0),U,2),M=$P(^BGPPEDCE(BGPRPT,N,X,0),U,3)
 .;I $D(BGPPROVS(C,L)) S $P(BGPPROVS(C,L),U,1)=M Q
 .S $P(BGPPROVS(C,L),U,1)=M
 S X=0 F  S X=$O(^BGPPEDPE(BGPRPT,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDPE(BGPRPT,N,X,0),U),L=$P(^BGPPEDPE(BGPRPT,N,X,0),U,2),M=$P(^BGPPEDPE(BGPRPT,N,X,0),U,3)
 .;I $D(BGPPROVS(C,L)) S $P(BGPPROVS(C,L),U,2)=M Q
 .S $P(BGPPROVS(C,L),U,2)=M
 S X=0 F  S X=$O(^BGPPEDBE(BGPRPT,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDBE(BGPRPT,N,X,0),U),L=$P(^BGPPEDBE(BGPRPT,N,X,0),U,2),M=$P(^BGPPEDBE(BGPRPT,N,X,0),U,3)
...
SETNMA ;
 ...
 Q
SETNMA1 ;
 S X=0 F  S X=$O(^BGPPEDCE(Z,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDCE(Z,N,X,0),U),L=$P(^BGPPEDCE(Z,N,X,0),U,2),M=$P(^BGPPEDCE(Z,N,X,0),U,3)
 .;I $D(BGPPROVS(C,L)) S $P(BGPPROVS(C,L),U,1)=M Q
 .S $P(BGPPROVS(C,L),U,1)=$P($G(BGPPROVS(C,L)),U,1)+M
 S X=0 F  S X=$O(^BGPPEDPE(Z,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDPE(Z,N,X,0),U),L=$P(^BGPPEDPE(Z,N,X,0),U,2),M=$P(^BGPPEDPE(Z,N,X,0),U,3)
 .;I $D(BGPPROVS(C,L)) S $P(BGPPROVS(C,L),U,2)=M Q
 .S $P(BGPPROVS(C,L),U,2)=$P($G(BGPPROVS(C,L)),U,2)+M
 S X=0 F  S X=$O(^BGPPEDBE(Z,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDBE(Z,N,X,0),U),L=$P(^BGPPEDBE(Z,N,X,0),U,2),M=$P(^BGPPEDBE(Z,N,X,0),U,3)
 .;I $D(BGPPROVS(C,L)) S $P(BGPPROVS(C,L),U,3)=M Q
...
SETEXA(T,N,P) ;EP - set denominator
 ...
 NEW X,Y,Z
 S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 .I T=1 S $P(BGPEXCT(X,T),U,2)=$P($G(^BGPPEDCE(X,N)),U,P)
 .I T=2 S $P(BGPEXCT(X,T),U,2)=$P($G(^BGPPEDPE(X,N)),U,P)
 .I T=3 S $P(BGPEXCT(X,T),U,2)=$P($G(^BGPGPDBE(X,N)),U,P)
 Q
V(T,R,N,P,ND) ;EP ;SPDX
 I $G(BGPAREAA) G VA
 NEW X
 I T=1 S X=$P($G(^BGPPEDCE(R,N)),U,P) Q $S(X]"":X,1:0)
 I T=2 S X=$P($G(^BGPPEDPE(R,N)),U,P) Q $S(X]"":X,1:0)
 I T=3 S X=$P($G(^BGPPEDBE(R,N)),U,P) Q $S(X]"":X,1:0)
 Q ""
VA ;
 NEW X,V,C S X=0,C="" F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 .I T=1 S C=C+$P($G(^BGPPEDCE(X,N)),U,P)
 .I T=2 S C=C+$P($G(^BGPPEDPE(X,N)),U,P)
 .I T=3 S C=C+$P($G(^BGPPEDBE(X,N)),U,P)
 .Q
 Q $S(C]"":C,1:0)
