BGP8DPP ; IHS/CMI/LAB - IHS GPRA 08 SELECTED REPORT DRIVER ALL PATIENTS ;
 ;;8.0;IHS CLINICAL REPORTING;**2**;MAR 12, 2008
 ;
 ;
 W:$D(IOF) @IOF
 W !!,$$CTR("2008 Clinical Performance Measure Report (Selected Measures)",80)
...
SUM ;display summary of this report
 ...
 W !!,"These measures will be calculated: " S X=0 F  S X=$O(BGPIND(X)) Q:X'=+X  W $P(^BGPINDE(X,0),U,3)," ; "
...
EI ;
 ...
 Q
PI ;
 S X=0 F  S X=$O(^BGPINDEC("APR",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDEC(X,0),U,1))=""
 Q
SI ;
 K BGPIND
 D EN^BGP8DSI
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP8DPP",ZTDTH="",ZTDESC="GPRA 08 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
NODEV ;
