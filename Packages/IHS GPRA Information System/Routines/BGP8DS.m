...
START ;
 ...
 .S BGPCNT=10
GO .;
 .W !,$P(^BGPINDE(BGPL,0),U,3),!
 .W !,"Denominator(s):"
 .S BGPX=0 F  S BGPX=$O(^BGPINDE(BGPL,61,"B",BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 ..S BGPY=0 F  S BGPY=$O(^BGPINDE(BGPL,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY!(BGPQUIT)  D
 ...
 ...I BGPRTYPE=4,'$D(^BGPINDE(BGPL,61,BGPY,12,"B",BGPINDT))
 ...S BGPZ=0 F  S BGPZ=$O(^BGPINDE(BGPL,61,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ!(BGPQUIT)  D
 ....I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ....W !,^BGPINDE(BGPL,61,BGPY,1,BGPZ,0)
 ....Q
 ...;W !
 ...Q
 ..Q
 .I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 .W !!,"Numerator(s):"
 .S BGPX=0 F  S BGPX=$O(^BGPINDE(BGPL,62,"B",BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 ..S BGPY=0 F  S BGPY=$O(^BGPINDE(BGPL,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY!(BGPQUIT)  D
 ...
 ...S BGPZ=0 F  S BGPZ=$O(^BGPINDE(BGPL,62,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ!(BGPQUIT)  D
 ....I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ....W !,^BGPINDE(BGPL,62,BGPY,1,BGPZ,0)
 ....Q
 ...;W !
 ...Q
 ..Q
 ...
 .W !!,"Logic" S BGPX=0 F  S BGPX=$O(^BGPINDE(BGPL,BGPNODE,BGPX)) Q:BGPX'=+BGPX  D
 ..I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ..W !,^BGPINDE(BGPL,BGPNODE,BGPX,0)
 .W !!,"Performance Measure Description" S BGPX=0 F  S BGPX=$O(^BGPINDE(BGPL,51,BGPX)) Q:BGPX'=+BGPX  D
 ..I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ..W !,^BGPINDE(BGPL,51,BGPX,0)
 .I $O(^BGPINDE(BGPL,52,0)) W !,"Past Performance and/or Target" S BGPX=0 F  S BGPX=$O(^BGPINDE(BGPL,52,BGPX)) Q:BGPX'=+BGPX  D
 ..I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ..W !,^BGPINDE(BGPL,52,BGPX,0)
 .I $O(^BGPINDE(BGPL,55,0)) W !,"Source" S BGPX=0 F  S BGPX=$O(^BGPINDE(BGPL,55,BGPX)) Q:BGPX'=+BGPX  D
 ..I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ..W !,^BGPINDE(BGPL,55,BGPX,0)
...
PRINTL ;print one line
 ...
 .;S X=0 F  S X=$O(^BGPINDE(BGPL,72,X)) Q:X'=+X  W !,^BGPINDE(BGPL,72,X,0)
 .D H1
 Q:BGPQUIT
 S BGPPCNT=BGPPCNT+1
 W !,$E($P(^DPT(DFN,0),U),1,22),?24,$$HRN^AUPNPAT(DFN,DUZ(2)),?31,$E(BGPCOM,1,14),?46,BGPSEX,?49,BGPAGE
 W ?53,$P(^XTMP("BGP8D",BGPJ,BGPH,"LIST",BGPL,BGPCOM,BGPSEX,BGPAGE,DFN),"|||",1)
 W:BGPL=43 " NUMERATOR: "
 W ?65,$P(^XTMP("BGP8D",BGPJ,BGPH,"LIST",BGPL,BGPCOM,BGPSEX,BGPAGE,DFN),"|||",2)
 Q
 ;
HEADER ;EP
 G:'BGPGPG HEADER1
...
H1 ;
 ...
 S X=0,C=0 F  S X=$O(^BGPINDE(BGPL,BGPTSUB,X)) Q:X'=+X  S C=C+1
 ...
 S X=0 F  S X=$O(^BGPINDE(BGPL,BGPTSUB,X)) Q:X'=+X  W !,^BGPINDE(BGPL,BGPTSUB,X,0)
