...
TESTNTL ;
 ...
 S LORIND(4)="",LORIND(3)=""
 S LORILIST(3)=""
 D EP(.ERR,1,2522,"BGP 08 SEL ALL PATS","S",.LORIND,1,3040000,3000000,1,"A",.LORILIST,"","","B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPINDT,BGPIND,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPLSTT,BGPLIST,BGPLPRV,BGPLPROV,BGPROT,BGPRTIME,BGPMFITI) ;EP - called from GUI to produce COM REPORT CI05-RPT-LOC-COM
...
EP1 ;
 ...
 S BGPGUI=1
 S IOM=80,BGPIOSL=55
 S BGPRTYPE=4,BGP8RPTH="A",BGPALLPT=1
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LOCALL^BGP8GALL",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT ALL 05" D ^%ZTLOAD Q
 Q
LOCALL ;
 D ^BGP8D1
 K ^TMP($J,"BGPGUI")
