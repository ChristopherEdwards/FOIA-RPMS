...
TESTNTL ;
 ...
 S LORISUL(1)=""
 S LORISUL(2)=""
 D EP(.ERR,1,2522,"BGP 08 AREA GPU","A",.LORISUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOAGP^BGP8GAPU",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT 06" D ^%ZTLOAD Q
 Q
AOAGP ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^XBLM("PRINT^BGP8PARP","^TMP($J,""BGPGUI"",")
