...
TESTNTL ;
 ...
 S LORIND(4)="",LORIND(3)=""
 S LORILIST(3)=""
 D EP(.ERR,1,2522,"BGP 08 SELECTED IND REPORT",338,"S",.LORIND,1,3040000,3000000,1,"A",.LORILIST,"","","B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPINDT,BGPIND,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPLSTT,BGPLIST,BGPLPRV,BGPLPROV,BGPROT,BGPRTIME,BGPMFITI) ;EP - called from GUI to produce COM REPORT CI05-RPT-LOC-COM
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LOCCOM^BGP8GCOM",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT COM 05" D ^%ZTLOAD Q
 Q
LOCCOM ;
 D ^BGP8D1
 K ^TMP($J,"BGPGUI")
