...
TESTNTL ;
 S ERR=""
 D EP(.ERR,1,2522,"BGP 08 PATIENT FORECAST",338,1,3040000,3000000,1,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRT1,BGPDIVI,BGPABD,BGPAED,BGPCLN,BGPPATS,BGPRTIME) ;EP - called from GUI to produce national GPU report (OTH-GPU)
 ;  BGPUSER - DUZ
 ;  BGPDUZ2 - DUZ(2)
 ;  BGPOPTN - OPTION NAME
...
EP1 ;
 ...
 S BGPGUI=1
 S IOM=80,BGPIOSL=55
 S BGPRTYPE=1,BGP8RPTH="",BGP8GPU=1,BGPALLPT=1,BGPBEN=3
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="GUIP^BGP8GFOR",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI GPU GPRA REPORT 06" D ^%ZTLOAD Q
 Q
GUIP ;
 D PROC^BGP8DPA
 K ^TMP($J,"BGP8DPA")
 S IOM=80,BGPIOSL=55
 D GUIR^XBLM("PRINT^BGP8DPAW","^TMP($J,""BGP8DPA"",")
