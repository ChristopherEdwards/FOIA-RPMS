...
TESTNTL ;
 ...
 S LORIND(4)="",LORIND(3)=""
 S LORILIST(3)=""
 D EP(.ERR,1,2522,"BGP 08 SEL PATIENT PANEL",338,"D",.LORIND,1,3040000,3000000,1801,"A",.LORILIST,"","","B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPINDT,BGPIND,BGPQTR,BGPPER,BGPVDT,BGPSEAT,BGPLSTT,BGPLIST,BGPLPRV,BGPLPROV,BGPROT,BGPRTIME) ;EP - called from GUI to produce PP REPORT CI05-RPT-LOC-PP
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LOCPP^BGP8GPP",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT COM 05" D ^%ZTLOAD Q
 Q
LOCPP ;
 D ^BGP8D10
 K ^TMP($J,"BGPGUI")
