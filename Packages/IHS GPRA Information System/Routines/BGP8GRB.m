...
DEBUG(RETVAL,BGPSTR) ;run the debugger
 D DEBUG^%Serenji("FORD^BGP8GRB(.RETVAL,.BGPSTR)")
 Q
 ;
FOR(RETVAL,BGPSTR) ;-- gpra patient forcast report
 S X="MERR^BGP8GU",@^%ZOSF("TRAP") ; m error trap
 N BGPI,BGPGDA,P,R,BGPLST,BGPDIV,BGPBADT,BGPEADT,BGPCLN,BGPPATL,BGPPATI,BGPPAT,BGPCLNL,BGPST,BGPSTI,BGPCNT
 S P="|",R="~"
 S RETVAL="^BGPTMP("_$J_")"
 S BGPI=0
 S BGPOPT="CRS 08 PATIENT FORECAST REPORT"
 ...
 D EP^BGP8GFOR(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPLST,BGPDIV,BGPBADT,BGPEADT,.BGPCLN,.BGPPAT,BGPRT)
 S BGPI=BGPI+1
...
FORD(RETVAL,BGPSTR) ;-- return the patient forecast denominator
 ...
 S BGPI=0
 S BGPGUI=1
 S BGPOPT="CRS 08 PATIENT FORECAST DENOMINATOR DEFINITIONS"
 D ^XBFMK
 D GUILOG(BGPOPT,BGPRDT)
 S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 K ZTSAVE S ZTSAVE("*")=""
 ...
 ;S IOM=80,BGPIOSL=55
 ;S BGPGUI=1
 ;D GUIR^XBLM("CPPRINT^BGP8DPA","^TMP($J,""BGP8DPAD"",")
 ;N BGPI
 ;S BGPI=0
 ;S RETVAL="^BGPTMP("_$J_")"
 ;S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 ;N X,C
 ;S X=0,C=0 F  S X=$O(^TMP($J,"BGP8DPAD",X)) Q:X'=+X  D
 ;. S C=C+1
 ;. N BGPDATA
 ;. S BGPDATA=$G(^TMP($J,"BGP8DPAD",X))
 ;. S BGPI=BGPI+1
 ;. S ^BGPTMP($J,BGPI)=BGPDATA_$C(30)
 ;S ^BGPTMP($J,BGPI+1)=$C(31)
 ;K ^TMP($J,"BGP8DPAD"),BGPGUI
 ;
AEO(RETVAL,BGPSTR) ;-- area EO report
 ...
 S BGPOPT="CRS 08 AREA EO REPORT"
 N I
 ...
 S RETVAL="^BGPTMP("_$J_")"
 S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 D EP^BGP8GAEO(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPRT,.BGPLIST,BGPQTR,BGPPER,BGPBAS,BGPBEN,BGPOT,BGPRE,BGPHC)
 S BGPI=BGPI+1
 S ^BGPTMP($J,BGPI)=+$G(BGPERR)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)
 D EN^XBVK("BGP")
 Q
 ;
GUILOG(BGPOPT,RDT) ;-- log in the GUI REPORT FILE
 D NOW^%DTC
 S BGPNOW=$G(%)
 K DD,D0,DIC
 S X=DUZ_$$NOW^XLFDT
 S DIC="^BGPGUIE(",DIC(0)="L",DIADD=1,DLAYGO=90534.08,DIC("DR")=".02////"_DUZ_";.03////"_$S(RDT]"":RDT,1:$$NOW^XLFDT)_";.05///"_BGPOPT_";.06///R;.07///P"
...
FOR9(RETVAL,BGPSTR) ;-- gpra patient forcast report
 ...
 S RETVAL="^BGPTMP("_$J_")"
 S BGPI=0
 S BGPOPT="CRS 08 PATIENT FORECAST REPORT 09"
 I $G(BGPSTR)="" D CATSTR^BGP8GU(.BGPSTR,.BGPSTR)
 S BGPLST=$P(BGPSTR,P)
 S BGPDIV=$P(BGPSTR,P,2)
 I BGPDIV]"" S BGPDIV=$O(^DG(40.8,"B",BGPDIV,0))
 ...
 .. S BGPPAT(BGPCNT)=BGPDA
 S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 D EP^BGP8GFO9(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPLST,BGPDIV,BGPBADT,BGPEADT,.BGPCLN,.BGPPAT,BGPRT)
 S BGPI=BGPI+1
 S ^BGPTMP($J,BGPI)=+$G(BGPERR)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)
 D EN^XBVK("BGP")
