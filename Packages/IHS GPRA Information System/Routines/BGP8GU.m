BGP8GU ; IHS/CMI/LAB - BGP Gui Utilities 10/29/2004 3:28:39 PM 19 Sep 2005 5:28 PM 27 Apr 2008 10:28 PM ;
...
ERR(ERR) ; BMX ADO SCHEMA ERROR PROCESSOR
 N X
 S X="0|"_ERR_$C(31)  ;cmi/maw changed to end of table marker crs 8.0 p2
 S @RETVAL@(1)=X
 Q
 ;
...
LST(RETVAL,BGPSTR) ;-- list files
 ...
 Q
 ;
DELRPT(RETVAL,BGPSTR) ;-- delete a report out of the BGP 08 GUI REPORT file
...
CHK(BGPRET,BGPSTR) ;check report status
 ...
 N BGPDA
 S BGPDA=0 F  S BGPDA=$O(^BGPGUIE("AUSR",BGPDUZ,BGPDA)) Q:'BGPDA  D
 . N BGPIEN
 . S BGPIEN=0 F  S BGPIEN=$O(^BGPGUIE("AUSR",BGPDUZ,BGPDA,BGPIEN)) Q:'BGPIEN  D
 .. N BGPDATA,BGPNM,BGPUSER,BGPST,BGPET,BGPTOR,BGPRS,BGPTOO
 .. S BGPDATA=$G(^BGPGUIE(BGPIEN,0))
 .. S BGPNM=$P(BGPDATA,U)
 ...
 ... S DIK="^BGPGUIE(",DA=BGPIEN D ^DIK
 .. S BGPTOR=$P(BGPDATA,U,5)
 .. S BGPRS=$$GET1^DIQ(90534.08,BGPIEN,.06)
 .. S BGPTOO=$$GET1^DIQ(90534.08,BGPIEN,.07)
 .. S BGPI=BGPI+1
...
LABTAXC(RETVAL,BGPSTR) ;EP - from remote procedure
 S X="MERR^BGP8GU",@^%ZOSF("TRAP") ; m error trap
 N BGPI,BGPJ,BGPDATA,BGPDA,BGPRT,P,BGPOPT,BGPR
 S P="|"
 S BGPI=0
 S BGPERR=""
 S BGPOPT=$P(BGPSTR,P)
 S BGPOPT="CRS 08 LAB TAXONOMY REPORT"  ;8.0 p2
 ...
 S RETVAL="^BGPTMP("_$J_")"
 S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 D EP^BGP8GLTX(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPRT,BGPR)
 S BGPI=BGPI+1
 S ^BGPTMP($J,BGPI)=+$G(BGPERR)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)
 D EN^XBVK("BGP")
...
MEDTAXC(RETVAL,BGPSTR) ;EP - from remote procedure
 ...
 S BGPERR=""
 S BGPOPT=$P(BGPSTR,P)
 S BGPOPT="CRS 08 MEDICATION TAXONOMY REPORT"  ;8.0 p2
 S BGPRT=$P(BGPSTR,P,2)
 S BGPR=$P(BGPSTR,P,3)
 S BGPJ=0
 K ^BGPTMP($J)
 S RETVAL="^BGPTMP("_$J_")"
 S ^BGPTMP($J,BGPI)="T00250DATA"_$C(30)
 D EP^BGP8GMTX(.BGPERR,DUZ,DUZ(2),BGPOPT,BGPRT,BGPR)
 S BGPI=BGPI+1
 S ^BGPTMP($J,BGPI)=+$G(BGPERR)_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)
 D EN^XBVK("BGP")
