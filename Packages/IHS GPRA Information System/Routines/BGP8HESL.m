BGP8HESL ; IHS/CMI/LAB - IHS gpra print 28 Apr 2008 3:42 PM ;
...
GO .;
 ...
 .S X=$P(^BGPHEIE(BGPL,0),U,3) D S(X,1,1)
 .S X=" " D S(X,1,1)
 .S X="Denominator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPHEIE(BGPL,61,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPHEIE(BGPL,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...I $P(^BGPHEIE(BGPL,61,BGPY,0),U,2)'[BGPRTYPE Q  ;not a denom def for this report
 ...S BGPZ=0 F  S BGPZ=$O(^BGPHEIE(BGPL,61,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S Y=^BGPHEIE(BGPL,61,BGPY,1,BGPZ,0) D S(Y,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .S X="Numerator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPHEIE(BGPL,62,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPHEIE(BGPL,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...I $P(^BGPHEIE(BGPL,62,BGPY,0),U,2)'[BGPRTYPE Q  ;not a denom def for this report
 ...S BGPZ=0 F  S BGPZ=$O(^BGPHEIE(BGPL,62,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S X=^BGPHEIE(BGPL,62,BGPY,1,BGPZ,0) D S(X,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPHEIE(BGPL,11,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPHEIE(BGPL,11,BGPX,0) D S(X,1,1)
 .S X=" " D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPHEIE(BGPL,51,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPHEIE(BGPL,51,BGPX,0) D S(X,1,1)
 .S X=" " D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPHEIE(BGPL,52,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPHEIE(BGPL,52,BGPX,0) D S(X,1,1)
 .D H1
...
HEADER1 ;
 ...
 W !,$$CTR("***  IHS 2008 HEDIS Clinical Performance Report Patient List  ***",80),!
...
H1 ;
 ...
 S Y=0 F  S Y=$O(^BGPHEIE(BGPL,71,Y)) Q:Y'=+Y  S X=^BGPHEIE(BGPL,71,Y,0) D S(X,1,1)
 S X=" " D S(X,1,1)
