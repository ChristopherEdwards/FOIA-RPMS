BGP8NPL ; IHS/CMI/LAB - national patient list 20 Dec 2004 9:24 AM ; 30 Jun 2008  5:21 PM
...
INTRO ;
 ...
 W !,"a National GPRA Report performance measure or a list of both those patients"
 W !,"who met and those who did not meet a National GPRA Report performance"
...
TP ;get time period
 D XIT
 S BGPRTYPE=1,BGP8RPTH="",BGPNPL=1,BGPINDT="G",BGP8GPU=1
SI ;
 K DIRUT
 K BGPIND
 D EN^BGP8DSI
...
SI1 ;NOW SELECT ONE OR MORE W/IN THE TOPIC
 ...
 .K BGPX S BGPO=0,X=0,BGPC=0 F  S BGPO=$O(^BGPNPLE(BGPCR,BGPIND,BGPO)) Q:BGPO'=+BGPO!($D(DIRUT))  D
...
SUM ;display summary of this report
 W:$D(IOF) @IOF
 W !,$$CTR("SUMMARY OF ",$S(BGPRTYPE=7:"OTHER",1:"")_"NATIONAL GPRA REPORT TO BE GENERATED")
 W !,$$CTR($$RPTVER^BGP8BAN,80)
 W !!,"The date ranges for this report are:"
 W !?5,"Report Period: ",?31,$$FMTE^XLFDT(BGPBD)," to ",?31,$$FMTE^XLFDT(BGPED)
 W !?5,"Previous Year Period: ",?31,$$FMTE^XLFDT(BGPPBD)," to ",?31,$$FMTE^XLFDT(BGPPED)
...
TPNON ;get time period
 D XIT
 S BGPRTYPE=7,BGP8RPTH="",BGPNPL=1,BGPINDT="G",BGP8GPU=1,BGPONMR=1,BGPRTC="U"
 G SI
