BGP8PD19 ; IHS/CMI/LAB - print ind ; 03 Jul 2008  8:56 AM
...
I1AGE1 ;
 S C=C+1
 S BGPF="013.B.1"_BGPX S BGPPC=$O(^BGPINDEC("C",BGPF,0))
...
I1AGE2 ;
 S D=D+1
 S BGPF="013.B.2"_BGPX S BGPPC=$O(^BGPINDEC("C",BGPF,0))
...
I1AGEP ;
 ...
 S X=" " D S(X,1,1) S X="# A. Refusals w/ % of Total Visits" D S(X,1,1)
 F X=1:1:8 S V=$P(BGPDAC(X),U,4) S Y=V D S(Y,,X+1)
 S X="% A. Refusals w/ % of Total Visits" D S(X,1,1)
 F X=1:1:8 S V=$P(BGPDAC(X),U,5) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
PR ; 
 ;S X=^BGPINDE(BGPIC,53,1,0) D S(X,1,1) D H9
