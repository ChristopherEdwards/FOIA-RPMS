...
OTHSUM ;EP
 ...
 ...S X=" "_$P(^BGPINDEC(BGPPC,19),U,4)
 ...I $P(^BGPINDEC(BGPPC,19),U,7)]"" D S(X,1,1) S X=" "_$P(^BGPINDEC(BGPPC,19),U,7)
 ...
 ...S $P(X,U,5)=$P(^BGPINDEC(BGPPC,19),U,2),$P(X,U,6)=$P(^BGPINDEC(BGPPC,19),U,3)
 ...D S(X,1,1)
 ..E  D
 ...S X=" "_$P(^BGPINDEC(BGPPC,19),U,4)
 ...I $P(^BGPINDEC(BGPPC,19),U,7)]"" D S(X,1,1) S X=" "_$P(^BGPINDEC(BGPPC,19),U,7)
 ...
 ...S $P(X,U,5)=$TR($P(^BGPINDEC(BGPPC,19),U,2),"$","^"),$P(X,U,6)=$P(^BGPINDEC(BGPPC,19),U,3)
 ...D S(X,1,1)
 ...S X="" I $P(^BGPINDEC(BGPPC,19),U,9)]""!($P(^BGPINDEC(BGPPC,19),U,10)]"")!($P(^BGPINDEC(BGPPC,19),U,11)]"") S $P(X,U,5)=$TR($P(^BGPINDEC(BGPPC,19),U,10),"$","^") D
 ....S $P(X,U,6)=$P(^BGPINDEC(BGPPC,19),U,11)
 ...I X]"" D S(X,1,1)
 D SDP
 Q
 ;
H1 ;
 ...
 S X="",$P(X,U,2)="Current",$P(X,U,3)="Previous",$P(X,U,4)="Baseline",$P(X,U,5)="2007",$P(X,U,6)="Goal" D S(X,1,1)
...
H2 ;
 ...
 S X="",$P(X,U,2)="Current",$P(X,U,3)="Previous",$P(X,U,4)="Baseline",$P(X,U,5)="Current",$P(X,U,6)="2007",$P(X,U,7)="Goal" D S(X,1,1)
 S X=$TR($J("",80)," ","-") D S(X,1,1)
 Q
SDPX ;
 I BGPRTYPE'=7 Q  ;national gpra only
 ...
 ..S X=" " D S(X,1,1)
 ..S XX=" "_$P(^BGPINDEC(BGPPC,19),U,4)
 ..I $P(^BGPINDEC(BGPPC,19),U,7)]"" D S(XX,1,1) S XX=" "_$P(^BGPINDEC(BGPPC,19),U,7)
 ..I $P(^BGPINDEC(BGPPC,19),U,12)]"" D S(XX,1,1) S XX=" "_$P(^BGPINDEC(BGPPC,19),U,12)
 ..S F=$O(^TMP($J,"SUMMARYDEL DETAIL PAGE OTHER",BGPC,BGPO,BGPPC,0))
 ..S F=$P(^TMP($J,"SUMMARYDEL DETAIL PAGE OTHER",BGPC,BGPO,BGPPC,F),U,4)
 ..S $P(XX,U,5)=$P(^BGPINDEC(BGPPC,19),U,2),$P(XX,U,6)=$P(^BGPINDEC(BGPPC,19),U,3)
 ...
 ....S Y="" I $P(^BGPINDEC(BGPPC,19),U,9)]""!($P(^BGPINDEC(BGPPC,19),U,10)]"")!($P(^BGPINDEC(BGPPC,19),U,11)]"") S $P(Y,U,6)=$TR($P(^BGPINDEC(BGPPC,19),U,9),"$","^"),$P(Y,U,7)=$TR($P(^BGPINDEC(BGPPC,19),U,10),"$","^") D
 .....S $P(Y,U,8)=$P(^BGPINDEC(BGPPC,19),U,11)
 ....I Y]"" D S(Y,1,1)
 ...D S(X,1,1)
 Q
SDP ;
 ...
 ..S $P(XX,U,6)=$P(^BGPINDEC(BGPPC,19),U,2),$P(XX,U,7)=$P(^BGPINDEC(BGPPC,19),U,3)
 ..S BGPSN=0,BGPCNT=0 F  S BGPSN=$O(^TMP($J,"SUMMARYDEL DETAIL PAGE OTHER",BGPC,BGPO,BGPPC,BGPSN)) Q:BGPSN'=+BGPSN  S BGPCNT=BGPCNT+1 D
 ...S BGPSASU=$P(^BGPGPDCE(BGPSN,0),U,9),X=$O(^AUTTLOC("C",BGPSASU,0)),BGPSNAM=$S(X:$P(^DIC(4,X,0),U),1:"?????"),BGPSNAM=$S($P(^BGPGPDCE(BGPSN,0),U,17):"+"_BGPSNAM,1:BGPSNAM)
 ...I $P(^BGPINDEC(BGPPC,0),U,4)["014."!($P(^BGPINDEC(BGPPC,0),U,4)["023.")!($P(^BGPINDEC(BGPPC,0),U,4)["016")!($P($G(^BGPINDEC(BGPPC,19)),U,13)) D  I 1
 ....S X="",$P(X,U,1)=BGPSASU_" "_BGPSNAM
 ...
 ....;S $P(X,U,5)=$P(^BGPINDEC(BGPPC,19),U,2),$P(X,U,6)=$P(^BGPINDEC(BGPPC,19),U,3)
 ...I BGPCNT=1 D S(XX,1,1) D
 ....;S Y="" I $P(^BGPINDEC(BGPPC,19),U,9)]""!($P(^BGPINDEC(BGPPC,19),U,10)]"") S $P(Y,U,6)=$TR($P(^BGPINDEC(BGPPC,19),U,9),"$","^"),$P(Y,U,7)=$TR($P(^BGPINDEC(BGPPC,19),U,10),"$","^")
 ....S Y="" I $P(^BGPINDEC(BGPPC,19),U,9)]""!($P(^BGPINDEC(BGPPC,19),U,10)]"")!($P(^BGPINDEC(BGPPC,19),U,11)]"") S $P(Y,U,6)=$TR($P(^BGPINDEC(BGPPC,19),U,9),"$","^"),$P(Y,U,7)=$TR($P(^BGPINDEC(BGPPC,19),U,10),"$","^") D
 .....S $P(Y,U,8)=$P(^BGPINDEC(BGPPC,19),U,11)
 ....I Y]"" D S(Y,1,1)
 ...D S(X,1,1)
