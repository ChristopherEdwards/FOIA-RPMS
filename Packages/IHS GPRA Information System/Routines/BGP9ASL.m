BGP9ASL ; IHS/CMI/LAB - DISPLAY IND LISTS 28 Apr 2006 2:10 PM ;
...
EN ;EP -- main entry point
 D EN^VALM("BGP 09 AREA SELECT SITES")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ; -- init variables and list array
 I BGPRTYPE=1 D NGR^BGP9ASL1 Q
 I BGPRTYPE=3 D HED^BGP9ASL1 Q
 I BGPRTYPE=5 D ELD^BGP9ASL1 Q
 I BGPRTYPE=6 D PED^BGP9ASL1 Q
 I BGPRTYPE=7 D ONM^BGP9ASL1 Q
 I BGPRTYPE=8 D EO^BGP9ASL1 Q
 Q
FAC(S) ;
...
G1 ;
 ...
 S BGPBEN=1
 S BGPFILE=90536.03
 S BGPX=0 F  S BGPX=$O(^BGPGPDCN(BGPX)) Q:BGPX'=+BGPX  D
 . Q:'$D(^BGPGPDCN(BGPX,0))
 . S V=^BGPGPDCN(BGPX,0)
 ...
 . S BGPFAC=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPGPDCN(BGPX,0),U,17):" (CHS ONLY)",1:"")
 . S BGPEBD=$$DATE^BGP9UTL($P(V,U))
 . S BGPEED=$$DATE^BGP9UTL($P(V,U,2))
 . S BGPEBBD=$$DATE^BGP9UTL($P(V,U,5))
 . S BGPEBED=$$DATE^BGP9UTL($P(V,U,6))
 . S BGPEDRR=$$DATE^BGP9UTL($P(V,U,13))
...
G3 ;
 ...
 S BGPFILE=90537.03
 S BGPX=0 F  S BGPX=$O(^BGPHEDCN(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPHEDCN(BGPX,0))
 . S V=^BGPHEDCN(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
 ...
 .Q:$P(V,U,14)'=BGPBEN
 . S BGPSU=$E($$SU($P(V,U,11)),1,10)
 . S BGPFAC=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPHEDCN(BGPX,0),U,17):" (CHS ONLY)",1:"")
 . S BGPEBD=$$DATE^BGP9UTL($P(V,U))
 . S BGPEED=$$DATE^BGP9UTL($P(V,U,2))
 . S BGPEBBD=$$DATE^BGP9UTL($P(V,U,5))
 . S BGPEBED=$$DATE^BGP9UTL($P(V,U,6))
...
G5 ;
 ...
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPFILE=90538.03
 S BGPX=0 F  S BGPX=$O(^BGPELDCN(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPELDCN(BGPX,0))
 . S V=^BGPELDCN(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
 ...
 .Q:$P(V,U,14)'=BGPBEN
 . S BGPSU=$E($$SU($P(V,U,11)),1,10)
 . S BGPFAC=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPELDCN(BGPX,0),U,17):" (CHS ONLY)",1:"")
 . S BGPEBD=$$DATE^BGP9UTL($P(V,U))
 . S BGPEED=$$DATE^BGP9UTL($P(V,U,2))
 . S BGPEBBD=$$DATE^BGP9UTL($P(V,U,5))
 . S BGPEBED=$$DATE^BGP9UTL($P(V,U,6))
...
G6 ;
 ...
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPFILE=90538.03
 S BGPX=0 F  S BGPX=$O(^BGPPEDCN(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPPEDCN(BGPX,0))
 . S V=^BGPPEDCN(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
...
G9 ;
 ...
 S BGPBBD=BGPBD-X,BGPBBD=$E(BGPBBD,1,3)_$E(BGPBD,4,7)
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPX=0 F  S BGPX=$O(^BGPGPDCN(BGPX)) Q:'BGPX  D
 . Q:'$D(^BGPGPDCN(BGPX,0))
 . S V=^BGPGPDCN(BGPX,0)
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
...
G8 ;childhood weight
 ...
 S BGPBBD=BGPBD-X,BGPBBD=$E(BGPBBD,1,3)_$E(BGPBD,4,7)
 S BGPBED=BGPED-X,BGPBED=$E(BGPBED,1,3)_$E(BGPED,4,7)
 S BGPX=0 F  S BGPX=$O(^BGPEOCN(BGPX)) Q:'BGPX  I BGPX S V=^BGPEOCN(BGPX,0) D
 .Q:$P(V,U)'=BGPBD
 .Q:$P(V,U,2)'=BGPED
 .Q:$P(V,U,7)'=BGPPER
 .Q:$P(V,U,12)'=8
 .;Q:'$O(^BGPEOCN(BGPX,88888,0))
 .Q:$P(V,U,5)'=BGPBBD
 .Q:$P(V,U,6)'=BGPBED
 .Q:$P(V,U,14)'=BGPBEN
 . S BGPSU=$E($$SU($P(V,U,11)),1,10)
 . S BGPFAC=$E($$FAC($P(V,U,9)),1,13)_$S($P(^BGPEOCN(BGPX,0),U,17):" (CHS ONLY)",1:"")
 . S BGPEBD=$$DATE^BGP9UTL($P(V,U))
 . S BGPEED=$$DATE^BGP9UTL($P(V,U,2))
 . S BGPEBBD=$$DATE^BGP9UTL($P(V,U,5))
 . S BGPEBED=$$DATE^BGP9UTL($P(V,U,6))
