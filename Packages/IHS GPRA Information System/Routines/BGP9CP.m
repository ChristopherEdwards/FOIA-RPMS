BGP9CP ; IHS/CMI/LAB - IHS gpra print 02 Jul 2008 9:06 AM ; 
...
HDR1 ;
 Q:'BGPTEXD
 S BGPX=0 F  S BGPX=$O(^BGPCMSMN(BGPPLSTL,21,BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 .I $Y>(BGPIOSL-3) D HDR Q:BGPQUIT
 .W !,^BGPCMSMN(BGPPLSTL,21,BGPX,0)
 Q
LIST1 ;DISPLAY LIST 1
 K BGPL2
 I $O(^BGPCMSMN(BGPPLSTL,73,0)) D:BGPTEXD HDR D  ;if there is a first page header
 .S BGPX=0 F  S BGPX=$O(^BGPCMSMN(BGPPLSTL,73,BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 ..I $Y>(BGPIOSL-3) D HDR Q:BGPQUIT
 ..W !,^BGPCMSMN(BGPPLSTL,73,BGPX,0)
 I $O(^BGPCMSMN(BGPPLSTL,73,0)) W !,$TR($J("",80)," ","-")
 Q:BGPQUIT
 D L1H
 Q:BGPQUIT
 S BGPAST=0
 I '$D(^XTMP("BGP9C1",BGPJ,BGPH,"LIST",BGPIND,BGPPLSTL)) W !!,"No Visits to report" Q
 S BGPNAME="" F  S BGPNAME=$O(^XTMP("BGP9C1",BGPJ,BGPH,"LIST",BGPIND,BGPPLSTL,BGPNAME)) Q:BGPNAME=""!(BGPQUIT)  D
 .S DFN=0 F  S DFN=$O(^XTMP("BGP9C1",BGPJ,BGPH,"LIST",BGPIND,BGPPLSTL,BGPNAME,DFN)) Q:DFN'=+DFN!(BGPQUIT)  D
 ..S BGPVSIT=0 F  S BGPVSIT=$O(^XTMP("BGP9C1",BGPJ,BGPH,"LIST",BGPIND,BGPPLSTL,BGPNAME,DFN,BGPVSIT)) Q:BGPVSIT'=+BGPVSIT!(BGPQUIT)  D
 ...S BGPVSIT0=$G(^AUPNVSIT(BGPVSIT,0))
 ...S BGPVINP=$O(^AUPNVINP("AD",BGPVSIT,0))
 ...I $Y>(BGPIOSL-4) D HDR Q:BGPQUIT  D L1H
 ...W !!
 ...S BGPPEX=^XTMP("BGP9C1",BGPJ,BGPH,"LIST",BGPIND,BGPPLSTL,BGPNAME,DFN,BGPVSIT)
 ...
 ...I '$P(^BGPCMSMN(BGPPLSTL,0),U,6) D
 ....W ?42,$$DATE^BGP9UTL($P($P(BGPVSIT0,U),"."))_"-"_$$DATE^BGP9UTL($$DSCH^BGP9CU(BGPVINP)) I BGPIND=1!(BGPIND=3) W $S(BGPPEX[2!(BGPPEX[1):"*",1:"")
 ....W ?62,$E($$VAL^XBDIQ1(9000010.02,BGPVINP,.07),1,18)
 ...I $P(^BGPCMSMN(BGPPLSTL,0),U,6) D
 ....W ?42,$E($$VAL^XBDIQ1(9000001,DFN,1111),1,19)
 ....W ?62,$$DATE^BGP9UTL($P($P(BGPVSIT0,U),"."))_"-"_$$DATE^BGP9UTL($$DSCH^BGP9CU(BGPVINP))
 ...S BGPCOUNT(BGPIND,BGPPLSTL)=$G(BGPCOUNT(BGPIND,BGPPLSTL))+1
 ...X ^BGPCMSMN(BGPPLSTL,1)
 Q:BGPQUIT
 I $Y>(BGPIOSL-4) D HDR Q:BGPQUIT  D L1H
 W !!,"TOTAL VISITS: ",+$G(BGPCOUNT(BGPIND,BGPPLSTL))
 Q:'$P(^BGPCMSMN(BGPPLSTL,0),U,5)
 W !!,"TOTAL VISITS THAT WOULD BE EXCLUDED: ",+$G(BGPEXCP(BGPIND,BGPPLSTL))
 Q
L1H ;EP - list one header
 I $Y>(BGPIOSL-3) D HDR Q:BGPQUIT
 ;W !,$TR($J("",80)," ","-")
 I '$P(^BGPCMSMN(BGPPLSTL,0),U,6) W !,"PATIENT NAME",?27,"HRN",?34,"SEX",?38,"AGE",?42,"HOSP DATES",?62,"ADMISSION TYPE"
 I $P(^BGPCMSMN(BGPPLSTL,0),U,6) W !,"PATIENT NAME",?27,"HRN",?34,"SEX",?38,"AGE",?42,"CLASS/BENEFICIARY",?64,"HOSP DATES"
 W !,$TR($J("",80)," ","-")
 ;I $G(BGPEXCL)=1 Q
 I '$P(^BGPCMSMN(BGPPLSTL,0),U,5) Q
...
HEADER1 ;
 ...
 W $$CTR("*** IHS 2009 CMS Hospital Quality Reporting Initiative ***",80)
 ...
 I $G(BGPIND) S X="Topic:  "_$P(^BGPCMSIN(BGPIND,0),U,3) W !,$$CTR(X,80)
 I $G(BGPPLSTL),$P($G(^BGPCMSMN(BGPPLSTL,71)),U,2)]"" S X="Performance Measure: "_$P(^BGPCMSMN(BGPPLSTL,71),U,2) W !,$$CTR(X,80)
 I $G(BGPPLSTL) S X="Patient List:  "_$P(^BGPCMSMN(BGPPLSTL,0),U,4) W !,$$CTR(X,80)
 W !,$TR($J("",80)," ","-")
 ;I $G(BGPPLSTL) W !,$P($G(^BGPCMSMN(BGPPLSTL,71)),U) D
 ;.I $P($G(^BGPCMSMN(BGPPLSTL,71)),U,3)]"" W !,$P($G(^BGPCMSMN(BGPPLSTL,71)),U,3)
