...
PROC ;EP
 ...
 D XTMP^BGP9UTL("BGP08CPL","National GPRA Comp List")
 D XTMP^BGP9UTL("BGP9DNP","National GPRA Patient List")
 D XTMP^BGP9UTL("BGP9D","CRS Local Patient List")
...
N ;
 I BGPRTYPE=1,$G(BGPEXPT),'$G(BGP9GPU) S ^BGPGPDCN(BGPRPT,88888,0)="^90536.0388888A^"_BGPCHWC_"^"_BGPCHWC
 I $G(BGPYWCHW) S ^BGPGPDCN(BGPRPT,88888,0)="^90536.0388888A^"_BGPCHWC_"^"_BGPCHWC
 S BGPET=$H
 Q
 ;
PROCCY ;EP - current time period
 ...
 S BGPEDATE=BGPED,BGPTIME=1,BGPBDATE=BGPBD,BGPGBL=$S(BGPRTYPE=3:"^BGPHEDCN(",BGPRTYPE=5:"^BGPELDCN(",BGPRTYPE=8:"^BGPEOCN(",1:"^BGPGPDCN(")
 S BGP365=BGPBDATE
 ...
 I $G(BGPIISO) D ISS^BGP9DPA1 K ^TMP($J,"A") Q  ;SCHEDULING OPTION
 D CALCIND
 I BGPRTYPE=1,$G(BGPYWCHW),'$G(BGP9GPU) D
...
PROCPY ;
 ...
 Q:$P(^DPT(DFN,0),U,2)=""
 Q:"FM"'[$P(^DPT(DFN,0),U,2)
 S BGPEDATE=BGPPED,BGPTIME=2,BGPBDATE=BGPPBD,BGPGBL=$S(BGPRTYPE=3:"^BGPHEDPN(",BGPRTYPE=5:"^BGPELDPN(",BGPRTYPE=8:"^BGPEOPN(",1:"^BGPGPDPN(")
 S BGP365=BGPBDATE
...
PROCBY ;
 ...
 Q:$P(^DPT(DFN,0),U,2)=""
 Q:"FM"'[$P(^DPT(DFN,0),U,2)
 S BGPEDATE=BGPBED,BGPTIME=3,BGPBDATE=BGPBBD,BGPGBL=$S(BGPRTYPE=3:"^BGPHEDBN(",BGPRTYPE=5:"^BGPELDBN(",BGPRTYPE=8:"^BGPEOBN(",1:"^BGPGPDBN(")
 S BGP365=BGPBDATE
 ...
 Q
CALCIND ;
 I BGPRTYPE=3 D CALCIND^BGP9DCHI Q
 I BGPRTYPE=5 D CALCIND^BGP9DCEI Q
 I BGPRTYPE=8 D CALCIND^BGP9EOCI Q
 D CALCIND^BGP9DCI
 Q
