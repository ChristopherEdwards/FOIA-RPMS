BGP9D53 ; IHS/CMI/LAB - measure calc ;
...
IK ;EP
 ...
 S BGPNV=$$CHL^BGP9D53(DFN,BGP365,BGPEDATE)
 S BGPN1=+BGPNV
...
CHL(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP9UTL($P(BGPG(1),U))_U_"V73.88"
 S %=P_"^LAST DX V73.98;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)) Q 1_U_"V73.88"_U_$$DATE^BGP9UTL($P(BGPG(1),U))
 ;check cpt taxonomy
 S T=$O(^ATXAX("B","BGP CHLAMYDIA CPTS",0))
 I T D  I X Q 1_U_$P(X,U,3)_U_$$DATE^BGP9UTL($P(X,U,2))
 .S X=$$CPT^BGP9DU(P,BDATE,EDATE,T,6) I X]"" Q
 .S X=$$TRAN^BGP9DU(P,BDATE,EDATE,T,6)
 ...
 ...Q:'$D(^AUPNVLAB(X,0))
 ...I BGPLT,$P(^AUPNVLAB(X,0),U),$D(^ATXLAB(BGPLT,21,"B",$P(^AUPNVLAB(X,0),U))) S BGPC=1_U_"lab test"_U_$$DATE^BGP9UTL((9999999-D)) Q
 ...Q:'T
 ...S J=$P($G(^AUPNVLAB(X,11)),U,13) Q:J=""
 ...Q:'$$LOINC(J,T)
 ...S R=$P(^AUPNVLAB(X,0),U,4)
 ...S BGPC=1_U_"lab loinc"_U_$$DATE^BGP9UTL((9999999-D))
 ...Q
 I BGPC Q BGPC
 ...
 ...Q:'$D(^AUPNVMIC(X,0))
 ...I BGPLT,$P(^AUPNVMIC(X,0),U),$D(^ATXLAB(BGPLT,21,"B",$P(^AUPNVMIC(X,0),U))) S BGPC=1_U_"v micro test"_U_$$DATE^BGP9UTL((9999999-D)) Q
 ...Q:'T
 ...S J=$P($G(^AUPNVMIC(X,11)),U,13) Q:J=""
 ...Q:'$$LOINC(J,T)
 ...S R=$P(^AUPNVMIC(X,0),U,4)
 ...S BGPC=1_U_"v micro loinc"_U_$$DATE^BGP9UTL((9999999-D))
 ...Q
 Q BGPC
LOINC(A,B) ;
 NEW %
