BGP9D722 ; IHS/CMI/LAB - measure AHR.A ;
 ;;9.0;IHS CLINICAL REPORTING;;JUL 1, 2009
 ;
 ;
ACEALG(P,BDATE,EDATE) ;EP
 K BGPG
 D ACEIALG1^BGP9C11(P,EDATE,.BGPG)
 S X=$O(BGPG(X))
 I 'X Q ""
...
ACECONT(P,BDATE,EDATE,NMIB,NMIE) ;EP does patient have an ACEI contraidication
 ...
 I $D(BGPG(1)) Q 1_U_"ACEI contra POV:  "_$$DATE^BGP9UTL($P(BGPG(1),U))_" ["_$P(BGPG(1),U,2)_"]    "_$$VAL^XBDIQ1(9000010.07,+$P(BGPG(1),U,4),.04)
 ;
 ...
 ...S BGPG=1_U_"NMI ACEI: "_$$VAL^XBDIQ1(9000022,N,.04)_"   "_$$DATE^BGP9UTL($P(^AUPNPREF(N,0),U,3))_"  "_$$VAL^XBDIQ1(9000022,X,1101)
 ..Q
 .Q
 I BGPG Q BGPG
 ;nmi in refusal file for ACEI
 ...
 ..S N=0 F  S N=$O(^AUPNPREF("AA",P,50,X,D,N)) Q:N'=+N  D
 ...Q:$P($G(^AUPNPREF(N,0)),U,7)'="N"
 ...S BGPG=1_U_"NMI ARB: "_$$VAL^XBDIQ1(9000022,N,.04)_"   "_$$DATE^BGP9UTL($P(^AUPNPREF(N,0),U,3))_"  "_$$VAL^XBDIQ1(9000022,X,1101)
 ..Q
 .Q
 I BGPG Q BGPG
 ;S X=$$CPTI^BGP9DU(P,NMIB,NMIE,+$$CODEN^ICPTCOD("G8029"))
 ;I X Q 1_U_"arb contra CPT code G8029: "_$$DATE^BGP9UTL($P(X,U,2))
 ;S X=$$TRANI^BGP9DU(P,NMIB,NMIE,+$$CODEN^ICPTCOD("G8029"))
 ;I X Q 1_U_"arb contra Tran Code G8029: "_$$DATE^BGP9UTL($P(X,U,2))
 Q ""
...
ACEREF(P,BDATE,EDATE) ;EP
 ...
 ...S G=1_U_"ACEI Refusal "_$$DATE^BGP9UTL(Y)
 ..Q
 .Q
 I G Q G
 S T=$O(^ATXAX("B","BGP HEDIS ARB MEDS",0))
 ...
 ..S N=0 F  S N=$O(^AUPNPREF("AA",P,50,X,D,N)) Q:N'=+N!(G)  D
 ...;Q:$P($G(^AUPNPREF(N,0)),U,7)'="N"
 ...S G=1_U_"ARB Refusal "_$$DATE^BGP9UTL(Y)
 ..Q
 .Q
 Q G
STATALG(P,BDATE,EDATE,RPB,RPE) ;EP
 ...
 .I N["STATIN"!(N["STATINS") S BGPC=BGPC+1,BGPY(BGPC)=1_U_"alg statin POV:  "_$$DATE^BGP9UTL($P(BGPG(X),U))_"  ["_$P(BGPG(X),U,2)_"]  "_N
 I BGPC Q BGPC
...
STATCON(P,BDATE,EDATE,NMIB,NMIE) ;EP does patient have an STATIN contraidication
 ...
 ;
 ;pregnant
 S X=$$PREG^BGP9D7(P,BDATE,EDATE) I X Q 1_U_"contra statin - pregnant"
 ...
 ..S N=0 F  S N=$O(^AUPNPREF("AA",P,50,X,D,N)) Q:N'=+N  D
 ...Q:$P($G(^AUPNPREF(N,0)),U,7)'="N"
 ...S BGPG=1_U_"NMI STATIN: "_$$VAL^XBDIQ1(9000022,N,.04)_"   "_$$DATE^BGP9UTL($P(^AUPNPREF(N,0),U,3))_"  "_$$VAL^XBDIQ1(9000022,X,1101)
 ..Q
 .Q
 I BGPG Q BGPG
 ;breastfeeding
 K BGPG S Y="BGPG(",X=P_"^LAST DX V24.1;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE) S E=$$START1^APCLDF(X,Y)
 I $D(BGPG(1)) Q 1_U_"STATIN contra POV:  "_$$DATE^BGP9UTL($P(BGPG(1),U))_" ["_$P(BGPG(1),U,2)_"]    "_$$VAL^XBDIQ1(9000010.07,+$P(BGPG(1),U,4),.04)
 ;now check education
 K BGPG
 S Y="BGPG("
...
STATREF(P,BDATE,EDATE) ;EP
 ...
 ..S N=0 F  S N=$O(^AUPNPREF("AA",P,50,X,D,N)) Q:N'=+N!(G)  D
 ...Q:$P($G(^AUPNPREF(N,0)),U,7)'="R"
 ...S G=1_U_"Statin Refusal "_$$DATE^BGP9UTL(Y)
 ..Q
 .Q
 Q G
