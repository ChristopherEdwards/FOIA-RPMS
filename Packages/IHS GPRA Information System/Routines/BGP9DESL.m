BGP9DESL ; IHS/CMI/LAB - FY 09 DISPLAY IND LISTS ;
...
EN ;EP -- main entry point for GPRA LIST DISPLAY
 D EN^VALM("BGP 09 ELDER LIST SELECTION")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ1
 ...
 ;
HDR ; -- header code
 S VALMHDR(1)="IHS FY 09 ELDER Performance Measure Lists of Patients"
...
INIT ; -- init variables and list array
 ...
 .I $P(^BGPELIN(X,0),U,5)]"" S C=C+1 D  Q
 ..S BGPGLIST(C,0)=C_")",$E(BGPGLIST(C,0),5)=$P(^BGPELIN(X,0),U,5),BGPGLIST("IDX",C,C)=X I $D(BGPLIST(X)) S BGPGLIST(C,0)="*"_BGPGLIST(C,0)
 .I $P(^BGPELIN(X,0),U,5)="" S C=C+1 D
 ..S BGPGLIST(C,0)="NO patient list available for measure:  "_$P(^BGPELIN(X,0),U,4),BGPGLIST("IDX",C,C)=X,BGPNOLI(X)="" I $D(BGPLIST(X)) S BGPGLIST(C,0)="*"_BGPGLIST(C,0)
 S (VALMCNT,BGPHIGH)=C
 Q
 ;
HELP ; -- help code
