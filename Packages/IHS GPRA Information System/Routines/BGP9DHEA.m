...
ASU ;
 ...
 I C=1 S BGPRPTT="F",BGPSUCNT=1,Y=$O(BGPSUL(0)),X=$P(^BGPHEDCN(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 I C>1 S BGPRPTT="A"
ZIS ;call to XBDBQUE
 D ^XBFMK
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP9DHEA",ZTDTH="",ZTDESC="HEDIS REPORT" D ^%ZTLOAD D EXIT Q
 Q
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP9DHEA",XBRX="EXIT^BGP9DHEA",XBNS="BGP"
 D ^XBDBQUE
 Q
 ;
NODEV1 ;
