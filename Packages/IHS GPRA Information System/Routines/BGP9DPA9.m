...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP9DPA9",ZTDTH="",ZTDESC="CRS 09 SCHED REPORT FOR 09" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D ^XBFMK
 K DIC,DIR,DFN
...
BDMG(BGPBD,BGPED,BGPCLN) ;EP - GUI DMS Entry Point
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BGP9DPA9",ZTDESC="GUI CN GRPA APPT" D ^%ZTLOAD
 D EOJ
 Q
GUIEP ;EP
 D PROC
 K ^TMP($J,"BGP9DPA")
 S IOM=80
 D GUIR^XBLM("PRINT^BGP9DPA9","^TMP($J,""BGP9DPA"",")
...
GUIECP ;EP
 K ^TMP($J,"BGP9DPA")
 S IOM=80
 D GUIR^XBLM("CPPRINT^BGP9DPA9","^TMP($J,""BGP9DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP9DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP9DPA",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIN(BGPIEN,11,X,0)=BGPDATA,C=C+1
