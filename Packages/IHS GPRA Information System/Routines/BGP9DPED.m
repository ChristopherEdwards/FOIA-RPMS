BGP9DPED ; IHS/CMI/LAB - IHS gpra print ;
...
DEL ;EP - create delimited output file
 ...
 K BGPSUMP
 D DEL1
 D ^BGP9DPEE ;print lists to delimited file
 ...
 I '$D(BGPGUI) D
 .S XBFLT=1,XBFN=BGPDELF_".txt",XBMED="F",XBTLE="GPRA 09 DELIMITED PT ED OUTPUT",XBQ="N",XBF=0
 .D ^XBGSAVE
...
DEL1 ;EP
 ...
 .D S(" ",1,1),S(" ",1,1)
 .S X=$P(^BGPPEIN(BGPIC,0),U,2) D S(X,1,1)
 .D S(" ",1,1)
 .S X="Denominator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPPEIN(BGPIC,61,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIN(BGPIC,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPPEIN(BGPIC,61,BGPY,11,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S Y=^BGPPEIN(BGPIC,61,BGPY,11,BGPZ,0) D S(Y,1,1)
 ....Q
 ...Q
 ..Q
 .D S(" ",1,1)
 .S X="Numerator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPPEIN(BGPIC,62,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIN(BGPIC,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPPEIN(BGPIC,62,BGPY,11,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S X=^BGPPEIN(BGPIC,62,BGPY,11,BGPZ,0) D S(X,1,1)
 ....Q
 ...Q
 ..Q
 .D S(" ",1,1)
 .S BGPNODE=11
 .S BGPX=0 F  S BGPX=$O(^BGPPEIN(BGPIC,BGPNODE,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPPEIN(BGPIC,BGPNODE,BGPX,0) D S(X,1,1)
 .D @BGPIC
 Q
EXIT ;
 K ^TMP($J)
...
C(X,X2,X3) ;
 ...
 ;
1 ;
 D S(" ",1,1) ;S X=$P(^BGPPEIN(BGPIC,0),U,2) D S(X,1,1)
 D H1^BGP9PDL1
 D S(" ",1,1)
 ...
 Q
2 ;
 D S(" ",1,1) D S(" ",1,1) ;S X=$P(^BGPPEIN(BGPIC,0),U,2) D S(X,1,1)
 D H1^BGP9PDL1
 D S(" ",1,1)
 ...
 ..I $P($G(^BGPPEDCN(X,11)),U,4)]"",$P(^BGPPEDCN(X,11),U,4)<BGPCYD S BGPCYD=$P(^BGPPEDCN(X,11),U,4)
 ..I $P($G(^BGPPEDPN(X,11)),U,4)]"",$P(^BGPPEDPN(X,11),U,4)<BGPPRD S BGPPRD=$P(^BGPPEDPN(X,11),U,4)
 ..I $P($G(^BGPPEDBN(X,11)),U,4)]"",$P(^BGPPEDBN(X,11),U,4)<BGPBLD S BGPBLD=$P(^BGPPEDBN(X,11),U,4)
 ...
 .S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 ..I $P($G(^BGPPEDCN(X,11)),U,5)>BGPCYD S BGPCYD=$P(^BGPPEDCN(X,11),U,5)
 ..I $P($G(^BGPPEDPN(X,11)),U,5)>BGPPRD S BGPPRD=$P(^BGPPEDPN(X,11),U,5)
 ..I $P($G(^BGPPEDBN(X,11)),U,5)>BGPBLD S BGPBLD=$P(^BGPPEDBN(X,11),U,5)
 S X="" D S(X,1,1)
