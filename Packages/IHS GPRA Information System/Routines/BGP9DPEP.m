BGP9DPEP ; IHS/CMI/LAB - IHS gpra print 01 Jul 2008 7:54 PM ; 
 ;;9.0;IHS CLINICAL REPORTING;;JUL 1, 2009
 ;
 ;
PRINT ;
 S BGPRTYPE=6,BGP9RPTH=""
 K ^TMP($J)
 K BGPDELIM
 I BGPROT="D" G DEL
...
PRINT1 ;EP
 ...
 .W !!,$P(^BGPPEIN(BGPIC,0),U,2)
 .I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 .W !!,"Denominator(s):"
 .S BGPX=0 F  S BGPX=$O(^BGPPEIN(BGPIC,61,"B",BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIN(BGPIC,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY!(BGPQUIT)  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPPEIN(BGPIC,61,BGPY,11,BGPZ)) Q:BGPZ'=+BGPZ!(BGPQUIT)  D
 ....I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ....W !,^BGPPEIN(BGPIC,61,BGPY,11,BGPZ,0)
 ....Q
 ...Q
 ..Q
 .I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 .W !!,"Numerator(s):"
 .S BGPX=0 F  S BGPX=$O(^BGPPEIN(BGPIC,62,"B",BGPX)) Q:BGPX'=+BGPX!(BGPQUIT)  D
 ..S BGPY=0 F  S BGPY=$O(^BGPPEIN(BGPIC,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY!(BGPQUIT)  D
 ...S BGPZ=0 F  S BGPZ=$O(^BGPPEIN(BGPIC,62,BGPY,11,BGPZ)) Q:BGPZ'=+BGPZ!(BGPQUIT)  D
 ....I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ....W !,^BGPPEIN(BGPIC,62,BGPY,11,BGPZ,0)
 ....Q
 ...;W !
 ...Q
 ..Q
 .S BGPNODE=11
 .W ! S BGPX=0 F  S BGPX=$O(^BGPPEIN(BGPIC,BGPNODE,BGPX)) Q:BGPX'=+BGPX  D
 ..I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT
 ..W !,^BGPPEIN(BGPIC,BGPNODE,BGPX,0)
 .D @BGPIC
 .Q:BGPQUIT
 ;
 D ^BGP9DPEL
 ...
 Q
1 ;
 I $Y>(BGPIOSL-6) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIN(BGPIC,0),U,2)
 D H1^BGP9DPH
...
2 ;
 ...
 ..I $Y>(BGPIOSL-3) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIN(BGPIC,0),U,2) D H1^BGP9DPH W !
 ...
 I $Y>(BGPIOSL-6) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIN(BGPIC,0),U,2) D H1^BGP9DPH W !
 ...
 ..I $P($G(^BGPPEDCN(X,11)),U,4)]"",$P($G(^BGPPEDCN(X,11)),U,4)<BGPCYD S BGPCYD=$P($G(^BGPPEDCN(X,11)),U,4)
 ..I $P($G(^BGPPEDPN(X,11)),U,4)]"",$P($G(^BGPPEDPN(X,11)),U,4)<BGPPRD S BGPPRD=$P($G(^BGPPEDPN(X,11)),U,4)
 ..I $P($G(^BGPPEDBN(X,11)),U,4)]"",$P($G(^BGPPEDBN(X,11)),U,4)<BGPBLD S BGPBLD=$P($G(^BGPPEDBN(X,11)),U,4)
 ...
 I $Y>(BGPIOSL-4) D HEADER Q:BGPQUIT  W !,$P(^BGPPEIN(BGPIC,0),U,2) D H1^BGP9DPH W !
 W !,"Minimum Time Spent",!,"All Providers",!,"(minutes)"
 W ?20,$$C(BGPCYD,0,8),?35,$$C(BGPPRD,0,8),?58,$$C(BGPBLD,0,8),!
 I $G(BGPAREAA) D
 .S (BGPCYD,BGPPRD,BGPBLD)=""
 .S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 ..I $P($G(^BGPPEDCN(X,11)),U,5)>BGPCYD S BGPCYD=$P($G(^BGPPEDCN(X,11)),U,5)
 ..I $P($G(^BGPPEDPN(X,11)),U,5)>BGPPRD S BGPPRD=$P($G(^BGPPEDPN(X,11)),U,5)
 ..I $P($G(^BGPPEDBN(X,11)),U,5)>BGPBLD S BGPBLD=$P($G(^BGPPEDBN(X,11)),U,5)
 I '$G(BGPAREAA) D
...
HEADER1 ;
 ...
 I '$G(BGPSEAT) W $$CTR("*** IHS 2009 Patient Education with Community Specified Report ***",80),!
 I $G(BGPSEAT) W $$CTR("*** IHS 2009 Patient Education with Patient Panel Population Report ***",80),!
...
SETNM ;
 ...
 S X=0 F  S X=$O(^BGPPEDCN(BGPRPT,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDCN(BGPRPT,N,X,0),U),L=$P(^BGPPEDCN(BGPRPT,N,X,0),U,2),M=$P(^BGPPEDCN(BGPRPT,N,X,0),U,3)
 .;I $D(BGPPROVS(C,L)) S $P(BGPPROVS(C,L),U,1)=M Q
 .S $P(BGPPROVS(C,L),U,1)=M
 S X=0 F  S X=$O(^BGPPEDPN(BGPRPT,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDPN(BGPRPT,N,X,0),U),L=$P(^BGPPEDPN(BGPRPT,N,X,0),U,2),M=$P(^BGPPEDPN(BGPRPT,N,X,0),U,3)
 .;I $D(BGPPROVS(C,L)) S $P(BGPPROVS(C,L),U,2)=M Q
 .S $P(BGPPROVS(C,L),U,2)=M
 S X=0 F  S X=$O(^BGPPEDBN(BGPRPT,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDBN(BGPRPT,N,X,0),U),L=$P(^BGPPEDBN(BGPRPT,N,X,0),U,2),M=$P(^BGPPEDBN(BGPRPT,N,X,0),U,3)
...
SETNMA ;
 ...
 Q
SETNMA1 ;
 S X=0 F  S X=$O(^BGPPEDCN(Z,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDCN(Z,N,X,0),U),L=$P(^BGPPEDCN(Z,N,X,0),U,2),M=$P(^BGPPEDCN(Z,N,X,0),U,3)
 .;I $D(BGPPROVS(C,L)) S $P(BGPPROVS(C,L),U,1)=M Q
 .S $P(BGPPROVS(C,L),U,1)=$P($G(BGPPROVS(C,L)),U,1)+M
 S X=0 F  S X=$O(^BGPPEDPN(Z,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDPN(Z,N,X,0),U),L=$P(^BGPPEDPN(Z,N,X,0),U,2),M=$P(^BGPPEDPN(Z,N,X,0),U,3)
 .;I $D(BGPPROVS(C,L)) S $P(BGPPROVS(C,L),U,2)=M Q
 .S $P(BGPPROVS(C,L),U,2)=$P($G(BGPPROVS(C,L)),U,2)+M
 S X=0 F  S X=$O(^BGPPEDBN(Z,N,X)) Q:X'=+X  D
 .S C=$P(^BGPPEDBN(Z,N,X,0),U),L=$P(^BGPPEDBN(Z,N,X,0),U,2),M=$P(^BGPPEDBN(Z,N,X,0),U,3)
 .;I $D(BGPPROVS(C,L)) S $P(BGPPROVS(C,L),U,3)=M Q
...
SETEXA(T,N,P) ;EP - set denominator
 ...
 NEW X,Y,Z
 S X=0 F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 .I T=1 S $P(BGPEXCT(X,T),U,2)=$P($G(^BGPPEDCN(X,N)),U,P)
 .I T=2 S $P(BGPEXCT(X,T),U,2)=$P($G(^BGPPEDPN(X,N)),U,P)
 .I T=3 S $P(BGPEXCT(X,T),U,2)=$P($G(^BGPGPDBN(X,N)),U,P)
 Q
V(T,R,N,P,ND) ;EP ;SPDX
 I $G(BGPAREAA) G VA
 NEW X
 I T=1 S X=$P($G(^BGPPEDCN(R,N)),U,P) Q $S(X]"":X,1:0)
 I T=2 S X=$P($G(^BGPPEDPN(R,N)),U,P) Q $S(X]"":X,1:0)
 I T=3 S X=$P($G(^BGPPEDBN(R,N)),U,P) Q $S(X]"":X,1:0)
 Q ""
VA ;
 NEW X,V,C S X=0,C="" F  S X=$O(BGPSUL(X)) Q:X'=+X  D
 .I T=1 S C=C+$P($G(^BGPPEDCN(X,N)),U,P)
 .I T=2 S C=C+$P($G(^BGPPEDPN(X,N)),U,P)
 .I T=3 S C=C+$P($G(^BGPPEDBN(X,N)),U,P)
 .Q
 Q $S(C]"":C,1:0)
