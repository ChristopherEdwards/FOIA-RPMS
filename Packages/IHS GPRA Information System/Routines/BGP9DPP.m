BGP9DPP ; IHS/CMI/LAB - IHS GPRA 09 SELECTED REPORT DRIVER ALL PATIENTS 29 Apr 2009 8:36 PM ; 
 ;;9.0;IHS CLINICAL REPORTING;;JUL 1, 2009
 ;
 ;
 W:$D(IOF) @IOF
 W !!,$$CTR("2009 Clinical Performance Measure Report (Selected Measures)",80)
...
SUM ;display summary of this report
 W:$D(IOF) @IOF
 W !,$$CTR("SUMMARY OF 2009 CLINICAL MEASURE PERFORMANCE REPORT TO BE GENERATED")
 W !!,"The date ranges for this report are:"
 W !?5,"Report Period: ",?31,$$FMTE^XLFDT(BGPBD)," to ",?31,$$FMTE^XLFDT(BGPED)
 W !?5,"Previous Year Period: ",?31,$$FMTE^XLFDT(BGPPBD)," to ",?31,$$FMTE^XLFDT(BGPPED)
 W !?5,"Baseline Period: ",?31,$$FMTE^XLFDT(BGPBBD)," to ",?31,$$FMTE^XLFDT(BGPBED)
 ...
 W !!,"These measures will be calculated: " S X=0 F  S X=$O(BGPIND(X)) Q:X'=+X  W $P(^BGPINDN(X,0),U,3)," ; "
 W !!,"Lists will be produced for these measures: "
 S X=0 F  S X=$O(BGPLIST(X)) Q:X'=+X  W $P(^BGPINDN(X,0),U,3)," ; "
 D PT^BGP9DSL
 I BGPROT="" G LISTS
ZIS ;call to XBDBQUE
 D REPORT^BGP9UTL
...
NODEV1 ;
 ...
 Q
DI ;
 S X=0 F  S X=$O(^BGPINDNC("ADM",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDNC(X,0),U,1))=""
 Q
CI ;
 S X=0 F  S X=$O(^BGPINDNC("ACARD",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDNC(X,0),U,1))=""
 Q
WI ;
 S X=0 F  S X=$O(^BGPINDNC("AWH",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDNC(X,0),U,1))=""
 Q
EI ;
 S X=0 F  S X=$O(^BGPINDNC("AEL",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDNC(X,0),U,1))=""
 Q
PI ;
 S X=0 F  S X=$O(^BGPINDNC("APR",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDNC(X,0),U,1))=""
 Q
SI ;
 K BGPIND
 D EN^BGP9DSI
...
LISTS ;any lists with measures?
 ...
 I $D(BGPLIST) D RT^BGP9DSL I '$D(BGPLIST)!($D(BGPQUIT)) G LISTS ;get report type for each list
 Q
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP9DPP",ZTDTH="",ZTDESC="GPRA 09 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP9DPP",XBRX="XIT^BGP9DPP",XBNS="BGP"
 D ^XBDBQUE
 ;D XIT
 Q
 ;
