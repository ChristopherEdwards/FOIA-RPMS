BGP9DSTM ; IHS/CMI/LAB - national patient list 20 Dec 2004 9:24 AM 07 Mar 2008 2:31 PM ; 
...
TP ;get time period
 D XIT
 S BGPRTYPE=1,BGP9RPTH="",BGPNPL=1,BGPINDT="G",BGP9GPU=1,BGP9NPLT=1
SI ;
 K DIRUT
 K BGPIND
 D EN^BGP9DSI
...
SI1 ;NOW SELECT ONE OR MORE W/IN THE TOPIC
 ...
 .K BGPX S BGPO=0,X=0,BGPC=0 F  S BGPO=$O(^BGPNPLN("AN",BGPIND,BGPO)) Q:BGPO'=+BGPO!($D(DIRUT))  D
 ..S X=$O(^BGPNPLN(BGPCR,BGPIND,BGPO,0))
 ..;I BGPRTYPE=1,$P(^BGPNPLN(X,0),U,4)'="N" Q
 ..;I BGPRTYPE=7,$P(^BGPNPLN(X,0),U,4)'="O" Q
 ..S BGPX(BGPO,X)="",BGPC=BGPC+1
 .;display the choices
 .W !!!,"Please select one or more of these report choices within the",!,IORVON,$P(^BGPINDN(BGPIND,0),U,3),IORVOFF," performance measure topic.",!
 .K BGPY S X=0,BGPC=0,BGPO=0 F  S BGPO=$O(BGPX(BGPO)) Q:BGPO'=+BGPO!($D(DIRUT))  S X=0 F  S X=$O(BGPX(BGPO,X)) Q:X'=+X!($D(DIRUT))  S BGPC=BGPC+1 W !?5,BGPC,")",?9,$P(^BGPNPLN(X,0),U,3) S BGPY(BGPC)=X
 ...
 I $D(BGPQUIT) G TP
 I '$D(BGPINDL) G TP
 D RT^BGP9DSL I '$D(BGPLIST)!($D(BGPQUIT)) G SI1
...
SUM ;display summary of this report
 ...
 .W !?2,$P(^BGPNPLN(Y,0),U,3),":  ",$P(^DIBT(BGPINDL(X,Y,"TEMP"),0),U)
 D PT^BGP9DSL
 I BGPROT="" G COMM
ZIS ;call to XBDBQUE
 D REPORT^BGP9UTL
...
DRIVER ;
 ...
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP9NPL",XBRX="XIT^BGP9NPL",XBNS="BGP"
 D ^XBDBQUE
 Q
 ;
NODEV1 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP9NPL",ZTDTH="",ZTDESC="NATIONAL GPRA & PART REPORT 06" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
EN1 ;EP Help
 ...
 S X=0 F  S X=$O(^BGPNPLN(BGPII,11,X)) Q:X'=+X  W !?3,^BGPNPLN(BGPII,11,X,0)
...
CT ;EP - create search templates and write message
 W !
 I '$G(BGPDELIM) D HEADER^BGP9DPH
 I '$G(BGPDELIM) W !,$$REPEAT^XLFSTR("-",80)
 I $G(BGPDELIM) S X=" " D S^BGP9NPLD(X,1,1) S X=" " D S^BGP9NPLD(X,1,1)
 S X=0 F  S X=$O(^XTMP("BGP9DNP",BGPJ,BGPH,"LIST",X)) Q:X'=+X  D
 .S Y=0 F  S Y=$O(^XTMP("BGP9DNP",BGPJ,BGPH,"LIST",X,Y)) Q:Y'=+Y  D
 ..S T=$G(BGPINDL(X,Y,"TEMP"))
 ..I T="" Q
 ..I '$D(^DIBT(T,0)) Q
 ..L +^DIBT(T):10 I '$T Q
 ..S C=0
 ..S P=0 F  S P=$O(^XTMP("BGP9DNP",BGPJ,BGPH,"LIST",X,Y,P)) Q:P'=+P  D
 ...S C=C+1
 ...S ^DIBT(T,1,P)=""
 ...Q
 ..L -^DIBT(T)
 ..I '$G(BGPDELIM) W !!,"Search template: ",$P(^DIBT(T,0),U)," created with ",C," members." Q
 ..S X="Search template: "_$P(^DIBT(T,0),U)_" created with "_C_" members." D S^BGP9NPLD(X,1,1)
 .Q
 Q
