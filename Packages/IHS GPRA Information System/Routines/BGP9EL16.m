BGP9EL16 ; IHS/CMI/LAB - print ind 1 ;
...
I1AGEP ;
 ...
 W !,"# w/Retinal Evaluation or",!," refusal (GPRA)"
...
PR ; 
 ...
 W !,"# C. w/ Other Eye",!," exams"
 S T=27 F X=1:1:4 S N=$P(BGPDAC(X),U,8),O=$P(BGPDAP(X),U,9) W ?T,$J($FN((N-O),"+,",1),6) S T=T+12
BL ;
 D HEADER^BGP9DPH Q:BGPQUIT  W !,^BGPELIN(BGPIC,53,1,0) D H3
 ...
 S T=27 F X=1:1:4 S N=$P(BGPDAC(X),U,7),O=$P(BGPDAB(X),U,7) W ?T,$J($FN((N-O),"+,",1),6) S T=T+12
  W !,"# C. w/ Other Eye",!," exams"
 S T=27 F X=1:1:4 S N=$P(BGPDAC(X),U,8),O=$P(BGPDAB(X),U,9) W ?T,$J($FN((N-O),"+,",1),6) S T=T+12
 Q
