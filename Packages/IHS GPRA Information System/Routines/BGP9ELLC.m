BGP9ELLC ; IHS/CMI/LAB - print ind ;
...
I1AGEP ;
 ...
 S X="# w/Mammogram recorded w/in 2 yrs (GPRA)" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,2) S Y=V D S(Y,,X+1)
 S X="% w/Mammogram recorded w/in 2 yrs (GPRA)" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,3) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="A. # w/Refusals w/ % of Total Mammo" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,4) S Y=V D S(Y,,X+1)
 S X="% A. w/Refusals w/ % of Total Mammo" D S(X,1,1)
