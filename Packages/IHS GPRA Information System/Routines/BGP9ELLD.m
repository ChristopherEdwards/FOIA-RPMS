BGP9ELLD ; IHS/CMI/LAB - print ind ;
...
FEMALE ;
 ...
 Q
I1AGE1 ;
 S BGPF="ELD."_$P(BGPX,".")_"."_($P(BGPX,".",2)+I) S BGPPC=$O(^BGPELIIN("C",BGPF,0))
 I I=0 D
...
I1AGEP ;
 ...
 S BGPL1="w/CRC screening (GPRA)",P1=2,P2=3 D DSP
 S BGPL1="A. w/Refusals w/ % of Total CRC",P1=4,P2=5 D DSP
...
PR ; 
 ...
 S BGPL1="w/CRC screening (GPRA)",P1=2,P2=3 D DSPP
 S BGPL1="A. w/Refusals w/ % of Total CRC",P1=4,P2=5 D DSPP
 ...
 S X="A. w/Refusals w/ % of Total CRC" D S(X,1,1)
...
BL ;
 ...
 S BGPL1="w/CRC screening (GPRA)",P1=2,P2=3 D DSPB
 S BGPL1="A. w/Refusals w/ % of Total CRC",P1=4,P2=5 D DSPB
...
DSP ;
 ...
 S X="% "_BGPL1 D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,P2) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 Q
DSPP ;
 S X="# "_BGPL1 D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,P1) S Y=V D S(Y,,X+1)
 S X="% "_BGPL1 D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,P2) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 Q
 ;
DSPB ;
 S X="# "_BGPL1 D S(X,1,1)
 ...
 S X="% "_BGPL1 D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAB(X),U,P2) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 Q
 ;
V(T,R,N,P) ;EP
 I $G(BGPAREAA) G VA
