BGP9ELLO ; IHS/CMI/LAB - print ind ;
...
I1AGE ;EP
 ...
 K BGPDAC,BGPDAP,BGPDAB S (C,D,E,F,G,H,I,J,K)=0 F BGPX="OSB.1","OSB.2","OSB.3","OSB.4" D I1AGE1,I1AGE2
 D I1AGEP
 Q
I1AGE1 ;
 S C=C+1
...
I1AGEP ;
 ...
 S X="# w/osteoporosis screening in past 2 years" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,2) S Y=V D S(Y,,X+1)
 S X="% w/osteoporosis screening in past 2 years" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,3) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="# A. w/Refusals w/ % of Total Screening" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,4) S Y=V D S(Y,,X+1)
 S X="% A. w/Refusals w/ % of Total Screening" D S(X,1,1)
