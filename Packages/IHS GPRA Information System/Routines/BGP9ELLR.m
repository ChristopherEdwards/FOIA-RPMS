BGP9ELLR ; IHS/CMI/LAB - print ind ;
...
I1AGE1 ;
 S C=C+1
 S BGPF="EFR."_BGPX S BGPPC=$O(^BGPELIIN("C",BGPF,0))
...
I1AGE2 ;
 S D=D+1
 S BGPF="EFR."_$P(BGPX,".")_"."_($P(BGPX,".",2)+3) S BGPPC=$O(^BGPELIIN("C",BGPF,0))
...
I1AGE3 ;
 S E=E+1
 S BGPF="EFR."_$P(BGPX,".")_"."_($P(BGPX,".",2)+6) S BGPPC=$O(^BGPELIIN("C",BGPF,0))
 ;set 4th piece to numerator and 5th to %
 S BGPNF=$P(^BGPELIIN(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90538.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
I1AGE4 ;
 S F=F+1
 S BGPF="EFR."_$P(BGPX,".")_"."_($P(BGPX,".",2)+9) S BGPPC=$O(^BGPELIIN("C",BGPF,0))
 ;set 4th piece to numerator and 5th to %
 S BGPNF=$P(^BGPELIIN(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90538.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
I1AGE5 ;
 S G=G+1
 S BGPF="EFR."_$P(BGPX,".")_"."_($P(BGPX,".",2)+12) S BGPPC=$O(^BGPELIIN("C",BGPF,0))
 ;set 4th piece to numerator and 5th to %
 S BGPNF=$P(^BGPELIIN(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90538.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
I1AGE6 ;
 S H=H+1
 S BGPF="EFR."_$P(BGPX,".")_"."_($P(BGPX,".",2)+15) S BGPPC=$O(^BGPELIIN("C",BGPF,0))
 ;set 4th piece to numerator and 5th to %
 S BGPNF=$P(^BGPELIIN(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90538.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
PR ; 
 ...
 S X="% w/ fall risk screen/Dx/refusal" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,3) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="A. # w/ fall risk screen" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,4) S Y=V D S(Y,,X+1)
 S X="% A. w/ fall risk screen" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,5) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1)
 S X="B. # w/ history of fall" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,6) S Y=V D S(Y,,X+1)
 S X="% B. w/ history of fall" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,7) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="C. # w/ fall injury" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,8) S Y=V D S(Y,,X+1)
 S X="% C. w/ fall injury" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,9) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="D. # w/ abnormal gait" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,10) S Y=V D S(Y,,X+1)
 S X="% D. w/ abnormal gait" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,11) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="E. # w/ refusal" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,12) S Y=V D S(Y,,X+1)
 S X="% E. w/ refusal" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,13) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 ;percentage changes
 S X=" " D S(X,1,1) S X="CHANGE FROM PREV YR %" D S(X,1,1)
 S X="# w/ fall risk screen/Dx/refusal" D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,3),O=$P(BGPDAP(X),U,3) S Y=$$SB^BGP9ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="A. w/ fall risk screen" D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,5),O=$P(BGPDAP(X),U,5) S Y=$$SB^BGP9ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="B. w/ history of fall" D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,7),O=$P(BGPDAP(X),U,7) S Y=$$SB^BGP9ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="C. w/ fall injury" D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,9),O=$P(BGPDAP(X),U,9) S Y=$$SB^BGP9ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="D. w/ abnormal gait" D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,11),O=$P(BGPDAP(X),U,11) S Y=$$SB^BGP9ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="E. w/ refusal" D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,13),O=$P(BGPDAP(X),U,13) S Y=$$SB^BGP9ELL2($J((N-O),6,1)) D S(Y,,X+1)
BL ;
 ...
 S X="% w/ fall risk screen/Dx/refusal" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAB(X),U,3) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="A. # w/ fall risk screen" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAB(X),U,4) S Y=V D S(Y,,X+1)
 S X="% A. w/ fall risk screen" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAB(X),U,5) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1)
 S X="B. # w/ history of fall" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAB(X),U,6) S Y=V D S(Y,,X+1)
 S X="% B. w/ history of fall" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAB(X),U,7) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="C. # w/ fall injury" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAB(X),U,8) S Y=V D S(Y,,X+1)
 S X="% C. w/ fall injury" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAB(X),U,9) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="D. # w/ abnormal gait" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAB(X),U,10) S Y=V D S(Y,,X+1)
 S X="% D. w/ abnormal gait" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAB(X),U,11) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="E. # w/ refusal" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAB(X),U,12) S Y=V D S(Y,,X+1)
 S X="% E. w/ refusal" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAB(X),U,13) S Y=$$SB^BGP9ELL2($J(V,6,1)) D S(Y,,X+1)
 ;percentage changes
 S X=" " D S(X,1,1) S X="CHANGE FROM BASE YR %" D S(X,1,1)
 S X="# w/ fall risk screen/Dx/refusal" D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,3),O=$P(BGPDAB(X),U,3) S Y=$$SB^BGP9ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="A. w/ fall risk screen" D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,5),O=$P(BGPDAB(X),U,5) S Y=$$SB^BGP9ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="B. w/ history of fall" D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,7),O=$P(BGPDAB(X),U,7) S Y=$$SB^BGP9ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="C. w/ fall injury" D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,9),O=$P(BGPDAB(X),U,9) S Y=$$SB^BGP9ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="D. w/ abnormal gait" D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,11),O=$P(BGPDAB(X),U,11) S Y=$$SB^BGP9ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="E. w/ refusal" D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,13),O=$P(BGPDAB(X),U,13) S Y=$$SB^BGP9ELL2($J((N-O),6,1)) D S(Y,,X+1)
 Q
