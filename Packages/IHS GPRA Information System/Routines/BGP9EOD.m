BGP9EOD ; IHS/CMI/LAB - IHS GPRA 09 REPORT DRIVER ;
...
CHOICE ;
 ...
 W !?11,"July 1, 2008 - June 30, 2009, baseline period of July 1, 1999"
 W !?11,"- June 30, 2009, and AI/AN patients only)"
...
INTRO ;
 ...
 .W !,"This report will be run for the Report Period July 1, 2008 through "
 .W !,"June 30, 2009 with a Baseline Year of July 1, 1999 through June 30, 2000."
...
NT .;END TEST STUFF
 ...
 I $D(DIRUT) D XIT Q
 S BGPINDZ=Y
 I BGPINDZ="S" D EN^BGP9EOSI I '$D(BGPIND) W !!,"No measures selected" G CHOICE
 I BGPINDZ="A" S X=0 F  S X=$O(^BGPEOMN(X)) Q:X'=+X  S BGPIND(X)=""
 D TAXCHK^BGP9XTEO
TP ;get time period
 S BGPRTYPE=8
 S (BGPBD,BGPED,BGPTP)=""
...
ZIS ;call to XBDBQUE
 ...
 I BGPRPT="" D XIT Q
 I BGPEXPT D
 .W !!,"A file will be created called BG09",$P(^AUTTLOC(DUZ(2),0),U,10)_".EO"_BGPRPT," and will reside",!,"in the ",BGPUF," directory.",!
 ...
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPEOCN(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPEOPN(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPEOBN(" D ^DIK K DIK D XIT Q
 I $D(IO("Q")) G TSKMN
DRIVER ;
 D ^BGP9D1
 U IO
 D ^BGP9EOP
 D ^%ZISC
 I BGPEXPT D GS^BGP9EOUT
 D XIT
 Q
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP9EOD",XBRX="XIT^BGP9EOD",XBNS="BGP"
 D ^XBDBQUE
 Q
 ;
NODEV1 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP9EOD",ZTDTH="",ZTDESC="EO 2008 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
LISTS ;any lists with measures?
 ...
 I $D(BGPLIST) D RT^BGP9EOSL I '$D(BGPLIST)!($D(BGPQUIT)) G LISTS ;get report type for each list
 Q
CHKY ;
