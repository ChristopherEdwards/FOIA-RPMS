BGP9GAHE ; IHS/CMI/LAB - AREA NATIONAL GPRA REPORT ;
...
TESTNTL ;
 ...
 S BGPSUL(1)=""
 S BGPSUL(2)=""
 D EP(.ERR,1,2522,"BGP 09 AREA HEDIS","A",.LORISUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRPTT,BGPSUL,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPROT,BGPRTIME,BGPFILE) ;EP - called from GUI to produce national gpra report (AO-AGP)
 ...
 ;            from BGP 09 HEDIS DATA CURRENT that match the following:
 ;            (SEE BGP9ASL or CALL ME)
 ;  BGPSUL  - array containing Iens from BGP 09 DATA CURRENT that the user selected
 ;  BGPSUL - ARRAY OF IENS FROM THE D GET^BGP9ASL(.BGPSUL,.BGPFILE,3,....... call that the user selected
...
EP1 ;
 ...
 S BGPSUCNT=0
 S BGPSUCNT=C
 I BGPSUCNT=1 S Y=$O(BGPSUL(0)),X=$P(^BGPHEDCN(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOHED^BGP9GAHE",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI AREA HEDIS REPORT 06" D ^%ZTLOAD Q
 Q
AOHED ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^XBLM("PRINT^BGP9PHEP","^TMP($J,""BGPGUI"",")
