BGP9GAON ; IHS/CMI/LAB - AREA OTHER NATIONAL MEASURES GPRA REPORT 01 Jul 2008 1:10 PM ; 
...
TESTNTL ;
 ...
 S BGPSUL(1)=""
 S BGPSUL(2)=""
 D EP(.ERR,1,2522,"BGP 09 AREA OTHER NATIONAL MEASURES REPORT","A",.BGPSUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRPTT,BGPSUL,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPROT,BGPRTIME,BGPHC,BGPFILE) ;EP - called from GUI to produce national gpra report (AO-AGP)
 ...
 ;  BGPSUL  - array containing Iens from BGP 09 ONM DATA CURRENT that the user selected
 ;  BGPSUL - ARRAY OF IENS FROM THE D GET^BGP9ASL(.BGPSUL,.BGPFILE,5,....... call that the user selected
...
EP1 ;
 ...
 S BGPSUCNT=0
 S BGPSUCNT=C
 I BGPSUCNT=1 S Y=$O(BGPSUL(0)),X=$P(^BGPGPDCN(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 ...
 S IOM=80,BGPIOSL=55
 S BGPAREAA=1
 S BGPRTYPE=7,BGP9RPTH=""
 S BGPHOME=$P($G(^BGPSITE(DUZ(2),0)),U,2)
 S X=0 F  S X=$O(^BGPINDN("ON",1,X)) Q:X'=+X  S BGPIND(X)=""
 S BGPINDT="G"
 I '$G(BGPHC) D
 ...
 S BGPNOW=$$NOW^XLFDT() S BGPNOW=$P(BGPNOW,".")_"."_$$RZERO^BGP9UTL($P(BGPNOW,".",2),6)
 S BGPFONN1="CRSONMNT1"_$P(^AUTTLOC(DUZ(2),0),U,10)_2008063000000000_$$D^BGP9UTL(BGPNOW)_"_"_$$LZERO^BGP9UTL(BGPC,6)_".TXT"
 S BGPFONN2="CRSONMNT2"_$P(^AUTTLOC(DUZ(2),0),U,10)_2008063000000000_$$D^BGP9UTL(BGPNOW)_"_"_$$LZERO^BGP9UTL(BGPC,6)_".TXT"
 S BGPFONN3="CRSONMNT3"_$P(^AUTTLOC(DUZ(2),0),U,10)_2008063000000000_$$D^BGP9UTL(BGPNOW)_"_"_$$LZERO^BGP9UTL(BGPC,6)_".TXT"
 ...
 D TSKMN
 S BGPRET=BGPGIEN
 S $P(BGPRET,"~",3)=$G(BGPFONN1)_","_$G(BGPFONN2)_","_$G(BGPFONN3)
 Q
 ;
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AONM^BGP9GAON",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI AREA ONM REPORT" D ^%ZTLOAD Q
 Q
AONM ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 ;cmi/anch/maw added 5/12/2008 for word output
 D GUIR^XBLM("PRINT^BGP9PARQ","^TMP($J,""BGPGUI"",")
