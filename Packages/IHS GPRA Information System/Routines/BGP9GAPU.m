BGP9GAPU ; IHS/CMI/LAB - AREA NATIONAL GPRA REPORT ;
...
TESTNTL ;
 ...
 S BGPSUL(1)=""
 S BGPSUL(2)=""
 D EP(.ERR,1,2522,"BGP 09 AREA GPU","A",.BGPSUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRPTT,BGPSUL,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPROT,BGPRTIME,BGPFILE) ;EP - called from GUI to produce national gpra report (AO-AGP)
 ...
 ;  ;  BGPSUL - ARRAY OF IENS FROM THE D GET^BGP9ASL(.BGPSUL,.BGPFILE,9,....... call that the user selected
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOAGP^BGP9GAPU",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT 06" D ^%ZTLOAD Q
 Q
AOAGP ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
