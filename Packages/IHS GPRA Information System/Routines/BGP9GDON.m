BGP9GDON ; IHS/CMI/LAB - GUI Other National Measurer Report ;
 ;;9.0;IHS CLINICAL REPORTING;**1**;JUL 01, 2009
 ;
 ;
TESTNTL ;
 S ERR=""
 D EP(.ERR,1,2522,"BGP 09 OTHER NATIONAL MEASURES REPORT",338,1,3040000,3000000,1,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPEXPT,BGPROT,BGPRTIME,BGPMFITI,BGPHC,BGPFILE) ;EP - called from GUI to produce national GPU report (OTH-GPU)
 ;  BGPUSER - DUZ
 ;  BGPDUZ2 - DUZ(2)
 ;  BGPOPTN - OPTION NAME
...
EP1 ;
 ...
 S BGPGUI=1
 S IOM=80,BGPIOSL=55
 S BGPRTYPE=7,BGP9RPTH=""  ;,BGP9GPU=1
 ...
 S BGPRTC="U"
 I $G(BGPHC) D  ;cmi/maw 6/18/2008 change the dates if they select hard coded
 ...
 S BGPFILEN="" I BGPEXPT S BGPFILEN="BG09"_$P(^AUTTLOC(DUZ(2),0),U,10)_".ONM"_BGPRPT_"  in directory "_BGPUF
 S BGPDELT=""
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="OTHONM^BGP9GDON",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI ONM REPORT 09" D ^%ZTLOAD Q
 Q
OTHONM ;
 D ^BGP9D1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^XBLM("^BGP9DP","^TMP($J,""BGPGUI"",")
