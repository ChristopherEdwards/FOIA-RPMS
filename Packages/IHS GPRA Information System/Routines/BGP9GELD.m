BGP9GELD ; IHS/CMI/LAB - GUI COM REPORT ;
...
TESTELD ;
 ...
 S BGPND(4)="",BGPND(3)=""
 S BGPLIST(3)=""
 D EP(.ERR,1,2522,"BGP 09 ELDER REPORT",338,"S",.BGPND,1,3040000,3000000,1,"A",.BGPLIST,"","",1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
...
EP1 ;
 ...
 I BGPZZ="A" S X=0 F  S X=$O(^BGPELIN(X)) Q:X'=+X  S BGPIND(X)=""
 I BGPZZ='"A" S BGPEXPT=""
...
BY ;get baseline year
 ...
 S BGPFILEN=""
 I BGPEXPT S BGPFILEN="BG09"_$P(^AUTTLOC(DUZ(2),0),U,10)_".EL"_BGPRPT_" in directory "_BGPUF
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="OTHELD^BGP9GELD",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI ELDER 05 REPORT" D ^%ZTLOAD Q
 Q
OTHELD ;
 D ^BGP9D1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^XBLM("^BGP9DELP","^TMP($J,""BGPGUI"",")
