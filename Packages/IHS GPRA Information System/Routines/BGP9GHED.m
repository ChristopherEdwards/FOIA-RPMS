BGP9GHED ; IHS/CMI/LAB - IHS Diabetes Audit 2003 ;
...
TESTNTL ;
 ...
 D EP(.ERR,1,2522,"BGP 09 HEDIS REPORT",338,1,3040000,3000000,1,1,"A",.BGPLST,"","","B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPEXPT,BGPLSTT,BGPLIST,BGPLPRV,BGPLPROV,BGPROT,BGPRTIME,BGPMFITI,BGPFILE) ;EP - called from GUI to produce national GPU report (OTH-HED)
 ...
 ;        NOTE:  THE GLOBAL TO USE TO DISPLAY THE AVAILABLE LISTS IS ^BGPHEIN(
 ;        e.g.  BGPLIST="A"
 ;              BGPLIST(3)=""
 ;
...
BY ;get baseline year
 ...
 S BGPFILEN="" I BGPEXPT S BGPFILEN="BG09"_$P(^AUTTLOC(DUZ(2),0),U,10)_".HE"_BGPRPT_" in directory "_BGPUF
 S BGPDELT=""
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="OTHHED^BGP9GHED",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI HEDIS GPRA REPORT 06" D ^%ZTLOAD Q
 Q
OTHHED ;
 D ^BGP9D1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^XBLM("^BGP9DHEP","^TMP($J,""BGPGUI"",")
