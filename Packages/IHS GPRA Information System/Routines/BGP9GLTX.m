BGP9GLTX ; IHS/CMI/LAB - IHS Diabetes Audit 2003 ;
 ;;9.0;IHS CLINICAL REPORTING;**1**;JUL 01, 2009
 ;
 ;
TESTCMP ;
 S ERR=""
 D EP(.ERR,1,2522,"BGP 09 LAB TAX REPORT",$$NOW^XLFDT)
 W !,ERR
 Q
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LTX^BGP9GLTX",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI LAB TAXONOMY REPORT" D ^%ZTLOAD Q
 Q
LTX ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D @$S(BGPRT=1:"GPRA^BGP9DLT",BGPRT=7:"ONM^BGP9DLT",BGPRT=2:"CRS^BGP9DLT",BGPRT=4:"ELDER^BGP9DLT",BGPRT=3:"HEDIS^BGP9DLT",BGPRT=5:"CMS^BGP9DLT",BGPRT=8:"EO^BGP9DLT")  ;cmi/maw added 10/30/2008
 D GUIR^XBLM("PRINT^BGP9DLT","^TMP($J,""BGPGUI"",")
...
XIT ;
 K ^TMP($J)
 ;D EN^XBVK("BGP")  ;done in LABTAXC^BGP9GU
 S:$D(ZTQUEUED) ZTREQ="@"
