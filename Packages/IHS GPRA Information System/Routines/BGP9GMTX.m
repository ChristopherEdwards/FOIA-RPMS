BGP9GMTX ; IHS/CMI/LAB - IHS Diabetes Audit 2003 ;
 ;;9.0;IHS CLINICAL REPORTING;**1**;JUL 01, 2009
 ;
 ;
TESTCMP ;
 S ERR=""
 D EP(.ERR,1,2522,"BGP 09 MED TAX REPORT",$$NOW^XLFDT)
 W !,ERR
 Q
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="LTX^BGP9GMTX",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI MEDICATION TAXONOMY REPORT" D ^%ZTLOAD Q
 Q
LTX ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D @$S(BGPRT=1:"GPRA^BGP9DMT",BGPRT=7:"ONM^BGP9DMT",BGPRT=2:"CRS^BGP9DMT",BGPRT=4:"ELDER^BGP9DMT",BGPRT=3:"HEDIS^BGP9DMT",BGPRT=5:"CMS^BGP9DMT",BGPRT=8:"EO^BGP9DMT")  ;cmi/maw added 10/30/2008
 D GUIR^XBLM("PRINT^BGP9DMT","^TMP($J,""BGPGUI"",")
...
XIT ;
 K ^TMP($J)
 ;D EN^XBVK("BGP") done in MEDTAXC^BGP9GU
 S:$D(ZTQUEUED) ZTREQ="@"
