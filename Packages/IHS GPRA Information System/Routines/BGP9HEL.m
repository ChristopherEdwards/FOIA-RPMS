BGP9HEL ; IHS/CMI/LAB - IHS gpra print 01 Jul 2008 8:00 PM ; 
...
DEL ;
 ...
 D ^BGP9HEHH
 D DEL1
 D ^BGP9HESL ;print lists to delimited file
 ...
 I '$D(BGPGUI) D
 .S XBFLT=1,XBFN=BGPDELF_".txt",XBMED="F",XBTLE="CRS 2009 HEDIS DELIMITED OUTPUT",XBQ="N"
 .D ^XBGSAVE
...
DEL1 ;EP
 ...
 .S X=$P(^BGPHEIN(BGPIC,0),U,3) D S(X,1,1)
 .S X=" " D S(X,1,1)
 .S X="Denominator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPHEIN(BGPIC,61,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPHEIN(BGPIC,61,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...I $P(^BGPHEIN(BGPIC,61,BGPY,0),U,2)'[BGPRTYPE Q  ;not a denom def for this report
 ...S BGPZ=0 F  S BGPZ=$O(^BGPHEIN(BGPIC,61,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S Y=^BGPHEIN(BGPIC,61,BGPY,1,BGPZ,0) D S(Y,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .S X="Numerator(s):" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPHEIN(BGPIC,62,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPHEIN(BGPIC,62,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...I $P(^BGPHEIN(BGPIC,62,BGPY,0),U,2)'[BGPRTYPE Q  ;not a denom def for this report
 ...S BGPZ=0 F  S BGPZ=$O(^BGPHEIN(BGPIC,62,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S X=^BGPHEIN(BGPIC,62,BGPY,1,BGPZ,0) D S(X,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 .I $O(^BGPHEIN(BGPIC,11,0)) D S("Logic:",1,1) S BGPX=0 F  S BGPX=$O(^BGPHEIN(BGPIC,11,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPHEIN(BGPIC,11,BGPX,0) D S(X,1,1)
 .I $O(^BGPHEIN(BGPIC,51,0)) D S(" ",1,1) S X="Performance Measure Description :" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPHEIN(BGPIC,51,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPHEIN(BGPIC,51,BGPX,0) D S(X,1,1)
 .S X=" " D S(X,1,1) D S("HEDIS Rates:",1,1)
 .D:'$O(^BGPHEIN(BGPIC,52,0)) S("Not Reported",1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPHEIN(BGPIC,52,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPHEIN(BGPIC,52,BGPX,0) D S(X,1,1)
 .X ^BGPHEIN(BGPIC,4)
 Q
HEADER ;EP
 S Y=$P(^VA(200,DUZ,0),U,2),$E(Y,35)=$$FMTE^XLFDT(DT) D S(Y,1,1)
 I BGPRTYPE=3 S Y="*** IHS 2009 HEDIS Clinical Performance Report***" D S(Y,1,1)
