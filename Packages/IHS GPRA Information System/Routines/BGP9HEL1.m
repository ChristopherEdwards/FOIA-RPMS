BGP9HEL1 ; IHS/CMI/LAB - print ind 1 01 Jul 2008 8:00 PM ; 
 ;;9.0;IHS CLINICAL REPORTING;;JUL 1, 2009
 ;
 ;
IP ;EP
 D H1
 S BGPPC1=$P(^BGPHEIN(BGPIC,0),U,6) D PI
 Q
PI ;EP
 S BGPDENP=0
 ;S BGPPC2=0 F  S BGPPC2=$O(^BGPHEIIN("AB",BGPPC1,BGPPC2)) Q:BGPPC2=""  S BGPPC=$O(^BGPHEIIN("AB",BGPPC1,BGPPC2,0)) D PI1
 ;S BGPPC3=0 F  S BGPPC3=$O(^BGPHEIIN("AB",BGPPC1,BGPPC3)) Q:BGPPC3=""  S BGPPC=$O(^BGPHEIIN("ABC",BGPPC1,BGPPC3,0)),BGPPC2=$P(^BGPHEIIN(BGPPC,12),U,4) D PI1
...
PI1 ;
 ...
 .I $E($P(^BGPHEIIN(BGPPC,0),U,4),1,3)="I.B"!($E($P(^BGPHEIIN(BGPPC,0),U,4),1,3)="I.F") Q
 .S Y=$P(^BGPHEIIN(BGPPC,0),U,17) D S(Y,1,1)
 .I $P(^BGPHEIIN(BGPPC,0),U,18)]"" D
 ..I BGPRTYPE'=4,$P(^BGPHEIIN(BGPPC,0),U,18)["GPRA D" Q
 ..D S($P(^BGPHEIIN(BGPPC,0),U,18),1,1)
 .I $P(^BGPHEIIN(BGPPC,0),U,21)]"" D
 ..I BGPRTYPE'=4,$P(^BGPHEIIN(BGPPC,0),U,18)["GPRA D" Q
 ..D S($P(^BGPHEIIN(BGPPC,0),U,21),1,1)
 .S Y=BGPCYD_"^^"_BGPPRD_"^^^"_BGPBLD D S(Y,,2)
 .S BGPDENP=1
 ;get numerator value of measure and calc %
 I $P(^BGPHEIIN(BGPPC,0),U,4)["C-"!($P(^BGPHEIIN(BGPPC,0),U,4)["I.") D
 .S BGPDF=$P(^BGPHEIIN(BGPPC,0),U,8)
 .;get denominator value of measure
 .S BGPNP=$P(^DD(90537.03,BGPDF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 .S BGPCYD=$$V(1,BGPRPT,N,P)
 ...
 ;I $P($G(^BGPHEIIN(BGPPC,12)),U,4)="37.2.1" S X=" " D S(X,1,1)
 S Y=" " D S(Y,1,1) S Y=$P(^BGPHEIIN(BGPPC,0),U,15) D S(Y,1,1)
 I $P(^BGPHEIIN(BGPPC,0),U,16)]"" S Y=$P(^BGPHEIIN(BGPPC,0),U,16) D S(Y,1,1)
 I $P(^BGPHEIIN(BGPPC,0),U,19)]"" S Y=$P(^BGPHEIIN(BGPPC,0),U,19) D S(Y,1,1)
 D H2
 Q
...
V(T,R,N,P) ;EP
 I $G(BGPAREAA) G VA
 NEW X
 I T=1 S X=$P($G(^BGPHEDCN(R,N)),U,P) Q $S(X]"":X,1:0)
 I T=2 S X=$P($G(^BGPHEDPN(R,N)),U,P) Q $S(X]"":X,1:0)
 I T=3 S X=$P($G(^BGPHEDBN(R,N)),U,P) Q $S(X]"":X,1:0)
 Q ""
VA ;
