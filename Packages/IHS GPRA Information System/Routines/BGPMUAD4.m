...
P1 ;Write individual measure
 ...
 S STRING2=$$NUM61("P")
 S STRING3=$$NUM61("B")
 D SUMMARY1^BGPMUAP4(STRING1,STRING2,STRING3)
 ;First population 
 ...
 D S^BGPMUDEL(X,1,1)
 S X="# w/lowest BP < 140/90"_U_$P(STRING1,U,2)_U_$J($P(STRING1,U,5),5,1)_U_$P(STRING2,U,2)_U_$J($P(STRING2,U,5),5,1)_U_$FN(PRD1,",+",1)_U_$P(STRING3,U,2)_U_$J($P(STRING3,U,5),5,1)_U_$FN(PRN1,",+",1)
 D S^BGPMUDEL(X,2,1)
 S X="# w/o BP or lowest BP >=140/90"_U_$P(STRING1,U,8)_U_$J($P(STRING1,U,9),5,1)_U_$P(STRING2,U,8)_U_$J($P(STRING2,U,9),5,1)_U_$FN(PRD7,",+",1)_U_$P(STRING3,U,8)_U_$J($P(STRING3,U,9),5,1)_U_$FN(PRN7,",+",1)
 D S^BGPMUDEL(X,1,1)
...
P2 ;Do the Details
 ...
 S X="during the reporting period, whose blood pressure reading during their most" D S^BGPMUDEL(X,1,1)
 S X="recent encounter was < 140/90 mmHg, if any." D S^BGPMUDEL(X,1,1)
 S X="" D S^BGPMUDEL(X,1,1)
...
DATA(NODE) ;GET DATA
 ...
 S X=NAME_U_HRN_U_COMM_U_SEX_U_AGE_U_DEN_U_NUM1_" "_NUM2
 I NUM3'="" S X=X_";"_NUM3_"  "_NUM4
 D S^BGPMUDEL(X,1,1)
 Q
XML0061 ;XML output for diabetes BP control
 ...
 S TOTN=$P(STRING,U,2)
 S TOTE=$P(STRING,U,4)
 S BGPXML(1)="3"_U_U_+TOTD_U_TOTN_U_TOTE
 K ^TMP("BGPMU0061",$J)
 Q
