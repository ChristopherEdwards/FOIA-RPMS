BGPMUAP6 ; IHS/MSC/MGH - MU EP  measures NQF0062;25-Jul-2011 10:34;DU
 ;;11.1;IHS CLINICAL REPORTING SYSTEM;**1**;JUN 27, 2011;Build 106
 ;Printout for Diabetes:Nephropathy screening
NEPH ;EP
 D P1
 K ^TMP("BGPMU0062")
 Q
P1 ;Write individual measure
 ...
 I $Y>(BGPIOSL-3) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
 W !!,"# w/nephropathy test or",?33,$P(STRING1,U,2),?38,$J($P(STRING1,U,5),5,1),?44,$P(STRING2,U,2),?49,$J($P(STRING2,U,5),5,1),?56,$J($FN(PRD1,",+",1),6),?64,$P(STRING3,U,2),?68,$J($P(STRING3,U,5),5,1),?74,$J($FN(PRN1,",+",1),6)
 I $Y>(BGPIOSL-3) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
 W !,"evidence"
 I $Y>(BGPIOSL-3) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
 W !,"# w/o nephropathy test or",?33,$P(STRING1,U,8),?38,$J($P(STRING1,U,9),5,1),?44,$P(STRING2,U,8),?49,$J($P(STRING2,U,9),5,1),?56,$J($FN(PRD4,",+",1),6),?64,$P(STRING3,U,8),?68,$J($P(STRING3,U,9),5,1),?74,$J($FN(PRN4,",+",1),6)
 I $Y>(BGPIOSL-3) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
 W !,"evidence"
 I $Y>(BGPIOSL-3) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
...
P2 ;Do the Details
 ...
 S X="during the reporting period, who had a nephropathy screening test or evidence of" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="nephropathy, or who were treated with ACE inhibitors/ARBs, if any." D W^BGPMUPP(X,0,1,BGPPTYPE)
 ...
 S X="ACE=ACE/ARB Medication" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="ACR=Urine Microalbumin Laboratory Test" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="NEP=Nephropathy Screening" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X=""  D W^BGPMUPP(X,0,1,BGPPTYPE)
...
SUMMARY1(STRING1,STRING2,STRING3,CT) ;Summmary setup
 ...
 S ^TMP("BGPMU SUMMARY",$J,BGPIC)="0062^119"
 S DESC="18-75 # w/nephropathy test or evid"
 S LINE=""
 S LINE="MU.EP.0062.1"_U_DESC_U_$P(STRING1,U,4)_U_$P(STRING1,U,1)_U_$P(STRING1,U,2)_U_$P(STRING1,U,5)_U_U_U_U_U
