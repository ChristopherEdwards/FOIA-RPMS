...
P1 ;Write individual measure
 ...
 S STRING2=$$NUM387("P")
 S STRING3=$$NUM387("B")
 D SUMMARY1^BGPMUBP2(STRING1,STRING2,STRING3)
 ;First population
 ...
 D S^BGPMUDEL(X,2,1)
 S X="Pts 18+ w/stage IC thru IIIC breast cancer"_U_$P(STRING1,U,1)_U_U_$P(STRING2,U,1)_U_U_U_$P(STRING3,U,1)
 D S^BGPMUDEL(X,1,1)
 S X="# Excluded (Exc)"_U_$P(STRING1,U,4)_U_U_$P(STRING2,U,4)_U_U_U_$P(STRING3,U,4)
 D S^BGPMUDEL(X,1,1)
 S X="Pts 18+ w/stage IC thru IIIC breast cancer less Exc"_U_$P(STRING1,U,3)_U_U_$P(STRING2,U,3)_U_U_U_$P(STRING3,U,3)
 D S^BGPMUDEL(X,1,1)
 S X="# w/tamoxifen or aromatase inhibitor therapy"_U_$P(STRING1,U,2)_U_$J($P(STRING1,U,5),5,1)_U_$P(STRING2,U,2)_U_$J($P(STRING2,U,5),5,1)_U_PRD1_U_$P(STRING3,U,2)_U_$J($P(STRING3,U,5),5,1)_U_PRN1
 D S^BGPMUDEL(X,2,1)
 S X="# w/o tamoxifen or aromatase inhibitor therapy"_U_$P(STRING1,U,8)_U_$J($P(STRING1,U,9),5,1)_U_$P(STRING2,U,8)_U_$J($P(STRING2,U,9),5,1)_U_PRD7_U_$P(STRING3,U,8)_U_$J($P(STRING3,U,9),5,1)_U_PRN7
 D S^BGPMUDEL(X,1,1)
...
P2 ;Do the Details
 ...
 S X="Patients 18+ with at least 2 office encounters with the EP during the reporting" D S^BGPMUDEL(X,1,1)
 S X="period AND who had Stage IC-IIIC, ER or PR positive breast cancer during or" D S^BGPMUDEL(X,1,1)
 S X="before any of these encounters AND who were prescribed tamoxifen or aromatase" D S^BGPMUDEL(X,1,1)
 S X="inhibitor therapy during the reporting period, if any." D S^BGPMUDEL(X,1,1)
 ...
 S X="BCDX=Breast Cancer Diagnosis" D S^BGPMUDEL(X,1,1)
 S X="BCST=Breast Cancer Stage IC-IIIC" D S^BGPMUDEL(X,1,1)
 S X="BCP=Breast Cancer ER or RP positive" D S^BGPMUDEL(X,1,1)
 S X="EN=Encounter" D S^BGPMUDEL(X,1,1)
 S X="MED=Tamoxifen or Aromatase Inhibitor Therapy" D S^BGPMUDEL(X,1,1)
...
DATA(NODE) ;GET DATA
 ...
 Q
XML0387 ;XML output for Breast cancer and tamoxifen
 ...
 S TOTN=$P(STRING,U,2)
 S TOTE=$P(STRING,U,4)
 S BGPXML(1)="71"_U_U_+TOTD_U_TOTN_U_TOTE
 K ^TMP("BGPMU0387",$J)
 Q
