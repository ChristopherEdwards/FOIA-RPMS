BGPMUFP5 ; IHS/MSC/MMT - Print MU EP measures NQF0075 ;20-JUL-2011 11:27;DU
...
P1 ;Write individual measure
 N X,Y,Z,DEN,NUM1,NUM2,PC1,PC2,PC3,PC4,STRING1,STRING2,STRING3
 N PRD,PRN,PRD1,PRN1,PRD2,PRN2,PRD3,PRN3
 S STRING1=$$D75("C")
 S STRING2=$$D75("P")
 S STRING3=$$D75("B")
 ...
 W !!,"Numerator 1"
 W !,"# w/LDL test result",?33,$P(STRING1,U,2),?38,$J($P(STRING1,U,3),5,1),?44,$P(STRING2,U,2),?49,$J($P(STRING2,U,3),5,1),?56,$J($FN(PRD,",+",1),6),?64,$P(STRING3,U,2),?68,$J($P(STRING3,U,3),5,1),?74,$J($FN(PRN,",+",1),6)
 W !,"# w/o LDL test result",?33,($P(STRING1,U,1)-$P(STRING1,U,2)),?38,$J($P(STRING1,U,5),5,1),?44,($P(STRING2,U,1)-$P(STRING2,U,2)),?49,$J($P(STRING2,U,5),5,1),?56,$J($FN(PRD1,",+",1),6)
 W ?64,($P(STRING3,U,1)-$P(STRING3,U,2)),?68,$J($P(STRING3,U,5),5,1),?74,$J($FN(PRN1,",+",1),6)
 I $Y>(BGPIOSL-5) D HEADER^BGPMUPH Q:BGPQUIT
 W !!,"Numerator 2"
 W !,"w/LDL < 100mg/dL",?33,$P(STRING1,U,6),?38,$J($P(STRING1,U,7),5,1),?44,$P(STRING2,U,6),?49,$J($P(STRING2,U,7),5,1),?56,$J($FN(PRD2,",+",1),6),?64,$P(STRING3,U,6),?68,$J($P(STRING3,U,7),5,1),?74,$J($FN(PRN2,",+",1),6)
 W !,"w/o LDL < 100mg/dL",?33,($P(STRING1,U,1)-$P(STRING1,U,6)),?38,$J($P(STRING1,U,8),5,1),?44,($P(STRING2,U,1)-$P(STRING2,U,6)),?49,$J($P(STRING2,U,8),5,1),?56,$J($FN(PRD3,",+",1),6)
 W ?64,($P(STRING3,U,1)-$P(STRING3,U,6)),?68,$J($P(STRING3,U,8),5,1),?74,$J($FN(PRN3,",+",1),6)
 I $D(BGPLIST(BGPIC)) D P2
 D SUM75
 K X,Y,Z,DEN,NUM1,NUM2,PC1,PC2,PC3,PC4,STRING1,STRING2,STRING3
 K PRD,PRN,PRD1,PRN1,PRD2,PRN2,PRD3,PRN3
 Q
D75(TF) ;Get the numbers for this measure
 N DATA
 S DEN=+$G(^TMP("BGPMU0075",$J,TF,"DEN"))
 S NUM1=+$G(^TMP("BGPMU0075",$J,TF,"NUM",1))
 S NUM2=+$G(^TMP("BGPMU0075",$J,TF,"NUM",2))
 I DEN=0 S (PC1,PC2,PC3,PC4)=0
 I DEN>0 D
 .S PC1=$$ROUND^BGPMUA01((NUM1/DEN),3)*100,PC2=$$ROUND^BGPMUA01(((DEN-NUM1)/DEN),3)*100
 .S PC3=$$ROUND^BGPMUA01((NUM2/DEN),3)*100,PC4=$$ROUND^BGPMUA01(((DEN-NUM2)/DEN),3)*100
 S DATA=DEN_U_NUM1_U_PC1_U_""_U_PC2_U_NUM2_U_PC3_U_PC4
 Q DATA
P2 ;Do the Details
 ...
 S X="AND (c) who had a complete lipid profile performed during the measurement year;" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="AND (d) whose LDL=C was <100 mg/dL, if any." D W^BGPMUPP(X,0,1,BGPPTYPE)
 ...
 S X="LDL=LDL Lab Test" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="CHL=Cholesterol Lab Test" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="HDL=High Density Lipoprotein Lab Test" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="TRI=Triglycerides Test" D W^BGPMUPP(X,0,1,BGPPTYPE)
 I $Y>(BGPIOSL-8) D HEADERL^BGPMUPH Q:BGPQUIT
 S X="Numerator 1: Patients with LDL Test" D W^BGPMUPP(X,0,2,BGPPTYPE)
 ...
 I $Y>(BGPIOSL-8) D HEADERL^BGPMUPH Q:BGPQUIT
 S X="Numerator 2: Patients with LDL Test Result < 100mg/dL" D W^BGPMUPP(X,0,2,BGPPTYPE)
 S X="" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S PTCT=0
 W !,"PATIENT NAME",?23,"HRN",?30,"COMMUNITY",?42,"SEX",?46,"AGE",?50,"DENOMINATOR",?64,"NUMERATOR"
 S LINE="",$P(LINE,"-",81)="" W !,LINE
...
DATA(NODE) ;DISPLAY DATA
 ...
 W:(DEN2'="")!(NUM2'="") !,?50,DEN2,?66,NUM2
 W:NUM3'="" !,?66,NUM3
 K X,NAME,HRN,DEN,DEN1,DEN2,NUM1,NUM2,NUM3,AGE,DFN,LINE,COMM,SEX
 K NUMVALS,NUMDATES
 Q
XML75 ;Populate the BGPXML array with data for each population/numerator
 ...
 S BGPXML(1)="0075.1"_U_""_U_+$G(^TMP("BGPMU0075",$J,"C","DEN"))_U_+$G(^TMP("BGPMU0075",$J,"C","NUM",1))
 S BGPXML(1)="0075.2"_U_""_U_+$G(^TMP("BGPMU0075",$J,"C","DEN"))_U_+$G(^TMP("BGPMU0075",$J,"C","NUM",2))
 K ^TMP("BGPMU0075",$J)
 Q
SUM75 ;Populate "BGPMU SUMMARY" for IVD LDL
 ;Initialize iCare/summary global
 N BGPDNCNT,BGPSSTR,J,N
 S BGPDNCNT=0
 K ^TMP("BGPMU SUMMARY",$J,BGPIC)
 S ^TMP("BGPMU SUMMARY",$J,BGPIC)="0075"_U_"N/A"
 S N=1 ;only 1 numerator
 ;Setup summary page/iCare ^TMP global
 S CDEN1CT=+$G(^TMP("BGPMU0075",$J,"C","DEN"))                 ;current
 S CNUM1CT=+$G(^TMP("BGPMU0075",$J,"C","NUM",1))
 S CNUM2CT=+$G(^TMP("BGPMU0075",$J,"C","NUM",2))
 S CMP=$S(CDEN1CT>0:$$ROUND^BGPMUA01(CNUM1CT/CDEN1CT,3)*100,1:0)
 S CMP2=$S(CDEN1CT>0:$$ROUND^BGPMUA01(CNUM2CT/CDEN1CT,3)*100,1:0)
 S PDEN1CT=+$G(^TMP("BGPMU0075",$J,"P","DEN"))                 ;previous
 S PNUM1CT=+$G(^TMP("BGPMU0075",$J,"P","NUM",1))
 S PNUM2CT=+$G(^TMP("BGPMU0075",$J,"P","NUM",2))
 S PMP=$S(PDEN1CT>0:$$ROUND^BGPMUA01(PNUM1CT/PDEN1CT,3)*100,1:0)
 S PMP2=$S(PDEN1CT>0:$$ROUND^BGPMUA01(PNUM2CT/PDEN1CT,3)*100,1:0)
 S BDEN1CT=+$G(^TMP("BGPMU0075",$J,"B","DEN"))                 ;baseline
 S BNUM1CT=+$G(^TMP("BGPMU0075",$J,"B","NUM",1))
 S BNUM2CT=+$G(^TMP("BGPMU0075",$J,"B","NUM",2))
 S BMP=$S(BDEN1CT>0:$$ROUND^BGPMUA01(BNUM1CT/BDEN1CT,3)*100,1:0)
 S BMP2=$S(BDEN1CT>0:$$ROUND^BGPMUA01(BNUM2CT/BDEN1CT,3)*100,1:0)
 ;FIRST NUMERATOR
 S BGPDNCNT=BGPDNCNT+1
 S BGPSSTR="MU.EP.0075.1"_U_"18+ # w/LDL test result"_U_""_U_CDEN1CT_U_CNUM1CT_U_CMP_U_U_U_U
 ; 11 12 13 14
 ...
 ;SECOND NUMERATOR
 S BGPDNCNT=BGPDNCNT+1,BGPSSTR=""
 S BGPSSTR="MU.EP.0075.2"_U_"18+ # w/LDL < 100mg/dL"_U_""_U_CDEN1CT_U_CNUM2CT_U_CMP2_U_U_U_U
 ; 11 12 13 14
 S BGPSSTR=BGPSSTR_U_""_U_PDEN1CT_U_PNUM2CT_U_PMP2
 ; 15 16 17 18
 S BGPSSTR=BGPSSTR_U_""_U_BDEN1CT_U_BNUM2CT_U_BMP2
 S ^TMP("BGPMU SUMMARY",$J,BGPIC,BGPDNCNT)=BGPSSTR
 Q
