BGPMUFP6 ; IHS/MSC/MMT - Print MU EP measures NQF0073 ;20-JUL-2011 11:27;DU
...
P1 ;Write individual measure
 ...
 W !!,"# w/lowest BP < 140/90",?33,$P(STRING1,U,2),?38,$J($P(STRING1,U,3),5,1),?44,$P(STRING2,U,2),?49,$J($P(STRING2,U,3),5,1),?56,$J($FN(PRD,",+",1),6),?64,$P(STRING3,U,2),?68,$J($P(STRING3,U,3),5,1),?74,$J($FN(PRN,",+",1),6)
 W !,"# w/o BP or lowest BP",?33,($P(STRING1,U,1)-$P(STRING1,U,2)),?38,$J($P(STRING1,U,5),5,1),?44,($P(STRING2,U,1)-$P(STRING2,U,2)),?49,$J($P(STRING2,U,5),5,1),?56,$J($FN(PRD1,",+",1),6)
 ...
 Q
D73(TF) ;Get the numbers for this measure
 N DATA
 S DEN=+$G(^TMP("BGPMU0073",$J,TF,"DEN"))
 S NUM1=+$G(^TMP("BGPMU0073",$J,TF,"NUM"))
 I DEN=0 S (PC1,PC2)=0
 I DEN>0 D
...
P2 ;Do the Details
 ...
 S X="AND (c) whose most recent blood pressure is under control (<140/90 mmHg)," D W^BGPMUPP(X,0,1,BGPPTYPE)
...
DATA(NODE) ;DISPLAY DATA
 ...
 S:$P(NODE,U,3)'="" NUM1=$P(NODE,U,3)_" "_$P(NODE,U,4)
 S:$P(NODE,U,5)'="" NUM2=$P(NODE,U,5)_" "_$P(NODE,U,6)
 I $Y>(BGPIOSL-2) D
 .D HEADERL^BGPMUPH Q:BGPQUIT
 .W !,"PATIENT NAME",?23,"HRN",?30,"COMMUNITY",?42,"SEX",?46,"AGE",?50,"DENOMINATOR",?64,"NUMERATOR"
 .S LINE="",$P(LINE,"-",81)="" W !,LINE
 W !,NAME,?23,HRN,?30,COMM,?43,SEX,?46,AGE,?50,DEN1,?64,$S(NOTMET:"NM:",1:"M:"),NUM1
 W:(DEN2'="")!($G(NUM2)'="") !,?50,DEN2,?66,NUM2
 K X,NAME,HRN,DEN,DEN1,DEN2,NUM1,NUM2,NUM3,AGE,DFN,LINE,COMM,SEX
 K NUMVALS,NUMDATES
 Q
XML73 ;Populate the BGPXML array with data for each population/numerator
 ...
 S BGPXML(1)="201"_U_""_U_+$G(^TMP("BGPMU0073",$J,"C","DEN"))_U_+$G(^TMP("BGPMU0073",$J,"C","NUM"))
 K ^TMP("BGPMU0073",$J)
 Q
SUM73 ;Populate "BGPMU SUMMARY" for IVD BP control
 ;Initialize iCare/summary global
 N BGPDNCNT,BGPSSTR,J,N
 S BGPDNCNT=0
 K ^TMP("BGPMU SUMMARY",$J,BGPIC)
 S ^TMP("BGPMU SUMMARY",$J,BGPIC)="0073"_U_"201"
 S N=1 ;only 1 numerator
 ;Setup summary page/iCare ^TMP global
 S CDEN1CT=+$G(^TMP("BGPMU0073",$J,"C","DEN"))                 ;current
 S CNUM1CT=+$G(^TMP("BGPMU0073",$J,"C","NUM"))
 S CMP=$S(CDEN1CT>0:$$ROUND^BGPMUA01(CNUM1CT/CDEN1CT,3)*100,1:0)
 S PDEN1CT=+$G(^TMP("BGPMU0073",$J,"P","DEN"))                 ;previous
 S PNUM1CT=+$G(^TMP("BGPMU0073",$J,"P","NUM"))
 S PMP=$S(PDEN1CT>0:$$ROUND^BGPMUA01(PNUM1CT/PDEN1CT,3)*100,1:0)
 S BDEN1CT=+$G(^TMP("BGPMU0073",$J,"B","DEN"))                 ;baseline
 S BNUM1CT=+$G(^TMP("BGPMU0073",$J,"B","NUM"))
 S BMP=$S(BDEN1CT>0:$$ROUND^BGPMUA01(BNUM1CT/BDEN1CT,3)*100,1:0)
 ;FIRST NUMERATOR
 S BGPDNCNT=BGPDNCNT+1
 S BGPSSTR="MU.EP.0073.1"_U_"18+ # w/lowest BP < 140/90"_U_""_U_CDEN1CT_U_CNUM1CT_U_CMP_U_U_U_U
 ; 11 12 13 14
