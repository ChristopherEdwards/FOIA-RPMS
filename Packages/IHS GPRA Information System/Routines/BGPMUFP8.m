BGPMUFP8 ; IHS/MSC/MMT - Print MU EP measures NQF0084 ;20-JUL-2011 11:27;DU
...
P1 ;Write individual measure
 ...
 W !,"less Exc"
 I $Y>(BGPIOSL-3) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
 W !!,"# w/warfarin therapy",?33,$P(STRING1,U,2),?38,$J($P(STRING1,U,5),5,1),?44,$P(STRING2,U,2),?49,$J($P(STRING2,U,5),5,1),?56,$J($FN(PRD1,",+",1),6),?64,$P(STRING3,U,2),?68,$J($P(STRING3,U,5),5,1),?74,$J($FN(PRN1,",+",1),6)
 I $Y>(BGPIOSL-3) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
 W !,"# w/o warfarin therapy",?33,$P(STRING1,U,8),?38,$J($P(STRING1,U,9),5,1),?44,$P(STRING2,U,8),?49,$J($P(STRING2,U,9),5,1),?56,$J($FN(PRD4,",+",1),6),?64,$P(STRING3,U,8),?68,$J($P(STRING3,U,9),5,1),?74,$J($FN(PRN4,",+",1),6)
 I $Y>(BGPIOSL-3) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
...
P2 ;Do the Details
 ...
 S PTCT=0
 D HEADERL^BGPMUPH
 S X="Patients 18+ with at least 2 outpatient or nursing facility encounters with the" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="EP during the reporting period AND who had diagnosis of heart failure on or" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="before the encounters AND a diagnosis of atrial fibrillation before or during" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="the reporting period AND who were prescribed warfarin therapy during the" D W^BGPMUPP(X,0,1,BGPPTYPE)
 ...
 S X="AF=Atrial Fibrillation Diagnosis" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="EN=Encounter" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="MED=Warfarin Therapy" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="" D W^BGPMUPP(X,0,1,BGPPTYPE)
 ...
 Q
DATA(NODE) ;GET DATA
 N NAME,HRN,DEN,NUM,AGE,DFN,SEX,COMM,NUM1,NUM2,DEN1,DEN2,DEN3,DEN4,LINE
 S DFN=$P(NODE,U,1)
...
SUMMARY1(STRING1,STRING2,STRING3,CT) ;Summmary setup
 ...
 S ^TMP("BGPMU SUMMARY",$J,BGPIC)="0084^200"
 S DESC="18+ # w/warfarin therapy"
 S LINE=""
 S LINE="MU.EP.0084.1"_U_DESC_U_$P(STRING1,U,4)_U_$P(STRING1,U,1)_U_$P(STRING1,U,2)_U_$P(STRING1,U,5)_U_U_U_U_U
...
XML84 ;Populate the BGPXML array with data for each population/numerator
 ; BGPXMLOUT(i)=PQRI number^""^Denominator Count^Numerator Count^Exclusion Count
 S BGPXML(1)="200"_U_""_U_+$G(^TMP("BGPMU0084",$J,"C","DEN"))_U_+$G(^TMP("BGPMU0084",$J,"C","NUM"))_U_+$G(^TMP("BGPMU0084",$J,"C","EXC"))
 K ^TMP("BGPMU0084",$J)
 Q
