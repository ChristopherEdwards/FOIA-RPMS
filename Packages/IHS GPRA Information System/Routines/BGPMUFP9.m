BGPMUFP9 ; IHS/MSC/MMT - Print MU EP measures NQF0052 ;20-JUL-2011 11:27;DU
...
P1 ;Write individual measure
 ...
 W !,"Pts 18-49 w/low back pain",?33,$P(STRING1,U,1),?44,$P(STRING2,U,1),?64,$P(STRING3,U,1)
 I $Y>(BGPIOSL-3) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
 W !!,"# w/o spinal imaging",?33,$P(STRING1,U,2),?38,$J($P(STRING1,U,5),5,1),?44,$P(STRING2,U,2),?49,$J($P(STRING2,U,5),5,1),?56,$J($FN(PRD1,",+",1),6),?64,$P(STRING3,U,2),?68,$J($P(STRING3,U,5),5,1),?74,$J($FN(PRN1,",+",1),6)
 I $Y>(BGPIOSL-3) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
 W !,"# w/spinal imaging",?33,$P(STRING1,U,8),?38,$J($P(STRING1,U,9),5,1),?44,$P(STRING2,U,8),?49,$J($P(STRING2,U,9),5,1),?56,$J($FN(PRD4,",+",1),6),?64,$P(STRING3,U,8),?68,$J($P(STRING3,U,9),5,1),?74,$J($FN(PRN4,",+",1),6)
 I $Y>(BGPIOSL-3) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
 I $D(BGPLIST(BGPIC)) D P2
 ;
 Q
NUM52(TF) ;Get the numbers for this measure
 N ARRAY,DEN,NUM,EXC,NOT,PC1,PC11,PC2,PC13,NNUM,PC14
 S DEN=+$G(^TMP("BGPMU0052",$J,TF,"DEN"))
 S NUM=+$G(^TMP("BGPMU0052",$J,TF,"NUM"))
 S NOT=+$G(^TMP("BGPMU0052",$J,TF,"NOT"))
 S EXC=+$G(^TMP("BGPMU0052",$J,TF,"EXC"))
 S NNUM=DEN-EXC
...
P2 ;Do the Details
 ...
 S PTCT=0
 D HEADERL^BGPMUPH
 S X="Patients 18-49 with a primary diagnosis of low back pain and who had an" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="emergency department, outpatient, orthopedic, or chiropractic encounter with" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="the EP during the reporting period and who DID NOT HAVE an imaging study" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="within 28 days of the FIRST diagnosis of low back pain during the reporting" D W^BGPMUPP(X,0,1,BGPPTYPE)
 ...
 S X="LBP=Low Back Pain Diagnosis" D W^BGPMUPP(X,0,1,BGPPTYPE)
 ...
 .K BGPARR
 .D PTLSORT^BGPMUUTL(.BGPARR,"^TMP(""BGPMU0052"","_$J_",""PAT"",""C"",""NOT"")")
 ...
 .K BGPARR
 .D PTLSORT^BGPMUUTL(.BGPARR,"^TMP(""BGPMU0052"","_$J_",""PAT"",""C"",""NUM"")")
 .S PT=0 F  S PT=$O(BGPARR(PT)) Q:PT=""  D
 ..S PTCT=PTCT+1
 ..S NODE=$G(BGPARR(PT))
 ..D DATA(NODE)
 I BGPLIST="A" D
 .K BGPARR
 .D PTLSORT^BGPMUUTL(.BGPARR,"^TMP(""BGPMU0052"","_$J_",""PAT"",""C"",""EXC"")")
 .S PT=0 F  S PT=$O(BGPARR(PT)) Q:PT=""  D
 ..S PTCT=PTCT+1
 ..S NODE=$G(BGPARR(PT))
 ..D DATA(NODE)
...
DATA(NODE) ;GET DATA
 ...
 S NUM=$P(NODE,U,3)
 S NUM1=$P(NUM,";",1)_" "_$$DATE^BGPMUUTL($P($P(NUM,";",2),".",1))
...
SUMMARY1(STRING1,STRING2,STRING3,CT) ;Summmary setup
 ...
 S ^TMP("BGPMU SUMMARY",$J,BGPIC)="0052^N/A"
 S DESC="18-49 # w/o spinal imaging"
 S LINE=""
 S LINE="MU.EP.0052.1"_U_DESC_U_$P(STRING1,U,4)_U_$P(STRING1,U,1)_U_$P(STRING1,U,2)_U_$P(STRING1,U,5)_U_U_U_U_U
 ...
 Q
XML52 ;Populate the BGPXML array with data for each population/numerator
 ; BGPXMLOUT(i)=PQRI number^""^Denominator Count^Numerator Count^Exclusion Count
 S BGPXML(1)="0052"_U_""_U_+$G(^TMP("BGPMU0052",$J,"C","DEN"))_U_+$G(^TMP("BGPMU0052",$J,"C","NUM"))_U_+$G(^TMP("BGPMU0052",$J,"C","EXC"))
 K ^TMP("BGPMU0052",$J)
 Q
