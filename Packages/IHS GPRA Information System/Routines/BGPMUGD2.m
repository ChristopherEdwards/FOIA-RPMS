BGPMUGD2 ; IHS/MSC/MMT - MU EP measures NQF0083 ;20-Aug-2011 11:26;DU
...
P1 ;Write individual measure
 ...
 S STRING2=$$NUM83("P")
 S STRING3=$$NUM83("B")
 D SUMMARY1^BGPMUGP2(STRING1,STRING2,STRING3)
 ;population
 ...
 D S^BGPMUDEL(X,2,1)
 S X="Pts 18+ w/heart failure and LVEF < 40%"_U_$P(STRING1,U,1)_U_U_$P(STRING2,U,1)_U_U_U_$P(STRING3,U,1) D S^BGPMUDEL(X,1,1)
 S X="# Excluded (Exc)"_U_$P(STRING1,U,4)_U_U_$P(STRING2,U,4)_U_U_U_$P(STRING3,U,4)
 D S^BGPMUDEL(X,1,1)
 S X="Pts 18+ w/heart failure and LVEF < 40% less Exc"_U_$P(STRING1,U,3)_U_U_$P(STRING2,U,3)_U_U_U_$P(STRING3,U,3)
 D S^BGPMUDEL(X,1,1)
 S X="# w/beta-blocker therapy"_U_$P(STRING1,U,2)_U_$J($P(STRING1,U,5),5,1)_U_$P(STRING2,U,2)_U_$J($P(STRING2,U,5),5,1)_U_$FN(PRD1,",+",1)_U_$P(STRING3,U,2)_U_$J($P(STRING3,U,5),5,1)_U_$FN(PRN1,",+",1)
 D S^BGPMUDEL(X,2,1)
 S X="# w/o beta-blocker therapy"_U_$P(STRING1,U,8)_U_$J($P(STRING1,U,9),5,1)_U_$P(STRING2,U,8)_U_$J($P(STRING2,U,9),5,1)_U_$FN(PRD7,",+",1)_U_$P(STRING3,U,8)_U_$J($P(STRING3,U,9),5,1)_U_$FN(PRN7,",+",1)
 D S^BGPMUDEL(X,1,1)
...
P2 ;Do the Details
 ...
 S X="Patients 18+ with at least 2 outpatient encounters OR at least 2 nursing facility" D S^BGPMUDEL(X,1,1)
 S X="encounters with the EP during the reporting period AND a diagnosis of heart failure" D S^BGPMUDEL(X,1,1)
 S X="AND a Left Ventricular Ejection Faction (LVEF) of < 40% before the any of these" D S^BGPMUDEL(X,1,1)
 S X="encounters during the reporting period AND who were prescribed beta-blocker therapy, if any." D S^BGPMUDEL(X,1,1)
 ...
 S X="LVEF=Left Ventricular Ejection Faction< 40%" D S^BGPMUDEL(X,1,1)
...
DATA(NODE) ;GET DATA
 ...
 S NUM=$P(NODE,U,3)
 S NUM1=$P(NUM,";",1)
 S X=NAME_U_HRN_U_COMM_U_SEX_U_AGE_U_DEN_U_NUM1 D S^BGPMUDEL(X,1,1)
 Q
