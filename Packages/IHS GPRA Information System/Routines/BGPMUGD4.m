BGPMUGD4 ; IHS/MSC/MMT - MU EP measures NQF0047 ;20-Aug-2011 11:26;DU
...
P1 ;Write individual measure
 ...
 S STRING2=$$NUM47("P")
 S STRING3=$$NUM47("B")
 D SUMMARY1^BGPMUGP4(STRING1,STRING2,STRING3)
 ;population
 ...
 D S^BGPMUDEL(X,2,1)
 S X="Pts 5-40 w/persistent asthma"_U_$P(STRING1,U,1)_U_U_$P(STRING2,U,1)_U_U_U_$P(STRING3,U,1) D S^BGPMUDEL(X,1,1)
 S X="# Excluded (Exc)"_U_$P(STRING1,U,4)_U_U_$P(STRING2,U,4)_U_U_U_$P(STRING3,U,4)
 D S^BGPMUDEL(X,1,1)
 S X="Pts 5-40 w/persistent asthma less Exc"_U_$P(STRING1,U,3)_U_U_$P(STRING2,U,3)_U_U_U_$P(STRING3,U,3)
 D S^BGPMUDEL(X,1,1)
 S X="# w/asthma medication prescribed"_U_$P(STRING1,U,2)_U_$J($P(STRING1,U,5),5,1)_U_$P(STRING2,U,2)_U_$J($P(STRING2,U,5),5,1)_U_$FN(PRD1,",+",1)_U_$P(STRING3,U,2)_U_$J($P(STRING3,U,5),5,1)_U_$FN(PRN1,",+",1)
 D S^BGPMUDEL(X,2,1)
 S X="# w/o asthma medication prescribed"_U_$P(STRING1,U,8)_U_$J($P(STRING1,U,9),5,1)_U_$P(STRING2,U,8)_U_$J($P(STRING2,U,9),5,1)_U_$FN(PRD7,",+",1)_U_$P(STRING3,U,8)_U_$J($P(STRING3,U,9),5,1)_U_$FN(PRN7,",+",1)
 D S^BGPMUDEL(X,1,1)
 S X="" D S^BGPMUDEL(X,1,1)
 I $D(BGPLIST(BGPIC)) D P2
 Q
NUM47(TF) ;Get the numbers for this measure
 N ARRAY,DEN,NUM,EXC,NOT,PC1,PC11,PC2,PC14,PC21,PC13,NNUM
 S DEN=+$G(^TMP("BGPMU0047",$J,TF,"DEN"))
 S NUM=+$G(^TMP("BGPMU0047",$J,TF,"NUM"))
 S NOT=+$G(^TMP("BGPMU0047",$J,TF,"NOT"))
 S EXC=+$G(^TMP("BGPMU0047",$J,TF,"EXC"))
 S NNUM=DEN-EXC
...
P2 ;Do the Details
 ...
 S X="Patients aged 5-40 years with a diagnosis of mild, moderate, or severe" D S^BGPMUDEL(X,1,1)
 S X="persistent asthma who had at least 2 office visits with the EP during the" D S^BGPMUDEL(X,1,1)
 S X="reporting period and were prescribed either an inhaled corticosteroid or an" D S^BGPMUDEL(X,1,1)
 S X="alternative asthma medication during the reporting period, if any." D S^BGPMUDEL(X,1,1)
 ...
 .K BGPARR
 .D PTLSORT^BGPMUUTL(.BGPARR,"^TMP(""BGPMU0047"","_$J_",""PAT"",""C"",""NOT"")")
 ...
 .K BGPARR
 .D PTLSORT^BGPMUUTL(.BGPARR,"^TMP(""BGPMU0047"","_$J_",""PAT"",""C"",""NUM"")")
 .S PT=0 F  S PT=$O(BGPARR(PT)) Q:PT=""  D
 ..S PTCT=PTCT+1
 ..S NODE=$G(BGPARR(PT))
 ..D DATA(NODE)
 I BGPLIST="A" D
 .K BGPARR
 .D PTLSORT^BGPMUUTL(.BGPARR,"^TMP(""BGPMU0047"","_$J_",""PAT"",""C"",""EXC"")")
 .S PT=0 F  S PT=$O(BGPARR(PT)) Q:PT=""  D
 ..S PTCT=PTCT+1
 ..S NODE=$G(BGPARR(PT))
 ..D DATA(NODE)
