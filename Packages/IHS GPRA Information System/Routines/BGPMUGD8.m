BGPMUGD8 ; IHS/MSC/MMT - MU EP measures NQF0088 ;09-Sep-2011 11:26;DU
...
P1 ;Write individual measure
 ...
 S STRING2=$$NUM88("P")
 S STRING3=$$NUM88("B")
 D SUMMARY1^BGPMUGP8(STRING1,STRING2,STRING3)
 ;population
 ...
 D S^BGPMUDEL(X,2,1)
 S X="Pts 18+ w/diabetic retinopathy"_U_$P(STRING1,U,1)_U_U_$P(STRING2,U,1)_U_U_U_$P(STRING3,U,1)
 D S^BGPMUDEL(X,1,1)
 S X="# Excluded (Exc)"_U_$P(STRING1,U,4)_U_U_$P(STRING2,U,4)_U_U_U_$P(STRING3,U,4)
 D S^BGPMUDEL(X,1,1)
 S X="Pts 18+ w/diabetic retinopathy less Exc"_U_$P(STRING1,U,3)_U_U_$P(STRING2,U,3)_U_U_U_$P(STRING3,U,3)
 D S^BGPMUDEL(X,1,1)
 S X="# w/macular or fundus exam"_U_$P(STRING1,U,2)_U_$J($P(STRING1,U,5),5,1)_U_$P(STRING2,U,2)_U_$J($P(STRING2,U,5),5,1)_U_$FN(PRD1,",+",1)_U_$P(STRING3,U,2)_U_$J($P(STRING3,U,5),5,1)_U_$FN(PRN1,",+",1)
 D S^BGPMUDEL(X,2,1)
 S X="w/macular edema and severity"
 D S^BGPMUDEL(X,1,1)
 S X="of retinopathy findings"
 D S^BGPMUDEL(X,1,1)
 S X="# w/o macular or fundus exam"_U_$P(STRING1,U,8)_U_$J($P(STRING1,U,9),5,1)_U_$P(STRING2,U,8)_U_$J($P(STRING2,U,9),5,1)_U_$FN(PRD7,",+",1)_U_$P(STRING3,U,8)_U_$J($P(STRING3,U,9),5,1)_U_$FN(PRN7,",+",1)
 D S^BGPMUDEL(X,2,1)
 S X="or w/exam but no macular edema"
 D S^BGPMUDEL(X,1,1)
 S X="and/or no severity of retinopathy findings"
 D S^BGPMUDEL(X,1,1)
...
P2 ;Do the Details
 ...
 S X="Patients 18+ who had at least 2 office & outpatient consultation," D S^BGPMUDEL(X,1,1)
 S X="ophthalmological services, nursing facility, or domiciliary encounters with" D S^BGPMUDEL(X,1,1)
 S X="the EP during the reporting period AND a diagnosis of diabetic retinopathy" D S^BGPMUDEL(X,1,1)
 S X="AND who had a macular or fundus exam performed which included documentation of" D S^BGPMUDEL(X,1,1)
 S X="the level of severity of retinopathy and the presence or absence of macular" D S^BGPMUDEL(X,1,1)
 S X="edema during 1 or more the encounters with the EP during the reporting" D S^BGPMUDEL(X,1,1)
 ...
 S X="RET=Diabetic Retinopathy Diagnosis" D S^BGPMUDEL(X,1,1)
 S X="EN=Encounter" D S^BGPMUDEL(X,1,1)
 S X="EXAM=Macular or Fundus Exam" D S^BGPMUDEL(X,1,1)
 S X="ME=Macular Edema Diagnosis" D S^BGPMUDEL(X,1,1)
 S X="LOS=Diabetic Retinopathy Level of Severity" D S^BGPMUDEL(X,1,1)
