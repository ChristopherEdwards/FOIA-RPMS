BGPMUGP4 ; IHS/MSC/MMT - Print MU EP measures NQF0047 ;20-Aug-2011 11:27;DU
...
P1 ;Write individual measure
 ...
 W !,"Pts 5-40 w/persistent",?33,$P(STRING1,U,1),?44,$P(STRING2,U,1),?64,$P(STRING3,U,1)
 W !,"asthma"
 I $Y>(BGPIOSL-3) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
 W !,"# Excluded (Exc)",?33,$P(STRING1,U,4),?44,$P(STRING2,U,4),?64,$P(STRING3,U,4)
 I $Y>(BGPIOSL-4) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
 W !,"Pts 5-40 w/persistent",?33,$P(STRING1,U,3),?44,$P(STRING2,U,3),?64,$P(STRING3,U,3)
 W !,"asthma less Exc"
 I $Y>(BGPIOSL-4) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
 W !!,"# w/asthma medication",?33,$P(STRING1,U,2),?38,$J($P(STRING1,U,5),5,1),?44,$P(STRING2,U,2),?49,$J($P(STRING2,U,5),5,1),?56,$J($FN(PRD1,",+",1),6),?64,$P(STRING3,U,2),?68,$J($P(STRING3,U,5),5,1),?74,$J($FN(PRN1,",+",1),6)
 W !,"prescribed"
 I $Y>(BGPIOSL-4) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
 W !,"# w/o asthma medication",?33,$P(STRING1,U,8),?38,$J($P(STRING1,U,9),5,1),?44,$P(STRING2,U,8),?49,$J($P(STRING2,U,9),5,1),?56,$J($FN(PRD4,",+",1),6),?64,$P(STRING3,U,8),?68,$J($P(STRING3,U,9),5,1),?74,$J($FN(PRN4,",+",1),6)
 W !,"prescribed"
 I $Y>(BGPIOSL-3) D HEADER^BGPMUPH,HDRBLK^BGPMUPH Q:BGPQUIT
...
P2 ;Do the Details
 ...
 S PTCT=0
 D HEADERL^BGPMUPH
 S X="Patients aged 5-40 years with a diagnosis of mild, moderate, or severe" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="persistent asthma who had at least 2 office visits with the EP during the" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="reporting period and were prescribed either an inhaled corticosteroid or an" D W^BGPMUPP(X,0,1,BGPPTYPE)
 S X="alternative asthma medication during the reporting period, if any."  D W^BGPMUPP(X,0,1,BGPPTYPE)
...
SUMMARY1(STRING1,STRING2,STRING3,CT) ;Summmary setup
 ...
 S ^TMP("BGPMU SUMMARY",$J,BGPIC)="0047^53"
 S DESC="5-40 # w/asthma medication"
 S LINE=""
 S LINE="MU.EP.0047.1"_U_DESC_U_$P(STRING1,U,4)_U_$P(STRING1,U,1)_U_$P(STRING1,U,2)_U_$P(STRING1,U,5)_U_U_U_U_U
 ...
 Q
XML47 ;Populate the BGPXML array with data for each population/numerator
 ; BGPXMLOUT(i)=PQRI number^""^Denominator Count^Numerator Count^Exclusion Count
 S BGPXML(1)="53"_U_""_U_+$G(^TMP("BGPMU0047",$J,"C","DEN"))_U_+$G(^TMP("BGPMU0047",$J,"C","NUM"))_U_+$G(^TMP("BGPMU0047",$J,"C","EXC"))
 K ^TMP("BGPMU0047",$J)
 Q
