BGPMUHD4 ; IHS/MSC/SAT - Delimited MU measure STK-10;07-Apr-2011 13:15;DU
 ...
 ;
 ; VTE1^BGPMUP6 = delimited routine for 0371 VTE-1 Prophylaxis within 24 hours
 ; VTE2^BGPMUP6 = delimited routine for 0372 VTE-2 Prophylaxis for ICU pts
 ; VTE3^BGPMUP7 = delimited routine for 0373 VTE-3 Anticoagulation overlap therapy
 ; VTE4^BGPMUP7 = delimited routine for 0374 VTE-4 Platelet monitoring for UFH
 ; VTE5^BGPMUP8 = delimited routine for 0375 VTE-5 VTE discharge instructions
 ; VTE6^BGPMUP8 = delimited routine for 0376 VTE-6 Potentially preventable VTE
 ;Delimited output for Stroke Measure 0441
STK10 ;EP
 D P1B
 K ^TMP("BGPMU0441")
 Q
P1B ;Write individual measure
 ...
 S BGPPROV=""
 S BGPPTYPE="D"
 S STRING1=$$441^BGPMUHP5("C")
 S STRING2=$$441^BGPMUHP5("P")
 S STRING3=$$441^BGPMUHP5("B")
 D SUM441
 ...
 D S^BGPMUDEL(X,1,1)
 S X="# Discharges for Pts w/Isc/Hemm stroke"_U_$P(STRING1,U,1)_U_U_$P(STRING2,U,1)_U_U_U_$P(STRING3,U,1)_U_U
 D S^BGPMUDEL(X,1,1)
 S X="# Excluded (Exc)"_U_$P(STRING1,U,4)_U_U_$P(STRING2,U,4)_U_U_U_$P(STRING3,U,4)
 D S^BGPMUDEL(X,1,1)
 S X="# Discharges for Pts w/Isc/Hemm stroke Less Exc"_U_($P(STRING1,U,1)-$P(STRING1,U,4))_U_U_($P(STRING2,U,1)-$P(STRING2,U,4))_U_U_U_($P(STRING3,U,1)-$P(STRING3,U,4))
 D S^BGPMUDEL(X,1,1)
 S X="# w/rehabilitation service at discharge"_U_$P(STRING1,U,2)_U_$J($P(STRING1,U,5),5,1)_U_$P(STRING2,U,2)_U_$J($P(STRING2,U,5),5,1)_U_$FN(PRD2,",+",1)_U_$P(STRING3,U,2)_U_$J($P(STRING3,U,5),5,1)_U_$FN(PRD3,",+",1)
 D S^BGPMUDEL(X,2,1)
 S X="# w/o rehabilitation service at discharge"_U_$P(STRING1,U,3)_U_$J($P(STRING1,U,6),5,1)_U_$P(STRING2,U,3)_U_$J($P(STRING2,U,6),5,1)_U_$FN(PRD4,",+",1)_U_$P(STRING3,U,3)_U_$J($P(STRING3,U,6),5,1)_U_$FN(PRD5,",+",1)
 D S^BGPMUDEL(X,1,1)
...
TC ;Do the Details
 ...
 S X="Ischemic or hemorrhagic stroke patients who were assessed for rehabilitation services, if any." D S^BGPMUDEL(X,2,1)
 ...
 S X="ISC=Inpatient w/diagnosis of ischemic or hemorrhagic stroke" D S^BGPMUDEL(X,1,1)
...
DEXC441(NODE) ;GET DATA
 ...
 ;
 ;
SUM441 ;Populate "BGPMU SUMMARY" for Stroke Measure 0441
 ;Initialize iCare/summary global
 N BGPDNCNT,BGPSSTR,J,N
 S J=$J
 S BGPDNCNT=0
 K ^TMP("BGPMU SUMMARY",J,BGPIC)
 S ^TMP("BGPMU SUMMARY",J,BGPIC)="0441"_U_"STK-10"
 S N=1 ;only 1 numerator
 ;Setup summary page/iCare ^TMP global
 S CDEN1CT=+$G(^TMP("BGPMU0441",J,"C","DEN"))                 ;current
 S CNUM1CT=+$G(^TMP("BGPMU0441",J,"C","NUM"))
 S CEXC1CT=+$G(^TMP("BGPMU0441",J,"C","EXC"))
 S CMP=$S((CDEN1CT-CEXC1CT)>0:$$ROUND^BGPMUA01(CNUM1CT/(CDEN1CT-CEXC1CT),3)*100,1:0)
 S PDEN1CT=+$G(^TMP("BGPMU0441",J,"P","DEN"))                 ;previous
 S PNUM1CT=+$G(^TMP("BGPMU0441",J,"P","NUM"))
 S PEXC1CT=+$G(^TMP("BGPMU0441",J,"P","EXC"))
 S PMP=$S((PDEN1CT-PEXC1CT)>0:$$ROUND^BGPMUA01(PNUM1CT/(PDEN1CT-PEXC1CT),3)*100,1:0)
 S BDEN1CT=+$G(^TMP("BGPMU0441",J,"B","DEN"))                 ;baseline
 S BNUM1CT=+$G(^TMP("BGPMU0441",J,"B","NUM"))
 S BEXC1CT=+$G(^TMP("BGPMU0441",J,"B","EXC"))
 S BMP=$S((BDEN1CT-BEXC1CT)>0:$$ROUND^BGPMUA01(BNUM1CT/(BDEN1CT-BEXC1CT),3)*100,1:0)
 S BGPDNCNT=BGPDNCNT+1
 S BGPSSTR="MU.EP.0441."_N_U_"# w/rehab service at discharge"_U_CEXC1CT_U_CDEN1CT_U_CNUM1CT_U_CMP_U_U_U_U
 ; 11 12 13 14
 ...
 Q
 ;
XML0441 ;XML output for Stroke Measure 0441
 ; BGPXML(i)=Population Number^Numerator Number^Denominator Count
 N STRING
 S STRING=$$441^BGPMUHP5("C")
 S BGPXML(1)="NQF_0441"_U_U_+$P(STRING,U,1)_U_+$P(STRING,U,2)
 K ^TMP("BGPMU0441",$J)
 Q
 ;
