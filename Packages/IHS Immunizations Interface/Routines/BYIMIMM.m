...
OUT I '$D(ZTQUEUED) D
 ...
 .D PATH
 .I OPATH="" D NOPATH Q
 .S FILE="izdata_"_$G(ASUFAC)_"_"_(DT+17000000)_"_"_$P($H,",",2)_$S($G(BYIMTEST):"_test",1:"")_"."_BYIMEXT
...
HFSARES ;EP;RESTART EXPORT WHEN FILE NOT CREATED 
 ...
 D LOG^BYIMIMM4(FILE,"E",DFNCNT,IMMCNT,,,,OPATH)
 K ^BYIMTMP("NUM")
 D ADDEX
 Q
 ;-----
...
ADDEX ;EP;TO PROCESS ADDITIONAL EXPORT SITES
 ...
 .D LOG^BYIMIMM4(FILE,"E",DFNCNT,IMMCNT,,,,BYIMOUT)
 Q
 ;-----
LE ;PROCESS SEQUENTIAL EXPORT
 ...
 .S FILE="NoImmsToExp_"_$G(ASUFAC)_"_"_(DT+17000000)_"_"_$P($H,",",2)_".dat"
 .S DFNCNT=0
 .S IMMCNT=0
 .D PATH^BYIMIMM
 .D LOG^BYIMIMM4(FILE,"E",DFNCNT,IMMCNT,,,,OPATH)
 .S:'$G(BYIMTEST) $P(^BYIMPARA(DUZ(2),"LAST EXPORT",DT),U,2)="NO IMMS"
 .Q:$D(ZTQUEUED)
...
PAUSE ;EP;FOR PAUSE READ
 ...
 K DIR
 S DIR(0)="E"
 S:'$D(DIR("A")) DIR("A")="Press <ENTER> to continue..."
 D ^DIR
 K DIR
 S BYIMPAUS=X
 Q
 ;-----
CLEAN ;CLEAN OUT MESSAGES WITH NO RXA SEGMENT AND BLANK RXR AND OBX SEGMENTS
 ...
 Q
 ;-----
NEW(DFN) ;DETERMINE IF PATIENT HAS NEW IMM'S THAT HAVEN'T BEEN EXPORTED
 N X,Y,Z
 S Y=0
 S X=0
 ...
 Q Y
 ;-----
IMMCNT(DFN) ;COUNT PT'S IMM'S TO BE EXPORTED
 N X,Y,Z
 S Y=0
 S X=0
 F  S X=$O(^AUPNVIMM("AC",DFN,X)) Q:'X  D
 .I $G(BYIMALL)=2 S Y=Y+1 Q
 .I '$D(^BYIMEXP("D",X)) S Y=Y+1
 Q Y
 ;-----
