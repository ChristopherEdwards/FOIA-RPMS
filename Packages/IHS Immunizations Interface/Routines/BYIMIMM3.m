...
VFCD(VSIT) ;EP;TO RETURN THE VFC DATE
 ...
 Q X
 ;-----
IVFC(INDA) ;EP;TO RETURN THE VFC CODE
 N X,Y,Z,VFC
...
ETH(DFN)  ;EP;TO RETURN ETHNICITY
 N X,Y,Z
 S Z=""
 I '$O(^DPT(+$G(DFN),.06,0)) S Z="^UNKNOWN BY PATIENT^CDCREC" Q Z
 S X=0
 ...
 .S Y=Y_"^CDCREC"
 .S Z=$S(Z]"":(Z_"~"),1:"")_Y
 S:Z="" Z="9999-4^UNKNOWN BY PATIENT^CDCREC"
 Q Z
 ;-----
...
PROT(INDA) ;EP;TO DETERMINE PROTECTION FLAG
 Q:'INDA ""
 N X
 S X=$P($G(^BIP(+$G(INDA),0)),U,24),X=$S(X=1:"N",X=0:"Y",1:"N")
 Q X
 ;-----
