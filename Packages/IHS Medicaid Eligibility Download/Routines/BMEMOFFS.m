BMEMOFFS ; IHS/PHXAO/TMJ - Print FALL OFFS to Master List for a specific Log ; 
...
PRINT ;PRINT CLOSED DISCLOSURES BY DATE
 ;W !
 S BMEBY=$S(BMEBY=0:"[BME MASTER FALLOFF]",1:"@INTERNAL(#.05)")
 S FLDS="[BME MASTER OFF]",BY=BMEBY,DIC="^BMEMASTR(",L=0
 I BMEALL=1 S FR=BMELOGDT,TO=BMELOGDT
 K DHIT,DIOEND,DIOBEG
 D EN1^DIP
END ;
 K BMEMBD,BMEMED,X,DD0,B,BMEALL,BMEBY,BMELOG Q
 ;
