BDGICS3 ; IHS/OIT/LJF - OBSERVATIONS CODING STATUS ;  [ 04/08/2004  4:02 PM ]
 ...
 ;IHS/OIT/LJF 02/16/2006 PATCH 1005 new routine
 ...
 ;
 D ZIS^BDGF("PQ","EN^BDGICS3","OBSERVATION CODING STATUS","BDGBM;BDGEM")
 Q
 ;
 ;
EN ;EP; -- main entry point for BDG IC CODE STATUS DS
 ...
 D EN^VALM("BDG IC CODE STATUS OBS")
 D CLEAR^VALM1
 Q
 ;
HDR ;EP; -- header code
...
INIT ;EP; -- init variables and list array
 ...
 K ^TMP("BDGICS3",$J),^TMP("BDGICS3A",$J)
 S VALMCNT=0
 ;
 ; loop through observation visits for date range
 ...
 .. ;
 .. Q:$$GET1^DIQ(9000010,VST,.07)'="OBSERVATION"
 .. Q:$$GET1^DIQ(9000010,VST,.11)="DELETED"
 .. Q:$$GET1^DIQ(9000010,VST,.06,"I")'=DUZ(2)  ;visit from different facility
 .. ;
 .. ;increment # observations per month
 .. S COUNT(MONTH,"OBS")=$G(COUNT(MONTH,"OBS"))+1
 .. ;
 .. ; check if uncoded
 ...
 .. I $$GET1^DIQ(9000010,VST,1106)]"" S COUNT(MONTH,"EXP")=$G(COUNT(MONTH,"EXP"))+1  ;cmi/maw 10/20/2008 PATCH 1010 new export date
 ;
 ; build display array
 ;    monthly counts heading
 S LINE=$$PAD($$PAD("Month/Year",13)_"# Obsrv Vsts",26)
 ...
 I $D(^TMP("BDGICS3A",$J,"U")) D
 . S LINE=$$PAD($$PAD("Visit Date",15)_"Patient Name",45)
 . S LINE=$$PAD(LINE_"Chart #",58)_"Srv"
 . S LINE=$$PAD(LINE,64)_"Insurance"
 . D SET("",.VALMCNT),SET(LINE,.VALMCNT)
 ;
 S DATE=0 F  S DATE=$O(^TMP("BDGICS3A",$J,"U",DATE)) Q:'DATE  D
 . S VST=0 F  S VST=$O(^TMP("BDGICS3A",$J,"U",DATE,VST)) Q:'VST  D
 .. ;
 .. S DFN=$$GET1^DIQ(9000010,VST,.05,"I")                 ;pat ien
 .. S LINE=$$PAD($$NUMDATE^BDGF(DATE\1),15)               ;admit date
 .. S LINE=$$PAD(LINE_$E($$GET1^DIQ(2,DFN,.01),1,22),45)  ;name
 .. S LINE=$$PAD(LINE_$J($$HRCN^BDGF2(DFN,DUZ(2)),6),58)
 .. ;
 .. S ADM=$O(^DGPM("AVISIT",VST,0))
 .. I ADM S LINE=LINE_$E($$ADMSRV^BDGF1(ADM,DFN),1,3)     ;service
 .. ;
 ...
 I $D(^TMP("BDGICS3A",$J,"E")) D
 . I $E(IOST,1,2)="P-" D SET("@@@",.VALMCNT)   ;mark errors for paper
 . S LINE=$$PAD("Visit Date",16)_"Patient Name"
 . S LINE=$$PAD($$PAD(LINE,38)_"Chart #",48)_"Error Message"
 . D SET("",.VALMCNT),SET(LINE,.VALMCNT)
 ;
 S DATE=0 F  S DATE=$O(^TMP("BDGICS3A",$J,"E",DATE)) Q:'DATE  D
 . S VST=0 F  S VST=$O(^TMP("BDGICS3A",$J,"E",DATE,VST)) Q:'VST  D
 .. ;
 .. S DFN=$$GET1^DIQ(9000010,VST,.05,"I")                    ;pat ien
 .. S LINE=$$PAD($$NUMDATE^BDGF(DATE\1),16)                  ;admit date
 .. S LINE=$$PAD(LINE_$E($$GET1^DIQ(2,DFN,.01),1,18),38)     ;name
 .. S LINE=LINE_$J($$HRCN^BDGF2(DFN,DUZ(2)),6)               ;chart #
 .. S LINE=$$PAD(LINE,48)_^TMP("BDGICS3A",$J,"E",DATE,VST)   ;err msg
 .. D SET(LINE,.VALMCNT)
 ;
 I '$D(^TMP("BDGICS3",$J)) D SET("NO DATA FOUND",.VALMCNT)
 K ^TMP("BDGICS3A",$J)
 Q
 ;
ERR(MSG,VST,DATE) ; increment error count and save for listing
 ...
 S ^TMP("BDGICS3A",$J,"E",DATE,VST)=MSG
 Q
 ;
UNCODED(DATE,VST) ; save uncoded visits by discharge date
 ...
 S ^TMP("BDGICS3A",$J,"U",DATE,VST)=""
 Q
 ;
SET(DATA,NUM) ; put data line into display array
 S NUM=NUM+1
 S ^TMP("BDGICS3",$J,NUM,0)=DATA
 Q
 ;
PRINT ; print report to paper
 NEW BDGL,FIRST
 U IO S FIRST=1 D HDG
 S BDGL=0 F  S BDGL=$O(^TMP("BDGICS3",$J,BDGL)) Q:'BDGL  D
 . I ^TMP("BDGICS3",$J,BDGL,0)="@@@" S FIRST=1 Q  ;beginning of errors
 . I $Y>(IOSL-4) D HDG
 . W !,^TMP("BDGICS3",$J,BDGL,0)
 D ^%ZISC,EXIT
 Q
 ;
HDG ; heading when printing to paper
 W @IOF W !?25,"OBSERVATIONS CODING STATUS REPORT"
 D HDR F I=1:1 Q:'$D(VALMHDR(I))  W !,VALMHDR(I)
 I FIRST W !,$$REPEAT^XLFSTR("=",80),! S FIRST=0 Q
 W !,"Visit Date",?24,"Patient Name",?44,"Chart #"
