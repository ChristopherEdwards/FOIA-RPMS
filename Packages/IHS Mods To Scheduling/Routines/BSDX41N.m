...
FLOWOUT ; <DISPLAY>
 ...
 ;X APCHSCKP Q:$D(APCHSQIT)
 I 'APCHSNPG S APCHSP="",$P(APCHSP,"-",APCHSMXL+9)="" S BSDXI=BSDXI+1 S ^BSDXTMP($J,BSDXI)=" "_APCHSP_$C(30)
 ;X APCHSCKP Q:$D(APCHSQIT)
 I 'APCHSNPG S BSDXI=BSDXI+1 S ^BSDXTMP($J,BSDXI)=$C(30)
 Q
FLOWCHK ; <SCREEN>
...
FLOWHD ;ENTRY POINT
 ; DISPLAY HEADER
 ;X APCHSCKP Q:$D(APCHSQIT)
 S BSDXI=BSDXI+1 S ^BSDXTMP($J,BSDXI)=APCHSFCN_$C(30)
 I $O(^APCHSFLC(APCHSFDF,3,0)) S BSDXTMP=" "_"Clinics limited to:" S X=0 F  S X=$O(^APCHSFLC(APCHSFDF,3,X)) Q:'X  G:APCHSNPG FLOWHD S BSDXI=BSDXI+1 S ^BSDXTMP($J,BSDXI)=BSDXTMP_$$FILL^BSDX41(22-$L(BSDXTMP))_$P(^DIC(40.7,X,0),U)_$C(30)
 ;X APCHSCKP Q:$D(APCHSQIT)
 G:APCHSNPG FLOWHD
 F APCHSII=0:0 S APCHSII=$O(APCHSTB(APCHSII)) Q:'APCHSII  S BSDXI=BSDXI+1 S ^BSDXTMP($J,BSDXI)=$$FILL^BSDX41(11-APCHSTB(APCHSII))_APCHSTB(APCHSII,"L")_$C(30)
 Q
