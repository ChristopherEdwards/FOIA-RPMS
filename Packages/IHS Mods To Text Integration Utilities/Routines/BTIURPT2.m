BTIURPT2 ; IHS/ITSC/LJF - Review documents by Visit Date ;
 ...
 ; Copy of ^TIURPTTL - IHS Review document titles by visit date
 ;   -- used to set up view of all documents for a patient by visit date
 ...
 ;   -- changed beginning date default to T-6M
...
PATIENT ; Select Patient
 ...
 K DFN I $G(TIUZIHS) S DFN=+TIUZIHS       ;check if caled by other app
 E  S DFN=+$$PATIENT^TIULA                ;else use VA code
 I +DFN'>0 S VALMQUIT=1 Q
DOCTYPE ; Select Document Type(s)
 ;commented out VA code below so can view all documents for patient & date
 ;N TIUDCL
...
SCREEN ;
 ...
 S SCREEN=1,SCREEN(1)="AIHS1^"_DFN        ;use IHS xref
 ;D CHECKADD(.TIUTYP)                     ;original VA
ERLY ;S TIUEDFLT=$S(TIUCLASS=3:"T-2",TIUCLASS=244:"T-30",1:"T-7")           ;original VA
 S TIUEDFLT="T-6M"                                                      ;default to 6 months
 ;S TIUEDT=$S($D(TIUQUIK):1,1:$$EDATE^TIULA("Reference","",TIUEDFLT))   ;original VA
 S TIUEDT=$S($D(TIUQUIK):1,1:$$EDATE^TIULA("Visit","",TIUEDFLT))        ;select by visit date
 I +$G(DIROUT) S VALMQUIT=1 Q
 I TIUEDT'>0 G SCREEN
 ;S TIULDT=$S($D(TIUQUIK):9999999,1:$$LDATE^TIULA("Reference"))         ;original VA code
 S TIULDT=$S($D(TIUQUIK):9999999,1:$$LDATE^TIULA("Visit"))              ;select by visit date
 I +$G(DIROUT) S VALMQUIT=1 Q
 I TIULDT'>0 G ERLY
...
GATHER(TIUI,TIUPREF,CLASS) ; Find/sort records for the list
 ...
 S TIUSFLD=".03"                      ;sort by visit
 ;
 ; IHS loop thru all doc types, not just selection by user
 ...
 . . . . ;I TIUTP=81,(+TYPES>1),($P(TYPES(TIUT),U,4)="NOT PICKED"),'+$$DADINTYP(TIUIFN,.TYPES) Q ;don't screen addendums
...
RESOLVE(DA,DR) ; Call DIQ1 to resolve field values
 Q $$GET1^DIQ(9000010,+$$GET1^DIQ(8925,DA,.03,"I"),.01,"I")_U_$$GET1^DIQ(8925,DA,DR,"I")_U_$$GET1^DIQ(8925,DA,1201,"I")  ;IHS list of fields
...
ADDELMNT(DA,TIUCNT,APPEND) ; Add each element to the list
 ...
 ;S TIUREC=$$SETFLD^VALM1(TIULST4,TIUREC,"LAST I/LAST 4")         ;original VA
 ;S TIUREC=$$SETFLD^VALM1(PT,TIUREC,"PATIENT NAME")               ;original VA
 ;S TIUREC=$$SETFLD^VALM1(DOC,TIUREC,"DOCUMENT TYPE")             ;original VA
 ; S TIUREC=$$SETFLD^VALM1(ADT,TIUREC,"ADMISSION DATE")           ;original VA
 ;S TIUREC=$$SETFLD^VALM1(EDT,TIUREC,"REF DATE")                  ;original VA
 ;S TIUREC=$$SETFLD^VALM1(SDT,TIUREC,"SIG DATE")                  ;original VA
 ...
 S TIUREC=$$SETFLD^VALM1(EDT,TIUREC,"DOC DATE")
