BTIURPT3 ; IHS/ITSC/LJF - Review documents by Reference Date ;
 ;;1.0;TEXT INTEGRATION UTILITIES;;NOV 04, 2004
 ; Copy of ^TIURPTTL - IHS Browse docuemnt texts by reference date
...
DOCTYPE ; Select Document Type(s)
 ;commented out VA code so search is done on all documents
 ;N TIUDCL
...
SCREEN ;
 ...
 ;D CHECKADD(.TIUTYP)                   ;original VA - always include addendums
ERLY ;S TIUEDFLT=$S(TIUCLASS=3:"T-2",TIUCLASS=244:"T-30",1:"T-7")   ;original VA
 S TIUEDFLT="T-6M"                                              ;default now 6 months
...
BUILD(STATUS,TYPES,SCREEN,EARLY,LATE) ; Build List
 ...
 K ^TMP("TIURIHS",$J) S (TIUZLN,TIUZCNT)=0             ;added code
...
GATHER(TIUI,TIUPREF,CLASS) ; Find/sort records for the list
 ...
 . . . . ;I TIUTP=81,(+TYPES>1),($P(TYPES(TIUT),U,4)="NOT PICKED"),'+$$DADINTYP(TIUIFN,.TYPES) Q ;original VA-don't screen out addendums
...
ADDELMNT(DA,TIUCNT,APPEND) ; Add each element to the list
 ...
 ;
 D NOTES^BTIURPT(DA,"R")    ; IHS call to display docuemnt text
 ;
...
CLEAN ; Clean up your mess!
 ...
 K VALMY
 K TIUZCNT,TIUZLN         ;clean up IHS variables too
 Q
URGENCY(TIUDA) ;EP - What is the urgency of the current document
 ; called by ^BTIURPT as well as this routine
