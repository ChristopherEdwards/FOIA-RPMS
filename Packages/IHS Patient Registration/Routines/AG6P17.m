AG6P17 ;IHS/ASDST/GTH - Patient Registration 6.0 Patch 17 ;   [ 04/03/2003  11:52 AM ]
 ...
 NEW AGQUIT
 S AGQUIT=0
 I '$$VCHK("AG","6.0",2) S AGQUIT=2
 ;
 I '$$VCHK("AUPN","99.1",2) S AGQUIT=2
 ;
 S X=$$PATCH^XPDUTL("AUPN*99.1*9")
 I X=0 W !,$$CJ^XLFSTR("AUPN v99.1 Patch 9 NOT INSTALLED",IOM) S AGQUIT=2
 ;
 I '$$VCHK("AUT","98.1",2) S AGQUIT=2
 ;
 S X=$$PATCH^XPDUTL("AUT*98.1*12")
 I X=0 W !,$$CJ^XLFSTR("AUT v98.1 Patch 12 NOT INSTALLED",IOM) S AGQUIT=2
 ;
 NEW DA,DIC
...
OPTSAV(AGM) ;
 ...
 I $D(^XTMP("AG6P17",6.17,"OPTSAV",AGM)) D BMES^XPDUTL("NOT SAVED.  Option '"_AGM_"' has previously been saved.") Q
 I '$D(^XTMP("AG6P17")) S ^XTMP("AG6P17",0)=$$FMADD^XLFDT(DT,30)_U_DT_U_"AG6P17 - SAVE OPTION CONFIGURATIONS."
 NEW I,A
 S I=$O(^DIC(19,"B",AGM,0))
 I 'I D BMES^XPDUTL("NOT SAVED.  Option '"_AGM_"' not found in OPTION file.") Q
 S A=0
 F  S A=$O(^DIC(19,I,10,A)) Q:'A  S ^XTMP("AG6P17",6.17,"OPTSAV",AGM,A)=$P(^DIC(19,+^DIC(19,I,10,A,0),0),U,1)_U_$P(^DIC(19,I,10,A,0),U,2,3)
 Q
 ;
