AG71ENV ;IHS/SD/EFG - Patient Registration 7.1 ENVIRONMENT CHECKER ;   
 ...
 W !,$$CJ^XLFSTR("Need at least Kernel patch 1009....patch 1009 Present",IOM)
 ;
 I $$VERSION^XPDUTL("AG")="7.1" W !,$$CJ^XLFSTR("Version 7.1 already Present",IOM) G CONT
 I '$$VCHK("AG","7.0",2) S AGQUIT=2
 ;
 S X=$$LAST^XPDUTL("IHS PATIENT REGISTRATION","7.0")
 I $P(X,U)<5 W !,$$CJ^XLFSTR("AG v7.0 Patch 5 NOT INSTALLED",IOM) S AGQUIT=2
 ;
CONT ;EP
 ...
 I $P(X,U)<15 W !,$$CJ^XLFSTR("AUPN v99.1 Patch 15 NOT INSTALLED",IOM) S AGQUIT=2
 ;
 I '$$VCHK("AUT","98.1",2) S AGQUIT=2
 ;
 S X=$$LAST^XPDUTL("IHS DICTIONARIES (POINTERS)","98.1")
 I $P(X,U)<15 W !,$$CJ^XLFSTR("AUT v98.1 Patch 15 NOT INSTALLED",IOM) S AGQUIT=2
 ;
 NEW DA,DIC
