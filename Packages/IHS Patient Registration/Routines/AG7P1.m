AG7P1 ;IHS/SD/EFG - Patient Registration 7.0 Patch 1 ;   [ 05/08/2003  1:14 PM ]
 ...
 ;
 S X=$$PATCH^XPDUTL("AUPN*99.1*10")
 I X=0 W !,$$CJ^XLFSTR("AUPN v99.1 Patch 10 NOT INSTALLED",IOM) S AGQUIT=2
 ;
...
OPTSAV(AGM) ;
 ...
 I $D(^XTMP("AG7P1",7.1,"OPTSAV",AGM)) D BMES^XPDUTL("NOT SAVED.  Option '"_AGM_"' has previously been saved.") Q
 I '$D(^XTMP("AG7P1")) S ^XTMP("AG7P1",0)=$$FMADD^XLFDT(DT,30)_U_DT_U_"AG7P1 - SAVE OPTION CONFIGURATIONS."
 NEW I,A
 S I=$O(^DIC(19,"B",AGM,0))
 I 'I D BMES^XPDUTL("NOT SAVED.  Option '"_AGM_"' not found in OPTION file.") Q
 S A=0
 F  S A=$O(^DIC(19,I,10,A)) Q:'A  S ^XTMP("AG7P1",7.1,"OPTSAV",AGM,A)=$P(^DIC(19,+^DIC(19,I,10,A,0),0),U,1)_U_$P(^DIC(19,I,10,A,0),U,2,3)
 Q
 ;
