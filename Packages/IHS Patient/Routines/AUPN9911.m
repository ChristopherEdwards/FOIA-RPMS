AUPN9911 ;IHS/CMI/LAB,GTH,EFG,SDR - AUPN 99.1 PATCH 11 ; [ 10/09/03  12:35 PM ]
 ;;99.1;IHS DICTIONARIES (PATIENT);**11**;JUN 13, 2003;Build 9
 ;
 ...
 I $$VCHK("AUPN","99.1",2)
 ;
 I '$$INSTALLD("AUT*98.1*12") D SORRY(2)
 ;
 NEW DA,DIC
...
VCHK(AUPNPRE,AUPNVER,AUPNQUIT) ; Check versions needed.
 ...
 ;
PRE ;EP - From KIDS.
 D BMES^XPDUTL("Beginning Pre-install routine (PRE^AUPN9910).")
 ...
 ;
POST ;EP - From KIDS.
 D BMES^XPDUTL("Beginning post-install routine (POST^AUPN99P10).")
 ;
...
MAIL ; Send install mail message.
 D BMES^XPDUTL("Delivering AUPN*99.1*11 install message to select users...")
 NEW DIFROM,XMSUB,XMDUZ,XMTEXT,XMY
 KILL ^TMP("AUPN9910",$J)
 S ^TMP("AUPN9911",$J,1)=" --- AUPN v 99.1, Patch 11, has been installed into this uci ---"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   S ^TMP("AUPN9910",$J,(%+1))=" "_^(%,0)
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""AUPN9910"",$J,",XMY(1)="",XMY(DUZ)=""
