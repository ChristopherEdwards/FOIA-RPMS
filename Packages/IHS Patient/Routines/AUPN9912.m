AUPN9912 ;IHS/CMI/LAB,GTH,EFG,SDR - AUPN 99.1 PATCH 12 ; [ 10/09/03  12:35 PM ]
 ;;99.1;IHS DICTIONARIES (PATIENT);**12**;JUN 13, 2003;Build 9
 ;
...
VCHK(AUPNPRE,AUPNVER,AUPNQUIT) ; Check versions needed.
 ...
 ;
PRE ;EP - From KIDS.
 D BMES^XPDUTL("Beginning Pre-install routine (PRE^AUPN9912).")
 ...
 ;
POST ;EP - From KIDS.
 D BMES^XPDUTL("Beginning post-install routine (POST^AUPN99P12).")
 ;
...
MAIL ; Send install mail message.
 ...
 KILL ^TMP("AUPN9912",$J)
 S ^TMP("AUPN9912",$J,1)=" --- AUPN v 99.1, Patch 12, has been installed into this uci ---"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   S ^TMP("AUPN9912",$J,(%+1))=" "_^(%,0)
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""AUPN9912"",$J,",XMY(1)="",XMY(DUZ)=""
