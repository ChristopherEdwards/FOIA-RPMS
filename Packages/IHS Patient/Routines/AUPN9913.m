AUPN9913 ;IHS/CMI/LAB,GTH,EFG,SDR - AUPN 99.1 PATCH 13 ; [ 04/20/2004  7:35 AM ]
 ;;99.1;IHS DICTIONARIES (PATIENT);**13**;JUN 13, 2003;Build 9
 ;
...
VCHK(AUPNPRE,AUPNVER,AUPNQUIT) ; Check versions needed.
 ...
 ;
PRE ;EP - From KIDS.
 D BMES^XPDUTL("Beginning Pre-install routine (PRE^AUPN9913).")
 ...
 ;
POST ;EP - From KIDS.
 D BMES^XPDUTL("Beginning post-install routine (POST^AUPN9913).")
 ;
...
MAIL ; Send install mail message.
 D BMES^XPDUTL("Delivering AUPN*99.1*12 install message to select users...")
 NEW DIFROM,XMSUB,XMDUZ,XMTEXT,XMY
 KILL ^TMP("AUPN9913",$J)
 S ^TMP("AUPN9913",$J,1)=" --- AUPN v 99.1, Patch 13, has been installed into this uci ---"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   S ^TMP("AUPN9913",$J,(%+1))=" "_^(%,0)
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""AUPN9913"",$J,",XMY(1)="",XMY(DUZ)=""
