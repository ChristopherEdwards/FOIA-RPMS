AUPN9914 ;IHS/CMI/LAB,GTH,EFG,SDR - AUPN 99.1 PATCH 14 ; [ 08/31/04  4:14 PM ]
 ;;99.1;IHS DICTIONARIES (PATIENT);**14**;JUN 13, 2003;Build 9
 ;
 ...
 I $$VCHK("AUPN","99.1",2)
 ;
 I '$$INSTALLD("AUT*98.1*13") D SORRY(2)
 ;
 NEW DA,DIC
...
VCHK(AUPNPRE,AUPNVER,AUPNQUIT) ; Check versions needed.
 ...
 ;
PRE ;EP - From KIDS.
 K ^AUPNTEMP("POST INIT PATCH 14")
 I $P(^DD(9000001,.14,0),U,2)[6 S ^AUPNTEMP("POST INIT PATCH 14")=6
 D BMES^XPDUTL("Beginning Pre-install routine (PRE^AUPN9914).")
 ...
 ;
 ;delete dd fields
 D MES^XPDUTL("   Checking for installation of AUPN*99.1*11.")
 I '$$INSTALLD("AUPN*99.1*11") D  I 1
 . D MES^XPDUTL("AUPN*99.1*11 NOT installed.  Deleting dd fields .")
 . S DIK="^DD(9000010,",DA=15001,DA(1)=9000010 D ^DIK
 . S DIK="^DD(9000010,",DA=15003,DA(1)=9000010 D ^DIK
 . S DIK="^DD(9000010,",DA=.01,DA(1)=9000010 D ^DIK
 . S DIK="^DD(9000010,",DA=.02,DA(1)=9000010 D ^DIK
 . S DIK="^DD(9000010,",DA=.13,DA(1)=9000010 D ^DIK
 . S DIK="^DD(9000010,",DA=.22,DA(1)=9000010 D ^DIK
 . S DIK="^DD(9000010,",DA=.05,DA(1)=9000010 D ^DIK
 . S DIK="^DD(9000010.11,",DA=.011,DA(1)=9000010.11 D ^DIK
 D MES^XPDUTL("Pre-install routine is complete.")
 Q
 ;
POST ;EP - From KIDS.
 D BMES^XPDUTL("Beginning post-install routine (POST^AUPN9914).")
 ;
 ...
 ;
 ; Re-index "C" x-ref of AUPNMSP file
 S DIK="^AUPNMSP("
 S DIK(1)=".02^C"
 D ENALL^DIK
 ;
 ...
 ;
 D ^AUPNSEC
 ;if still pointing to file 6 run init
 I $G(^AUPNTEMP("POST INIT PATCH 14"))=6 D
 . S DIK="^DD(9000001,",DA=.14,DA(1)=9000010 D ^DIK
 . D ^AUP6INIT
 . K ^AUPNTEMP("POST INIT PATCH 14")
 ;
...
MAIL ; Send install mail message.
 D BMES^XPDUTL("Delivering AUPN*99.1*14 install message to select users...")
 NEW DIFROM,XMSUB,XMDUZ,XMTEXT,XMY
 KILL ^TMP("AUPN9914",$J)
 S ^TMP("AUPN9914",$J,1)=" --- AUPN v 99.1, Patch 14, has been installed into this uci ---"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   S ^TMP("AUPN9914",$J,(%+1))=" "_^(%,0)
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""AUPN9914"",$J,",XMY(1)="",XMY(DUZ)=""
