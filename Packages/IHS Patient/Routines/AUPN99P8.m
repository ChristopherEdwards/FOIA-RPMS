AUPN99P8 ;IHS/CMI/LAB,GTH,EFG,SDR - AUPN 99.1 PATCH 8 ; [ 05/09/2003  7:59 AM ]
 ;;99.1;IHS DICTIONARIES (PATIENT);**8,9,10**;JUN 13, 2003;Build 9
 ;
...
VCHK(AUPNPRE,AUPNVER,AUPNQUIT) ; Check versions needed.
 ...
 ;
PRE ;EP - From KIDS.
 D BMES^XPDUTL("Beginning Pre-install routine (PRE^AUPN99P8).")
 ...
 ;
POST ;EP - From KIDS.
 D BMES^XPDUTL("Beginning post-install routine (POST^AUPN99P8).")
 ;
...
MAIL ; Send install mail message.
 D BMES^XPDUTL("Delivering AUPN*99.1*8 install message to select users...")
 NEW DIFROM,XMSUB,XMDUZ,XMTEXT,XMY
 KILL ^TMP("AUPN99P8",$J)
 S ^TMP("AUPN99P8",$J,1)=" --- AUPN v 99.1, Patch 8, has been installed into this uci ---"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   S ^TMP("AUPN99P8",$J,(%+1))=" "_^(%,0)
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""AUPN99P8"",$J,",XMY(1)="",XMY(DUZ)=""
