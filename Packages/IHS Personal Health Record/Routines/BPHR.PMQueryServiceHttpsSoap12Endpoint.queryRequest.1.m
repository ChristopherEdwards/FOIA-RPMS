 ;BPHR.PMQueryServiceHttpsSoap12Endpoint.queryRequest.1
 ;(C)InterSystems, generated for class BPHR.PMQueryServiceHttpsSoap12Endpoint.queryRequest.  Do NOT edit. 10/22/2016 08:43:39AM
 ;;4D794532;BPHR.PMQueryServiceHttpsSoap12Endpoint.queryRequest
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..request)=1 Set r%request=r%request.%ConstructClone(1,.cloned),i%request=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%request If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zInvoke(%Client,%Action,request) public {
 Set ..request=$get(request)
 Do %Client.InvokeClient($this,"queryRequest",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="queryRequestResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BPHR.PMQueryServiceHttpsSoap12Endpoint.queryRequest",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BPHR.PMQueryServiceHttpsSoap12Endpoint.queryRequest")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BPHR.PMQueryServiceHttpsSoap12Endpoint.queryRequest")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BPHR.PMQueryServiceHttpsSoap12Endpoint.queryRequest"))
 If $IsObject(namespaces) {
   If namespaces.Stable,namespaces.CurrentNamespace="http://www.ihs.hhs.gov/phr/xsd",'tmpi||(typesPrefix'="") {
     Set topAttrs=""
   } Else {
     Set popAtEnd=1,saveTopPrefix=topPrefix,saveTypesPrefix=typesPrefix,saveAttrsPrefix=attrsPrefix,saveUsePrefix=usePrefix
     Set sc=namespaces.PushNodeForExport("http://www.ihs.hhs.gov/phr/xsd",$get(local,0),tmpi,"",,.topPrefix,.topAttrs,.typesPrefix,.attrsPrefix,.usePrefix)
     If 'sc Quit sc
   }
   Set beginprefix=$select(namespaces.ElementQualified&&usePrefix:typesPrefix,1:"")
   If topAttrs'="" Set temp=temp_" "_topAttrs
   If tag="" Set tag="queryRequest"
   ...
   If tag="" Set tag=typesPrefix_"queryRequest"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"queryRequest"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="request",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BPHR.ax21.PMRequest"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BPHR.PMQueryServiceHttpsSoap12Endpoint.queryRequest",.imports,.classes)
...
zXMLImportInternal()
 ...
 Set nsIndex=$get(@(tree)@("ns","http://www.ihs.hhs.gov/phr/xsd"))
 Set (node,ref)=nodeArg
 If ($listget($get(@(tree)@(node,0)),1)'="e")||(tag'=@(tree)@(node)) Goto XMLImportMalformed
 If bareProjection Quit $$Error^%apiOBJ(6386,"BPHR.PMQueryServiceHttpsSoap12Endpoint.queryRequest")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="queryRequestResult" {
   ...
   If (class="") || (class="PMQueryResponse") {
     Set class="BPHR.ax21.PMQueryResponse"
   } Else {
     If $length(class,":")=2 Set class=$piece(class,":",2)
     Set class=$get(^oddCOM("BPHR.ax21.PMQueryResponse",85,"t",class))_$get(^oddXML("BPHR.ax21.PMQueryResponse","t",class)) If class="" Set class=0
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..request) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BPHR.PMQueryServiceHttpsSoap12Endpoint.queryRequest).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BPHR.PMQueryServiceHttpsSoap12Endpoint.queryRequest",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BPHR.ax21.PMQueryResponse).%New() If newobject="" Quit ""
	Set ..%Result=newobject
	Quit newobject }
zrequestNewObject() public {
	Set newobject=##class(BPHR.ax21.PMRequest).%New() If newobject="" Quit ""
	Set ..request=newobject
	Quit newobject }
