 ;BPHR.ax21.PMRequest.1
 ;(C)InterSystems, generated for class BPHR.ax21.PMRequest.  Do NOT edit. 10/22/2016 08:43:39AM
 ;;564B7152;BPHR.ax21.PMRequest
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%"%%OID"=""
	If deep>0 {
		If $isobject(..query)=1 Set r%query=r%query.%ConstructClone(1,.cloned),i%query=""
	}
	Quit 1 }
%NormalizeObject() public {
	If '$system.CLS.GetModified() Quit 1
	If m%loginId Set:i%loginId'="" i%loginId=(..loginIdNormalize(i%loginId))
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%query If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
%ValidateObject(force=0) public {
	Set sc=1
	If '$system.CLS.GetModified() Quit sc
	If m%loginId Set iv=..loginId If iv'="" Set rc=(..loginIdIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"loginId",iv)
	If m%password Set iv=..password If iv'="" Set rc=(..passwordIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"password",iv)
	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BPHR.ax21.PMRequest",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BPHR.ax21.PMRequest")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BPHR.ax21.PMRequest")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BPHR.ax21.PMRequest"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"PMRequest"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"PMRequest"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..loginId
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"loginId"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"loginId>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..password
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"password"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"password>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..query
 If $IsObject(val) , deepFlag {
   Set id=""
   If encoded,'inlineFlag {
     Set temp=$select($parameter("BPHR.ax21.Query","XMLSUMMARY")'="":-1,1:1)
     Set id=+$get(idlist(temp*val))
     If 'soap12 , (id=0) Set id=$increment(idlist),oreflist(temp*id)=val,idlist(temp*val)=id
   }
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"query "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="query",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BPHR.ax21.Query"),local=1,savexsiAttrs=xsiAttrs
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BPHR.ax21.PMRequest" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BPHR.ax21.PMRequest",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BPHR.ax21.PMRequest")
 If encoded {
 ...
 If tag="query" {
   ...
   If encoded,$$XMLImportId() {
     ...
     If (class="") || (class="Query") {
       Set class="BPHR.ax21.Query"
     } Else {
       If $length(class,":")=2 Set class=$piece(class,":",2)
       Set class=$get(^oddCOM("BPHR.ax21.Query",85,"s",class))_$get(^oddXML("BPHR.ax21.Query","s",class)) If class="" Set class=0
...
zXMLIsObjectEmpty(ignoreNull)
 If ..loginId'="" Quit 0
 If ..password'="" Quit 0
 If $IsObject(..query) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BPHR.ax21.PMRequest).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BPHR.ax21.PMRequest",top,format,namespacePrefix,input,refOnly,.schema)
zqueryNewObject() public {
	Set newobject=##class(BPHR.ax21.Query).%New() If newobject="" Quit ""
	Set ..query=newobject
	Quit newobject }
