 ;BPHR.ax21.Query.1
 ;(C)InterSystems, generated for class BPHR.ax21.Query.  Do NOT edit. 10/22/2016 08:43:39AM
 ;;79723173;BPHR.ax21.Query
 ;
%NormalizeObject() public {
	If '$system.CLS.GetModified() Quit 1
	If m%euid Set:i%euid'="" i%euid=(..euidNormalize(i%euid))
	If m%from Set:i%from'="" i%from=(..fromNormalize(i%from))
	If m%providerDomainAddress Set:i%providerDomainAddress'="" i%providerDomainAddress=(..providerDomainAddressNormalize(i%providerDomainAddress))
	If m%to Set:i%to'="" i%to=(..toNormalize(i%to))
	Quit 1 }
%ValidateObject(force=0) public {
	Set sc=1
	If '$system.CLS.GetModified() Quit sc
	If m%euid Set iv=..euid If iv'="" Set rc=(..euidIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"euid",iv)
	If m%from Set iv=..from If iv'="" Set rc=(..fromIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"from",iv)
	If m%providerDomainAddress Set iv=..providerDomainAddress If iv'="" Set rc=(..providerDomainAddressIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"providerDomainAddress",iv)
	If m%to Set iv=..to If iv'="" Set rc=(..toIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"to",iv)
	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BPHR.ax21.Query",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BPHR.ax21.Query")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BPHR.ax21.Query")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BPHR.ax21.Query"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"Query"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"Query"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..euid
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"euid"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"euid>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..from
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"from"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"from>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..providerDomainAddress
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"providerDomainAddress"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"providerDomainAddress>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..to
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"to"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"to>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BPHR.ax21.Query",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BPHR.ax21.Query")
 If encoded {
...
XMLImportElements() ;
 ...
 If tag="providerDomainAddress" {
   ...
   Set ..providerDomainAddress=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
 If tag="to" {
   If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
   If encoded,$$XMLImportId() {
...
zXMLIsObjectEmpty(ignoreNull)
 If ..euid'="" Quit 0
 If ..from'="" Quit 0
 If ..providerDomainAddress'="" Quit 0
 If ..to'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BPHR.ax21.Query).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BPHR.ax21.Query",top,format,namespacePrefix,input,refOnly,.schema)
