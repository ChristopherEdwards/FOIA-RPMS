BCHDL ; IHS/TUCSON/LAB - DOWNLOAD PATIENT DATA TO REMOTE ;  [ 06/17/02  7:27 AM ]
 ;;1.0;IHS RPMS CHR SYSTEM;**12,13**;OCT 28, 1996
 ;
...
START ;EP
 ...
 W "This routine is to be run whenever a CHR wants to download the patient",!,"demographic data to their remote device.  The set of patients that is"
 W !,"downloaded can be selected based on items such as community of"
 W !,"residence or where the patient is registered.",!
 ;W !,"A file of records will be created called 'chrpat#.imp'."
 ...
 ;
CONT ;
 S DIR(0)="Y",DIR("A")="Do you wish to continue and create a download patient file",DIR("B")="N" K DA D ^DIR K DIR
 G:$D(DIRUT) XIT
 G:'Y XIT
FACHRN ;
 W !!
...
QUEUE ;EP
 ...
 D WAIT^DICD,^BCHDL1,XIT,XIT1
 Q
QUEUE1 ;
 S XBRC="^BCHDL1",XBRX="XIT^BCHDL",XBNS="BCH;IO*",XBIOP="HOME",XBFQ=1
 D ^XBDBQUE
 D XIT,XIT1
 Q
SCREEN ;EP
 S X2=$E(DT,1,3)_"0101",X1=DT D ^%DTC S BCHJD=X+1
 S BCHPTVS="P",BCHTYPE="P",BCHFILE="CHR"_BCHCHR_"."_BCHJD
 D SMENU
