BHLINIT ; ; 7-JUL-1997
 ...
 S DIOVRD=1,U="^",DIFQ=0,DIFROM="1.0" W !,"This version (#1.0) of 'BHLINIT' was created on 7-JUL-1997"
 W !?9,"(at TUCAPD, by VA FileMan V.21.0)",!
...
EN ; ENTER HERE TO BYPASS THE PRE-INIT PROGRAM
 ...
 I DIFROM,$D(^%ZTSK) S X="BHLINIS" X ^%ZOSF("TEST") D:$T PAC^BHLINIS($T(IXF),.DIFROM)
...
PKG S X=$P($T(IXF),";",3),DIC="^DIC(9.4,",DIC(0)="",DIC("S")="I $P(^(0),U,2)="""_$P(X,U,2)_"""",X=$P(X,U) D ^DIC S DIFROM(0)=+Y K DIC
 Q
 ;
IXF ;;IHS SUPPORT FOR HL7 INTERFACES^BHL;0
