BHLRLABU ;cmi/sitka/maw - BHL ORU/R01 Ref Lab inbound Lab Message  [ 04/24/2003  2:24 PM ] ; 19 Mar 2004  2:24 PM
 ;;3.01;BHL IHS Interfaces with GIS;**13,14,16**;JUL 01, 2001
 ;
 ;UNILAB INBOUND
 ;
 ;this routine will file the inbound unilab lab message.
 ;9/28/2004 cmi/anch/maw added collection date and accession # to BUL
 ;10/1/2004 cmi/anch/maw added collection date and accession # to SNDBUL
 ;
...
CHKPAT ;-- lookup the patient by chart
 ...
 S BHLPAT=$$CHKPAT^BHLU(PAT,BHLDUZ2)
...
OBR ;-- parse the OBR segment
 N BHLR
 S BHLR="OBR"
 S BHLODA=0 F  S BHLODA=$O(@BHLTMP@(BHLODA)) Q:BHLODA=""!($G(BHLERR("FATAL")))  D
 . K BHLPAR,BHLTST,BHLPSEQ,BHLCLTE,BHLRSLT,BHLREFR,BHLREFL,BHLREFH
 . K BHLABN,BHLUNIT,BHLRESDT
 . S BHLACC=$G(@BHLTMP@(BHLODA,3))
 . I $G(BHLACC)="" S BHLACC=$G(@BHLTMP@(BHLODA,2))
 ...
 . S BHLPAR=BHLODA
 . D MAP("ORDER")
 . I '$G(BHLCLT) D  Q  ;quit if no test match 4/16/2004
 .. S BHLERR("FATAL")=1
 .. D BUL
...
NTEOBR ;-- parse the NTE in the OBR segment
 N BHLR
 S BHLR="NTE"
 S BHLNDA=0 F  S BHLNDA=$O(@BHLSTMP@(BHLNDA)) Q:BHLNDA=""  D
 . S:'$D(BHLCMT(BHLNDA)) BHLCMT(BHLNDA)=0
 . S (BHLCMT(BHLNDA),X)=$TR($G(@BHLSTMP@(BHLNDA,3)),"""","'")
 . Q:$G(X)=""
 . K DIC,DD,DO
...
NTE ;-- parse the NTE segment
 ...
 . S:'$D(BHLCMT(BHLNDA)) BHLCMT(BHLNDA)=0
 . S (BHLCMT(BHLNDA),X)=$TR($G(@BHLSSTMP@(BHLNDA,3)),"""","'") ;cmi/maw modified line 10/26/2003
 . ;S (BHLCMT(BHLNDA),X)=$G(@BHLSSTMP@(BHLNDA,3))  ;cmi/maw new line 10/20/2003
 . K DIC,DD,DO
 . S DIC="^BLRTXLOG("_BHLSEQ_",30,"
 . S DA(1)=BHLSEQ,DIC(0)="L",DIC("P")=$P(^DD(9009022,3001,0),"^",2)
 . ;D ^DIC  cmi/maw 9/7/2004 old code, changed to FILE^DICN
 . D FILE^DICN
 Q
 ;
...
TESTMAP ;
 N BHLZDA
 K BHLCLTI,BHLCLT
 S BHLZDA=0 F  S BHLZDA=$O(^BLRRL("BTST",BHLTST,BHLRL,BHLZDA)) Q:'BHLZDA!$G(BHLCLT)  D
 ...
 ;I $G(BHLCLT)="" S BHLERCD="NOLAB" X BHLERR
 Q
 ;
...
FILE ;-- file the data in BLRTXLOG
 ...
 I $G(BHLCLT)="" S BHLERCD="NOLAB" X BHLERR D BUL Q  ;cmi/flag/maw 4/8/2004 quit if not lab match
 ...
 ;I $G(BHLCLT)="" S BHLERCD="NOLAB" X BHLERR Q  ;cmi/flag/maw changed this 1/22/2003
