BUD1RP71 ; IHS/CMI/LAB - UDS REPORT DRIVER TABLE ;
...
PRINT ;EP
 ...
 S BUDQUIT=0
 S BUDTELB="Hispanic/Latino"
 D P71
 Q:BUDQUIT
 S BUDTELB="Non-Hispanic/Latino"
 D P71
 Q:BUDQUIT
 ;D R
 ...
 Q:BUDQUIT
B7 ;
 S BUDTELB="Hispanic/Latino",BUDTELL=1
 D P7B
 Q:BUDQUIT
 S BUDTELB="Non-Hispanic/Latino",BUDTELL=2
 D P7B
 Q:BUDQUIT
 W "Unreported/Refused to Report Ethnicity",!
 W $$REPEAT^XLFSTR("_",80),!
...
P71A ;W !!,$$REPEAT^XLFSTR("_",80),!
 ...
 W "1a",?5,"| Asian" D R1
 D R
 W "1b1",?5,"| Native Hawaiian" D R1
 D R
 W "1b2",?5,"| Pacific Islander" D R1
 D R
 W "1c",?5,"| Black/African" D R1
 W ?5,"| American" D R1
 D R
 W "1d",?5,"| American Indian/" D R1
 W ?5,"| Alaska Native" D R1
 D R
 W "1e",?5,"| White" D R1
 D R
 W "1f",?5,"| More than One" D R1
 W ?5,"| Race" D R1
 D R
 W "1g",?5,"| Unreported/" D R1
 W ?5,"| Refused" D R1
 W ?5,"| to Report Race" D R1
 D R
 W ?5,"| Subtotal"
...
P7B ;
 ...
 W BUDTELB,!
 W $$REPEAT^XLFSTR("_",80),!
 W "1a",?5,"| Asian" S X=1 D R1B
 D RB
 W "1b1",?5,"| Native Hawaiian" S X=2 D R1B
 D RB
 W "1b2",?5,"| Pacific Islander" S X=3 D R1B
 D RB
 W "1c",?5,"| Black/African" D R2B
 W ?5,"| American" S X=4 D R1B
 D RB
 W "1d",?5,"| American Indian/" D R2B
 W ?5,"| Alaska Native" S X=5 D R1B
 D RB
 W "1e",?5,"| White" S X=6 D R1B
 D RB
 W "1f",?5,"| More than One" D R2B
 W ?5,"| Race" S X=7 D R1B
 D RB
 W "1g",?5,"| Unreported/" D R2B
 W ?5,"| Refused" D R2B
...
P7C ;
 ...
 W BUDTELB,!
 W $$REPEAT^XLFSTR("_",80),!
 W "1a",?5,"| Asian" S X=1 D R1C
 D RC
 W "1b1",?5,"| Native Hawaiian" S X=2 D R1C
 D RC
 W "1b2",?5,"| Pacific Islander" S X=3 D R1C
 D RC
 W "1c",?5,"| Black/African" D R2C
 W ?5,"| American" S X=4 D R1C
 D RC
 W "1d",?5,"| American Indian/" D R2C
 W ?5,"| Alaska Native" S X=5 D R1C
 D RC
 W "1e",?5,"| White" S X=6 D R1C
 D RC
 W "1f",?5,"| More than One" D R2C
 W ?5,"| Race" S X=7 D R1C
 D RC
 W "1g",?5,"| Unreported/" D R2C
 W ?5,"| Refused" D R2C
...
R2C ;EP
 ...
 Q
LISTS ;
 I $G(BUDPRGHL) S BUDGPG=0 D PRGHL^BUD1RP7I
 I $G(BUDPRGRL) S BUDGPG=0 D PRGRL^BUD1RP7I
 ;I $G(BUDPRGEL) S BUDGPG=0 D PRGEL^BUD1RP7I
 I $G(BUDHTRL) S BUDGPG=0 D HTRL^BUD1RP7J
 I $G(BUDHTCRL) S BUDGPG=0 D HTCRL^BUD1RP7J
 I $G(BUDHTURL) S BUDGPG=0 D HTURL^BUD1RP7J
 ;I $G(BUDHTEL) S BUDGPG=0 D HTEL^BUD1RP7K
 ;I $G(BUDHTCEL) S BUDGPG=0 D HTCEL^BUD1RP7K
 ;I $G(BUDHTUEL) S BUDGPG=0 D HTUEL^BUD1RP7K
 I $G(BUDDMRL) S BUDGPG=0 D DMRL^BUD1RP7L
 I $G(BUDDMR1L) S BUDGPG=0 D DMR1L^BUD1RP7L
 I $G(BUDDMR2L) S BUDGPG=0 D DMR2L^BUD1RP7L
 I $G(BUDDMR3L) S BUDGPG=0 D DMR3L^BUD1RP7T
 I $G(BUDDMR4L) S BUDGPG=0 D DMR4L^BUD1RP7T
 ;I $G(BUDDMEL) S BUDGPG=0 D DMEL^BUD1RP7M
 ;I $G(BUDDME1L) S BUDGPG=0 D DME1L^BUD1RP7M
 ;I $G(BUDDME2L) S BUDGPG=0 D DME2L^BUD1RP7M
 ;I $G(BUDDME3L) S BUDGPG=0 D DME3L^BUD1RP7S
 K ^XTMP("BUD1RP7",BUDJ,BUDH)
 Q
 ;
 ;
T7H ;EP
