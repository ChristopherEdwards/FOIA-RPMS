BUD1RPC1 ; IHS/CMI/LAB - UDS TABLE 6 11 Dec 2007 12:15 PM ;
...
PT ;
 ...
 S:C=21.1 C=69
 S:C=21.2 C=70
 I $G(BUDT6L) S ^XTMP("BUD1RPT1",BUDJ,BUDH,"T6",C,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV)=$E(BUDVAL,1,20)
PT1 S $P(BUDT6("V"),U,C)=$P(BUDT6("V"),U,C)+1
 ...
 Q
OT ;
 I $D(^XTMP("BUD1RPT1",BUDJ,BUDH,"T6",C,BUDCOM,BUDAGE,BUDSEX,DFN)) Q  ;patient already a hit on this line
 S ^XTMP("BUD1RPT1",BUDJ,BUDH,"ORPHANS",C,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV)=$E(BUDVAL,1,20)
 Q
T6 ;EP
...
SERV ;
 ...
 ..S C=22 D PT1 S ^XTMP("BUD1RPT1",BUDJ,BUDH,"T6",C,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","MAM",BUDW)=^TMP($J,"MAMMS",BUDW)
...
PAP1 S BUDVAL=$$PAP(BUDV) I BUDVAL]"" S C=23 D PT
 ...
 ..S C=23 D PT1 S ^XTMP("BUD1RPT1",BUDJ,BUDH,"T6",C,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","PAP",BUDW)=^TMP($J,"PAPS",BUDW)
 ...
 S Y=0 S BUDVAL=$$L26^BUD1RPC2(BUDV) I BUDVAL]"" S C=26 D PT
 S BUDVAL=$$L26A^BUD1RPC2(BUDV) I BUDVAL]"" S C="26.1" D PT
 S BUDVAL=$$L26B^BUD1RPC2(BUDV) I BUDVAL]"" S C="26.2" D PT
 S BUDVAL=$$L26C^BUD1RPC2(BUDV) I BUDVAL]"" S C="26.3" D PT
 S BUDVAL=$$L26D^BUD1RPC2(BUDV) I BUDVAL]"" S C="26.4" D PT
 S BUDVAL=$$L27^BUD1RPC2(BUDV) I BUDVAL]"" S C=27 D PT
 S BUDVAL=$$L28^BUD1RPC2(BUDV) I BUDVAL]"" S C=28 D PT
 S BUDVAL=$$L29^BUD1RPC2(BUDV) I BUDVAL]"" S C=29 D PT
 S BUDVAL=$$L30^BUD1RPC2(BUDV) I BUDVAL]"" S C=30 D PT
 S BUDVAL=$$L31^BUD1RPC2(BUDV) I BUDVAL]"" S C=31 D PT
 S BUDVAL=$$L32^BUD1RPC2(BUDV) I BUDVAL]"" S C=32 D PT
 S BUDVAL=$$L33^BUD1RPC2(BUDV) I BUDVAL]"" S C=33 D PT
 S BUDVAL=$$L34^BUD1RPC2(BUDV) I BUDVAL]"" S C=34 D PT
 Q
CONTRA(BUDV) ;
...
PAP(BUDV) ;
 ...
 Q ""
IMM(BUDV) ;
 S T=$O(^ATXAX("B","BUD 11 CPT IMM LINE 24",0))
 S G="" I T S X=0 F  S X=$O(^AUPNVCPT("AD",BUDV,X)) Q:X'=+X!(G]"")  D
 .S Z=$P(^AUPNVCPT(X,0),U)
 .I $$ICD^ATXCHK(Z,T,1) S G=$P($$CPT^ICPTCOD(Z),U,2)
 .Q
 ...
 S G="",X=0 F  S X=$O(^AUPNVIMM("AD",BUDV,X)) Q:X'=+X!(G]"")  D
 .S Z=$P(^AUPNVIMM(X,0),U),Z=+$P(^AUTTIMM(Z,0),U,3)
 .S BUDTIEN=$O(^BUDLCNTL("B","IMMUNIZATIONS LINE 24",0))
 .I $D(^BUDLCNTL(BUDTIEN,11,"B",Z)) S G=Z_" imm"
 .Q
 Q G
HIV(BUDV) ;
