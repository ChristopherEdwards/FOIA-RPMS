BUD2RP6B ; IHS/CMI/LAB - UDS REPORT DRIVER TABLE 6B ;
...
EN2 ;
 ...
 S BUDTAXT="B6" D TAXCHK^BUD2XTCH
 D YEAR
 I BUDYEAR="" D EOJ Q
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BUD2RP6B",ZTDTH="",ZTDESC="UDS 12 REPORT TABLE 6B" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D EN^XBVK("BUD")
 Q
...
M ;EP - called from option
 ...
 S X=0 F  S X=$O(BUDIND(X)) Q:X'=+X  X ^BUDRLST2(X,1)
 G EN1
PROC ;EP - called from taskman
 S BUDJ=$J,BUDH=$H
 S ^XTMP("BUD2RP6B",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^BUD TABLE 6B LISTS"
 ;NOW LOOP PATS
...
PRGALST ;EP - list of pregnant females
 ...
 S X=$$AGB(BUDAGEP)
 S ^XTMP("BUD2RP6B",BUDJ,BUDH,"PRGA",X,BUDAGEP,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDP,"*",2)
 Q
AGB(N) ;
...
PRINT ;EP - called from taskman
 ...
 D HEADER^BUD2RPTP Q:BUDQUIT  D T6BH
 ...
 D LINE1
 I $Y>(IOSL-3) D HEADER^BUD2RPTP Q:BUDQUIT  D T6BH
 ...
 I $Y>(IOSL-12) D HEADER^BUD2RPTP Q:BUDQUIT  D T6BH
 ...
 D LINE
 I $Y>(IOSL-20) D HEADER^BUD2RPTP Q:BUDQUIT  D T6BH
...
GENI ;general introductions
 ...
 W !,"December 31, 2012.  Based on the UDS defintion, to be considered a"
