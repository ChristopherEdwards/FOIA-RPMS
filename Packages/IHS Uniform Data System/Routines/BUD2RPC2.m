BUD2RPC2 ; IHS/CMI/LAB - UDS TABLE 6 11 Dec 2007 12:15 PM ;
...
L26A(BUDV) ;EP
 ;age 9-72 months
 S G=""
 S A=$$AGE^BUD2UTL2(DFN,2,$$VD^APCLV(BUDV))
 I A<9 Q G
 I A>72 Q G
 S G="" S X=0 F  S X=$O(^AUPNVCPT("AD",BUDV,X)) Q:X'=+X!(G]"")  D
 .S Z=$$VAL^XBDIQ1(9000010.18,X,.01)
...
SHI(V,LV) ;
 ...
 I $G(BUDT4IPP) D
 .S ^XTMP("BUD2RPT1",BUDJ,BUDH,"T4IPPL",$G(V),$G(LV),BUDCOM,BUDSEX,BUDAGE,DFN)=$G(LV)
 Q
HI ;
...
GUAR(P,BD,ED) ;guarantor
 ...
 Q A
 ;
WC(P,BD,ED) ;workman's comp goes in line 11 per Duane
 NEW X,Y,Z,SD
 ...
 Q Y
 ;
TPL(P,BD,ED) ;workman's comp goes in line 11 per Duane
 NEW X,Y,Z,SD
 ;find an injury in date range
 S SD=$$FMADD^XLFDT(BD,-1)
 S Y=0  ;no wc
...
TINS(A,P) ;
 ...
 .S ^XTMP("BUD2RPT1",BUDJ,BUDH,"T4PMIS",$G(P),$G(T),BUDAGE,BUDCOM,BUDSEX,DFN)=""
 Q
 ;
...
MIGRANT ;
 ...
 .Q:'$G(BUDT4CHA)
 .S T="Migratory" S ^XTMP("BUD2RPT1",BUDJ,BUDH,"T4CHAR",14,T,BUDCOM,BUDSEX,BUDAGE,DFN)=""
 I $P(M,U,1)="S" S BUDT4V(15)=BUDT4V(15)+1,BUDT4V(16)=BUDT4V(16)+1 D
 .Q:'$G(BUDT4CHA)
 .S T="Seasonal"
 .S ^XTMP("BUD2RPT1",BUDJ,BUDH,"T4CHAR",15,T,BUDCOM,BUDSEX,BUDAGE,DFN)=""
 Q
HOMELESS ;
 ...
 S ^XTMP("BUD2RPT1",BUDJ,BUDH,"T4CHAR",L,T,BUDCOM,BUDSEX,BUDAGE,DFN)=""
 Q
VET ;
 S V=$$VALI^XBDIQ1(2,DFN,1901)
 I V="Y" S T="Veteran" S BUDT4V(25)=BUDT4V(25)+1,^XTMP("BUD2RPT1",BUDJ,BUDH,"T4CHAR",25,T,BUDCOM,BUDSEX,BUDAGE,DFN)=""
 Q
SCHOOL ;IF ALL VISITS ARE CLINIC SCHOOL??
 ...
 S ^XTMP("BUD2RPT1",BUDJ,BUDH,"T4CHAR",24,"School Based Patient",BUDCOM,BUDSEX,BUDAGE,DFN)=""
 Q
MIG(P,D) ;EP
