BUD2RPTD ; IHS/CMI/LAB - UDS REPORT PROCESSOR 05 Oct 2012 5:03 PM ; 
...
GETV ;EP - get all visits for this patient and tally in BUDTV
 ...
 .Q:BUDVLOC=""
 .Q:'$D(^BUDRSITE(BUDSITE,11,BUDVLOC))  ;not valid location
 .Q:"AHSORMEI"'[$P(^AUPNVSIT(BUDVSIT,0),U,7)
 .S BUDCLINC=$$CLINIC^APCLV(BUDVSIT,"C")
 .S BUDTIEN=$O(^BUDRCNTL("B","FIRST LEVEL CLINIC EXCLUSIONS",0))
 .I BUDCLINC]"",$D(^BUDRCNTL(BUDTIEN,11,"B",BUDCLINC)) Q  ;not a clinic code we want in any table
...
MEDC .;NOW CHECK FOR MEDICAL CARE, CAN ONLY HAVE 1 PER LOCATION OF ENCOUNTER
 .S S=0
 .S BUDTIEN=$O(^BUDRCNTL("B","MEDICAL CARE LINE NUMBERS",0))
 .;S BUDPP=$$PRIMPROV^APCLV(BUDVSIT,"D")
 .I $E($$VAL^XBDIQ1(9000010,BUDVSIT,.06),1,3)="CHS",BUDPP=15 S BUDLINE=2 G MEDC1
 .S BUDY=$O(^BUDRTFIV("C",BUDPP,0)) I BUDY="" S BUDLINE=35 G MEDC1
 .S BUDLINE=$O(^BUDRTFIV("AA",BUDPP,""))
MEDC1 .S S=0
 .I $D(^BUDRCNTL(BUDTIEN,11,"B",BUDLINE)) D
...
DENT .;NOW CHECK FOR DENTAL CARE
 .S S=0
 .S BUDTIEN=$O(^BUDRCNTL("B","DENTAL LINE NUMBERS",0))
 .S S=0
 .I $D(^BUDRCNTL(BUDTIEN,11,"B",BUDLINE)) D
...
MH .;NOW CHECK FOR MH CARE
 .S S=0
 .S BUDTIEN=$O(^BUDRCNTL("B","MENTAL HEALTH LINE NUMBERS",0))
...
SUB .;
 .S S=0
 .S BUDTIEN=$O(^BUDRCNTL("B","SUBSTANCE LINE NUMBERS",0))
 .S S=0
 .I $D(^BUDRCNTL(BUDTIEN,11,"B",BUDLINE)) D
...
VISION .;
 .S S=0
 .S BUDTIEN=$O(^BUDRCNTL("B","VISION LINE NUMBERS",0))
 .S S=0
 .I $D(^BUDRCNTL(BUDTIEN,11,"B",BUDLINE)) D
...
OTH .;can have only 1 in each category
 .S BUDTIEN=$O(^BUDRTFIV("B",22,0))
 .S S=0
 .I $D(^BUDRTFIV(BUDTIEN,11,"B",BUDPP)) D
...
ENAB .;NOW CHECK FOR ENABLING
 .S S=0
 .S BUDTIEN=$O(^BUDRCNTL("B","ENABLING LINE NUMBERS",0))
 .S S=0
 .I $D(^BUDRCNTL(BUDTIEN,11,"B",BUDLINE)) D
...
SET .S BUDTV=BUDTV+1
 ...
 .S BUDTIEN=$O(^BUDRCNTL("B","CLINIC EXCLUSIONS",0))
 .I BUDCLINC]"",$D(^BUDRCNTL(BUDTIEN,11,"B",BUDCLINC)) Q  ;exclude these clinics
...
WDEL ;EP - write out delimited file 9d
 ...
 S ^BUDDATA(1)=X
 S D=0,C=1 F  S D=$O(^XTMP("BUD2RP9DEL",BUDJ,BUDH,D)) Q:D'=+D  D
 .S X=0 F  S X=$O(^XTMP("BUD2RP9DEL",BUDJ,BUDH,D,X)) Q:X'=+X  D
 ..S C=C+1 S ^BUDDATA(C)=^XTMP("BUD2RP9DEL",BUDJ,BUDH,D,X)
