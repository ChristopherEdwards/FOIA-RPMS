BUD2RPTL ; IHS/CMI/LAB - UDS print lists 12 Dec 2007 12:12 PM 02 Dec 2012 10:43 AM ;
...
T3A ;
 S BUDP=0
 D T3H Q:BUDQUIT
 S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"3A",BUDAGE)) Q:BUDAGE'=+BUDAGE!(BUDQUIT)  D
 .S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ...S DFN="" F  S DFN=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ...
 ....S BUDV=0 F  S BUDV=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCOM,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  D
 .....I $Y>(IOSL-3) D T3H Q:BUDQUIT
...
T5 ;
 S BUDP=0
 S BUD2L="" F   S BUD2L=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"T5",BUD2L)) Q:BUD2L=""!(BUDQUIT)  D
 .Q:BUD2L=35  ;NO UNCATEGORIZED
 .S BUD2L2="" F  S BUD2L2=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"T5",BUD2L,BUD2L2)) Q:BUD2L2=""!(BUDQUIT)  D
 ..S BUD2LL=BUD2L_$S(BUD2L2=0:"",1:BUD2L2)
 ..S BUDY=$O(^BUDRTFIV("B",BUD2LL,0)),BUDY=$P(^BUDRTFIV(BUDY,0),U,2)_" "_$P(^BUDRTFIV(BUDY,0),U,3)_" "_$P(^BUDRTFIV(BUDY,0),U,4)
 ..S BUDSUBT="Line "_BUD2LL_"   "_BUDY
 ..D T5H Q:BUDQUIT
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"T5",BUD2L,BUD2L2,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ...S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"T5",BUD2L,BUD2L2,BUDCOM,BUDAGE)) Q:BUDAGE=""!(BUDQUIT)  D
 ....S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"T5",BUD2L,BUD2L2,BUDCOM,BUDAGE,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 .....S DFN=0 F  S DFN=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"T5",BUD2L,BUD2L2,BUDCOM,BUDAGE,BUDSEX,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D T5W
 .....Q
 ...Q
 ..Q
 .Q
...
T5W W !,$E($P(^DPT(DFN,0),U,1),1,22),?24,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2))
 W ?36,$E(BUDCOM,1,12),?51,$P(^DPT(DFN,0),U,2),?55,$$AGE^AUPNPAT(DFN,BUDCAD),?60,$E($P($$RACE^BUD2RPTC(DFN),U,4),1,16)," (",$P($$RACE^BUD2RPTC(DFN),U,3),")"
 K BUDVLST S BUDV=0 F  S BUDV=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"T5",BUD2L,BUD2L2,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  D
...
TZ ;
 S BUDP=0
 D TZH Q:BUDQUIT
 S BUDYY=0 F  S BUDYY=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"ZNEW",BUDYY)) Q:BUDYY'=+BUDYY!(BUDQUIT)  D TZ1
 Q:BUDQUIT
 ;now list unknown residence
 S BUDYY="Unknown Residence" D TZ2
 Q
TZ2 ;
 ;S BUDZIP="" F  S BUDZIP=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"Z",BUDYY,BUDZIP)) Q:BUDZIP=""!(BUDQUIT)  D
 D
 .S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"Z",BUDYY,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ..S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"Z",BUDYY,BUDCOM,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 ...S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"Z",BUDYY,BUDCOM,BUDSEX,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ....S DFN="" F  S DFN=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"Z",BUDYY,BUDCOM,BUDSEX,BUDNAME,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 .....I $Y>(IOSL-3) D TZH Q:BUDQUIT
 .....W !,$E($P(^DPT(DFN,0),U,1),1,22),?24,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2)),?36,$E(BUDCOM,1,12),?51,$P(^DPT(DFN,0),U,2),?55,BUDYY
 .....S BUDV=0 F  S BUDV=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"Z",BUDYY,BUDCOM,BUDSEX,BUDNAME,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  D
 ......I $Y>(IOSL-3) D TZH Q:BUDQUIT
 ......W !?5,$$FMTE^XLFDT($P($P(^AUPNVSIT(BUDV,0),U),".")),?25,$E($$PRIMPROV^APCLV(BUDV,"E"),1,14),?42,$P(^AUPNVSIT(BUDV,0),U,7),?45,$E($$CLINIC^APCLV(BUDV,"E"),1,14),?62,$E($$LOCENC^APCLV(BUDV,"E"),1,14)
 W !
 Q
TZ1 ;
 S BUDZIP="" F  S BUDZIP=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"ZNEW",BUDYY,BUDZIP)) Q:BUDZIP=""!(BUDQUIT)  D
 .S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"ZNEW",BUDYY,BUDZIP,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ..S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"ZNEW",BUDYY,BUDZIP,BUDCOM,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 ...S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"ZNEW",BUDYY,BUDZIP,BUDCOM,BUDSEX,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ....S DFN="" F  S DFN=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"ZNEW",BUDYY,BUDZIP,BUDCOM,BUDSEX,BUDNAME,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 .....I $Y>(IOSL-3) D TZH Q:BUDQUIT
 ...
 .....S BUDV=0 F  S BUDV=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"ZNEW",BUDYY,BUDZIP,BUDCOM,BUDSEX,BUDNAME,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  D
 ......I $Y>(IOSL-3) D TZH Q:BUDQUIT
 ......W !?5,$$FMTE^XLFDT($P($P(^AUPNVSIT(BUDV,0),U),".")),?25,$E($$PRIMPROV^APCLV(BUDV,"E"),1,14),?42,$P(^AUPNVSIT(BUDV,0),U,7),?45,$E($$CLINIC^APCLV(BUDV,"E"),1,14),?62,$E($$LOCENC^APCLV(BUDV,"E"),1,14)
 W !
 Q
...
T6 ;
 S BUDP=0
 F BUD2L=1,2,3,4,67,68,5:1:14,60,15:1:19,61,40:1:43,21,69,70,22,23,24,65,66,25,26,62,63,64,71,27:1:34 D
 .Q:'$D(^XTMP("BUD2RPT1",BUDJ,BUDH,"T6",BUD2L))
 .D T6H Q:BUDQUIT
 .W !!,"Line ",$S($P($T(@BUD2L),";;",3)]"":$P($T(@BUD2L),";;",3),1:BUD2L),"   ",$P($T(@BUD2L),";;",2)
 .S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"T6",BUD2L,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ..S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"T6",BUD2L,BUDCOM,BUDAGE)) Q:BUDAGE=""!(BUDQUIT)  D
 ...S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"T6",BUD2L,BUDCOM,BUDAGE,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 ....S DFN=0 F  S DFN=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"T6",BUD2L,BUDCOM,BUDAGE,BUDSEX,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D T6W
 ....Q
 ...Q
 ..Q
 .Q
 ...
 Q
T6W ;
 I $Y>(IOSL-3) D T6H Q:BUDQUIT  W !!,"Line ",$S($P($T(@BUD2L),";;",3)]"":$P($T(@BUD2L),";;",3),1:BUD2L),"   ",$P($T(@BUD2L),";;",2)
 W !,$E($P(^DPT(DFN,0),U,1),1,22),?24,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2))
 W ?36,$E(BUDCOM,1,12),?51,$P(^DPT(DFN,0),U,2),?55,$$AGE^AUPNPAT(DFN,BUDCAD),?60,$E($P($$RACE^BUD2RPTC(DFN),U,4),1,16)," (",$P($$RACE^BUD2RPTC(DFN),U,3),")"
 K BUDVRR S BUDV=0,BUDVC=0 F  S BUDV=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"T6",BUD2L,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  S BUDVC=BUDVC+1,BUDVRR($$VD^APCLV(BUDV),BUDVC)=BUDV
 S BUDVD=0 F  S BUDVD=$O(BUDVRR(BUDVD)) Q:BUDVD=""  S BUDVC=0 F  S BUDVC=$O(BUDVRR(BUDVD,BUDVC)) Q:BUDVC=""  D
 .I $Y>(IOSL-3) D T6H Q:BUDQUIT  W !!,"Line ",$S($P($T(@BUD2L),";;",3)]"":$P($T(@BUD2L),";;",3),1:BUD2L),"   ",$P($T(@BUD2L),";;",2)
 .S BUDV=BUDVRR(BUDVD,BUDVC)
 .S Z=^XTMP("BUD2RPT1",BUDJ,BUDH,"T6",BUD2L,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV)
 ...
 .Q
 I BUD2L=22 D
 .S BUDW=0 F  S BUDW=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"T6",BUD2L,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","MAM",BUDW)) Q:BUDW'=+BUDW!(BUDQUIT)  D
 ..W !?5,$P(^XTMP("BUD2RPT1",BUDJ,BUDH,"T6",BUD2L,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","MAM",BUDW),U,2),?25,$P(^(BUDW),U,1)
 I BUD2L=23 D
 .S BUDW=0 F  S BUDW=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"T6",BUD2L,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","PAP",BUDW)) Q:BUDW'=+BUDW!(BUDQUIT)  D
 ..W !?5,$P(^XTMP("BUD2RPT1",BUDJ,BUDH,"T6",BUD2L,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","PAP",BUDW),U,2),?25,$P(^(BUDW),U,1)
 Q
T6H ;
 G:'BUDGPG T6H1
 K DIR I $E(IOST)="C",IO=IO(0),'$D(ZTQUEUED) W ! S DIR(0)="EO" D ^DIR K DIR I Y=0!(Y="^")!($D(DTOUT)) S BUDQUIT=1 Q
