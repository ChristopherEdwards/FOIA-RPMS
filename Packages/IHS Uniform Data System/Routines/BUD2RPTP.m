BUD2RPTP ; IHS/CMI/LAB - UDS REPORT PRINT 30 Dec 2012 10:44 AM 22 Oct 2012 12:16 PM ; 
...
PRINT ;EP
 ...
 D PATLISTS
 D ^%ZISC
 I $G(BUDT9D) D WDEL^BUD2RPTD
 D EOJ
 Q
 ;
EOJ ;
 K ^TMP($J)
 K ^XTMP("BUD2RPT1",BUDJ,BUDH)
 K ^XTMP("BUD2RPT9DEL",BUDJ,BUDH)
 Q
T9 ;
 D T9^BUD2RPP2
 Q
T4 ;
 ...
 Q
T3A ;
 I $G(BUDSTMP) D STEMP^BUD2RPTS
 S BUDPG=0
 D HEADER Q:BUDQUIT  D T3ASH
 ;print out each line
 S BUDZ=0 F  S BUDZ=$O(^BUDRTTA("AC",BUDZ)) Q:BUDZ>38!(BUDQUIT)  D
 .S BUDC=$O(^BUDRTTA("AC",BUDZ,0))
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T3ASH
 .S BUDY=^BUDRTTA(BUDC,0)
 ...
 S BUDC=$O(^BUDRTTA("B",43,0))
 I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T3ASH
 W !,$P(^BUDRTTA(BUDC,0),U,2),?5,$P(^BUDRTTA(BUDC,0),U,3)
 S BUDC=$O(^BUDRTTA("AC",39,0))
 ...
 Q
T3ASH ;
 W !,$$CTR($G(^BUDRTTA(1,11)),80),!
 F Y=2:1:4 S X=$O(^BUDRTTA("B",Y,0)) W !,$P(^BUDRTTA(X,0),U,2),?5,$P(^BUDRTTA(X,0),U,3),?33,$P(^BUDRTTA(X,0),U,4),?50,$P(^BUDRTTA(X,0),U,5),?68,$P(^BUDRTTA(X,0),U,6)
 W !,$TR($J("",80)," ","-")
 Q
T3BSH ;
 W !!,$$CTR("TABLE 3B -",80)
...
TZ ;
 ...
 ;print out each line
 S BUDZTOT=0
 ;S ^XTMP("BUD2RPT1",BUDJ,BUDH,"ZIP","OTHER ZIP CODES")=0
 ;GET TOTAL
 S BUDZ="" F  S BUDZ=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"ZIP",BUDZ)) Q:BUDZ=""  D
 .S BUDY=^XTMP("BUD2RPT1",BUDJ,BUDH,"ZIP",BUDZ)
 ...
 .;S X=BUDY/BUDZTOT I X<.001 K ^XTMP("BUD2RPT1",BUDJ,BUDH,"ZIP",BUDZ) S ^XTMP("BUD2RPT1",BUDJ,BUDH,"ZIP","OTHER ZIP CODES")=^XTMP("BUD2RPT1",BUDJ,BUDH,"ZIP","OTHER ZIP CODES")+BUDY D
 .S ^XTMP("BUD2RPT1",BUDJ,BUDH,"ZIPORDER",(9999999-BUDY),BUDZ)=BUDY
 .D
 ..;now reset the list so the others collate at the end
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"Z",BUDZ,BUDCOM)) Q:BUDCOM=""  D
 ...S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"Z",BUDZ,BUDCOM,BUDSEX)) Q:BUDSEX=""  D
 ....S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"Z",BUDZ,BUDCOM,BUDSEX,BUDNAME)) Q:BUDNAME=""  D
 .....S DFN=0 F  S DFN=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"Z",BUDZ,BUDCOM,BUDSEX,BUDNAME,DFN)) Q:DFN'=+DFN  D
 ......S X=0 F  S X=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"Z",BUDZ,BUDCOM,BUDSEX,BUDNAME,DFN,X)) Q:X'=+X  D
 .......S ^XTMP("BUD2RPT1",BUDJ,BUDH,"ZNEW",BUDYY,BUDZ,BUDCOM,BUDSEX,BUDNAME,DFN,X)=""
 .......K ^XTMP("BUD2RPT1",BUDJ,BUDH,"Z",BUDZ,BUDCOM,BUDSEX,BUDNAME,DFN,X)
 S BUDYY=0 F  S BUDYY=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"ZIPORDER",BUDYY)) Q:BUDYY=""  D
 .S BUDZ="" F  S BUDZ=$O(^XTMP("BUD2RPT1",BUDJ,BUDH,"ZIPORDER",BUDYY,BUDZ)) Q:BUDZ=""!(BUDQUIT)  D
 ..I $Y>(IOSL-4) D HEADER Q:BUDQUIT  D TZSH
 ..S BUDY=^XTMP("BUD2RPT1",BUDJ,BUDH,"ZIP",BUDZ)
 ..W !?8,BUDZ,?40,$$C(BUDY),!,BUD80L
 Q:BUDQUIT
 W !?8,"Unknown Residence",?40,$$C($G(^XTMP("BUD2RPT1",BUDJ,BUDH,"ZIP","Unknown Residence")))
 W !!?8,"TOTAL",?40,$$C(BUDZTOT),!
 Q
T5 ;print table 5
 S BUDPG=0 D HEADER Q:BUDQUIT  D T5SH
 ;lines 1-8
 F BUDL=1:1:7 S BUDY=$O(^BUDRTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .Q:'BUDY
 .W !
 .I +BUDL<10 W " "
 .W BUDL,".",?6,$P(^BUDRTFIV(BUDY,0),U,2)
 .I $P(^BUDRTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,3)
 ...
 F BUDL="9A","9B",10 S BUDY=$O(^BUDRTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !
 .I +BUDL<10 W " "
 .W $$LOW^XLFSTR(BUDL),".",?6,$P(^BUDRTFIV(BUDY,0),U,2)
 .I $P(^BUDRTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,3)
 ...
 F BUDL=11:1:14 S BUDY=$O(^BUDRTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDRTFIV(BUDY,0),U,2) I $P(^BUDRTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,3)
 ...
 F BUDL=16:1:17 S BUDY=$O(^BUDRTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDRTFIV(BUDY,0),U,2) I $P(^BUDRTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,3)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),0)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),2)
 .W !,BUD80L
T518 F BUDL=18 S BUDY=$O(^BUDRTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDRTFIV(BUDY,0),U,2) I $P(^BUDRTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,3)
 ...
 F BUDL="20A","20A1","20A2","20B","20C" S BUDY=$O(^BUDRTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDRTFIV(BUDY,0),U,2)
 .I $P(^BUDRTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,3)
 .I $P(^BUDRTFIV(BUDY,0),U,4)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,4)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),0)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),2)
 .W !,BUD80L
 I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 W !,"20.",?10,"Mental Health Services",!?10,"(Lines 20a - c)",?60,$$C($P(BUDTAB5(20),U,1)),?71,$$C($P(BUDTAB5(20),U,2)),!,BUD80L
 F BUDL=21 S BUDY=$O(^BUDRTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDRTFIV(BUDY,0),U,2) I $P(^BUDRTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,3)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),0)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),0)
 .W !,BUD80L
 F BUDL=22 S BUDY=$O(^BUDRTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?8,$P(^BUDRTFIV(BUDY,0),U,2) I $P(^BUDRTFIV(BUDY,0),U,3)]"" W !?8,$P(^BUDRTFIV(BUDY,0),U,3)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),0)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),0)
 .W !,BUD80L
 F BUDL="22A","22B","22C" S BUDY=$O(^BUDRTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDRTFIV(BUDY,0),U,2)
 .I $P(^BUDRTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,3)
 .I $P(^BUDRTFIV(BUDY,0),U,4)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,4)
 ...
 F BUDL=23 S BUDY=$O(^BUDRTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDRTFIV(BUDY,0),U,2) I $P(^BUDRTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,3)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),2)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),2)
 .W !,BUD80L
 F BUDL=24:1:25 S BUDY=$O(^BUDRTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDRTFIV(BUDY,0),U,2) I $P(^BUDRTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,3)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),0)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),2)
 .W !,BUD80L
 F BUDL=26,27,"27a","27b",28 S BUDY=$O(^BUDRTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDRTFIV(BUDY,0),U,2) I $P(^BUDRTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,3)
 ...
 F BUDL="30A","30B","30C" S BUDY=$O(^BUDRTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDRTFIV(BUDY,0),U,2) I $P(^BUDRTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,3)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),2)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),2)
 ...
 I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
  F BUDL=31,32 S BUDY=$O(^BUDRTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDRTFIV(BUDY,0),U,2) I $P(^BUDRTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDRTFIV(BUDY,0),U,3)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),2)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),2)
...
HEADER2 S BUDPG=BUDPG+1,BUDTPG=$G(BUDTPG)+1
 S X=$$CTR($P(^DIC(4,BUDSITE,0),U),60),$E(X,3)=$P(^VA(200,DUZ,0),U,2),$E(X,10)="UDS 2012",$E(X,70)="Page ",$E(X,75)=BUDPG W !,X
 ;W !?3,$P(^VA(200,DUZ,0),U,2),?10,"UDS 2012",$$CTR($P(^DIC(4,BUDSITE,0),U),80),?70,"Page ",BUDPG
