BUDAENV ; IHS/CMI/LAB - environmental check 13 Jan 2013 7:22 AM ;
...
ENV ;
 ...
 I $$VERSION^XPDUTL("BGP")'>"12.9" D SORRY(2)
 Q
 ;
 ;
PRE ;
 ;ADD ALL OTHERS BACK IN
 F DA=1:1:50 S DIK="^BUDQCNTL(" D ^DIK
 F DA=1:1:50 S DIK="^BUDQIL(" D ^DIK
 F DA=1:1:50 S DIK="^BUDQTFIV(" D ^DIK
 F DA=1:1:50 S DIK="^BUDQTTA(" D ^DIK
 F DA=1:1:50 S DIK="^BUDQLST2(" D ^DIK
 Q
POST ;
 ;move site parameters from 04 to 05 on first time install only
 I '$O(^BUDQSITE(0)) D
 .S BUDX=0 F  S BUDX=$O(^BUDRSITE(BUDX)) Q:BUDX'=+BUDX   D
 ..M ^BUDQSITE(BUDX)=^BUDRSITE(BUDX)
 ..S DA=BUDX,DIK="^BUDQSITE(" D IX1^DIK
 D ^BUDAX
...
INC ;UPDATE INCOME LEVELS
 S X=8 F  S X=$O(^BUDQIL("B",X)) Q:X'=+X  D
 .S Y=$O(^BUDQIL("B",X,0))
 .S C=X-8
 .S $P(^BUDQIL(Y,0),U,2)=(C*4020)+39630
 .S $P(^BUDQIL(Y,0),U,3)=(C*5030)+49560
 .S $P(^BUDQIL(Y,0),U,4)=(C*4620)+45570
 Q
