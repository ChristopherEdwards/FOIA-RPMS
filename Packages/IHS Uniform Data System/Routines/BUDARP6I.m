BUDARP6I ; IHS/CMI/LAB - UDS REPORT DRIVER TABLE 6B ; 14 Jan 2014  2:27 PM
...
LISTS ;EP
 ...
 I $G(BUDPAP1L) S BUDGPG=0 D PAP1L^BUDARP6P
 I $G(BUDPAP2L) S BUDGPG=0 D PAP2L^BUDARP6P
 I $G(BUDWAC1L) S BUDGPG=0 D WAC1L^BUDARP6F
 I $G(BUDWAC2L) S BUDGPG=0 D WAC2L^BUDARP6F
 I $G(BUDAWS1L) S BUDGPG=0 D AWS1L^BUDARP6F
 I $G(BUDAWS2L) S BUDGPG=0 D AWS2L^BUDARP6G
 I $G(BUDTUA1L) S BUDGPG=0 D TUA1L^BUDARP6T
 I $G(BUDTUA2L) S BUDGPG=0 D TUA2L^BUDARP6T
 I $G(BUDTCI1L) S BUDGPG=0 D TCI1L^BUDARP6T
 I $G(BUDTCI2L) S BUDGPG=0 D TCI2L^BUDARP6T
 I $G(BUDAPT1L) S BUDGPG=0 D APT1L^BUDARP6S
 I $G(BUDAPT2L) S BUDGPG=0 D APT2L^BUDARP6S
 I $G(BUDCAD1L) S BUDGPG=0 D CAD1L^BUDARP6J
 I $G(BUDCAD2L) S BUDGPG=0 D CAD2L^BUDARP6J
 I $G(BUDIVD1L) S BUDGPG=0 D IVD1L^BUDARP6K
 I $G(BUDIVD2L) S BUDGPG=0 D IVD2L^BUDARP6K
 I $G(BUDCRC1L) S BUDGPG=0 D CRC1L^BUDARP6L
 I $G(BUDCRC2L) S BUDGPG=0 D CRC2L^BUDARP6L
 Q
PAUSE ;
...
PRGAL ;EP
 ...
 D PRGAH Q:BUDQUIT
 I '$D(^XTMP("BUDARP6B",BUDJ,BUDH,"PRGA")) W !!,"No patients to report." Q
...
PRGAL1 ;
 ...
 W !,BUDAB,!
 S BUDSTOT=0
 S BUDA=0 F  S BUDA=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"PRGA",BUDAB,BUDA)) Q:BUDA'=+BUDA!(BUDQUIT)  D
 .S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"PRGA",BUDAB,BUDA,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"PRGA",BUDAB,BUDA,BUDNAME,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ...S DFN=0 F  S DFN=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"PRGA",BUDAB,BUDA,BUDNAME,BUDCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ...
 ....S BUDALL=^XTMP("BUDARP6B",BUDJ,BUDH,"PRGA",BUDAB,BUDA,BUDNAME,BUDCOM,DFN)
...
IMM1L ;EP
 S BUDP=0,BUDQUIT=0,BUDTOT=0
 D IMM1H Q:BUDQUIT
 I '$D(^XTMP("BUDARP6B",BUDJ,BUDH,"IMM1")) W !!,"No patients to report.",! Q
 D IMM1L1
...
IMM1L1 ;
 ...
 S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"IMM1",BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 .S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"IMM1",BUDNAME,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ..S DFN=0 F  S DFN=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"IMM1",BUDNAME,BUDCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ...I $Y>(IOSL-3) D IMM1H Q:BUDQUIT
 ...
 ...S BUDTOT=BUDTOT+1
 ...S BUDALL=$P(^XTMP("BUDARP6B",BUDJ,BUDH,"IMM1",BUDNAME,BUDCOM,DFN),"|||",1)
 ...F BUDX=1:1 S BUDV=$P(BUDALL,U,BUDX) Q:BUDV=""!(BUDQUIT)  D
 ....I $Y>(IOSL-3) D IMM1H Q:BUDQUIT
 ....W ?4,BUDV,!
 Q
...
IMM2L ;EP
 S BUDP=0,BUDQUIT=0,BUDTOT=0
 D IMM2H Q:BUDQUIT
 I '$D(^XTMP("BUDARP6B",BUDJ,BUDH,"IMM2")) W !!,"No patients to report.",! Q
 D IMM2L1
...
IMM2L1 ;
 ...
 S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"IMM2",BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 .S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"IMM2",BUDNAME,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ..S DFN=0 F  S DFN=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"IMM2",BUDNAME,BUDCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ...I $Y>(IOSL-3) D IMM2H Q:BUDQUIT
 ...W $E($P(^DPT(DFN,0),U,1),1,25),?29,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2)),?41,$E(BUDCOM,1,25),?70,$P(^DPT(DFN,0),U,2),?75,$$AGE^AUPNPAT(DFN,BUDED),!
 ...S BUDTOT=BUDTOT+1
 ...S BUDALL=^XTMP("BUDARP6B",BUDJ,BUDH,"IMM2",BUDNAME,BUDCOM,DFN)
 ...F BUDX=1:1 S BUDV=$P(BUDALL,U,BUDX) Q:BUDV=""!(BUDQUIT)  D
 ....I $Y>(IOSL-3) D IMM2H Q:BUDQUIT
 ....W ?4,BUDV,!
 Q
