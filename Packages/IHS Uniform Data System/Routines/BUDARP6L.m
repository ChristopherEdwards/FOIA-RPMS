BUDARP6L ; IHS/CMI/LAB - UDS REPORT DRIVER TABLE 6B 30 Dec 2013 8:09 PM 14 Dec 2013 1:24 PM ; 
...
CRC1L ;EP
 ...
 D CRC1H Q:BUDQUIT
 I '$D(^XTMP("BUDARP6B",BUDJ,BUDH,"CRC1")) W !!,"No patients to report.",! Q
 D CRC1L1
...
CRC1L1 ;
 ...
 S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"CRC1",BUDAGE)) Q:BUDAGE=""!(BUDQUIT)  D
 .S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"CRC1",BUDAGE,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"CRC1",BUDAGE,BUDNAME,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ...S DFN=0 F  S DFN=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"CRC1",BUDAGE,BUDNAME,BUDCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ....I $Y>(IOSL-3) D CRC1H Q:BUDQUIT
 ...
 ....S BUDTOT=BUDTOT+1
 ....S BUDALL=^XTMP("BUDARP6B",BUDJ,BUDH,"CRC1",BUDAGE,BUDNAME,BUDCOM,DFN)
...
CRC2L ;EP
 S BUDP=0,BUDQUIT=0,BUDTOT=0
 D CRC2H Q:BUDQUIT
 I '$D(^XTMP("BUDARP6B",BUDJ,BUDH,"CRC2")) W !!,"No patients to report.",! Q
 D CRC2L1
...
CRC2L1 ;
 ...
 S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"CRC2",BUDAGE)) Q:BUDAGE=""!(BUDQUIT)  D
 .S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"CRC2",BUDAGE,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"CRC2",BUDAGE,BUDNAME,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ...S DFN=0 F  S DFN=$O(^XTMP("BUDARP6B",BUDJ,BUDH,"CRC2",BUDAGE,BUDNAME,BUDCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ....I $Y>(IOSL-3) D CRC2H Q:BUDQUIT
 ....W !,$E($P(^DPT(DFN,0),U,1),1,25),?29,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2)),?41,$E(BUDCOM,1,25),?70,$P(^DPT(DFN,0),U,2),?75,BUDAGE,!
 ....S BUDTOT=BUDTOT+1
 ....S BUDALL=^XTMP("BUDARP6B",BUDJ,BUDH,"CRC2",BUDAGE,BUDNAME,BUDCOM,DFN)
 ....W ?5,$P(BUDALL,U,1),?30,$P(BUDALL,U,2)
 Q
CRC2H ;
 G:'BUDGPG CRC2H1
