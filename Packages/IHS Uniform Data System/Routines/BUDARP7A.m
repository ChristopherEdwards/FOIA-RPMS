BUDARP7A ; IHS/CMI/LAB - UDS REPORT PROCESSOR 01 Dec 2013 5:11 PM ;
...
PRGHLST ;EP - list of pregnant females
 ...
 S BUDRACEX=$$RACE^BUDARPTC(DFN),BUDRACE=$P(BUDRACEX,U,2),BUDRACEP=$P(BUDRACEX,U,5)
 S BUDRACEE=$$RACE^BUDARP7I(BUDRACE)
 S BUDR=""
 S BUDETHN=$P($$HISP^BUDARPTC(DFN),U,1)
...
SETSECTH ;
 ...
 S ^XTMP("BUDARP7",BUDJ,BUDH,"PRGH",BUDAGEP,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDP,"*",2)_"#"_$P(BUDHIV,"*",2)
 Q
 ;
...
PRGRLST ;EP - list of pregnant females
 ...
 S BUDP=$$PREG(DFN,$$FMADD^XLFDT(BUDED,-609),BUDED)
 I '$P(BUDP,U) Q  ;not pregnant
 S BUDHISPN=$$HISP^BUDARPTC(DFN)
 S BUDHISP=$P($$HISP^BUDARPTC(DFN),U,1)  ;1=hispanic 2=non hispanic
 ;S BUDHISP1=BUDHISP+2  ;set piece
 ;
 S BUDR1=$$RACE^BUDARPTC(DFN)
 S BUDR=$P(BUDR1,U,1)  ;LINE
 S ^XTMP("BUDARP7",BUDJ,BUDH,"PRGR",$P(BUDR1,U,5),BUDHISP,BUDCOM,BUDAGEP,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"*",2)
 Q
 ;
PRGELST ;EP - list of pregnant females
 ...
 S BUDP=$$PREG(DFN,$$FMADD^XLFDT(BUDED,-609),BUDED)
 I '$P(BUDP,U) Q  ;not pregnant
 S BUDRACE=$$HISP^BUDARPTC(DFN)
 I +BUDRACE=1 S BUDRACE="Hispanic or Latino"
 I +BUDRACE=2 S BUDRACE="All Others"
 S ^XTMP("BUDARP7",BUDJ,BUDH,"PRGE",BUDRACE,BUDCOM,BUDAGEP,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"*",2)
 Q
 ;
