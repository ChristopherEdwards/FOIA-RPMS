BUDARP7C ; IHS/CMI/LAB - UDS REPORT PROCESSOR 01 Dec 2013 5:11 PM ;
...
DM ;EP - list of DM
 ...
 ;S BUDRACE=$$RACE^BUDARP7I(BUDRACE)
...
SETSECTC ;
 ...
 I $G(BUDDMRL) S ^XTMP("BUDARP7",BUDJ,BUDH,"DMR",BUDRACEP,+BUDETHN,BUDCOM,BUDAGE,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"^",2)
...
SET ;
 ...
 I $G(BUDDMR1L),BUDLINE=11 S ^XTMP("BUDARP7",BUDJ,BUDH,"DMR1",BUDRACEP,+BUDETHN,BUDCOM,BUDAGE,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"^",2)_"^"_$P(BUDHGB,U,2)
 I $G(BUDDMR2L),BUDLINE=12.1 S ^XTMP("BUDARP7",BUDJ,BUDH,"DMR2",BUDRACEP,+BUDETHN,BUDCOM,BUDAGE,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"^",2)_"^"_$P(BUDHGB,U,2)
 I $G(BUDDMR3L),BUDLINE=12.2 S ^XTMP("BUDARP7",BUDJ,BUDH,"DMR3",BUDRACEP,+BUDETHN,BUDCOM,BUDAGE,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"^",2)_"^"_$P(BUDHGB,U,2)
 I $G(BUDDMR4L),BUDLINE=13 S ^XTMP("BUDARP7",BUDJ,BUDH,"DMR4",BUDRACEP,+BUDETHN,BUDCOM,BUDAGE,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"^",2)_"^"_$P(BUDHGB,U,2)
 Q
 ;
HGBA1C(P,BDATE,EDATE) ;EP
 NEW BUDG,BUDT,BUDC,E,%,L,T,BUDLT,D,X,J,C,G
 S BUDC=0
 S G=$$CPT^BUDADU(P,BDATE,EDATE,$O(^ATXAX("B","BUD HGBA1C CPTS",0)),5)
 ...
 S C=0,C=$O(BUDT(D,C))
 S X=$P(BUDT(D,C),U,1)
 I $$UP^XLFSTR(X)="COMMENT" Q 13_U_$P(BUDT(D,C),U,2)_" "_$$DATE^BUDAUTL1(9999999-D)_" (no result) "_X
 I X="" D  Q G
 .S G=""
 .I $P(BUDT(D,C),U,2)="CPT 3046F" S G=13_U_$P(BUDT(D,C),U,2)_" "_$$DATE^BUDAUTL1(9999999-D) Q
 .I $P(BUDT(D,C),U,2)="CPT 3045F" S G=12.2_U_$P(BUDT(D,C),U,2)_" "_$$DATE^BUDAUTL1(9999999-D) Q
 .I $P(BUDT(D,C),U,2)="CPT 3044F" S G=11_U_$P(BUDT(D,C),U,2)_" "_$$DATE^BUDAUTL1(9999999-D) Q
 .I $P(BUDT(D,C),U,2)="CPT 83036" S G=13_U_$P(BUDT(D,C),U,2)_" "_$$DATE^BUDAUTL1(9999999-D) Q
 .I $P(BUDT(D,C),U,2)="CPT 83037" S G=13_U_$P(BUDT(D,C),U,2)_" "_$$DATE^BUDAUTL1(9999999-D) Q
 .S G=13_U_$P(BUDT(D,C),U,2)_" DID NOT HAVE A RESULT "_$$DATE^BUDAUTL1(9999999-D) Q
 S X=$$STRIP^XLFSTR(X," ")  ;strip spaces
 I X[">9" Q 13_U_$P(BUDT(D,C),U,2)_" "_X_" "_X_$$DATE^BUDAUTL1(9999999-D)
 S X=$$STV(X)
 I X="" Q 13_U_$P(BUDT(D,C),U,2)_" DID NOT HAVE A RESULT "_$$DATE^BUDAUTL1(9999999-D)
 I +X>9 Q 13_U_$P(BUDT(D,C),U,2)_" "_X_" "_$$DATE^BUDAUTL1(9999999-D)
 I +X<7 Q 11_U_$P(BUDT(D,C),U,2)_" "_X_" "_$$DATE^BUDAUTL1(9999999-D)
 I +X<8 Q 12.1_U_$P(BUDT(D,C),U,2)_" "_X_" "_$$DATE^BUDAUTL1(9999999-D)
 Q 12.2_U_$P(BUDT(D,C),U,2)_" "_X_" "_$$DATE^BUDAUTL1(9999999-D)
 ;Q 0_U_$P(BUDT(D,C),U,2)_" "_$$DATE^BUDAUTL1(9999999-D)_" result: "_X
 ;
RACE(R) ;EP
 I R="Unreported" Q 8
 I R="Asian" Q 1
...
PROBDMM ;now check for med
 K BUDMEDS1 D GETMEDS^BUDAUTL2(P,BDATE,EDATE,"BUD DIABETES MEDS TAX","BUD DIABETES MEDS NDC",,,.BUDMEDS1)
