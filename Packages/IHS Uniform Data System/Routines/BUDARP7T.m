BUDARP7T ; IHS/CMI/LAB - UDS REPORT DRIVER TABLE 6B ;
...
DMR4L ;EP
 ...
 D DMR4H Q:BUDQUIT
 I '$D(^XTMP("BUDARP7",BUDJ,BUDH,"DMR4")) W !!,"No patients to report." Q
 D DMR4L1
 I $Y>(IOSL-3) D DMR4H G:BUDQUIT DMR4LX
 W !!,"TOTAL DIABETES PATIENTS 18-75 W/A1C >9% BY RACE AND HISPANIC ",!,"OR LATINO IDENTITY: ",BUDTOT,!
DMR4LX ;
 Q
DMR4L1 ;
 I $Y>(IOSL-7) D DMR4H Q:BUDQUIT
 S BUDTOT=0
 S BUDRACE="" F  S BUDRACE=$O(^XTMP("BUDARP7",BUDJ,BUDH,"DMR4",BUDRACE)) Q:BUDRACE=""!(BUDQUIT)  D
 .S BUDETH="" F  S BUDETH=$O(^XTMP("BUDARP7",BUDJ,BUDH,"DMR4",BUDRACE,BUDETH)) Q:BUDETH=""!(BUDQUIT)  D DMR4L2
 Q
DMR4L2 ;
 S BUDSTOT=0
 S BUDRACEL=$$RACEL^BUDARP7I(BUDRACE,BUDETH)
 W !,BUDRACEL
 S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUDARP7",BUDJ,BUDH,"DMR4",BUDRACE,BUDETH,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 .S BUDA="" F  S BUDA=$O(^XTMP("BUDARP7",BUDJ,BUDH,"DMR4",BUDRACE,BUDETH,BUDCOM,BUDA)) Q:BUDA=""!(BUDQUIT)  D
 ..S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUDARP7",BUDJ,BUDH,"DMR4",BUDRACE,BUDETH,BUDCOM,BUDA,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ...S DFN=0 F  S DFN=$O(^XTMP("BUDARP7",BUDJ,BUDH,"DMR4",BUDRACE,BUDETH,BUDCOM,BUDA,BUDNAME,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ....I $Y>(IOSL-3) D DMR4H Q:BUDQUIT  W !,BUDRACEL,!
 ...
 ....S BUDALL=^XTMP("BUDARP7",BUDJ,BUDH,"DMR4",BUDRACE,BUDETH,BUDCOM,BUDA,BUDNAME,DFN)
 ....S BUDPPV=$P(BUDALL,"^",1)
 ....W ?5,$P(BUDALL,"^",2),!
 ...
 I $Y>(IOSL-4) D DMR4H Q:BUDQUIT  W !,BUDRACEL,!
...
DMR3 ;EP
 ...
 W !,$$CTR($$LOC,80)
 W !,$$CTR("UDS 2013",80)
 W !!,"All DM Patients w/A1c >=8 and <=9 by Race & Hisp Identity (Table 7)",!
 D GENI
 D PAUSE
 W !!,"This report provides a list by race and Hispanic or Latino Identity of "
 W !,"patients age 18 to 75 years old who have had two medical visits during"
 ...
 W !,"through the end of the report period, and without a diagnosis of "
 W !,"polycystic ovaries, gestational diabetes, or steroid-induced diabetes"
 W !,"and with a most recent hemoglobin A1c greater than or equal to 8% and "
 W !,"less than or equal to 9%."
 W !
 Q
DMR3L ;EP
 S BUDP=0,BUDQUIT=0,BUDTOT=0
 D DMR3H Q:BUDQUIT
 I '$D(^XTMP("BUDARP7",BUDJ,BUDH,"DMR3")) W !!,"No patients to report." Q
 D DMR3L1
 I $Y>(IOSL-3) D DMR3H G:BUDQUIT DMR3LX
 W !!,"TOTAL DIABETES PATIENTS 18-75 W/A1C >=8 and <=9% BY RACE AND HISPANIC ",!,"OR LATINO IDENTITY: ",BUDTOT,!
DMR3LX ;
 Q
DMR3L1 ;
 I $Y>(IOSL-7) D DMR3H Q:BUDQUIT
 S BUDTOT=0
 S BUDRACE="" F  S BUDRACE=$O(^XTMP("BUDARP7",BUDJ,BUDH,"DMR3",BUDRACE)) Q:BUDRACE=""!(BUDQUIT)  D
 .S BUDETH="" F  S BUDETH=$O(^XTMP("BUDARP7",BUDJ,BUDH,"DMR3",BUDRACE,BUDETH)) Q:BUDETH=""!(BUDQUIT)  D DMR3L2
 Q
DMR3L2 ;
 S BUDSTOT=0
 S BUDRACEL=$$RACEL^BUDARP7I(BUDRACE,BUDETH)
 W !,BUDRACEL
 S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUDARP7",BUDJ,BUDH,"DMR3",BUDRACE,BUDETH,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 .S BUDA="" F  S BUDA=$O(^XTMP("BUDARP7",BUDJ,BUDH,"DMR3",BUDRACE,BUDETH,BUDCOM,BUDA)) Q:BUDA=""!(BUDQUIT)  D
 ..S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUDARP7",BUDJ,BUDH,"DMR3",BUDRACE,BUDETH,BUDCOM,BUDA,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ...S DFN=0 F  S DFN=$O(^XTMP("BUDARP7",BUDJ,BUDH,"DMR3",BUDRACE,BUDETH,BUDCOM,BUDA,BUDNAME,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ...
 ....S BUDHISV=$$HISP^BUDARPTC(DFN)
 ....W ?24,$P(BUDHISV,U,3)," (",$P(BUDHISV,U,2),")",!
 ....S BUDALL=^XTMP("BUDARP7",BUDJ,BUDH,"DMR3",BUDRACE,BUDETH,BUDCOM,BUDA,BUDNAME,DFN)
 ....S BUDPPV=$P(BUDALL,"^",1)
 ....W ?5,$P(BUDALL,"^",2),!
 ....F BUDX=1:1 S BUDV=$P(BUDPPV,U,BUDX) Q:BUDV=""!(BUDQUIT)  D
 .....I $Y>(IOSL-3) D DMR3H Q:BUDQUIT  W !,BUDRACEL,!
...
DMR3H1 ;
 ...
 W !,$$CTR("***  RPMS Uniform Data System (UDS)  ***",80)
 W !,$$CTR("Patient List for Table 7, Section C",80)
 W !,$$CTR("Diabetes w/A1c >=8 and <=9 by Race and Hispanic or Latino Identity",80),!
 W $$CTR($P(^DIC(4,BUDSITE,0),U),80),!
 S X="Reporting Period: "_$$FMTE^XLFDT(BUDBD)_" to "_$$FMTE^XLFDT(BUDED) W $$CTR(X,80),!
 W $TR($J("",80)," ","-")
 I BUDP=0 D
