BUDBRP6B ; IHS/CMI/LAB - UDS REPORT DRIVER TABLE 6B ; 30 Jan 2015  3:12 PM
...
EN2 ;
 S BUDSITE=""
 S DIC="^BUDBSITE(",DIC(0)="AEMQ",DIC("A")="Enter your site: " D ^DIC
 I Y=-1 D EOJ Q
 S BUDSITE=+Y
 ...
 S BUDTAXT="B6" D TAXCHK^BUDBXTCH
 D YEAR
 I BUDYEAR="" D EOJ Q
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BUDBRP6B",ZTDTH="",ZTDESC="UDS 14 REPORT TABLE 6B" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D EN^XBVK("BUD")
 Q
...
M ;EP - called from option
 ...
 S X=0 F  S X=$O(BUDIND(X)) Q:X'=+X  X ^BUDBLST2(X,1)
 G EN1
PROC ;EP - called from taskman
 S BUDJ=$J,BUDH=$H
 S ^XTMP("BUDBRP6B",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^BUD TABLE 6B LISTS"
 ;NOW LOOP PATS
 S DFN=0 F  S DFN=$O(^AUPNPAT(DFN)) Q:DFN'=+DFN  D
 .K ^TMP($J)
 .Q:'$D(^AUPNPAT(DFN,0))
 .Q:'$D(^DPT(DFN,0))
 .;I DUZ=2881 Q:$$HRN^AUPNPAT(DFN,DUZ(2))'=445587
 .;;I DUZ=2881 Q:'$D(^DIBT(14161,1,DFN))
 .;I DUZ=2881 Q:DFN'=26580
...
LORIN .;DO NOT COUNT BASED ON CLASSIFICATION IN V6.0
 ...
 .D GETV^BUDBRPTD  ;get visits that meet criteria
...
PRGALST ;EP - list of pregnant females
 ...
 S X=$$AGB(BUDAGEP)
 S ^XTMP("BUDBRP6B",BUDJ,BUDH,"PRGA",X,BUDAGEP,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDP,"*",2)
 Q
AGB(N) ;
...
PROB S T=$O(^ATXAX("B","BGP GPRA PREGNANCY DIAGNOSES",0))
 ...
 S BUDD=G,BUDALL=BUDALL_"Problem List: "_$$VAL^XBDIQ1(9000011,Z,.01)_" on "_$$DATE^BUDBUTL1(G)
MA ;now check for abortion or miscarriage
 ...
 S BUDG=$$LASTPRC^BUDBUTL1(P,"BGP ABORTION PROCEDURES",BDATE,EDATE)
 I BUDG Q 0
 ...
 S %=$$CPT^BUDBDU(P,BUDD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT MISCARRIAGE",0))
 S %=$$CPT^BUDBDU(P,BUDD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT ABORTION",0))
 S %=$$TRAN^BUDBDU(P,BUDD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT MISCARRIAGE",0))
 S %=$$TRAN^BUDBDU(P,BUDD,EDATE,T,3)
 I %]"" Q 0
...
PRINT ;EP - called from taskman
 ...
 D HEADER^BUDBRPTP Q:BUDQUIT  D T6BH
 ...
 D LINE1
 I $Y>(IOSL-3) D HEADER^BUDBRPTP Q:BUDQUIT  D T6BH
 ...
 I $Y>(IOSL-12) D HEADER^BUDBRPTP Q:BUDQUIT  D T6BH
 ...
 D LINE
 I $Y>(IOSL-20) D HEADER^BUDBRPTP Q:BUDQUIT  D T6BH
...
GENI ;general introductions
 ...
 W !,"December 31, 2014.  Based on the UDS definition, to be considered a"
