...
TUA1L ;EP
 ...
 D TUA1H Q:BUDQUIT
 I '$D(^XTMP("BUDBRP6B",BUDJ,BUDH,"TUA1")) W !!,"No patients to report.",! Q
 D TUA1L1
 I $Y>(IOSL-3) D TUA1H Q:BUDQUIT
 W !!,"TOTAL PATIENTS WITH TOBACCO USE SCREEN AND CESSATION INTERVENTION: ",BUDTOT,!
 Q
TUA1L1 ;
 I $Y>(IOSL-7) D TUA1H Q:BUDQUIT
 S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUDBRP6B",BUDJ,BUDH,"TUA1",BUDAGE)) Q:BUDAGE=""!(BUDQUIT)  D
 .S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUDBRP6B",BUDJ,BUDH,"TUA1",BUDAGE,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUDBRP6B",BUDJ,BUDH,"TUA1",BUDAGE,BUDNAME,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ...S DFN=0 F  S DFN=$O(^XTMP("BUDBRP6B",BUDJ,BUDH,"TUA1",BUDAGE,BUDNAME,BUDCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ....I $Y>(IOSL-3) D TUA1H Q:BUDQUIT
 ...
 ....S BUDTOT=BUDTOT+1
 ....S BUDALL=^XTMP("BUDBRP6B",BUDJ,BUDH,"TUA1",BUDAGE,BUDNAME,BUDCOM,DFN)
 ....I BUDALL="" W ?5,"Never" Q
 ....S BUDSCR=$P(BUDALL,"|"),BUDUSER=$P(BUDALL,"|",2),BUDCESS=$P(BUDALL,"|",3)
 ....W ?5,$P(BUDSCR,U,2),?20,$E($P(BUDSCR,U,1),1,18) I BUDUSER]"" W ?40,"USER: ",BUDUSER," CESS: ",$P(BUDCESS,U,1)_" "_$P(BUDCESS,U,2)
 ....;S %=$P(BUDALL,"|",2)
 ....;W ?60,$P(%,U)
 Q
TUA1H ;
...
TUA2L ;EP
 S BUDP=0,BUDQUIT=0,BUDTOT=0
 D TUA2H Q:BUDQUIT
 I '$D(^XTMP("BUDBRP6B",BUDJ,BUDH,"TUA2")) W !!,"No patients to report.",! Q
 D TUA2L1
 I $Y>(IOSL-3) D TUA2H Q:BUDQUIT
 W !!,"TOTAL PATIENTS WITHOUT TOBACCO USE SCREEN OR CESSATION INERVENTION: ",BUDTOT,!
 Q
TUA2L1 ;
 I $Y>(IOSL-7) D TUA2H Q:BUDQUIT
 S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUDBRP6B",BUDJ,BUDH,"TUA2",BUDAGE)) Q:BUDAGE=""!(BUDQUIT)  D
 .S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUDBRP6B",BUDJ,BUDH,"TUA2",BUDAGE,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUDBRP6B",BUDJ,BUDH,"TUA2",BUDAGE,BUDNAME,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ...S DFN=0 F  S DFN=$O(^XTMP("BUDBRP6B",BUDJ,BUDH,"TUA2",BUDAGE,BUDNAME,BUDCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ....I $Y>(IOSL-3) D TUA2H Q:BUDQUIT
 ....W !,$E($P(^DPT(DFN,0),U,1),1,25),?29,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2)),?41,$E(BUDCOM,1,25),?70,$P(^DPT(DFN,0),U,2),?75,BUDAGE,!
 ....S BUDTOT=BUDTOT+1
 ....S BUDALL=^XTMP("BUDBRP6B",BUDJ,BUDH,"TUA2",BUDAGE,BUDNAME,BUDCOM,DFN)
 ....I BUDALL="" W ?5,"Never" Q
 ....S BUDSCR=$P(BUDALL,"|"),BUDUSER=$P(BUDALL,"|",2),BUDCESS=$P(BUDALL,"|",3)
 ....W ?5,$P(BUDSCR,U,2),?20,$E($P(BUDSCR,U,1),1,18) I BUDUSER]"" W ?40,"USER: ",BUDUSER," CESS: ",$P(BUDCESS,U,1)_" "_$P(BUDCESS,U,2)
 ....;S %=$P(BUDALL,"|",2)
