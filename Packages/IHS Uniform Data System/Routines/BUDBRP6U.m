BUDBRP6U ; IHS/CMI/LAB - UDS REPORT PROCESSOR ;
...
TOBACCO(P,BDATE,EDATE) ;
 ...
 I Y]"" Q Y
 ;NOW CHECK DX
 S Y=$$LASTDX^BUDBUTL1(P,"BUD TOBACCO SCREEN DXS",BDATE,EDATE)
 I Y S Y=$P(Y,U,2)_U_$P(Y,U,3) Q Y
 S Y=$$CPTI^BUDBDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("1034F"),U,1))
 I Y S Y="1034F"_U_$P(Y,U,2) Q Y
 S Y=$$CPTI^BUDBDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("1035F"),U,1))
...
TOBCESS(P,BDATE,EDATE,PDATE) ;EP
 ...
 I %]"" Q %
 S Y=""
 S Y=$$CPTI^BUDBDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("99406"),U,1))
 I Y S Y="99406"_U_$P(Y,U,2) Q Y
 S Y=$$CPTI^BUDBDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("99407"),U,1))
 I Y S Y="99407"_U_$P(Y,U,2) Q Y
 S Y=$$CPTI^BUDBDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("4000F"),U,1))
 I Y S Y="4000F"_U_$P(Y,U,2) Q Y
 S Y=$$CPTI^BUDBDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("4001F"),U,1))
 I Y S Y="4001F"_U_$P(Y,U,2) Q Y
 NEW BUDMEDS1,T1,M,E,G,Z,N,C,BUDLPED
 S BUDLPED=""
 D GETMEDS^BUDBUTL2(P,BDATE,EDATE,,,,,.BUDMEDS1)
...
H ;EP ; ASTHMA
 ...
 K ^TMP($J,"A")
 I BUDAST="" S X="",X=$$ASTHTHER(DFN,BUDBD,BUDED) I X]"" S ^XTMP("BUDBRP6B",BUDJ,BUDH,"APT2",BUDAGE,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDAST,U)_U_$P(X,U,2) Q
 ...
 .I $G(BUDAPT2L) D
 ..I BUDASTT="" S ^XTMP("BUDBRP6B",BUDJ,BUDH,"APT2",BUDAGE,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDAST,U)_U_$P(BUDASTT,U,2)
 .I $G(BUDAPT1L) D
 ..I BUDASTT]"" S ^XTMP("BUDBRP6B",BUDJ,BUDH,"APT1",BUDAGE,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDAST,U)_U_$P(BUDASTT,U,2)
 Q
SABA(P,BD,ED) ;
 NEW BUDMEDS1,G,A,C,M,V,V1D,BGPHSABA
 S G="",BGPHSABA=""
 D GETMEDS^BUDBUTL2(P,BD,ED,"BGP PQA SABA MEDS","BGP PQA SABA NDC",,,.BUDMEDS1)
 ...
 I 'BGPHSABA Q ""
 S G=""
 D GETMEDS^BUDBUTL2(P,BD,ED,"BGP PQA CONTROLLER MEDS","BGP PQA CONTROLLER NDC",,,.BUDMEDS1)
...
ASTHMA(P,BDATE,EDATE) ;EP
 ...
 S Y=""
 S Y=$$CPTI^BUDBDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("1038F"),U,1))
 I Y S G($P(Y,U,2))="CPT: 1038F"
 S Y=$$CPTI^BUDBDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("4015F"),U,1))
...
AST1039(P,BDATE,EDATE) ;EP
 ...
 I '$D(^TMP($J,"A",1)) Q ""
 S Y=""
 S Y=$$CPTI^BUDBDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("1039F"),U,1))
