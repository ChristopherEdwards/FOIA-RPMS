...
ADOLWT ;EP - called from xbdbque
 ...
 Q:$$PREG^BUDBRP6B(DFN,$$FMADD^XLFDT(BUDED,-609),BUDED)
 ...
 .I $G(BUDWAC2L) D
 ..I G'=3 S ^XTMP("BUDBRP6B",BUDJ,BUDH,"WAC2",BUDAGE,$P(^DPT(DFN,0),U),BUDCOM,DFN)=BUDPAP
 .I $G(BUDWAC1L) D
 ..I G=3 S ^XTMP("BUDBRP6B",BUDJ,BUDH,"WAC1",BUDAGE,$P(^DPT(DFN,0),U),BUDCOM,DFN)=BUDPAP
 Q
 ;
 ;
...
BMIPROC(P,BDATE,EDATE) ;EP
 NEW A
 S A=$$LASTDX^BUDBUTL1(P,"BUD BMI PROCS",BDATE,EDATE)
 I A Q "POV "_$P(A,U,2)
 Q ""
REF(P,BDATE,EDATE) ;EP
 NEW H,W
 S H=$$REFRNU^BUDBUTL1(P,9999999.07,$O(^AUTTMSR("B","HT",0)),BDATE,EDATE)
 I H="" Q 1
 S W=$$REFRNU^BUDBUTL1(P,9999999.07,$O(^AUTTMSR("B","WT",0)),BDATE,EDATE)
 I W="" Q 1
 Q ""
...
NUTR(P,BDATE,EDATE) ;EP
 ...
 S %=$$CPT^BUDBDU(P,BDATE,EDATE,T,5)
 I %]"" Q "CPT: "_$P(%,U,2)
 Q ""
PA(P,BDATE,EDATE) ;EP
 NEW BUDALLED,BUDG,X,E,%,D,T,Y
...
ADULT ;EP
 ...
 Q:$$LASTDX^BUDBUTL1(DFN,"V66.7",$$FMADD^XLFDT($$VD^APCLV(BUDLASTV),-(6*30.5)),$$VD^APCLV(BUDLASTV))
 ...
 I BUDBMI="" S BUDPLAN=$$PLAN(DFN,BUDBD,BUDED) I BUDPLAN]"" S G=0 S ^XTMP("BUDBRP6B",BUDJ,BUDH,"AWS2",BUDAGE,$P(^DPT(DFN,0),U),BUDCOM,DFN)=BUDBMI_U_$S(BUDOW]"":"OVERWEIGHT",BUDUW]"":"UNDERWEIGHT",1:"")_U_BUDPLAN Q  ;NO BMI
 I BUDBMI="" Q
 I BUDAGE<65 D  G N
...
N I BUDOW="",BUDUW="" Q  ;not over/underweight & HAD BMI
 S BUDWTDT=$$BMI^BUDBRP6R(DFN,$$FMADD^XLFDT($$VD^APCLV(BUDLASTV),-(6*30.5)),$$VD^APCLV(BUDLASTV),BUDAGE)
 ...
 .I $G(BUDAWS2L) D
 ..I 'G S ^XTMP("BUDBRP6B",BUDJ,BUDH,"AWS2",BUDAGE,$P(^DPT(DFN,0),U),BUDCOM,DFN)=BUDBMI_U_$S(BUDOW]"":"OVERWEIGHT",BUDUW]"":"UNDERWEIGHT",1:"")_U_BUDPLAN
 .I $G(BUDAWS1L) D
 ..I G S ^XTMP("BUDBRP6B",BUDJ,BUDH,"AWS1",BUDAGE,$P(^DPT(DFN,0),U),BUDCOM,DFN)=BUDBMI_U_$S(BUDOW]"":"OVERWEIGHT",BUDUW]"":"UNDERWEIGHT",1:"")_U_BUDPLAN
 Q
 ;
PLAN(P,BD,ED) ;
 ...
 ;S BD=D,ED=D
 ;check for cpt codes
 S G=$$CPT^BUDBUTL1(P,BD,ED,$O(^ATXAX("B","BUD WT PLAN CPTS",0)),5)
 I G]"" Q $P(G,U,2)_" on "_$$FMTE^XLFDT($P(G,U,1))
 S G=$$LASTDX^BUDBUTL1(P,"BUD WT PLAN DXS",BD,ED)
...
G ;EP
 ...
 S X=$$GETV^BUDBRP6U(DFN,BUDDOB,BUDED,BUDSITE)
 Q:X<2
 K BUDTOBS,BUDTOBD
 ...
 S BUDUSER=$$TOBACCO^BUDBRP6U(DFN,BUDTOBDD,BUDED)
 S BUDCESS=""
 I BUDTOBS]"",BUDUSER="" S BUDGOT=1
 I BUDTOBS]"",BUDUSER]"" S BUDCESS=$$TOBCESS^BUDBRP6U(DFN,BUDTOBDD,BUDED) I BUDCESS]"" S BUDGOT=1
 I BUDGOT S BUDSECG1("ABM")=$G(BUDSECG1("ABM"))+1
S ;put the rest in demoninator
 S BUDSECG1("PTS")=$G(BUDSECG1("PTS"))+1 D
 .I $G(BUDTUA2L) D
 ..I 'BUDGOT S ^XTMP("BUDBRP6B",BUDJ,BUDH,"TUA2",BUDAGE,$P(^DPT(DFN,0),U),BUDCOM,DFN)=BUDTOBS_"|"_$S(BUDUSER]"":$P(BUDUSER,U,1)_" "_$$DATE^BUDBUTL1($P(BUDUSER,U,2))_"|"_$P(BUDCESS,U,1)_" "_$$DATE^BUDBUTL1($P(BUDCESS,U,3)),1:"")
 .I $G(BUDTUA1L) D
 ..I BUDGOT S ^XTMP("BUDBRP6B",BUDJ,BUDH,"TUA1",BUDAGE,$P(^DPT(DFN,0),U),BUDCOM,DFN)=BUDTOBS_"|"_$S(BUDUSER]"":$P(BUDUSER,U,1)_" "_$$DATE^BUDBUTL1($P(BUDUSER,U,2))_"|"_$P(BUDCESS,U,1)_" "_$$DATE^BUDBUTL1($P(BUDCESS,U,3)),1:"")
 Q
 ;
TOBACCO(P,BDATE,EDATE) ;
 ...
 S BUDPV=$$LASTDX^BUDBUTL1(P,"BUD TOBACCO SCREEN DXS",BDATE,EDATE)
 I BUDPV S BUDLAST($P(BUDPV,U,3))=$P(BUDPV,U,2)_U_$$FMTE^XLFDT($P(BUDPV,U,3))_U_$P(BUDPV,U,3)  ; Q BUDTOB
 S BUDCPT=$$CPT^BUDBUTL1(P,BDATE,EDATE,$O(^ATXAX("B","BUD TOBACCO SCREEN CPTS",0)),7)
