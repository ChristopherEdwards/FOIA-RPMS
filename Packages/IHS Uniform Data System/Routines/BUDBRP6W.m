BUDBRP6W ; IHS/CMI/LAB - UDS REPORT PROCESSOR 01 Dec 2014 4:03 PM ;
...
ROTA(P,BDATE,EDATE) ;EP
 ...
 S X=$$LASTDXI^BUDBUTL1(P,"008.61",$$DOB^AUPNPAT(P),EDATE)
 I X]"" Q "1^ROTAVIRUS Evidence: "_$P(X,U,2)_" on "_$$DATE^BUDBUTL1($P(X,U,3))
 S X=$$LASTDX^BUDBUTL1(P,"BUD ROTA CONTRA DXS",$$DOB^AUPNPAT(P),EDATE)
 I X]"" Q "1^ROTAVIRUS Contraindication: "_$P(X,U,2)_" on "_$$DATE^BUDBUTL1($P(X,U,3))
 I $$PLCODE^BUDBDU(P,"008.61") Q "1^ROTAVIRUS Evidence: 008.61 on Problem List"
 S X=$$PLTAX^BUDBDU(P,"BUD ROTA CONTRA DXS") I X Q "1^ROTAVIRUS Contraindication: "_$P(X,U,2)_" on Problem List"
 F BUDZ=119,74,116,122 S X=$$ROTACONT(P,BUDZ,EDATE) Q:X]""
 I X]"" Q "1^ROTAVIRUS Contraindication IM package: "_$$DATE^BUDBUTL1($P(X,U))_" "_$P(X,U,2)
 S G=""
 ...
 .I N["NEOMYCIN" S G="1^ROTAVIRUS Contraindiction: "_$$DATE^BUDBUTL1($P($P($G(^GMR(120.8,X,0)),U,4),"."))_"  Allergy Tracking: "_N
 I G]"" Q G
...
ROT3IMM ;get all immunizations
 ...
 ....S BUDOPV(9999999-$P(ED,"."))="POV: "_Y_" on "_$$DATE^BUDBUTL1((9999999-$P(ED,"."))),BUDAPOV(9999999-$P(ED,"."))="POV: "_Y_" on "_$$DATE^BUDBUTL1((9999999-$P(ED,".")))
 S (X,Y)="",C=0 F  S X=$O(BUDOPV(X)) Q:X'=+X  S C=C+1 D
 .I C=1 S Y=X Q
 .I $$FMDIFF^XLFDT(X,Y)<11 K BUDOPV(X) Q
 .S Y=X
