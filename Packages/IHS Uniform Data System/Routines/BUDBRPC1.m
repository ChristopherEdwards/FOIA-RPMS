BUDBRPC1 ; IHS/CMI/LAB - UDS TABLE 6 ;
...
PT ;
 ...
 S:C=21.1 C=69
 S:C=21.2 C=70
 I $G(BUDT6L) S ^XTMP("BUDBRPT1",BUDJ,BUDH,"T6",C,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV)=$G(^XTMP("BUDBRPT1",BUDJ,BUDH,"T6",C,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV))_$E(BUDVAL,1,20)_U
PT1 S $P(BUDT6("V"),U,C)=$P(BUDT6("V"),U,C)+1
 ...
 Q
OT ;
 I $D(^XTMP("BUDBRPT1",BUDJ,BUDH,"T6",C,BUDCOM,BUDAGE,BUDSEX,DFN)) Q  ;patient already a hit on this line
 S ^XTMP("BUDBRPT1",BUDJ,BUDH,"ORPHANS",C,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV)=$E(BUDVAL,1,20)
 Q
T6 ;EP
 S BUDV=0 F  S BUDV=$O(^TMP($J,"VISITS356A",BUDV)) Q:BUDV'=+BUDV  D
 .S BUDP=$$PRIMPROV^APCLV(BUDV,"D") Q:BUDP=""
 .;DO LINE 1-2(A)
 .;S X=$$NEWLY($P(^AUPNVSIT(BUDV,0),U,2),BUDBD,BUDED) I $P(X,U)=1 S C=2 S BUDVAL=$P(X,U,2) D PT
...
SERV1 ;NOW DO SERVICES ON FULL LIST
 ...
 Q
PRIMDX ;
 I BUDPOV="042."!(BUDPOV="079.53")!($E(BUDPOV,1,3)="V08") S C=1 S BUDVAL=BUDPOV D PT D  Q
 .;was this the first dx for this patient ever?
 .I $$NEWLY($P(^AUPNVSIT(BUDV,0),U,5),BUDV,BUDBD,BUDED) S C=2,BUDVAL=BUDPOV D PT
...
SERV ;
 ...
 ..S C=22 D PT1 S ^XTMP("BUDBRPT1",BUDJ,BUDH,"T6",C,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","MAM",BUDW)=^TMP($J,"MAMMS",BUDW)
...
PAP1 S BUDVAL=$$PAP(BUDV) I BUDVAL]"" S C=23 D PT
 ...
 ..S C=23 D PT1 S ^XTMP("BUDBRPT1",BUDJ,BUDH,"T6",C,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","PAP",BUDW)=^TMP($J,"PAPS",BUDW)
 ...
 S Y=0 S BUDVAL=$$L26^BUDBRPC2(BUDV) I BUDVAL]"" S C=26 D PT
 S BUDVAL=$$L26A^BUDBRPC2(BUDV) I BUDVAL]"" S C="26.1" D PT
 S BUDVAL=$$L26B^BUDBRPC2(BUDV) I BUDVAL]"" S C="26.2" D PT
 S BUDVAL=$$L26C^BUDBRPC2(BUDV) I BUDVAL]"" S C="26.3" D PT
 S BUDVAL=$$L26D^BUDBRPC2(BUDV) I BUDVAL]"" S C="26.4" D PT
 S BUDVAL=$$L27^BUDBRPC2(BUDV) I BUDVAL]"" S C=27 D PT
 S BUDVAL=$$L28^BUDBRPC2(BUDV) I BUDVAL]"" S C=28 D PT
 S BUDVAL=$$L29^BUDBRPC2(BUDV) I BUDVAL]"" S C=29 D PT
 S BUDVAL=$$L30^BUDBRPC2(BUDV) I BUDVAL]"" S C=30 D PT
 S BUDVAL=$$L31^BUDBRPC2(BUDV) I BUDVAL]"" S C=31 D PT
 S BUDVAL=$$L32^BUDBRPC2(BUDV) I BUDVAL]"" S C=32 D PT
 S BUDVAL=$$L33^BUDBRPC2(BUDV) I BUDVAL]"" S C=33 D PT
 S BUDVAL=$$L34^BUDBRPC2(BUDV) I BUDVAL]"" S C=34 D PT
 Q
CONTRA(BUDV) ;
...
SEASFLU(BUDV) ;
 ...
 .I Z=15!(Z=16)!(Z=111)!(Z=135)!(Z=140)!(Z=141)!(Z=144)!(Z=149)!(Z=150)!(Z=151)!(Z=153)!(Z=155)!(Z=158)!(Z=161) S G=Z_" imm"
 .Q
 Q G
H1N1FLU(BUDV) ;
 S G="" S X=0 F  S X=$O(^AUPNVCPT("AD",BUDV,X)) Q:X'=+X!(G]"")  D
...
PAP(BUDV) ;
 ...
 .I Z="V76.2"!(Z="V72.3")!(Z="V72.31")!(Z="V72.32") S G=Z  ;!(Z="V72.32")!(Z="V76.47")!(Z="V67.01")!($E(Z,1,5)="795.0") S G=Z
 I G]"" Q "V POV: "_G
...
IMM(BUDV) ;
 ...
 S G="",X=0 F  S X=$O(^AUPNVIMM("AD",BUDV,X)) Q:X'=+X!(G]"")  D
 .S Z=$P(^AUPNVIMM(X,0),U),Z=+$P(^AUTTIMM(Z,0),U,3)
 .S BUDTIEN=$O(^BUDBCNTL("B","IMMUNIZATIONS LINE 24",0))
 .I $D(^BUDBCNTL(BUDTIEN,11,"B",Z)) S G=Z_" imm"
 .Q
 Q G
HIV(BUDV) ;
...
NEWLY(P,V,BD,ED) ;
 ...
 K BUDG
 S Y="BUDG("
 S X=P_"^FIRST DX [BUD HIV DXS;DURING "_$$FMTE^XLFDT($$DOB^AUPNPAT(P))_"-"_$$FMTE^XLFDT(ED) S E=$$START1^APCLDF(X,Y)
 I '$D(BUDG(1)) Q ""
 I $P(BUDG(1),U,5)'=V Q ""  ;not this visit so no need to check further
 ;now check problem list date of onset, if date of onset is prior to the report period quit
 S T=$O(^ATXAX("B","BUD HIV DXS",0))
 S (X,G)=0,Z="" F  S X=$O(^AUPNPROB("AC",P,X)) Q:X'=+X  D
 .Q:$P(^AUPNPROB(X,0),U,12)="D"
 .Q:$P(^AUPNPROB(X,0),U,13)=""  ;no date of onset so don't bother
 .S Y=$P(^AUPNPROB(X,0),U)
 .Q:'$$ICD^ATXCHK(Y,T,9)
 .I $P(^AUPNPROB(X,0),U,13)<$$VD^APCLV(V) S G=1 Q
 .Q
 I G=1 Q ""
 Q 1
