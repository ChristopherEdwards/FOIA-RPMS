BUDBRPL2 ; IHS/CMI/LAB - UDS print lists ;
 ;;9.0;IHS/RPMS UNIFORM DATA SYSTEM;;FEB 02, 2015;Build 42
 ;
T52 ;EP
 S BUDP=0,BUDX2L=35
 I '$D(^XTMP("BUDBRPT1",BUDJ,BUDH,"T5",BUDX2L)) D T52H W !!,"No Uncategorized Provider visits to report." Q
 D T52H
 S BUDX2L=35,BUDX2L2=0,BUDY=$O(^BUDBTFIV("B",BUDX2L,0)),BUDY=$P(^BUDBTFIV(BUDY,0),U,2)
 W !!,"Line ",BUDX2L,"   ",BUDY
...
T5W ;
 ...
 W ?51,$P(^DPT(DFN,0),U,2),?55,$$AGE^AUPNPAT(DFN,BUDCAD),?60,$E($P($$RACE^BUDBRPTC(DFN),U,4),1,15)," (",$P($$RACE^BUDBRPTC(DFN),U,3),")"
...
T51 ;EP
 S BUDNEWR=1
 S BUDX2L="" F  S BUDX2L=$O(^XTMP("BUDBRPT1",BUDJ,BUDH,"T51",BUDX2L)) Q:BUDX2L=""!(BUDQUIT)  D
 .S BUDX2L2="" F  S BUDX2L2=$O(^XTMP("BUDBRPT1",BUDJ,BUDH,"T51",BUDX2L,BUDX2L2)) Q:BUDX2L2=""!(BUDQUIT)  D
 ...
 ..W !!,"Line ",BUDX2LL,"   ",BUDY
 ..S BUDPROV="" F  S BUDPROV=$O(^XTMP("BUDBRPT1",BUDJ,BUDH,"T51",BUDX2L,BUDX2L2,BUDPROV)) Q:BUDPROV=""!(BUDQUIT)  D
 ...I $Y>(IOSL-3) D T51H Q:BUDQUIT  W !!,"Line ",BUDX2LL,"   ",BUDY
 ...W !,BUDPROV,?35,^XTMP("BUDBRPT1",BUDJ,BUDH,"T51",BUDX2L,BUDX2L2,BUDPROV)
 .Q
 W !
 Q
T51H ;
...
T3A ;EP
 S BUDP=0
 D T3H Q:BUDQUIT
 S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUDBRPT1",BUDJ,BUDH,"3A",BUDAGE)) Q:BUDAGE'=+BUDAGE!(BUDQUIT)  D
 .S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUDBRPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUDBRPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ...S DFN="" F  S DFN=$O(^XTMP("BUDBRPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ...
 ....S BUDV=0 F  S BUDV=$O(^XTMP("BUDBRPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCOM,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  D
 .....I $Y>(IOSL-3) D T3H Q:BUDQUIT
