BUDCDSI ; IHS/CMI/LAB - DISPLAY IND LISTS ;
...
EN ;EP -- main entry point for APCH HMR DISPLAY
 D EN^VALM("BUD 15 INDICATOR SELECTION")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ; -- init variables and list array
 K BUDTIND S BUDHIGH=""
 S (X,Y,Z,C)=0 F  S X=$O(^BUDCLST2("OR",X)) Q:X'=+X  S Y=0 F  S Y=$O(^BUDCLST2("OR",X,Y))  Q:Y'=+Y  D
 .S C=C+1,BUDTIND(C,0)=C_")",$E(BUDTIND(C,0),5)=$P(^BUDCLST2(Y,0),U,1),BUDTIND(C,C)=Y I $D(BUDIND(Y)) S BUDTIND(C,0)="*"_BUDTIND(C,0)
 .Q
 S (VALMCNT,BUDHIGH)=C
 Q
HELP ; -- help code
