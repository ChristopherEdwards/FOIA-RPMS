BUDCENV ; IHS/CMI/LAB - environmental check 13 Jan 2014 7:22 AM ; 19 Dec 2014  3:11 PM
...
ENV ;
 ...
 I $$VERSION^XPDUTL("BGP")'>"15.9" D MES^XPDUTL($$CJ^XLFSTR("Version 16 of CRS (BGP) is required.  Not installed",80)) D SORRY(2)
 I $$VERSION^XPDUTL("BUD")'>8.9 D MES^XPDUTL($$CJ^XLFSTR("Version 9.0 of UDS (BUD) is required.  Not installed",80)) D SORRY(2)
 Q
 ;
 ;
PRE ;
 ;ADD ALL OTHERS BACK IN
 F DA=1:1:50 S DIK="^BUDCCNTL(" D ^DIK
 F DA=1:1:50 S DIK="^BUDCIL(" D ^DIK
 F DA=1:1:50 S DIK="^BUDCTFIV(" D ^DIK
 F DA=1:1:50 S DIK="^BUDCTTA(" D ^DIK
 F DA=1:1:50 S DIK="^BUDCLST2(" D ^DIK
 F DA=1:1:999 S DIK="^BUDCTSC(" D ^DIK
 F DA=1:1:999 S DIK="^BUDCTSSC(" D ^DIK
 Q
POST ;
 ;move site parameters from 04 to 05 on first time install only
 I '$O(^BUDCSITE(0)) D
 .S BUDX=0 F  S BUDX=$O(^BUDQSITE(BUDX)) Q:BUDX'=+BUDX   D
 ..M ^BUDCSITE(BUDX)=^BUDBSITE(BUDX)
 ..S DA=BUDX,DIK="^BUDCSITE(" D IX1^DIK
...
SETICD ;
 ...
 S BUDX=0 F  S BUDX=$O(^BUDCTSC(BUDX)) Q:BUDX'=+BUDX  D
 .S BUDY=0 F  S BUDY=$O(^BUDCTSC(BUDX,7,BUDY)) Q:BUDY'=+BUDY  D
 ..F X=2,3 S $P(^BUDCTSC(BUDX,7,BUDY,0),U,X)=""
 K ^BUDCTSC("AC")
 S BUDX=0 F  S BUDX=$O(^BUDCTSC(BUDX)) Q:BUDX'=+BUDX  D
 .S BUDY=0 F  S BUDY=$O(^BUDCTSC(BUDX,7,BUDY)) Q:BUDY'=+BUDY  D
 ..S I=$P(^BUDCTSC(BUDX,7,BUDY,0),U,1)
 ..S I=$$ICDDX^ICDEX(I,DT)
 ..S $P(^BUDCTSC(BUDX,7,BUDY,0),U,2)=$P(I,U,1)_U_$P(I,U,20)
 S DIK="^BUDCTSC(" D IXALL^DIK
 ;NOW DO TABLE 6B/7 LISTS
DX ;DX 11 NODE
 S BUDX=0 F  S BUDX=$O(^BUDCTSSC(BUDX)) Q:BUDX'=+BUDX  D
 .S BUDY=0 F  S BUDY=$O(^BUDCTSSC(BUDX,11,BUDY)) Q:BUDY'=+BUDY  D
 ..F X=2,3 S $P(^BUDCTSSC(BUDX,11,BUDY,0),U,X)=""
 K ^BUDCTSSC("AD")
 S BUDX=0 F  S BUDX=$O(^BUDCTSSC(BUDX)) Q:BUDX'=+BUDX  D
 .S BUDY=0 F  S BUDY=$O(^BUDCTSSC(BUDX,11,BUDY)) Q:BUDY'=+BUDY  D
 ..S I=$P(^BUDCTSSC(BUDX,11,BUDY,0),U,1)
 ..S I=$$ICDDX^ICDEX(I,DT)
 ..S $P(^BUDCTSSC(BUDX,11,BUDY,0),U,2)=$P(I,U,1)
 ..S $P(^BUDCTSSC(BUDX,11,BUDY,0),U,3)=$P(I,U,20)
 S DIK="^BUDCTSSC(" D IXALL^DIK
 Q
LAB ;EP
...
INC ;UPDATE INCOME LEVELS
 S X=8 F  S X=$O(^BUDCIL("B",X)) Q:X'=+X  D
 .S Y=$O(^BUDCIL("B",X,0))
 .S C=X-8
 .S $P(^BUDCIL(Y,0),U,2)=(C*4160)+40890
 .S $P(^BUDCIL(Y,0),U,3)=(C*5200)+51120
 .S $P(^BUDCIL(Y,0),U,4)=(C*4780)+47010
 Q
