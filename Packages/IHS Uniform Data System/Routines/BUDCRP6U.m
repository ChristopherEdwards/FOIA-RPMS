BUDCRP6U ; IHS/CMI/LAB - UDS REPORT PROCESSOR ;
...
TOBACCO(P,BDATE,EDATE) ;
 ...
 .I F="LIGHT TOBACCO SMOKER" S G=1
 .I F="HEAVY TOBACCO SMOKER" S G=1
 .Q:'G
 .S Y=F_U_D
 .Q
 I Y]"" Q Y
 ...
 .I $D(^BUDCTSSC("AD",Y,T)) S G=$P(BUDG(X),U,2)_U_$$FMTE^XLFDT($P(BUDG(X),U,1))_U_$P(BUDG(X),U,1)
 I G S Y=$P(G,U,1)_U_$P(G,U,3) Q Y
 S Y=$$CPTI^BUDCDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("1034F"),U,1))
 I Y S Y="1034F"_U_$P(Y,U,2) Q Y
 S Y=$$CPTI^BUDCDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("1035F"),U,1))
...
TOBCESS(P,BDATE,EDATE,PDATE) ;EP
 ...
 I %]"" Q %
 S Y=""
 S Y=$$CPTI^BUDCDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("99406"),U,1))
 I Y S Y="99406"_U_$P(Y,U,2) Q Y
 S Y=$$CPTI^BUDCDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("99407"),U,1))
 I Y S Y="99407"_U_$P(Y,U,2) Q Y
 S Y=$$CPTI^BUDCDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("4000F"),U,1))
 I Y S Y="4000F"_U_$P(Y,U,2) Q Y
 S Y=$$CPTI^BUDCDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("4001F"),U,1))
 I Y S Y="4001F"_U_$P(Y,U,2) Q Y
 NEW BUDMEDS1,T1,M,E,G,Z,N,C,BUDLPED
 S BUDLPED=""
 D GETMEDS^BUDCUTL2(P,BDATE,EDATE,,,,,.BUDMEDS1)
...
H ;EP ; ASTHMA
 ...
 K ^TMP($J,"A")
 I BUDAST="" S X="",X=$$ASTHTHER(DFN,BUDBD,BUDED) I X]"" S ^XTMP("BUDCRP6B",BUDJ,BUDH,"APT2",BUDAGE,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDAST,U)_U_$P(X,U,2) Q
 Q:BUDAST=""  ;no asthma diagnosis
 Q:$E(BUDAST,1,5)="J45.2"  ;last dx was intermittent
 Q:$$AST1039(DFN,$P(BUDAST,U,2),BUDED)]""  ;had a 1039f after the asthma dx/1038f
 ;QUIT IF PL J45.2x
 Q:$$PLCL^BUDCDU(P,"INTERMITTENT ASHTMA DXS")
 ...
 .I $G(BUDAPT2L) D
 ..I BUDASTT="" S ^XTMP("BUDCRP6B",BUDJ,BUDH,"APT2",BUDAGE,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDAST,U)_U_$P(BUDASTT,U,2)
 .I $G(BUDAPT1L) D
 ..I BUDASTT]"" S ^XTMP("BUDCRP6B",BUDJ,BUDH,"APT1",BUDAGE,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDAST,U)_U_$P(BUDASTT,U,2)
 Q
SABA(P,BD,ED) ;
 NEW BUDMEDS1,G,A,C,M,V,V1D,BGPHSABA
 S G="",BGPHSABA=""
 D GETMEDS^BUDCUTL2(P,BD,ED,"BGP PQA SABA MEDS","BGP PQA SABA NDC",,,.BUDMEDS1)
 ...
 I 'BGPHSABA Q ""
 S G=""
 D GETMEDS^BUDCUTL2(P,BD,ED,"BGP PQA CONTROLLER MEDS","BGP PQA CONTROLLER NDC",,,.BUDMEDS1)
...
ASTHMA(P,BDATE,EDATE) ;EP
 ...
 ..S D=0
 ..I $D(^AUPNVPOV(Y,0)) S %=$P(^AUPNVPOV(Y,0),U),J=$$VAL^XBDIQ1(9000010.07,Y,.01)
 ...
 S Y=""
 S Y=$$CPTI^BUDCDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("1038F"),U,1))
 I Y S G($P(Y,U,2))="CPT: 1038F"
 S Y=$$CPTI^BUDCDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("4015F"),U,1))
...
AST1039(P,BDATE,EDATE) ;EP
 ...
 I '$D(^TMP($J,"A",1)) Q ""
 S Y=""
 S Y=$$CPTI^BUDCDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("1039F"),U,1))
...
GETV(P,BD,ED,SITE) ;EP - get all visits for this patient and COUNT MEDICAL VISITS
 ...
 .Q:VLOC=""
 .Q:'$D(^BUDCSITE(SITE,11,VLOC))  ;not valid location
 .Q:"AHSORMEI"'[$P(^AUPNVSIT(VSIT,0),U,7)
 .S CLINC=$$CLINIC^APCLV(VSIT,"C")
 .S TIEN=$O(^BUDCCNTL("B","FIRST LEVEL CLINIC EXCLUSIONS",0))
 .I CLINC]"",$D(^BUDCCNTL(TIEN,11,"B",CLINC)) Q  ;not a clinic code we want in any table
...
MEDC .;NOW CHECK FOR MEDICAL CARE, CAN ONLY HAVE 1 PER LOCATION OF ENCOUNTER
 .S S=0
 .S TIEN=$O(^BUDCCNTL("B","MEDICAL CARE LINE NUMBERS",0))
 .;S PP=$$PRIMPROV^APCLV(VSIT,"D")
 .I $E($$VAL^XBDIQ1(9000010,VSIT,.06),1,3)="CHS",PP=15 S LINE=2 G MEDC1
 .S Y=$O(^BUDCTFIV("C",PP,0)) I Y="" S LINE=35 G MEDC1
 .S LINE=$O(^BUDCTFIV("AA",PP,""))
MEDC1 .S S=0
 .I $D(^BUDCCNTL(TIEN,11,"B",LINE)) D
