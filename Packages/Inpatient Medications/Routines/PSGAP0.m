...
PAT ;
 ...
 I $D(^TMP($J,$E(PSGAPWDN,1,20),TM,PN)) S ^(PN)=$P(PSJPSEX,"^",2)_"^"_$E($P(PSJPDOB,"^",2),1,10)_";"_PSJPAGE_"^"_VA("PID")_"^"_PSJPDX_"^"_$S(PSJPRB]"":PSJPRB,1:"*NF*")_"^"_$E($P(PSJPAD,"^",2),1,10)_"^"_$E($P(PSJPTD,"^",2),1,10)_"^"_+PSJPWT
 Q
 ;
GDT ;
...
ENQ ; queued entry point
 K ^TMP($J) N RB,ATM,TM,DRGI,DRGN,DRGT,ON,PST,PSIVUP,PSJORIFN,QST,SLS,XTYPE
 D @("P"_PSGSS),^PSGAPP D ^%ZISC
 Q
 ;
PG ;
