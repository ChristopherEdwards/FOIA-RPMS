PSGDSP1 ;BIR/CML3-PRINT BLANK DISCHARGE ORDERS FORM ;21-Jun-2004 17:08;PLS
 ;;5.0; INPATIENT MEDICATIONS ;**8,60**;16 DEC 97
 ; Modified - IHS/CIA/PLS - 12/12/03 - Line PAT+4
 ;                          06/21/04 - Line PRINT+14
EN ;
...
PAT ;
 ...
 S PI=$P(PSJPSEX,U,2)_U_$E($P(PSJPDOB,U,2),1,10)_";"_PSJPAGE_U_PSJPPID_U_PSJPDX_U_$S(PSJPRB]"":PSJPRB,1:"*NF*")_U_$E($P(PSJPAD,U,2),1,10)_U_$E($P(PSJPTD,U,2),1,10)_U_$E(PSGOD,1,8)_U_SC_U_+PSJPWT  ; IHS/CIA/PLS - 12/12/03 SSN to HRN
 Q
 ;
PRINT ;
 ...
 F Q=$Y+5:1:IOSL-1 W !
 ; IHS/CIA/PLS - 06/21/04 - Changed SSN to HRN
 ;W !?2,PPN,?40,PSSN,?78-$L(PDOB),PDOB Q
 W !?2,PPN,?40,$G(PSJPPID),?78-$L(PDOB),PDOB Q
 ;
OP ;
