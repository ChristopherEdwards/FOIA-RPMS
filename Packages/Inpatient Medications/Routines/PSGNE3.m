PSGNE3 ;BIR/CML3,MLM-DETERMINE DEFAULT FOR START & STOP TIMES ;09 Feb 99 / 7:50 AM
 ;;5.0; INPATIENT MEDICATIONS ;**4,26,47,50,63,69,105,80**;16 DEC 97
 ;
...
SF I '$O(^PS(55,PSGP,5,"AUS",PSGDT)),'$D(^PS(53.1,"AC",PSGP)),+$G(^PS(55,PSGP,5.1)) S $P(^PS(55,PSGP,5.1),U)=""
 ...
 S PSGNEFD="",PSGNEW=$S('$P(PSJSYSW0,U,4):0,+$G(^PS(55,PSGP,5.1))'>PSGDT:0,1:+$G(^PS(55,PSGP,5.1)))
 ...
 S X1=$P(PSGNESD,"."),X2=$S($P(PSJSYSW0,"^",3):+$P(PSJSYSW0,"^",3),1:14) D C^%DTC
 I PSGNEDFD,$S($P(PSGNEDFD,"^")["L":PSGS0XT!PSGS0Y,1:1) D DFD
...
ENSD(SCH,AT,LI,OSD) ;Find start date/time for orders.
 ...
 I 'AT,INT>23 S:$P(PSJSYSW0,U,5)!($E(LI,11,12)>30) AT=$E($$FMADD^XLFDT(LI,0,1,0,0),9,10) S:'AT AT=$E(LI,9,10) S:AT="00" AT=24 ;S:AT=23&($E(LI,11,12)>30) AT=24
 ...
 I '$O(PSG(LI)) S X=OSD F  S ND=X D  Q:ND>LI  S:(INT>24)&('$G(PSJREN)) INT=24 S DAYS=INT\24,HRS=(-INT\24*24+INT) S X=$$FMADD^XLFDT($S($P(ND,".",2)=24:$P(ND,".")_".23",1:ND),DAYS,HRS) S:$P(X,".")'>$P(ND,".") X=$$FMADD^XLFDT(X,1,0,0,0)
