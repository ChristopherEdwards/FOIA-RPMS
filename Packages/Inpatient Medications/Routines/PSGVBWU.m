PSGVBWU ;BIR/CML3,MV-GET ORDERS FOR COMPLETE/VERIFY ;15 May 98 / 9:26 AM
 ;;5.0; INPATIENT MEDICATIONS ;**3,44,47,67,58**;16 DEC 97
 ;
...
CNTHEAD ; Header for order count.
 ...
 W @IOF,!,?16,"Pending/Non-Verified Order Totals by Ward Group",!!,?33,"Pending",?56,"Non-Verified",!
 W "Ward Group",?30,"IV",?40,"UD",?55,"IV",?67,"UD",!!
 Q
 ;
ENGORD ; get and sort order
 ...
 .S ND=$G(^PS(53.1,O,0)),PSJPRIO=$P($G(^(.2)),U,4),LD=$P($G(^PS(53.1,O,0)),U,16),ON=O_"P"
 ...
 F O=0:0 S O=$O(^PS(53.1,"AS","N",PSGP,O)) Q:'O  S LD=$P($G(^PS(53.1,O,0)),U,16) S ON=O_"P" D SET
