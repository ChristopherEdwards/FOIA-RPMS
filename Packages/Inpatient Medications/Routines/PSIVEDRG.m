PSIVEDRG ;BIR/MLM-ENTER/EDIT DRUGS FOR IV ORDER ;16 Mar 99 / 2:14 PM
 ;;5.0; INPATIENT MEDICATIONS ;**21,33,50,65,74,84**;16 DEC 97
 ;
...
1 ; Strength/Volume
 ...
 ;D:$D(X) IT G:'$D(X)!($G(X)["?") AMT S DRG(3)=X I X="" S Y=$G(^DD($S(DRGT="AD":53.157,1:53.158),1,4)) X Y G AMT
 ; changed below to remove ref to ^DD
...
IT ; Input Transform for Strength/Volume.
 ...
 I DRGT="AD" K:X'?.6N.".".8N!('X) X I $D(X) S X=X_" "_$$ENU^PSIVUTL(DRG(1)) W " ",$P(X," ",2) Q
 I $D(X) K:X=""!($A(X)=45)!(X'=+X)!(X>9999)!(X<.01) X I $D(X) S X=X_" ML" W " ML"
 W:'$D(X) $C(7),"??"
 Q
 ;
...
SAVEDRG(NEW,OLD)   ;Store/restore DRG array.
 ...
 F XX="AD","SOL" D
 . I $G(OLD(XX,0)) S NEW(XX,0)=OLD(XX,0)
...
CHK ; Check if drug is already part of order
 ...
 .I $E($P(DRG(DRGT,+TDRG),U,2),1,$L(X))=X W $P($P(DRG(DRGT,+TDRG),U,2),X,2) D ASKCHK Q
 .S TDRGP=$P(DRG(DRGT,TDRG),U) F J=0:0 S J=$O(^PS(FIL,TDRGP,3,J)) Q:'J!$G(DDONE)  I $E($P(^PS(FIL,TDRGP,3,J,0),U),1,$L(X))=X W $P($P(^PS(FIL,TDRGP,3,J,0),U),X,2)," ",$P(DRG(DRGT,TDRG),U,2) D ASKCHK Q
 Q
 ;
ASKCHK ; Do you want a drug that was previously selected.
 S I=DRG(DRGT,TDRG) W " ",$P(I,U,3),$S($P(I,U,4):" ("_$P(I,U,4)_")",1:""),!,"...OK" S %=1 D YN^DICN ; Q:%=1
...
GTADSOL ;If there're multiple ad/sol matched to an OI then display so user to select ad/sol
 ...
 ;S DIR("A")="Please select the following "_DRGTN_" 1 - "_+PSIVOI("DILIST",0)_": "
 ...
 . ;D DINIV^PSJDIN($S(PSIVOI="AD":52.6,1:52.7),+PSIVOIND)
 K PSIVOI
 Q
