PSIVPRO ;BIR/PR,MLM-PROFILE AN ORDER ;01 OCT 96 / 9:48 AM
 ;;5.0; INPATIENT MEDICATIONS ;**38,58,85**;16 DEC 97
 ;
...
PSIVST ;
 ...
 Q
REACT ;
 S PSJL="#       Additive",PSJL=$$SETSTR^VALM1("Last fill",PSJL,36,9)
 S PSJL=$$SETSTR^VALM1("Type",PSJL,53,4),PSJL=$$SETSTR^VALM1("Start   Stop    Stat",PSJL,59,20)
 D SETTMP^PSJLMPRI
HDL ; Display type heading.
 S PSJL=""
 D ACL:PSIVST="A",POL:PSIVST="P",NVL:PSIVST="N",NOL:PSIVST="X",NOC:PSIVST="" S X=""
...
ENPL ;
 ...
 . NEW X F X=0:0 S X=$O(DRUGNAME(X)) Q:'X  S:X>1 PSJL="" S PSJL=$$SETSTR^VALM1(DRUGNAME(X),PSJL,$S(X=1:9,1:10),$S(X=1:34,1:65)) D:X=1 V D SETTMP^PSJLMPRI
...
SOL ;
 ...
 . S:NAD=1 PSJL=$$SETSTR^VALM1("in",PSJL,9,11)
 . D NAME^PSIVUTL(DRG("SOL",NAD),L,.NAME,0)
 . F X=0:0 S X=$O(NAME(X)) Q:'X  S:(NAD>1!(X>1)) PSJL="" S PSJL=$$SETSTR^VALM1(NAME(X),PSJL,12,34) D:X=1&PSJNOAD V D SETTMP^PSJLMPRI
 . S PSJL=""
 Q
AD ;
 NEW NAME
 ...
 . F X=0:0 S X=$O(NAME(X)) Q:'X  S:(NAD>1!(X>1)) PSJL="" S PSJL=$$SETSTR^VALM1(NAME(X),PSJL,9,34) D:(NAD=1&(X=1)) V D SETTMP^PSJLMPRI
 . S PSJL=""
 Q
 ;
V S Y=$S(ON["V":$P($G(^PS(55,DFN,"IV",+ON,9)),U),1:"")
 ...
 E  S Y="**   N/P  **"
 S PSJL=$$SETSTR^VALM1(Y,PSJL,36,12)
 S PSJL=$$SETSTR^VALM1(" #"_$S(ON["V":+$P($G(^PS(55,DFN,"IV",+ON,9)),U,2),1:0),PSJL,49,3)
 S PSIVX2=PSIVX2+1
 D REST
 Q
ACL ;
 F X3=1:1:66 S PSJL=PSJL_"-" S:X3=34 PSJL=PSJL_" A c t i v e "
 Q
NVL ;
 F X3=1:1:66 S PSJL=PSJL_"-" S:X3=34 PSJL=PSJL_" N o n - V e r i f i e d "
 Q
POL ;
 F X3=1:1:66 S PSJL=PSJL_"-" S:X3=34 PSJL=PSJL_" P e n d i n g "
 Q
NOL ;
...
NOC ;
 ...
 Q
REST ;
 S PSJL=$$SETSTR^VALM1(P(4),PSJL,55,1)
 S PSJL=$$SETSTR^VALM1($E($$ENDTC^PSGMI(P(2)),1,5),PSJL,60,5)
 S PSJL=$$SETSTR^VALM1($E($$ENDTC^PSGMI(P(3)),1,5),PSJL,68,5)
 S PSJL=$$SETSTR^VALM1($S(P(17)="R"&(ON'["V"):"R/I",1:P(17)),PSJL,75,1)
 S PSJL=$$SETSTR^VALM1($S(ON["P":P("PRY"),1:""),PSJL,77,1)
 Q
XCHK ;
