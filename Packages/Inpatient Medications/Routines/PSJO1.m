PSJO1 ;BIR/CML3,PR-GET UNIT DOSE/IV ORDERS FOR INPATIENT ;15 May 98 / 9:28 AM
 ;;5.0; INPATIENT MEDICATIONS ;**3,47,56,58,109**;16 DEC 97
 ;
...
IVSET1 ;
 ...
 I P(17)="P" S STAT="C"_$S($P($G(^PS(53.1,+ON,.2)),U,5)="S":"A",$P($G(^(0)),U,24)="R":"C",1:"B")
...
EN(PSJPTYP) ; enter here
 ...
 F SD="I","N" S O=0 F  S O=$O(^PS(53.1,"AS",SD,PSGP,O)) Q:'O  S ON=+O_"P",X=$P($G(^PS(53.1,+O,0)),U,4) I $S(PSJPTYP=3:1,PSJPTYP=1&("FI"[X):0,1:1) D NVSET
 ...
 S O=0,SD="P" F  S O=$O(^PS(53.1,"AS",SD,PSGP,O)) Q:'O  S ON=O_"P",X=$P($G(^PS(53.1,+O,0)),U,4) I $S(PSJPTYP=3:1,PSJPTYP=1&("FI"[X):0,1:1) D @$S("FI"[X:"IVSET",1:"NVSET")
...
NVSET ; Set up orders from 53.1.
 ...
 S ST=$P($G(^PS(53.1,O,0)),U,7),START=-$P($G(^(2)),U,2) S:ST="" ST="z"
 S C=$S(SD="N":"BA",SD="I":"BB",$P($G(^PS(53.1,O,.2)),U,4)="S":"CA",$P($G(^(0)),U,24)="R":"CC",1:"CB")
