...
EN	; ENTER HERE TO BYPASS THE PRE-INIT PROGRAM
	S DIFQ=0,DIFQ(2)=0 K DIRUT,DTOUT,DUOUT
	F DIFRIR=1:1:2 S DIFRRTN="^IBINIT"_$E("56",DIFRIR) D @DIFRRTN
	W:1 !,"I AM GOING TO SET UP THE FOLLOWING FILES:" F I=1:2:174 S DIF(I)=^UTILITY("DIF",$J,I) D 1 G Q:DIFQ!$D(DIRUT) K DIF(I)
	S DIFROM="2.0" D PKG:'$D(DIFROM(0)),^IBINIT1 G Q:'$D(DIFQ) S DIK(0)="AB"
	F DIF=1:2:174 S %=^UTILITY("DIF",$J,DIF),DIK=$P(%,";",5),N=$P(%,";",3),D=$P(%,";",4)_U_N D D K DIFQ(N)
	...
	I DIFROM F DIF=1:2:174 S %=^UTILITY("DIF",$J,DIF),N=+$P(%,";",3) I N,$P(%,";",8)="y" S ^DD(N,0,"VR")=DIFROM
