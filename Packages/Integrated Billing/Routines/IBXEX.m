IBXEX ; GENERATED FROM 'IB NEW EXEMPTION' INPUT TEMPLATE(#2572), FILE 354.1;11/29/04
 D DE G BEGIN
DE S DIE="^IBA(354.1,",DIC=DIE,DP=354.1,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^IBA(354.1,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,2) S:%]"" DE(2)=% S %=$P(%Z,U,3) S:%]"" DE(3)=% S %=$P(%Z,U,4) S:%]"" DE(4)=% S %=$P(%Z,U,5) S:%]"" DE(5)=% S %=$P(%Z,U,6) S:%]"" DE(6)=% S %=$P(%Z,U,7) S:%]"" DE(7)=% S %=$P(%Z,U,8) S:%]"" DE(8)=%
 I  S %=$P(%Z,U,10) S:%]"" DE(9)=% S %=$P(%Z,U,11) S:%]"" DE(10)=% S %=$P(%Z,U,15) S:%]"" DE(12)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="IBXEX",DQ=1
 ...
 M DIEZAR=^DIE(2572,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2572,U="^"
...
7 S DW="0;7",DV="P200'I",DU="",DLB="USER ADDING ENTRY",DIFLD=.07
 ...
 G RD:X="@",Z
X7 Q
8 S DW="0;8",DV="RDI",DU="",DLB="DATE/TIME ADDED",DIFLD=.08
 S Y="NOW"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X8 S %DT="STXR" D ^%DT S X=Y K:Y<1 X
 Q
 ;
9 S DW="0;10",DV="R*S",DU="",DLB="ACTIVE",DIFLD=.1
 S DE(DW)="C9^IBXEX"
...
C9F1 Q
X9 Q
10 D:$D(DG)>9 F^DIE17,DE S DQ=10,DW="0;11",DV="FO",DU="",DLB="ELECTRONIC SIGNATURE",DIFLD=.11
 S DQ(10,2)="S Y(0)=Y S Y(0)=Y S Y=""<Hidden>"""
 S X=$G(IBASIG)
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD:X="@",Z
...
X11 I '$D(IBPRIOR) S Y="@99"
 Q
12 S DW="0;15",DV="D",DU="",DLB="PRIOR YEAR THRESHOLDS",DIFLD=.15
 S DE(DW)="C12^IBXEX"
 S X=IBPRIOR
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
