IBXFI5 ; GENERATED FROM 'IBDF EDIT DATA FIELD' INPUT TEMPLATE(#2578), FILE 357.5;11/29/04
 D DE G BEGIN
...
BEGIN S DNM="IBXFI5",DQ=1
 ...
 M DIEZAR=^DIE(2578,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2578,U="^"
...
X14 S:X="" Y="@4"
 Q
15 S DW="0;7",DV="FX",DU="",DLB="HOW SHOULD THE LABEL APPEAR? CHOOSE FROM {B,U}",DIFLD=.07
 G RE
X15 K:$L(X)>2!($L(X)<1)!("BU"'[$E(X,1))!("UB"'[$E(X,2)) X
 I $D(X),X'?.ANP K X
 Q
 ;
16 S DQ=17 ;@4
17 S DW="0;11",DV="NJ3,0XOR",DU="",DLB="TEXT STARTING ROW",DIFLD=.11
 S DQ(17,2)="S Y(0)=Y S:Y=+Y Y=Y+1"
 G RE
X17 S:+X=X X=X-1 K:+X'=X!(X>200)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
18 S DW="0;10",DV="NJ3,0XOR",DU="",DLB="TEXT STARTING COLUMN",DIFLD=.1
...
X26 S (IBY,IBX)=1
 Q
27 S D=0 K DE(1) ;2
 S DIFLD=2,DGO="^IBXFI51",DC="8^357.52I^2^",DV="357.52MF",DW="0;1",DOW="SUBFIELD LABEL",DLB="Select "_DOW S:D DC=DC_D
