...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=2,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.11)) S %Z=^(.11) S %=$P(%Z,U,1) S:%]"" DE(16)=%
...
C2F1 N X,X1,X2 S DIXR=19 D C2X1(U) K X2 M X2=X D C2X1("O") K X1 M X1=X
 D
...
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,D=0 K DE(1) ;1
 S DIFLD=1,DGO="^IBXSC12",DC="3^2.01^.01^",DV="2.01MFX",DW="0;1",DOW="ALIAS",DLB="Select "_DOW S:D DC=DC_D
...
C8F1 Q
X8 Q
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="0;5",DV="RP11'",DU="",DLB="MARITAL STATUS",DIFLD=.05
 S DE(DW)="C9^IBXSC11"
 S DU="DIC(11,"
 G RE
...
X11 S:IBDR20'["14" Y="@15"
 Q
12 D:$D(DG)>9 F^DIE17,DE S DQ=12,DW="VET;1",DV="RSX",DU="",DLB="VETERAN (Y/N)?",DIFLD=1901
 S DE(DW)="C12^IBXSC11"
 S DU="Y:YES;N:NO;"
 G RE
...
X15 S:IBDR20'["15" Y="@16"
 Q
16 D:$D(DG)>9 F^DIE17,DE S DQ=16,DW=".11;1",DV="Fa",DU="",DLB="STREET ADDRESS [LINE 1]",DIFLD=.111
 S DE(DW)="C16^IBXSC11",DE(DW,"INDEX")=1
 G RE
C16 G C16S:$D(DE(16))[0 K DB
 D ^IBXSC17
C16S S X="" G:DG(DQ)=X C16F1 K DB
 D ^IBXSC18
C16F1 N X,X1,X2 S DIXR=21 D C16X1(U) K X2 M X2=X D C16X1("O") K X1 M X1=X
 D
...
C16X1(DION) K X
 ...
 Q
C16F2 Q
X16 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>35!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
 ;
