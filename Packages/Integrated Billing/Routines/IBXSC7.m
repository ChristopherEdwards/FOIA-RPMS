IBXSC7 ; GENERATED FROM 'IB SCREEN7' INPUT TEMPLATE(#2569), FILE 399;11/29/04
 D DE G BEGIN
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,6) S:%]"" DE(8)=%
 I $D(^("U")) S %Z=^("U") S %=$P(%Z,U,1) S:%]"" DE(21)=% S %=$P(%Z,U,2) S:%]"" DE(22)=% S %=$P(%Z,U,3) S:%]"" DE(18)=% S %=$P(%Z,U,5) S:%]"" DE(13)=% S %=$P(%Z,U,6) S:%]"" DE(16)=% S %=$P(%Z,U,7) S:%]"" DE(15)=%
 I $D(^("U2")) S %Z=^("U2") S %=$P(%Z,U,2) S:%]"" DE(9)=% S %=$P(%Z,U,3) S:%]"" DE(10)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="IBXSC7",DQ=1
 ...
 M DIEZAR=^DIE(2569,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2569,U="^"
...
8 S DW="0;6",DV="RS",DU="",DLB="TIMEFRAME OF BILL",DIFLD=.06
 ...
 G RE
X8 Q
9 S DW="U2;2",DV="NJ3,0",DU="",DLB="COVERED DAYS",DIFLD=216
 G RE
X9 K:+X'=X!(X>999)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
10 S DW="U2;3",DV="NJ4,0",DU="",DLB="NON-COVERED DAYS",DIFLD=217
 G RE
X10 K:+X'=X!(X>9999)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
11 S DQ=12 ;@72
12 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=12 D X12 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X12 S:IBDR20'["72" Y="@73"
 Q
13 S DW="U;5",DV="RFOX",DU="",DLB="IS THIS A SENSITIVE RECORD?",DIFLD=155
...
X14 S:X=0 Y=156
 Q
15 S DW="U;7",DV="FOX",DU="",DLB="R.O.I. FORM(S) COMPLETED?",DIFLD=157
 S DQ(15,2)="S Y(0)=Y S Y=$S(Y:""YES"",Y=0:""NO"",1:"""")"
 G RE
X15 I $D(X) D YN^IBCU
 I $D(X),X'?.ANP K X
 Q
 ;
16 S DW="U;6",DV="RFOX",DU="",DLB="ASSIGNMENT OF BENEFITS",DIFLD=156
 S DQ(16,2)="S Y(0)=Y S Y=$S(Y:""YES"",Y=0:""NO"",1:"""")"
 G RE
X16 I $D(X) D YN^IBCU I $D(X) X:X=0 ^DD(399,156,9.3) K IBRATY
 I $D(X),X'?.ANP K X
 Q
 ;
...
X17 S:'$D(IBOX) Y="@73"
 Q
18 S DW="U;3",DV="RFOX",DU="",DLB="POWER OF ATTORNEY COMPLETED?",DIFLD=153
 S DQ(18,2)="S Y(0)=Y S Y=$S(Y:""YES"",Y=0:""NO"",1:"""")"
 G RE
X18 I $D(X) D YN^IBCU
 I $D(X),X'?.ANP K X
 Q
...
X20 S:IBDR20'["73" Y="@75"
 Q
21 S DW="U;1",DV="RDX",DU="",DLB="STATEMENT COVERS FROM",DIFLD=151
 S DE(DW)="C21^IBXSC7"
 G RE
...
C21F1 Q
X21 S %DT="ETP" D ^%DT S X=Y K:Y<1 X I $D(X) D DDAT^IBCU4 K IB00
 Q
 ;
22 D:$D(DG)>9 F^DIE17,DE S DQ=22,DW="U;2",DV="RDX",DU="",DLB="STATEMENT COVERS TO",DIFLD=152
 S DE(DW)="C22^IBXSC7"
 G RE
...
C22S S X="" G:DG(DQ)=X C22F1 K DB
 D ^IBXSC72
C22F1 Q
X22 S %DT="ETP" D ^%DT S X=Y K:Y<1 X I $D(X) D DDAT1^IBCU4 K IB00
 Q
 ;
23 S DQ=24 ;@75
24 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=24 D X24 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X24 S:IBDR20'["75" Y="@76"
 Q
25 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=25 D X25 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X25 D RCD^IBCU1
 Q
26 D:$D(DG)>9 F^DIE17,DE S DQ=26,D=0 K DE(1) ;42
 S DIFLD=42,DGO="^IBXSC73",DC="9^399.042PAI^RC^",DV="399.042MR*P399.2'X",DW="0;1",DOW="REVENUE CODE",DLB="Select "_DOW S:D DC=DC_D
 S DU="DGCR(399.2,"
 G RE:D I $D(DSC(399.042))#2,$P(DSC(399.042),"I $D(^UTILITY(",1)="" X DSC(399.042) S D=$O(^(0)) S:D="" D=-1 G M26
 S D=$S($D(^DGCR(399,DA,"RC",0)):$P(^(0),U,3,4),$O(^(0))'="":$O(^(0)),1:-1)
M26 I D>0 S DC=DC_D I $D(^DGCR(399,DA,"RC",+D,0)) S DE(26)=$P(^(0),U,1)
 G RE
R26 D DE
 S D=$S($D(^DGCR(399,DA,"RC",0)):$P(^(0),U,3,4),1:1) G 26+1
 ;
27 D:$D(DG)>9 F^DIE17 G ^IBXSC74
