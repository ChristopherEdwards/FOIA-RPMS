...
DE S DIE="^DGCR(399,D0,""RC"",",DIC=DIE,DP=399.042,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^DGCR(399,D0,"RC",DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(1)=% S %=$P(%Z,U,2) S:%]"" DE(2)=% S %=$P(%Z,U,3) S:%]"" DE(3)=% S %=$P(%Z,U,4) S:%]"" DE(4)=% S %=$P(%Z,U,5) S:%]"" DE(5)=% S %=$P(%Z,U,6) S:%]"" DE(7)=% S %=$P(%Z,U,7) S:%]"" DE(8)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="IBXSC73",DQ=1+D G B
1 S DW="0;1",DV="MR*P399.2'X",DU="",DLB="REVENUE CODE",DIFLD=.01
 S DE(DW)="C1^IBXSC73"
 S DU="DGCR(399.2,"
 G RE:'D S DQ=2 G 2
...
C1F1 Q
X1 S D="D",DIC("S")="I $P(^(0),""^"",3)" D IX^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
X6 I $P(^DGCR(399,DA(1),0),U,19)'=2 S Y="@758"
 Q
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW="0;6",DV="P81'",DU="",DLB="PROCEDURE",DIFLD=.06
 S DE(DW)="C7^IBXSC73"
 S DU="ICPT("
 G RE
