IBXST ; GENERATED FROM 'IB STATUS' INPUT TEMPLATE(#2562), FILE 399;11/29/04
 D DE G BEGIN
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 I $D(^("S")) S %Z=^("S") S %=$P(%Z,U,9) S:%]"" DE(4)=% S %=$P(%Z,U,12) S:%]"" DE(11)=% S %=$P(%Z,U,14) S:%]"" DE(14)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="IBXST",DQ=1
 ...
 M DIEZAR=^DIE(2562,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2562,U="^"
...
X10 I $P(^DGCR(399,DA,"S"),"^",12)]"" S Y="@94"
 Q
11 S DW="S;12",DV="RD",DU="",DLB="DATE FIRST PRINTED",DIFLD=12
 S DE(DW)="C11^IBXST"
 S X=DT
 S Y=X
...
C11F1 Q
X11 S %DT="TX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
...
X12 S Y="@99"
 Q
13 S DQ=14 ;@94
14 D:$D(DG)>9 F^DIE17,DE S DQ=14,DW="S;14",DV="D",DU="",DLB="DATE LAST PRINTED",DIFLD=14
 S DE(DW)="C14^IBXST"
 S X=DT
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
