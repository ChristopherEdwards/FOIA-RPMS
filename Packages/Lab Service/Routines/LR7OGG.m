LR7OGG ;SLC/STAFF- Interim report rpc grid ;6/23/97  15:34
...
GRIDDATA ;
 ...
 S NUM=1 F  S NUM=$O(^TMP("LR7OGX",$J,"INPUT",NUM)) Q:NUM<1  S TESTNUM=+^(NUM) D
 ...
 S DATACNT=0 F  S DATACNT=$O(^TMP("LR7OG",$J,"DATA",DATACNT)) Q:DATACNT<1  S LINE=^(DATACNT) D
 .S OUTCNT=OUTCNT+1,^TMP("LR7OGX",$J,"OUTPUT",OUTCNT)=LINE
 S OUTCNT=OUTCNT+1,ABLINE=OUTCNT
 S ^TMP("LR7OGX",$J,"OUTPUT",OUTCNT)="0^0^0"
 S (ABTCNT,ATSEQ)=0 F  S ATSEQ=$O(^TMP("LR7OG",$J,"ABTSEQ",ATSEQ)) Q:ATSEQ<1  D
 .S ABTCNT=ABTCNT+1
 .S $P(^TMP("LR7OG",$J,"ABTSEQ",ATSEQ),U)=ABTCNT
 .S OUTCNT=OUTCNT+1
 .S ^TMP("LR7OGX",$J,"OUTPUT",OUTCNT)=^TMP("LR7OG",$J,"ABTSEQ",ATSEQ)
 S (ABDCNT,ADSEQ)=0 F  S ADSEQ=$O(^TMP("LR7OG",$J,"ABDSEQ",ADSEQ)) Q:ADSEQ<1  D
 .S ABDCNT=ABDCNT+1
 .S $P(^TMP("LR7OG",$J,"ABDSEQ",ADSEQ),U)=ABDCNT
 .S OUTCNT=OUTCNT+1
 .S ^TMP("LR7OGX",$J,"OUTPUT",OUTCNT)=^TMP("LR7OG",$J,"ABDSEQ",ADSEQ)
 S (ABCNT,ADSEQ)=0 F  S ADSEQ=$O(^TMP("LR7OG",$J,"ABDATA",ADSEQ)) Q:ADSEQ<1  D
 .S ADCNT=+^TMP("LR7OG",$J,"ABDSEQ",ADSEQ)
 .S ATSEQ=0 F  S ATSEQ=$O(^TMP("LR7OG",$J,"ABDATA",ADSEQ,ATSEQ)) Q:ATSEQ<1  D
 ...
 S TESTSEQ=0  F  S TESTSEQ=$O(^TMP("LR7OG",$J,"TESTSPEC",TESTSEQ)) Q:TESTSEQ<1  D
 .S SPEC=0 F  S SPEC=$O(^TMP("LR7OG",$J,"TESTSPEC",TESTSEQ,SPEC)) Q:SPEC<1  S LINE=^(SPEC) D
 ..S OUTCNT=OUTCNT+1
 ..S ^TMP("LR7OGX",$J,"OUTPUT",OUTCNT)=LINE
 S $P(^TMP("LR7OGX",$J,"OUTPUT",1),U,5)=OUTCNT
 S NUM=0 F  S NUM=$O(^TMP("LR7OG",$J,"COMMENT",NUM)) Q:NUM<1  S LINE=^(NUM) D
 .S OUTCNT=OUTCNT+1
 .S ^TMP("LR7OGX",$J,"OUTPUT",OUTCNT)=LINE
 K ^TMP("LR7OG",$J)
 Q
