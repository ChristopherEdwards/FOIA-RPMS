LR7OU0 ;slc/dcm - HL7 Utilities/Conversions ;8/11/97
 ;;5.2;LR;**121,187,265,299,1022**;September 20, 2007
...
HL7DT(DATE) ; FM -> HL7 format
 ...
 ;Q X
FMDATE(DATE) ; EP - HL7 -> FM format
 Q $$HL7TFM^XLFDT(DATE)
 ;N X
 ;S X="" I DATE S X=$E(DATE,1,4)-1700_$E(DATE,5,8)_$S($L($E(DATE,9)):"."_$E(DATE,9,14),1:"")
 ;Q X
NMSPACE(PKG) ; EP - Returns pkg namespace
 N X S X=$P($G(^DIC(9.4,PKG,0)),"^",2)
 Q X
UVID(X,SPEC,NID,NSEC,NNAME,MSG,SS) ; EP - Set Universal ID
 ;X=Test ptr to 60
 ;NID=National ID
 ;NNAME=National Name
 ...
 Q X
SAMP(SAMPLE,SPECIMEN) ; EP - File 62,61 -> HL7 Source of Specimen code
 ...
 Q X
LRSAMP(SAMPLE) ;EP - HL7 -> File 62 sample format
 ;Sample=Source of Specimen code
 N X
 S X=$P(SAMPLE,";",4)
 Q X
LRSPEC(SAMPLE) ;EP - HL7 -> File 61 Specimen format
 ;Sample=Source of Specimen code
 N X
 ...
 Q X
ACTCODE(TYPE) ;EP - Lab Collection type -> HL7 Specimen Action Code
 ;TYPE=WC, LC, SP, I, 3, A
 N X
 S X=$S(TYPE="SP":1,TYPE="WC":"O",TYPE="I":2,TYPE=3:3,TYPE="A":"A",1:"L")
 Q X
LRACTCOD(TYPE) ;EP - HL7 Specimen Action Code -> Lab Collection type
 ;Type=1, 2, 3, A, O, L
 N X
 S X=$S(TYPE=1:"SP",TYPE=2:"I",TYPE=3:3,TYPE="A":"A",TYPE="O":"WC",1:"LC")
 Q X
URG(URGENCY) ;EP - Lab Urgency -> HL7 Priority code
 ...
 Q X
LRURG(URGENCY) ;EP - HL7 Priority -> Lab Urgency
 ;URGENCY=HL7 Priority code
 N X
 S X=$P(URGENCY,";",2)
 Q X
FLAG(FLAG) ; EP - Return HL7 Flag code
