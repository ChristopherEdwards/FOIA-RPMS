LRAPAUA ; IHS/DIR/AAB - AUTOPSY LIST 2/9/98 10:26 ; [ 07/08/1998 9:43 AM ]
...
SET Q:'$D(^LR(LRDFN,"AU"))!('$D(^(0))#2)  S Z=^("AU"),LRAC=$P(Z,U,6) Q:$P(LRAC," ")'=LRABV  S X=^(0),DFN=$P(X,"^",3),(LRDPF,X)=+$P(X,"^",2) Q:'X  S P(0)=$P(^DIC(X,0),"^"),X=^DIC(X,0,"GL") D PT^LRX Q:$G(VAERR)  S LRP=PNM
 ...
 I $L(LRP),LRDFN S ^TMP($J,LRP,LRDFN)=HRCN_"^"_+Z_"^"_LRAC_"^"_$S(P(0)="PATIENT":"",1:P(0))  ;IHS/ANMC/CLS 11/1/95
 Q
 ;
...
H I $D(LR("F")),IOST?1"C".E D M^LRU Q:LR("Q")
 ...
 D F^LRU W !,LRO(68)," list from ",LRSTR," to ",LRLST,!,"Patient",?35,"HRCN",?46,"Autopsy#",?58,"Autopsy Date",!,LR("%") Q  ;IHS/ANMC/CLS 11/1/95
 ;
