LRAPPF ; IHS/DIR/AAB - ANATOMIC PATH FILE SORT 8/13/95 21:59 ;
...
P I '$D(^LR(LRDFN,0)) K ^LR(LRXR,LRSDT,LRDFN) Q
 ...
 ;S F=$P(^LR(LRDFN,0),"^",2),N=$P(^(0),"^",3),F(1)=$S(F=2:"",1:$P(^DIC(F,0),"^")),X=^DIC(F,0,"GL"),N=@(X_N_",0)"),N(1)=$P(N,"^"),N(9)=$P(N,"^",9)
 S F=$P(^LR(LRDFN,0),"^",2),(DFN,N)=$P(^(0),"^",3),F(1)=$S(F=2:"",1:$P(^DIC(F,0),"^")),X=^DIC(F,0,"GL"),N=@(X_N_",0)"),N(1)=$P(N,"^"),(SSN,N(9))=$P(N,"^",9) D SSN^LRU  ;IHS/ANMC/CLS 11/1/95
...
GL ;S ^TMP($J,F,N(1),LRDFN,LRI)="",^TMP($J,"S",H(2),N)=F(1)_"^"_N(1)_"^"_N(9) Q
 S ^TMP($J,F,N(1),LRDFN,LRI)="",^TMP($J,"S",H(2),N)=F(1)_"^"_N(1)_"^"_HRCN Q  ;IHS/ANMC/CLS 11/1/95
 ;
END D V^LRU Q
