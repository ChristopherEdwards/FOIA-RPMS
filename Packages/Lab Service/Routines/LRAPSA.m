...
W ;S X=^LR(LRDFN,0),LRA(9)=$S(LRSS'="AU":^(LRSS,LRI,0),1:^("AU")),LRTK=+LRA(9),Y=$P(X,"^",3),(LRDPF,X)=$P(X,"^",2),X=^DIC(X,0,"GL"),X=@(X_Y_",0)"),LRP=$P(X,"^"),SSN=$P(X,"^",9) D SSN^LRU
 S X=^LR(LRDFN,0),LRA(9)=$S(LRSS'="AU":^(LRSS,LRI,0),1:^("AU")),LRTK=+LRA(9),(DFN,Y)=$P(X,"^",3),(LRDPF,X)=$P(X,"^",2),X=^DIC(X,0,"GL"),X=@(X_Y_",0)"),LRP=$P(X,"^"),SSN=$P(X,"^",9) D SSN^LRU  ;IHS/ANMC/CLS 11/1/95
 ...
 D:$Y>(IOSL-4) H Q:LR("Q")  W !!,LRAN,?16,LRA(6),"  ",LRP," ",HRCN S LRW=$S(LRA(6)'[1700:LRA(6),1:"") I LRSS="AU" D AU Q  ;IHS/ANMC/CLS 11/1/95
...
H1 ;D H Q:LR("Q")  W !!,LRAN,?16,LRA(6),"  ",LRP," ",SSN Q
 D H Q:LR("Q")  W !!,LRAN,?16,LRA(6),"  ",LRP," ",HRCN Q  ;IHS/ANMC/CLS 11/1/95
