LRAPTT ; IHS/DIR/AAB - TURNAROUND TIME PATH 2/13/98 09:36 ; [ 07/09/1998 9:14 AM ]
 ;;5.2;LR;**1002,1006**;SEP 01, 1998
 ;
 ;;5.2;LAB SERVICE;**1,72,201**;Sep 27, 1994
 D ^LRAP Q:'$D(Y)
...
S D ^LRUP Q:$G(LREND)  S LRX=P("F") S:'$D(LRF(LRX))#2 LRF(LRX)=0
 S:LRC LRF=LRF+1,LRF(LRX)=LRF(LRX)+1 S LRM=LRM+1,LRM(LRX)=LRM(LRX)+1,X1=LRC,X2=LRR D ^%DTC S:X=0 X="<1" S LRT=X I X>2 S LRY=X-1,Y=0,X=$P(LRR,".") D D
 ...
 S:'LRR LRR="?" S ^TMP($J,$E(LRR,1,3),Z)=H(5)_U_LRP_U_HRCN_U_H("F")_U_H(9)_U_LRT_U_H(4)_U_X_U_LRD S:LRC LRA=LRA+1,LRA(LRX)=LRA(LRX)+1 Q  ;IHS/ANMC/CLS 11/1/95
