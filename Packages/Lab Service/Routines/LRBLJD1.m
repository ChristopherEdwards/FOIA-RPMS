LRBLJD1 ; IHS/DIR/AAB - POOL COMPONENTS 10/9/97 07:24 ; [ 04/29/98 10:11 AM ]
 ...
 ;;5.2;LAB SERVICE;**25,90**;Sep 27, 1994
 ...
 W !!,"Pool will contain the following ",$P(^LAB(66,LRE(4),0),"^")," units:",!?3,"ID #",?22,"Expiration date"
 S A=0 F B=0:1 S A=$O(LRP(A)) Q:'A  W !,$J(A,2),?3,$P(LRP(A),"^",2),?15,$P(LRP(A),"^",3),?22 S Y=$P(LRP(A),"^",4) D D^LRU W Y
 S LRM=LRM*B W !!,"ALL OK " S %=1 D YN^LRU I %'=1 D K K LRP Q
ID R !!,"Select UNIT ID number for POOL: ",X:DTIME I X=""!(X[U) W !,$C(7),"UNITS selected were NOT pooled !",! D K Q
 ...
 D S Q
C W !,"Selection ",N," (Unit ID to pool): " R X:DTIME I X=""!(X[U) K Y Q
 I LR,$A(X)<58,$A(X)>47 D ^LRBLBU G:'$D(X) C
