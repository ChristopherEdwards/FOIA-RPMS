LRBLPC1 ; IHS/DIR/FJE - PT ADM,RX SPECIALTY,ICD9CM CODES 11/18/91 20:36 ;
 ;;5.2;LR;;NOV 01, 1997
 ;
...
A S Y=$S($D(^DGPM(LRX,0)):^(0),1:""),LR=$P(Y,"^",16) W !,"Adm:",+$E(Y,4,5)_"/"_+$E(Y,6,7)_"/"_$E(Y,2,3) S Z=$P(Y,"^",17) I Z S Z=$S($D(^DGPM(Z,0)):+^(0),1:"") W ?13,"Discharge:",+$E(Z,4,5)_"/"_+$E(Z,6,7)_"/"_$E(Z,2,3) ;MAS
 ...
 F C=0:0 S C=$O(LRF(C)) Q:'C!(LR("Q"))  I $D(^ICD9(C)) S W=^(C,0) D:$Y>(IOSL-9) H Q:LR("Q")  W !,$P(W,"^"),?10,$P(W,"^",3)
 F C=0:0 S C=$O(LRC(C)) Q:'C!(LR("Q"))  I $D(^ICD0(C)) S W=^(C,0) D:$Y>(IOSL-9) H Q:LR("Q")  W !,$P(W,"^"),?10,$P(W,"^",4)
 Q
...
H I $D(LR("D")) D H2^LRBLTXA Q
 ...
 ;
SET K ^LRO(69.2,LRAA,7,DUZ) L +^LRO(69.2,LRAA,7,0) S X=^LRO(69.2,LRAA,7,0),X(1)=$O(^(0)),^(0)=$P(X,"^",1,2)_"^"_X(1)_"^"_($P(X,"^",4)-1) L -^LRO(69.2,LRAA,7,0) Q
