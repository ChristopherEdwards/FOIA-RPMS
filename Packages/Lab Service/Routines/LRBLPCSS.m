LRBLPCSS ; IHS/DIR/FJE - PRE-OP COMPONENT SELECTION 11/7/94 13:50 ;
 ;;5.2;LR;;NOV 01, 1997
 ;
...
P W !!,"Select OPERATION (1-",B,"): " R X:DTIME Q:X["^"!(X="")  I X<1!(X>B)!(+X'=X) W $C(7),!,"Select a number from 1 to ",B G P
 D B Q
B W "  ",$P(A(X),"^"),!,$P(A(X),"^",2) S X=$P(A(X),"^",3) I X,$D(^ICPT(X,0)) S LRCPT=X W !,"CPT file number: ",LRCPT F Y=0:0 S Y=$O(^ICPT(X,"D",Y)) Q:'Y  W !,^(Y,0)
...
A Q:'$D(^ICPT(0))  W ! S DIC="^ICPT(",DIC(0)="AEQMZ",DIC("A")="Select OPERATION: ",DIC("S")="I $P(^(0),""^"",3),$P(^DIC(81.1,$P(^DIC(81.1,$P(^ICPT(Y,0),""^"",3),0),""^"",3),0),""^"")=""SURGERY""" D ^DIC K DIC Q:Y<1  S X=+Y
 ...
 W !,"CPT file number: ",X F Z=0:0 S Z=$O(^ICPT(X,"D",Z)) Q:'Z  W !,^(Z,0)
 S LRCPT=X D C Q
 ;
SET ; also from MSB^LRBLS
 L +^LAB(66.5) S DA=X,^LAB(66.5,X,0)=X,Z=^LAB(66.5,0),^(0)=$P(Z,"^",1,2)_"^"_X_"^"_($P(Z,"^",4)+1) L -^LAB(66.5) X:$D(^DD(66.5,.01,1,1,1)) ^(1) Q
EN ;
 ...
 S:$D(LRR) LRK(C)="",LRK(C,1)="MSBOS:"_A_" operation: "_$P(^ICPT(LRCPT,0),"^",2) Q
 ;
