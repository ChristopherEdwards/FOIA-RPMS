...
LOOP ;K ^TMP($J) S LRQ=0,SSN=$P(^LRO(69.2,LRAA,7,DUZ,1,LRDFN,0),"^",10),X=^LR(LRDFN,0),LRPABO=$P(X,"^",5),LRPRH=$P(X,"^",6),LRDPF=$P(X,U,2) D H S LR("F")=1
 K ^TMP($J) S LRQ=0,SSN=$P(^LRO(69.2,LRAA,7,DUZ,1,LRDFN,0),"^",10),X=^LR(LRDFN,0),LRPABO=$P(X,"^",5),LRPRH=$P(X,"^",6),LRDPF=$P(X,U,2),DFN=$P(X,U,3) D SSN^LRU D H S LR("F")=1  ;IHS/ANMC/CLS 11/1/95
...
H I $D(LR("F")),IOST?1"C".E D M^LRU Q:LR("Q")
 ...
 D F^LRU W !,"TRANSFUSION SERVICE/BLOOD BANK REPORT from ",LRSTR," to ",LRLST,!,"PATIENT: ",LRP," ",HRCN," ",$J(LRPABO,2)," ",LRPRH  ;IHS/ANMC/CLS 11/1/95
