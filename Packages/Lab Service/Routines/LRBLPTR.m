...
QUE U IO K ^TMP($J),^TMP("LRBL",$J) D L^LRU,S^LRU,H S LR("F")=1
 ...
 S LRP=0 F  S LRP=$O(^TMP($J,"B",LRP)) Q:LRP=""!(LR("Q"))  F P=0:0 S P=$O(^TMP($J,"B",LRP,P)) Q:'P!(LR("Q"))  S HRCN=^(P) D W  ;IHS/ANMC/CLS 11/1/95
...
W ;D:$Y>(IOSL-6) H Q:LR("Q")  S (LRE,LRF)="" W !!,"Patient: ",LRP,?41,"SSN: ",SSN D:$O(^LR(P,1.9,0)) B Q:LR("Q")
 D:$Y>(IOSL-6) H Q:LR("Q")  S (LRE,LRF)="" W !!,"Patient: ",LRP,?41,"HRCN: ",HRCN D:$O(^LR(P,1.9,0)) B Q:LR("Q")  ;IHS/ANMC/CLS 11/1/95
...
W1 F LRI=0:0 S LRI=$O(^TMP($J,P,C,LRD,LRI)) Q:'LRI!(LR("Q"))  S W=^(LRI) D P
 ...
 Q
P D:$Y>(IOSL-6) H1 Q:LR("Q")  W !,$P(W,"^"),?13,C(1),?18,$P(W,"^",8),?23,$P(W,"^",9),?28,$P(W,"^",2),?43,$P(W,"^",4),?46,$P(W,"^",5),?52,$P(W,"^",6),?62,$P(W,"^",3),?77,$P(W,"^",7)
...
PT ;S X=^LR(P,0),Y=$P(X,"^",3),LRDPF=$P(X,U,2),X=^DIC(LRDPF,0,"GL"),Y=@(X_Y_",0)"),LRP=$P(Y,"^"),SSN=$P(Y,"^",9) D SSN^LRU S ^TMP($J,"B",LRP,P)=SSN Q
 S X=^LR(P,0),(DFN,Y)=$P(X,"^",3),LRDPF=$P(X,U,2),X=^DIC(LRDPF,0,"GL"),Y=@(X_Y_",0)"),LRP=$P(Y,"^"),SSN=$P(Y,"^",9) D SSN^LRU S ^TMP($J,"B",LRP,P)=HRCN Q  ;IHS/ANMC/CLS 11/1/95
...
H I $D(LR("F")),IOST?1"C".E D M^LRU Q:LR("Q")
 ...
 W !,"Unit ID",?13,"Comp",?19,"(#)",?23,"(ml)",?28,"Relocated",?43,"CK",?46,"By",?52,"Location",?62,"Transfused",?77,"RXN"
 W !,LR("%") Q
H1 ;D H Q:LR("Q")  W !!,"Patient: ",LRP,?41,"SSN: ",SSN Q
 D H Q:LR("Q")  W !!,"Patient: ",LRP,?41,"HRCN: ",HRCN Q  ;IHS/ANMC/CLS 11/1/95
