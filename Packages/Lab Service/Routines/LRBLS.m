LRBLS ; IHS/DIR/FJE - BLOOD BANK SUPERVISOR OPTS 3/9/94 09:42 ;
 ;;5.2;LR;;NOV 01, 1997
 ;
...
MSB ;max surg blood order edit
 ...
 D:'$D(^LAB(66.5,X,0)) SET^LRBLPCSS F A=0:0 S A=$O(^ICPT(X,"D",A)) Q:'A  W !,^(A,0)
...
SNO W ! S DIC="^LAB(61.3,",DIC(0)="AEMOQ",DIC("S")="I $P(^(0),U,5)=""AN""!($P(^(0),U,5)=""AB"")",DIC("A")="Select ANTIGEN or ANTIBODY: " D ^DIC K DIC G:Y<1 END
 S DA=+Y,DR=".04;.06;7;5",DIE=61.3 D ^DIE K DA,DIE,DR,DIC G SNO
...
COM W ! S (DIC,DIE)="^LAB(66,",DIC(0)="AEQLM",DLAYGO=66 D ^DIC K DIC,DLAYGO G:X=""!(X[U) END S DA=+Y,LR=$S($P(Y,U,2)["PEDIATRIC":1,1:0),DR=".01:.19;S:LR Y=.23;.21;.22;.23:.28;1;2;3:999" D ^DIE K DA,DR,DIE,DIC G COM
...
ASK W ! S DIC="^LAB(69.9,1,8,",DIC(0)="AEQM",DIC("A")="Select BLOOD BANK DEFAULT OPTION: " D ^DIC K DIC G:Y<1 END
 S DA=+Y,DIE="^LAB(69.9,1,8,",DR=".02:.05;.09" D ^DIE G ASK
 ;
