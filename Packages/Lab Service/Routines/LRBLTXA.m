LRBLTXA ; IHS/DIR/FJE - TRANSFUSION FOLLOW-UP 2/18/93 09:55 ;
 ;;5.2;LR;;NOV 01, 1997
 ;
...
QUE U IO K ^TMP($J) S LR("D")=0,Z(2)=$O(^LAB(61,"B","SERUM",0)),Z(3)=$O(^LAB(61,"B","BLOOD",0)),Z(5)=$O(^LAB(61,"B","PLASMA",0)) D L^LRU,S^LRU,H S LR("F")=1
 ...
 S LRP=0 F A=0:0 S LRP=$O(^TMP($J,LRP)) Q:LRP=""!(LR("Q"))  F LRDFN=0:0 S LRDFN=$O(^TMP($J,LRP,LRDFN)) Q:'LRDFN!(LR("Q"))  S LRP(1)=^(LRDFN),SSN=$P(LRP(1),"^"),LRDPF=$P(^LR(LRDFN,0),U,2),DFN=$P(^(0),U,3) D SSN^LRU,T  ;IHS/ANMC/CLS 11/1/95
...
M S J(2)=1,W(1)=W(1)+1,DFN=$P(LRP(1),"^",3),W(4)=$P(LRP(1),"^",4) D:$Y>(IOSL-6) H1 Q:LR("Q")
 ...
 W:W(1)=1 !,LRP," HRCN:",HRCN," Loc: ",$P(LRP(1),"^",2) D:W(1)=1 A,C W !,T(1) W:T'=Z(2)&(T'=Z(3))&(T'=Z(5)) ?13,$E($P(^LAB(61,T,0),"^"),1,7)  ;IHS/ANMC/CLS 11/1/95
...
H1 ;D H Q:LR("Q")  W:W(1)>1 !,LRP," SSN:",SSN," Loc: ",$P(LRP(1),"^",2) D:W(1)>1 C Q
 D H Q:LR("Q")  W:W(1)>1 !,LRP," HRCN:",HRCN," Loc: ",$P(LRP(1),"^",2) D:W(1)>1 C Q  ;IHS/ANMC/CLS 11/1/95
H2 ;D H Q:LR("Q")  W !,LRP," SSN:",SSN," Loc: ",$P(LRP(1),"^",2) Q
 D H Q:LR("Q")  W !,LRP," HRCN:",HRCN," Loc: ",$P(LRP(1),"^",2) Q  ;IHS/ANMC/CLS 11/1/95
