LRORD ;VA/DALOI/CJS - LAZY ACCESSION LOGGING ; 22-Oct-2013 09:22 ; MKK
 ;;5.2;LAB SERVICE;**1010,1011,1012,100,1013,1014,121,153,1015,286,1027,291,1030,1033**;NOV 01, 1997
 ;
 ; IHS Patch 1033:
 ;      (1) Restores IHS/DIR/MJL PATCH 1008 line that comments out line G0+12
 ;      (2) Repaced code in % subroutine so that it uses FileMan DIR call
 ; 
 D ENTRYAUD^BLRUTIL("^LRORD 0.0")
 ...
 ;
EN ;; EP
 D ENTRYAUD^BLRUTIL("EN^LRORD 0.0")
 ...
 . W $C(7),!,"ROLLOVER ",$S($P(^LAB(69.9,1,"RO"),U,2):"IS RUNNING.",1:"HAS NOT RUN.")
 ...
 ;
EN1 ;;EP - from LROR4
 D ENTRYAUD^BLRUTIL("EN1^LRORD 0.0")
 D ^LRPARAM
 ...
 I '$G(LRECT),LRORDR=""  D %("WANT TO ENTER COLLECTION TIMES","YES")  S LRECT='(%["N") G KILL:$E(%)="^"     ; IHS/MSC/MKK - LR*5.2*1033
 ...
 ;
L5 ; EP 
 D ENTRYAUD^BLRUTIL("L5^LRORD 0.0")
 ...
 ;
G0 ; EP
 D ENTRYAUD^BLRUTIL("G0^LRORD 0.0")
 ...
 ; 
 ; Following commented out for IHS.  Originally done for LR*5.2*1008.  Comment accidently left off
 ; in LR*5.2*1030, but restored with LR*5.2*1033
 ; I LRWP>40 S LRFIRST=0 ; Don't automatically display "long" test lists. ; IHS/DIR/MJL PATCH 1008
 G G5
 ;
G1 ; EP
 ...
 ;
GET ; EP 
 D ENTRYAUD^BLRUTIL("GET^LRORD 0.0")
 D Q15^LRORD2
 ...
 D ENTRYAUD^BLRUTIL("GET^LRORD 9.0")
 G GET
 ;
G5 ; EP
 D ENTRYAUD^BLRUTIL("G5^LRORD 0.0")
 G KILL:LRWP<1
...
EN01 ;EP - LAB COLLECT ORDER ENTRY
 D ENTRYAUD^BLRUTIL("EN01^LRORD 0.0")
 ;
ORDER ; EP
 D ENTRYAUD^BLRUTIL("ORDER^LRORD 0.0")
...
SENDPAT ; EP - S %=2 W !,"Do you want copies of the orders" D YN^DICN Q:%=-1  S:%=1 LRSLIP="" I %=0 D QUIZ G SENDPAT
 D ENTRYAUD^BLRUTIL("SENDPAT^LRORD 0.0")
...
IMMCOL ;EP - IMMEDIATE LAB COLLECTION
 D ENTRYAUD^BLRUTIL("IMMCOL^LRORD 0.0")
...
WARDCOL ; EP
 D ENTRYAUD^BLRUTIL("WARDCOL^LRORD 0.0")
 S %=2
 W !,"Do you want copies of the orders" D YN^DICN Q:%=-1  S:%=1 LRSLIP="" I %=0 D QUIZ G WARDCOL
 S LRORDR="WC"
 D ENTRYAUD^BLRUTIL("ENTER WARDCOL^LRORD","BLROPT")      ; <<<<<<< DEBUG
 S:$G(BLROPT)=""!($G(BLROPT(0))'=$P(XQY0,U)) BLROPT="FASTORD",BLROPT(0)=$P(XQY0,U)  ;IHS/OIRM TUC/AAB 2/13/97  -- IHS/OIT/MKK -- LR*5.2*1027 -- Restored line
 D LRORD
 D ENTRYAUD^BLRUTIL("WARDCOL^LRORD 9.0")
 Q
 ;
LEDI ; EP - Laboratory Electronic Data Exchange
 D ENTRYAUD^BLRUTIL("LEDI^LRORD 0.0")
 ...
 D LRORD,^LRORDK
 D ENTRYAUD^BLRUTIL("LEDI^LRORD 9.0")
 Q
 ;
 ...
 ;
QUIZ ; EP
 D ENTRYAUD^BLRUTIL("QUIZ^LRORD 0.0")
...
START(RESULT,BPCPARAM)  ;EP
 D ENTRYAUD^BLRUTIL("START^LRORD 0.0")
 ...
 D ENTRYAUD^BLRUTIL("START^LRORD 9.0")
 Q
 ; ----- END IHS/OIT/MKK - LR*5.2*1027 
