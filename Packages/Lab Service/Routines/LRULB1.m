...
W S A=0 I $D(^LRO(68,LRAA,1,LRAD,1,LRAN,4,0))#2 S Z=0 F A=0:1 S Z=$O(^LRO(68,LRAA,1,LRAD,1,LRAN,4,Z)) Q:'Z!(LR("Q"))  S X=+^(Z,0) D LIST
 ...
 W:A<1 !?8,HRCN,?27,LRLLOC W:$L(LRC(5)) !,LRC(5) W !!,LR("%") Q  ;IHS/ANMC/CLS 08/18/96
LIST ;S X=$S($D(^LAB(60,X,0)):$P(^(0),"^"),1:"??") W:A=0 !?7,SSN,?27,LRLLOC W:A>0 ! W ?50,$E(X,1,30)
 S X=$S($D(^LAB(60,X,0)):$P(^(0),"^"),1:"??") W:A=0 !?7,HRCN,?27,LRLLOC W:A>0 ! W ?50,$E(X,1,30)  ;IHS/ANMC/CLS 08/18/96
 Q
...
H I $D(LR("F")),IOST?1"C".E D M^LRU Q:LR("Q")
 ...
 W !,"#= Not PATIENT file",?48,"|-------Date/time----|",!,"Acc no",?14,"Name",?24,"Spec/Sample",?39,"Results",?48,"Received",?65,"Taken",!?14,"HRCN",?24,"Location",?58,"Tests"  ;IHS/ANMC/CLS 08/18/96
 W !,LR("%") Q
