...
K ;S Y=^LRO(69.2,LRAA,7,DUZ,1,LRB,0) W !,$P(Y,"^",2),"  SSN:",$P(Y,U,10)," deleted."
 S Y=^LRO(69.2,LRAA,7,DUZ,1,LRB,0) W !,$P(Y,"^",2),"  HRCN:",$P(Y,U,10)," deleted."  ;IHS/ANMC/CLS 08/18/96
...
P ;Q:'$O(^LRO(69.2,LRAA,7,DUZ,1,0))  K L W ! S DIC="^LRO(69.2,LRAA,7,DUZ,1,",DIC(0)="AEQ",DIC("A")="Select patient to delete: ",DIC("W")="W ""  SSN:"",$P(^(0),U,10)",D="C" D IX^DIC K DIC Q:Y<1  S LRB=+Y D K G P
 Q:'$O(^LRO(69.2,LRAA,7,DUZ,1,0))  K L W ! S DIC="^LRO(69.2,LRAA,7,DUZ,1,",DIC(0)="AEQ",DIC("A")="Select patient to delete: ",DIC("W")="W ""  HRCN:"",$P(^(0),U,10)",D="C" D IX^DIC K DIC Q:Y<1  S LRB=+Y D K G P  ;IHS/ANMC/CLS 08/18/96
