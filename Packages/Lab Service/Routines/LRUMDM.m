...
I I LRA]"" Q:'$D(^LRO(69.2,LRAA,7,DUZ,1,LRDFN,1))  Q:LRA'=^(1)
 ...
 S X=^LR(LRDFN,0),(DFN,Y)=$P(X,"^",3),(LRDPF,X)=$P(X,"^",2),X=^DIC(X,0,"GL"),V=@(X_Y_",0)"),LRP=$P(V,"^"),SSN=$P(V,"^",9),LRL=$S($D(@(X_Y_".1)")):^(.1),$D(^LR(LRDFN,.1)):^(.1),1:"") D SSN^LRU  ;IHS/ANMC/CLS 08/18/96
 ;D:$Y>(IOSL-6)!($D(LRK)) H Q:LR("Q")  W !,"SSN:",SSN,?19,"LOC:",LRL,?44,"Patient: ",LRP D T
 D:$Y>(IOSL-6)!($D(LRK)) H Q:LR("Q")  W !,"HRCN:",HRCN,?19,"LOC:",LRL,?44,"Patient: ",LRP D T  ;IHS/ANMC/CLS 08/18/96
 Q:LR("Q")  W !,LR("%1") Q
...
H I $D(LR("F")),IOST?1"C".E D M^LRU Q:LR("Q")
 ...
 D F^LRU W !,"List for: ",$P(^VA(200,DUZ,0),"^") W:LRA]"" ?40,"PT GRP: ",LRA W:LRE ?40,LRE(1) W:IOST'?1"C".E !,"Work copy- DO NOT PUT IN PATIENT'S CHART" W !,LR("%") Q  ;IHS/ANMC/CLS 08/18/96
H1 ;D H Q:LR("Q")  I W(1)>1 W !,"SSN:",SSN,?19,"LOC:",LRL,?44,"Patient: ",LRP
 D H Q:LR("Q")  I W(1)>1 W !,"HRCN:",HRCN,?19,"LOC:",LRL,?44,"Patient: ",LRP  ;IHS/ANMC/CLS 08/18/96
