LRUPQ1 ; IHS/DIR/FJE - LAB RESULTS BY ACCESSION AREA (COND'T) 3/8/94 09:03 ;
 ;;5.2;LR;;NOV 01, 1997
 ;
 ...
 D:$Y>(IOSL-6) H^LRUPQ Q:LR("Q")  W !,$J(N,5) S X=^LR(LRDFN,0),(DFN,Y)=$P(X,"^",3),(LRDPF,X)=$P(X,"^",2),P(0)=$P(^DIC(X,0),"^"),X=^DIC(X,0,"GL")  ;IHS/ANMC/CLS 08/18/96
 S X=@(X_Y_",0)"),SSN=$P(X,"^",9),LRP=$P(X,"^") D SSN^LRU
 ;W ?7 W:P(0)'="PATIENT" "#" W $E(LRP,1,20),?28,SSN(1),?34,$E(A(7),1,5)
 W ?7 W:P(0)'="VA PATIENT" "#" W $E(LRP,1,18),?27,HRCN,?34,$E(A(7),1,5)  ;IHS/ANMC/CLS/ 08/18/96
...
H ;D H^LRUPQ Q:LR("Q")  W !,LR(4),!,LRP," ",SSN," (continued from pg ",LRQ-1,")" Q
 D H^LRUPQ Q:LR("Q")  W !,LR(4),!,LRP," ",HRCN," (continued from pg ",LRQ-1,")" Q  ;IHS/ANMC/CLS 08/18/96
