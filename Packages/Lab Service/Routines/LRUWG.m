...
ST R !!,"Start from accn #: ",N(1):DTIME G:N(1)=""!(N(1)[U) END G:N(1)'?1N.N ST
 ...
 S ZTRTN="QUE^LRUWG" D BEG^LRUTL G:POP!($D(ZTSK)) END  ;IHS/ANMC/CLS 08/18/96
...
L W !,$J(Z,3),") ",$J($S(T(5)>.999:T(5),1:T(5)*1000),5),?14,$P(N,"^",6) W:$L($P(N,"^",6))>21 !
 ...
 S X=$P(N,"^",5) W ?29,N(7),?41,$S(X:$P($G(^VA(200,X,0)),"^",2),1:X),?45,$P(N,"^",2),?52,$E($P(N,"^"),1,20),?73,$P(N,"^",3),!?13 F X=1:1:IOM-13 W "-"  ;IHS/ANMC/CLS 08/18/96
 Q
H I $D(LR("F")),IOST?1"C".E D M^LRU Q:LR("Q")
 D F^LRU W !,^TMP($J,W,0)," Worklist",?40,"(* = STAT)"
 ;W !,"COUNT",?6,"ACC#",?17,"RESULT",?37,"Completed",?49,"Tech",?54,"ID",?60,"PATIENT",!,LR("%"),! Q
 W !,"COUNT",?6,"ACC#",?14,"RESULT",?29,"Completed",?40,"Tech",?45,"ID",?52,"PATIENT",?72,"LOCATION",!,LR("%"),! Q  ;IHS/ANMC/CLS 08/18/96
...
TT F T=0:0 S T=$O(^LRO(68,W,1,LRAD,1,N,4,T)) Q:'T!(T=F)
 ...
 S X=^LR(LRDFN,0),(DFN,Y)=$P(X,"^",3),LRDPF=$P(X,U,2),(X,LRPF)=^DIC($P(X,"^",2),0,"GL"),X=@(X_Y_",0)"),LRP=$P(X,"^"),SSN=$P(X,"^",9) D SSN^LRU  ;IHS/ANMC/CLS 08/18/96
...
STF ;
 ...
 I LRPF="^LAB(62.3," S C(6)=N,N=N/1000,^TMP($J,W,1,T,N)=LRP_"^"_HRCN_"^"_LRLLOC_"^"_T(1)_"^"_V_"^"_T(4)_"^"_T(6),N=C(6) Q  ;IHS/ANMC/CLS 08/18/96
 ;S ^TMP($J,W,1,T,N)=LRP_"^"_SSN(1)_"^"_LRLLOC_"^"_T(1)_"^"_V_"^"_T(4)_"^"_T(6) Q
 S ^TMP($J,W,1,T,N)=LRP_"^"_HRCN_"^"_LRLLOC_"^"_T(1)_"^"_V_"^"_T(4)_"^"_T(6) Q  ;IHS/ANMC/CLS 08/18/96
