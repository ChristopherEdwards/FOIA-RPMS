LEXAMD ; ISL Look-up Modifiers                    ; 10-15-97
 ;;2.0;LEXICON UTILITY;**6**;Sep 23, 1996
 ;
 ; LEXX  IEN file 757.01 of an expression w/Modifiers
 ;
EN(LEXX) ; Look-up Modifiers
 S LEXX=+($G(LEXX)) Q:+($G(^TMP("LEXSCH",$J,"MOD",0)))=0  Q:+($G(LEXX))'>2  Q:'$D(^LEX(757.01,+($G(LEXX)),0))  Q:+($P($G(^LEX(757.01,+LEXX,1)),"^",6))=0  Q:'$D(^LEX(757.01,"APAR",LEXX))  N LEXXN D ARY
 Q
ARY ; Build Array of Modified Terms
 N LEXLVL,LEXO,LEXI,LEXN,LEXA,LEXT,LEXDSP,LEXDES,LEXL S LEXI=0,LEXXN=$G(^LEX(757.01,LEXX,0)),LEXA(0)=1,LEXA(1,LEXX)=LEXXN,LEXLVL=+($G(LEX("LVL"))) S:LEXLVL=0 LEXLVL=1
...
FND ; Build List of Modifiers Found (LEXFND)
 ...
 . . I LEXO=1 S LEXDES=$$DES(LEXI),LEXDSP=$$SO^LEXASO(LEXI,$G(LEXSHOW),1)
