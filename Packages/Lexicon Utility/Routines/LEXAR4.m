LEXAR4 ; ISL Look-up Response (Select Entry)      ; 10-15-97
 ;;2.0;LEXICON UTILITY;**4,5,6**;Sep 23, 1996
 ;
SEL(LEXUR) ; Select # on list
 ...
 ; Set concept level, if modifiers are allowed build list   PCH 6
 S LEXLVL=+($G(LEX("LVL"))) I LEXLVL'>1,+LEXEXP>2,$D(^LEX(757.01,+LEXEXP,0)),+($G(^TMP("LEXSCH",$J,"MOD",0)))>0 D EN^LEXAMD(LEXEXP)
 ; Quit if modifiers found at next level   PCH 6
...
SET(LEXEXP) ; Set LEX("SEL") Nodes
 ...
 ; If selected from the list increment frequency PCH 4
...
INC(LEXMC) ; Increment frequency counter in ^LEX(757)
 ...
 Q
FQ ; Edit Concept Frequency PCH 5 remove test code
