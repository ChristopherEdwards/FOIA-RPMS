LEXXFQ ; ISL Set Frequencies in 757.001           ; 02-06-97
 ;;2.0;LEXICON UTILITY;**4**;Sep 23, 1996
 Q
EN ; Update term frequencies when not found  (at site)
 S ZTRTN="UP^LEXXFQ",ZTDESC="Update Term Frequency in file 757.001",ZTIO="",ZTDTH=$H D ^%ZTLOAD,HOME^%ZIS K Y,ZTSK,ZTDESC,ZTDTH,ZTIO,ZTRTN Q
 Q
EN2 ; Reset term frequencies to export values (at IRMFO)
 S ZTRTN="RE^LEXXFQ",ZTDESC="Reset Term Frequencies in file 757.001",ZTIO="",ZTDTH=$H D ^%ZTLOAD,HOME^%ZIS K Y,ZTSK,ZTDESC,ZTDTH,ZTIO,ZTRTN Q
CHK ; Check frequencies                       (at site or IRMFO)
 N LEXI,LEXC S (LEXI,LEXC)=0 F  S LEXI=$O(^LEX(757,LEXI)) Q:+LEXI=0  S:'$D(^LEX(757.001,LEXI)) LEXC=LEXC+1
 I '$D(ZTQUEUED) W:LEXC>0 !!,LEXC," Concepts do not have frequencies set",!! W:LEXC'>0 !!,"All concepts have frequencies set",!!
 Q
UP ; Update frequencies
 S:$D(ZTQUEUED) ZTREQ="@"
 N LEX1,LEX2,LEXU,LEXUC,LEXDC,LEXMA,LEXT,LEXL,LEXH S (LEXDC,LEXU,LEXUC,LEXT,LEXL,LEXMA)=0,LEXH="."
 S LEX1=$$HACK W:'$D(ZTQUEUED) !!,"Initializing Global",!,"  Start:     ",$P(LEX1,"^",2),!,"  "
 ...
 W:'$D(ZTQUEUED) LEXH
 S:LEXT>0 $P(^LEX(757.001,0),"^",4)=LEXT S:LEXL>0 $P(^LEX(757.001,0),"^",3)=LEXL S:$D(ZTQUEUED) ZTREQ="@"
 S LEX2=$$HACK W:'$D(ZTQUEUED) !,"  Finished:  ",$P(LEX2,"^",2),!,"  Time:      ",$$TIME($P(LEX1,"^",1),$P(LEX2,"^",1)),!
 Q
RE ; Reset frequencies
 ...
 S:LEXT>0 $P(^LEX(757.001,0),"^",4)=LEXT S:LEXL>0 $P(^LEX(757.001,0),"^",3)=LEXL S:$D(ZTQUEUED) ZTREQ="@" Q
SET ; Set frequency
 N DIK,DIC,DA,LEXFQ S LEXMA=+($G(LEXMA)) Q:'$D(^LEX(757,LEXMA,0))  S DIC="^LEX(757.001,",DA=LEXMA,LEXFQ=+($$FQ(LEXMA)) D:$D(^LEX(757.001,DA)) KILL^LEXNDX2 S ^LEX(757.001,LEXMA,0)=LEXMA_"^"_LEXFQ_"^"_LEXFQ D SET^LEXNDX2 Q
FQ(LEXX) ; Frequency
 ;
...
SM(LEXX) ; Semantic Map (757.1)
 N LEXMC,LEXCL,LEXSA S LEXSA=0,LEXMC=+($G(LEXX)),LEXX=0 Q:'$D(^LEX(757,LEXMC,0))
 F  S LEXSA=$O(^LEX(757.1,"B",LEXMC,LEXSA)) Q:+LEXSA=0  S LEXCL=+($P($G(^LEX(757.1,LEXSA,0)),"^",2)) I LEXCL=3!(LEXCL=6) S LEXX=1
 Q LEXX
HACK(LEXX) ; Time Hack
 N X,%,%H,%I
