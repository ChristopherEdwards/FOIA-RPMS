MCAROC1 ; GENERATED FROM 'MCARCATH1' PRINT TEMPLATE (#3698) ; 11/29/04 ; (FILE 691.1, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 M DXS=^DIPT(3698,"DXS")
 S I(0)="^MCAR(691.1,",J(0)=691.1
 D N:$X>49 Q:'DN  W ?49 W "WARD/CLINIC: "
 S X=$G(^MCAR(691.1,D0,0)) S Y=$P(X,U,6) S Y=$S(Y="":Y,$D(^SC(Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 D N:$X>4 Q:'DN  W ?4 W "REF HOSP OR PHYS: "
 S Y=$P(X,U,5) S Y=$S(Y="":Y,$D(^MCAR(697.1,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 ...
 G A1R
A1 ;
 S X=$G(^MCAR(691.1,D0,3,D1,0)) W ?17 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^MCAR(695.3,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 Q
A1R ;
...
B1R ;
 ...
 G C1R
C1 ;
 S X=$G(^MCAR(691.1,D0,.3,D1,0)) D N:$X>26 Q:'DN  W ?26 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^MCAR(695.5,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,99)
 Q
C1R ;
 ...
 G D1R
D1 ;
 S X=$G(^MCAR(691.1,D0,.4,D1,0)) W ?19 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^MCAR(695.4,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,40)
 Q
D1R ;
 ...
 G E1R
E1 ;
 S X=$G(^MCAR(691.1,D0,6,D1,0)) W ?17 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^MCAR(695,Y,0))#2:$P(^(0),U),1:Y) S Y=$S(Y="":Y,$D(^PSDRUG(Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,40)
