MCAROC3 ; GENERATED FROM 'MCARCATH3' PRINT TEMPLATE (#3696) ; 11/29/04 ; (FILE 691.1, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 S X=$G(^MCAR(691.1,D0,20)) W ?28 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^MCAR(695.9,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
...
A1 ;
 ...
 D N:$X>33 Q:'DN  W ?33 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^MCAR(696.1,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,17)
 D N:$X>33 Q:'DN  W ?33 S Y=$P(X,U,4) W:Y]"" $J(Y,7,2)
 W ?42 S Y=$P(X,U,3) S Y=$S(Y="":Y,$D(^MCAR(696.2,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,25)
 Q
A1R ;
...
B1 ;
 ...
 S X=$G(^MCAR(691.1,D0,23,D1,0)) W ?15 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^MCAR(696,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 D N:$X>44 Q:'DN  W ?44 W "TO:"
 W ?49 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^MCAR(696,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 Q
B1R ;
 ...
 G C1R
C1 ;
 S X=$G(^MCAR(691.1,D0,25,D1,0)) D N:$X>33 Q:'DN  W ?33 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^MCAR(696.1,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,17)
 W ?52 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^MCAR(696.2,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,25)
 Q
C1R ;
...
D1 ;
 ...
 W ?41 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^MCAR(696.2,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,25)
 Q
D1R ;
...
I1 ;
 ...
 S X=$G(^MCAR(691.1,D0,36,D1,0)) D N:$X>9 Q:'DN  W ?9 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^MCAR(696,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 D N:$X>33 Q:'DN  W ?33 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^MCAR(696.1,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,17)
 D N:$X>33 Q:'DN  W ?33 S Y=$P(X,U,4) W:Y]"" $J(Y,7,2)
 W ?42 S Y=$P(X,U,3) S Y=$S(Y="":Y,$D(^MCAR(696.4,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,25)
 Q
I1R ;
 K Y
 Q
