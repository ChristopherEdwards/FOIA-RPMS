MCAROPE ; GENERATED FROM 'MCAROPE' PRINT TEMPLATE (#3727) ; 11/29/04 ; (FILE 699, MARGIN=80)
 G BEGIN
N W !
...
F1R ;
 ...
 G G1R
G1 ;
 S X=$G(^MCAR(699,D0,26,D1,0)) D N:$X>17 Q:'DN  W ?17 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^PSDRUG(Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,40)
 W "  "
 W ?0,$E($P(X,U,2),1,30)
 Q
G1R ;
 ...
 G H1R
H1 ;
 S X=$G(^MCAR(699,D0,203,D1,0)) S DIWL=5,DIWR=69 D ^DIWP
 Q
H1R ;
 D 0^DIWW
