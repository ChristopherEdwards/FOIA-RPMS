MCOBGC ; GENERATED FROM 'MCCONSULTBRPR' PRINT TEMPLATE (#3749) ; 11/29/04 ; (FILE 699.5, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 S X=$G(^MCAR(699.5,D0,0)) S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^MCAR(690,Y,0))#2:$P(^(0),U),1:Y) S Y=$S(Y="":Y,$D(^DPT(Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 ...
 G A1R
A1 ;
 S X=$G(^MCAR(699.5,D0,20,D1,0)) S DIWL=19,DIWR=73 D ^DIWP
 Q
A1R ;
 D 0^DIWW
 D ^DIWW
 ...
 G B1R
B1 ;
 S X=$G(^MCAR(699.5,D0,21,D1,0)) S DIWL=18,DIWR=72 D ^DIWP
 Q
B1R ;
 D 0^DIWW
 D ^DIWW
 ...
 G C1R
C1 ;
 S X=$G(^MCAR(699.5,D0,22,D1,0)) S DIWL=5,DIWR=59 D ^DIWP
 Q
C1R ;
 D 0^DIWW
 W ?61 S MCFILE=699.5 D DISP^MCMAG K DIP K:DN Y
 D ^DIWW
