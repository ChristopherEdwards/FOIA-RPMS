MCOBGN ; GENERATED FROM 'MCARGINONBRPR' PRINT TEMPLATE (#3751) ; 11/29/04 ; (FILE 699, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 G A1R
A1 ;
 S X=$G(^MCAR(699,D0,18,D1,0)) S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^MCAR(699.88,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 Q
A1R ;
 ...
 G B1R
B1 ;
 S X=$G(^MCAR(699,D0,20,D1,0)) S DIWL=19,DIWR=73 D ^DIWP
 Q
B1R ;
 D 0^DIWW
 D ^DIWW
 ...
 G C1R
C1 ;
 S X=$G(^MCAR(699,D0,21,D1,0)) S DIWL=18,DIWR=72 D ^DIWP
 Q
C1R ;
 D 0^DIWW
 D ^DIWW
 ...
 G D1R
D1 ;
 S X=$G(^MCAR(699,D0,22,D1,0)) S DIWL=19,DIWR=73 D ^DIWP
 Q
D1R ;
 D 0^DIWW
 D ^DIWW
 ...
 G E1R
E1 ;
 S X=$G(^MCAR(699,D0,23,D1,0)) S DIWL=16,DIWR=70 D ^DIWP
 Q
E1R ;
 D 0^DIWW
 D ^DIWW
