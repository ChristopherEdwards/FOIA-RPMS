MCOBPA ; GENERATED FROM 'MCARPACALEADBRPR' PRINT TEMPLATE (#3756) ; 11/29/04 ; (FILE 698.2, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 M DXS=^DIPT(3756,"DXS")
 S I(0)="^MCAR(698.2,",J(0)=698.2
 ...
 S X=$G(^MCAR(698.2,D0,1)) S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^MCAR(695.8,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,60)
 ...
 G A1R
A1 ;
 S X=$G(^MCAR(698.2,D0,10,D1,0)) S DIWL=17,DIWR=71 D ^DIWP
 Q
A1R ;
 D 0^DIWW
 D ^DIWW
 D T Q:'DN  D N D N:$X>4 Q:'DN  W ?4 W "PROCEDURE SUMMARY: "
 S X=$G(^MCAR(698.2,D0,.2)) S DIWL=26,DIWR=80 S Y=$P(X,U,2) S X=Y D ^DIWP
 D 0^DIWW
 D ^DIWW
 D T Q:'DN  W ?2 S MCFILE=698.2 D DISP^MCMAG K DIP K:DN Y
