MCOBPFT ; GENERATED FROM 'MCPFTBRPR' PRINT TEMPLATE (#3753) ; 11/29/04 ; (FILE 700, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 M DXS=^DIPT(3753,"DXS")
 S I(0)="^MCAR(700,",J(0)=700
 W ?0 W "DATE/TIME: "
...
C1R ;
 ...
 S X=$G(^MCAR(700,D0,.2)) S DIWL=26,DIWR=80 S Y=$P(X,U,2) S X=Y D ^DIWP
 D 0^DIWW
 D ^DIWW
...
D1R ;
 ...
 G E1R
E1 ;
 S X=$G(^MCAR(700,D0,7,D1,0)) D N:$X>16 Q:'DN  W ?16 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^VA(200,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,35)
 Q
E1R ;
 K Y K DIWF
 Q
