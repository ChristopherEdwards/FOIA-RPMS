MCOBRHA ; GENERATED FROM 'MCRHBRPR1' PRINT TEMPLATE (#3761) ; 11/29/04 ; (FILE 701, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 G A1R
A1 ;
 S X=$G(^MCAR(701,D0,6,D1,0)) D N:$X>4 Q:'DN  W ?4 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^MCAR(695,Y,0))#2:$P(^(0),U),1:Y) S Y=$S(Y="":Y,$D(^PSDRUG(Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,40)
