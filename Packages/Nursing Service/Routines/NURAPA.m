NURAPA ; GENERATED FROM 'NURA-I-A1106B' INPUT TEMPLATE(#2756), FILE 213.2;11/29/04
 D DE G BEGIN
DE S DIE="^NURSA(213.2,",DIC=DIE,DP=213.2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^NURSA(213.2,DA,""))=""
 ...
 I $D(^(.5)) S %Z=^(.5) S %=$P(%Z,U,1) S:%]"" DE(17)=% S %=$P(%Z,U,2) S:%]"" DE(18)=% S %=$P(%Z,U,3) S:%]"" DE(19)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="NURAPA",DQ=1
 ...
 M DIEZAR=^DIE(2756,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2756,U="^"
