NURAPB ; GENERATED FROM 'NURA-P-EXCEPTION' PRINT TEMPLATE (#3772) ; 11/29/04 ; (FILE 213.5, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 M DXS=^DIPT(3772,"DXS")
 S I(0)="^NURSA(213.5,",J(0)=213.5
 S I(1)=1,J(1)=213.51 F D1=0:0 Q:$O(^NURSA(213.5,D0,1,D1))'>0  X:$D(DSC(213.51)) DSC(213.51) S D1=$O(^(D1)) Q:D1'>0  D:$X>0 T Q:'DN  D A1
 G A1R
A1 ;
 S X=$G(^NURSA(213.5,D0,1,D1,0)) D N:$X>0 Q:'DN  W ?0 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^DPT(Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,20)
