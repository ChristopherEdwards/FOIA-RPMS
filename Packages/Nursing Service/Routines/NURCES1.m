...
VM ;EXTRACTS LATEST VITAL MEASUREMENTS AND ABNORMAL V/M FOR THE PAST 24 HOURS
 ...
 Q
SETDATAR ;
 F Y=0:0 S Y=$O(^GMR(120.5,"AA",DFN,GMR(X),GMRDAT,Y)) Q:Y'>0!GMRDATS  I '$D(^GMR(120.5,Y,2)) D SETNODE
 Q
SETNODE ;
 S GMRL=$S($D(^GMR(120.5,Y,0)):^(0),1:"") I GMRL'="" K GMRVX S GMRVX=X,GMRVX(0)=$P(GMRL,"^",8) D EN1^GMRVSAS0 S GMRDAT(X)=$P(GMRL,"^",8)_$S('$D(GMRVX(1)):"",'GMRVX(1):"",1:"*"),GMRDATS=1
 Q
