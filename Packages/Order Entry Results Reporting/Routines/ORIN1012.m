ORIN1012 ;IHS/CIA/PLS - KIDS Inits for OR patch 1012 ;24-Apr-2014 18:11;PLS
 ;;3.0;ORDER ENTRY/RESULTS REPORTING;**1012**;Dec 17, 1997
 ;=================================================================
EC ;EP - Environment check
 Q
PRE ;EP - Preinit
 D CHGODNM("OR GTX SNOMED DESC ID","OR GTX SNOMED CONCEPT ID")
 D CHGODNM("OR GTX SNOMED CONCEPT ID","OR GTX SNMDCNPTID")
 Q
 ;
POST ;EP - Postinit
 D CHGPSO
 D CHGODDM("OR GTX CODE","1:20")
 I $$REGRPC^CIAURPC("ORWDXIHS ORDSMCID","CIAV VUECENTRIC")
 I $$REGRPC^CIAURPC("ORWDXIHS UPSNMD","CIAV VUECENTRIC")
 Q
 ;
CHGPSO ;EP-
 N DNM
 ...
 D ADDPMT("OR GTX SNMDCNPTID","CONSULT/REQUEST TRACKING","SNOMED Concept ID:","Concept ID",7.3,-1)
 ...
 D ADDPMT("OR GTX SNMDCNPTID","LAB SERVICE","SNOMED Concept ID:","Concept ID",10.6,-1)
 D ADDPMT("OR GTX SNMDCNPTID","OUTPATIENT PHARMACY","SNOMED Concept ID:","Concept ID",10.6,-1)
 D ADDPMT("OR GTX SNMDCNPTID","PHARMACY DATA MANAGEMENT","SNOMED Concept ID:","Concept ID",10.6,-1)
...
CHKPSO1(DNM) ;EP-
 N DLG,PMT,IEN,FDA
 S DLG=$$FIND1^DIC(101.41,,"XQ",DNM)
 S PMT=$$FIND1^DIC(101.41,,"XQ","OR GTX SNMDCNPTID")
 Q:'PMT!'DLG
 S IEN=$O(^ORD(101.41,DLG,10,"D",PMT,0))
 Q:'IEN
...
CHKPSO6(DNM) ;EP-
 ...
 S FDA(101.412,IEN_","_DLG_",",17)="S Y=$$ID^BEHOORSY K:'$L(Y) Y"
 D FILE^DIE("","FDA")
 Q
CHKPSO7(DNM) ;EP-
 N DLG,PMT,IEN,FDA
