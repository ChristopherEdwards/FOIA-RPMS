ORPRS06 ; slc/dcm - Driving Miss ChartCopy ;6/10/97  15:42
 ;;3.0;ORDER ENTRY/RESULTS REPORTING;**11,69,99**;Dec 17, 1997
MAIN ; Control module
...
CLINIC ; Sets up call for clinic patients
 N ORAPT,ORI
 S ORAPT=9999999-OREARLY
 F  S ORAPT=$O(^SC(ORSSC,"S",ORAPT)) Q:ORAPT'>0!(ORAPT>(9999999-ORLATE))  D
 . S ORI=0 F  S ORI=$O(^SC(ORSSC,"S",ORAPT,1,ORI)) Q:ORI'>0  D
 .. I '$L($P(^SC(ORSSC,"S",ORAPT,1,ORI,0),U,9)) S DFN=+^(0)
 .. I +$G(DFN) D PATIENT(DFN,OREARLY,ORLATE,ORHPRM,$G(ORCONTX),$G(ORSSC))
 D ^%ZISC
 I $D(ZTSK) D KILL^%ZTLOAD K ZTSK
 Q
