...
MEMODATA ;
 ...
 .S IDT=9999999-SDATE,(RCNT,RQUIT)=0
 .F  S IDT=$O(^GMR(120.5,"AA",DFN,TESTNUM,IDT)) Q:IDT<1  Q:IDT>EDT  D  Q:RQUIT
 ..S DATANUM=0 F  S DATANUM=$O(^GMR(120.5,"AA",DFN,TESTNUM,IDT,DATANUM)) Q:DATANUM<1  D  Q:RQUIT
 ...Q:$P($G(^GMR(120.5,DATANUM,2)),U)  S ZERO=$G(^(0))
 ...Q:$P(ZERO,U,2)'=DFN  Q:$P(ZERO,U,3)'=TESTNUM
 ...S RESULT=$$STRIP^ORCMEDIT($P(ZERO,U,8)),DATETIME=$P(ZERO,U),LOC=+$P(ZERO,U,5),ENTERBY=+$P(ZERO,U,6)
 ...S ^TMP("ORQQV",$J,"RESULTS",IDT)=DATETIME_U_LOC_U_ENTERBY ;$$FMTE^XLFDT(DATETIME)_"  Location: "_$P($G(^SC(+$P(ZERO,U,5),0)),U)_"  Entered by: "_$P($G(^VA(200,+$P(ZERO,U,6),0)),U)
...
GRIDDATA ;
 ...
 ...Q:$P($G(^GMR(120.5,DATANUM,2)),U)  S ZERO=$G(^(0))
 ...Q:$P(ZERO,U,2)'=DFN  Q:$P(ZERO,U,3)'=TESTNUM
 ...S RESULT=$$STRIP^ORCMEDIT($P(ZERO,U,8)),DATETIME=$P(ZERO,U)
