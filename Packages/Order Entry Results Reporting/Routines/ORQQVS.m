ORQQVS ; slc/CLA,STAFF - Functions which return patient visits ;8/19/98@13:36:57
...
LIST(ORY,PT,ORSDT,OREDT,LOC) ; return visits for a patient between start & end dates for a location, if no location return all visits
 ...
 I ORSDT="" D
 .I '$L(LOC) S ORSDT=$$UP^XLFSTR($$GET^XPAR("USR^SRV.`"_+$G(ORSRV)_"^DIV^SYS^PKG","ORQQVS SEARCH RANGE START",1,"E"))
 .I ORSDT="" S ORSDT="T-730" ;default start date is two years ago
 I OREDT="" D
 .I '$L(LOC) S OREDT=$$UP^XLFSTR($$GET^XPAR("USR^SRV.`"_+$G(ORSRV)_"^DIV^SYS^PKG","ORQQVS SEARCH RANGE STOP",1,"E"))
