PSOBMST ;BIR/LAW-black line resolver ;26-Apr-2004 10:26;PLS
 ;;7.0;OUTPATIENT PHARMACY;**2,71**;DEC 1997
...
EN01 I $D(PSOIOS),PSOIOS]"" D DEVBAR
 ...
 K PSOCPN,PSOLBLCP
 ; IHS/CIA/PLS - 01/16/04 - Commented out next two lines and added call to IHS label generator.
 ;F PI=1:1 Q:$P(PPL,",",PI)=""  S RX=$P(PPL,",",PI) S PSOBLALL=1,RXRP(RX)=1_"^"_$G(COPIES)_"^"_$S($G(SIDE):1,1:0) S:$G(PSOBXPRT(RX)) RXPR(RX)=PSOBXPRT(RX) D  D C^PSOLBL K RXRP(+$G(PSOBLRX)),RXPR(+$G(PSOBLRX)),RXFL(+$G(PSOBLRX)) K PSOBLRX,RXP
 ;.S:$D(PBXRF(RX)) RXFL(RX)=PBXRF(RX)
