PSOCSTD ;BHAM ISC/SAB - daily rx cost compilation ; 06/03/92 15:05
 ;;7.0;OUTPATIENT PHARMACY;**4,17,28,89**;DEC 1997
...
START K ^TMP($J) G:$E(DT,6,7)="01" MTH
 ...
 S (TNR,TNO,TNP)=0
SRCH S PSDT=BEGDATE_".0000001" F  S PSDT=$O(^PSRX("AL",PSDT)) Q:'PSDT!($E(PSDT,1,7)>ENDDATE)  S (OR,RF)=0 D SRCH1 S:'$D(BDT) BDT=PSDT
 S PSDT=BEGDATE_".0000001" F  S PSDT=$O(^PSRX("AM",PSDT)) Q:'PSDT!($E(PSDT,1,7)>ENDDATE)  D SRCH2 S:'$D(BDT) BDT=PSDT
