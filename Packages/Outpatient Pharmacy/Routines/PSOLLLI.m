PSOLLLI ;BIR/JLC - LASER LABELS INITIALIZATION ;08-Feb-2011 07:10;SM
 ;;7.0;OUTPATIENT PHARMACY;**120,157,1003,1005,1006,1008,1010**;DEC 1997
 ;
...
REF F XXX=0:0 S XXX=$O(^PSRX(RX,XTYPE,XXX)) Q:+XXX'>0  D
 ...
 .S QTY=$P(^PSRX(RX,XTYPE,XXX,0),"^",4),PHYS=$S($D(^VA(200,+$P(^PSRX(RX,XTYPE,XXX,0),"^",17),0)):$P(^(0),"^"),$D(^VA(200,+$P(^PSRX(RX,0),"^",4),0)):$P(^(0),"^"),1:"UNKNOWN") D 6^VADPT,PID^VADPT6 S SSNPN=$G(VA("PID"))
...
OSET ;
 ...
 .; IHS/MSC/PLS - 10/10/07
 .;S TECH=$P($G(^VA(200,+$P(A,"^",16),0)),"^")
 .S TECH=$$LBLINI^APSPLBL(RX,"O")
 .S QTY=$P(A,"^",7),PHYS=$S($D(^VA(200,+$P(A,"^",4),0)):$P(^(0),"^"),1:"UKN") D 6^VADPT,PID^VADPT6 S SSNPN=$G(VA("PID"))
 .S DAYS=$P(A,"^",8)
 ...
 S QTY=$P(A,"^",4),PHYS=$S($D(^VA(200,+$P(A,"^",17),0)):$P(^(0),"^"),$D(^VA(200,+$P(^PSRX(RX,0),"^",4),0)):$P(^(0),"^"),1:"UNKNOWN") D 6^VADPT,PID^VADPT6 S SSNPN=$G(VA("PID"))
 S DAYS=$P(A,"^",10)
 Q
DOUB ;
 Q:'$D(RXFL(RX))
