PSOORUT1 ;BIR/SAB - utility routine for oerr interface ;16-Nov-2007 11:19;SM
 ;;7.0;OUTPATIENT PHARMACY;**1,30,46,132,1005,1006**;DEC 1997
 ;External reference to ^PSDRUG supported by DBIA 221
 ;External reference to ^PSXOPUTL supported by DBIA 2203
 ;called from HD^PSOORUTL
 ; Modified - IHS/CIA/PLS - 01/15/04 - SHOW+5 and DISPL+15
...
DISPL S IEN=IEN+1 N PSOID,PSOCMOP,STATLTH
 ...
 S ^TMP("PSOPF",$J,IEN,0)=$S(PSOSTA="ACTIVE OTHER PHARMACY":"  ",1:$J(PSOCNT,2))_$S($L(PSOCNT)<3:" ",1:"")_$P(^PSRX(+PSODATA,0),"^")_$S($G(^PSRX(+PSODATA,"IB")):"$",1:"")_PSOLNT_$P(PSODRG,"^")
 ...
 ; IHS/CIA/PLS - 01/25/04 - Next line commented out and a $G added
 ;F PSOX=0:0 S PSOX=$O(^PSRX(+PSODATA,1,PSOX)) Q:'PSOX  I +^PSRX(+PSODATA,1,PSOX,0)=PSOLF,$P(^PSRX(+PSODATA,1,PSOX,0),"^",16) S PSOLF=PSOLF_"^R"
 F PSOX=0:0 S PSOX=$O(^PSRX(+PSODATA,1,PSOX)) Q:'PSOX  I +$G(^PSRX(+PSODATA,1,PSOX,0))=PSOLF,$P(^PSRX(+PSODATA,1,PSOX,0),"^",16) S PSOLF=PSOLF_"^R"
 K PSOX
...
STA K ZSTA
 ...
 S IEN=IEN+1 I PSOSTA="ZNONVA" S ZSTA=PSOSTA,PSOSTA="Outside Medications"
