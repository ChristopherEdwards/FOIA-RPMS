PSORENW3 ;IHS/DSD/JCM - EDIT TEMPLATE FOR RENEW RX ORDER ENTRY ;04-Jun-2013 13:07;DU
 ;;7.0;OUTPATIENT PHARMACY;**11,20,46,100,117,1006,1008,1017**;DEC 1997;Build 40
 ...
 ;                          06/04/13 - Changed label 69 to 70 and added label 69 and lines at DSPLY+24 and JUMP+4
 ;
START ;
 D INIT
 ;
...
69 S PSORENW("FLD")=69 D DSCMED^APSPDIR(.PSORENW)
 G:PSORENW("DFLG") END G:PSORENW("FIELD") @PSORENW("FIELD")
 ; IHS/MSC/PLS - 01/23/09 - Added Cash Due prompt
70 I $$GET1^DIQ(9009033,PSOSITE,319,"I") S PSORENW("FLD")=70 D CASHDUE^APSPDIR(.PSORENW)
 G:PSORENW("DFLG") END G:PSORENW("FIELD") @PSORENW("FIELD")
 ;
...
JUMP ;
 ...
 ; IHS/MSC/PLS - 06/04/13 - changed 69 to 70 and added 69 to $S
 S PSONEW("FIELD")=$S(+Y=7:40,+Y=9999999.13:45,+Y=27:50,+Y=9999999.06:60,+Y=17:61,+Y=28:63,+Y=24:65,+Y=26:67,+Y=9999999.07:85,+Y=9999999.12:87,+Y=9999999.25:68,+Y=9999999.28:69,+Y=9999999.26:70,1:PSONEW("FIELD"))
 Q
DSPLY ;called from PSORENW0
 ...
 ; IHS/MSC/PLS - 06/04/13
 W !,"DISCHARGE MEDICATION: "_$$EXTERNAL^DILFD(52,9999999.28,,$G(PSONEW("DSCMED")))
