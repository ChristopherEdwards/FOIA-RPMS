...
PROCESS ;EP
 S APCDJ=$J,APCDH=$H
 S ^XTMP("APCDCAFE",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^"_"PCC CODING W/IN 3 DAYS REPORT"
...
N ..;
 ...
 ..Q:$$CIT(APCDVIEN,APCDNOD)  ;coded within 3 days
 ..S ^XTMP("APCDCAFE",APCDJ,APCDH,"VISITS",$$SORT(APCDVIEN,APCDSORT),APCDVIEN)=""
 ..S APCDCOUN($$SORT(APCDVIEN,APCDSORT))=$G(APCDCOUN($$SORT(APCDVIEN,APCDSORT)))+1
 ..Q
 .Q
 Q
 ;
CIT(V,N) ;
 ;get CHART AUDIT STATUS, if null or I check visit date and quit
 NEW X,Y,Z,D
 S Z=0
 S X=$P($G(^AUPNVSIT(V,11)),U,11)
 I X="I"!(X="") S Y=$$FMDIFF^XLFDT(DT,$$VD^APCLV(V)) I Y<N S Z=1 Q Z_U_"NOT REVIEWED/COMPLETE"
 I X="I"!(X="") Q 0_U_"NOT REVIEWED YET"
 S X=0,Y="" F  S X=$O(^AUPNVCA("AD",V,X)) Q:X'=+X  I $P($G(^AUPNVCA(X,0)),U,4)="R" S Y=X
 I 'Y S D=$$FMDIFF^XLFDT(DT,$$VD^APCLV(V)) I D<N S Z=1 Q Z_U_"NOT REVIEWED/COMPLETE"
 I 'Y Q 1
 S D=$$FMDIFF^XLFDT($P($P(^AUPNVCA(Y,0),U),"."),$$VD^APCLV(V)) I D<N Q 1
 Q 0_U_$$FMTE^XLFDT($P($P(^AUPNVCA(Y,0),U),"."))
DATE(D) ;
 NEW X,Y
 S X=$P(D,".")
...
XIT ; Clean up and exit
 K ^XTMP("APCDCAFE",APCDJ,APCDH,"VISITS")
 D EN^XBVK("APCD")
 Q
PRINT1 ; Print report 2
 ...
 S APCDCRN=1
 D HEAD
 I '$D(^XTMP("APCDCAFE",APCDJ,APCDH,"VISITS")) W !!,"There are no visits that were not reviewed within ",APCDNOD,"days." Q
 S APCDS="" F  S APCDS=$O(^XTMP("APCDCAFE",APCDJ,APCDH,"VISITS",APCDS)) Q:APCDS=""!($D(APCDQUIT))  D
 .I $Y>(IOSL-3) D HEAD Q:$D(APCDQUIT)
 .W !!,APCDS,?40,APCDCOUN(APCDS)
 S APCDCRN=0
 I APCDCORL="C" Q
 D HEAD
 S APCDS="" F  S APCDS=$O(^XTMP("APCDCAFE",APCDJ,APCDH,"VISITS",APCDS)) Q:APCDS=""!($D(APCDQUIT))  D
 .S APCDV="" F  S APCDV=$O(^XTMP("APCDCAFE",APCDJ,APCDH,"VISITS",APCDS,APCDV)) Q:APCDV=""!($D(APCDQUIT))  D
...
PRN1 ;EP
 ...
 W !,$$FMTE^XLFDT($P(APCDVR,U)),?19,APCDHRN,?31,$P($$CIT(APCDV,APCDNOD),U,2),?50,$E($P(^DIC(4,$P(APCDVR,U,6),0),U),1,10),?61,$P(APCDVR,U,7)
...
HEAD1 ;
 ...
 W !,$$CTR($S(APCDCRN="C":"COUNT",1:"LIST")_" OF VISITS NOT MARKED AS REVIEWED/COMPLETE",80)
 W !,$$CTR("WITHIN "_APCDNOD_" DAYS OF THE VISIT DATE",80)
 S X="VISIT Date Range: "_APCDBDD_" through "_APCDEDD W !,$$CTR(X,80)
 I APCDCRN W !!,"CLINIC",?40,"# OF VISITS" I 1
 E  W !!,"VISIT DATE",?19,"HRN",?31,"DATE REVIEWED",?50,"LOCATION",?61,"SC",?64,"CL",?67,"HOSP LOC",?77,"DEC"
 W !,APCD80S
 Q
...
COVPAGE ;
 W !,$$FMTE^XLFDT(DT),?70,"Page: ",APCDPG
 W !?29,"PCC Data Entry Module"
 W !,$$CTR($S(APCDCORL="C":"COUNT",1:"LIST")_" OF VISITS NOT MARKED AS REVIEWED/COMPLETE",80)
 W !,$$CTR("WITHIN "_APCDNOD_" DAYS OF THE VISIT DATE.",80)
