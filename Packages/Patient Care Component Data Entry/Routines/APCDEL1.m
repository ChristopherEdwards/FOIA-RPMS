APCDEL1 ; IHS/CMI/LAB - UPDATE VISIT W/LIST MAN ;
 ;;2.0;IHS PCC SUITE;**5**;MAY 14, 2009;Build 18
 ;
 ;
 ;
...
OTHER ;EP - called from protocol to process other mnemonics
 ...
 D CLEAR^VALM1
 K APCDDISP,APCDSEL,^TMP("APCDEL",$J),C,X,I,K,J,APCDHIGH,APCDCUT,APCDCSEL,APCDCNTL,APCDC,APCDANS,APCDI,APCDCRIT,APCDTEXT
 K VALMHDR,VALMCNT
 D BACK
 Q
...
BACK ;EP go back to listman
 D TERM^VALM0
 S VALMBCK="R"
 D:'$G(APCDSECM) INIT^APCDEL
 D:$G(APCDSECM)=1 INIT2^APCDEL
 D:$G(APCDSECM)=2 INIT3^APCDEL
 D HDR^APCDEL
 K DIR
 D KDIE
 K X,Y,Z,I
...
GETMNE ; GET MNEMONIC
 ...
 I $D(APCDMOD) W !!,"Switching to Modify Mode for ONE Mnemonic ONLY!" S APCDMODE="M",APCDVLK=$G(APCDVSIT) D GETMNE K APCDVLK,APCDMOD S APCDMODE="A" W !!,"Switching back to ENTER Mode!" Q
 Q
 ;
