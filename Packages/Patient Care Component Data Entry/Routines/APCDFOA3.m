APCDFOA3 ; IHS/CMI/LAB - USER INTERFACE TO SELECT ICD PROCEDURE CODES ;
...
ASK1 ;
 S APCDA=0
 K APCD("LOW"),APCD("HI")
 S DIR("A")=$S('$D(APCDTABL):"Enter Operation/Procedure code (or range of codes)",1:"Enter another Operation/Procedure code (or range of codes)") D SETDIR^APCDFOA4,^DIR K DIR
 I "^"[Y G X1
 D PROCESS
...
DISPLAY ;EP - SHOW CODES IN RANGE SELECTED
 W:$D(IOF) @IOF
 W !!,"ICD OPERATION/PROCEDURES codes in this range =>",!! W $P(APCD("LOW")," ") S APCDDFN=$O(^ICD0("BA",APCD("LOW"),"")) W ?9,$P($$ICDOP^ICDCODE(APCDDFN),U,3)
 S APCD=APCD("LOW"),APCDCNT=IOSL-2 F  S APCD=$O(^ICD0("BA",APCD)) Q:APCD]APCD("HI")  S APCDDFN=$O(^(APCD,"")) W !,$P(APCD," "),?9,$P($$ICDOP^ICDCODE(APCDDFN),U,3) S APCDCNT=APCDCNT-1 I APCDCNT=0 S APCDCNT=IOSL-2 D  I 'APCDR Q
...
SETUTIL ;SET UP TMP NODES TO STORE CODES
 ...
 .S APCDDFN=$O(^ICD0("BA",APCD("LOW"),"")),^XTMP("APCDFOA",APCDJOB,APCDBT,"DEPOV","ICDDFN",APCDDFN,APCDIRNG)=""
 .S APCD=APCD("LOW") F  S APCD=$O(^ICD0("BA",APCD)) Q:APCD]APCD("HI")  S APCDDFN=$O(^(APCD,"")) S ^XTMP("APCDFOA",APCDJOB,APCDBT,"DEPOV","ICDDFN",APCDDFN,APCDIRNG)=""
 .Q
 Q
RANGES ;DISPLAY TABLE OF ALL RANGES
 W:$D(IOF) @IOF
 W !!,"ICD OPERATION/PROCEDURE Code Range(s) Selected So Far =>",!
...
CHECK ;ASKS USER IF SATISFIED WITH ENTERED RANGES
 W ! S DIR(0)="Y",DIR("B")="Y",DIR("A")="Are all of the ICD OPERATION/PROCEDURE ranges okay" D ^DIR K DIR
 W !
 Q
 ;
EOJ ;
