...
LOOK ; ENTRY POINT - LOOKUP USER RESPONSE; SET TMP NODES
 S DIC="^ICD0(",DIC(0)="EMF",ICDSYS=APCDSYS D ^DIC K DIC,DR
 I Y<0 S APCDA=1 W $C(7),!,"  ??  Invalid ",$S(APCDTYP="LOW":"LOW",1:"HIGH")," Code" S APCD("NO DISPLAY")=1 G X3
LOOKSTAR ;
 S:APCDTYP="LOW" APCD("LOW")=$P($$ICDOP^ICDEX(+Y,,,"I"),U,2)_" "
 I APCDTYP="LOW",APCDONE S APCD("HI")=APCD("LOW") D ^APCDFOA5
 I APCDTYP="HI" S APCD("HI")=$P($$ICDOP^ICDEX(+Y,,,"I"),U,2)_" " D  I 'APCD("NO DISPLAY") D DISPLAY^APCDFOA3,^APCDFOA5
