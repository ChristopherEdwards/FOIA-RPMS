...
EOJ ;EP
 ...
 Q
 ;; ;
EN ; -- main entry point for APCH HMR DISPLAY
 D EN^VALM("APCH HMR DISPLAY")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
 ...
 ;
HDR ; -- header code
 S VALMHDR(1)="Health Summary Health Maintenance Item Logic Display"
 Q
 ;
INIT ; -- init variables and list array
 K APCHDISP,APCHSEL,APCHHIGH,APCHLIST,APCHCSEL
 S APCHHIGH=0,X=0,N="" F  S N=$O(^APCHSURV("B",N)) Q:N=""  S X=0 F  S X=$O(^APCHSURV("B",N,X)) Q:X'=+X  I $P(^APCHSURV(X,0),U,3)'="D",$P(^APCHSURV(X,0),U,7)="R" S APCHHIGH=APCHHIGH+1,APCHSEL(APCHHIGH)=X
...
DISPLAY ;gather in ^TMP and display
 K ^TMP("APCHHMR1",$J)
 S ^TMP("APCHHMR1",$J,0)=0
 S APCHC=0
 S APCHX=0 F  S APCHX=$O(APCHCSEL(APCHX)) Q:APCHX'=+APCHX  S APCHHMR=APCHSEL(APCHX),Y="Reminder:",$E(Y,20)=$P(^APCHSURV(APCHHMR,0),U) S APCHC=APCHC+1 D S(Y,$S(APCHC=1:0,1:2)) D
 ...
 ;
 K ^TMP("APCHHMR1",$J,0)
 D ARRAY^XBLM("^TMP(""APCHHMR1"",$J,","HEALTH MAINTENANCE REMINDERS DESCRIPTIONS")
 Q
S(Y,F,C,T) ;set up array
 ...
 Q
S1 ;
 S %=$P(^TMP("APCHHMR1",$J,0),U)+1,$P(^TMP("APCHHMR1",$J,0),U)=%
 S ^TMP("APCHHMR1",$J,%,0)=X
 Q
