...
EP ;EP - called from option to select and display a hmr
 ...
 D ^XBFMK
 S DIC="^APCHSURV(",DIC("A")="Select HEALTH REMINDER to Modify: ",DIC(0)="AEMQ",DIC("S")="I $P(^(0),U,3)'=""D"",$P(^(0),U,7)=""R""" D ^DIC
 I Y=-1 D EXIT Q
 S APCHHMR=+Y
EN ; -- main entry point for APCH MODIFY HMR
 D EN^VALM("APCH MODIFY HMR")
 D CLEAR^VALM1
 D FULL^VALM1
 D EXIT
 Q
 ;
HDR ; -- header code
 S VALMHDR(1)="Modify Health Maintenance Reminder Criteria"
 Q
 ;
INIT ; -- init variables and list array
 K ^TMP("APCHHMRU",$J)
 S ^TMP("APCHHMRU",$J,0)=0
...
S(Y,F,C,T) ;set up array
 ...
 Q
S1 ;
 S %=$P(^TMP("APCHHMRU",$J,0),U)+1,$P(^TMP("APCHHMRU",$J,0),U)=%
 S ^TMP("APCHHMRU",$J,%,0)=X
 Q
 ;
...
MOD ;EP - called from protocol
 I '$G(APCHHMR) W !,"Protocol entry not defined." H 3 D BACK Q
 D FULL^VALM1
 D HM
 D BACK
 Q
HM ;EP - update methods
 W:$D(IOF) @IOF
 W !,"You may add a new sex, age range, frequency combination or edit and existing",!,"one for the ",$P(^APCHSURV(APCHHMR,0),U)," reminder.",!
 D DISPHM
...
DISPHM ;
 S APCHC=0 K APCHSEL
 Q:'$D(^APCHSURV(APCHHMR,11))
 S (APCHQUIT,APCHC)=0 K APCHSEL
 S APCHGIEN=0
 F  S APCHGIEN=$O(^APCHSURV(APCHHMR,11,APCHGIEN)) Q:APCHGIEN'=+APCHGIEN!(APCHQUIT)  S APCHSEX=$P(^APCHSURV(APCHHMR,11,APCHGIEN,0),U),APCHSEXR=$S(APCHSEX="F":"FEMALE",APCHSEX="M":"MALE",APCHSEX="U":"UNKNOWN",APCHSEX="B":"ALL GENDERS",1:"") D
 .S APCHA=0 F  S APCHA=$O(^APCHSURV(APCHHMR,11,APCHGIEN,11,APCHA)) Q:APCHA'=+APCHA!(APCHQUIT)  D
 ..S APCHC=APCHC+1,APCHSEL(APCHC)=APCHHMR_U_APCHGIEN_U_APCHA W !?5,APCHC,")  ",?9,APCHSEXR,?22,$$WAGE(APCHHMR,APCHGIEN,APCHA),?50,$$WF(APCHHMR,APCHGIEN,APCHA)
...
MAX ;
 ...
 S APCH3=Y
FREQ ;
 W !!,"Now enter the frequency for ",$S(APCH1="F":"FEMALES",APCH1="M":"MALES",APCH1="B":"ALL GENDERS",APCH1="U":"UNKNOWN"),", ages ",$$W(APCH2)," to ",$$W(APCH3),!,"It must be in the form: 2Y for every 2 years, 3M for every 3 months, etc.",!
 S APCH4=""
 S DIR(0)="F^2:10",DIR("A")="Enter FREQUENCY" KILL DA D ^DIR KILL DIR
 I $D(DIRUT) G AHM
 D INP^APCHSMU G:'$D(X) FREQ
 S APCH4=Y
 W !!,"The following will be added:",!,?5,$S(APCH1="F":"FEMALES",APCH1="M":"MALES",APCH1="U":"UNKNOWN",APCH1="B":"ALL GENDERS"),", ages ",$$W(APCH2)," to ",$$W(APCH3)," reminder due every ",$$W(APCH4)
CONTA ;
 ...
 ;add to multiple
 S (X,N,G,C)=0 F  S X=$O(^APCHSURV(APCHHMR,11,X)) Q:X'=+X  S:$P(^APCHSURV(APCHHMR,11,X,0),U)=APCH1 G=X S N=X,C=C+1
 I 'G S N=N+1,G=N
 ;G is first level subscript , C is total number of entries
 S ^APCHSURV(APCHHMR,11,0)="^9001018.11S^"_G_"^"_C
 S ^APCHSURV(APCHHMR,11,G,0)=APCH1
 S (N,X)=0 F  S X=$O(^APCHSURV(APCHHMR,11,G,11,X)) Q:X'=+X  S N=X
 S N=N+1  ;N is second level subscript
 S ^APCHSURV(APCHHMR,11,G,11,0)="^9001018.1111^"_N_"^"_N
 S ^APCHSURV(APCHHMR,11,G,11,N,0)=APCH2_U_APCH3_U_APCH4
 S DA=APCHHMR,DIK="^APCHSURV(" D IX^DIK
 Q
 Q
DHM ;delete pov
 W:$D(IOF) @IOF
