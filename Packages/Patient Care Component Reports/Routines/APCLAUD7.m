...
ADD ; SECTION FOR ADDING CODES TO TABLE
 I '$D(APCLTABL) S APCLTABL(APCL("LOW"))=APCL("HI") G X1
 S APCLDONE=0
 S APCLVAL="" F  S APCLVAL=$O(APCLTABL(APCLVAL)) Q:APCLVAL=""   D GETCASE,ADDPROC Q:APCLDONE
 D:'APCLDONE SETRANGE
...
SET12 ;;
SET13 ;;
 S APCLTABL(APCL("LOW"))=APCL("HI")
 Q
 ;
DEL ; SECTION FOR DELETING CODES FROM TABLE
 I '$D(APCLTABL) G X2
 S APCLDONE=0
 S APCLVAL="" F  S APCLVAL=$O(APCLTABL(APCLVAL)) Q:APCLVAL=""  D GETCASE,DELPROC Q:APCLDONE
X2 Q
 ;
...
DEL2 ;;
 ...
 E  S APCLTABL($O(^ICD9("BA",APCL("HI"))))=APCLTABL(APCLVAL) K APCLTABL(APCLVAL)
 S APCLDONE=1
 Q
 ;
DEL3 ;;
DEL6 ;;
 S APCLTABL($O(^ICD9("BA",APCL("HI"))))=APCLTABL(APCLVAL) K APCLTABL(APCLVAL)
 S APCLDONE=1
 Q
 ;
DEL4 ;;
...
DEL12 ;;
 ...
 S APCLTABL(APCLVAL)=APCL("NEWHI")
 I APCLCASE=10 S APCLDONE=1
 Q
 ;
DEL9 ;;
 D @$S($E(APCLTABL(APCLVAL))?1N:"NUMBER",1:"LETTER")
 S APCLTABL($O(^ICD9("BA",APCL("HI"))))=APCLTABL(APCLVAL)
 S APCLTABL(APCLVAL)=APCL("NEWHI")
 S APCLDONE=1
 Q
 ;
...
GETCASE ; SUBROUTINE TO DETERMINE APCLCASE # FROM INPUT CODE RANGE
 S APCL("TLOW")=APCLVAL,APCL("THI")=APCLTABL(APCLVAL)
