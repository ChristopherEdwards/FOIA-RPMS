APCLD71P ; IHS/CMI/LAB - 2007 DIABETES AUDIT PRINT ;
 ...
 S APCLPD=0,APCLGUIC=0 F  S APCLPD=$O(^XTMP("APCLDM71",APCLJOB,APCLBTH,"AUDIT",APCLPD)) Q:APCLPD'=+APCLPD!(APCLQUIT)  D
 .I $G(APCLGUI),APCLGUIC W !,"ZZZZZZZ",!  ;maw
 .S APCLGUIC=1
 .W:$D(IOF) @IOF
 .W $$CTR("ASSESSMENT OF DIABETES CARE, 2007         DATE AUDIT RUN: "_$$FMTE^XLFDT(APCLRED),80)
 ...
 .W !,$S($G(APCLPPN):"NAME: "_$P($G(^DPT(APCLPD,0)),U),1:"") S J=$S($G(APCLPPN):$L($P(^DPT(APCLPD,0),U))+10,1:0) W ?J,"DESIG PRIMARY CARE PROV:  ",$$I(15)
 ...
 .;D PAGE Q:APCLQUIT
CUML ;
 I APCLPREP=4!(APCLPREP=3) D CUML^APCLD714
DONE ;
 K ^TMP($J)
 K ^XTMP("APCLDM71",APCLJOB,APCLBTH),APCLJOB,APCLBTH
 Q
I(I) ;
 Q ^XTMP("APCLDM71",APCLJOB,APCLBTH,"AUDIT",APCLPD,I)
 ;
PAGE ;
