...
PPRC I $E(IOST)="C",IO=IO(0),$Y>(IOSL-4) W ! S DIR(0)="EO" D ^DIR K DIR I Y=0!(Y="^")!($D(DTOUT)) S APCLQUIT="" Q
 ...
 F I=1:1 Q:'$D(@APCLF@(I))  S APCLPRC=@APCLF@(I) W !?3,I,".",?7,$P($$ICDOP^ICDCODE(APCLPRC),U,2),?15,$P($$ICDOP^ICDCODE(APCLPRC),U,5),"  (",@APCLA,")" S APCLLINO=APCLLINO+1 I $Y>(IOSL-8) D FF I $D(X),X=U G PEXIT  ;cmi/anch/maw 9/12/2007 csv
