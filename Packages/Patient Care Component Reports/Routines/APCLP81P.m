APCLP81P ; IHS/CMI/LAB - 2003 DIABETES AUDIT PRINT ;
 ...
 S APCLPD=0,APCLGUIC=0 F  S APCLPD=$O(^XTMP("APCLP81",APCLJOB,APCLBTH,"AUDIT",APCLPD)) Q:APCLPD'=+APCLPD!(APCLQUIT)  D
 .I $G(APCLGUI),APCLGUIC W !,"ZZZZZZZ"
 .S APCLGUIC=1
 .W:$D(IOF) @IOF
 .W !,$$CTR("ASSESSMENT PREDIABETES/METABOLIC SYNDROME CARE, FY 2007",80)
 ...
 .W !,$S($G(APCLPPN):"NAME: "_$P($G(^DPT(APCLPD,0)),U),1:"") S J=$S($G(APCLPPN):$L($P(^DPT(APCLPD,0),U))+10,1:0) W ?J,"DPCP: ",$$I(15)
 .W !!,"CLASSIFICATION (all that apply):"
 .W !?1,"1 IFG - ",$$I(200)
 .S X=200 F  S X=$O(^XTMP("APCLP81",APCLJOB,APCLBTH,"AUDIT",APCLPD,X)) Q:X>200.99!(X="")  D
 ..W !?3,^XTMP("APCLP81",APCLJOB,APCLBTH,"AUDIT",APCLPD,X)
 .W !?1,"2 IGT - ",$$I(210)
 .S X=210 F  S X=$O(^XTMP("APCLP81",APCLJOB,APCLBTH,"AUDIT",APCLPD,X)) Q:X>210.99!(X="")  D
 ..W !?3,^XTMP("APCLP81",APCLJOB,APCLBTH,"AUDIT",APCLPD,X)
 .W !?1,"3 METABOLIC SYNDROME - ",$$I(220)
 .S X=220 F  S X=$O(^XTMP("APCLP81",APCLJOB,APCLBTH,"AUDIT",APCLPD,X)) Q:X>220.99!(X="")  D
 ..W !?3,^XTMP("APCLP81",APCLJOB,APCLBTH,"AUDIT",APCLPD,X)
 .W !?1,"OTHER ABNORMAL GLUCOSE (790.29) - ",$$I(230)
 .S X=230 F  S X=$O(^XTMP("APCLP81",APCLJOB,APCLBTH,"AUDIT",APCLPD,X)) Q:X>230.99!(X="")  D
 ..W !?3,^XTMP("APCLP81",APCLJOB,APCLBTH,"AUDIT",APCLPD,X)
 .W !?1,"CMS Register DX: ",$$I(22)_" "_$$I(24)
 .I $D(^XTMP("APCLP81",APCLJOB,APCLBTH,"AUDIT",APCLPD,25)) W !?1,^(25)
 .I $D(^XTMP("APCLP81",APCLJOB,APCLBTH,"AUDIT",APCLPD,26)) W !?1,^(26)
...
CUML ;
 I APCLPREP=2!(APCLPREP=3) D CUML^APCLP814
DONE ;
 K ^TMP($J)
 K ^XTMP("APCLP81",APCLJOB,APCLBTH),APCLJOB,APCLBTH
 Q
I(I) ;
 Q ^XTMP("APCLP81",APCLJOB,APCLBTH,"AUDIT",APCLPD,I)
 ;
PAGE ;
