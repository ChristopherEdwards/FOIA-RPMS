...
ASK1 ;
 S APCLA=0
 K APCL("LOW"),APCL("HI")
 S DIR("A")=$S('$D(APCLTBLE):"ENTER ICD PROCEDURE",1:"ENTER ANOTHER ICD PROCEDURE") D SETDIR^APCLTAXK,^DIR K DIR
 I "^"[Y G X1
 D PROCESS
...
PROCESS ;EVALUATE USER RESPONSE
 ...
 I $E(X)="[" D TAX G X2
 I X'["-" S APCLTYP="LOW",APCLONE=1 D LOOK^APCLTAXK G X2
 ...
 . S APCLSAVE("X")=X
 . I $L(X,"-")=3 S X=$P(APCLSAVE("X"),"-",2),APCLTYP="LOW" D LOOK^APCLTAXK I 'APCLA S X=$P(APCLSAVE("X"),"-",3),APCLTYP="HI" W ! D LOOK^APCLTAXK Q
 . I $L(APCLSAVE("X"),"-")=2 S X=$E(X,2,99),APCLTYP="LOW",APCLONE=1 D LOOK^APCLTAXK
 E  S APCLSAVE("X")=X S APCLTYP="LOW",X=$P(APCLSAVE("X"),"-") D LOOK^APCLTAXK I 'APCLA S APCLTYP="HI",X=$P(APCLSAVE("X"),"-",2) W ! D LOOK^APCLTAXK
X2 Q
 ;
...
RANGES ;DISPLAY TABLE OF ALL RANGES
 W:$D(IOF) @IOF
 W !!,"ICD Procedure Code Range(s) Selected So Far =>",!
