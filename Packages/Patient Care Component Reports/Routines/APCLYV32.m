...
PRV S APCLPRV=$O(^AUPNVPRV("AD",APCLVDFN,APCLPRV))
 ...
 I $P(^DD(9000010.06,.01,0),U,2)[200 S APCLCLSC=$$PROVCLSC^XBFUNC1(X) W ?41,$E(APCLCLSC,1,3) G PRVJMP
 S X=$P(^DIC(6,X,0),"^",4)
...
POV S APCLPV=$O(^AUPNVPOV("AD",APCLVDFN,APCLPV))
 ...
 W ?45,APCLVPOV,?56,$E(APCLNAR,1,23)
 G POV
 ;
...
PRC S APCLVPRC=$O(^AUPNVPRC("AD",APCLVDFN,APCLVPRC))
 ...
 I APCLFPV W ! I $Y>(IOSL-5) D PAGE Q:APCLSTOP="^"  W !
 I 'APCLFPV S APCLFPV=1
 W ?45,APCLPRC,?56,$E(APCLNAR,1,23)
 G PRC
 ;
FINAL ;
 Q:APCLCL'="A"
...
HD1 S Y=DT X ^DD("DD") W !,Y
 ...
 W !!,"NAME",?16,"HRCN",?22,"AGE",?27,"VISIT DATE",?41,"PRV",?46,"ICD",?56,"PROV NARRATIVE",!
 Q
 ;
