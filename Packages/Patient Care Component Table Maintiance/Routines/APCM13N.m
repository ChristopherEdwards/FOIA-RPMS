APCM13N ;IHS/CMI/LAB - MU PATIENT LIST;
...
INTRO ;
 ...
 W !,$$CTR("IHS 2013 Stage 1 MU Patient List for Hospitals/CAHs")
 ...
 I APCMINDZ="A" S X=0 F  S X=$O(^APCM13OB(X)) Q:X'=+X  I $P(^APCM13OB(X,0),U,2)="H" S APCMIND(X)=""
SI D LISTS
SI1 ;NOW SELECT ONE OR MORE W/IN THE TOPIC
 ...
 K APCMINDO
 S APCMIND=0 F  S APCMIND=$O(APCMLIST(APCMIND)) Q:APCMIND'=+APCMIND  S APCMINDO($P(^APCM13OB(APCMIND,0),U,4),APCMIND)=""
 S APCMORD=0 F  S APCMORD=$O(APCMINDO(APCMORD)) Q:APCMORD'=+APCMORD!(APCMQ)!($D(DIRUT))  D
 .S APCMIND=$O(APCMINDO(APCMORD,0))
 .S APCMCR="AH"
 .K APCMX S APCMO=0,X=0,APCMC=0 F  S APCMO=$O(^APCMM13L(APCMCR,APCMIND,APCMO)) Q:APCMO'=+APCMO!($D(DIRUT))  D
 ..S X=$O(^APCMM13L(APCMCR,APCMIND,APCMO,0))
 ..;I APCMRPTT=1,$P(^APCMM13L(X,0),U,4)'="N" Q
 ..;I APCMRPTT=7,$P(^APCMM13L(X,0),U,4)'="O" Q
 ..S APCMX(APCMO,X)="",APCMC=APCMC+1
 .;display the choices
 .W !!!,"Please select one or more of these report choices within the",!,IORVON,$P(^APCM13OB(APCMIND,0),U,5),IORVOFF," objective.",!
 .K APCMY S X=0,APCMC=0,APCMO=0 F  S APCMO=$O(APCMX(APCMO)) Q:APCMO'=+APCMO!($D(DIRUT))  S X=0 F  S X=$O(APCMX(APCMO,X)) Q:X'=+X!($D(DIRUT))  S APCMC=APCMC+1 W !?5,APCMC,")",?9,$P(^APCMM13L(X,0),U,3) S APCMY(APCMC)=X
 ...
 ;get report type
 I $D(DIRUT) G SI
 K APCMQUIT ;D RT^APCM13SL I '$D(APCMLIST)!($D(APCMQUIT)) G SI
RT ;
 S APCMSUM=""
...
ATTEST ;get answers to attestation questions for each provider.
 ...
 .D
 ..I '$P(^APCM13OB(APCMY,0),U,13) S DIR(0)="Y",DIR("A")="Does "_$E($P(^DIC(4,APCMFAC,0),U,1),1,25)_" attest to this",DIR("B")="Y" KILL DA D ^DIR KILL DIR
 ..I $P(^APCM13OB(APCMY,0),U,13) S DIR(0)="S^Y:YES;N:NO;X:No Registry Available",DIR("A")="Does "_$E($P(^DIC(4,APCMFAC,0),U,1),1,25)_" attest to this",DIR("B")="Y" KILL DA D ^DIR KILL DIR
 ..I $D(DIRUT) S APCMQ=1 Q
...
DRIVER ;
 ...
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^APCM13N",XBRX="XIT^APCM13N",XBNS="APCM"
 D ^XBDBQUE
 Q
 ;
NODEV1 ;
...
A ;fiscal year
 ...
 W !!,"Enter the Federal Fiscal Year for which report is to be run.  Use a 4 digit",!,"year, e.g. 2013."
 S DIR(0)="D^::EP"
 S DIR("A")="Enter Year"
