...
EN ;EP -- main entry point for SELECTION
 D EN^VALM("APCM 13 S1 INDICATOR SEL")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ; -- init variables and list array
 ...
 S (X,Y,Z,C)=0 F  S X=$O(^APCM13OB("EOORDER",X)) Q:X'=+X  F  S Y=$O(^APCM13OB(APCMXREF,X,Y)) Q:Y'=+Y  D
 .S C=C+1,APCMTIND(C,0)=C_")",$E(APCMTIND(C,0),5)="("_$P(^APCM13OB(Y,0),U,3)_") "_$P(^APCM13OB(Y,0),U,5),APCMTIND(C,C)=Y I $D(APCMIND(Y)) S APCMTIND(C,0)="*"_APCMTIND(C,0)
 .Q
...
ADDALL ;
 ...
 ;
MS ;
 F X=1:1:APCMHIGH I $P($G(^APCM13OB(X,0)),U,3)="M" S APCMIND(X)=""
 D BACK
 Q
 ;
CM ;
 F X=1:1:APCMHIGH I $P($G(^APCM13OB(X,0)),U,3)="C" S APCMIND(X)=""
 D BACK
 Q
 ;
REM ;
