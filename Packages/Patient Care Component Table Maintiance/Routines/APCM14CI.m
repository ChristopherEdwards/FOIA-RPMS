APCM14CI ;IHS/CMI/LAB - MU REPORT;
...
CALCIND ;EP - CALCULATE ALL MEASURES
 ...
 .Q:$P(^APCM14OB(APCMIC,0),U,6)="A"
 .Q:$P(^APCM14OB(APCMIC,0),U,12)=0
 .K APCMSTOP,APCMVAL,APCMVALU,A,B,C,D,E,F,G,H,I,J,K,M,N,O,P,Q,R,S,T,V,W,X,Y,Z
 .S APCMVALU=""
 .I $D(^APCM14OB(APCMIC,1)) X ^APCM14OB(APCMIC,1)
 Q
CALCINDA ;EP - CALCULATE ATTESTATION MEASURES
 S APCMIC=0 F  S APCMIC=$O(APCMIND(APCMIC)) Q:APCMIC'=+APCMIC  D
 .;Q:$P(^APCM14OB(APCMIC,0),U,6)="R"
 .Q:$P(^APCM14OB(APCMIC,0),U,12)=1
...
S1(BQITYP,BQIVAL) ; Return data by patient for iCare into global reference BQIGREF
 ...
 S BQIIDTA=$G(^APCM14OB(APCMI,0))
 S BQIDTA=$G(^APCM14OB(APCMI,14))
 ;
 ...
 S $P(@BQIGREF@(DFN,APCMIC),U,1)=$P(^APCM14OB(APCMIC,0),U,3)
...
HADVH(P,R,BD,ED,VSTS,APCMMETH) ;EP - had visit H or ER A,O,S,M
 ...
 .Q:$$LOCENC^APCLV(V)'=APCMFAC  ;should be to facility report is being run for as there may be multiple ERs on multi divisional databases
 .S G=$$VD^APCLV(V)
 Q G  ;quit on the date of the visit
