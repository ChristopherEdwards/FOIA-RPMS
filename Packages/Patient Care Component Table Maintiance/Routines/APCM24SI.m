...
EN ;EP -- main entry point for SELECTION
 D EN^VALM("APCM 24 S2 INDICATOR SEL")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
 ...
 ;
HDR ; -- header code
 S VALMHDR(1)="IHS 2014/2015 Stage 2 MU Performance Measures"
...
INIT ; -- init variables and list array
 ...
 S (X,Y,Z,C)=0 F  S X=$O(^APCM24OB(APCMXREF,X)) Q:X'=+X  F  S Y=$O(^APCM24OB(APCMXREF,X,Y)) Q:Y'=+Y  D
 .S C=C+1,APCMTIND(C,0)=C_")",$E(APCMTIND(C,0),5)="("_$P(^APCM24OB(Y,0),U,3)_") "_$P(^APCM24OB(Y,0),U,5),APCMTIND(C,C)=Y I $D(APCMIND(Y)) S APCMTIND(C,0)="*"_APCMTIND(C,0)
 .Q
...
ADDALL ;
 ...
 ;
MS ;
 S X=0 F  S X=$O(APCMTIND(X)) Q:X'=+X  S Y=APCMTIND(X,X) I $P(^APCM24OB(Y,0),U,3)="M" S APCMIND(APCMTIND(X,X))=""
 D BACK
 Q
 ;
CM ;
 S X=0 F  S X=$O(APCMTIND(X)) Q:X'=+X  S Y=APCMTIND(X,X) I $P(^APCM24OB(Y,0),U,3)="C" S APCMIND(APCMTIND(X,X))=""
 D BACK
 Q
REM ;
