...
VS ;EP - CALCULATE VITAL SIGNS
 ...
 S APCMFIED=$P(^APCM25OB(APCMIC,0),U,8)
 S APCMFIEN=$P(^APCM25OB(APCMIC,0),U,9)
 S APCMBPON=0,APCMNOHW=0
 S (APCMD1,APCMN1)=0
 I APCMRPTT=1 D  Q
 .S APCMP=0 F  S APCMP=$O(APCMPRV(APCMP)) Q:APCMP'=+APCMP  D
 ..;S F=$P(^APCM25OB(APCMIC,0),U,18),APCMNP=$P(^DD(9001304.0311,F,0),U,4),N=$P(APCMNP,";"),P=$P(APCMNP,";",2)
 ..;S APCME=$$V^APCM25ER(1,APCMRPT,N,P,APCMP,"I",APCMRPTT) Q:APCME="X"  ;don't bother as this one is excluded
 ..;D S^APCM25E1(APCMRPT,APCMIC,1,APCMP,APCMRPTT,APCMTIME,F,1)
 ..S APCMFIED=$P(^APCM25OB(APCMIC,0),U,8)
 ..S APCMFIEN=$P(^APCM25OB(APCMIC,0),U,9)
 ..S APCMBPON=0,APCMNOHW=0
 ..I $G(APCMADDQ("ANS",APCMIC,24,APCMP))="Yes" D  Q
 ...;SET EXCLUSION
 ...S F=$P(^APCM25OB(APCMIC,0),U,11) D S^APCM25E1(APCMRPT,APCMIC,"Yes",APCMP,APCMRPTT,APCMTIME,F,1)
 ...;SET SKIP
 ...S F=$P(^APCM25OB(APCMIC,0),U,18)
 ...D S^APCM25E1(APCMRPT,APCMIC,"X",APCMP,APCMRPTT,APCMTIME,F,1)
 ...;set associated measures with skip "X"
 ...S X=0 F  S X=$O(^APCM25OB(APCMIC,29,X)) Q:X'=+X  D
 ....S Y=$P(^APCM25OB(APCMIC,29,X,0),U,1)
 ....S Y=$O(^APCM25OB("B",Y,0))
 ....S F=$P(^APCM25OB(Y,0),U,18)
 ....D S^APCM25E1(APCMRPT,Y,"X",APCMP,APCMRPTT,APCMTIME,F,1)
 ..I $G(APCMADDQ("ANS",APCMIC,25,APCMP))="Yes"!($G(APCMADDQ("ANS",APCMIC,27,APCMP))="Yes") D  G VSC
 ...S Z=$O(^APCM25OB("B","S2.008.EP.2",0)),APCMFIED=$P(^APCM25OB(Z,0),U,8),APCMFIEN=$P(^APCM25OB(Z,0),U,9)
 ...S APCMNOBP=1  ;no BP use S2.008.EP.2
 ...F Z="S2.008.EP.1","S2.008.EP" S Y=$O(^APCM25OB("B",Z,0)) D
 ....S F=$P(^APCM25OB(Y,0),U,18)
 ....D S^APCM25E1(APCMRPT,Y,"X",APCMP,APCMRPTT,APCMTIME,F,1)
 ....I Z="S2.008.EP" Q
 ....S F=$P(^APCM25OB(Y,0),U,11)
 ....D S^APCM25E1(APCMRPT,Y,"Yes",APCMP,APCMRPTT,APCMTIME,F,1)
 ...S Y=$O(^APCM25OB("B","S2.008.EP.2",0))
 ...S F=$P(^APCM25OB(Y,0),U,11)
 ...D S^APCM25E1(APCMRPT,Y,"N/A",APCMP,APCMRPTT,APCMTIME,F,1)
 ..I $G(APCMADDQ("ANS",APCMIC,28,APCMP))="Yes" D  G VSC
 ...S APCMNOHW=1  ;no BP use S2.008.EP.1
 ...S Z=$O(^APCM25OB("B","S2.008.EP.1",0)),APCMFIED=$P(^APCM25OB(Z,0),U,8),APCMFIEN=$P(^APCM25OB(Z,0),U,9)
 ...F Z="S2.008.EP","S2.008.EP.2" S Y=$O(^APCM25OB("B",Z,0)) D
 ....S F=$P(^APCM25OB(Y,0),U,18)
 ....D S^APCM25E1(APCMRPT,APCMIC,"X",APCMP,APCMRPTT,APCMTIME,F,1)
 ....I Z="S2.008.EP" Q
 ....S F=$P(^APCM25OB(Y,0),U,11)
 ....D S^APCM25E1(APCMRPT,Y,"Yes",APCMP,APCMRPTT,APCMTIME,F,1)
 ...S Y=$O(^APCM25OB("B","S2.008.EP.1",0))
 ...S F=$P(^APCM25OB(Y,0),U,11)
 ...D S^APCM25E1(APCMRPT,Y,"N/A",APCMP,APCMRPTT,APCMTIME,F,1)
 ..I $G(APCMADDQ("ANS",APCMIC,24,APCMP))="No" D  G VSC
 ...F Z="S2.008.EP.1","S2.008.EP.2" S Y=$O(^APCM25OB("B",Z,0)) D
 ....S F=$P(^APCM25OB(Y,0),U,18)
 ....D S^APCM25E1(APCMRPT,APCMIC,"X",APCMP,APCMRPTT,APCMTIME,F,1)
VSC ..;
 ..Q:'$D(APCMHVTP(APCMP))  ;no visits to this provider for this patient so don't bother, the patient is not in the denominator
...
VS1 ;set denominator value into field
 ...
 S F=APCMFIED ;$P(^APCM25OB(APCMIC,0),U,8)  ;denom field for this measure
 D S^APCM25E1(APCMRPT,APCMIC,1,APCMP,APCMRPTT,APCMTIME,F)
 S APCMVALU="VISIT: "_$$DATE^APCM1UTL(APCMHVTP(APCMP))
 ;numerator?
 ...
 S APCMVALU=APCMVALU_"|||"_$P(APCMEP,U,2)_"|||"_$P(APCMEP,U,1)
 S F=APCMFIEN ;$P(^APCM25OB(APCMIC,0),U,9)
 D S^APCM25E1(APCMRPT,APCMIC,$P(APCMEP,U,1),APCMP,APCMRPTT,APCMTIME,F)
 D SETLIST^APCM25E1
 Q
...
ST ;EP - CALCULATE SMOKING STATUS
 ...
 I APCMRPTT=1 D  Q
 .S APCMP=0 F  S APCMP=$O(APCMPRV(APCMP)) Q:APCMP'=+APCMP  D
 ..I $D(APCM25ON(APCMP,APCMTIME)) S F=$P(^APCM25OB(APCMIC,0),U,11) D S^APCM25E1(APCMRPT,APCMIC,"Provider is excluded from this measure as he/she did not see anyone over 13 during the EHR reporting period.",APCMP,APCMRPTT,APCMTIME,F,1) Q
 ..Q:$$AGE^AUPNPAT(DFN,APCMBDAT)<13
 ..Q:'$D(APCMHVTP(APCMP))  ;no visits to this provider for this patient so don't bother, the patient is not in the denominator
 ..D ST1
 I APCMRPTT=2 D
 .S APCMP=APCMFAC
 .I $D(APCM25ON(APCMP,APCMTIME)) S F=$P(^APCM25OB(APCMIC,0),U,11) D S^APCM25E1(APCMRPT,APCMIC,"Hospital is excluded from this measure as did not admit anyone over 13 during the EHR reporting period.",APCMP,APCMRPTT,APCMTIME,F,1) Q
 .Q:$$AGE^AUPNPAT(DFN,APCMBDAT)<13
 .Q:'$D(APCMHVTP(APCMP))
 .D ST1
 .Q
...
CS ;EP - CLINICAL SUMMARIES ON EACH VISIT
 ...
 .I $D(APCMOFFV(APCMP,APCMTIME)) S F=$P(^APCM25OB(APCMIC,0),U,11) D
 ...
 .S F=$P(^APCM25OB(APCMIC,0),U,8)  ;denom field for this measure
 .D S^APCM25E1(APCMRPT,APCMIC,$P(APCMEP,U),APCMP,APCMRPTT,APCMTIME,F)
 .;S APCMVALU="VISIT: "_$$DATE^APCM1UTL(APCMHVTP(APCMP))
