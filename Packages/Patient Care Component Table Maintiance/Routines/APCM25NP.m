...
NONE ;
 ...
 D H1
 D W^APCM25EH(" ",0,0,APCMPTYP)
 D W^APCM25EH("Total # of patients on list: 0",0,0,APCMPTYP)
 Q
NPL11 ;
 D
NEXT .;
 ...
 .;.D W^APCM25EH(^APCMM25L(APCMLIEN,11,APCMX,0),0,1,APCMPTYP)
...
NEXT1 ;
 ...
 S APCMCNT=10
GO ;
 ;D W^APCM25EH($P(^APCM25OB(APCMINDB,0),U,5),0,1,APCMPTYP)
 ...
 D W^APCM25EH("Total # of patients on list: "_+$G(APCMPCNT),0,2,APCMPTYP)
...
PRINTL ;print one line
 ...
 D
 .D W^APCM25EH($E($P(^DPT(DFN,0),U),1,22),0,1,APCMPTYP)
 .D W^APCM25EH($$HRN^AUPNPAT(DFN,DUZ(2)),0,0,APCMPTYP,2,24)
 .D W^APCM25EH($E(APCMP,1,24),0,0,APCMPTYP,3,31)
 .D W^APCM25EH(APCMCOM,0,0,APCMPTYP,4,57)
 .D W^APCM25EH(APCMSEX,0,0,APCMPTYP,5,70)
 .D W^APCM25EH(APCMAGE,0,0,APCMPTYP,6,75)
 .D W^APCM25EH($P(^XTMP("APCM1D",APCMJ,APCMH,"LIST",APCMORD,APCMINDB,APCMLORD,APCMLIEN,APCMP,APCMCOM,APCMSEX,APCMAGE,DFN),"|||",1),0,$S(APCMPTYP="P":1,1:0),APCMPTYP,7,1)
 .D W^APCM25EH($P(^XTMP("APCM1D",APCMJ,APCMH,"LIST",APCMORD,APCMINDB,APCMLORD,APCMLIEN,APCMP,APCMCOM,APCMSEX,APCMAGE,DFN),"|||",2),0,0,APCMPTYP,8,40)
PRINTL1 .;D W^APCM25EH(" ",0,1,APCMPTYP)
 K ^TMP($J,"A")
 Q
 ;
HEADER ;EP
...
HEADER1 ;
 ...
 D W^APCM25EH($$CTR("***** CONFIDENTIAL PATIENT INFORMATION, COVERED BY THE PRIVACY ACT *****",80),0,1,APCMPTYP)
 I APCMPTYP="P" W !?3,$P(^VA(200,DUZ,0),U,2),?35,$$FMTE^XLFDT(DT),?70,"Page ",APCMGPG,!
 I APCMRPTT=1 D W^APCM25EH($$CTR("** IHS Modified Stage 2 Meaningful Use Performance Measure Report for EPs **",80),0,1,APCMPTYP)
 I APCMRPTT=2 D W^APCM25EH($$CTR("** IHS Modified Stage 2 MU Performance Report for Eligible Hospitals/CAHs **",80),0,1,APCMPTYP)
 D W^APCM25EH($$CTR($P(^DIC(4,DUZ(2),0),U),80),0,1,APCMPTYP)
 S X="Reporting Period: "_$$FMTE^XLFDT(APCMBD)_" to "_$$FMTE^XLFDT(APCMED) D W^APCM25EH($$CTR(X,80),0,1,APCMPTYP)
 ;W !,$$CTR($P(^APCM25OB(APCMINDB,0),U,5))
 D W^APCM25EH($TR($J("",80)," ","-"),0,1,APCMPTYP)
 Q
H1 ;
 ;S X=$S(APCMLIST="A":"Entire Patient List",APCMLIST="R":"Random Patient List",1:"Patient List by Provider: "_APCMLPROV) D W^APCM25EH(X,0,1,APCMPTYP)
 D W^APCM25EH($P(^APCM25OB(APCMINDB,0),U,5),0,$S(APCMPTYP="D":3,1:1),APCMPTYP)
 S X=0 F  S X=$O(^APCMM25L(APCMLIEN,11,X)) Q:X'=+X  D W^APCM25EH(^APCMM25L(APCMLIEN,11,X,0),0,1,APCMPTYP)
 ;S N=$S(APCMCOUN<2:22,1:22) S X=0 F  S X=$O(^APCM25OB(APCMINDB,N,X)) Q:X'=+X  D W^APCM25EH(^APCM25OB(APCMINDB,N,X,0),0,1,APCMPTYP)
 D W^APCM25EH("PATIENT NAME",0,2,APCMPTYP)
 D W^APCM25EH("HRN",0,0,APCMPTYP,2,24)
 D W^APCM25EH($S(APCMRPTT=1:"EP",1:"HOSPITAL/CAH"),0,0,APCMPTYP,3,31)
 D W^APCM25EH("COMMUNITY",0,0,APCMPTYP,4,57)
 D W^APCM25EH("SEX",0,0,APCMPTYP,5,70)
 D W^APCM25EH("AGE",0,0,APCMPTYP,6,75)
 D W^APCM25EH("DENOM",0,$S(APCMPTYP="P":1,1:0),APCMPTYP,7,3)
 D W^APCM25EH("NUMERATOR",0,0,APCMPTYP,8,40)
 D W^APCM25EH($TR($J("",80)," ","-"),0,1,APCMPTYP)
 Q
