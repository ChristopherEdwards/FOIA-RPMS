...
HDR1 ;
 ...
 I APCMRPTT=1 D W("*** IHS Modified Stage 2 Meaningful Use Performance Measure Report for EPs ***",1,1,APCMPTYP)
 D W("Alternate Stage 1 Exclusions",1,1,APCMPTYP)
 I APCMRPTT=2 D W("** IHS Modified Stage 2 MU Performance Report for Eligible Hospitals/CAHs **",1,1,APCMPTYP)
 ...
 I APCMHPG'=1 D W^APCM2AEH(" ",0,2,APCMPTYP) Q
 I APCMRPTT=2 D W^APCM2AEH(" ",0,2,APCMPTYP) Q
 ...
 F  S X=$O(K(X)) Q:X=""  D
 .I Y="" S $E(Y,3)=$$SN(X) Q
 .I Y]"" S $E(Y,40)=$$SN(X) D W^APCM2AEH(Y,0,1,APCMPTYP) S Y=""
 I Y]"" D W^APCM2AEH(Y,0,1,APCMPTYP)
 ;D ENDTIME
 Q
SN(N) ;EP
 Q $P(N,",",1)_", "_$P(N,",",2)
