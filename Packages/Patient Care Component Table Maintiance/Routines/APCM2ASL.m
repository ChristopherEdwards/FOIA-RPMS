...
EN ;EP -- main entry point for GPRA LIST DISPLAY
 D EN^VALM("APCM 25 S2 LIST SELECTION")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ1
 ...
 ;
HDR ; -- header code
 S VALMHDR(1)="IHS Modified Stage 2 MU Performance Measure Lists of Patients"
...
INIT ; -- init variables and list array
 ...
 S (Y,C,I)=0 F  S Y=$O(^APCM25OB(APCMXREF,Y)) Q:Y'=+Y  Q:Y=""  S X=0 F  S X=$O(^APCM25OB(APCMXREF,Y,X)) Q:X'=+X  D
 .Q:'$D(APCMIND(X))
 .I $P(^APCM25OB(X,0),U,7)="" S C=C+1 D  Q
 ..S APCMGLST(C,0)=C_")",$E(APCMGLST(C,0),5)=$P(^APCM25OB(X,0),U,5),APCMGLST("IDX",C,C)=X I $D(APCMLIST(X)) S APCMGLST(C,0)="*"_APCMGLST(C,0)
 .I $P(^APCM25OB(X,0),U,7)=1 Q  ;S C=C+1 D
 .;.S APCMGLST(C,0)="NO patient list available for measure:  "_$P(^APCM25OB(X,0),U,5),APCMGLST("IDX",C,C)=X,APCMNOLI(X)="" I $D(APCMLIST(X)) S APCMGLST(C,0)="*"_APCMGLST(C,0)
...
REPORT ;EP
 ...
 L +^APCMM25C:30 I '$T W !!,"Unable to lock global, try later." G REPORTX
 K DIC S X=APCMBD,DIC(0)="L",DIC="^APCMM25C(",DLAYGO=9001304.03,DIADD=1,DIC("DR")=".02////"_APCMED_";.05////"_DUZ(2)_";.06////"_$S(APCMRPT=1:"E",1:"H")_";.07////"_$$NOW^XLFDT()
 ...
 I APCMRPTT=1 D  S ^APCMM25C(APCMRPT,11,0)="^9001304.0311AV^"_C_"^"_C
 .S X=0,C=0 F  S X=$O(APCMPRV(X)) Q:X'=+X  S C=C+1 D
 ..S ^APCMM25C(APCMRPT,11,C,0)=X_";VA(200,"
 ..S ^APCMM25C(APCMRPT,11,"B",X_";VA(200,",C)=""
 I APCMRPTT=2 D  S ^APCMM25C(APCMRPT,11,0)="^9001304.0311AV^"_C_"^"_C
 .S X=0,C=1,X=APCMFAC D
 ..S ^APCMM25C(APCMRPT,11,C,0)=X_";AUTTLOC("
 ..S ^APCMM25C(APCMRPT,11,"B",X_";AUTTLOC(",C)=""
REPORTX ;
 D ^XBFMK
