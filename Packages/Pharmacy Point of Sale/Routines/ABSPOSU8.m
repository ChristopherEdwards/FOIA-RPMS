...
EOPQ(LINESBOT,PARAM,EOPXCODE) ;EP -
 ...
 N X,Y,%,DIR
 ;
 I '$G(IOSL) Q:$Q 0 Q  ;if we don't know page length, then not at end
 S LINESBOT=$S($G(LINESBOT):LINESBOT,1:6)
 I ($Y+LINESBOT)<IOSL Q:$Q 0 Q  ;not at end of page
 ; -- Okay, we're at end of page
 I $G(PARAM)["M" W !,?($S($G(IOM):IOM,1:80)-12),"-- More --"
 ;
 I '$$PAUSE Q:$Q 1 Q  ;user wants out
 X $G(EOPXCODE)
 ;
 Q:$Q 0 Q
 ;======================================================================
PAUSE() ;3/31/93
 ...
 ;I $E(IOST,1)'="C" Q:$Q 1 Q
 I '$$TOSCREEN^ABSPOSU5 Q:$Q 1 Q
 K DIR
 S DIR(0)="E" D ^DIR
 Q:$Q Y Q  ;Y=1 to continue, 0 to quit.
 ;===================================================================
ENDRPT() ;EP - end of report.  Pause until user presses return (or timeout)
 I '$$TOSCREEN^ABSPOSU5 W:$Y @IOF Q:$Q 1 Q
...
DEVICE(DEV,RTN,TITLE,MULTI) ;EP
 ...
 ;No parameters are required.  DEV can be set alone, or if queing
 ;set to variables needed for queing.
 ;DEV   - DEFAULT device, "HOME" if undefined.
 ;RTN   - Routine name if queing is selected.
 ;TITLE - Description for the task log if queing is selected.
 ...
 W !!
 S ZTSAVE("PAGE")=""
 I $D(RTN) S %ZIS="QM" ; Ask if queing is allowed only if RTN is set.
 ...
 S PAGE=0
 I '$D(IO("Q")) U IO S Y=1 G DEVQ ;Queing not selected
 S ZTRTN=RTN ;Routine entry point for queing.
 S ZTIO=ION ;Output device for queing.
 S ZTDESC=$G(TITLE) ;Report title if queing is selected.
 S ZTSAVE("*")="" ;All variables in memory for queing.
...
HEADER(PROGRAM,TITLE1,TITLE2,RUNTIME,NOFF,UL) ;
 ...
 I $G(TITLE2)'="." DO
 . I $D(TITLE1) D WCENTER^ABSPOSU9(TITLE1)
 . I $D(TITLE2) D WCENTER^ABSPOSU9(TITLE2)
 I $G(TITLE2)="." DO
 . S N=""
 . F  S N=$O(TITLE1(N)) Q:N=""  D WCENTER^ABSPOSU9($G(TITLE1(N)))
