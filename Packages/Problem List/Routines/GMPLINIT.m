GMPLINIT	; ; 25-AUG-1994
	;;2.0;Problem List;;Aug 25, 1994
	;
	K DIF,DIFQ,DIFQR,DIFQN,DIK,DDF,DDT,DTO,D0,DLAYGO,DIC,DIDUZ,DIR,DA,DIFROM,DFR,DTN,DIX,DZ,DIRUT,DTOUT,DUOUT
	S U="^",DIFQ=0,DIFROM="2.0" W !,"This version (#2.0) of 'GMPLINIT' was created on 25-AUG-1994"
	W !?9,"(at SLC, by VA FileMan V.20.0)",!
...
GO	;
	W !,"I HAVE TO RUN AN ENVIRONMENT CHECK ROUTINE." D PKG,^GMPLIENV Q:'$D(DIFQ)  D NOW^%DTC S DIFROM("PRE")=%
EN	; ENTER HERE TO BYPASS THE PRE-INIT PROGRAM
	S DIFQ=0 K DIRUT,DTOUT,DUOUT
	F DIFRIR=1:1:1 S DIFRRTN="^GMPLINI"_$E("5",DIFRIR) D @DIFRRTN
	W:1 !,"I AM GOING TO SET UP THE FOLLOWING FILES:" F I=1:2:18 S DIF(I)=^UTILITY("DIF",$J,I) D 1 G Q:DIFQ!$D(DIRUT) K DIF(I)
	S DIFROM="2.0" D PKG:'$D(DIFROM(0)),^GMPLINI1 G Q:'$D(DIFQ) S DIK(0)="AB"
	F DIF=1:2:18 S %=^UTILITY("DIF",$J,DIF),DIK=$P(%,";",5),N=$P(%,";",3),D=$P(%,";",4)_U_N D D K DIFQ(N)
	K DIFQR D ^GMPLINI2,^GMPLINI3
	L  S DUZ=DIDUZ W:1 !,"NO"_$P("TE THAT FILE",U,DSEC)_" SECURITY-CODE PROTECTION HAS BEEN MADE"
	D ^GMPLIPST,NOW^%DTC S DIFROM("INIT")=%
	I DIFROM F DIF=1:2:18 S %=^UTILITY("DIF",$J,DIF),N=+$P(%,";",3) I N,$P(%,";",8)="y" S ^DD(N,0,"VR")=DIFROM
	...
	I DIFROM,$D(^%ZTSK) S X="GMPLINIS" X ^%ZOSF("TEST") D:$T PAC^GMPLINIS($T(IXF),.DIFROM)
