AQAOT07 ; GENERATED FROM 'AQAO ACT REOPEN' INPUT TEMPLATE(#1141), FILE 9002168.5;05/13/96
 D DE G BEGIN
DE S DIE="^AQAO(5,",DIC=DIE,DP=9002168.5,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^AQAO(5,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(1)=% S %=$P(%Z,U,5) S:%]"" DE(2)=% S %=$P(%Z,U,6) S:%]"" DE(13)=% S %=$P(%Z,U,11) S:%]"" DE(14)=% S %=$P(%Z,U,13) S:%]"" DE(6)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET I X'?.ANP S DDER=1 Q 
 ...
 Q
BEGIN S DNM="AQAOT07",DQ=1
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=1141,U="^"
...
X1 K:$L(X)>10!($L(X)<1)!'(X'?1P.E) X
 ...
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;5",DV="RS",DU="",DLB="PLAN STATUS",DIFLD=.05
 S DE(DW)="C2^AQAOT07"
...
X3 S AQAOSTAT=$P(^AQAO(5,D0,0),U,5)
 Q
...
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="0;13",DV="RF",DU="",DLB="DELETION REASON",DIFLD=.13
 G RE
X6 K:$L(X)>30!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
