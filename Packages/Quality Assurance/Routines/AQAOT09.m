AQAOT09 ; GENERATED FROM 'AQAO FINDINGS' INPUT TEMPLATE(#1137), FILE 9002168.8;05/13/96
 D DE G BEGIN
DE S DIE="^AQAO(8,",DIC=DIE,DP=9002168.8,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^AQAO(8,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(2)=% S %=$P(%Z,U,2) S:%]"" DE(3)=% S %=$P(%Z,U,3) S:%]"" DE(4)=% S %=$P(%Z,U,4) S:%]"" DE(6)=% S %=$P(%Z,U,5) S:%]"" DE(5)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET I X'?.ANP S DDER=1 Q 
 ...
 Q
BEGIN S DNM="AQAOT09",DQ=1
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=1137,U="^"
...
C2 G C2S:$D(DE(2))[0 K DB S X=DE(2),DIC=DIE
 K ^AQAO(8,"B",$E(X,1,30),DA)
C2S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 S ^AQAO(8,"B",$E(X,1,30),DA)=""
 Q
...
C3 G C3S:$D(DE(3))[0 K DB S X=DE(3),DIC=DIE
 K ^AQAO(8,"C",$E(X,1,30),DA)
C3S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 S ^AQAO(8,"C",$E(X,1,30),DA)=""
 Q
...
X4 K:+X'=X!(X>9999)!(X<1)!(X?.E1"."1N.N) X
 Q
 ;
5 S DW="0;5",DV="S",DU="",DLB="TYPE OF FINDING",DIFLD=.05
 S DU="1:EXCEPTION TO CRITERIA;"
 G RE
X5 Q
6 S DW="0;4",DV="S",DU="",DLB="INACTIVE?",DIFLD=.04
 S DU="I:INACTIVE;"
 G RE
X6 Q
7 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=7 D X7 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X7 S DLAYGO=9002168.8
 Q
8 G 0^DIE17
