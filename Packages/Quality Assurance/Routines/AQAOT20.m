AQAOT20 ; GENERATED FROM 'AQAO SURG PROC FIRST' INPUT TEMPLATE(#1172), FILE 9002166.9;05/13/96
 D DE G BEGIN
...
SET I X'?.ANP S DDER=1 Q 
 ...
 Q
BEGIN S DNM="AQAOT20",DQ=1
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=1172,U="^"
1 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=1 D X1 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X1 S AQAOPCZ=$P(^AQAOCC(9,DA,0),U)
 Q
2 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=2 D X2 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X2 W !,$P(^ICD0(AQAOPCZ,0),U),"  ",$P(^(0),U,4)," has been added . . ."
 Q
3 S DW="0;5",DV="S",DU="",DLB="RELATED TO?",DIFLD=.05
