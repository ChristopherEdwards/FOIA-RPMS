...
DE S DIE="^AQAGP(",DIC=DIE,DP=9002166.4,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^AQAGP(DA,""))=""
 I $D(^("ADT")) S %Z=^("ADT") S %=$P(%Z,U,73) S:%]"" DE(1)=% S %=$P(%Z,U,74) S:%]"" DE(2)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET I X'?.ANP S DDER=1 Q 
 ...
 Q
BEGIN S DNM="AQAOT232",DQ=1
1 S DW="ADT;73",DV="RF",DU="",DLB="RETURN TO ICU GLOBAL REF",DIFLD=1073
 S Y="2,AQAOTR,0"
...
X1 K:$L(X)>10!($L(X)<1) X
 ...
 Q
 ;
2 S DW="ADT;74",DV="RS",DU="",DLB="RETURN TO ICU DUP OK",DIFLD=1074
 S DU="1:YES;0:NO;"
 S Y="YES"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
