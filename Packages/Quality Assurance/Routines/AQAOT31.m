AQAOT31 ; GENERATED FROM 'AQAO INITIAL CLOSE' INPUT TEMPLATE(#1159), FILE 9002167;05/13/96
 D DE G BEGIN
DE S DIE="^AQAOC(",DIC=DIE,DP=9002167,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^AQAOC(DA,""))=""
 I $D(^(1)) S %Z=^(1) S %=$P(%Z,U,1) S:%]"" DE(1)=%
 I $D(^("FINAL")) S %Z=^("FINAL") S %=$P(%Z,U,1) S:%]"" DE(4)=% S %=$P(%Z,U,2) S:%]"" DE(5)=% S %=$P(%Z,U,3) S:%]"" DE(6)=% S %=$P(%Z,U,5) S:%]"" DE(2)=% S %=$P(%Z,U,7) S:%]"" DE(7)=% S %=$P(%Z,U,8) S:%]"" DE(9)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET I X'?.ANP S DDER=1 Q 
 ...
 Q
BEGIN S DNM="AQAOT31",DQ=1
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=1159,U="^"
...
7 S DW="FINAL;7",DV="*P9002169.3'O",DU="",DLB="FINAL OCC OUTCOME LEVEL",DIFLD=97
 ...
 S DU="AQAO1(3,"
 S X=$S($D(AQAOCLS(7)):AQAOCLS(7),1:"")
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z
X7 Q
8 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=8 D X8 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X8 S:$G(AQAOCLS(2))=1 Y="@2"
 Q
9 S DW="FINAL;8",DV="*P9002169.3'O",DU="",DLB="ULTIMATE PATIENT OUTCOME",DIFLD=98
 S DQ(9,2)="S Y(0)=Y S:Y]"""" Y=$P(^AQAO1(3,Y,0),U)_""   ""_$P(^(0),U,5)"
 S DU="AQAO1(3,"
 G RE
X9 S DIC("S")="I $P(^AQAO1(3,Y,0),U,3)="""",$P(^(0),U,5)'=""""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
10 S DQ=11 ;@2
11 D:$D(DG)>9 F^DIE17 G ^AQAOT311
