AQAOT39 ; GENERATED FROM 'AQAO CRITERIA FIRST' INPUT TEMPLATE(#1168), FILE 9002166.5;05/13/96
 D DE G BEGIN
DE S DIE="^AQAOCC(5,",DIC=DIE,DP=9002166.5,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^AQAOCC(5,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,5) S:%]"" DE(6)=% S %=$P(%Z,U,6) S:%]"" DE(9)=% S %=$P(%Z,U,7) S:%]"" DE(12)=% S %=$P(%Z,U,8) S:%]"" DE(15)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X5 S:$P(^AQAO1(6,AQAOCR,0),U,2)=4 Y="@4"
 Q
6 S DW="0;5",DV="RS",DU="",DLB="YES/NO/NA",DIFLD=.05
