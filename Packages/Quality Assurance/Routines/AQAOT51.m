AQAOT51 ; GENERATED FROM 'AQAO IND DISPLAY' PRINT TEMPLATE (#1316) ; 05/13/96 ; (FILE 9002168.2, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1316,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
...
A1R ;
 ...
 S X=$G(^AQAO(2,D0,0)) D N:$X>21 Q:'DN  W ?21 S Y=$P(X,U,4) W:Y]"" $S($D(DXS(25,Y)):DXS(25,Y),1:Y)
 ...
 S X=$G(^AQAO(2,D0,0)) D N:$X>59 Q:'DN  W ?59 S Y=$P(X,U,3) W:Y]"" $S($D(DXS(26,Y)):DXS(26,Y),1:Y)
 D N:$X>0 Q:'DN  W ?0 W "MED STAFF FUNCTION:"
 S X=$G(^AQAO(2,D0,1)) D N:$X>21 Q:'DN  W ?21 S Y=$P(X,U,3) W:Y]"" $S($D(DXS(27,Y)):DXS(27,Y),1:Y)
...
B1R ;
 ...
 S I(1)="""QTM""",J(1)=9002168.25 F D1=0:0 Q:$O(^AQAO(2,D0,"QTM",D1))'>0  X:$D(DSC(9002168.25)) DSC(9002168.25) S D1=$O(^(D1)) Q:D1'>0  D:$X>21 T Q:'DN  D C1
 G C1R
C1 ;
...
C1R ;
 ...
 S I(1)="""M""",J(1)=9002168.23 F D1=0:0 Q:$O(^AQAO(2,D0,"M",D1))'>0  S D1=$O(^(D1)) D:$X>29 T Q:'DN  D D1
 G D1R
D1 ;
...
D1R ;
 ...
 D N:$X>29 Q:'DN  W ?29 W "==============="
 S DIXX(1)="E1",I(0,0)=D0 S I(0,0)=$S($D(D0):D0,1:"") X DXS(7,9.2) S X="" S D0=I(0,0)
 G E1R
E1 ;
 ...
 D N:$X>62 Q:'DN  W ?62 S Y=$P(X,U,2) W:Y]"" $S($D(DXS(28,Y)):DXS(28,Y),1:Y)
...
E1R ;
 ...
 D N:$X>29 Q:'DN  W ?29 W "=========="
 S DIXX(1)="F1",I(0,0)=D0 S I(0,0)=$S($D(D0):D0,1:"") X DXS(10,9.2) S X="" S D0=I(0,0)
 G F1R
F1 ;
