AQAOT77 ; GENERATED FROM 'AQAO STATUS WITH SUM' PRINT TEMPLATE (#1294) ; 05/13/96 ; (FILE 9002168.5, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1294,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
 ...
 W ?33 S Y=$P(X,U,5) W:Y]"" $S($D(DXS(3,Y)):DXS(3,Y),1:Y)
 ...
 G A1R
A1 ;
 W ?73 X DXS(2,9.2) S DIP(101)=$S($D(^AQAO1(1,D0,0)):^(0),1:"") S X=$P(DIP(101),U,2) S D0=I(0,0) S D1=I(1,0) K DIP K:DN Y W $E(X,1,5)
 Q
A1R ;
 D T Q:'DN  D N D N:$X>24 Q:'DN  W ?24 W "** Indicator Assessment **"
 S I(1)="""IND""",J(1)=9002168.55 F D1=0:0 Q:$O(^AQAO(5,D0,"IND",D1))'>0  S D1=$O(^(D1)) D:$X>24 T Q:'DN  D B1
 G B1R
B1 ;
...
B1R ;
 ...
 D ^DIWW
 S I(1)="""SUMM""",J(1)=9002168.51 F D1=0:0 Q:$O(^AQAO(5,D0,"SUMM",D1))'>0  S D1=$O(^(D1)) D:$X>24 NX^DIWW D C1
 G C1R
C1 ;
