AQAOT84 ; GENERATED FROM 'AQAO AUTO WORKSHEET' PRINT TEMPLATE (#1314) ; 05/13/96 ; (FILE 9002167, MARGIN=80)
 G BEGIN
N W !
T W:$X ! I '$D(DIOT(2)),DN,$D(IOSL),$S('$D(DIWF):1,$P(DIWF,"B",2):$P(DIWF,"B",2),1:1)+$Y'<IOSL,$D(^UTILITY($J,1))#2,^(1)?1U1P1E.E X ^(1)
 S DISTP=DISTP+1,DILCT=DILCT+1 D:'(DISTP#100) CSTP^DIO2
 Q
DT I $G(DUZ("LANG"))>1,Y W $$OUT^DIALOGU(Y,"DD") Q
 I Y W $P("JAN^FEB^MAR^APR^MAY^JUN^JUL^AUG^SEP^OCT^NOV^DEC",U,$E(Y,4,5))_" " W:Y#100 $J(Y#100\1,2)_"," W Y\10000+1700 W:Y#1 "  "_$E(Y_0,9,10)_":"_$E(Y_"000",11,12) Q
 W Y Q
M D @DIXX
 Q
BEGIN ;
 S:'$D(DN) DN=1 S DISTP=$G(DISTP),DILCT=$G(DILCT)
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1314,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
 D N:$X>2 Q:'DN  W ?2 W "Occurrence ID: "
 S X=$G(^AQAOC(D0,0)) W ?0,$E($P(X,U,1),1,7)
 D N:$X>30 Q:'DN  W ?30 W "Occ Date: "
 S Y=$P(X,U,4) D DT
 W "   ("
 X DXS(1,9.2) S DIP(101)=$S($D(^AQAO(2,D0,0)):^(0),1:"") S X=$P($P(DIP(102),$C(59)_$P(DIP(101),U,4)_":",2),$C(59),1) S D0=I(0,0) K DIP K:DN Y W X
 W ")"
 D N:$X>0 Q:'DN  W ?0 W "Patient Chart #: "
 S APCDLOOK=DUZ(2) K DIP K:DN Y
 X DXS(2,9.3) S DIP(202)=$S($D(^AUPNPAT(D0,41,D1,0)):^(0),1:"") S X=$P(DIP(202),U,2) S D0=I(0,0) S D1=I(101,0) K DIP K:DN Y W X
 W ?0 K APCDLOOK K DIP K:DN Y
 D N:$X>28 Q:'DN  W ?28 W "Visit Date: "
 S X=$G(^AQAOC(D0,0)) S Y=$P(X,U,3) S Y=$S(Y="":Y,$D(^AUPNVSIT(Y,0))#2:$P(^(0),U,1),1:Y) D DT
 W "  ("
 X DXS(3,9.2) S DIP(101)=$S($D(^AUPNVSIT(D0,0)):^(0),1:"") S X=$P($P(DIP(102),$C(59)_$P(DIP(101),U,7)_":",2),$C(59),1) S D0=I(0,0) K DIP K:DN Y W X
 W ")  "
 D N:$X>8 Q:'DN  W ?8 W "Service: "
 S X=$G(^AQAOC(D0,0)) S Y=$P(X,U,7) S Y=$S(Y="":Y,$D(^DIC(49,Y,0))#2:$P(^(0),U,1),1:Y) W $E(Y,1,30)
 D N:$X>4 Q:'DN  W ?4 W "Ward/Clinic: "
 S Y=$P(X,U,6) S Y=$S(Y="":Y,$D(^SC(Y,0))#2:$P(^(0),U,1),1:Y) W $E(Y,1,30)
 D T Q:'DN  D N D N:$X>6 Q:'DN  W ?6 W "Indicator: "
 S Y=$P(X,U,8) S Y=$S(Y="":Y,$D(^AQAO(2,Y,0))#2:$P(^(0),U,1),1:Y) W $E(Y,1,7)
 W "   "
 X DXS(4,9.2) S X=$P(DIP(101),U,2) S D0=I(0,0) K DIP K:DN Y W X
 D N:$X>4 Q:'DN  W ?4 W "Review Type: "
 X DXS(5,9.2) S X=$S('$D(^AQAO(3,+$P(DIP(101),U,1),0)):"",1:$P(^(0),U,1)) S D0=I(0,0) K DIP K:DN Y W X
 D N:$X>54 Q:'DN  W ?54 W "Case Status: "
 S X=$G(^AQAOC(D0,1)) S Y=$P(X,U,1) W:Y]"" $S($D(DXS(11,Y)):DXS(11,Y),1:Y)
 D N:$X>0 Q:'DN  W ?0 X DXS(6,9) K DIP K:DN Y W X
 X DXS(7,9.2) S X=X="DELETED",DIP(3)=X S X=$P(DIP(1),U,12),DIP(4)=X S X=1,DIP(5)=X S X="",X=$S(DIP(3):DIP(4),DIP(5):X) K DIP K:DN Y W X
 D T Q:'DN  D N D N:$X>0 Q:'DN  W ?0 W "Case Summary: "
 S I(1)="""CASE""",J(1)=9002167.02 F D1=0:0 Q:$O(^AQAOC(D0,"CASE",D1))'>0  S D1=$O(^(D1)) D:$X>16 T Q:'DN  D A1
 G A1R
A1 ;
 S X=$G(^AQAOC(D0,"CASE",D1,0)) S DIWL=15,DIWR=78 D ^DIWP
 Q
A1R ;
 D A^DIWW
 D T Q:'DN  D N D N D N:$X>0 Q:'DN  W ?0 W " "
 S I(100)="^AQAO(2,",J(100)=9002168.2 S I(0,0)=D0 S DIP(1)=$S($D(^AQAOC(D0,0)):^(0),1:"") S X=$P(DIP(1),U,8),X=X S D(0)=+X S D0=D(0) I D0>0 D B1
 G B1R
B1 ;
 S I(200)="^AQAO(3,",J(200)=9002168.3 S I(100,0)=D0 S DIP(101)=$S($D(^AQAO(2,D0,1)):^(1),1:"") S X=$P(DIP(101),U,1),X=X S D(0)=+X S D0=D(0) I D0>0 D A2
 G A2R
A2 ;
 S I(300)="^AQAQX(",J(300)=9002166.1 S I(200,0)=D0 S DIP(201)=$S($D(^AQAO(3,D0,0)):^(0),1:"") S X=$P(DIP(201),U,3),X=X S D(0)=+X S D0=D(0) I D0>0 D A3
 G A3R
A3 ;
 S I(301)="""PG""",J(301)=9002166.11 F D1=0:0 Q:$O(^AQAQX(D0,"PG",D1))'>0  X:$D(DSC(9002166.11)) DSC(9002166.11) S D1=$O(^(D1)) Q:D1'>0  D:$X>3 T Q:'DN  D A4
 G A4R
A4 ;
 D T Q:'DN  D N D N:$X>0 Q:'DN  W ?0 X DXS(8,9) K DIP K:DN Y W X
 Q
A4R ;
 Q
A3R ;
 K J(300),I(300) S:$D(I(200,0)) D0=I(200,0)
 Q
A2R ;
 K J(200),I(200) S:$D(I(100,0)) D0=I(100,0)
 D T Q:'DN  D N D N:$X>29 Q:'DN  W ?29 W "REVIEW CRITERIA"
 D N:$X>29 Q:'DN  W ?29 W "==============="
 S DIXX(2)="B2",I(100,0)=D0 S I(100,0)=$S($D(D0):D0,1:"") X DXS(9,9.2) S X="" S D0=I(100,0)
 G B2R
B2 ;
 I $D(DSC(9002169.6)) X DSC(9002169.6) E  Q
 W:$X>46 ! S I(200)="^AQAO1(6,",J(200)=9002169.6
 D T Q:'DN  D N D N:$X>0 Q:'DN  W ?0 S DIP(201)=$S($D(^AQAO1(6,D0,0)):^(0),1:"") S X=$P(DIP(201),U,1)_":  " K DIP K:DN Y W X
 D N:$X>59 Q:'DN  W ?59 X DXS(10,9) K DIP K:DN Y W X
 D N:$X>78 Q:'DN  W ?78 W " "
 S I(201)="""CD""",J(201)=9002169.61 F D1=0:0 Q:$O(^AQAO1(6,D0,"CD",D1))'>0  X:$D(DSC(9002169.61)) DSC(9002169.61) S D1=$O(^(D1)) Q:D1'>0  D:$X>81 T Q:'DN  D B3
 G B3R
B3 ;
 W "  "
 S X=$G(^AQAO1(6,D0,"CD",D1,0)) S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^AQAO1(4,Y,0))#2:$P(^(0),U,1),1:Y) W $E(Y,1,6)
 W "  "
 Q
B3R ;
 Q
B2R ;
 K J(200),I(200) S:$D(I(100,0)) D0=I(100,0)
 Q
B1R ;
 K J(100),I(100) S:$D(I(0,0)) D0=I(0,0)
 D T Q:'DN  D N D N D N:$X>0 Q:'DN  W ?0 W "COMPLETED ON ____________________"
 D N:$X>34 Q:'DN  W ?34 W "BY _______________________________"
 K Y K DIWF
