AQAOT94 ; GENERATED FROM 'AQAO LONG DISPLAY-E1' PRINT TEMPLATE (#1273) ; 05/13/96 ; (FILE 9002167, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1273,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
...
A1R ;
 ...
 S X=$G(^AQAOC(D0,1)) S Y=$P(X,U,1) W:Y]"" $S($D(DXS(28,Y)):DXS(28,Y),1:Y)
...
F1 ;
 ...
 D N:$X>19 Q:'DN  W ?19 X DXS(12,9.2) S X=$S(DIP(102):DIP(103),DIP(104):X) K DIP K:DN Y W X
 S DIP(101)=$S($D(^AQAOCC(7,D0,0)):^(0),1:"") S X=$P(DIP(101),U,1),X=X S X=X\1 K DIP K:DN Y W X
 X DXS(13,9.3) S X=X_"]",DIP(105)=X S X=1,DIP(106)=X S X="",X=$S(DIP(103):DIP(105),DIP(106):X) K DIP K:DN Y W X
 Q
F1R ;
 K J(100),I(100) S:$D(I(0,0)) D0=I(0,0)
 S DIXX(1)="G1",I(0,0)=D0 S I(0,0)=$S($D(D0):D0,1:"") X DXS(14,9.2) S X="" S D0=I(0,0)
 G G1R
G1 ;
 ...
 D N:$X>64 Q:'DN  W ?64 X DXS(15,9) K DIP K:DN Y W X
 Q
G1R ;
 K J(100),I(100) S:$D(I(0,0)) D0=I(0,0)
