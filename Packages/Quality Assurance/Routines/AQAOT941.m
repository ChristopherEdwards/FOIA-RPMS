AQAOT941 ; GENERATED FROM 'AQAO LONG DISPLAY-E1' PRINT TEMPLATE (#1273) ; 05/13/96 ; (continued)
 G BEGIN
N W !
...
BEGIN ;
 ...
 X DXS(16,9.2) S X=$P(DIP(101),U,2) S D0=I(0,0) K DIP K:DN Y W X
 D N:$X>41 Q:'DN  W ?41 X DXS(17,9.2) S DIP(3)=X S X=1,DIP(4)=X S X="",X=$S(DIP(2):DIP(3),DIP(4):X) K DIP K:DN Y W X
 S X=$G(^AQAOC(D0,1)) S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^AQAO1(2,Y,0))#2:$P(^(0),U,1),1:Y) W $E(Y,1,50)
 D N:$X>52 Q:'DN  W ?52 W "Action: "
 X DXS(18,9.2) S X=$P(DIP(101),U,2) S D0=I(0,0) K DIP K:DN Y W X
 X DXS(19,9.5) S DIP(6)=X S X=",",X=$P(DIP(6),X) S Y=X,X=DIP(5),X=X_Y,X=$S(DIP(2):DIP(3),DIP(4):X) K DIP K:DN Y W $E(X,1,10)
 ...
 G H1R
H1 ;
 D N:$X>66 Q:'DN  W ?66 X DXS(20,9.3) S DIP(2)=X S X=",",X=$P(DIP(2),X) K DIP K:DN Y W $E(X,1,10)
 Q
H1R ;
...
J1 ;
 ...
 D N:$X>9 Q:'DN  W ?9 W "Finding: "
 X DXS(21,9.2) S DIP(101)=$S($D(^AQAO(8,D0,0)):^(0),1:"") S X=$P(DIP(101),U,2) S D0=I(0,0) S D1=I(1,0) K DIP K:DN Y W X
 D N:$X>52 Q:'DN  W ?52 W "Action: "
 X DXS(22,9.2) S DIP(101)=$S($D(^AQAO(6,D0,0)):^(0),1:"") S X=$P(DIP(101),U,2) S D0=I(0,0) S D1=I(1,0) K DIP K:DN Y W X
 X DXS(23,9.5) S DIP(6)=X S X=",",X=$P(DIP(6),X) S Y=X,X=DIP(5),X=X_Y,X=$S(DIP(2):DIP(3),DIP(4):X) K DIP K:DN Y W $E(X,1,10)
 ...
 G A2R
A2 ;
 D N:$X>66 Q:'DN  W ?66 X DXS(24,9.3) S DIP(2)=X S X=",",X=$P(DIP(2),X) K DIP K:DN Y W $E(X,1,10)
 Q
A2R ;
 D N:$X>1 Q:'DN  W ?1 W "Review Comments: "
