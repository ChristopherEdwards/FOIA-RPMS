BMC4P6 ;IHS/OIT/FCJ - BMC 4.0 PATCH 5
 ;;4.0;REFERRED CARE INFO SYSTEM;**6**;JAN 09, 2006
 ;ORIGINAL ROUTINE FR BMC4P5
 ;
 I '$G(IOM) D HOME^%ZIS
 ;
 ...
 D HELP^XBHELP("INTROE","BMC4P6")
 I '$$DIR^XBDIR("E","","","","","",1) D SORRY(2) Q
 I $G(XPDENV)=1 S (XPDDIQ("XPZ1"),XPDDIQ("XPZ2"))=0 D HELP^XBHELP("INTROI","BMC4P6") I '$$DIR^XBDIR("E","","","","","",1) D SORRY(2)
 Q
 ;
SORRY(X) ;
 KILL DIFQ
...
INSTALLD(BMC) ; Determine if patch BMC was installed, where BMC is
 ...
 ; -----------------------------------------------------
PRE ;EP - From KIDS.
 I $$NEWCP^XPDUTL("PRE1","AUDS^BMC4P6")
...
MAIL ;
 ...
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""BMC4P6"",$J,",XMY(1)="",XMY(DUZ)=""
...
INTROE ; Intro text during KIDS Environment check.
 ...
 ;;(1) General Retrieval Reports-changed name from CHS Amt Auth to
 ;;    Date to CHS Paid-Auth to Date to clarify what is being printed
 ;;(2) Patch number is now displayed in the Menu Heading
 ;;(3) Fix the FY 10 year display for CHS PO's
 ;;(4) Changed the Edit secondary referral option to allow editing
 ;;    closed secondary referrals
 ;;(5) Added a separate menu option for adding Medical Hx Comments
 ;;    located under Edit a referral option
 ;;
 ;;###
 ;
...
AUDS ;EP - From KIDS.
 ...
 S ^XTMP("BMC4P6",0)=$$FMADD^XLFDT(DT,10)_"^"_DT_"^"_$P($P($T(+1),";",2)," ",3,99)
 NEW BMC
 S BMC=0
 F  S BMC=$O(^XTMP("XPDI",XPDA,"FIA",BMC)) Q:'BMC  D
 . I '$D(^XTMP("BMC4P6",BMC,"DDA")) S ^XTMP("BMC4P6",BMC,"DDA")=$G(^DD(BMC,0,"DDA"))
 . D MES^XPDUTL(" File "_$$RJ^XLFSTR(BMC,12)_" - "_$$LJ^XLFSTR(^XTMP("XPDI",XPDA,"FIA",BMC),30)_"- DD audit was '"_$G(^XTMP("BMC4P6",BMC,"DDA"))_"'"),MES^XPDUTL($$RJ^XLFSTR("Set to 'Y'",69))
 . S ^DD(BMC,0,"DDA")="Y"
 .Q
...
AUDR ; Restore the file data audit values to their original values.
 ...
 NEW BMC
 S BMC=0
 F  S BMC=$O(^XTMP("BMC4P6",BMC)) Q:'BMC  D
 . S ^DD(BMC,0,"DDA")=^XTMP("BMC4P6",BMC,"DDA")
