BMC4P9 ;IHS/OIT/FCJ - BMC 4.0 PATCH 9 ; 16 Feb 2011  2:54 PM
 ;;4.0;REFERRED CARE INFO SYSTEM;**9**;JAN 09, 2006;Build 45
 ;ORIGINAL ROUTINE FR BMC4P8
 ;
 I '$G(IOM) D HOME^%ZIS
 ;
 ...
 D HELP^XBHELP("INTROE","BMC4P9")
 I '$$DIR^XBDIR("E","","","","","",1) D SORRY(2) Q
 I $G(XPDENV)=1 S (XPDDIQ("XPZ1"),XPDDIQ("XPZ2"))=0 D HELP^XBHELP("INTROI","BMC4P9") I '$$DIR^XBDIR("E","","","","","",1) D SORRY(2)
 Q
 ;
SORRY(X) ;
 KILL DIFQ
...
INSTALLD(BMC) ; Determine if patch BMC was installed, where BMC is
 ...
 ; -----------------------------------------------------
PRE ;EP - From KIDS.
 I $$NEWCP^XPDUTL("PRE1","AUDS^BMC4P9")
...
MAIL ;
 ...
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""BMC4P9"",$J,",XMY(1)="",XMY(DUZ)=""
...
INTROE ; Intro text during KIDS Environment check.
 ...
 ;;
 ;;    Multiple changes for ICD-10, see the
 ;;    requirement's documentation for additional information
 ;;
 ;;
 ;;###
 ;
...
GREET ;;To add to mail message.
 ...
 ;;Please refer to patch "bmc*4.0*9".
 ;;  
...
AUDS ;EP - From KIDS.
 ...
 S ^XTMP("BMC4P9",0)=$$FMADD^XLFDT(DT,10)_"^"_DT_"^"_$P($P($T(+1),";",2)," ",3,99)
 NEW BMC
 S BMC=0
 F  S BMC=$O(^XTMP("XPDI",XPDA,"FIA",BMC)) Q:'BMC  D
 . I '$D(^XTMP("BMC4P9",BMC,"DDA")) S ^XTMP("BMC4P9",BMC,"DDA")=$G(^DD(BMC,0,"DDA"))
 . D MES^XPDUTL(" File "_$$RJ^XLFSTR(BMC,12)_" - "_$$LJ^XLFSTR(^XTMP("XPDI",XPDA,"FIA",BMC),30)_"- DD audit was '"_$G(^XTMP("BMC4P9",BMC,"DDA"))_"'"),MES^XPDUTL($$RJ^XLFSTR("Set to 'Y'",69))
 . S ^DD(BMC,0,"DDA")="Y"
 .Q
...
AUDR ; Restore the file data audit values to their original values.
 ...
 NEW BMC
 S BMC=0
 F  S BMC=$O(^XTMP("BMC4P9",BMC)) Q:'BMC  D
 . S ^DD(BMC,0,"DDA")=^XTMP("BMC4P9",BMC,"DDA")
