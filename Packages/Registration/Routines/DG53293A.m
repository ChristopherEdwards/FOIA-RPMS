DG53293A ;AR/BP - ALS EXTRACT; ; 7/26/00 1:15pm
...
EN ; -- Entry point for manually running extract.
 ...
 ;
 ;
START ; -- Called from POSTINST^DG53293C and EN^DG53293D
 I $D(DUZ)'=11 D  Q
 ...
 . S DGARY(3)=("     To restart the ALS Extract type D EN^DG53293A.")
 . D MES^XPDUTL(.DGARY)
 ;
 D BMES^XPDUTL()
...
GETDFN(DGTATION) ;
 ...
 S DGN=0
 F  S DGN=$O(^XTMP("DGALS","S",DGTATION,DGN)) Q:'DGN  DO
 .;;piece out ssn
 .F DGP=1:1 S DGSSN=$P(^XTMP("DGALS","S",DGTATION,DGN),"^",DGP) Q:'DGSSN  DO
 . . S DGDFN=$$DFN(DGSSN)
...
SENDATA(DGTATION) ;
 ...
 . D SETMAIL^DG53293B(DGTATION,DGDFN)
 .;
 . I (($P($H,",",2))#10) Q
 . I '$D(ZTQUEUED) W " ."
 .;
