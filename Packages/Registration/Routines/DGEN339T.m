...
EN ; Main entry point to populate the IVMB HEC VSE NOTIFICATION mail group
 ...
 ;
 N DIR,DIRUT,DUOUT,MGIEN,DIC
 ;
 S DIC=3.8
 S DIC(0)="MZ"
 S X="IVMB HEC VSE NOTIFICATION"
 D ^DIC K DIC
 I '(+Y>0) D  Q
 . W !!?5,*7,"Apparently the IVMB HEC VSE NOTIFICATION mail group was not"
 . W !?5,"created or set up correctly by the DG*5.3*339 patch installation."
...
LOCAL(MGIEN) ; Add local members to mail group
 ...
 . S DIR("?",1)="Select a member to add to the IVMB HEC VSE NOTIFICATION mail group"
...
REMOTE ; Add remote members to mail group
 ...
 . Q:$D(DIRUT)!($D(DUOUT))
 . S:'Y ABORT=1
 S ZFDA(3.8,"?1,",.01)="IVMB HEC VSE NOTIFICATION"
