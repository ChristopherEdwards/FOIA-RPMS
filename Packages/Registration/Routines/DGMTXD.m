DGMTXD ; GENERATED FROM 'DGMT ENTER/EDIT DEPENDENTS' INPUT TEMPLATE(#2442), FILE 408.22;10/29/04
 D DE G BEGIN
DE S DIE="^DGMT(408.22,",DIC=DIE,DP=408.22,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGMT(408.22,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,6) S:%]"" DE(3)=% S %=$P(%Z,U,9) S:%]"" DE(2)=% S %=$P(%Z,U,10) S:%]"" DE(5)=% S %=$P(%Z,U,11) S:%]"" DE(7)=% S %=$P(%Z,U,12) S:%]"" DE(9)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="DGMTXD",DQ=1
 ...
 M DIEZAR=^DIE(2442,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2442,U="^"
1 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=1 D X1 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X1 K DGFIN
 Q
2 S DW="0;9",DV="S",DU="",DLB="PERMANENTLY INCAPABLE OF SELF-SUPPORT",DIFLD=.09
 S DU="1:YES;0:NO;"
 G RE
X2 Q
3 S DW="0;6",DV="SX",DU="",DLB="DID THE CHILD LIVE WITH YOU",DIFLD=.06
 S DE(DW)="C3^DGMTXD"
 S DU="1:YES;0:NO;"
 G RE
...
X4 S:X Y="@10"
 Q
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW="0;10",DV="SX",DU="",DLB="DID YOU CONTRIBUTE TO THE CHILD'S SUPPORT",DIFLD=.1
 S DU="1:YES;0:NO;"
 G RE
X5 D CON^DGMTDD2
 Q
 ;
6 S DQ=7 ;@10
7 S DW="0;11",DV="SX",DU="",DLB="DID CHILD HAVE ANY INCOME",DIFLD=.11
 S DE(DW)="C7^DGMTXD"
 S DU="1:YES;0:NO;"
 G RE
...
X8 S:'X Y="@998"
 Q
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="0;12",DV="SX",DU="",DLB="WAS THE CHILD'S INCOME AVAILABLE TO YOU",DIFLD=.12
 S DE(DW)="C9^DGMTXD"
 S DU="1:YES;0:NO;"
 G RE
