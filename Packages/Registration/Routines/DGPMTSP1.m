DGPMTSP1 ; IHS/ADC/PDW/ENM - BSR PRINT, CONT. ;  [ 03/25/1999  11:48 AM ]
...
A D DATE
 S (AT,PL,W)=0
 ;  GL (Ward) Order
 ;IHS  change files to loop--42ward to 40.8med ctr div (t.s.)
 ;F W1=0:0 S W=$O(^DIC(42,W)) Q:W'>0  I $D(^DIC(42,W,0)) S BD("W")=^(0),ORDER=$S($D(^DIC(42,W,"ORDER")):+^("ORDER"),1:0) D:+ORDER WARD,TOTAL,^DGPMBSP2
 F DV1=0:0 S DV=$O(^DIC(40.8,DV)) Q:DV'>0  D  ;IHS
 . F TS1=0:0 S TS=$O(^DIC(40.8,DV,"TS",TS)) Q:TS'>0  D
 .. I $D(^DIC(40.8,DV,"TS",TS,0)) S BD("W")=^(0),ORDER=$S($D(^DIC(42,W,"ORDER")):+^("ORDER"),1:0) D:+ORDER WARD,TOTAL,^DGPMBSP2
 K TC,TL,TB,C,I,W1,W2,OOS,T,T1,W,X1,X2,FY("B"),FY("L"),ORDER,BD,X,FY("M")
Q Q
 ;
DATE K ^UTILITY("DGWOR",$J),^UTILITY("DGWON",$J),^UTILITY("DGWOS",$J),^UTILITY("DGWTOR",$J),^UTILITY("DGWNN",$J)
 ...
 S X=+$E(RD,2,3),X=$S(X&(X#4):365,1:366) ;  Number of days in (report date) year
 ...
 I X2=28,X1'="00",'(X1#4) S X2=29 ;  if leap year
...
WARD S BD("CB")=$S($D(^DG(41.9,+W,"C",RD,0)):+$P(^(0),"^",4),1:0) ;  Cum Beds
 ...
 F %=FY("B"):0 S %=$O(^DG(41.9,+W,"C",%)) Q:'%!(%>RD)  I $D(^DG(41.9,+W,"C",%,1)) S X=^(1) I $P(X,"^",9) S BD("OSD")=BD("OSD")+($P(X,"^",9)=$P(X,"^",10)),BD("OS")=BD("OS")+$P(X,"^",9)
