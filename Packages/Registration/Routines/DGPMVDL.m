DGPMVDL ;ALB/MIR - DELETE PATIENT MOVEMENTS ; [ 09/13/2001  3:59 PM ]
...
D1 S DGPMER=0 F I=0:0 S I=$O(^DGPM("APMV",DFN,DGPMCA,I)) Q:I'>0  S J=$O(^(I,0)) I $D(^DGPM(J,0)),($P(^(0),"^",15)]"") S DGPMER=1 Q
 ...
 Q
1 S DGMSG="Patient admission has been deleted for admit date: "_$$FMTE^XLFDT(+DGPMAN,"5DZ"),DGMSG1="Deleted Admission" D MSG^DGPTMSG1
 S DIK="^DGPT(",DA=$P(DGPMAN,"^",16) D ^DIK:DA>0 ; delete PTF record
 S DA=$O(^DGS(41.1,"AMVT",DGPMDA,0)) I DA S DIE="^DGS(41.1,",DR="17///@" D ^DIE ;remove scheduled admission reference in 41.1
 ;
 ;IHS/ANMC/LJF 3/09/2001 added IHS nodes to ^utility
 ...
 F DGI=DGPMDA:0 S DGI=$O(^DGPM("CA",DGPMDA,DGI)) Q:'DGI  I $D(^DGPM(DGI,0)) D
 . S DGPMTYP=$P(^(0),"^",2),DA=DGI,DIK="^DGPM(",^UTILITY("DGPM",$J,DGPMTYP,DA,"P")=^(0),^("A")=""
 . S ^UTILITY("DGPM",$J,DGPMTYP,DA,"IHSP")=$G(^DGPM(DGI,"IHS")),^UTILITY("DGPM",$J,DGPMTYP,DA,"IHSA")=""
 . D ^DIK
