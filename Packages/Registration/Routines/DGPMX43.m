...
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,5) S:%]"" DE(1)=% S %=$P(%Z,U,6) S:%]"" DE(3)=% S %=$P(%Z,U,7) S:%]"" DE(4)=%
 I $D(^("LD")) S %Z=^("LD") S %=$P(%Z,U,1) S:%]"" DE(6)=% S %=$P(%Z,U,2) S:%]"" DE(7)=%
 I $D(^("USR")) S %Z=^("USR") S %=$P(%Z,U,3) S:%]"" DE(9)=% S %=$P(%Z,U,4) S:%]"" DE(11)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="0;5",DV="R*P9999999.91'",DU="",DLB="TRANSFER FACILITY",DIFLD=.05
 S DE(DW)="C1^DGPMX43"
 S DU="AUTTTFAC("
 G RE
...
C1S S X="" G:DG(DQ)=X C1F1 K DB
 ...
 I $P(^DGPM(DA,0),"^",2)=3 S A1B2TAG="ADM" D ^A1B2XFR
C1F1 Q
X1 S DIC("S")="I $P($G(^AUTTTFAC(+Y,0)),U,2)=""""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
C3F1 Q
X3 S DIC("S")="I $P($G(^BDGWD(+Y,0)),U,3)'=""I""" D W^DGPMVDD I $D(X) D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X I $D(X) D WARD^DGPMVDD K:$D(DGOOS) X K DGOOS
 Q
 ;
...
C4F1 Q
X4 K:'$D(DGPMT) X I $D(X) S DIC("S")="I $D(^DG(405.4,""W"",+$P(^DGPM(DA,0),""^"",6),+Y)) D OCC^DGPMRB I 'DGPMOC" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X I $D(X) D ROOM^DGPMVDD K:$D(DGOOS) X K DGOOS
 Q
 ;
5 S DQ=6 ;@42
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="LD;1",DV="RP406.41'",DU="",DLB="REASON FOR LODGING",DIFLD=30.01
 S DU="DG(406.41,"
 G RE
X6 Q
7 S DW="LD;2",DV="F",DU="",DLB="LODGING COMMENTS",DIFLD=30.02
 G RE
X7 K:$L(X)>30!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
...
X8 I DGPMP=^DGPM(DA,0) S Y=""
 Q
9 S DW="USR;3",DV="RP200'a",DU="",DLB="LAST EDITED BY",DIFLD=102
 S DE(DW)="C9^DGPMX43"
 S DU="VA(200,"
 S X=DUZ
 S Y=X
...
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW="USR;4",DV="RD",DU="",DLB="LAST EDITED ON",DIFLD=103
 S %=$P($H,",",2),X=DT_(%\60#60/100+(%\3600)+(%#60/10000)/100)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X11 S %DT="STX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
12 G 0^DIE17
