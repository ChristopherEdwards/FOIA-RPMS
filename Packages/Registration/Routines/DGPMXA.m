DGPMXA ; GENERATED FROM 'DGPM ASIH ADMIT' INPUT TEMPLATE(#1838), FILE 405;10/29/04
 D DE G BEGIN
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(2)=% S %=$P(%Z,U,6) S:%]"" DE(4)=% S %=$P(%Z,U,7) S:%]"" DE(5)=% S %=$P(%Z,U,11) S:%]"" DE(8)=% S %=$P(%Z,U,12) S:%]"" DE(6)=%
...
BEGIN S DNM="DGPMXA",DQ=1
 ...
 M DIEZAR=^DIE(1838,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=1838,U="^"
