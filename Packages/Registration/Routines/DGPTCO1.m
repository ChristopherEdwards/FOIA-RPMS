DGPTCO1 ;ALB/MJK - Census Status Report ; 5/30/01 11:26am
 ;;5.3;Registration;**136,383,432**;Aug 13, 1993
 ;
...
CHKCUR ; -- checks if new PTF Census Date record is needed
 ...
 S DGIEN=$O(^DG(45.86,"B",+$G(DGIEN),0)) Q:'DGIEN
 ; look at last census closeout date
 S DGCLOSE=$P($G(^DG(45.86,DGIEN,0)),U,2) Q:'DGCLOSE
 I DT>DGCLOSE D ADDREC
 Q
ADDREC ; -- add new record
 N DA,DIE,DR,DGYR,DGMONTH,DGSTRT,DGENDT,DO,DIC,ERR,X,Y
 ; first inactivate last record
 S DA=DGIEN,DIE="^DG(45.86,",DR=".04////0" D ^DIE K DIE,DR,DA
 ...
 Q
ERR ;
 D BMES^XPDUTL("Problem with PTF CENSUS DATE File (#45.86) Update.")
 D BMES^XPDUTL("Please contact your ADP Coordinator.")
 Q
 ;
