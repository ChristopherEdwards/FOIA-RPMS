DGPTFM0 ;ALB/MAC - ROUTINE TO DISPLAY PROCEDURE CODES ON THE MAS SCREEN IN PTF LOAD/EDIT ; AUG 1 1989@1200
...
PD F J1=1:1:5 S L=$P(P(J),U,J1+4),L1=0,L3=1 D:+L PD1 S L1=1,L=$S($D(P(J+1)):$P(P(J+1),U,J1+4),1:"") D:+L PD1
 Q
PD1 S L2=$S($D(^ICD0(+L,0)):^(0),1:""),P2=P2+1,L4=$P(L2,"^",1),L4=L4_$E("   ",1,3-$L($P(L4,".",2))) W:L3 ! S:L3 L3=0 W ?L1*40,$J(P2,3)," ",$J(L4,7)," ",$E($P(^(0),U,4),1,25) K P2(P2) S P2(P2)=J+L1_U_J1 Q
...
C ; -- help for surgery
 ...
 F L=1:1:S2 Q:'$D(S2(L))  I $D(S(+S2(L),1)),$D(^DGPT(PTF,"S",+S(+S2(L),1),0)),$D(^ICD0(+$P(^(0),"^",7+$P(S2(L),"^",2)),0)) W !?5,$J(L,2),": ",$J($P(^(0),"^"),7)," - ",$E($P(^(0),"^",4),1,40)
 Q
 ;
DX ; -- help for dx's
 W !!,"Enter the item #'s of the diagnoses, 1-",M2,", that you wish to delete:"
 S UTL="^UTILITY($J,""M2"")"
 F L=1:1:M2 Q:'$D(@UTL@(L))  I $D(^DGPT(PTF,"M",+@UTL@(L),0)),$D(^ICD9(+$P(^(0),"^",4+$P(@UTL@(L),"^",2)),0)) W !?5,$J(L,2),": ",$J($P(^(0),"^"),7)," - ",$E($P(^(0),"^",3),1,40)
 K UTL,L Q
 ;
