DGPTFMO ;ALB/JDS - DGPTF PRINT TEMPLATE ; 8/23/01 3:51pm
...
WRITE D HEAD:$Y>(IOSL-12) G Q:'DN S Y=$P(DGM,U,10),DGL=+$P(DGM,U,2),DGL=$S($D(^DIC(42.4,DGL,0)):^(0),1:""),DGL=$P(DGL,U,1) D D^DGPTUTL
 ...
 W:T(I)=1 !,"Discharge "
 S DGF="" F J=5:1:15 I J#10 S DGICD=$S($D(^ICD9(+$P(DGM,U,J),0)):^(0),1:"") I DGICD]"" W:DGF="" !!?13,"DX: " W $P(DGICD,U,3)_" ("_$P(DGICD,U)_")",!?17 S DGF=1
...
SUR D HEAD:$Y>(IOSL-7) G Q:'DN S S1=^DGPT(D0,"S",I,0),Y=+S1 D D^DGPTUTL W !!,"   Date of Surg: ",Y,?45,"Chief Surg: " S L=";"_$P(^DD(45.01,4,0),U,3),L1=";"_$P(S1,U,4)_":" W $P($P(L,L1,2),";",1)
 ...
 W !!,?7,"Surg/pro: " F K=1:1:5 S L=$P(S1,U,K+7) I L'="" W $S($D(^ICD0(+L,0)):$P(^(0),U,4)_" ("_$P(^(0),U,1)_")",1:"**********-"_L),!?17
 ;-- display expanded codes
 S DG300=$S($D(^DGPT(D0,"S",I,300)):^(300),1:"") I DG300]"" D PRN3^DGPTFM8
 K DG300
 Q
PROC S DGF="" F I=1:1:5 S DGOP=$S($D(^ICD0(+$P(DGOP1,U,I),0)):^(0),1:"") I DGOP W:'DGF !!?6,"Procedure: " W $P(DGOP,U,4)_" ("_$P(DGOP,U)_")",!?17 S DGF=1
 Q
601 ;print the procedures/dates from the 601 procedure multiple (eff. 10/1/87)
 F J=5:1:9 S DGPROC=$S($D(^ICD0(+$P(DG601,U,J),0)):^(0),1:"") I DGPROC W !?17,$P(DGPROC,U,4)_" ("_$P(DGPROC,U)_")"
 Q
DXLS S DXLS=$S($D(^ICD9(+$P(DGPT,U,10),0)):^(0),1:"") I DXLS]"" W !!?11,"DXLS: ",$P(DXLS,U,3)_" ("_$P(DXLS,U)_")"
 I 'DXLS S DGP=$S($D(^ICD9(+$P(DGPT,U,11),0)):^(0),1:"") I DGP]"" W !!," Principal Diag: ",$P(DGP,U,3)_" ("_$P(DGP,U)_")"
 F %=16:1:24 S DGICD=$S($D(^ICD9(+$P(DGPT,U,%),0)):^(0),1:"") I DGICD]"" W !?17,$P(DGICD,U,3)_" ("_$P(DGICD,U)_")"
