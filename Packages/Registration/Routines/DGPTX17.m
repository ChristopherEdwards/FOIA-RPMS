...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=2,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.11)) S %Z=^(.11) S %=$P(%Z,U,5) S:%]"" DE(14)=% S %=$P(%Z,U,12) S:%]"" DE(15)=%
 I $D(^(.321)) S %Z=^(.321) S %=$P(%Z,U,2) S:%]"" DE(1)=% S %=$P(%Z,U,3) S:%]"" DE(4)=% S %=$P(%Z,U,12) S:%]"" DE(6)=% S %=$P(%Z,U,13) S:%]"" DE(3)=%
 I $D(^(.52)) S %Z=^(.52) S %=$P(%Z,U,5) S:%]"" DE(7)=% S %=$P(%Z,U,6) S:%]"" DE(9)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X2 S:X'="Y" Y=.32103
 Q
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".321;13",DV="SX",DU="",DLB="AGENT ORANGE EXPOSURE LOCATION",DIFLD=.3213
 S DU="K:KOREAN DMZ;V:VIETNAM;"
 G RE
X3 S DFN=DA D AO^DGLOCK
 Q
 ;
4 S DW=".321;3",DV="RSX",DU="",DLB="RADIATION EXPOSURE INDICATED?",DIFLD=.32103
 S DE(DW)="C4^DGPTX17"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
X5 S:X'="Y" Y=.525
 Q
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW=".321;12",DV="SX",DU="",DLB="RADIATION EXPOSURE METHOD",DIFLD=.3212
 S DU="N:NAGASAKI/HIROSHIMA;T:NUCLEAR TESTING;B:BOTH;"
 G RE
X6 S DFN=DA D IR^DGLOCK Q
 Q
 ;
7 S DW=".52;5",DV="RSX",DU="",DLB="POW STATUS INDICATED?",DIFLD=.525
 S DE(DW)="C7^DGPTX17"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
X8 S:X'="Y" Y="@42"
 Q
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW=".52;6",DV="P22'X",DU="",DLB="POW CONFINEMENT LOCATION",DIFLD=.526
 S DU="DIC(22,"
 G RE
X9 S DFN=DA D POW^DGLOCK
 Q
 ;
10 S DQ=11 ;@42
11 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=11 D X11 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X11 S:+DGJUMP'=4 Y="@991"
 Q
12 S DQ=13 ;@41
13 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=13 D X13 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X13 S DGJUMP=$P(DGJUMP,"4,",2)
 Q
14 S DW=".11;5",DV="P5'a",DU="",DLB="STATE",DIFLD=.115
 S DE(DW)="C14^DGPTX17",DE(DW,"INDEX")=1
 S DU="DIC(5,"
 G RE
...
C14S S X="" G:DG(DQ)=X C14F1 K DB
 D ^DGPTX19
C14F1 N X,X1,X2 S DIXR=26 D C14X1(U) K X2 M X2=X D C14X1("O") K X1 M X1=X
 D
...
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW=".11;12",DV="FXOa",DU="",DLB="ZIP+4",DIFLD=.1112
 ...
 S DE(DW)="C15^DGPTX17",DE(DW,"INDEX")=1
 G RE
C15 G C15S:$D(DE(15))[0 K DB
 D ^DGPTX110
C15S S X="" G:DG(DQ)=X C15F1 K DB
 D ^DGPTX111
C15F1 N X,X1,X2 S DIXR=22 D C15X1(U) K X2 M X2=X D C15X1("O") K X1 M X1=X
 D
...
C15X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C15F2 S DIXR=50 D C15X2(U) K X2 M X2=X D C15X2("O") K X1 M X1=X
 D
...
X15 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>20!($L(X)<5) X I $D(X) D ZIPIN^VAFADDR
 ...
 Q
 ;
16 D:$D(DG)>9 F^DIE17 G ^DGPTX112
