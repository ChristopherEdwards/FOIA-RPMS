...
DE S DIE="^DGPT(D0,""M"",",DIC=DIE,DP=45.02,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^DGPT(D0,"M",DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,3) S:%]"" DE(5)=% S %=$P(%Z,U,4) S:%]"" DE(8)=% S %=$P(%Z,U,5) S:%]"" DE(22)=% S %=$P(%Z,U,6) S:%]"" DE(27)=% S %=$P(%Z,U,7) S:%]"" DE(32)=% S %=$P(%Z,U,18) S:%]"" DE(10)=%,DE(13)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X11 S (DGNFLD,Y)="@20"
 Q
12 S DQ=13 ;@15
13 S DW="0;18",DV="S",DU="",DLB="TREATED FOR SC CONDITION",DIFLD=18
 S DU="1:YES;2:NO;"
 S Y="NO"
 G Y
X13 Q
...
C22S S X="" G:DG(DQ)=X C22F1 K DB
 ...
 X "N X S X=""DGRUDD01"" X ^%ZOSF(""TEST"") Q:'$T  N DG1 S DG1=+$P(^DGPT(DA(1),0),""^"",1) D:(DG1>0) ADGRU^DGRUDD01(DG1)"
C22F1 Q
X22 K K S DIC("S")="S DGI=5 D EN^DGPTFJC I 'DGER" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
C27S S X="" G:DG(DQ)=X C27F1 K DB
 ...
 X "N X S X=""DGRUDD01"" X ^%ZOSF(""TEST"") Q:'$T  N DG1 S DG1=+$P(^DGPT(DA(1),0),""^"",1) D:(DG1>0) ADGRU^DGRUDD01(DG1)"
C27F1 Q
X27 K K S DIC("S")="S DGI=6 D EN^DGPTFJC I 'DGER" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
C32S S X="" G:DG(DQ)=X C32F1 K DB
 D ^DGPTX53
C32F1 Q
X32 K K S DIC("S")="S DGI=7 D EN^DGPTFJC I 'DGER" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
