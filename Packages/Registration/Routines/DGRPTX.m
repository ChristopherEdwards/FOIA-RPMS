DGRPTX ; GENERATED FROM 'DGRPT 10-10T REGISTRATION' INPUT TEMPLATE(#2464), FILE 2;10/15/12
 D DE G BEGIN
...
BEGIN S DNM="DGRPTX",DQ=1
 ...
 M DIEZAR=^DIE(2464,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2464,U="^"
...
C3F1 Q
X3 Q
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="TYPE;1",DV="RP391'",DU="",DLB="TYPE",DIFLD=391
...
C4S S X="" G:DG(DQ)=X C4F1 K DB
 S X=DG(DQ),DIC=DIE
 I ($T(AVAFC^VAFCDD01)'="") S VAFCF="391;" D AVAFC^VAFCDD01(DA)
C4F1 N X,X1,X2 S DIXR=327 D C4X1(U) K X2 M X2=X D C4X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^DPT("APTYPE",X,DA)
...
C4F2 Q
X4 Q
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW="VET;1",DV="RSX",DU="",DLB="VETERAN (Y/N)?",DIFLD=1901
 S DE(DW)="C5^DGRPTX"
 S DU="Y:YES;N:NO;"
 G RE
...
C7S S X="" G:DG(DQ)=X C7F1 K DB
 D ^DGRPTX2
C7F1 N X,X1,X2 S DIXR=21 D C7X1(U) K X2 M X2=X D C7X1("O") K X1 M X1=X
 D
...
C9S S X="" G:DG(DQ)=X C9F1 K DB
 D ^DGRPTX4
C9F1 N X,X1,X2 S DIXR=23 D C9X1(U) K X2 M X2=X D C9X1("O") K X1 M X1=X
 D
