...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.362)) S %Z=^(.362) S %=$P(%Z,U,12) S:%]"" DE(12)=%
...
X5 S DFN=DA D SV^DGLOCK
 Q
 ;
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW=".322;13",DV="RSX",DU="",DLB="ENVIRONMENTAL CONTAMINANTS?",DIFLD=.322013
 S DE(DW)="C6^DGRPTX18"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
X7 S DFN=DA D SV^DGLOCK
 Q
 ;
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW=".3;1",DV="RSX",DU="",DLB="SERVICE CONNECTED?",DIFLD=.301
 S DE(DW)="C8^DGRPTX18"
 S DU="Y:YES;N:NO;"
 G RE
