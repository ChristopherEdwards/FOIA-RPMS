...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.36)) S %Z=^(.36) S %=$P(%Z,U,1) S:%]"" DE(3)=%
 I $D(^(.362)) S %Z=^(.362) S %=$P(%Z,U,13) S:%]"" DE(1)=% S %=$P(%Z,U,14) S:%]"" DE(2)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X2 S DFN=DA D MV^DGLOCK
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".36;1",DV="*P8'X",DU="",DLB="PRIMARY ELIGIBILITY CODE",DIFLD=.361
 S DE(DW)="C3^DGRPTX24"
 S DU="DIC(8,"
 G RE
...
C3S S X="" G:DG(DQ)=X C3F1 K DB
 ...
 D AUTOUPD^DGENA2(DA)
C3F1 Q
X3 S DFN=DA D EV^DGLOCK I $D(X) D ECD^DGLOCK1
 Q
 ;
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,D=0 K DE(1) ;361
 S DIFLD=361,DGO="^DGRPTX25",DC="3^2.0361IP^E^",DV="2.0361M*P8'X",DW="0;1",DOW="ELIGIBILITY",DLB=$P($$EZBLD^DIALOG(8042,DOW),":") S:D DC=DC_D
 S DU="DIC(8,"
