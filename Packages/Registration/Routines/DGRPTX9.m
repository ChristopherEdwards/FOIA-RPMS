...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.21)) S %Z=^(.21) S %=$P(%Z,U,1) S:%]"" DE(6)=% S %=$P(%Z,U,2) S:%]"" DE(8)=% S %=$P(%Z,U,10) S:%]"" DE(10)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".11;16",DV="S",DU="",DLB="BAD ADDRESS INDICATOR",DIFLD=.121
 ...
 G RE
X4 Q
5 S DW="0;5",DV="RP11'",DU="",DLB="MARITAL STATUS",DIFLD=.05
 S DE(DW)="C5^DGRPTX9"
 S DU="DIC(11,"
 G RE
...
C5F1 Q
X5 Q
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW=".21;1",DV="FX",DU="",DLB="K-NAME OF PRIMARY NOK",DIFLD=.211
...
C6S S X="" G:DG(DQ)=X C6F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 D:($T(ADGRU^DGRUDD01)'="") ADGRU^DGRUDD01(DA)
C6F1 N X,X1,X2 S DIXR=41 D C6X1(U) K X2 M X2=X D C6X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . I '$G(XUNOTRIG) N XUNOTRIG S XUNOTRIG=1 D DELCOMP^XLFNAME2(2,.DA,.211,1.02) Q
 K X M X=X2 I $G(X(1))]"" D
 . I '$G(XUNOTRIG) N XUNOTRIG S XUNOTRIG=1,DG20NAME=X D NARY^DPTNAME(.DG20NAME),UPDCOMP^XLFNAME2(2,.DA,.211,.DG20NAME,1.02,+$P($G(^DPT(DA,"NAME")),U,2),"CL35") K DG20NAME Q
 G C6F2
C6X1(DION) K X
 ...
 Q
C6F2 Q
X6 K:$L(X)>35!($L(X)<3) X I $D(X) K DG20NAME S DG20NAME=X,(X,DG20NAME)=$$FORMAT^DPTNAME(.DG20NAME,3,35) K:'$L(X) X,DG20NAME
 I $D(X),X'?.ANP K X
 Q
 ;
...
10 D:$D(DG)>9 F^DIE17,DE S DQ=10,DW=".21;10",DV="RSX",DU="",DLB="K-ADDRESS SAME AS PATIENT'S?",DIFLD=.2125
 S DU="Y:YES;N:NO;"
 S Y="NO"
 G Y
...
X11 I X="Y" S DGADD=".21" D AD^DGRPE S Y=.21011
 Q
12 D:$D(DG)>9 F^DIE17 G ^DGRPTX10
