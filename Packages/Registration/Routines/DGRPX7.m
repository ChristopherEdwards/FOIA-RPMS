DGRPX7 ; GENERATED FROM 'DG LOAD EDIT SCREEN 7' INPUT TEMPLATE(#2407), FILE 2;03/21/13
 D DE G BEGIN
...
BEGIN S DNM="DGRPX7",DQ=1
 ...
 M DIEZAR=^DIE(2407,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2407,U="^"
1 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=1 D X1 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X1 S:DGDR'["701" Y="@702"
 Q
2 S DW="TYPE;1",DV="RP391'",DU="",DLB="TYPE",DIFLD=391
...
C2S S X="" G:DG(DQ)=X C2F1 K DB
 S X=DG(DQ),DIC=DIE
 I ($T(AVAFC^VAFCDD01)'="") S VAFCF="391;" D AVAFC^VAFCDD01(DA)
C2F1 N X,X1,X2 S DIXR=327 D C2X1(U) K X2 M X2=X D C2X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^DPT("APTYPE",X,DA)
...
X3 D SC7^DGRPV
 Q
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="VET;1",DV="RSX",DU="",DLB="VETERAN (Y/N)?",DIFLD=1901
 S DE(DW)="C4^DGRPX7"
 S DU="Y:YES;N:NO;"
 G RE
...
X4 I $D(X) S:'$D(DPTX) DFN=DA D:'$D(^XUSEC("DG ELIGIBILITY",DUZ)) VAGE^DGLOCK:X="Y" I $D(X) D:$D(DFN) EV^DGLOCK
 Q
 ;
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW=".3;1",DV="RSX",DU="",DLB="SERVICE CONNECTED?",DIFLD=.301
 S DE(DW)="C5^DGRPX7"
 S DU="Y:YES;N:NO;"
 G RE
...
X10 I $P($G(^DPT(DFN,.361)),U)="V" I $P($G(^DPT(DFN,.361)),U,3)="H" S Y="@293"
 Q
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW=".3;4",DV="S",DU="",DLB="P&T",DIFLD=.304
 S DE(DW)="C11^DGRPX7"
 S DU="Y:YES;N:NO;"
 G RE
...
X15 W !,"No editing P&T Data, Eligibility record verified by HEC"
 Q
16 D:$D(DG)>9 F^DIE17 G ^DGRPX73
