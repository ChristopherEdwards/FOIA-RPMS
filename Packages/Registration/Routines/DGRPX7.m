DGRPX7 ; GENERATED FROM 'DG LOAD EDIT SCREEN 7' INPUT TEMPLATE(#2407), FILE 2;10/29/04
 D DE G BEGIN
...
X15 S DFN=DA D EV^DGLOCK I $D(X) S L=$S($D(^DPT(DA,0)):$P(^(0),U,9),1:X) W:X?1"SS".E "  ",L S:X?1"SS".E X=L I X'=L K:$L(X)>8!($L(X)<7)!'(X?.N) X
 ...
 Q
 ;
16 D:$D(DG)>9 F^DIE17,DE S DQ=16,DW=".31;4",DV="*P4'",DU="",DLB="CLAIM FOLDER LOCATION",DIFLD=.314
 S DE(DW)="C16^DGRPX7"
 S DU="DIC(4,"
 G RE
...
C16S S X="" G:DG(DQ)=X C16F1 K DB
 D ^DGRPX74
C16F1 Q
X16 S DIC("S")="I $$GET1^DIQ(4,+Y,99)'=""""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
17 S DQ=18 ;@702
