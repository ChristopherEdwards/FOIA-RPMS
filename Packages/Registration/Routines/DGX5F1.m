...
X17 S (DGNFLD,Y)="@27"
 Q
18 S DQ=19 ;@25
19 S DW="0;18",DV="S",DU="",DLB="TREATED FOR SC CONDITION",DIFLD=18
 S DU="1:YES;2:NO;"
 S Y="NO"
 G Y
X19 Q
...
C25S S X="" G:DG(DQ)=X C25F1 K DB
 ...
 X "N X S X=""DGRUDD01"" X ^%ZOSF(""TEST"") Q:'$T  N DG1 S DG1=+$P(^DGPT(DA(1),0),""^"",1) D:(DG1>0) ADGRU^DGRUDD01(DG1)"
C25F1 Q
X25 K K S DIC("S")="S DGI=5 D EN^DGPTFJC I 'DGER" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
C29S S X="" G:DG(DQ)=X C29F1 K DB
 D ^DGX5F3
C29F1 Q
X29 K K S DIC("S")="S DGI=6 D EN^DGPTFJC I 'DGER" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
