VALMXP ; GENERATED FROM 'VALM LIST TEMPLATE' PRINT TEMPLATE (#1194) ; 08/21/95 ; (FILE 409.61, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1194,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
