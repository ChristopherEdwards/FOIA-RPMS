SDAMXOE ; GENERATED FROM 'SD ENCOUNTER ENTRY' INPUT TEMPLATE(#2461), FILE 409.68;10/29/04
 D DE G BEGIN
...
BEGIN S DNM="SDAMXOE",DQ=1
 ...
 M DIEZAR=^DIE(2461,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2461,U="^"
...
C5 G C5S:$D(DE(5))[0 K DB
 ...
 S X=DE(5),DIC=DIE
 X "Q  N SDX S SDX=X,X=""VSIT"" X ^%ZOSF(""TEST"") I $T S X=SDX D SUB^VSIT"
 S X=DE(5),DIC=DIE
 ;
C5S S X="" G:DG(DQ)=X C5F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 X "Q  N SDX S SDX=X,X=""VSIT"" X ^%ZOSF(""TEST"") I $T S X=SDX D ADD^VSIT"
 S X=DG(DQ),DIC=DIE
 Q  K DIV S DIV=X,D0=DA,DIV(0)=D0 S Y(1)=$S($D(^SCE(D0,0)):^(0),1:"") S X=$P(Y(1),U,20),X=X S DIU=X K Y S X=DIV S X=$$IEN2VID^VSIT(X) X ^DD(409.68,.05,1,3,1.4)
C5F1 Q
X5 Q
