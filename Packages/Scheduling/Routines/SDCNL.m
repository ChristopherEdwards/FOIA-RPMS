SDCNL ;ALB/LDB - CANCELLED APPOINTMENT LETTER ; [ 09/13/2001  2:20 PM ]
...
START ;F SDX=SDBD:0 S SDX=$N(^DPT(SDPT,"S",SDX)) Q:SDX>(SDED+.9999)!(SDX'>0)  S SDAP=^DPT(SDPT,"S",SDX,0),SDV2=0 I $P(SDAP,"^",2)["C" D MDIV I SDV2!'SDV S SDC=+SDAP D CHK1 I 'SDOK D CHK  ;IHS/ANMC/LJF 8/18/2000
 ...
 Q
PR S SDLET="" F A0=0:0 S SDLET=$O(^UTILITY($J,SDLET)) Q:'SDLET  S (B0,X7)="" F A1=0:0 S A5=B0,B0=$O(^UTILITY($J,SDLET,B0)) D:B0="" R Q:B0=""  D R:A5&(B0'=A5) S A=B0 D ^SDLT F A2=0:0 S X7=$O(^UTILITY($J,SDLET,B0,X7)) Q:X7=""  D S,WRAPP^SDLT
...
CHK1 S SDOK=0 I '$D(^SC(+SDAP,"S",SDX)) Q
 ;I $D(^SC(+SDAP,"S",SDX)) F P=0:0 S P=$N(^SC(+SDAP,"S",SDX,1,P)) Q:P'>0  I $P(^(P,0),"^")=SDPT S SDOK=1  ;IHS/ANMC/LJF 8/18/2000
 I $D(^SC(+SDAP,"S",SDX)) F P=0:0 S P=$O(^SC(+SDAP,"S",SDX,1,P)) Q:P'>0  I $P(^(P,0),"^")=SDPT S SDOK=1  ;IHS/ANMC/LJF 8/18/2000
 Q
