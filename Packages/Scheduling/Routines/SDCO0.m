SDCO0 ;ALB/RMO - Build List Area - Check Out; 11 FEB 1993 10:00 am ; 10/27/99 12:56pm [ 07/12/2000  9:27 AM ]
 ;;5.3;Scheduling;**20,44,132,180**;Aug 13, 1993
 ;IHS/ANMC/LJF 7/12/2000 removed display of classification data
 ;                       showed that stop codes are required for IHS
 ;
EN(SDARY,SDOE,SDSTART,SDTOT) ;Entry point Called by Ck Out & Apt Mgr Exp Dis
 S SDTOT=0
 ;D CL(SDARY,SDOE,SDSTART,.SDTOT)  ;IHS/ANMC/LJF 7/12/2000
...
PR(SDARY,SDOE,SDSTART,SDTOT) ;Build Provider (Pg: 1  Row: SDSTART+8-END  Col: 1-40)
 ...
 ;S SDLINE=SDSTART+8  ;IHS/ANMC/LJF 7/12/2000
 S SDLINE=SDSTART     ;IHS/ANMC/LJF 7/12/2000
...
DX(SDARY,SDOE,SDSTART,SDTOT) ;Build Diagnosis (Pg: 1  Row: SDSTART+8-END  Col: 42-80)
 N SDCNT,SDDXS,SDDXD,SDVPOV,SDLINE
 ;S SDLINE=SDSTART+8   ;IHS/ANMC/LJF 7/12/2000
 S SDLINE=SDSTART      ;IHS/ANMC/LJF 7/12/2000
 ...
 . S SDCNT=SDCNT+1
 . S SDLINE=SDLINE+1
 . S SDDXD=$$DX^SDCO41(+SDDXS(SDVPOV))
...
SC(SDARY,SDOEP,SDSTART,SDTOT) ;Build Stop Codes (Pg: 2  Row: SDTOT+1  Col: 1-80)
 ...
 ;D SET(SDARY,SDLINE," STOP CODES ",5,IORVON,IORVOFF,"","","",.SDTOT)   ;IHS/ANMC/LJF 7/12/2000
 D SET(SDARY,SDLINE," STOP CODES / PROCEDURES [Required] ",5,IORVON,IORVOFF,"","","",.SDTOT)     ;IHS/ANMC/LJF 7/12/2000
 ;D SET(SDARY,SDLINE,"[Stop Codes Not Required / Procedures Required]",28,"","","","","",.SDTOT)   ;IHS/ANMC/LJF 7/12/2000
...
AE(SDARY,SDOE,SDLINE,SDTOT,SDONE) ; -- add/edits
 ...
 S (SDCNT,SDVCPT)=0
 N MODINFO,MODPTR,MODTEXT,PTR,MODCODE,CPTINFO
 ...
 .; N CPTINFO
 . S CPTINFO=$$CPT^ICPTCOD(+$G(CPTS(SDVCPT)),,1)
