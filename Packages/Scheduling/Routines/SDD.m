...
START D:'$D(DT) DT^SDUTL S (YP,PG,SD,SDU)=0,Y=DT X ^DD("DD") S DAT=Y D HD
 ;F SCI=0:0 S SD=$S(VAUTC:$N(^SC("B",SD)),1:$N(VAUTC(SD))) Q:SD<0  S SC=0 F SCC=0:0 S SC=$N(^SC("B",SD,SC)) Q:SC<0  S SD0=^SC(SC,0) I $P(SD0,U,3)="C" S SDNM=SD D SETX^SDD0 G:SDU END  ;IHS/ANMC/LJF 8/2/2000
 F SCI=0:0 S SD=$S(VAUTC:$O(^SC("B",SD)),1:$O(VAUTC(SD))) Q:SD=""  S SC=0 F SCC=0:0 S SC=$O(^SC("B",SD,SC)) Q:SC=""  S SD0=^SC(SC,0) I $P(SD0,U,3)="C" S SDNM=SD D SETX^SDD0 G:SDU END  ;IHS/ANMC/LJF 8/2/2000
