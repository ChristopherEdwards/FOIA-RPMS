SDDPA ;MAN/GRR,ALB/TMP - DISPLAY APPOINTMENTS ; [ 09/13/2001  2:25 PM ]
 ;;5.3;Scheduling;**140**;Aug 13, 1993
 ;IHS/ANMC/LJF 11/30/2000 contains $N but not used by IHS
 ;              7/14/2001 removed call to ^DD("FUNC",2,1) - has error
 ;
...
1 U IO S SDSTR=$S($D(^DPT(DFN,0)):^(0),1:""),SDN=$P(SDSTR,U),SDSSN=$P(SDSTR,U,9),%DT="R",X="N" D ^%DT W @IOF,!,"APPOINTMENTS FOR: ",$E(SDN,1,22),?42,$E(SDSSN,1,3),"-",$E(SDSSN,4,5),"-",$E(SDSSN,6,9),?54,"PRINTED: ",$$FMTE^XLFDT(Y,"5")
 G:$N(^DPT(DFN,"S",HDT))'>0 NO S NDT=HDT,L=0
EN1 F J=1:1 S NDT=$N(^DPT(DFN,"S",NDT)) Q:NDT'>0!(SDRG&(NDT>SDEDT))  I $S($P(^(NDT,0),"^",2)']"":1,$P(^(0),"^",2)["NT":1,$P(^(0),"^",2)["I":1,SDRG:1,1:0) D CHKSO,FLEN S ^UTILITY($J,L)=NDT_"^"_SC_"^"_COV_"^"_APL_"^^"_SDNS_"^"_SDBY
 ;
 ;IHS/ANMC/LJF 7/14/2001 time function has error
 ...
 G:L'>0 NO F ZZ=1:1:L S AT=$S($P(^UTILITY($J,ZZ),"^",2)'?.N:1,1:0) W !! S Y=$P($P(^(ZZ),"^",1),".",1) D DT^SDM0 S X=$P(^(ZZ),"^",1),X=$$TIME^BDGF(X) W " ",$J(X,8) D MORE Q:SDEND
 ;IHS/ANMC/LJF 7/14/2001 end of mod
 ;
 G END
 ;
...
RANGE D DATE^SDUTL Q:POP  S HDT=BEGDATE,SDEDT=ENDDATE_.9,SDRG=1,SDONE=0
 I $D(^DPT(DFN,"ARCH","AB","S")) S X=$N(^("S",0)) I $D(^DPT(DFN,"ARCH",X)) F A=0:0 S A=$N(^DPT(DFN,"ARCH",X,1,A)) Q:A'>0  S Z=^(A,0),B=$P(Z,"^",3),C=$P(Z,"^",4),D=$P(Z,"^",5),E=$P(Z,"^",2) I B'<HDT&(B'>SDEDT)!(C'<HDT&(C'>SDEDT)) D ARCH
 Q
...
FLEN S SC=+^DPT(DFN,"S",NDT,0),L=L+1,COV=$S($P(^DPT(DFN,"S",NDT,0),"^",11)=1:" (COLLATERAL) ",1:"") I $D(^SC(SC,"S",NDT)) F ZL=0:0 S ZL=$N(^SC(SC,"S",NDT,1,ZL)) Q:ZL<0  I +^(ZL,0)=DFN S APL=$P(^SC(SC,"S",NDT,1,ZL,0),"^",2) Q
 Q
