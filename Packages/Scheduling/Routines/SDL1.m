SDL1 ;BSN/GRR,ALB/LDB - PRE-APPOINTMENT LETTERS ; 7/7/04 10:45am
 ;;5.3;Scheduling;**106,330,340,407,398,1015**;Aug 13, 1993;Build 21
 ;IHS/ANMC/LJF 12/07/2000 changed SSN to HRCN
 ;
 ;******************MODIFICATIONS***********************************
 ;
...
NO I $D(VAUTN),'$O(^UTILITY($J,0)),'$D(^UTILITY($J,"C")) D
 ...
 ;I $D(^UTILITY($J,"C")) W @IOF F X=0:0 S X=$O(^UTILITY($J,"C",X)) Q:'X  W !!,$P(^DPT(X,0),"^")," ",$P(^(0),"^",9)," HAS FUTURE APPTS., but" D NOCL  ;IHS/ANMC/LJF 12/07/2000
 I $D(^UTILITY($J,"C")) W @IOF F X=0:0 S X=$O(^UTILITY($J,"C",X)) Q:'X  W !!,$P(^DPT(X,0),"^")," ",$$HRCN^BDGF2(X,+$G(DUZ(2)))," HAS FUTURE APPTS., but" D NOCL   ;IHS/ANMC/LJF 12/07/2000
