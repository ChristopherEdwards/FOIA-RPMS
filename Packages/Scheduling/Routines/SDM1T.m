SDM1T ; GENERATED FROM 'SDM1' INPUT TEMPLATE(#2483), FILE 2;02/09/12
...
BEGIN S DNM="SDM1T",DQ=1
 ...
 M DIEZAR=^DIE(2483,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2483,U="^"
...
C1F1 N X,X1,X2 S DIXR=22 D C1X1(U) K X2 M X2=X D C1X1("O") K X1 M X1=X
 D
...
C5F1 N X,X1,X2 S DIXR=25 D C5X1(U) K X2 M X2=X D C5X1("O") K X1 M X1=X
 D
...
C6F1 N X,X1,X2 S DIXR=26 D C6X1(U) K X2 M X2=X D C6X1("O") K X1 M X1=X
 D
...
C7F1 N X,X1,X2 S DIXR=27 D C7X1(U) K X2 M X2=X D C7X1("O") K X1 M X1=X
 D
...
C8S S X="" G:DG(DQ)=X C8F1 K DB
 D ^SDM1T1
C8F1 N X,X1,X2 S DIXR=23 D C8X1(U) K X2 M X2=X D C8X1("O") K X1 M X1=X
 D
...
C8X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C8F2 S DIXR=51 D C8X2(U) K X2 M X2=X D C8X2("O") K X1 M X1=X
 D
...
X8 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>20!($L(X)<5) X I $D(X) D ZIPIN^VAFADDR
 ...
 Q
 ;
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW=".11;7",DV="NJ3,0XOa",DU="",DLB="COUNTY",DIFLD=.117
...
X9 S Z0=$S($D(^DPT(D0,.11)):+$P(^(.11),"^",5),1:0) K:'Z0 X Q:'Z0!'$D(^DIC(5,Z0,1,0))  S DIC="^DIC(5,Z0,1,",DIC(0)="QEM" D ^DIC S X=+Y K:Y'>0 X K Z0,DIC
 Q
 ;
10 D:$D(DG)>9 F^DIE17,DE S DQ=10,DW=".13;1",DV="Fa",DU="",DLB="PHONE NUMBER [RESIDENCE]",DIFLD=.131
 S DE(DW)="C10^SDM1T"
 G RE
