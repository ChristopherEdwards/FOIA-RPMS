...
C1F1 N X,X1,X2 S DIXR=26 D C1X1(U) K X2 M X2=X D C1X1("O") K X1 M X1=X
 D
...
C2F1 N X,X1,X2 S DIXR=22 D C2X1(U) K X2 M X2=X D C2X1("O") K X1 M X1=X
 D
...
C2X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C2F2 S DIXR=50 D C2X2(U) K X2 M X2=X D C2X2("O") K X1 M X1=X
 D
...
X2 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>20!($L(X)<5) X I $D(X) D ZIPIN^VAFADDR
 ...
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".11;7",DV="NJ3,0XO",DU="",DLB="COUNTY",DIFLD=.117
...
X3 S Z0=$S($D(^DPT(D0,.11)):+$P(^(.11),"^",5),1:0) K:'Z0 X Q:'Z0!'$D(^DIC(5,Z0,1,0))  S DIC="^DIC(5,Z0,1,",DIC(0)="QEM" D ^DIC S X=+Y K:Y'>0 X K Z0,DIC
 Q
 ;
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".13;1",DV="F",DU="",DLB="PHONE NUMBER [RESIDENCE]",DIFLD=.131
 S DE(DW)="C4^SDM1T5"
 G RE
