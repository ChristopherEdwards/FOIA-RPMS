SDMULT0 ;ALB/TMP - MAKE MULTI-CLINIC APPOINTMENTS ; [ 09/13/2001  2:33 PM ]
 ;;5.3;Scheduling;**41**;AUG 13, 1993
 ;IHS/ANMC/LJF 11/30/2000 changed $N to $O
 ;
START ;W !,"The following clinics have been selected: ",! F I=0:0 S I=$N(SDC1(I)) Q:I'>0  W !,$P(SDC1(I),"^",1),?45,+$P(SDC1(I),"^",2)," MINUTE APPOINTMENT"   ;IHS/ANMC/LJF 11/30/2000
 W !,"The following clinics have been selected: ",! F I=0:0 S I=$O(SDC1(I)) Q:I'>0  W !,$P(SDC1(I),"^",1),?45,+$P(SDC1(I),"^",2)," MINUTE APPOINTMENT"   ;IHS/ANMC/LJF 11/30/2000 $N->$O
...
OVR ;S SD1=0 F G1=0:0 S G1=$N(SDC(G1)),SD1=SD1+1 Q:G1'>0  D S1,AV Q:'FND  S (SDSTRTDT,SDDT(SD1))=SDAPP   ;IHS/ANMC/LJF 11/30/2000
 S SD1=0 F G1=0:0 S G1=$O(SDC(G1)),SD1=SD1+1 Q:G1'>0  D S1,AV Q:'FND  S (SDSTRTDT,SDDT(SD1))=SDAPP   ;IHS/ANMC/LJF 11/30/2000 $N->$O
A I 'FND W:'$D(SDNEXT) !,"No available slots found" W:'$D(SDNEXT) " on the same day in all the selected clinics for this",!,"  date range" G END
 ...
 D FND^SDMULT1 G END
LOOKA ;S SD1=0 F G1=0:0 S G1=$N(SDC(G1)),SD1=SD1+1 Q:G1'>0  I SDDT(SD1)-SDSTRTDT D S1 D:SDSTRTDT'>SDMAX AV Q:'FND  S (SDSTRTDT,SDDT(SD1))=SDAPP   ;IHS/ANMC/LJF 11/30/2000
 S SD1=0 F G1=0:0 S G1=$O(SDC(G1)),SD1=SD1+1 Q:G1'>0  I SDDT(SD1)-SDSTRTDT D S1 D:SDSTRTDT'>SDMAX AV Q:'FND  S (SDSTRTDT,SDDT(SD1))=SDAPP   ;IHS/ANMC/LJF 11/30/2000 $N->$O
 G A
AV S SL=$S($D(^SC(SC,"SL")):^("SL"),1:"") I SL']"" W !,*7,"No 'SL' node defined - cannot proceed with this clinic" Q
 ...
 ;I '$D(SDJ),$N(^SC(SC,"ST",SDSTRTDT))'>0 Q   ;IHS/ANMC/LJF 11/30/2000
 I '$D(SDJ),$O(^SC(SC,"ST",SDSTRTDT))'>0 Q   ;IHS/ANMC/LJF 11/30/2000
 S SDATE=$S($E(SDSTRTDT,6,7):SDSTRTDT,$E(SDSTRTDT,4,5):SDSTRTDT+1,1:SDSTRTDT+101)
LOOP ;I '$D(SDJ),$N(^SC(+SC,"ST",SDATE-1))'>0 Q   ;IHS/ANMC/LJF 11/30/2000
 I '$D(SDJ),$O(^SC(+SC,"ST",SDATE-1))'>0 Q   ;IHS/ANMC/LJF 11/30/2000
 G:$D(^HOLIDAY(SDATE))&('SDSOH) NEXT I $D(^SC(+SC,"ST",SDATE,1)) S SDP=^(1) G CHECK
 ;S (X,SDATE1)=SDATE D DOW^SDM0 G:'$D(SDJ(Y)) NEXT S SDZ=$N(^SC(+SC,"T"_Y,0)) I SDZ>SDATE S SDATE1=SDZ   ;IHS/ANMC/LJF 11/30/2000
 S (X,SDATE1)=SDATE D DOW^SDM0 G:'$D(SDJ(Y)) NEXT S SDZ=$O(^SC(+SC,"T"_Y,0)) I SDZ>SDATE S SDATE1=SDZ   ;IHS/ANMC/LJF 11/30/2000 $N->$O
 ;S SDZ=$N(^SC(+SC,"T"_Y,SDATE1)) I SDZ<0!($S('$D(^SC(+SC,"T"_Y,SDZ,1)):1,^(1)']"":1,1:0))!(SDZ>SDATE) K:SDZ<0!(SDZ>SDMAX) SDJ(Y) G NEXT   ;IHS/ANMC/LJF 11/30/2000
 S SDZ=$O(^SC(+SC,"T"_Y,SDATE1)) I SDZ=""!($S('$D(^SC(+SC,"T"_Y,+SDZ,1)):1,^(1)']"":1,1:0))!(SDZ>SDATE) K:SDZ<0!(SDZ>SDMAX) SDJ(Y) G NEXT   ;IHS/ANMC/LJF 11/30/2000 $N->$O
