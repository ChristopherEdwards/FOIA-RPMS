SDROUT1 ;MAN/GRR - ROUTING SLIPS ; [ 09/13/2001  2:41 PM ]
 ;;5.3;Scheduling;**3**;Aug 13, 1993
...
OSET I ORDER="" S ^UTILITY($J,NAME,DFN,GDATE,SC)="" Q
...
EN S VAR="DIV^ORDER^SDX^DFN^SDREP^SDSTART",DGPGM="EN1^SDROUT1"
...
EN1 ; -- main entry point
 ...
 ;S NDATE=$N(^DPT(DFN,"S",SDATE)) I NDATE\1'=SDATE G NOCA   ;IHS/ANMC/LJF 11/30/2000
 S NDATE=$O(^DPT(DFN,"S",SDATE)) I NDATE\1'=SDATE G NOCA   ;IHS/ANMC/LJF 11/30/2000 $N->$O
 S Y=DT D DTS^SDUTL S APDATE=Y
 ;K SDA F GDATE=SDATE:0 S GDATE=$N(^DPT(DFN,"S",GDATE)) Q:GDATE<0!(GDATE\1-SDATE)  I $P(^(GDATE,0),"^",2)="I"!($P(^(0),"^",2)="") D GOT   ;IHS/ANMC/LJF 11/30/2000
 K SDA F GDATE=SDATE:0 S GDATE=$O(^DPT(DFN,"S",GDATE)) Q:('GDATE)!(GDATE\1-SDATE)  I $P(^(GDATE,0),"^",2)="I"!($P(^(0),"^",2)="") D GOT   ;IHS/ANMC/LJF 11/30/2000 $N->$O
...
SC I $D(^DPT(DFN,.36)),$D(^DIC(8,+^DPT(DFN,.36),0)),$P(^(0),"^",9)=13 S V=1 Q
 ;S V=0 F M=0:0 S M=$N(^SC(SC,"S",GDATE,1,M)) Q:M'>0  I $D(^(M,0)),+^(0)=DFN,$P(^(0),"^",9)'["C" S V=$P(^(0),"^",10) Q:V']""  S V=$S($D(^DIC(8,+V,0)):$P(^(0),"^",9)=13,1:0) Q   ;IHS/ANMC/LJF 11/30/2000
 S V=0 F M=0:0 S M=$O(^SC(SC,"S",GDATE,1,M)) Q:M'>0  I $D(^(M,0)),+^(0)=DFN,$P(^(0),"^",9)'["C" S V=$P(^(0),"^",10) Q:V']""  S V=$S($D(^DIC(8,+V,0)):$P(^(0),"^",9)=13,1:0) Q   ;IHS/ANMC/LJF 11/30/2000 $N->$O
 Q
...
REP S SDREP=1 G:SDX["ALL" ALL S %DT("A")="REPRINT ADD-ONS THAT WERE RUN ON WHAT DATE: ",%DT="AEX" D ^%DT K %DT("A") I Y<1 S POP=1 Q
 S SDSTART=Y Q
ALL W !,"ENTER ",$S(ORDER=1:"TERMINAL DIGIT",ORDER=2:"CLINIC NAME",1:"PATIENT NAME")," TO BEGIN REPRINT FROM: " R X:DTIME I X["?" D HELP G ALL
 I "^"[X S POP=1 Q
 I ORDER=1,X'?4N W !,*7,"MUST BE 4 NUMERICS" G ALL
 S SDSTART=X Q
DQ S ZTREQ="@" G EN1
HELP W !!,"THE REPRINT WILL BEGIN PRINTING AT THE ",$S(ORDER=1:"TERMINAL DIGIT",ORDER=2:"CLINIC NAME",1:"PATIENT NAME")," YOU SPECIFY",!
