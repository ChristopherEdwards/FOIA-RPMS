...
SORT ;Sort Records
 ...
 Q
PRT ;
 S SDWLIN=0 F  S SDWLIN=$O(^TMP("SDWLRPS2",$J,"A",SDWLIN)) Q:SDWLIN=""  W !,"Institution: ",$$GET1^DIQ(4,SDWLIN_",",".01"),! D
 .D PRA
 Q
PRA ;
 S SDWLSC=0,(SDWLX,SDWLXT)=0 F  S SDWLSC=$O(^TMP("SDWLRPS2",$J,"A",SDWLIN,SDWLSC)) Q:SDWLSC=""  D
 ...
 W !,?21,"Total #: ",SDWLXT
 I $D(SDWLSPT) D SCR G END:$D(DUOUT) D HD,HD1
 Q
PRT1 ;
 N DFN
 S SDWLSCC=0 F  S SDWLSCC=$O(^TMP("SDWLRPS2",$J,"D",SDWLSCC)) Q:SDWLSCC=""  Q:$$S^%ZTLOAD  D
 .W !,"******* ",SDWLSCC," *******",!
 .S SDWLINS=0 F  S SDWLINS=$O(^TMP("SDWLRPS2",$J,"D",SDWLSCC,SDWLINS)) Q:SDWLINS=""  D  W !
 ..W !,$P($G(^DIC(4,SDWLINS,0)),U,1),! I '$G(^TMP("SDWLRPS2",$J,"D",SDWLSCC,SDWLINS)) W !,"*** No Patient Records to Report ***" D:$D(SDWLSPT) SCR D HD,HD1 Q
 ..S SDWLSC=0 F  S SDWLSC=$O(^TMP("SDWLRPS2",$J,"D",SDWLSCC,SDWLINS,SDWLSC)) Q:SDWLSC=""  D
 ...S SDWLWT="" F  S SDWLWT=$O(^TMP("SDWLRPS2",$J,"D",SDWLSCC,SDWLINS,SDWLSC,SDWLWT)) Q:SDWLWT=""  D
 ....S SDWLDA=0 F  S SDWLDA=$O(^TMP("SDWLRPS2",$J,"D",SDWLSCC,SDWLINS,SDWLSC,SDWLWT,SDWLDA)) Q:SDWLDA=""  D
