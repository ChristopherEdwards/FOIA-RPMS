...
DE S DIE="^SDV(D0,""CS"",",DIC=DIE,DP=409.51,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^SDV(D0,"CS",DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,2) S:%]"" DE(1)=% S %=$P(%Z,U,3) S:%]"" DE(2)=% S %=$P(%Z,U,5) S:%]"" DE(3)=%
 I $D(^(1)) S %Z=^(1) S %=$P(%Z,U,1) S:%]"" DE(4)=%
...
2 S DW="0;3",DV="*P44'",DU="",DLB="ASSOCIATED CLINIC",DIFLD=3
 ...
 G RD:X="@",Z
X2 Q
3 S DW="0;5",DV="R*P409.1'",DU="",DLB="APPOINTMENT TYPE",DIFLD=5
 S DE(DW)="C3^SDXACSE1"
 S DU="SD(409.1,"
 S X=SDAPTYP
 S Y=X
...
C3F1 Q
X3 Q
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="1;1",DV="SI",DU="",DLB="COMPUTER GENERATED?",DIFLD=11
 S DE(DW)="C4^SDXACSE1"
 S DU="1:YES;"
 S X=1
 S Y=X
...
X6 I $P(SDCPT(SDJ),U,4)']"" S Y="@99"
 Q
7 S DW="PR;2",DV="*P409.71'X",DU="",DLB="PROCEDURE 2",DIFLD=22
 S DU="SD(409.71,"
 S X=$P(SDCPT(SDJ),U,4)
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X8 I $P(SDCPT(SDJ),U,5)']"" S Y="@99"
 Q
9 S DW="PR;3",DV="*P409.71'X",DU="",DLB="PROCEDURE 3",DIFLD=23
 S DU="SD(409.71,"
 S X=$P(SDCPT(SDJ),U,5)
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X10 I $P(SDCPT(SDJ),U,6)']"" S Y="@99"
 Q
11 S DW="PR;4",DV="*P409.71'X",DU="",DLB="PROCEDURE 4",DIFLD=24
 S DU="SD(409.71,"
 S X=$P(SDCPT(SDJ),U,6)
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X12 I $P(SDCPT(SDJ),U,7)']"" S Y="@99"
 Q
13 S DW="PR;5",DV="*P409.71'X",DU="",DLB="PROCEDURE 5",DIFLD=25
 S DU="SD(409.71,"
 S X=$P(SDCPT(SDJ),U,7)
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
