ASUTP34 ; GENERATED FROM 'ASUOTP34' PRINT TEMPLATE (#2370) ; 09/07/00 ; (FILE 9002031, MARGIN=132)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(2370,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
...
A1 ;
 ...
 W ?20 D ^ASURO3UT K DIP K:DN Y
 D N:$X>40 Q:'DN  W ?40 X DXS(2,9.2) S DIP(101)=$S($D(^ASUMX(D0,0)):^(0),1:"") S X=$P(DIP(101),U,4) S D0=I(0,0) S D1=I(1,0) K DIP K:DN Y W X
 D N:$X>125 Q:'DN  W ?125 X DXS(3,9.2) S DIP(101)=$S($D(^ASUMX(D0,0)):^(0),1:"") S X=$S('$D(^ASUL(9,+$P(DIP(101),U,6),0)):"",1:$P(^(0),U,1)) S D0=I(0,0) S D1=I(1,0) K DIP K:DN Y W $E(X,1,3)
 ...
 D N:$X>121 Q:'DN  W ?121 S DIP(1)=$S($D(^ASUMS(D0,1,D1,0)):^(0),1:"") S X=$P(DIP(1),U,5),X=$S(X="":"",1:$E(X)_"."_$E(X,2)) K DIP K:DN Y W X
 Q
A1R ;
 K Y
 Q
