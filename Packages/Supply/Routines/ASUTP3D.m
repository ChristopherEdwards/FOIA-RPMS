ASUTP3D ; GENERATED FROM 'ASUOTP3D' PRINT TEMPLATE (#2371) ; 09/07/00 ; (FILE 9002031, MARGIN=132)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(2371,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
