...
MSP I SRFLG=3 S SRSS="ALL" G DEV
 ...
 D DEV
 Q
OPER N CPT,SRCPT K SROPERS S SRX=^SRF(SRTN,"OP"),SROPER=$P(SRX,"^"),CPT=$P(SRX,"^",2),SRCPT=$S(CPT:$P($$CPT^ICPTCOD(CPT),"^",2),1:"CPT NOT ENTERED") I CPT D PMOD
 S SROPER=SROPER_" ("_SRCPT_")" I $O(^SRF(SRTN,13,0)) S SROPER=SROPER_",  OTHER OPERATIONS: " S OTH=0 F  S OTH=$O(^SRF(SRTN,13,OTH)) Q:'OTH  D OTHER
 Q
OTHER ; other procedures
 S SRLONG=1,SROPERS=$P(^SRF(SRTN,13,OTH,0),"^"),CPT=$P($G(^SRF(SRTN,13,OTH,2)),"^"),SRCPT=$S(CPT:$P($$CPT^ICPTCOD(CPT),"^",2),1:"CPT NOT ENTERED") I CPT D OMOD
 S SROPERS=SROPERS_" ("_SRCPT_")"
 I $L(SROPER)+$L(SROPERS)>250 S SROPER=SROPER_" ...",OTH=999 Q
 S SROPER=SROPER_$S(OTH=1:" ",1:", ")_SROPERS
 Q
OMOD N SRCMOD,SRCOMMA,X I $O(^SRF(SRTN,13,OTH,"MOD",0)) D
 .S (SRCOMMA,SRI)=0,SRCMOD="",SRCPT=SRCPT_"-" F  S SRI=$O(^SRF(SRTN,13,OTH,"MOD",SRI)) Q:'SRI  D
 ..S SRM=$P(^SRF(SRTN,13,OTH,"MOD",SRI,0),"^"),SRCMOD=$P($$MOD^ICPTMOD(SRM,"I"),"^",2)
 ..S SRCPT=SRCPT_$S(SRCOMMA:",",1:"")_SRCMOD,SRCOMMA=1
 Q
PMOD N SRCMOD,SRCOMMA,X I $O(^SRF(SRTN,"OPMOD",0)) D
