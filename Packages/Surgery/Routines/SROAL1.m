SROAL1 ;BIR/ADM - LOAD PREOPERATIVE LAB DATA ; [ 12/01/99  2:34 PM ]
 ;;3.0; Surgery ;**18,38,47,54,65,71,81,88**;24 Jun 93
 ;
...
SRAT ; Get test and data name(s) for test from file 139.2.
 ...
 D ^SROAL11 S SRCON=$P($G(^SRF(SRTN,"CON")),"^") I SRCON D CONCC
...
TMCHK W !!,"Do you want to enter 'Time the Operation Began' at this time ?  YES//  " R SRYN:DTIME I '$T!(SRYN["^") S SRSOUT=1 Q
 ...
 W ! K DR S DR=".22T",DA=SRTN,DIE=130 D ^DIE K DR S SRAOP=$P($G(^SRF(SRTN,.2)),"^",2) I 'SRAOP S SRSOUT=1
 Q
...
CREAT ; from SROACL1 for creatinine
 ...
 .I $D(SRAT(SRAT)) S $P(^SRF(SRTN,201),"^",SRLB)=SRAT(SRAT),$P(^(202),"^",SRLB)=SRAD(SRAT)
 K SRLB
 Q
