SROAR2 ;BIR/MAM - ANNUAL REPORT, ONE SPECIALTY ; [ 11/17/99  6:46 AM ]
 ;;3.0; Surgery ;**34,60,50,88**;24 Jun 93
...
SET ; set local variables
 ...
 S (CPT,CNT)=0 F  S CPT=$O(^SRF(SRTN,13,CPT)) Q:CPT=""  S CNT=CNT+1 S Y=$P($G(^SRF(SRTN,13,CPT,2)),"^") I Y S X=$$CPT^ICPTCOD(Y),CPT(CNT)=$P(X,"^",2,3)
 S CPT("*")=$P(^SRF(SRTN,"OP"),"^",2) I CPT("*")'="" S X=$$CPT^ICPTCOD(CPT("*")),CPT("*")=$P(X,"^",2,3)
