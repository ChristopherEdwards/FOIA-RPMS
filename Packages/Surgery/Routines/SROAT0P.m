SROAT0P ;B'HAM ISC/MAM - ATTENDING SURGEON REPORT (CONT.) ; [ 07/27/98   2:33 PM ]
...
SET ; print case
 ...
 S X=$P($G(^SRF(SRTN,.1)),"^",16),SRATC=$S(X=0:"0. STAFF",X=1:"1. ATTENDING IN O.R.",X=2:"2. ATTENDING IN O.R. SUITE",X=3:"3. ATTEND NOT PRESENT, BUT AVAILABLE",1:"ATTENDING CODE NOT ENTERED")
...
OPS S SROPER=$P(^SRF(SRTN,"OP"),"^"),OPER=0 F I=0:0 S OPER=$O(^SRF(SRTN,13,OPER)) Q:OPER=""  D OTHER
 ...
 W !!,SRDT,?12,VADM(1),?50,SRSS,?112,SRSUR,!,SRTN,?12,VA("PID"),?50,SRDIAG,?112,SRFST,! W:$P($G(^SRF(SRTN,30)),"^") "ABORTED" W ?12,SRATC,?50,SROP(1),?112,SRTWO
 I $D(SROPS(2)) W !,?40,SROPS(2) I $D(SROPS(3)) W !,?40,SROPS(3) I $D(SROP(4)) W !,?40,SROP(4)
 Q
