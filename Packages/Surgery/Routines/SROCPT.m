SROCPT ;BIR/MAM,ADM - PRINT DESCRIPTION OF CPT CODE ON LOOKUP ; [ 05/14/99  11:28 AM ]
 ;;3.0; Surgery ;**3,31,88**;24 Jun 93
1 N SRCODE,SRDES,SRI,SRX S SRCODE=Y,SRX=$$CPTD^ICPTCOD(SRCODE,"SRDES")
 F SRI=1:1:SRX D EN^DDIOL(SRDES(SRI),"","!,?8")
 Q
DISPLAY ; output principal CPT
 ...
 .S SRZ=$P(^SRF(SRDA,"OPMOD",SRI,0),"^"),SRY=$$MOD^ICPTMOD(SRZ,"I"),SRX=SRX_$P(SRY,"^",2)_" "_$E($P(SRY,"^",3),1,57) D EN^DDIOL(SRX,"","!,?7") S SRX="           -"
 Q
OTHDISP ; output other procedure CPT
 ...
 .S SRZ=$P(^SRF(SRDA(1),13,SRDA,"MOD",SRI,0),"^"),SRY=$$MOD^ICPTMOD(SRZ,"I"),SRX=SRX_$P(SRY,"^",2)_" "_$E($P(SRY,"^",3),1,57) D EN^DDIOL(SRX,"","!,?7") S SRX="           -"
 Q
DES ; get short name and description
 N X,Z S (SRCODE,SRK)=Y,SRY=$$CPT^ICPTCOD(SRCODE),SRW=$P(SRY,"^",2)_"  "_$P(SRY,"^",3)
 S SRY=$$CPTD^ICPTCOD(SRCODE,"SRDES"),SRK=SRK_" " F SRI=1:1:SRY D  Q:$L(SRK_" "_X)>245  S SRK=SRK_" "_X
...
ACTIV(SRTN,SRCODE) ; screen for active CPT codes
 ...
 S SROK=$P($$CPT^ICPTCOD(SRCODE,SRSDATE),"^",7)
 Q SROK
IN ; check CPT input
