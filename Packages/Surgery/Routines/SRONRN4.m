SRONRN4 ;B'HAM ISC/MAM - NURSE INTRAOP REPORT (FORMAT 0) ; 30 OCT 91 11:35 AM
 ;;3.0; Surgery ;**18,22,25,48**;24 Jun 93
 I $O(^SRF(SRTN,21,0)) D UL W !,"Thermal Unit: " S (TH,CNT)=0 F  S TH=$O(^SRF(SRTN,21,TH)) Q:'TH!SRSOUT  S CNT=CNT+1 D THERM
 Q:SRSOUT  I $Y+14>IOSL D FOOT Q:SRSOUT  I $E(IOST)="P" D HDR Q:SRSOUT
 I $O(^SRF(SRTN,1,0)) D UL W !,"Prosthesis Installed: " S (PRO,CNT)=0 F  S PRO=$O(^SRF(SRTN,1,PRO)) Q:'PRO!(SRSOUT)  S CNT=CNT+1 D PRO
 Q:SRSOUT  K PRO,UNIT,MODEL,SERIAL,SIZE,CNT,X,TH,TEMP,ON,OFF,Y,I,STERILE,QTY,Z
 I $Y+16>IOSL D FOOT Q:SRSOUT  I $E(IOST)="P" D HDR Q:SRSOUT
 ...
 I $O(^SRF(SRTN,26,0)) D UL W !,"Irrigation Solution(s): " S IRR=0 F  S IRR=$O(^SRF(SRTN,26,IRR)) Q:'IRR!(SRSOUT)  S X=^SRF(SRTN,26,IRR,0),SOLU=$P(^SRO(133.6,X,0),"^") D IRR
