SRONRNF ;B'HAM ISC/MAM - NURSE INTRAOP REPORT (FORMAT 0) ; 21 OCT 91  11:00 AM
 ...
 D UL K SRSUR S X=$P(SR(.1),"^",4) I X S SRSUR=$P(^VA(200,X,0),"^")
 S:'$D(SRSUR) SRSUR="NOT ENTERED" W !,"Surgeon: ",?17,SRSUR K SRSUR
 S X=$P(SR(.1),"^",5) I X S SRSUR=$P(^VA(200,X,0),"^")
 S:'$D(SRSUR) SRSUR="N/A" W ?40,"First Assistant:",?59,SRSUR K SRSUR
 S X=$P(SR(.1),"^",13) I X S SRSUR=$P(^VA(200,X,0),"^")
 S:'$D(SRSUR) SRSUR="NOT ENTERED" W !,"Attending Surg:",?17,SRSUR K SRSUR
 S X=$P(SR(.1),"^",6) I X S SRSUR=$P(^VA(200,X,0),"^")
 S:'$D(SRSUR) SRSUR="N/A" W ?40,"Second Assistant:",?59,SRSUR
 K SRANES S X=$P(SR(.3),"^") I X S SRANES=$P(^VA(200,X,0),"^")
 K SRANESA S X=$P(SR(.3),"^",3) I X S SRANESA=$P(^VA(200,X,0),"^")
 I $D(SRANES)!$D(SRANESA) S:'$D(SRANESA) SRANESA="N/A" S:'$D(SRANES) SRANES="NOT ENTERED" W !,"Anesthetist: ",?17,SRANES,?40,"Assistant Anesth:",?59,SRANESA K SRANES
 I $O(^SRF(SRTN,28,0)) D UL W !,"Other Scrubbed Assistants: " K OTHER S CNT=0 S SHEMP=0 F  S SHEMP=$O(^SRF(SRTN,28,SHEMP)) Q:'SHEMP!SRSOUT  S CNT=CNT+1 D OTHER
 G:SRSOUT END I $Y+13>IOSL D FOOT G:SRSOUT END I $E(IOST)="P" D HDR G:SRSOUT END
 D ^SRONRN1 K SR(0) I SRSOUT G END
 D FOOT S SRSOUT=1
