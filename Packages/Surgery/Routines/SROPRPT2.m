SROPRPT2 ;B'HAM ISC/MAM - OPERATION REPORT (FORMAT 1) ; [ 12/07/98  8:16 AM ]
 ...
 Q:SRSOUT  K NONUR,NURSE
 I $Y+13>IOSL D FOOT Q:SRSOUT  I $E(IOST)="P" D HDR Q:SRSOUT
 D UL W !,"Anesthesia Technique(s):" S ANE=0 F  S ANE=$O(^SRF(SRTN,6,ANE)) Q:'ANE!SRSOUT  D ANE
 I '$O(^SRF(SRTN,6,0)) W " N/A"
 K X,SET,ANE,AGNT,CNT,DRUG,SRSP,SRAN,OTHER,OTH,OPS
 I $Y+13>IOSL D FOOT Q:SRSOUT  I $E(IOST)="P" D HDR Q:SRSOUT
 D UL K SRTIME S Y=$P(SR(.2),"^") I Y D D^DIQ S SRTIME=$P(Y,"@")_"  "_$P(Y,"@",2)
 W !,"Anesthesia Begin: "_$S($D(SRTIME):SRTIME,1:"N/A")
 K SRTIME S Y=$P(SR(.2),"^",4) I Y D D^DIQ S SRTIME=$P(Y,"@")_"  "_$P(Y,"@",2)
 W ?40,"Anesthesia End: "_$S($D(SRTIME):SRTIME,1:"N/A")
 K SRTIME S Y=$P(SR(.2),"^",2) I Y D D^DIQ S SRTIME=$P(Y,"@")_"  "_$P(Y,"@",2)
 W !,"Operation Begin:  "_$S($D(SRTIME):SRTIME,1:"N/A")
 K SRTIME S Y=$P(SR(.2),"^",3) I Y D D^DIQ S SRTIME=$P(Y,"@")_"  "_$P(Y,"@",2)
 W ?40,"Operation End:  "_$S($D(SRTIME):SRTIME,1:"N/A")
 I $Y+13>IOSL D FOOT Q:SRSOUT  I $E(IOST)="P" D HDR Q:SRSOUT
 D ^SROPRPT3
 Q
NURSE ; nurse info
...
ANE ; print anesthesia technique
 Q:SRSOUT  S A=^SRF(SRTN,6,ANE,0),Y=$P(A,"^"),C=$P(^DD(130.06,.01,0),"^",2) D:Y'="" Y^DIQ W !,?2,Y W:$P(A,"^",3)="Y" "  (PRINCIPAL)" D AGENT
 Q
AGENT ; print agents
 Q:SRSOUT!($P(A,"^")="N")  W !,"     Agent: " I '$O(^SRF(SRTN,6,ANE,1,0)) W "NONE ENTERED" Q
