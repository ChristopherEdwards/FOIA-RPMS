...
LIST ; list existing procedures
 ...
 W !,$S(CNT<10:" ",1:"")_CNT_". "_$P(OTHER(CNT),"^")_" (CPT: "_$P(OTHER(CNT),"^",2)_")"
 Q
...
EDIT ; edit one procedure
 D HDR W ! S DA=SROTHER,DIE="^SRF("_SRTN_",13,",DA(1)=SRTN,DR=".01T;3T" D ^DIE K DR,DIE
 Q
...
NEW D HDR W ! K DIR,DA S DIR(0)="130.16,.01",DIR("A")="Other Operative Procedure" D ^DIR I Y=""!$D(DTOUT)!$D(DUOUT) Q
 ...
 K DR,DIE S DA=+Y,DA(1)=SRTN,DR="3T",DIE="^SRF("_SRTN_",13," D ^DIE K DR
 Q
