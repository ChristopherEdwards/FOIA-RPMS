...
SET ; set variables & print info
 ...
 S SROP(1)=$P(^SRF(SRTN,"OP"),"^"),X=$P(^("OP"),"^",2),CPT=$S(X:$P($$CPT^ICPTCOD(X),"^",2),1:"MISSING") I X S Y=CPT D SSPRIN^SROCPT S CPT=Y
 S SROP(1)=SROP(1)_" (CPT: "_CPT_")"
 S CNT=1,OP=0 F  S OP=$O(^SRF(SRTN,13,OP)) Q:'OP  D
 .S CNT=CNT+1,SROP(CNT)=$P(^SRF(SRTN,13,OP,0),"^"),X=$P($G(^SRF(SRTN,13,OP,2)),"^"),CPT=$S(X:$P($$CPT^ICPTCOD(X),"^",2),1:"MISSING")
 .I X S Y=CPT,SRDA=OP D SSOTH^SROCPT S CPT=Y
 .S SROP(CNT)=SROP(CNT)_" (CPT: "_CPT_")"
