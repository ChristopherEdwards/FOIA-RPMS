AUM101 ;IHS/SD/DMJ,SDR - ICD 9 CODES FOR FY 2010 ; [ 08/18/2003  11:02 AM ]
 ;;10.2;TABLE MAINTENANCE;;MAR 09, 2010
START ;EP
 I $$VERSION^XPDUTL("BCSV")>0 D ^AUM91RL,^AUM101RL Q  ;if BCSV loaded, reload 2009 ICD codes, 2010 ICD codes
SVARS ;;A,C,E,F,L,M,N,O,P,R,S,T,V;Single-character work variables.
 NEW DA,DIC,DIE,DINUM,DLAYGO,DR,@($P($T(SVARS),";",3))
 S U="^"
 D RSLT("Beginning FY 2010 ICD Update.")
 D DASH,ICD9NEW
 D DASH,ICD9REV
 D DASH,ICD9INAC
 D DASH,ICD0NEW
...
DIE ;EP
 ...
 Q
E(L) I L="ICD9NEW" Q $P($P($T(@L^AUM101A),";",3),":",1)
 I L="ICD9VNEW" Q $P($P($T(@L^AUM101E),";",3),":",1)
 I L="ICD9ENEW" Q $P($P($T(@L^AUM101B),";",3),":",1)
 I L="ICD9REV" Q $P($P($T(@L^AUM101C),";",3),":",1)
 I L="ICD9DINA" Q $P($P($T(@L^AUM101D),";",3),":",1)
 I L="ICD9EINA" Q $P($P($T(@L^AUM101D),";",3),":",1)
 I L="ICD9PNEW" Q $P($P($T(@L^AUM101D),";",3),":",1)
 I L="ICD9PREV" Q $P($P($T(@L^AUM101E),";",3),":",1)
 I L="ICD9PINA" Q $P($P($T(@L^AUM101E),";",3),":",1)
 I L="ICD9OINA" Q $P($P($T(@L^AUM101D),";",3),":",1)
 I L="ICD0OREV" Q $P($P($T(@L^AUM101C),";",3),":",1)
 I L="ICD9OREV" Q $P($P($T(@L^AUM101C),";",3),":",1)
...
ICD9INAC ;
 ...
 F AUMI=1:1 S AUMLN=$P($T(ICD9EINA+AUMI^AUM101D),";;",2) Q:AUMLN="END"  D
...
ICD0REV ;
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9PREV+AUMI^AUM101E),";;",2) Q:AUMLN="END"  D
 .S AUMLN=$TR(AUMLN,"abcdefghijklmnopqrstuvwxyz","ABCDEFGHIJKLMNOPQRSTUVWXYZ")
 .S Y=$$IXDIC("^ICD0(","ILX","AB",$P(AUMLN,U))
 .I Y=-1 D RSLT("ERROR:  Lookup/Add of CODE '"_$P(AUMLN,U)_"' FAILED.") Q
 .S DA=+Y
