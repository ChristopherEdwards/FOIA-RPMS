AUM13EN ;IHS/OIT/NKD - ENVIRONMENT CHECK/PRE/POST-INSTALL FOR ICD 9 CODES 08/30/12 ; 
...
VCHK(AUMPRE,AUMVER,AUMQUIT) ; Check versions needed.
 ...
 I AUMV<AUMVER S XPDQUIT=AUMQUIT W *7,!,$$CJ^XLFSTR("^^^^**NEEDS FIXED**^^^^",IOM) Q 0
 Q 1
 ;
VCHK2(AUMPRE,AUMVER,AUMQUIT) ; Check patch level
 NEW AUMV
 S AUMV=$$VERSION^XPDUTL(AUMPRE)
 I $L(AUMV)<1 S XPDQUIT=AUMQUIT W !,$$CJ^XLFSTR("Need at least "_AUMPRE_" v "_AUMVER_"....."_AUMPRE_" Not Installed",IOM),*7,!,$$CJ^XLFSTR("^^^^**NEEDS TO BE FIXED**^^^^",IOM) Q 1
 ...
 I (AUMV=$P(AUMVER,".",1,2))&(PTCH<$P(AUMVER,".",3)) KILL DIFQ S XPDQUIT=AUMQUIT W *7,!,$$CJ^XLFSTR("^^^^**NEEDS TO BE FIXED**^^^^",IOM)
 Q 1
