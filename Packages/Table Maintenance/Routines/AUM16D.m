AUM16D ;IHS/OIT/NKD - ICD10 UPDATE FY2016 - 10/16/2015 ;
...
POST ; EP - MAIN EP FR KIDS
 ...
 . F CNT=1:1 S TMP=$P($T(@(TAG)+CNT^AUM16M),";;",2) Q:TMP="END"  S @LOG@("M",TMP)=$P($T(@(TAG)+CNT^AUM16M),";;",3),@LOG@("M",TMP,$P($T(@(TAG)+CNT^AUM16M),";;",4))=""
 . F CNT=1:1 S TMP=$P($T(@(TAG_"D")+CNT^AUM16M),";;",2) Q:TMP="END"  S @LOG@($E(TMP))=$P($T(@(TAG_"D")+CNT^AUM16M),";;",3)
...
NEW ; GENERIC NEW ENTRY DRIVER
 ...
 S NEWIEN(1)=$$IEN^AUM16U($$SYS(AUMT,1))  ; GET NEXT AVAILABLE IEN BASED ON SYS
 ;
...
INACT ; GENERIC INACTIVATE DRIVER
 ...
 . ; DON'T NULL SHORT/LONG FIELDS
 . Q:'$D(@AUMV)  Q:"P3^P4"[AUMV
