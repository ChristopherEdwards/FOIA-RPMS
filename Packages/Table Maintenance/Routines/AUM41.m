AUM41 ;IHS/ASDST/DMJ,SDR,GTH - ICD 9 CODES FOR FY 2004 ; [ 09/09/2003  11:47 AM ]
...
SVARS ;;A,C,E,F,L,M,N,O,P,R,S,T,V;Single-character work variables.
 ...
 NEW DA,DIC,DIE,DINUM,DLAYGO,DR,A,C,E,F,L,M,N,O,P,R,S,T,V
 S U="^"
 ;
 D RSLT("Beginning FY 2004 ICD Update.")
...
DIE ;EP
 ...
 Q
E(L) I L="ICD9NEW" Q $P($P($T(@L^AUM41A),";",3),":",1)
 I L="ICD9VNEW" Q $P($P($T(@L^AUM41C),";",3),":",1)
 I L="ICD9ENEW" Q $P($P($T(@L^AUM41B),";",3),":",1)
 I L="ICD9REV" Q $P($P($T(@L^AUM41C),";",3),":",1)
 I L="ICD9DINA" Q $P($P($T(@L^AUM41D),";",3),":",1)
 I L="ICD9PNEW" Q $P($P($T(@L^AUM41D),";",3),":",1)
 I L="ICD9PREV" Q $P($P($T(@L^AUM41B),";",3),":",1)
 I L="ICD9PINA" Q $P($P($T(@L^AUM41C),";",3),":",1)
...
ICD9NEW ;
 ...
 F AUMI=1:1 S AUMLN=$P($T(ICD9NEW+AUMI^AUM41A),";;",2) Q:AUMLN="END"  D
 ...
 . S DR=DR_";9999999.04///3031001"       ;date added
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9VNEW+AUMI^AUM41C),";;",2) Q:AUMLN="END"  D
 . S Y=$$IXDIC("^ICD9(","ILX","AB",$P(AUMLN,U),80)
 . I Y=-1 D RSLT("ERROR:  Lookup/Add of CODE '"_$P(AUMLN,U)_"' FAILED.") Q
 . S DA=+Y
 . S DR="3///"_$P(AUMLN,U,2)                   ;diagnosis
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9ENEW+AUMI^AUM41B),";;",2) Q:AUMLN="END"  D
 . S Y=$$IXDIC("^ICD9(","ILX","AB",$P(AUMLN,U),80)
 . I Y=-1 D RSLT("ERROR:  Lookup/Add of CODE '"_$P(AUMLN,U)_"' FAILED.") Q
 . S DA=+Y
 . S DR="3///"_$P(AUMLN,U,2)                   ;diagnosis
...
ICD9INAC ;
 ...
 F AUMI=1:1 S X=$P($T(ICD9DINA+AUMI^AUM41D),";;",2) Q:X="END"  D
...
ICD9REV ;
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9REV+AUMI^AUM41C),";;",2) Q:AUMLN="END"  D
 . S Y=$$IXDIC("^ICD9(","ILX","AB",$P(AUMLN,U),80)
 . I Y=-1 D RSLT("ERROR:  Lookup/Add of CODE '"_$P(AUMLN,U)_"' FAILED.") Q
 . S DA=+Y
 . S DR="3///"_$P(AUMLN,U,2)                   ;diagnosis
...
ICD0NEW ;
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9PNEW+AUMI^AUM41D),";;",2) Q:AUMLN="END"  D
...
ICD0REV ;
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9PREV+AUMI^AUM41B),";;",2) Q:AUMLN="END"  D
 . S Y=$$IXDIC("^ICD0(","ILX","AB",$P(AUMLN,U))
 . I Y=-1 D RSLT("ERROR:  Lookup/Add of CODE '"_$P(AUMLN,U)_"' FAILED.") Q
 . S DA=+Y
 . S DR="4///"_$P(AUMLN,U,2)                   ;operation/procedure
...
ICD0INAC ;
 ...
 D RSLT($J("",8)_"----     -----------")
 NEW AUMI,DA,DIE,DR,X
 F AUMI=1:1 S X=$P($T(ICD9PINA+AUMI^AUM41C),";;",2) Q:X="END"  D
