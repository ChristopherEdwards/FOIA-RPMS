AUM61 ;IHS/ASDST/DMJ,SDR - ICD 9 CODES FOR FY 2006 ; [ 08/18/2003  11:02 AM ]
...
SVARS ;;A,C,E,F,L,M,N,O,P,R,S,T,V;Single-character work variables.
 ...
 S U="^"
 ;
 D RSLT("Beginning FY 2006 ICD Update.")
 D DRGS^AUM61E  ;update DRGs
 D DASH,ICD9NEW
 D DASH,ICD9INAC
 D DASH,ICD9REV
 D DASH,ICD0NEW
...
DIE ;EP
 ...
 Q
E(L) I L="ICD9NEW" Q $P($P($T(@L^AUM61A),";",3),":",1)
 I L="ICD9VNEW" Q $P($P($T(@L^AUM61E),";",3),":",1)
 I L="ICD9ENEW" Q $P($P($T(@L^AUM61B),";",3),":",1)
 I L="ICD9REV" Q $P($P($T(@L^AUM61C),";",3),":",1)
 I L="ICD9DINA" Q $P($P($T(@L^AUM61D),";",3),":",1)
 I L="ICD9PNEW" Q $P($P($T(@L^AUM61D),";",3),":",1)
 I L="ICD9PREV" Q $P($P($T(@L^AUM61B),";",3),":",1)
 I L="ICD9PINA" Q $P($P($T(@L^AUM61E),";",3),":",1)
 I L="ICD9OINA" Q $P($P($T(@L^AUM61D),";",3),":",1)
...
ICD9NEW ;
 ...
 F AUMI=1:1 S AUMLN=$P($T(ICD9NEW+AUMI^AUM61A),";;",2) Q:AUMLN="END"  D
 ...
 . S DR=DR_";9999999.04///3051001"       ;date added
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9VNEW+AUMI^AUM61E),";;",2) Q:AUMLN="END"  D
 . S AUMLN=$TR(AUMLN,"abcdefghijklmnopqrstuvwxyz","ABCDEFGHIJKLMNOPQRSTUVWXYZ")
 . S Y=$$IXDIC("^ICD9(","ILX","AB",$P(AUMLN,U),80)
 . I Y=-1 D RSLT("ERROR:  Lookup/Add of CODE '"_$P(AUMLN,U)_"' FAILED.") Q
 . S DA=+Y
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9ENEW+AUMI^AUM61B),";;",2) Q:AUMLN="END"  D
 . S AUMLN=$TR(AUMLN,"abcdefghijklmnopqrstuvwxyz","ABCDEFGHIJKLMNOPQRSTUVWXYZ")
 . S Y=$$IXDIC("^ICD9(","ILX","AB",$P(AUMLN,U),80)
 . I Y=-1 D RSLT("ERROR:  Lookup/Add of CODE '"_$P(AUMLN,U)_"' FAILED.") Q
 . S DA=+Y
...
ICD9INAC ;
 ...
 F AUMI=1:1 S X=$P($T(ICD9DINA+AUMI^AUM61D),";;",2) Q:X="END"  D
...
ICD9OINA ;
 ...
 D RSLT($J("",8)_"----     -----------")
 NEW AUMI,DA,DIE,DR,X
 F AUMI=1:1 S X=$P($T(ICD9OINA+AUMI^AUM61D),";;",2) Q:X="END"  D
 . S Y=$$IXDIC("^ICD9(","ILX","AB",$P(X,U))
 . I Y=-1 D RSLT(" CODE '"_X_"' not found (that's OK).") Q
 . S DA=+Y,AUMDA=+Y
 . S DIE="^ICD9("
...
ICD9REV ;
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9REV+AUMI^AUM61C),";;",2) Q:AUMLN="END"  D PROCESS
 F AUMI=1:1 S AUMLN=$P($T(ICD9REV2+AUMI^AUM61E),";;",2) Q:AUMLN="END"  D PROCESS
 Q
 ;
...
ICD0NEW ;
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9PNEW+AUMI^AUM61D),";;",2) Q:AUMLN="END"  D
 . S AUMLN=$TR(AUMLN,"abcdefghijklmnopqrstuvwxyz","ABCDEFGHIJKLMNOPQRSTUVWXYZ")
...
ICD0REV ;
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9PREV+AUMI^AUM61B),";;",2) Q:AUMLN="END"  D
 . S AUMLN=$TR(AUMLN,"abcdefghijklmnopqrstuvwxyz","ABCDEFGHIJKLMNOPQRSTUVWXYZ")
 . S Y=$$IXDIC("^ICD0(","ILX","AB",$P(AUMLN,U))
 . I Y=-1 D RSLT("ERROR:  Lookup/Add of CODE '"_$P(AUMLN,U)_"' FAILED.") Q
 . S DA=+Y
...
ICD0INAC ;
 ...
 D RSLT($J("",8)_"----     -----------")
 NEW AUMI,DA,DIE,DR,X
 F AUMI=1:1 S X=$P($T(ICD9PINA+AUMI^AUM61E),";;",2) Q:X="END"  D
