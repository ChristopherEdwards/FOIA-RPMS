AUM71 ;IHS/ASDST/DMJ,SDR - ICD 9 CODES FOR FY 2007 ; [ 08/18/2003  11:02 AM ]
...
SVARS ;;A,C,E,F,L,M,N,O,P,R,S,T,V;Single-character work variables.
 ...
 S U="^"
 ;
 D RSLT("Beginning FY 2007 ICD Update.")
 D DRGS^AUM71E  ;update DRGs
 D DASH,ICD9NEW
 D DASH,ICD9INAC
 D DASH,ICD9REV
 D DASH,ICD0NEW
 D DASH,ICD0REV
 D DASH,ICD0INAC
 D DASH,ICD9OINA  ;codes being inactivated other than from Fed. Registry
 D DASH,ICD0OREV  ;codes being edited othen than from Fed. Registry
 D DASH,ICD9OREV,DASH  ;codes being edited other than from Fed. Registry
 D RSLT("End FY 2007 ICD Update.")
 Q
 ; -----------------------------------------------------
...
DIE ;EP
 ...
 Q
E(L) I L="ICD9NEW" Q $P($P($T(@L^AUM71A),";",3),":",1)
 I L="ICD9VNEW" Q $P($P($T(@L^AUM71E),";",3),":",1)
 I L="ICD9ENEW" Q $P($P($T(@L^AUM71B),";",3),":",1)
 I L="ICD9REV" Q $P($P($T(@L^AUM71C),";",3),":",1)
 I L="ICD9DINA" Q $P($P($T(@L^AUM71D),";",3),":",1)
 I L="ICD9PNEW" Q $P($P($T(@L^AUM71D),";",3),":",1)
 I L="ICD9PREV" Q $P($P($T(@L^AUM71B),";",3),":",1)
 I L="ICD9PINA" Q $P($P($T(@L^AUM71E),";",3),":",1)
 I L="ICD9OINA" Q $P($P($T(@L^AUM71D),";",3),":",1)
 I L="ICD0OREV" Q $P($P($T(@L^AUM71C),";",3),":",1)
 I L="ICD9OREV" Q $P($P($T(@L^AUM71C),";",3),":",1)
...
ICD9NEW ;
 ...
 F AUMI=1:1 S AUMLN=$P($T(ICD9NEW+AUMI^AUM71A),";;",2) Q:AUMLN="END"  D ICD9NPRC
 F AUMI=1:1 S AUMLN=$P($T(ICD9NEW2+AUMI^AUM71E),";;",2) Q:AUMLN="END"  D ICD9NPRC
 D NEWVCODS
 Q
ICD9NPRC ;
 ...
 S DR=DR_";9999999.04///3061001"       ;date added
...
NEWVCODS ;  loads NEW V-CODES
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9VNEW+AUMI^AUM71E),";;",2) Q:AUMLN="END"  D
 . S AUMLN=$TR(AUMLN,"abcdefghijklmnopqrstuvwxyz","ABCDEFGHIJKLMNOPQRSTUVWXYZ")
 . S Y=$$IXDIC("^ICD9(","ILX","AB",$P(AUMLN,U),80)
 . I Y=-1 D RSLT("ERROR:  Lookup/Add of CODE '"_$P(AUMLN,U)_"' FAILED.") Q
 . S DA=+Y
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9ENEW+AUMI^AUM71B),";;",2) Q:AUMLN="END"  D
 . S AUMLN=$TR(AUMLN,"abcdefghijklmnopqrstuvwxyz","ABCDEFGHIJKLMNOPQRSTUVWXYZ")
 . S Y=$$IXDIC("^ICD9(","ILX","AB",$P(AUMLN,U),80)
 . I Y=-1 D RSLT("ERROR:  Lookup/Add of CODE '"_$P(AUMLN,U)_"' FAILED.") Q
 . S DA=+Y
...
ICD9INAC ;
 ...
 F AUMI=1:1 S X=$P($T(ICD9DINA+AUMI^AUM71D),";;",2) Q:X="END"  D
...
ICD9OINA ;
 ...
 D RSLT($J("",8)_"----     -----------")
 NEW AUMI,DA,DIE,DR,X
 F AUMI=1:1 S X=$P($T(ICD9OINA+AUMI^AUM71D),";;",2) Q:X="END"  D
 . S Y=$$IXDIC("^ICD9(","ILX","AB",$P(X,U))
 . I Y=-1 D RSLT(" CODE '"_X_"' not found (that's OK).") Q
 . S DA=+Y,AUMDA=+Y
 . S DIE="^ICD9("
...
ICD9REV ;
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9REV+AUMI^AUM71C),";;",2) Q:AUMLN="END"  D PROCESS
 Q
 ;
...
ICD0NEW ;
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9PNEW+AUMI^AUM71D),";;",2) Q:AUMLN="END"  D
 . S AUMLN=$TR(AUMLN,"abcdefghijklmnopqrstuvwxyz","ABCDEFGHIJKLMNOPQRSTUVWXYZ")
...
ICD0REV ;
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9PREV+AUMI^AUM71B),";;",2) Q:AUMLN="END"  D
 . S AUMLN=$TR(AUMLN,"abcdefghijklmnopqrstuvwxyz","ABCDEFGHIJKLMNOPQRSTUVWXYZ")
 . S Y=$$IXDIC("^ICD0(","ILX","AB",$P(AUMLN,U))
 . I Y=-1 D RSLT("ERROR:  Lookup/Add of CODE '"_$P(AUMLN,U)_"' FAILED.") Q
 . S DA=+Y
...
ICD0INAC ;
 ...
 D RSLT($J("",8)_"----     -----------")
 NEW AUMI,DA,DIE,DR,X
 F AUMI=1:1 S X=$P($T(ICD9PINA+AUMI^AUM71E),";;",2) Q:X="END"  D
...
ICD9OREV ;
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD9OREV+AUMI^AUM71C),";;",2) Q:AUMLN="END"  D
 .S AUMLN=$TR(AUMLN,"abcdefghijklmnopqrstuvwxyz","ABCDEFGHIJKLMNOPQRSTUVWXYZ")
 .S Y=$$IXDIC("^ICD9(","ILX","AB",$P(AUMLN,U),80)
 .I Y=-1 D RSLT("ERROR:  Lookup/Add of CODE '"_$P(AUMLN,U)_"' FAILED.") Q
 .S DA=+Y
...
ICD0OREV ;
 ...
 D RSLT($J("",8)_"----      -----------")
 NEW AUMDA,AUMI,AUMLN,DA,DIE,DR
 F AUMI=1:1 S AUMLN=$P($T(ICD0OREV+AUMI^AUM71C),";;",2) Q:AUMLN="END"  D
 .S AUMLN=$TR(AUMLN,"abcdefghijklmnopqrstuvwxyz","ABCDEFGHIJKLMNOPQRSTUVWXYZ")
 .S Y=$$IXDIC("^ICD0(","ILX","AB",$P(AUMLN,U))
 .I Y=-1 D RSLT("ERROR:  Lookup/Add of CODE '"_$P(AUMLN,U)_"' FAILED.") Q
 .S DA=+Y
