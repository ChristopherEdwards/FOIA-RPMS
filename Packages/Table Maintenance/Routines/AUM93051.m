AUM93051 ; DSM/GTH - STANDARD TABLE UPDATES (1), 7MAY93 MEMO ; [ 05/11/93  10:17 AM ]
...
IEN(X,%,Y) ;
 S Y=$O(@(X_"""C"",%,0)"))
 I 'Y S Y=$T(@%^AUM9305M) I Y NEW Z S Z=E D  S:Y<0 Y="" S E=Z
...
AREANEW ;
 S E="New Area Codes"
 F T=1:1 S L=$T(AREANEW+T^AUM9305A) Q:$P(L,";",3)="END"  I $P(L,U,$L(L,U))="Y" D ADDAREA
 Q
 ;
ADDAREA ;
...
SUNEW ;
 ...
 F T=1:1 S L=$T(SUNEW+T^AUM9305A) Q:$P(L,";",3)="END"  I $P(L,U,$L(L,U))="Y" D ADDSU
 Q
 ;
ADDSU ;
...
LOCNEW ;
 ...
 F T=1:1 S L=$T(LOCNEW+T^AUM9305A) Q:$P(L,";",3)="END"  I $P(L,U,$L(L,U)) D ADDLOC
 Q
 ;
ADDLOC ;
...
LOCMOD ;
 ...
 F T=1:2 S L=$T(LOCMOD+T^AUM9305A) Q:$P(L,";",3)="END"  S L("TO")=$T(LOCMOD+T+1^AUM9305A) I $P(L("TO"),U,$L(L("TO"),U))="Y" D
...
CNTYMOD ;
 ...
 F T=1:2 S L=$T(CNTYMOD+T^AUM9305A) Q:$P(L,";",3)="END"  S L("TO")=$T(CNTYMOD+T+1^AUM9305A) I $P(L("TO"),U,$L(L("TO"),U)) D
