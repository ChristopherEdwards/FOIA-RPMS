AUM93101 ; DSD/GTH - STANDARD TABLE UPDATES (1), 22OCT93 BANYAN ; [ 09/08/93  4:01 PM ]
...
IEN(X,%,Y) ;EP
 S Y=$O(@(X_"""C"",%,0)"))
 I 'Y S Y=$T(@%^AUM9310M) I Y NEW Z S Z=E D  S:Y<0 Y="" S E=Z
...
AREANEW ;
 S E="New Area Codes"
 F T=1:1 S L=$T(AREANEW+T^AUM9310A) Q:$P(L,";",3)="END"  I $P(L,U,$L(L,U))="Y" D ADDAREA
 Q
 ;
ADDAREA ;
...
SUNEW ;
 ...
 F T=1:1 S L=$T(SUNEW+T^AUM9310A) Q:$P(L,";",3)="END"  I $P(L,U,$L(L,U))="Y" D ADDSU
 Q
 ;
ADDSU ;
...
LOCNEW ;
 ...
 F T=1:1 S L=$T(LOCNEW+T^AUM9310A) Q:$P(L,";",3)="END"  I $P(L,U,$L(L,U))="Y" D ADDLOC
 Q
 ;
ADDLOC ;
 S L=$P(L,";;",2),A=$P(L,U),S=$P(L,U,2),F=$P(L,U,3),N=$P(L,U,4),L=A_"  "_S_"  "_F_"  "_N,%=A_S_F,%=$O(^AUTTLOC("C",%,0))
 I % D RSLT(E(1)_E_" : ASUFAC EXISTS => "_L) D  Q
 ...
 . ; if name field does not match, edit name field
 .S %=$O(^AUTTLOC("C",A_S_F,0)),%=$P(^AUTTLOC(%,0),U)
 .Q:'%  Q:'$D(^DIC(4,%,0))  Q:N=$P(^DIC(4,%,0),U)
 .S DIE="^DIC(4,",DA=%,DR=".01///"_N D DIE D:$D(Y) RSLT(E(0)_E_" : EDIT INSTITUTION FAILED => "_L) D:'$D(Y) RSLT(E_" : INSTITUTION NAME UPDATED => "_L)
 .Q
...
LOCMOD ;
 ...
 F T=1:2 S L=$T(LOCMOD+T^AUM9310A) Q:$P(L,";",3)="END"  S L("TO")=$T(LOCMOD+T+1^AUM9310A) I $P(L("TO"),U,$L(L("TO"),U))="Y" D
...
CNTYMOD ;
 ...
 F T=1:2 S L=$T(CNTYMOD+T^AUM9310A) Q:$P(L,";",3)="END"  S L("TO")=$T(CNTYMOD+T+1^AUM9310A) I $P(L("TO"),U,$L(L("TO"),U))="Y" D
