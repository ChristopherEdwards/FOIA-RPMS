AUM93122 ; DSD/GTH - STANDARD TABLE UPDATES (2), 23DEC93 BANYAN ; [ 12/23/93  3:00 PM ]
...
COMMNEW ;
 ...
 F T=1:1 S L=$T(COMMNEW+T^AUM9312A) Q:$P(L,";",3)="END"  I $P(L,U,$L(L,U))="Y" D ADDCOMM
 Q
 ;
ADDCOMM ;
 ...
 S P("O")=$$IEN^AUM93121("^AUTTCTY(",S_O) Q:'P("O")
 S P("A")=$$IEN^AUM93121("^AUTTAREA(",A) Q:'P("A")
 S P("V")=$$IEN^AUM93121("^AUTTSU(",A_V) Q:'P("V")
...
COMMMOD ;
 ...
 F T=1:2 S L=$T(COMMMOD+T^AUM9312B) Q:$P(L,";",3)="END"  S L("TO")=$T(COMMMOD+T+1^AUM9312B) I $P(L("TO"),U,$L(L("TO"),U))="Y" D
...
CLINNEW ;
 ...
 F T=1:1 S L=$T(CLINNEW+T^AUM9312A) Q:$P(L,";",3)="END"  I $P(L,U,$L(L,U))="Y" D ADDCLIN
 Q
 ;
ADDCLIN ;
...
CLINMOD ;
 ...
 ;
PKG ;
 ; This subroutine is for patch 1, to update the current version
 ...
 S DIE="^DIC(9.4,",DR="13///94.1" D DIE
 I $D(Y) D RSLT(E(0)_"COULD NOT UPDATE CURRENT VERSION IN PACKAGE FILE") I 1
 E  D RSLT("CURRENT VERSION of AUM updated TO 94.1.")
 S DA=$O(^DIC(9.4,"C","AUM",0))
 I '$D(^DIC(9.4,DA,22,0)) S ^(0)="^9.49I^^"
 Q:$O(^DIC(9.4,DA,22,"B",94.1,0))
 S X=94.1,DIC="^DIC(9.4,"_DA_",22,",DIC("DR")="1///DECEMBER 15, 1993",DA(1)=DA D FILE
 Q
 ;
