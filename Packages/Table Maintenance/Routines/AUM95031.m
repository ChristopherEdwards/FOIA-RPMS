AUM95031 ; DSD/GTH - STANDARD TABLE UPDATES (1), 03MAR95 BANYAN ; [ 03/06/95  3:18 PM ]
...
IEN(X,%,Y) ;EP
 S Y=$O(@(X_"""C"",%,0)"))
 I 'Y S Y=$T(@%^AUM9503M) I Y NEW Z S Z=E D  S:Y<0 Y="" S E=Z
...
AREANEW ;
 S E="New Area Codes"
 F T=1:1 S L=$T(AREANEW+T^AUM9503A) Q:$P(L,";",3)="END"  D ADDAREA
 Q
 ;
ADDAREA ;
...
SUNEW ;
 ...
 F T=1:1 S L=$T(SUNEW+T^AUM9503A) Q:$P(L,";",3)="END"  D ADDSU
 Q
 ;
ADDSU ;
...
LOCNEW ;
 ...
 F T=1:1 S L=$T(LOCNEW+T^AUM9503A) Q:$P(L,";",3)="END"  D ADDLOC
 Q
 ;
ADDLOC ;
...
LOCMOD ;
 ...
 F T=1:2 S L=$T(LOCMOD+T^AUM9503A) Q:$P(L,";",3)="END"  S L("TO")=$T(LOCMOD+T+1^AUM9503A) D
...
CNTYNEW ;
 ...
 F T=1:1 S L=$T(CNTYNEW+T^AUM9503A) Q:$P(L,";",3)="END"  D ADDCNTY
 Q
 ;
ADDCNTY ;
...
CNTYMOD ;
 ...
 F T=1:2 S L=$T(CNTYMOD+T^AUM9503A) Q:$P(L,";",3)="END"  S L("TO")=$T(CNTYMOD+T+1^AUM9503A) D
