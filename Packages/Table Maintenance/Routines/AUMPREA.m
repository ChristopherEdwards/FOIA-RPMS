AUMPREA ;IHS/VEN/TOAD - AUM v 10.1 pre-init ;
 ;;10.1;TABLE MAINTENANCE;**1**;OCT 16, 2009
 ;
 ; This is the pre-init for AUM*9.1*4. It strips all control chars
 ; out of the ^AUTTAREA global prior to the install of the patch.
 ;
...
CHECK ; troubleshooting entry point
 ;
 N AUMNAME S AUMNAME="^AUTTAREA" ; the name value of each node of ^AUTTAREA
 ...
 F AUMCNT=1:1 D  S AUMNAME=$Q(@AUMNAME) Q:AUMNAME=""  ; walk ^AUTTAREA
 . ;
 ...
 ;
 ;
CLEANALL ; AUM*10.1 PRE-INIT: Remove Control Characters from ^AUTTAREA
 ;
 ...
 N AUMCNTN S AUMCNTN=0 ; how many node names had control characters
 ;
 N AUMCNT ; count nodes, walk ^AUTTAREA
 ...
 . N AUMEMP S AUMEMP=AUMCLN ; change name from ^AUTTAREA(*)
 . ;S $E(AUMEMP,1,9)="^TMP(""AUM"","_$J_"," ; to ^TMP("AUM",$J,*)
 . S $E(AUMEMP,1,10)="^TMP(""AUM"","_$J_"," ; to ^TMP("AUM",$J,*)
 . ;IHS/OIT/CLS 09/17/2008 change to equal length of global root ^AUTTAREA(
 ...
 . N AUTTAREA S AUTTAREA=AUMNAME,$E(AUTTAREA,1,AUMLENG)="^AUTTAREA(" ; change back
 . K @(@AUMNAME@(U)) ; delete node in ^AUTTAREA whose bad name we saved off
 . N AUMVALU S AUMVALU=@AUMNAME ; get the saved, clean value
 . S @AUTTAREA=AUMVALU ; copy cleaned up node back into ^AUTTAREA
 . N AUMSUB S AUMSUB=$QS(AUMNAME,3) ; get the main subscript
 . K @AUMNAME@(U) ; delete the saved node name to avoid it
 . D MES^XPDUTL(AUTTAREA_"="_AUMVALU) ; report nodes as we copy them back
 K ^TMP("AUM",$J) ; clean up rest of temp space
 ;
 D BMES^XPDUTL(AUMCNT-1_$$T("MSG+1")) ; # nodes in ^AUTTAREA were scanned.
 ...
 ; Your ^AUTTAREA global is [now] free of control characters.
...
MSG ; messages to display
 ;; nodes in ^AUTTAREA were scanned.
 ;; instances of control characters were found and removed,
 ;; of them from node names, 
 ;; from values.
 ;;Your ^AUTTAREA global is 
 ;;free of control characters.
 ;;Replacing the bad node names found in ^AUTTAREA
 ;;Removing control characters from your ^AUTTAREA global...
 ;;AUM*10.1 PRE-INIT
 ;
 ;
