AUMPRECM ;IHS/VEN/TOAD - AUM v 10.1 pre-init ;
 ;;10.1;TABLE MAINTENANCE;**1**;OCT 16, 2009
 ;
 ; This is the pre-init for AUM*9.1*4. It strips all control chars
 ; out of the ^AUTTCOM global prior to the install of the patch.
 ;
...
CHECK ; troubleshooting entry point
 ;
 N AUMNAME S AUMNAME="^AUTTCOM" ; the name value of each node of ^AUTTCOM
 ...
 F AUMCNT=1:1 D  S AUMNAME=$Q(@AUMNAME) Q:AUMNAME=""  ; walk ^AUTTCOM
 . ;
 ...
 ;
 ;
CLEANALL ; AUM*10.1 PRE-INIT: Remove Control Characters from ^AUTTCOM
 ;
 ...
 N AUMCNTN S AUMCNTN=0 ; how many node names had control characters
 ;
 N AUMCNT ; count nodes, walk ^AUTTCOM
 ...
 . N AUMEMP S AUMEMP=AUMCLN ; change name from ^AUTTCOM(*)
 . ;S $E(AUMEMP,1,9)="^TMP(""AUM"","_$J_"," ; to ^TMP("AUM",$J,*)
 . S $E(AUMEMP,1,9)="^TMP(""AUM"","_$J_"," ; to ^TMP("AUM",$J,*)
 . ;IHS/OIT/CLS 09/17/2008 change to equal length of global root ^AUTTCOM(
 ...
 . N AUTTCOM S AUTTCOM=AUMNAME,$E(AUTTCOM,1,AUMLENG)="^AUTTCOM(" ; change back
 . K @(@AUMNAME@(U)) ; delete node in ^AUTTCOM whose bad name we saved off
 . N AUMVALU S AUMVALU=@AUMNAME ; get the saved, clean value
 . S @AUTTCOM=AUMVALU ; copy cleaned up node back into ^AUTTCOM
 . N AUMSUB S AUMSUB=$QS(AUMNAME,3) ; get the main subscript
 . K @AUMNAME@(U) ; delete the saved node name to avoid it
 . D MES^XPDUTL(AUTTCOM_"="_AUMVALU) ; report nodes as we copy them back
 K ^TMP("AUM",$J) ; clean up rest of temp space
 ;
 D BMES^XPDUTL(AUMCNT-1_$$T("MSG+1")) ; # nodes in ^AUTTCOM were scanned.
 ...
 ; Your ^AUTTCOM global is [now] free of control characters.
...
MSG ; messages to display
 ;; nodes in ^AUTTCOM were scanned.
 ;; instances of control characters were found and removed,
 ;; of them from node names, 
 ;; from values.
 ;;Your ^AUTTCOM global is 
 ;;free of control characters.
 ;;Replacing the bad node names found in ^AUTTCOM
 ;;Removing control characters from your ^AUTTCOM global...
 ;;AUM*10.1 PRE-INIT
 ;
 ;
