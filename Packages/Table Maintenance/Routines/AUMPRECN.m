...
CHECK ; troubleshooting entry point
 ;
 N AUMNAME S AUMNAME="^AUTTCTY" ; the name value of each node of ^AUTTCTY
 ...
 F AUMCNT=1:1 D  S AUMNAME=$Q(@AUMNAME) Q:AUMNAME=""  ; walk ^AUTTCTY
 . ;
 ...
 ;
 ;
CLEANALL ; AUM*10.1 PRE-INIT: Remove Control Characters from ^AUTTCTY
 ;
 ...
 N AUMCNTN S AUMCNTN=0 ; how many node names had control characters
 ;
 N AUMCNT ; count nodes, walk ^AUTTCTY
 ...
 . N AUMEMP S AUMEMP=AUMCLN ; change name from ^AUTTCTY(*)
 . ;S $E(AUMEMP,1,9)="^TMP(""AUM"","_$J_"," ; to ^TMP("AUM",$J,*)
 . S $E(AUMEMP,1,9)="^TMP(""AUM"","_$J_"," ; to ^TMP("AUM",$J,*)
 . ;IHS/OIT/CLS 09/17/2008 change to equal length of global root ^AUTTCTY(
 ...
 . N AUTTCTY S AUTTCTY=AUMNAME,$E(AUTTCTY,1,AUMLENG)="^AUTTCTY(" ; change back
 . K @(@AUMNAME@(U)) ; delete node in ^AUTTCTY whose bad name we saved off
 . N AUMVALU S AUMVALU=@AUMNAME ; get the saved, clean value
 . S @AUTTCTY=AUMVALU ; copy cleaned up node back into ^AUTTCTY
 . N AUMSUB S AUMSUB=$QS(AUMNAME,3) ; get the main subscript
 . K @AUMNAME@(U) ; delete the saved node name to avoid it
 . D MES^XPDUTL(AUTTCTY_"="_AUMVALU) ; report nodes as we copy them back
 K ^TMP("AUM",$J) ; clean up rest of temp space
 ;
 D BMES^XPDUTL(AUMCNT-1_$$T("MSG+1")) ; # nodes in ^AUTTCTY were scanned.
 ...
 ; Your ^AUTTCTY global is [now] free of control characters.
...
MSG ; messages to display
 ;; nodes in ^AUTTCTY were scanned.
 ;; instances of control characters were found and removed,
 ;; of them from node names, 
 ;; from values.
 ;;Your ^AUTTCTY global is 
 ;;free of control characters.
 ;;Replacing the bad node names found in ^AUTTCTY
 ;;Removing control characters from your ^AUTTCTY global...
 ;;AUM*10.1 PRE-INIT
 ;
 ;
