AUMPREST ;IHS/VEN/TOAD - AUM v 10.1 ;
 ;;10.1;TABLE MAINTENANCE;**1**;OCT 16, 2009
 ;
 ; This is the pre-init for AUM*9.1*2. It strips all control chars
 ; out of the ^DIC(5, global prior to the install of the patch.
 ;
...
CHECK ; troubleshooting entry point
 ;
 N AUMNAME S AUMNAME="^DIC(5,0)" ; the name value of each node of ^DIC(5,
 ...
 F AUMCNT=1:1 D  S AUMNAME=$Q(@AUMNAME) Q:AUMNAME=""  ; walk ^DIC(5,
 . ;
 ...
 ;
 ;
CLEANALL ; AUM*10.1 PRE-INIT: Remove Control Characters from ^DIC(5,
 ;
 ...
 N AUMCNTN S AUMCNTN=0 ; how many node names had control characters
 ;
 N AUMCNT ; count nodes, walk ^DIC(5,
 ...
 . N AUMEMP S AUMEMP=AUMCLN ; change name from ^DIC(5,(*)
 . ;S $E(AUMEMP,1,9)="^TMP(""AUM"","_$J_"," ; to ^TMP("AUM",$J,*)
 . S $E(AUMEMP,1,7)="^TMP(""AUM"","_$J_"," ; to ^TMP("AUM",$J,*)
 . ;IHS/OIT/CLS 09/17/2008 change to equal length of global root ^DIC(5,
 ...
 . N AUTTST S AUTTST=AUMNAME,$E(AUTTST,1,AUMLENG)="^DIC(5,(" ; change back
 . K @(@AUMNAME@(U)) ; delete node in ^DIC(5, whose bad name we saved off
 . N AUMVALU S AUMVALU=@AUMNAME ; get the saved, clean value
 . S AUTTST=AUMVALU ; copy cleaned up node back into ^DIC(5,
 ...
 ;
 D BMES^XPDUTL(AUMCNT-1_$$T("MSG+1")) ; # nodes in ^DIC(5, were scanned.
 ...
 ; Your ^DIC(5, global is [now] free of control characters.
...
MSG ; messages to display
 ;; nodes in ^DIC(5, were scanned.
 ;; instances of control characters were found and removed,
 ;; of them from node names, 
 ;; from values.
 ;;Your ^DIC(5, global is 
 ;;free of control characters.
 ;;Replacing the bad node names found in ^DIC(5,
 ;;Removing control characters from your ^DIC(5, global...
 ;;AUM*10.1 PRE-INIT
 ;
 ;
