...
CHECK ; troubleshooting entry point
 ;
 N AUMNAME S AUMNAME="^AUTTSU" ; the name value of each node of ^AUTTSU
 ...
 F AUMCNT=1:1 D  S AUMNAME=$Q(@AUMNAME) Q:AUMNAME=""  ; walk ^AUTTSU
 . ;
 ...
 ;
 ;
CLEANALL ; AUM*10.1 PRE-INIT: Remove Control Characters from ^AUTTSU
 ;
 ...
 N AUMCNTN S AUMCNTN=0 ; how many node names had control characters
 ;
 N AUMCNT ; count nodes, walk ^AUTTSU
 ...
 . N AUMEMP S AUMEMP=AUMCLN ; change name from ^AUTTSU(*)
 . ;S $E(AUMEMP,1,9)="^TMP(""AUM"","_$J_"," ; to ^TMP("AUM",$J,*)
 . S $E(AUMEMP,1,8)="^TMP(""AUM"","_$J_"," ; to ^TMP("AUM",$J,*)
 . ;IHS/OIT/CLS 09/17/2008 change to equal length of global root ^AUTTSU(
 ...
 . N AUTTSU S AUTTSU=AUMNAME,$E(AUTTSU,1,AUMLENG)="^AUTTSU(" ; change back
 . K @(@AUMNAME@(U)) ; delete node in ^AUTTSU whose bad name we saved off
 . N AUMVALU S AUMVALU=@AUMNAME ; get the saved, clean value
 . S @AUTTSU=AUMVALU ; copy cleaned up node back into ^AUTTSU
 . N AUMSUB S AUMSUB=$QS(AUMNAME,3) ; get the main subscript
 . K @AUMNAME@(U) ; delete the saved node name to avoid it
 . D MES^XPDUTL(AUTTSU_"="_AUMVALU) ; report nodes as we copy them back
 K ^TMP("AUM",$J) ; clean up rest of temp space
 ;
 D BMES^XPDUTL(AUMCNT-1_$$T("MSG+1")) ; # nodes in ^AUTTSU were scanned.
 ...
 ; Your ^AUTTSU global is [now] free of control characters.
...
MSG ; messages to display
 ;; nodes in ^AUTTSU were scanned.
 ;; instances of control characters were found and removed,
 ;; of them from node names, 
 ;; from values.
 ;;Your ^AUTTSU global is 
 ;;free of control characters.
 ;;Replacing the bad node names found in ^AUTTSU
 ;;Removing control characters from your ^AUTTSU global...
 ;;AUM*10.1 PRE-INIT
 ;
 ;
