AUMSCBD ;IHS/OIT/NKD - SCB UPDATE - DRIVER 12/07/2012 ;
 ...
 ; 12/10/13 - Modified processing for full table update (ASuFac/StCtyCom tables)
 ;          - Modified processing for inactivates (allow post-processing to occur)
 ;          - Removed ACT tag as the UPD tag performs the same functionality
 ;          - Added Field DD check before updating
 ;          - Modified display to use new mapping format and local modification display
 Q
...
POST ; EP -- MAIN EP
 ...
 . ; IHS/OIT/NKD AUM*14.0*1 - BEGIN NEW CODE
 ...
 . ;D ENTRY(AUMT,AUMA,$P(^AUMDATA(AUMCNT,0),U,3,8)) ; IHS/OIT/NKD AUM*14.0*1
 . D ENTRY(AUMT,AUMA,$P(^AUMDATA(AUMCNT,0),U,3,9))
 I $D(^TMP("AUM",$J,"ALL")) D ALL^AUMSCBA  ; IHS/OIT/NKD AUM*14.0*1 - FULL TABLE UPDATE
 D RSLT($$REPEAT^XLFSTR("-",20))  ; IHS/OIT/NKD AUM*14.0*1
 I HDR["EDT" D PKLST^AUMSCBU
 Q
 ;ENTRY(AUMT,AUMA,L,AUMAIEN) ; MAIN UPDATE DRIVER  ; IHS/OIT/NKD AUM*14.0*1
ENTRY(AUMT,AUMA,L,AUMAIEN,AUMQ) ; MAIN UPDATE DRIVER
 ;N P1,P2,P3,P4,P5,P6,P1A,P2A,P3A,P4A,P5A,P6A,L1,F1,F2,F3  ; IHS/OIT/NKD AUM*14.0*1
 ;N FDA,NEWIEN,ERR,CNT,CNT2,TEXT,INA  ; IHS/OIT/NKD AUM*14.0*1
 ...
 ;F CNT=1:1:6 S @("P"_CNT)=$P(L,U,CNT),@("P"_CNT_"A")="" ; IHS/OIT/NKD AUM*14.0*1
 ...
 Q:$D(AUME)
 ; SEARCH
 S:$D(AUMAIEN) AUMI=AUMAIEN  ; IHS/OIT/NKD AUM*14.0*1 - FULL TABLE UPDATE
 S:'AUMI&$D(AUMD("SEA")) AUMI=$$SEARCH(AUMD("SEA"))
 ; INACTIVATE
 ;I INA,$D(AUMD("INA")) D INACT Q  ; IHS/OIT/NKD AUM*14.0*1 - INA PROCESSING
 I INA,$D(AUMD("INA")) D INACT
 ;Q:INA  ; IHS/OIT/NKD AUM*14.0*1 - INA PROCESSING
 ; NEW
 ;I 'AUMI,$D(AUMD("NEW")) D NEW  ; IHS/OIT/NKD AUM*14.0*1 - INA PROCESSING
 I 'INA,'AUMI,$D(AUMD("NEW")) D NEW
 Q:'AUMI
 ; UPDATE
 ;I $D(AUMD("UPD")) D UPDATE  ; IHS/OIT/NKD AUM*14.0*1 - INA PROCESSING
 I 'INA,$D(AUMD("UPD")) D UPDATE
 ; ACTIVATE
 ;I AUMA="ACT",$D(AUMD("ACT")) D ACT  ; IHS/OIT/NKD AUM*14.0*1 - REMOVED ACT TAG
 ; WP
 I $D(AUMD("WP")) D WP
 ; POST-UPDATE
...
INACT ; GENERIC INACTIVATE DRIVER
 ;N FDA,ERR,F1,F2,F3,CNT  ; IHS/OIT/NKD AUM*14.0*1
 ...
 ;S F1=$P(AUMD("INA"),U,1),F2=$P(AUMD("INA"),U,2)  ; IHS/OIT/NKD AUM*14.0*1
 S F1=$P(AUMD("INA"),U,1),F2=$P(AUMD("INA"),U,2),AUMR=""
 F CNT=1:1:$L(F2,";") D
 . S F3=$P(F2,";",CNT)
 . I '$D(@$P(F3,"|",2)) D ERR("SYSTEM ERROR - Unassigned local variable: "_$P(F3,"|",2)) Q
 . Q:(@$P(F3,"|",2))']""  ; IHS/OIT/NKD AUM*14.0*1 - VALUE CHECK
 . Q:($$GET1^DID(F1,$P(F3,"|",1),"","LABEL")']"")  ; IHS/OIT/NKD AUM*14.0*1 - FIELD CHECK
 . S FDA(F1,AUMI_",",$P(F3,"|",1))=@$P(F3,"|",2)
 . S AUMR=AUMR_$P(F3,"|",1)_"|"_$TR($$GET1^DIQ(F1,AUMI,$P(F3,"|",1),"I"),U,";;")_"|"_$P(F3,"|",2)_U  ; IHS/OIT/NKD AUM*14.0*1
 D UPDATE^DIE($S($L($P(AUMD("INA"),U,3))<1:"E",1:""),"FDA",,"ERR")
 I $D(ERR) D ERR("SYSTEM ERROR - Inactivate failed") Q
 ; IHS/OIT/NKD AUM*14.0*1 - DISPLAY FOR INACTIVATE - START NEW CODE
 I AUMM']"" D
 . I AUMA="DEL" S AUMM=AUMA_" :" Q
 ...
 . . S AUMM="INA :"
 ; IHS/OIT/NKD AUM*14.0*1 - END NEW CODE
 ;D DISP  ; IHS/OIT/NKD AUM*14.0*1 - INA PROCESSING
 Q
...
UPDATE ; GENERIC UPDATE DRIVER
 ...
 . F CNT=1:1:$L(AUMR,U) Q:$P(AUMR,U,CNT)']""  D
 . . Q:$$GET1^DIQ(F1,AUMI,$P($P(AUMR,U,CNT),"|",1),"I")=$TR($P($P(AUMR,U,CNT),"|",2),";;",U)
 . . Q:$P($P(AUMR,U,CNT),"|",1)=".28"  ; IHS/OIT/NKD AUM*14.0*1 - DON'T PUT LAST MODIFIED DATE IN DEBUG OUTPUT
 . . ; IHS/OIT/NKD AUM*13.0*1 - CHANGED DEBUG OUTPUT TO INCLUDE DATA
 . . ;S AUML=AUML_$P($P(AUMR,U,CNT),"|",1)_"|"_$P($P(AUMR,U,CNT),"|",2)_"|"_@$P($P(AUMR,U,CNT),"|",3)_U  ; IHS/OIT/NKD AUM*14.0*1 - EXTERNAL FORMAT OF FIELD
 . . S AUML=AUML_$$GET1^DID(F1,$P($P(AUMR,U,CNT),"|",1),"","LABEL")_"|"_$P($P(AUMR,U,CNT),"|",2)_"|"_@$P($P(AUMR,U,CNT),"|",3)_U
 . . S AUMM="MOD :"
 Q
 ; IHS/OIT/NKD AUM*14.0*1 - REMOVED ACT TAG - START OLD CODE
...
WP ; GENERIC WORD-PROCESSING DRIVER
 ...
 N AUMWP1,AUMWP2  ; IHS/OIT/NKD AUM*13.0*3 - WP ARRAY COMPARISON
 S F1=$P(AUMD("WP"),U,1),F2=$P(AUMD("WP"),U,2)
 F CNT=1:1:$L(F2,";") D
 . S F3=$P(F2,";",CNT)
 . I '$D(@$P(F3,"|",2)) D ERR("SYSTEM ERROR - Unassigned local variable: "_$P(F3,"|",2)) Q
 . D TEXT(.@($P(F3,"|",2)))
 . S AUMWP1=$$GET1^DIQ(F1,AUMI_",",$P(F3,"|",1),,"AUMWP1")  ; IHS/OIT/NKD AUM*13.0*3 - WP ARRAY COMPARISON
 . D WP^DIE(F1,AUMI_",",$P(F3,"|",1),,$P(F3,"|",2))
 . ; IHS/OIT/NKD AUM*13.0*3 - WP ARRAY COMPARISON - START NEW CODE
 . S AUMWP2=$$GET1^DIQ(F1,AUMI_",",$P(F3,"|",1),,"AUMWP2")
 . ;I $$WPCHK(.AUMWP1,.AUMWP2) S AUML=AUML_$P(F3,"|",1)_U,AUMM=$S(AUMM]"":AUMM,1:"MOD :")  ; IHS/OIT/NKD AUM*14.0*1 - EXTERNAL FORMAT OF FIELD
...
DISP ; EP - GENERIC DISPLAY DRIVER
 N F1,F2,F3,CNT,AUMS,AUMH  ; IHS/OIT/NKD AUM*14.0*1 - HDR DISPLAY
 Q:AUMP
 I $D(AUMD("DSP")) D
 . S AUMS=""
 . S INAD=$S(INA:"I",1:"")  ; IHS/OIT/NKD AUM*14.0*1 - INA DISPLAY
 ...
 . ; IHS/OIT/NKD AUM*14.0*1 - HDR DISPLAY - START NEW CODE
 . I $D(AUMD("HDR")) D
 . . S F1=$P(AUMD("HDR"),U,1)
 ...
 . ; IHS/OIT/NKD AUM*14.0*1 - END NEW CODE
 . I '$G(AUMQ) D RSLT(AUMS),TAGCNT  ; IHS/OIT/NKD AUM*14.0*1
 . I AUMM]"",(($G(AUMQ)=1)!(AUMM["ERR")) D RSLT(AUMS)  ; IHS/OIT/NKD AUM*14.0*1
 . I $G(AUMQ)=1 D TAGCNT  ; IHS/OIT/NKD AUM*14.0*1 - LOCAL MODIFICATIONS DISPLAY
 . S AUMP=1
 . ; IHS/OIT/NKD AUM*14.0*1 - DEBUG OUTPUT - BEGIN NEW CODE
 . I $D(AUMDEBUG),AUMM'["ERR",AUMM'["NEW",AUMM'["ADD" D
 . . Q:AUML']""
...
WPCHK(AUMWP1,AUMWP2) ; COMPARE WP ARRAYS AND RETURN 1 IF NOT EQUAL
 ...
 E  F CNT=1:1:AUMTO S:$$CLEAN($G(AUMWP1(CNT)))'=$$CLEAN($G(AUMWP2(CNT))) AUMRES=1  ;IHS/OIT/NKD AUM*13.0*4 $G ADDED
 Q AUMRES
 ; IHS/OIT/NKD AUM*14.0*1
