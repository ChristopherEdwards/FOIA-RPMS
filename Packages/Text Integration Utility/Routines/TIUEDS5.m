...
DE S DIE="^TIU(8925,",DIC=DIE,DP=8925,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^TIU(8925,DA,""))=""
 ...
 I $D(^(12)) S %Z=^(12) S %=$P(%Z,U,1) S:%]"" DE(17)=% S %=$P(%Z,U,2) S:%]"" DE(1)=% S %=$P(%Z,U,9) S:%]"" DE(20)=%
...
X19 I +$$PROVIDER^TIUPXAP1(DUZ,DT)'>0 S Y="@9"
 Q
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,DW="12;9",DV="*P200'R",DU="",DLB="ATTENDING PHYSICIAN",DIFLD=1209
 S DU="VA(200,"
 G RE
X20 S DIC("S")="I '+$$ISTERM^USRLM(+Y),+$$PROVIDER^TIUPXAP1(+Y,DT)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
X21 S Y="@10"
 Q
22 S DQ=23 ;@9
23 D:$D(DG)>9 F^DIE17 G ^TIUEDS10
