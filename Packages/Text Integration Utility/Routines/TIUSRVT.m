TIUSRVT ; SLC/JM - Server functions for templates 8/23/2001
 ;;1.0;TEXT INTEGRATION UTILITIES;**76,80,102,105,119,125**;Jun 20, 1997
 ;
...
ISEDITOR(TIUY,ROOT,USER) ; Returns TRUE if user is a Template Editor
 ...
 Q
LISTOWNR(TIUY,FROM,DIR) ; Return subset of personal owners
 N I,IEN,CNT S CNT=44,I=0
 F  S FROM=$O(^VA(200,"B",FROM),DIR) Q:FROM=""  D  Q:I=CNT
 .S IEN=0 F  S IEN=$O(^VA(200,"B",FROM,IEN)) Q:'IEN  D  Q:I=CNT
 ..I $O(^TIU(8927,"AROOT",IEN,0)) D
 ...S I=I+1 S TIUY(I)=IEN_"^"_FROM
 Q
 ;
 ; Internal Routines
 ;
...
NODEDATA(TIUDA) ;Returns template node data
 ...
 ..I RDN'="" S $P(DATA,U,16)=RDIEN_U_$P(RDN,U,1)
