...
TYP ;EP for selecting Billing Entity
 ...
 ;S DIR(0)="SO^1:MEDICARE;2:MEDICAID;3:PRIVATE INSURANCE;4:NON-BENEFICIARY PATIENTS;5:BENEFICIARY PATIENTS;6:SPECIFIC INSURER"  ;abm*2.6*11 VMBP#8
 S DIR(0)="SO^1:MEDICARE;2:MEDICAID;3:PRIVATE INSURANCE;4:NON-BENEFICIARY PATIENTS;5:BENEFICIARY PATIENTS;6:SPECIFIC INSURER;7:VETERANS MEDICAL BENEFIT PROG"  ;abm*2.6*11 VMBP#8
 ;I '$D(ABMY("RNG")) S DIR(0)=DIR(0)_";7:SPECIFIC PATIENT"  ;abm*2.6*11 VMBP#8
 I '$D(ABMY("RNG")) S DIR(0)=DIR(0)_";8:SPECIFIC PATIENT"  ;abm*2.6*11 VMBP#8
 S DIR("A")="Select TYPE of BILLING ENTITY to Display"
 D ^DIR K DIR Q:$D(DIRUT)!$D(DIROUT)
 ;start old code abm*2.6*11 VMBP#8
 ;S ABMY("TYP")=$S(Y=1:"R",Y=2:"D",Y=3:"P",Y=4:"N",Y=5:"I",1:Y),ABMY("TYP","NM")=Y(0)
 ;G INS:Y=6,PAT:Y=7
 ;end old code start new code VMBP#8
