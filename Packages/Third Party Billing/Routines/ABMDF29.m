ABMDF29 ; IHS/SD/SDR - ADA-2006 Dental Export Routine ;   
 ...
 ;
 ; IHS/SD/SDR - v2.5 p13 - IM25742
 ;   Correction to tooth number showing on multiple lines
 ;
 ; IHS/SD/SDR - v2.6 CSV
 ; IHS/SD/PMT - HEAT8604 - Corrected report to start at line 1, not line 2
 ; IHS/SD/SDR - HEAT11745 - FL27 - leave it printing tooth mnemonic, not tooth synonym; was printing "7D" instead of "D"
 ;************************************************************************************
 K ABM
 S U="^"
 S ABMP("EXP")=29                ; Export mode
...
BODY ;
 ...
 ..;S:ABMF(ABM("F"))["D" $P(ABMF(ABM("F")),U,4)=$P($G(^ADEOPS(ABMOPS,0)),U,4) ; Tooth synonym (27)  ;abm*2.6*6 HEAT11745
 .;
 .; Procedure date (24)
 ...
 ..S ABM("MORE")=1
 ..D ^ABMDF29X                 ; Print form
 ..S ABM("I")=0
 ;
OTHER ;
 ; Other charges on page 3?
 I $P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),9)),U,21)'="" D
 .;S $P(ABMF(38),U)=$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),9)),U,21)  ;Other charges (32)  HEAT8604
 .S $P(ABMF(37),U)=$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),9)),U,21)  ;Other charges (32)
 .;S ABM("TCHRG")=ABM("TCHRG")+$P(ABMF(38),U)  ;add to total charges (33)  HEAT8604
 .S ABM("TCHRG")=ABM("TCHRG")+$P(ABMF(37),U)  ;add to total charges (33)
 ;
...
RESET ;
 ...
 ;F ABM("F")=26:1:36 K ABMF(ABM("F"))  ;HEAT8604
 F ABM("F")=25:1:36 K ABMF(ABM("F"))  ;HEAT8604
 ;S ABM("F")=26  ;HEAT8604
 S ABM("F")=25  ;HEAT8604
 Q
 ;
TAIL ;END OF FORM
 ...
 ;S $P(ABMF(39),U)=ABM("TCHRG")   ; Total fee charged on bill HEAT8604
 S $P(ABMF(38),U)=ABM("TCHRG")   ; Total fee charged on bill
