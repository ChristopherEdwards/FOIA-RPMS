ABPAPDB ; GENERATED FROM 'ABPAPDB' PRINT TEMPLATE (#747) ; 08/29/91 ; (FILE 9002270.02, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 D N:$X>0 Q:'DN  W ?0 S X="*** CLAIM/PAYMENTS SCREEN ***",X=$J("",$S($D(DIWR)+$D(DIWL)=2:DIWR-DIWL+1,$D(IOM):IOM,1:80)-$L(X)\2-$X)_X K DIP K:DN Y W X
 D N:$X>0 Q:'DN  W ?0 W "==============================================================================="
 ...
 S X=$S($D(^ABPVAO(D0,0)):^(0),1:"") D N:$X>61 Q:'DN  W ?61 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^AUTTLOC(Y,0))#2:$P(^(0),U,1),1:Y) S Y=$S(Y="":Y,$D(^DIC(4,Y,0))#2:$P(^(0),U,1),1:Y) W $E(Y,1,18)
 D N:$X>0 Q:'DN  W ?0 W "-------------------------------------------------------------------------------"
 K Y
 Q
HEAD ;
