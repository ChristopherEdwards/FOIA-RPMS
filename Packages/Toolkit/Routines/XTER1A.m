XTER1A ;ISC-SF.SEA/JLI - VA error reporting ;04/20/99  14:26
 ...
 ;THIS ROUTINE CONTAINS LOCAL MODIFICATIONS BY IHS/ANMC/LJF 5/21/99
 ;
TWO ;
 S XTNUM=2
ONE ;
...
T1 S X=$O(^%ZTER(1,%XTZDAT,1,X),-1) R XTQ:0 Q:XTQ'=""  G T2:X'>0,T1:'($D(^(X,"ZE"))#2) S XTP=^("ZE"),XTS=""
 ...
 ;TWO LINES ARE COMMENTED OUT AND REPLACED BY NEW LINES TO DISPLAY
 ;OPTION RATHER THAN UCI,VOL WHEN LISTING ERRORS IN ERROR TRAP
 ;ORIGINAL MODIFICATION BY IHS/ANMC/LJF 5/21/99
 ...
 ;----- BEGIN IHS MODIFICATION
 ;ADD NEW LINE TO CALL NEW SUBROUTINE IHSXQY0
 D IHSXQY0
 ;LINE BELOW IS COMMENTED OUT AND REPLACED BY NEW LINE TO DISPLAY
 ;OPTION RATHER THAN UCI,VOL
 ;ORIGINAL MODIFICATION BY IHS/ANMC/LJF 5/20/99
 S XTXT=XTXT_"  "_$P($S('$D(^%ZTER(1,%XTZDAT,1,X,"J")):"",1:^("J")),U,4)_"  "_$J($P($S('$D(^("J")):"",1:^("J")),U,5),7)_"  "_$P($S('$D(^("I")):"",1:^("I")),U)
 S XTXT=XTXT_" "_$J($P($S('$D(^%ZTER(1,%XTZDAT,1,X,"J")):"",1:^("J")),U,5),7)_"  "_$P($S('$D(^("I")):"",1:^("I")),U)
 ;----- END IHS MODIFICATION
...
INTRACT ;
 G INTRACT^XTER1A1
 ;----- BEGIN IHS MODIFICATION
 ;ADD NEW SUBROUTINE IHSXQY0
 ;ORIGINAL MODIFICATION BY IHS/ANMC/LJF 5/20/99
IHSXQY0 ;Find option name
 NEW IHS,FOUND,STR
 S (FOUND,IHS)=0
 F  S IHS=$O(^%ZTER(1,%XTZDAT,1,X,"ZV",IHS)) Q:'IHS  Q:FOUND  D
 . Q:$G(^%ZTER(1,%XTZDAT,1,X,"ZV",IHS,0))'="XQY0"
 . S STR=$P($G(^%ZTER(1,%XTZDAT,1,X,"ZV",IHS,"D")),U)
 . S STR=$E(STR,1,18)_$$REPEAT^XLFSTR(" ",(18-$L(STR))),FOUND=1
