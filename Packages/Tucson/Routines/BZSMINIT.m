BZSMINIT ; ; 06-APR-2003
 ...
 S DIOVRD=1,U="^",DIFQ=0,DIFROM="1.0" W !,"This version (#1.0) of 'BZSMINIT' was created on 06-APR-2003"
 W !?9,"(at DULCE HL.CTR, by VA FileMan V.22.0)",!
...
EN ; ENTER HERE TO BYPASS THE PRE-INIT PROGRAM
 ...
 I DIFROM,$D(^%ZTSK) S X="BZSMINIS" X ^%ZOSF("TEST") D:$T PAC^BZSMINIS($T(IXF),.DIFROM)
...
PKG S X=$P($T(IXF),";",3),DIC="^DIC(9.4,",DIC(0)="",DIC("S")="I $P(^(0),U,2)="""_$P(X,U,2)_"""",X=$P(X,U) D ^DIC S DIFROM(0)=+Y K DIC
 Q
 ;
IXF ;;TAO A/R WRITE-OFF^BZSM;656
