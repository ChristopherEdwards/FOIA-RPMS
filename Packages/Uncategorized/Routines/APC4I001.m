...
Q Q
 ...
 ;;=APCD ECOD (ECOD)^3001109.1524^^9000001^3^^3001109
 ...
 ;;=APCD CC (CC)^3000710.1321^^9000010^^^3000917^
 ...
 ;;=APCD ECO2 (ECO2)^3010109.0952^M^9000010^3^@^3010109
 ...
 ;;=X $P(^DD(9000010,9999,0),U,5,99) S Y(1)=X S I(0,0)=$S($D(D0):D0,1:"") S X=Y(1) K DIC S DIC="^AUPNVPOV(",DIC(0)="NMF" D ^DIC S (D,D0,D(0))=+Y
 ;;^UTILITY(U,$J,"DIE",1785,0)
 ;;=APCD ECO2 (ECO2)^3010109.0952^M^9000001^3^@^3010109
 ;;^UTILITY(U,$J,"DIE",1785,"DIAB",1,1,9000010,1)
 ;;=LOOKUP:V POV:
 ;;^UTILITY(U,$J,"DIE",1785,"DIAB",1,2,9000010.07,0)
 ;;=.18;"Enter 2nd E-CODE"
 ;;^UTILITY(U,$J,"DIE",1785,"DIAB",2,0,9000001,0)
 ;;=LOOKUP:VISIT:
 ;;^UTILITY(U,$J,"DIE",1785,"DR",1,9000001)
 ;;=S APCDPAT=DA D ^APCDVLK S:APCDLOOK APCDLOOK="`"_APCDLOOK;^9000010^AUPNVSIT(^^X DR(99,1,9.2) S Y(101)=$S($D(^AUPNVSIT(D0,0)):^(0),1:"") S X=$P(Y(101),U,1) S D0=I(0,0) S X=$S(D(0)>0:D(0),1:"");
