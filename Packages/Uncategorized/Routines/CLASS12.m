CLASS12 ;$O DPT B X-REF [ 03/10/1999  2:28 PM ]
...
START ;
 ...
 K D,X,TOTALCT,LASTNAME,HOB2,HOBDFN,HOBBY,TONISEX,GRANTOT
 Q
PRINT ;GET DFN FOR NAME TO PRINT
 S X="" S X=$O(^CLASSDMO("B",D,X)) Q:X'=+X  S GRANDTOT=GRANDTOT+1 D PRINT1
 Q
PRINT1 ;PRINT ACTUAL NAME
 W !?5,TOTALCT,?15,$P(^CLASSDMO(X,0),U)
 W ?40,X
 W ?45,$P(^CLASSDMO(X,0),U,10)
 S LASTNAME=X
...
HOBBIES ;If Hobbies Exist - get one hobby dfn only
 S HOB2=""
 F  S HOB2=$O(^CLASSDMO(X,1,"B",HOB2)) Q:HOB2=""  D
 . S HOBDFN="" F  S HOBDFN=$O(^CLASSDMO(X,1,"B",HOB2,0)) Q:HOBDFN'=+HOBDFN  S HOBBY=$P(^CLASSDMO(X,1,HOBDFN,0),U),HOBBPRNT=$P(^CLASSHOB(HOBBY,0),U) Q
 Q
