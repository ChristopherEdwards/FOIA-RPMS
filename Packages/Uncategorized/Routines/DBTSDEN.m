DBTSDEN ;BAO/DMH  pull patient DENTAL exams [ 11/24/1999  3:55 PM ]
 ;   11/24/99   new dental call looking at the aupnvden file now
 ;                 instead of the adepcd file
 ;                 copy of the DBTSEXDE routine with changes
 ;
 ;
 ;
START ;
...
EX ;
 ...
 .Q:REC=""
 .Q:'$D(^ADEPCD(DBTS("IEN"),"PCC"))
 .S DBTS("VDFN")=$P(^ADEPCD(DBTS("IEN"),"PCC"),"^",1)   ;11/24/99
 ...
 .S DBTS("VID")=DBTS("LOC")_"|9000010|"_DBTS("VDFN")  ;dmh 11-24-99
 .D MODCK
 .Q:DBTS("OK")="N"
 .D NARR
 .S ARRAY=ARRAY+1
 .S DBTSRET(ARRAY)=DBTS("ID")_U_DBTS("LOC")_U_DBTS("AU")_U_DBTS("PAT")_U_"DENTAL EXAM"_U_99_U_DBTS("VDATE")_U_DBTS("PNAR")_U_DBTS("VID")
 .S ^DBTSTEMP(1,ARRAY)=DBTSRET(ARRAY)_$C(13)_$C(10)
 .D LOG
 .Q
 I ARRAY=0 S DBTSRET(1)="-2" S ^DBTSTEMP(1,1)="-2"_$C(13)_$C(10)
 I $D(^DBTSPAT(DBTSP,"DEN")) S $P(^DBTSPAT(DBTSP,"DEN"),"^",2)=DT
 Q
MODCK ;
 S DBTS("OK")="Y"
 S DBTS("AU")="A"
 ...
 ;I $P(^DBTSPAT(DBTSP,"DEN"),"^",2)="" Q   ;11-16-99
...
NARR ;  loop thru the dental ada multiple and use codes for narrative 
 ...
 .S NL=($L(DBTS("PNAR")))+($L(NARR)) Q:NL>380    ;11/24/99  dmh
