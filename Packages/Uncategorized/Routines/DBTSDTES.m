DBTSDTES ;BAO/DMH  pull patient  DIAGNOSIS [ 10/26/1999  2:52 PM ]
 ;
...
TEST ;
 ...
 I DBTS("LOC")="" S DBTSRET(1)="-1" S ^DBTSTEMP($J,1)="-1"_$C(13)_$C(10) Q
 S DBTS("FN")="9000010.07"   ;put C on end for cardiac only--took off
 ;                           ;   for all diag now 10-7-99  dmh
L ;
 S DD=106
 F  S DD=$O(^DBTSPAT(DD)) Q:+DD>107  D LOOP
 Q
LOOP ;
 S DBTSP=DD
 S DBTS("IEN")=0
EX ;
 ...
 .S ARRAY=ARRAY+1
 .;Q:ARRAY>200
 .;S ^DBTSDINA(0)=$G(^DBTSDINA(0))+1
 .;S ^DBTSDINA(DBTSP,DBTS("ID"))=$G(^DBTSDINA(DBTSP,DBTS("ID")))+1
 .S DBTSTEMP($J,ARRAY)=DBTS("ID")_U_DBTS("LOC")_U_DBTS("AU")_U_DBTS("PAT")_U_"DIAGNOSIS"_U_DBTS("ICD")_U_DBTS("ICDNAME")_U_DBTS("VDATE")_U_DBTS("PNAR")_U_DBTS("VID")_$C(13)_$C(10)
 .;S ^DBTSTEMP($J,ARRAY)=DBTS("ID")_U_DBTS("LOC")_U_DBTS("AU")_U_DBTS("PAT")_U_"DIAGNOSIS"_U_DBTS("ICD")_U_DBTS("ICDNAME")_U_DBTS("VDATE")_U_DBTS("PNAR")_U_DBTS("VID")_$C(13)_$C(10)
 .D LOG
 .Q
 I ARRAY=0 S DBTSRET(1)="-2" S ^DBTSTEMP($J,1)="-2"_$C(13)_$C(10)
 I $D(^DBTSPZZ(DBTSP,"ICD")) S $P(^DBTSPZZ(DBTSP,"ICD"),"^",2)=DT
 Q
CARDCK ;
 Q
 S DBTS("CFL")="N"
...
MODCK ;
 ...
 S DBTS("AU")="A"
 ;Q                         ;for testing
 Q:'$D(^DBTSPZZ(DBTSP,"ICD"))
 I $P(^DBTSPZZ(DBTSP,"ICD"),"^",2)="" Q
 S DBTS("MREC")=$G(^DBTSPZZ(DBTSP,"ICD"))
 Q:DBTS("MREC")=""
...
LOG ;  update the patient log for the type of lab test
 ...
 I '$D(^DBTSPZZ(DBTSP)) D
 .K ^DBTSPZZ("B",DBTSP)
 .S ^DBTSPZZ(DBTSP,0)=DBTSP
 .S ^DBTSPZZ("B",DBTSP,DBTSP)=""
 .;S X=DBTSP,DINUM=X,DIC(0)="XNL",DIC="^DBTSPAT(" D FILE^DICN
 I '$D(^DBTSPZZ(DBTSP,"ICD")) S ^DBTSPZZ(DBTSP,"ICD")=DBTS("IEN")_"^" Q
 I $G(DBTS("LDFN"))<DBTS("IEN") S $P(^DBTSPZZ(DBTSP,"ICD"),"^",1)=DBTS("IEN")
 Q
