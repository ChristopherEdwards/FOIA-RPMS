DBTSEKG ;BAO/DMH  pull patient EKG dates [ 11/16/1999  9:06 PM ]
 ;
 ;
 ;
START ;
 ;
EKG(DBTSGBL,DBTSP)          ;dbtsret is return value, dbtsp input value of patient ien
 ;
 ;
TEST ;
 K ^DBTSTEMP(1)
 ...
 S DBTS("IEN")=0
 I '$D(^DBTSEXDI("B","EKG")) S ^DBTSTEMP(1,1)="-2"_$C(13)_$C(10) ;may want to chg.
EX ;
 ...
 .S ARRAY=ARRAY+1
 .S DBTSRET(ARRAY)=DBTS("ID")_U_DBTS("LOC")_U_DBTS("AU")_U_DBTS("PAT")_U_"EKG"_U_DBTS("PROC")_U_DBTS("PROCNAME")_U_DBTS("VDATE")_U_DBTS("PNAR")_U_DBTS("VID")
 .S ^DBTSTEMP(1,ARRAY)=DBTSRET(ARRAY)_$C(13)_$C(10)
 .D LOG
 .Q
 I ARRAY=0 S DBTSRET(1)="-2" S ^DBTSTEMP(1,1)="-2"_$C(13)_$C(10)
 I $D(^DBTSPAT(DBTSP,"EKG")) S $P(^DBTSPAT(DBTSP,"EKG"),"^",2)=DT
 Q
EKGCK ;
 S DBTS("CFL")="N"
...
MODCK ;
 ...
 ;I $P(^DBTSPAT(DBTSP,"EKG"),"^",2)="" Q    ;11-16-99
