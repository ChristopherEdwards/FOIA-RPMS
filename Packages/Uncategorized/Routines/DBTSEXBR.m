DBTSEXBR ;BAO/DMH  pull patient BREAST exams [ 11/16/1999  8:58 PM ]
 ;
 ;
 ;
START ;
...
EX ;
 ...
 .S DBTS("V")=$P($G(^AUPNVSIT(DBTS("VDFN"),0)),U,1)
 .Q:DBTS("V")=""
 .S DBTS("V")=$P(DBTS("V"),".",1)
 .Q:DBTS("V")=""
 .S DBTS("DT")=DBTS("V")
 ...
 .S ARRAY=ARRAY+1
 .S DBTSRET(ARRAY)=DBTS("ID")_U_DBTS("LOC")_U_DBTS("AU")_U_DBTS("PAT")_U_DBTS("EXNAME")_U_DBTS("CODE")_U_DBTS("VDATE")_U_DBTS("PNAR")_U_DBTS("VID")
 .S ^DBTSTEMP(1,ARRAY)=DBTSRET(ARRAY)_$C(13)_$C(10)
 .D LOG
 .Q
 I ARRAY=0 S DBTSRET(1)="-2" S ^DBTSTEMP(1,1)="-2"_$C(13)_$C(10)
 I $D(^DBTSPAT(DBTSP,"BRE")) S $P(^DBTSPAT(DBTSP,"BRE"),"^",2)=DT
 Q
MODCK ;
 S DBTS("OK")="Y"
 S DBTS("AU")="A"
 Q:'$D(^DBTSPAT(DBTSP,"BRE"))
 S DBTS("MREC")=$G(^DBTSPAT(DBTSP,"BRE"))
 Q:DBTS("MREC")=""
 S DBTS("LDFN")=$P(DBTS("MREC"),U,1)
 I $P(^DBTSPAT(DBTSP,"BRE"),"^",2)="" Q
...
LOG ;  update the patient log for the type of lab test
 ...
 I '$D(^DBTSPAT(DBTSP,"BRE")) S ^DBTSPAT(DBTSP,"BRE")=DBTS("IEN")_"^" Q
 I $G(DBTS("LDFN"))<DBTS("IEN") S $P(^DBTSPAT(DBTSP,"BRE"),"^",1)=DBTS("IEN")
 Q
