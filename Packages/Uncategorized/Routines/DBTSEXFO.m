DBTSEXFO ;BAO/DMH  pull patient DIABETES FOOT exams [ 11/16/1999  9:15 PM ]
 ;
 ;
 ;
START ;
...
EX ;
 ...
 I $D(^DBTSPAT(DBTSP,"FTE")) S $P(^DBTSPAT(DBTSP,"FTE"),"^",2)=DT
 Q
MODCK ;
 S DBTS("OK")="Y"
 S DBTS("AU")="A"
 Q:'$D(^DBTSPAT(DBTSP,"FTE"))
 ;I $P(^DBTSPAT(DBTSP,"FTE"),"^",2)="" Q  ;11-16-99
 S DBTS("MREC")=$G(^DBTSPAT(DBTSP,"FTE"))
 Q:DBTS("MREC")=""
...
LOG ;  update the patient log for the type of lab test
 ...
 I '$D(^DBTSPAT(DBTSP,"FTE")) S ^DBTSPAT(DBTSP,"FTE")=DBTS("IEN")_"^" Q
 I $G(DBTS("LDFN"))<DBTS("IEN") S $P(^DBTSPAT(DBTSP,"FTE"),"^",1)=DBTS("IEN")
 Q
