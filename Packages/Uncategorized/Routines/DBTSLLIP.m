DBTSLLIP ;BAO/DMH  pull patient LAB TESTS FOR LIPIDS  [ 11/16/1999  9:29 PM ]
...
START ;
 ;
LIP(DBTSGBL,DBTSP)          ;dbtsret is return value, dbtsp input value of patient ien
 ;
 ;
TEST ;
 K ^DBTSTEMP(1)
 ...
 ;
 ;
 S DBTS("LOG")=$G(^DBTSPAT(DBTSP,"LIP"))
 ...
 .;
 .;
 .I +DBTS("RESULTS")=0 S DBTS("RESULTS")=""   ;sometimes comment comes
 .I DBTS("RESULTS")="" Q
 ...
 .S DBTSRET(ARRAY)=DBTS("ID")_U_DBTS("LOC")_U_AU_U_DBTS("PAT")_U_"LAB-LIPIDS"_U_VD
 .F I=3,4,5,7 S RR=$P(ARR(VD),"^",I) D
 ..S R=$P(RR,"|",1)
 ..I R'="" S R=+R
 ..I R=0 S R=""
 ...
 I $D(^DBTSPAT(DBTSP,"LIP")) S $P(^DBTSPAT(DBTSP,"LIP"),"^",2)=DT
 K ARR,DBTS
 Q
LABCK ;
 K DBTS("LABID")
...
MODCK ;
 S DBTS("OK")="Y"
 S DBTS("AU")="A"
 Q:'$D(^DBTSPAT(DBTSP,"LIP"))
 ;I $P(^DBTSPAT(DBTSP,"LIP"),"^",2)="" Q   ;11-16-99
 S DBTS("MREC")=$G(^DBTSPAT(DBTSP,"LIP"))
 Q:DBTS("MREC")=""
...
LOG ;  update the patient log for the type of lab test
 ...
 I '$D(^DBTSPAT(DBTSP,"LIP")) S ^DBTSPAT(DBTSP,"LIP")=DBTS("VLABIEN")_"^" Q
 S DBTS("LDFN")=$P(^DBTSPAT(DBTSP,"LIP"),"^",1)   ;2/9/99 ADDED
 I $G(DBTS("LDFN"))<DBTS("VLABIEN") S $P(^DBTSPAT(DBTSP,"LIP"),"^",1)=DBTS("VLABIEN")
 Q
