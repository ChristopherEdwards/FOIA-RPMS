DBTSLREN ;BAO/DMH  pull patient LAB TESTS FOR RENAL  [ 11/16/1999  9:32 PM ]
...
START ;
 ;
REN(DBTSGBL,DBTSP)          ;dbtsret is return value, dbtsp input value of patient ien
 ;
 ;
TEST ;
 K ^DBTSTEMP(1)
 ...
 ;
 ;
 S DBTS("LOG")=$G(^DBTSPAT(DBTSP,"REN"))
 ...
 .;
 .;
 .I +DBTS("RESULTS")=0 S DBTS("RESULTS")=""   ;comment in results 2/99
 .I DBTS("RESULTS")="" Q
 ...
 .S DBTSRET(ARRAY)=DBTS("ID")_U_DBTS("LOC")_U_AU_U_DBTS("PAT")_U_"LAB-RENAL"_U_VD
 .F I=1,2 S RR=$P(ARR(VD),"^",I) D
 ..S R=$P(RR,"|",1)
 ..I R'="" S R=+R
 ..I R=0 S R=""
 ...
 I $D(^DBTSPAT(DBTSP,"REN")) S $P(^DBTSPAT(DBTSP,"REN"),"^",2)=DT
 K ARR,DBTS
 Q
LABCK ;
 K DBTS("LABID")
 ...
 Q:DBTS("N")=""
 ;
 ;    check to see if the lab test is coded to "R" type for RENAL
 ;
 S DBTS("LABREC")=$G(^DBTSLAB(DUZ(2),11,DBTS("N"),0))
 Q:DBTS("LABREC")=""
 I $P(DBTS("LABREC"),U,2)'="R" Q
...
MODCK ;
 S DBTS("OK")="Y"
 S DBTS("AU")="A"
 Q:'$D(^DBTSPAT(DBTSP,"REN"))
 ;I $P(^DBTSPAT(DBTSP,"REN"),"^",2)="" Q   ;11-16-99
 S DBTS("MREC")=$G(^DBTSPAT(DBTSP,"REN"))
 Q:DBTS("MREC")=""
...
LOG ;  update the patient log for the type of lab test
 ...
 I '$D(^DBTSPAT(DBTSP,"REN")) S ^DBTSPAT(DBTSP,"REN")=DBTS("VLABIEN")_"^" Q
 S DBTS("LDFN")=$P(^DBTSPAT(DBTSP,"REN"),"^",1)   ;2/9/99 ADDED
 I $G(DBTS("LDFN"))<DBTS("VLABIEN") S $P(^DBTSPAT(DBTSP,"REN"),"^",1)=DBTS("VLABIEN")
 Q
