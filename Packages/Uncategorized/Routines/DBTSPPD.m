DBTSPPD ;BAO/DMH  pull patient PPD SKIN TEST [ 11/16/1999  10:02 PM ]
 ;
 ;
 ;
START ;
 ;
PPD(DBTSGBL,DBTSP)          ;dbtsret is return value, dbtsp input value of patient ien
 ;
 ;
TEST ;
 K ^DBTSTEMP(1)
...
EX ;
 ...
 .S ARRAY=ARRAY+1
 .S DBTSRET(ARRAY)=DBTS("ID")_U_DBTS("LOC")_U_DBTS("AU")_U_DBTS("PAT")_U_"PPD"_U_DBTS("RESULT")_U_DBTS("READING")_U_DBTS("VDATE")_U_DBTS("PNAR")_U_DBTS("VID")
 .S ^DBTSTEMP(1,ARRAY)=DBTSRET(ARRAY)_$C(13)_$C(10)
 .D LOG
 .Q
 I ARRAY=0 S DBTSRET(1)="-2" S ^DBTSTEMP(1,1)="-2"_$C(13)_$C(10)
 I $D(^DBTSPAT(DBTSP,"PPD")) S $P(^DBTSPAT(DBTSP,"PPD"),"^",2)=DT
 Q
RESULT ;
...
MODCK ;
 ...
 ;I $P(^DBTSPAT(DBTSP,"PPD"),"^",2)="" Q    ;11-16-99
