IS00018J ;Compiled from script 'Generated: HL IHS LAB R01 UNILAB IN-I' on AUG 14, 2006
 ;Part 11
 ;Copyright 2006 SAIC
EN S INI(2)=0 F  S INI(2)=$O(@INV@("OBX13",INI(1),INI(2))) Q:'INI(2)  S INI=INI(2) D
...
H4 K DXS
 ...
 .Q
 K DXS
 S INI(1)=0 F  S INI(1)=$O(@INV@("OBX16",INI(1))) Q:'INI(1)  S INI=INI(1) D
 .S INI(2)=0 F  S INI(2)=$O(@INV@("OBX16",INI(1),INI(2))) Q:'INI(2)  S INI=INI(2) D
 ..S (INX,X)=@INV@("OBX16",INI(1),INI(2))
 ..I $P($G(INTHL7F2),U,4) S X=$$SUBESC^INHUT7(X,INDELIMS,"I")
 ..S @INV@("OBX16",INI(1),INI(2))=$G(X) I '$D(X) D ERROR^INHS("Variable 'OBX16' failed input transform in iteration #"_INI(1)_","_INI(2)_". Processing continues.",0),ERROR^INHS("  Value = '"_INX_"'",0)
 ..Q
 .Q
 K DXS
 S INI(1)=0 F  S INI(1)=$O(@INV@("OBX17",INI(1))) Q:'INI(1)  S INI=INI(1) D
 .S INI(2)=0 F  S INI(2)=$O(@INV@("OBX17",INI(1),INI(2))) Q:'INI(2)  S INI=INI(2) D
 ..S (INX,X)=@INV@("OBX17",INI(1),INI(2))
 ..I $P($G(INTHL7F2),U,4) S X=$$SUBESC^INHUT7(X,INDELIMS,"I")
 ..S @INV@("OBX17",INI(1),INI(2))=$G(X) I '$D(X) D ERROR^INHS("Variable 'OBX17' failed input transform in iteration #"_INI(1)_","_INI(2)_". Processing continues.",0),ERROR^INHS("  Value = '"_INX_"'",0)
 ..Q
 .Q
 K DXS
 S INI(1)=0 F  S INI(1)=$O(@INV@("OBX18",INI(1))) Q:'INI(1)  S INI=INI(1) D
...
H1 ;Entering REQUIRED section.
 ...
 Q:$G(INSTERR) $S($G(INREQERR)>INSTERR:INREQERR,1:INSTERR)  D MAIN^BHLRLABU
