IS00022F ;Compiled from script 'Generated: HL IHS LAB R01 SONORA QUEST IN-I' on AUG 14, 2006
 ;Part 7
 ;Copyright 2006 SAIC
EN I '$D(X) D ERROR^INHS("Variable 'ORC12' failed input transform. Processing continues.",0),ERROR^INHS("  Value = '"_INX_"'",0)
 K DXS
 S (INX,X)=$G(@INV@("ORC13"))
 ...
 ..I $P($G(INTHL7F2),U,4) S X=$$SUBESC^INHUT7(X,INDELIMS,"I")
 ..S @INV@("OBR8",INI(1))=$G(X) I '$D(X) D ERROR^INHS("Variable 'OBR8' failed input transform in iteration #"_INI(1)_". Processing continues.",0),ERROR^INHS("  Value = '"_INX_"'",0)
 ..Q
 .K DXS
 .S INI(1)=0 F  S INI(1)=$O(@INV@("OBR9",INI(1))) Q:'INI(1)  S INI=INI(1) D
 ..S (INX,X)=@INV@("OBR9",INI(1))
 ..I $P($G(INTHL7F2),U,4) S X=$$SUBESC^INHUT7(X,INDELIMS,"I")
 ..S @INV@("OBR9",INI(1))=$G(X) I '$D(X) D ERROR^INHS("Variable 'OBR9' failed input transform in iteration #"_INI(1)_". Processing continues.",0),ERROR^INHS("  Value = '"_INX_"'",0)
 ..Q
 .K DXS
 .S INI(1)=0 F  S INI(1)=$O(@INV@("OBR10",INI(1))) Q:'INI(1)  S INI=INI(1) D
 ..S (INX,X)=@INV@("OBR10",INI(1))
 ..I $P($G(INTHL7F2),U,4) S X=$$SUBESC^INHUT7(X,INDELIMS,"I")
 ..S @INV@("OBR10",INI(1))=$G(X) I '$D(X) D ERROR^INHS("Variable 'OBR10' failed input transform in iteration #"_INI(1)_". Processing continues.",0),ERROR^INHS("  Value = '"_INX_"'",0)
 ..Q
 .K DXS
 .S INI(1)=0 F  S INI(1)=$O(@INV@("OBR11",INI(1))) Q:'INI(1)  S INI=INI(1) D
 ..S (INX,X)=@INV@("OBR11",INI(1))
9 ..D EN^IS00022G
 .D F3^IS00022G
 G F2^IS00022J
