VENPCCMP ; IHS/OIT/GIS - ORDERABLE PREFERENCE MANAGER ;
 ;;2.6;PCC+;;NOV 12, 2007
 ;
 ; DEAD CODE IN 2.5
 ; 
NEW N EF,CPT,PGRP,LIST,STATUS,X,Y,%,TITLE,DTOUT,DUOUT,DIRUT,%Y
...
LIST(EF,CPT,PGRP) ;  EP-LIST THE ITEMS
 N MN,GRP,SIEN,REC,X,NAME,CODE,HDR,CTAG,STAG
 S MN=$P($G(^VEN(7.98,CPT,0)),U,3)
 S GRP=PGRP_MN,SIEN=0,REC=""
 F  S SIEN=$O(^VEN(7.93,"AC",GRP,SIEN)) Q:'SIEN  D
...
S1 N %,CODE,DIR,Y,DIRUT,DTOUT,DUOUT
 ...
 S %=$L(LIST,U)-$$MAX(+EF,+CPT) I %>0 W !!,"You have exceeded the maximum number of items allowed!",!,"Delete ",%," item",$S(%>1:"s",1:"")," before proceeding",!  H 2 Q 0
...
SAVE(LIST,EF,CPT,PGRP) ; EP-DELETE THE OLD LIST AND SAVE THE NEW ONE
 I $D(^VEN(7.41,+$G(EF),0)),$D(^VEN(7.98,+$G(CPT),0)),$L(LIST),PGRP,PGRP=PGRP\1,PGRP>0,PGRP<5
 E  Q
 N CTAG,D,D0,DI,DIE,DIG,DIH,DIU,DIV,DQ,DR,HDR,PCE,STAG,DIK,DIC,IX,MN,DA,HIEN,MN,IX,CODE,DIW,HDR,STAG,CTAG,DICR,%,%Y
 S DIK="^VEN(7.93,"
 S MN=$P($G(^VEN(7.98,+CPT,0)),U,3)
 S IX=PGRP_MN
 S DA=0 F  S DA=$O(^VEN(7.93,"AC",IX,DA)) Q:'DA  D ^DIK
