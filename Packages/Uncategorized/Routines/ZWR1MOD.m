...
ASK R !!,"Comment ? ",COM,! I COM?1"?".E D  G ASK
 ...
 S ZFLG=0
 S FILE=0
 F  S FILE=$O(^DIC(FILE)) Q:'FILE  I $D(^DIC(FILE,0,"GL")) S X=$P(^("GL"),"^",2),X=$P(X,"("),TMP(X)=""
 F  S GN=$O(^CacheTempJ($J,GN)) Q:GN=""  D
 . I CASEFLTR,GN?.E1L.E Quit
 . S GNN="^"_GN
 . I SKIP[(";"_GNN_";") U $P W GN_"  "_"Skipped",! Q
 . I 'ZFLG s IOP=ZOUTDIR_"\SEARCH.txt",IOPAR="WNS" ;;JEG; Add Path
 . i 'ZFLG,$$exist(IOP) w "Skipping ",IOP," - already exists",! Q
 . I 'ZFLG w "Opening ",IOP,!
 . I 'ZFLG S IO=IOP O IO:("WNS"):10 E  S IO="" ;;JEG;Cache Open
 . I 'ZFLG,$G(IO)="" u $p w "error opening "_IOP,! r "Waiting for user...",junk q
 . ;d OUT^%IS i $G(IO)="" u $p w "error opening "_IOP,! r "Waiting for user...",junk q  ;;JEG;Disabled
 . U $P W GN_$S('$D(TMP(GN)):"  Non-FileMan Global",1:""),!
 . U IO
 . I 'ZFLG W COM,!
 . I 'ZFLG W "Cache "_$TR($ZD(H,2)," ","-")_" "_$ZT($P(H,",",2))_" SEARCH FOR:"_SVAL,!
 . s cnt=0
 . S ZFLG=1
 . D WALK(GNN)
 . ;C IO U $P
...
WALK(G) ;walk through global G, convert subscripts and values as necessary, dump out
 ...
 . S NAME=$NA(@G),NAME=$$UP^XLFSTR(NAME)
 . S VAL=$$CGV(@G),VAL=$$UP^XLFSTR(VAL)
 . I NAME'[SVAL,VAL'[SVAL Q
 . W NAME_"=",!,VAL,!
