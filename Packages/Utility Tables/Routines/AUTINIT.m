AUTINIT ;IHS/DIR/DIFROM - IHS DICTIONARIES (POINTERS) INIT ROUTINE; [ 03/04/98   8:10 AM ]
 ...
 S DIOVRD=1,U="^",DIFQ=0,DIFROM="98.1" W !,"This version (#98.1) of 'AUTINIT' was created on 25-NOV-1997"
...
GO ;
 W !,"I HAVE TO RUN AN ENVIRONMENT CHECK ROUTINE." D PKG,^AUTPREI Q:'$D(DIFQ)  D NOW^%DTC S DIFROM("PRE")=%
EN ; ENTER HERE TO BYPASS THE PRE-INIT PROGRAM
 S DIFQ=0 K DIRUT,DTOUT,DUOUT
 F DIFRIR=1:1:1 S DIFRRTN="^AUTINIT"_$E("5",DIFRIR) D @DIFRRTN
 W:1 !,"I AM GOING TO SET UP THE FOLLOWING FILES:" F I=1:2:152 S DIF(I)=^UTILITY("DIF",$J,I) D 1 G Q:DIFQ!$D(DIRUT) K DIF(I)
 S DIFROM="98.1" D PKG:'$D(DIFROM(0)),^AUTINIT1 G Q:'$D(DIFQ) S DIK(0)="AB"
 F DIF=1:2:152 S %=^UTILITY("DIF",$J,DIF),DIK=$P(%,";",5),N=$P(%,";",3),D=$P(%,";",4)_U_N D D K DIFQ(N)
 K DIFQR D ^AUTINIT2,^AUTINIT3
 L  S DUZ=DIDUZ W:1 !,"NO"_$P("TE THAT FILE",U,DSEC)_" SECURITY-CODE PROTECTION HAS BEEN MADE"
 D ^AUTPOS1,NOW^%DTC S DIFROM("INIT")=%
 I DIFROM F DIF=1:2:152 S %=^UTILITY("DIF",$J,DIF),N=+$P(%,";",3) I N,$P(%,";",8)="y" S ^DD(N,0,"VR")=DIFROM
 ...
 I DIFROM,$D(^%ZTSK) S X="AUTINIS" X ^%ZOSF("TEST") D:$T PAC^AUTINIS($T(IXF),.DIFROM)
...
PKG S X=$P($T(IXF),";",3),DIC="^DIC(9.4,",DIC(0)="",DIC("S")="I $P(^(0),U,2)="""_$P(X,U,2)_"""",X=$P(X,U) D ^DIC S DIFROM(0)=+Y K DIC
 Q
 ;
IXF ;;IHS DICTIONARIES (POINTERS)^AUT;12
