AUTP22X ;IHS/VEN/TOAD - AUT patch 22 pre-init ;
 ...
 ; clear out control characters found in the ^ACPT globals in both
...
CHECK ; troubleshooting entry point
 ;
 N AUMNAME S AUMNAME="^AUTTRLSH" ; the name value of each node of ^AUTTRLSH
 ...
 F AUMCNT=1:1 D  S AUMNAME=$Q(@AUMNAME) Q:AUMNAME=""  ; walk ^AUTTRLSH
 . ;
 ...
 ;
 ;
CLEANALL ; AUM*8.0*3 PRE-INIT: Remove Control Characters from ^AUTTRLSH
 ;
 ...
 N AUMCNTN S AUMCNTN=0 ; how many node names had control characters
 ;
 N AUMCNT ; count nodes, walk ^AUTTRLSH
 ...
 . N AUMEMP S AUMEMP=AUMCLN ; change name from ^AUTTRLSH(*)
 . ;S $E(AUMEMP,1,6)="^TMP(""AUM"","_$J_"," ; to ^TMP("AUM",$J,*)
 . S $E(AUMEMP,1,10)="^TMP(""AUM"","_$J_"," ; to ^TMP("AUM",$J,*)
 . ;IHS/OIT/CLS 09/17/2008 change to equal length of global root ^AUTTRLSH( 
 ...
 . N AUTTRLSH S AUTTRLSH=AUMNAME,$E(AUTTRLSH,1,AUMLENG)="^AUTTRLSH(" ; change back
 . K @(@AUMNAME@(U)) ; delete node in ^AUTTRLSH whose bad name we saved off
 . N AUMVALU S AUMVALU=@AUMNAME ; get the saved, clean value
 . S @AUTTRLSH=AUMVALU ; copy cleaned up node back into ^AUTTRLSH
 . N AUMSUB S AUMSUB=$QS(AUMNAME,3) ; get the main subscript
 . K @AUMNAME@(U) ; delete the saved node name to avoid it
 . D MES^XPDUTL(AUTTRLSH_"="_AUMVALU) ; report nodes as we copy them back
 K ^TMP("AUM",$J) ; clean up rest of temp space
 ;
 D BMES^XPDUTL(AUMCNT-1_$$T("MSG+1")) ; # nodes in ^AUTTRLSH were scanned.
 ...
 ; Your ^AUTTRLSH global is [now] free of control characters.
...
MSG ; messages to display
 ;; nodes in ^AUTTRLSH were scanned.
 ;; instances of control characters were found and removed,
 ;; of them from node names, 
 ;; from values.
 ;;Your ^AUTTRLSH global is 
 ;;free of control characters.
 ;;Replacing the bad node names found in ^AUTTRLSH
 ;;Removing control characters from your ^AUTTRLSH global...
 ;;AUM*8.0*3 PRE-INIT
 ;
 ;
