...
1 ;
 NEW D,E,F,I,I1,J,J1,L,S,T,W,X1,Z
2 ;
 S (E(1),E)=""
 ;make uppercase---keep only alpha---drop duplicate's
 F I=1:1:$L(X) S L=$E(X,I) Q:L=","  I L?1A S:$A(L)>95 L=$C($A(L)-32) I L'=E(1) S E(1)=L,E=E_L
 S E="#"_E
 D CONV
 S F=$E(E,1,3)
 F I=4:2 Q:I>$L(E)  I +$E(E,I-2,I-1)'=+$E(E,I,I+1) S F=F_$E(E,I,I+1)
 Q $E(F_"0000000000000",2,9)
 ;
CONV ;Convert word to numerics (first four sounds)
 S X=$P($T(SH),";;",2),D=0
 F I=1:1 S S=$P(X,";",I) Q:S']""!(D)  F J="S","N" S F=S_J,T=31_$S(J="S":24,1:21) D E Q:D
 F F="TIARY","TEARY","TIARE","TEARE" S T=31_$E(F,2,5) D E Q:D
 S X=$P($T(END+1),";;",2)
 F I=1:1 S J=$P(X,";",I) Q:J']""  S F=$P(J,":",1),T=$P(J,":",2) D E Q:D
 ;F I=1,2 S X(I)=$P($T(VOWEL+I),";;",2)
DOUB ;Double Vowels
 S D=0
 F J=1,2 S X=$P($T(VOWEL+J),";;",2) D
