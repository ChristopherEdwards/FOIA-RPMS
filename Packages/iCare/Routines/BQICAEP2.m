...
FLUD(BQDFN,RESULT) ;EP - Flu Diagnosis
 ...
 S UID=$S($G(ZTSK):"Z"_ZTSK,1:$J)
 ;S TIME="T-14M"
 S X=$$TAX^BQITRUTL(TMFRAME,"SURVEILLANCE ILI NO TMP NEEDED",1,BQDFN,9000010.07,"","",.TREF)
...
TUB(BQDFN,RESULT) ; EP - Tuberculosis
 ...
 S RESULT(1)=0,CT=1
 S TREF=$NA(^TMP("BQITAX",$J)) K @TREF
 S BDXX=$$TAX^BQITRUTL(TMFRAME,"SURVEILLANCE TUBERCULOSIS",1,BQDFN,9000010.07,"","",.TREF)
 I BDXX D DXF(BDXX)
 K @TREF
 ;
...
SYP(BQDFN,RESULT) ;EP - Syphilis
 ...
 ;
 D
 . F TAX="BQI SYPHILIS TP-AB LOINC","BQI SYPHILIS TP-AB TEST TAX" D BLD^BQITUTL(TAX,.TREF)
 . S SEARCH(1)="POS"_U_"=",SEARCH(2)="0"_U_">"
 . S XTP=$$LAB^BQICAUTL(TMFRAME,0,BQDFN,"",.SEARCH,.TREF)
 . K @TREF
 . F TAX="BQI SYPHILIS REAGIN LOINC","BQI SYPHILIS REAGIN TEST TAX" D BLD^BQITUTL(TAX,.TREF)
 ...
 ;. F TAX="BQI SYPHILIS REAGIN LOINC","BQI SYPHILIS REAGIN TEST TAX" D BLD^BQITUTL(TAX,.TREF)
 ;. S SEARCH(1)="POS"_U_"=",SEARCH(2)="0"_U_">"
 ;. S XSC=$$LAB^BQICAUTL(TMFRAME,0,BQDFN,"",.SEARCH,.TREF)
 ;. I 'XSC Q
 ;. K @TREF
 ;. F TAX="BQI SYPHILIS TP-AB LOINC","BQI SYPHILIS TP-AB TEST TAX" D BLD^BQITUTL(TAX,.TREF)
 ;. S SEARCH(1)="POS"_U_"=",SEARCH(2)="0"_U_">",DATE=$P(XSC,U,2)
 ;. S XTP=$$LBB^BQICAUTL(30,0,DATE,BQDFN,"",.SEARCH,.TREF)
 ;
 ;I XTP,XSC D
 ;. NEW XBGT,XBET,XBGS,XBES,XFPT,XFPS
 ;. S XBGT=$P(XTP,U,2),XBET=DT,XBGS=$P(XSC,U,2),XBES=DT
 ;. ;TMFRAME,TAX,NIT,PTDFN,FREF,PRB,SAME,TREF,START,END
 ;. S XFPT=$$TAX^BQITRUTL("","BQI SYPHILIS FALSE POS DXS",1,BQDFN,9000010.07,"","",.TREF,XBGT,XBET)
 ;. S XFPS=$$TAX^BQITRUTL("","BQI SYPHILIS FALSE POS DXS",1,BQDFN,9000010.07,"","",.TREF,XBGS,XBES)
 ;. I XFPT!XFPS Q
 ;. D LB^BQICAEP1(XTP,CT) S CT=CT+1
...
BCG(BQDFN,TMFRAME) ;EP - BCG Vaccination
 ...
 . S IEN=""
 . F  S IEN=$O(^AUPNVCPT("AA",BQDFN,TIEN,STDT)) Q:STDT=""!(STDT>ENDT)  D  Q:QFL
 .. S IEN=""
