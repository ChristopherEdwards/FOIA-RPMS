BQIMUPRH ;VNGT/HS/ALA-MU Provider Hover ; 15 Apr 2011  1:23 PM
 ...
 ;
 ;
EN(DATA,TMFRAME,PROV,CNT) ; EP -- BQI MU GET PROV HOVER
...
RTE(PRV) ; EP
 ...
 NEW PNUM,PEXC,CURR,PREV,SORT,USN,TYPE
 I TMFRAME=90,'$D(^BQIPROV(PRV,20)) S QQF=1 Q
 I TMFRAME=12,'$D(^BQIPROV(PRV,10)) S QQF=1 Q
 S CT=CT+1,PCT=PCT+1
 I DUZ=PRV S PRVR=PRV,SORT=1_"_"_$P($G(^VA(200,PRV,0)),U,1)
 ...
 Q
 ;
CQM(DATA,TMFRAME,PROV,CNT) ; EP -- BQI MU GET PROV CQM HOVER
 NEW UID,II,HDR,C1,C2,C3,C4,NAME,HEAD,HX,PEC,SORT,QFL,PCT,CT,DDATA,CPER,PPER
 S UID=$S($G(ZTSK):"Z"_ZTSK,1:$J)
 ...
 .. I QQF S QQF=0 Q
 .. S @DATA@(II)=@DATA@(II)_U_PROV_":"_C1_":"_C2_":"_C3_":"_C4_":"_CT_$C(30)
 .. F BJ=3:1:$L(@DATA@(II),U) I $P(@DATA@(II),U,BJ)="" S $P(@DATA@(II),U,BJ)="None"
 ;
 F  S PROV=$O(^BQIPROV(PROV)) Q:'PROV  D  Q:QFL
 . I PROV=DUZ Q
